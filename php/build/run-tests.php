...
if (empty($environment['TEMP'])) {
	...
	} else if (get_count($environment)==1) {
...
$test_files = array();
$exts_tested = get_count($exts_to_test);
$exts_skipped = 0;
$ignored_by_ext = 0;
sort($exts_to_test);
$test_dirs = array();
...
";
	...
	if ($bork_info === null) {
		if (isset($section_text['REDIRECTTEST'])) {
			...
			if (!isset($section_text['PHPDBG']) && @get_count($section_text['FILE']) + @get_count($section_text['FILEEOF']) + @get_count($section_text['FILE_EXTERNAL']) != 1) {
			...
			if ((@get_count($section_text['EXPECT']) + @get_count($section_text['EXPECTF']) + @get_count($section_text['EXPECTREGEX'])) != 1) {
	...
	if (isset($section_text['REDIRECTTEST'])) {
		...
		if (!empty($IN_REDIRECT['TESTS'])) {
			...
			$test_cnt += @get_count($test_files) - 1;
			$test_idx--;
	...
	if (is_array($org_file) || @get_count($section_text['REDIRECTTEST']) == 1) {
...
{
	$idx1 = 0; $cnt1 = @get_count($ar1);
	$idx2 = 0; $cnt2 = @get_count($ar2);
	$diff = array();
	$old1 = array();
	$old2 = array();
...
function compute_summary()
{
	...
	$n_total = get_count($test_results);
...
';
	...
	if (get_count($PHP_FAILED_TESTS['SLOW'])) {
...
';
	...
	if (get_count($PHP_FAILED_TESTS['XFAILED'])) {
		$failed_test_summary .= '
=====================================================================
...
';
	...
	if (get_count($PHP_FAILED_TESTS['BORKED'])) {
		$failed_test_summary .= '
=====================================================================
BORKED TEST SUMMARY
---------------------------------------------------------------------
';
	...
	if (get_count($PHP_FAILED_TESTS['FAILED'])) {
		$failed_test_summary .= '
=====================================================================
FAILED TEST SUMMARY
---------------------------------------------------------------------
';
	...
	if (get_count($PHP_FAILED_TESTS['WARNED'])) {
		$failed_test_summary .= '
=====================================================================
WARNED TEST SUMMARY
---------------------------------------------------------------------
';
	...
	if (get_count($PHP_FAILED_TESTS['LEAKED'])) {
		$failed_test_summary .= '
=====================================================================
LEAKED TEST SUMMARY
---------------------------------------------------------------------
