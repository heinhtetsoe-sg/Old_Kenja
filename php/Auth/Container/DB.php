...
class Auth_Container_DB extends Auth_Container
{
    ...
    function Auth_Container_DB($dsn)
    {
        ...
        $this->log('Auth_Container_DB::_connect('.$dsn.') called.', AUTH_LOG_DEBUG);
    ...
    function _prepare()
    {
        ...
        if ($this->options['auto_quote'] && $this->db->dsn['phptype'] != 'sqlite') {
            if (strpos('.', $this->options['table']) === false) {
                ...
                for ($i = 0, $count = get_count($t); $i < $count; $i++)
...
$this->log('$password{'.$password.'}', AUTH_LOG_DEBUG);
$this->log('Challenge Org{'.$res[$this->options['passwordcol']].'}', AUTH_LOG_DEBUG);
$this->log('loginchallenege{'.$this->_auth_obj->session['loginchallenege'].'}', AUTH_LOG_DEBUG);
            $res[$this->options['passwordcol']] = md5($res[$this->options['passwordcol']]
                    .$this->_auth_obj->session['loginchallenege']);

$this->log('Challenge Hashed{'.$res[$this->options['passwordcol']].'}', AUTH_LOG_DEBUG);
            // UGLY cannot avoid without modifying verifyPassword
            if ($this->options['cryptType'] == 'md5') {
                $res[$this->options['passwordcol']] = md5($res[$this->options['passwordcol']]);
            }
$this->log('Challenge Hashed Hashed{'.$res[$this->options['passwordcol']].'}', AUTH_LOG_DEBUG);
    ...
    function listUsers()
    {
        ...
        $this->log('Found '.get_count($retVal).' users.', AUTH_LOG_DEBUG);
        return $retVal;
    }
