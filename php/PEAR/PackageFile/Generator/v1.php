...
" <maintainers>\n";
        ...
        if (isset($pkginfo['changelog']) && get_count($pkginfo['changelog']) > 0) {
            $ret .= " <changelog>\n";
...
'http://pear.php.net/dtd/package-2.0.xsd',
        ...
        if (isset($arr['developer']) && get_count($arr['developer']) == 1) {
            $arr['developer'] = $arr['developer'][0];
        }

        if (isset($arr['contributor']) && get_count($arr['contributor']) == 1) {
            $arr['contributor'] = $arr['contributor'][0];
        }

        if (isset($arr['helper']) && get_count($arr['helper']) == 1) {
    ...
    {
        //- if any install-as/platform exist, create a generic release and fill it with
        if (get_count($package['platform']) || get_count($package['install-as'])) {
            $generic = array();
            $genericIgnore = array();
            ...
            if (get_count($package['platform'])) {
                ...
                foreach ($package['platform'] as $file => $os) {
                    ...
                    if ($os{0} == '!') {
                        ...
                        $oses[substr($os, 1)] = get_count($oses);
                    } else {
                        if (isset($oses[$os])) {
                            continue;
                        }
                        $oses[$os] = get_count($oses);
                    }
                }
                //- create a release for each platform encountered and fill with
                foreach ($oses as $os => $releaseNum) {
                    ...
                    if (!get_count($release[$releaseNum]['filelist']['install'])) {
                        unset($release[$releaseNum]['filelist']['install']);
                    }
                    if (!get_count($release[$releaseNum]['filelist']['ignore'])) {
                        unset($release[$releaseNum]['filelist']['ignore']);
                    }
                }
                if (get_count($generic) || get_count($genericIgnore)) {
                    $release[get_count($oses)] = array();
                    if (get_count($generic)) {
                        foreach ($generic as $file) {
                            ...
                            $release[get_count($oses)]['filelist']['install'][] =
                                array(
                                    'attribs' => array(
                                        'name' => $file,
                                        'as' => $installas,
                    ...
                    if (get_count($genericIgnore)) {
                        foreach ($genericIgnore as $file) {
                            $release[get_count($oses)]['filelist']['ignore'][] =
                                array(
                                    'attribs' => array(
                                        'name' => $file,
                                    )
                ...
                if (get_count($release['filelist']['install']) == 1) {
    ...
    function _processPhpDeps($deps)
    {
        ...
        foreach ($test as $dep) {
            ...
            if (isset($dep['min'])) {
                $min[$dep['min']] = get_count($min);
            }
            if (isset($dep['max'])) {
                $max[$dep['max']] = get_count($max);
            }
        }
        if (get_count($min) > 0) {
