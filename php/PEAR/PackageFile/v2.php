...
class PEAR_PackageFile_v2
{
    ...
    function isEquivalent($pf1)
    {
        ...
        for ($i1 = 0; $i1 < get_count($yourmaintainers); $i1++) {
            $reset = false;
            for ($i2 = 0; $i2 < get_count($mymaintainers); $i2++) {
    ...
    function getConfigureOptions()
    {
        ...
        if (isset($releases['configureoption'])) {
            ...
            for ($i = 0; $i < get_count($releases['configureoption']); $i++) {
    ...
    function getDirtree()
    {
        if (isset($this->_packageInfo['dirtree']) && get_count($this->_packageInfo['dirtree'])) {
    ...
    function _mergeTag($manip, $contents, $order)
    {
        ...
        if (is_array($manip[$tag]) && !empty($manip[$tag]) && isset($manip[$tag][0])) {
            ...
            if (is_array($manip[$tag]) && !get_count($manip[$tag])) {
                $manip[$tag] = $contents;
            } else {
