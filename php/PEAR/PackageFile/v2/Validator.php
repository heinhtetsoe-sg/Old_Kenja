...
class PEAR_PackageFile_v2_Validator
{
    ...
    {
        $ret = array();
        if (get_count($pieces = explode('|', $key)) > 1) {
            $ret['choices'] = array();
        ...
        if (get_count($attrs = explode('->', $key)) > 1) {
    ...
    function _validateFilelist($list = false, $allowignore = false, $dirs = '')
    {
        ...
        if (!$allowignore && isset($list['file'])) {
            ...
            foreach ($list['file'] as $i => $file)
            {
                ...
                unset($file['attribs']);
                if (get_count($file) && $this->_curState != PEAR_VALIDATE_DOWNLOADING) { // has tasks
