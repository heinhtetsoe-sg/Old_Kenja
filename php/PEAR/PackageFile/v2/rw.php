...
class PEAR_PackageFile_v2_rw extends PEAR_PackageFile_v2
{
    ...
    function deleteMaintainer($handle)
    {
        ...
        foreach (array('lead', 'developer', 'contributor', 'helper') as $role) {
            ...
            if ($found !== false) {
                unset($this->_packageInfo[$role][$found]);
                if (!get_count($this->_packageInfo[$role]) && $role == 'lead') {
                    $this->_isValid = 0;
                }
                if (!get_count($this->_packageInfo[$role])) {
                    unset($this->_packageInfo[$role]);
                    return true;
                }
                $this->_packageInfo[$role] =
                    array_values($this->_packageInfo[$role]);
                if (get_count($this->_packageInfo[$role]) == 1) {
                           ...
                           $conflicts = false)
    {
        ...
        if ($exclude) {
            if (is_array($exclude) && get_count($exclude) == 1) {
                $exclude = $exclude[0];
            }
            $dep['exclude'] = $exclude;
        }
    ...
    function &_getCurrentRelease($strict = true)
    {
        if ($p = $this->getPackageType()) {
            ...
            if (isset($this->_packageInfo[$p][0])) {
                return $this->_packageInfo[$p][get_count($this->_packageInfo[$p]) - 1];
            } else {
