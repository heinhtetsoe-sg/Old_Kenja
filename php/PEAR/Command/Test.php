    ...
    function doRunTests($command, $options, $params)
    {
                ...
                if (isset($options['phpunit'])) {
                    ...
                    if (get_count($dir)) {
                        foreach ($dir as $p) {
                            $p = realpath($p);
                            if (!get_count($tests) ||
                                  (get_count($tests) && strlen($p) < strlen($tests[0]))) {
        ...
        if (isset($options['tapoutput'])) {
            ...
            if (get_count($failed)) {
                ...
                $output .= get_count($passed) . " PASSED TESTS\n";
                $output .= get_count($skipped) . " SKIPPED TESTS\n";
                $output .= get_count($failed) . " FAILED TESTS:\n";
        ...
        $this->ui->outputData(get_count($passed) . ' PASSED TESTS', $command);
        $this->ui->outputData(get_count($skipped) . ' SKIPPED TESTS', $command);
        if (get_count($failed)) {
            $this->ui->outputData(get_count($failed) . ' FAILED TESTS:', $command);
            foreach ($failed as $failure) {
                $this->ui->outputData($failure, $command);
            }
        }

        if (get_count($failed) == 0) {
            return true;
        }
