...
class PEAR_Frontend_CLI extends PEAR_Frontend
{
        ...
        foreach ($xml['paramgroup'] as $group) {
            ...
            if (isset($group['param'])) {
                if (method_exists($script, 'postProcessPrompts')) {
                    ...
                    if (!is_array($prompts) || get_count($prompts) != get_count($group['param'])) {
    ...
    function confirmDialog($params)
    {
        ...
        do {
            ...
            $tried   = true;
        } while (is_array($answers) && get_count(array_filter($answers)) != get_count($prompts));
    ...
    function _tableRow($columns, $rowparams = array(), $colparams = array())
    {
        ...
        if (get_count($columns) > $this->params['ncols']) {
            $this->params['ncols'] = get_count($columns);
        }
    ...
    function _endTable()
    {
        ...
        for ($i = 0; $i < get_count($table_data); $i++) {
            ...
            $rowlines = array();
            if ($height > 1) {
                for ($c = 0; $c < get_count($data); $c++) {
                    $rowlines[$c] = preg_split('/(\r?\n|\r)/', $data[$c]);
                    if (get_count($rowlines[$c]) < $height) {
