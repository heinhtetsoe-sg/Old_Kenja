<?php
  ...
  $chars = isset($_GET["selectChars"]) ? $_GET["selectChars"] : "";
...
$(function() {
    ...
    $('td').on('click', function() {
        selectDomElm(this);//$(this).select();
        document.execCommand('copy');
        document.forms.formID.chars.value += $(this).text();
	});
});
...
function backChar() {
  var line = document.forms.formID.chars.value;
  if (line.length == 1) {
    document.forms.formID.chars.value = "";
  }
  if (line.length > 0) {
    document.forms.formID.chars.value = line.substring(0, line.length - 1);
  }
}
...
<font size="4"><B>コード [$scope]</B></font>
 <input type=text name="selectChars" id="chars" SIZE=30 value="$chars">
 <input type=button value="<" onClick="backChar();">
