...
class knja130aForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knja130aForm1", "POST", "knja130aindex.php", "", "knja130aForm1");
        ...
        $z010name1 = "";
        $query = knja130aQuery::getNameMst("Z010", "00");
        ...
        if ($disable == 1) {
            ...
            $query = knja130aQuery::getAuth2($model);
        }
        $row1 = array();
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $extra = "onchange=\"return btn_submit('knja130a'),AllClearList();\"";
        ...
        if ($model->field["MIRISYU"] == "" || $model->field["RISYU"] == "" || $model->field["RISYUTOUROKU"] == "") {
            $query = knja130aQuery::getRisyuMirsyu($model);
            $result = $db->query($query);
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                // 値がYなら「しない」それ以外は「する」
        ...
        View::toHTML($model, "knja130aForm1.html", $arg);
    }
}
//生徒リストToリスト作成
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJA130A");
