...
class knjz290a_2SubForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform1", "POST", "knjz290a_2index.php", "", "subform1");
        ...
        if ($model->Properties["useCurriculumcd"] == '1') {
            if (!isset($model->warning) && isset($model->staffcd) && isset($model->classcd) && isset($model->school_kind) && isset($model->sdate)) {
                $query = knjz290a_2Query::getRow2($model->staffcd, $model->classcd, $model->sdate, $model);
        ...
        //職員情報
        $info = $db->getRow(knjz290a_2Query::getStfInfo2($model->staffcd), DB_FETCHMODE_ASSOC);
        ...
        if($model->staffcd) {
            $result = $db->query(knjz290a_2Query::getClassSdateEdate($model->staffcd, $model));
        ...
        /******************/
        //教科コンボ
        $query = knjz290a_2Query::getName($model);
        ...
        View::toHTML($model, "knjz290a_2SubForm1.html", $arg);
    }
}
