...
class knja233bForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knja233bForm1", "POST", "knja233bindex.php", "", "knja233bForm1");
        ...
        /**********/
        //学期コンボボックスを作成する
        $query = knja233bQuery::getSemester($model);
        ...
        //学年コンボ
        $query = knja233bQuery::getClassMst($model);
        makeCmb($objForm, $arg, $db, $query, "CLASSCD", $model->field["CLASSCD"], "onChange=\"return btn_submit('knja233b');\"", 1);

        //テスト種別コンボ
        $query = knja233bQuery::getSubclassMst($model);
        makeCmb($objForm, $arg, $db, $query, "SUBCLASSCD", $model->field["SUBCLASSCD"], "onChange=\"return btn_submit('knja233b');\"", 1);
        ...
        $arg["el"]["DATE"] = View::popUpCalendar2($objForm, "DATE", $value, "reload=true", " btn_submit('knja233b')", "");
        ...
        //セキュリティーチェック
        $securityCnt = $db->getOne(knja233bQuery::getSecurityHigh());
        ...
        if (!$model->field["OUTPUTLANG"]) $model->field["OUTPUTLANG"] = 1;
        $extra = array("id=\"OUTPUTLANG1\"", "id=\"OUTPUTLANG2\"");
        $radioArray = knjCreateRadio($objForm, "OUTPUTLANG", $model->field["OUTPUTLANG"], $extra, $radioValue, get_count($radioValue));
        ...
        View::toHTML($model, "knja233bForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    //クラス一覧
    $opt = array();
    $result = $db->query(knja233bQuery::getChairDat($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
