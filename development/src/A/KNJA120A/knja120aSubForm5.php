...
class knja120aSubForm5
{
    public function main(&$model)
    {
        $objForm = new form();
        $arg["start"]   = $objForm->get_start("list", "POST", "knja120aindex.php", "", "edit");
        ...
        $db = Query::dbCheckOut();
        if ($model->schregno) {
            $result = $db->query(knja120aQuery::getAward($model));
        ...
        if (VARS::get("cmd") == "right_list") {
            $arg["reload"]  = "parent.edit_frame.location.href='knja120aindex.php?cmd=edit'";
        }

        View::toHTML($model, "knja120aSubForm5.html", $arg);
    }
}
