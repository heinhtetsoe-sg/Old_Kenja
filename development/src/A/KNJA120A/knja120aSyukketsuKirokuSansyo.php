...
class knja120aSyukketsuKirokuSansyo
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform8", "POST", "knja120aindex.php", "", "subform8");
        ...
        if ($db->getOne(knja120aQuery::checkSchoolMst()) > 0) {
            $schoolcd    = sprintf("%012d", SCHOOLCD);
            $school_kind = $db->getOne(knja120aQuery::getSchoolKind($model));
        }
        ...
        //出欠の記録
        $sick = $late = $early = 0;
        $result = $db->query(knja120aQuery::getAttendSemesDat($model, $knjSchoolMst));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            if ($row["SUSPEND"] != "") {
                $suspend += (int)$row["SUSPEND"];
            }
            if ($row["MOURNING"] != "") {
                $mourning += (int)$row["MOURNING"];
            }
            if ($row["SICK"] != "") {
                $sick += (int)$row["SICK"];
            }
        ...
        View::toHTML($model, "knja120aSyukketsuKirokuSansyo.html", $arg);
    }
}
