...
class knja134hForm1
{
    public function main(&$model)
    {
        ...
        $arg["TITLE"]   = "高等部指導要録印刷画面";
         /* Add by PP for Title 2020-01-17 end */

        //フォーム作成
        $arg["start"]   = $objForm->get_start("knja134hForm1", "POST", "knja134hindex.php", "", "knja134hForm1");
        ...
        // Add by PP for current course 2020-01-10 start
        $extra .= " onclick=\"current_cursor('GAKUNEN_KONGOU');return btn_submit('clickchange');\" id=\"GAKUNEN_KONGOU\" ";
        // Add by PP for current course 2020-01-17 end
        $arg["data"]["GAKUNEN_KONGOU"] = knjCreateCheckBox($objForm, "GAKUNEN_KONGOU", "1", $extra."aria-label='法定クラスの学年混合'", "");
        ...
        if ($model->field["OUTPUT"] == 1) {
            $arg["student"] = 1;
            $query = knja134hQuery::getHrClass($model);
        } else {
            $arg["hr_class"] = 2;
            $query = knja134hQuery::getGrade($model);
        }
        $row1 = array();
        ...
        if ($model->field["HR_CLASS_TYPE"] == "1" && $model->field["GAKUNEN_KONGOU"] == "1" && $model->field["OUTPUT"] == "2") {
            ...
            /* Add by PP for current cursor 2020-01-10 start */
            $extra = "id=\"GRADE_HR_CLASS\" onchange=\"current_cursor('GRADE_HR_CLASS');return btn_submit('knja134h'),AllClearList();\"";
            /* Add by PP for current cursor 2020-01-17 end */
        ...
        /* Add by PP for current cursor 2020-01-10 start */
        $extra = array("id=\"CHITEKI1\" onclick =\" current_cursor('CHITEKI1');return btn_submit('chitekichange');\"", "id=\"CHITEKI2\" onclick =\"current_cursor('CHITEKI2'); return btn_submit('chitekichange');\"");
        /* Add by PP for current cursor 2020-01-17 end */
        ...
        //宮城県の場合、画面切り替え
        $z010name1 = "";
        $query = knja134hQuery::getNameMst("Z010", "00");
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $z010name1 = $row["NAME1"];
        }
...
/*
        ...
        if ($model->field["MIRISYU"] == "" || $model->field["RISYU"] == "" || $model->field["RISYUTOUROKU"] == "") {
            $query = knja134hQuery::getRisyuMirsyu($model);
            $result = $db->query($query);
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
                // 値がYなら「しない」それ以外は「する」
 ...
 */
        ...
        View::toHTML($model, "knja134hForm1.html", $arg);
    }
}
//生徒リストToリスト作成
function makeStudentList(&$objForm, &$arg, $db, $model)
{
    ...
    if ($model->field["OUTPUT"] == 1) {
        if ($model->field["HR_CLASS_TYPE"] == '1') {
            ...
            $query .= "INNER JOIN SCHREG_REGD_GDAT T3 ON T3.YEAR = T2.YEAR AND T3.GRADE = T2.GRADE AND T3.SCHOOL_KIND = 'H' ";
        } else {
        ...
        if ($model->field["HR_CLASS_TYPE"] == "1" && $model->field["GAKUNEN_KONGOU"] == "1") {
            ...
            $query = knja134hQuery::getHrClassAuth($model, CTRL_YEAR, CTRL_SEMESTER);
            $result = $db->query($query);
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    $extra = "multiple style=\"width: 230px; height: 320px; \"  ondblclick=\"move1('right', $select)\" aria-label='出力対象一覧'";
    ...
    $extra = "id=\"btn_rights$select\" style=\"height:20px;width:40px\" onclick=\"moves('right', $select);\" $label";
    // Edit by PP for PC-Talker(voice) 2020-01-17 end
    ...
    $extra = "id=\"btn_lefts$select\" style=\"height:20px;width:40px\" onclick=\"moves('left', $select);\" $label";
    // Edit by PP for PC-Talker(voice) 2020-01-17 end
    ...
    $extra = "id=\"btn_right1$select\" style=\"height:20px;width:40px\" onclick=\"move1('right', $select);\" $label";
    // Edit by PP for PC-Talker(voice) 2020-01-17 end
    ...
    $extra = "id=\"btn_left1$select\" style=\"height:20px;width:40px\" onclick=\"move1('left',  $select);\" $label";
    // Edit by PP for PC-Talker(voice) 2020-01-17 end
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJA134H");
