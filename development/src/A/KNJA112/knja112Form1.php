...
class knja112Form1
{
    function main(&$model)
    {
        ...
        $arg["start"]   = $objForm->get_start("main", "POST", "knja112index.php", "", "main");
        ...
        //その他(身体状態)
        $query = knja112Query::getNameMst("A025");
        $extra = "onchange=\"btn_submit('main');\"";
        makeCmb($objForm, $arg, $db, $query, $model->handicap, "HANDICAP", $extra, 1, "BLANK");

        //右クラスコンボ
        $query = knja112Query::getHrClass($model);
        ...
        View::toHTML($model, "knja112Form1.html", $arg); 
    }
}
//コンボ作成
...
function makeStudentList(&$objForm, &$arg, $db, &$model) {
    ...
    if ($model->cmd == 'change_hr_class' ) {
        ...
        $result = $db->query(knja112Query::getGhrStudents($model));
        $model->leftList = array();
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt_left[] = array("label" => $row["HR_NAME"].$row["ATTENDNO"]."番　".$row["SCHREGNO"]."　".$row["NAME"]."(".$row["HANDICAP_NAME"].")",
                                "value" => $row["SCHREGNO"]);
            $cnt++;
            $model->leftList[$row["SCHREGNO"]] = $row["SCHREGNO"];
        }
        $result->free();
    }
    $arg["RIGHT_NUM"] = $cnt;
    ...
    $cnt = 0;
    $opt_right = array();
    $result = $db->query(knja112Query::getHrStudents($model));
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $opt_right[]= array("label" => $row["HR_NAME"].$row["ATTENDNO"]."番　".$row["SCHREGNO"]."　".$row["NAME"]."(".$row["HANDICAP_NAME"].")",
                            "value" => $row["SCHREGNO"]);
        $cnt++;
    }
    $result->free();
