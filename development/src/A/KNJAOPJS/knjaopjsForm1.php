...
class knjaopjsForm1 {
    ...
    function main(&$model) {
        ...
        $arg["start"] = $objForm->get_start("edit", "POST", "knjaopjsindex.php", "", "edit");
...
//        makeStudentList($objForm, $arg, $db, $model);
        ...
        View::toHTML($model, "knjaopjsForm1.html", $arg);
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    ...
    //対象クラスリストを作成する
    $query = knjaopjsQuery::getGradeHrClass($model);
    $result = $db->query($query);
    $opt1 = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
