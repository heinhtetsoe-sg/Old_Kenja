...
class knja124jsForm1 {

    function main(&$model) {

        $objForm = new form;

        $arg["start"] = $objForm->get_start("edit", "POST", "knja124jsindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //印影登録有無
        $query = knja124jsQuery::getInkanUmu();
        ...
        $query = knja124jsQuery::getHrClass($model);
        ...
        //生徒リスト
        $studentArray = array();
        $query = knja124jsQuery::getStudents($model);
        ...
        View::toHTML($model, "knja124jsForm1.html", $arg);
    }
}
...
function setData(&$objForm, &$arg, $db, $model, $studentRow) {
    ...
    //HTRAINREMARK_DAT 取得
    $query = knja124jsQuery::getTrainRow($model, $schregno);
    $tRow = $db->getRow($query, DB_FETCHMODE_ASSOC);

    //行動の記録・特別活動の記録
    $behavior = "";
    $resultb = $db->query(knja124jsQuery::getBehavior($model, $schregno));
    ...
    //学習記録データ
    $query = knja124jsQuery::getStudyRec($model, $schregno);
    $result = $db->query($query);
    $study = "";
    ...
    //出欠記録データ
    $attend = $db->getRow(knja124jsQuery::getAttendRec($model, $schregno), DB_FETCHMODE_ASSOC);

    //表示データのHASHを取得
    $model->hash[$schregno] = $model->makeHash($tRow, $behavior, $study, $attend);

    //署名データ取出し
    $query = knja124jsQuery::getOpinionsSignature($model, $schregno);
    $opinionsSignature = $db->getRow($query, DB_FETCHMODE_ASSOC);

    //印影の表示
    $query = knja124jsQuery::getIneiFlg($model, $schregno);
