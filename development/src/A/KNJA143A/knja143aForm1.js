function btn_submit(cmd) {
    if (cmd == 'grade') {
        AllClearList();
        document.forms[0].TERM_EDATE.value = "";
    }
...
function chkOut2Print(val) {
    if (val == "2") {
        document.forms[0].OUT2PRINT_SIDE1.disabled = false;
        document.forms[0].OUT2PRINT_SIDE2.disabled = false;
    } else {
        document.forms[0].OUT2PRINT_SIDE1.disabled = true;
        document.forms[0].OUT2PRINT_SIDE2.disabled = true;
    }

    var year = document.forms[0].CTRL_YEAR.value;
    if (val == 1 && document.forms[0].useFormNameA143A.value != "") {
            // B5用紙
            year = Number(year) + 1;
    } else {
        var gradeCd = document.forms[0].selectGradeCd.value;
        // 卒業年(1年の時は3年後)
        year = Number(year) + (4 - Number(gradeCd));
    }
    document.forms[0].TERM_EDATE.value = year + "/03/31";
}
//印刷
