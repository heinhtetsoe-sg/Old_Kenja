...
class knja610vForm1
{
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"] = $objForm->get_start("knja610vForm1", "POST", "knja610vindex.php", "", "knja610vForm1");

        //データベース接続
        $db = Query::dbCheckOut();

        //年度コンボボックスを作成する/////////////////////////////////////
        $query = knja610vQuery::getSelectYear();
        ...
        if ($model->Properties["use_prg_schoolkind"] == "1") {
            $arg["schkind"] = "1";
            $query = knja610vQuery::getSchkind($model);
            $extra = "onchange=\"return btn_submit('init');\"";
        ...
        //学期コンボボックスを作成する/////////////////////////////////////
        $opt_sem = array();
        $query = knja610vQuery::getSelectSeme($model->field["YEAR"]);
        ...
        if ($model->Properties["use_school_detail_gcm_dat"] == "1") {
            //課程学科コンボ
            $query = knja610vQuery::getCourseMajor($model);
        ...
        if ($model->field["GAKKI"] != "9" || in_array($model->tableName, array('TESTITEM_MST_COUNTFLG_NEW_SDIV', 'TESTITEM_MST_COUNTFLG_NEW_GCM_SDIV'))) {
            $query = knja610vQuery::getTestItem($model);
            $result = $db->query($query);
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        $opt_class=array();
        $query = knja610vQuery::getAuth($model);
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        View::toHTML($model, "knja610vForm1.html", $arg); 
    }
}
