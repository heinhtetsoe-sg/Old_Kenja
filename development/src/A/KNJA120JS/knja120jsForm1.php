...
class knja120jsForm1 {

    function main(&$model) {

        $objForm = new form;

        $arg["start"] = $objForm->get_start("edit", "POST", "knja120jsindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //HTRAINREMARK_DAT 取得
        $query = knja120jsQuery::getTrainRow($model);
        $tRow = $db->getRow($query, DB_FETCHMODE_ASSOC);

        //行動の記録・特別活動の記録
        $behavior = "";
        $resultb = $db->query(knja120jsQuery::getBehavior($model));
        ...
        //学習記録データ
        $query = knja120jsQuery::getStudyRec($model);
        ...
        //出欠記録データ
        $attend = $db->getRow(knja120jsQuery::getAttendRec($model), DB_FETCHMODE_ASSOC);
        ...
        //署名データ取出し
        $query = knja120jsQuery::getOpinionsSignature($model);
        $opinionsSignature = $db->getRow($query, DB_FETCHMODE_ASSOC);

        //印影登録有無
        $query = knja120jsQuery::getInkanUmu();
        ...
        //印影の表示
        $query = knja120jsQuery::getIneiFlg($model);
        ...
        View::toHTML($model, "knja120jsForm1.html", $arg);
    }
}
