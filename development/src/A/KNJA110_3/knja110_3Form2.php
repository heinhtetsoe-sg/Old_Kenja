...
class knja110_3Form2 
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knja110_3index.php", "", "edit");
        $arg["reload"] = "";

        if($model->TRANSFER_SDATE && !$model->isWarning()){
            $Row = knja110_3Query::getRow($model);
        }else{
           $Row =& $model->field;
        }
        $db = Query::dbCheckOut();
        //異動区分
        $result = $db->query(knja110_3Query::getTransfercd());
        $opt = array();
        ...
        $objForm->ae( array("type"        => "select",
                            ...
                            "extrahtml"   => "onChange=\"return check(this)\"",
                            "options"     => $opt ));
...
*/
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "parent.top_frame.location.href='knja110_3index.php?cmd=list';";
        }
        $arg["check"]  = "checktest('".$Row["TRANSFERCD"]."');";

        View::toHTML($model, "knja110_3Form2.html", $arg);
    }
}
?>
