...
class knja134pForm1
{
    public function main(&$model)
    {
        ...
        $arg["TITLE"]   = "小学部指導要録印刷画面";
         /* Add by PP for Title 2020-01-17 end */

        //フォーム作成
        $arg["start"] = $objForm->get_start("knja134pForm1", "POST", "knja134pindex.php", "", "knja134pForm1");
        ...
        $query = knja134PQuery::getNameMst("Z010", "00");
        $z010name1 = $db->getOne($query);

        $model->field["HR_CLASS_TYPE"] = ($model->field["HR_CLASS_TYPE"] == "") ? "1" : $model->field["HR_CLASS_TYPE"];
        if ($model->Properties["seitoSidoYorokuUseHrClassType"] == '1' || $model->Properties["seitoSidoYorokuUseHrClassTypeP"] == '1') {
        ...
        if ($disable == 1) {
            $arg["student"] = 1;
            $query = knja134pQuery::getHrClass($model);
        } else {
            $arg["hr_class"] = 2;
            $query = knja134pQuery::getGrade($model);
        }
        ...
        if ($model->field["HR_CLASS_TYPE"] == "1" && $model->field["GAKUNEN_KONGOU"] == '1' && $model->field["OUTPUT"] == "2") {
            ...
            // Add by PP for current cursor 2020-01-10 start
            $extra = "id=\"GRADE_HR_CLASS\" onchange=\"current_cursor('GRADE_HR_CLASS');return btn_submit('knja134p'),AllClearList();\"";
            // Add by PP for current cursor 2020-01-17 end
        ...
        if ($z010name1 == "naraken") {
            $arg["data"]["CHITEKI3_TITLE"] = "知的障害（視覚障害等）";
            $radioValue[] = 3;
            $extra[] = "id=\"CHITEKI3\" onclick=\"current_cursor('CHITEKI3');return btn_submit('clickchange');\"";
        }
        /* Add by PP for current cursor 2020-01-17 end */
        ...
        //帳票種別チェックボックス
        $name = array("SEITO", "SIMEI", "SCHZIP", "SCHOOLZIP", "GAKUSHU", "KOUDO", "MONGON", "NOT_PRINT_GUARANTOR");
        if ($model->Properties["notShowOnlineSelect"] != "1") {
            $arg["showOnline"] = "1";
            $name[] = "ONLINE";
        }
        foreach ($name as $key => $val) {
            if (in_array($val, array("SCHZIP", "SCHOOLZIP", "MONGON", "ONLINE"))) {
            ...
            if ($val == "MONGON") {
                $extra .= ($model->field["koudo"] || $model->cmd == "") ? "" : " disabled";
            }
        ...
        View::toHTML($model, "knja134pForm1.html", $arg);
    }
}
//リストToリスト作成
function makeListToList(&$objForm, &$arg, $db, $model)
{
    ...
    $opt1 = array();
    if ($disable == 1) {
        $query = knja134pQuery::getStudentList($model, 'list', $selectdata);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $query = knja134pQuery::getHrClassAuth($model, 'list', $selectdata);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    $extra = "multiple style=\"width:230px; height:230px\" ondblclick=\"move1('left',$disable)\" aria-label='生徒一覧'";
    ...
    if ($selectdata) {
        if ($disable == 1) {
            //生徒一覧リストを作成する
            $query = knja134pQuery::getStudentList($model, 'select', $selectdata);
            $result = $db->query($query);
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                    $opt1[] = array('label' => $row["LABEL" ],
                                    'value' => $row["VALUE"]);
            ...
            $query = knja134pQuery::getHrClassAuth($model, 'select', $selectdata);
            $result = $db->query($query);
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                if ($model->field["HR_CLASS_TYPE"] == "1" && $model->field["GAKUNEN_KONGOU"] == '') {
                    if (substr($row["VALUE"], 0, 2) != $model->field["GRADE_HR_CLASS"]) {
...
{
    //印刷ボタンを作成する
    // Add by PP for PC-Talker(voice) 2020-01-10 start
    $extra = "id =\"PRINT\" onclick=\"current_cursor('PRINT'); return btn_submit('update');\"";
    // Add by PP for PC-Talker(voice) 2020-01-17 end
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJA134P");
    ...
    knjCreateHidden($objForm, "notPrintFinschooltypeName", $model->Properties["notPrintFinschooltypeName"]);
    knjCreateHidden($objForm, "HTRAINREMARK_DAT_INDEPENDENT_REMARK_SIZE_P", $model->Properties["HTRAINREMARK_DAT_INDEPENDENT_REMARK_SIZE_P"]);
    knjCreateHidden($objForm, "HTRAINREMARK_DAT_BEHAVEREC_REMARK_SIZE_P_disability", $model->Properties["HTRAINREMARK_DAT_BEHAVEREC_REMARK_SIZE_P_disability"]);
    knjCreateHidden($objForm, "HTRAINREMARK_HDAT_ENT_DISABILITY_REMARK_SIZE_P", $model->Properties["HTRAINREMARK_HDAT_ENT_DISABILITY_REMARK_SIZE_P"]);
    knjCreateHidden($objForm, "HTRAINREMARK_HDAT_ENT_DISABILITY_REMARK_SIZE_P_disability", $model->Properties["HTRAINREMARK_HDAT_ENT_DISABILITY_REMARK_SIZE_P_disability"]);
    knjCreateHidden($objForm, "HTRAINREMARK_DAT_TOTALREMARK_SIZE_P", $model->Properties["HTRAINREMARK_DAT_TOTALREMARK_SIZE_P"]);
    knjCreateHidden($objForm, "HTRAINREMARK_DAT_TOTALREMARK_SIZE_P_disability", $model->Properties["HTRAINREMARK_DAT_TOTALREMARK_SIZE_P_disability"]);
    ...
    knjCreateHidden($objForm, "HTRAINREMARK_DAT_ATTENDREC_REMARK_SIZE_P_disability", $model->Properties["HTRAINREMARK_DAT_ATTENDREC_REMARK_SIZE_P_disability"]);
    ...
    knjCreateHidden($objForm, "seitoSidoYorokuUseHrClassTypeP", $model->Properties["seitoSidoYorokuUseHrClassTypeP"]);
}
