...
class knja122oForm1
{
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knja122oForm1", "POST", "knja122oindex.php", "", "knja122oForm1");
        ...
        View::toHTML($model, "knja122oForm1.html", $arg); 
...
function makeBtn(&$objForm, &$arg, $db, $model)
{
    //所見処理
    if ($model->syomeiBtn) {
        $query = knja122oQuery::getGdat();
        $result = $db->query($query);
        $gdatArray = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $gdatArray[] = $row["SCHOOL_KIND"];
        }
        $query = knja122oQuery::getSchoolmst();
        $schoolMst = $db->getRow($query, DB_FETCHMODE_ASSOC);

        $query = knja122oQuery::getNameMstCd2("Z001", $schoolMst["SCHOOLDIV"]);
        ...
        $linkSaki = array("P" => array("name" => "小学", "ikkatu" => "124PS", "syomei" => "120PS", "cancel" => "123PS"),
                          ...
                          "H" => array("name" => "高校", "ikkatu" => $Z001["NAMESPARE1"] == "1" ? "124MS" : "124OO", "syomei" => $Z001["NAMESPARE1"] == "1" ? "120MS" : "120OO", "cancel" => $Z001["NAMESPARE1"] == "1" ? "123MS" : "123OO")
                         );
        if (count($gdatArray) == "1") {
            $arg["gdatOnly"] = "1";
            $linkId = $linkSaki[$gdatArray[0]]["ikkatu"];
            $linkIdLow = strtolower($linkId);
//            $paraUrl = str_replace($linkId,    "124OO", $model->setUrl);
//            $paraUrl = str_replace($linkIdLow, "124oo", $paraUrl);
            ...
            $linkIdLow = strtolower($linkId);
//            $paraUrl = str_replace($linkId,    "120O0", $model->setUrl);
//            $paraUrl = str_replace($linkIdLow, "120oo", $paraUrl);
            $extra  = " onClick=\" wopen('".REQUESTROOT."/A/KNJA{$linkId}/knja{$linkIdLow}index.php?EXE_TYPE=PRINCIPAL&setUrl=".$paraUrl."&RNDM=".$model->randm."&SEND_AUTH=".AUTHORITY."'";
            $extra .= ",'SUBWIN',0,0,screen.availWidth,screen.availHeight);\"";
            ...
            $linkIdLow = strtolower($linkId);
 //           $paraUrl = str_replace($linkId,    "123OO", $model->setUrl);
//            $paraUrl = str_replace($linkIdLow, "123oo", $paraUrl);
            $extra  = " onClick=\" wopen('".REQUESTROOT."/A/KNJA{$linkId}/knja{$linkIdLow}index.php?EXE_TYPE=PRINCIPAL&setUrl=".$paraUrl."&RNDM=".$model->randm."&SEND_AUTH=".AUTHORITY."'";
            $extra .= ",'SUBWIN',0,0,screen.availWidth,screen.availHeight);\"";
            ...
            foreach ($gdatArray as $key => $val) {
                ...
                $linkIdLow = strtolower($linkId);
                $paraUrl = str_replace("122O", $linkId, $model->setUrl);
                $paraUrl = str_replace("122o", $linkIdLow, $paraUrl);
                $extra  = " onClick=\" wopen('".REQUESTROOT."/A/KNJA{$linkId}/knja{$linkIdLow}index.php?EXE_TYPE=PRINCIPAL&setUrl=".$paraUrl."&RNDM=".$model->randm."&SEND_AUTH=".AUTHORITY."'";
                $extra .= ",'SUBWIN',0,0,screen.availWidth,screen.availHeight);\"";
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJA122O");
	//2010.02.09
