...
class knja124psForm1 {

    function main(&$model) {

        $objForm = new form;

        $arg["start"] = $objForm->get_start("edit", "POST", "knja124psindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //印影登録有無
        $query = knja124psQuery::getInkanUmu();
        ...
        $query = knja124psQuery::getHrClass($model);
        ...
        //生徒リスト
        $studentArray = array();
        $query = knja124psQuery::getStudents($model);
        ...
        View::toHTML($model, "knja124psForm1.html", $arg);
    }
}
...
function setData(&$objForm, &$arg, $db, $model, $studentRow) {
    ...
    $query = knja124psQuery::getTrainRow($model, $schregno);
    $tRow = $db->getRow($query, DB_FETCHMODE_ASSOC);

    //行動の記録・特別活動の記録
    $behavior = "";
    $resultb = $db->query(knja124psQuery::getBehavior($model, $schregno));
    ...
    //学習記録データ
    $query = knja124psQuery::getStudyRec($model, $schregno);
    $result = $db->query($query);
    $study = "";
    ...
    //出欠記録データ
    $attend = $db->getRow(knja124psQuery::getAttendRec($model, $schregno), DB_FETCHMODE_ASSOC);

    //表示データのHASHを取得
    $model->hash[$schregno] = $model->makeHash($tRow, $behavior, $study, $attend);

    //署名データ取出し
    $query = knja124psQuery::getOpinionsSignature($model, $schregno);
    $opinionsSignature = $db->getRow($query, DB_FETCHMODE_ASSOC);

    //印影の表示
    $query = knja124psQuery::getIneiFlg($model, $schregno);
