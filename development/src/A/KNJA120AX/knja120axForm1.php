...
class knja120axForm1
{
    function main(&$model)
    {
        $objForm = new form;
        $arg["start"] = $objForm->get_start("edit", "POST", "knja120axindex.php", "", "edit");

        $db = Query::dbCheckOut();

        //年度学期
        $query = knja120axQuery::getYearSeme();
        ...
        //年組
        $query = knja120axQuery::getHrClass($model);
        $extra = "onChange=\"return btn_submit('edit');\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["HR_CLASS"], "HR_CLASS", $extra, 1);

        //入力項目
        $query = knja120axQuery::getInputVal($model);
        $extra = "onChange=\"return btn_submit('edit');\"";
        ...
        $query = knja120axQuery::getStudentsVal($model, $tableNm, $fieldNm);
        ...
        //Hidden作成
        makeHidden($objForm, $schregNos, $model);

        Query::dbCheckIn($db);

        $arg["finish"]  = $objForm->get_finish();

        View::toHTML($model, "knja120axForm1.html", $arg);
    }
}
...
//hidden
function makeHidden(&$objForm, $schregNos, $model) {
