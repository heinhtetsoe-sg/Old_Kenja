...
class knja110_2bSubHist
{
    function main(&$model)
    {
        $objForm        = new form;
        $arg["start"]   = $objForm->get_start("sel", "POST", "knja110_2bindex.php", "", "sel");
        ...
        $rirekiFlg = false;
        $query = knja110_2bQuery::getGuardianHist($model);
        ...
        if (isset($model->hist_issuedate) && !isset($model->warning) && $model->cmd != "changeCmb") {
            if ($rirekiFlg && $model->hist_issuedate) {
                $query = knja110_2bQuery::getGuardianHistData($model, $model->schregno, $model->hist_issuedate);
                $setRow = $db->getRow($query, DB_FETCHMODE_ASSOC);
            } else if (!$rirekiFlg) {
                $query = knja110_2bQuery::getGuardianAddr($model, $model->schregno, "");
                $setRow = $db->getRow($query, DB_FETCHMODE_ASSOC);
            }
            //クリア
        ...
        //性別コンボボックス
        $query = knja110_2bQuery::getNameMst("Z002");
        $extra = "";
        ...
        //続柄コンボボックス
        $query = knja110_2bQuery::getNameMst("H201");
        $extra = "";
        ...
        //戻るボタン
        $link = REQUESTROOT."/A/KNJA110_2B/knja110_2bindex.php?cmd=back&ini2=1";
        ...
        View::toHTML($model, "knja110_2bSubHist.html", $arg);
    }
}
//makeCmb
