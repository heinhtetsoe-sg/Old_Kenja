...
class knja062bs1Form2 {
    function main(&$model) {
        $objForm        = new form;
        $arg["start"]   = $objForm->get_start("edit", "POST", "knja062bs1index.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        if (!isset($model->warning)) {
            $Row = $db->getRow(knja062bs1Query::getData($model), DB_FETCHMODE_ASSOC);
        } else {
            $Row =& $model->fields;
        }

        //担任区分コンボ
        $query = knja062bs1Query::getTrDiv();
        ...
        //担任
        $query = knja062bs1Query::getStaff($model);
        ...
        if (VARS::post("cmd") != "edit" && VARS::post("cmd") != "" && !isset($model->warning)){
            $arg["reload"]  = "window.open('knja062bs1index.php?cmd=updlist','left_frame');";
        }

        View::toHTML($model, "knja062bs1Form2.html", $arg);
    }
}
