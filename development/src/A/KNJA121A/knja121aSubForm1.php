...
class knja121aSubForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform1", "POST", "knja121aindex.php", "", "subform1");
        ...
        $opt_year = array();
		$year_flg = false;
        $result = $db->query(knja121aQuery::selectQueryYear($model));
        ...
        //学期一覧取得
        $semeList = $db->getCol(knja121aQuery::getSemesterMst($model));

        //通知表所見
        $query = knja121aQuery::getHreportremarkDat($model);
        ...
        //観点・評価
        $result = $db->query(knja121aQuery::getHreportremarkDetailDat($model, "1"));
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //出力項目取得
        $koumoku = array();
        $result = $db->query(knja121aQuery::getNameMst($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //特別活動等の記録
        $dataCnt = 1;
        $result = $db->query(knja121aQuery::getHreportremarkDetailDat($model));
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        View::toHTML($model, "knja121aSubForm1.html", $arg);
    }
}
?>
