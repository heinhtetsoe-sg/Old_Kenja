...
class knja123psForm1 {

    function main(&$model) {

        $objForm = new form;

        $arg["start"] = $objForm->get_start("edit", "POST", "knja123psindex.php", "", "edit");
        ...
        //行動の記録・特別活動の記録
        $behavior = "";
        $resultb = $db->query(knja123psQuery::getBehavior($model));
        ...
        //出欠記録データ
        $attend = $db->getRow(knja123psQuery::getAttendRec($model), DB_FETCHMODE_ASSOC);
        ...
        //署名データ取出し
        $query = knja123psQuery::getOpinionsSignature($model);
        $opinionsSignature = $db->getRow($query, DB_FETCHMODE_ASSOC);

        //印影登録有無
        $query = knja123psQuery::getInkanUmu();
        ...
        //印影の表示
        $query = knja123psQuery::getIneiFlg($model);
        ...
        View::toHTML($model, "knja123psForm1.html", $arg);
    }
}
