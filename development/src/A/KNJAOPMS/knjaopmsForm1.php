...
class knjaopmsForm1 {
    ...
    function main(&$model) {
        ...
        $arg["start"] = $objForm->get_start("edit", "POST", "knjaopmsindex.php", "", "edit");
...
//        makeStudentList($objForm, $arg, $db, $model);
        ...
        View::toHTML($model, "knjaopmsForm1.html", $arg);
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    ...
    //対象クラスリストを作成する
    $query = knjaopmsQuery::getGradeHrClass($model);
    $result = $db->query($query);
    $opt1 = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
