...
class knja124sForm1 {
    ...
    function main(&$model) {
        ...
        $arg["start"] = $objForm->get_start("edit", "POST", "knja124sindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //印影登録有無
        $query = knja124sQuery::getInkanUmu();
        ...
        $query = knja124sQuery::getHrClass($model);
        ...
        //生徒リスト
        $studentArray = array();
        $query = knja124sQuery::getStudents($model);
        ...
        View::toHTML($model, "knja124sForm1.html", $arg);
    }
}
...
function setData(&$objForm, &$arg, $db, $model, $studentRow) {
    ...
    //HTRAINREMARK_DAT 取得
    $query = knja124sQuery::getTrainRow($model, $schregno);
    $tRow = $db->getRow($query, DB_FETCHMODE_ASSOC);

    //学習記録データ
    $query = knja124sQuery::getStudyRec($model, $schregno);
    $result = $db->query($query);
    $study = "";
    ...
    //出欠記録データ
    $attend = $db->getRow(knja124sQuery::getAttendRec($model, $schregno), DB_FETCHMODE_ASSOC);

    //表示データのHASHを取得
    $model->hash[$schregno] = $model->makeHash($tRow, $study, $attend);

    //署名データ取出し
    $query = knja124sQuery::getOpinionsSignature($model, $schregno);
    $opinionsSignature = $db->getRow($query, DB_FETCHMODE_ASSOC);

    //印影の表示
    $query = knja124sQuery::getIneiFlg($model, $schregno);
