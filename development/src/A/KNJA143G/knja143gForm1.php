...
class knja143gForm1
{
    function main(&$model){
        ...
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("knja143gForm1", "POST", "knja143gindex.php", "", "knja143gForm1");

        //DB接続
        $db = Query::dbCheckOut();

        //学期マスタ
        $query = knja143gQuery::getSemeMst(CTRL_YEAR, CTRL_SEMESTER);
        ...
        if ($model->field["RISHUU"] === '1') {
            $extra = "checked='checked' onclick=\"return btn_submit('knja143g');\" ";
        } else {
            $extra = "onclick=\"return btn_submit('knja143g');\"";
        }
        ...
        if ($model->field["RISHUU"] === '1') {
            //履修履歴コンボ
            $arg["rirekiCodeSet"] = "1";
            $query = knja143gQuery::getRirekiCode($Row_Mst["YEAR"]);
            $extra = "onChange=\"return btn_submit('knja143g');\"";
        ...
        if( !isset($model->field["TERM_EDATE"]) ) 
            $model->field["TERM_EDATE"] = (CTRL_YEAR + 2)."/03/31";
        $arg["data"]["TERM_EDATE"]=View::popUpCalendar($objForm,"TERM_EDATE",$model->field["TERM_EDATE"]);

        //クラス
        $query = knja143gQuery::getAuth(CTRL_YEAR, CTRL_SEMESTER);
        $class_flg = false;
        $row1 = array();
        ...
        $query = knja143gQuery::getSchno($model, CTRL_YEAR, CTRL_SEMESTER);
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        View::toHTML($model, "knja143gForm1.html", $arg); 
    }
}
...
function makeHidden(&$objForm, $model)
{
    ...
    $objForm->ae(createHiddenAe("PRGID", "KNJA143G"));
