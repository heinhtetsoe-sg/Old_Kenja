...
class knja133oForm1
{
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knja133oForm1", "POST", "knja133oindex.php", "", "knja133oForm1");
        ...
        if ($model->cmd != "sslExe") {
            ...
            $arg["APP"]["APPHOST"] = '"../../..'.APPLET_ROOT.'_OLD/KNJA133S"';
        }
        ...
        View::toHTML($model, "knja133oForm1.html", $arg); 
...
function makeBtn(&$objForm, &$arg, $db, $model)
{
    //所見処理
    if ($model->syomeiBtn) {
        $query = knja133oQuery::getGdat();
        $result = $db->query($query);
        $gdatArray = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $gdatArray[] = $row["SCHOOL_KIND"];
        }
        $query = knja133oQuery::getSchoolmst();
        $schoolMst = $db->getRow($query, DB_FETCHMODE_ASSOC);

        $query = knja133oQuery::getNameMstCd2("Z001", $schoolMst["SCHOOLDIV"]);
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJA133O");
    //2010.02.09
