<?php
class knja120ooForm1 {
    function main(&$model) {

        $objForm = new form;
        $arg["start"] = $objForm->get_start("edit", "POST", "knja120ooindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //HTRAINREMARK_DAT 取得
        $query = knja120ooQuery::getTrainRow($model);
        $tRow = $db->getRow($query, DB_FETCHMODE_ASSOC);

        //学習記録データ
        $query = knja120ooQuery::getStudyRec($model);
        ...
        //出欠記録データ
        $attend = $db->getRow(knja120ooQuery::getAttendRec($model), DB_FETCHMODE_ASSOC);
        ...
        //署名データ取出し
        $query = knja120ooQuery::getOpinionsSignature($model);
        $opinionsSignature = $db->getRow($query, DB_FETCHMODE_ASSOC);

        //印影登録有無
        $query = knja120ooQuery::getInkanUmu();
        ...
        //印影の表示
        $query = knja120ooQuery::getIneiFlg($model);
        ...
        if ($model->Properties["seitoSidoYorokuFieldSize"] == 1) {
            //活動内容
            $extra = "readonly=\"readonly\" style=\"height:120px;\"";
        ...
        View::toHTML($model, "knja120ooForm1.html", $arg);
    }
}
