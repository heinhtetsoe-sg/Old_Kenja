...
class knja110_2aSubHist
{
    public function main(&$model)
    {
        ...
        $rirekiFlg = false;
        $query = knja110_2aQuery::getGuardianHist($model);
        ...
        if (isset($model->hist_issuedate) && !isset($model->warning) && $model->cmd != "changeCmb") {
            if ($rirekiFlg && $model->hist_issuedate) {
                $query = knja110_2aQuery::getGuardianHistData($model, $model->schregno, $model->hist_issuedate);
                $setRow = $db->getRow($query, DB_FETCHMODE_ASSOC);
            } elseif (!$rirekiFlg) {
                $query = knja110_2aQuery::getGuardianAddr($model, $model->schregno, "");
                $setRow = $db->getRow($query, DB_FETCHMODE_ASSOC);
            }
            //クリア
        ...
        View::toHTML($model, "knja110_2aSubHist.html", $arg);
    }
}
//makeCmb
