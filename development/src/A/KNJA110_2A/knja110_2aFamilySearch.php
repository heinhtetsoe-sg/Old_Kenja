...
class stucd {
    ...
    function getGrd_Cls() {
       ...
       global $db;
        ...
        $query .= "       GRADE || '_' || HR_CLASS AS GC, GRADE, HR_CLASS, ";
        ...
        if($this->cmd == "btn_search") {
            if ($this->stucd || $this->familyNo || $this->schregno || $this->grd_cls || $this->name || $this->name_kana || $this->guard_telno) {
                ...
                if(strlen($this->grd_cls) && $this->grd_cls != 00) {
                    list($grade,$hr_class) = preg_split("/_/",$this->grd_cls);
    ...
    //フォームタブ(開始)
    function getStart() {
        return $this->form->get_start("stucd","POST","knja110_2aFamilySearch.php","","stucd");
    }
    //フォームタブ(終了)
    function getFinish() {
...
function apply_stucd(obj,cmd) {
    ...
    f.document.forms[0].searchVal.value = setData;
    f.btn_submit('apply');
}
