...
class knja062s1Form1 {
    function main(&$model) {
        $objForm      = new form;
        $arg["start"] = $objForm->get_start("list", "POST", "knja062s1index.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //クラス情報
        $hrInfo = $db->getRow(knja062s1Query::getHrInfo($model), DB_FETCHMODE_ASSOC);
        ...
        if (VARS::request("cmd") == "list_edit") {
            $arg["reload"] = "window.open('knja062s1index.php?cmd=edit','right_frame');";
        }

        View::toHTML($model, "knja062s1Form1.html", $arg);
    }
}

//リスト作成
function makeList(&$objForm, &$arg, $db, $model) {
    $result = $db->query(knja062s1Query::getList($model, "", "LIST"));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        array_walk($row, "htmlspecialchars_array");

        if ($row["SEMESTER"] == $model->getParam["SEMESTER"]) {
            $row["TR_NAME"] = View::alink("knja062s1index.php", $row["TR_NAME"], "target=right_frame",
                                     array("cmd"         => "edit",
