...
class knja063as1Form1
{
    function main(&$model)
    {
        $objForm        = new form;
        $arg["start"]   = $objForm->get_start("list", "POST", "knja063as1index.php", "", "list");
        $db             = Query::dbCheckOut();
        ...
        //年度・学期
        $arg["term"] = $db->getOne(knja063as1Query::getTerm($model));

        //対象複式クラスコンボ
        $query = knja063as1Query::getGhrCd($model);
        $extra = "tabindex=\"1\" onchange=\"return btn_submit('list');\"";
        makeCmb($objForm, $arg, $db, $query, $model->ghr_cd, "GHR_CD", $extra, 1);

        //担任名
        $arg["teacher"] = $db->getOne(knja063as1Query::getTeacher($model));

        //ソート
        $model->sorttype = $db->getOne(knja063as1Query::getSort());
        ...
        $arg["sort1"] = View::alink("knja063as1index.php", "性別＋氏名かな".$mark1, "target=_self tabindex=\"-1\"", 
                                array("cmd"   => "list",
                                      "sort1" => ($model->sort["1"] == "1")?"0":"1",
                                      "s_id"  => "1") );

        $arg["sort2"] = View::alink("knja063as1index.php", "氏名かな＋性別".$mark2, "target=_self tabindex=\"-1\"", 
                                array("cmd"   => "list",
                                      "sort2" => ($model->sort["2"] == "1")?"0":"1",
                                      "s_id"  => "2") );

        $arg["sort3"] = View::alink("knja063as1index.php", "複式出席番号順".$mark3, "target=_self tabindex=\"-1\"", 
                                array("cmd"   => "list",
                                      "sort3" => ($model->sort["3"] == "1")?"0":"1",
                                      "s_id"  => "3") );

        $arg["sort4"] = View::alink("knja063as1index.php", "年組番順".$mark4, "target=_self tabindex=\"-1\"", 
                                array("cmd"   => "list",
        ...
        $result = $db->query(knja063as1Query::getStudents($model));
        ...
        $subdata  = "wopen('".REQUESTROOT."/A/KNJA063A/knja063aindex.php?cmd=main&SEND_AUTH={$model->auth}";
        ...
        View::toHTML($model, "knja063as1Form1.html", $arg); 
    }
}
//コンボ作成
