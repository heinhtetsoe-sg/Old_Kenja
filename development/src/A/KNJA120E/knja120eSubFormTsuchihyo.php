<?php
//ビュー作成用クラス
class knja120eSubFormTsuchihyo
{
    public function main(&$model)
    {
        ...
        $arg = array();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("subform1", "POST", "knja120eindex.php", "", "subformTsuchihyo");
        ...
        if ($model->Properties["useTuutisyoSyokenNendo"] == "1") {
            //年度コンボ（通知表所見）
            $opt_year = array();
            $result = $db->query(knja120DQuery::selectQueryYear($model));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                $opt_year[] = array("label" => $row["YEAR"],"value" => $row["YEAR"]);
                if ($model->year_cmb == "") {
                    $model->year_cmb = $row["YEAR"];
                }
            }
        ...
        $query = knja120eQuery::selectQueryGuide($model);
        ...
        View::toHTML($model, "knja120eSubFormTsuchihyo.html", $arg);
    }
}
