...
class knja228Form1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knja228index.php", "", "main");
        ...
        $query = knja228Query::getNendo();
        makeCmb($objForm, $arg, $db, $query, "YEAR", $model->year, $extra, 1);

//        //年組コンボボックス
//        $extra = "onchange=\"return btn_submit('main');\" tabindex=-1";
//        $query = knja228Query::getNenKumi('A023', $model->year);
...
//        //塾名
//        $query = knja228Query::getPriSchoolName($model->preischoolcd);
//        $arg["data"]["PRISCHOOL_NAME"] = $db->getOne($query);

//        //教室名
//        $query = knja228Query::getPriSchoolClassName($model->preischoolcd, $model->preischoolClassCd);
//        $arg["data"]["PRISCHOOL_CLASS_NAME"] = $db->getOne($query);

        //ラジオボタン
        $opt_radio = array(1, 2);
        $model->disppattern = ($model->disppattern == "") ? "1" : $model->disppattern;
        $extra = array("id=\"DISP_PATTERN1\" onchange=\"initsel();btn_submit('year');\"", "id=\"DISP_PATTERN2\" onchange=\"initsel();btn_submit('year');\"");
        $radioArray = knjCreateRadio($objForm, "DISP_PATTERN", $model->disppattern, $extra, $opt_radio, get_count($opt_radio));
        ...
        View::toHTML($model, "knja228Form1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->selected[0]) {
        $query = knja228Query::getSelectInfo($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
//     else {
//        $query = knja228Query::getSelect2($model);
//        $result = $db->query($query);
//        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
//            $leftList[]  = array('label' => $row["LABEL"], 'value' => $row["VALUE"]);
//            $left_schregno[] = $row["SCHREGNO"];
...
//    }
    ...
    //一覧
    $rightList = array();
    $query = knja228Query::getSelect($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJA228");
