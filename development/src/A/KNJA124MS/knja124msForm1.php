...
class knja124msForm1 {

    function main(&$model) {

        $objForm = new form;

        $arg["start"] = $objForm->get_start("edit", "POST", "knja124msindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //印影登録有無
        $query = knja124msQuery::getInkanUmu();
        ...
        $query = knja124msQuery::getHrClass($model);
        ...
        //生徒リスト
        $studentArray = array();
        $query = knja124msQuery::getStudents($model);
        ...
        View::toHTML($model, "knja124msForm1.html", $arg);
    }
}
...
function setData(&$objForm, &$arg, $db, $model, $studentRow) {
    ...
    //HTRAINREMARK_DAT 取得
    $query = knja124msQuery::getTrainRow($model, $schregno);
    $tRow = $db->getRow($query, DB_FETCHMODE_ASSOC);

    //学習記録データ
    $query = knja124msQuery::getStudyRec($model, $schregno);
    $result = $db->query($query);
    $study = "";
    ...
    //HR_ATTEND_DAT
    $attend = $db->getOne(knja124msQuery::getHrAttend($model, $schregno));

    //表示データのHASHを取得
    $model->hash[$schregno] = $model->makeHash($tRow, $study, $attend);

    //署名データ取出し
    $query = knja124msQuery::getOpinionsSignature($model, $schregno);
    $opinionsSignature = $db->getRow($query, DB_FETCHMODE_ASSOC);

    //印影の表示
    $query = knja124msQuery::getIneiFlg($model, $schregno);
