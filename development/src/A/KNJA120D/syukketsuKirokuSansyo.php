...
class syukketsuKirokuSansyo {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("syukketsu", "POST", "knja120dindex.php", "", "syukketsu");
        ...
        if ($db->getOne(knja120dQuery::checkSchoolMst()) > 0) {
            $schoolcd    = sprintf("%012d", SCHOOLCD);
            $school_kind = $db->getOne(knja120dQuery::getSchoolKind($model));
        }
        ...
        //出欠の記録
        $sick = $late = $early = 0;
        $result = $db->query(knja120dQuery::getAttendSemesDat($model, $knjSchoolMst));
