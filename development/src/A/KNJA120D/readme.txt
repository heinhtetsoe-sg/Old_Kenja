...
$Id: readme.txt 76793 2020-09-11 06:11:39Z ishii $
...
2008/03/17  1.リネームKNJA120 → KNJA120D
...
2013/07/31  1.テーブル追加に伴う修正(HTRAINREMARK_DETAIL_DAT)

2013/08/14  1.プロパティーtrain_ref_1_2_3_field_sizeに２を追加
...
2013/08/30  1.部活動取込ボタンを仕様とを変更
              - 総合所見及び指導上参考となる諸事項の選択したフォーカス位置からコピーする
            2.取込ボタンの色を再度変更
            3.部活動の取込ボタンのデフォルトをTRAIN_REF2（真ん中）に変更

2013/09/09  1.全年度参照ボタン追加
...
2013/10/28  1.総合所見及び指導上参考となる諸事項に3年生は調査書取込ボタンを追加
                - 同一年度のHEXAM_ENTREMARK_DATのTRAIN_REF1、TRAIN_REF2、TRAIN_REF3をセット

2013/10/29  1.総合所見及び指導上参考となる諸事項に3年生は調査書取込ボタンをカット(2013/10/28修正分)
            2.調査書より読込ボタンを追加
              - 3年生のみボタンを表示

2014/03/17  1.プロパティseitoSidoYoroku_dat_SpecialactremarkSize設定時の文字チェックを追加
...
2015/02/23  1.選択、取込ボタンを押した場合、テキストエリアの色をピンクに変更
                - 更新後は元に戻す
            2.委員会取込、部活動取込ボタンのIE11からの仕様(document.selectionが廃止)に対応

2015/02/25  1.特別活動所見の取込フラグの指定を修正

2015/02/27  1.学年(SCHREG_REGD_GDATのGRADE_CDで判断)により下記のボタンを追加
              - 1,2年次は通知票取込ボタン(プロパティ「tutihyoYoriYomikomiHyoujiFlg」= '1'で表示)
              - 3年次は調査書取込ボタン

2015/03/03  1.3年次で留年生の場合は、調査書より読込みボタンおよび調査書取込ボタンをグレーアウトする
...
2015/06/19  1.「通知書所見参照」ボタンは海上学園は非表示とする

2015/07/24  1.部活動取込ボタン押下時、最初に対応レコードのみを取得し、その後で項番を付けて表示するよう修正

2016/02/24  1.部活動取込ボタン押下時のカーソル合わせたところに色を変更するよう修正(CVS未登録だったものを登録)
            2.パラメータ（SCHOOL_KIND=H）追加
...
2017/09/13  1.Properties["train_ref_1_2_3_field_size"]を'-'区切りで設定できるよう修正（指導上参考となる諸事項の3分割の各項目）

2017/09/14  1.指導上参考となる諸事項の3分割の各項目はプロパティーseitoSidoYoroku_train_ref_1_2_3_field_size、seitoSidoYoroku_train_ref_1_2_3_gyo_sizeを優先して使用する
...
2018/08/27  1.ボタンカット
            -- 委員会参照、委員会取込、部活動参照、部活動取込、資格参照
            2.ボタン追加
            -- 委員会選択、部活動選択、記録備考選択、検定選択、賞選択
            3.更新時は更新ボタンを押下できないよう変更

2019/01/15  1.共通プログラムに変更
            -- 委員会選択、賞選択、部活動選択、記録備考選択、検定選択

2019/01/21  1.出欠の記録備考のサイズチェック追加
            -- プロパティー「seitoSidoYoroku_dat_Attendrec_RemarkSize」の対応漏れ
            2.留年のチェックをGRADE_CDからGRADEに変更
...
2019/02/27  1.条件を「3年生のとき」→「卒業可能学年のとき」に変更。読込ボタンの使用不可条件カット

2019/09/19  1.「出欠の記録参照」は「KNJX_SYUKKETSUKIROKU」を呼び出すよう修正

2019/11/20  1.「総合所見及び指導上参考となる諸事項 5)その他」に「特記事項なし」チェックボックスを追加

2020/02/18  1.「年間出欠備考選択」ボタンを追加
            -- プロパティー「useReasonCollectionBtn」が'1'のとき表示

2020/02/21  1.生徒が選択されていない場合、参照系のボタンを非活性にするよう修正

2020/03/12  1.「通知表所見参照」ボタンで表示される画面を共通「KNJX_TUUCHISYOKEN_SELECT」の通知表所見参照を表示するよう修正

2020/04/09  1.総合所見及び指導上参考となる諸事項の欄へ「調査書取込」ボタンを追加
