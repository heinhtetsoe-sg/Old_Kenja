...
class knja110bSubForm2
{
    function main(&$model)
    {
        ...
        $arg = array();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knja110bSubForm2", "POST", "knja110bindex.php", "", "knja110bSubForm2");
        ...
        for ($i = 0; $i < get_count($schregnos); $i++) {
            ...
            $query = knja110bQuery::getEappDate($schregnos[$i]);
            $sch_eApp_ent[] = $db->getRow($query, DB_FETCHMODE_ASSOC);

            //更新するときに履歴に入れるデータ
            $query = knja110bQuery::getStudent_data_before($model, $schregnos[$i]);
            $histArray[] = $db->getRow($query, DB_FETCHMODE_ASSOC); //今までのデータ

            //履歴の最終日
            $last_hist = CTRL_YEAR.'/04/01';
            $query = knja110bQuery::get_last_hist_expiredate($schregnos[$i]);
           ...
           if ($last_hist_expiredate > $last_hist) {
            ...
            //入学日付
            $query = knja110bQuery::get_ent_date($schregnos[$i]);
        ...
        View::toHTML($model, "knja110bSubForm2.html", $arg);
    }
}
