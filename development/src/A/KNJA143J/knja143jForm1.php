...
class knja143jForm1
{
    function main(&$model){
        ...
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("knja143jForm1", "POST", "knja143jindex.php", "", "knja143jForm1");
        ...
        for ($i = 1; $i <= get_count($opt_output); $i++) {
            $objForm->ae( array("type"       => "radio",
                                ...
                                "extrahtml"  => $extra[$i-1]. " onchange=\"return btn_submit('changeOutput');\"",
                                "multiple"   => $opt_output));
        ...
        //学期マスタ
        $query = knja143jQuery::getSemeMst(CTRL_YEAR, CTRL_SEMESTER);
        ...
        //学期
        $objForm->ae(createHiddenAe("SEMESTER", $Row_Mst["SEMESTER"]));
        $arg["data"]["SEMESTER"] = $Row_Mst["SEMESTERNAME"];
        ...
        //クラス
        $query = knja143jQuery::getAuth(CTRL_YEAR, CTRL_SEMESTER);
        $class_flg = false;
        $row1 = array();
        ...
        //checkbox
        $checked = $model->cmd == "" || $model->cmd == "changeOutput" || $model->field["PRINT_STATION"] == "1" ? " checked " : "";
        $extra = "id='PRINT_STATION' {$checked} ";
        $arg["data"]["PRINT_STATION"] = knjCreateCheckBox($objForm, "PRINT_STATION", "1", $extra);
        ...
        $query = knja143jQuery::getSchno($model, CTRL_YEAR, CTRL_SEMESTER);
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        View::toHTML($model, "knja143jForm1.html", $arg); 
    }
}
...
function makeHidden(&$objForm, $model)
{
    ...
    $objForm->ae(createHiddenAe("PRGID", "KNJA143J"));
