...
class knja143rForm1
{
    function main(&$model){
        ...
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("knja143rForm1", "POST", "knja143rindex.php", "", "knja143rForm1");
        ...
        $extra = array("id=\"DISP1\" onClick=\"return btn_submit('knja143r')\"", "id=\"DISP2\" onClick=\"return btn_submit('knja143r')\"");
        ...
        $query = knja143rQuery::getSemeMst($ctrl_year, $ctrl_semester);
        ...
        $row1 = array();
        $class_flg = false;
        $query = knja143rQuery::getAuth($model, $ctrl_year, $ctrl_semester);
        ...
        if ($model->field["DISP"] == 1) {
            ...
            $query = knja143rQuery::getSchno2($model,$ctrl_year,$ctrl_semester);
            $result = $db->query($query);
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        View::toHTML($model, "knja143rForm1.html", $arg); 
    }
}
...
function makeHidden(&$objForm, $Row_Mst, $model)
{
    ...
    createHidden($objForm, "PRGID", "KNJA143R");
