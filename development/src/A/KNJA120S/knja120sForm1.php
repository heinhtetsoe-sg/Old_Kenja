...
class knja120sForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        $arg["start"] = $objForm->get_start("edit", "POST", "knja120sindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //HTRAINREMARK_DAT 取得
        $query = knja120sQuery::getTrainRow($model);
        $tRow = $db->getRow($query, DB_FETCHMODE_ASSOC);

        //学習記録データ
        $query = knja120sQuery::getStudyRec($model);
        ...
        //出欠記録データ
        $attend = $db->getRow(knja120sQuery::getAttendRec($model), DB_FETCHMODE_ASSOC);
        ...
        //署名データ取出し
        $query = knja120sQuery::getOpinionsSignature($model);
        $opinionsSignature = $db->getRow($query, DB_FETCHMODE_ASSOC);

        //印影登録有無
        $query = knja120sQuery::getInkanUmu();
        ...
        //印影の表示
        $query = knja120sQuery::getIneiFlg($model);
        ...
        View::toHTML($model, "knja120sForm1.html", $arg);
    }
}
