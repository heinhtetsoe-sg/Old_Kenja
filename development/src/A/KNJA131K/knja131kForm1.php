...
//フォーム作成////////////////////////////////////////////////////////////////////////
$arg["start"]   = $objForm->get_start("knja131kForm1", "POST", "knja131kindex.php", "", "knja131kForm1");
...
$db = Query::dbCheckOut();
        $z010name1 = "";
        $query = knja131kQuery::getNameMst("Z010", "00");
...
if ($disable == 1){
    ...
    $query = knja131kQuery::getAuth2($model);
}
$result = $db->query($query);
while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
...
$objForm->ae( array("type"       => "select",
                    ...
                    "extrahtml"  => "onchange=\"return btn_submit('knja131k'),AllClearList();\"",
...
/*
$db = Query::dbCheckOut();
$query = knja131kQuery::getAuth($model);
$result = $db->query($query);
while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
    $row1[]= array('label' => $row["LABEL"],
                    'value' => $row["VALUE"]);
...
$objForm->ae( array("type"       => "select",
                    ...
                    "size"       => "1",
                    "value"      => $model->field["GRADE_HR_CLASS"],
                    "extrahtml"  => "onchange=\"return btn_submit('knja131k');\"",
                    "options"    => isset($row1)?$row1:array()));
...
View::toHTML($model, "knja131kForm1.html", $arg); 
}
}
?>
