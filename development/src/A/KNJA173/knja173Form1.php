...
class knja173Form1 {
    function main(&$model){
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knja173Form1", "POST", "knja173index.php", "", "knja173Form1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //学年コンボ
        $query = knja173Query::getGrade($model);
        $extra = "onChange=\"btn_submit('knja173')\"";
        ...
        View::toHTML($model, "knja173Form1.html", $arg);
    }
}
//makeCmb
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    //クラス一覧
    $row1 = array();
    $result = $db->query(knja173Query::getAuth($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model, $seme, $semeflg)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJA173");
