...
class knja130kForm1 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knja130kForm1", "POST", "knja130kindex.php", "", "knja130kForm1");
        ...
        $extra = "onchange=\"return btn_submit('knja130k'), AllClearList();\"";
        ...
        View::toHTML($model, "knja130kForm1.html", $arg); 
    }
}
/****************************************** 以下関数 ******************************************************/
//生徒リストToリスト作成
function makeStudentList(&$objForm, &$arg, $db, $model) {
    //対象外の生徒取得
    $query = knja130kQuery::getSchnoIdou($model);
    ...
    //対象者リストを作成する
    $query = knja130kQuery::getStudent($model, 'list', $selectdata);
    $result = $db->query($query);
    $opt1 = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $idou = "　";
    ...
    //生徒一覧リストを作成する
    if($selectdata){
        $query = knja130kQuery::getStudent($model, 'select', $selectdata);
        $result = $db->query($query);
        $opt1 = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $idou = "　";
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJA130K");
