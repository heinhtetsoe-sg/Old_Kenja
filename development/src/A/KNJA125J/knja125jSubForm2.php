...
class knja125jSubForm2
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform2", "POST", "knja125jindex.php", "", "subform2");
        ...
        if ($db->getOne(knja125jQuery::checkSchoolMst()) > 0) {
            ...
            $school_kind    = $db->getOne(knja125jQuery::getSchoolKind($model));
        }
        ...
        for ($i=1; $i<=$model->control["学期数"]; $i++) {
            $result = $db->query(knja125jQuery::getAttendSemesDat($model, $i, $knjSchoolMst));
        ...
        View::toHTML($model, "knja125jSubForm2.html", $arg);
    }
}
