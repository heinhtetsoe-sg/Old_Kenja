...
class knja125jSyukketsuKirokuSansyo
{
    ...
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform8", "POST", "knja125jindex.php", "", "subform8");
        ...
        if ($db->getOne(knja125jQuery::checkSchoolMst()) > 0) {
            $schoolcd    = sprintf("%012d", SCHOOLCD);
            $school_kind = $db->getOne(knja125jQuery::getSchoolKind2($model));
        }
        ...
        //出欠の記録
        $sick = $late = $early = 0;
        $result = $db->query(knja125jQuery::getAttendSemesDat2($model, $knjSchoolMst));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            if ($row["SUSPEND"] != "") {
                $suspend += $row["SUSPEND"];
            }
            if ($row["MOURNING"] != "") {
                $mourning += $row["MOURNING"];
            }
            if ($row["SICK"] != "") {
                $sick += $row["SICK"];
            }
        ...
        View::toHTML($model, "knja125jSyukketsuKirokuSansyo.html", $arg);
    }
}
