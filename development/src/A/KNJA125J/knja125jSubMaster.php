...
class knja125jSubMaster
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("submaster", "POST", "knja125jindex.php", "", "submaster");

        //DB接続
        $db = Query::dbCheckOut();

        //学年取得
        $getGradeName = $db->getOne(knja125jQuery::getGradeName($model));

        //マスタ情報
        if ($model->cmd === 'teikei') {
            $setTitle = '道徳';
        } elseif ($model->cmd === 'teikei_act') {
            $setTitle = '総合的な学習の時間の記録（学習活動）';
        } else {
        ...
        View::toHTML($model, "knja125jSubMaster.html", $arg);
    }
}
...
{
    $i = 0;
    $query = knja125jQuery::getHtrainRemarkTempDat($model);
