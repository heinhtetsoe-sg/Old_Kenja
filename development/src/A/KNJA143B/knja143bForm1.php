...
class knja143bForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knja143bForm1", "POST", "knja143bindex.php", "", "knja143bForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        $sinnyu_tennyu_extra_1 = "id=\"SINNYU_TENNYU1\" onclick=\"btn_submit('knja143b');\"";
        $sinnyu_tennyu_extra_2 = "id=\"SINNYU_TENNYU2\" onclick=\"btn_submit('knja143b');\"";
        ...
        if ($model->field["SINNYU_TENNYU"] != 1) {
            $extra = "onChange=\"btn_submit('knja143b')\"";
        } else {
            $extra .= "";
        }
        ...
        knjCreateHidden($objForm, "PRGID",         "KNJA143B");
        knjCreateHidden($objForm, "STAFFCD",       STAFFCD);
        knjCreateHidden($objForm, "CHK_SDATE", $model->field["YEAR"] . '/04/01');
        knjCreateHidden($objForm, "CHK_EDATE", ((int)$model->field["YEAR"] + 1) . '/03/31');
        ...
        View::toHTML($model, "knja143bForm1.html", $arg);
    }
}
/****************************************************** 以下関数 ************************************************************/
//対象者一覧リストToリスト作成
function makeListToList(&$objForm, &$arg, $db, $model) {
    //対象者一覧
    $opt = array();
    $query = knja143bQuery::getCategoryName($model);
    if ($model->cmd == 'search') {
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
