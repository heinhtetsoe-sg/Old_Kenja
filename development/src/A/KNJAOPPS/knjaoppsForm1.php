...
class knjaoppsForm1 {
    ...
    function main(&$model) {
        ...
        $arg["start"] = $objForm->get_start("edit", "POST", "knjaoppsindex.php", "", "edit");
...
//        makeStudentList($objForm, $arg, $db, $model);
        ...
        View::toHTML($model, "knjaoppsForm1.html", $arg);
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    ...
    //対象クラスリストを作成する
    $query = knjaoppsQuery::getGradeHrClass($model);
    $result = $db->query($query);
    $opt1 = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
