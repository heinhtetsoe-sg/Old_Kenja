...
class knja120xForm1
{
    ...
    {
        $objForm = new form;
        $arg["start"] = $objForm->get_start("edit", "POST", "knja120xindex.php", "", "edit");

        $db = Query::dbCheckOut();

        //年度学期
        $query = knja120xQuery::getYearSeme();
        ...
        //年組
        $query = knja120xQuery::getHrClass($model);
        $extra = "onChange=\"return btn_submit('edit');\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["HR_CLASS"], "HR_CLASS", $extra, 1);

        //入力項目
        $query = knja120xQuery::getInputVal($model);
        $extra = "onChange=\"return btn_submit('edit');\"";
        ...
        $query = knja120xQuery::getStudentsVal($model, $tableNm, $fieldNm);
        ...
        View::toHTML($model, "knja120xForm1.html", $arg);
    }
}
