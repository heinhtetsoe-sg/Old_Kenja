...
class knja174Form1 {
    function main(&$model){
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knja174Form1", "POST", "knja174index.php", "", "knja174Form1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //学年コンボ
        $query = knja174Query::getGrade($model);
        $extra = "onChange=\"btn_submit('knja174')\"";
        ...
        View::toHTML($model, "knja174Form1.html", $arg);
    }
}
//makeCmb
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    //クラス一覧
    $row1 = array();
    $result = $db->query(knja174Query::getAuth($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model, $seme, $semeflg)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJA174");
