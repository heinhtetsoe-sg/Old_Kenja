...
class knja190bForm1
{
    function main(&$model){
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knja190bForm1", "POST", "knja190bindex.php", "", "knja190bForm1");
        ...
        $objForm->ae( array("type"       => "select",
                            ...
                            "extrahtml"  => "onChange=\"return btn_submit('knja190b');\"",
                            "options"    => $opt));
        ...
        $click = " onClick=\"return btn_submit('knja190b');\"";
        ...
        $value_flg = false;
        $max_len = 0;
        $query = knja190bQuery::getAuth($model, CTRL_YEAR, $ga);
        ...
        if ($model->field["CHOICE"] == "1") {
            ...
            $query = knja190bQuery::getSchno($model, CTRL_YEAR, $ga);
            $result = $db->query($query);
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
                //クラス名称調整
        ...
        if (!($model->field["HUKUSIKI_KIRIKAE"] == "2" && $model->Properties["useSpecial_Support_Hrclass"] == '1')) {
            ...
            if ($model->field["CHOICE"] == "1") {
                //テーブルの有無チェック
                $query = knja190bQuery::checkTableExist();
                $table_cnt = $db->getOne($query);
                if ($table_cnt > 0) {
                    //生徒項目名取得
                    $sch_label = $db->getOne(knja190bQuery::getSchName($model));
                }
            }
        }
        ...
        View::toHTML($model, "knja190bForm1.html", $arg); 
    }
}
?>
