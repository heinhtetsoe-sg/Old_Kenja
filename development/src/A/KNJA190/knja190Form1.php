...
class knja190Form1
{
    function main(&$model){
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knja190Form1", "POST", "knja190index.php", "", "knja190Form1");
        ...
        $objForm->ae( array("type"       => "select",
                            ...
                            "extrahtml"  => "onChange=\"return btn_submit('knja190');\"",
                            "options"    => $opt));
        ...
        $click = " onClick=\"return btn_submit('knja190');\"";
        ...
        if ($model->field["CHOICE"] == "3") {
            $query = knja190Query::getDomitory($model, CTRL_YEAR);
        } else {
            $query = knja190Query::getAuth($model, CTRL_YEAR, $ga);
        }
        ...
        if ($model->field["CHOICE"] == "1") {
            ...
            $query = knja190Query::getSchno($model,CTRL_YEAR,$ga);
            $result = $db->query($query);
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
                //クラス名称調整
        ...
        if (!($model->field["HUKUSIKI_KIRIKAE"] == "2" && $model->Properties["useSpecial_Support_Hrclass"] == '1')) {
            ...
            if ($model->field["CHOICE"] == "1" || ($model->field["CHOICE"] == "2" && ($model->Properties["useSchool_KindField"] == "1" && SCHOOLKIND != ""))) {
                //テーブルの有無チェック
                $query = knja190Query::checkTableExist();
                $table_cnt = $db->getOne($query);
                if ($table_cnt > 0) {
                    //生徒項目名取得
                    $sch_label = $db->getOne(knja190Query::getSchName($model));
                }
            }
        }
        ...
        if ($model->Properties["knja190PreferSendAddress"] != "1") {
            $arg["show_sonota"] = "1";
        }
        ...
        if ($model->field["OUTPUT"] == 2 || $model->field["OUTPUT"] == 4){
            $abled = "" ;
        }else {
            $abled = "disabled" ;
        }
        ...
        //開始位置（行）コンボボックスを作成する///////////////////////////////////////////////////////////////////////
        if ($model->Properties["knja190Form"] == "KNJA190_6.frm") {
        ...
        //開始位置（列）コンボボックスを作成する////////////////////////////////////////////////////////////////////////
        $z010 = $db->getOne(knja190Query::getZ010Name1());
        if ($z010 == 'higashiosaka' || $z010 == 'risshisha' || ($model->Properties["knja190Form"] == "KNJA190_6.frm")) {
        ...
        knjCreateHidden($objForm, "knja190Form", $model->Properties["knja190Form"]);
        knjCreateHidden($objForm, "knja190PreferSendAddress", $model->Properties["knja190PreferSendAddress"]);
        ...
        View::toHTML($model, "knja190Form1.html", $arg); 
    }
}
?>
