...
class knja121kForm1
{
    ...
    {
        $objForm = new form;
        $arg["start"] = $objForm->get_start("edit", "POST", "knja121kindex.php", "", "edit");
        //警告メッセージを表示しない場合
        if ((isset($model->schregno) && !isset($model->warning)) || !isset($model->schregno)){
            $row = knja121kQuery::getTrainRow($model->schregno, $model->exp_year);
            $arg["NOT_WARNING"] = 1;
...
***/
        ...
        $cnt = knja121kQuery::getKindaiJudgment();
        if ($cnt > 0) {
        } else {
...
*/
        ...
        //CSV処理
        $fieldSize  = "TOTALSTUDYACT=240,";
        $fieldSize .= "TOTALSTUDYVAL=240,";
        $fieldSize .= "SPECIALACTREMARK=0,";
        $fieldSize .= "TOTALREMARK=1305,";
        $fieldSize .= "ATTENDREC_REMARK=60,";
        $fieldSize .= "VIEWREMARK=120,";
        $fieldSize .= "BEHAVEREC_REMARK=0";

        $objForm->ae( array("type"      => "button",
                            "name"      => "btn_check1",
                            "value"     => "データCSV",
                            "extrahtml" => " onClick=\" wopen('".REQUESTROOT."/X/KNJX180/knjx180index.php?SEND_AUTH=".AUTHORITY."&SEND_PRGID=KNJA121K&FIELDSIZE=".$fieldSize."','SUBWIN2',0,0,screen.availWidth,screen.availHeight);\"" ));
        ...
        View::toHTML($model, "knja121kForm1.html", $arg);
    }
}
?>
