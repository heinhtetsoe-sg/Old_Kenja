...
class knja125pSubForm2
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform2", "POST", "knja125pindex.php", "", "subform2");
        ...
        if ($db->getOne(knja125pQuery::checkSchoolMst()) > 0) {
            $schoolcd       = sprintf("%012d", SCHOOLCD);
            $school_kind    = $db->getOne(knja125pQuery::getSchoolKind($model));
        }
        ...
        for ($i=1; $i<=$model->control["学期数"]; $i++) {
            $result = $db->query(knja125pQuery::getAttendSemesDat($model, $i, $knjSchoolMst));
        ...
        View::toHTML($model, "knja125pSubForm2.html", $arg);
    }
}