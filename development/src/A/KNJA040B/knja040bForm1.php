...
class knja040bForm1
{
    public function main($model)
    {
        ...
        //動作条件チェック
        } elseif ($db->getOne(knja040bQuery::checktoStart()) == "0") {
        ...
        $arg["data"]["NOWDATE"]  = knja040bQuery::getMaxUpdate($db);
        ...
        //学年コンボボックス
        $query = knja040bQuery::getGrade($model);
        $extra = "";
        makeCmb($objForm, $arg, $db, $query, $model->grade, "GRADE", $extra, 1, "");

        //課程学科コンボ
        $query = knja040bQuery::getCourseMajor(($model->new_year));
        ...
        //学校マスタ取得
        $query = knja040bQuery::getSchoolMst($model);
        ...
        if ($model->field["GUARD_EXPIREDATE"]) {
            ...
            $query = knja040bQuery::getA023GradeRange($model->schoolKind);
            $addYear = $db->getOne($query);
            //中高一貫は、＋6年
            if ($model->schoolKind == "J" && $schoolDiv["NAMESPARE2"] == "1") {
                $value = ($eDate[0] + 3 + $addYear)."/03/31";
            } else {
                $value = ($eDate[0] + $addYear)."/03/31";
            }
        }
        ...
        Query::dbCheckIn($db);
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knja040bindex.php", "", "main");
        $arg["finish"]  = $objForm->get_finish();

        View::toHTML($model, "knja040bForm1.html", $arg);
    }
}
