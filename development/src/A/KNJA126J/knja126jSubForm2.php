...
class knja126jSubForm2
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform2", "POST", "knja126jindex.php", "", "subform2");
        ...
        if ($db->getOne(knja126jQuery::checkSchoolMst()) > 0) {
            $schoolcd       = sprintf("%012d", SCHOOLCD);
            $school_kind    = $db->getOne(knja126jQuery::getSchoolKind($model));
        }
        ...
        for($i=1; $i<=$model->control["学期数"]; $i++) {
            $result = $db->query(knja126jQuery::getAttendSemesDat($model, $i, $knjSchoolMst));
        ...
        View::toHTML($model, "knja126jSubForm2.html", $arg);
    }
}
?>