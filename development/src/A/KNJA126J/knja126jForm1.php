...
class knja126jForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knja126jindex.php", "", "edit");
        ...
        if($model->schregno){
            ...
            //学習記録データ
            $query = knja126jQuery::getStudyRec($model);
            ...
            //出欠記録データ
            $attend = $db->getRow(knja126jQuery::getAttendRec($model), DB_FETCHMODE_ASSOC);

            //HTRAINREMARK_DATのハッシュ値取得
            $hash = ($model->schregno && $row) ? $model->makeHash($row, $behavior, $study, $attend) : "";
            //ATTEST_OPINIONS_DATのハッシュ値取得
            $opinion = $db->getRow(knja126jQuery::getOpinionsDat($model), DB_FETCHMODE_ASSOC);
        ...
        //学習活動
        $extra = "style=\"height:118px;\"onkeyup=\"charCount(this.value, {$model->totalstudyact_gyou}, ({$model->totalstudyact_moji} * 2), true);\"";
        ...
        //観点
        $extra = "style=\"height:118px;\"onkeyup=\"charCount(this.value, {$model->viewremark_gyou}, ({$model->viewremark_moji} * 2), true);\"";
        $arg["data"]["VIEWREMARK"] = knjCreateTextArea($objForm, "VIEWREMARK", $model->viewremark_gyou, ((int)$model->viewremark_moji * 2 + 1), "soft", $extra, $row["VIEWREMARK"]);
        $arg["data"]["VIEWREMARK_COMMENT"] = "(全角".$model->viewremark_moji."文字X".$model->viewremark_gyou."行まで)";

        //評価
        $extra = "style=\"height:118px;\"onkeyup=\"charCount(this.value, {$model->totalstudyval_gyou}, ({$model->totalstudyval_moji} * 2), true);\"";
        ...
        View::toHTML($model, "knja126jForm1.html", $arg);
    }
}
...
{
    if((AUTHORITY < DEF_UPDATE_RESTRICT) || !$opinion || $schoolkind != 'J'){
        //更新ボタン
    ...
    //CSVボタン
    $extra = ($model->schregno && $schoolkind == 'J') ? "onClick=\" wopen('".REQUESTROOT."/X/KNJX_A125J/knjx_a125jindex.php?cmd=sign&FIELDSIZE=".$fieldSize."&EXP_YEAR=".$model->exp_year."&EXP_SEMESTER=".$model->exp_semester."&SCHREGNO=".$model->schregno."&AUTH=".AUTHORITY."&PRGID=".PROGRAMID."','SUBWIN2',0,0,screen.availWidth,screen.availHeight);\"" : "disabled";
