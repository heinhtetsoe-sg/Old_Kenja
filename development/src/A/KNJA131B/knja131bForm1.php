...
//フォーム作成////////////////////////////////////////////////////////////////////////
$arg["start"]   = $objForm->get_start("knja131bForm1", "POST", "knja131bindex.php", "", "knja131bForm1");
...
if ($disable == 1){
	...
	$query = knja131bQuery::getAuth2($model);
}
...
$objForm->ae( array("type"       => "select",
					...
					"extrahtml"	 => "onchange=\"return btn_submit('knja131b'),AllClearList();\"",
...
/*
$db = Query::dbCheckOut();
$query = knja131bQuery::getAuth($model);
$result = $db->query($query);
while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
    $row1[]= array('label' => $row["LABEL"],
                    'value' => $row["VALUE"]);
...
$objForm->ae( array("type"       => "select",
                    ...
                    "size"       => "1",
                    "value"      => $model->field["GRADE_HR_CLASS"],
                 	"extrahtml"  => "onchange=\"return btn_submit('knja131b');\"",
                    "options"    => isset($row1)?$row1:array()));
...
View::toHTML($model, "knja131bForm1.html", $arg); 
}
}
?>
