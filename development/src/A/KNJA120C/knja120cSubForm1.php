...
class knja120cSubForm1
{
    function main(&$model)
    {
        ...
        $arg = array();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("subform1", "POST", "knja120cindex.php", "", "subform1");
        ...
        if ($model->Properties["useTuutisyoSyokenNendo"] == "1") {
            //年度コンボ（通知表所見）
            $opt_year = array();
            $result = $db->query(knja120CQuery::selectQueryYear($model));
        ...
        $query = knja120cQuery::selectQueryGuide($model);
        ...
        View::toHTML($model, "knja120cSubForm1.html", $arg);
    }
}
?>
