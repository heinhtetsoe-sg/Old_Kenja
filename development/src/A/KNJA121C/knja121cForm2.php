...
class knja121cForm2
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("form2", "POST", "knja121cindex.php", "", "form2");
        ...
        //記録の取得
        $Row = $row = array();
        $result = $db->query(knja121cQuery::getBehavior($model));
        ...
        if ($model->Properties["sanshouReport"] == "1") {
            ...
            //行動の記録項目名取得
            $result = $db->query(knja121cQuery::getNameMst($model, "D035"));
            $rfActItem = array();
            ...
            for($h=1; $h<11; $h++) {
                $rcd = sprintf("%02d", $h);
                for($i=1; $i<=$model->control["学期数"]; $i++) {
                    $result = $db->query(knja121cQuery::getBehaviorSemesDat($model, $i, $rcd));
                    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            //特別活動の記録項目名取得
            $result = $db->query(knja121cQuery::getNameMst($model, "D034"));
            $rfSpeItem = array();
            while ($rowItem = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            for($h=1; $h<6; $h++) {
                $rcd = sprintf("%02d", $h);
                for($i=1; $i<=$model->control["学期数"]; $i++) {
                    $result = $db->query(knja121cQuery::getHreportremarkDetailDat($model, $i, "01", $rcd));
                    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            for($h=1; $h<3; $h++) {
                $rcd = sprintf("%02d", $h);
                $result = $db->query(knja121cQuery::getHreportremarkDetailDat($model, "9", "02", $rcd));
                while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $extra = "onclick=\"loadwindow('" .REQUESTROOT ."/A/KNJA121C_SHOKEN/knja121c_shokenindex.php?cmd=edit&SCHREGNO={$model->schregno}&frame='+getFrameName(self), event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 700, 350)\"";
        ...
        View::toHTML($model, "knja121cForm2.html", $arg);
    }
}
?>