...
class knja121cSubMaster
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("submaster", "POST", "knja121cindex.php", "", "submaster");

        //DB接続
        $db = Query::dbCheckOut();
        
        //学年取得
        $getGradeName = $db->getOne(knja121cQuery::getGradeName($model));
        ...
        View::toHTML($model, "knja121cSubMaster.html", $arg); 
    }
}
...
function makeList(&$objForm, &$arg, $db, $model) {
    $i = 0;
    $query = knja121cQuery::getHtrainRemarkTempDat($model);
