<?php
class knja123ooForm1 {
    function main(&$model) {

        $objForm = new form;
        $arg["start"] = $objForm->get_start("edit", "POST", "knja123ooindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //HTRAINREMARK_DAT 取得
        $query = knja123ooQuery::getTrainRow($model);
        $tRow = $db->getRow($query, DB_FETCHMODE_ASSOC);

        //学習記録データ
        $query = knja123ooQuery::getStudyRec($model);
        ...
        //出欠記録データ
        $attend = $db->getRow(knja123ooQuery::getAttendRec($model), DB_FETCHMODE_ASSOC);
        ...
        //署名データ取出し
        $query = knja123ooQuery::getOpinionsSignature($model);
        $opinionsSignature = $db->getRow($query, DB_FETCHMODE_ASSOC);

        //印影登録有無
        $query = knja123ooQuery::getInkanUmu();
        ...
        //印影の表示
        $query = knja123ooQuery::getIneiFlg($model);
        ...
        if ($model->Properties["seitoSidoYoroku_dat_TotalstudyactSize"]) {
            ...
            $arg["data"]["TOTALSTUDYACT"] = KnjCreateTextArea($objForm, "TOTALSTUDYACT", $model->totalstudyact_gyou, ($model->totalstudyact_moji * 2 + 1), "soft", $extra, $tRow["TOTALSTUDYACT"]);
        ...
        if ($model->Properties["seitoSidoYoroku_dat_TotalstudyvalSize"]) {
            ...
            $extra = "style=\"height:{$height}px;\" ";
            $arg["data"]["TOTALSTUDYVAL"] = KnjCreateTextArea($objForm, "TOTALSTUDYVAL", $model->totalstudyval_gyou, ($model->totalstudyval_moji * 2 + 1), "soft", $extra, $tRow["TOTALSTUDYVAL"]);
        ...
        View::toHTML($model, "knja123ooForm1.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg, $ineiFlg, $model)
{
    ...
    if ($model->exe_type == "PRINCIPAL") {
        ...
        $extra = "onclick=\"loadwindow('knja123ooindex.php?cmd=form2', 0, 0, 460, 400)\"";
