...
class syukketsuKirokuSansyo {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("syukketsu", "POST", "knja120index.php", "", "syukketsu");
        ...
        if ($db->getOne(knja120Query::checkSchoolMst()) > 0) {
            $schoolcd    = sprintf("%012d", SCHOOLCD);
            $school_kind = $db->getOne(knja120Query::getSchoolKind($model));
        }
        ...
        //出欠の記録
        $sick = $late = $early = 0;
        $result = $db->query(knja120Query::getAttendSemesDat($model, $knjSchoolMst));
