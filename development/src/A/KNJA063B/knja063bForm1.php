...
class knja063bForm1
{
    function main(&$model)
    {
        ...
        $arg["start"]   = $objForm->get_start("main", "POST", "knja063bindex.php", "", "main");

        //データベース接続
        $db = Query::dbCheckOut();

        //年度・学期コンボ
        $query = knja063bQuery::getTerm($model);
        $extra = "onchange=\"btn_submit('main');\"";
        makeCmb($objForm, $arg, $db, $query, $model->term, "TERM", $extra, 1);
        $arg["hr_class"] = 1;

        //左クラスコンボ
        $query = knja063bQuery::getGhrCd($model);
        $extra = "onchange=\"btn_submit('main');\"";
        makeCmb($objForm, $arg, $db, $query, $model->ghr_cd, "GHR_CD", $extra, 1);

        //右クラスコンボ
        $query = knja063bQuery::getHrClass($model);
        ...
        //生徒の登録チェック
        $model->getGhrStudentCnt = $db->getOne(knja063bQuery::getLcStudents($model, "COUNT"));
        ...
        View::toHTML($model, "knja063bForm1.html", $arg); 
    }
}
//コンボ作成
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->cmd == 'change_hr_class' ) {
        ...
        $query = knja063bQuery::getLcStudents($model);
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    $cnt = 0;
    $opt_right = array();
    $query = knja063bQuery::getHrStudents($model);
    $result = $db->query($query);
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeBtn(&$objForm, &$arg, $model) {
    ...
    if ($model->getGhrStudentCnt == 0) {
        ...
        $subdata  = "wopen('".REQUESTROOT."/A/KNJA063BS1/knja063bs1index.php?cmd=&SEND_AUTH={$model->auth}";
        $subdata .= "&SEND_PRGID=KNJA063B&SEND_YEAR={$model->year}&SEND_SEMESTER={$model->semester}&SEND_GHR_CD={$model->ghr_cd}";
        $subdata .= "&SUBWIN=SUBWIN2','SUBWIN2',0,0,screen.availWidth,screen.availHeight);";

        if (!$model->prgid) {
            $extra = "onclick=\"if (openCheck2()) {closecheck(); $subdata}\"";
        } else {
            $extra = "onclick=\"if (openCheck2()) {$subdata}\"";
        }
    }
