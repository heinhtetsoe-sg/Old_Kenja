...
class knjp720Ikkatsu {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("Ikkatsu", "POST", "knjp720index.php", "", "Ikkatsu");
        ...
        //校種表示
        $arg["data"]["SCHOOL_KIND"] = $db->getOne(knjp720Query::getSchKind($model, $model->schoolkind));

        //交付種別表示
        $arg["data"]["REDUCTION_DIV_CD"] = $db->getOne(knjp720Query::getReductionDivMst($model, $model->schoolkind, $model->reduction_div_cd));

        //開始日付
        $arg["data"]["S_YEAR_MONTH"] = str_replace("\n", "", $my->MyMonthWin2($objForm, "S_YEAR_MONTH", $model->field["S_YEAR_MONTH"]));

        //終了日付
        $arg["data"]["E_YEAR_MONTH"] = str_replace("\n", "", $my->MyMonthWin2($objForm, "E_YEAR_MONTH", $model->field["E_YEAR_MONTH"]));
        ...
        knjCreateHidden($objForm, "REDUCTION_DIV_CD", $model->reduction_div_cd);
        ...
        if ((VARS::post("cmd") == "Ikkatsu" || $model->cmd == "Ikkatsu2") && !isset($model->warning)) {
            $arg["reload"] = "window.open('knjp720index.php?cmd=list&SCHOOL_KIND=".$model->schoolkind."&REDUCTION_DIV_CD=".$model->reduction_div_cd."&GRADE=".$model->grade."&GRADE_HR_CLASS=".$model->grade_hr_class."&CLUBCD=".$model->clubcd."&RIGHT_FRAME=Ikkatsu','left_frame');";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjp720Ikkatsu.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, &$model) {
    ...
    $query = knjp720Query::getSchList($model);
