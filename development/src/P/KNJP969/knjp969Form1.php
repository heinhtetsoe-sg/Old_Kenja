...
class knjp969Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjp969Form1", "POST", "knjp969index.php", "", "knjp969Form1");
        ...
        //学期コンボ
        $query = knjp969Query::getSemester();
        $extra = "onchange=\"return btn_submit('knjp969'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //出力選択ラジオボタン 1:学年 2:クラス
        $opt_output = array(1, 2);
        $model->field["OUTPUT"] = ($model->field["OUTPUT"] == "") ? "1" : $model->field["OUTPUT"];
        $extra = array("id=\"OUTPUT1\" onclick =\" return btn_submit('knjp969');\"", "id=\"OUTPUT2\" onclick =\" return btn_submit('knjp969');\"");
        $radioArray = knjCreateRadio($objForm, "OUTPUT", $model->field["OUTPUT"], $extra, $opt_output, get_count($opt_output));
        foreach($radioArray as $key => $val) $arg["data"][$key] = $val;

        //学年コンボ
        $query = knjp969Query::getGrade();
        $extra = ($model->field["OUTPUT"] == "2") ? "onchange=\"return btn_submit('knjp969'), AllClearList();\"" : "";
        ...
        View::toHTML($model, "knjp969Form1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    $opt = array();
    $query = knjp969Query::getHrClass($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $opt[] = array('label' => $row["LABEL"],
                       'value' => $row["VALUE"]);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJP969");
