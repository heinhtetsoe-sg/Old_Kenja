...
function btn_submit(cmd) {
    ...
    if (cmd == 'update') {
        ...
        if (document.forms[0].TO_LM_CD.value == "") {
            alert("振替端数項目指定を指定してください。");
            return;
        }

        hasuu = document.forms[0].selectHasuu;
        hasuu.value = "";
        sep = "";
        for (var i = 0; i < document.forms[0].category_selected.length; i++) {
            hasuu.value = hasuu.value + sep + document.forms[0].category_selected.options[i].value;
            sep = ",";
        }
    }
