...
class knjp706bForm2 
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjp706bindex.php", "", "edit");
        ...
        if (!$model->isWarning()) {
            $query = knjp706bQuery::getPaidData($model);
        ...
        //徴収変更
        $query = knjp706bQuery::getNamecd($model->year, "G214");
        ...
        //入金区分
        $query = knjp706bQuery::getNamecd($model->year, "G205");
        $extra = "$paidDisabled";
        ...
        //返金データ選択
        $query = knjp706bQuery::getRepaySeq($model);
        ...
        //返金額
        $query = knjp706bQuery::getMRepay($model);
        ...
        //返金区分
        $query = knjp706bQuery::getNamecd($model->year, "G209");
        $extra = "$repayDisabled";
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "parent.top_frame.location.href='knjp706bindex.php?cmd=list';";
        }

        View::toHTML($model, "knjp706bForm2.html", $arg);
    }
}
//makeCmb
