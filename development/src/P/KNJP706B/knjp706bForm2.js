function btn_submit(cmd) {
    ...
    //項目更新
    if (cmd == "dueUpdate") {
        if (document.forms[0].TMP_S_EXIST_FLG.value == "1") {
            if (document.forms[0].PAY_DATE.value == "") {
                alert('納入期限が未入力です。');
                return false;
            }
        } else {
            if ((document.forms[0].MONEY_DUE.value != "" && document.forms[0].PAY_DATE.value == "")
                || (document.forms[0].MONEY_DUE.value == "" && document.forms[0].PAY_DATE.value != "")
            ) {
                alert('入金必要額と納入期限が未入力です。');
                return false;
            }
        }
    }
