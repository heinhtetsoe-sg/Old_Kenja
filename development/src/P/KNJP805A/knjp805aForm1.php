...
class knjp805aForm1
{
    ...
    {
        $objForm      = new form();
        $arg["start"] = $objForm->get_start("edit", "POST", "knjp805aindex.php", "", "edit");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        if (!isset($model->warning) && ($model->cmd == 'read' || $model->cmd == 'readTest')) {
            $model->printSchregno = $comma = "";
            foreach ($model->arr_schregData as $key => $val) {
                if ($model->field["GO_PRINT:".$val]) {
                    $model->printSchregno .= $comma.$model->field["GO_PRINT:".$val];
                    $comma = ",";
                }
            }
        }
        $arg["TOP"]["PRINT_SCHREGNO"]  = knjCreateHidden($objForm, "PRINT_SCHREGNO", $model->printSchregno);
        ...
        //文面コンボ
        $opt = array();
        $query = knjp805aQuery::getDocumentMst();
        ...
        $query = knjp805aQuery::getDocumentMst();
        $result = $db->query($query);
        $docOpt[] = array('label' => "", 'value' => "");
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $docOpt[] = array('label' => $row["LABEL"], 'value' => $row["VALUE"]);
        }
        ...
        //生徒データ表示
        $query = knjp805aQuery::getStudentInfoData($model);
        $result = $db->query($query);
        if ($model->field["GRADE"] != '') {
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {

                //更新用
                $model->arr_schregData[] = $row["SCHREGNO"];
                //IDセット
                $setId = $row["SCHREGNO"];

                //印刷チェックボックス
                $row["GO_PRINT"] = knjCreateCheckBox($objForm, "GO_PRINT:".$setId, $row["SCHREGNO"], " class=\"printCheck\" ");
                ...
                //文面コンボ
                $extra = "";
                $row["DOCUMENTCD"] = knjCreateCombo($objForm, "DOCUMENTCD-".$setId, $model->field["DOCUMENTCD-".$setId], $docOpt, $extra, 1);
        ...
        $arg["TOP"]["PRGID"]           = knjCreateHidden($objForm, "PRGID", "KNJP805A");
        ...
        View::toHTML5($model, "knjp805aForm1.html", $arg);
    }
}
//makeCmb
