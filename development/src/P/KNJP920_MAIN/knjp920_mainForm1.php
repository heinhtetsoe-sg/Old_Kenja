...
class knjp920_mainForm1 {
    function main(&$model) {
        //オブジェクト作成
        $objForm        = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjp920_mainindex.php", "", "main");
        
        if (!isset($model->warning) && $model->getAuth != "" && $model->cmd !== 'edit') {
            $Row = knjp920_mainQuery::getRow($model);
        } else {
            $Row =& $model->field;
        }
        ...
        $model->getHonjimeCount = $db->getOne(knjp920_mainQuery::getCloseFlgData($model));

        //支出伺の状況を取得(比較時に利用)
        if ($model->getRequestNo) {
            $model->getApproval = $db->getOne(knjp920_mainQuery::getOutgoData($model, "APPROVAL"));
            $model->getCancel   = $db->getOne(knjp920_mainQuery::getOutgoData($model, "CANCEL"));
            //支出伺画面から遷移してきていない場合
            if(!$model->getOutgoRequestNo) {
                $model->getOutgoRequestNo = $db->getOne(knjp920_mainQuery::getOutgoData($model, "REQUEST_NO"));
            }
        } else {
            $model->getApproval = "";
            $model->getCancel   = "";
        ...
        $query = knjp920_mainQuery::getLevyMDiv($model);
        ...
        //起案者
        $extra = "";
        $query = knjp920_mainQuery::getRequestStaff($model);
        ...
        for ($i = 1;$i <= 4; $i++) {
            $extra = "";
            $query = knjp920_mainQuery::getTraderMst($model);
        ...
        View::toHTML($model, "knjp920_mainForm1.html", $arg); 
    }
}
//コンボ作成
...
function makeBtn(&$objForm, &$arg, $model, $Row) {
    ...
    for ($i = 1; $i <= $model->koumoku; $i++) {
        if ($model->getRequestNo == "") {
            ...
            $subdata  = "wopen('".REQUESTROOT."/P/KNJP920_MEISAI/knjp920_meisaiindex.php?cmd=main&SEND_AUTH={$model->auth}";
            $subdata .= "&SEND_PRGID=KNJP920_MAIN&SEND_KOUNYU_L_CD={$Row["KOUNYU_L_CD"]}&SEND_KOUNYU_M_CD={$Row["KOUNYU_M_CD"]}";
    ...
    if ($model->getRequestNo == "") {
        ...
        $subdata .= "&SEND_PRGID=KNJP920_MAIN&SEND_OUTGO_L_CD={$Row["KOUNYU_L_CD"]}&SEND_OUTGO_M_CD={$Row["KOUNYU_M_CD"]}";
        $subdata .= "&SEND_SCHOOL_KIND={$Row["SCHOOL_KIND"]}&SEND_REQUEST_NO={$model->getOutgoRequestNo}&SEND_YEAR={$Row["YEAR"]}";
