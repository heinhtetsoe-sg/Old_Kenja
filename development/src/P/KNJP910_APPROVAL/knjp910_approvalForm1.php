...
class knjp910_approvalForm1 {
    function main(&$model) {
        //オブジェクト作成
        $objForm        = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjp910_approvalindex.php", "", "main");
        
        if (!isset($model->warning) && $model->getAuth != "" && $model->cmd != "check") {
            $Row = knjp910_approvalQuery::getRow($model);
        } else {
            $Row =& $model->field;
        }
        ...
        $model->getHonjimeCount = $db->getOne(knjp910_approvalQuery::getCloseFlgData($model));

        //決裁の状況を取得(比較時に利用)
        if ($model->getOutgoLMcd) {
            $model->getHenkinApproval = $db->getOne(knjp910_approvalQuery::getLevyData($model, "HENKIN_APPROVAL"));
            $model->getApproval = $db->getOne(knjp910_approvalQuery::getLevyData($model, "APPROVAL"));
            $model->getCancel   = $db->getOne(knjp910_approvalQuery::getLevyData($model, "CANCEL"));
        } else {
        ...
        //支出項目
        $query = knjp910_approvalQuery::getLevyMDiv($model);
        $arg["data"]["LEVY_M_NAME"] = $db->getOne($query);

        //支出金額
        $query = knjp910_approvalQuery::getRequestGk($model);
        ...
        //受取人氏名
        $extra = "";
        $query = knjp910_approvalQuery::getOutgoStaff($model);
        ...
        View::toHTML($model, "knjp910_approvalForm1.html", $arg); 
    }
}
//コンボ作成
...
function makeBtn(&$objForm, &$arg, $model) {
    ...
    $subdata .= "&SEND_PRGID=KNJP910_APPROVAL&SEND_OUTGO_L_CD={$model->getOutgoLcd}&SEND_OUTGO_M_CD={$model->getOutgoMcd}";
