...
class knjp910Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjp910Form1", "POST", "knjp910index.php", "", "knjp910Form1");
        ...
        //新規ボタン
        $subdata  = "wopen('".REQUESTROOT."/P/KNJP910_MAIN/knjp910_mainindex.php?cmd=main&&SEND_AUTH=".AUTHORITY."&SEND_SCHOOL_KIND=".$model->schoolKind."&SEND_YEAR={$model->field["YEAR"]}&SUBWIN=SUBWIN2','SUBWIN2',0,0,screen.availWidth,screen.availHeight);";
        ...
        //検索結果
        $query = knjp910Query::selectQuery($db, $model);
        ...
        if($searchCnt > $checkCnt) {
            ...
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                if ($model->cmd == "search") {
                    ...
                    $subdata = "wopen('".REQUESTROOT."/P/KNJP910_MAIN/knjp910_mainindex.php?&cmd=main&SEND_SCHOOL_KIND=".$row["SCHOOL_KIND"]."&SEND_YEAR=".$row["YEAR"]."&SEND_OUTGO_L_CD=".$row["OUTGO_L_CD"]."&SEND_OUTGO_M_CD=".$row["OUTGO_M_CD"]."&SEND_OUTGO_L_M_CD=".$row["OUTGO_L_CD"].$row["OUTGO_M_CD"]."&SEND_REQUEST_NO=".$row["REQUEST_NO"]."&SEND_AUTH=".AUTHORITY.$extra;
                    $row["REQUEST_NO"] = View::alink("#", htmlspecialchars($row["REQUEST_NO"]),"onclick=\"$subdata\"");
                    
                    $row["OUTGO_L_NAME"] = $db->getOne(knjp910Query::getLevyLDiv($model, $row["OUTGO_L_CD"]));
                    $row["OUTGO_M_NAME"] = $db->getOne(knjp910Query::getLevyMDiv($model, $row["OUTGO_L_CD"], $row["OUTGO_M_CD"]));
        ...
        View::toHTML($model, "knjp910Form1.html", $arg);
    }
}
//コンボ作成
