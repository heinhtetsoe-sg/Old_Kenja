...
class knjp900_mainForm1 {
    function main(&$model) {
        //オブジェクト作成
        $objForm        = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjp900_mainindex.php", "", "main");

        if (!isset($model->warning) && $model->getAuth != "" && $model->cmd !== 'edit') {
            $Row = knjp900_mainQuery::getRow($model);
        } else {
            $Row =& $model->field;
        }
        ...
        $model->getHonjimeCount = $db->getOne(knjp900_mainQuery::getCloseFlgData($model));

        //入金項目と決済の状況を取得(比較時に利用)
        if ($model->getIncomeLMcd) {
            $model->getApproval = $db->getOne(knjp900_mainQuery::getLevyData($model, "APPROVAL"));
            $model->getCancel   = $db->getOne(knjp900_mainQuery::getLevyData($model, "CANCEL"));
        } else {
            $model->getApproval = "";
            $model->getCancel   = "";
        }
        ...
        //起案者
        $query = knjp900_mainQuery::getRequestStaff($model, $Row["REQUEST_STAFF"]);
        ...
        //収入金額
        $arg["data"]["REQUEST_GK"] = $Row["REQUEST_GK"];
        ...
        View::toHTML($model, "knjp900_mainForm1.html", $arg); 
    }
}
//コンボ作成
...
function makeBtn(&$objForm, &$arg, $model, $Row) {
    ...
    for ($i = 1; $i <= $model->koumoku; $i++) {
        if ($model->getIncomeLMcd == "") {
            ...
            $subdata  = "wopen('".REQUESTROOT."/P/KNJP900_SCHREG/knjp900_schregindex.php?cmd=main&SEND_AUTH={$model->auth}";
            $subdata .= "&SEND_PRGID=KNJP900_MAIN&SEND_INCOME_L_CD={$Row["INCOME_L_CD"]}&SEND_INCOME_M_CD={$Row["INCOME_M_CD"]}";
    ...
    if ($model->sumTotalPrice != $Row["REQUEST_GK"]) {
        ...
        $subdata  = "wopen('".REQUESTROOT."/P/KNJP900_APPROVAL/knjp900_approvalindex.php?cmd=main&SEND_AUTH={$model->auth}";
        $subdata .= "&SEND_PRGID=KNJP900_MAIN&SEND_INCOME_L_CD={$Row["INCOME_L_CD"]}&SEND_INCOME_M_CD={$Row["INCOME_M_CD"]}";
...
function makeHidden(&$objForm, $model, $Row) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJP951");
