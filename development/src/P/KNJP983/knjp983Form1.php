...
class knjp983Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("sel", "POST", "knjp983index.php", "", "sel");
        ...
        $flg = "";
        $opt = array();
        $result = $db->query(knjp983Query::selectYearQuery());
        ...
        //業者年度一覧取得
        $opt_left = array();
        $result = $db->query(knjp983Query::selectQuery($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //業者一覧取得
        $opt_right = array();
        $result = $db->query(knjp983Query::selectTraderQuery($model));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        /****************/
        //extra
        $link = REQUESTROOT."/P/KNJP983_2/knjp983_2index.php?mode=1";
