...
function checkRenban(obj, div) {
    var rennban = obj.value;
    if (rennban.length == 0) {
        alert('連番を指定して下さい。');
        if (div == '1') {
            obj.value = document.forms[0].MAX_RENBAN1.value;
        } else if (div == '2') {
            obj.value = document.forms[0].MAX_RENBAN2.value;
        }
        obj.focus();
        return false;
    }
    if (div == '1') {
        if (parseInt(rennban) < parseInt(document.forms[0].MAX_RENBAN1.value)) {
            alert('連番は、' + document.forms[0].MAX_RENBAN1.value + '以上を指定して下さい。');
            obj.focus();
            return false;
        }
    } else if (div == '2') {
        if (parseInt(rennban) < parseInt(document.forms[0].MAX_RENBAN2.value)) {
            alert('連番は、' + document.forms[0].MAX_RENBAN2.value + '以上を指定して下さい。');
            obj.focus();
            return false;
        }
    }
