...
class knjp716Form2
{
    function main(&$model)
    {
        ...
        $arg["start"] = $objForm->get_start("edit", "POST", "knjp716index.php", "", "edit");
        ...
        $opt = array();
        $value_flg = false;
        $query = knjp716Query::get_Grade($model);
        ...
        if (isset($model->warning) || $model->cmd == "reload" || $model->cmd == "change2") {
            ...
            $query = knjp716Query::getRow($model, 1);
            $Row = $db->getRow($query, DB_FETCHMODE_ASSOC);
        }

        //入金済みデータの件数
        $paid_money  = $db->getOne(knjp716Query::getPaidMoney($model->taisyouYear, "COLLECT_MONEY_PAID_M_DAT", $model->exp_grpcd));
        $paid_money += $db->getOne(knjp716Query::getPaidMoney($model->taisyouYear, "COLLECT_MONEY_PAID_S_DAT", $model->exp_grpcd));
        ...
        if (isset($model->warning) || $model->cmd == "reload") {
            ...
            $result = $db->query(knjp716Query::ReloadSelectClass($model));
        } else {
            $result = $db->query(knjp716Query::GetSelectClass($model));
        }
        ...
        //クラス一覧取得
        $result = $db->query(knjp716Query::GetClass($model, $tempcd));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $label = $row["GRADE"].$row["HR_CLASS"]."：".htmlspecialchars($row["HR_NAME"]);
        ...
        if (VARS::get("cmd") != "edit") {
            $arg["jscript"] = "window.open('knjp716index.php?cmd=list','left_frame');";
        }

        $arg["finish"]  = $objForm->get_finish();

        View::toHTML($model, "knjp716Form2.html", $arg);
    }
