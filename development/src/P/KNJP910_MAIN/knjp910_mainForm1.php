...
class knjp910_mainForm1 {
    function main(&$model) {
        //オブジェクト作成
        $objForm        = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjp910_mainindex.php", "", "main");
        
        if (!isset($model->warning) && $model->getAuth != "" && $model->cmd !== 'edit' && $model->cmd !== 'torikomi') {
            $Row = knjp910_mainQuery::getRow($model);
        } else {
            $Row =& $model->field;
        }
        ...
        $model->getHonjimeCount = $db->getOne(knjp910_mainQuery::getCloseFlgData($model));

        //返金チェックと入金科目と決済の状況を取得(比較時に利用)
        if ($model->getRequestNo) {
            $model->getHenkinFlg = $db->getOne(knjp910_mainQuery::getLevyData($model, "HENKIN_FLG"));
            $model->getHenkinApproval = $db->getOne(knjp910_mainQuery::getLevyData($model, "HENKIN_APPROVAL"));
            $model->getCollectLMSGrpcd = $db->getOne(knjp910_mainQuery::getLevyData($model, "COLLECT"));
            $model->getIncomeLcd = $db->getOne(knjp910_mainQuery::getLevyData($model, "INCOME_L_CD"));
            $model->getApproval  = $db->getOne(knjp910_mainQuery::getLevyData($model, "APPROVAL"));
            $model->getCancel    = $db->getOne(knjp910_mainQuery::getLevyData($model, "CANCEL"));
        } else {
            $model->getHenkinFlg = "";
        ...
        $model->getZanGk = "";
        if ($model->getSeisanRequestNo) {
            $model->getSiharaiGk = $db->getOne(knjp910_mainQuery::getSeisanGk($model, "SIHARAI_GK"));
            $model->getZanGk = $db->getOne(knjp910_mainQuery::getSeisanGk($model, "ZAN_GK"));
            if ($Row["REQUEST_GK"] == "") {
        ...
        if ($Row["HENKIN_FLG"] == "1") {
            ...
            $query = knjp910_mainQuery::getCollectSDiv($model);
        ...
        //収入科目
        $extra = "onchange=\"return btn_submit('edit');\"";
        $query = knjp910_mainQuery::getIncomeLDiv($model);
        makeCombo($objForm, $arg, $db, $query, $Row["INCOME_L_CD"], "INCOME_L_CD", $extra, 1, "BLANK", $model);

        //収入残高を表示
        //収入高
        $getIncomeRequestGk = $db->getOne(knjp910_mainQuery::getIncomeSumRequestGk($model, $Row["INCOME_L_CD"]));

        //支出高(入力伝票以外)
        $getOutgoRequestGk = $db->getOne(knjp910_mainQuery::getOutGoSumRequestGk($model, $Row));
        ...
        //支出項目
        $extra = "onchange=\"return btn_submit('edit');\"";
        $query = knjp910_mainQuery::getLevyMDiv($model);
        ...
        //収入伺い日までの収入残高を表示
        //収入高
        $getIncomeRequestGk = $db->getOne(knjp910_mainQuery::getIncomeSumRequestGk($model, $Row["INCOME_L_CD"], $Row["REQUEST_DATE"]));

        //支出高(入力伝票以外)
        $getOutgoRequestGk = $db->getOne(knjp910_mainQuery::getOutGoSumRequestGk($model, $Row, $Row["REQUEST_DATE"]));
        ...
        //起案者
        $extra = "";
        $query = knjp910_mainQuery::getRequestStaff($model);
        ...
        for ($i = 1; $i <= $model->koumoku; $i++) {
            ...
            if ($model->cmd !== 'edit' && $model->cmd !== 'torikomi') {
                ...
                //単価(税込)
                if ($Row["COMMODITY_CNT".$i] > 0) {
                    $arg["data"]["COMMODITY_PRICE".$i] = $Row["COMMODITY_PRICE".$i] + ($Row["TOTAL_TAX".$i] / $Row["COMMODITY_CNT".$i]);
                }
                //数量
                $arg["data"]["COMMODITY_CNT".$i] = $Row["COMMODITY_CNT".$i];
                //金額(税込)
                $arg["data"]["TOTAL_PRICE_ZEINUKI".$i] = $Row["TOTAL_PRICE".$i];
                //備考
            ...
            //サブミット表示用
                ...
                //単価
                if ($model->setRow["COMMODITY_CNT".$i] > 0) {
                    $arg["data"]["COMMODITY_PRICE".$i] = $model->setRow["COMMODITY_PRICE".$i] + ($model->setRow["TOTAL_TAX".$i] / $model->setRow["COMMODITY_CNT".$i]);
                }
                //数量
                $arg["data"]["COMMODITY_CNT".$i] = $model->setRow["COMMODITY_CNT".$i];
                //金額(税抜き)
                $arg["data"]["TOTAL_PRICE_ZEINUKI".$i] = $model->setRow["TOTAL_PRICE".$i];
                //備考
        ...
        //業者コード
        $extra = "";
        $query = knjp910_mainQuery::getTraderCd($model);
        makeCombo($objForm, $arg, $db, $query, $Row["TRADER_CD"], "TRADER_CD", $extra, 1, "BLANK", $model);
        if ($model->cmd === 'torikomi') {
            $traderRow = array();
            $traderRow = $db->getRow(knjp910_mainQuery::getTraderData($model), DB_FETCHMODE_ASSOC);
        ...
        //支払方法
        $extra = "";
        $query = knjp910_mainQuery::getNameMst("G217");
        ...
        View::toHTML($model, "knjp910_mainForm1.html", $arg); 
    }
}
//コンボ作成
...
function makeBtn(&$objForm, &$arg, $model, $Row) {
    ...
    for ($i = 1; $i <= $model->koumoku; $i++) {
        if ($model->getRequestNo == "") {
            ...
            $subdata  = "wopen('".REQUESTROOT."/P/KNJP910_SCHREG/knjp910_schregindex.php?cmd=main&SEND_AUTH={$model->auth}";
            $subdata .= "&SEND_PRGID=KNJP910_MAIN&SEND_OUTGO_L_CD={$Row["OUTGO_L_CD"]}&SEND_OUTGO_M_CD={$Row["OUTGO_M_CD"]}";
    ...
    if ($model->getRequestNo == "") {
        ...
        $subdata  = "wopen('".REQUESTROOT."/P/KNJP910_APPROVAL/knjp910_approvalindex.php?cmd=main&SEND_AUTH={$model->auth}";
        $subdata .= "&SEND_PRGID=KNJP910_MAIN&SEND_OUTGO_L_CD={$Row["OUTGO_L_CD"]}&SEND_OUTGO_M_CD={$Row["OUTGO_M_CD"]}";
    ...
    if ($model->getKounyuRequestNo != "" || $Row["KOUNYU_NO"] != "") {
        ...
        $subdata .= "&SEND_PRGID=KNJP910_MAIN&SEND_KOUNYU_L_CD={$model->getOutgoLcd}&SEND_KOUNYU_M_CD={$model->getOutgoMcd}";
    ...
    if ($model->getSekouRequestNo != "" || $Row["SEKOU_NO"] != "") {
        ...
        $subdata .= "&SEND_PRGID=KNJP910_MAIN&SEND_SEKOU_L_CD={$model->getOutgoLcd}&SEND_SEKOU_M_CD={$model->getOutgoMcd}";
    ...
    if ($model->getSeisanRequestNo != "" || $Row["SEISAN_NO"] != "") {
        ...
        $subdata .= "&SEND_PRGID=KNJP910_MAIN&SEND_SEISAN_L_CD={$model->getOutgoLcd}&SEND_SEISAN_M_CD={$model->getOutgoMcd}";
