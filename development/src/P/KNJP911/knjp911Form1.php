...
class knjp911Form1 {
    function main(&$model) {
        ...
        $objForm        = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjp911index.php", "", "main");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //本締めデータチェック
        $model->getCount = "";
        $model->getCount = $db->getOne(knjp911Query::getCloseFlgData());
        if ($model->getCount > 0) {
            $arg["COMENT"] = '(本締め 実行済み)';
            $disabled = "disabled";
        }
        ...
        View::toHTML($model, "knjp911Form1.html", $arg); 
    }
}
//ボタン作成
