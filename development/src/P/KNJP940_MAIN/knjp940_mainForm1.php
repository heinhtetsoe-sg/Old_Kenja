...
class knjp940_mainForm1 {
    function main(&$model) {
        //オブジェクト作成
        $objForm        = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjp940_mainindex.php", "", "main");
        
        if (!isset($model->warning) && $model->getAuth != "" && $model->cmd !== 'edit') {
            $Row = knjp940_mainQuery::getRow($model);
        } else {
            $Row =& $model->field;
        }
        ...
        $model->getHonjimeCount = $db->getOne(knjp940_mainQuery::getCloseFlgData($model));

        //支出伺の決済の状況を取得(比較時に利用)
        if ($model->getRequestNo) {
            $model->getApproval = $db->getOne(knjp940_mainQuery::getOutgoData($model, "APPROVAL"));
            $model->getCancel   = $db->getOne(knjp940_mainQuery::getOutgoData($model, "CANCEL"));
            //支出伺画面から遷移してきていない場合
            if(!$model->getOutgoRequestNo) {
                $model->getOutgoRequestNo = $db->getOne(knjp940_mainQuery::getOutgoData($model, "REQUEST_NO"));
            }
        } else {
            $model->getApproval = "";
            $model->getCancel   = "";
        ...
        $query = knjp940_mainQuery::getLevyMDiv($model);
        ...
        //精算者
        $extra = "";
        $query = knjp940_mainQuery::getRequestStaff($model);
        ...
        View::toHTML($model, "knjp940_mainForm1.html", $arg); 
    }
}
//コンボ作成
function makeCombo(&$objForm, &$arg, $db, $query, &$value, $name, $extra, $size, $blank = "", $model) {
    ...
    $result->free();
    if ($name == 'REQUEST_STAFF' || $name == 'GENKIN_JURYOU_STAFF' || $name == 'SUITOU_STAFF') {
...
function makeBtn(&$objForm, &$arg, $model, $Row) {
    ...
    if ($model->getRequestNo == "") {
        ...
        $subdata .= "&SEND_PRGID=KNJP940_MAIN&SEND_OUTGO_L_CD={$Row["SEISAN_L_CD"]}&SEND_OUTGO_M_CD={$Row["SEISAN_M_CD"]}";
...
function makeHidden(&$objForm, $model, $Row) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJP955");
