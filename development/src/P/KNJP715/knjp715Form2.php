...
class knjp715Form2 
{
    function main(&$model)
    {
        ...
        if (!$model->isWarning() && $model->cmd != "changeGroup" && $model->cmd != "updEdit") {
            $query = knjp715Query::getPaidData($model);
        ...
        //会計項目
        $query = knjp715Query::getMMst($model);
        $extra = "$dueDisabledSexe $dueDisabled onChange=\" btn_submit('changeGroup')\"";
        makeCmb($objForm, $arg, $db, $query, $Row["COLLECT_LM_CD"], "COLLECT_LM_CD", $extra, 1, "BLANK");

        //単価
        $query = knjp715Query::getMMst($model, $Row["COLLECT_LM_CD"]);
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "parent.top_frame.location.href='knjp715index.php?cmd=list';";
        }

        View::toHTML($model, "knjp715Form2.html", $arg);
    }
}
//makeCmb
