...
class knjp701Form2
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjp701index.php", "", "edit");
        ...
        if(!isset($model->warning)){
            if ($model->cmd === 'bank') {
                ...
                $Row = knjp701Query::getRow($model, $model->collect_l_cd);
            }
        } else {
            $Row =& $model->field;
        }
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "window.open('knjp701index.php?cmd=list','left_frame');";
        }
        
        $arg["finish"]  = $objForm->get_finish();
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。 
        View::toHTML($model, "knjp701Form2.html", $arg);
    }
}
//makeCmb
