...
class knjp984_2Form1
{
    function main(&$model)
    {
        ...
        $objForm = new form;
        //フォーム作成
        //$arg["start"]   = $objForm->get_start("sel", "POST", "knjp984_2index.php", "", "sel");
        $arg["start"] = $objForm->get_start("knjp984_2Form1", "POST", "knjp984_2index.php", "", "knjp984_2Form1");

        $db = Query::dbCheckOut();
        
        //年度取得
        $query = knjp984_2Query::selectYearQuery($model);
        ...
        //グループ設定
        $result = $db->query(knjp984_2Query::selectGroupQuery());   
        $opt = array();
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
            $opt[] = array("label" => $row["LEVY_GROUP_CD"]."  ".$row["LEVY_GROUP_NAME"], "value" => $row["LEVY_GROUP_CD"]);
        }
        //グループに所属する一覧取得
        $result = $db->query(knjp984_2Query::selectQuery($model));   
        ...
        if (is_array($opt_left_id)){
            $result = $db->query(knjp984_2Query::selectNoGroupQuery($opt_left_id,$model));   
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        //終了ボタンを作成する
        $link = REQUESTROOT."/P/KNJP984/knjp984index.php";
        ...
        View::toHTML($model, "knjp984_2Form1.html", $arg);
    }
}
?>
