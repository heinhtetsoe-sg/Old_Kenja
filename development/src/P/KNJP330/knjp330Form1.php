...
class knjp330Form1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjp330Form1", "POST", "knjp330index.php", "", "knjp330Form1");
        ...
        $extra = array("id=\"OUTPUT1\" onclick =\" return btn_submit('knjp330');\"", "id=\"OUTPUT2\" onclick =\" return btn_submit('knjp330');\"", "id=\"OUTPUT3\" onclick =\" return btn_submit('knjp330');\"");
        ...
        if ($model->output != 1) {
            //都道府県コンボ
            $query = knjp330Query::GetPrefecturescd("COMB");
            $extra = "onchange =\" return btn_submit('knjp330ken');\"";
            makeCombo($objForm, $arg, $db, $query, $model->todoufuken, "TODOUFUKEN", $extra, 1, "ALL");

            if ($model->output == 3){
                if ($model->cmd == "knjp330ken"){
                    $model->hrclass = "";
                    $model->cmd = "knjp330";
                }
                //クラスコンボ
                $query = knjp330Query::GetClass($model, "COMB");
                $extra = "onchange =\" return btn_submit('knjp330');\"";
                makeCombo($objForm, $arg, $db, $query, $model->hrclass, "HRCLASS", $extra, 1);
            }
        }

        //リストToリスト
        if ($model->output == 1) $query = knjp330Query::GetPrefecturescd("LIST");
        if ($model->output == 2) $query = knjp330Query::GetClass($model, "LIST");
        if ($model->output == 3) $query = knjp330Query::GetSchreg($model);
...
//        $extra = array("id=\"YOUSIKI1\"", "id=\"YOUSIKI2\"");
//        $radioArray = knjCreateRadio($objForm, "YOUSIKI", $model->field["YOUSIKI"], $extra, $opt, get_count($opt));
//        foreach($radioArray as $key => $val) $arg["data"][$key] = $val;
        ...
        View::toHTML($model, "knjp330Form1.html", $arg); 
    }
}
...
function makeHidden(&$objForm, &$arg)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJP330");
