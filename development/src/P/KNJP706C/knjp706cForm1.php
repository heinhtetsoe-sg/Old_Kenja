...
class knjp706cForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjp706cindex.php", "", "edit");

        //生徒の名前を取得
        $row = $db->getRow(knjp706cQuery::getStudentName($model),DB_FETCHMODE_ASSOC);
        $arg["NAME"] = $row["SCHREGNO"]."　".$row["NAME"];
        $risyuuCnt = $db->getOne(knjp706cQuery::getRisyuuCnt($model));
        $zanMusyouKaisu = $row["MUSYOU_KAISU"] < $risyuuCnt ? 0 : $row["MUSYOU_KAISU"] - $risyuuCnt;
        $arg["MUSYOU_KAISU"] = $zanMusyouKaisu;

        //リスト取得
        $result = $db->query(knjp706cQuery::getList($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            array_walk($row, "htmlspecialchars_array");
            $row["COLLECT_S_CD"] = View::alink("knjp706cindex.php", $row["COLLECT_GRP_CD"].":".$row["COLLECT_L_CD"].$row["COLLECT_M_CD"].$row["COLLECT_S_CD"], "target=bottom_frame",
            ...
            if (strlen($row["PAID_MONEY_DIV"])) {
                $row["PAID_MONEY_DIV"] = $db->getOne(knjp706cQuery::getName($model->year, "G205", $row["PAID_MONEY_DIV"]));
            }
        ...
        View::toHTML($model, "knjp706cForm1.html", $arg);
    }
}
?>
