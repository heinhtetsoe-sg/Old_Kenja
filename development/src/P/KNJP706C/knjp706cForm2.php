...
class knjp706cForm2 
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjp706cindex.php", "", "edit");
        ...
        if (!$model->isWarning()) {
            $query = knjp706cQuery::getPaidData($model);
        ...
        //入金区分
        $query = knjp706cQuery::getNamecd($model->year, "G205");
        $extra = "$paidDisabled";
        ...
        //返金データ選択
        $query = knjp706cQuery::getRepaySeq($model);
        ...
        //返金額
        $query = knjp706cQuery::getSRepay($model);
        ...
        //返金区分
        $query = knjp706cQuery::getNamecd($model->year, "G209");
        $extra = "$repayDisabled";
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "parent.top_frame.location.href='knjp706cindex.php?cmd=list';";
        }

        View::toHTML($model, "knjp706cForm2.html", $arg);
    }
}
//makeCmb
