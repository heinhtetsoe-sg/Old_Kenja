...
class knjp190kform2
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjp190kindex.php", "", "edit");

        //警告メッセージを表示しない場合
        if(isset($model->schregno) && isset($model->grantcd) && !isset($model->warning)){
            $Row = knjp190kQuery::getRow($model->grantcd,$model->schregno);
...
#        if ($temp_cd==$Row["SCHREGNO"] ) $cd_change = true;
        ...
        if (VARS::get("cmd") != "edit" && VARS::get("cmd") != "clear" && ($cd_change==true || $model->isload != 1)){
                $arg["reload"]  = "window.open('knjp190kindex.php?cmd=list&SCHREGNO=$model->schregno','right_frame');";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjp190kForm2.html", $arg);
    }
}        
