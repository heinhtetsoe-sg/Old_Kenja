...
class knjp972Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjp972Form1", "POST", "knjp972index.php", "", "knjp972Form1");
        ...
        //学期コンボ
        $query = knjp972Query::getSemester();
        $extra = "onchange=\"return btn_submit('knjp972');\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //学年コンボ
        $query = knjp972Query::getGrade();
        $extra = "onchange=\"return btn_submit('knjp972');\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        //校種
        $query = knjp972Query::getSchoolKind($model);
        $model->schoolKind = $db->getOne($query);

        //収入科目コンボ
        $query = knjp972Query::getIncomeLMst($model);
        $extra = "";
        ...
        View::toHTML($model, "knjp972Form1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    $opt = array();
    $query = knjp972Query::getHrClass($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $opt[] = array('label' => $row["LABEL"],
                       'value' => $row["VALUE"]);
...
function makeHidden(&$objForm, $model, &$arg) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJP972");
