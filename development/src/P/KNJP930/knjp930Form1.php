...
class knjp930Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjp930Form1", "POST", "knjp930index.php", "", "knjp930Form1");
        ...
        //新規ボタン
        $subdata  = "wopen('".REQUESTROOT."/P/KNJP930_MAIN/knjp930_mainindex.php?cmd=main&&SEND_AUTH=".AUTHORITY."&SEND_SCHOOL_KIND=".$model->schoolKind."&SEND_YEAR={$model->field["YEAR"]}&SUBWIN=SUBWIN2','SUBWIN2',0,0,screen.availWidth,screen.availHeight);";
        ...
        //検索結果
        $query = knjp930Query::selectQuery($db, $model);
        ...
        if($searchCnt > $checkCnt) {
            ...
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                if ($model->cmd == "search") {
                    ...
                    $subdata = "wopen('".REQUESTROOT."/P/KNJP930_MAIN/knjp930_mainindex.php?&cmd=main&SEND_SCHOOL_KIND=".$row["SCHOOL_KIND"]."&SEND_YEAR=".$row["YEAR"]."&SEND_SEKOU_L_CD=".$row["SEKOU_L_CD"]."&SEND_SEKOU_M_CD=".$row["SEKOU_M_CD"]."&SEND_SEKOU_L_M_CD=".$row["SEKOU_L_CD"].$row["SEKOU_M_CD"]."&SEND_REQUEST_NO=".$row["REQUEST_NO"]."&SEND_AUTH=".AUTHORITY.$extra;
                    $row["REQUEST_NO"] = View::alink("#", htmlspecialchars($row["REQUEST_NO"]),"onclick=\"$subdata\"");
                    
                    $row["SEKOU_L_NAME"] = $db->getOne(knjp930Query::getLevyLDiv($model, $row["SEKOU_L_CD"]));
                    $row["SEKOU_M_NAME"] = $db->getOne(knjp930Query::getLevyMDiv($model, $row["SEKOU_L_CD"], $row["SEKOU_M_CD"]));
                    ...
                    //支出伺のチェック
                    $RowOutgo = array();
                    $RowOutgo = $db->getRow(knjp930Query::getOutgoData($model, $setRequestNo), DB_FETCHMODE_ASSOC);
        ...
        View::toHTML($model, "knjp930Form1.html", $arg);
    }
}
//コンボ作成
