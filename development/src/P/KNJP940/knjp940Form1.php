...
class knjp940Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjp940Form1", "POST", "knjp940index.php", "", "knjp940Form1");
        ...
        //新規ボタン
        $subdata  = "wopen('".REQUESTROOT."/P/KNJP940_MAIN/knjp940_mainindex.php?cmd=main&&SEND_AUTH=".AUTHORITY."&SEND_SCHOOL_KIND=".$model->schoolKind."&SEND_YEAR={$model->field["YEAR"]}&SUBWIN=SUBWIN2','SUBWIN2',0,0,screen.availWidth,screen.availHeight);";
        ...
        //検索結果
        $query = knjp940Query::selectQuery($db, $model);
        ...
        if($searchCnt > $checkCnt) {
            ...
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                if ($model->cmd == "search") {
                    ...
                    $subdata = "wopen('".REQUESTROOT."/P/KNJP940_MAIN/knjp940_mainindex.php?&cmd=main&SEND_SCHOOL_KIND=".$row["SCHOOL_KIND"]."&SEND_YEAR=".$row["YEAR"]."&SEND_SEISAN_L_CD=".$row["SEISAN_L_CD"]."&SEND_SEISAN_M_CD=".$row["SEISAN_M_CD"]."&SEND_SEISAN_L_M_CD=".$row["SEISAN_L_CD"].$row["SEISAN_M_CD"]."&SEND_REQUEST_NO=".$row["REQUEST_NO"]."&SEND_OUTGO_REQUEST_NO=".$row["OUTGO_REQUEST_NO"]."&SEND_AUTH=".AUTHORITY.$extra;
                    $row["REQUEST_NO"] = View::alink("#", htmlspecialchars($row["REQUEST_NO"]),"onclick=\"$subdata\"");
                    
                    $row["SEISAN_L_NAME"] = $db->getOne(knjp940Query::getLevyLDiv($model, $row["SEISAN_L_CD"]));
                    $row["SEISAN_M_NAME"] = $db->getOne(knjp940Query::getLevyMDiv($model, $row["SEISAN_L_CD"], $row["SEISAN_M_CD"]));
                    ...
                    //支出伺のチェック
                    $RowOutgo = array();
                    $RowOutgo = $db->getRow(knjp940Query::getOutgoData($model, $setRequestNo), DB_FETCHMODE_ASSOC);
        ...
        View::toHTML($model, "knjp940Form1.html", $arg);
    }
}
//コンボ作成
