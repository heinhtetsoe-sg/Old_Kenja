...
class knjp900_approvalForm1 {
    function main(&$model) {
        //オブジェクト作成
        $objForm        = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjp900_approvalindex.php", "", "main");
        
        if (!isset($model->warning) && $model->getAuth != "" && $model->cmd != "check") {
            $Row = knjp900_approvalQuery::getRow($model);
        } else {
            $Row =& $model->field;
        }
        ...
        $model->getHonjimeCount = $db->getOne(knjp900_approvalQuery::getCloseFlgData($model));

        //決裁の状況と指定の収入の支出額の合計を取得(キャンセル時に利用)
        if ($model->getIncomeLMcd) {
            $model->getOutgoSumGk = $db->getOne(knjp900_approvalQuery::getOutgoSumGk($model));
            $model->getApproval = $db->getOne(knjp900_approvalQuery::getLevyData($model, "APPROVAL"));
        } else {
            $model->getApproval = "";
        }
        ...
        //収入科目
        $query = knjp900_approvalQuery::getLevyMDiv($model);
        $arg["data"]["LEVY_M_NAME"] = $db->getOne($query);

        //収入金額
        $model->getRequestGk = $db->getOne(knjp900_approvalQuery::getRequestGk($model));
        $arg["data"]["REQUEST_GK"] = $model->getRequestGk;
        knjCreateHidden($objForm, "REQUEST_GK", $model->getRequestGk);

        //収入番号
        $query = knjp900_approvalQuery::getIncomeNo($model);
        ...
        //受取人氏名
        $query = knjp900_approvalQuery::getIncomeStaff($model, $Row["INCOME_STAFF"]);
        $setStaff = $db->getOne($query);
        $arg["data"]["INCOME_STAFF"] = $setStaff;
        //決済承認者
        knjCreateHidden($objForm, "INCOME_STAFF", $Row["INCOME_STAFF"]);
        ...
        View::toHTML($model, "knjp900_approvalForm1.html", $arg); 
    }
}
//コンボ作成
...
function makeBtn(&$objForm, &$arg, $model) {
    ...
    $subdata .= "&SEND_PRGID=KNJP900_APPROVAL&SEND_INCOME_L_CD={$model->getIncomeLcd}&SEND_INCOME_M_CD={$model->getIncomeMcd}";
