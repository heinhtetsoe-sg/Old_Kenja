...
class knjp950Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjp950Form1", "POST", "knjp950index.php", "", "knjp950Form1");
        ...
        $extra = "onchange=\"return btn_submit('edit');\"";
        $query = knjp950Query::getYosanSetDiv();
        ...
        $extra = "onchange=\"return btn_submit('edit');\"";
        $query = knjp950Query::getLevyLDiv($model);
        ...
        if ($model->cmd == "data_set"){
            if (!isset($model->warning)){
                $Row = $db->getRow(knjp950Query::getYosanData($model), DB_FETCHMODE_ASSOC);
            } else {
                $Row =& $model->field;
            }
        } else {
        ...
        //予算項目
        $extra = "";
        $query = knjp950Query::getLevyMDiv($model);
        ...
        $arg["data"]["REQUEST_GK"] = knjCreateTextBox($objForm, $Row["REQUEST_GK"], "REQUEST_GK", 9, 9, $extra);
        
        //前年度予算額
        $extra = " STYLE=\"text-align:right;background:darkgray\" readOnly ";
        $arg["data"]["BOFORE_REQUEST_GK"] = knjCreateTextBox($objForm, $Row["BOFORE_REQUEST_GK"], "BOFORE_REQUEST_GK", 9, 9, $extra);
        
        //増減
        $setSagaku = $Row["REQUEST_GK"] - $Row["BOFORE_REQUEST_GK"];
        $extra = " STYLE=\"text-align:right;background:darkgray\" readOnly ";
        $arg["data"]["REQUEST_SAGAKU"] = knjCreateTextBox($objForm, $setSagaku, "REQUEST_SAGAKU", 9, 9, $extra);
        ...
        View::toHTML($model, "knjp950Form1.html", $arg);
    }
}
...
function makeList(&$arg, $db, $model) {
    ...
    $query = knjp950Query::getList($model);
    $result = $db->query($query);
    $sumRequestGk = "";
    $sumBeforeRequestGk = "";
    $sumRequestSagaku = "";
    while ($rowlist = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $rowlist["LEVY_M_NAME"] = $db->getOne(knjp950Query::getLevyMDivName($model, $rowlist["YOSAN_L_CD"], $rowlist["YOSAN_M_CD"]));
