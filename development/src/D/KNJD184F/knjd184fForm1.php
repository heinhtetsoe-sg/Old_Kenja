...
class knjd184fForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd184fForm1", "POST", "knjd184findex.php", "", "knjd184fForm1");
        ...
        //学期コンボ作成
        $query = knjd184fQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd184f'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //クラスコンボ作成
        $query = knjd184fQuery::getGradeHrClass($model);
        $extra = "onchange=\"return btn_submit('knjd184f'), AllClearList();\"";
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd184f')\"".$disabled;
        ...
        View::toHTML($model, "knjd184fForm1.html", $arg); 
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    //対象外の生徒取得
    $query = knjd184fQuery::getSchnoIdou($model);
    ...
    //対象者リストを作成する
    $query = knjd184fQuery::getSchno($model);
    $result = $db->query($query);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD184F");
