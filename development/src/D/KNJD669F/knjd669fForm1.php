...
class knjd669fForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd669fForm1", "POST", "knjd669findex.php", "", "knjd669fForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //学期コンボ作成
        $query = knjd669fQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd669f'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //学年コンボ作成
        $query = knjd669fQuery::getGrade($model->field["SEMESTER"], $model);
        $extra = "onchange=\"return btn_submit('knjd669f'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        //学校校種取得
        $model->schoolkind = $db->getOne(knjd669fQuery::getSchoolKind($model->field["GRADE"]));

        //テストコンボ作成
        $query = knjd669fQuery::getTest($model->field["SEMESTER"], $model);
        $extra = "onchange=\"return btn_submit('knjd669f'), AllClearList();\"";
        ...
        View::toHTML($model, "knjd669fForm1.html", $arg); 
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    ...
    //対象者リストを作成する
    $query = knjd669fQuery::getSubclass($model, $model->field["SEMESTER"]);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD669F");
