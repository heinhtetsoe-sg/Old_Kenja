...
class knjd175dForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd175dForm1", "POST", "knjd175dindex.php", "", "knjd175dForm1");
        ...
        //学期コンボの設定
        $query = knjd175dQuery::getSemester();
        $extra = "onchange=\"return btn_submit('gakki'),AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["GAKKI"], "GAKKI", $extra, 1);

        //クラス選択コンボボックスを作成する
        $query = knjd175dQuery::getAuth($model);
        $extra = "onchange=\"return btn_submit('knjd175d'),AllClearList();\"";
        ...
        View::toHTML($model, "knjd175dForm1.html", $arg); 
    }
}
...
{
    $query = knjd175dQuery::getSchnoIdou($model);
    $result = $db->query($query);
    $opt_idou = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    //対象者リストを作成する
    $result = $db->query(knjd175dQuery::getListStudent($model));
    $opt1 = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", KNJD175D);
