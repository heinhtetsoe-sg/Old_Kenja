...
// kanji=漢字
// $Id: knjd125eForm1.php 56581 2017-10-22 12:37:16Z maeshiro $
...
class knjd125eForm1
{
    function main(&$model)
    {
        ...
        $arg["start"]    = $objForm->get_start("main", "POST", "knjd125eindex.php", "", "main");
        $arg["YEAR"]     = CTRL_YEAR;
        ...
        $result = $db->query(knjd125eQuery::getSemesterMst());
        ...
        $result = $db->query(knjd125eQuery::getTestItemName($model));
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        $result = $db->query(knjd125eQuery::selectSubclassQuery());
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        $result = $db->query(knjd125eQuery::selectChairQuery($model));
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        $result = $db->query(knjd125eQuery::selectExecuteDateQuery($model, $opt_chair_cmb));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
...
//echo $execute_date;
        //遅刻何回で欠課とするかの指数取得
        $absent = array();
        $absent = $db->getRow(knjd125eQuery::getScAbsentCov($model),DB_FETCHMODE_ASSOC);
        ...
        $offdaysFlg = $db->getRow(knjd125eQuery::getOffdaysFlg(CTRL_YEAR),DB_FETCHMODE_ASSOC);
        //帳票パラメータ
        ...
        if ($absent["ABSENT_COV"] == "0" || $absent["ABSENT_COV"] == "2") { 
            $result = $db->query(knjd125eQuery::GetAttendData($model->field["SUBCLASSCD"],$absent["ABSENT_COV"],$absent["ABSENT_COV_LATE"], $offdaysFlg));
        } else {
            $result = $db->query(knjd125eQuery::GetAttendData2($model->field["SUBCLASSCD"],$absent["ABSENT_COV_LATE"], $offdaysFlg));
        }
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        //合併先科目の単位固定／加算フラグ
        $model->creditflg = $db->getOne(knjd125eQuery::getCalculateCreditFlg($model->field["SUBCLASSCD"]));

        //成績入力対象科目の判別
        if ($db->getOne(knjd125eQuery::getSubjectFlg("D003", $model->field["SUBCLASSCD"]))) {
            $model->subjectflg = "D"; //中学・英語（素点・平常・会話）
        } else if ($db->getOne(knjd125eQuery::getSubjectFlg("D002", $model->field["SUBCLASSCD"]))) {
...
//echo implode("','", $opt_chair_cmb);
        $result = $db->query(knjd125eQuery::selectQuery($model, $execute_date, $opt_chair_cmb));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            //表示切替
            $seme_kind = $model->field["SEMESTER"] .substr($model->field["TESTITEM"],0,2);
            if ($model->show_all != "on") {
                if ($row["SCORE" .$seme_kind ."_FLG"] == "1" || 
                    $row["SCORE" .$seme_kind ."_FLG"] == "2") continue;
            }
            ...
            foreach ($ctrl_name as $code => $col)
            {
                ...
                if ($sem == $model->field["SEMESTER"] && $kind."01" == $model->field["TESTITEM"]) {
                        ...
                        $value1 = ($row[$col."_FLG"] == "1") ? $row[$col."_SUPP"] : "";
                        $value2 = ($row[$col."_FLG"] == "2") ? $row[$col."_SUPP"] : "";
                        $name1 = $col ."_1" ."-" .$counter;
                        $name2 = $col ."_2" ."-" .$counter;
                        $extrahtml = "STYLE=\"text-align: right\" onChange=\"this.style.background='#ccffcc'\" onblur=\"calc(this);\"";
                        $row[$col]  = "<td align=\"center\" bgcolor=\"" .$bgcolor ."\">" ."<input name='".$name1."' value=\"".$value1."\" type='text' maxlength='3' size='3' " .$extrahtml .">" ."</td>";
                ...
                //ラベルのみ
                    ...
                    //追試・見込
                    if ($row[$col."_FLG"] == "1" || $row[$col."_FLG"] == "2") {
                    ...
                    //追試・見込以外
                        ...
                        $bgcolor = $row[$col."_COLOR"];
                        $value = ($row[$col."_FLG"] == "3" && strlen($row[$col."_PASS"])) ? $row[$col."_PASS"] : $row[$col];
        ...
        View::toHTML($model, "knjd125eForm1.html", $arg);
    }
}
?>
