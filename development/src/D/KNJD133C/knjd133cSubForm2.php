...
class knjd133cSubForm2
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform2", "POST", "knjd133cindex.php", "", "subform2");
        ...
        if ($model->Properties["KNJD133C_semesCombo"] == "1") {
            $arg["SEME_NAME"]  = $db->getOne(knjd133cQuery::getSemesterCmb($model->semester));
            $arg["NO_SEME"]    = "";
        } else {
            $arg["SEME_NAME"]  = "";
            $arg["NO_SEME"]    = "1";
        ...
        //学年
        $query = knjd133cQuery::getGrade($model);
        ...
        if ($cnt > 1) {
            ...
            //表示
            $grade = $db->getRow(knjd133cQuery::getGrade($model), DB_FETCHMODE_ASSOC);
        ...
        View::toHTML($model, "knjd133cSubForm2.html", $arg);
    }
}
//コンボ作成
...
{
    $datacnt = 0;
    $query = knjd133cQuery::getHtrainRemarkTempDat($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
