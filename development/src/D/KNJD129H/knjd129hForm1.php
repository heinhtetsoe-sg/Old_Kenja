...
class knjd129hForm1
{
    function main(&$model)
    {
        ...
        $arg["start"]    = $objForm->get_start("main", "POST", "knjd129hindex.php", "", "main");
        ...
        //学期コンボ
        $query = knjd129hQuery::getSemester();
        ...
        View::toHTML($model, "knjd129hForm1.html", $arg);
    }
}
...
function makeHead(&$objForm, &$arg, $db, $model) {
    //科目別設定があるか
    $testSubCnt = $db->getOne(knjd129hQuery::getTestSubCnt($model));
    ...
    $result = $db->query(knjd129hQuery::getTestName($model, $testSubCnt));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $count++;
        if ($sem != $row["SEMESTER"]) {
            $sem = $row["SEMESTER"];
...
function makeMeisai(&$objForm, &$arg, $db, &$model) {
    ...
    //一覧表示
    $colorFlg = false;
    $result = $db->query(knjd129hQuery::selectQuery($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        //学籍＋科目
        ...
        foreach ($model->testcdArray as $key => $codeArray) {
            ...
            $scoreRow = array();
            $query = knjd129hQuery::getScore(CTRL_YEAR, $testcd, $model, $rowKey);
            $scoreRow = $db->getRow($query, DB_FETCHMODE_ASSOC);
            if (substr($testcd, 0, 5) == substr($model->field["SEMESTER"].str_replace("-", "", $model->field["TESTIK"]), 0, 5)) {
                //(素点)(ＡＡ)(得点)テキストいずれか入力済みの時
                if ($scoreRow["SCORE_FLG"] == "2") $score_flg = true;
            }
        }
        //処理済を表示しない(素点)(ＡＡ)(得点)
        if (!$model->dispZumi) {
            if ($score_flg) continue;
        }
...
function getScoreSansyutu($db, $testcd, $model, $schregNo, $rowKey, $subclassCd, $groupCd) {
    $score = "";
    //個人素点
    $query = knjd129hQuery::getRecordRankSdiv($model, $rowKey);
    $result = $db->query($query);
    $testArray = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    //平均
    $query = knjd129hQuery::getRecordAvgSdiv($model, $subclassCd, $groupCd);
    $result = $db->query($query);
    $testAvgArray = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    if ($model->field["SEMESTER"] == "1") {
        if (strlen($testArray["10201"]) == 0) {
            //期末がない場合は、単位なし
        } else if (str_replace("-", "", $model->field["TESTIK"]) == $model->TYUKAN1) {
