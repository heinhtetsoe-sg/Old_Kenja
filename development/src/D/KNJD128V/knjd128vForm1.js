...
window.addEventListener(
    "load",
    function (event) {
        if (sessionStorage.getItem("KNJD128V_CurrentCursor") != null) {
            document.title = "";
            document.getElementById(sessionStorage.getItem("KNJD128V_CurrentCursor")).focus();
        }
        setTimeout(function () {
...
function current_cursor(para) {
    document.title = "";
    sessionStorage.setItem("KNJD128V_CurrentCursor", para);
}
/* Add by Kaung for PC-Talker 2020-02-20 end */

function btn_submit(cmd, calc_testcd) {
    /* Add by Kaung for PC-Talker 2020-02-10 start */
    if (sessionStorage.getItem("KNJD128V_CurrentCursor") != null) {
        document.title = "";
        parent.top_frame.document.title = "";
        document.getElementById(sessionStorage.getItem("KNJD128V_CurrentCursor")).blur();
    }
    setTimeout(function () {
        parent.top_frame.document.title = "佐賀県教務支援システム Ver0.01";
    }, 100);
    ...
    if (cmd == "reset") {
        ...
        if (calc_testcd != "") {
            document.forms[0].CALC_TESTCD.value = calc_testcd;
        }
    ...
    //1:算出ボタン非表示
    if (document.forms[0].KNJD128V_SANSHUTU_BTN_un_hyoji.value !== "1" && document.forms[0].knjd128vShowBtnCalcOnTop.value !== "1") {
        document.forms[0].btn_calc.disabled = true; //算出ボタン(下)
    }
    //見込点/参考点/備考ボタン/算出ボタン(上)
    if (document.forms[0].disBtnName.value !== "") {
        var disBtnList = document.forms[0].disBtnName.value.split(",");
        for (var k = 0; k < disBtnList.length; k++) {
            //テキストボックス名でまわす
...
function calc(obj) {
    ...
    var perfectFromName = nam.split("-")[0] + "_PERFECT_FROM";
    var perfectFromNumber = nam.split("-")[1];
    perfectFromObject = eval('document.forms[0]["' + perfectFromName + "-" + perfectFromNumber + '"]');
    var perfectFrom = perfectFromObject ? parseInt(perfectFromObject.value) : 0; //-10 or 0
...
function newwin(SERVLET_URL) {
    ...
    /* Add by PP for PC-Talker 2020-02-10 start */
    setTimeout(function () {
        document.title = "成績入力画面";
    }, 100);
            ...
            if (startFlg) {
                if (clipTextArray[j][i] != undefined) {
                    ...
                    if (perfectObject) {
                        ...
                        //花園の場合、平常点の入力範囲は「-10～10」とする
                        perfectFromObject = eval('document.forms[0]["' + objectNameArray[k] + "_PERFECT_FROM" + "-" + perfectNumber + '"]');
                        perfectFrom = perfectFromObject ? parseInt(perfectFromObject.value) : 0; //-10 or 0
...
// 青山学院の特別措置者表示機能を追加
// 入力可能なテキストにカーソルがある時、授業時数と欠課時数で判別し、氏名と欠課時数の文字色を変更する
function changeColor(obj) {
    if (document.forms[0].z010name1.value !== "aoyama") {
        return;
    }

    var sem;
    if (obj.name.match(/SCORE1/)) {
        sem = "1"; //1学期
    } else if (obj.name.match(/SCORE2/)) {
        sem = "2"; //2学期
    } else {
        sem = "3"; //年間
    }
    var lineCnt = document.forms[0].COUNTER.value; //行数
    for (var i = 0; i < lineCnt; i++) {
        changeColorId1 = document.getElementById("CHANGE_COLOR_ID1" + "-" + i); //氏名
        changeColorId2 = document.getElementById("CHANGE_COLOR_ID2" + "-" + i); //欠課時数
        changeColorFlg = document.forms[0]["CHANGE_COLOR_FLG" + sem + "-" + i].value;
        if (changeColorFlg == "RED") {
            changeColorId1.style.color = "#ff0000"; //赤色
            changeColorId2.style.color = "#ff0000"; //赤色
        } else if (changeColorFlg == "BLUE") {
            changeColorId1.style.color = "#0070c0"; //青色
            changeColorId2.style.color = "#0070c0"; //青色
        }
    }
}
