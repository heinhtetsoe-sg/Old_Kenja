...
class knjd128Form1
{
    public function main(&$model)
    {
        ...
        $arg["start"]    = $objForm->get_start("main", "POST", "knjd128index.php", "", "main");
        $arg["YEAR"]     = CTRL_YEAR;
        ...
        //テスト名取得
        $semename = $testname = "";
        $query = knjd128Query::getTestName($model);
        ...
        $arg["SEM_VALUE_NAME"] = "評価";
        $sem_value_name = $semename ."評価";
        if (strlen($model->field["TESTCD2"])) {
            $arg["sem_value"] = $model->field["TESTCD2"];
            $arg["COLSPAN_SEMENAME"] = "2";
            $arg["COLSPAN_FOOTER"] = "12";
            if ($model->field["TESTCD2"] == "19901") {
                $arg["SEM_VALUE_NAME"] = "７月<BR>評価";
                $sem_value_name = "７月評価";
            } elseif ($model->field["TESTCD2"] == "29901") {
                $arg["SEM_VALUE_NAME"] = "12月<BR>評価";
                $sem_value_name = "12月評価";
            }
        } else {
            $arg["sem_value"] = "";
            $arg["COLSPAN_SEMENAME"] = "1";
            $arg["COLSPAN_FOOTER"] = "11";
        }
        $objForm->ae(array("type"      => "hidden",
                            "name"      => "SEM_VALUE_NAME",
                            "value"     => $sem_value_name ));
        ...
        foreach ($testcdArray as $testkey => $testval) {
            ...
            if ($testval == $model->field["TESTCD2"]) {
                $ctrl_name[$testkey] = "SEM_VALUE";
            }
        }
        ...
        foreach ($ctrl_name as $code => $col) {
            if ($col == "TST_SCORE") {
                $header[$code] = $semename."－".$testname;
            }
            if ($col == "SEM_VALUE") {
                $header[$code] = $sem_value_name;
            }
        }
        $header[get_count($header)] = "不振";
        $header[get_count($header)] = "不振理由";
        $header[get_count($header)] = $model->lastColumn;
        $objUp->setHeader(array_values($header));

        //科目コンボ
        $opt_sbuclass = array();
        $result = $db->query(knjd128Query::selectSubclassQuery($model));
        //教育課程対応
        ...
        //講座コンボ
        $opt_chair = array();
        $result = $db->query(knjd128Query::selectChairQuery($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //CSV出力ファイル名
        $objUp->setFileName(CTRL_YEAR."年度_".$subclassname."_".$chairname."_"."成績入力(".$semename."_".$testname.").csv");
        ...
        $admin_key = array();
        $result = $db->query(knjd128Query::selectContolCodeQuery($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($seme_test != "0") {
            $result = $db->query(knjd128Query::selectExecuteDateQuery($model, $seme_test, $kind_test, $item_test));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //遅刻何回で欠課とするかの指数取得
        $absent = array();
        $absent = $db->getRow(knjd128Query::getScAbsentCov($model), DB_FETCHMODE_ASSOC);

        //文字評定
        if ($model->gen_ed == substr($model->field["SUBCLASSCD"], 0, 2)) {
            $assess = array();
            $result = $db->query(knjd128Query::getAssessMark());
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $offdaysFlg = $db->getRow(knjd128Query::getOffdaysFlg(CTRL_YEAR), DB_FETCHMODE_ASSOC);
        //帳票パラメータ
        $objForm->ae(array("type"      => "hidden",
                            "name"      => "SUB_OFFDAYS",
        ...
        if ($absent["ABSENT_COV"] == "0" || $absent["ABSENT_COV"] == "2" || $absent["ABSENT_COV"] == "4" || $absent["ABSENT_COV"] == "5") {
            $result = $db->query(knjd128Query::getAttendData($model->field["CHAIRCD"], $model->field["SUBCLASSCD"], $absent["ABSENT_COV"], $absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        } else {
            $result = $db->query(knjd128Query::getAttendData2($model->field["CHAIRCD"], $model->field["SUBCLASSCD"], $absent["ABSENT_COV"], $absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        }
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //合併先科目の単位固定／加算フラグ
        $model->creditflg = $db->getOne(knjd128Query::getCalculateCreditFlg($model->field["SUBCLASSCD"], $model));

        //一覧表示
        if (strlen($execute_date)) {
            $colorFlg = false;
            $result = $db->query(knjd128Query::selectQuery($model, $execute_date));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                //学籍番号をHiddenで保持
                ...
                foreach ($ctrl_name as $code => $col) {
                    ...
                    //管理者コントロール
                        ...
                        //考査満点マスタ
                        $query = knjd128Query::getPerfect(CTRL_YEAR, $model->field["SUBCLASSCD"], $testcdArray[$code], $row["GRADE"], $row["COURSE"], $model);
        ...
        //累積現在日
        $cur_date = $db->getRow(knjd128Query::getMax($model->field["SUBCLASSCD"], $model), DB_FETCHMODE_ASSOC);
        ...
        foreach ($testcdArray as $testkey => $testval) {
            ...
            if ($model->useRecordChkfinDat == "1") {
                $query = knjd128Query::getRecordChkfinDat($model, $testval, $recordDiv);
            } else {
                $query = knjd128Query::getRecordSchChrTest($model, $testval);
            }
        ...
        $recordDiv = "2"; // 2:評定・評価
        foreach ($testcdArray as $testkey => $testval) {
            if ($model->field["TESTCD2"] != $testval) {
                continue;
            }
        ...
        View::toHTML($model, "knjd128Form1.html", $arg);
    }
}
