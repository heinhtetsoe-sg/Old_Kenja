...
class knjd105iForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd105iForm1", "POST", "knjd105iindex.php", "", "knjd105iForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        $extra = array("id=\"CATEGORY_IS_CLASS1\" onClick=\"return btn_submit('knjd105i')\"", "id=\"CATEGORY_IS_CLASS2\" onClick=\"return btn_submit('knjd105i')\"");
        ...
        //学期コンボ作成
        $query = knjd105iQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd105i'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //テストコンボ作成
        $query = knjd105iQuery::getTest($model->field["SEMESTER"]);
        makeCmb($objForm, $arg, $db, $query, "TESTCD", $model->field["TESTCD"], "", 1);

        //学年コンボ作成
        $query = knjd105iQuery::getGradeHrClass($model->field["SEMESTER"], $model, "GRADE");
        $extra = "onchange=\"return btn_submit('change_grade'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        if ($model->field["CATEGORY_IS_CLASS"] == 2){
            //クラスコンボ作成
            $query = knjd105iQuery::getGradeHrClass($model->field["SEMESTER"], $model, "HR_CLASS");
            $extra = "onchange=\"return btn_submit('knjd105i'), AllClearList();\"";
        ...
        //コメントデータ取得
        if ($model->cmd == '') {
            $query = knjd105iQuery::getComment();
        ...
        for ($i = 1; $i <= 5; $i++) {
            $extra = " id=\"COMMENT".$i."\" style=\"overflow: hidden;\"";
            $arg["data"]["COMMENT".$i] = knjCreateTextArea($objForm, "COMMENT".$i, 1, 12*2, "soft", $extra, $model->field["COMMENT".$i]);

            KnjCreateHidden($objForm, "COMMENT".$i."_KETA", 12*2);
            KnjCreateHidden($objForm, "COMMENT".$i."_GYO", 1);
            KnjCreateHidden($objForm, "COMMENT".$i."_STAT", "statusareaCOMMENT".$i);
        }
        ...
        View::toHTML5($model, "knjd105iForm1.html", $arg); 
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["CATEGORY_IS_CLASS"] == 1) {
        ...
        $opt1 = array();
        $opt2 = array();
        $selected = explode(",", $model->field["selected"]);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            if ($model->cmd == "knjd105iupdated" && in_array($row["VALUE"], $selected)) {
        ...
        //対象外の生徒取得
        $query = knjd105iQuery::getSchnoIdou($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt_idou = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //対象者リストを作成する
        $query = knjd105iQuery::getStudent($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt1 = array();
        $opt2 = array();
        $selected = explode(",", $model->field["selected"]);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            if ($model->cmd == "knjd105iupdated" && in_array($row["SCHREGNO"], $selected)) {
                $opt2[] = array('label' => $row["SCHREGNO"].$idou.$row["ATTENDNO"]."番".$idou.$row["NAME_SHOW"],
                                'value' => $row["SCHREGNO"]);
            } else {
...
function makeBtn(&$objForm, &$arg) {
    ...
    $arg["button"]["btn_update"] = knjCreateBtn($objForm, "btn_update", "コメント更新", $extra);
    //印刷ボタンを作成する
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD105I");
    knjCreateHidden($objForm, "IMAGE_PATH", DOCUMENTROOT."/src/image");
    knjCreateHidden($objForm, "selected");
