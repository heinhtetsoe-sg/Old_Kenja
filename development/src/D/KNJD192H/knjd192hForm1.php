...
class knjd192hForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd192hForm1", "POST", "knjd192hindex.php", "", "knjd192hForm1");
        //DB接続
        $db = Query::dbCheckOut();

        //学校名取得
        $query = knjd192hQuery::getSchoolname();
        ...
        $extra = array("id=\"CATEGORY_IS_CLASS1\" onClick=\"return btn_submit('knjd192h')\"", "id=\"CATEGORY_IS_CLASS2\" onClick=\"return btn_submit('knjd192h')\"");
        ...
        //学期コンボ作成
        $query = knjd192hQuery::getSemester();
        $extra = "onchange=\"return btn_submit('change_grade'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //学年コンボ作成
        $query = knjd192hQuery::getGradeHrClass($model->field["SEMESTER"], $model, "GRADE");
        $extra = "onchange=\"return btn_submit('change_grade'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        if ($model->field["CATEGORY_IS_CLASS"] == 2){
            //クラスコンボ作成
            $query = knjd192hQuery::getGradeHrClass($model->field["SEMESTER"], $model, "HR_CLASS");
            $extra = "onchange=\"return btn_submit('knjd192h'), AllClearList();\"";
            makeCmb($objForm, $arg, $db, $query, "HR_CLASS", $model->field["HR_CLASS"], $extra, 1);
        }

        //テストコンボ作成
        $query = knjd192hQuery::getTest($model->field["SEMESTER"], $model->field["GRADE"]);
        $opt = array();
        ...
        //同一出席番号をグループで出力
        $extra = $model->field["SAME_ATTENDNO_PAGE"] == "1" ? "checked" : "";
        $extra .= " id=\"SAME_ATTENDNO_PAGE\"";
        if ($model->field["CATEGORY_IS_CLASS"] == '2'){
            $extra .= " disabled ";
        }
        $arg["data"]["SAME_ATTENDNO_PAGE"] = knjCreateCheckBox($objForm, "SAME_ATTENDNO_PAGE", "1", $extra, "");
        ...
        View::toHTML($model, "knjd192hForm1.html", $arg); 
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["CATEGORY_IS_CLASS"] == 1){
        ...
        $result->free();
        $extra = "multiple style=\"height:280px;width:230px;\" ondblclick=\"move1('left')\"";
        $arg["data"]["CATEGORY_NAME"] = knjCreateCombo($objForm, "CATEGORY_NAME", "", $opt1, $extra, 20);

        $arg["data"]["NAME_LIST"] = 'クラス一覧';

        //出力対象一覧リストを作成する//
        $extra = "multiple style=\"height:280px;width:230px;\" ondblclick=\"move1('right')\"";
        ...
        //対象外の生徒取得
        $query = knjd192hQuery::getSchnoIdou($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt_idou = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //対象者リストを作成する
        $query = knjd192hQuery::getStudent($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt1 = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $idou = "　";
        ...
        $result->free();
        $extra = "multiple style=\"height:280px;width:230px;\" ondblclick=\"move1('left', 1)\"";
        $arg["data"]["CATEGORY_NAME"] = knjCreateCombo($objForm, "CATEGORY_NAME", "", $opt1, $extra, 20);

        $arg["data"]["NAME_LIST"] = '生徒一覧';

        //出力対象一覧リストを作成する//
        $extra = "multiple style=\"height:280px;width:230px;\" ondblclick=\"move1('right', 1)\"";
        $arg["data"]["CATEGORY_SELECTED"] = knjCreateCombo($objForm, "CATEGORY_SELECTED", "", array(), $extra, 20);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD192H");
