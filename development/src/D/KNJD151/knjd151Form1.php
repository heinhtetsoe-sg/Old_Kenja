...
class knjd151Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd151Form1", "POST", "knjd151index.php", "", "knjd151Form1");
        ...
        //クラス選択コンボボックスを作成する
        $db = Query::dbCheckOut();
        $query = knjd151Query::getAuth($model->control["年度"], $ga);
        ...
        $objForm->ae(array("type"       => "select",
                            ...
                            "extrahtml"  => "onchange=\"return btn_submit('knjd151'),AllClearList();\"",
        ...
        //テストコンボ作成 NO001
        $db = Query::dbCheckOut();
        $query = knjd151Query::getTestkind($model->control["年度"], $ga, $model);
        $result = $db->query($query);
        $opt_kind = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd151', 'on')\"".$disabled;
        $date_textbox = knjCreateTextBox($objForm, $model->field["DATE"], "DATE", 12, 12, $extra);
        //異動対象日付（カレンダー）
        global $sess;
        $extra = "onclick=\"tmp_list('knjd151', 'off'); loadwindow('" .REQUESTROOT ."/common/calendar.php?name=DATE&frame='+getFrameName(self) + '&date=' + document.forms[0]['DATE'].value + '&CAL_SESSID=$sess->id&reload=true', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 200)\"";
        $date_button = knjCreateBtn($objForm, "btn_calen", "･･･", $extra);
        //異動対象日付
        $arg["data"]["DATE"] = View::setIframeJs().$date_textbox.$date_button;

        //対象外の生徒取得
        $query = knjd151Query::getSchnoIdou($model, $ga);
        $result = $db->query($query);
        $opt_idou = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        View::toHTML($model, "knjd151Form1.html", $arg);
    }
}
