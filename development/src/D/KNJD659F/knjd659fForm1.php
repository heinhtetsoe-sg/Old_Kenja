...
class knjd659fForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd659fForm1", "POST", "knjd659findex.php", "", "knjd659fForm1");
        ...
        //学期コンボ作成
        $query = knjd659fQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd659f'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);
        //学年末は今学期とする
        $seme = $model->field["SEMESTER"] == "9" ? CTRL_SEMESTER : $model->field["SEMESTER"];

        //学年コンボ作成
        $query = knjd659fQuery::getSchoolName();
        $schoolName = $db->getOne($query);
        if ($schoolName == 'CHIBEN') {
            $query = knjd659fQuery::getGradeChiben($seme, $model);
            $extra = "onchange=\"return btn_submit('knjd659f'), AllClearList();\"";
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
        } else {
            $query = knjd659fQuery::getGrade($seme, $model);
            $extra = "onchange=\"return btn_submit('knjd659f'), AllClearList();\"";
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
        }
        ...
        View::toHTML($model, "knjd659fForm1.html", $arg); 
    }
}
...
function makeClassList(&$objForm, &$arg, $db, $model, $seme) {
    ...
    //クラス一覧リストを作成する
    $query = knjd659fQuery::getHrClass($seme, $model->field["GRADE"]);
...
//    knjCreateHidden($objForm, "CTRL_DATE", CTRL_DATE);
    knjCreateHidden($objForm, "PRGID", "KNJD659F");
