...
class knjd185oForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd185oForm1", "POST", "knjd185oindex.php", "", "knjd185oForm1");
        ...
        //学期コンボの設定
        $query = knjd185oQuery::getSemester();
        $extra = "onchange=\"return btn_submit('gakki'),AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["SEMESTER"], "SEMESTER", $extra, 1);

        //クラス選択コンボボックスを作成する
        $query = knjd185oQuery::getAuth($model);
        $extra = "onchange=\"return btn_submit('knjd185o'),AllClearList();\"";
        ...
        //異動対象日付
        if ($model->field["NINTEI_DATE"] == "") $model->field["NINTEI_DATE"] = str_replace("-","/",CTRL_DATE);
        $arg["data"]["NINTEI_DATE"] = View::popUpCalendar($objForm ,"NINTEI_DATE" ,$model->field["NINTEI_DATE"]);
        ...
        View::toHTML($model, "knjd185oForm1.html", $arg); 
    }
}
...
{
    $query = knjd185oQuery::getSchnoIdou($model);
    $result = $db->query($query);
    $opt_idou = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    //対象者リストを作成する
    $result = $db->query(knjd185oQuery::getListStudent($model));
    $opt1 = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD185O");
