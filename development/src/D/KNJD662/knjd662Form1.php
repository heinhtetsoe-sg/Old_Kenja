...
class knjd662Form1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd662Form1", "POST", "knjd662index.php", "", "knjd662Form1");
        ...
        $extra = array("id=\"CATEGORY_IS_CLASS1\" onClick=\"return btn_submit('knjd662')\"", "id=\"CATEGORY_IS_CLASS2\" onClick=\"return btn_submit('knjd662')\"");
        ...
        //データ種別コンボ作成
        $query = knjd662Query::getDataDiv();
        $extra = "onchange=\"return btn_submit('knjd662')\"";
        makeCmb($objForm, $arg, $db, $query, "DATA_DIV", $model->field["DATA_DIV"], $extra, 1);

        //テスト名称コンボ作成
        $query = knjd662Query::getMockName($model);
        makeCmb($objForm, $arg, $db, $query, "MOCKCD", $model->field["MOCKCD"], "", 1);

        //学年コンボ作成
        $query = knjd662Query::getGradeHrClass(CTRL_SEMESTER, $model, "GRADE");
        $extra = "onchange=\"return btn_submit('knjd662'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        if ($model->field["CATEGORY_IS_CLASS"] == 2){
            //クラスコンボ作成
            $query = knjd662Query::getGradeHrClass(CTRL_SEMESTER, $model, "HR_CLASS");
            $extra = "onchange=\"return btn_submit('knjd662'), AllClearList();\"";
        ...
        $extra  = array("id=\"SELECT_DIV1\" onclick=\"return btn_submit('knjd662')\"", "id=\"SELECT_DIV2\" onclick=\"return btn_submit('knjd662')\"", "id=\"SELECT_DIV3\" onclick=\"return btn_submit('knjd662')\"");
...
//        foreach($radioArray as $key => $val) $arg["data"][$key] = $val;
        ...
        View::toHTML($model, "knjd662Form1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["CATEGORY_IS_CLASS"] == 1){
        ...
        //対象者リストを作成する
        $query = knjd662Query::getStudent($model, CTRL_SEMESTER);
        $result = $db->query($query);
        $opt1 = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD662");
