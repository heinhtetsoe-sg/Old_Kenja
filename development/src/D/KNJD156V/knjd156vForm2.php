...
class knjd156vForm2 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjd156vindex.php", "", "edit");
        $db = Query::dbCheckOut();

        //テスト種別コンボボックスを作成する
        $query = knjd156vQuery::getTest($model);
        makeCmb($objForm, $arg, $db, $query, "TESTCD", $model->testcd, "onchange=\"return btn_submit('edit');\"", 1);

        //警告メッセージを表示しない場合
        if (!isset($model->warning)){
            $Row = $db->getRow(knjd156vQuery::getRow($model), DB_FETCHMODE_ASSOC);
        } else {
            $Row =& $model->field;
        }
        ...
        if (VARS::request("cmd") != "edit" && !isset($model->warning)){
            $arg["reload"]  = "parent.left_frame.location.href='knjd156vindex.php?cmd=list';";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjd156vForm2.html", $arg);
    }
}
