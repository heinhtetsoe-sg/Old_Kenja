...
class knjd619qForm1 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd619qForm1", "POST", "knjd619qindex.php", "", "knjd619qForm1");
        ...
        //学期コンボ作成
        $query = knjd619qQuery::getSemester(0);
        $extra = "onchange=\"return btn_submit('knjd619q');\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);
        //学年末は今学期とする
        $seme = $model->field["SEMESTER"] == "9" ? CTRL_SEMESTER : $model->field["SEMESTER"];

        //学科名コンボ
        $query = knjd619qQuery::getCourseMajor($model);
        $extra = "onchange=\"return btn_submit('knjd619q');\"";
        makeCmb($objForm, $arg, $db, $query, "MAJOR", $model->field["MAJOR"], $extra, 1);

        //学年コンボ作成
        $query = knjd619qQuery::getGrade($seme, $model);
        $extra = "onchange=\"return btn_submit('knjd619q');\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        //テストコンボ作成
        $query = knjd619qQuery::getTest($model, $model->field["SEMESTER"], $model->field["GRADE"]);
        $extra = "onchange=\"return btn_submit('knjd619q');\"";
        makeCmb($objForm, $arg, $db, $query, "TESTKINDCD", $model->field["TESTKINDCD"], $extra, 1);

        //出欠集計日付作成
        $model->yearSdate = str_replace("-", "/", $db->getOne(knjd619qQuery::getYearSdate()));
        ...
        //欠点
        $query = knjd619qQuery::getGdat($model->field["GRADE"]);
        $h_j = $db->getOne($query);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJD619Q");
        ...
        View::toHTML($model, "knjd619qForm1.html", $arg); 
    }
}
//コンボ作成
