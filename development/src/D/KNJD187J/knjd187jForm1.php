...
class knjd187jForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd187jForm1", "POST", "knjd187jindex.php", "", "knjd187jForm1");
        ...
        //学期コンボ作成
        $query = knjd187jQuery::getSemester();
        ...
        //対象学期の開始日・終了日
        $sDate = $db->getOne(knjd187jQuery::getSemesterDate("SDATE",$model->field["SEMESTER"]));
        $eDate = $db->getOne(knjd187jQuery::getSemesterDate("EDATE",$model->field["SEMESTER"]));
        ...
        $extra = array("id=\"DISP1\" onClick=\"return btn_submit('knjd187j')\"", "id=\"DISP2\" onClick=\"return btn_submit('knjd187j')\"");
        $radioArray = knjCreateRadio($objForm, "DISP", $model->field["DISP"], $extra, $opt_disp, get_count($opt_disp));
        foreach($radioArray as $key => $val) $arg["data"][$key] = $val;

        if ($model->field["DISP"] == 1) {
            //学年コンボ
            $query = knjd187jQuery::getGrade($model);
            $extra = "onChange=\"return btn_submit('knjd187j');\"";
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
        } else if ($model->field["DISP"] == 2) {
            //クラスコンボ作成
            $query = knjd187jQuery::getHrClass(CTRL_YEAR, $seme, $model);
            $extra = "onchange=\"return btn_submit('knjd187j');\"";
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd187j', 'on')\"".$disabled;
        ...
        View::toHTML($model, "knjd187jForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model, $seme) {
    ...
    //対象外の生徒取得
    $query = knjd187jQuery::getSchnoIdou($model, $seme);
    ...
    //対象者リストを作成する
    if ($model->field["DISP"] == 2) {
        $query = knjd187jQuery::getStudent($model, $seme);
        $result = $db->query($query);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD187J");
