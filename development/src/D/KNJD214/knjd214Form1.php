...
class knjd214Form1
{
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd214Form1", "POST", "knjd214index.php", "", "knjd214Form1");
        ...
        //学年コンボ
        $query = knjd214Query::getSelectGrade($model);
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], "onChange=\"return btn_submit('knjd214');\"", 1);
        ...
        View::toHTML($model, "knjd214Form1.html", $arg); 
    }
}
...
function testCmb(&$objForm, &$arg, $db, $query, $name, &$value, $extra, $size, $model = "")
{
    //学期マスタ
    $optSem = array();
    $query = knjd214Query::getSemester();
    ...
    foreach($optSem as $sem => $semName) {
        $query = knjd214Query::getTestItem($model, $sem);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    ...
    $result = $db->query(knjd214Query::getAuth($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
