...
class knjd184mForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd184mForm1", "POST", "knjd184mindex.php", "", "knjd184mForm1");
        ...
        //学期コンボ
        $query = knjd184mQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd184m'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);
        //学年末は今学期とする
        $seme = $model->field["SEMESTER"] == "9" ? CTRL_SEMESTER : $model->field["SEMESTER"];

        //年組コンボ
        $query = knjd184mQuery::getGradeHrClass($model, $seme);
        $extra = "onchange=\"return btn_submit('knjd184m'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE_HR_CLASS", $model->field["GRADE_HR_CLASS"], $extra, 1);

        //テストコンボ作成
        $query = knjd184mQuery::getTest($model, $model->field["SEMESTER"], substr($model->field["GRADE_HR_CLASS"], 0, 2));
        makeCmb($objForm, $arg, $db, $query, "TESTCD", $model->field["TESTCD"], "", 1);

        //校種
        $query = knjd184mQuery::getGradeHrClass($model, $seme, $model->field["GRADE_HR_CLASS"]);
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd184m', 'on')\"".$disabled;
        $date_textbox = knjCreateTextBox($objForm, $model->field["DATE"], "DATE", 12, 12, $extra);
        //異動対象日付（カレンダー）
        global $sess;
        $extra = "onclick=\"tmp_list('knjd184m', 'off'); loadwindow('" .REQUESTROOT ."/common/calendar.php?name=DATE&frame='+getFrameName(self) + '&date=' + document.forms[0]['DATE'].value + '&CAL_SESSID=$sess->id&reload=true', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 200)\"";
        ...
        //設定の初期値取得
        $dataTmp = array();
        $query = knjd184mQuery::getHreportConditionDat($model);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJD184M");
        ...
        View::toHTML($model, "knjd184mForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model, $seme) {
    //対象外の生徒取得
    $query = knjd184mQuery::getSchnoIdou($model, $seme);
    $result = $db->query($query);
    $opt_idou = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    //対象者リスト
    $query = knjd184mQuery::getStudent($model, $seme);
    $result = $db->query($query);
