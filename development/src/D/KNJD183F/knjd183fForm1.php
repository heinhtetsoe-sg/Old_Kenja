...
class knjd183fForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd183fForm1", "POST", "knjd183findex.php", "", "knjd183fForm1");
        ...
        //学期コンボ作成
        $query = knjd183fQuery::getSemester();
        ...
        //対象学期の開始日・終了日
        $sDate = $db->getOne(knjd183fQuery::getSemesterDate("SDATE", $model->field["SEMESTER"]));
        $eDate = $db->getOne(knjd183fQuery::getSemesterDate("EDATE", $model->field["SEMESTER"]));
        knjCreateHidden($objForm, "SDATE", str_replace("-", "/", $sDate));
        knjCreateHidden($objForm, "EDATE", str_replace("-", "/", $eDate));

        //クラスコンボ作成
        $query = knjd183fQuery::getHrClass(CTRL_YEAR, $seme, $model);
        $extra = "onchange=\"return btn_submit('knjd183f');\"";
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd183f', 'on')\"".$disabled;
        $date_textbox = knjCreateTextBox($objForm, $model->field["DATE"], "DATE", 12, 12, $extra);
        //異動対象日付（カレンダー）
        global $sess;
        $extra = "onclick=\"tmp_list('knjd183f', 'off'); loadwindow('" .REQUESTROOT ."/common/calendar.php?name=DATE&frame='+getFrameName(self) + '&date=' + document.forms[0]['DATE'].value + '&CAL_SESSID=$sess->id&reload=true', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 200)\"";
        ...
        View::toHTML($model, "knjd183fForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model, $seme)
{
    ...
    //対象外の生徒取得
    $query = knjd183fQuery::getSchnoIdou($model, $seme);
    ...
    //対象者リストを作成する
    $query = knjd183fQuery::getStudent($model, $seme);
    $result = $db->query($query);
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD183F");
