...
class knjd135rSubForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform1", "POST", "knjd135rindex.php", "", "subform1");
        ...
        //学年
        $query = knjd135rQuery::getGrade($model,$model->schno, $model->sem);
        ...
        View::toHTML($model, "knjd135rSubForm1.html", $arg); 
    }
}
//コンボ作成
...
function makeList(&$objForm, &$arg, $db, $model) {
    $datacnt = 0;
    $query = knjd135rQuery::getHtrainRemarkTempDat($model, $model->sem, $model->subField["GRADE"]);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
