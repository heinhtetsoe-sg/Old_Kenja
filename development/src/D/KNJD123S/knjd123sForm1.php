...
class knjd123sForm1
{
    public function main(&$model)
    {
        ...
        $arg["start"]    = $objForm->get_start("main", "POST", "knjd123sindex.php", "", "main");
        $arg["YEAR"]     = CTRL_YEAR;
        ...
        $query = knjd123sQuery::getTestName($model);
        ...
        $result = $db->query(knjd123sQuery::selectSubclassQuery($model->gen_ed, $model));
        ...
        $result = $db->query(knjd123sQuery::selectChairQuery($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
//                            "KS" => "#ff0099"); //病欠
        ...
        $admin_key = array();
        $result = $db->query(knjd123sQuery::selectContolCodeQuery($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($seme_test != "0") {
            $result = $db->query(knjd123sQuery::selectExecuteDateQuery($model, $seme_test, $kind_test));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //遅刻何回で欠課とするかの指数取得
        $absent = array();
        $absent = $db->getRow(knjd123sQuery::getScAbsentCov($model), DB_FETCHMODE_ASSOC);

        //文字評定
//        if ($model->gen_ed == $model->field["SUBCLASSCD"]) {
        if ($model->gen_ed == substr($model->field["SUBCLASSCD"], 0, 2)) {//NO005
            $assess = array();
            $result = $db->query(knjd123sQuery::getAssessMark());
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $offdaysFlg = $db->getRow(knjd123sQuery::getOffdaysFlg(CTRL_YEAR), DB_FETCHMODE_ASSOC);
        //帳票パラメータ
        ...
        if ($absent["ABSENT_COV"] == "0" || $absent["ABSENT_COV"] == "2" || $absent["ABSENT_COV"] == "4" || $absent["ABSENT_COV"] == "5") {
            $result = $db->query(knjd123sQuery::getAttendData($model->field["CHAIRCD"], $model->field["SUBCLASSCD"], $absent["ABSENT_COV"], $absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        } else {
            $result = $db->query(knjd123sQuery::getAttendData2($model->field["CHAIRCD"], $model->field["SUBCLASSCD"], $absent["ABSENT_COV"], $absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        }
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //合併先科目の単位固定／加算フラグ
        $model->creditflg = $db->getOne(knjd123sQuery::getCalculateCreditFlg($model->field["SUBCLASSCD"], $model));
        //if (isset($model->creditflg)) echo "))". $model->creditflg ."((" ."<BR>";

        //一覧表示
//        $result = $db->query(knjd123sQuery::selectQuery($model));
        $colorFlg = false;
        $result = $db->query(knjd123sQuery::selectQuery($model, $execute_date));//---2005.06.21Add
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //学籍番号をHiddenで保持
            ...
            if (is_array($assess)) {
                if ($model->Properties["useAssessSubclassMst"] == '1') {
                    //科目別評定マスタの設定があるか
                    $query = knjd123sQuery::getAssessSubclassMstMark($model, CTRL_YEAR, $row["GRADE"], $row["COURSE"], $model->field["SUBCLASSCD"], $row["GRAD_VALUE"], "cnt");
                    $cnt = $db->getOne($query);
                    if (0 < $cnt) {
                        //科目別評定マスタ
                        $query = knjd123sQuery::getAssessSubclassMstMark($model, CTRL_YEAR, $row["GRADE"], $row["COURSE"], $model->field["SUBCLASSCD"], $row["GRAD_VALUE"], "mark");
            ...
            if ($model->Properties["useProvFlg"] == '1') {
                ...
                $objUp->setSize(array(7=>3,8=>3,9=>3,10=>3,11=>3,12=>3,13=>3,14=>3,15=>3,16=>3,17=>3,18=>3,19=>3,20=>3,21=>3));
            } else {
                $objUp->setType(array(7=>'S',8=>'S',9=>'S',10=>'S',11=>'S',12=>'S',13=>'S',14=>'S',15=>'S',16=>'S',17=>'S',18=>'S',19=>'S',20=>'S'));
                $objUp->setSize(array(7=>3,8=>3,9=>3,10=>3,11=>3,12=>3,13=>3,14=>3,15=>3,16=>3,17=>3,18=>3,19=>3,20=>3));
            }
...
//              if (strlen($row[$col."_DI"]) && $sem != "8" && $sem_val == "2") $row[$col] = $row[$col."_DI"];
                ...
                //管理者コントロール
                    ...
                    //考査満点マスタ
                    $query = knjd123sQuery::getPerfect(CTRL_YEAR, $model->field["SUBCLASSCD"], $perfectcd[$code], $row["GRADE"], $row["COURSE"], $model);
        ...
        //累積現在日
        $cur_date = $db->getRow(knjd123sQuery::getMax($model->field["SUBCLASSCD"], $model), DB_FETCHMODE_ASSOC);
        ...
        foreach ($testcdArray as $testkey => $testval) {
            ...
            if ($model->useRecordChkfinDat == "1") {
                $query = knjd123sQuery::getRecordChkfinDat($model, $testval, $recordDiv);
            } else {
                $query = knjd123sQuery::getRecordSchChairTestDat($model, $testval, $recordDiv);
            }
...
//                            "value"     => ($model->field["SUBCLASSCD"] == $model->gen_ed ? $model->gen_ed : "" ) ) );
        ...
        $objForm->ae(array("type"      => "hidden",
                            "name"      => "PRGID",
                            "value"     => "KNJD123S" ));//---2005.05.24Modify
        ...
        View::toHTML($model, "knjd123sForm1.html", $arg);
    }
}
