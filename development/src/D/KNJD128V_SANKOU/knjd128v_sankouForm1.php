...
class knjd128v_sankouForm1
{
    function main(&$model)
    {
        $objForm = new form;

        $arg["start"]    = $objForm->get_start("main", "POST", "knjd128v_sankouindex.php", "", "main");
        ...
        if ($model->Properties["use_school_detail_gcm_dat"] == '1') {
            ...
            $query = knjd128v_sankouQuery::getCourseMajor($model);
        ...
        $query = knjd128v_sankouQuery::getSubclassMst($model);
        ...
        if ($model->Properties["useCurriculumcd"] == '1') {
            if ($model->field["SUBCLASSCD"]) {
                ...
                $schoolKind = $db->getOne(knjd128v_sankouQuery::getSchoolkindQuery());
            }
            $schoolKind = $db->getOne(knjd128v_sankouQuery::getAdminSchoolKind($schoolKind));
        ...
        $query = knjd128v_sankouQuery::selectChairQuery($model);
        ...
        View::toHTML($model, "knjd128v_sankouForm1.html", $arg);
    }
}
...
function makeHead(&$objForm, &$arg, $db, $model) {
    //科目別設定があるか
    $testSubCnt = $db->getOne(knjd128v_sankouQuery::getTestSubCnt($model));
    ...
    $head2 = "";
    $testcdArray = array();
    $result = $db->query(knjd128v_sankouQuery::getTestName($model, $testSubCnt));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($row["TESTCD"] == $model->field["TESTCD"]) $send_testname = $row["SEMESTERNAME"]."<BR>".$row["TESTITEMNAME"]."<BR>"."参考点";
    }
    $result->free();
...
function makeMeisai(&$objForm, &$arg, $db, &$model) {
    ...
    //成績データ取得
    $scoreArray = array();
    $result = $db->query(knjd128v_sankouQuery::getScore($model, $execute_date));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    //参考点データ取得
    $recinfoArray = array();
    $result = $db->query(knjd128v_sankouQuery::getRecordInfo($model, $execute_date));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    //一覧表示
    $colorFlg = false;
    $result = $db->query(knjd128v_sankouQuery::selectQuery($model, $execute_date));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        //学籍番号をHiddenで保持
        ...
        $row[$name] = knjCreateTextBox($objForm, $recinfoRow["SANKOU"], $name."-".$counter, 3, 3, $extra);
        //テキストの名前をセット
        if ($counter == 0) {
        ...
        //縦計(参考点)
        if (is_numeric($recinfoRow["SANKOU"])) {
           $term_recinfo[] = $recinfoRow["SANKOU"];
        }
