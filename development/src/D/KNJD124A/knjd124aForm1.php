...
class knjd124aForm1
{
    public function main(&$model)
    {
        ...
        $arg["start"]    = $objForm->get_start("main", "POST", "knjd124aindex.php", "", "main");
        $arg["YEAR"]     = CTRL_YEAR;
        ...
        $query = knjd124aQuery::getTestName($model);
        ...
        $header = array("0" => "科目コード",
                        ...
                        "0884" => "履修単位",
                        "0885" => "修得単位",
                        "21" => $model->lastColumn);//2005.09.28ALP
        $objUp->setHeader(array_values($header));
        
        //科目コンボ
        $opt_sbuclass = array();
        $opt_sbuclass[] = array("label" => "", "value" => "");
        $result = $db->query(knjd124aQuery::selectSubclassQuery($model->gen_ed, $model));
        ...
        $result = $db->query(knjd124aQuery::selectChairQuery($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
//                            "KS" => "#ff0099"); //病欠
        ...
        //管理者コントロール
        $model->testcd = "";    //講座コードをどの考査にセットするかを判断するコードの値を保持用 NO009Add
        $seme_kind_item_s = ""; //最初の値を保持用 NO009Add
        $seme_kind_item_e = ""; //最後の値を保持用 NO009Add
        $soten_flg = false;     //素点の指定があるかどうかのフラグを保持用 NO009Add
        $admin_key = array();
        $result = $db->query(knjd124aQuery::selectContolCodeQuery($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $admin_key[] = $row["CONTROL_CODE"];
            $admin_key2[$row["CONTROL_CODE"]]=$row["CONTROL_CODE"];//NO001
        ...
        //素点指定があるかつ最初と最後の値が等しい場合のみ、$model->testcdに値をセットする。 NO009Add
        ...
        //時間割講座テストより試験日を抽出---2005.06.21Add
        ...
        if ($seme_test != "0") {
            $result = $db->query(knjd124aQuery::selectExecuteDateQuery($model, $seme_test, $kind_test));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //遅刻何回で欠課とするかの指数取得
        $absent = array();
        $absent = $db->getRow(knjd124aQuery::getScAbsentCov($model), DB_FETCHMODE_ASSOC);

        //文字評定
//        if ($model->gen_ed == $model->field["SUBCLASSCD"]) {
        if ($model->gen_ed == substr($model->field["SUBCLASSCD"], 0, 2)) {//NO005
            $assess = array();
            $result = $db->query(knjd124aQuery::getAssessMark());
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $offdaysFlg = $db->getRow(knjd124aQuery::getOffdaysFlg(CTRL_YEAR), DB_FETCHMODE_ASSOC);
        //帳票パラメータ
        ...
        if ($absent["ABSENT_COV"] == "0" || $absent["ABSENT_COV"] == "2") {
            $result = $db->query(knjd124aQuery::getAttendData($model->field["CHAIRCD"], $model->field["SUBCLASSCD"], $absent["ABSENT_COV"], $absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        } else {
            $result = $db->query(knjd124aQuery::getAttendData2($model->field["CHAIRCD"], $model->field["SUBCLASSCD"], $absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        }
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //合併先科目の単位固定／加算フラグ
        $model->creditflg = $db->getOne(knjd124aQuery::getCalculateCreditFlg($model->field["SUBCLASSCD"], $model));
        //if (isset($model->creditflg)) echo "))". $model->creditflg ."((" ."<BR>";

        //一覧表示
//        $result = $db->query(knjd124aQuery::selectQuery($model));
        $colorFlg = false;
        $result = $db->query(knjd124aQuery::selectQuery($model, $execute_date));//---2005.06.21Add
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //学籍番号をHiddenで保持
            ...
            if (is_array($assess)) {
                if ($model->Properties["useAssessSubclassMst"] == '1') {
                    //科目別評定マスタの設定があるか
                    $query = knjd124aQuery::getAssessSubclassMstMark($model, CTRL_YEAR, $row["GRADE"], $row["COURSE"], $model->field["SUBCLASSCD"], $row["GRAD_VALUE"], "cnt");
                    $cnt = $db->getOne($query);
                    if (0 < $cnt) {
                        //科目別評定マスタ
                        $query = knjd124aQuery::getAssessSubclassMstMark($model, CTRL_YEAR, $row["GRADE"], $row["COURSE"], $model->field["SUBCLASSCD"], $row["GRAD_VALUE"], "mark");
            ...
            $objUp->setType(array(7=>'S',8=>'S',9=>'S',10=>'S',11=>'S',12=>'S',13=>'S',14=>'S',15=>'S',16=>'S',17=>'S',18=>'S',19=>'S',20=>'S',21=>'S'));//2005.09.28ALP
            $objUp->setSize(array(7=>3,8=>3,9=>3,10=>3,11=>3,12=>3,13=>3,14=>3,15=>3,16=>3,17=>3,18=>3,19=>2,20=>2,21=>5));//2005.09.28ALP
            ...
            foreach ($ctrl_name as $code => $col) {
                ...
                $edit_flg = true;  //テキストボックス表示フラグ
                $sem = substr($code, 1, 1);
                $sem_val = substr($code, 3, 1);//---2005.06.08Add
...
#                              || strtotime($row["TRANSFER_EDATE"]) >= strtotime($model->control["学期終了日付"][$sem])) {
                ...
                //欠課時数情報('-','=')---2005.06.08Add
                //NO002
//              if (strlen($row[$col."_DI"]) && $sem != "8" && $sem_val == "2") $row[$col] = $row[$col."_DI"];
                if (strlen($row[$col."_DI"]) && $sem_val == "2") {
                    $row[$col] = $row[$col."_DI"];
                }

                //出欠情報---2005.05.24Modify
...
#                } elseif(AUTHORITY == DEF_UPDATABLE || in_array($code, $admin_key)) {      //2005/05/23
                    ...
                    //出欠情報がある場合はそれを表示
                    $value = $row[$col];//---2005.05.24Modify
                    ...
                    //考査満点マスタ
                    $query = knjd124aQuery::getPerfect(CTRL_YEAR, $model->field["SUBCLASSCD"], $perfectcd[$code], $row["GRADE"], $row["COURSE"], $model);
            ...
            //CSV書き出し
            $csv[] = $model->lastColumn;//2005.09.28ALP
        ...
        foreach ($ctrl_name as $code => $col) {
            if (isset($term_data[$col])) {
                ...
                //平均
                $arg[$col."_AVG"]=round((array_sum($term_data[$col])/get_count($term_data[$col]))*10)/10;//2005.09.28ALP
                //最高点と最低点を求める
        ...
        //累積現在日
        $cur_date = $db->getRow(knjd124aQuery::getMax($model->field["SUBCLASSCD"], $model), DB_FETCHMODE_ASSOC);
        ...
        foreach ($testcdArray as $testkey => $testval) {
            ...
            if ($model->useRecordChkfinDat == "1") {
                $query = knjd124aQuery::getRecordChkfinDat($model, $testval, $recordDiv);
            } else {
                $query = knjd124aQuery::getRecordSchChairTestDat($model, $testval, $recordDiv);
            }
        ...
        $calcFlg1 = false;
        foreach ($testcdArray as $testkey => $testval) {
            if (isset($admin_key2[$testkey])) {
                $calcFlg1 = true;
            }
        }
        //学年成績が入力可か。
        $testcdArray = array("0882" => "99900");
        $calcFlg2 = false;
        foreach ($testcdArray as $testkey => $testval) {
            if (isset($admin_key2[$testkey])) {
                $calcFlg2 = true;
        ...
        //算出ボタン
        $disCalc = ($calcFlg1 && $calcFlg2) ? "" : " disabled";
        $extra = "onclick=\"return clickedBtnCalc();\"".$disCalc;
...
//                            "value"     => ($model->field["SUBCLASSCD"] == $model->gen_ed ? $model->gen_ed : "" ) ) );
        ...
        $objForm->ae(array("type"      => "hidden",
                            "name"      => "PRGID",
                            "value"     => "KNJD124A" ));//---2005.05.24Modify
        ...
        View::toHTML($model, "knjd124aForm1.html", $arg);
    }
}
