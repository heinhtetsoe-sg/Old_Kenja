...
function clickedBtnCalc() {
    ...
    var avg;
    for (j = 0; j < lineCnt; j++) { //各生徒をループ
        total = 0;
        bunbo = 0;
        for (k = 0; k < nameArray.length; k++) { //各素点をループ
            scoreObject = eval("document.forms[0][\"" + nameArray[k] + "-" + j + "\"]");
            if (scoreObject) {
                //ブランクはスルー
                if (scoreObject.value == ''){
                    continue;
                }
                //数字であるのかチェック
                if (isNaN(scoreObject.value)){
                    continue;
                }
        ...
        if (0 < bunbo) {
            avg = total / bunbo;
            avg = Math.floor(avg); //切り捨て
        } else {
            avg = "";
        }
        gradObject = eval("document.forms[0][\"" + "GRAD_SCORE" + "-" + j + "\"]");
        gradObject.value = avg; //学年成績
    }
}
