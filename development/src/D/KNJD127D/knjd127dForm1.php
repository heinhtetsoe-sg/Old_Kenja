...
class knjd127dForm1
{
    public function main(&$model)
    {
        ...
        $arg["start"]    = $objForm->get_start("main", "POST", "knjd127dindex.php", "", "main");
        $arg["YEAR"]     = CTRL_YEAR;
        ...
        $query = knjd127dQuery::getTestName($model);
...
//                        "0322" => $semename[3]."－".$testname[30201]."－評価",
                        "0883" => "学年評価",
                        "0882" => "学年評定",
                        "0884" => "履修単位",
                        "0885" => "修得単位",
        ...
        $result = $db->query(knjd127dQuery::selectSubclassQuery($model->gen_ed, $model));
        //教育課程対応
        ...
        $result = $db->query(knjd127dQuery::selectChairQuery($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
//                        ,"0322" => "SEM3_TERM_VALUE"
        ...
        $ctrl_name[] = "SEM1_INTR_LEVEL";
        $ctrl_name[] = "SEM1_LEVEL";
        $ctrl_name[] = "SEM2_INTR_LEVEL";
        $ctrl_name[] = "SEM2_LEVEL";
        ...
        $admin_key = array();
        $result = $db->query(knjd127dQuery::selectContolCodeQuery());
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($seme_test != "0") {
            $result = $db->query(knjd127dQuery::selectExecuteDateQuery($model, $seme_test, $kind_test, $item_test));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //遅刻何回で欠課とするかの指数取得
        $absent = array();
        $absent = $db->getRow(knjd127dQuery::getScAbsentCov($model), DB_FETCHMODE_ASSOC);

        //文字評定
        if ($model->gen_ed == substr($model->field["SUBCLASSCD"], 0, 2)) {
            $assess = array();
            $result = $db->query(knjd127dQuery::getAssessMark());
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $offdaysFlg = $db->getRow(knjd127dQuery::getOffdaysFlg(CTRL_YEAR), DB_FETCHMODE_ASSOC);
        //帳票パラメータ
        ...
        if ($absent["ABSENT_COV"] == "0" || $absent["ABSENT_COV"] == "2" || $absent["ABSENT_COV"] == "4" || $absent["ABSENT_COV"] == "5") {
            $result = $db->query(knjd127dQuery::getAttendData($model->field["CHAIRCD"], $model->field["SUBCLASSCD"], $absent["ABSENT_COV"], $absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        } else {
            $result = $db->query(knjd127dQuery::getAttendData2($model->field["CHAIRCD"], $model->field["SUBCLASSCD"], $absent["ABSENT_COV"], $absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        }
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //合併先科目の単位固定／加算フラグ
        $model->creditflg = $db->getOne(knjd127dQuery::getCalculateCreditFlg($model->field["SUBCLASSCD"], $model));

        //一覧表示
        $colorFlg = false;
        $result = $db->query(knjd127dQuery::selectQuery($model, $execute_date));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //5評価
            if (strlen($row["SEM1_INTR_VALUE"])) {
                $query = knjd127dQuery::getAssessMstMark($row["SEM1_INTR_VALUE"]);
                $row["SEM1_INTR_LEVEL"] = $db->getOne($query);
            }
            if (strlen($row["SEM1_VALUE"])) {
                $query = knjd127dQuery::getAssessMstMark($row["SEM1_VALUE"]);
                $row["SEM1_LEVEL"] = $db->getOne($query);
            }

            if (strlen($row["SEM2_INTR_VALUE"])) {
                $query = knjd127dQuery::getAssessMstMark($row["SEM2_INTR_VALUE"]);
                $row["SEM2_INTR_LEVEL"] = $db->getOne($query);
            }
            if (strlen($row["SEM2_VALUE"])) {
                $query = knjd127dQuery::getAssessMstMark($row["SEM2_VALUE"]);
                $row["SEM2_LEVEL"] = $db->getOne($query);
            }
            ...
            foreach ($ctrl_name as $code => $col) {
                ...
                //管理者コントロール
                    ...
                    //考査満点マスタ
                    $query = knjd127dQuery::getPerfect(CTRL_YEAR, $model->field["SUBCLASSCD"], $perfectcd[$code], $row["GRADE"], $row["COURSE"], $model);
        ...
        //累積現在日
        $cur_date = $db->getRow(knjd127dQuery::getMax($model->field["SUBCLASSCD"], $model), DB_FETCHMODE_ASSOC);
        ...
        foreach ($testcdArray as $testkey => $testval) {
            ...
            if ($model->useRecordChkfinDat == "1") {
                $query = knjd127dQuery::getRecordChkfinDat($model, $testval, $recordDiv);
            } else {
                $query = knjd127dQuery::getRecordSchChairTestDat($model, $testval, $recordDiv);
            }
...
//                          "value"     => $semename[3] ) );
        ...
        View::toHTML($model, "knjd127dForm1.html", $arg);
    }
}
