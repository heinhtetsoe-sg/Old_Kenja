...
class knjd615iForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd615iForm1", "POST", "knjd615iindex.php", "", "knjd615iForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //学期コンボ作成
        $query = knjd615iQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd615i')\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        if ($model->Properties["use_school_detail_gcm_dat"] == '1') {
            $arg["USE_MAJOR"] = '1';
            //学科名コンボ
            $query = knjd615iQuery::getCourseMajor($model);
            $extra = "onchange=\"return btn_submit('knjd615i');\"";
            makeCmb($objForm, $arg, $db, $query, "MAJOR", $model->field["MAJOR"], $extra, 1);
        }

        //学年コンボ作成
        $query = knjd615iQuery::getGradeHrClass($model->field["SEMESTER"], $model, "GRADE");
        $extra = "onchange=\"return btn_submit('knjd615i')\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        //テストコンボ作成
        $query = knjd615iQuery::getTest($model, $model->field["SEMESTER"], $model->field["GRADE"]);
        ...
        View::toHTML($model, "knjd615iForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["PRINT_DIV"] == 1) {
        //対象クラスリストを作成する
        $query = knjd615iQuery::getGradeHrClass($model->field["SEMESTER"], $model, "HR_CLASS_LIST");
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD615I");
