...
class knjd176dForm1 {
    function main(&$model){
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd176dForm1", "POST", "knjd176dindex.php", "", "knjd176dForm1");
        //年度テキスト
        ...
        /**********/
        //クラス選択コンボボックスを作成する/
        $query = knjd176dQuery::getAuth($model);
        $opt = array();
        ...
        $extra = "onchange=\"return btn_submit('knjd176d'),AllClearList();\"";
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd176d')\"".$disabled;
        ...
        /******************/
        //生徒一覧
        $query = knjd176dQuery::getSchnoIdou($model,$ga);
        $result = $db->query($query);
        $opt_idou = array();
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        //対象者リストを作成する
        $query = knjd176dQuery::getTaisyousyaList($model);
        $result = $db->query($query);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJD176D");
        ...
        View::toHTML($model, "knjd176dForm1.html", $arg);
    }
}
?>
