...
class knjd105qForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd105qForm1", "POST", "knjd105qindex.php", "", "knjd105qForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        $extra = array("id=\"CATEGORY_IS_CLASS1\" onClick=\"return btn_submit('knjd105q')\"", "id=\"CATEGORY_IS_CLASS2\" onClick=\"return btn_submit('knjd105q')\"");
        ...
        //学期コンボ作成
        $query = knjd105qQuery::getSemester($model);
        $extra = "onchange=\"return btn_submit('change_seme'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        ($model->Properties["use_school_detail_gcm_dat"] = '1') ;
        if ($model->Properties["use_school_detail_gcm_dat"] == '1') {
            $arg["USE_MAJOR"] = '1';
            //学科名コンボ
            $query = knjd105qQuery::getCourseMajor($model);
            $extra = "onchange=\"return btn_submit('change_major');\"";
            makeCmb($objForm, $arg, $db, $query, "MAJOR", $model->field["MAJOR"], $extra, 1);
        }

        //テストコンボ作成
        $query = knjd105qQuery::getTest($model);
        $extra = "onchange=\"return btn_submit('change_testcd'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "TESTCD", $model->field["TESTCD"], $extra, 1);

        //学年コンボ作成
        $query = knjd105qQuery::getGradeHrClass($model->field["SEMESTER"], $model, "GRADE");
        $extra = "onchange=\"return btn_submit('change_grade'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        if ($model->field["CATEGORY_IS_CLASS"] == 2){
            //クラスコンボ作成
            $query = knjd105qQuery::getGradeHrClass($model->field["SEMESTER"], $model, "HR_CLASS");
            $extra = "onchange=\"return btn_submit('knjd105q'), AllClearList();\"";
...
//        $arg["data"]["HOGOSHA_PRINT"] = knjCreateCheckBox($objForm, "HOGOSHA_PRINT", "1", $extra, "");
        ...
        $model->field["SDATE_DEFAULT"] = str_replace('-', '/', knjd105qQuery::getBeforeTestNextDate($db, $model));
        if ($model->field["SDATE"] == '' || $model->cmd == 'change_seme' || $model->cmd == 'change_testcd' || $model->cmd == 'change_major') {
            $model->field["SDATE"] = $model->field["SDATE_DEFAULT"];
        }
        $arg["data"]["SDATE"] = View::popUpCalendar($objForm, "SDATE", $model->field["SDATE"]);
        $model->field["EDATE_DEFAULT"] = str_replace('-', '/', knjd105qQuery::getEdate($db, $model));
        if ($model->field["EDATE"] == '' || $model->cmd == 'change_seme' || $model->cmd == 'change_testcd' || $model->cmd == 'change_major') {
            $model->field["EDATE"] =  $model->field["EDATE_DEFAULT"];
        }
        ...
        View::toHTML($model, "knjd105qForm1.html", $arg); 
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["CATEGORY_IS_CLASS"] == 1){
        ...
        //対象外の生徒取得
        $query = knjd105qQuery::getSchnoIdou($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt_idou = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //対象者リストを作成する
        $query = knjd105qQuery::getStudent($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt1 = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $idou = "　";
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD105Q");
    ...
    knjCreateHidden($objForm, "SDATE_DEFAULT", $model->field["SDATE_DEFAULT"]);
    knjCreateHidden($objForm, "EDATE_DEFAULT", $model->field["EDATE_DEFAULT"]);
    knjCreateHidden($objForm, "YEAR_SDATE", CTRL_YEAR."/04/01");
    knjCreateHidden($objForm, "YEAR_EDATE", (CTRL_YEAR + 1)."/03/31");
