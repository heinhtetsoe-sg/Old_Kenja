...
class knjd124hForm1
{
    public function main(&$model)
    {
        ...
        $arg["start"]    = $objForm->get_start("main", "POST", "knjd124hindex.php", "", "main");
        $arg["YEAR"]     = CTRL_YEAR;
        ...
        $result = $db->query(knjd124hQuery::selectSubclassQuery($model->gen_ed, $model));
        //教育課程対応
        ...
        $result = $db->query(knjd124hQuery::selectChairQuery($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $query = knjd124hQuery::getTestName($model);
        ...
        $ctrl_name = array("1111" => "SEM1_INTR_SCORE"
                          ...
                          ,"1902" => "SEM1_VALUE"
                          ,"1912" => "SEM1_VALUE_KARI"
                          ,"2111" => "SEM2_INTR_SCORE"
                          ,"2211" => "SEM2_TERM_SCORE"
                          ,"2902" => "SEM2_VALUE"
                          ,"2912" => "SEM2_VALUE_KARI"
                          ,"3211" => "SEM3_TERM_SCORE"
                          ,"9921" => "GRAD_SCORE_HEIJOU"
                          ,"9912" => "GRAD_SCORE"
                          ,"9902" => "GRAD_VALUE"
                          ,"9903" => "COMP_CREDIT"
                          ,"9904" => "GET_CREDIT");
        ...
        $result = $db->query(knjd124hQuery::selectContolCodeQuery());
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            if ($row["CONTROL_CODE"] == "1902" || $row["CONTROL_CODE"] == "2902" || $row["CONTROL_CODE"] == "1912" || $row["CONTROL_CODE"] == "2912") {
                continue;
            }
        ...
        if ($seme_test != "0") {
            $result = $db->query(knjd124hQuery::selectExecuteDateQuery($model, $seme_test, $kind_test, $item_test));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                $execute_date = $row["TEST_END_DATE"];
            }
        }
        //echo "CTRL_DATE = " . CTRL_DATE . "<BR>";
        //echo "sdate = " . $sdate . "<BR>";
        //echo "edate = " . $edate . "<BR>";
        //echo "execute_date = " . $execute_date . "<BR>";
        //遅刻何回で欠課とするかの指数取得
        $absent = array();
        $absent = $db->getRow(knjd124hQuery::getScAbsentCov($model), DB_FETCHMODE_ASSOC);

        //文字評定
        if ($model->gen_ed == substr($model->field["SUBCLASSCD"], 0, 2)) {
            $assess = array();
            $result = $db->query(knjd124hQuery::getAssessMark());
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $offdaysFlg = $db->getRow(knjd124hQuery::getOffdaysFlg(CTRL_YEAR), DB_FETCHMODE_ASSOC);
        //帳票パラメータ
        ...
        if ($absent["ABSENT_COV"] == "0" || $absent["ABSENT_COV"] == "2" || $absent["ABSENT_COV"] == "4" || $absent["ABSENT_COV"] == "5") {
            $result = $db->query(knjd124hQuery::getAttendData($model->field["CHAIRCD"], $model->field["SUBCLASSCD"], $absent["ABSENT_COV"], $absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        } else {
            $result = $db->query(knjd124hQuery::getAttendData2($model->field["CHAIRCD"], $model->field["SUBCLASSCD"], $absent["ABSENT_COV"], $absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        }
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //合併先科目の単位固定／加算フラグ
        $model->creditflg = $db->getOne(knjd124hQuery::getCalculateCreditFlg($model->field["SUBCLASSCD"], $model));

        //一覧表示
        $controlFlgBtnCalc = '';
        $controlFlgBtnUpd = '';
        $colorFlg = false;
        $result = $db->query(knjd124hQuery::selectQuery($model, $execute_date));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //学籍番号をHiddenで保持
            ...
            $objUp->setSize(array(7=>3,8=>3,9=>3,10=>3,11=>3,12=>3,13=>3,14=>3,15=>3,16=>3,17=>3,18=>3,19=>2,20=>2,21=>5));
            ...
            foreach ($ctrl_name as $code => $col) {
                ...
                if (!$edit_flg3 || !$edit_flg && $model->auth != DEF_UPDATABLE || ($code == "1902" || $code == "2902" || $code == "1912" || $code == "2912")) {
                    $setTextField .= "";
            ...
            foreach ($ctrl_name as $code => $col) {
                ...
                if (!$edit_flg3 || !$edit_flg && $model->auth != DEF_UPDATABLE || ($code == "1902" || $code == "2902" || $code == "1912" || $code == "2912")) {
                    $value = $row[$col];
                    if ($code == "1902" || $code == "2902" || $code == "9912") {
                        $spanId = $col."_SPAN_ID"."-".$counter;
                        $row[$col] = "<font color=\"#000000\"><span id=\"{$spanId}\">{$value}</span></font>";
                    } else {
                        $row[$col] = "<font color=\"#000000\">{$value}</font>";
                    }
                    //hidden
                    knjCreateHidden($objForm, $col."-".$counter, $value);
                //管理者コントロール
                } elseif (in_array($code, $admin_key)) {
                    ...
                    $objForm->ae(array("type"      => "text",
                                        ...
                                        "value"     => $value,
                                        "extrahtml" => " onKeyDown=\"keyChangeEntToTab2(this, '{$setTextField}', {$counter})\"; STYLE=\"text-align: right; width:30\" onChange=\"this.style.background='#ccffcc'; document.forms[0].changeVal.value = '1';\" onblur=\"calc(this);\" onKeyDown=\"keyChangeEntToTab(this)\" onPaste=\"return showPaste(this);\" id=\"{$col}-{$counter}\""));
                    $row[$col] = $objForm->ge($col."-".$counter);

                    //考査満点マスタ
                    $query = knjd124hQuery::getPerfect(CTRL_YEAR, $model->field["SUBCLASSCD"], $perfectcd[$code], $row["GRADE"], $row["COURSE"], $model);
            ...
            //入力エリアとキーをセットする(LASTCOLUMN)
            $objUp->setElementsValue($model->lastColumn."-".$counter, $model->lastColumn, $key);
            knjCreateHidden($objForm, $model->lastColumn."-".$counter, $model->lastColumn);

            //学年・・・総合点算出用
            knjCreateHidden($objForm, "HIDDEN_GRADE"."-".$counter, $row["GRADE"]);
        ...
        //累積現在日
        $cur_date = $db->getRow(knjd124hQuery::getMax($model->field["SUBCLASSCD"], $model), DB_FETCHMODE_ASSOC);
        ...
        foreach ($testcdArray as $testkey => $testval) {
            ...
            if ($model->useRecordChkfinDat == "1") {
                $query = knjd124hQuery::getRecordChkfinDat($model, $testval, $recordDiv);
            } else {
                $query = knjd124hQuery::getRecordSchChrTest($model, $testval);
            }
        ...
        ********************************************/
        ...
        //仮評定算出ボタン
        $isControlFlg1 = in_array("1111", $admin_key) || in_array("1211", $admin_key) ? true : false;
        $isControlFlg2 = in_array("2111", $admin_key) || in_array("2211", $admin_key) ? true : false;
        $disBtnCalc2 = (0 < $counter && ($isControlFlg1 || $isControlFlg2)) ? "" : " disabled ";
        $extra = "style=\"width:85px\" onclick=\"btnCalc('kari', '{$isControlFlg1}', '{$isControlFlg2}');\"";
        $arg["btn_calc2"] = knjCreateBtn($objForm, "btn_calc2", "仮評定算出", $disBtnCalc2.$extra);
        //総合点算出ボタン
        $extra = "style=\"width:85px\" onclick=\"btnCalc('sogo', '{$isControlFlg1}', '{$isControlFlg2}');\"";
        $arg["btn_calc"] = knjCreateBtn($objForm, "btn_calc", "総合点算出", $disBtnCalc.$extra);
        ...
        //科目別評定マスタ（ASSESS_SUBCLASS_MST）使用
        ...
        View::toHTML($model, "knjd124hForm1.html", $arg);
    }
}
