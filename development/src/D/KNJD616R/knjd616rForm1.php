...
class knjd616rForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd616rForm1", "POST", "knjd616rindex.php", "", "knjd616rForm1");
        ...
        //学期コンボ作成
        $query = knjd616rQuery::getSemester(0);
        $extra = "onchange=\"return btn_submit('knjd616rChseme'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);
        //学年末は今学期とする
        $seme = $model->field["SEMESTER"] == "9" ? CTRL_SEMESTER : $model->field["SEMESTER"];
        $query = knjd616rQuery::getSemester("", $model->field["SEMESTER"]);
        ...
        //学年コンボ作成
        $query = knjd616rQuery::getSchoolName();
        $schoolName = $db->getOne($query);
        $query = knjd616rQuery::getGrade($seme);
        $extra = "onchange=\"return btn_submit('knjd616r'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        //テストコンボ作成
        $query = knjd616rQuery::getTest($model->field["SEMESTER"], $model->field["GRADE"]);
        makeCmb($objForm, $arg, $db, $query, "TESTKINDCD", $model->field["TESTKINDCD"], "", 1);

        //出欠集計日付作成
        if ($model->field["SDATE"] == "" || $model->cmd == 'knjd616rChseme') {
        ...
        View::toHTML($model, "knjd616rForm1.html", $arg); 
    }
}
...
function makeClassList(&$objForm, &$arg, $db, $model, $seme) {
    ...
    //クラス一覧リストを作成する
    $query = knjd616rQuery::getHrClass($seme, $model->field["GRADE"]);
    $result = $db->query($query);
    $opt1 = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model, $seme) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD616R");
