...
class knjd681jForm1 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]  = $objForm->get_start("knjd681jForm1", "POST", "knjd681jindex.php", "", "knjd681jForm1");
        ...
        //学期コンボ
        $extra = "onChange=\"return btn_submit('knjd681j');\"";
        $query = knjd681jQuery::getSemester();
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //学年コンボ
        $extra = "onChange=\"return btn_submit('knjd681j');\"";
        $query = knjd681jQuery::getGrade();
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        //年組コンボ
        $extra = "onChange=\"return btn_submit('knjd681j');\"";
        $query = knjd681jQuery::getHrclass($model);
        ...
        View::toHTML($model, "knjd681jForm1.html", $arg); 
    }
}
...
{
    $row1 = array();
    $query = knjd681jQuery::getStudent($model);
    $result = $db->query($query);
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
...
function makeHidden(&$objForm, $model, $db) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD681J");
}
?>
