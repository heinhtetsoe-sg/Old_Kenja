...
class knjd155Form1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjd155index.php", "", "main");
        ...
        //学期コンボ作成
        $query = knjd155Query::getSemesterList();
        $extra = "onchange=\"return btn_submit('main');\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //テスト種別コンボ作成
        $query = knjd155Query::getTestList($model);
        $extra = "onchange=\"return btn_submit('main');\"";
        makeCmb($objForm, $arg, $db, $query, "TESTKINDCD", $model->field["TESTKINDCD"], $extra, 1, 1);

        //年組コンボ作成
        $query = knjd155Query::getGradeHrclass($model);
        $extra = "onchange=\"return btn_submit('main');\"";
        ...
        //一覧表示
        $colorFlg = false;
        $result = $db->query(knjd155Query::selectQuery($model));
        ...
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            $row["REMARK"] = KnjCreateTextArea($objForm, "REMARK-".$counter, $model->hexam_record_remark_dat_remark1_gyou, ($model->hexam_record_remark_dat_remark1_moji * 2 + 1), "soft", "style=\"height:{$height}px;\" onPaste=\"return showPaste(this);\"", $value);
        ...
        View::toHTML($model, "knjd155Form1.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg, $model) {
    ...
    //一括更新ボタンを作成する
    $link = REQUESTROOT."/D/KNJD155/knjd155index.php?cmd=replace&SEMESTER=".$model->field["SEMESTER"]."&GRADE_HR_CLASS=".$model->field["GRADE_HR_CLASS"]."&TESTKINDCD=".$model->field["TESTKINDCD"];
