...
class knjd186uForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd186uForm1", "POST", "knjd186uindex.php", "", "knjd186uForm1");
        ...
        //学期コンボ
        $extra = "onChange=\"return btn_submit('knjd186u');\"";
        $query = knjd186uQuery::getSemester();
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //表示指定ラジオボタン 1:クラス 2:個人
        $opt_disp = array(1, 2);
        $model->field["DISP"] = ($model->field["DISP"] == "") ? "1" : $model->field["DISP"];
        $extra = array("id=\"DISP1\" onClick=\"return btn_submit('knjd186u')\"", "id=\"DISP2\" onClick=\"return btn_submit('knjd186u')\"");
        ...
        if ($model->field["DISP"] == 1) {
            //学年コンボ
            $extra = "onChange=\"return btn_submit('knjd186u');\"";
            $query = knjd186uQuery::getGrade($model);
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
        } else if ($model->field["DISP"] == 2) {
            //年組コンボ
            $extra = "onChange=\"return btn_submit('knjd186u');\"";
            $query = knjd186uQuery::getHrClass($model);
        ...
        //学期末評価を出力しない(チェックボックス)
        //最終学期の時有効
        $query = knjd186uQuery::getMaxSemester($model);
        $maxSemester = $db->getOne($query);
        $disabled = "";
//        if ($maxSemester != $model->field["SEMESTER"]) {
//            $disabled = " disabled ";
//        }
        $extra  = "id=\"GRADING_PRINT\"";
        $extra .= ($model->field["GRADING_PRINT"] == "1") ? " checked " : "";
        $arg["data"]["GRADING_PRINT"] = knjCreateCheckBox($objForm, "GRADING_PRINT", "1", $extra.$disabled);
        ...
        View::toHTML($model, "knjd186uForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["DISP"] == 2) {
        //生徒取得
        $query = knjd186uQuery::getSchList($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //クラス名称調整
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD186U");
