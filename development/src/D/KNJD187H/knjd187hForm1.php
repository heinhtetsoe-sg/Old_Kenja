...
class knjd187hForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd187hForm1", "POST", "knjd187hindex.php", "", "knjd187hForm1");
        ...
        //学期コンボの設定
        $query = knjd187hQuery::getSemester();
        $extra = "onchange=\"return btn_submit('gakki'),AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["GAKKI"], "GAKKI", $extra, 1);

        //クラス選択コンボボックスを作成する
        $query = knjd187hQuery::getAuth($model);
        $extra = "onchange=\"return btn_submit('knjd187h'),AllClearList();\"";
        ...
        View::toHTML($model, "knjd187hForm1.html", $arg); 
    }
}
...
{
    $query = knjd187hQuery::getSchnoIdou($model);
    $result = $db->query($query);
    $opt_idou = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    //対象者リストを作成する
    $result = $db->query(knjd187hQuery::getListStudent($model));
    $opt1 = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD187H");
    ...
    knjCreateHidden($objForm, "KNJD187H_Pattern", $model->Properties["KNJD187H_Pattern"]);
