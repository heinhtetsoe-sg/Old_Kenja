...
class knjd425n_4Form1
{
    function main(&$model)
    {
        ...
        $arg = array();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("subform", "POST", "knjd425n_4index.php", "", "subform");
        ...
        //所感タイトル
        $arg["data"]["TITLE"] = $db->getOne(knjd425n_4Query::getHreportGuidanceKindNameHdat($model));
        //テーブルタイトル
        $arg["data"]["TABLE_TITLE"] = $db->getOne(knjd425n_4Query::getHreportGuidanceKindNameDat($model));

        //更新用の科目コード配列を初期化
        $model->subclasscdarry = array();

        //データ取得
        $query = knjd425n_4Query::getDetailRemark($model);
        ...
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            if ($row["SUBCLASSNAME"] !== null && $row["SUBCLASSNAME"] !== "") {
                ...
                //前年度「年間まとめ」参照 ボタン
                $extra = " onclick=\"loadwindow('knjd425n_4index.php?";
        ...
        View::toHTML($model, "knjd425n_4Form1.html", $arg);
    }
}
?>
