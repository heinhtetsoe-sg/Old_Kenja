...
class knjd131iSubForm2 {
    function main(&$model) {
        $objForm = new form;
        $db = Query::dbCheckOut();
        $arg = array();
        $arg["start"]   = $objForm->get_start("subform2", "POST", "knjd131iindex.php", "", "subform2");
        ...
        //委員会リスト
        $query = knjd131iQuery::getCommittee($model);
        $result = $db->query($query);
        $i = 0;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $row = str_replace("-","/",$row);
            $row["COMMITTEENAME_CHARGENAME"] = $row["SEQ"] . ":" . $row["NAME1"] . "　" . $row["COMMITTEENAME"] . " ／ " . $row["CHARGENAME"];
            //全てログイン年度のデータの為、初期値でチェック有
        ...
        View::toHTML($model, "knjd131iSubForm2.html", $arg);
    }
}
?>
