...
class knjd131iSubForm1 {
    function main(&$model) {
        ...
        $query = knjd131iQuery::getClub($model);
        $result = $db->query($query);
        $i = 0;
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $row = str_replace("-","/",$row);
            $row["CLUBNAME"]      = $row["CLUBNAME"];
            //部活動の在籍期間がログイン年度の場合、初期値でチェック有
            if ($row["SYEAR"]  <= CTRL_YEAR && ($row["EYEAR"] >= CTRL_YEAR || $row["EYEAR"] == "")) {
        ...
        View::toHTML($model, "knjd131iSubForm1.html", $arg);
    }
}
?>
