...
class knjd131iSubForm3 {
    function main(&$model) {
        $objForm = new form;
        $db = Query::dbCheckOut();
        $arg = array();
        $arg["start"]   = $objForm->get_start("subform1", "POST", "knjd131iindex.php", "", "subform1");
        
        //年度、学期、クラス表示
        $setYear = CTRL_YEAR.'年度';
        $setSemester = $db->getOne(knjd131iQuery::getSemesterName());
        $setHrClass = $db->getOne(knjd131iQuery::getHrClassName($model));
        $arg["YEAR"] = $setYear;
        ...
        if ($model->cmd !== 'replace') {
            ...
            $query = knjd131iQuery::getCommunication($model->field, $model->schregno);
        ...
        //戻るボタン
        $link = REQUESTROOT."/D/KNJD131I/knjd131iindex.php?cmd=back";
        $objForm->ae( array("type"        => "button",
                            "name"        => "btn_back",
                            "value"       => "戻 る",
        ...
        //部活動・委員会一括更新
        $extra = "onclick=\"return btn_submit('setupdate')\"";
        $arg["btn_setupdate"] = knjCreateBtn($objForm, "btn_setupdate", "部活動・委員会取込更新", $extra);
        ...
        View::toHTML($model, "knjd131iSubForm3.html", $arg);
    }
}
/********************************************** 以下関数 ***********************************************/
//クラス一覧リストToリスト作成
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    $query = knjd131iQuery::getStudent($model);
    $result = $db->query($query);
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
