...
class knjd210oForm1
{
    function main(&$model)
    {
        $db = Query::dbCheckOut();
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("main", "POST", "knjd210oindex.php", "", "main");
        ...
        $result = $db->query(knjd210oQuery::GetSemester());
        ...
        $result = $db->query(knjd210oQuery::GetGrade($seme));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        //処理種別(成績)
        $opt_exam = array();
        $result = $db->query(knjd210oQuery::GetName($model->seme));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        View::toHTML($model, "knjd210oForm1.html", $arg);
    }
}
?>
