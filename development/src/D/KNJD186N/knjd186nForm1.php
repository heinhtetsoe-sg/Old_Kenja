...
class knjd186nForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd186nForm1", "POST", "knjd186nindex.php", "", "knjd186nForm1");
        ...
        //学期コンボ作成
        $query = knjd186nQuery::getSemester($getCountsemester);
        $extra = "onchange=\"return btn_submit('knjd186n');\"";
        ...
        //クラスコンボ作成
        $query = knjd186nQuery::getHrClass(CTRL_YEAR, $seme, $model);
        $extra = "onchange=\"return btn_submit('knjd186n');\"";
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd186n', 'on')\"".$disabled;
        $date_textbox = knjCreateTextBox($objForm, $model->field["DATE"], "DATE", 12, 12, $extra);
        //異動対象日付（カレンダー）
        global $sess;
        $extra = "onclick=\"tmp_list('knjd186n', 'off'); loadwindow('" .REQUESTROOT ."/common/calendar.php?name=DATE&frame='+getFrameName(self) + '&date=' + document.forms[0]['DATE'].value + '&CAL_SESSID=$sess->id&reload=true', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 200)\"";
        ...
        View::toHTML($model, "knjd186nForm1.html", $arg);
    }
}

function makeStudentList(&$objForm, &$arg, $db, $model, $seme) {
    //対象外の生徒取得
    $query = knjd186nQuery::getSchnoIdou($model, $seme);
    ...
    //対象者リストを作成する
    $query = knjd186nQuery::getStudent($model, $seme);
    $result = $db->query($query);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD186N");
    ...
    //knjCreateHidden($objForm, "knjd186nAddAttendSubclassGetCredit", $model->Properties["knjd186nAddAttendSubclassGetCredit"]);
    //knjCreateHidden($objForm, "knjd186nNoPrintGakunenHyokaIfHyoteiNull", $model->Properties["knjd186nNoPrintGakunenHyokaIfHyoteiNull"]);
