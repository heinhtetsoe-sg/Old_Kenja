...
class knjd192rForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd192rForm1", "POST", "knjd192rindex.php", "", "knjd192rForm1");
        //DB接続
        $db = Query::dbCheckOut();

        //学校名取得
        $query = knjd192rQuery::getSchoolname();
        ...
        $extra = array("id=\"CATEGORY_IS_CLASS1\" onClick=\"return btn_submit('knjd192r')\"", "id=\"CATEGORY_IS_CLASS2\" onClick=\"return btn_submit('knjd192r')\"");
        ...
        //学期コンボ作成
        $query = knjd192rQuery::getSemester();
        $extra = "onchange=\"return btn_submit('change_grade'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //学科コンボ作成
        $query = knjd192rQuery::getCourseMajor($model);
        $extra = "onchange=\"return btn_submit('knjd192r');\"";
        makeCmb($objForm, $arg, $db, $query, "MAJOR", $model->field["MAJOR"], $extra, 1);

        //学年コンボ作成
        $query = knjd192rQuery::getGradeHrClass($model->field["SEMESTER"], $model, "GRADE");
        $extra = "onchange=\"return btn_submit('change_grade'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        if ($model->field["CATEGORY_IS_CLASS"] == 2){
            //クラスコンボ作成
            $query = knjd192rQuery::getGradeHrClass($model->field["SEMESTER"], $model, "HR_CLASS");
            $extra = "onchange=\"return btn_submit('knjd192r'), AllClearList();\"";
            makeCmb($objForm, $arg, $db, $query, "HR_CLASS", $model->field["HR_CLASS"], $extra, 1);
        }

        //テストコンボ作成
        $query = knjd192rQuery::getTest($model);
        $opt = array();
        ...
        $model->field["RETESTDATE"] = $model->field["RETESTDATE"] ? $model->field["RETESTDATE"] : str_replace('-', '/', CTRL_DATE);
        $arg["data"]["RETESTDATE"] = View::popUpCalendar($objForm, "RETESTDATE", $model->field["RETESTDATE"]);

        //リストToリスト作成
        makeStudentList($objForm, $arg, $db, $model);


        $model->field["KETTEN1"] = $model->field["KETTEN1"] ? $model->field["KETTEN1"] : 50;
        $model->field["KETTEN2"] = $model->field["KETTEN2"] ? $model->field["KETTEN2"] : 60;
        $extra = " style=\"text-align: right;\"  onblur=\"calc(this);\" id=\"KETTEN1\" ";
        $arg["data"]["KETTEN1"] = knjCreateTextBox($objForm, $model->field["KETTEN1"], "KETTEN1", 3, 3, $extra);
        $extra = " style=\"text-align: right;\"  onblur=\"calc(this);\" id=\"KETTEN2\" ";
        $arg["data"]["KETTEN2"] = knjCreateTextBox($objForm, $model->field["KETTEN2"], "KETTEN2", 3, 3, $extra);
        ...
        View::toHTML($model, "knjd192rForm1.html", $arg); 
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["CATEGORY_IS_CLASS"] == 1){
        ...
        //対象外の生徒取得
        $query = knjd192rQuery::getSchnoIdou($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt_idou = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //対象者リストを作成する
        $query = knjd192rQuery::getStudent($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt1 = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $idou = "　";
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD192R");
