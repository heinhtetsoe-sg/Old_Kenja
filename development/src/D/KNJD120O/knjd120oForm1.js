...
function calc(obj){
    ...
    var cnt2 = 0;
    //前期
    if (document.getElementById("SEM1_INTR_SCORE:"+a[1]) != null){
        var intr1 = document.getElementById("SEM1_INTR_SCORE:"+a[1]).value;
    }else{
        var intr1 = document.getElementById("REC").rows[row].cells[2].innerText;
    }
    if (!isNaN(intr1) && intr1 != ""){
        intr1 = parseInt(intr1);
        cnt++;
    }else{
        intr1 = 0;
    }
    if (document.getElementById("SEM1_TERM_SCORE:"+a[1]) != null){
        var term1 = document.getElementById("SEM1_TERM_SCORE:"+a[1]).value;
    }else{
        var term1 = document.getElementById("REC").rows[row].cells[3].innerText;
    }
    if (!isNaN(term1) && term1 != ""){
        term1 = parseInt(term1);
        cnt++;
    }else{
        term1 = 0;
    }
    document.getElementById("SEM1_AVG:"+a[1]).innerText = (term1 != "" && cnt > 0)? (intr1+term1)/cnt:"";
    //後期
    if (document.getElementById("SEM2_INTR_SCORE:"+a[1]) != null){
        var intr2 = document.getElementById("SEM2_INTR_SCORE:"+a[1]).value;
    }else{
        var intr2 = document.getElementById("REC").rows[row].cells[6].innerText;
    }
    if (!isNaN(intr2) && intr2 != ""){
        intr2 = parseInt(intr2);
        cnt2++;
    }else{
        intr2 = 0;
    }
    if (document.getElementById("SEM2_TERM_SCORE:"+a[1]) != null){
        var term2 = document.getElementById("SEM2_TERM_SCORE:"+a[1]).value;
    }else{
        var term2 = document.getElementById("REC").rows[row].cells[7].innerText;
    }
    if (!isNaN(term2) && term2 != ""){
        term2 = parseInt(term2);
        cnt2++;
    }else{
        term2 = 0;
    }
    document.getElementById("SEM2_AVG:"+a[1]).innerText = (term2 != "" && cnt2 > 0)? (intr2+term2)/cnt2:"";
        
    document.getElementById("ALL_AVG:"+a[1]).innerText = (term2 != "" && (cnt+cnt2) > 0)? Math.round((intr1+term1+intr2+term2)*10/(cnt+cnt2))/10:"";
}
        ...
        if (textFieldArray[i] + cnt == obj.name) {
            ...
            if (moveEnt == 37) {
                ...
                if (isFirstItem) {
                    ...
                    targetObject = document.getElementById(targetname.replace("-", ":"));
                    targetObject.focus();
                    return;
                }
