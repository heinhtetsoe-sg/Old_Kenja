...
class knjd420oForm1 {
    function main(&$model) {
        $objForm = new form;
        $arg["start"] = $objForm->get_start("edit", "POST", "knjd420oindex.php", "", "edit");
        ...
        $query = knjd420oQuery::getGuidancePattern($model);
        ...
        //学期コンボ
        $query = knjd420oQuery::getSemester($model);
        ...
        $query = knjd420oQuery::getItemName($model);
        $result = $db->query($query);
        ...
        //講座ごとの生徒一覧取得
        $model->schInfo = array();
        $query = knjd420oQuery::getChairSchreg($model);
        $result = $db->query($query);
        while ($schRow = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            if ($schRow["GUIDANCE_PATTERN"] && $model->subclasscd) {
                ...
                foreach ($model->itemNameArr as $itemNo => $item_remark) {
                    ...
                    $moji = 10;
                    $gyou = 25;
                    $val = $schRow["REMARK_{$itemNo}"];
                    if($model->cmd == 'edit' && $model->schField[$schRow["SCHREGNO"]]["REMARK"][$itemNo] != ""){
                        $val = $model->schField[$schRow["SCHREGNO"]]["REMARK"][$itemNo];
                    }
                    $tmpData["REMARK"] = knjCreateTextArea($objForm, $setName, "10", $moji * 2, "", $extra, $val);                    $tmpData["REMARK"] = knjCreateTextArea($objForm, $setName, "10", $moji * 2, "", $extra, $val);
        ...
        knjCreateHidden($objForm, "PRGID", "knjd420o");
        ...
        View::toHTML($model, "knjd420oForm1.html", $arg);
    }
}
...
function makeList(&$arg, $db, &$model) {
    ...
    $query = knjd420oQuery::getList($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    foreach ($setArr as $key => $remarkArr) {
        ...
        //リンクセット
        $setData["SET_SUBCLASSNAME"] = "<a href=\"knjd420oindex.php?cmd=list_set&SEMESTER={$semester}&SUBCLASSCD={$remarkArr["SET_SUBCLASSCD"]}&UNITCD={$remarkArr["UNITCD"]}&SORT={$model->sort}\">{$remarkArr["SET_SUBCLASSNAME"]}</a>";
