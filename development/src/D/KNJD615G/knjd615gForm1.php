...
class knjd615gForm1
{
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd615gForm1", "POST", "knjd615gindex.php", "", "knjd615gForm1");
        ...
        //学期コンボボックスを作成する
        $query = knjd615gQuery::getSemester();
        ...
        //学校名取得
        $query = knjd615gQuery::getSchoolname();
        ...
        if ($model->cmd == 'change_grade' || $model->cmd == '') {
            $query = knjd615gQuery::getGdat($model->field["GRADE"]);
            $h_j = $db->getOne($query);
            if ($h_j == 'J') {
        ...
        $opt_rank = array(1, 2, 3, 4, 5); //1:学級はカットになった為htmlのところでカットした(帳票の関係で送る値を変えないため)
        ...
        //カレンダーコントロール
        $value = isset($model->field["SDATE"]) ? $model->field["SDATE"] : str_replace("-", "/", $model->control["学期開始日付"][9]);
        ...
        View::toHTML($model, "knjd615gForm1.html", $arg); 
    }
}
/**************************************** 以下関数 **************************************************/
//コンボ作成
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    //クラス一覧
    $row1 = array();
    $result = $db->query(knjd615gQuery::getAuth($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model, $seme, $semeflg)
{
    ...
    $objForm->ae(createHiddenAe("PRGID", "KNJD615G"));
    $objForm->ae(createHiddenAe("cmd"));
    //学期
    $objForm->ae(createHiddenAe("SEME_DATE", $seme));
    //学期開始日付
    $objForm->ae(createHiddenAe("YEAR_SDATE", $model->control["学期開始日付"]["9"]));
    //学期終了日付
    $objForm->ae(createHiddenAe("YEAR_EDATE", $model->control["学期終了日付"]["9"]));
    //学期開始日付
