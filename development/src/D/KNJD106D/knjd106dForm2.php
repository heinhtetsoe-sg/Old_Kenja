...
class knjd106dForm2
{
    function main(&$model)
    {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjd106dindex.php", "", "edit");
        //DB接続
        $db = Query::dbCheckOut();

        //警告メッセージを表示しない場合
        if (!isset($model->warning)) {
            $Row = $db->getRow(knjd106dQuery::getRow($model), DB_FETCHMODE_ASSOC);
        } else {
            $Row =& $model->field;
        }
        ...
        //コース名コンボ設定
        if ($model->grade) {
            $query = knjd106dQuery::getCourseList($model->grade);
        } else {
            $query = knjd106dQuery::getCourseList('01');
        }
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "parent.left_frame.location.href='knjd106dindex.php?cmd=list';";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。 
        View::toHTML($model, "knjd106dForm2.html", $arg); 
    }
}
