...
class knjd624bForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd624bForm1", "POST", "knjd624bindex.php", "", "knjd624bForm1");
        ...
        $query = knjd624bQuery::getSemester();
        makeCmb($objForm, $arg, $db, $query, $model->field["SEMESTER"], "SEMESTER", $extra, 1);

        //テスト種別コンボボックスを作成する
        $extra = "onchange=\"return btn_submit('knjd624b');\"";
        $query = knjd624bQuery::getTestcd($model);
        makeCmb($objForm, $arg, $db, $query, $model->field["TESTCD"], "TESTCD", $extra, 1);

        //学年コンボボックスを作成する
        $extra = "onChange=\"return btn_submit('knjd624b');\"";
        $query = knjd624bQuery::getSelectGrade($model);
        ...
        View::toHTML($model, "knjd624bForm1.html", $arg); 
    }
}
...
{
    $query = knjd624bQuery::getSubclass($model);
...
{
    //テスト名称
    $result = $db->query(knjd624bQuery::getTestcd($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $objForm->ae(createHiddenAe("TESTNAMES".$row["VALUE"], $row["LABEL"]));
    }
    $result->free();

    $objForm->ae(createHiddenAe("PRGID", "KNJD624B"));
