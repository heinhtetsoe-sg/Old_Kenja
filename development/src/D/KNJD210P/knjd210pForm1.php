...
class knjd210pForm1
{
    function main(&$model)
    {
        $db = Query::dbCheckOut();
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("main", "POST", "knjd210pindex.php", "", "main");
        ...
        $result = $db->query(knjd210pQuery::GetSemester());
        ...
        $result = $db->query(knjd210pQuery::GetGrade($seme));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        //処理種別(成績)
        $opt_exam = array();
        $result = $db->query(knjd210pQuery::GetName($model->seme));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        View::toHTML($model, "knjd210pForm1.html", $arg);
    }
}
?>
