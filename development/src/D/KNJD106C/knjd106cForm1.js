...
function newwin(SERVLET_URL){
	...
	if (document.forms[0].SUBMIT_DATE.value == ""){
		alert('日付を指定してください。');
		return false;
	}

	document.forms[0].SUBMIT_DATE.value = document.forms[0].SUBMIT_DATE.value.replace("/","-");
	document.forms[0].SUBMIT_DATE.value = document.forms[0].SUBMIT_DATE.value.replace("/","-");
...
function disJuniPrint() {
    if (document.forms[0].ASSESS_LEVEL_PRINT.checked) {
        document.forms[0].JUNI_PRINT.disabled = true;
    } else {
        document.forms[0].JUNI_PRINT.disabled = false;
    }
}
