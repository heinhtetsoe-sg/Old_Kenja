...
class knjd186rForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd186rForm1", "POST", "knjd186rindex.php", "", "knjd186rForm1");
        ...
        //学期コンボ作成
        $query = knjd186rQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd186rChangeSemester'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //クラスコンボ作成
        $query = knjd186rQuery::getGradeHrClass($model);
        $extra = "onchange=\"return btn_submit('knjd186r'), AllClearList();\"";
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd186r', 'on')\"".$disabled;
        $date_textbox = knjCreateTextBox($objForm, $model->field["DATE"], "DATE", 12, 12, $extra);
        //異動対象日付（カレンダー）
        global $sess;
        $extra = "onclick=\"tmp_list('knjd186r', 'off'); loadwindow('" .REQUESTROOT ."/common/calendar.php?name=DATE&frame='+getFrameName(self) + '&date=' + document.forms[0]['DATE'].value + '&CAL_SESSID=$sess->id&reload=true', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 200)\"";
        ...
        //表示も印字する
        $extra = " id=\"HYOUSI\" ";
        $model->semesterDiv = $db->getOne(knjd186rQuery::getSemesterdiv());
        if ($model->field["SEMESTER"] == '9' || $model->field["SEMESTER"] == $model->semesterDiv) {
            // 最終学期、学年末に変更したらON
            if ($model->cmd == '' || $model->cmd == 'knjd186rChangeSemester') {
        ...
        View::toHTML($model, "knjd186rForm1.html", $arg); 
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    //対象外の生徒取得
    $query = knjd186rQuery::getSchnoIdou($model);
    ...
    //対象者リストを作成する
    $query = knjd186rQuery::getSchno($model);
    $result = $db->query($query);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD186R");
