...
function chkPattern() {
    ...
    var i;
    var tab;
    dis = (document.forms[0].SEMESTER.value == document.forms[0].SEMESTER_DIV.value || document.forms[0].SEMESTER.value == '9') ? false : true;
    window.bks = window.bks || [];
    if (dis) {
        window.bks.elems = [];
        var ids = ["KAIKINTAB1", "KAIKINTAB2"];
        for (i = 0; i < ids.length; i++) {
            tab = document.getElementById(ids[i]);
            window.bks.elems[i] = [tab, tab.parentElement];
            tab.parentElement.removeChild(tab);
        }
    } else {
        if (window.bks && window.bks.elems) {
            for (i = 0; i < window.bks.elems.length; i++) {
                window.bks.elems[i][1].appendChild(window.bks.elems[i][0]);
            }
        }
    }
}
