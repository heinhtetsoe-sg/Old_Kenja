...
class knjd232vForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd232vForm1", "POST", "knjd232vindex.php", "", "knjd232vForm1");
        $db = Query::dbCheckOut();
        ...
        //学期コンボボックスを作成する
        $opt_seme = array();
        $query = knjd232vQuery::getSelectSeme();
        ...
        if ($model->Properties["use_prg_schoolkind"] == "1") {
            $query = knjd232vQuery::getNameMstA023($model);
            $extra = "onChange=\"btn_submit('knjd232v')\";";
            makeCmb($objForm, $arg, $db, $query, $model->field["SCHOOL_KIND"], "SCHOOL_KIND", $extra, 1);
        }

        //学年コンボボックスを作成する
        $opt_grade=array();
        $query = knjd232vQuery::getSelectGrade($model);
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        if ($model->Properties["use_school_detail_gcm_dat"] == '1') {
            ...
            $result->free();
            $model->field["MAJOR"] = ($model->field["MAJOR"] == "") ? $opt_test[0]["value"] : $model->field["MAJOR"];
            $extra = "onchange=\"return btn_submit('knjd232v');\"";
            $arg["data"]["MAJOR"] = knjCreateCombo($objForm, "MAJOR", $model->field["MAJOR"], $opt_test, $extra, 1);
        }

        //改行
        if ($model->Properties["use_school_detail_gcm_dat"] == '1') {
            $arg["br1"] = "<br>";
        } else if ($model->Properties["use_prg_schoolkind"] == "1" || $model->Properties["use_school_detail_gcm_dat"] == '1') {
            $arg["br2"] = "<br>";
        }

        //テスト種別コンボボックスを作成する
        $opt_test = array();
        $query = knjd232vQuery::getTestKind($model);
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
            $opt_test[] = array('label' => $row["LABEL"],
                                'value' => $row["VALUE"]);
        ...
        $extra = "onchange=\"return btn_submit('knjd232v');\"";
        $arg["data"]["TESTKIND"] = knjCreateCombo($objForm, "TESTKIND", $model->field["TESTKIND"], $opt_test, $extra, 1);

        //SHR
        $opt = array();
        $value_flg = false;
        $query = knjd232vQuery::getShr($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        /**********/
        //上限値
        $query = knjd232vQuery::getSyuutokuJougenti($model);
        ...
        //評定 or 評価 or 得点
        $scoreDiv = substr($model->field["TESTKIND"], -2);
        if ($scoreDiv == '09') {
            $model->field["HYOUTEI_OR_HYOUKA"] = '評定';
        } else if ($scoreDiv == '08') {
            $model->field["HYOUTEI_OR_HYOUKA"] = '評価';
        } else {
            $model->field["HYOUTEI_OR_HYOUKA"] = '得点';
        }
        $arg["data"]["HYOUTEI_OR_HYOUKA"] = $model->field["HYOUTEI_OR_HYOUKA"];
        knjCreateHidden($objForm, "HYOUTEI_OR_HYOUKA",  $model->field["HYOUTEI_OR_HYOUKA"]);
        ...
        if (VARS::post("cmd") == 'knjd232v' || VARS::post("cmd") == '') {
        ...
        knjCreateHidden($objForm, "PRGID", "KNJD232V");
        ...
        View::toHTML($model, "knjd232vForm1.html", $arg); 
    }
}
