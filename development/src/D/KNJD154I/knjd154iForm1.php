...
class knjd154iForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd154iForm1", "POST", "knjd154iindex.php", "", "knjd154iForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        $extra = array("id=\"CATEGORY_IS_CLASS1\" onClick=\"return btn_submit('knjd154i')\"", "id=\"CATEGORY_IS_CLASS2\" onClick=\"return btn_submit('knjd154i')\"");
        ...
        //学期コンボ作成
        $query = knjd154iQuery::getSemester();
        $extra = "onchange=\"return btn_submit('change_semes'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //テストコンボ作成
        $query = knjd154iQuery::getTest($model->field["SEMESTER"]);
        makeCmb($objForm, $arg, $db, $query, "TESTCD", $model->field["TESTCD"], "", 1);

        //学年コンボ作成
        $query = knjd154iQuery::getGradeHrClass($model, $model->field["SEMESTER"], $model, "GRADE");
        $extra = "onchange=\"return btn_submit('change_grade'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        if ($model->field["CATEGORY_IS_CLASS"] == 2){
            //クラスコンボ作成
            $query = knjd154iQuery::getGradeHrClass($model, $model->field["SEMESTER"], $model, "HR_CLASS");
            $extra = "onchange=\"return btn_submit('knjd154i'), AllClearList();\"";
        ...
        //対象学期の開始日・終了日
        $sDate = $db->getOne(knjd154iQuery::getSemesterDate("SDATE","9"));
        $eDate = $db->getOne(knjd154iQuery::getSemesterDate("EDATE",$model->field["SEMESTER"]));
        ...
        View::toHTML($model, "knjd154iForm1.html", $arg); 
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["CATEGORY_IS_CLASS"] == 1){
        ...
        //対象外の生徒取得
        $query = knjd154iQuery::getSchnoIdou($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt_idou = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //対象者リストを作成する
        $query = knjd154iQuery::getStudent($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt1 = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $idou = "　";
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD154I");
    ...
    knjCreateHidden($objForm, "knjd154iNotPrintMajorname", $model->Properties["knjd154iNotPrintMajorname"]);
