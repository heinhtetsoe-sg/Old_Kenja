...
class knjd175aForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd175aForm1", "POST", "knjd175aindex.php", "", "knjd175aForm1");
        ...
        //学期コンボの設定
        $query = knjd175aQuery::getSemester();
        $extra = "onchange=\"return btn_submit('gakki'),AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["GAKKI"], "GAKKI", $extra, 1);

        //クラス選択コンボボックスを作成する
        $query = knjd175aQuery::getAuth($model);
        $extra = "onchange=\"return btn_submit('knjd175a'),AllClearList();\"";
        ...
        View::toHTML($model, "knjd175aForm1.html", $arg); 
    }
}
...
{
    $query = knjd175aQuery::getSchnoIdou($model);
    $result = $db->query($query);
    $opt_idou = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    //対象者リストを作成する
    $result = $db->query(knjd175aQuery::getListStudent($model));
    $opt1 = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD175A");
