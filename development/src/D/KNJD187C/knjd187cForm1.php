...
class knjd187cForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd187cForm1", "POST", "knjd187cindex.php", "", "knjd187cForm1");
        ...
        //学期コンボ作成
        $query = knjd187cQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd187c');\"";
        ...
        //対象学期の開始日・終了日
        $sDate = $db->getOne(knjd187cQuery::getSemesterDate("SDATE",$model->field["SEMESTER"]));
        $eDate = $db->getOne(knjd187cQuery::getSemesterDate("EDATE",$model->field["SEMESTER"]));
        ...
        $extra = array("id=\"DISP1\" onClick=\"return btn_submit('knjd187c')\"", "id=\"DISP2\" onClick=\"return btn_submit('knjd187c')\"");
        $radioArray = knjCreateRadio($objForm, "DISP", $model->field["DISP"], $extra, $opt_disp, get_count($opt_disp));
        foreach($radioArray as $key => $val) $arg["data"][$key] = $val;

        if ($model->field["DISP"] == 1) {
            //学年コンボ
            $query = knjd187cQuery::getGrade($model);
            $extra = "onChange=\"return btn_submit('knjd187c');\"";
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
        } else if ($model->field["DISP"] == 2) {
            //クラスコンボ作成
            $query = knjd187cQuery::getHrClass(CTRL_YEAR, $seme, $model);
            $extra = "onchange=\"return btn_submit('knjd187c');\"";
        ...
        View::toHTML($model, "knjd187cForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model, $seme) {
    ...
    //対象外の生徒取得
    $query = knjd187cQuery::getSchnoIdou($model, $seme);
    ...
    //対象者リストを作成する
    if ($model->field["DISP"] == 2) {
        $query = knjd187cQuery::getStudent($model, $seme);
        $result = $db->query($query);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD187C");
