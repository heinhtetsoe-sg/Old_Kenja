...
class knjd172kForm1
{
    function main(&$model){
        ...
        //フォーム作成/////////////////////////////////////////////////////////////////////////////////////////////
        $arg["start"]   = $objForm->get_start("knjd172kForm1", "POST", "knjd172kindex.php", "", "knjd172kForm1");
        ...
        if ($disable == 1){
            ...
            //$query = knjd172kQuery::getAuth($model->control["年度"],$ga);
            //---2005.06.01---↑---
        }else {
            $query = knjd172kQuery::getAuth2($model->control["年度"],$ga);
        }
        ...
        $objForm->ae( array("type"       => "select",
                            ...
                            "extrahtml"  => "onchange=\"return btn_submit('knjd172k'),AllClearList();\"",
        ...
        if ($disable == 1) {
            //異動対象日付（テキスト）
            $disabled = "";
            $extra = "onblur=\"isDate(this); tmp_list('knjd172k', 'on')\"".$disabled;
            $date_textbox = knjCreateTextBox($objForm, $model->field["DATE"], "DATE", 12, 12, $extra);
            //異動対象日付（カレンダー）
            global $sess;
            $extra = "onclick=\"tmp_list('knjd172k', 'off'); loadwindow('" .REQUESTROOT ."/common/calendar.php?name=DATE&frame='+getFrameName(self) + '&date=' + document.forms[0]['DATE'].value + '&CAL_SESSID=$sess->id&reload=true', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 200)\"";
        ...
        /* NO001 ↓ */
            ...
            if ($disable == 1){
                //対象外の生徒取得
                $query = knjd172kQuery::getSchnoIdou($model,$ga);
                $result = $db->query($query);
                $opt_idou = array();
                while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
                ...
                //対象者リストを作成する/////////////////////////////////////////////////////////////////////////////////////////////
                $db = Query::dbCheckOut();
                $query = knjd172kQuery::gettaisyo($model,$ga);
                ...
                //---2005.06.01---↓---
                $query = common::getHrClassAuth(CTRL_YEAR,$ga,AUTHORITY,STAFFCD);
                //$query = knjd172kQuery::gettaisyo2($model,$ga);
                //---2005.06.01---↑---
                $result = $db->query($query);
                while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        */
        ...
        View::toHTML($model, "knjd172kForm1.html", $arg); 
    }
}
?>
