...
class knjd181aForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd181aForm1", "POST", "knjd181aindex.php", "", "knjd181aForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //データ取得
        $dataTmp = array();
        $query = knjd181aQuery::getHreportConditionDat($model);
        ...
        //学期数取得
        $query = knjd181aQuery::getCountSemester($model);
        $getCountsemester = $db->getOne($query);

        //学期コンボ
        $query = knjd181aQuery::getSemester($getCountsemester);
        ...
        //年組コンボ
        $query = knjd181aQuery::getAuth($model);
        $extra = "onchange=\"return btn_submit('main')\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE_HR_CLASS", $model->hrClass, $extra, 1);

        //学校名取得
        $query = knjd181aQuery::getSchoolName();
        ...
        //学期
        $semeGr9 = array();
        $query = knjd181aQuery::getSemester2("9");
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $semeGr9 = $row;
        }
        ...
        //学期
        $semeGr = array();
        $query = knjd181aQuery::getSemester2($model->field["SEMESTER"]);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $semeGr = $row;
        }
        ...
        $query = knjd181aQuery::getGdat($grade);
        $h_j = $db->getOne($query);
        ...
        View::toHTML($model, "knjd181aForm1.html", $arg);
    }
}
/****************************************************** 以下関数 ************************************************************/
//コンボ作成
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    //クラス一覧
    $opt = array();
    $query = knjd181aQuery::getStudent($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $opt[] = array('label' => $row["LABEL"],
                       'value' => $row["VALUE"]);
