...
class knjd129sForm1
{
    public function main(&$model)
    {
        ...
        $arg["start"]    = $objForm->get_start("main", "POST", "knjd129sindex.php", "", "main");
        ...
        //CSV表示不可チェック
        $notUseCsv = $db->getOne(knjd129sQuery::getNameMstD058($model));
        $notCsvCnt = $db->getOne(knjd129sQuery::getNotusecsvCnt($model));
        ...
        $query = knjd129sQuery::getSubclassMst($model);
        ...
        $query = knjd129sQuery::selectChairQuery($model);
        ...
        View::toHTML($model, "knjd129sForm1.html", $arg);
    }
}
...
{
    //科目別設定があるか
    $testSubCnt = $db->getOne(knjd129sQuery::getTestSubCnt($model));
    if ($testSubCnt == "") {
        $testSubCnt = 0;
    }
    //テスト取得(パーツ)
    $testcdArray = array();
    $result = $db->query(knjd129sQuery::getTestName($model, $testSubCnt));
...
function makeTestMoto(&$objForm, &$arg, $db, $model)
{
    ...
    //算出元科目別設定があるか
    $testSubCnt = $db->getOne(knjd129sQuery::getTestMotoSubCnt($model));
    if ($testSubCnt == "") {
        $testSubCnt = 0;
    }

    $testcdMotoArray = array();
    $result = $db->query(knjd129sQuery::getTestMoto($model, $testSubCnt));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
{
    $z010Name1 = $db->getOne(knjd129sQuery::getNameMstZ010());
    $count = 0; //明細ヘッダの列数
    $countProv = 0;
    $semArray = array();
    $sem = "";
    ...
    $arg["HEAD9"] = $head9; //学年末
    $countT = $count - $countProv;
    $addSize = (10 < $countT) ? ($countT - 10) * 75 : 0;
...
function makeMeisai(&$objForm, &$arg, $db, &$model, &$objUp)
{
    ...
    /* CSV */
    /*******/
    $subclassname = $db->getOne(knjd129sQuery::getSubclassName($model));
    $chairname = $db->getOne(knjd129sQuery::getChairName($model));
    //CSV出力ファイル名
    ...
    //学校詳細マスタ
    $schoolRow = array();
    $schoolRow = $db->getRow(knjd129sQuery::getVSchoolMst(), DB_FETCHMODE_ASSOC);
    //時間割講座テストより試験日を抽出
    ...
    //一覧表示
    $colorFlg = false;
    $query = knjd129sQuery::getScore($model, $execute_date);
    ...
    $result = $db->query(knjd129sQuery::selectQuery($model, $execute_date));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        //学籍番号をHiddenで保持
        ...
        //割合設定
        $query = knjd129sQuery::getPercent(CTRL_YEAR, $testcd, $row["GRADE"], $row["COURSE"], $model);
        ...
        foreach ($model->testcdArray as $key => $codeArray) {
            ...
            if ($controlFlg == "1" && !strlen($scoreRow["SCORE_PASS"])) {
                ...
                //考査満点マスタ
                $query = knjd129sQuery::getPerfect(CTRL_YEAR, $testcd, $row["GRADE"], $row["COURSE"], $model);
                $perfect = "";
...
function makeHidden(&$objForm, $model, $execute_date)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD129S");
