...
class knjd121sForm1
{
    function main(&$model)
    {
        ...
        $arg["start"]    = $objForm->get_start("main", "POST", "knjd121sindex.php", "", "main");
        $arg["YEAR"]     = CTRL_YEAR;
        ...
        $query = knjd121sQuery::getTestName($model);
        ...
        $header = array("0" => "科目コード",
                        ...
                        "0131" => $semename[2]."－".$testname[10202]."－素点",
                        "0132" => $semename[2]."－".$testname[10202]."－評価",
                        "0182" => $semename[1]."評価",
        ...
        $result = $db->query(knjd121sQuery::selectSubclassQuery($model->gen_ed, $model));
        //教育課程対応
        ...
        $result = $db->query(knjd121sQuery::selectChairQuery($model));
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        $ctrl_name = array("0111" => "SEM1_INTR_SCORE"
                          ...
                          ,"0131" => "SEM1_TERM2_SCORE"
                          ,"0132" => "SEM1_TERM2_VALUE"
                          ,"0182" => "SEM1_VALUE"
        ...
        $admin_key = array();
        $result = $db->query(knjd121sQuery::selectContolCodeQuery());
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            ...
            $item_test = (substr($row["CONTROL_CODE"], 2, 1) == "3") ? "02" : "01" ;
        ...
        if ($seme_test != "0") {
            $result = $db->query(knjd121sQuery::selectExecuteDateQuery($model, $seme_test, $kind_test, $item_test));
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
            {
        ...
        //遅刻何回で欠課とするかの指数取得
        $absent = array();
        $absent = $db->getRow(knjd121sQuery::getScAbsentCov($model),DB_FETCHMODE_ASSOC);

        //文字評定
        if ($model->gen_ed == substr($model->field["SUBCLASSCD"],0,2)) {
            $assess = array();
            $result = $db->query(knjd121sQuery::GetAssessMark());
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
            {
        ...
        $offdaysFlg = $db->getRow(knjd121sQuery::getOffdaysFlg(CTRL_YEAR),DB_FETCHMODE_ASSOC);
        //帳票パラメータ
        ...
        if ($absent["ABSENT_COV"] == "0" || $absent["ABSENT_COV"] == "2") { 
            $result = $db->query(knjd121sQuery::GetAttendData($model->field["CHAIRCD"],$model->field["SUBCLASSCD"],$absent["ABSENT_COV"],$absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        } else {
            $result = $db->query(knjd121sQuery::GetAttendData2($model->field["CHAIRCD"],$model->field["SUBCLASSCD"],$absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        }
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        //合併先科目の単位固定／加算フラグ
        $model->creditflg = $db->getOne(knjd121sQuery::getCalculateCreditFlg($model->field["SUBCLASSCD"], $model));

        //一覧表示
        $colorFlg = false;
        $result = $db->query(knjd121sQuery::selectQuery($model, $execute_date));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            //学籍番号を配列で取得
            ...
            if (is_array($assess)) {
                if ($model->Properties["useAssessSubclassMst"] == '1') {
                    //科目別評定マスタの設定があるか
                    $query = knjd121sQuery::getAssessSubclassMstMark($model, CTRL_YEAR, $row["GRADE"], $row["COURSE"], $model->field["SUBCLASSCD"], $row["GRAD_VALUE"], "cnt");
                    $cnt = $db->getOne($query);
                    if (0 < $cnt) {
                        //科目別評定マスタ
                        $query = knjd121sQuery::getAssessSubclassMstMark($model, CTRL_YEAR, $row["GRADE"], $row["COURSE"], $model->field["SUBCLASSCD"], $row["GRAD_VALUE"], "mark");
            ...
            foreach ($ctrl_name as $code => $col)
            {
                ...
                //管理者コントロール
                    ...
                    $objForm->ae( array("type"      => "text",
                                        ...
                                        "value"     => $value,
                                        "extrahtml" => "STYLE=\"text-align: right\" onChange=\"this.style.background='#ccffcc'; document.forms[0].changeVal.value = '1';\" onblur=\"calc(this);\" onKeyDown=\"keyChangeEntToTab(this)\" onPaste=\"return show(this);\" "));
                    $row[$col] = $objForm->ge($col."-".$counter);

                    //考査満点マスタ
                    $query = knjd121sQuery::getPerfect(CTRL_YEAR, $model->field["SUBCLASSCD"], $perfectcd[$code], $row["GRADE"], $row["COURSE"], $model);
        ...
        //累積現在日
        $cur_date = $db->getRow(knjd121sQuery::GetMax($model->field["SUBCLASSCD"], $model), DB_FETCHMODE_ASSOC);
        ...
        foreach ($testcdArray as $testkey => $testval) {
            ...
            if ($model->useRecordChkfinDat == "1") {
                $query = knjd121sQuery::getRecordChkfinDat($model, $testval, $recordDiv);
            } else {
                $query = knjd121sQuery::getRecordSchChairTestDat($model, $testval, $recordDiv);
            }
        ...
        View::toHTML($model, "knjd121sForm1.html", $arg);
    }
}
?>
