...
class knjd617Form1 {
    function main(&$model) {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd617Form1", "POST", "knjd617index.php", "", "knjd617Form1");
        //DB接続
        $db = Query::dbCheckOut();
        //年度テキストボックスを作成する
        ...
        //学期コンボボックスを作成する
        $query = knjd617Query::getSemester();
        ...
        //学校名取得
        $query = knjd617Query::getSchoolname();
        $rowZ010 = $db->getRow($query, DB_FETCHMODE_ASSOC);
        $schoolName = $rowZ010["NAME1"];
        $schoolCode = $rowZ010["NAME2"];

        //学年コンボ
        $extra = "onChange=\"return btn_submit('knjd617');\"";
        $query = knjd617Query::getSelectGrade($model);
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        //テスト種別コンボ
        $query = knjd617Query::getTestItem($model->field["SEMESTER"]);
        $opt = array();
        ...
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            if (preg_match('/(220250|220410)/', $schoolCode)) { //「米子」、「境」
                ...
                if ($row["VALUE"] == '9900' && $model->field["SEMESTER"] == '9') {
                    $opt[] = array('label' => $row["VALUE"] . ':学年評価',
                                   'value' => $row["VALUE"] . "_1");
                }
                $opt[] = array('label' => $row["LABEL"],
                               'value' => $row["VALUE"] . "_2");
            } else if (preg_match('/220360/', $schoolCode)) { //「倉吉」
        ...
        $extra2 = " onclick=\"return btn_submit('knjd617');\"";
        ...
        $query = knjd617Query::getSemesterDetailMst(CTRL_YEAR, $semesterDetailS);
        $result = $db->query($query);
        ...
        if ($model->field["DATE_DIV"] == "1") {
            $query = knjd617Query::getSemesterDetailMst(CTRL_YEAR, "1");
            $result = $db->query($query);
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                $sDate = $row["SDATE"];
            }
        ...
        $query = knjd617Query::getSemesterDetailMst(CTRL_YEAR, $semesterDetailE);
        $result = $db->query($query);
        ...
        View::toHTML($model, "knjd617Form1.html", $arg); 
    }
}
/**************************************** 以下関数 **************************************************/
//コンボ作成
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    //クラス一覧
    $row1 = array();
    $result = $db->query(knjd617Query::getAuth($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model, $seme, $semeflg)
{
    ...
    $objForm->ae(createHiddenAe("PRGID", "KNJD617"));
