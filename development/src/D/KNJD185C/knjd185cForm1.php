...
class knjd185cForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd185cForm1", "POST", "knjd185cindex.php", "", "knjd185cForm1");
        ...
        //学期コンボ作成
        $query = knjd185cQuery::getSemester($getCountsemester);
        $extra = "onchange=\"return btn_submit('knjd185c');\"";
        ...
        $extra = array("id=\"DISP1\" onClick=\"return btn_submit('knjd185c')\"", "id=\"DISP2\" onClick=\"return btn_submit('knjd185c')\"");
        $radioArray = knjCreateRadio($objForm, "DISP", $model->field["DISP"], $extra, $opt_disp, get_count($opt_disp));
        foreach($radioArray as $key => $val) $arg["data"][$key] = $val;

        if ($model->field["DISP"] == 1) {
            //学年コンボ
            $query = knjd185cQuery::getGrade($model);
            $extra = "onChange=\"return btn_submit('knjd185c');\"";
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
        } else if ($model->field["DISP"] == 2) {
            //クラスコンボ作成
            $query = knjd185cQuery::getHrClass(CTRL_YEAR, $seme, $model);
            $extra = "onchange=\"return btn_submit('knjd185c');\"";
        ...
        // //異動対象日付（テキスト）
        // $disabled = "";
        // $extra = "onblur=\"isDate(this); tmp_list('knjd185c', 'on')\"".$disabled;
        // $date_textbox = knjCreateTextBox($objForm, $model->field["DATE"], "DATE", 12, 12, $extra);
        // //異動対象日付（カレンダー）
        // global $sess;
        // $extra = "onclick=\"tmp_list('knjd185c', 'off'); loadwindow('" .REQUESTROOT ."/common/calendar.php?name=DATE&frame='+getFrameName(self) + '&date=' + document.forms[0]['DATE'].value + '&CAL_SESSID=$sess->id&reload=true', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 200)\"";
        ...
        if ($model->field["CERTIF_DATE"] == "") {
            $cdateWk = $db->getOne(knjd185cQuery::getCertifDate($model));
            if ($cdateWk == "") {
        ...
        View::toHTML($model, "knjd185cForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model, $seme) {
    ...
    //対象外の生徒取得
    $query = knjd185cQuery::getSchnoIdou($model, $seme);
    ...
    //対象者リストを作成する
    if ($model->field["DISP"] == 2) {
        $query = knjd185cQuery::getStudent($model, $seme);
        $result = $db->query($query);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD185C");
