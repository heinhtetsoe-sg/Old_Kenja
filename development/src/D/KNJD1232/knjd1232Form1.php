...
// kanji=漢字
// $Id: knjd1232Form1.php,v 1.2 2011/04/14 12:36:52 nakamoto Exp $
...
class knjd1232Form1
{
    function main(&$model)
    {
        ...
        $arg["start"]    = $objForm->get_start("main", "POST", "knjd1232index.php", "", "main");
        $arg["YEAR"]     = CTRL_YEAR;
        ...
        $query = knjd1232Query::getTestName($model);
        ...
        if ($model->cmd != "subclasscd") {
            $db->query(knjd1232Query::insertEx_Std_RecQuery($model));
        }
        //科目コンボ
        $opt_sbuclass = array();
        $opt_sbuclass[] = array("label" => "", "value" => "");
        $result = $db->query(knjd1232Query::selectSubclassQuery($model->gen_ed, $model));
        //教育課程対応
        ...
        $result = $db->query(knjd1232Query::selectChairQuery($model));
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        $admin_key = array();
        $result = $db->query(knjd1232Query::selectContolCodeQuery());
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        if ($seme_test != "0") {
            $result = $db->query(knjd1232Query::selectExecuteDateQuery($model, $seme_test, $kind_test));
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
            {
        ...
        //遅刻何回で欠課とするかの指数取得
        $absent = array();
        $absent = $db->getRow(knjd1232Query::getScAbsentCov($model),DB_FETCHMODE_ASSOC);

        //文字評定
        if ($model->gen_ed == substr($model->field["SUBCLASSCD"],0,2)) {
            $assess = array();
            $result = $db->query(knjd1232Query::GetAssessMark());
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
            {
        ...
        $offdaysFlg = $db->getRow(knjd1232Query::getOffdaysFlg(CTRL_YEAR),DB_FETCHMODE_ASSOC);
        //帳票パラメータ
        ...
        if ($absent["ABSENT_COV"] == "0" || $absent["ABSENT_COV"] == "2") { 
            $result = $db->query(knjd1232Query::GetAttendData($model->field["CHAIRCD"],$model->field["SUBCLASSCD"],$absent["ABSENT_COV"],$absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        } else {
            $result = $db->query(knjd1232Query::GetAttendData2($model->field["CHAIRCD"],$model->field["SUBCLASSCD"],$absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        }
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        $model->creditflg = "";
        $motoCnt = 0;
        $result = $db->query(knjd1232Query::getCalculateCreditFlg($model->field["SUBCLASSCD"], $model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //一覧表示
        $colorFlg = false;
        $result = $db->query(knjd1232Query::selectQuery($model, $execute_date));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            //学籍番号を配列で取得
            ...
            foreach ($ctrl_name as $code => $col)
            {
                ...
                //管理者コントロール
                    ...
                    $objForm->ae( array("type"      => "text",
                                        ...
                                        "value"     => $value,
                                        "extrahtml" => "STYLE=\"text-align: right\" onChange=\"this.style.background='#ccffcc'\" onblur=\"calc(this);\" onKeyDown=\"keyChangeEntToTab(this)\" onPaste=\"return show(this);\" "));
                    $row[$col] = $objForm->ge($col."-".$counter);

                    //考査満点マスタ
                    $query = knjd1232Query::getPerfect(CTRL_YEAR, $model->field["SUBCLASSCD"], $perfectcd[$code], $row["GRADE"], $row["COURSE"], $model);
        ...
        //累積現在日
        $cur_date = $db->getRow(knjd1232Query::GetMax($model->field["SUBCLASSCD"], $model), DB_FETCHMODE_ASSOC);
...
******/
        ...
        if($model->field["CHK_FLG"] == "reset")
        {
            ...
            //データ取得
            $query = knjd1232Query::getSchChrTestData($model->field["CHAIRCD"]);
            #var_dump($query);
        ...
        foreach ($testcdArray as $testkey => $testval) {
            $chk = '';
            $dis = '';
            $query = knjd1232Query::getRecordChkfinDat($model, $testval);
        ...
        View::toHTML($model, "knjd1232Form1.html", $arg);
    }
}
?>
