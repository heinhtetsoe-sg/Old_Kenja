...
class knjd210dForm1
{
    function main(&$model)
    {
        $db = Query::dbCheckOut();
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("main", "POST", "knjd210dindex.php", "", "main");
        ...
        $result = $db->query(knjd210dQuery::GetSemester());
        ...
        $result = $db->query(knjd210dQuery::GetGrade($seme));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        //処理種別(成績)
        $opt_exam = array();
        $result = $db->query(knjd210dQuery::GetName($model->seme));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        View::toHTML($model, "knjd210dForm1.html", $arg);
    }
}
?>
