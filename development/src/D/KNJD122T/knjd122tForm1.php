...
// kanji=漢字
// $Id: knjd122tForm1.php 56586 2017-10-22 12:52:35Z maeshiro $
...
class knjd122tForm1
{
    function main(&$model)
    {
        ...
        $arg["start"]    = $objForm->get_start("main", "POST", "knjd122tindex.php", "", "main");
        $arg["YEAR"]     = CTRL_YEAR;
        ...
        $query = knjd122tQuery::getTestName($model);
        ...
        if ($model->cmd != "subclasscd") {
            $db->query(knjd122tQuery::insertEx_Std_RecQuery($model));
        }
        //科目コンボ
        $opt_sbuclass = array();
        $opt_sbuclass[] = array("label" => "", "value" => "");
        $result = $db->query(knjd122tQuery::selectSubclassQuery($model->gen_ed, $model));
        //教育課程対応
        ...
        $result = $db->query(knjd122tQuery::selectChairQuery($model));
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        $admin_key = array();
        $result = $db->query(knjd122tQuery::selectContolCodeQuery($model));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
...
//echo "最初=" .$seme_kind_item_s ." " ."最後=" .$seme_kind_item_e ." " ."素点フラグ=" .$soten_flg ." " ."testcd=" .$model->testcd;
        ...
        if ($seme_test != "0") {
            $result = $db->query(knjd122tQuery::selectExecuteDateQuery($model, $seme_test, $kind_test, $item_test));
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
            {
...
//echo "seme_test = " .$seme_test ." kind_test = " .$kind_test ." execute_date = " .$execute_date;
        ...
        //遅刻何回で欠課とするかの指数取得
        $absent = array();
        $absent = $db->getRow(knjd122tQuery::getScAbsentCov($model),DB_FETCHMODE_ASSOC);

        //文字評定
        if ($model->gen_ed == substr($model->field["SUBCLASSCD"],0,2)) {
            $assess = array();
            $result = $db->query(knjd122tQuery::GetAssessMark());
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
            {
        ...
        $offdaysFlg = $db->getRow(knjd122tQuery::getOffdaysFlg(CTRL_YEAR),DB_FETCHMODE_ASSOC);
        //帳票パラメータ
        ...
        if ($absent["ABSENT_COV"] == "0" || $absent["ABSENT_COV"] == "2" || $absent["ABSENT_COV"] == "4") { 
            $result = $db->query(knjd122tQuery::GetAttendData($model->field["CHAIRCD"],$model->field["SUBCLASSCD"],$absent["ABSENT_COV"],$absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        } else {
            $result = $db->query(knjd122tQuery::GetAttendData2($model->field["CHAIRCD"],$model->field["SUBCLASSCD"],$absent["ABSENT_COV"],$absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        }
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        $model->creditflg = "";
        $motoCnt = 0;
        $result = $db->query(knjd122tQuery::getCalculateCreditFlg($model->field["SUBCLASSCD"], $model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //一覧表示
        $colorFlg = false;
        $result = $db->query(knjd122tQuery::selectQuery($model, $execute_date));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            //学籍番号をHiddenで保持
            ...
            foreach ($ctrl_name as $code => $col)
            {
                ...
                //管理者コントロール
                    ...
                    //考査満点マスタ
                    $query = knjd122tQuery::getPerfect(CTRL_YEAR, $model->field["SUBCLASSCD"], $perfectcd[$code], $row["GRADE"], $row["COURSE"], $model);
        ...
        //累積現在日
        $cur_date = $db->getRow(knjd122tQuery::GetMax($model->field["SUBCLASSCD"], $model), DB_FETCHMODE_ASSOC);
    ...
    // "成績入力完了"処理 //
        ...
        foreach ($testcdArray as $testkey => $testval) {
            ...
            if ($model->useRecordChkfinDat == "1") {
                $query = knjd122tQuery::getRecordChkfinDat($model, $testval, $recordDiv);
            } else {
                $query = knjd122tQuery::getRecordSchChrTest($model, $testval);
            }
    ...
    // "成績入力完了"処理 おわり //
        ...
        View::toHTML($model, "knjd122tForm1.html", $arg);
    }
}
?>
