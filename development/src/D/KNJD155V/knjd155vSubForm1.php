...
class knjd155vSubForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]  = $objForm->get_start("sel", "POST", "knjd155vindex.php", "", "sel");
        ...
        $hr_name = $db->getOne(knjd155vQuery::getHrName($model));
        $testname = $db->getOne(knjd155vQuery::getTestName($model));
        ...
        View::toHTML($model, "knjd155vSubForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    //対象者リストを作成する
    $query = knjd155vQuery::getStudent($model);
    ...
    //生徒一覧リストを作成する//
    $query = knjd155vQuery::getStudent($model, "1");
    $result = $db->query($query);
    $opt2 = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
