...
class knjd155vForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjd155vindex.php", "", "main");
        ...
        //学期コンボ作成
        $query = knjd155vQuery::getSemesterList();
        $extra = "onchange=\"return btn_submit('main');\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //テスト種別コンボ作成
        $query = knjd155vQuery::getTestList($model);
        $extra = "onchange=\"return btn_submit('main');\"";
        makeCmb($objForm, $arg, $db, $query, "TESTKINDCD", $model->field["TESTKINDCD"], $extra, 1, 1);

        //年組コンボ作成
        $query = knjd155vQuery::getGradeHrclass($model);
        $extra = "onchange=\"return btn_submit('main');\"";
        ...
        //一覧表示
        $colorFlg = false;
        $result = $db->query(knjd155vQuery::selectQuery($model));
        ...
        View::toHTML($model, "knjd155vForm1.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg, $model) {
    ...
    //一括更新ボタンを作成する
    $link = REQUESTROOT."/D/KNJD155V/knjd155vindex.php?cmd=replace&SEMESTER=".$model->field["SEMESTER"]."&GRADE_HR_CLASS=".$model->field["GRADE_HR_CLASS"]."&TESTKINDCD=".$model->field["TESTKINDCD"];
