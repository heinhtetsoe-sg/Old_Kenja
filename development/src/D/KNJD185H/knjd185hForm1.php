...
class knjd185hForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd185hForm1", "POST", "knjd185hindex.php", "", "knjd185hForm1");
        ...
        //学期コンボ
        $extra = "onChange=\"return btn_submit('knjd185h');\"";
        $query = knjd185hQuery::getSemester();
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        $query = knjd185hQuery::getMaxSemester();
        ...
        $extra = array("id=\"DISP1\" onClick=\"return btn_submit('knjd185h')\"", "id=\"DISP2\" onClick=\"return btn_submit('knjd185h')\"");
        ...
        if ($model->field["DISP"] == 1) {
            //学年コンボ
            $extra = "onChange=\"return btn_submit('knjd185h');\"";
            $query = knjd185hQuery::getGrade($model);
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
        } else if ($model->field["DISP"] == 2) {
            //年組コンボ
            $extra = "onChange=\"return btn_submit('knjd185h');\"";
            $query = knjd185hQuery::getHrClass($model);
        ...
        //表示先生ラジオボタン 1:担任 2:学年主任
        $opt_disp = array(1, 2);
        $model->field["PRT_TEACHER"] = ($model->field["PRT_TEACHER"] == "") ? "1" : $model->field["PRT_TEACHER"];
        $extra = array("id=\"PRT_TEACHER1\"", "id=\"PRT_TEACHER2\"");
        $radioArray = knjCreateRadio($objForm, "PRT_TEACHER", $model->field["PRT_TEACHER"], $extra, $opt_disp, get_count($opt_disp));
        foreach($radioArray as $key => $val) $arg["data"][$key] = $val;
        ...
        View::toHTML($model, "knjd185hForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["DISP"] == 2) {
        //生徒取得
        $query = knjd185hQuery::getSchList($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //クラス名称調整
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD185H");
