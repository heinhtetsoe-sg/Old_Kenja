...
class knjd187uForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd187uForm1", "POST", "knjd187uindex.php", "", "knjd187uForm1");
        ...
        //学期コンボ作成
        $query = knjd187uQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd187u');\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"] , $extra, 1);
        //学年末は今学期とする
        $seme = $model->field["SEMESTER"] == "9" ? CTRL_SEMESTER : $model->field["SEMESTER"];

        //クラスコンボ作成
        $query = knjd187uQuery::getHrClass(CTRL_YEAR, $seme);
        $extra = "onchange=\"return btn_submit('knjd187u');\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE_HR_CLASS", $model->field["GRADE_HR_CLASS"], $extra, 1);

        //出力日付初期値セット
        if ($model->field["OUTPUT_DATE"] == "") {
            $model->field["OUTPUT_DATE"] = date("Y/m/d");
        }
        //出力日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('', 'on')\"".$disabled;
        $date_textbox = knjCreateTextBox($objForm, $model->field["OUTPUT_DATE"], "OUTPUT_DATE", 12, 12, $extra);
        //出力日付（カレンダー）
        global $sess;
        $extra = "onclick=\"tmp_list('', 'off'); loadwindow('" .REQUESTROOT ."/common/calendar.php?name=OUTPUT_DATE&frame='+getFrameName(self) + '&date=' + document.forms[0]['OUTPUT_DATE'].value + '&CAL_SESSID=$sess->id&reload=true', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 200)\"";
        $date_button = knjCreateBtn($objForm, "btn_calen", "･･･", $extra);
        //出力日付
        $arg["data"]["OUTPUT_DATE"] = View::setIframeJs().$date_textbox.$date_button;
        ...
        $query = knjd187uQuery::getGradeCd($grade);
        $gradeCd = $db->getOne($query);
        
        $extra = "";
        $query = knjd187uQuery::getTestItem($model, $gradeCd);
        makeCmb($objForm, $arg, $db, $query, "TESTCD", $model->field["TESTCD"], $extra, 1);
        
        $extra = "style=\"height:65px;\"";
        $arg["data"]["REMARK1"] = knjCreateTextArea($objForm, "REMARK1", 4, 47, "", $extra, $model->field["REMARK1"]);
        
        $extra = "style=\"height:35px;\"";
        $arg["data"]["REMARK2"] = knjCreateTextArea($objForm, "REMARK2", 2, 47, "", $extra, $model->field["REMARK2"]);
        ...
        View::toHTML($model, "knjd187uForm1.html", $arg);
    }
}
...
{
    //対象外の生徒取得
    $query = knjd187uQuery::getSchnoIdou($model, $seme);
    ...
    //対象者リストを作成する
    $query = knjd187uQuery::getStudent($model, $seme);
    $result = $db->query($query);
...
{
    //印刷ボタンを作成する
    $extra = "onclick=\"return btn_submit('check');\"";
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD187U");
