...
class knjd219eform1 {
    function main(&$model) {
        $objForm = new form;
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjd219eindex.php", "", "main");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        View::toHTML($model, "knjd219eForm1.html", $arg);
    }
}
/********************************************** 以下関数 **********************************************/
//リスト作成
function makeList(&$objForm, &$arg, $db, $model) {
    //警告メッセージを表示しない場合
    if (!isset($model->warning) && $model->cmd != "sim") {
        $query = knjd219eQuery::getList($model);
    ...
    for ($i = 1; $i <= $model->field["ASSESSLEVELCNT"]; $i++) {
        ...
        //標準の下限値
        $row["STANDARD_LOW"] = $standardLow[$i];
...
//コンボ作成
function makeCmb(&$objForm, &$arg, $db, $query, &$value, $name, $extra, $size, $blank = "", $fuka = "") {
    $opt = array();
    if ($blank == "BLANK") {
...
function makeBtn(&$objForm, &$arg, $model) {
    ...
    //照会ボタン
    $url = REQUESTROOT."/D/KNJD219E/knjd219eindex.php?cmd=inquiry&SEMESTER={$model->field["SEMESTER"]}&TESTKINDCD={$model->field["TESTKINDCD"]}&DIV={$model->field["DIV"]}&GRADE={$model->field["GRADE"]}";
