...
//フォーム作成////////////////////////////////////////////////////////////////////////
$arg["start"]   = $objForm->get_start("knjd302Form1", "POST", "knjd302index.php", "", "knjd302Form1");
...
//学期リスト
$db = Query::dbCheckOut();
$query = knjd302Query::getSemester($model);
...
$objForm->ae( array("type"       => "select",
					...
					"extrahtml"  => "onChange=\"return btn_submit('knjd302');\"",
                    "options"    => $opt_seme));
...
$db = Query::dbCheckOut();
$opt_grade=array();
$query = knjd302Query::getSelectGrade($model);
$result = $db->query($query);
$grade_flg = true;
while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
...
//$query = knjd302Query::getAuth($model);
//---2005.06.01---↑---
$result = $db->query($query);
while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
...
View::toHTML($model, "knjd302Form1.html", $arg); 
}
}
?>
