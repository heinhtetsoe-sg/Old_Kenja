...
class knjd292Form1
{
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd292Form1", "POST", "knjd292index.php", "", "knjd292Form1");
        ...
        //学期コンボボックスを作成する
        $query = knjd292Query::getSemester();
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], "onchange=\"return btn_submit('gakki');\"", 1);

        //年組コンボ
        $query = knjd292Query::getHrClass($model);
        makeCmb($objForm, $arg, $db, $query, "GRADE_HR_CLASS", $model->field["GRADE_HR_CLASS"], "onChange=\"return btn_submit('knjd292');\"", 1);
        ...
        View::toHTML($model, "knjd292Form1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    //生徒一覧
    $row1 = array();
    $result = $db->query(knjd292Query::getStudent($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD292");
