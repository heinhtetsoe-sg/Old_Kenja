...
class knjd154rForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd154rForm1", "POST", "knjd154rindex.php", "", "knjd154rForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //学期コンボ
        $query = knjd154rQuery::getSemester();
        ...
        $extra = "onchange=\"return btn_submit('knjd154r'), AllClearList();\"";
        ...
        if (!$model->graph) $model->graph = 1;
        $extra = array("id=\"GRAPH1\"", "id=\"GRAPH2\"");
        $radioArray = knjCreateRadio($objForm, "GRAPH", $model->graph, $extra, $radioValue, get_count($radioValue));
        ...
        knjCreateHidden($objForm, "PRGID",         "KNJD154R");
        ...
        View::toHTML($model, "knjd154rForm1.html", $arg);
    }
}
/****************************************************** 以下関数 ************************************************************/
//コンボ作成
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    //クラス一覧
    $opt = array();
    $query = knjd154rQuery::getStudent($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $opt[] = array('label' => $row["LABEL"],
                       'value' => $row["VALUE"]);
