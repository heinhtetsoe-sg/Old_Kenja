...
class knjd154hForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd154hForm1", "POST", "knjd154hindex.php", "", "knjd154hForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //学期コンボ
        $query = knjd154hQuery::getSemester();
        ...
        if ($model->Properties["useSpecial_Support_School"] == '1') {
            ...
            $opt = array(1, 2);
            $extra = array("id=\"HR_CLASS_TYPE1\" onclick=\"return btn_submit('knjd154h');\"", "id=\"HR_CLASS_TYPE2\" onclick=\"return btn_submit('knjd154h');\"");
        ...
        $extra = "onchange=\"return btn_submit('knjd154h'), AllClearList();\"";
        ...
        /****************/
        //帳票パターン「5(E)固定」
        knjCreateHidden($objForm, "TYOUHYOU_PATTERN", "5");
        ...
        if ($model->field["DATE_DIV"] == "1") {
            $query = knjd154hQuery::getSemesterDetailMst("1");
        ...
        knjCreateHidden($objForm, "PRGID",         "KNJD154H");
        ...
        View::toHTML($model, "knjd154hForm1.html", $arg);
    }
}
/****************************************************** 以下関数 ************************************************************/
//コンボ作成
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    //クラス一覧
    $opt = array();
    $query = knjd154hQuery::getStudent($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $opt[] = array('label' => $row["LABEL"],
                       'value' => $row["VALUE"]);
