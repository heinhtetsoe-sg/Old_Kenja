...
class knjd280dForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd280dForm1", "POST", "knjd280dindex.php", "", "knjd280dForm1");
        ...
        if ($model->field["OUTPUT"] == "1") {
            $arg["student"] = 1;
            $query = knjd280dQuery::getHrClass($model);
        } else {
            $arg["hr_class"] = 2;
            $query = knjd280dQuery::getGrade($model);
        }
        $row1 = array();
        ...
        $extra = "onchange=\"return btn_submit('knjd280d'),AllClearList();\"";
        ...
        View::toHTML($model, "knjd280dForm1.html", $arg); 
    }
}
//リストToリスト作成
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    if ($disable == "1"){
        $query = knjd280dQuery::getStudentList($model, 'list', $selectdata);
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        //生徒一覧リストを作成する
        if($selectdata){
            $query = knjd280dQuery::getStudentList($model, 'select', $selectdata);
            $result = $db->query($query);
            $opt1 = array();
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                    $opt1[] = array('label' => $row["LABEL" ],
        ...
        $query = knjd280dQuery::getHrClassAuth($model, 'list', $selectdata);
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        if($selectdata){
            $query = knjd280dQuery::getHrClassAuth($model, 'select', $selectdata);
            $result = $db->query($query);
            $opt1 = array();
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                    if (substr($row["VALUE"],0,2) != $model->field["GRADE_HR_CLASS"]) continue;
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD280D");
