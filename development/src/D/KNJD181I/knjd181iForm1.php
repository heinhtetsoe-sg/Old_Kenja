...
class knjd181iForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd181iForm1", "POST", "knjd181iindex.php", "", "knjd181iForm1");
        ...
        //学期コンボ作成
        $query = knjd181iQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd181i'), AllClearList();\"";
        ...
        if ($model->Properties["useSpecial_Support_School"] == '1') {
            ...
            $opt = array(1, 2);
            $extra = array("id=\"HR_CLASS_TYPE1\" onclick=\"return btn_submit('knjd181i');\"", "id=\"HR_CLASS_TYPE2\" onclick=\"return btn_submit('knjd181i');\"");
        ...
        $extra .= " onclick=\"return btn_submit('knjd181i');\" id=\"GAKUNEN_KONGOU\"";
        ...
        $arr_trcd = array();
        $value_flg = false;
        $query = knjd181iQuery::getAuth($model);
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd181i')\"".$disabled;
        ...
        //対象外の生徒取得
        $query = knjd181iQuery::getSchnoIdou($model, CTRL_YEAR, $model->field["SEMESTER"], $model->field["GRADE_HR_CLASS"], str_replace("/","-",$model->field["DATE"]));
        $result = $db->query($query);
        $opt_idou = array();
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        //対象者リストを作成する
        $query = knjd181iQuery::getSchno($model, CTRL_YEAR, $model->field["SEMESTER"], $model->field["GRADE_HR_CLASS"]);
        $result = $db->query($query);
        ...
        $query = knjd181iQuery::getPrintSemester();
        $extra = "";
        ...
        knjCreateHidden($objForm, "PRGID", "KNJD181I");
        ...
        knjCreateHidden($objForm, "HREPORTREMARK_DAT_REMARK3_SIZE_P", $model->Properties["HREPORTREMARK_DAT_REMARK3_SIZE_P"]);
        ...
        View::toHTML($model, "knjd181iForm1.html", $arg); 
    }
}
//コンボ作成
