...
class knjd410Form2 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjd410index.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //学部表示
        $arg["SCHOOL_KIND"] = $db->getOne(knjd410Query::getSchoolKind($model->gakubu_school_kind));
        
        //状態表示
        if ($model->condition) {
            $arg["CONDITION"] = $db->getOne(knjd410Query::getCondition($model->condition));
        }
        //グループコード
        if ($model->condition != "" && $model->groupcd != "") {
            $arg["GROUP_NAME"] = $db->getOne(knjd410Query::getGroupName($model, $model->condition, $model->groupcd));
        }
        ...
        if (VARS::post("cmd") == "update") {
            $arg["jscript"] = "window.open('knjd410index.php?cmd=list&shori=update','left_frame');";
        }
        ...
        View::toHTML($model, "knjd410Form2.html", $arg);
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    ...
    if (isset($model->warning)) {
        ...
        $result = $db->query(knjd410Query::getGhrStudents($model));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    $cnt = 0;
    $opt_right = array();
    $result = $db->query(knjd410Query::getHrStudents($model));
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
