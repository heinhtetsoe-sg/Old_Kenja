...
class knjd410SubForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform1", "POST", "knjd410index.php", "", "subform1");
        ...
        //対象学部表示
        $arg["SCHOOL_KIND"] = $db->getOne(knjd410Query::getSchoolKind($model->school_kind));

        //対象年組表示
        $arg["GRADE_HR_CLASS"] = $db->getOne(knjd410Query::getGradeHrclass($model, $model->hr_class));

        //生徒リストToリスト作成
        if ($model->cmd == "subform1A") {
            $l_query = ($model->selectdata2) ? knjd410Query::getRightSchList($model, "list", $model->selectdata2) : "";
        } else {
            $l_query = ($model->schregno) ? knjd410Query::getRightSchList($model, "list", $model->schregno) : "";
            $model->selectdata2 = $model->schregno;
        }
        $r_query = knjd410Query::getRightSchList($model, "list");
        makeListToList($objForm, $arg, $db, "data2", "CATEGORY_SELECTED2", "CATEGORY_NAME2", $l_query, $r_query, "15", $model);

        knjCreateHidden($objForm, "SCHREGNO", $model->schregno);

        //状態区分コンボ作成
        $query = knjd410Query::getCondition($model);
        $extra = "onchange=\"return btn_submit('subform1A');\"";
        makeCmb($objForm, $arg, $db, $query, "CONDITION", $model->condition, $extra, 1);

        //科目リストToリスト作成
        if ($model->cmd == "subform1A") {
            $l_query = ($model->selectdata) ? knjd410Query::getSubclassMst($model, $model->selectdata) : "";
        } else {
            $l_query = ($model->selectdata2) ? knjd410Query::getSchregCompSubclassDat($model, $model->selectdata2) : "";
        }
        $r_query = knjd410Query::getGradeKindCompSubclassDat($model);
        ...
        View::toHTML($model, "knjd410SubForm1.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg, $model) {
    ...
    //戻るボタン
    $link = REQUESTROOT."/D/KNJD410/knjd410index.php?cmd=";
