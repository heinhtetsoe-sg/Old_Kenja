...
class knjd659eForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd659eForm1", "POST", "knjd659eindex.php", "", "knjd659eForm1");
        ...
        //学期コンボ作成
        $query = knjd659eQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd659e'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);
        //学年末は今学期とする
        $seme = $model->field["SEMESTER"] == "9" ? CTRL_SEMESTER : $model->field["SEMESTER"];

        //学年コンボ作成
        $query = knjd659eQuery::getSchoolName();
        $schoolName = $db->getOne($query);
        if ($schoolName == 'CHIBEN') {
            $query = knjd659eQuery::getGradeChiben($seme, $model);
            $extra = "onchange=\"return btn_submit('knjd659e'), AllClearList();\"";
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
        } else {
            $query = knjd659eQuery::getGrade($seme, $model);
            $extra = "onchange=\"return btn_submit('knjd659e'), AllClearList();\"";
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
        }
        ...
        View::toHTML($model, "knjd659eForm1.html", $arg); 
    }
}
...
function makeClassList(&$objForm, &$arg, $db, $model, $seme) {
    ...
    //クラス一覧リストを作成する
    $query = knjd659eQuery::getHrClass($seme, $model->field["GRADE"]);
...
function makeHidden(&$objForm, $model, $seme) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD659E");
