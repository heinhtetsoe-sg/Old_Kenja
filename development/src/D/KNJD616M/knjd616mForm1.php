...
class knjd616mForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd616mForm1", "POST", "knjd616mindex.php", "", "knjd616mForm1");
        ...
        //学期コンボ作成
        $query = knjd616mQuery::getSemester(0);
        $extra = "onchange=\"return btn_submit('knjd616m'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);
        //学年末は今学期とする
        $seme = $model->field["SEMESTER"] == "9" ? CTRL_SEMESTER : $model->field["SEMESTER"];

        //学年コンボ作成
        $query = knjd616mQuery::getSchoolName();
        $schoolName = $db->getOne($query);
        $query = knjd616mQuery::getGrade($seme, $model);
        $extra = "onchange=\"return btn_submit('knjd616m'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        //テストコンボ作成
        $query = knjd616mQuery::getTest($model->field["SEMESTER"], $model->field["GRADE"]);
        makeCmb($objForm, $arg, $db, $query, "TESTKINDCD", $model->field["TESTKINDCD"], "", 1);

        //出欠集計開始日付作成
        if ($model->field["SDATE"] == "") {
            $model->field["SDATE"] = str_replace("-", "/", $db->getOne(knjd616mQuery::getSdate($model->field["GRADE"])));
        }
        ...
        View::toHTML($model, "knjd616mForm1.html", $arg); 
    }
}
...
function makeClassList(&$objForm, &$arg, $db, $model, $seme) {
    ...
    //クラス一覧リストを作成する
    $query = knjd616mQuery::getHrClass($seme, $model->field["GRADE"]);
...
function makeHidden(&$objForm, $model, $seme) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD616M");
    ...
    knjCreateHidden($objForm, "knjd616mPrintNullRemark", $model->Properties["knjd616mPrintNullRemark"]);
