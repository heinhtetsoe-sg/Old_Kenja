...
class knjd171gForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd171gForm1", "POST", "knjd171gindex.php", "", "knjd171gForm1");
        ...
        //学期コンボ
        $query = knjd171gQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd171g'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //年組コンボ
        $query = knjd171gQuery::getHrClass($model);
        $extra = "onchange=\"return btn_submit('knjd171g'), AllClearList();\"";
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this);btn_submit('knjd171g')\"".$disabled;
        ...
        View::toHTML($model, "knjd171gForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    //対象外の生徒取得
    $query = knjd171gQuery::getSchnoIdou($model);
    ...
    //一覧取得
    $query = knjd171gQuery::getStudent($model, "right");
    $result = $db->query($query);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD171G");
    ...
    knjCreateHidden($objForm, "knjdBehaviorsd_UseText", $model->Properties["knjdBehaviorsd_UseText"]);
