...
class knjd657bForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd657bForm1", "POST", "knjd657bindex.php", "", "knjd657bForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        $extra = array("id=\"CATEGORY_IS_CLASS1\" onClick=\"return btn_submit('knjd657b')\"", "id=\"CATEGORY_IS_CLASS2\" onClick=\"return btn_submit('knjd657b')\"");
        ...
        //学期コンボ作成
        $query = knjd657bQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd657b'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //模試グループ
        $query = knjd657bQuery::getGroupcd($model);
        $extra = "";
        makeCmb($objForm, $arg, $db, $query, "GROUPCD", $model->field["GROUPCD"], $extra, 1);

        //学年コンボ作成
        $query = knjd657bQuery::getGradeHrClass($model->field["SEMESTER"], $model, "GRADE");
        $extra = "onchange=\"return btn_submit('knjd657b'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        if ($model->field["CATEGORY_IS_CLASS"] == 2){
            //クラスコンボ作成
            $query = knjd657bQuery::getGradeHrClass($model->field["SEMESTER"], $model, "HR_CLASS");
            $extra = "onchange=\"return btn_submit('knjd657b'), AllClearList();\"";
        ...
        $opt_group = array(1, 2);
        $model->field["OKURIJOU_JUSYO"] = ($model->field["OKURIJOU_JUSYO"] == "") ? "2" : $model->field["OKURIJOU_JUSYO"];
        $extra = array("id=\"OKURIJOU_JUSYO1\"" , "id=\"OKURIJOU_JUSYO2\"");
        $radioArray = knjCreateRadio($objForm, "OKURIJOU_JUSYO", $model->field["OKURIJOU_JUSYO"], $extra, $opt_div, get_count($opt_div));
        foreach($radioArray as $key => $val) $arg["data"][$key] = $val;
        ...
        View::toHTML($model, "knjd657bForm1.html", $arg); 
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    if ($model->field["CATEGORY_IS_CLASS"] == 1){
        ...
        //対象者リストを作成する
        $query = knjd657bQuery::getStudent($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt1 = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD657B");
