...
class knjd671fForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd671fForm1", "POST", "knjd671findex.php", "", "knjd671fForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //学期コンボ作成
        $query = knjd671fQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd671f'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //DBの学校校種取得
        $model->dbSchoolKind = $db->getOne(knjd671fQuery::getDbSchoolKind());

        //学年コンボ作成
        $query = knjd671fQuery::getGrade($model->field["SEMESTER"], $model);
        $extra = "onchange=\"return btn_submit('knjd671f'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        //学校校種取得
        $model->schoolkind = $db->getOne(knjd671fQuery::getSchoolKind($model->field["GRADE"]));

        //テストコンボ作成
        $query = knjd671fQuery::getTest($model->field["SEMESTER"], $model);
        $extra = "onchange=\"return btn_submit('knjd671f'), AllClearList();\"";
        ...
        View::toHTML($model, "knjd671fForm1.html", $arg); 
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    ...
    //対象者リストを作成する
    $query = knjd671fQuery::getSubclass($model, $model->field["SEMESTER"]);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD671F");
    knjCreateHidden($objForm, "knjd671F_FORM_J", $model->Properties["knjd671F_FORM_J"]);
    knjCreateHidden($objForm, "knjd671F_FORM_H", $model->Properties["knjd671F_FORM_H"]);
