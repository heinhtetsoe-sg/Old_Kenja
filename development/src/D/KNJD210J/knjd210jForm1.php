...
class knjd210jForm1
{
    function main(&$model)
    {
        $db = Query::dbCheckOut();
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("main", "POST", "knjd210jindex.php", "", "main");
        ...
        $result = $db->query(knjd210jQuery::GetSemester());
        ...
        $result = $db->query(knjd210jQuery::GetGrade($seme));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            $opt[]      = array("label" => sprintf("%d",$row["GRADE"])."学年", "value" => $row["GRADE"]);
            if ($model->grade == $row["GRADE"]) $value_flg = true;
        }
        ...
        $opt_exam = array();
        $value_flg = false;
        $result = $db->query(knjd210jQuery::GetName($model->seme));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        $query = knjd210jQuery::getSubclasscd($model);
        ...
        View::toHTML($model, "knjd210jForm1.html", $arg);
    }
}
//コンボ作成
...
function makeListRireki(&$objForm, &$arg, $db, &$model) {
    //履歴一覧
    $query = knjd210jQuery::getListRireki($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($row["SUBCLASSCD"] == "ALL") {
            $row["SUBCLASSNAME"] = "全て";
        } else if ($row["SUBCLASSCD"] == "999999") {
