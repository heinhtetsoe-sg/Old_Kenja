...
/********************************************************************/
/* テスト個人成績表                                 山内 2006/06/06 */
/*                                                                  */
/* 変更履歴                                                         */
/* ･NO001：テストコンボを作成                       山城 2006/06/13 */
/********************************************************************/
...
class knjd151tForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd151tForm1", "POST", "knjd151tindex.php", "", "knjd151tForm1");
        ...
        //クラス選択コンボボックスを作成する
        $db = Query::dbCheckOut();
        $query = knjd151tQuery::getAuth($model->control["年度"], $ga);
        ...
        $objForm->ae(array("type"       => "select",
                            ...
                            "extrahtml"  => "onchange=\"return btn_submit('knjd151t'),AllClearList();\"",
        ...
        //テストコンボ作成 NO001
        $db = Query::dbCheckOut();
        $query = knjd151tQuery::getTestkind($model->control["年度"], $ga, $model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd151t', 'on')\"".$disabled;
        $date_textbox = knjCreateTextBox($objForm, $model->field["DATE"], "DATE", 12, 12, $extra);
        //異動対象日付（カレンダー）
        global $sess;
        $extra = "onclick=\"tmp_list('knjd151t', 'off'); loadwindow('" .REQUESTROOT ."/common/calendar.php?name=DATE&frame='+getFrameName(self) + '&date=' + document.forms[0]['DATE'].value + '&CAL_SESSID=$sess->id&reload=true', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 200)\"";
        $date_button = knjCreateBtn($objForm, "btn_calen", "･･･", $extra);
        //異動対象日付
        $arg["data"]["DATE"] = View::setIframeJs().$date_textbox.$date_button;

        //対象外の生徒取得
        $query = knjd151tQuery::getSchnoIdou($model, $ga);
        $result = $db->query($query);
        $opt_idou = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        View::toHTML($model, "knjd151tForm1.html", $arg);
    }
}
