...
class knjd175bForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd175bForm1", "POST", "knjd175bindex.php", "", "knjd175bForm1");
        ...
        //学期コンボの設定
        $query = knjd175bQuery::getSemester();
        $extra = "onchange=\"return btn_submit('gakki'),AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["GAKKI"], "GAKKI", $extra, 1);

        //クラス選択コンボボックスを作成する
        $query = knjd175bQuery::getAuth($model);
        $extra = "onchange=\"return btn_submit('knjd175b'),AllClearList();\"";
        ...
        View::toHTML($model, "knjd175bForm1.html", $arg); 
    }
}
...
{
    $query = knjd175bQuery::getSchnoIdou($model);
    $result = $db->query($query);
    $opt_idou = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    //対象者リストを作成する
    $result = $db->query(knjd175bQuery::getListStudent($model));
    $opt1 = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD175B");
    ...
    knjCreateHidden($objForm, "HREPORTREMARK_DAT_TOTALSTUDYTIME_SIZE_H", $model->Properties["HREPORTREMARK_DAT_TOTALSTUDYTIME_SIZE_H"]);
