...
class knjd656Form1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd656Form1", "POST", "knjd656index.php", "", "knjd656Form1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //学期コンボ作成
        $query = knjd656Query::getSemester();
        $extra = "onchange=\"return btn_submit('knjd656'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //模試グループ
        $query = knjd656Query::getGroupcd($model);
        $extra = "onchange=\"return btn_submit('knjd656'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GROUPCD", $model->field["GROUPCD"], $extra, 1);

        //模試コード
        $query = knjd656Query::getMockcd($model);
        $extra = "";
        ...
        View::toHTML($model, "knjd656Form1.html", $arg); 
    }
}

function makeStudentList(&$objForm, &$arg, $db, $model) {
    //対象クラスリストを作成する
    $query = knjd656Query::getGradeHrClass($model->field["SEMESTER"], $model, "GRADE");
    $result = $db->query($query);
    $opt1 = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $opt1[] = array('label' => sprintf("%d",$row["LABEL"]).'学年',
                        'value' => $row["VALUE"]);
    }
    $result->free();
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD656");
