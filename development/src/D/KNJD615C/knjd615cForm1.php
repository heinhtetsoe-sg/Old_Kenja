...
class knjd615cForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd615cForm1", "POST", "knjd615cindex.php", "", "knjd615cForm1");
        ...
        //学期コンボボックスを作成する
        $query = knjd615cQuery::getSemester();
        ...
        //年組コンボ
        $query = knjd615cQuery::getGradeHrClass($model);
        ...
        /* ラジオボタン */
        /****************/
        //帳票パターンラジオボタン 1:A 2:B 3:C 4:D
        $opt_rank = array(1, 2, 3, 4);
        $model->field["OUTPUT_PATTERN"] = ($model->field["OUTPUT_PATTERN"] == "") ? "4" : $model->field["OUTPUT_PATTERN"];
        $extra = array("id=\"OUTPUT_PATTERN1\"", "id=\"OUTPUT_PATTERN2\"", "id=\"OUTPUT_PATTERN3\"", "id=\"OUTPUT_PATTERN4\"");
        $radioArray = knjCreateRadio($objForm, "OUTPUT_PATTERN", $model->field["OUTPUT_PATTERN"], $extra, $opt_rank, get_count($opt_rank));
        ...
        View::toHTML($model, "knjd615cForm1.html", $arg); 
    }
}
/**************************************** 以下関数 **************************************************/
//コンボ作成
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    ...
    //対象者リストを作成する
    $query = knjd615cQuery::getSchno($model);
    $result = $db->query($query);
    $opt1 = array();
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
    ...
    //クラス一覧
    /*$row1 = array();
    $result = $db->query(knjd615cQuery::getAuth($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model, $seme, $semeflg)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD615C");
