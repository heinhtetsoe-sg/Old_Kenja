...
class knjd183hForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd183hForm1", "POST", "knjd183hindex.php", "", "knjd183hForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //学期数取得
        $query = knjd183hQuery::getCountSemester($model);
        $getCountsemester = $db->getOne($query);

        //学期コンボ
        $query = knjd183hQuery::getSemester($getCountsemester);
        ...
        //年組コンボ
        $query = knjd183hQuery::getAuth($model);
        $extra = "onchange=\"return btn_submit('main')\"";
        ...
        //学期（学年ごと）
        $semeGr9 = array();
        $query = knjd183hQuery::getSemesterGrade("9", substr($model->hrClass, 0, 2));
        ...
        //学期（学年ごと）
        $semeGr = array();
        $query = knjd183hQuery::getSemesterGrade($model->field["SEMESTER"], substr($model->hrClass, 0, 2));
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $semeGr = $row;
        }
        ...
        //順位の基準点
        $model->output_kijun = $model->output_kijun ? $model->output_kijun : '2';
        $opt_kijun = array(1, 2);
        $extra = array("id=\"OUTPUT_KIJUN1\"", "id=\"OUTPUT_KIJUN2\"");
        $radioArray = knjCreateRadio($objForm, "OUTPUT_KIJUN", $model->output_kijun, $extra, $opt_kijun, get_count($opt_kijun));
        ...
        knjCreateHidden($objForm, "PRGID",         "KNJD183H");
        ...
        View::toHTML($model, "knjd183hForm1.html", $arg);
    }
}
/****************************************************** 以下関数 ************************************************************/
//コンボ作成
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    //クラス一覧
    $opt = array();
    $query = knjd183hQuery::getStudent($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $opt[] = array('label' => $row["LABEL"],
                       'value' => $row["VALUE"]);
