...
class knjd156eForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd156eForm1", "POST", "knjd156eindex.php", "", "knjd156eForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //学期コンボ
        $query = knjd156eQuery::getSemester();
        ...
        $extra = "onchange=\"return btn_submit('knjd156e'), AllClearList();\"";
        ...
        $model->field["AVG_DIV"] = ($model->field["AVG_DIV"] == "") ? "5" : $model->field["AVG_DIV"];
        ...
        knjCreateHidden($objForm, "PRGID",         "KNJD156E");
        ...
        View::toHTML($model, "knjd156eForm1.html", $arg);
    }
}
/****************************************************** 以下関数 ************************************************************/
//コンボ作成
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    //クラス一覧
    $opt = array();
    $query = knjd156eQuery::getStudent($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $opt[] = array('label' => $row["LABEL"],
                       'value' => $row["VALUE"]);
