...
class knjd667Form1 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]  = $objForm->get_start("knjd667Form1", "POST", "knjd667index.php", "", "knjd667Form1");
        ...
        //学期コンボ
        $extra = "onChange=\"return btn_submit('knjd667');\"";
        $query = knjd667Query::getSemester();
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //学年コンボ
        $extra = "onChange=\"return btn_submit('knjd667');\"";
        $query = knjd667Query::getGrade();
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        //学校校種を取得
        $model->schoolkind = $db->getOne(knjd667Query::getSchoolKind($model));

        //テスト種別コンボ
        $extra = "onChange=\"return btn_submit('knjd667');\"";
        $query = knjd667Query::getTestitem($model);
        makeCmb($objForm, $arg, $db, $query, "TESTKIND_ITEMCD", $model->field["TESTKIND_ITEMCD"], $extra, 1);

        //コースグループコンボ
        $query = knjd667Query::getCourseGroup($model);
        ...
        View::toHTML($model, "knjd667Form1.html", $arg); 
    }
}
...
function makeHidden(&$objForm, $model, $db) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD667");
}
?>
