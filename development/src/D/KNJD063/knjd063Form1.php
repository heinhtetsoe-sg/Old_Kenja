...
$objForm = new form;
//フォーム作成
$arg["start"]   = $objForm->get_start("knjd063Form1", "POST", "knjd063index.php", "", "knjd063Form1");
...
$db = Query::dbCheckOut();
$opt_grade=array();
$query = knjd063Query::getSelectGrade($model);
...
$objForm->ae( array("type"       => "select",
					...
					"extrahtml"  => "onChange=\"return btn_submit('knjd063');\"",
                    "options"    => $opt_grade));
...
//クラス一覧リスト作成する NO008↓
$db = Query::dbCheckOut();
$query = knjd063Query::getAuth($model);
$result = $db->query($query);
while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
...
View::toHTML($model, "knjd063Form1.html", $arg); 
}
}
?>
