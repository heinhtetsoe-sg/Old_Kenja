...
class knjd132hQualified {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjd132hindex.php", "", "edit");
        ...
        $counter = 0;
        if ($model->schregno) {
            $result = $db->query(knjd132hQuery::getAward($model));
        ...
        if (VARS::get("cmd") == "right_list"){ 
            $arg["reload"]  = "parent.edit_frame.location.href='knjd132hindex.php?cmd=edit'";
        }

        View::toHTML($model, "knjd132hQualified.html", $arg);
    }
}
?>
