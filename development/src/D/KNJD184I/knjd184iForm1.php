...
class knjd184iForm1
{
    ...
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd184iForm1", "POST", "knjd184iindex.php", "", "knjd184iForm1");
        ...
        //学期コンボ
        $extra = "onChange=\"return btn_submit('knjd184i');\"";
        $query = knjd184iQuery::getSemester();
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        $query = knjd184iQuery::getMaxSemester();
        ...
        $extra = array("id=\"DISP1\" onClick=\"return btn_submit('knjd184i')\"", "id=\"DISP2\" onClick=\"return btn_submit('knjd184i')\"");
        ...
        if ($model->field["DISP"] == 1) {
            //学年コンボ
            $extra = "onChange=\"return btn_submit('knjd184i');\"";
            $query = knjd184iQuery::getGrade($model);
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
        } elseif ($model->field["DISP"] == 2) {
            //年組コンボ
            $extra = "onChange=\"return btn_submit('knjd184i');\"";
            $query = knjd184iQuery::getHrClass($model);
        ...
        if (!strlen($model->field["SDATE"])) {
            $model->field["SDATE"] = str_replace('-', '/', $db->getOne(knjd184iQuery::getSdate()));
        }
        ...
        View::toHTML($model, "knjd184iForm1.html", $arg);
    }
}
...
function makeCmb(&$objForm, &$arg, $db, $query, $name, &$value, $extra, $size, $blank = "")
{
    ...
    if ($name == "SEMESTER") {
        $value = ($value && $value_flg) ? $value : (CTRL_YEAR == '2020' && CTRL_SEMESTER == "3" ? "2" : CTRL_SEMESTER);
    } else {
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    ...
    if ($model->field["DISP"] == 2) {
        //生徒取得
        $query = knjd184iQuery::getSchList($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //クラス名称調整
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD184I");
    ...
    knjCreateHidden($objForm, "HREPORTREMARK_DETAIL_DAT_01_01_REMARK1_SIZE_P", $model->Properties["HREPORTREMARK_DETAIL_DAT_01_01_REMARK1_SIZE_P"]);
    knjCreateHidden($objForm, "HREPORTREMARK_DETAIL_DAT_01_02_REMARK1_SIZE_P", $model->Properties["HREPORTREMARK_DETAIL_DAT_01_02_REMARK1_SIZE_P"]);
    knjCreateHidden($objForm, "HREPORTREMARK_DETAIL_DAT_02_01_REMARK1_SIZE_P", $model->Properties["HREPORTREMARK_DETAIL_DAT_02_01_REMARK1_SIZE_P"]);
