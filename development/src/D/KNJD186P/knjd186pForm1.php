...
class knjd186pForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd186pForm1", "POST", "knjd186pindex.php", "", "knjd186pForm1");
        ...
        //学期コンボの設定
        $query = knjd186pQuery::getSemester();
        $extra = "onchange=\"return btn_submit('semester'),AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["SEMESTER"], "SEMESTER", $extra, 1);

        //クラス選択コンボボックスを作成する
        $query = knjd186pQuery::getAuth($model);
        $extra = "onchange=\"return btn_submit('knjd186p'),AllClearList();\"";
        ...
        View::toHTML($model, "knjd186pForm1.html", $arg); 
    }
}
...
{
    $query = knjd186pQuery::getSchnoIdou($model);
    $result = $db->query($query);
    $opt_idou = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    //対象者リストを作成する
    $result = $db->query(knjd186pQuery::getListStudent($model));
    $opt1 = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD186P");
