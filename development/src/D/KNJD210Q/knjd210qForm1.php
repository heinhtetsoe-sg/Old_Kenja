...
class knjd210qForm1
{
    function main(&$model)
    {
        $db = Query::dbCheckOut();
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("main", "POST", "knjd210qindex.php", "", "main");
        ...
        $result = $db->query(knjd210qQuery::GetSemester());
        ...
        $result = $db->query(knjd210qQuery::GetGrade($seme));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $opt_exam = array();
        $value_flg = false;
        $result = $db->query(knjd210qQuery::GetName($model->seme));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        $query = knjd210qQuery::getSubclasscd($model);
        ...
        View::toHTML($model, "knjd210qForm1.html", $arg);
    }
}
//コンボ作成
...
function makeListRireki(&$objForm, &$arg, $db, &$model) {
    ...
    //履歴一覧
    $query = knjd210qQuery::getListRireki($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
