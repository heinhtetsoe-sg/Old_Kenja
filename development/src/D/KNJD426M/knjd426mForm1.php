...
class knjd426mForm1 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd426mForm1", "POST", "knjd426mindex.php", "", "knjd426mForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //学期コンボ
        $query = knjd426mQuery::getSemester();
        ...
        if ($model->field["HUKUSIKI_RADIO"] == "1") {
            $query = knjd426mQuery::getSchoolKind($model);
            // Add by PP for current cursor 2020-01-10 start
        ...
        if ($model->field["HUKUSIKI_RADIO"] == "2") {
            $query = knjd426mQuery::getGhr($model);
            $ghr_cd_label = "実クラス";
        } else {
            $query = knjd426mQuery::getGradeHrClass($model);
            $ghr_cd_label = "年組";
        }
        // Add by PP for current cursor 2020-01-10 start
        ...
        if ($model->field["HUKUSIKI_RADIO"] == "2" && $model->field["GHR_CD"] != '') {
            $query = knjd426mQuery::getGhr($model, $model->field["GHR_CD"]);
        ...
        //学校名取得
        $query = knjd426mQuery::getSchoolName();
        ...
        for ($ii = 0;$ii < 5;$ii++) {
            //チェックボックスの値
            $chkval = (($model->cmd === null || $model->chkColumnBlock[$ii] == "1") ? "checked" : "");

            //チェックボックスの作成
            $idx = $ii+1;
            $extra = " id=\"CHK_COLUMNBLOCK{$idx}\" ".$chkval;
            $arg["data"]["CHK_COLUMNBLOCK".$idx] = knjCreateCheckBox($objForm, "CHK_COLUMNBLOCK".$idx, "1", $extra);
            $arg["data"]["CHK_COLUMNTTL".$idx] = "CHK_COLUMNBLOCK".$idx;
        }

        //出力する項目（右列）
        $arg["dispchkbox"] = "1";
        $query = knjd426mQuery::getGuidanceKindName($model, "2");
        $getarry1 = array();
        $idx = 0;
        ...
        for ($lpcnt = 0;$lpcnt < $lpmaxcnt;$lpcnt++) {
            ...
            //チェックボックスの値
            $chkval = (($model->cmd === null || $model->chkPrintBlock[$cIdx] == $idxval) ? "checked" : "");

            //チェックボックスの作成
            $extra = " id=\"CHK_PRINTBLOCK{$idxval}\" ".$chkval;
            $arg["data"]["CHK_PRINTBLOCK".$idxval] = knjCreateCheckBox($objForm, "CHK_PRINTBLOCK".$idxval, $getarry1[$lpcnt]["VALUE"], $extra);
            $arg["data"]["CHK_PRINTTTL".$idxval] = "CHK_PRINTBLOCK".$idxval;
            $arg["data"]["CHK_PRINTLABEL".$idxval] = $getarry1[$lpcnt]["LABEL"];
        }

        //出力クラスラジオ 1:法定クラス 2:実クラス
        $opt = array(1, 2);
        $extra = "";
        $model->field["OUTPUT_CLASS"] = $model->field["OUTPUT_CLASS"] ? $model->field["OUTPUT_CLASS"] : '1';
        $radioArray = knjCreateRadio($objForm, "OUTPUT_CLASS", $model->field["OUTPUT_CLASS"], $extra, $opt, get_count($opt));
        foreach($radioArray as $key => $val) $arg["data"][$key] = $val;
        ...
        knjCreateHidden($objForm, "PRGID",         "KNJD426M");
        ...
        View::toHTML($model, "knjd426mForm1.html", $arg);
    }
}
/****************************************************** 以下関数 ************************************************************/
//コンボ作成
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    //対象外の生徒取得
    $query = knjd426mQuery::getSchnoIdou($model, $seme);
    $result = $db->query($query);
    $opt_idou = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    $query = knjd426mQuery::getStudent($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
