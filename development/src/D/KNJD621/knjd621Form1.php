...
class knjd621Form1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd621Form1", "POST", "knjd621index.php", "", "knjd621Form1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //学期コンボボックスを作成する
        $extra = "onchange=\"return btn_submit('knjd621');\"";
        $query = knjd621Query::getSemester();
        makeCmb($objForm, $arg, $db, $query, $model->field["SEMESTER"], "SEMESTER", $extra, 1);

        //テスト種別コンボボックスを作成する
        $extra = "onchange=\"return btn_submit('knjd621');\"";
        $query = knjd621Query::getTestcd($model);
        makeCmb($objForm, $arg, $db, $query, $model->field["TESTCD"], "TESTCD", $extra, 1);

        //学年コンボボックスを作成する
        $query = knjd621Query::getSelectGrade($model);
        $extra = "onchange=\"return btn_submit('knjd621');\"";
        ...
        View::toHTML($model, "knjd621Form1.html", $arg); 
    }
}
...
{
    $query = knjd621Query::getSubclass($model);
...
{
    $objForm->ae(createHiddenAe("PRGID", "KNJD621"));
