...
class knjd653gForm1
{
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd653gForm1", "POST", "knjd653gindex.php", "", "knjd653gForm1");
        ...
        //学期コンボボックスを作成する
        $query = knjd653gQuery::getSemester();
        ...
        //学校名取得
        $query = knjd653gQuery::getSchoolname();
        $schoolName = $db->getOne($query);

        //学年コンボ
        $query = knjd653gQuery::getSelectGrade($model);
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], "onChange=\"return btn_submit('knjd653g');\"", 1);

        //テスト種別コンボ
        $query = knjd653gQuery::getTestItem($model->field["SEMESTER"]);
        ...
        View::toHTML($model, "knjd653gForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    //クラス一覧
    $row1 = array();
    $result = $db->query(knjd653gQuery::getAuth($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model, $seme, $semeflg)
{
    ...
    $objForm->ae(createHiddenAe("PRGID", "KNJD653G"));
    $objForm->ae(createHiddenAe("CTRL_DATE", CTRL_DATE));
    $objForm->ae(createHiddenAe("CTRL_SEMESTER", CTRL_SEMESTER));
    $objForm->ae(createHiddenAe("usePerfectCourseGroup", $model->Properties["usePerfectCourseGroup"]));
