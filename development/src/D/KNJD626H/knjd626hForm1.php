...
class knjd626hForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd626hForm1", "POST", "knjd626hindex.php", "", "knjd626hForm1");
        ...
        $query = knjd626hQuery::getSemester();
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //科目コンボ
        $extra = "onChange=\"return btn_submit('change');\"";
        $query = knjd626hQuery::getSubclass($model);
        ...
        View::toHTML($model, "knjd626hForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    ...
    //講座取得
    $query = knjd626hQuery::getChairList($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD626H");
}
