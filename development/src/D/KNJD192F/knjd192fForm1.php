...
class knjd192fForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd192fForm1", "POST", "knjd192findex.php", "", "knjd192fForm1");
        ...
        $extra = array("id=\"CATEGORY_IS_CLASS1\" onClick=\"return btn_submit('knjd192f')\"", "id=\"CATEGORY_IS_CLASS2\" onClick=\"return btn_submit('knjd192f')\"");
        ...
        //学期コンボ作成
        $query = knjd192fQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd192f'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1, $model);

        //テストコンボ作成
        $query = knjd192fQuery::getTest($model->field["SEMESTER"]);
        makeCmb($objForm, $arg, $db, $query, "TESTCD", $model->field["TESTCD"], "", 1, $model);

        //学年・年組コンボ作成
        $div = ($model->field["CATEGORY_IS_CLASS"] == 1) ? "GRADE" : "HR_CLASS";
        $query = knjd192fQuery::getGradeHrClass($model->field["SEMESTER"], $model, $div);
        $extra = "onchange=\"return btn_submit('knjd192f'), AllClearList();\"";
        ...
        View::toHTML($model, "knjd192fForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["CATEGORY_IS_CLASS"] == 1) {
        //対象クラスリストを作成する
        $query = knjd192fQuery::getGradeHrClass($model->field["SEMESTER"], $model, "HR_CLASS_LIST");
        ...
        //対象外の生徒取得
        $query = knjd192fQuery::getSchnoIdou($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt_idou = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //対象者リストを作成する
        $query = knjd192fQuery::getStudent($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt1 = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $idou = "　";
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD192F");
