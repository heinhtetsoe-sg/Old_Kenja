...
class knjd186fForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd186fForm1", "POST", "knjd186findex.php", "", "knjd186fForm1");
        ...
        //学期コンボ作成
        $query = knjd186fQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd186f'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //クラスコンボ作成
        $query = knjd186fQuery::getGradeHrClass($model);
        $extra = "onchange=\"return btn_submit('knjd186f'), AllClearList();\"";
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd186f', 'on')\"".$disabled;
        ...
        View::toHTML($model, "knjd186fForm1.html", $arg); 
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    //対象外の生徒取得
    $query = knjd186fQuery::getSchnoIdou($model);
    ...
    //対象者リストを作成する
    $query = knjd186fQuery::getSchno($model);
    $result = $db->query($query);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD186F");
