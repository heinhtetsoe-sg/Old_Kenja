...
class knjd134mForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd134mForm1", "POST", "knjd134mindex.php", "", "knjd134mForm1");
        ...
        //学期
        $query = knjd134mQuery::getSemesterName($model);
        $semester = $db->getOne($query);
        $arg["data"]["SEMESTER"] = $semester;

        //年組コンボ
        $extra = "onChange=\"return btn_submit('change');\"";
        $query = knjd134mQuery::getHrClass($model);
        ...
        View::toHTML($model, "knjd134mForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    ...
    //生徒取得
    $query = knjd134mQuery::getSchList($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD134M");
    knjCreateHidden($objForm, "CTRL_YEAR", $model->year);
    knjCreateHidden($objForm, "CTRL_SEMESTER", $model->semester);
