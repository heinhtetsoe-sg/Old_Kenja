...
class knjd183pForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd183pForm1", "POST", "knjd183pindex.php", "", "knjd183pForm1");
        ...
        //学期コンボ作成
        $query = knjd183pQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd183p'), AllClearList();\"";
        ...
        $arr_trcd = array();
        $value_flg = false;
        $query = knjd183pQuery::getAuth($model->field["SEMESTER"]);
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd183p', 'on')\"".$disabled;
        $date_textbox = knjCreateTextBox($objForm, $model->field["DATE"], "DATE", 12, 12, $extra);
        //異動対象日付（カレンダー）
        global $sess;
        $extra = "onclick=\"tmp_list('knjd183p', 'off'); loadwindow('" .REQUESTROOT ."/common/calendar.php?name=DATE&frame='+getFrameName(self) + '&date=' + document.forms[0]['DATE'].value + '&CAL_SESSID=$sess->id&reload=true', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 200)\"";
        $date_button = knjCreateBtn($objForm, "btn_calen", "･･･", $extra);
        //異動対象日付
        $arg["data"]["DATE"] = View::setIframeJs().$date_textbox.$date_button;

        //対象外の生徒取得
        $query = knjd183pQuery::getSchnoIdou(CTRL_YEAR, $model->field["SEMESTER"], $model->field["GRADE_HR_CLASS"], str_replace("/","-",$model->field["DATE"]));
        $result = $db->query($query);
        $opt_idou = array();
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        //対象者リストを作成する
        $query = knjd183pQuery::getSchno(CTRL_YEAR, $model->field["SEMESTER"], $model->field["GRADE_HR_CLASS"]);
        $result = $db->query($query);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJD183P");
        ...
        knjCreateHidden($objForm, "HREPORTREMARK_DAT_REMARK2_SIZE_P", $model->Properties["HREPORTREMARK_DAT_REMARK2_SIZE_P"]);
        ...
        View::toHTML($model, "knjd183pForm1.html", $arg); 
    }
}
//コンボ作成
