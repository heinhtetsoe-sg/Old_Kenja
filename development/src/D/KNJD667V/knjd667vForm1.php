...
class knjd667vForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd667vForm1", "POST", "knjd667vindex.php", "", "knjd667vForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        $extra = array("id=\"CATEGORY_IS_CLASS1\" onClick=\"return btn_submit('knjd667v')\"", "id=\"CATEGORY_IS_CLASS2\" onClick=\"return btn_submit('knjd667v')\"");
        ...
        //学期コンボ作成
        $query = knjd667vQuery::getSemester();
        $extra = "onchange=\"return btn_submit('change_grade'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //校種コンボ作成
        $query = knjd667vQuery::getSchoolKind($model);
        $extra = "onchange=\"return btn_submit('knjd667v'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SCHOOLKIND", $model->field["SCHOOLKIND"], $extra, 1);

        //学年コンボ作成
        $query = knjd667vQuery::getGradeHrClass($model->field["SEMESTER"], $model, "GRADE");
        $extra = "onchange=\"return btn_submit('change_grade'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        //テストコンボ作成
        $query = knjd667vQuery::getTest($model);
        $opt = array();
        ...
        //学年カラー指定コンボ作成
        $query = knjd667vQuery::getColor($model);
        $extra = "";
        makeCmb($objForm, $arg, $db, $query, "GRADECOLOR", $model->field["GRADECOLOR"], $extra, 1);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJD667V");
        ...
        View::toHTML($model, "knjd667vForm1.html", $arg); 
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["CATEGORY_IS_CLASS"] == 1){
        //対象クラスリストを作成する
        $query = knjd667vQuery::getGradeHrClass($model->field["SEMESTER"], $model, "HR_CLASS");
        $result = $db->query($query);
        $opt1 = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
//        //対象外のコース取得
//        $query = knjd667vQuery::getSchnoIdou($model, $model->field["SEMESTER"]);
//        $result = $db->query($query);
//        $opt_idou = array();
//        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
//        $result->free();
        ...
        //対象コースリストを作成する
        $query = knjd667vQuery::getCourse($model);
        $result = $db->query($query);
        $opt1 = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $idou = "　";
//            if (in_array($row["COURSECODE"],$opt_idou)) {
//                $idou = "●";
//            }
            $opt1[] = array('label' => $row["COURSECODE"].":".$row["COURSECODENAME"],
