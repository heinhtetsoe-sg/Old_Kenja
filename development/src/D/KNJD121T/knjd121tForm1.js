<!--kanji=漢字-->
<!-- <?php # $RCSfile: knjd121tForm1.js,v $ ?> -->
<!-- <?php # $Revision: 56586 $ ?> -->
<!-- <?php # $Date: 2017-10-22 21:52:35 +0900 (日, 22 10 2017) $ ?> -->
...
function btn_submit(cmd) {
    ...
    if (cmd == 'reset'){
        ...
        var testname3 = document.forms[0].TESTNAME10202.value;
        ...
        score_msg[2] = "（"+semename1+testname3+"素点）";
        ...
        for (var i = 0; i < 5; i++ ) {
            /****************************************************************/
            /* score_txt : テキストボックスの有無(true⇒存在する)           */
            /* score_cnt : 全て入力されているのか(true⇒全て入力されている) */
            /* score_chk : 入力完了にチェックされているのか(true⇒チェック) */
            /* score_not : 全ての入力欄が空なのか(true⇒全て空)             */
            /****************************************************************/
            // ①の場合
        ...
        //教科コード取得
        subclasscd_chk = document.forms[0].SUBCLASSCD.value;
        // チェック
        for (var i = 0; i < document.forms[0].elements.length; i++ ) {
            var e = document.forms[0].elements[i];
            var nam = e.name;
            // 素点入力チェック
            if (e.type == 'text' && (nam.match(/.VALUE./) || nam.match(/COMP_CREDIT./) || nam.match(/GET_CREDIT./))) {
                ...
                if (subclasscd_chk.substr(0, 2) == '90'){
                    //学年評定・履修単位・修得単位チェック
                    if (nam.match(/GRAD_VALUE./) || nam.match(/COMP_CREDIT./) || nam.match(/GET_CREDIT./)) {
                        var valueName1  = "GRAD_VALUE";
                        var valueName2  = "COMP_CREDIT";
                        var valueName3  = "GET_CREDIT";
                        var valueNumber = nam.split("-")[1];
                        valueObject1 = eval("document.forms[0][\"" + valueName1 + "-" + valueNumber + "\"]");
                        valueObject2 = eval("document.forms[0][\"" + valueName2 + "-" + valueNumber + "\"]");
                        valueObject3 = eval("document.forms[0][\"" + valueName3 + "-" + valueNumber + "\"]");

                        value_txt[7] = true;
                        if (valueObject1.value == '' && valueObject2.value == '' && valueObject3.value == '') {
                            value_cnt[7] = false;
                        } else {
                            value_not[7] = false;
                        }
            ...
            if (e.type == 'checkbox' && nam.match(/CHK_COMP_VALUE./)) {
                ...
                if (nam.match(/CHK_COMP_VALUE6/)) {value_chk[5] = e.checked; value_dis[5] = e.disabled;}
                if (nam.match(/CHK_COMP_VALUE7/)) {value_chk[6] = e.checked; value_dis[6] = e.disabled;}
                if (nam.match(/CHK_COMP_VALUE8/)) {value_chk[7] = e.checked; value_dis[7] = e.disabled;}
            }
        }
        // 成績入力完了チェックの入れ忘れメッセージ
