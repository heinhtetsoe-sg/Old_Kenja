...
class knjd171jForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd171jForm1", "POST", "knjd171jindex.php", "", "knjd171jForm1");
        ...
        //学期コンボ作成
        $query = knjd171jQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd171j'), AllClearList();\"";
        ...
        $arr_trcd = array();
        $value_flg = false;
        $query = knjd171jQuery::getAuth($model->field["SEMESTER"]);
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd171j', 'on')\"".$disabled;
        $date_textbox = knjCreateTextBox($objForm, $model->field["DATE"], "DATE", 12, 12, $extra);
        //異動対象日付（カレンダー）
        global $sess;
        $extra = "onclick=\"tmp_list('knjd171j', 'off'); loadwindow('" .REQUESTROOT ."/common/calendar.php?name=DATE&frame='+getFrameName(self) + '&date=' + document.forms[0]['DATE'].value + '&CAL_SESSID=$sess->id&reload=true', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 200)\"";
        ...
        //対象外の生徒取得
        $query = knjd171jQuery::getSchnoIdou(CTRL_YEAR, $model->field["SEMESTER"], $model->field["GRADE_HR_CLASS"], str_replace("/","-",$model->field["DATE"]));
        $result = $db->query($query);
        $opt_idou = array();
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        //対象者リストを作成する
        $query = knjd171jQuery::getSchno(CTRL_YEAR, $model->field["SEMESTER"], $model->field["GRADE_HR_CLASS"]);
        $result = $db->query($query);
        ...
        $query = knjd171jQuery::getAttendSemester();
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
            //「出欠の記録は出力しない」チェックボックス
            $check = ($checkArray[intval($row["SEMESTER"])] == $row["SEMESTER"] || $model->cmd == '' && $row["SEMESTER"] <= CTRL_SEMESTER) ? "checked" : "";
            $attsem = array();
            $attsem["CHECK"] = knjCreateCheckBox($objForm, "CHECK".$row["SEMESTER"], "on", $check, "");
            $attsem["CHECK_NAME"] = $row["SEMESTERNAME"];
            $arg["attsem"][] = $attsem;
        }
        $result->free();
        ...
        knjCreateHidden($objForm, "PRGID", "KNJD171J");
        ...
        View::toHTML($model, "knjd171jForm1.html", $arg); 
    }
}
//コンボ作成
