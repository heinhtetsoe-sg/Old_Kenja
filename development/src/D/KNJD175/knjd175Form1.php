...
class knjd175Form1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd175Form1", "POST", "knjd175index.php", "", "knjd175Form1");
        ...
        //クラス選択コンボボックスを作成する
        $query = knjd175Query::getAuth($model);
        $extra = "onchange=\"return btn_submit('knjd175'),AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["GRADE_HR_CLASS"], "GRADE_HR_CLASS", $extra, 1);

        //高校用
        $extraDisabled = substr($model->field["GRADE_HR_CLASS"], 1, 1) < 4 ? "" : "disabled";

        //印刷種別ラジオ作成
        $opt_printDiv = array("Both", "Front", "Back");
        $model->output = (!$model->output) ? $opt_printDiv[0] : $model->output;
        createRadio($objForm, $arg, "OUTPUT", $model->output, $extraDisabled, $opt_printDiv, get_count($opt_printDiv));
        ...
        //修了証記載日付
        if ($model->field["CERTIF_DATE"] == "") $model->field["CERTIF_DATE"] = str_replace("-","/",CTRL_DATE);
        $arg["data"]["CERTIF_DATE"] = View::popUpCalendarAlp($objForm   ,"CERTIF_DATE"  ,$model->field["CERTIF_DATE"], $extraDisabled);
        ...
        View::toHTML($model, "knjd175Form1.html", $arg); 
    }
}
...
{
    $query = knjd175Query::getSchnoIdou($model);
    $result = $db->query($query);
    $opt_idou = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    //対象者リストを作成する
    $result = $db->query(knjd175Query::getListStudent($model));
    $opt1 = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model)
{
    ...
    $objForm->ae(createHiddenAe("PRGID", "KNJD175"));
