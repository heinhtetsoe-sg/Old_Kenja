...
class knjd653aForm1
{
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd653aForm1", "POST", "knjd653aindex.php", "", "knjd653aForm1");
        ...
        //学期コンボボックスを作成する
        $query = knjd653aQuery::getSemester();
        ...
        //学校名取得
        $query = knjd653aQuery::getSchoolname();
        $schoolName = $db->getOne($query);

        //学年コンボ
        $query = knjd653aQuery::getSelectGrade($model);
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], "onChange=\"return btn_submit('knjd653a');\"", 1);

        //テスト種別コンボ
        $query = knjd653aQuery::getTestItem($model->field["SEMESTER"]);
        ...
        View::toHTML($model, "knjd653aForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    //クラス一覧
    $row1 = array();
    $result = $db->query(knjd653aQuery::getAuth($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model, $seme, $semeflg)
{
    ...
    $objForm->ae(createHiddenAe("PRGID", "KNJD653A"));
