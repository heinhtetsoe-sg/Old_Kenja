...
class knjd615rForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd615rForm1", "POST", "knjd615rindex.php", "", "knjd615rForm1");
        ...
        //学期コンボ作成
        $query = knjd615rQuery::getSemester(0);
        $extra = "onchange=\"return btn_submit('knjd615rChseme'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);
        //学年末は今学期とする
        $seme = $model->field["SEMESTER"] == "9" ? CTRL_SEMESTER : $model->field["SEMESTER"];
        $query = knjd615rQuery::getSemester("", $model->field["SEMESTER"]);
        ...
        //学年コンボ作成
        $query = knjd615rQuery::getSchoolName();
        $schoolName = $db->getOne($query);
        $query = knjd615rQuery::getGrade($seme);
        $extra = "onchange=\"return btn_submit('knjd615r'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        //テストコンボ作成
        $query = knjd615rQuery::getTest($model->field["SEMESTER"], $model->field["GRADE"]);
        makeCmb($objForm, $arg, $db, $query, "TESTKINDCD", $model->field["TESTKINDCD"], "", 1);

        //出欠集計日付作成
        if ($model->field["SDATE"] == "" || $model->cmd == 'knjd615rChseme') {
        ...
        $extra = array("id=\"OUTPUT_RANK1\"", "id=\"OUTPUT_RANK2\"", "id=\"OUTPUT_RANK3\"", "id=\"OUTPUT_RANK4\"", "id=\"OUTPUT_RANK5\"");
        ...
        View::toHTML($model, "knjd615rForm1.html", $arg); 
    }
}
...
function makeClassList(&$objForm, &$arg, $db, $model, $seme) {
    ...
    //クラス一覧リストを作成する
    $query = knjd615rQuery::getHrClass($seme, $model->field["GRADE"]);
    $result = $db->query($query);
    $opt1 = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    $result->free();
    $extra = "multiple style=\"height:150px;width:230px\" ondblclick=\"move1('left')\"";
    $arg["data"]["CLASS_NAME"] = knjCreateCombo($objForm, "CLASS_NAME", "", $opt1, $extra, 20);

    //対象クラスリストを作成する
    $extra = "multiple style=\"height:150px;width:230px\" ondblclick=\"move1('right')\"";
...
function makeHidden(&$objForm, $model, $seme) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD615R");
    ...
    knjCreateHidden($objForm, "knjd615rJugyoJisuLesson", $model->Properties["knjd615rJugyoJisuLesson"]);
