...
class knjd132iSubForm5 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjd132iindex.php", "", "edit");
        ...
        $counter = 0;
        if ($model->schregno) {
            $result = $db->query(knjd132iQuery::getAward($model));
        ...
        if (VARS::get("cmd") == "right_list"){ 
            $arg["reload"] = "parent.edit_frame.location.href='knjd132iindex.php?cmd=edit'";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjd132iSubForm5.html", $arg);
    }
}
