...
class knjd128aForm1
{
    public function main(&$model)
    {
        ...
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            //考査名にリンクを作成
            //リンクは編集可能学期のみ
            $getControlData = $db->getOne(knjd128aQuery::selectContolCodeQuery($model, $getControlcd[$row["VALUE_TEST"]]));
        ...
        $result = $db->query(knjd128aQuery::selectContolCodeQuery($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
