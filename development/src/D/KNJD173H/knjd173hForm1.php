...
class knjd173hForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd173hForm1", "POST", "knjd173hindex.php", "", "knjd173hForm1");
        ...
        //クラス選択コンボボックスを作成する/
        $db = Query::dbCheckOut();
        $query = knjd173hQuery::getAuth($model, $model->control["年度"],$ga);
        ...
        $objForm->ae( array("type"       => "select",
                            ...
                            "extrahtml"  => "onchange=\"return btn_submit('knjd173h'),AllClearList();\"",
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd173h')\"".$disabled;
        ...
        //対象外の生徒取得
        $db = Query::dbCheckOut();
        $query = knjd173hQuery::getSchnoIdou($model,$ga);
        $result = $db->query($query);
        $opt_idou = array();
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        View::toHTML($model, "knjd173hForm1.html", $arg); 
    }
}
?>
