...
// kanji=漢字
// $Id: knjd1222Form1.php,v 1.3 2012/06/05 07:22:55 hamasaki Exp $
...
class knjd1222Form1
{
    function main(&$model)
    {
        ...
        $arg["start"]    = $objForm->get_start("main", "POST", "knjd1222index.php", "", "main");
        $arg["YEAR"]     = CTRL_YEAR;
        ...
        $query = knjd1222Query::getTestName($model);
        ...
        if ($model->cmd != "subclasscd") {
            $db->query(knjd1222Query::insertEx_Std_RecQuery($model));
        }
        //科目コンボ
        $opt_sbuclass = array();
        $opt_sbuclass[] = array("label" => "", "value" => "");
        $result = $db->query(knjd1222Query::selectSubclassQuery($model->gen_ed, $model));
        //教育課程対応
        ...
        $result = $db->query(knjd1222Query::selectChairQuery($model));
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        $ctrl_name = array("0111" => "SEM1_INTR_SCORE"
                          ...
                          ,"0231" => "SEM2_TERM2_SCORE"//Add
                          ,"0232" => "SEM2_TERM2_VALUE"//Add
                          ,"0282" => "SEM2_VALUE"
                          ,"0882" => "GRAD_VALUE"
                          ,"0884" => "COMP_CREDIT"
                          ,"0885" => "GET_CREDIT");
        ...
        $admin_key = array();
        $result = $db->query(knjd1222Query::selectContolCodeQuery());
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            ...
            $item_test = ($row["CONTROL_CODE"] < "0231") ? "01" : "02" ;//Add
...
//echo "最初=" .$seme_kind_item_s ." " ."最後=" .$seme_kind_item_e ." " ."素点フラグ=" .$soten_flg ." " ."testcd=" .$model->testcd;
        ...
        if ($seme_test != "0") {
            $result = $db->query(knjd1222Query::selectExecuteDateQuery($model, $seme_test, $kind_test, $item_test));
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
            {
...
//echo "seme_test = " .$seme_test ." kind_test = " .$kind_test ." execute_date = " .$execute_date;
        ...
        //遅刻何回で欠課とするかの指数取得
        $absent = array();
        $absent = $db->getRow(knjd1222Query::getScAbsentCov($model),DB_FETCHMODE_ASSOC);

        //文字評定
        if ($model->gen_ed == substr($model->field["SUBCLASSCD"],0,2)) {
            $assess = array();
            $result = $db->query(knjd1222Query::GetAssessMark());
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
            {
        ...
        $offdaysFlg = $db->getRow(knjd1222Query::getOffdaysFlg(CTRL_YEAR),DB_FETCHMODE_ASSOC);
        //帳票パラメータ
        ...
        if ($absent["ABSENT_COV"] == "0" || $absent["ABSENT_COV"] == "2" || $absent["ABSENT_COV"] == "4") { 
            $result = $db->query(knjd1222Query::GetAttendData($model->field["CHAIRCD"],$model->field["SUBCLASSCD"],$absent["ABSENT_COV"],$absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        } else {
            $result = $db->query(knjd1222Query::GetAttendData2($model->field["CHAIRCD"],$model->field["SUBCLASSCD"],$absent["ABSENT_COV"],$absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        }
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        $model->creditflg = "";
        $motoCnt = 0;
        $result = $db->query(knjd1222Query::getCalculateCreditFlg($model->field["SUBCLASSCD"], $model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //一覧表示
        $colorFlg = false;
        $result = $db->query(knjd1222Query::selectQuery($model, $execute_date));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            //学籍番号を配列で取得
            ...
            //合併先科目の単位を抽出
            $model->data["COMBINED_CREDIT_SUM"][] = $row["COMP_CREDIT_SUM"]."-".$row["GET_CREDIT_SUM"]."-".$row["GET_CREDIT_SUM_Y"]."-".$row["MOTO_CNT"]."-".$row["CREDIT_CNT"];
            ...
            foreach ($ctrl_name as $code => $col)
            {
                ...
                //管理者コントロール
                    ...
                    $objForm->ae( array("type"      => "text",
                                        ...
                                        "value"     => $value,
                                        "extrahtml" => "STYLE=\"text-align: right; width:31\" onChange=\"this.style.background='#ccffcc'\" onblur=\"calc(this);\" onKeyDown=\"keyChangeEntToTab(this)\" onPaste=\"return show(this);\" "));
                    $row[$col] = $objForm->ge($col."-".$counter);

                    //考査満点マスタ
                    $query = knjd1222Query::getPerfect(CTRL_YEAR, $model->field["SUBCLASSCD"], $perfectcd[$code], $row["GRADE"], $row["COURSE"], $model);
        ...
        //累積現在日
        $cur_date = $db->getRow(knjd1222Query::GetMax($model->field["SUBCLASSCD"], $model), DB_FETCHMODE_ASSOC);
...
******/
        ...
        if($model->field["CHK_FLG"] == "reset")
        {
            ...
            //データ取得
            $query = knjd1222Query::getSchChrTestData($model->field["CHAIRCD"]);
            #var_dump($query);
        ...
        foreach ($testcdArray as $testkey => $testval) {
            $chk = '';
            $dis = '';
            $query = knjd1222Query::getRecordChkfinDat($model, $testval);
    ...
    // "成績入力完了"処理 おわり //
        ...
        View::toHTML($model, "knjd1222Form1.html", $arg);
    }
}
?>
