...
class knjd185rForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd185rForm1", "POST", "knjd185rindex.php", "", "knjd185rForm1");
        ...
        //学期コンボ作成
        $query = knjd185rQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd185rChangeSemester'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //クラスコンボ作成
        $query = knjd185rQuery::getGradeHrClass($model);
        $extra = "onchange=\"return btn_submit('knjd185r'), AllClearList();\"";
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd185r', 'on')\"".$disabled;
        $date_textbox = knjCreateTextBox($objForm, $model->field["DATE"], "DATE", 12, 12, $extra);
        //異動対象日付（カレンダー）
        global $sess;
        $extra = "onclick=\"tmp_list('knjd185r', 'off'); loadwindow('" .REQUESTROOT ."/common/calendar.php?name=DATE&frame='+getFrameName(self) + '&date=' + document.forms[0]['DATE'].value + '&CAL_SESSID=$sess->id&reload=true', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 200)\"";
        ...
        //表示も印字する
        $model->semesterDiv = $db->getOne(knjd185rQuery::getSemesterdiv());
        $extra = " id=\"HYOUSI\" ";
        if ($model->field["SEMESTER"] == '9' || $model->field["SEMESTER"] == $model->semesterDiv) {
            // 最終学期、学年末に変更したらON
            if ($model->cmd == '' || $model->cmd == 'knjd185rChangeSemester') {
                $model->field["HYOUSI"] = '1';
            }
        } else {
            // 最終学期、学年末以外は使用不可
            $extra .= " disabled ";
        ...
        View::toHTML($model, "knjd185rForm1.html", $arg); 
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    //対象外の生徒取得
    $query = knjd185rQuery::getSchnoIdou($model);
    ...
    //対象者リストを作成する
    $query = knjd185rQuery::getSchno($model);
    $result = $db->query($query);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD185R");
