...
class knjd679Form1 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]  = $objForm->get_start("knjd679Form1", "POST", "knjd679index.php", "", "knjd679Form1");
        ...
        //学期コンボ
        $extra = "onChange=\"return btn_submit('knjd679');\"";
        $query = knjd679Query::getSemester();
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //学年コンボ
        $extra = "onChange=\"return btn_submit('knjd679');\"";
        $query = knjd679Query::getGrade();
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        //年組コンボ
        $extra = "onChange=\"return btn_submit('knjd679');\"";
        $query = knjd679Query::getHrclass($model);
        ...
        View::toHTML($model, "knjd679Form1.html", $arg); 
    }
}
...
{
    $row1 = array();
    $query = knjd679Query::getStudent($model);
    $result = $db->query($query);
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
...
function makeHidden(&$objForm, $model, $db) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD679");
