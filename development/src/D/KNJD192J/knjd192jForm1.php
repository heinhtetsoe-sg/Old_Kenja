...
class knjd192jForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd192jForm1", "POST", "knjd192jindex.php", "", "knjd192jForm1");
        //DB接続
        $db = Query::dbCheckOut();

        //学校名取得
        $query = knjd192jQuery::getSchoolname();
        ...
        $extra = array("id=\"CATEGORY_IS_CLASS1\" onClick=\"return btn_submit('knjd192j')\"", "id=\"CATEGORY_IS_CLASS2\" onClick=\"return btn_submit('knjd192j')\"");
        ...
        //学期コンボ作成
        $query = knjd192jQuery::getSemester();
        $extra = "onchange=\"return btn_submit('change_grade'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //出欠集計開始日付
        $query = knjd192jQuery::getSemesterDetail($value);
        ...
        //学年コンボ作成
        $query = knjd192jQuery::getGradeHrClass($model->field["SEMESTER"], $model, "GRADE");
        $extra = "onchange=\"return btn_submit('change_grade'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        if ($model->field["CATEGORY_IS_CLASS"] == 2){
            //クラスコンボ作成
            $query = knjd192jQuery::getGradeHrClass($model->field["SEMESTER"], $model, "HR_CLASS");
            $extra = "onchange=\"return btn_submit('knjd192j'), AllClearList();\"";
            makeCmb($objForm, $arg, $db, $query, "HR_CLASS", $model->field["HR_CLASS"], $extra, 1);
        }

        //テストコンボ作成
        $query = knjd192jQuery::getTest($model->field["SEMESTER"], $model->field["GRADE"]);
        $opt = array();
...
//        }
        ...
        if ($model->cmd == 'change_grade' || $model->cmd == '') {
            $query = knjd192jQuery::getGdat($model->field["GRADE"]);
            $h_j = $db->getOne($query);
        ...
        $submit = $model->field["CATEGORY_IS_CLASS"] == '1' ? "" : " onChange=\"return btn_submit('knjd192j')\" ";
        //寮生のみ出力する
        ...
        View::toHTML($model, "knjd192jForm1.html", $arg); 
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["CATEGORY_IS_CLASS"] == 1){
        ...
        //対象外の生徒取得
        $query = knjd192jQuery::getSchnoIdou($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt_idou = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //対象者リストを作成する
        $query = knjd192jQuery::getStudent($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt1 = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $idou = "　";
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD192J");
