...
class knjd615mForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd615mForm1", "POST", "knjd615mindex.php", "", "knjd615mForm1");
        ...
        //学期コンボ作成
        $query = knjd615mQuery::getSemester(0);
        $extra = "onchange=\"return btn_submit('knjd615m'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);
        //学年末は今学期とする
        $seme = $model->field["SEMESTER"] == "9" ? CTRL_SEMESTER : $model->field["SEMESTER"];

        //学年コンボ作成
        $query = knjd615mQuery::getSchoolName();
        $schoolName = $db->getOne($query);
        $query = knjd615mQuery::getGrade($seme, $model);
        $extra = "onchange=\"return btn_submit('knjd615m'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        //テストコンボ作成
        $query = knjd615mQuery::getTest($model->field["SEMESTER"], $model->field["GRADE"]);
        makeCmb($objForm, $arg, $db, $query, "TESTKINDCD", $model->field["TESTKINDCD"], "", 1);

        //出欠集計開始日付作成
        if ($model->field["SDATE"] == "") {
            $model->field["SDATE"] = str_replace("-", "/", $db->getOne(knjd615mQuery::getSdate($model->field["GRADE"])));
        }
        ...
        View::toHTML($model, "knjd615mForm1.html", $arg); 
    }
}
...
function makeClassList(&$objForm, &$arg, $db, $model, $seme) {
    ...
    //クラス一覧リストを作成する
    $query = knjd615mQuery::getHrClass($seme, $model->field["GRADE"]);
...
function makeHidden(&$objForm, $model, $seme) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD615M");
    ...
    knjCreateHidden($objForm, "knjd615mPrintNullRemark", $model->Properties["knjd615mPrintNullRemark"]);
