...
class knjd126wForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("form1", "POST", "knjd126windex.php", "", "form1");
        ...
        if ($model->field["SCHOOL_KIND"] == 'P') {
            ...
            $model->HyoukaCount = $db->getOne(knjd126wQuery::getKantenHyoukaCount($model));
        }
        ...
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            if (empty($model->field["SCHREGNO"]) && $model->field["SUBCLASSCD"] != $row["TARGET_SUBCLASSCD"]) {
                ...
                $row["LINK"] = View::alink("knjd126windex.php", "下学年適用", "target=_self tabindex=\"-1\"", $hash);
            }
            ...
            if ($viewcd != "" && $adminInputFlg) {
                ...
                if ($model->field["SCHOOL_KIND"] != "P") {
                    ...
                    if ($model->Properties["useHyoukaHyouteiFlg"] == "1") {
                        $extra = " aria-label=\"".$inputname."の評価\" onChange=\"tmpSet(this);\" onblur=\"calc(this, '".$electdiv["ELECTDIV"]."');\"";
        ...
        View::toHTML($model, "knjd126wForm1.html", $arg);
    }
}
//コンボ作成
