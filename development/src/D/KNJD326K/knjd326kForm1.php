...
/*                                                                        */
/* 変更履歴                                                               */
/* ･NO001：5教科素点以外は、赤点表示なし                  山城 2005/07/12 */
/* ･NO002：1年生の場合のみ、第6回を宿題テストとする       山城 2005/07/12 */
/* ･NO003：平均処理を修正                                 山城 2005/07/12 */
/* ･NO004：席次テーブル追加による修正                     山城 2005/10/10 */
/* ･NO005：合計平均点は、計算して出力                     山城 2005/12/09 */
/* ･NO006：平均テーブル追加による平均処理を修正           仲本 2005/12/14 */
/* ･NO007：席次の出力条件を削除                           山城 2005/12/19 */
/* ･NO008：学期成績が2つ以下の場合学年成績'-'表示         山城 2006/02/25 */
/* ･NO009：中間期末共にKK又は、KSの場合1/2学期成績()表示  山城 2006/02/25 */
/**************************************************************************/
...
class knjd326kForm1
{
            ...
            if ($model->num != 0) {
               //共通関数View::alinkを使うとリンクの色を指定できないので使わずに設定
                if ($model->cmd != 'kakudai') {
                   $hash = "knjd326kindex.php?cmd=main&FLG=1";
                   $arg["hash1"] = "<a href=\"" .$hash. "\" target=\"_self\" >";
                   $arg["end_link1"] = "</a>";
                }
            }
           //次ページにデータがある場合にリンク設定
            if (($hantei=($model->gamensu - $model->num)) != 0) {
                if ($model->cmd != 'kakudai') {
                   $hash = "knjd326kindex.php?cmd=main&FLG=2";
    ...
    function makeButton(&$objForm, &$arg, &$model)
    {
        //拡大ボタン
        if ($model->cmd != 'kakudai') {
            $extra = " onClick=\" wopen('../KNJD326K/knjd326kindex.php?SCHREGNO={$model->schregno}&num={$model->num}&FLG={$model->flg}&gamensu={$model->gamensu}&nendo_flg={$model->nendo_flg}&last_grad={$model->last_grad}&last_hrcl={$model->last_hrcl}&last_atno={$model->last_atno}&bfor_grad={$model->bfor_grad}&bfor_hrcl={$model->bfor_hrcl}&bfor_atno={$model->bfor_atno}&cmd=kakudai&PROGRAMID=PROGRAMID','SUBWIN2',0,0,screen.availWidth,screen.availheight);\"";
            $arg["BUTTON1"]["BTN_KAKUDAI"] = $this->createBtn($objForm, "btn_kakudai", " 拡大 ", $extra);
        }

        //終了ボタン
        if ($model->cmd != 'kakudai') {
            $arg["BUTTON1"]["BTN_END"] = $this->createBtn($objForm, "btn_end", " 終了 ", " onClick=\"return closeWin();\"");
        } else {
            $model->cmd = 'main';
            $arg["BUTTON1"]["BTN_END"] = $this->createBtn($objForm, "btn_end", " 戻る ", " onClick=\"return closeWin();\"");
        }
    }
