...
class knjd210mForm1
{
    function main(&$model)
    {
        $db = Query::dbCheckOut();
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("main", "POST", "knjd210mindex.php", "", "main");
        ...
        $result = $db->query(knjd210mQuery::GetSemester());
        ...
        //処理学年
        $opt = array();
        $result = $db->query(knjd210mQuery::GetGrade());
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        //処理種別(成績)
        $opt_exam = array();
        $result = $db->query(knjd210mQuery::GetName($model->seme));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        View::toHTML($model, "knjd210mForm1.html", $arg);
    }
}
?>
