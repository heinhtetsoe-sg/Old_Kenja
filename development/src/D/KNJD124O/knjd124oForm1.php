...
class knjd124oForm1
{
    function main(&$model)
    {
        ...
        $arg["start"]    = $objForm->get_start("main", "POST", "knjd124oindex.php", "", "main");
        $arg["YEAR"]     = CTRL_YEAR;
        ...
        $query = knjd124oQuery::getTestName($model);
        ...
        if ($model->cmd != "subclasscd") {
            $db->query(knjd124oQuery::insertEx_Std_RecQuery($model));
        }
        //科目コンボ
        $opt_sbuclass = array();
        $opt_sbuclass[] = array("label" => "", "value" => "");
        $result = $db->query(knjd124oQuery::selectSubclassQuery($model->gen_ed, $model));
        //教育課程対応
        ...
        $result = $db->query(knjd124oQuery::selectChairQuery($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $admin_key = array();
        $result = $db->query(knjd124oQuery::selectContolCodeQuery());
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
//            $seme_test = (substr($row["CONTROL_CODE"], 1, 1) == "1") ? "1" : (substr($row["CONTROL_CODE"], 1, 1) == "2") ? "2" : "3" ;
        ...
        if ($seme_test != "0") {
            $result = $db->query(knjd124oQuery::selectExecuteDateQuery($model, $seme_test, $kind_test, $item_test));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //遅刻何回で欠課とするかの指数取得
        $absent = array();
        $absent = $db->getRow(knjd124oQuery::getScAbsentCov($model),DB_FETCHMODE_ASSOC);

        //文字評定
        if ($model->gen_ed == substr($model->field["SUBCLASSCD"],0,2)) {
            $assess = array();
            $result = $db->query(knjd124oQuery::GetAssessMark());
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($absent["ABSENT_COV"] == "0" || $absent["ABSENT_COV"] == "2" || $absent["ABSENT_COV"] == "4") { 
            $result = $db->query(knjd124oQuery::GetAttendData($model->field["CHAIRCD"],$model->field["SUBCLASSCD"],$absent["ABSENT_COV"],$absent["ABSENT_COV_LATE"], $model));
        } else {
            $result = $db->query(knjd124oQuery::GetAttendData2($model->field["CHAIRCD"],$model->field["SUBCLASSCD"],$absent["ABSENT_COV"],$absent["ABSENT_COV_LATE"], $model));
        }
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //合併先科目の単位固定／加算フラグ
        $model->creditflg = $db->getOne(knjd124oQuery::getCalculateCreditFlg($model->field["SUBCLASSCD"], $model));

        //一覧表示
        $result = $db->query(knjd124oQuery::selectQuery($model, $execute_date));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //学籍番号を配列で取得
        ...
        //累積現在日
        $cur_date = $db->getRow(knjd124oQuery::GetMax($model->field["SUBCLASSCD"], $model), DB_FETCHMODE_ASSOC);
        ...
        ******/
        ...
        if ($model->field["CHK_FLG"] == "reset") {
            ...
            $model->field["CHK_FLG"] = "";
            //データ取得
            $query = knjd124oQuery::getSchChrTestData($model->field["CHAIRCD"]);
        ...
        View::toHTML($model, "knjd124oForm1.html", $arg);
    }
}
?>
