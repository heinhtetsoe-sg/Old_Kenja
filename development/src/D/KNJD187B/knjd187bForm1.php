...
class knjd187bForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd187bForm1", "POST", "knjd187bindex.php", "", "knjd187bForm1");
        ...
        //学期コンボ
        $extra = "onChange=\"return btn_submit('knjd187b');\"";
        $query = knjd187bQuery::getSemester();
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        $query = knjd187bQuery::getMaxSemester();
        ...
        $extra = array("id=\"DISP1\" onClick=\"return btn_submit('knjd187b')\"", "id=\"DISP2\" onClick=\"return btn_submit('knjd187b')\"");
        ...
        if ($model->field["DISP"] == 1) {
            //学年コンボ
            $extra = "onChange=\"return btn_submit('knjd187b');\"";
            $query = knjd187bQuery::getGrade($model);
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
        } else if ($model->field["DISP"] == 2) {
            //年組コンボ
            $extra = "onChange=\"return btn_submit('knjd187b');\"";
            $query = knjd187bQuery::getHrClass($model);
        ...
        View::toHTML($model, "knjd187bForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["DISP"] == 2) {
        //生徒取得
        $query = knjd187bQuery::getSchList($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //クラス名称調整
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD187B");
