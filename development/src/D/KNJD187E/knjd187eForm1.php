...
class knjd187eForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd187eForm1", "POST", "knjd187eindex.php", "", "knjd187eForm1");
        ...
        //学期コンボ
        $query = knjd187eQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd187e'), AllClearList();\"";
        ...
        $extra = array("id=\"DISP1\" onClick=\"return btn_submit('knjd187e')\"", "id=\"DISP2\" onClick=\"return btn_submit('knjd187e')\"");
        $radioArray = knjCreateRadio($objForm, "DISP", $model->field["DISP"], $extra, $opt_disp, get_count($opt_disp));
        foreach($radioArray as $key => $val) $arg["data"][$key] = $val;

        //年組コンボ
        if ($model->field["DISP"] == "1") {
            $query = knjd187eQuery::getGrade($model, $seme);
        } else {
            $query = knjd187eQuery::getGradeHrClass($model, $seme);
        }
        $extra = "onchange=\"return btn_submit('knjd187e'), AllClearList();\"";
        ...
        //校種
        if ($model->field["DISP"] == "1") {
            $query = knjd187eQuery::getGrade($model, $seme, $model->field["GRADE_HR_CLASS"]);
        } else {
            $query = knjd187eQuery::getGradeHrClass($model, $seme, $model->field["GRADE_HR_CLASS"]);
        }
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd187e', 'on')\"".$disabled;
        $date_textbox = knjCreateTextBox($objForm, $model->field["DATE"], "DATE", 12, 12, $extra);
        //異動対象日付（カレンダー）
        global $sess;
        $extra = "onclick=\"tmp_list('knjd187e', 'off'); loadwindow('" .REQUESTROOT ."/common/calendar.php?name=DATE&frame='+getFrameName(self) + '&date=' + document.forms[0]['DATE'].value + '&CAL_SESSID=$sess->id&reload=true', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 200)\"";
        ...
        //設定の初期値取得
        $dataTmp = array();
        $query = knjd187eQuery::getHreportConditionDat($model);
        ...
        //基準点
        $extra = "align=\"right\" onchange=\"chkNum(this);\"";
        $arg["data"]["SCORELINE"] = knjCreateTextBox($objForm, $model->field["SCORELINE"] ? $model->field["SCORELINE"] : "30", "SCORELINE", 3, 3, $extra);
        ...
        View::toHTML($model, "knjd187eForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model, $seme) {
    //対象外の生徒取得
    $query = knjd187eQuery::getSchnoIdou($model, $seme);
    $result = $db->query($query);
    $opt_idou = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    if ($model->field["DISP"] == "1") {
        ...
        $query = knjd187eQuery::getStudent($model, $seme);
    }
    $result = $db->query($query);
    if ($model->field["DISP"] == "1") {
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD187E");
