...
class knjd126lForm1 {
    ...
    function main(&$model) {
        ...
        $arg["start"] = $objForm->get_start("main", "POST", "knjd126lindex.php", "", "main");
        ...
        //年組コンボ
        $query = knjd126lQuery::getGradeHrClass($model);
        ...
        $query = knjd126lQuery::selectSchoolKind($grade);
        ...
        if (strlen($model->field["SEQ"]) && ($model->cmd == "main" || $model->cmd == "change" || $model->cmd == "reset")) {
            $unit_test = $db->getRow(knjd126lQuery::getUnitTestDat($model, $model->field["SEQ"]), DB_FETCHMODE_ASSOC);
        ...
        //テンプレート呼び出し
        View::toHTML($model, "knjd126lForm1.html", $arg);
    }
}
...
function setViewData(&$objForm, &$arg, $db, $model) {
    ...
    $allWidth = 380;
    $result = $db->query(knjd126lQuery::getViewList($model));
...
function makeDataList(&$objForm, &$arg, $db, $model, &$objUp, $viewList) {
    //データ取得
    $getData = array();
    $query = knjd126lQuery::getScoreData($model);
    ...
    $colorFlg = false;
    $data = array();
    $query = knjd126lQuery::getSchList($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
