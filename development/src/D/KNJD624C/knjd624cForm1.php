...
class knjd624cForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd624cForm1", "POST", "knjd624cindex.php", "", "knjd624cForm1");
        ...
        //学期コンボ
        $extra = "onchange=\"return btn_submit('knjd624b');\"";
        $query = knjd624cQuery::getSemester();
        makeCmb($objForm, $arg, $db, $query, $model->field["SEMESTER"], "SEMESTER", $extra, 1);

        //学年コンボ
        $extra = "onChange=\"return btn_submit('knjd624c');\"";
        $query = knjd624cQuery::getGrade();
        makeCmb($objForm, $arg, $db, $query, $model->field["GRADE"], "GRADE", $extra, 1);

        //データ種別コンボ
        $extra = "onChange=\"return btn_submit('knjd624c');\"";
        $query = knjd624cQuery::getDatadiv();
        makeCmb($objForm, $arg, $db, $query, $model->field["DATA_DIV"], "DATA_DIV", $extra, 1);

        //テスト名称コンボ
        $extra = "onchange=\"return btn_submit('knjd624c');\"";
        $query = knjd624cQuery::getMockcd($model->field["DATA_DIV"]);
        ...
        View::toHTML($model, "knjd624cForm1.html", $arg); 
    }
}
...
function makeCmb(&$objForm, &$arg, $db, $query, &$value, $name, $extra, $size)
{
    ...
    if ($query) {
        ...
        if ($name == "CATEGORY_NAME") {
            $opt[] = array("label" => '999999:平均点（全科目）', "value" => '999999');
        }
...
{
    $query = knjd624cQuery::getSubclass($model);
...
{
    $objForm->ae(createHiddenAe("PRGID", "KNJD624C"));
