function btn_submit(cmd) {
    if (cmd == "execute") {
        ...
        if (
            new Date(eval(sdate[0]), eval(sdate[1]) - 1, eval(sdate[2])) > new Date(eval(chairdate[0]), eval(chairdate[1]) - 1, eval(chairdate[2])) ||
...
function disElectdiv() {
    ...
    //評定マスタ作成グレーアウト
    if (document.forms[0].knjd141c_useRelativeAssessCheckbox.value == "1") {
