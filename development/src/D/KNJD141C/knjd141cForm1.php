...
class knjd141cForm1
{
    public function main(&$model)
    {
        ...
        $arg["start"]   = $objForm->get_start("main", "POST", "knjd141cindex.php", "", "main");
        ...
        $query = knjd141cQuery::getSemester();
        makeCmb($objForm, $arg, $db, $query, $model->seme, "SEMESTER", $extra, 1, "");

        //学期開始終了日付（講座基準日チェック用）
        $query = knjd141cQuery::getSemester($model->seme);
        $semeRow = array();
        ...
        //処理種別(成績)
        $extra = "onChange=\"btn_submit('');\" ";
        $query = knjd141cQuery::getName($model, $model->seme);
        ...
        //科目コンボ
        if ($model->Properties["knjd141c_notUsetSubclassCombo"] == '1') {
            //プロパティー「knjd141c_notUsetSubclassCombo = 1」の時は、科目コンボはなし
            //処理は、全科目を選択した状態で、処理
            $arg["knjd141c_notUsetSubclassCombo"] = "";
            $model->subclasscd = "000000";
            knjCreateHidden($objForm, "SUBCLASSCD", $model->subclasscd);
        } else {
            $arg["knjd141c_notUsetSubclassCombo"] = 1;
            $extra = "onChange=\"disElectdiv();\" ";
            $query = knjd141cQuery::getSubclasscd($model, $gradeRow["SCHOOL_KIND"]);
        ...
        //評定マスタ作成
        knjCreateHidden($objForm, "knjd141c_useRelativeAssessCheckbox", $model->Properties["knjd141c_useRelativeAssessCheckbox"]);
        if ($model->Properties["knjd141c_useRelativeAssessCheckbox"] == '1') {
            $arg["knjd141c_useRelativeAssessCheckbox"] = 1;
        ...
        View::toHTML($model, "knjd141cForm1.html", $arg);
    }
}
//コンボ作成
...
{
    //履歴一覧
    $query = knjd141cQuery::getListRireki($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
