...
class knjd171fForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd171fForm1", "POST", "knjd171findex.php", "", "knjd171fForm1");
        ...
        //学期コンボ
        $query = knjd171fQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd171f'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //年組コンボ
        $query = knjd171fQuery::getHrClass($model);
        $extra = "onchange=\"return btn_submit('knjd171f'), AllClearList();\"";
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this);btn_submit('knjd171f')\"".$disabled;
        ...
        View::toHTML($model, "knjd171fForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    //対象外の生徒取得
    $query = knjd171fQuery::getSchnoIdou($model);
    ...
    //一覧取得
    $query = knjd171fQuery::getStudent($model, "right");
    $result = $db->query($query);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD171F");
