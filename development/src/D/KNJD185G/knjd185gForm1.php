...
class knjd185gForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd185gForm1", "POST", "knjd185gindex.php", "", "knjd185gForm1");
        ...
        //学期コンボ作成
        $query = knjd185gQuery::getSemester($getCountsemester);
        $extra = "onchange=\"return btn_submit('knjd185g'), AllClearList();\"";
...
//        $seme = $model->field["SEMESTER"] == "9" ? CTRL_SEMESTER : $model->field["SEMESTER"];
        ...
        //クラスコンボ作成
        $query = knjd185gQuery::getHrClass($model, $seme);
        $extra = "onchange=\"return btn_submit('knjd185g'), AllClearList();\"";
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd185g', 'on')\"".$disabled;
        $date_textbox = knjCreateTextBox($objForm, $model->field["DATE"], "DATE", 12, 12, $extra);
        //異動対象日付（カレンダー）
        global $sess;
        $extra = "onclick=\"tmp_list('knjd185g', 'off'); loadwindow('" .REQUESTROOT ."/common/calendar.php?name=DATE&frame='+getFrameName(self) + '&date=' + document.forms[0]['DATE'].value + '&CAL_SESSID=$sess->id&reload=true', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 200)\"";
        ...
        //作成日(テキスト)
        $disabled2 = "";
        $extra2 = "onblur=\"isDate(this); tmp_list('knjd185g', 'on')\"".$disabled2;
        $date_textbox2 = knjCreateTextBox($objForm, $model->field["PRTDATE"], "PRTDATE", 12, 12, $extra2);
        //異動対象日付（カレンダー）
        $extra2 = "onclick=\"tmp_list('knjd185g', 'off'); loadwindow('" .REQUESTROOT ."/common/calendar.php?name=PRTDATE&frame='+getFrameName(self) + '&date=' + document.forms[0]['PRTDATE'].value + '&CAL_SESSID=$sess->id&reload=true', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 200)\"";
        ...
        View::toHTML($model, "knjd185gForm1.html", $arg);
    }
}

function makeStudentList(&$objForm, &$arg, $db, $model, $seme) {
    //対象外の生徒取得
    $query = knjd185gQuery::getSchnoIdou($model, $seme);
    ...
    //対象者リストを作成する
    $query = knjd185gQuery::getStudent($model, $seme);
    $result = $db->query($query);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD185G");
