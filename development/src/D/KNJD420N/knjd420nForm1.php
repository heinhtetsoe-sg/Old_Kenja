...
class knjd420nForm1 {
    function main(&$model) {
        $objForm = new form;
        $arg["start"] = $objForm->get_start("edit", "POST", "knjd420nindex.php", "", "edit");
        ...
        //グループ情報
        $getGroupRow = array();
        $getGroupRow = $db->getRow(knjd420nQuery::getViewGradeKindSchreg($model, "set"), DB_FETCHMODE_ASSOC);
        if ($model->schregno) {
            $getGroupName = $db->getOne(knjd420nQuery::getGroupcd($model, $getGroupRow));
            if ($getGroupName) {
                $arg["GROUP_NAME"] = '履修科目グループ:'.$getGroupName;
            } else {
                $arg["GROUP_NAME"] = '履修科目グループ未設定';
            }
            $getConditionName = $db->getOne(knjd420nQuery::getConditionName($model, $getGroupRow["CONDITION"]));
            $arg["CONDITION_NAME"] = ($getConditionName) ? '('.$getConditionName.')' : "";
        }

        //学期コンボ
        $query = knjd420nQuery::getSemester($model);
        ...
        $query = knjd420nQuery::getGuidancePattern($model);
        ...
        $query = knjd420nQuery::getItemName($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            if ($row["ITEM_REMARK"] != '') {
                $arg["koumoku"][] = $row;
        ...
        //作成者取得
        $model->staffName = $db->getOne(knjd420nQuery::getStaffName($model));
        $extra = " id=\"STAFFNAME\" ";
        ...
        //科目コンボ
        $query = knjd420nQuery::getSubclass($model);
        $extra = "onChange=\"return btn_submit('edit');\"";
        ...
        //単元がセットされているか
        $get_div = $db->getOne(knjd420nQuery::getUnitAimDiv($model));
        $model->unit_aim_div = ($get_div == "1") ? "1" : "0";

        $warning = false;
        if ((isset($model->schregno) && !isset($model->warning) && $model->cmd != "set") || !isset($model->schregno)) {
            $result = $db->query(knjd420nQuery::getHreportTokushiSchregSubclassDat($model, ""));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //パターン取得
        $query = knjd420nQuery::getNameMstPattern($model, $model->schregInfo["GUIDANCE_PATTERN"], 'NAMESPARE1');
        $tmp = $db->getOne($query);
        ...
        knjCreateHidden($objForm, "PRGID", "knjd420n");
        ...
        View::toHTML($model, "knjd420nForm1.html", $arg);
    }
}
...
function makeList(&$arg, $db, &$model) {
    ...
    $query = knjd420nQuery::getList($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    foreach ($setArr as $key => $remarkArr) {
        ...
        //リンクセット
        $setData["SET_SUBCLASSNAME"] = "<a href=\"knjd420nindex.php?cmd=list_set&SEMESTER={$semester}&SUBCLASSCD={$remarkArr["SET_SUBCLASSCD"]}&UNITCD={$remarkArr["UNITCD"]}&SORT={$model->sort}\">{$remarkArr["SET_SUBCLASSNAME"]}</a>";
