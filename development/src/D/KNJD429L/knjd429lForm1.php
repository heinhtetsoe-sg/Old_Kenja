...
class knjd429lForm1 {
    function main(&$model) {
        //オブジェクト作成
        $objForm = new form;
         /* Add by PP for Title 2020-01-16 start */
        $arg["TITLE"]   = "通知表印刷（小学部）画面";
         /* Add by PP for Title 2020-01-16 end */
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd429lForm1", "POST", "knjd429lindex.php", "", "knjd429lForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //学期コンボ
        $query = knjd429lQuery::getSemester();
        // Add by PP for current cursor 2020-01-16 start
        $extra = "id=\"SEMESTER\" onchange=\"current_cursor('SEMESTER'); return btn_submit('main')\"";
        // Add by PP for current cursor 2020-01-16 end
        ...
        if ($model->field["HUKUSIKI_RADIO"] == "1") {
            $query = knjd429lQuery::getSchoolKind($model);
            // Add by PP for current cursor 2020-01-16 start
            $extra = "id=\"SCHOOL_KIND\" onchange=\"current_cursor('SCHOOL_KIND'); return btn_submit('change')\"";
            // Add by PP for current cursor 2020-01-16 end
        ...
        if ($model->field["HUKUSIKI_RADIO"] == "2") {
            $query = knjd429lQuery::getGhr($model);
            $ghr_cd_label = "実クラス";
        } else {
            $query = knjd429lQuery::getGradeHrClass($model);
            $ghr_cd_label = "年組";
        }
        // Add by PP for current cursor 2020-01-16 start
        ...
        if ($model->field["HUKUSIKI_RADIO"] == "2" && $model->field["GHR_CD"] != '') {
            $query = knjd429lQuery::getGhr($model, $model->field["GHR_CD"]);
        ...
        //学校名取得
        $query = knjd429lQuery::getSchoolName();
        ...
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            $arg["data"]["PATERN{$paternCnt}_LABEL"] = "<LABEL for=\"FRM_PATERN{$paternCnt}\">&nbsp;{$row["NAME1"]}</LABEL><br>";
            $paternCnt++;
        }
        $result->free();

        $query = knjd429lQuery::getPaternDefault($model);
        $defPatern = $db->getOne($query);
        $extra = array();
        $findFlg = false;
        foreach($opt as $key => $val) {
            array_push($extra, " id=\"FRM_PATERN{$val}\" ");
            if ($optVal[$key] == $defPatern) {
                $findFlg = true;
            }
        }
        $firstVal = $findFlg ? $defPatern : $firstVal;
        if ($model->cmd == "change" || $model->cmd == "changeHukusiki") {
            $model->field["FRM_PATERN"] = "";
        }

        $model->field["FRM_PATERN"] = $model->field["FRM_PATERN"] ? $model->field["FRM_PATERN"] : $firstVal;
        $radioArray = paternRadio($objForm, "FRM_PATERN", $model->field["FRM_PATERN"], $extra, $opt, get_count($opt), $optVal);
        foreach($radioArray as $key => $val) $arg["data"][$key] = $val;
        ...
        // 表紙印刷
        $checked = $model->field["HYOSI"] == "1" ? " checked" : "";
        $extra = "id=\"HYOSI\"";
        $arg["data"]["HYOSI"] = knjCreateCheckBox($objForm, "HYOSI", "1", $extra.$checked);
        
        // 裏表紙印刷
        $checked = $model->field["URA"] == "1" ? " checked" : "";
        $extra = "id=\"URA\" onchange=\"return btn_submit('main')\"";
        $arg["data"]["URA"] = knjCreateCheckBox($objForm, "URA", "1", $extra.$checked);

        // 押印欄
        $disabled =  $model->field["URA"] == "1" ? "" : " disabled";
        $checked = $model->field["OUIN"] == "1" ? " checked" : "";
        $extra = "id=\"OUIN\"";
        $arg["data"]["OUIN"] = knjCreateCheckBox($objForm, "OUIN", "1", $extra.$checked.$disabled);
        ...
        //終了
        // Edit by PP for PC-Talker(voice) 2020-01-16 start
        $extra = "onclick=\"closeWin();\" aria-label='終了'";
        // Edit by PP for PC-Talker(voice) 2020-01-16 end
        ...
        knjCreateHidden($objForm, "PRGID",         "KNJD429L");
        ...
        View::toHTML($model, "knjd429lForm1.html", $arg);
    }
}
/****************************************************** 以下関数 ************************************************************/
//コンボ作成
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    //対象外の生徒取得
    $query = knjd429lQuery::getSchnoIdou($model, $seme);
    $result = $db->query($query);
    $opt_idou = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    $query = knjd429lQuery::getStudent($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
