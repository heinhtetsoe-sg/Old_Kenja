...
class knjd062bForm1
{
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd062bForm1", "POST", "knjd062bindex.php", "", "knjd062bForm1");
        ...
        //学期コンボボックスを作成する
        $query = knjd062bQuery::getSemester();
        ...
        //学年コンボ
        $query = knjd062bQuery::getSelectGrade($model);
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], "onChange=\"return btn_submit('knjd062b');\"", 1);

        //テスト種別コンボ
        $query = knjd062bQuery::getTestItem($model->field["SEMESTER"]);
        ...
        View::toHTML($model, "knjd062bForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    //クラス一覧
    $row1 = array();
    $result = $db->query(knjd062bQuery::getAuth($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model, $seme, $semeflg)
{
    ...
    $objForm->ae(createHiddenAe("PRGID", "KNJD062B"));
