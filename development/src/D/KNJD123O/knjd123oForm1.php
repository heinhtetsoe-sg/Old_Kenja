...
// kanji=漢字
// $Id: knjd123oForm1.php,v 1.3 2012/06/05 02:02:15 hamasaki Exp $
...
class knjd123oForm1
{
    function main(&$model)
    {
        ...
        $arg["start"]    = $objForm->get_start("main", "POST", "knjd123oindex.php", "", "main");
        $arg["YEAR"]     = CTRL_YEAR;
        ...
        $query = knjd123oQuery::getTestName($model);
        ...
        if ($model->cmd != "subclasscd") {
            $db->query(knjd123oQuery::insertEx_Std_RecQuery($model));
        }
        //科目コンボ
        $opt_sbuclass = array();
        $opt_sbuclass[] = array("label" => "", "value" => "");
        $result = $db->query(knjd123oQuery::selectSubclassQuery($model->gen_ed, $model));
        ...
        $result = $db->query(knjd123oQuery::selectChairQuery($model));
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
...
//                            "KS" => "#ff0099"); //病欠
        ...
        $admin_key = array();
        $result = $db->query(knjd123oQuery::selectContolCodeQuery());
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            ...
            //最初の値を保持 NO009Add*
            if ($seme_kind_item_s == "") $seme_kind_item_s = $seme_test .$kind_test ."01";
            //最後の値を保持 NO009Add*
...
//echo "最初=" .$seme_kind_item_s ." " ."最後=" .$seme_kind_item_e ." " ."素点フラグ=" .$soten_flg ." " ."testcd=" .$model->testcd;
        ...
        if ($seme_test != "0") {
            $result = $db->query(knjd123oQuery::selectExecuteDateQuery($model, $seme_test, $kind_test));
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
            {
...
//echo "seme_test = " .$seme_test ." kind_test = " .$kind_test ." execute_date = " .$execute_date;
        ...
        //遅刻何回で欠課とするかの指数取得
        $absent = array();
        $absent = $db->getRow(knjd123oQuery::getScAbsentCov($model),DB_FETCHMODE_ASSOC);

        //文字評定
//        if ($model->gen_ed == $model->field["SUBCLASSCD"]) {
        if ($model->gen_ed == substr($model->field["SUBCLASSCD"],0,2)) {//NO005
            $assess = array();
            $result = $db->query(knjd123oQuery::GetAssessMark());
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
            {
        ...
        if ($absent["ABSENT_COV"] == "0" || $absent["ABSENT_COV"] == "2") { 
            $result = $db->query(knjd123oQuery::GetAttendData($model->field["CHAIRCD"],$model->field["SUBCLASSCD"],$absent["ABSENT_COV"],$absent["ABSENT_COV_LATE"], $model));
        } else {
            $result = $db->query(knjd123oQuery::GetAttendData2($model->field["CHAIRCD"],$model->field["SUBCLASSCD"],$absent["ABSENT_COV_LATE"], $model));
        }
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        //合併先科目の単位固定／加算フラグ
        $model->creditflg = $db->getOne(knjd123oQuery::getCalculateCreditFlg($model->field["SUBCLASSCD"], $model));
//if (isset($model->creditflg)) echo "))". $model->creditflg ."((" ."<BR>";

        //一覧表示
//        $result = $db->query(knjd123oQuery::selectQuery($model));
        $result = $db->query(knjd123oQuery::selectQuery($model, $execute_date));//---2005.06.21Add
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            //学籍番号を配列で取得
...
//if ($counter==0) {
//$notice_val = explode("-", $model->data["NOTICE_LATE"][0]);
//var_dump($notice_val);
//}
...
//if ($counter<3) {
//$combined_val = explode("-", $model->data["COMBINED_CREDIT_SUM"][$counter]);
//var_dump($combined_val);
//echo "<BR>";
//}
            ...
            foreach ($ctrl_name as $code => $col)
            {
                ...
                //$model->attend_data[$col."-".$counter] = "";//---2005.05.24Modify
                $model->attend_data[$col."-".$counter] = $row[$col."_DI"];//---NO003
...
#                              || strtotime($row["TRANSFER_EDATE"]) >= strtotime($model->control["学期終了日付"][$sem])) {
                ...
                if ($sem != "8" && !strlen($row["CHAIR_SEM".$sem])) {
                    $edit_flg = false;
                    if ($sem <= CTRL_SEMESTER) $row[$col."_COLOR"]="#ffff00";//---2005.05.24Modify
                }
...
#                } elseif(AUTHORITY == DEF_UPDATABLE || in_array($code, $admin_key)) {      //2005/05/23
                    ...
                    $objForm->ae( array("type"      => "text",
                                        ...
                                        "value"     => $value,
                                        "extrahtml" => "STYLE=\"text-align: right\" onChange=\"this.style.background='#ccffcc'\" onblur=\"calc(this);\" onKeyDown=\"keyChangeEntToTab(this)\" "));
        ...
        //累積現在日
        $cur_date = $db->getRow(knjd123oQuery::GetMax($model->field["SUBCLASSCD"], $model), DB_FETCHMODE_ASSOC);
...
******/
        ...
        if($model->field["CHK_FLG"] == "reset")
        {
            ...
            //データ取得
            $query = knjd123oQuery::getSchChrTestData($model->field["CHAIRCD"]);
            #var_dump($query);
...
//                            "value"     => ($model->field["SUBCLASSCD"] == $model->gen_ed ? $model->gen_ed : "" ) ) );
        ...
        $objForm->ae( array("type"      => "hidden",
                            "name"      => "PRGID",
                            "value"     => "KNJD123" )  );//---2005.05.24Modify
        ...
        View::toHTML($model, "knjd123oForm1.html", $arg);
    }
}
?>
