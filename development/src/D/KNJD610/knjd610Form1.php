...
class knjd610Form1
{
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd610Form1", "POST", "knjd610index.php", "", "knjd610Form1");
        ...
        //学期コンボボックスを作成する
        $query = knjd610Query::getSemester();
        ...
        //学校名取得
        $query = knjd610Query::getSchoolname();
        $schoolName = $db->getOne($query);

        //学年コンボ
        $query = knjd610Query::getSelectGrade($model);
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], "onChange=\"return btn_submit('knjd610');\"", 1);

        //テスト種別コンボ
        $query = knjd610Query::getTestItem($model->field["SEMESTER"]);
        ...
        View::toHTML($model, "knjd610Form1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    //クラス一覧
    $row1 = array();
    $result = $db->query(knjd610Query::getAuth($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model, $seme, $semeflg)
{
    ...
    $objForm->ae(createHiddenAe("PRGID", "KNJD610"));
    ...
    //単位マスタのABSENCE_HIGHはDECIMALか
    $objForm->ae(createHiddenAe("creditMstAbsenceHighIsDecimal", $model->creditMstAbsenceHighIsDecimal));
    //教育課程コード
