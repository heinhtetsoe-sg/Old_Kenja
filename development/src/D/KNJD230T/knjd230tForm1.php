...
class knjd230tForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd230tForm1", "POST", "knjd230tindex.php", "", "knjd230tForm1");

        //年度
        $arg["data"]["YEAR"] = CTRL_YEAR;

        //学年コンボボックスを作成する
        $query = knjd230tQuery::getGrade($model);
        $extra = "onChange=\"return btn_submit('knjd230t');\"";
        ...
        View::toHTML($model, "knjd230tForm1.html", $arg);
    }
}
...
{
    $row1 = array();
    $query = knjd230tQuery::getAuth($model);
...
function makeHidden(&$objForm, $model)
{
        ...
        knjCreateHidden($objForm, "PRGID", "KNJD230T");
        knjCreateHidden($objForm, "cmd", "");
        knjCreateHidden($objForm, "COUNTFLG", $model->testTable);  //テスト種別テーブル
        knjCreateHidden($objForm, "TESTKINDCD", "9900");           //テスト種別
        knjCreateHidden($objForm, "YEAR", CTRL_YEAR);              //年度
        knjCreateHidden($objForm, "SEMESTER", "9");                //学期
        knjCreateHidden($objForm, "CTRL_SEMESTER", CTRL_SEMESTER); //処理学期
