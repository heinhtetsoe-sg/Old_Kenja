...
class knjd185pForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd185pForm1", "POST", "knjd185pindex.php", "", "knjd185pForm1");
        ...
        //学期コンボ作成
        $query = knjd185pQuery::getSemester($getCountsemester);
        $extra = "onchange=\"return btn_submit('knjd185p');\"";
        ...
        //クラスコンボ作成
        $query = knjd185pQuery::getHrClass(CTRL_YEAR, $seme, $model);
        $extra = "onchange=\"return btn_submit('knjd185p');\"";
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd185p', 'on')\"".$disabled;
        ...
        View::toHTML($model, "knjd185pForm1.html", $arg);
    }
}

function makeStudentList(&$objForm, &$arg, $db, $model, $seme) {
    //対象外の生徒取得
    $query = knjd185pQuery::getSchnoIdou($model, $seme);
    ...
    //対象者リストを作成する
    $query = knjd185pQuery::getStudent($model, $seme);
    $result = $db->query($query);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD185P");
