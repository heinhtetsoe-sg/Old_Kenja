...
class knjd192eForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd192eForm1", "POST", "knjd192eindex.php", "", "knjd192eForm1");
        //DB接続
        $db = Query::dbCheckOut();

        //学校名取得
        $query = knjd192eQuery::getSchoolname();
        ...
        $extra = array("id=\"CATEGORY_IS_CLASS1\" onClick=\"return btn_submit('knjd192e')\"", "id=\"CATEGORY_IS_CLASS2\" onClick=\"return btn_submit('knjd192e')\"");
        ...
        if ($model->field["DATE_DIV"] == "") {
            $model->field["DATE_DIV"] = $model->Properties["knjd192eDefaultAttendDateDiv"] ? $model->Properties["knjd192eDefaultAttendDateDiv"] : "1";
        }
        $opt_datediv = array(1, 2);
        $extra2 = " onclick=\"return btn_submit('knjd192e');\"";
        ...
        //学期コンボ作成
        $query = knjd192eQuery::getSemester();
        $extra = "onchange=\"return btn_submit('change_grade'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //出欠集計開始日付
        $query = knjd192eQuery::getSemesterDetail($value);
        ...
        if ($model->field["DATE_DIV"] == "1") {
            $query = knjd192eQuery::getSemesterDetailMst(CTRL_YEAR, "1");
            $result = $db->query($query);
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                $sDate = $row["SDATE"];
            }
        ...
        //学年コンボ作成
        $query = knjd192eQuery::getGradeHrClass($model->field["SEMESTER"], $model, "GRADE");
        $extra = "onchange=\"return btn_submit('change_grade'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        if ($model->field["CATEGORY_IS_CLASS"] == 2){
            //クラスコンボ作成
            $query = knjd192eQuery::getGradeHrClass($model->field["SEMESTER"], $model, "HR_CLASS");
            $extra = "onchange=\"return btn_submit('knjd192e'), AllClearList();\"";
            makeCmb($objForm, $arg, $db, $query, "HR_CLASS", $model->field["HR_CLASS"], $extra, 1);
        }

        //テストコンボ作成
        $query = knjd192eQuery::getTest($model->field["SEMESTER"], $model->field["GRADE"]);
        $opt = array();
        ...
        //学校マスタ取得
        $schInfo = $db->getRow(knjd192eQuery::getSchoolMst($model), DB_FETCHMODE_ASSOC);
        ...
        View::toHTML($model, "knjd192eForm1.html", $arg); 
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["CATEGORY_IS_CLASS"] == 1){
        ...
        //対象外の生徒取得
        $query = knjd192eQuery::getSchnoIdou($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt_idou = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //対象者リストを作成する
        $query = knjd192eQuery::getStudent($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt1 = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $idou = "　";
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD192E");
