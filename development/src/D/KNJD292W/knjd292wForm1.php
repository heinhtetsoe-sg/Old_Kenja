...
class knjd292wForm1
{
    public function main(&$model)
    {
        ...
        $objForm = new form();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd292wForm1", "POST", "knjd292windex.php", "", "knjd292wForm1");
        ...
        //学期コンボボックスを作成する
        $query = knjd292wQuery::getSemester();
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], "onchange=\"return btn_submit('gakki');\"", 1);

        //年組コンボ
        $query = knjd292wQuery::getHrClass($model);
        makeCmb($objForm, $arg, $db, $query, "GRADE_HR_CLASS", $model->field["GRADE_HR_CLASS"], "onChange=\"return btn_submit('knjd292w');\"", 1);
        ...
        View::toHTML($model, "knjd292wForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    //生徒一覧
    $row1 = array();
    $result = $db->query(knjd292wQuery::getStudent($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD292W");
    knjCreateHidden($objForm, "cmd");
    knjCreateHidden($objForm, "YEAR", CTRL_YEAR);
    knjCreateHidden($objForm, "CTRL_SEMESTER", CTRL_SEMESTER);
    knjCreateHidden($objForm, "LOGIN_DATE", CTRL_DATE);
    knjCreateHidden($objForm, "HID_SELECTED");
