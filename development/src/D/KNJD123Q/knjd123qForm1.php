...
class knjd123qForm1
{
    function main(&$model)
    {
        ...
        $arg["start"]    = $objForm->get_start("main", "POST", "knjd123qindex.php", "", "main");
        $arg["YEAR"]     = CTRL_YEAR;
        ...
        $result = $db->query(knjd123qQuery::getNameMstD022());
        ...
        $query = knjd123qQuery::getTestName($model);
        ...
        if ($model->cmd != "subclasscd") {
            $db->query(knjd123qQuery::insertEx_Std_RecQuery($model));
        }
        //科目コンボ
        $opt_sbuclass = array();
        $opt_sbuclass[] = array("label" => "", "value" => "");
        $result = $db->query(knjd123qQuery::selectSubclassQuery($model->gen_ed, $model));
        //教育課程対応
        ...
        $result = $db->query(knjd123qQuery::selectChairQuery($model));
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        $admin_key = array();
        $result = $db->query(knjd123qQuery::selectContolCodeQuery());
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        if ($seme_test != "0") {
            $result = $db->query(knjd123qQuery::selectExecuteDateQuery($model, $seme_test, $kind_test, $item_test));
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
            {
        ...
        //遅刻何回で欠課とするかの指数取得
        $absent = array();
        $absent = $db->getRow(knjd123qQuery::getScAbsentCov($model),DB_FETCHMODE_ASSOC);
        ...
        $offdaysFlg = $db->getRow(knjd123qQuery::getOffdaysFlg(CTRL_YEAR),DB_FETCHMODE_ASSOC);
        //帳票パラメータ
        ...
        if ($absent["ABSENT_COV"] == "0" || $absent["ABSENT_COV"] == "2" || $absent["ABSENT_COV"] == "4") { 
            $result = $db->query(knjd123qQuery::GetAttendData($model->field["CHAIRCD"],$model->field["SUBCLASSCD"],$absent["ABSENT_COV"],$absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        } else {
            $result = $db->query(knjd123qQuery::GetAttendData2($model->field["CHAIRCD"],$model->field["SUBCLASSCD"],$absent["ABSENT_COV"],$absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        }
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        $model->creditflg = "";
        $motoCnt = 0;
        $result = $db->query(knjd123qQuery::getCalculateCreditFlg($model->field["SUBCLASSCD"], $model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //一覧表示
        $colorFlg = false;
        $result = $db->query(knjd123qQuery::selectQuery($model, $execute_date));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            //学籍番号を配列で取得
...
//          $row["NAME_SHOW"]   = $row["SCHREGNO"]." ".$row["NAME_SHOW"];
            ...
            foreach ($ctrl_name as $code => $col)
            {
                ...
                //管理者コントロール
                    ...
                    //考査満点マスタ
                    $query = knjd123qQuery::getPerfect(CTRL_YEAR, $model->field["SUBCLASSCD"], $perfectcd[$code], $row["GRADE"], $row["COURSE"], $model);
            ...
            if (0 < get_count($admin_key)) {
                $objForm->ae( array("type"      => "text",
                                    ...
                                    "maxlength" => "20",
                                    "value"     => $row["REMARK"],
                                    "extrahtml" => "STYLE=\"WIDTH:100%\" WIDTH=\"100%\" onChange=\"this.style.background='#ccffcc'\" onKeyDown=\"keyChangeEntToTab(this)\" onPaste=\"return show(this);\" "));
        ...
        //累積現在日
        $cur_date = $db->getRow(knjd123qQuery::GetMax($model->field["SUBCLASSCD"], $model), DB_FETCHMODE_ASSOC);
...
******/
        ...
        if($model->field["CHK_FLG"] == "reset")
        {
            ...
            //データ取得
            $query = knjd123qQuery::getSchChrTestData($model->field["CHAIRCD"]);
            #var_dump($query);
        ...
        foreach ($testcdArray as $testkey => $testval) {
            $chk = '';
            $dis = '';
            $query = knjd123qQuery::getRecordChkfinDat($model, $testval);
    ...
    // "成績入力完了"処理 おわり //
        ...
        View::toHTML($model, "knjd123qForm1.html", $arg);
    }
}
?>
