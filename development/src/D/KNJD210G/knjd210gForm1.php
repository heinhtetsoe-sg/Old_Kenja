...
class knjd210gForm1
{
    function main(&$model)
    {
        $db = Query::dbCheckOut();
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("main", "POST", "knjd210gindex.php", "", "main");
        ...
        $result = $db->query(knjd210gQuery::GetSemester());
        ...
        $result = $db->query(knjd210gQuery::GetGrade($seme));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            $model->gradeArray[] = array("label" => sprintf("%d",$row["GRADE"])."学年", "value" => $row["GRADE"]);
        }
        if (0 < get_count($model->gradeArray)) $model->gradeArray[] = array("label" => "-全て-", "value" => "99");

        $objForm->ae( array("type"       => "select",
                            "name"       => "GRADE",
                            "size"       => "1",
                            "value"      => $model->grade,
                            "options"    => $model->gradeArray));
        $arg["GRADE"]   = $objForm->ge("GRADE");

        //処理種別(成績)
        $opt_exam = array();
        $result = $db->query(knjd210gQuery::GetName($model->seme));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        View::toHTML($model, "knjd210gForm1.html", $arg);
    }
}
?>
