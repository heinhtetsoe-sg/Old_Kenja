...
class knjd126iForm2
{
    function main(&$model)
    {
        ...
        $arg["start"]    = $objForm->get_start("form2", "POST", "knjd126iindex.php", "", "form2");
        $arg["YEAR"]     = CTRL_YEAR;
            ...
            foreach ($view_key as $code => $col) {
                ...
                if(in_array($model->field["SEMESTER"], $admin_key) && DEF_UPDATE_RESTRICT <= AUTHORITY) {
                    ...
                    if ($model->Properties["displayHyoutei"] != "1") {
                        $extra = "STYLE=\"text-align: center\" onChange=\"this.style.background='#ccffcc'\" onblur=\"calc(this, '".$electdiv."');\" onPaste=\"return showPaste(this);\" id=\"STATUS{$code}-{$counter}\"";
        ...
        View::toHTML($model, "knjd126iForm2.html", $arg);
    }
}
//コンボ作成
