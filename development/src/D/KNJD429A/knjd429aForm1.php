...
class knjd429aForm1 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd429aForm1", "POST", "knjd429aindex.php", "", "knjd429aForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //学期コンボ
        $query = knjd429aQuery::getSemester();
        ...
        if ($model->field["HUKUSIKI_RADIO"] == "1") {
            $query = knjd429aQuery::getSchoolKind($model);
            // Add by PP for current cursor 2020-01-16 start
        ...
        if ($model->field["HUKUSIKI_RADIO"] == "2") {
            $query = knjd429aQuery::getGhr($model);
            $ghr_cd_label = "実クラス";
        } else {
            $query = knjd429aQuery::getGradeHrClass($model);
            $ghr_cd_label = "年組";
        }
        // Add by PP for current cursor 2020-01-16 start
        ...
        if ($model->field["HUKUSIKI_RADIO"] == "2" && $model->field["GHR_CD"] != '') {
            $query = knjd429aQuery::getGhr($model, $model->field["GHR_CD"]);
        ...
        //学校名取得
        $query = knjd429aQuery::getSchoolName();
        ...
        $query = knjd429aQuery::getPaternDefault($model);
        $defPatern = $db->getOne($query);
        $firstVal = $defPatern ? $defPatern : $firstVal;
        if ($model->cmd == "main" || $model->cmd == "changeHukusiki") {
        ...
        knjCreateHidden($objForm, "PRGID",         "KNJD429A");
        ...
        View::toHTML($model, "knjd429aForm1.html", $arg);
    }
}
/****************************************************** 以下関数 ************************************************************/
//コンボ作成
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    //対象外の生徒取得
    $query = knjd429aQuery::getSchnoIdou($model, $seme);
    $result = $db->query($query);
    $opt_idou = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    $query = knjd429aQuery::getStudent($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
