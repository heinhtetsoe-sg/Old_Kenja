...
class knjd192bForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd192bForm1", "POST", "knjd192bindex.php", "", "knjd192bForm1");
        ...
        $extra = array("id=\"CATEGORY_IS_CLASS1\" onClick=\"return btn_submit('knjd192b')\"", "id=\"CATEGORY_IS_CLASS2\" onClick=\"return btn_submit('knjd192b')\"");
        ...
        //学期コンボ作成
        $query = knjd192bQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd192b'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //テストコンボ作成
        $query = knjd192bQuery::getTest($model->field["SEMESTER"]);
        makeCmb($objForm, $arg, $db, $query, "TESTCD", $model->field["TESTCD"], "", 1);

        //学年コンボ作成
        $query = knjd192bQuery::getGradeHrClass($model->field["SEMESTER"], $model, "GRADE");
        $extra = "onchange=\"return btn_submit('change_grade'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        if ($model->field["CATEGORY_IS_CLASS"] == 2){
            //クラスコンボ作成
            $query = knjd192bQuery::getGradeHrClass($model->field["SEMESTER"], $model, "HR_CLASS");
            $extra = "onchange=\"return btn_submit('knjd192b'), AllClearList();\"";
        ...
        $extra .= " id=\"ASSESS_LEVEL\" onclick=\"return btn_submit('knjd192b');\"";
        $arg["data"]["ASSESS_LEVEL"] = knjCreateCheckBox($objForm, "ASSESS_LEVEL", "1", $extra, "");

        //読替ありチェックボックス
        $extra  = ($model->field["ASSESS_LEVEL_REP"] == "1" || $model->field["ASSESS_LEVEL"] == "1" || $model->cmd == "") ? "checked" : "";
        $extra .= " id=\"ASSESS_LEVEL_REP\"";
        $arg["data"]["ASSESS_LEVEL_REP"] = knjCreateCheckBox($objForm, "ASSESS_LEVEL_REP", "1", $extra, "");

        $arg["ASSESS"] = ($model->field["ASSESS_LEVEL"] == "1") ? "" : "1";

        //平均・席次ラジオボタンのラベル
        $arg["data"]["GROUP_LABEL"] = ($model->field["ASSESS_LEVEL"] == "1") ? '平均・段階値' : '平均・順位';

        //平均・席次・偏差値ラジオボタン 1:学級・学年 2:学級・コース 3:コース・学年
        ...
        if ($model->cmd == 'change_grade' || $model->cmd == '') {
            $query = knjd192bQuery::getGdat($model->field["GRADE"]);
            $h_j = $db->getOne($query);
            if ($h_j == 'J') {
        ...
        View::toHTML($model, "knjd192bForm1.html", $arg); 
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["CATEGORY_IS_CLASS"] == 1){
        ...
        //対象外の生徒取得
        $query = knjd192bQuery::getSchnoIdou($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt_idou = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //対象者リストを作成する
        $query = knjd192bQuery::getStudent($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt1 = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $idou = "　";
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD192B");
