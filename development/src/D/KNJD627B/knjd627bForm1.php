...
class knjd627bForm1
{
    public function main(&$model)
    {
        ...
        //対象年における3年次のクラス一覧（３年次のみ）を取得表示
        $query = knjd627bQuery::selectClassListInYear($model, CTRL_SEMESTER, 3);
        ...
        foreach ($values as $val) {
            $extra[] = " id=\"PRINT_TARGET_KIND{$val}\"";
            $arg["radio"]["PRINT_TARGET_KIND{$val}_ID"] = "PRINT_TARGET_KIND{$val}";
        }
        $radioArray = knjCreateRadio($objForm, "PRINT_TARGET_KIND", $model->printTarget, $extra, $values, get_count($values));
        ...
        $query = knjd627bQuery::selectStudentOfClsOnYr($model, CTRL_SEMESTER);
        $result = $db->query($query);
        while (is_null($result) == false && $row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //HTML出力終了
        $arg["start"]  = $objForm->get_start("main", "POST", "knjd627bindex.php", "", "main");
        $arg["finish"] = $objForm->get_finish();
        View::toHTML($model, "knjd627bForm1.html", $arg);
    }
}
