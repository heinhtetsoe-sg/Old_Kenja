...
class knjd672Form1 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]  = $objForm->get_start("knjd672Form1", "POST", "knjd672index.php", "", "knjd672Form1");
        ...
        //学期コンボ
        $extra = "onChange=\"return btn_submit('knjd672');\"";
        $query = knjd672Query::getSemester();
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //学年コンボ
        $extra = "onChange=\"return btn_submit('knjd672');\"";
        $query = knjd672Query::getGrade();
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        //年組コンボ
        $extra = "onChange=\"return btn_submit('knjd672');\"";
        $query = knjd672Query::getHrclass($model);
        makeCmb($objForm, $arg, $db, $query, "GRADE_HR_CLASS", $model->field["GRADE_HR_CLASS"], $extra, 1);

        //学校校種を取得
        $model->schoolkind = $db->getOne(knjd672Query::getSchoolKind($model));

        //テスト種別コンボ
        $extra = "onChange=\"return btn_submit('knjd672');\"";
        $query = knjd672Query::getTestitem($model);
        ...
        View::toHTML($model, "knjd672Form1.html", $arg); 
    }
}
...
{
    $row1 = array();
    $query = knjd672Query::getStudent($model);
    $result = $db->query($query);
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
...
function makeHidden(&$objForm, $model, $db) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD672");
}
?>
