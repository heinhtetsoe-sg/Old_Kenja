...
class knjd210eForm1
{
    function main(&$model)
    {
        $db = Query::dbCheckOut();
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("main", "POST", "knjd210eindex.php", "", "main");
        ...
        $result = $db->query(knjd210eQuery::GetSemester());
        ...
        //処理学年
        $opt = array();
        $result = $db->query(knjd210eQuery::GetGrade());
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        //処理種別(成績)
        $opt_exam = array();
        $result = $db->query(knjd210eQuery::GetName($model->seme));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        View::toHTML($model, "knjd210eForm1.html", $arg);
    }
}
?>
