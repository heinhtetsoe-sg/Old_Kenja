...
class knjd157aForm1
{
    public function main(&$model)
    {
        ...
        $objForm = new form();
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd157aForm1", "POST", "knjd157aindex.php", "", "knjd157aForm1");
        ...
        $extra = array("id=\"CATEGORY_IS_CLASS1\" onClick=\"return btn_submit('knjd157a')\"", "id=\"CATEGORY_IS_CLASS2\" onClick=\"return btn_submit('knjd157a')\"");
        ...
        //学期コンボ作成
        $query = knjd157aQuery::getSemester();
        $extra = "onchange=\"return btn_submit('change_semes'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //テストコンボ作成
        $query = knjd157aQuery::getTest($model->field["SEMESTER"]);
        makeCmb($objForm, $arg, $db, $query, "TESTCD", $model->field["TESTCD"], "", 1);

        //学年コンボ作成
        $query = knjd157aQuery::getGradeHrClass($model, $model->field["SEMESTER"], "GRADE");
        $extra = "onchange=\"return btn_submit('change_grade'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        if ($model->field["CATEGORY_IS_CLASS"] == 2) {
            //クラスコンボ作成
            $query = knjd157aQuery::getGradeHrClass($model, $model->field["SEMESTER"], "HR_CLASS");
            $extra = "onchange=\"return btn_submit('knjd157a'), AllClearList();\"";
            makeCmb($objForm, $arg, $db, $query, "HR_CLASS", $model->field["HR_CLASS"], $extra, 1);
        }

        //講座・類型ラジオボタン 1:講座 2:類型
        $opt_div = array(1, 2);
        $model->field["type_course"] = ($model->field["type_course"] == "") ? "1" : $model->field["type_course"];
        $extra = array("id=\"type_course1\" checked", "id=\"type_course2\"");
        $radioArray = knjCreateRadio($objForm, "type_course", $model->field["type_course"], $extra, $opt_div, get_count($opt_div));
        foreach ($radioArray as $key => $val) {
            $arg["data"][$key] = $val;
        }
        ...
        View::toHTML($model, "knjd157aForm1.html", $arg);
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model)
{
    if ($model->field["CATEGORY_IS_CLASS"] == 1) {
        ...
        //対象者リストを作成する
        $query = knjd157aQuery::getStudent($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt1 = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD157A");
    ...
    knjCreateHidden($objForm, "knjd157aNotPrintMajorname", $model->Properties["knjd157aNotPrintMajorname"]);
