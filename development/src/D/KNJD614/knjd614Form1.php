...
class knjd614Form1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd614Form1", "POST", "knjd614index.php", "", "knjd614Form1");
        ...
        //学期コンボ作成
        $query = knjd614Query::getSemester();
        $extra = "onchange=\"return btn_submit('knjd614'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);
        //学年末は今学期とする
        $seme = $model->field["SEMESTER"] == "9" ? CTRL_SEMESTER : $model->field["SEMESTER"];

        //学年コンボ作成
        $query = knjd614Query::getSchoolName();
        $schoolName = $db->getOne($query);
        if ($schoolName == 'CHIBEN') {
            $query = knjd614Query::getGradeChiben($seme);
            $extra = "onchange=\"return btn_submit('knjd614'), AllClearList();\"";
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
        } else {
            $query = knjd614Query::getGrade($seme);
            $extra = "onchange=\"return btn_submit('knjd614'), AllClearList();\"";
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
        }

        //テストコンボ作成
        $query = knjd614Query::getTest($model->field["SEMESTER"]);
        ...
        View::toHTML($model, "knjd614Form1.html", $arg); 
    }
}
...
function makeClassList(&$objForm, &$arg, $db, $model, $seme) {
    ...
    //クラス一覧リストを作成する
    $query = knjd614Query::getHrClass($seme, $model->field["GRADE"]);
...
function makeHidden(&$objForm, $model, $seme) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD614");
