...
class knjd128f_2Form1
{
    public function main(&$model)
    {
        ...
        $arg["start"]    = $objForm->get_start("main", "POST", "knjd128f_2index.php", "", "main");
        $arg["YEAR"]     = CTRL_YEAR;
        ...
        //テスト名取得
        $semename = $testname = "";
        $query = knjd128f_2Query::getTestName($model);
        ...
        //科目コンボ
        $opt_sbuclass = array();
        $result = $db->query(knjd128f_2Query::selectSubclassQuery($model));
        //教育課程対応
        ...
        //講座コンボ
        $opt_chair = array();
        $result = $db->query(knjd128f_2Query::selectChairQuery($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $result = $db->query(knjd128f_2Query::selectContolCodeQuery($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($seme_test != "0") {
            $result = $db->query(knjd128f_2Query::selectExecuteDateQuery($model, $seme_test, $kind_test, $item_test));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //遅刻何回で欠課とするかの指数取得
        $absent = array();
        $absent = $db->getRow(knjd128f_2Query::getScAbsentCov($model), DB_FETCHMODE_ASSOC);

        //文字評定
        if ($model->gen_ed == substr($model->field["SUBCLASSCD"], 0, 2)) {
            $assess = array();
            $result = $db->query(knjd128f_2Query::getAssessMark());
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $offdaysFlg = $db->getRow(knjd128f_2Query::getOffdaysFlg(CTRL_YEAR), DB_FETCHMODE_ASSOC);
        //帳票パラメータ
        $objForm->ae(array("type"      => "hidden",
                            "name"      => "SUB_OFFDAYS",
        ...
        if ($absent["ABSENT_COV"] == "0" || $absent["ABSENT_COV"] == "2" || $absent["ABSENT_COV"] == "4" || $absent["ABSENT_COV"] == "5") {
            $result = $db->query(knjd128f_2Query::getAttendData($model->field["CHAIRCD"], $model->field["SUBCLASSCD"], $absent["ABSENT_COV"], $absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        } else {
            $result = $db->query(knjd128f_2Query::getAttendData2($model->field["CHAIRCD"], $model->field["SUBCLASSCD"], $absent["ABSENT_COV"], $absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        }
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //合併先科目の単位固定／加算フラグ
        $model->creditflg = $db->getOne(knjd128f_2Query::getCalculateCreditFlg($model->field["SUBCLASSCD"], $model));

        //一覧表示
        if (strlen($execute_date)) {
            $colorFlg = false;
            $result = $db->query(knjd128f_2Query::selectQuery($model, $execute_date));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                //学籍番号をHiddenで保持
                knjCreateHidden($objForm, "SCHREGNO"."-".$counter, $row["SCHREGNO"]);
                knjCreateHidden($objForm, "TAKESEMES"."-".$counter, $row["TAKESEMES"]);
                //クラス-出席番(表示)
                ...
                foreach ($ctrl_name as $code => $col) {
                    ...
                    //管理者コントロール
                        ...
                        //考査満点マスタ
                        $query = knjd128f_2Query::getPerfect(CTRL_YEAR, $model->field["SUBCLASSCD"], $testcdArray[$code], $row["GRADE"], $row["COURSE"], $model);
        ...
        //累積現在日
        $cur_date = $db->getRow(knjd128f_2Query::getMax($model->field["SUBCLASSCD"], $model), DB_FETCHMODE_ASSOC);
        ...
        foreach ($testcdArray as $testkey => $testval) {
            ...
            $query = knjd128f_2Query::getRecordChkfinDat($model, $testval, $recordDiv);
            //} else {
            //    $query = knjd128f_2Query::getRecordSchChrTest($model, $testval);
            //}
        ...
        } elseif ($model->prgid == "KNJD128F") {
            $jump  = REQUESTROOT."/D/KNJD128F/knjd128findex.php";
        ...
        View::toHTML($model, "knjd128f_2Form1.html", $arg);
    }
}
