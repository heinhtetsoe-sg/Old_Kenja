...
class knjd187rForm1
{
    public function main(&$model)
    {
        ...
        //学期
        $query = knjd187rQuery::getSemester();
        $result = $db->query($query);
        $list = array();
        while (is_null($result) == false && $row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $list[] = array("label" => $row["LABEL"] , "value" => $row["VALUE"]);
            if ($row["VALUE"] == $model->semester) {
                $model->semester_startDate = $row["SDATE"];
                $model->semester_endDate   = $row["EDATE"];
            }
        }
        $result->free();
        $arg["SEMESTER"] = knjCreateCombo($objForm, "SEMESTER", $model->semester, $list, $postback, 1);

        //出欠集計日付
        $arg["SDATE"] = str_replace("-", "/", $model->semester_startDate);
        $arg["EDATE"] = View::popUpCalendar2($objForm, "EDATE", str_replace("-", "/", CTRL_DATE), "", "", "");

        //対象年におけるクラス一覧
        $query = knjd187rQuery::selectClassListInYear($model);
        $result = $db->query($query);
        $list = array();
        while (is_null($result) == false && $row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $grade   = $row["GRADE"];
        ...
        $query = knjd187rQuery::selectStudentOfClsOnYr($model);
        $result = $db->query($query);
        while (is_null($result) == false && $row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        knjCreateHidden($objForm, "SDATE", str_replace("-", "/", $model->semester_startDate));
        knjCreateHidden($objForm, "HID_LIMIT_DATE", str_replace("-", "/", $model->semester_endDate));
        ...
        //HTML出力終了
        $arg["start"]  = $objForm->get_start("main", "POST", "knjd187rindex.php", "", "main");
        $arg["finish"] = $objForm->get_finish();
        View::toHTML($model, "knjd187rForm1.html", $arg);
    }
}
