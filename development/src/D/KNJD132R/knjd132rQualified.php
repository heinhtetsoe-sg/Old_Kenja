...
class knjd132rQualified {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("qualified", "POST", "knjd132rindex.php", "", "qualified");
        ...
        $counter = 0;
        if ($model->schregno) {
            $result = $db->query(knjd132rQuery::getAward($model));
        ...
        if (VARS::get("cmd") == "right_list"){ 
            $arg["reload"] = "parent.edit_frame.location.href='knjd132rindex.php?cmd=edit'";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjd132rQualified.html", $arg);
    }
}
