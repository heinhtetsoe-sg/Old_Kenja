...
// kanji=漢字
// $Id: knjd125bForm1.php 76259 2020-08-27 08:52:32Z arakaki $
...
class knjd125bForm1
{
    public function main(&$model)
    {
        ...
        $arg["start"]    = $objForm->get_start("main", "POST", "knjd125bindex.php", "", "main");
        $arg["YEAR"]     = CTRL_YEAR;
        ...
        $result = $db->query(knjd125bQuery::selectSubclassQuery($model->gen_ed));
        ...
        $result = $db->query(knjd125bQuery::selectChairQuery($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //CSV出力ファイル名
        $objUp->setFileName(CTRL_YEAR."年度_".$subclassname."_".$chairname."_"."成績入力(基本).csv");
        ...
        $admin_key = array();
        $result = $db->query(knjd125bQuery::selectContolCodeQuery());
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($seme_test != "0") {
            $result = $db->query(knjd125bQuery::selectExecuteDateQuery($model, $seme_test, $kind_test));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //遅刻何回で欠課とするかの指数取得
        $absent = array();
        $absent = $db->getRow(knjd125bQuery::getScAbsentCov($model), DB_FETCHMODE_ASSOC);

        //前期評点・後期評点・学年評定
        $assess = array();
        $result = $db->query(knjd125bQuery::getAssessMark());
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $offdaysFlg = $db->getRow(knjd125bQuery::getOffdaysFlg(CTRL_YEAR), DB_FETCHMODE_ASSOC);
        //帳票パラメータ
        ...
        if ($absent["ABSENT_COV"] == "0" || $absent["ABSENT_COV"] == "2") {
            $result = $db->query(knjd125bQuery::getAttendData($model->field["CHAIRCD"], $model->field["SUBCLASSCD"], $absent["ABSENT_COV"], $absent["ABSENT_COV_LATE"], $offdaysFlg));
        } else {
            $result = $db->query(knjd125bQuery::getAttendData2($model->field["CHAIRCD"], $model->field["SUBCLASSCD"], $absent["ABSENT_COV_LATE"], $offdaysFlg));
        }
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //合併先科目の単位固定／加算フラグ
        $model->creditflg = $db->getOne(knjd125bQuery::getCalculateCreditFlg($model->field["SUBCLASSCD"]));

        //一覧表示
        $colorFlg = false;
        $result = $db->query(knjd125bQuery::selectQuery($model, $execute_date));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //学籍番号を配列で取得
            ...
            $objUp->setSize(array(7=>3,8=>2,9=>3,10=>2,11=>3,12=>2,13=>3,14=>2,15=>3,16=>2,17=>3,18=>2,19=>1,20=>2,21=>2,22=>5));
            ...
            foreach ($ctrl_name as $code => $col) {
                ...
                //管理者コントロール
                    ...
                    //考査満点マスタ
                    $query = knjd125bQuery::getPerfect(CTRL_YEAR, $model->field["SUBCLASSCD"], $perfectcd[$code], $row["GRADE"], $row["COURSE"]);
        ...
        //累積現在日
        $cur_date = $db->getRow(knjd125bQuery::GetMax($model->field["SUBCLASSCD"]), DB_FETCHMODE_ASSOC);
        ...
        if ($model->field["CHK_FLG"] == "reset") {
            ...
            //データ取得
            $query = knjd125bQuery::getSchChrTestData($model->field["CHAIRCD"]);
        ...
        View::toHTML($model, "knjd125bForm1.html", $arg);
    }
}
