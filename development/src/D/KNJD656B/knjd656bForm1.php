...
class knjd656bForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd656bForm1", "POST", "knjd656bindex.php", "", "knjd656bForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //学期コンボ作成
        $query = knjd656bQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd656b'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //学年コンボ作成
        $query = knjd656bQuery::getGradeHrClass($model->field["SEMESTER"], $model, "GRADE");
        $extra = "onchange=\"return btn_submit('knjd656b'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        //模試グループ
        $query = knjd656bQuery::getGroupcd($model);
        $extra = "onchange=\"return btn_submit('knjd656b'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GROUPCD", $model->field["GROUPCD"], $extra, 1);

        //模試コード
        $query = knjd656bQuery::getMockcd($model);
        $extra = "";
        ...
        View::toHTML($model, "knjd656bForm1.html", $arg); 
    }
}

function makeStudentList(&$objForm, &$arg, $db, $model) {
    //対象クラスリストを作成する
    $query = knjd656bQuery::getGradeHrClass($model->field["SEMESTER"], $model, "HR_CLASS");
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD656B");
