...
class knjd128v_mikomiForm1
{
    function main(&$model)
    {
        $objForm = new form;

        $arg["start"]    = $objForm->get_start("main", "POST", "knjd128v_mikomiindex.php", "", "main");
        ...
        if ($model->Properties["use_school_detail_gcm_dat"] == '1') {
            ...
            $query = knjd128v_mikomiQuery::getCourseMajor($model);
        ...
        $query = knjd128v_mikomiQuery::getSubclassMst($model);
        ...
        if ($model->Properties["useCurriculumcd"] == '1') {
            if ($model->field["SUBCLASSCD"]) {
                ...
                $schoolKind = $db->getOne(knjd128v_mikomiQuery::getSchoolkindQuery());
            }
            $schoolKind = $db->getOne(knjd128v_mikomiQuery::getAdminSchoolKind($schoolKind));
        ...
        $query = knjd128v_mikomiQuery::selectChairQuery($model);
        ...
        View::toHTML($model, "knjd128v_mikomiForm1.html", $arg);
    }
}
...
function makeHead(&$objForm, &$arg, $db, $model) {
    //科目別設定があるか
    $testSubCnt = $db->getOne(knjd128v_mikomiQuery::getTestSubCnt($model));
    ...
    $head2 = "";
    $testcdArray = array();
    $result = $db->query(knjd128v_mikomiQuery::getTestName($model, $testSubCnt));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($row["TESTCD"] == $model->field["TESTCD"]) $send_testname = $row["SEMESTERNAME"]."<BR>".$row["TESTITEMNAME"]."<BR>"."見込点";
    }
    $result->free();
    ...
    $arg["FOOT_COLSPAN"] = 3 + $count; //画面全体幅
    return $testcdArray;
}
...
function makeMeisai(&$objForm, &$arg, $db, &$model) {
    ...
    //成績データ取得
    $scoreArray = array();
    $result = $db->query(knjd128v_mikomiQuery::getScore($model, $execute_date));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    //見込点データ取得
    $recinfoArray = array();
    $result = $db->query(knjd128v_mikomiQuery::getRecordInfo($model, $execute_date));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    //一覧表示
    $colorFlg = false;
    $result = $db->query(knjd128v_mikomiQuery::selectQuery($model, $execute_date));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        //学籍番号をHiddenで保持
        ...
        //成績データ
        $scoreRow = array();
        $scoreRow = $scoreArray[$row["SCHREGNO"]][$model->field["TESTCD"]];
        $disFlg = ($scoreRow["VALUE_DI"] == "*" || $scoreRow["VALUE_DI"] == "**") ? "" : " disabled "; //見込点は、欠席者のみ入力可
        knjCreateHidden($objForm, "MIKOMI_VALUE_DI"."-".$counter, $scoreRow["VALUE_DI"]); //更新時に使用
        //見込点データ
        $recinfoRow = array();
        $recinfoRow = $recinfoArray[$row["SCHREGNO"]][$model->field["TESTCD"]];
        //見込点
        $name = "MIKOMI";
        $extra = "STYLE=\"text-align: right;\" onChange=\"this.style.background='#ccffcc'\" onblur=\"calc(this);\" onKeyDown=\"keyChangeEntToTab(this);\" onPaste=\"return showPaste(this);\"";
        $row[$name] = knjCreateTextBox($objForm, $recinfoRow["MIKOMI"], $name."-".$counter, 3, 3, $extra.$disFlg);
        //テキストの名前をセット
        if ($counter == 0) {
        ...
        //縦計(見込点)
        if (is_numeric($recinfoRow["MIKOMI"]) && ($scoreRow["VALUE_DI"] == "*" || $scoreRow["VALUE_DI"] == "**")) {
           $term_recinfo[] = $recinfoRow["MIKOMI"];
        }
    ...
    //縦計(見込点)
    //合計
    $arg["RECINFO_SUM"] = isset($term_recinfo) ? array_sum($term_recinfo) : "";
    //平均
    $arg["RECINFO_AVG"] = isset($term_recinfo) ? round((array_sum($term_recinfo)/get_count($term_recinfo))*10)/10 : "";
    if (isset($term_recinfo)) {
        //最高点と最低点を求める
        array_multisort ($term_recinfo, SORT_NUMERIC);
        $max = get_count($term_recinfo)-1;
    }
    //最高点
    $arg["RECINFO_MAX"] = isset($term_recinfo) ? $term_recinfo[$max] : "";
    //最低点
    $arg["RECINFO_MIN"] = isset($term_recinfo) ? $term_recinfo[0] : "";
}
