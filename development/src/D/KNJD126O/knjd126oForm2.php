...
class knjd126oForm2 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("form2", "POST", "knjd126oindex.php", "", "form2");
...
//        if ($view_cnt > 0) $view_key[9] = substr($model->field["SUBCLASSCD"], 0, 2)."99";
        ...
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            if (empty($model->field["SCHREGNO"]) && $model->field["SUBCLASSCD"] != $row["TARGET_SUBCLASSCD"]) {
                ...
                $row["LINK"] = View::alink("knjd126oindex.php", "下の学年", "target=_self tabindex=\"-1\"", $hash);
            }
        ...
        View::toHTML($model, "knjd126oForm2.html", $arg);
    }
}
//コンボ作成
