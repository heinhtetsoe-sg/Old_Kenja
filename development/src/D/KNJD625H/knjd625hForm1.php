...
class knjd625hForm1
{
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd625hForm1", "POST", "knjd625hindex.php", "", "knjd625hForm1");
        ...
        //学期コンボボックスを作成する
        $query = knjd625hQuery::getSemester();
        ...
        //学年コンボ
        $query = knjd625hQuery::getSelectGrade($model);
        ...
        View::toHTML($model, "knjd625hForm1.html", $arg); 
    }
}
/**************************************** 以下関数 **************************************************/
//コンボ作成
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    //クラス一覧
    $row1 = array();
    $result = $db->query(knjd625hQuery::getAuth($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model, $seme, $semeflg)
{
    ...
    $objForm->ae(createHiddenAe("PRGID", "KNJD625H"));
