...
class knjd626cForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd626cForm1", "POST", "knjd626cindex.php", "", "knjd626cForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //学期コンボ作成
        $query = knjd626cQuery::getSemester();
        $extra = "onchange=\"return btn_submit('change_grade');\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //学年コンボ作成
        $query = knjd626cQuery::getGrade($model->field["SEMESTER"], $model);
        $extra = "onchange=\"return btn_submit('change_grade');\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        //テストコンボ作成
        $query = knjd626cQuery::getTest($model->field["SEMESTER"], $model->field["GRADE"]);
        $opt = array();
        ...
        View::toHTML($model, "knjd626cForm1.html", $arg); 
    }
}
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD626C");
