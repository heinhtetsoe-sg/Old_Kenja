...
class knjd185jForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd185jForm1", "POST", "knjd185jindex.php", "", "knjd185jForm1");
        ...
        //学期コンボ
        $extra = "onChange=\"return btn_submit('knjd185j');\"";
        $query = knjd185jQuery::getSemester();
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //表示指定ラジオボタン 1:クラス 2:個人
        $opt_disp = array(1, 2);
        $model->field["DISP"] = ($model->field["DISP"] == "") ? "1" : $model->field["DISP"];
        $extra = array("id=\"DISP1\" onClick=\"return btn_submit('knjd185j')\"", "id=\"DISP2\" onClick=\"return btn_submit('knjd185j')\"");
        ...
        if ($model->field["DISP"] == 1) {
            //学年コンボ
            $extra = "onChange=\"return btn_submit('knjd185j');\"";
            $query = knjd185jQuery::getGrade($model);
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
            $schKindStr = $db->getOne(knjd185jQuery::getSchoolKind($model->field["GRADE"]));
        } else if ($model->field["DISP"] == 2) {
            //年組コンボ
            $extra = "onChange=\"return btn_submit('knjd185j');\"";
            $query = knjd185jQuery::getHrClass($model);
            makeCmb($objForm, $arg, $db, $query, "GRADE_HR_CLASS", $model->field["GRADE_HR_CLASS"], $extra, 1);
            $schKindStr = $db->getOne(knjd185jQuery::getSchoolKind(substr($model->field["GRADE_HR_CLASS"], 0, 2)));
        }

        //テスト
        $query = knjd185jQuery::getTest($model);
        $extra = "";
        ...
        //試験科目のみ出力
        $extra = ($model->field["TESTSUBCLASSONLY"] == "1" || $model->cmd == '') ? "checked" : "";
        $extra .= " id=\"TESTSUBCLASSONLY\" onclick =\"kubun();\"";
        $arg["data"]["TESTSUBCLASSONLY"] = knjCreateCheckBox($objForm, "TESTSUBCLASSONLY", "1", $extra, "");

        //担任所見欄出力
        $extra = ($model->field["COMMUNICATION"] == "1" || $model->cmd == '') ? "checked" : "";
        $extra .= " id=\"COMMUNICATION\"";
        $arg["data"]["COMMUNICATION"] = knjCreateCheckBox($objForm, "COMMUNICATION", "1", $extra, "");
        ...
        View::toHTML($model, "knjd185jForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["DISP"] == 2) {
        //生徒取得
        $query = knjd185jQuery::getSchList($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //クラス名称調整
...
function makeHidden(&$objForm, $model, $schKindStr) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD185J");
    ...
    knjCreateHidden($objForm, "tutisyoSyokenFieldSize", ($model->Properties["HREPORTREMARK_DAT_COMMUNICATION_SIZE_".$schKindStr] != "" ? $model->Properties["HREPORTREMARK_DAT_COMMUNICATION_SIZE_".$schKindStr] : "50 * 3"));
}
?>
