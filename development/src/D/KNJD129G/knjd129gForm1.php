...
class knjd129gForm1
{
    function main(&$model)
    {
        ...
        $arg["start"]    = $objForm->get_start("main", "POST", "knjd129gindex.php", "", "main");
        ...
        //学期コンボ
        $query = knjd129gQuery::getSemester();
        ...
        View::toHTML($model, "knjd129gForm1.html", $arg);
    }
}
...
function makeHead(&$objForm, &$arg, $db, $model) {
    //科目別設定があるか
    $testSubCnt = $db->getOne(knjd129gQuery::getTestSubCnt($model));
    ...
    $result = $db->query(knjd129gQuery::getTestName($model, $testSubCnt));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $count++;
        if ($sem != $row["SEMESTER"]) {
            $sem = $row["SEMESTER"];
...
function makeMeisai(&$objForm, &$arg, $db, &$model) {
    ...
    //一覧表示
    $colorFlg = false;
    $result = $db->query(knjd129gQuery::selectQuery($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        //学籍＋科目
        ...
        foreach ($model->testcdArray as $key => $codeArray) {
            ...
            //成績データ
            $scoreRow = array();
            $query = knjd129gQuery::getScore(CTRL_YEAR, $testcd, $model, $rowKey);
            ...
            if ($testcd == $model->field["SEMESTER"].str_replace("-", "", $model->field["TESTIK"])) {
                ...
                $extra = " onChange=\"this.style.background='#ccffcc'\" onblur=\"this.value=toInteger(this.value);\" ";
                $row[$col] = knjCreateTextBox($objForm, $score, "SCORE_".$rowKey, 3, 3, $extra);
            //ラベルのみ
            } else {
                $row["FONTCOLOR"] = $fontcolorFlgRed ? "#ff0000" : "#000000";
...
function getScoreSansyutu($db, $testcd, $model, $schregNo, $rowKey, $subclassCd, $groupCd) {
    $score = "";
    //個人素点
    $query = knjd129gQuery::getRecordRankSdiv($model, $rowKey);
    $result = $db->query($query);
    $testArray = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    //平均
    $query = knjd129gQuery::getRecordAvgSdiv($model, $subclassCd, $groupCd);
    $result = $db->query($query);
    $testAvgArray = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $testAvgArray[$row["SEMESTER"].$row["TESTKINDCD"].$row["TESTITEMCD"]] = $row["AVG"];
    }
    $result->free();

    if ($model->field["SEMESTER"] == "1") {
        if (str_replace("-", "", $model->field["TESTIK"]) == $model->TYUKAN1) {
            $score = round($testAvgArray["10101"] * 0.8, 1);
        } else {
            $score = round($testAvgArray["10201"] * 0.8, 1);
        }
    } else {
        if (str_replace("-", "", $model->field["TESTIK"]) == $model->TYUKAN1) {
            if (strlen($testArray["20102"]) > 0 && strlen($testArray["20201"]) > 0) {
                $score = round($testAvgArray["20101"] * (($testArray["20102"] + $testArray["20201"]) / 2) / (($testAvgArray["20102"] + $testAvgArray["20201"]) / 2) * 0.8, 1);
            } else if (strlen($testArray["20102"]) == 0 && strlen($testArray["20201"]) > 0) {
                $score = round($testArray["20201"] * $testAvgArray["20101"] / $testAvgArray["20201"] * 0.8, 1);
            } else if (strlen($testArray["20102"]) > 0 && strlen($testArray["20201"]) == 0) {
                $score = round($testArray["20102"] * $testAvgArray["20101"] / $testAvgArray["20102"] * 0.8, 1);
            }
        } else if (str_replace("-", "", $model->field["TESTIK"]) == $model->TYUKAN2) {
            if (strlen($testArray["20101"]) > 0 && strlen($testArray["20201"]) > 0) {
                $score = round($testAvgArray["20102"] * (($testArray["20101"] + $testArray["20201"]) / 2) / (($testAvgArray["20101"] + $testAvgArray["20201"]) / 2) * 0.8, 1);
            } else if (strlen($testArray["20101"]) == 0 && strlen($testArray["20201"]) > 0) {
                $score = round($testArray["20201"] * $testAvgArray["20102"] / $testAvgArray["20201"] * 0.8, 1);
            } else if (strlen($testArray["20101"]) > 0 && strlen($testArray["20201"]) == 0) {
                $score = round($testArray["20101"] * $testAvgArray["20102"] / $testAvgArray["20101"] * 0.8, 1);
            }
        } else if (str_replace("-", "", $model->field["TESTIK"]) == $model->KIMATU) {
            if (strlen($testArray["20101"]) > 0 && strlen($testArray["20102"]) > 0) {
                $score = round($testAvgArray["20201"] * (($testArray["20101"] + $testArray["20102"]) / 2) / (($testAvgArray["20101"] + $testAvgArray["20102"]) / 2) * 0.8, 1);
            } else if (strlen($testArray["20101"]) == 0 && strlen($testArray["20102"]) > 0) {
                $score = round($testArray["20102"] * $testAvgArray["20201"] / $testAvgArray["20102"] * 0.8, 1);
            } else if (strlen($testArray["20101"]) > 0 && strlen($testArray["20102"]) == 0) {
                $score = round($testArray["20101"] * $testAvgArray["20201"] / $testAvgArray["20101"] * 0.8, 1);
            }
        }
    }
    return $score;
