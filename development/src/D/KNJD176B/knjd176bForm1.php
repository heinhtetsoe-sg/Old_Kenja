...
class knjd176bForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd176bForm1", "POST", "knjd176bindex.php", "", "knjd176bForm1");
        ...
        //クラス選択コンボボックスを作成する/
        $db = Query::dbCheckOut();
        $query = knjd176bQuery::getAuth($model->control["年度"],$ga);
        ...
        $objForm->ae( array("type"       => "select",
                            ...
                            "extrahtml"  => "onchange=\"return btn_submit('knjd176b'),AllClearList();\"",
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd176b')\"".$disabled;
        ...
        //対象外の生徒取得
        $db = Query::dbCheckOut();
        $query = knjd176bQuery::getSchnoIdou($model,$ga);
        $result = $db->query($query);
        $opt_idou = array();
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        View::toHTML($model, "knjd176bForm1.html", $arg); 
    }
}
?>
