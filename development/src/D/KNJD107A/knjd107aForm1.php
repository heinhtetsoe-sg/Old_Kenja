...
class knjd107aForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd107aForm1", "POST", "knjd107aindex.php", "", "knjd107aForm1");
        ...
        //学期コンボ作成
        $query = knjd107aQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd107a'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //クラスコンボ作成
        $query = knjd107aQuery::getHrClass(CTRL_YEAR, $model->field["SEMESTER"]);
        $extra = "onchange=\"return btn_submit('knjd107a'), AllClearList();\"";
        ...
        View::toHTML($model, "knjd107aForm1.html", $arg);
    }
}

function makeStudentList(&$objForm, &$arg, $db, $model, $seme) {
    //対象外の生徒取得
    $query = knjd107aQuery::getSchnoIdou($model, $seme);
    ...
    //対象者リストを作成する
    $query = knjd107aQuery::getStudent($model, $seme);
    $result = $db->query($query);
    $opt1 = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $idou = "　";
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD107A");
