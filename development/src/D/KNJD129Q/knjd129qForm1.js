function sort_submit(sort_div) {
    document.forms[0].SORT_DIV.value = sort_div;
    btn_submit('main');
}
...
function clickedBtnUdpate(disFlg) {
    if (disFlg) {
        ...
        if (document.forms[0].showBunpuHyou.value == '1') {
            if (document.forms[0].GROUP_DIV.value == '1') {
                document.forms[0].H_CHAIRCD.value = document.forms[0].CHAIRCD.value;
            }
            if (document.forms[0].GROUP_DIV.value == '2') {
                document.forms[0].H_GRADE.value = document.forms[0].GRADE.value;
            }
            if (document.forms[0].GROUP_DIV.value == '3') {
                document.forms[0].H_GRADE_COURSE.value = document.forms[0].GRADE_COURSE.value;
            }
            if (document.forms[0].GROUP_DIV.value == '4') {
                document.forms[0].H_CHAIR_GROUP_CD.value = document.forms[0].CHAIR_GROUP_CD.value;
            }
        } else {
            document.forms[0].H_CHAIRCD.value = document.forms[0].CHAIRCD.value;
        }
        ...
        if (document.forms[0].showBunpuHyou.value == '1') {
            ...
            if (document.forms[0].GROUP_DIV.value == '3') {
                document.forms[0].GRADE_COURSE.value = document.forms[0].H_GRADE_COURSE.value;
            }
            if (document.forms[0].GROUP_DIV.value == '4') {
                document.forms[0].CHAIR_GROUP_CD.value = document.forms[0].H_CHAIR_GROUP_CD.value;
            }
        } else {
            document.forms[0].CHAIRCD.value = document.forms[0].H_CHAIRCD.value;
        }
    ...
    if (document.forms[0].showBunpuHyou.value == '1') {
        ...
        if (document.forms[0].GROUP_DIV.value == '3') {
            document.forms[0].GRADE_COURSE.disabled = disFlg;
        }
        if (document.forms[0].GROUP_DIV.value == '4') {
            document.forms[0].CHAIR_GROUP_CD.disabled = disFlg;
        }
    } else {
        document.forms[0].CHAIRCD.disabled = disFlg;
    }
...
//分布表
function updateBunpu(obj, testcd) {
//alert('level='+level);
    if (testcd != '9990009') {
        return;
    }
    ...
    var name = obj.name;
    var targetName = name.split("-")[0];
    var lineCnt = document.forms[0].COUNT.value;
    var levelCnt = new Array();
    for (var i = 0; i < lineCnt; i++) {
        scoreObject = eval("document.forms[0][\"" + targetName + "-" + i + "\"]");
        if (!isNaN(scoreObject.value) && scoreObject.value != '' || scoreObject.value == "*") {
            scoreInt = parseInt(scoreObject.value);
            if (levelCnt[scoreInt]) {
                levelCnt[scoreInt] += 1;
            } else {
                levelCnt[scoreInt] = 1;
            }
    ...
    var levelMax = document.forms[0].LEVEL_MAX.value;
    for (var level = 1; level <= levelMax; level++) {
        targetname = 'CNT_ID' + '-' + level;
        targetObject = document.getElementById(targetname);
        if (targetObject) {
            //人数
            cntInt = levelCnt[level];
            if (cntInt) {
                targetObject.innerHTML = cntInt;
            } else {
                targetObject.innerHTML = "";
            }
            //割合
            targetname = 'PERCENT_ID' + '-' + level;
            targetObject = document.getElementById(targetname);
            if (cntInt) {
                bunbo = lineCnt;
                avg = parseFloat(cntInt / bunbo) * 100 * 10;
