...
class knjd131Form1 {
    function main(&$model) {
        $objForm = new form;
        $arg["start"] = $objForm->get_start("edit", "POST", "knjd131index.php", "", "edit");
        ...
        if ((isset($model->schregno) && !isset($model->warning)) || !isset($model->schregno)) {
            $row = $db->getRow(knjd131Query::getTrainRow($model, $model->schregno),DB_FETCHMODE_ASSOC);
            $arg["NOT_WARNING"] = 1;
        } else {
            $row =& $model->field;
        }
        ...
        View::toHTML($model, "knjd131Form1.html", $arg);
    }
}
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD131");
...
function makeBtn(&$objForm, &$arg, $model, $db) {
    ...
    //csv
    $extra = " onClick=\" wopen('".REQUESTROOT."/X/KNJX154/knjx154index.php?FIELDSIZE=".$fieldSize."','SUBWIN2',0,0,screen.availWidth,screen.availHeight);\"";
    $arg["button"]["btn_csv"] = createBtn($objForm, "btn_csv", "ＣＳＶ出力", $extra);
    //終了
    $extra = "onclick=\"closeWin();\"";
    $arg["button"]["btn_end"] = createBtn($objForm, "btn_end", "終 了", $extra);
    //一括更新ボタン
    $link  = REQUESTROOT."/D/KNJD131/knjd131index.php?cmd=replace&SCHREGNO=".$model->schregno."&SEMESTER=".$model->field["SEMESTER"];
