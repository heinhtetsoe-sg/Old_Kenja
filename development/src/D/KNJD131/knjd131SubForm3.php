...
class knjd131SubForm3 {
    function main(&$model) {
        $objForm = new form;
        $db = Query::dbCheckOut();
        $arg = array();
        $arg["start"]   = $objForm->get_start("subform1", "POST", "knjd131index.php", "", "subform1");
        
        //年度、学期、クラス表示
        $setYear = CTRL_YEAR.'年度';
        $setSemester = $db->getOne(knjd131Query::getSemesterName());
        $setHrClass = $db->getOne(knjd131Query::getHrClassName($model));
        $arg["YEAR"] = $setYear;
        ...
        $opt = array();
        $value_flg = false;
        $query = knjd131Query::getSemester();
        ...
        if ($model->cmd !== 'replace') {
            ...
            $query = knjd131Query::getCommunication($model->field, $model->schregno);
        ...
        //戻るボタン
        $link = REQUESTROOT."/D/KNJD131/knjd131index.php?cmd=back";
        $objForm->ae( array("type"        => "button",
                            "name"        => "btn_back",
                            "value"       => "戻 る",
        ...
        View::toHTML($model, "knjd131SubForm3.html", $arg);
    }
}
/********************************************** 以下関数 ***********************************************/
//クラス一覧リストToリスト作成
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    $query = knjd131Query::getStudent($model);
    $result = $db->query($query);
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
