...
class knjd417form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjd417index.php", "", "main");
        ...
        //構成種類コンボ
        $query = knjd417Query::getCompositionTypeList();
        ...
        $savedat = array();
        $savecnt = 0;
        $query = knjd417Query::getHDKindNameDat($model, $nameCd2);
        ...
        if (($savedat["KIND_NAME"] === "" || $savedat["KIND_NAME"] === null) 
            && ($model->compositionname === "" || $model->compositionname === null)) {
                if ($nameCd2 !== "") {
                    $query = knjd417Query::getCompositionTypeSmplList("D090", true, $nameCd2);
        ...
        if (get_count($typechk) > 0 && ($nameSpare1 == "1" || $nameSpare1 == "2")) {
            ...
            for ($i = 1; $i <= $model->compcnt; $i++) {
                ...
                if ($model->cmd == "" || $model->cmd == "change" || $model->cmd == "change2" || $model->cmd == "reset" || $model->cmd == "level") {
                    ...
                    //取込ボタン処理
                    $query = knjd417Query::getCompositionTypeSmplList($nameSpare3, true, $model->field["ASSESSMARK_".$i]);
                ...
                $query = knjd417Query::getCompositionTypeSmplList($nameSpare3, true);
        ...
        if ($nameSpare1 == '3') {
            ...
            if ($model->Properties["unuseEduPlan_Group_GuidancePattern"] == "1" && $model->cmd == "change") {
                ...
                $query2 = knjd417Query::getDefaultGuidancePattern($model);
                $model->guidance_pattern = $db->getOne($query2);
            }
            $query = knjd417Query::getGuidancePattern($model);
            ...
            //指導計画帳票パターン別設定
            $model->pattern = array();
            $query = knjd417Query::getPattern($model->guidance_pattern);
            $result = $db->query($query);
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            if ($model->guidance_pattern) {
                //データ取得
                $query = knjd417Query::getHreportGuidanceItemNameDat($model);
        ...
        View::toHTML($model, "knjd417Form1.html", $arg);
    }
}
