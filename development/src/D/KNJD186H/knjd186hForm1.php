...
class knjd186hForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd186hForm1", "POST", "knjd186hindex.php", "", "knjd186hForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //学期コンボ
        $query = knjd186hQuery::getSemester();
        ...
        if ($model->field["SEMESTER"] && $value_flg) {
            ...
            $maxSemester = $db->getOne(knjd186hQuery::getMaxSemester());
        ...
        knjCreateHidden($objForm, "PRGID",         "KNJD186H");
        ...
        View::toHTML($model, "knjd186hForm1.html", $arg);
    }
}
/****************************************************** 以下関数 ************************************************************/
//コンボ作成
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    if (!$chairInState) {
        //全講座
        $query = knjd186hQuery::getAllChair($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    $opt = array();
    if ($chairInState) {
        $query = knjd186hQuery::getStudent($model, $chairInState);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
