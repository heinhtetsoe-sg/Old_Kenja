...
function btn_submit(cmd, columnname, patternDataDiv, itemname)
{
    if (cmd == 'reset') {
        ...
        if (document.forms[0].KNJD183C_semesCombo) {
            sendSemester = document.forms[0].SEMESTER.value;
        } else {
            sendSemester = "";
        }

        link = 'knjd183cindex.php?cmd='+cmd+'&CHR='+chr+'&SEMESTER='+sendSemester+'&COLUMNNAME='+columnname+"&PATTERN_DATA_DIV="+patternDataDiv+"&ITEMNAME="+itemname+"&TARGETID="+currentFocus.id;
...
if (window.addEventListener) {
    window.addEventListener('load', function (e) {
        ...
        if (document.getElementById("item_header") != null) {
...
function chackMozi(obj, moji, gyou)
{
    var nyuryokuMoji = obj.value;
    if(nyuryokuMoji.length > (moji * gyou)){
        alert('文字数が適切ではありません');
