...
class knjd137hForm3 {
    ...
    function main(&$model) {
        ...
        $row3 = $db->getRow(knjd137hQuery::getCourseMajorCd($model), DB_FETCHMODE_ASSOC);
        if($row3['COURSECD'] == '4' && $row3['MAJORCD'] == '903'){
        	$pattern_type = 1;
        } else if($row3['COURSECD'] == '4' && $row3['MAJORCD'] == '902'){
        	$pattern_type = 2;
        } else {
        	$pattern_type = 3;
        }
        ...
        //自立活動
        $extra = ($pattern_type == 1)?' disabled="disabled"':'';
        ...
        $extra = ($pattern_type == 1 || $pattern_type == 2)?' disabled="disabled"':'';
        $arg["data"]["TOTALSTUDYTIME"] = getTextOrArea($objForm, "TOTALSTUDYTIME", $model->getPro["TOTALSTUDYTIME"]["moji"], $model->getPro["TOTALSTUDYTIME"]["gyou"], $row["TOTALSTUDYTIME"], $model, $extra);
        ...
        View::toHTML($model, "knjd137hForm3.html", $arg);
    }
}
//makeCmb
