...
window.onload = function () {
        if (sessionStorage.getItem("KNJD429MForm1_CurrentCursor") != null) {
            document.title = "";
            document.getElementById(sessionStorage.getItem("KNJD429MForm1_CurrentCursor")).focus();
        }
};

function current_cursor(para) {
    sessionStorage.setItem("KNJD429MForm1_CurrentCursor", para);
}
...
/* Add by PP for CurrentCursor 2020-01-10 start */
    if (sessionStorage.getItem("KNJD429MForm1_CurrentCursor") != null) {
                document.title = "";
        document.getElementById(sessionStorage.getItem("KNJD429MForm1_CurrentCursor")).blur();
    }
    /* Add by PP for CurrentCursor 2020-01-17 end */
...
function toggleOuin() {
    if (document.forms[0].URABYOSI.checked == true) {
        document.forms[0].OUIN.disabled = false;
        document.forms[0].OUIN.checked = true;
    } else {
        document.forms[0].OUIN.checked = false;
        document.forms[0].OUIN.disabled = true;
    }
}
...
function newwin(SERVLET_URL) {
    ...
    if (!document.forms[0].FRM_PATERN) {
        alert('{rval MSG310}' + "\n帳票パターンが表示されていません。\n帳票パターンの登録・選択をお願いします。");
        return false;
    }
