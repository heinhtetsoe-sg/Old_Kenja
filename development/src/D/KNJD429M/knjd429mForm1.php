...
class knjd429mForm1 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd429mForm1", "POST", "knjd429mindex.php", "", "knjd429mForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //学期コンボ
        $query = knjd429mQuery::getSemester();
        ...
        if ($model->field["HUKUSIKI_RADIO"] == "1") {
            $query = knjd429mQuery::getSchoolKind($model);
            // Add by PP for current cursor 2020-01-10 start
        ...
        if ($model->field["HUKUSIKI_RADIO"] == "2") {
            $query = knjd429mQuery::getGhr($model);
            $ghr_cd_label = "実クラス";
        } else {
            $query = knjd429mQuery::getGradeHrClass($model);
            $ghr_cd_label = "年組";
        }
        // Add by PP for current cursor 2020-01-10 start
        ...
        if ($model->field["HUKUSIKI_RADIO"] == "2" && $model->field["GHR_CD"] != '') {
            $query = knjd429mQuery::getGhr($model, $model->field["GHR_CD"]);
        ...
        //学校名取得
        $query = knjd429mQuery::getSchoolName();
        ...
        // 裏表紙印刷
        $extra = "id=\"URABYOSI\" onchange=\"toggleOuin();\" checked";
        $arg["data"]["URABYOSI"] = knjCreateCheckBox($objForm, "URABYOSI", "1", $extra);

        // 押印欄
        $extra = "id=\"OUIN\" checked";
        $arg["data"]["OUIN"] = knjCreateCheckBox($objForm, "OUIN", "1", $extra);
        ...
        $query = knjd429mQuery::getPaternDefault($model);
        ...
        knjCreateHidden($objForm, "PRGID",         "KNJD429M");
        ...
        View::toHTML($model, "knjd429mForm1.html", $arg);
    }
}
/****************************************************** 以下関数 ************************************************************/
//コンボ作成
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    //対象外の生徒取得
    $query = knjd429mQuery::getSchnoIdou($model, $seme);
    $result = $db->query($query);
    $opt_idou = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    $query = knjd429mQuery::getStudent($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function paternRadio(&$objForm, $name, $value, $extra, $multi, $count, $valArray)
{
    ...
    for ($i = 1; $i <= $count; $i++) {
        ...
        $radioArray = knjCreateRadio($objForm, $name, $value, $ext, $opt, get_count($opt));
