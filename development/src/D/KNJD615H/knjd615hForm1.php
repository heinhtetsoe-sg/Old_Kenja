...
class knjd615hForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd615hForm1", "POST", "knjd615hindex.php", "", "knjd615hForm1");
        ...
        //学期コンボ作成
        $query = knjd615hQuery::getSemester($model, 0);
        $extra = "onchange=\"return btn_submit('knjd615h'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);
        //学年末は今学期とする
        $seme = $model->field["SEMESTER"] == "9" ? CTRL_SEMESTER : $model->field["SEMESTER"];

        //学年コンボ作成
        $query = knjd615hQuery::getSchoolName();
        $schoolName = $db->getOne($query);
        $query = knjd615hQuery::getGrade($seme, $model);
        $extra = "onchange=\"return btn_submit('knjd615h'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
        
        //帳票パターン
        $selectForm = array();
        if ($model->Properties["knjd615hSelectForm"] == "") {
            $selectForm = array(1, 2, 3, 4, 5);
        } else {
            $selectForm = explode(",", $model->Properties["knjd615hSelectForm"]);
        }
        ...
        //テストコンボ作成
        $query = knjd615hQuery::getTest($model, $model->field["GRADE"], 1);
        $opt = array();
        $value_flg = false;
        ...
        $result->free();
        if ($add == false) {
            $query = knjd615hQuery::getTest($model, $model->field["GRADE"], 2);
            $opt = array();
            $value_flg = false;
            $result = $db->query($query);
            $add = false;
        ...
        //データ選択
        $extra = ($model->field["DATA_SELECT1"] == "1") ? "checked" : "";
        $extra .= " id=\"DATA_SELECT1\"";
        $arg["data"]["DATA_SELECT1"] = knjCreateCheckBox($objForm, "DATA_SELECT1", "1", $extra, "");

        $extra = ($model->field["DATA_SELECT2"] == "1") ? "checked" : "";
        $extra .= " id=\"DATA_SELECT2\"";
        $arg["data"]["DATA_SELECT2"] = knjCreateCheckBox($objForm, "DATA_SELECT2", "1", $extra, "");

        $extra = ($model->field["DATA_SELECT3"] == "1") ? "checked" : "";
        $extra .= " id=\"DATA_SELECT3\"";
        $arg["data"]["DATA_SELECT3"] = knjCreateCheckBox($objForm, "DATA_SELECT3", "1", $extra, "");

        $extra = ($model->field["DATA_SELECT4"] == "1") ? "checked" : "";
        $extra .= " id=\"DATA_SELECT4\"";
        $arg["data"]["DATA_SELECT4"] = knjCreateCheckBox($objForm, "DATA_SELECT4", "1", $extra, "");

        $extra = ($model->field["DATA_SELECT5"] == "1") ? "checked" : "";
        $extra .= " id=\"DATA_SELECT5\"";
        $arg["data"]["DATA_SELECT5"] = knjCreateCheckBox($objForm, "DATA_SELECT5", "1", $extra, "");

        //出欠集計開始日付作成
        if ($model->field["SDATE"] == "") {
            $model->field["SDATE"] = str_replace("-", "/", $db->getOne(knjd615hQuery::getSdate($model->field["GRADE"])));
        }
        ...
        $model->field["OUTPUT_RANK"] = ($model->field["OUTPUT_RANK"] == "") ? ($model->Properties["knjd615hDefaultOutputRank"] ? $model->Properties["knjd615hDefaultOutputRank"] : "2") : $model->field["OUTPUT_RANK"];
        ...
        $model->field["OUTPUT_KIJUN"] = $model->field["OUTPUT_KIJUN"] ? $model->field["OUTPUT_KIJUN"] : ($model->Properties["knjd615hDefaultOutputKijun"] ? $model->Properties["knjd615hDefaultOutputKijun"] : '1');
        $opt_kijun = array(1, 2);
        ...
        $model->field["BIKO_KIND"] = $model->field["BIKO_KIND"] ? $model->field["BIKO_KIND"] : '2';
        $opt = array(1, 2, 3);
        ...
        View::toHTML($model, "knjd615hForm1.html", $arg); 
    }
}
...
function makeClassList(&$objForm, &$arg, $db, $model, $seme) {
    ...
    //クラス一覧リストを作成する
    $query = knjd615hQuery::getHrClass($seme, $model->field["GRADE"]);
    $result = $db->query($query);
    $opt1 = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model, $seme) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD615H");
    ...
    knjCreateHidden($objForm, "knjd615hPrintNullRemark", $model->Properties["knjd615hPrintNullRemark"]);
    knjCreateHidden($objForm, "knjd615hPrintPerfect", $model->Properties["knjd615hPrintPerfect"]);
    
    knjCreateHidden($objForm, "OUTPUT_PATERN", '3');
