...
class knjd131tSubForm5 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjd131tindex.php", "", "edit");
        ...
        $counter = 0;
        if ($model->schregno) {
            $result = $db->query(knjd131tQuery::getAward($model));
        ...
        //取込ボタン
        $extra = "disabled style=\"color:#1E90FF;font:bold\" onclick=\"return dataPositionSet('REMARK3');\"";
        ...
        if (VARS::get("cmd") == "right_list"){ 
            $arg["reload"] = "parent.edit_frame.location.href='knjd131tindex.php?cmd=edit'";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjd131tSubForm5.html", $arg);
    }
}
