...
class knjd184oForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd184oForm1", "POST", "knjd184oindex.php", "", "knjd184oForm1");
        ...
        //クラス選択コンボボックスを作成する
        $query = knjd184oQuery::getAuth($model);
        $extra = "onchange=\"return btn_submit('knjd184o'),AllClearList();\"";
        ...
        //hiddenを作成する(必須)
        makeHidden($objForm, $model, $sdate, $edate);
        ...
        View::toHTML($model, "knjd184oForm1.html", $arg); 
    }
}
...
{
    $query = knjd184oQuery::getSchnoIdou($model);
    $result = $db->query($query);
    $opt_idou = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    //対象者リストを作成する
    $result = $db->query(knjd184oQuery::getListStudent($model));
    $opt1 = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
//Hidden作成
function makeHidden(&$objForm, $model, $sdate, $edate)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD184O");
    ...
    knjCreateHidden($objForm, "SEME_FROM", str_replace('-', '/', $sdate));
    knjCreateHidden($objForm, "SEME_TO", str_replace("-", "/", $edate));
