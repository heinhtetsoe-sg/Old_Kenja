...
class knjd187lForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd187lForm1", "POST", "knjd187lindex.php", "", "knjd187lForm1");
        ...
        //学期コンボ作成
        $query = knjd187lQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd187l');\"";
        ...
        //対象学期の開始日・終了日
        $sDate = $db->getOne(knjd187lQuery::getSemesterDate("SDATE",$model->field["SEMESTER"]));
        $eDate = $db->getOne(knjd187lQuery::getSemesterDate("EDATE",$model->field["SEMESTER"]));
        knjCreateHidden($objForm, "SDATE", str_replace("-","/",$sDate));
        knjCreateHidden($objForm, "EDATE", str_replace("-","/",$eDate));

        //クラスコンボ作成
        $query = knjd187lQuery::getHrClass(CTRL_YEAR, $seme, $model);
        $extra = "onchange=\"return btn_submit('knjd187l');\"";
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd187l', 'on')\"".$disabled;
        $date_textbox = knjCreateTextBox($objForm, $model->field["DATE"], "DATE", 12, 12, $extra);
        //異動対象日付（カレンダー）
        global $sess;
        $extra = "onclick=\"tmp_list('knjd187l', 'off'); loadwindow('" .REQUESTROOT ."/common/calendar.php?name=DATE&frame='+getFrameName(self) + '&date=' + document.forms[0]['DATE'].value + '&CAL_SESSID=$sess->id&reload=true', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 200)\"";
        ...
        View::toHTML($model, "knjd187lForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model, $seme) {
    ...
    //対象外の生徒取得
    $query = knjd187lQuery::getSchnoIdou($model, $seme);
    ...
    //対象者リストを作成する
    $query = knjd187lQuery::getStudent($model, $seme);
    $result = $db->query($query);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD187L");
