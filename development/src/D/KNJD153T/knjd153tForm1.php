...
class knjd153tForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"]  = $objForm->get_start("knjd153tForm1", "POST", "knjd153tindex.php", "", "knjd153tForm1");
        ...
        //学期コンボ作成
        $query = knjd153tQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd153t'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GAKKI", $model->field["GAKKI"], $extra, 1);

        //クラスコンボ作成
        $query = knjd153tQuery::getAuth($model);
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd153t', 'on')\"".$disabled;
        $date_textbox = knjCreateTextBox($objForm, $model->field["DATE"], "DATE", 12, 12, $extra);
        //異動対象日付（カレンダー）
        global $sess;
        $extra = "onclick=\"tmp_list('knjd153t', 'off'); loadwindow('" .REQUESTROOT ."/common/calendar.php?name=DATE&frame='+getFrameName(self) + '&date=' + document.forms[0]['DATE'].value + '&CAL_SESSID=$sess->id&reload=true', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 200)\"";
        ...
        //テストコンボ作成
        $query = knjd153tQuery::getTestKindcd($model);
        makeCmb($objForm, $arg, $db, $query, "TESTKINDCD", $model->field["TESTKINDCD"], "", 1);

        //偏差値出力チェックボックス
        $extra  = ($model->field["DEVIATION"] == "on") ? "checked" : "";
        $extra .= " id=\"DEVIATION\"";
        $arg["data"]["DEVIATION"] = knjCreateCheckBox($objForm, "DEVIATION", "on", $extra, "");
        ...
        View::toHTML($model, "knjd153tForm1.html", $arg); 
    }
}
...
{
    //対象外の生徒取得
    $query = knjd153tQuery::getSchnoIdou($model);
    $result = $db->query($query);
    $opt_idou = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    //対象者リストを作成する
    $query = knjd153tQuery::getStudent($model);
    $result = $db->query($query);
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD153T");
