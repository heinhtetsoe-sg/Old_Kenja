...
class knjd187dForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd187dForm1", "POST", "knjd187dindex.php", "", "knjd187dForm1");
        ...
        //学期コンボ作成
        $query = knjd187dQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd187d');\"";
        ...
        //対象学期の開始日・終了日
        $sDate = $db->getOne(knjd187dQuery::getSemesterDate("SDATE",$model->field["SEMESTER"]));
        $eDate = $db->getOne(knjd187dQuery::getSemesterDate("EDATE",$model->field["SEMESTER"]));
        ...
        $extra = array("id=\"DISP1\" onClick=\"return btn_submit('knjd187d')\"", "id=\"DISP2\" onClick=\"return btn_submit('knjd187d')\"");
        $radioArray = knjCreateRadio($objForm, "DISP", $model->field["DISP"], $extra, $opt_disp, get_count($opt_disp));
        foreach($radioArray as $key => $val) $arg["data"][$key] = $val;

        if ($model->field["DISP"] == 1) {
            //学年コンボ
            $query = knjd187dQuery::getGrade($model);
            $extra = "onChange=\"return btn_submit('knjd187d');\"";
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
        } else if ($model->field["DISP"] == 2) {
            //クラスコンボ作成
            $query = knjd187dQuery::getHrClass(CTRL_YEAR, $seme, $model);
            $extra = "onchange=\"return btn_submit('knjd187d');\"";
        ...
        View::toHTML($model, "knjd187dForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model, $seme) {
    ...
    //対象外の生徒取得
    $query = knjd187dQuery::getSchnoIdou($model, $seme);
    ...
    //対象者リストを作成する
    if ($model->field["DISP"] == 2) {
        $query = knjd187dQuery::getStudent($model, $seme);
        $result = $db->query($query);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD187D");
