...
class knjd139eForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjd139eindex.php", "", "main");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        $query = knjd139eQuery::getNameMst($setNameCd);
        ...
        //年組コンボ作成
        $query = knjd139eQuery::getGradeHrclass($model);
        $extra = "onchange=\"return btn_submit('main');\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE_HR_CLASS", $model->field["GRADE_HR_CLASS"], $extra, 1, 1);

        //クラス形態
        $query = knjd139eQuery::getClassKeitai($model);
        $model->classKeitai = $db->getOne($query);

        //科目コンボ作成
        $query = knjd139eQuery::getSubclassMst($model->field["GRADE_HR_CLASS"], $model);
        $extra = "onchange=\"return btn_submit('main')\"";
        ...
        //一覧表示
        $result = $db->query(knjd139eQuery::selectQuery($model));
        ...
        knjCreateHidden($objForm, "PRGID", "KNJD139E");
        ...
        View::toHTML($model, "knjd139eForm1.html", $arg);
    }
}
