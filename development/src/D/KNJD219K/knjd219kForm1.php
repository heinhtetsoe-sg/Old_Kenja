...
class knjd219kform1 {
    function main(&$model) {
        $objForm = new form;
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjd219kindex.php", "", "main");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        View::toHTML($model, "knjd219kForm1.html", $arg);
    }
}
/********************************************** 以下関数 **********************************************/
//履歴表示
function makeListRireki(&$objForm, &$arg, $db, $model) {
    //履歴一覧
    $query = knjd219kQuery::getListRireki($model);
