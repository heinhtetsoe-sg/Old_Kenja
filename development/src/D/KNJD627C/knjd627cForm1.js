...
function btn_submit(cmd, handler) {
    ...
    document.forms[0].action = 'knjd627cindex.php';
    document.forms[0].HID_EVENT_FROM.value = handler.name;
    document.forms[0].HID_CLASS_SELECTED.value = '';
    for (var idx = 0; idx < document.forms[0].CLASS_SELECTED.options.length; idx++) {
        if (0 < idx) {
            document.forms[0].HID_CLASS_SELECTED.value += ',';
        }
        document.forms[0].HID_CLASS_SELECTED.value += document.forms[0].CLASS_SELECTED.options[idx].value;
    }
...
function validate() {
    if (document.forms[0].TERM.value.length < 1) {
        alert('{rval MSG301}' + '\n\n( 前期後期選択 )');
