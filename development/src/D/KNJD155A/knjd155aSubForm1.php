...
class knjd155aSubForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]  = $objForm->get_start("sel", "POST", "knjd155aindex.php", "", "sel");
        ...
        $hr_name = $db->getOne(knjd155aQuery::getHrName($model));
        $testname = $db->getOne(knjd155aQuery::getTestName($model));
        ...
        $arg["data"]["REMARK"] = KnjCreateTextArea($objForm, "REMARK", $model->hexam_record_remark_dat_remark1_gyou, ((int)$model->hexam_record_remark_dat_remark1_moji * 2 + 1), "soft", $extra, $model->replace_data["field"]["REMARK"]);
        ...
        View::toHTML($model, "knjd155aSubForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    ...
    //対象者リストを作成する
    $query = knjd155aQuery::getStudent($model);
    ...
    //生徒一覧リストを作成する//
    $query = knjd155aQuery::getStudent($model, "1");
    $result = $db->query($query);
    $opt2 = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
