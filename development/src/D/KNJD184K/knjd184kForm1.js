...
function newwin(SERVLET_URL) {
    ...
    if (document.forms[0].SUMMARY_DATE.value == '') {
        alert('異動対象日付及び出欠集計日付を指定して下さい。');
        return;
    }

    //最終学期が選択されている時に記載日付がブランクならエラー(それ以外の学期ではブランク許可)
    var semesOpt = document.forms[0].SEMESTER.options;
    var isLastSemes = semesOpt.selectedIndex == semesOpt.length - 1;
    if (document.forms[0].DESC_DATE.value == '' && isLastSemes) {
        alert('記載日付を指定して下さい。');
        return;
    }
