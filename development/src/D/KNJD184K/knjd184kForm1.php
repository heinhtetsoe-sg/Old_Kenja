...
class knjd184kForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd184kForm1", "POST", "knjd184kindex.php", "", "knjd184kForm1");
        ...
        //学期コンボ
        $extra = "onChange=\"return btn_submit('knjd184k');\"";
        $query = knjd184kQuery::getSemester();
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //表示指定ラジオボタン 1:クラス 2:個人
        $opt_disp = array(1, 2);
        $model->field["DISP"] = ($model->field["DISP"] == "") ? "1" : $model->field["DISP"];
        $extra = array("id=\"DISP1\" onClick=\"return btn_submit('knjd184k')\"", "id=\"DISP2\" onClick=\"return btn_submit('knjd184k')\"");
        ...
        if ($model->field["DISP"] == 1) {
            ...
            $query = knjd184kQuery::getGrade($model);
            $model->field["GRADE"] = $model->changeDispFlg ? substr($model->field["GRADE_HR_CLASS"], 0, 2) : $model->field["GRADE"];
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
        } else if ($model->field["DISP"] == 2) {
            //年組コンボ
            $extra = "onChange=\"return btn_submit('chgHrClass');\"";
            $opt = array();
            $tmpArr = array();
            $result = $db->query(knjd184kQuery::getHrClass($model));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                $opt[] = array("label" => $row["LABEL"],
                            "value" => $row["VALUE"]);
                if ($model->changeDispFlg && substr($row["VALUE"], 0, 2) == $model->field["GRADE"]) {
                    $tmpArr[] = $row["VALUE"];
                }
            }
            $model->field["GRADE_HR_CLASS"] = ($model->changeDispFlg) ? $tmpArr[0] : $model->field["GRADE_HR_CLASS"];
        ...
        //記載日付
        if (!strlen($model->field["DESC_DATE"])) {
            $model->field["DESC_DATE"] = str_replace("-","/", CTRL_DATE);
        }
        $arg["data"]["DESC_DATE"]       = View::popUpCalendar($objForm, "DESC_DATE", $model->field["DESC_DATE"]);

        //記載日付の印刷チェックボックス
        $extra  = $model->field["DESC_DATE_PRINT"] == "1" ? "checked" : "";
        $extra .= " id=\"DESC_DATE_PRINT\"";
        $arg["data"]["DESC_DATE_PRINT"] = knjCreateCheckBox($objForm, "DESC_DATE_PRINT", "1", $extra, "");

        //課程修了
        $disabled = "";
        if ($model->field["SEMESTER"] != $db->getOne(knjd184kQuery::getMaxSemester())) {
            $model->field["PRINT_FOOTER"] = "";
            $disabled = " disabled=\"disabled\" ";
        }
        $extra = $model->field["PRINT_FOOTER"] == "1" ? "checked" : "";
        $extra .= " id=\"PRINT_FOOTER\" ".$disabled;
        $arg["data"]["PRINT_FOOTER"] = knjCreateCheckBox($objForm, "PRINT_FOOTER", "1", $extra, "");
        ...
        View::toHTML($model, "knjd184kForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    //年組取得
    $query = knjd184kQuery::getHrClass($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        if ($model->field["DISP"] == 2) {
            //年組のMAX文字数取得
    ...
    if ($model->field["DISP"] == 2) {
        //生徒取得
        $query = knjd184kQuery::getSchList($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //クラス名称調整
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD184K");
    ...
    knjCreateHidden($objForm, "predisp", $model->field["DISP"]);
    knjCreateHidden($objForm, "minHrClass");
}
?>
