...
class knjd186jForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd186jForm1", "POST", "knjd186jindex.php", "", "knjd186jForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        if ($model->semester && $value_flg) {
            ...
            $maxSemester = $db->getOne(knjd186jQuery::getMaxSemester());
        ...
        //学年コンボ
        $query = knjd186jQuery::getIBGrade($model);
        $extra = "onchange=\"return btn_submit('main');\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->grade, $extra, 1);

        //年組コンボ
        $query = knjd186jQuery::getAuth($model);
        $extra = "onchange=\"return btn_submit('main')\"";
        ...
        knjCreateHidden($objForm, "PRGID",         "KNJD186J");
        ...
        View::toHTML($model, "knjd186jForm1.html", $arg);
    }
}
/****************************************************** 以下関数 ************************************************************/
//コンボ作成
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    //クラス一覧
    $opt = array();
    $query = knjd186jQuery::getStudent($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $opt[] = array('label' => $row["LABEL"],
                       'value' => $row["VALUE"]);
