...
class knjd129aForm1
{
    public function main(&$model)
    {
        ...
        $arg["start"]    = $objForm->get_start("main", "POST", "knjd129aindex.php", "", "main");
        $arg["YEAR"]     = CTRL_YEAR;
        ...
        $query = knjd129aQuery::getTestName($model);
        ...
        $header = array("0" => "科目コード",
                        ...
                        "2911" => $semename[2]."平常点",
                        "2902" => $semename[2]."成績",
                        "3111" => $semename[3]."－".$testname[30101],
        ...
        $result = $db->query(knjd129aQuery::selectSubclassQuery($model->gen_ed, $model));
        //教育課程対応
        if ($model->Properties["useCurriculumcd"] == '1') {
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                if ($row["CLASSCD"].'-'.$row["SCHOOL_KIND"].'-'.$row["CURRICULUM_CD"].'-'.$row["SUBCLASSCD"] !== $row["COMBINED_CLASSCD"].'-'.$row["COMBINED_SCHOOL_KIND"].'-'.$row["COMBINED_CURRICULUM_CD"].'-'.$row["COMBINED_SUBCLASSCD"]) {
            ...
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                if ($row["SUBCLASSCD"] !== $row["COMBINED_SUBCLASSCD"]) {
        ...
        $result = $db->query(knjd129aQuery::selectChairQuery($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $result = $db->query(knjd129aQuery::selectContolCodeQuery($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($seme_test != "0") {
            $result = $db->query(knjd129aQuery::selectExecuteDateQuery($model, $seme_test, $kind_test, $item_test));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //遅刻何回で欠課とするかの指数取得
        $absent = array();
        $absent = $db->getRow(knjd129aQuery::getScAbsentCov($model), DB_FETCHMODE_ASSOC);

        //文字評定
        if ($model->gen_ed == substr($model->field["SUBCLASSCD"], 0, 2)) {
            $assess = array();
            $result = $db->query(knjd129aQuery::getAssessMark());
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $offdaysFlg = $db->getRow(knjd129aQuery::getOffdaysFlg(CTRL_YEAR), DB_FETCHMODE_ASSOC);
        //帳票パラメータ
        ...
        if ($absent["ABSENT_COV"] == "0" || $absent["ABSENT_COV"] == "2" || $absent["ABSENT_COV"] == "4" || $absent["ABSENT_COV"] == "5") {
            $result = $db->query(knjd129aQuery::getAttendData($model->field["CHAIRCD"], $model->field["SUBCLASSCD"], $absent["ABSENT_COV"], $absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        } else {
            $result = $db->query(knjd129aQuery::getAttendData2($model->field["CHAIRCD"], $model->field["SUBCLASSCD"], $absent["ABSENT_COV"], $absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        }
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //合併先科目の単位固定／加算フラグ
        $model->creditflg = $db->getOne(knjd129aQuery::getCalculateCreditFlg($model->field["SUBCLASSCD"], $model));

        //一覧表示
        $colorFlg = false;
        $result = $db->query(knjd129aQuery::selectQuery($model, $execute_date));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //学籍番号をHiddenで保持
            ...
            foreach ($ctrl_name as $code => $col) {
                ...
                knjCreateHidden($objForm, $col."_DI"."-".$counter, $row[$col."_DI"]);
                knjCreateHidden($objForm, $col."_PASS"."-".$counter, $row[$col."_PASS"]);
                //見込点情報
                ...
                //管理者コントロール
                    ...
                    $value = $row[$col];
                    //テキストボックスを作成
                    $width = ($code == "9902" || $code == "9903" || $code == "9904") ? "25" : "30";
                    $objForm->ae(array("type"      => "text",
                                        "name"      => $col."-".$counter,
                                        "size"      => "3",
                                        "maxlength" => ($code == "9903" || $code == "9904") ? "2" : "3",
                                        "value"     => $value,
                                        "extrahtml" => "STYLE=\"text-align: right; width:{$width}\" onChange=\"this.style.background='#ccffcc'\" onblur=\"calc(this);\" onKeyDown=\"keyChangeEntToTab2(this, '{$setTextField}', {$counter})\" onPaste=\"return showPaste(this);\" id=\"{$col}-{$counter}\""));
                    $row[$col] = $objForm->ge($col."-".$counter);

                    //考査満点マスタ
                    $query = knjd129aQuery::getPerfect(CTRL_YEAR, $model->field["SUBCLASSCD"], $perfectcd[$code], $row["GRADE"], $row["COURSE"], $model);
                    $perfect = ($model->usePerfect == 'true') ? $db->getOne($query) : 999;
                    if ($perfect == "") {
                        $perfect = 999;
                    }
                    //満点チェック用を作成
        ...
        //累積現在日
        $cur_date = $db->getRow(knjd129aQuery::getMax($model->field["SUBCLASSCD"], $model), DB_FETCHMODE_ASSOC);
        ...
        foreach ($testcdArray as $testkey => $testval) {
            ...
            if ($model->useRecordChkfinDat == "1") {
                $query = knjd129aQuery::getRecordChkfinDat($model, $testval, $recordDiv);
            } else {
                $query = knjd129aQuery::getRecordSchChrTest($model, $testval);
            }
        ...
        *********************************************/
        ...
        View::toHTML($model, "knjd129aForm1.html", $arg);
    }
}
