...
class knjd154kForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd154kForm1", "POST", "knjd154kindex.php", "", "knjd154kForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        $extra = "onchange=\"return btn_submit('knjd154k'), AllClearList();\"";
        ...
        //プレビュー/印刷
        $arg["button"]["btn_csv"] = knjCreateBtn($objForm, "btn_csv", "CSV出力", "onclick=\"return btn_submit('csv');\"");
        //終了
        ...
        knjCreateHidden($objForm, "PRGID",         "KNJD154K");
        ...
        View::toHTML($model, "knjd154kForm1.html", $arg);
    }
}
/****************************************************** 以下関数 ************************************************************/
//コンボ作成
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    //クラス一覧
    $opt = array();
    $query = knjd154kQuery::getStudent($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $opt[] = array('label' => $row["LABEL"],
                       'value' => $row["VALUE"]);
