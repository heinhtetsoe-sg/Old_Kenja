...
class knjd139cForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjd139cindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //学期コンボ
        $query = knjd139cQuery::getSemester();
        $extra = "onChange=\"return btn_submit('edit');\"";
        makeCmb($objForm, $arg, $db, $query, $model->semester, "SEMESTER", $extra, 1, "");

        //警告メッセージを表示しない場合
        if ((isset($model->schregno) && !isset($model->warning)) || !isset($model->schregno)) {
            $row = $db->getRow(knjd139cQuery::getHreportremarkDat($model, $model->schregno), DB_FETCHMODE_ASSOC);
            $row_detail = $db->getRow(knjd139cQuery::getHreportremarkDetailDat($model, $model->schregno), DB_FETCHMODE_ASSOC);
            $arg["NOT_WARNING"] = 1;
        } else {
            $row =& $model->field;
        }
        ...
        //道徳
        $arg["data"]["MORAL"] = getTextOrArea($objForm, "MORAL", $model->moral_moji, $model->moral_gyou, $row_detail["REMARK1"], $model);
        setInputChkHidden($objForm, "MORAL", $model->moral_moji, $model->moral_gyou, $arg);

        //備考
        $arg["data"]["COMMUNICATION"] = getTextOrArea($objForm, "COMMUNICATION", $model->comm_moji, $model->comm_gyou, $row["COMMUNICATION"], $model);
        setInputChkHidden($objForm, "COMMUNICATION", $model->comm_moji, $model->comm_gyou, $arg);
        ...
        View::toHTML5($model, "knjd139cForm1.html", $arg);
    }
}
