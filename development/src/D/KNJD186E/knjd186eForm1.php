...
class knjd186eForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd186eForm1", "POST", "knjd186eindex.php", "", "knjd186eForm1");
        ...
        //学期コンボ作成
        $query = knjd186eQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd186e'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //クラスコンボ作成
        $query = knjd186eQuery::getGradeHrClass($model);
        $extra = "onchange=\"return btn_submit('knjd186e'), AllClearList();\"";
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd186e', 'on')\"".$disabled;
        $date_textbox = knjCreateTextBox($objForm, $model->field["DATE"], "DATE", 12, 12, $extra);
        //異動対象日付（カレンダー）
        global $sess;
        $extra = "onclick=\"tmp_list('knjd186e', 'off'); loadwindow('" .REQUESTROOT ."/common/calendar.php?name=DATE&frame='+getFrameName(self) + '&date=' + document.forms[0]['DATE'].value + '&CAL_SESSID=$sess->id&reload=true', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 200)\"";
        ...
        //「欠課時数0を表示しない」チェックボックスを作成
        $extra = $model->field["KEKKAJISU0"] == "1" ? "checked" : "";
        $extra .= " id=\"KEKKAJISU0\" ";
        $arg["data"]["KEKKAJISU0"] = knjCreateCheckBox($objForm, "KEKKAJISU0", "1", $extra, "");
        ...
        View::toHTML($model, "knjd186eForm1.html", $arg); 
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    //対象外の生徒取得
    $query = knjd186eQuery::getSchnoIdou($model);
    ...
    //対象者リストを作成する
    $query = knjd186eQuery::getSchno($model);
    $result = $db->query($query);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD186E");
