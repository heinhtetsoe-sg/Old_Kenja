...
function calc(obj) {
    ...
    //合計を計算する
    if (nam.match(/SEM1_INTR_SCORE./) || nam.match(/SEM1_TERM_SCORE./) ||
        nam.match(/SEM2_INTR_SCORE./) || nam.match(/SEM2_TERM_SCORE./) ||
        nam.match(/SEM3_TERM_SCORE./)) {
        calcTotal(obj);
    }
}
...
function calcTotal(obj) {
    var nam = obj.name;
    //合計を計算する
    var gyoNo = nam.split("-")[1];
    sem1IObject = eval("document.forms[0][\"SEM1_INTR_SCORE-" + gyoNo + "\"]");
    sem1TObject = eval("document.forms[0][\"SEM1_TERM_SCORE-" + gyoNo + "\"]");
    sem2IObject = eval("document.forms[0][\"SEM2_INTR_SCORE-" + gyoNo + "\"]");
    sem2TObject = eval("document.forms[0][\"SEM2_TERM_SCORE-" + gyoNo + "\"]");
    sem3TObject = eval("document.forms[0][\"SEM3_TERM_SCORE-" + gyoNo + "\"]");
    var sem1I = sem1IObject.value;
    var sem1T = sem1TObject.value;
    var sem2I = sem2IObject.value;
    var sem2T = sem2TObject.value;
    var sem3T = sem3TObject.value;
    //1学期合計
    if (nam.match(/SEM1_INTR_SCORE./) || nam.match(/SEM1_TERM_SCORE./)) {
        var total;
        var totalName = "SEM1_SCORE";
        if (sem1I != '' || sem1T != '') {
            total = 0;
            if (sem1I != '') total += parseInt(sem1I);
            if (sem1T != '') total += parseInt(sem1T);
        } else {
            total = '';
        }
        //ラベル
        document.all[(totalName) + '_SPAN_ID' + '-' + (gyoNo)].innerHTML = total;
        //テキストまたはhidden
        totalObject = eval("document.forms[0][\"" + totalName + "-" + gyoNo + "\"]");
        totalObject.value = total;
    }
    //2学期合計
    if (nam.match(/SEM2_INTR_SCORE./) || nam.match(/SEM2_TERM_SCORE./)) {
        var total;
        var totalName = "SEM2_SCORE";
        if (sem2I != '' || sem2T != '') {
            total = 0;
            if (sem2I != '') total += parseInt(sem2I);
            if (sem2T != '') total += parseInt(sem2T);
        } else {
            total = '';
        }
        //ラベル
    ...
    if (nam.match(/SEM1_INTR_SCORE./) || nam.match(/SEM1_TERM_SCORE./) ||
        ...
        nam.match(/SEM3_TERM_SCORE./)) {
        var total;
        var totalName = "GRAD_SCORE";
        if (sem1I != '' || sem1T != '' || sem2I != '' || sem2T != '' || sem3T != '') {
            total = 0;
            if (sem1I != '') total += parseInt(sem1I);
            if (sem1T != '') total += parseInt(sem1T);
            if (sem2I != '') total += parseInt(sem2I);
            if (sem2T != '') total += parseInt(sem2T);
            if (sem3T != '') total += parseInt(sem3T);
        } else {
            total = '';
        }
        //ラベル
