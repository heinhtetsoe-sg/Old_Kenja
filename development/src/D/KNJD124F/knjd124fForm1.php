...
class knjd124fForm1
{
    public function main(&$model)
    {
        ...
        $arg["start"]    = $objForm->get_start("main", "POST", "knjd124findex.php", "", "main");
        $arg["YEAR"]     = CTRL_YEAR;
        ...
        $result = $db->query(knjd124fQuery::selectSubclassQuery($model->gen_ed, $model));
        //教育課程対応
        ...
        $result = $db->query(knjd124fQuery::selectChairQuery($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $query = knjd124fQuery::getTestName($model);
        ...
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            //ソート（三角マーク）
            $cmdName = "sort" .$row["VALUE_TEST"];
            $arg["SORT_MARK".$row["VALUE_TEST"]] = ($model->sort == '{$cmdName}') ? '(▼)' : '';
            //ソート（素点）
            $name = $row["TESTITEMNAME"];
            $extra = "onClick=\"return btn_submit('{$cmdName}');\"";
            $arg["TESTNAME".$row["VALUE_TEST"]] = View::alink("#", htmlspecialchars($name), $extra);
        }

        //ソート（三角マーク）
        $arg["SORT_MARK0"] = ($model->sort == '')      ? '(▲)' : '';
        $arg["SORT_MARK1"] = ($model->sort == 'sort1') ? '(▼)' : '';
        $arg["SORT_MARK2"] = ($model->sort == 'sort2') ? '(▼)' : '';
        $arg["SORT_MARK9"] = ($model->sort == 'sort9') ? '(▼)' : '';
        //ソート（各学期合計、総合計）
        $name = "合計";
        $extra = "onClick=\"return btn_submit('sort1');\"";
        $arg["SORT_TOTAL1"] = View::alink("#", htmlspecialchars($name), $extra);
        $name = "合計";
        $extra = "onClick=\"return btn_submit('sort2');\"";
        $arg["SORT_TOTAL2"] = View::alink("#", htmlspecialchars($name), $extra);
        $name = "総合計";
        $extra = "onClick=\"return btn_submit('sort9');\"";
        $arg["SORT_TOTAL9"] = View::alink("#", htmlspecialchars($name), $extra);
        //ソート（クラス-出席番号）
        $name2 = '出席番号';
        $extra = "onClick=\"return btn_submit('');\"";
        $arg["SORT_CLASS"] = View::alink("#", htmlspecialchars($name2), $extra);
        //hidden（帳票パラメータ）
        ...
        $result = $db->query(knjd124fQuery::selectContolCodeQuery($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            if (substr($row["CONTROL_CODE"], 1, 3) == "911") {
                continue;
            }
        ...
        if ($seme_test != "0") {
            $result = $db->query(knjd124fQuery::selectExecuteDateQuery($model, $seme_test, $kind_test, $item_test));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //遅刻何回で欠課とするかの指数取得
        $absent = array();
        $absent = $db->getRow(knjd124fQuery::getScAbsentCov($model), DB_FETCHMODE_ASSOC);

        //文字評定
        if ($model->gen_ed == substr($model->field["SUBCLASSCD"], 0, 2)) {
            $assess = array();
            $result = $db->query(knjd124fQuery::getAssessMark());
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $offdaysFlg = $db->getRow(knjd124fQuery::getOffdaysFlg(CTRL_YEAR), DB_FETCHMODE_ASSOC);
        //帳票パラメータ
        ...
        if ($absent["ABSENT_COV"] == "0" || $absent["ABSENT_COV"] == "2" || $absent["ABSENT_COV"] == "4" || $absent["ABSENT_COV"] == "5") {
            $result = $db->query(knjd124fQuery::getAttendData($model->field["CHAIRCD"], $model->field["SUBCLASSCD"], $absent["ABSENT_COV"], $absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        } else {
            $result = $db->query(knjd124fQuery::getAttendData2($model->field["CHAIRCD"], $model->field["SUBCLASSCD"], $absent["ABSENT_COV"], $absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        }
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //合併先科目の単位固定／加算フラグ
        $model->creditflg = $db->getOne(knjd124fQuery::getCalculateCreditFlg($model->field["SUBCLASSCD"], $model));

        //一覧表示
        $colorFlg = false;
        $result = $db->query(knjd124fQuery::selectQuery($model, $execute_date));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //学籍番号をHiddenで保持
            ...
            foreach ($ctrl_name as $code => $col) {
                ...
                if (!$edit_flg && $model->auth != DEF_UPDATABLE || ($code == "1911" || $code == "2911" || $code == "9911")) {
                    $setTextField .= "";
            ...
            foreach ($ctrl_name as $code => $col) {
                ...
                if (!$edit_flg && $model->auth != DEF_UPDATABLE || ($code == "1911" || $code == "2911" || $code == "9911")) {
                    $value = $row[$col];
                    if ($code == "1911" || $code == "2911" || $code == "9911") {
                ...
                //管理者コントロール
                    ...
                    //考査満点マスタ
                    $query = knjd124fQuery::getPerfect(CTRL_YEAR, $model->field["SUBCLASSCD"], $perfectcd[$code], $row["GRADE"], $row["COURSE"], $model);
                    $perfect = ($model->usePerfect == 'true') ? $db->getOne($query) : 100;
                    if ($perfect == "") {
                        $perfect = 100;
                    }
                    if ($code == "1911" || $code == "2911") {
                        $perfect = 200;
                    }
                    if ($code == "9911") {
                        $perfect = 500;
        ...
        //累積現在日
        $cur_date = $db->getRow(knjd124fQuery::getMax($model->field["SUBCLASSCD"], $model), DB_FETCHMODE_ASSOC);
        ...
        foreach ($testcdArray as $testkey => $testval) {
            ...
            if ($model->useRecordChkfinDat == "1") {
                $query = knjd124fQuery::getRecordChkfinDat($model, $testval, $recordDiv);
            } else {
                $query = knjd124fQuery::getRecordSchChrTest($model, $testval);
            }
        ...
        View::toHTML($model, "knjd124fForm1.html", $arg);
    }
}
