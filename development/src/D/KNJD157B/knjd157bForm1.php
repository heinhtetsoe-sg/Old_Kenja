...
class knjd157bForm1
{
    public function main(&$model)
    {
        ...
        $objForm = new form();
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd157bForm1", "POST", "knjd157bindex.php", "", "knjd157bForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        $extra = array("id=\"CATEGORY_IS_CLASS1\" onClick=\"return btn_submit('knjd157b')\"", "id=\"CATEGORY_IS_CLASS2\" onClick=\"return btn_submit('knjd157b')\"");
        ...
        //学期コンボ作成
        $query = knjd157bQuery::getSemester();
        $extra = "onchange=\"return btn_submit('change_semes'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //学年コンボ作成
        $query = knjd157bQuery::getGradeHrClass($model, $model->field["SEMESTER"], "GRADE");
        $extra = "onchange=\"return btn_submit('change_grade'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        if ($model->field["CATEGORY_IS_CLASS"] == 2) {
            //クラスコンボ作成
            $query = knjd157bQuery::getGradeHrClass($model, $model->field["SEMESTER"], "HR_CLASS");
            $extra = "onchange=\"return btn_submit('knjd157b'), AllClearList();\"";
        ...
        View::toHTML($model, "knjd157bForm1.html", $arg);
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model)
{
    if ($model->field["CATEGORY_IS_CLASS"] == 1) {
        //対象クラスリストを作成する
        $query = knjd157bQuery::getGradeHrClass($model, $model->field["SEMESTER"], $model, "HR_CLASS");
        ...
        //対象者リストを作成する
        $query = knjd157bQuery::getStudent($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt1 = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD157B");
    ...
    knjCreateHidden($objForm, "knjd157bNotPrintMajorname", $model->Properties["knjd157bNotPrintMajorname"]);
