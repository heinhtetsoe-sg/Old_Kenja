...
class knjd128aForm1
{
    function main(&$model)
    {
        ...
        $arg["start"]    = $objForm->get_start("main", "POST", "knjd128aindex.php", "", "main");
        $arg["YEAR"]     = CTRL_YEAR;
        ...
        $result = $db->query(knjd128aQuery::selectSubclassQuery($model->gen_ed, $model));
        //教育課程対応
        ...
        $result = $db->query(knjd128aQuery::selectChairQuery($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $query = knjd128aQuery::getTestName($model);
        ...
        $result = $db->query(knjd128aQuery::selectContolCodeQuery());
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            if (!isset($perfectcd[$row["CONTROL_CODE"]])) continue;
            ...
            if (substr($row["CONTROL_CODE"], 1, 1) == "9" || substr($row["CONTROL_CODE"], 3, 1) != "1") continue;
        ...
        if ($seme_test != "0") {
            $result = $db->query(knjd128aQuery::selectExecuteDateQuery($model, $seme_test, $kind_test, $item_test));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //遅刻何回で欠課とするかの指数取得
        $absent = array();
        $absent = $db->getRow(knjd128aQuery::getScAbsentCov($model),DB_FETCHMODE_ASSOC);

        //文字評定
        if ($model->gen_ed == substr($model->field["SUBCLASSCD"],0,2)) {
            $assess = array();
            $result = $db->query(knjd128aQuery::GetAssessMark());
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $offdaysFlg = $db->getRow(knjd128aQuery::getOffdaysFlg(CTRL_YEAR),DB_FETCHMODE_ASSOC);
        //帳票パラメータ
        ...
        if ($absent["ABSENT_COV"] == "0" || $absent["ABSENT_COV"] == "2" || $absent["ABSENT_COV"] == "4" || $absent["ABSENT_COV"] == "5") { 
            $result = $db->query(knjd128aQuery::GetAttendData($model->field["CHAIRCD"],$model->field["SUBCLASSCD"],$absent["ABSENT_COV"],$absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        } else {
            $result = $db->query(knjd128aQuery::GetAttendData2($model->field["CHAIRCD"],$model->field["SUBCLASSCD"],$absent["ABSENT_COV"],$absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        }
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //合併先科目の単位固定／加算フラグ
        $model->creditflg = $db->getOne(knjd128aQuery::getCalculateCreditFlg($model->field["SUBCLASSCD"], $model));

        //一覧表示
        $colorFlg = false;
        $result = $db->query(knjd128aQuery::selectQuery($model, $execute_date));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //学籍番号を配列で取得
            ...
            foreach ($ctrl_name as $code => $col) {
                ...
                //管理者コントロール
                    ...
                    $objForm->ae( array("type"      => "text",
                                        ...
                                        "value"     => $value,
                                        "extrahtml" => " onKeyDown=\"keyChangeEntToTab2(this, '{$setTextField}', {$counter})\"; STYLE=\"text-align: right; width:30\" onChange=\"this.style.background='#ccffcc'\" onblur=\"calc(this);\" onKeyDown=\"keyChangeEntToTab(this)\" onPaste=\"return show(this);\" "));
                    $row[$col] = $objForm->ge($col."-".$counter);

                    //考査満点マスタ
                    $query = knjd128aQuery::getPerfect(CTRL_YEAR, $model->field["SUBCLASSCD"], $perfectcd[$code], $row["GRADE"], $row["COURSE"], $model);
        ...
        //累積現在日
        $cur_date = $db->getRow(knjd128aQuery::GetMax($model->field["SUBCLASSCD"], $model), DB_FETCHMODE_ASSOC);
        ...
        foreach ($testcdArray as $testkey => $testval) {
            ...
            if ($model->useRecordChkfinDat == "1") {
                $query = knjd128aQuery::getRecordChkfinDat($model, $testval, $recordDiv);
            } else {
                $query = knjd128aQuery::getRecordSchChrTest($model, $testval);
            }
        ...
        View::toHTML($model, "knjd128aForm1.html", $arg);
    }
}
?>
