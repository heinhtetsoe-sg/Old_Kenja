...
class knjd132fQualified {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("qualified", "POST", "knjd132findex.php", "", "qualified");
        ...
        $counter = 0;
        if ($model->schregno) {
            $result = $db->query(knjd132fQuery::getAward($model));
        ...
        if (VARS::get("cmd") == "right_list"){ 
            $arg["reload"] = "parent.edit_frame.location.href='knjd132findex.php?cmd=edit'";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjd132fQualified.html", $arg);
    }
}
