...
class knjd141bForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd141bForm1", "POST", "knjd141bindex.php", "", "knjd141bForm1");
        ...
        //学期コンボ作成
        $query = knjd141bQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd141b');\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //対象学期の開始日・終了日
        $sDate = $db->getOne(knjd141bQuery::getSemesterDate("SDATE", $model->field["SEMESTER"]));
        $eDate = $db->getOne(knjd141bQuery::getSemesterDate("EDATE", $model->field["SEMESTER"]));
        ...
        $extra = array("id=\"DISP1\" onClick=\"return btn_submit('knjd141b')\"", "id=\"DISP2\" onClick=\"return btn_submit('knjd141b')\"");
        ...
        if ($model->field["DISP"] == 1) {
            //学年コンボ
            $query = knjd141bQuery::getGrade($model);
            $extra = "onChange=\"return btn_submit('knjd141b');\"";
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
        } elseif ($model->field["DISP"] == 2) {
            //クラスコンボ作成
            $query = knjd141bQuery::getHrClass(CTRL_YEAR, $model->field["SEMESTER"], $model);
            $extra = "onchange=\"return btn_submit('knjd141b');\"";
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd141b', 'off')\"".$disabled;
        $date_textbox = knjCreateTextBox($objForm, $model->field["DATE"], "DATE", 12, 12, $extra);
        //異動対象日付（カレンダー）
        global $sess;
        $extra = "onclick=\"tmp_list('knjd141b', 'off'); loadwindow('" .REQUESTROOT ."/common/calendar.php?name=DATE&frame='+getFrameName(self) + '&date=' + document.forms[0]['DATE'].value + '&CAL_SESSID=$sess->id', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 200)\"";
        ...
        View::toHTML($model, "knjd141bForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model, $seme)
{
    ...
    if ($model->field["DISP"] == 2) {
        ...
        $arg["data"]["TITLE_LEFT"]  = "対象クラス一覧";
        $arg["data"]["TITLE_RIGHT"] = "クラス一覧";
    }

    //対象外の生徒取得
    $query = knjd141bQuery::getSchnoIdou($model, $seme);
    ...
    //年組取得
    $query = knjd141bQuery::getHrClass(CTRL_YEAR, $seme, $model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        if ($model->field["DISP"] != 2) {
            //一覧リスト（右側）
    ...
    //対象者リストを作成する
    if ($model->field["DISP"] == 2) {
        $query = knjd141bQuery::getStudent($model, $seme);
        $result = $db->query($query);
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD141B");
