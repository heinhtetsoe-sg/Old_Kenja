...
class knjd186cForm1 {
    function main(&$model) {
        $objForm = new form;
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd186cForm1", "POST", "knjd186cindex.php", "", "knjd186cForm1");
        ...
        //学期コンボボックスを作成する
        $query = knjd186cQuery::getSemester($model);
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], "onChange=\"return btn_submit('knjd186c');\"", 1);

        // 学科
        $query = knjd186cQuery::getCourseMajor($model);
        $extra = "onchange=\"return btn_submit('knjd186c');\"";
        makeCmb($objForm, $arg, $db, $query, "MAJOR", $model->field["MAJOR"], $extra, 1);
                             
        //クラスコンボ
        $query = knjd186cQuery::getSelectHrclass($model);
        $extra = "onchange=\"return btn_submit('knjd186c');\"";
        ...
        View::toHTML($model, "knjd186cForm1.html", $arg); 
    }
}
/*********************************************** 以下関数 *******************************************************/
/**********/
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    //クラス一覧
    $row1 = array();
    $query = knjd186cQuery::getAuth($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID",         "KNJD186C");
