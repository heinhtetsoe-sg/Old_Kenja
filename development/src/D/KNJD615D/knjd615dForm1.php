...
class knjd615dForm1
{
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd615dForm1", "POST", "knjd615dindex.php", "", "knjd615dForm1");
        ...
        //学期コンボボックスを作成する
        $query = knjd615dQuery::getSemester();
        ...
        //学校名取得
        $query = knjd615dQuery::getSchoolname();
        ...
        if ($model->cmd == 'change_grade' || $model->cmd == '') {
            $query = knjd615dQuery::getGdat($model->field["GRADE"]);
            $h_j = $db->getOne($query);
            if ($h_j == 'J') {
        ...
        View::toHTML($model, "knjd615dForm1.html", $arg); 
    }
}
/**************************************** 以下関数 **************************************************/
//コンボ作成
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    //クラス一覧
    $row1 = array();
    $result = $db->query(knjd615dQuery::getAuth($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model, $seme, $semeflg)
{
    ...
    $objForm->ae(createHiddenAe("PRGID", "KNJD615D"));
