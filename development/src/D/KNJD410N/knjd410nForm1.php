...
class knjd410nForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjd410nindex.php", "", "edit");
        ...
        if ($model->Properties["useGradeKindCompGroupSemester"] == "1") {
            //学期コンボ作成
            $query = knjd410nQuery::getSemester($model);
            $extra = "onchange=\"return btn_submit('combo');\"";
            makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->semester, $extra, 1, $model);
        }

        //学部コンボ作成
        $query = knjd410nQuery::getSchoolKind();
        $extra = "onchange=\"return btn_submit('combo');\"";
        ...
        //年組コンボ
        $query = knjd410nQuery::getHrClass($model);
        ...
        if ($model->hukusiki_radio == "2") {
            $query = knjd410nQuery::getHrClass2($model);
            $extra = "onchange=\"btn_submit('combo');\"";
        ...
        //一覧表示
        $key = "";
        $query = knjd410nQuery::getList($model, "");
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            if ($key !== $row["GAKUBU_SCHOOL_KIND"].'-'.$row["CONDITION"]) {
                $cnt = $db->getOne(knjd410nQuery::getList($model, $row["CONDITION"]));
            ...
            //状態区分
            $row["CONDITION_NAME"] = $db->getOne(knjd410nQuery::getCondition($row["CONDITION"]));
            //グループ名
            $row["GROUP_NAME"] = $db->getOne(knjd410nQuery::getGroupName($model, $row["CONDITION"], $row["GROUPCD"]));
            //件数
            $row["SCHREG_COUNT"] = $db->getOne(knjd410nQuery::getSchregCnt($model, $row["CONDITION"], $row["GROUPCD"]));
        ...
        if (VARS::get("shori") != "update") {
            if ($model->cmd == "combo") {
                $model->gakubu_school_kind = "";
                $model->condition = "";
                $model->groupcd = "";
            }
            $arg["jscript"] = "window.open('knjd410nindex.php?cmd=edit','right_frame')";
        }
        ...
        View::toHTML($model, "knjd410nForm1.html", $arg);
    }
}
