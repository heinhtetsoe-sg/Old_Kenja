...
class knjd126nForm1
{
    function main(&$model)
    {
        /* フォーム作成 */
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("main", "POST", "knjd126nindex.php", "", "main");
        ...
        /* 年組 */
        $query = knjd126nQuery::selectGradeHrClass($model);
        ...
        $query = knjd126nQuery::selectSchoolKind($grade);
        ...
        $query = knjd126nQuery::selectSubclasscd($model);
        ...
        /* 処理学期 */
        $query = knjd126nQuery::selectSemester($model);
        ...
        /* テンプレート呼び出し */
        View::toHTML($model, "knjd126nForm1.html", $arg);
    }
}
...
function setViewData(&$objForm, &$arg, $db, &$model) {
    ...
    //テスト単元・観点一覧取得
    $unitViewList = array();
    $result = $db->query(knjd126nQuery::getUnitViewList($model));
...
function setViewDataSeme(&$objForm, &$arg, $db, &$model, $unitViewList) {
    ...
    //学期・観点一覧取得
    $semeViewList = array();
    $result = $db->query(knjd126nQuery::getSemeViewList($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeDataList(&$objForm, &$arg, $db, &$model, $unitViewList, $semeViewList, $isSelectCmb) {
    ...
    $getData = array();
    $getScoreOmomi = array();
    $query = knjd126nQuery::getScoreData($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    if ($isSelectCmb && $model->cmd == "calc") {
        //観点データ（集計）取得・・・処理学期のみ
        $query = knjd126nQuery::getCalcJviewstatRecord($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //観点データ（DB）取得・・・処理学期以外
        $query = knjd126nQuery::getJviewstatRecord($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    $query = knjd126nQuery::getSchList($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            if ($model->Properties["useJviewHyouka_DankaichiSansyou"] == "1") {
                ...
                if ($semester == "9") {
                    ...
                    if ($model->Properties["useAssessSubclassMst"] == "1") {
                        $query = knjd126nQuery::getSubclassAssess($model, $row["GRADE"], $row["TOTALCD"], $model->field["SUBCLASSCD"], $totalScore);
                    } else {
                        $query = knjd126nQuery::getAssess($semester, $totalScore);
                    }
                    $setMark = $db->getOne($query);
                } else {
                    $setSemeTotal .= "<td width=\"40\">".$totalScore."</td>";
                    $totalScore = strlen($totalScore) > 0 ? $totalScore : 0;
                    $query = knjd126nQuery::getJviewStatLevel($model, $totalScore, $semester);
                    $setMark = $db->getOne($query);
                }
                ...
                $setSemeTotal .= "<td width=\"40\">".$totalScore."</td>";
                if ($model->Properties["useAssessSubclassMst"] == "1") {
                    $query = knjd126nQuery::getSubclassAssess($model, $row["GRADE"], $row["TOTALCD"], $model->field["SUBCLASSCD"], $setCalcRes);
                } else {
                    $query = knjd126nQuery::getAssess($semester, $setCalcRes);
                }
                $setMark = $db->getOne($query);
                $model->totalSchregArray[$row["SCHREGNO"]][$semester]["MARK"] = $setMark;
                $setSemeTotal .= "<td width=\"40\">".$setMark."</td>";
