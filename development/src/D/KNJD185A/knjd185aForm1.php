...
class knjd185aForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd185aForm1", "POST", "knjd185aindex.php", "", "knjd185aForm1");
        ...
        //学期コンボ
        $extra = "onChange=\"return btn_submit('knjd185a');\"";
        $query = knjd185aQuery::getSemester();
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //表示指定ラジオボタン 1:クラス 2:個人
        $opt_disp = array(1, 2);
        $model->field["DISP"] = ($model->field["DISP"] == "") ? "1" : $model->field["DISP"];
        $extra = array("id=\"DISP1\" onClick=\"return btn_submit('knjd185a')\"", "id=\"DISP2\" onClick=\"return btn_submit('knjd185a')\"");
        ...
        if ($model->field["DISP"] == 1) {
            //学年コンボ
            $extra = "onChange=\"return btn_submit('knjd185a');\"";
            $query = knjd185aQuery::getGrade($model);
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
            $schKindStr = $db->getOne(knjd185aQuery::getSchoolKind($model->field["GRADE"]));
        } elseif ($model->field["DISP"] == 2) {
            //年組コンボ
            $extra = "onChange=\"return btn_submit('knjd185a');\"";
            $query = knjd185aQuery::getHrClass($model);
            makeCmb($objForm, $arg, $db, $query, "GRADE_HR_CLASS", $model->field["GRADE_HR_CLASS"], $extra, 1);
            $schKindStr = $db->getOne(knjd185aQuery::getSchoolKind(substr($model->field["GRADE_HR_CLASS"], 0, 2)));
        }

        //テスト
        $query = knjd185aQuery::getTest($model);
        $extra = "";
        ...
        View::toHTML($model, "knjd185aForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    ...
    if ($model->field["DISP"] == 2) {
        //生徒取得
        $query = knjd185aQuery::getSchList($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //クラス名称調整
...
function makeHidden(&$objForm, $model, $schKindStr)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD185A");
