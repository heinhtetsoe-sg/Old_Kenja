...
class knjd130cForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjd130cindex.php", "", "edit");
        ...
        //学期コンボ
        $query = knjd130cQuery::getSemesterQuery();
        $extra = "onChange=\"return btn_submit('gakki');\"";
        makeCmb($objForm, $arg, $db, $query, "GAKKI", $model->gakki, $extra.$disable, 1);

        //警告メッセージを表示しない場合
        if ((isset($model->schregno) && !isset($model->warning)) || !isset($model->schregno)) {
            $Row    = $db->getRow(knjd130cQuery::getHreportremarkDat($model->schregno, $model->gakki), DB_FETCHMODE_ASSOC);
            $Row9   = $db->getRow(knjd130cQuery::getHreportremarkDat($model->schregno, "9"), DB_FETCHMODE_ASSOC);
            $arg["NOT_WARNING"] = 1;
        } else {
            $Row    =& $model->field;
            $Row9   =& $model->field9;
        ...
        foreach ($model->textList as $field => $val) {
            $value = ($val["flg"]) ? $Row9[$field] : $Row[$field];
            $name = ($val["flg"]) ? $field."_9" : $field;
            $id = "id=\"".$name."\"";
            if ($model->getPro[$field]["gyou"] == 1) {
                ...
                $extra = "style=\"height:{$height}px;white-space: pre;\" ". $id;
            ...
            setInputChkHidden($objForm, $name, $model->getPro[$field]["moji"], $model->getPro[$field]["gyou"]);
            $arg[$name."_LABEL"] = $val["label"];
        }

        //表示切替
        if ($model->school_kind == "J") {
            $arg["SCHOOLKIND_IS_J"] = 1;
        } else {
            $arg["SCHOOLKIND_IS_NOT_J"] = 1;
        }
        ...
        View::toHTML5($model, "knjd130cForm1.html", $arg);
    }
}
