...
class knjd219bForm2 {
    function main(&$model) {
        ...
        /* ヘッダ */
        /**********/
        $arg["sepa"]["TEST_NAME"]   = $db->getOne(knjd219bQuery::getTestName($model));
        $arg["sepa"]["DIV_NAME"]    = $div_name;
        $arg["sepa"]["GRADE_NAME"]  = $db->getOne(knjd219bQuery::getGradeName($model));
        ...
        View::toHTML($model, "knjd219bForm2.html", $arg);
    }
}
/********************************************** 以下関数 **********************************************/
//リスト作成
function makeList(&$objForm, &$arg, $db, $model) {
    ...
    $query = knjd219bQuery::getListInquiry($model);
