...
class knjd187mForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd187mForm1", "POST", "knjd187mindex.php", "", "knjd187mForm1");
        ...
        //学期コンボ作成
        $query = knjd187mQuery::getSemester();
        ...
        //対象学期の開始日・終了日
        $sDate = $db->getOne(knjd187mQuery::getSemesterDate("SDATE", $model->field["SEMESTER"]));
        $eDate = $db->getOne(knjd187mQuery::getSemesterDate("EDATE", $model->field["SEMESTER"]));
        knjCreateHidden($objForm, "SDATE", str_replace("-", "/", $sDate));
        knjCreateHidden($objForm, "EDATE", str_replace("-", "/", $eDate));

        //クラスコンボ作成
        $query = knjd187mQuery::getHrClass(CTRL_YEAR, $seme, $model);
        $extra = "onchange=\"return btn_submit('knjd187m');\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE_HR_CLASS", $model->field["GRADE_HR_CLASS"], $extra, 1);

        //異動対象日付初期値セット
        if ($model->field["DATE"] == "" && $model->cmd == "") {
            $model->field["DATE"] = str_replace("-", "/", CTRL_DATE);
        } elseif ($model->field["DATE"] == "" || $model->cmd == "chgSeme") {
            $model->field["DATE"] = str_replace("-", "/", $eDate);
        }
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd187m', 'on')\"".$disabled;
        $date_textbox = knjCreateTextBox($objForm, $model->field["DATE"], "DATE", 12, 12, $extra);
        //異動対象日付（カレンダー）
        global $sess;
        $extra = "onclick=\"tmp_list('knjd187m', 'off'); loadwindow('" .REQUESTROOT ."/common/calendar.php?name=DATE&frame='+getFrameName(self) + '&date=' + document.forms[0]['DATE'].value + '&CAL_SESSID=$sess->id&reload=true', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 200)\"";
        ...
        View::toHTML($model, "knjd187mForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model, $seme)
{
    ...
    //対象外の生徒取得
    $query = knjd187mQuery::getSchnoIdou($model, $seme);
    ...
    //対象者リストを作成する
    $query = knjd187mQuery::getStudent($model, $seme);
    $result = $db->query($query);
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD187M");
