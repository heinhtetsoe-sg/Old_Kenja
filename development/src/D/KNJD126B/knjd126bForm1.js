function btn_submit(cmd) {
    ...
    if (cmd == 'reset'){
        if (!confirm('{rval MSG106}'))
            return false;
    } else if (cmd == 'update_on' || cmd == 'update_off'){
        //更新権限チェック
        ...
        var jitugi_flg = document.forms[0].JITUGI_FLG.value;
        for (var i = 0; i < document.forms[0].elements.length; i++ ) {
            ...
            if (e.type == 'text' && e.value != '') {
                ...
                //実技科目の中間および調整点
                if (0 < parseInt(jitugi_flg) && (nam.match(/SCORE.111./) || nam.match(/SCORE.901./))) {
                    if (e.value != '') {
                        alert('{rval MSG901}'+'\n実技科目の場合、中間および調整点は入力できません。');
                        return false;
                    }
                //普通科目の調整点
                } else if (parseInt(jitugi_flg) < 1 && nam.match(/SCORE.901./)) {
                    if (e.id == '') {
                        alert('{rval MSG305}'+'\n基準点設定画面にて、基準点を設定してください。');
                        return false;
                    }
                    var adjust_value = parseInt(e.id);
                    if (score < 0 || score > adjust_value) {
                        alert('{rval MSG914}'+'0点～'+adjust_value+'点以内で入力してください。値：' + e.value);
                        return false;
                    }
                } else {
                    //満点チェック
        ...
        /*****************************************************************/
/***
        var value_flg = document.forms[0].VALUE_FLG.value;
        document.forms[0].VALUE_CAL_FLG.value = true;
        if (value_flg) {
            if (!confirm('学期評定を自動計算するかどうか？\n\n自動計算する場合は「ＯＫ」、しない場合は「キャンセル」を押して下さい。')) {
                document.forms[0].VALUE_CAL_FLG.value = false;
            } else {
                document.forms[0].VALUE_CAL_FLG.value = true;
            }
        }
***/
        if (cmd == 'update_on')  document.forms[0].VALUE_CAL_FLG.value = true;
        if (cmd == 'update_off') document.forms[0].VALUE_CAL_FLG.value = false;
        cmd = 'update';

        //更新中は、他の科目・講座を選択できないようにする（グレーアウト）。
        //一旦、hidden変数に科目・講座の値を保持する。
        document.forms[0].HID_SUBCLASSCD.value = document.forms[0].SUBCLASSCD.value;
        document.forms[0].HID_CHAIRCD.value = document.forms[0].CHAIRCD.value;
    ...
    document.forms[0].btn_update_on.disabled = true;
    document.forms[0].btn_update_off.disabled = true;
...
function calc(obj){
    ...
    //普通科目の調整点
    } else if (parseInt(jitugi_flg) < 1 && nam.match(/SCORE.901./) && obj.value != '') {
        if (obj.id == '') {
            alert('{rval MSG305}'+'\n基準点設定画面にて、基準点を設定してください。');
            return;
        }
        var adjust_value = parseInt(obj.id);
        if (score < 0 || score > adjust_value) {
            alert('{rval MSG914}'+'0点～'+adjust_value+'点以内で入力してください。');
            obj.value = obj.defaultValue;
            return;
        }
    //その他
...
function changeValue(obj) {
    ...
    document.forms[0].CHANGE_FLG.value = 'on';
}

function btnEnd() {
    if (document.forms[0].CHANGE_FLG.value == 'on') {
...
function showPaste(obj) {
    ...
    //テキストボックスの名前の配列を作る
    var nameArray = new Array("SCORE1111",
                              "SCORE1211",
                              "SCORE1901",
                              "VALUE1900",
                              "SCORE2111",
        ...
        for (k = 0; k < objectNameArray.length; k++) { //テキストボックス名でまわす
            ...
            if (startFlg) {
                if (clipTextArray[j][i] != undefined) {
                    ...
                    //実技科目の中間および調整点
                    if (0 < parseInt(jitugi_flg) && (objectNameArray[k].match(/SCORE.111/) || objectNameArray[k].match(/SCORE.901/))) {
                        if (valScore != '') {
                            alert('{rval MSG901}'+'\n実技科目の場合、中間および調整点は入力できません。');
                            return false;
                        }
                    //普通科目の調整点
                    } else if (parseInt(jitugi_flg) < 1 && objectNameArray[k].match(/SCORE.901/) && clipTextArray[j][i] != '' && targetObject) {
                        if (targetObject.id == '') {
                            alert('{rval MSG305}'+'\n基準点設定画面にて、基準点を設定してください。');
                            return false;
                        }
                        var adjust_value = parseInt(targetObject.id);
                        if (valScore < 0 || valScore > adjust_value) {
                            alert('{rval MSG914}'+'0点～'+adjust_value+'点以内で入力してください。');
                            return false;
                        }
                    //その他
