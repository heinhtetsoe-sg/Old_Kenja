    ...
    public function main(&$model)
    {
        ...
        if ($model->Properties["useProvFlg"] == '1') {
            $csvHeader = array("0" => "科目コード",
                               ...
                               "0131" => $semename[1]."－".$testname[10202]."－素点",
                               "0132" => $semename[1]."－".$testname[10202]."－評価",
                               "0182" => $semename[1]."評価",
        ...
        $admin_key = array();
        $result = $db->query(knjd121sQuery::selectContolCodeQuery($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($absent["ABSENT_COV"] == "0" || $absent["ABSENT_COV"] == "2" || $absent["ABSENT_COV"] == "4" || $absent["ABSENT_COV"] == "5") {
            ...
            $result = $db->query(knjd121sQuery::getAttendData2($model->field["CHAIRCD"], $model->field["SUBCLASSCD"], $absent["ABSENT_COV"], $absent["ABSENT_COV_LATE"], $offdaysFlg, $model));
        }
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
