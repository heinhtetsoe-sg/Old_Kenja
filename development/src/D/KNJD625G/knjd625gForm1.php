...
class knjd625gForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd625gForm1", "POST", "knjd625gindex.php", "", "knjd625gForm1");
        ...
        //学期コンボ作成
        $query = knjd625gQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd625g');\"";
        ...
        //対象学期の開始日・終了日
        $sDate = $db->getOne(knjd625gQuery::getSemesterDate("SDATE",$model->field["SEMESTER"]));
        $eDate = $db->getOne(knjd625gQuery::getSemesterDate("EDATE",$model->field["SEMESTER"]));
...
//        $extra = array("id=\"DISP1\" onClick=\"return btn_submit('knjd625g')\"", "id=\"DISP2\" onClick=\"return btn_submit('knjd625g')\"");
//        $radioArray = knjCreateRadio($objForm, "DISP", $model->field["DISP"], $extra, $opt_disp, get_count($opt_disp));
//        foreach($radioArray as $key => $val) $arg["data"][$key] = $val;

        //表示指定ラジオ 1:クラス固定
        $model->field["DISP"] = '1';
        knjCreateHidden($objForm, "DISP", $model->field["DISP"]);

        if ($model->field["DISP"] == 1) {
            //学年コンボ
            $query = knjd625gQuery::getGrade($model);
            $extra = "onChange=\"return btn_submit('knjd625g');\"";
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
        } else if ($model->field["DISP"] == 2) {
            //クラスコンボ作成
            $query = knjd625gQuery::getHrClass(CTRL_YEAR, $seme, $model);
            $extra = "onchange=\"return btn_submit('knjd625g');\"";
            makeCmb($objForm, $arg, $db, $query, "GRADE_HR_CLASS", $model->field["GRADE_HR_CLASS"], $extra, 1);
        }

//        //テストコンボ作成
//        $query = knjd625gQuery::getTest($model->field["SEMESTER"]);
...
//        //異動対象日付（テキスト）
//        $disabled = "";
//        $extra = "onblur=\"isDate(this); tmp_list('knjd625g', 'on')\"".$disabled;
//        $date_textbox = knjCreateTextBox($objForm, $model->field["DATE"], "DATE", 12, 12, $extra);
//        //異動対象日付（カレンダー）
//        global $sess;
//        $extra = "onclick=\"tmp_list('knjd625g', 'off'); loadwindow('" .REQUESTROOT ."/common/calendar.php?name=DATE&frame='+getFrameName(self) + '&date=' + document.forms[0]['DATE'].value + '&CAL_SESSID=$sess->id&reload=true', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 200)\"";
...
//        $arg["data"]["DATE"] = View::setIframeJs().$date_textbox.$date_button;
        ...
        View::toHTML($model, "knjd625gForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model, $seme) {
    ...
    //対象外の生徒取得
    $query = knjd625gQuery::getSchnoIdou($model, $seme);
    ...
    //対象者リストを作成する
    if ($model->field["DISP"] == 2) {
        $query = knjd625gQuery::getStudent($model, $seme);
        $result = $db->query($query);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD625G");
