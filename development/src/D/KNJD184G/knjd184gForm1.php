...
class knjd184gForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjd184gForm1", "POST", "knjd184gindex.php", "", "knjd184gForm1");
        ...
        //学期コンボ作成
        $query = knjd184gQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjd184g'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //クラスコンボ作成
        $query = knjd184gQuery::getGradeHrClass($model);
        $extra = "onchange=\"return btn_submit('knjd184g'), AllClearList();\"";
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd184g')\"".$disabled;
        ...
        View::toHTML($model, "knjd184gForm1.html", $arg); 
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    //対象外の生徒取得
    $query = knjd184gQuery::getSchnoIdou($model);
    ...
    //対象者リストを作成する
    $query = knjd184gQuery::getSchno($model);
    $result = $db->query($query);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD184G");
