...
function newwin(SERVLET_URL){
    ...
    var sdate, edate;
    if (document.forms[0].ADDRESS.checked) {
        if (document.forms[0].PRINT_DATE.value != '') {
            sdate = document.forms[0].YEAR_SDATE.value;
            edate = document.forms[0].YEAR_EDATE.value;
            if (edate < document.forms[0].PRINT_DATE.value) {
                alert("印刷日付が年度の範囲外です。");
                return;
            } else if (document.forms[0].PRINT_DATE.value < sdate) {
                alert("印刷日付が年度の範囲外です。");
                return;
            }
        }
