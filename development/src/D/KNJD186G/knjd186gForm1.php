...
class knjd186gForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjd186gForm1", "POST", "knjd186gindex.php", "", "knjd186gForm1");
        ...
        //学期コンボ作成
        $query = knjd186gQuery::getSemester($getCountsemester);
        $extra = "onchange=\"return btn_submit('knjd186g');\"";
        ...
        //クラスコンボ作成
        $query = knjd186gQuery::getHrClass(CTRL_YEAR, $seme, $model);
        $extra = "onchange=\"return btn_submit('knjd186g');\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE_HR_CLASS", $model->field["GRADE_HR_CLASS"], $extra, 1);

        //テストコンボ作成
        $query = knjd186gQuery::getTestcd($model);
        $extra = "";
        ...
        //異動対象日付（テキスト）
        $disabled = "";
        $extra = "onblur=\"isDate(this); tmp_list('knjd186g', 'on')\"".$disabled;
        $date_textbox = knjCreateTextBox($objForm, $model->field["DATE"], "DATE", 12, 12, $extra);
        //異動対象日付（カレンダー）
        global $sess;
        $extra = "onclick=\"tmp_list('knjd186g', 'off'); loadwindow('" .REQUESTROOT ."/common/calendar.php?name=DATE&frame='+getFrameName(self) + '&date=' + document.forms[0]['DATE'].value + '&CAL_SESSID=$sess->id&reload=true', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 200)\"";
        ...
        $dates = $db->getRow(knjd186gQuery::getYearDate(), DB_FETCHMODE_ASSOC);
        
        knjCreateHidden($objForm, "YEAR_SDATE", str_replace('-', '/', $dates["SDATE"]));
        knjCreateHidden($objForm, "YEAR_EDATE", str_replace('-', '/', $dates["EDATE"]));
        ...
        //保護者住所チェックボックスを作成
        $extra = $model->field["ADDRESS"] == "1" ? "checked" : "";
        $extra .= " id=\"ADDRESS\" ";
        $arg["data"]["ADDRESS"] = knjCreateCheckBox($objForm, "ADDRESS", "1", $extra, "");

        //欠点テキストボックス
        if ($model->field["KETTEN"] == '') $model->field["KETTEN"] = "4";
        ...
        View::toHTML($model, "knjd186gForm1.html", $arg);
    }
}

function makeStudentList(&$objForm, &$arg, $db, $model, $seme) {
    //対象外の生徒取得
    $query = knjd186gQuery::getSchnoIdou($model, $seme);
    ...
    //対象者リストを作成する
    $query = knjd186gQuery::getStudent($model, $seme);
    $result = $db->query($query);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJD186G");
