...
class knjd231mForm1 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjd231mindex.php", "", "edit");
        $db = Query::dbCheckOut();

        //警告メッセージを表示しない場合
        $query = knjd231mQuery::getSpecialReasonDiv($model->qualified_cd);
        ...
        /******************/
        //特殊事情
        $query = knjd231mQuery::getSpecialReasonDiv();
        $opt = array();
        $value_flg = false;
        ...
        //学年クラス
        $query = knjd231mQuery::getHrclass();
        $opt = array();
        $value_flg = false;
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        /* リスト */
        /**********/
        $query = knjd231mQuery::getList($model);
        $result = $db->query($query);
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "parent.left_frame.location.href='knjd231mindex.php?cmd=list';";
        }
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjd231mForm1.html", $arg);
    }
}
?>
