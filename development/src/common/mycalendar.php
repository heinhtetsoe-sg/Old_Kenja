 ...
 */
 ...
 //GYY/MM/DDを西暦YYYY-MM-DDに変換
   ...
   function ChgWToJ_KANJI($reqday){
    ...
    function MyMonthWin(&$form, $name, $value="", $extra = "")
    // Add by PP PC-Talker 2020-02-20 end
    {
...
//        $img .= "onClick=\"monthWin('" .REQUESTROOT ."/common/mymonth.html', '$name', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}());\">";
        // Add by PP for PC-Talker 2020-02-03 start
        $img .= "id=\"$name\" $extra onclick=\"loadwindow('" .REQUESTROOT ."/common/monthcalendar.php?name=$name&frame='+getFrameName(self) + '&date=' + document.forms[0]['$name'].value + '&CAL_SESSID=$sess->id&', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 200, 150)\">";
		// Add by PP PC-Talker 2020-02-20 end
...
//---20090824 kurata-----------------------------------------------------------------//
   ...
   if($value != "")
   {
     ...
     //②後半部
       // Add by PP for PC-Talker cursor 2020-02-03 start
        $command = "onFocus = \"this.blur();\" tabindex=\"-1\""; 
        // Add by PP for PC-Talker cursor 2020-02-20 end 
       //textフィールド部
        $form->ae( array("type"        => "text",
                         "name"        => $name,
                         "size"        => 12,
       ...
       //読込ボタン部
        ...
        $img = "\n<A  HREF=\"#\" ";
//         $img .= "onClick=\"monthWin('" .REQUESTROOT ."/common/mymonth.html', '$name', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}());  ";
        $img .= "id=\"$name\" aria-label=\"カレンダー\" onclick=\"loadwindow('" .REQUESTROOT ."/common/monthcalendar.php?name=$name&frame='+getFrameName(self) + '&date=' + document.forms[0]['$name'].value + '&extra=".$extra."&CAL_SESSID=$sess->id&', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 200, 150); ";
...
//20100519
    ...
    function MyCalendarWin3(&$form, $name, $value="", $extra = "", $reload = "", $extraClick = "")
    {
        ...
        $era = "（";
        $this->getWareki();
        for($nengo=0; $nengo<get_count($this->def_nengo); $nengo++){
            if ($nengo != 0){
                $era .= "　";
            }
            $era .= $this->def_nengo[$nengo]."：".$this->nengoID2[$nengo];
        }
        $era .= "）";

        $span1 = "<span nowrap onMouseOver=\"AcceptnoMousein(event, '例）昭和50年10月10日　⇒　3501010　<br><br>例）平成5年7月8日　　　⇒　4050708　<br><br>$era')\" onMouseOut=\"AcceptnoMouseout()\">";
        $span2 = "</span>";
...
//月コントロール作成
    function MyMonthWin3(&$form, $name, $value="", $extra="", $reload = "")
    {
        ...
        $span1 = "<span nowrap onMouseOver=\"AcceptnoMousein(event, '例）昭和50年10月　⇒　35010　<br><br>例）平成5年7月　　　⇒　40507　<br><br>$era')\" onMouseOut=\"AcceptnoMouseout()\">";
        $span2 = "</span>";
...
//////////////////////////////////////////////////////////////////////
   //西暦YYYY-MM-DDをNYYMMDDに変換
    function ChgWToN($reqday){
        ...
        switch($patrn){
        case 1:
            return sprintf("%d%02d", $this->nengoID2[$gen-1]
                      ,$number[0]-$this->nengoYear[$gen-1]+1);
        case 2:
            return sprintf("%d%02d%02d", $this->nengoID2[$gen-1]
                      ,$number[0]-$this->nengoYear[$gen-1]+1, $number[1]);
        default:
            return sprintf("%d%02d%02d%02d", $this->nengoID2[$gen-1]
                      ,$number[0]-$this->nengoYear[$gen-1]+1, $number[1], $number[2]);
        }
    }
