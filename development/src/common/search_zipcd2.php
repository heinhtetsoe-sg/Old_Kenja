...
class SearchZipcd
{
    ...
    function getZipList(){
        ...
        if ($this->zip || $this->addr){
            ...
            $query .= "    NEW_ZIPCD, ";
            $query .= "    OLD_ZIPCD, ";
            $query .= "    (SELECT PREF_CD FROM PREF_MST WHERE PREF_NAME = PREF) AS PREF_CD, ";
            $query .= "    PREF, ";
            $query .= "    CITY, ";
            $query .= "    CITY_KANA, ";
            $query .= "    TOWN, ";
            ...
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
                $opt[] = array("label" => sprintf("%s　|　%s　|　%s", $row["NEW_ZIPCD"], $row["PREF"] .$row["CITY"] .$row["TOWN"], $row["PREF_CD"]),
                                "value" => $row["NEW_ZIPCD"]
                                );
            }
            Query::dbCheckIn($db);    
        ...
        $this->form->ae( array("type"       => "select",
                            ...
                            "size"       => "10",
                            "value"      => "",
                            "extrahtml"   => "onclick=\"setAddrText(this);\" ondblclick=\"apply_address(this, '$this->addrname', '$this->zipname', '$this->prefname', '$this->prefname2');\" STYLE=\"WIDTH:100%\" WIDTH=\"100%\"",
    ...
    function main($rq){
        //テキストボックスの名前
        if ($rq["prefname"]){
            $this->prefname   = $rq["prefname"];
        }
        if ($rq["prefname2"]){
            $this->prefname2  = $rq["prefname2"];
        }
        if ($rq["addrname"]){
    ...
    function getBtnApply(){
        ...
        $this->form->ae( array("type" => "button",
                            "name"        => "apply",
                            "value"       => "反 映",
                            "extrahtml"   => "onClick=\"apply_address(document.forms[0].ZIPLIST, '$this->addrname', '$this->zipname', '$this->prefname', '$this->prefname2');\"" ) );

        $button["apply"] = $this->form->ge("apply");
    }
    //フォームタブ(開始)
    function getStart(){
        return $this->form->get_start("zip", "POST", "search_zipcd2.php", "", "zip");
    }
    //フォームタブ(終了)
    function getFinish(){
    ...
    function getPrefName2(){
        return $this->prefname2;
    }
}
if ($_GET["cmd"] == "apply"){
...
function apply_address(obj, addrname, zipname, prefname, prefname2) {
    if(obj.selectedIndex >= 0){
        var val = obj.options[obj.selectedIndex].text;    
        var arr = val.split("　|　");
        f.document.forms[0][zipname].value = arr[0];
        f.document.forms[0][addrname].value = arr[1];
        f.document.forms[0][prefname].value = arr[2];
        f.document.forms[0][prefname2].value = arr[2];
        f.closeit();
    }else{
        alert("住所が選択されていません");
    }
...
<script language="JavaScript">
    if (document.forms[0].ZIPLIST.length == 1){
        ...
        f.document.forms[0]['<?php echo $objZip->getPrefName() ?>'].value = arr[2];
        f.document.forms[0]['<?php echo $objZip->getPrefName2() ?>'].value = arr[2];
