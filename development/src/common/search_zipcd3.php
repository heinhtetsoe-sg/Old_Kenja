...
class SearchZipcd
{
    ...
    function getZipList(){
        ...
        if ($this->zip || $this->addr){
            ...
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
                $opt[] = array("label" => sprintf("%s　|　%s　|　%s", $row["NEW_ZIPCD"], $row["PREF"] .$row["CITY"] .$row["TOWN"], $row["CITYCD"]),
                                "value" => $row["NEW_ZIPCD"]
                                );
            }
            Query::dbCheckIn($db);    
        ...
        $this->form->ae( array("type"       => "select",
                            ...
                            "size"       => "10",
                            "value"      => "",
                            "extrahtml"   => "onclick=\"setAddrText(this);\" ondblclick=\"apply_address(this, '$this->addrname', '$this->zipname', '$this->cityname');\" STYLE=\"WIDTH:100%\" WIDTH=\"100%\"",
    ...
    function getBtnApply(){
        ...
        $this->form->ae( array("type" => "button",
                            "name"        => "apply",
                            "value"       => "反 映",
                            "extrahtml"   => "onClick=\"apply_address(document.forms[0].ZIPLIST, '$this->addrname', '$this->zipname', '$this->cityname');\"" ) );

        $button["apply"] = $this->form->ge("apply");
    }
    //フォームタブ(開始)
    function getStart(){
        return $this->form->get_start("zip", "POST", "search_zipcd3.php", "", "zip");
    }
    //フォームタブ(終了)
    function getFinish(){
...
function apply_address(obj, addrname, zipname, cityname) {
    if(obj.selectedIndex >= 0){
        var val = obj.options[obj.selectedIndex].text;    
        var arr = val.split("　|　");
        f.document.forms[0][zipname].value = arr[0];
        f.document.forms[0][addrname].value = arr[1];
        f.document.forms[0][cityname].value = arr[2];
        f.closeit();
    }else{
        alert("住所が選択されていません");
    }
...
<script language="JavaScript">
    if (document.forms[0].ZIPLIST.length == 1){
        ...
        f.document.forms[0]['<?php echo $objZip->getCityName() ?>'].value = arr[2];
