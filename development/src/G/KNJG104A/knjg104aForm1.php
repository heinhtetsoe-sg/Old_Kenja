...
class knjg104aForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjg104aForm1", "POST", "knjg104aindex.php", "", "knjg104aForm1");
        ...
        //学期コンボ
        $query = knjg104aQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjg104a'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //出力指定選択 (1:クラス指定 2:個人指定)
        $opt = array(1, 2);
        $model->field["CHOICE"] = ($model->field["CHOICE"] == "") ? "1" : $model->field["CHOICE"];
        $click = " onClick=\"return btn_submit('knjg104a');\"";
        ...
        if ($model->field["CHOICE"] == "2") {
            ...
            //年組コンボ作成
            $query = knjg104aQuery::getHrClass($model);
            $extra = "onchange=\"return btn_submit('knjg104a'), AllClearList();\"";
        ...
        View::toHTML($model, "knjg104aForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["CHOICE"] == "1") {
        ...
        $query = knjg104aQuery::getStudent($model);
    }
    ...
    //リストの項目名
    $arg["data"]["LIST_LABEL"] = ($model->field["CHOICE"] == "1") ? 'クラス' : '生徒';
    //全体サイズ
