...
class knjg103Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjg103Form1", "POST", "knjg103index.php", "", "knjg103Form1");
        ...
        //学期コンボ
        $query = knjg103Query::getSemester();
        $extra = "onchange=\"return btn_submit('knjg103'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //出力指定選択 (1:クラス指定 2:個人指定)
        $opt = array(1, 2);
        $model->field["CHOICE"] = ($model->field["CHOICE"] == "") ? "1" : $model->field["CHOICE"];
        $click = " onClick=\"return btn_submit('knjg103');\"";
        ...
        if ($model->field["CHOICE"] == "2") {
            ...
            //年組コンボ作成
            $query = knjg103Query::getHrClass($model);
            $extra = "onchange=\"return btn_submit('knjg103'), AllClearList();\"";
        ...
        View::toHTML($model, "knjg103Form1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["CHOICE"] == "1") {
        ...
        $query = knjg103Query::getStudent($model);
    }
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "FORM", "3");
