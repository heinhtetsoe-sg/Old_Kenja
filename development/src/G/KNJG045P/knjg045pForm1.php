...
class knjg045pForm1
{
    ...
    {
        $objForm        = new form;
        $arg["start"]   = $objForm->get_start("main", "POST", "knjg045pindex.php", "", "main");
        ...
        if (!isset($model->warning) && $model->cmd != "read1" && $model->cmd != "read2" && $model->cmd != "lesson") {
            $query = knjg045pQuery::getDiary($model);
        ...
        //天気
        $query = knjg045pQuery::getWeather($model);
        $extra = "";
        makeCmb($objForm, $arg, $db, $query, $diaryData["WEATHER"], "WEATHER", $extra, 1, "BLANK");

        //学年タイトル
        $query = knjg045pQuery::getGrade($model);
        ...
        View::toHTML($model, "knjg045pForm1.html", $arg); 
    }
}
//makeCmb
...
{
    $query = knjg045pQuery::getPermRequest($model, $cd);
    $result = $db->query($query);
    $setData = "";
    $sep = "";
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
{
    $query = knjg045pQuery::getSemester($model);
    $result = $db->query($query);
    $befSdate = "";
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function getLessonData($db, $model, $grade)
{
    $attendLesson = 0;
    $eventLesson = 0;
    $query = knjg045pQuery::getMaxDate($model);
    ...
    $query = knjg045pQuery::getAttendLesson($model, $monthIn, $grade);
    $attendLesson = $db->getOne($query);

    if ($dateArray[2] != $lastDay) {
        $fDay = $dateArray[0]."-".$dateArray[1]."-01";
        $eDay = $dateArray[0]."-".$dateArray[1]."-".$dateArray[2];
        $query = knjg045pQuery::getHoliCnt($model, $grade, $fDay, $eDay);
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJG045P");
