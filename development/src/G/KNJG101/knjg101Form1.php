...
class knjg101Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjg101Form1", "POST", "knjg101index.php", "", "knjg101Form1");
        ...
        //学期コンボ
        $query = knjg101Query::getSemester();
        $extra = "onchange=\"return btn_submit('knjg101'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //出力指定選択 (1:クラス指定 2:個人指定)
        $opt = array(1, 2);
        $model->field["CHOICE"] = ($model->field["CHOICE"] == "") ? "1" : $model->field["CHOICE"];
        $click = " onClick=\"return btn_submit('knjg101');\"";
        ...
        if ($model->field["CHOICE"] == "2") {
            ...
            //年組コンボ作成
            $query = knjg101Query::getHrClass($model);
            $extra = "onchange=\"return btn_submit('knjg101'), AllClearList();\"";
            makeCmb($objForm, $arg, $db, $query, "GRADE_HR_CLASS", $model->field["GRADE_HR_CLASS"], $extra, 1);
        }

        $extra = ($model->field["NO_PRINT_FUKU"] == "1") ? "checked" : "";
        $extra .= " id=\"NO_PRINT_FUKU\"";
        $arg["data"]["NO_PRINT_FUKU"] = knjCreateCheckBox($objForm, "NO_PRINT_FUKU", "1", $extra, "");
        ...
        View::toHTML($model, "knjg101Form1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["CHOICE"] == "1") {
        ...
        $query = knjg101Query::getStudent($model);
    }
    ...
    //校種取得
    $model->schKind = $db->getOne(knjg101Query::getSchoolKind($model));

    //生徒項目名切替処理
    $sch_label = "";
    if ($model->field["CHOICE"] == "2") {
        //テーブルの有無チェック
        $query = knjg101Query::checkTableExist();
        $table_cnt = $db->getOne($query);
        if ($table_cnt > 0 && ($model->field["GRADE_HR_CLASS"] || ($model->Properties["useSchool_KindField"] == "1" && SCHOOLKIND != ""))) {
            //生徒項目名取得
            $sch_label = $db->getOne(knjg101Query::getSchName($model));
        }
    }
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "FORM", "1");
