...
class knjg106Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjg106Form1", "POST", "knjg106index.php", "", "knjg106Form1");
        ...
        //学期コンボ
        $query = knjg106Query::getSemester();
        $extra = "onchange=\"return btn_submit('knjg106'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //出力指定選択 (1:クラス指定 2:個人指定)
        $opt = array(1, 2);
        $model->field["CHOICE"] = ($model->field["CHOICE"] == "") ? "1" : $model->field["CHOICE"];
        $click = " onClick=\"return btn_submit('knjg106');\"";
        ...
        if ($model->field["CHOICE"] == "2") {
            ...
            //年組コンボ作成
            $query = knjg106Query::getHrClass($model);
            $extra = "onchange=\"return btn_submit('knjg106'), AllClearList();\"";
        ...
        View::toHTML($model, "knjg106Form1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["CHOICE"] == "1") {
        ...
        $query = knjg106Query::getStudent($model);
    }
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJG106");
