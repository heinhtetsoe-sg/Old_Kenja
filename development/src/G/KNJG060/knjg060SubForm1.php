...
class knjg060SubForm1
{
    function main(&$model)
    {
        ...
        $arg = array();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjg060SubForm1", "POST", "knjg060index.php", "", "knjg060SubForm1");
        ...
        $arg["NAME"] = $db->getOne(knjg060Query::getName($model->subSchregno));

        //通学データ
        $query = knjg060Query::getSchreg_envir_dat($model);
        ...
        for ($tugakuCnt = 1; $tugakuCnt <= 7; $tugakuCnt++) {
            ...
            if ($studentTugaku["FLG_".$tugakuCnt] == 2) {
                ...
                $query = knjg060Query::getStationName($studentTugaku["JOSYA_".$tugakuCnt]);
                list($josya_mei, $rosen_mei) = $db->getRow($query);
                $query = knjg060Query::getStationName($studentTugaku["GESYA_".$tugakuCnt]);
        ...
        View::toHTML($model, "knjg060SubForm1.html", $arg);
    }
}
