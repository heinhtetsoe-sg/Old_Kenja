...
class knjl320uForm1 {
    function main(&$model) {
        $objForm = new form;
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl320uForm1", "POST", "knjl320uindex.php", "", "knjl320uForm1");
        ...
        //入試制度コンボ
        $query = knjl320uQuery::getNameMst($model->examyear, "L003");
        $extra = "onChange=\"return btn_submit('knjl320u')\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["APPLICANTDIV"], "APPLICANTDIV", $extra, 1, "TOP", "");

        //入試区分
        $query = knjl320uQuery::getNameMst($model->examyear, "L004");
        $extra = "onChange=\"return btn_submit('knjl320u')\"";
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL320U");
        ...
        View::toHTML($model, "knjl320uForm1.html", $arg);
    }
}
//makeCmb
