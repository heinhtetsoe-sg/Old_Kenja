...
class knjl036fForm2 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjl036findex.php", "", "edit");
        ...
        if (!isset($model->warning) && ($model->cmd == "edit2" || $model->cmd == "reset") && $model->year && $model->applicantdiv && $model->recno) {
            $query = knjl036fQuery::getRow($model->year, $model->applicantdiv, $model->recno);
        ...
        //中学・高校の表示切替
        $arg["isShowH"] = ($model->applicantdiv == "2") ? "1" : "";
        $arg["isShowJ"] = ($model->applicantdiv == "2") ? "" : "1";
        ...
        if (!isset($model->warning) && $model->cmd != "edit") {
            $arg["reload"]  = "parent.left_frame.location.href='knjl036findex.php?cmd=list"
                            . "&year=" .$model->year."&APPLICANTDIV=" .$model->applicantdiv."';";
        }
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl036fForm2.html", $arg);
    }
}
