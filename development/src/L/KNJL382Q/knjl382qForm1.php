...
class knjl382qForm1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjl382qindex.php", "", "edit");
        ...
        //コピー用年度コンボ
        $yearQuery = knjl382qQuery::getCopyYear();
        ...
        //県名コンボ
        $prefQuery = knjl382qQuery::getPrefcd();
        ...
        while($prefRow = $prefResult->fetchRow(DB_FETCHMODE_ASSOC)){
            $opt[] = array("value"  =>  $prefRow["GROUPPREF"],
                           "label"  =>  $prefRow["GROUPPREF"]."：".$prefRow["PREF_NAME"]);
            $exist[] = $prefRow["GROUPPREF"];
        }
        $extra = " onchange=\"btn_submit('change');\"";
        if(!in_array($model->left_field["SEARCH_PREF"], $exist)){
            $model->left_field["SEARCH_PREF"] = "";
        }
        
        $arg["SEARCH_PREF"] = knjCreateCombo($objForm, "SEARCH_PREF", $model->left_field["SEARCH_PREF"], $opt, $extra, 1);
        ...
        if ($model->cmd == "changeMockyear") {
            $arg["reload"] = "window.open('knjl382qindex.php?cmd=edit', 'right_frame')";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。 
        View::toHTML($model, "knjl382qForm1.html", $arg); 
    }
}
...
{
    $result = $db->query(knjl382qQuery::getList($model));
