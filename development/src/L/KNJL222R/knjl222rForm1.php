...
class knjl222rForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl222rForm1", "POST", "knjl222rindex.php", "", "knjl222rForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl222rQuery::getNameMst($model, $model->ObjYear, "L003"));
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl222rQuery::getNameMst($model, $model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL222R");
        ...
        View::toHTML($model, "knjl222rForm1.html", $arg); 
