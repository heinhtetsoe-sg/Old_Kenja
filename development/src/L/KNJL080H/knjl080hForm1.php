...
class knjl080hForm1
{
    function main(&$model)
    {
        ...
        //入試制度
        $opt = array();
        $result = $db->query(knjl080hQuery::GetName("L003",$model->ObjYear));
        ...
        $result = $db->query(knjl080hQuery::GetLeftList($model));

        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            $opt_left[] = array("label" => $row["EXAMNO"]."：".$row["NAME"], "value" => $row["EXAMNO"]);
            $tmp_id[]   = $row["EXAMNO"];
        }
        ...
        //合格者一覧
        $opt_right = array();
        $result = $db->query(knjl080hQuery::GetRightList($model));

        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            if (!in_array($row["EXAMNO"], $tmp_id)) {
                $opt_right[]    = array("label" => $row["EXAMNO"]."：".$row["NAME"], "value" => $row["EXAMNO"]);
            }
        }
        ...
        $arg["start"]   = $objForm->get_start("sel", "POST", "knjl080hindex.php", "", "sel");

        $arg["finish"]  = $objForm->get_finish();

        View::toHTML($model, "knjl080hForm1.html", $arg); 
    }
}
?>
