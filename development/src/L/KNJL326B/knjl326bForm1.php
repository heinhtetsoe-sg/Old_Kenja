...
class knjl326bForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl326bForm1", "POST", "knjl326bindex.php", "", "knjl326bForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl326bQuery::getNameMst($model->ObjYear, "L003"));
        ...
        $extra = "onchange=\"return btn_submit('knjl326b');\"";
        $arg["data"]["APPLICANTDIV"] = knjCreateCombo($objForm, "APPLICANTDIV", $model->field["APPLICANTDIV"], $opt, $extra, 1);

        //入試区分
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl326bQuery::getNameMst($model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        $click = " onclick =\" return btn_submit('knjl326b');\"";
        ...
        View::toHTML($model, "knjl326bForm1.html", $arg);
    }
}
...
{
    knjCreateHidden($objForm, "PRGID", "KNJL326B");
