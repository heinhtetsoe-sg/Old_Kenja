...
class knjl314Form1
{
	...
	$objForm = new form;
	//フォーム作成
	$arg["start"]   = $objForm->get_start("knjl314Form1", "POST", "knjl314index.php", "", "knjl314Form1");
	...
	$opt_apdv_typ = array();
	$db = Query::dbCheckOut();
	$result = $db->query(knjl314Query::get_apct_div("L003",$model->ObjYear));	/* NO001 */
	...
	$objForm->ae( array("type"       => "select",
						...
						"extrahtml"  => " onChange=\"return btn_submit('knjl314');\"",
	...
	$defoult     = 1 ;
	$db = Query::dbCheckOut();
	$result = $db->query(knjl314Query::get_test_div($model->ObjYear));	/* NO001 */
	...
	//指定範囲テキスト作成(２科目合計未満)		/* NO002 */
	$objForm->ae( array("type"       => "hidden",
    	                "name"       => "TESTSCR",
                        "size"       => 3,
                        "maxlength"  => 3,
						"extrahtml"  => "STYLE=\"text-align: right\" onblur=\"this.value=toInteger(this.value)\"",
						"value"      => isset($model->field["TESTSCR"])?$model->field["TESTSCR"]:"201"));
	...
	View::toHTML($model, "knjl314Form1.html", $arg); 
	}
}
?>
