...
function doSubmit() {
    ...
    if (document.forms[0].SPECIALS.length > 0) {
        for (var i = 0; i < document.forms[0].SPECIALS.length; i++) {
            specials_value = document.forms[0].SPECIALS.options[i].value;
            if (specials_value.match(/_/)) {
                specials_array = document.forms[0].SPECIALS.options[i].value.split("_");
                specials_value = specials_array[1];
            }
            attribute3.value = attribute3.value + sep + specials_value;
            sep = ",";
        }
    }
    ...
    if (document.forms[0].APPROVED.length > 0) {
        for (var i = 0; i < document.forms[0].APPROVED.length; i++) {
            approved_value = document.forms[0].APPROVED.options[i].value;
            if (approved_value.match(/_/)) {
                approved_array = document.forms[0].APPROVED.options[i].value.split("_");
                approved_value = approved_array[1];
            }
            attribute4.value = attribute4.value + sep + approved_value;
            sep = ",";
        }
    }
...
function move2(side, left, right, sort){
            ...
            else
 {
    ...
    if (sort || side == "right" || side == "sel_del_all") {
        //sort
        temp1 = temp1.sort();
