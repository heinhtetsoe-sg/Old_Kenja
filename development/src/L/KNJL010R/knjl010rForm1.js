function btn_submit(cmd, zip, gzip, zadd, gadd) {
    ...
    if (cmd == 'addnew') {
        ...
        for (i = 1; i <= 4; i++) {
            if (document.forms[0]["TESTDIV"+i].checked) {
                addnewChk = true;
            }
        }
        if (!addnewChk) {
...
//新規ボタン押し下げ後、氏名欄にカーソルを移動する
function moveCursor() {
    document.forms[0].NAME.focus();
}
//欠席日数計算
function sumAbsense() {
    var ab5 = document.forms[0].ABSENCE_5.value || 0;
    var ab6 = document.forms[0].ABSENCE_6.value || 0;

    document.getElementById('ABSENCE_TOTAL').innerHTML = parseInt(ab5) + parseInt(ab6);
}
