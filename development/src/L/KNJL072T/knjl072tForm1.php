...
class knjl072tForm1
{
    function main(&$model) {
        ...
        //入試制度
        $opt = array();
        $result = $db->query(knjl072tQuery::GetName("L003",$model->ObjYear));
        ...
        $opt = array();
        $value_flg = false;
        $query = knjl072tQuery::getMajorcd();
        ...
        //募集枠数
        $arg["data"]["CAPACITY"] = 0;
        $result = $db->query(knjl072tQuery::getCapacity($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $result = $db->query(knjl072tQuery::GetLeftList($model));
        ...
        //候補者一覧
        $opt_right = array();
        $result = $db->query(knjl072tQuery::GetRightList($model));
        ...
        $arg["start"]   = $objForm->get_start("sel", "POST", "knjl072tindex.php", "", "sel");

        $arg["finish"]  = $objForm->get_finish();

        View::toHTML($model, "knjl072tForm1.html", $arg);
    }
}
