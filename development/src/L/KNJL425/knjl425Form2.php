...
class knjl425Form2 {
    function main(&$model) {
        $objForm = new form;
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjl425index.php", "", "edit");
        ...
        if ($model->cmd == "reset" || $model->cmd == "linkClick") {
            $query = knjl425Query::getRecruitSendYmst($model);
        ...
        if (VARS::get("cmd") != "edit" && VARS::get("cmd") != "linkClick"){
            $arg["reload"]  = "parent.left_frame.location.href='knjl425index.php?cmd=list';";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl425Form2.html", $arg); 
    }
}
?>
