...
class knjl033gForm2 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjl033gindex.php", "", "edit");
        ...
        if (!isset($model->warning) && ($model->cmd == "edit2") && $model->year && $model->applicantdiv && $model->item_cd) {
            $query = knjl033gQuery::getRow($model->year, $model->applicantdiv, $model->item_cd);
        ...
        //入試制度コンボ
        $extra = "";
        $query = knjl033gQuery::getApplicantdiv($model->year);
        ...
        //略称名テキストボックス
        $arg["data"]["ITEM_ABBV"] = knjCreateTextBox($objForm, $Row["REMARK6"], "ITEM_ABBV", 10, 10, "");
        ...
        $arg["data"]["REMARK7"] = knjCreateCheckBox($objForm, "REMARK7", "1", $extra);
        ...
        if (!isset($model->warning) && $model->cmd != "edit"){
            $arg["reload"]  = "parent.left_frame.location.href='knjl033gindex.php?cmd=list"
                            . "&year=" .$model->year."';";
        }
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl033gForm2.html", $arg);
    }
}
