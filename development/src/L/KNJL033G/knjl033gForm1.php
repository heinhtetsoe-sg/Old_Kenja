...
class knjl033gForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjl033gindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //年度設定
        $result = $db->query(knjl033gQuery::selectYearQuery());
        $opt = array();
        //レコードが存在しなければ処理年度を登録
        if ($result->numRows() == 0) { 
        ...
        if (!$flg) {
            if (!isset($model->year)) {
                ...
                $model->year = $db->getOne(knjl033gQuery::DeleteAtExist($model));
            }
            $arg["reload"][] = "parent.right_frame.location.href='knjl033gindex.php?cmd=edit"
                             . "&year=" .$model->year."';";
        }
        ...
        //課程学科件数取得
        $a_cnt = array();
        $result = $db->query(knjl033gQuery::getDataCnt($model->year));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //テーブルの中身の作成
        $a_bifKey = "";
        $query = knjl033gQuery::selectQuery($model->year);
        ...
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            $row["ITEM_CD"]         = View::alink("knjl033gindex.php", $row["ITEM_CD"], "target=\"right_frame\"", $hash);
        ...
        View::toHTML($model, "knjl033gForm1.html", $arg);
    }
}
?>
