...
class knjl073bForm1
{
    function main(&$model) {
        ...
        $query = knjl073bQuery::getNameMst("L003", $model->ObjYear);
        makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->applicantdiv, $extra, 1);

        //入試区分コンボボックス
        $extra = "onchange=\"return btn_submit('main');\" tabindex=-1";
        $query = knjl073bQuery::getNameMst("L004", $model->ObjYear);
        ...
        //抽出合否区分コンボ
        $query = knjl073bQuery::getJudgmentDiv($model);
        $extra = "";
        makeCmb($objForm, $arg, $db, $query, "JUDGMENT_DIV_SEARCH", $model->judgment_div_search, $extra, 1, "BLANK");

        //合否詳細区分マスタ
        $query = knjl073bQuery::getJudgmentDiv($model, $model->judgment_div_search);
        $jdgRow = $db->getRow($query, DB_FETCHMODE_ASSOC);

        //抽出合否区分から変更合否区分を制限
        $judgment_div_controll = array("01" => "13"
                                      ,"02" => "14"
                                      ,"03" => "15"
                                      ,"13" => "01"
                                      ,"14" => "02"
        ...
        //変更合否区分コンボ
        $query = knjl073bQuery::getJudgmentDiv($model, $judgment_div_controll[$model->judgment_div_search]);
        $extra = "";
        ...
        if ($model->applicantdiv != "" && $model->testdiv != "" && ($model->judgment_div_search != "" || $model->avg_from != "" || $model->avg_to != ""))
        {
            ...
            //データ取得
            $result = $db->query(knjl073bQuery::simSql($model, $jdgRow));
            ...
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
            {
                ...
                //一般入試の確約区分が「01:学特合」の生徒は、
                //「01:特進コース学業特待（特Ａ）」で抽出した時、
                //更新チェックボックスにチェックが入っている状態で表示する。
                $extra .= ($row["PROMISE_DIV"] == "01" && $model->judgment_div_search == "01") ? " checked" : "";
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjl073bindex.php", "", "main");

        //フォーム終わり
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl073bForm1.html", $arg);
    }
}
