...
class knjl780hForm1
{
    public function main(&$model)
    {
        ...
        //学校種別
        $query = knjl780hQuery::getNameMst($model->examYear, "L003");
        $extra = "onchange=\"return btn_submit('read');\"";
        makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->applicantdiv, $extra, 1, "");

        //入試種別
        $query = knjl780hQuery::getTestdiv($model);
        $extra = "onchange=\"return btn_submit('read');\"";
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjl780hindex.php", "", "main");

        //フォーム終わり
        $arg["finish"] = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl780hForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    ...
    $leftCnt = $rightCnt = 0;
    $query = knjl780hQuery::getBaseDatData($model);
