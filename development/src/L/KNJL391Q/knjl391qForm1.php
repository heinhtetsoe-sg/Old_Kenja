...
class knjl391qForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjl391qindex.php", "", "main");
        ...
        $query = knjl391qQuery::getExamDate();
        ...
        $extra = array("id=\"PLACE1\"".$option, "id=\"PLACE2\"".$option, "id=\"PLACE3\"".$option, "id=\"PLACE4\"".$option);
        $label = array("PLACE1" => "すべて", "PLACE2" => "指定", "PLACE3" => "国内", "PLACE4" => "海外");
        $radioArray = knjCreateRadio($objForm, "PLACE", $model->field["PLACE"], $extra, $opt, get_count($opt));
        $arg["PLACE"] = "";
        $sp = "";
        foreach ($radioArray as $key => $val) {
            if($key == "PLACE2"){
                $place = $sp.$val."<LABEL for=\"".$key."\">".$label[$key]."</LABEL>";
            }else{
        ...
        if ($model->field["PLACE"] == "2") {
            //試験会場のコンボボックス
            $query = knjl391qQuery::getExamPlace();
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL391Q");
        ...
        View::toHTML($model, "knjl391qForm1.html", $arg);
    }
