...
class SearchNames {
    ...
    function getNameList(){
        ...
        if (strlen($this->names) || strlen($this->name_kana)) {
            ...
            $query .= "ORDER BY VISIT_NO ";
            //受験番号、氏名、氏名かな取得
            $result = $db->query($query);
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
                $opt[] = array("label" => $row["VISIT_NO"]."　".$row["NAME"]."　(".$row["NAME_KANA"].")",
                               "value" => $row["VISIT_NO"]
                               );
            }
            Query::dbCheckIn($db);
        ...
        $this->form->ae( array("type"       => "select",
                               ...
                               "size"       => "11",
                               "value"      => "",
                               "extrahtml"   => " ondblclick=\"apply_name(this,'".$this->visitNo."','".$this->zip."','".$this->gzip."','".$this->zadd."','".$this->gadd."');\" STYLE=\"WIDTH:100%\"",
...
function apply_name(obj,visitNo,zip,gzip,zadd,gadd) {
    if (obj.selectedIndex >= 0) {
        var val = obj.options[obj.selectedIndex].text;    
        var arr = val.split("　");
        f.document.forms[0]['VISIT_NO'].value = arr[0];
        if(f.btn_submit('reference',zip,gzip,zadd,gadd) != true) {
            f.closeit();
        } else {
            f.document.forms[0]['VISIT_NO'].value = visitNo;
        }
    } else {
        alert("名前が選択されていません");
    }
