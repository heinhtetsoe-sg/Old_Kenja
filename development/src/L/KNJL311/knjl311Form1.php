...
class knjl311Form1
{
	...
	$objForm = new form;
	//フォーム作成
	$arg["start"]   = $objForm->get_start("knjl311Form1", "POST", "knjl311index.php", "", "knjl311Form1");
	...
	$opt_apdv_typ = array();
	$db = Query::dbCheckOut();
	$result = $db->query(knjl311Query::get_apct_div("L003",$model->ObjYear));	/* NO001 */
	...
	$objForm->ae( array("type"       => "select",
						...
						"extrahtml"  => " onChange=\"return btn_submit('knjl311');\"",
	...
	$defoult     = 0 ;
	$db = Query::dbCheckOut();
	$result = $db->query(knjl311Query::get_test_div($model->ObjYear));	/* NO001 */
	...
	$exam_type = $db->getOne(knjl311Query::get_exam_div("L005", $model->field["EXAM_TYPE"]));
	Query::dbCheckIn($db);
    knjCreateHidden($objForm, "EXAM_TYPE", $model->field["EXAM_TYPE"]);
	$arg["data"]["EXAM_TYPE"] = $exam_type;
	//試験会場選択コンボボックスを作成する
	$db = Query::dbCheckOut();
	$result = $db->query(knjl311Query::get_hall_data($model->field["TESTDV"],$model->field["EXAM_TYPE"]));
	...
	View::toHTML($model, "knjl311Form1.html", $arg); 
	}
}
?>
