...
class knjl602aForm2 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjl602aindex.php", "", "edit");
        ...
        if (!isset($model->warning) && ($model->cmd == "edit2" || $model->cmd == "reset") && $model->year && $model->applicantdiv && $model->testdiv && $model->totalcd && $model->testsubclasscd) {
            $query = knjl602aQuery::getRow($model->year, $model->applicantdiv, $model->testdiv, $model->totalcd, $model->testsubclasscd);
        ...
        /**************/
        //試験コースコンボ
        $query = knjl602aQuery::getEntExamCourse($model);
        makeCombo($objForm, $arg, $db, $query, $Row["TOTALCD"], "TOTALCD", "", 1, "blank");

        //試験科目コンボ
        $query = knjl602aQuery::getTestSubclass($model, $model->applicantdiv);
        ...
        $arg["data"]["MAGNIFYING"] = knjCreateTextBox($objForm, $Row["MAGNIFYING"], "MAGNIFYING", 3, 3, $extra);
        ...
        if (VARS::get("cmd") != "edit") {
            $arg["reload"]  = "parent.left_frame.location.href='knjl602aindex.php?cmd=list2"
                            . "&year=".$model->year."&applicantdiv=".$model->applicantdiv."&testdiv=".$model->testdiv."';";

        }
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl602aForm2.html", $arg);
    }
}
//コンボ作成
