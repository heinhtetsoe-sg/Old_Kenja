...
class knjl510gForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjl510gForm1", "POST", "knjl510gindex.php", "", "knjl510gForm1");
        ...
        //extra
        $extra = " onChange=\"return btn_submit('knjl510g');\"";

        //入試制度コンボの設定
        $query = knjl510gQuery::getApctDiv("L003", $model->ObjYear);
        makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->field["APPLICANTDIV"], $extra, 1);

        //入試区分コンボの設定
        $query = knjl510gQuery::getTestDiv("L004", $model->ObjYear);
        makeCmb($objForm, $arg, $db, $query, "TESTDIV", $model->field["TESTDIV"], $extra, 1);

        //ラジオボタン 1:受験者 2:保護者
        $opt_div = array(1, 2);
        $model->field["FINSCHOOLDIV"] = ($model->field["FINSCHOOLDIV"] == "") ? "1" : $model->field["FINSCHOOLDIV"];
        $extra = array("id=\"FINSCHOOLDIV1\" onClick=\"return btn_submit('knjl510g')\"", "id=\"FINSCHOOLDIV2\" onClick=\"return btn_submit('knjl510g')\"");
        ...
        /**********/
        //一覧リスト取得
        $result = $db->query(knjl510gQuery::getList($model));
        $row1 = array();
        ...
        View::toHTML($model, "knjl510gForm1.html", $arg); 
    }
}
//コンボ作成
