...
class knjl305bForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl305bForm1", "POST", "knjl305bindex.php", "", "knjl305bForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl305bQuery::getNameMst($model->ObjYear, "L003"));
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl305bQuery::getNameMst($model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL305B");
        ...
        View::toHTML($model, "knjl305bForm1.html", $arg); 
