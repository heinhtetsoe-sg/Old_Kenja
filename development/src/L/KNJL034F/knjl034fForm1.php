...
class knjl034fForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjl034findex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //年度設定
        $result = $db->query(knjl034fQuery::selectYearQuery());
        $opt = array();
        //レコードが存在しなければ処理年度を登録
        if ($result->numRows() == 0) { 
        ...
        if (!$flg) {
            if (!isset($model->year)) {
                ...
                $model->year = $db->getOne(knjl034fQuery::DeleteAtExist($model));
            }
            $arg["reload"][] = "parent.right_frame.location.href='knjl034findex.php?cmd=edit"
                             . "&year=" .$model->year."';";
        }
        ...
        //件数取得
        $a_cnt = array();
        $result = $db->query(knjl034fQuery::getDataCnt($model->year));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //件数取得
        $e_cnt = array();
        $result = $db->query(knjl034fQuery::getDataCnt($model->year, "EXEMPTION_CD"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //テーブルの中身の作成
        $a_bifKey = $e_bifKey = "";
        $query = knjl034fQuery::selectQuery($model->year);
...
//            $row["EXEMPTION_CD"]    = View::alink("knjl034findex.php", $row["EXEMPTION_CD"], "target=\"right_frame\"", $hash);
            $row["EXEMPTION"]       = $row["EXEMPTION_CD"] .":". $row["EXEMPTION_NAME"];
            $row["ITEM_CD"]         = View::alink("knjl034findex.php", $row["ITEM_CD"], "target=\"right_frame\"", $hash);
        ...
        View::toHTML($model, "knjl034fForm1.html", $arg);
    }
}
?>
