...
class knjl034fForm2 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjl034findex.php", "", "edit");
        ...
        if (!isset($model->warning) && ($model->cmd == "edit2" || $model->cmd == "reset") && $model->year && $model->applicantdiv && $model->item_cd && $model->exemption_cd) {
            $query = knjl034fQuery::getRow($model->year, $model->applicantdiv, $model->item_cd, $model->exemption_cd);
        ...
        $query = knjl034fQuery::getApplicantdiv($model->year);
        makeCombo($objForm, $arg, $db, $query, $Row["APPLICANTDIV"], "APPLICANTDIV", $extra, 1, $model, "blank");

        //免除コードコンボ
        $extra = "";
        $query = knjl034fQuery::getExemptionCd($model->year, $Row["APPLICANTDIV"]);
        makeCombo($objForm, $arg, $db, $query, $Row["EXEMPTION_CD"], "EXEMPTION_CD", $extra, 1, $model, "blank");

        //費目コードコンボ
        $extra = "";
        $query = knjl034fQuery::getItemCd($model->year, $Row["APPLICANTDIV"]);
        ...
        if (!isset($model->warning) && $model->cmd != "edit"){
            $arg["reload"]  = "parent.left_frame.location.href='knjl034findex.php?cmd=list"
                            . "&year=" .$model->year."';";
        }
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl034fForm2.html", $arg);
    }
}
