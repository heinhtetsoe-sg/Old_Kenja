...
class knjl124iForm1
{
    function main(&$model) {
        ...
        //入試制度コンボボックス
        $model->field["APPLICANTDIV"] = ($model->field["APPLICANTDIV"] == "") ? "2" : $model->field["APPLICANTDIV"];
        $extra = "onchange=\"return btn_submit('main');\" ";
        $query = knjl124iQuery::getNameMst("L003", $model->ObjYear);
        makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->field["APPLICANTDIV"], $extra, 1);

        //入試区分コンボボックス
        $model->field["TESTDIV"] = ($model->field["TESTDIV"] == "") ? "1" : $model->field["TESTDIV"];
        $extra = "onchange=\"return btn_submit('main');\" ";
        $query = knjl124iQuery::getTestDiv($model);
        ...
        //合格点
        if ($model->cmd ="main") $model->field["BORDER_SCORE"] = $db->getOne(knjl124iQuery::getBorderScore($model));
        ...
        //合格者数
        $arg["TOP"]["COUNT"] = $db->getOne(knjl124iQuery::getPassCount($model));

        //確定結果一覧
        $totalCount = 0;
        $query = knjl124iQuery::SelectQuery($model);
        ...
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            $row["BORDER_SCORE"] = View::alink("knjl124iindex.php", $row["BORDER_SCORE"], "", $hash);
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjl124iindex.php", "", "main");

        //フォーム終わり
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl124iForm1.html", $arg);
    }
}
