...
class knjl080cForm1
{
    function main(&$model)
    {
        ...
        //入試制度
        $opt = array();
        $result = $db->query(knjl080cQuery::GetName("L003",$model->ObjYear));
        ...
        //入試区分
        $opt = array();
        $result = $db->query(knjl080cQuery::GetName("L004",$model->ObjYear));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            $opt[]  = array("label" => $row["NAMECD2"]."：".$row["NAME1"], "value" => $row["NAMECD2"]);
        ...
        $result = $db->query(knjl080cQuery::GetLeftList($model));

        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            $opt_left[] = array("label" => $row["EXAMNO"]."：".$row["JUDGE"].$row["NAME"], "value" => $row["EXAMNO"]);
            $tmp_id[]   = $row["EXAMNO"];
        }
        ...
        //合格者一覧
        $opt_right = array();
        $result = $db->query(knjl080cQuery::GetRightList($model));

        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            if (!in_array($row["EXAMNO"], $tmp_id)) {
                $opt_right[]    = array("label" => $row["EXAMNO"]."：".$row["JUDGE"].$row["NAME"], "value" => $row["EXAMNO"]);
            }
        }
        ...
        $arg["start"]   = $objForm->get_start("sel", "POST", "knjl080cindex.php", "", "sel");

        $arg["finish"]  = $objForm->get_finish();

        View::toHTML($model, "knjl080cForm1.html", $arg); 
    }
}
?>
