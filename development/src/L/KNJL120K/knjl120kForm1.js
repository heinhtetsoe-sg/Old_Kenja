...
function clickRadio(val){
    ...
    //2005.12.29 minei edit
    //‡ŠiŽÒ‚ÌCSVŽæž‚Ý‚Í•s‰Â
    fObj = document.forms[0];
    CmbIdx = fObj.APPLI_TYPE.selectedIndex
    if( val == 1 && fObj.APPLI_TYPE.options[CmbIdx].value == 1){
      alert('{rval MSG300}');
      fObj.csv[1].checked =true;
      fObj.csvfile.disabled =true;

    }else{
      //org
      if (val == 1){
          fObj.csvfile.disabled =false;
      }else{
          fObj.csvfile.disabled =true;
      }
...
//2005.12.29 minei add
function change_apptype()
{
    //‡ŠiŽÒ‚ÌCSVŽæž‚Ý‚Í•s‰Â
    fObj = document.forms[0];
    CmbIdx = fObj.APPLI_TYPE.selectedIndex
    if( fObj.csv[0].checked && fObj.APPLI_TYPE.options[CmbIdx].value == 1){
      fObj.csv[1].checked =true;
      fObj.csvfile.disabled =true;
    }
}
