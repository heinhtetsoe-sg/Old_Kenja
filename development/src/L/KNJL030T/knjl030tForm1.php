...
class knjl030tForm1
{
    function main(&$model)
    {
        ...
        //入試制度
        $result = $db->query(knjl030tQuery::getName($model->year, "L003"));
        $opt_app = array();
        ...
        //会場一覧
        $result = $db->query(knjl030tQuery::selectQuery($model));
        $arg["data"] = array();
        ...
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            ...
            $row["EXAMHALL_NAME"] = View::alink("#",htmlspecialchars($row["EXAMHALL_NAME"]),
                            "onclick=\"loadwindow('knjl030tindex.php?cmd=edit&mode=update&examhallcd=".$row["EXAMHALLCD"] ."',event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(),400,200);\"");

            $s_length = strlen((int)$row["S_RECEPTNO"]) + (5 - strlen((int)$row["S_RECEPTNO"])) * 6;
            $row["S_RECEPTNO"] = str_pad((int)$row["S_RECEPTNO"], $s_length, "&nbsp;", STR_PAD_LEFT);

            $e_length = strlen((int)$row["E_RECEPTNO"]) + (5 - strlen((int)$row["E_RECEPTNO"])) * 6;
        ...
        $objForm->ae( array("type"        => "button",
                            "name"        => "btn_add",
                            "value"       => "会場追加",
                            "extrahtml"   => "onclick=\"loadwindow('knjl030tindex.php?cmd=edit&mode=insert',body.clientWidth/2-200,body.clientHeight/2-100,400,200);\"" ) );
        ...
        View::toHTML($model, "knjl030tForm1.html", $arg); 
    }
}
?>
