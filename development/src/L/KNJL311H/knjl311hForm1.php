...
class knjl311hForm1
{
	...
	$objForm = new form;
	//フォーム作成
	$arg["start"]   = $objForm->get_start("knjl311hForm1", "POST", "knjl311hindex.php", "", "knjl311hForm1");
	...
	$opt_apdv_typ = array();
	$db = Query::dbCheckOut();
	$result = $db->query(knjl311hQuery::get_apct_div("L003",$model->ObjYear));	
	...
	$objForm->ae( array("type"       => "select",
						...
						"extrahtml"  => " onChange=\"return btn_submit('knjl311h');\"",
	...
	$defoult     = 0 ;
	$db = Query::dbCheckOut();
	$result = $db->query(knjl311hQuery::get_test_div("L004",$model->ObjYear,$model->field["APDIV"]));	
	...
	$opt_exam_typ = array();
	$db = Query::dbCheckOut();
	$result = $db->query(knjl311hQuery::get_exam_div("L005",$model->ObjYear));	
	...
	//試験会場選択コンボボックスを作成する
	$db = Query::dbCheckOut();
	$result = $db->query(knjl311hQuery::get_hall_data($model->field["TESTDV"],$model->field["EXAM_TYPE"]));
	...
	View::toHTML($model, "knjl311hForm1.html", $arg); 
	}
}
?>
