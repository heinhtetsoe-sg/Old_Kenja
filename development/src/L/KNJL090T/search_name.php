    ...
    function getNameList(){
        ...
        if (strlen($this->names) || strlen($this->name_kana)){
            ...
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
                ...
                $opt[] = array("label" => make_space('aaaaa', $examno) . $examno."　".$row["NAME"]."　(".$row["NAME_KANA"].")",
                               "value" => $row["EXAMNO"]
                               );
            }
            Query::dbCheckIn($db);    
...
function setAddrText(obj){
    try{
        ...
        var arr = val.split("　");
        document.forms[0].names.value = arr[0].replace(/^[^0-9]*/,"");
...
function apply_name(obj,examno) {
    if(obj.selectedIndex >= 0){
        var val = obj.options[obj.selectedIndex].text;    
        var arr = val.split("　");
        f.document.forms[0]['EXAMNO'].value = arr[0].replace(/^[^0-9]*/,"");
