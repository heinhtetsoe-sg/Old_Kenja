...
class knjl362Form1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjl362Form1", "POST", "knjl362index.php", "", "knjl362Form1");
        ...
        //extra
        $extra = " onChange=\"return btn_submit('knjl362');\"";

        //入試制度コンボの設定
        $query = knjl362Query::getApctDiv("L003", $model->ObjYear);
        makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->field["APPLICANTDIV"], $extra, 1);

        //入試区分コンボの設定
        if ($model->isGojouOnly && $model->field["FINSCHOOLDIV"] == "2") {
            // 五條で塾
            $all = array("value" => "9", "label" => "-- 全て --");
            $opt = array();
            $opt[] = $all;
            $extra = "";
            $size = 1;
            $model->field["TESTDIV"] = $all["value"];
            $arg["data"]["TESTDIV"] = knjCreateCombo($objForm, "TESTDIV", $all["value"], $opt, $extra, $size);
        } else {
            $query = knjl362Query::getTestDiv("L004", $model->ObjYear);
        ...
        $extra = array("id=\"FINSCHOOLDIV1\" onClick=\"return btn_submit('knjl362')\"", "id=\"FINSCHOOLDIV2\" onClick=\"return btn_submit('knjl362')\"", "id=\"FINSCHOOLDIV3\" onClick=\"return btn_submit('knjl362')\"", "id=\"FINSCHOOLDIV4\" onClick=\"return btn_submit('knjl362')\"");
        ...
        /**********/
        //一覧リスト取得
        $result = $db->query(knjl362Query::getList($model));
        $row1 = array();
        ...
        View::toHTML($model, "knjl362Form1.html", $arg); 
    }
}
//コンボ作成
