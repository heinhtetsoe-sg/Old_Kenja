function btn_submit(cmd) {
    ...
    if (cmd == 'back'|| cmd == 'next') {
        ...
        if (document.forms[0].HID_S_RECEPTNO.value != "" && (document.forms[0].APPLICANTDIV.disabled || document.forms[0].TESTDIV.disabled || document.forms[0].TESTSUBCLASSCD.disabled)) {
            if (!confirm('{rval MSG108}')) {
                return true;
            }
        }
        //検索番号を引き継ぐために、入力制限を解除
        if (document.forms[0].S_RECEPTNO.disabled) {
            document.forms[0].S_RECEPTNO.disabled = false;
        }
    }
    ...
    //終了
    if (cmd == 'end') {
        if ((document.forms[0].APPLICANTDIV.disabled) || (document.forms[0].TESTDIV.disabled) || (document.forms[0].TESTSUBCLASSCD.disabled) || (document.forms[0].S_RECEPTNO.disabled) ) {
            if (confirm('{rval MSG108}')) {
                closeWin();
            }
            return false;
