...
class knjl312jForm1
{
	...
	$objForm = new form;
	//フォーム作成
	$arg["start"]   = $objForm->get_start("knjl312jForm1", "POST", "knjl312jindex.php", "", "knjl312jForm1");
...
//	$opt_apdv_typ = array();
//	$db = Query::dbCheckOut();
//	$result = $db->query(knjl312jQuery::get_apct_div("L003",$model->ObjYear));	
//	$opt_apdv_typ[]= array("label" => "-- 全て --", "value" => "0");
//
...
//						"extrahtml"  => " onChange=\"return btn_submit('knjl312j');\"",
...
//	$opt_test_dv[]= array("label" => "",  "value" => "");
	$result = $db->query(knjl312jQuery::get_test_div("L004",$model->ObjYear,$model->field["APDIV"]));	
...
//	$opt_exam_typ = array();
//	$db = Query::dbCheckOut();
//	$result = $db->query(knjl312jQuery::get_exam_div("L005",$model->ObjYear));	
...
//	$arg["data"]["EXAM_TYPE"] = $objForm->ge("EXAM_TYPE");
	...
	$opt_sbcd_typ = array();
	$db = Query::dbCheckOut();
	$result = $db->query(knjl312jQuery::get_subclas_div("L009",$model));	
	...
	//試験会場選択コンボボックスを作成する
	$db = Query::dbCheckOut();
	$result = $db->query(knjl312jQuery::get_hall_data($model->field["TESTDV"],$model->field["EXAM_TYPE"],$model->field["SUBCLASS_TYPE"]));
	...
	View::toHTML($model, "knjl312jForm1.html", $arg); 
	}
}
?>
