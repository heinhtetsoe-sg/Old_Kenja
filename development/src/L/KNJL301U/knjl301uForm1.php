...
class knjl301uForm1 {
    function main(&$model) {
        $objForm = new form;
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl301uForm1", "POST", "knjl301uindex.php", "", "knjl301uForm1");
        ...
        //入試制度コンボ
        $query = knjl301uQuery::getNameMst($model->examyear, "L003");
        $extra = "onChange=\"return btn_submit('knjl301u')\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["APPLICANTDIV"], "APPLICANTDIV", $extra, 1, "TOP", "");

        //入試区分
        $query = knjl301uQuery::getNameMst($model->examyear, "L004");
        $extra = "onChange=\"return btn_submit('knjl301u')\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["TESTDIV"], "TESTDIV", $extra, 1, "TOP", "");

        //会場一覧リストToリスト
        $row1 = array();
        $query = knjl301uQuery::getHallData($model);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL301U");
        ...
        View::toHTML($model, "knjl301uForm1.html", $arg);
    }
}
//makeCmb
