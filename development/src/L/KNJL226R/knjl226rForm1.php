...
class knjl226rForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl226rForm1", "POST", "knjl226rindex.php", "", "knjl226rForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl226rQuery::getNameMst($model, $model->ObjYear, "L003"));
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl226rQuery::getNameMst($model, $model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL226R");
        ...
        View::toHTML($model, "knjl226rForm1.html", $arg); 
