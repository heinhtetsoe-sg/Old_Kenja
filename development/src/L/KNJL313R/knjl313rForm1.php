...
class knjl313rForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl313rForm1", "POST", "knjl313rindex.php", "", "knjl313rForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl313rQuery::getNameMst($model, $model->ObjYear, "L003"));
        ...
        $extra = " onchange=\"return btn_submit('knjl313r');\"";
        $arg["data"]["APPLICANTDIV"] = knjCreateCombo($objForm, "APPLICANTDIV", $model->field["APPLICANTDIV"], $opt, $extra, 1);

        //入試区分
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl313rQuery::getNameMst($model, $model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl313rQuery::getCourseMajorcd($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        }
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl313rQuery::getEntexamCourseMst($model->ObjYear, $model->field["APPLICANTDIV"], $model->field["TESTDIV"], $model->field["COURSE_MAJORCD"]));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL313R");
        ...
        View::toHTML($model, "knjl313rForm1.html", $arg); 
