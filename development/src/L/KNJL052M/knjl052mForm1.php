...
class knjl052mForm1
{
    ...
    {
        $objForm      = new form;
        $arg["start"] = $objForm->get_start("main", "POST", "knjl052mindex.php", "", "main");
        ...
        //会場情報
        $query = knjl052mQuery::getHallDat($model);
        ...
        //科目情報
        $query = knjl052mQuery::getSubclassDetail($model);
        $result = $db->query($query);
        $subclassDat = array();
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        foreach ($subclassDat as $subclassKey => $subclassVal) {
            ...
            foreach ($hallDat as $hallKey => $hallVal) {
                $query = knjl052mQuery::getEntexamControlDat($model, $subclassVal["TESTVAL"], $hallVal["EXAMHALLCD"]);
        ...
        View::toHTML($model, "knjl052mForm1.html", $arg);
    }
}
?>