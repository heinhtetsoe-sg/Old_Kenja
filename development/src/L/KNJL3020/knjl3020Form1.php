...
class knjl3020Form1
{
    function main(&$model)
    {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("sel", "POST", "knjl3020index.php", "", "sel");
        $db = Query::dbCheckOut();        

        //年度コンボボックス
        $query = knjl3020Query::selectYearQuery($model);
        ...
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
            ...
            if ($model->examyear==$row["ENTEXAMYEAR"]) $j++;
        }
        if ($j==0) {
...
function makeList(&$objForm, &$arg, $db, $model) {
    //年度科目一覧取得
    $query = knjl3020Query::selectQuery($model);
