...
class knjl511hForm1 {
    function main(&$model) {
        ...
        if ((!isset($model->warning)) && (!is_array($existdata))) {
            //データを取得
            $Row = knjl511hQuery::get_edit_data($model);
...
//        $setTextField[] = "FS_GRDDIV";
        ...
        $setTextField[] = "GTELNO";
        for ($i=1; $i <= 5; $i++) {
            $setTextField[] = "RELANAME{$i}";
            $setTextField[] = "RELATIONSHIP{$i}";
            $setTextField[] = "WORKPLACE{$i}";
            $setTextField[] = "RELA_AGE{$i}";
        }
        knjCreateHidden($objForm, "setTextField", implode(',', $setTextField));

        //学校種別
        $extra = "onchange=\"return btn_submit('changeTest');\"";
        $query = knjl511hQuery::getNameMst($model->year, "L003");
        makeCmb($objForm, $arg, $db, $query, $model->applicantdiv, "APPLICANTDIV", $extra, 1, "");

        //入試判別
        $extra = "onchange=\"return btn_submit('changeTest');\"";
        $query = knjl511hQuery::getDistinctId($model);
        makeCmb($objForm, $arg, $db, $query, $model->distinctId, "DISTINCT_ID", $extra, 1, "");
        ...
        //学校名
        $query = knjl511hQuery::getFinschoolName($Row["FS_CD"]);
        ...
        //卒業区分
        $query = knjl511hQuery::getNameMst($model->year, "L016");
        $extra = "onChange=\"change_flg();\" onkeydown=\"changeEnterToTab(this)\"";
        $Row["FS_GRDDIV"] = ($Row["FS_GRDDIV"]) ? $Row["FS_GRDDIV"]: "2";
//        makeCmb($objForm, $arg, $db, $query, $Row["FS_GRDDIV"], "FS_GRDDIV", $extra, 1, "");

        // 入寮希望
        $domiCheck = ($Row["DORMITORY_FLG"] == "1") ? " checked": "";
        $extra = "id=\"DORMITORY_FLG\"";
        $arg["data"]["DORMITORY_FLG"] = knjCreateCheckBox($objForm, "DORMITORY_FLG", "1", $extra.$domiCheck);
        ...
        //名称マスタより和暦の元号を取得
        $result = $db->query(knjl511hQuery::get_calendarno($model->year));
        ...
        //続柄コンボ
        $query = knjl511hQuery::getNameMst($model->year, "H201");
        $extra = "onChange=\"change_flg()\" onkeydown=\"changeEnterToTab(this)\"";
        ...
        for ($i=1; $i <= 5; $i++) {
            //氏名
            $extra = "STYLE=\"ime-mode: active\" onChange=\"change_flg()\" id=\"RELANAME{$i}\" onkeydown=\"changeEnterToTab(this)\"";
            $arg["data"]["RELANAME{$i}"] = knjCreateTextBox($objForm, $Row["RELANAME{$i}"], "RELANAME{$i}", 40, 60, $extra);

            //勤務先・在学校
            $extra = "STYLE=\"ime-mode: active\" onChange=\"change_flg();\" id=\"WORKPLACE{$i}\" onkeydown=\"changeEnterToTab(this)\"";
            $arg["data"]["WORKPLACE{$i}"] = knjCreateTextBox($objForm, $Row["WORKPLACE{$i}"], "WORKPLACE{$i}", 40, 120, $extra);

            //続柄コンボ
            $query = knjl511hQuery::getNameMst($model->year, "H201");
            $extra = "onChange=\"change_flg()\" onkeydown=\"changeEnterToTab(this)\"";
            makeCmb($objForm, $arg, $db, $query, $Row["RELATIONSHIP{$i}"], "RELATIONSHIP{$i}", $extra, 1, "BLANK");

            //年齢
            $extra = "style=\"text-align:right; ime-mode: inactive;\" onblur=\"this.value=toInteger(this.value);\" onkeydown=\"changeEnterToTab(this)\"";
            $arg["data"]["RELA_AGE{$i}"] = knjCreateTextBox($objForm, $Row["RELA_AGE{$i}"], "RELA_AGE{$i}", 3, 3, $extra);
        }
        ...
        //かな検索ボタン
        $extra = "style=\"width:80px\" onclick=\"loadwindow('" .REQUESTROOT ."/L/KNJL511H/search_name.php?cmd=search&year='+document.forms[0]['year'].value+'&applicantdiv='+document.forms[0]['APPLICANTDIV'].value+'&distinctId='+document.forms[0]['DISTINCT_ID'].value+'&examno='+document.forms[0]['EXAMNO'].value+'&NAME_SESSID={$sess->id}&frame='+getFrameName(self), event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 260)\"";
        ...
        $arg["start"] = $objForm->get_start("main", "POST", "knjl511hindex.php", "", "main");
        $arg["finish"]  = $objForm->get_finish();
        View::toHTML($model, "knjl511hForm1.html", $arg);
    }
}
//makeCmb
