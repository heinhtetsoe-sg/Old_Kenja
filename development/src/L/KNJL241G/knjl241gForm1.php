...
class knjl241gForm1
{
    function main(&$model) {
        ...
        $query = knjl241gQuery::getNameMst("L003", $model->year);
        makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->applicantdiv, $extra, 1);

        //出身学校コンボ
        $extra = "onChange=\"return btn_submit('main');\" tabindex=-1";
        $query = knjl241gQuery::getFinSchool($model);
        ...
        if ($model->applicantdiv != "" && $model->finschoolcd != "") {
            ...
            //データ取得
            $result = $db->query(knjl241gQuery::SelectQuery($model));
            ...
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                ...
                $row["YOUROKU_DATA"] = knjCreateCheckBox($objForm, "YOUROKU_DATA"."-".$row["EXAMNO"], "1", $extra.$checked);

                //健康診断受領チェック
                $checked = $row["GET_MEDEXAM"] == "1" ? " checked " : "";
                $extra = "onClick=\"bgcolorYellow(this, '{$row["EXAMNO"]}');\"";
                $row["MEDEXAM_DATA"] = knjCreateCheckBox($objForm, "MEDEXAM_DATA"."-".$row["EXAMNO"], "1", $extra.$checked);

                //スポーツ振興受領チェック
                $checked = $row["GET_SPORTS"] == "1" ? " checked " : "";
                $disabled = substr($row['TESTDIV'], 0, 1) == '3' ? "" : " disabled ";
                $extra = "onClick=\"bgcolorYellow(this, '{$row["EXAMNO"]}');\"";
                $row["SPORTS_DATA"] = knjCreateCheckBox($objForm, "SPORTS_DATA"."-".$row["EXAMNO"], "1", $extra.$checked.$disabled);

                //印刷チェック
                $disabled = $row["GET_YOUROKU"] == "1" && $row["GET_MEDEXAM"] == "1" 
                        && ($row["GET_SPORTS"] == "1" || substr($row['TESTDIV'], 0, 1) != '3') ? "" : " disabled ";
                $extra = "onClick=\"printBtnColor();\"";
                $row["PRINT_DATA"] = knjCreateCheckBox($objForm, "PRINT_DATA"."-".$row["EXAMNO"], "1", $extra.$disabled);

                //印刷状態
                $row["PRINT_STATE"] = $row["PRINTFLG"] == "1" ? "済" : "";

                //hidden
                knjCreateHidden($objForm, "DEF_YOUROKU-".$row["EXAMNO"], $row["GET_YOUROKU"]);
                knjCreateHidden($objForm, "DEF_MEDEXAM-".$row["EXAMNO"], $row["GET_MEDEXAM"]);
                knjCreateHidden($objForm, "DEF_SPORTS-".$row["EXAMNO"], $row["GET_SPORTS"]);
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjl241gindex.php", "", "main");

        //フォーム終わり
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl241gForm1.html", $arg);
    }
}
...
function makeHidden(&$objForm, $model, $arr_examno) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJL241G");
