...
function apply_name(obj,examno,zip,gzip,zadd,gadd) {
    if(obj.selectedIndex >= 0){
        var val = obj.options[obj.selectedIndex].text;    
        var arr = val.split("　");
        f.document.forms[0]['FS_CD'].value = arr[0];
        f.document.forms[0]['FS_NAME'].value = arr[1];
//        f.document.getElementById("FS_NAME_ID").innerHTML = arr[1];
        f.closeit();
    }else{
        alert("名前が選択されていません");
    }
...
?>
        f.document.forms[0]['FS_NAME'].value = "";
//        f.document.getElementById("FS_NAME_ID").innerHTML = "";
    }
</script>
<?php
}
