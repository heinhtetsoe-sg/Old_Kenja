...
class knjl008fForm1 {
    function main(&$model) {
        $objForm        = new form;
        $arg["start"]   = $objForm->get_start("main", "POST", "knjl008findex.php", "", "main");
        ...
        //入試制度コンボ
        $query = knjl008fQuery::getNameMst($model->year, "L003", "1");
        $extra = "onChange=\"return btn_submit('main')\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["APPLICANTDIV"], "APPLICANTDIV", $extra, 1, "");

        //入試区分
        $query = knjl008fQuery::getNameMst($model->year, ($model->field["APPLICANTDIV"] == "1") ? "L024" : "L004");
        $extra = "onChange=\"return btn_submit('main')\"";
        ...
        $arg["top"]["S_RECEPTNO"] = knjCreateTextBox($objForm, $model->field['S_RECEPTNO'], "S_RECEPTNO", 4, 4, $extra);
        ...
        //一覧データ出力
        $schcnt = 0;
        $result = $db->query(knjl008fQuery::getSch($model, $model->field["TESTDIV"]));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            array_walk($row, "htmlspecialchars_array");

            $link = REQUESTROOT."/L/KNJL010F/knjl010findex.php?cmd=reference&SEND_PRGID=KNJL008F&SEND_AUTH={$model->auth}&SEND_APPLICANTDIV={$model->field["APPLICANTDIV"]}&SEND_EXAMNO={$row["EXAMNO"]}";
        ...
        View::toHTML($model, "knjl008fForm1.html", $arg); 
    }
}
//makeCmb
