...
class knjl324rForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl324rForm1", "POST", "knjl324rindex.php", "", "knjl324rForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl324rQuery::getNameMst($model, $model->ObjYear, "L003"));
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl324rQuery::getNameMst($model, $model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL324R");
        ...
        View::toHTML($model, "knjl324rForm1.html", $arg); 
