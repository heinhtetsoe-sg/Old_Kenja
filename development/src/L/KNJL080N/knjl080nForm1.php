...
class knjl080nForm1
{
    function main(&$model)
    {
        ...
        //入試制度
        $query = knjl080nQuery::GetName("L003", $model->ObjYear);
        ...
        $result = $db->query(knjl080nQuery::GetName("L004", $model->ObjYear));
        ...
        //専併区分コンボボックス
        $extra = "Onchange=\"btn_submit('main');\"";
        $query = knjl080nQuery::GetName("L006", $model->ObjYear);
        ...
        $query = knjl080nQuery::GetLeftList($model);
        ...
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            if ($row["LEFT_FLG"] == "1") {
                //対象者一覧
                $setPayMoney = $row["PAY_MONEY"] ? number_format($row["PAY_MONEY"]) : "";
                $setPayMoney = $setPayMoney ? "(\\{$setPayMoney})" : "";
        ...
        $arg["start"]   = $objForm->get_start("sel", "POST", "knjl080nindex.php", "", "sel");

        $arg["finish"]  = $objForm->get_finish();

        View::toHTML($model, "knjl080nForm1.html", $arg); 
    }
}
