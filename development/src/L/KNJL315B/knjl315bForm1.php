...
class knjl315bForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl315bForm1", "POST", "knjl315bindex.php", "", "knjl315bForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl315bQuery::getNameMst($model->ObjYear, "L003", $model));
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl315bQuery::getNameMst($model->ObjYear, "L004", $model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        $extra = "onchange=\"return btn_submit('knjl315b');\"";
        $arg["data"]["TESTDIV"] = knjCreateCombo($objForm, "TESTDIV", $model->field["TESTDIV"], $opt, $extra, 1);

        //受験科目
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl315bQuery::getNameMst($model->ObjYear, "L009", $model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        }
        $opt[] = array('label' => "--総合計--",'value' => "9");
        $model->field["TESTSUBCLASSCD"] = ($model->field["TESTSUBCLASSCD"]) ? $model->field["TESTSUBCLASSCD"] : $opt[0]["value"];
        $extra = "";
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL315B");
        ...
        View::toHTML($model, "knjl315bForm1.html", $arg); 
