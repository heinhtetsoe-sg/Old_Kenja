...
class knjl306bForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl306bForm1", "POST", "knjl306bindex.php", "", "knjl306bForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl306bQuery::getNameMst($model->ObjYear, "L003"));
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl306bQuery::getNameMst($model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL306B");
        ...
        View::toHTML($model, "knjl306bForm1.html", $arg); 
