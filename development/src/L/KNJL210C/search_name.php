...
class SearchNames
{
    ...
    function getNameList(){
        ...
        if (strlen($this->names) || strlen($this->name_kana)){
            ...
            $query .= "ORDER BY PRE_RECEPTNO ";
            //受験番号、氏名、氏名かな取得
            $result = $db->query($query);
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
                $opt[] = array("label" => $row["PRE_RECEPTNO"]."　".$row["NAME"]."　(".$row["NAME_KANA"].")",
                               "value" => $row["PRE_RECEPTNO"]
                               );
            }
            Query::dbCheckIn($db);    
        }
        $this->form->ae( array("type"       => "select",
                               ...
                               "size"       => "11",
                               "value"      => "",
                               "extrahtml"   => " ondblclick=\"apply_name(this,'".$this->receptno."');\" STYLE=\"WIDTH:100%\"",
...
function apply_name(obj,receptno) {
    if(obj.selectedIndex >= 0){
        var val = obj.options[obj.selectedIndex].text;    
        var arr = val.split("　");
        f.document.forms[0]['PRE_RECEPTNO'].value = arr[0];
        if(f.btn_submit('reference') != true) {
            f.closeit();
        } else {
            f.document.forms[0]['PRE_RECEPTNO'].value = receptno;
        }
    }else{
        alert("名前が選択されていません");
    }
