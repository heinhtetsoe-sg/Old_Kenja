...
class knjl365kForm1
{
    ...
    $objForm = new form;
    //フォーム作成
    $arg["start"]   = $objForm->get_start("knjl365kForm1", "POST", "knjl365kindex.php", "", "knjl365kForm1");
    $db = Query::dbCheckOut();
    ...
    $result = $db->query(knjl365kQuery::GetTestdiv($model));
    ...
    $opt = array();
    $value_flg = false;
    $query = knjl365kQuery::getSpecialReasonDiv($model);
    ...
    //中高判別フラグを作成する
    $jhflg = 0;
    $row = $db->getOne(knjl365kQuery::GetJorH());
    if ($row == 1){
        $jhflg = 1;
    }else {
        $jhflg = 2;
    ...
    View::toHTML($model, "knjl365kForm1.html", $arg); 
    }
}
?>
