...
class knjl033nForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjl033nindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //年度設定
        $result    = $db->query(knjl033nQuery::selectYearQuery());
        $opt       = array();
        //レコードが存在しなければ処理年度を登録
        if ($result->numRows() == 0) { 
        ...
        if(!$flg) {
            if (!isset($model->year)) {
                ...
                $model->year = $db->getOne(knjl033nQuery::DeleteAtExist($model));
            }
            $arg["reload"][] = "parent.right_frame.location.href='knjl033nindex.php?cmd=edit"
                             . "&year=" .$model->year."';";
        }
        ...
        //テーブルの中身の作成
        $query = knjl033nQuery::selectQuery($model);
        $result = $db->query($query);
            ...
            $row["JUDGE"] = View::alink("knjl033nindex.php", $row["JUDGENAME"], "target=\"right_frame\"", $hash);
            $arg["data"][] = $row;
        }
        $result->free();
        ...
        View::toHTML($model, "knjl033nForm1.html", $arg);
    }
}
?>
