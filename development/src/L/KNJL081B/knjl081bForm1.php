...
class knjl081bForm1
{
    function main(&$model)
    {
        ...
        //入試制度
        $query = knjl081bQuery::GetName("L003", $model->ObjYear);
        ...
        $result = $db->query(knjl081bQuery::GetName("L004", $model->ObjYear));
        ...
        //正規合否区分
        $opt = array();
        $query = knjl081bQuery::GetCourseJudgment($model);
        $model->judgmentArray = array();
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $model->judgmentArray[$row["JUDGMENT_DIV"]] = $row;
        ...
        $query = knjl081bQuery::GetLeftList($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $row["PRODATE"] = str_replace("-","/",$row["PRODATE"]);
            //$tanText = $row["TANFLG"] ? "【単】" : "";
            if ($row["LEFT_FLG"] == "1") {
                //対象者一覧
                $opt_left[]  = array("label" => $row["EXAMNO"]."：".$row["NAME"]."：".$row["JUDGMENT_COURSE_ABBV"]."：".$row["PRODATE"], "value" => $row["EXAMNO"].":".$row["PRODATE"].":".$row["JUDGMENT_DIV"].":".$row["SUB_ORDER"]);
            } else {
                //合格者一覧
                $opt_right[] = array("label" => $row["EXAMNO"]."：".$row["NAME"]."：".$row["JUDGMENT_COURSE_ABBV"], "value" => $row["EXAMNO"].":".$row["PRODATE"].":".$row["JUDGMENT_DIV"].":".$row["SUB_ORDER"]);
            }
        }
        //対象者一覧
        ...
        $arg["start"]   = $objForm->get_start("sel", "POST", "knjl081bindex.php", "", "sel");

        $arg["finish"]  = $objForm->get_finish();

        View::toHTML($model, "knjl081bForm1.html", $arg); 
    }
}
