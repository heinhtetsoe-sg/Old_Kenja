...
class knjl306rForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl306rForm1", "POST", "knjl306rindex.php", "", "knjl306rForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl306rQuery::getNameMst($model, $model->ObjYear, "L003"));
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl306rQuery::getNameMst($model, $model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL306R");
        ...
        View::toHTML($model, "knjl306rForm1.html", $arg); 
    }
}
?>
