...
class knjl781hForm1
{
    public function main(&$model)
    {
        ...
        //学校種別
        $query = knjl781hQuery::getNameMst($model->examYear, "L003");
        $extra = "onchange=\"return btn_submit('read');\"";
        makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->applicantdiv, $extra, 1, "");

        //入試区分
        $query = knjl781hQuery::getTestdiv($model);
        $extra = "onchange=\"return btn_submit('read');\"";
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL781H");
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjl781hindex.php", "", "main");

        //フォーム終わり
        $arg["finish"] = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl781hForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    ...
    $leftCnt = $rightCnt = 0;
    $query = knjl781hQuery::getBaseDatData($model);
