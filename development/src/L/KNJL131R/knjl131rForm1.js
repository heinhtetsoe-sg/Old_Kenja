...
function bgcolorYellow(obj, examno) {
    ...
    if (changeData) {
        ...
        if (document.forms[0]["YOUROKU_DATA-" + examno].checked && document.forms[0]["MEDEXAM_DATA-" + examno].checked) {
...
function yourokuKensinCheckDisabled(setFlg) {
    for (var i=0; i < document.forms[0].elements.length; i++) {
        var e = document.forms[0].elements[i];
        var nam = e.name;
        if (e.type == 'checkbox' && (nam.split("-")[0] == "YOUROKU_DATA" || nam.split("-")[0] == "MEDEXAM_DATA")) {
            e.disabled = setFlg;
        }
    }
}
...
function setCombDisabled() {
    if (document.forms[0].btn_update.style.background == "" && document.forms[0].btn_updatePrint.style.background == "") {
        ...
        document.forms[0].HID_PRINT_ZUMI.value = document.forms[0].PRINT_ZUMI.options[document.forms[0].PRINT_ZUMI.selectedIndex].value;
