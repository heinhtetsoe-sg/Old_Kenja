...
class knjl131rForm1
{
    function main(&$model) {
        ...
        $query = knjl131rQuery::getNameMst("L003", $model->year, $model->fixApplicantDiv);
        makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->applicantdiv, $extra, 1);

        //出身学校コンボ
        $extra = "onChange=\"return btn_submit('main');\" tabindex=-1";
        $query = knjl131rQuery::getFinSchool($model);
        ...
        if ($model->applicantdiv != "" && $model->finschoolcd != "") {
            ...
            //データ取得
            $result = $db->query(knjl131rQuery::SelectQuery($model));
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjl131rindex.php", "", "main");

        //フォーム終わり
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl131rForm1.html", $arg);
    }
}
...
function makeHidden(&$objForm, $model, $arr_examno) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJL131R");
