...
class knjl318uForm1 {
    function main(&$model) {
        $objForm = new form;
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl318uForm1", "POST", "knjl318uindex.php", "", "knjl318uForm1");
        ...
        //入試制度コンボ
        $query = knjl318uQuery::getNameMst($model->examyear, "L003");
        $extra = "onChange=\"return btn_submit('knjl318u')\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["APPLICANTDIV"], "APPLICANTDIV", $extra, 1, "TOP", "");

        //入試区分
        $query = knjl318uQuery::getNameMst($model->examyear, "L004");
        $extra = "onChange=\"return btn_submit('knjl318u')\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["TESTDIV"], "TESTDIV", $extra, 1, "TOP", "");

        //出力順ラジオ（1:受験番号、2:成績、3:氏名かな）
        $opt = array(1, 2, 3);
        ...
        //氏名出力チェックボックス
        $extra = "id=\"PRINT_NAME\"";
        $arg["data"]["PRINT_NAME"] = knjCreateCheckBox($objForm, "PRINT_NAME", "1", $extra);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL318U");
        ...
        View::toHTML($model, "knjl318uForm1.html", $arg);
    }
}
//makeCmb
