...
class knjl351bForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl351bForm1", "POST", "knjl351bindex.php", "", "knjl351bForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl351bQuery::getNameMst($model->ObjYear, "L003"));
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl351bQuery::getNameMst($model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL351B");
        ...
        View::toHTML($model, "knjl351bForm1.html", $arg); 
