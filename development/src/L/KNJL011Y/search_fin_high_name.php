...
class SearchNames
{
    ...
    public function getNameList()
    {
        ...
        if (strlen($this->names) || strlen($this->name_kana) || strlen($this->schoolcd)) {
            ...
            $query .= "  WHERE  ";
            if ($this->cmd == "apply") {
                $query .= "     T1.FINSCHOOLCD = '" .$this->schoolcd ."' ";
            } else {
                if (strlen($this->names)) {
                    $query .= " T1.FINSCHOOL_NAME like '%" .$this->names2 ."%' ";
                }
                if (strlen($this->names) && strlen($this->name_kana)) {
                    $query .= " AND ";
                }
                if (strlen($this->name_kana)) {
                    $query .= " T1.FINSCHOOL_KANA like '%" .$this->name_kana2 ."%' ";
                }
            }
    ...
    {
        return $this->form->get_start("name", "POST", "search_fin_high_name.php", "", "name");
    }
    //フォームタブ(終了)
...
function apply_name(obj,examno,zip,gzip,zadd,gadd) {
    if(obj.selectedIndex >= 0){
        var val = obj.options[obj.selectedIndex].text;    
        var arr = val.split("　");
        f.document.forms[0]['SH_SCHOOLCD'].value = arr[0];
        f.document.getElementById("SH_FS_NAME_ID").innerHTML = arr[1];
        var val2 = obj.options[obj.selectedIndex].value;
        var arr2 = val2.split("　");
        f.document.getElementById("SH_RITSU_NAME_ID").innerHTML = arr2[2];
        f.closeit();
...
<script language="JavaScript">
    ...
    ?>
        f.document.getElementById("SH_FS_NAME_ID").innerHTML = "";
        f.document.getElementById("SH_RITSU_NAME_ID").innerHTML = "";
    }
</script>
    <?php
}
