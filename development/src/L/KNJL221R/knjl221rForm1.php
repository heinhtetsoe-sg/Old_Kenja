...
class knjl221rForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl221rForm1", "POST", "knjl221rindex.php", "", "knjl221rForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl221rQuery::getNameMst($model, $model->ObjYear, "L003"));
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl221rQuery::getNameMst($model, $model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL221R");
        ...
        View::toHTML($model, "knjl221rForm1.html", $arg); 
