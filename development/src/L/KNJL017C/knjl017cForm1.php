...
class knjl017cForm1
{
    ...
    {
        $objForm      = new form;
        $arg["start"] = $objForm->get_start("main", "POST", "knjl017cindex.php", "", "main");
        ...
        if ($model->examno == "" || $model->cmd == "showdiv") {
            $db = Query::dbCheckOut();
            $result = $db->query(knjl017cQuery::get_name_cd($model->year, 'L003'));
            ...
            $model->examno = $db->getOne(knjl017cQuery::getMinExam($model->year, $defApplicantDiv));
            Query::dbCheckIn($db);
        }
        ...
        if ((!isset($model->warning)) && (!is_array($existdata))) {
            //データを取得
            $Row = knjl017cQuery::get_edit_data($model);
        ...
        $testdivNameArray = array(); //重複受験番号のコメントに使用
        //入試区分コンボ
        $result     = $db->query(knjl017cQuery::get_name_cd($model->year, 'L004'));
        $opt_testdiv = array();
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            ...
            $testdivNameArray[$row["NAMECD2"]] = $row["NAME1"];
        }
        ...
        //専併コンボ
        $result     = $db->query(knjl017cQuery::get_name_cd($model->year, 'L006'));
        $opt_shdiv = array();
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        //五條併願チェックボックス
        $extra  = "id=\"SHIFT_DESIRE_FLG1\" onchange=\"change_flg()\"";
        $extra .= strlen($Row["SHIFT_DESIRE_FLG1"]) ? " checked" : "";
        $arg["data"]["SHIFT_DESIRE_FLG1"] = knjCreateCheckBox($objForm, "SHIFT_DESIRE_FLG1", "1", $extra, "");

        //和歌山併願チェックボックス
        $extra  = "id=\"SHIFT_DESIRE_FLG2\" onchange=\"change_flg()\"";
        $extra .= strlen($Row["SHIFT_DESIRE_FLG2"]) ? " checked" : "";
        $arg["data"]["SHIFT_DESIRE_FLG2"] = knjCreateCheckBox($objForm, "SHIFT_DESIRE_FLG2", "1", $extra, "");
        ...
        $opt[] = array("label" => "1:Ⅰ型", "value" => "1");
        $opt[] = array("label" => "2:Ⅱ型", "value" => "2");
        $arg["data"]["SUB_ORDER"] = knjCreateCombo($objForm, "SUB_ORDER", $Row["SUB_ORDER"], $opt, "", 1);
        ...
        //性別コンボ
        $result     = $db->query(knjl017cQuery::get_name_cd($model->year, 'Z002'));
        $opt        = array();
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            $opt[] = array("label" => $row["NAMECD2"].":".$row["NAME1"],
        ...
        $result = $db->query(knjl017cQuery::getPrefMst());
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        //所在地教育委員会コンボ
        $query = knjl017cQuery::getEdboard();
        ...
        $FsNatpName = array();
        $FsDistName = array();
        $result     = $db->query(knjl017cQuery::getFinschoolcd($model->year, $Row["FS_CD"]));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        $objForm->ae( array("type"        => "text",
                            ...
                            "size"        => 7,
                            "maxlength"   => 7,
                            "extrahtml"   => "onchange=\"change_flg()\" onblur=\"hiddenWin('" .REQUESTROOT ."/L/KNJL017C/search_fin_name.php?cmd=apply&year='+document.forms[0]['year'].value+'&examno='+document.forms[0]['EXAMNO'].value+'&schoolcd='+document.forms[0]['FINSCHOOLCD'].value+'&frame='+getFrameName(self))\"",
                            "value"       => $Row["FS_CD"] ) );
        ...
        //塾
        $PsName     = array();
        $result     = $db->query(knjl017cQuery::getPrischoolcd($model->year, $Row["PRISCHOOLCD"]));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        $opt       = array();
        $opt[]     = array("label" => "", "value" => "");
        $result    = $db->query(knjl017cQuery::get_name_cd($model->year, 'H201'));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            $opt[] = array("label" => $row["NAMECD2"].":".$row["NAME1"],
                           "value" => $row["NAMECD2"]);
        ...
        //------------------------------内申科目---------------------------------
        $result = $db->query(knjl017cQuery::get_name_cd($model->year, "L008"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        //重複受験番号のコメント
        $arg["data"]["RECOM_EXAMNO_COMENT"] = "一般入試・表現力入試とも出願している場合のみ入力。一般入試は表現力入試の受験番号。表現力入試は一般入試の受験番号。";
        ...
        $objForm->ae( array("type"        => "button",
                            ...
                            "value"       => "かな検索",
                            "extrahtml"   => "style=\"width:70px\" onclick=\"loadwindow('" .REQUESTROOT ."/L/KNJL017C/search_name.php?cmd=search&year='+document.forms[0]['year'].value+'&examno='+document.forms[0]['EXAMNO'].value+'&NAME_SESSID={$sess->id}&frame='+getFrameName(self), event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 260)\"") );
        ...
        $objForm->ae( array("type"        => "button",
                            ...
                            "value"       => "かな検索",
                            "extrahtml"   => "style=\"width:70px\" onclick=\"loadwindow('" .REQUESTROOT ."/L/KNJL017C/search_pri_name.php?cmd=search&year='+document.forms[0]['year'].value+'&examno='+document.forms[0]['EXAMNO'].value+'&NAME_SESSID={$sess->id}&frame='+getFrameName(self), event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}()-180, 320, 370)\"") );
        ...
        View::toHTML($model, "knjl017cForm1.html", $arg);
    }
}
