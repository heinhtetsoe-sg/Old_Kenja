...
class knjl072gForm1
{
    function main(&$model) {
        ...
        $query = knjl072gQuery::getNameMst("L003", $model->year);
        makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->applicantdiv, $extra, 1);

        //入試区分コンボ
        $extra = "onchange=\"return btn_submit('main');\" tabindex=-1";
        $query = knjl072gQuery::getNameMst("L004", $model->year);
        makeCmb($objForm, $arg, $db, $query, "TESTDIV", $model->testdiv, $extra, 1);

        //専併区分コンボ
        $extra = "onchange=\"return btn_submit('main');\" tabindex=-1";
        $query = knjl072gQuery::getNameMst("L006", $model->year);
        makeCmb($objForm, $arg, $db, $query, "SHDIV", $model->shdiv, $extra, 1);

        //第１志望コンボ
        $extra = "onchange=\"return btn_submit('main');\" tabindex=-1";
        $query = knjl072gQuery::getExamcourse($model);
        ...
        $query = knjl072gQuery::getNameMst("L013", $model->year);
        ...
        if ($model->applicantdiv != "" && $model->testdiv != "" && $model->shdiv != "" && $model->examcourse != "") {
            ...
            //データ取得
            $result = $db->query(knjl072gQuery::SelectQuery($model));
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjl072gindex.php", "", "main");

        //フォーム終わり
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl072gForm1.html", $arg);
    }
}
