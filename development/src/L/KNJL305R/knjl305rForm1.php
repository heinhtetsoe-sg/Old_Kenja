...
class knjl305rForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl305rForm1", "POST", "knjl305rindex.php", "", "knjl305rForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl305rQuery::getNameMst($model, $model->ObjYear, "L003"));
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl305rQuery::getNameMst($model, $model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL305R");
        ...
        View::toHTML($model, "knjl305rForm1.html", $arg); 
