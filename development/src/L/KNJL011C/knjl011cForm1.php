...
class knjl011cForm1
{
    ...
    {
        $objForm      = new form();
        $arg["start"] = $objForm->get_start("main", "POST", "knjl011cindex.php", "", "main");
        ...
        if ((!isset($model->warning)) && (!is_array($existdata))) {
            //データを取得
            $Row = knjl011cQuery::getEditData($model);
        ...
        //------------------------------志願者情報-------------------------------------
        //入試制度コンボ
        $result     = $db->query(knjl011cQuery::getNameCd($model->year, 'L003'));
        $opt        = array();
        ...
        //入試区分コンボ
        $result     = $db->query(knjl011cQuery::getNameCd($model->year, 'L004'));
        $opt_testdiv = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //専併コンボ
        $result     = $db->query(knjl011cQuery::getNameCd($model->year, 'L006'));
        $opt_shdiv = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $arg["data"][$fieldname."_NAME"] = $model->sdfName;

        //選択受験科目コンボ
        $result     = $db->query(knjl011cQuery::getNameCd($model->year, 'L009'));
        ...
        //性別コンボ
        $result     = $db->query(knjl011cQuery::getNameCd($model->year, 'Z002'));
        $opt        = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array("label" => $row["NAMECD2"].":".$row["NAME1"],
                           "value" => $row["NAMECD2"]);
        ...
        //名称マスタより和暦の元号を取得
        $result = $db->query(knjl011cQuery::getCalendarNo($model->year));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            if ($calno == "") {
                $calno = $row["NAMECD2"];
        ...
        $result = $db->query(knjl011cQuery::getPrefMst());
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //所在地教育委員会コンボ
        $query = knjl011cQuery::getEdboard();
        ...
        $FsNatpName = array();
        $FsDistName = array();
        $result     = $db->query(knjl011cQuery::getFinschoolcd($model->year, $Row["FS_CD"]));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $objForm->ae(array("type"        => "text",
                            ...
                            "size"        => 7,
                            "maxlength"   => 7,
                            "extrahtml"   => "onchange=\"change_flg()\" onblur=\"hiddenWin('" .REQUESTROOT ."/L/KNJL011C/search_fin_name.php?cmd=apply&year='+document.forms[0]['year'].value+'&examno='+document.forms[0]['EXAMNO'].value+'&schoolcd='+document.forms[0]['FINSCHOOLCD'].value+'&frame='+getFrameName(self))\"",
                            "value"       => $Row["FS_CD"] ));
        ...
        //塾
        $PsName     = array();
        $result     = $db->query(knjl011cQuery::getPrischoolcd($model->year, $Row["PRISCHOOLCD"]));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $result    = $db->query(knjl011cQuery::getNameCd($model->year, 'H201'));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array("label" => $row["NAMECD2"].":".$row["NAME1"],
                           "value" => $row["NAMECD2"]);
        }
        ...
        //------------------------------内申科目---------------------------------
        $result = $db->query(knjl011cQuery::getNameCd($model->year, "L008"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($model->isGojou) {
            ...
            //併願校（高校）
            $shFsName    = array();
            $result     = $db->query(knjl011cQuery::getFinHighschoolcd($Row["SH_SCHOOLCD"]));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            $objForm->ae(array("type"        => "text",
                                ...
                                "size"        => 7,
                                "maxlength"   => 7,
                                "extrahtml"   => "onchange=\"change_flg()\" onblur=\"hiddenWin('" .REQUESTROOT ."/L/KNJL011C/search_fin_high_name.php?cmd=apply&year='+document.forms[0]['year'].value+'&examno='+document.forms[0]['EXAMNO'].value+'&schoolcd='+document.forms[0]['SH_SCHOOLCD'].value+'&frame='+getFrameName(self))\"",
        ...
        $objForm->ae(array("type"        => "button",
                            ...
                            "value"       => "かな検索",
                            "extrahtml"   => "style=\"width:70px\" onclick=\"loadwindow('" .REQUESTROOT ."/L/KNJL011C/search_name.php?cmd=search&year='+document.forms[0]['year'].value+'&examno='+document.forms[0]['EXAMNO'].value+'&NAME_SESSID={$sess->id}&frame='+getFrameName(self), event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 260)\""));
        ...
        $objForm->ae(array("type"        => "button",
                            ...
                            "value"       => "かな検索",
                            "extrahtml"   => "style=\"width:70px\" onclick=\"loadwindow('" .REQUESTROOT ."/L/KNJL011C/search_pri_name.php?cmd=search&year='+document.forms[0]['year'].value+'&examno='+document.forms[0]['EXAMNO'].value+'&NAME_SESSID={$sess->id}&frame='+getFrameName(self), event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}()-180, 320, 370)\""));
        ...
        View::toHTML($model, "knjl011cForm1.html", $arg);
    }
}
