...
class knjl414Form1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjl414index.php", "", "edit");
        ...
        //分類
        $query = knjl414Query::getRecruitClass($model);
        $extra = "onchange=\"return btn_submit('chgYear');\"";
        ...
        $this_cnt = $db->getOne(knjl414Query::getRecruitSendYmstCopy($model->leftYear, "cnt"));
        knjCreateHidden($objForm, "THIS_YEAR_CNT", $this_cnt);

        $preYear = $model->leftYear-1;
        $pre_cnt = $db->getOne(knjl414Query::getRecruitSendYmstCopy($preYear, "cnt"));
        ...
        if ($model->cmd == "chgYear") {
            $model->leftSendCd = "";
            $arg["reload"] = "parent.right_frame.location.href='knjl414index.php?cmd=edit';";
        }

        $arg["finish"] = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl414Form1.html", $arg); 
    }
}
//makeCmb
...
function makeList(&$arg, $db, $model) {
    $result = $db->query(knjl414Query::getEventCnt($model));
    $eventArray = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    $result = $db->query(knjl414Query::getList($model));
    $befEvent = "";
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
