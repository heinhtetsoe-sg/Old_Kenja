...
class knjl033yForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjl033yindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //テーブルの中身の作成
        $query = knjl033yQuery::selectQuery($model);
        $result = $db->query($query);
            ...
            $row["JUDGE"] = View::alink("knjl033yindex.php", $row["JUDGENAME"], "target=\"right_frame\"", $hash);
            $arg["data"][] = $row;
        }
        $result->free();
        ...
        View::toHTML($model, "knjl033yForm1.html", $arg);
    }
}
?>
