...
class knjl033yForm2
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjl033yindex.php", "", "edit");
        ...
        if (!isset($model->warning) && ($model->cmd == "edit2") && $model->applicantdiv && $model->judge_kind != ""){
            $query = knjl033yQuery::getRow($model->applicantdiv, $model->judge_kind);
        ...
        $query = knjl033yQuery::getNameMst($model, "L003", "APPLICANTDIV");
        ...
        //入試制度コンボ
        $extra = "onchange=\"return btn_submit('edit');\"";
        $query = knjl033yQuery::getNameMst($model, "L025", "JUDGE_KIND");
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "parent.left_frame.location.href='knjl033yindex.php?cmd=list';";
        }
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl033yForm2.html", $arg);
    }
}
