...
class knjl073fForm1
{
    function main(&$model) {
        ...
        $query = knjl073fQuery::getNameMst("L003", $model->year, "2");
        makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->applicantdiv, $extra, 1);

        //入試区分コンボ
        $extra = "onchange=\"return btn_submit('main');\" tabindex=-1";
        $query = knjl073fQuery::getNameMst("L004", $model->year);
        makeCmb($objForm, $arg, $db, $query, "TESTDIV", $model->testdiv, $extra, 1);

        //入試回数コンボ
        $extra = "onchange=\"return btn_submit('main');\" tabindex=-1";
        $query = knjl073fQuery::getTestdiv0($model->year, $model->testdiv);
        makeCmb($objForm, $arg, $db, $query, "TESTDIV0", $model->testdiv0, $extra, 1);

        //コース区分コンボ
        $extra = "onchange=\"return btn_submit('main');\" tabindex=-1";
        $query = knjl073fQuery::getExamcourse1();
        ...
        $query = knjl073fQuery::getNameMst("L013", $model->year);
        makeCmb($objForm, $arg, $db, $query, "JUDGEDIV", $model->judgediv, $extra, 1, "BLANK");

        //合格コースコンボ
        $disSuc = ($model->judgediv == "1" || $model->judgediv == "3") ? "" : " disabled";
        $extra = "" .$disSuc;
        $query = knjl073fQuery::getSucCourse($model);
        makeCmb($objForm, $arg, $db, $query, "SUC_COURSE", $model->suc_course, $extra, 1, "BLANK");
        ...
        if ($model->applicantdiv != "" && $model->testdiv != "" && $model->testdiv0 != "" && $model->examcourse1 != "") {
            ...
            //データ取得
            $result = $db->query(knjl073fQuery::SelectQuery($model));
            ...
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                ...
                $row["SLIDE_FLG"] = ($row["SLIDE_FLG"] == "1") ? "レ" : "";
                $row["SHIFT_DESIRE_FLG"] = ($row["SHIFT_DESIRE_FLG"] == "1") ? "レ" : "";
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjl073findex.php", "", "main");

        //フォーム終わり
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl073fForm1.html", $arg);
    }
}
