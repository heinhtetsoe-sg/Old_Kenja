...
class knjl514jForm1 {
    ...
    function main(&$model) {
        ...
        //データ取込
        //学校種別
        $query = knjl514jQuery::getNameMst($model->ObjYear, "L003");
        $extra = " onChange=\"btn_submit('main')\"";
        makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->field["APPLICANTDIV"], $extra, 1, "");
        //入試種別
        $query = knjl514jQuery::getTestDiv($model->ObjYear, $model->field["APPLICANTDIV"]);
        $extra = "";
        makeCmb($objForm, $arg, $db, $query, "TESTDIV", $model->field["TESTDIV"], $extra, 1, "");
        //入試方式
        $query = knjl514jQuery::getExamType($model->ObjYear, $model->field["APPLICANTDIV"]);
        $extra = "";
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjl514jindex.php", "", "main");
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl514jForm1.html", $arg);
    }
}
