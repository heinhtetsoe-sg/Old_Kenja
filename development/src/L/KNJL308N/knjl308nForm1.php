...
class knjl308nForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl308nForm1", "POST", "knjl308nindex.php", "", "knjl308nForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl308nQuery::getNameMst($model->ObjYear, "L003"));
        ...
        $extra = " onchange=\"return btn_submit('knjl308n');\"";
        $arg["data"]["APPLICANTDIV"] = knjCreateCombo($objForm, "APPLICANTDIV", $model->field["APPLICANTDIV"], $opt, $extra, 1);

        //入試区分
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl308nQuery::getNameMst($model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        //志望区分コンボ
        $query = knjl308nQuery::getExamcourse($model->ObjYear, $model->field["APPLICANTDIV"], $model->field["TESTDIV"]);
        $extra = "onChange=\"btn_submit('knjl308n');\"";
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL308N");
        ...
        View::toHTML($model, "knjl308nForm1.html", $arg); 
