...
class knjl090qForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjl090qindex.php", "", "main");
...
/****************************************** GET DATA!! ***************************************************************/        
        ...
        $Row = $db->getRow(knjl090qQuery::get_edit_data($model), DB_FETCHMODE_ASSOC);
        if (!is_array($Row)) {
        ...
        if (strlen($model->examno)) {
            $model->testdiv = $db->getOne(knjl090qQuery::getMaxtestdiv($model));    //最大testdiv取得
            knjCreateHidden($objForm, "TESTDIV", $model->testdiv);

            //受付データ取得
            $Row2 = $db->getRow(knjl090qQuery::getRecept($model), DB_FETCHMODE_ASSOC);
        }
        ...
        //受験科目取得
        $counter = 1;
        $testsubclasscd = array();
        $result = $db->query((SCHOOLKIND == "P")?knjl090qQuery::getTestSubclasscdP($model->year):knjl090qQuery::getTestSubclasscd($model->year));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $arg["data"]["TESTSUBCLASSCD".$counter]  = $row["NAME2"];
            $testsubclasscd[$row["NAMECD2"]] = $counter;
            $counter++;
        }
        $result->free();

        //志願者得点データ
        $result = $db->query(knjl090qQuery::getScore($model));
        while($Row4 = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $counter = $testsubclasscd[$Row4["TESTSUBCLASSCD"]];
            $arg["data"]["SCORE".$counter]  = ($Row4["ATTEND_FLG"] == '0') ? "＊" : $Row4["SCORE"];
        }
        $result->free();

        //判定名称
        $result = $db->query(knjl090qQuery::getName($model->year, array("L013")));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
/*********************************************************** 表示 *************************************************************/
        ...
        //入試制度
        $query = knjl090qQuery::get_name_cd($model->year, "L003");
        ...
        $arg["data"]["EXAMNO"] = knjCreateTextBox($objForm, $model->examno, "EXAMNO", $examNoLength, $examNoLength, $extra);

        if ($model->examno != "") {
            //受験区分取得したい
            $kubunRow = $db->getRow(knjl090qQuery::getKubun($model), DB_FETCHMODE_ASSOC);
            $arg["data"]["KUBUN"] = $kubunRow["KUBUN_EXAMCOURSE"]."　　".$kubunRow["KUBUN_TESTDIV"];
            knjCreateHidden($objForm, "KUBUN", $arg["data"]["KUBUN"]);
        }

        //合否コンボ
        knjl090qForm1::FormatOpt($opt["L013"]);
        if (strlen($model->examno)) $opt["L013"] = knjl090qForm1::GetOpt($db, $model->year, array("L013"));
        $arg["data"]["JUDGEMENT"] = knjl090qForm1::CreateCombo($objForm, "JUDGEMENT", $Row["JUDGEMENT"], "100", $opt["L013"], "onChange=\"change_flg(), btn_submit('change')\";");
        $model->judgement = $Row["JUDGEMENT"];
...
/**************************************** CREATE 15 COMBOS ! ************************************************************/
        ...
        knjl090qForm1::FormatOpt($opt["L006"]);
        knjl090qForm1::FormatOpt($opt["L011"]);
        knjl090qForm1::FormatOpt($opt["L012"]);
        ...
        if ($judgeNameSpare1[$model->judgement] == '1') {
            ...
            //手続区分に追加
            $opt["L011"] = knjl090qForm1::GetOpt($db, $model->year, array("L011"));
            ...
            if ($model->procedurediv == "1") {
                ...
                //手続日を編集可能
                $disabled_date = "";
                $opt["L006"] = knjl090qForm1::GetOpt($db, $model->year, array("L006"));
                $opt["L012"] = knjl090qForm1::GetOpt($db, $model->year, array("L012"));
        ...
        //手続区分コンボ
        $arg["data"]["PROCEDUREDIV"] = knjl090qForm1::CreateCombo($objForm, "PROCEDUREDIV", $model->procedurediv, "100", $opt["L011"], "onChange=\"change_flg(), btn_submit('change')\";");

        //手続日
        $arg["data"]["PROCEDUREDATE"] = View::popUpCalendar2($objForm, "PROCEDUREDATE", str_replace("-", "/", $Row["PROCEDUREDATE"]), "", "", $disabled_date);

        //入学区分コンボ
        $arg["data"]["ENTDIV"] = knjl090qForm1::CreateCombo($objForm, "ENTDIV", $model->entdiv, "100", $opt["L012"], "onChange=\"change_flg(), btn_submit('change')\";");
...
/***************************************************** CREATE BUTTONS ********************************************************/
        ...
        global $sess;
        //かな検索ボタン
        $extra = "style=\"width:80px\" onclick=\"loadwindow('" .REQUESTROOT ."/L/KNJL090Q/search_name.php?cmd=search&year='+document.forms[0]['year'].value+'&applicantdiv='+document.forms[0]['APPLICANTDIV'].value+'&examno='+document.forms[0]['EXAMNO'].value+'&NAME_SESSID={$sess->id}&frame='+getFrameName(self), event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 260)\"";
        ...
        View::toHTML($model, "knjl090qForm1.html", $arg);
    }
    ...
    function GetOpt(&$db, $year, $namecd, $flg=1, $namecd2="", $namecd3="") {
        ...
        if (is_array($namecd)) {
            $result = $db->query(knjl090qQuery::getName($year, $namecd));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
            {
