...
class knjl509iForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjl509iindex.php", "", "edit");
        ...
        $query = knjl509iQuery::selectYearQuery($model);
        ...
        //テーブルの中身の作成
        $query = knjl509iQuery::selectQuery($model);
        ...
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            $hash = array("cmd"           => "edit2",
                          ...
                          "GENERAL_MARK"  => $row["GENERAL_MARK"] );

            $row["GENERAL_CD"] = View::alink("knjl509iindex.php", $row["GENERAL_CD"], "target=\"right_frame\"", $hash);
            $arg["data"][] = $row;
        }
        $result->free();
        ...
        if (!isset($model->warning) && VARS::post("cmd") == "copy") {
            $arg["reload"][] = "parent.right_frame.location.href='knjl509iindex.php?cmd=edit"
                             . "&year=" .$model->year."';";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl509iForm1.html", $arg);
    }
}
