...
class knjl325rForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl325rForm1", "POST", "knjl325rindex.php", "", "knjl325rForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl325rQuery::getNameMst($model, $model->ObjYear, "L003"));
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl325rQuery::getNameMst($model, $model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL325R");
        ...
        View::toHTML($model, "knjl325rForm1.html", $arg); 
