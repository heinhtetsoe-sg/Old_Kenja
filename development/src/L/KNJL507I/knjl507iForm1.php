...
class knjl507iForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjl507iindex.php", "", "edit");
        ...
        $query = knjl507iQuery::selectYearQuery($model);
        ...
        //テーブルの中身の作成
        $query = knjl507iQuery::selectQuery($model);
        ...
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            $row["GENERAL_CD"] = View::alink("knjl507iindex.php", $row["GENERAL_CD"], "target=\"right_frame\"", $hash);
            $arg["data"][] = $row;
        }
        $result->free();
        ...
        if (!isset($model->warning) && VARS::post("cmd") == "copy") {
            $arg["reload"][] = "parent.right_frame.location.href='knjl507iindex.php?cmd=edit"
                             . "&year=" .$model->year."';";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl507iForm1.html", $arg);
    }
}
