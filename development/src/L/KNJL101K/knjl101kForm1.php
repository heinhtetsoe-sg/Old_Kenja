...
class knjl101kForm1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjl101kindex.php", "", "main");
        $db           = Query::dbCheckOut();

        //ヘッダ
        $result = $db->query(knjl101kQuery::getName(array("L003","L006"),$model->year));
        $opt = array();
        $opt["L003"] = array();
        $opt["L006"] = array();
        ...
        if ($model->cmd == "main"){
            ...
            //合格点マスタ
            $query = knjl101kQuery::selectQueryPassingmark($model);
        ...
        //学科コース取得
        $result = $db->query(knjl101kQuery::selectQueryCourse($model));
        $opt = array();
        ...
        if ($model->cmd == "main"){
            ...
            //合格者取得
            $query = knjl101kQuery::selectQuerySuccess_cnt($model);
            $row = $db->getRow($query, DB_FETCHMODE_ASSOC);
...
#                $arg["CAPA_CNT"]  = floor((float) $model->field["BACK_RATE"] * (float) $row["SUCCESS_CNT"]/100);
        ...
        $result = $db->query(knjl101kQuery::selectQueryPassingmarkAll($model));
        $arg["data"] = array();
        $sum = array();
        ...
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            ...
            $cd = $row["COURSECD"] .$row["MAJORCD"].$row["EXAMCOURSECD"];
            $row["COURSE"] = $cd;
            $row["BORDER_SCORE1"] = View::alink("knjl101kindex.php" ,$row["BORDER_SCORE1"],"",
                                    array("cmd"         => "edit",
            ...
            $row["BORDER_SCORE2"] = View::alink("knjl101kindex.php" ,$row["BORDER_SCORE2"],"",
                                    array("cmd"         => "edit",
                                        "ENTEXAMYEAR"   => $row["ENTEXAMYEAR"],
                                        "TESTDIV"       => $row["TESTDIV"],
                                        "SHDIV"         => '2',
...
//            $row["INCREASE"] = ((int) $row["S_SUCCESS_CNT"]+(int) $row["CAPA_CNT1"]+(int) $row["CAPA_CNT2"])-(int) $row["CAPACITY"];   #2005/12/27
		...
		$db = Query::dbCheckOut();
		$row = $db->getOne(knjl101kQuery::GetJorH());
		if ($row == 0){
	        $objForm->ae( array("type"      => "button",
    	                        "name"      => "btn_check",
        	                    "value"     => "得点分布表出力",
        ...
        View::toHTML($model, "knjl101kForm1.html", $arg); 
    }
}
?>
