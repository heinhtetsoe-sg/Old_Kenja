...
class knjl384qForm1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjl384qindex.php", "", "edit");
        ...
        //コピー用年度コンボ
        $yearQuery = knjl384qQuery::getCopyYear();
        ...
        if ($model->cmd == "changeMockyear") {
            $arg["reload"] = "window.open('knjl384qindex.php?cmd=edit', 'right_frame')";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。 
        View::toHTML($model, "knjl384qForm1.html", $arg); 
    }
}
...
{
    $result = $db->query(knjl384qQuery::getList($model));
