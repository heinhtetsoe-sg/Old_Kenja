...
class knjl303uForm1 {
    function main(&$model) {
        $objForm = new form;
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl303uForm1", "POST", "knjl303uindex.php", "", "knjl303uForm1");
        ...
        //入試制度コンボ
        $query = knjl303uQuery::getNameMst($model->examyear, "L003");
        $extra = "onChange=\"return btn_submit('knjl303u')\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["APPLICANTDIV"], "APPLICANTDIV", $extra, 1, "TOP", "");

        //入試区分
        $query = knjl303uQuery::getNameMst($model->examyear, "L004");
        $extra = "onChange=\"return btn_submit('knjl303u')\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["TESTDIV"], "TESTDIV", $extra, 1, "TOP", "");

        //会場一覧リストToリスト
        $row1 = array();
        $query = knjl303uQuery::getHallData($model);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL303U");
        ...
        View::toHTML($model, "knjl303uForm1.html", $arg);
    }
}
//makeCmb
