...
class knjl347kForm1
{
    ...
    $objForm = new form;
    //フォーム作成
    $arg["start"]   = $objForm->get_start("knjl347kForm1", "POST", "knjl347kindex.php", "", "knjl347kForm1");
    $db = Query::dbCheckOut();
    ...
    $result = $db->query(knjl347kQuery::GetTestdiv($model));
    ...
    $opt = array();
    $value_flg = false;
    $query = knjl347kQuery::getSpecialReasonDiv($model);
    ...
    //中高判別フラグを作成する
    $jhflg = 0;
    $row = $db->getOne(knjl347kQuery::GetJorH());
    if ($row == 1){
        $jhflg = 1;
    }else {
        $jhflg = 2;
    ...
    View::toHTML($model, "knjl347kForm1.html", $arg);
    }
}
?>
