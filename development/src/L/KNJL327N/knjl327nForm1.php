...
class knjl327nForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl327nForm1", "POST", "knjl327nindex.php", "", "knjl327nForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl327nQuery::getNameMst($model->ObjYear, "L003"));
        ...
        $extra = "onchange=\"return btn_submit('knjl327n');\"";
        $arg["data"]["APPLICANTDIV"] = knjCreateCombo($objForm, "APPLICANTDIV", $model->field["APPLICANTDIV"], $opt, $extra, 1);

        //入試区分
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl327nQuery::getNameMst($model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        //帳票種類ラジオボタン 1:結果報告 2:封筒(角3) 3:ラベル 4:入学確約書
        $opt_kind = array(1, 2, 3, 4);
        $model->field["OUTPUT_KIND"] = ($model->field["OUTPUT_KIND"] == "") ? "1" : $model->field["OUTPUT_KIND"];
        $click = " onclick =\" return chkOutputDiv();\"";
        $extra = array("id=\"OUTPUT_KIND1\"".$click, "id=\"OUTPUT_KIND2\"".$click, "id=\"OUTPUT_KIND3\"".$click, "id=\"OUTPUT_KIND4\"".$click);
        $radioArray = knjCreateRadio($objForm, "OUTPUT_KIND", $model->field["OUTPUT_KIND"], $extra, $opt_kind, get_count($opt_kind));
        foreach($radioArray as $key => $val) $arg["data"][$key] = $val;

        //対象者ラジオボタン 1:全員 2:合格者 3:不合格者 4:志願者
        $opt_output = array(1, 2, 3, 4);
        $model->field["OUTPUT_DIV"] = ($model->field["OUTPUT_DIV"] == "") ? "1" : $model->field["OUTPUT_DIV"];
        $click = "";
        $extra = array("id=\"OUTPUT_DIV1\"".$click, "id=\"OUTPUT_DIV2\"".$click, "id=\"OUTPUT_DIV3\"".$click, "id=\"OUTPUT_DIV4\"".$click);
        $radioArray = knjCreateRadio($objForm, "OUTPUT_DIV", $model->field["OUTPUT_DIV"], $extra, $opt_output, get_count($opt_output));
        foreach($radioArray as $key => $val) $arg["data"][$key] = $val;

        //出力範囲ラジオボタン 1:全員 2:指定
        $opt_output = array(1, 2);
        $model->field["OUTPUT"] = ($model->field["OUTPUT"] == "") ? "1" : $model->field["OUTPUT"];
        $click = " onclick =\" return btn_submit('knjl327n');\"";
        ...
        $arg["data"]["EXAMNO"] = knjCreateTextBox($objForm, $value, "EXAMNO", 4, 4, $extra);
        ...
        View::toHTML($model, "knjl327nForm1.html", $arg); 
    }
}
...
function makeHidden(&$objForm, $model) {
    knjCreateHidden($objForm, "PRGID", "KNJL327N");
