...
class knjl015yForm1 {
    ...
    function main(&$model) {
        ...
        //データ取込
        //学校種別
        $query = knjl015yQuery::getNameMst($model->ObjYear, "L003");
        $extra = " onChange=\"btn_submit('chgAppAppDiv')\"";
        makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->field["APPLICANTDIV"], $extra, 1, "");

        //入試区分1
        $model->field["TESTDIV"] = ($model->cmd == "chgAppAppDiv") ? "" : $model->field["TESTDIV"];
        $namecd1 = ($model->field["APPLICANTDIV"] == "1") ? "L024" : "L004"; 
        $query = knjl015yQuery::getNameMst($model->ObjYear, $namecd1);
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjl015yindex.php", "", "main");
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl015yForm1.html", $arg);
    }
}
