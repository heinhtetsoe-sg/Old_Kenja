...
class knjl315rForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl315rForm1", "POST", "knjl315rindex.php", "", "knjl315rForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl315rQuery::getNameMst($model, $model->ObjYear, "L003"));
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl315rQuery::getNameMst($model, $model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL315R");
        ...
        View::toHTML($model, "knjl315rForm1.html", $arg); 
