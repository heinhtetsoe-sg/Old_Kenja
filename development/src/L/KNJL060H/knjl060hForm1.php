...
class knjl060hForm1
{
    function main(&$model)
    {
        $db = Query::dbCheckOut();
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("main", "POST", "knjl060hindex.php", "", "main");
        ...
        //入試制度
        $opt = array();
        $result = $db->query(knjl060hQuery::GetName("L003", $model->examyear));
        ...
        //入試区分
        $opt = array();
        $result = $db->query(knjl060hQuery::GetName("L004", $model->examyear));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            $opt[]    = array("label" => $row["NAMECD2"]."：".$row["NAME1"], "value" => $row["NAMECD2"]);
        ...
        View::toHTML($model, "knjl060hForm1.html", $arg);
    }
}
?>
