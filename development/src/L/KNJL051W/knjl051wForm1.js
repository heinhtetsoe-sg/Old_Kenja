function btn_submit(cmd) {
    ...
    //終了
    if (cmd == 'end') {
        if ((document.forms[0].APPLICANTDIV.disabled) || (document.forms[0].TESTDIV.disabled) || (document.forms[0].KAIJI_SUBCLASSCD.disabled)) {
...
function Setflg(obj){
    ...
    document.forms[0].HID_KAIJI_SUBCLASSCD.value = document.forms[0].KAIJI_SUBCLASSCD.options[document.forms[0].KAIJI_SUBCLASSCD.selectedIndex].value;

    document.forms[0].APPLICANTDIV.disabled = true;
    document.forms[0].TESTDIV.disabled = true;
    document.forms[0].KAIJI_SUBCLASSCD.disabled = true;
            ...
            if (startFlg) {
                ...
                if (clipTextArray[gyouCnt][retuCnt] != undefined) { //対象となるデータがあれば
                    ...
                    if (targetObject) { //テキストボックスがあれば(テキストボックスはあったりなかったりする)
                        checkTxt = clipTextArray[gyouCnt][retuCnt];
                        if (checkTxt == "*") {
                        } else if (4 < encodeURIComponent(checkTxt).replace(/%../g,"x").length) {
                            alert('{rval MSG901}' + '\n【' + checkTxt + '】４バイト以下で入力してください。');
                            return false;
                        } else if (checkTxt > eval(aPerfect[targetObject.id])) {
