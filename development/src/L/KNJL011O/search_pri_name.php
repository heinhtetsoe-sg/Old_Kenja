...
class SearchNames
{
    ...
    function getNameList(){
        ...
        if (strlen($this->names) || strlen($this->name_kana) || strlen($this->schoolcd)){
            $db = Query::dbCheckOut();
            $query  = "";
            $query .= " SELECT  T1.PRISCHOOLCD, T1.PRISCHOOL_NAME, T1.PRISCHOOL_KANA ";
            $query .= "   FROM  PRISCHOOL_MST T1, ";
            $query .= "         PRISCHOOL_YDAT T2 ";
            $query .= "  WHERE  T2.YEAR = '".$this->year."' ";
            $query .= "    AND  T2.PRISCHOOLCD = T1.PRISCHOOLCD ";
            if ($this->cmd == "apply"){
                $query .= " AND T2.PRISCHOOLCD = '" .sprintf("%07d", $this->schoolcd) ."' ";
            } else {
                if (strlen($this->names)){
                    $query .= " AND T1.PRISCHOOL_NAME like '%" .$this->names2 ."%' ";
                }
                if (strlen($this->name_kana)){
                    $query .= " AND T1.PRISCHOOL_KANA like '%" .$this->name_kana2 ."%' ";
                }
            }
            ...
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
                $opt[] = array("label" => $row["PRISCHOOLCD"]."　".$row["PRISCHOOL_NAME"]."　(".$row["PRISCHOOL_KANA"].")",
        ...
        $this->form->ae( array("type"       => "select",
                               ...
                               "size"       => "20",
                               "value"      => "",
                               "extrahtml"   => " ondblclick=\"apply_name(this, '".$this->flg."');\" STYLE=\"WIDTH:100%\"",
    ...
    function getBtnSearch(){
     ...
     //フォームタブ(開始)
    function getStart(){
        return $this->form->get_start("name", "POST", "search_pri_name.php", "", "name");
    }
    //フォームタブ(終了)
    function getFinish(){
...
function apply_name(obj, flg) {
    if(obj.selectedIndex >= 0){
        var val = obj.options[obj.selectedIndex].text;    
        var arr = val.split("　");
        if (flg == '2') {
            f.document.forms[0]['PS_CD2'].value = arr[0];
            f.document.getElementById("PS_NAME_ID2").innerHTML = arr[1];
        } else {
            f.document.forms[0]['PS_CD'].value = arr[0];
            f.document.getElementById("PS_NAME_ID").innerHTML = arr[1];
        }
        f.closeit();
    }else{
        alert("名前が選択されていません");
        ...
        <tr class="no_search_line">
          <td>
            <table width="100%" border="0" cellspacing="1" cellpadding="2">
              <tr class="no_search">
                <th nowrap width="40%" align="right">&nbsp;塾名&nbsp;</th>
                <td width="*" bgcolor="#ffffff"><?php echo $objName->getTextName() ?></td>
              </tr>
              <tr class="no_search">
                <th nowrap width="40%">&nbsp;塾名かな&nbsp;</th>
...
if ($_GET["flg"] == "2"){
?>
        f.document.getElementById("PS_NAME_ID2").innerHTML = "";
<?php
} else {
?>
        f.document.getElementById("PS_NAME_ID").innerHTML = "";
<?php
}
?>
    }
