...
class knjl422Form1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjl422index.php", "", "edit");
        ...
        //分類
        $query = knjl422Query::getRecruitClass($model);
        $extra = "onchange=\"return btn_submit('chgYear');\"";
        ...
        $this_cnt = $db->getOne(knjl422Query::getRecruitEventYmstCopy($model->leftYear, "cnt"));
        knjCreateHidden($objForm, "THIS_YEAR_CNT", $this_cnt);

        $preYear = $model->leftYear-1;
        $pre_cnt = $db->getOne(knjl422Query::getRecruitEventYmstCopy($preYear, "cnt"));
        ...
        if ($model->cmd == "chgYear") {
            $model->leftEventCd = "";
            $arg["reload"] = "parent.right_frame.location.href='knjl422index.php?cmd=edit';";
        }

        $arg["finish"] = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl422Form1.html", $arg); 
    }
}
//makeCmb
...
function makeList(&$arg, $db, $model) {
    $result = $db->query(knjl422Query::getList($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
