...
class knjl422Form2
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjl422index.php", "", "edit");
        ...
        if ($model->cmd == "reset" || $model->cmd == "linkClick") {
            $query = knjl422Query::getRecruitEventYmst($model);
        ...
        if (VARS::get("cmd") != "edit" && VARS::get("cmd") != "linkClick"){
            $arg["reload"]  = "parent.left_frame.location.href='knjl422index.php?cmd=list';";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl422Form2.html", $arg); 
    }
}
