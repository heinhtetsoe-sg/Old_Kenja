...
class knjl233yForm2
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjl233yindex.php", "", "edit");
        ...
        if (!isset($model->warning) && ($model->cmd == "edit2") && $model->applicantdiv && $model->judge_kind != ""){
            $query = knjl233yQuery::getRow($model->applicantdiv, $model->judge_kind);
        ...
        $query = knjl233yQuery::getNameMst($model, "L003", "APPLICANTDIV");
        ...
        //入試制度コンボ
        $extra = "onchange=\"return btn_submit('edit');\"";
        $query = knjl233yQuery::getNameMst($model, "L025", "JUDGE_KIND");
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "parent.left_frame.location.href='knjl233yindex.php?cmd=list';";
        }
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl233yForm2.html", $arg);
    }
}
