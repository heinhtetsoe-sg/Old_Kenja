...
class knjl343kForm1
{
    ...
    //フォーム作成
    $arg["start"]   = $objForm->get_start("knjl343kForm1", "POST", "knjl343kindex.php", "", "knjl343kForm1");
    $db = Query::dbCheckOut();
    ...
    $result = $db->query(knjl343kQuery::GetTestdiv($model));
    ...
    $objForm->ae( array("type"      => "select",
                        ...
                        "value"     => $model->testdiv,
                        "extrahtml" => "",
//                      "extrahtml" => "onchange=\" return btn_submit('knjl343k');\"",
                        "options"   => $opt_testdiv ) );
    ...
    $opt = array();
    $value_flg = false;
    $query = knjl343kQuery::getSpecialReasonDiv($model);
    ...
    $query = knjl343kQuery::GetPrischool($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        $opt_data[] = array("label" => $row["PRISCHOOLCD"]."　".$row["PRISCHOOL_NAME"],
    ...
    //中高判別フラグを作成する
    $jhflg = 0;
    $row = $db->getOne(knjl343kQuery::GetJorH());
    if ($row == 1){
        $jhflg = 1;
    }else {
        $jhflg = 2;
    ...
    View::toHTML($model, "knjl343kForm1.html", $arg); 
    }
}
?>
