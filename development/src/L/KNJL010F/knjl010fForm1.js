function btn_submit(cmd, zip, gzip, zadd, gadd) {
    ...
    //新規ボタン
    if (cmd == 'addnew') {
        if (document.forms[0].APPLICANTDIV.value == "" || document.forms[0].TEST_L_DIV.value == "") {
            alert('{rval MSG301}\n( 入試制度 )');
            return true;
        }
    }
...
//重複受験チェック
function disTestdiv(obj) {
    if (obj.name == 'TESTDIV16') {
        document.forms[0].TESTDIV9.disabled = obj.checked;
        document.forms[0].TESTDIV12.disabled = obj.checked;
        document.forms[0].TESTDIV18.disabled = obj.checked;
    }
    if (obj.name == 'TESTDIV9') {
        document.forms[0].TESTDIV16.disabled = obj.checked;
        document.forms[0].TESTDIV12.disabled = obj.checked;
        document.forms[0].TESTDIV18.disabled = obj.checked;
    }
    if (obj.name == 'TESTDIV12') {
        document.forms[0].TESTDIV9.disabled = obj.checked;
        document.forms[0].TESTDIV16.disabled = obj.checked;
        document.forms[0].TESTDIV18.disabled = obj.checked;
    }
    if (obj.name == 'TESTDIV18') {
        document.forms[0].TESTDIV9.disabled = obj.checked;
        document.forms[0].TESTDIV12.disabled = obj.checked;
        document.forms[0].TESTDIV16.disabled = obj.checked;
    }
    if (obj.name == 'TESTDIV10') {
        document.forms[0].TESTDIV13.disabled = obj.checked;
    }
    if (obj.name == 'TESTDIV13') {
        document.forms[0].TESTDIV10.disabled = obj.checked;
    }
}

//Ａ方式を選択した時、「算数、英語」のラジオボタンを入力不可にする。
//つまり、Ｂ方式を選択した時、有効にする。
function disSubcd(obj, testdiv) {
    subcd = eval("document.forms[0][\"TESTSUBCLASSCD" + testdiv + "\"]");
    subcd[0].disabled = (obj.value == '5') ? false : true;
    subcd[1].disabled = (obj.value == '5') ? false : true;
}
