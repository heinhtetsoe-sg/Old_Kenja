...
class knjl034nForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl034nForm1", "POST", "knjl034nindex.php", "", "knjl034nForm1");
        ...
        //履歴一覧
        $query = knjl034nQuery::getList($model);
        ...
        while ($rirekiRow = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            if ($rirekiRow["NORMAL_PASSCOURSECD_NAME_SET"]) {
                $rirekiRow["NORMAL_PASSCOURSECD_NAME"] = $db->getOne(knjl034nQuery::getNameMst($model->ObjYear, "SEIKI", $model, $rirekiRow["NORMAL_PASSCOURSECD_NAME_SET"]));
            }
            if ($rirekiRow["EARLY_PASSCOURSECD_NAME_SET"]) {
                $rirekiRow["EARLY_PASSCOURSECD_NAME"] = $db->getOne(knjl034nQuery::getNameMst($model->ObjYear, "TANGAN", $model, $rirekiRow["EARLY_PASSCOURSECD_NAME_SET"]));
            }
        ...
        if (!isset($model->warning) && ($model->cmd == "select")) {
            $query = knjl034nQuery::getList($model, "select");
        ...
        //合否区分の名称
        $extra = "";
        $arg["data"]["JUDGMENT_COURSE_NAME"] = knjCreateTextBox($objForm, $Row["JUDGMENT_COURSE_NAME"], "JUDGMENT_COURSE_NAME", 30, 30, $extra);
        ...
        $value_flg = false;
        $result = $db->query(knjl034nQuery::getNameMst($model->ObjYear, "SEIKI", $model, ""));
        ...
        $opt[] = array('label' => "", 'value' => "");
        $value_flg = false;
        $result = $db->query(knjl034nQuery::getNameMst($model->ObjYear, "TANGAN", $model, ""));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL034N");
        ...
        View::toHTML($model, "knjl034nForm1.html", $arg); 
