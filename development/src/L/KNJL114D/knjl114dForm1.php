...
class knjl114dForm1
{
    public function main(&$model)
    {
        ...
        $arg["data"]["YEAR"] = knjCreateCombo($objForm, "YEAR", $model->ObjYear, $opt, $extra, 1);

        //志願区分
        $query = knjl114dQuery::getNameMst($model->ObjYear, "L058");
        $extra = "";
        makeCmb($objForm, $arg, $db, $query, "DESIREDIV", $model->field["DESIREDIV"], $extra, 1);

        //入試区分
        $maxTestDiv = $db->getOne(knjl114dQuery::getMaxTestDiv($model->ObjYear));
        $model->field["TESTDIV"] = ($model->field["TESTDIV"]) ? $model->field["TESTDIV"]: $maxTestDiv;
        $query = knjl114dQuery::getNameMst($model->ObjYear, "L004");
        $extra = "";
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjl114dindex.php", "", "main");
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl114dForm1.html", $arg);
    }
}
