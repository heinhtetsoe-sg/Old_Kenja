function btn_submit(cmd) {
    ...
    //CSV
    if (cmd=="csv" && document.forms[0].csv[0].checked) {
        if (document.forms[0].PRO_DATE.value == '') {
            alert('日付を入力して下さい。');
            return false;
        }
    }
    
    if (cmd=="csv" && document.forms[0].csv[1].checked) {
        if (document.getElementById("TETUDUKI_ZUMI_CHECK").checked == false && document.getElementById("TETUDUKI_ALL_CHECK").checked == false) {
            alert('CSV書出を行う対象が選択されていません。');
            return false;
        }
    }
...
function check(obj) {
    ...
    if (getname == "TETUDUKI_ZUMI_CHECK") {
        if (document.getElementById(getname).checked == true) {
            document.getElementById("TETUDUKI_ALL_CHECK").checked = false;
        }
    } else if (getname == "TETUDUKI_ALL_CHECK") {
        if (document.getElementById(getname).checked == true) {
            document.getElementById("TETUDUKI_ZUMI_CHECK").checked = false;
        }
    }
}
