...
class knjl126iForm1
{
    public function main(&$model)
    {
        ...
        //入試制度
        $extra = "";
        $query = knjl126iQuery::getNameMst($model, "L003", "2");
        makeCmb($objForm, $arg, $db, $query, $model->field["APPLICANTDIV"], "APPLICANTDIV", $extra, 1, "");

        //入試区分コンボ
        $extra = "";
        $query = knjl126iQuery::getEntexamTestDivMst($model);
        makeCmb($objForm, $arg, $db, $query, $model->field["TESTDIV"], "TESTDIV", $extra, 1);

        //抽出ラジオボタン 3:全員 1:男子のみ 2:女子のみ
        $opt = array(1, 2, 3);
        if (!$model->field["SEX"]) {
            $model->field["SEX"] = 3;
        }
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjl126iindex.php", "", "main");
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl126iForm1.html", $arg);
    }
}
