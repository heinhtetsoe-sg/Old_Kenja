...
class knjl312bForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl312bForm1", "POST", "knjl312bindex.php", "", "knjl312bForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl312bQuery::getNameMst($model->ObjYear, "L003"));
        ...
        $extra = " onchange=\"return btn_submit('knjl312b');\"";
        $arg["data"]["APPLICANTDIV"] = knjCreateCombo($objForm, "APPLICANTDIV", $model->field["APPLICANTDIV"], $opt, $extra, 1);

        //入試区分
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl312bQuery::getNameMst($model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl312bQuery::getNameMst($model->ObjYear, "L009"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        //試験会場選択コンボボックスを作成する
        $result = $db->query(knjl312bQuery::getEntexamHallDat($model->ObjYear, $model->field["APPLICANTDIV"], $model->field["TESTDIV"]));
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL312B");
        ...
        View::toHTML($model, "knjl312bForm1.html", $arg); 
