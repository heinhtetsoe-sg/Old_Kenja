...
class knjl323rForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl323rForm1", "POST", "knjl323rindex.php", "", "knjl323rForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl323rQuery::getNameMst($model, $model->ObjYear, "L003"));
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl323rQuery::getNameMst($model, $model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL323R");
        ...
        View::toHTML($model, "knjl323rForm1.html", $arg); 
