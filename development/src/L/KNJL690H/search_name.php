...
class SearchNames
{
    ...
    function getNameList(){
        ...
        if (strlen($this->names) || strlen($this->name_kana)){
            ...
            $query .= "WHERE R1.ENTEXAMYEAR  = '".$this->year."'";
            $query .= "  AND R1.APPLICANTDIV = '".$this->applicantdiv."'";
            // $query .= "  AND R1.TESTDIV      = '".$this->testdiv."'";
            $query .= "  AND R1.EXAM_TYPE    = '".$this->examtype."'";
            ...
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
                $opt[] = array("label" => $row["RECEPTNO"]."　".$row["NAME"]."　(".$row["NAME_KANA"].")",
                               "value" => $row["RECEPTNO"]
                               );
            }
            Query::dbCheckIn($db);
        ...
        $this->form->ae( array("type"       => "select",
                               ...
                               "size"       => "11",
                               "value"      => "",
                               "extrahtml"   => " ondblclick=\"apply_name(this,'".$this->receptno."','".$this->zip."','".$this->zadd."');\" STYLE=\"WIDTH:100%\"",
    ...
    function main($rq){
        ...
        // }
        if ($rq["examtype"]){
            $this->examtype = $rq["examtype"];
        }
        if ($rq["receptno"]){
            $this->receptno = $rq["receptno"];
            
            $query  = "SELECT T1.ZIPCD, T1.ADDRESS1, T1.GZIPCD, T1.GADDRESS1 ";
            $query .= "FROM ";
            $query .= "     ENTEXAM_RECEPT_DAT R1 ";
            $query .= "     INNER JOIN ENTEXAM_APPLICANTADDR_DAT T1 ";
            $query .= "             ON R1.ENTEXAMYEAR  = T1.ENTEXAMYEAR ";
            $query .= "            AND R1.APPLICANTDIV = T1.APPLICANTDIV ";
            $query .= "            AND R1.EXAMNO       = T1.EXAMNO ";
            $query .= "WHERE R1.ENTEXAMYEAR  = '".$this->year."'";
            ...
            $query .= "  AND R1.TESTDIV      = '".$this->testdiv."'";
            $query .= "  AND R1.EXAM_TYPE    = '".$this->examtype."'";
            $query .= "  AND R1.RECEPTNO     = '".$this->receptno."'";
...
function apply_name(obj,receptno,zip,zadd) {
    if(obj.selectedIndex >= 0){
        var val = obj.options[obj.selectedIndex].text;    
        var arr = val.split("　");
        f.document.forms[0]['RECEPTNO'].value = arr[0];
        if(f.btn_submit('reference',zip,zadd) != true) {
            f.closeit();
        } else {
            f.document.forms[0]['RECEPTNO'].value = receptno;
        }
    }else{
        alert("名前が選択されていません");
    }
