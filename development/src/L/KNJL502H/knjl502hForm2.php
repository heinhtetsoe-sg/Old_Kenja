...
class knjl502hForm2 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjl502hindex.php", "", "edit");
        ...
        if (!isset($model->warning) && ($model->cmd == "edit2" || $model->cmd == "reset") && $model->year && $model->applicantdiv && $model->examType) {
            $query = knjl502hQuery::getRow($model->year, $model->applicantdiv, $model->examType);
            $Row = $db->getRow($query, DB_FETCHMODE_ASSOC);
            $query  = knjl502hQuery::getRow2($model->year, $model->applicantdiv, $model->examType);
            $result = $db->query($query);
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                $Row["SUBCLASS_".$row["SUBCLASSCD"]] = $row["SUBCLASSCD"];
            }
        } else {
            $Row =& $model->field;
        }
        ...
        if ($model->applicantdiv == "") {
            ...
            $query = knjl502hQuery::getNameMst($model, "L003");
            makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->applicantdiv, $extra, 1, "");
        }
        $arg["data"]["APPLICANTDIV"] = $db->getOne(knjl502hQuery::getNameMst($model, "L003", $model->applicantdiv));
        ...
        if (VARS::get("cmd") != "edit") {
            $arg["reload"]  = "parent.left_frame.location.href='knjl502hindex.php?cmd=list2"
                            . "&year=".$model->year."&applicantdiv=".$model->applicantdiv."';";

        }
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl502hForm2.html", $arg);
    }
}
//makeCmb
