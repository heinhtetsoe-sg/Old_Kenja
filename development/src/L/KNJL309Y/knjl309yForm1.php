...
class knjl309yForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjl309yForm1", "POST", "knjl309yindex.php", "", "knjl309yForm1");
        ...
        //入試制度コンボボックス
        $extra = "onchange=\"return btn_submit('knjl309y');\"";
        $query = knjl309yQuery::getNameMst("L003", $model->ObjYear);
        ...
        $query = knjl309yQuery::getNameMst($namecd, $model->ObjYear);
        ...
        //特別判定希望者のみ出力チェックボックス
        $extra  = ($model->field["SHIFT_DESIRE_FLG"] == "1") ? "checked" : "";
        $extra .= " id=\"SHIFT_DESIRE_FLG\"";
        $extra .= ($model->field["APPLICANTDIV"] == "2" && $model->field["TESTDIV"] == "1") ? "" : " disabled";
        ...
        View::toHTML($model, "knjl309yForm1.html", $arg); 
    }
}
...
{
    $query = knjl309yQuery::getExamHall($model);
    $result = $db->query($query);
    $opt = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $s_examno = $db->getOne(knjl309yQuery::getExamNo($model, $row["S_RECEPTNO"]));
        $e_examno = $db->getOne(knjl309yQuery::getExamNo($model, $row["E_RECEPTNO"]));
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJL309Y");
