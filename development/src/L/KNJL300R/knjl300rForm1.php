...
class knjl300rForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl300rForm1", "POST", "knjl300rindex.php", "", "knjl300rForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl300rQuery::getNameMst($model, "L003"));
        ...
        $extra = " onchange=\"return btn_submit('knjl300r');\"";
        ...
        if ($model->field["APPLICANTDIV"] == "2") {
            $result = $db->query(knjl300rQuery::getNameMst($model, "L024"));
        } else {
            $result = $db->query(knjl300rQuery::getNameMst($model, "L004"));
        }
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        //試験会場選択コンボボックスを作成する
        $result = $db->query(knjl300rQuery::getEntexamHallDat($model->ObjYear, $model->field["APPLICANTDIV"], $model->field["TESTDIV"]));
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL300R");
        ...
        View::toHTML($model, "knjl300rForm1.html", $arg); 
