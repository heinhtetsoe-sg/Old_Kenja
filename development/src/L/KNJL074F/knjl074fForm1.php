...
class knjl074fForm1
{
    function main(&$model) {
        ...
        $query = knjl074fQuery::getNameMst("L003", $model->year, "1");
        makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->applicantdiv, $extra, 1);

        //入試区分コンボ
        $extra = "onchange=\"return btn_submit('main');\" tabindex=-1";
        $query = knjl074fQuery::getNameMst("L024", $model->year);
        makeCmb($objForm, $arg, $db, $query, "TESTDIV", $model->testdiv, $extra, 1);

        //受験型コンボ
        $extra = "onchange=\"return btn_submit('main');\"";
        $query = knjl074fQuery::getExamType($model->year, $model->testdiv);
        ...
        if ($model->testdiv == "5") {
            //対象者ラジオボタン 1:すべて 2:第５回入試対象者 3:特別入試対象者
            $opt = array(1, 2, 3);
        ...
        if ($model->applicantdiv != "" && $model->testdiv != "") {
            ...
            //データ取得
            $result = $db->query(knjl074fQuery::SelectQuery($model));
            ...
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                ...
                $query = knjl074fQuery::getJudgeKind($model->year, $model->applicantdiv);
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjl074findex.php", "", "main");

        //フォーム終わり
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl074fForm1.html", $arg);
    }
}
