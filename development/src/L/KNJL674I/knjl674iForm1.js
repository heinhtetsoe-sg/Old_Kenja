...
function checkHonorCmb(obj) {
    if (document.forms[0].HONORDIV.value == '' && document.forms[0].HONOR_REASONDIV.value == '') {
        alert('{rval MSG301}' + '( 特待コードと特待理由のどちらも空白には指定できません。 )');
        document.forms[0][obj.name].value = document.forms[0]['HID_' + obj.name].value;
        return false;
    }
    btn_submit('main');
}
...
function current_knjxfin_after_event() {
    //学校検索画面検索結果行ダブルクリック→FINSCHOOLCD設定後 (apply_finschoolgetParametr3()にて実行)
    //学校名のみ表示
