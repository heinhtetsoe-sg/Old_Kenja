...
class knjl210bForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjl210bindex.php", "", "main");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl210bQuery::getNameMst($model->ObjYear, "L003"));
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl210bQuery::getNameMst($model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL210B");
        ...
        View::toHTML($model, "knjl210bForm1.html", $arg);
    }
