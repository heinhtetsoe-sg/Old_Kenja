...
class knjl081fForm1
{
    function main(&$model)
    {
        ...
        //入試制度
        $query = knjl081fQuery::GetName("L003", $model->ObjYear);
        ...
        $result = $db->query(knjl081fQuery::GetName($namecd1, $model->ObjYear));
        ...
        if ($model->applicantdiv == "2") {
            $extra = "Onchange=\"btn_submit('main');\"";
            $query = knjl081fQuery::getTestdiv0($model->ObjYear, $model->testdiv);
        ...
        $arg["RIGHT_TITLE"] = "手続者一覧（辞退者は除く）";

        //対象者・合格者一覧
        $opt_left = $opt_right = array();
        $query = knjl081fQuery::GetLeftList($model);
        ...
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            if ($row["LEFT_FLG"] == "1") {
                ...
                //合格者一覧
                $query = knjl081fQuery::getPayMoney($model, $row["RECEPTNO"]);
        ...
        $arg["start"]   = $objForm->get_start("sel", "POST", "knjl081findex.php", "", "sel");

        $arg["finish"]  = $objForm->get_finish();

        View::toHTML($model, "knjl081fForm1.html", $arg); 
    }
}
