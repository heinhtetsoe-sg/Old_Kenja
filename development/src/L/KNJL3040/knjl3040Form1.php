...
class knjl3040Form1 {
    function main(&$model) {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjl3040index.php", "", "edit");
        ...
        //年度コンボ
        $query = knjl3040Query::getYear($model);
        ...
        $query = knjl3040Query::getNameMst($model->leftYear, "L003");
        ...
        //リスト作成
        $result = $db->query(knjl3040Query::getList($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
             ...
             $row["TESTDIV_NAME"] = View::alink("knjl3040index.php", $row["TESTDIV_NAME"], "target=\"right_frame\"", $hash);
             $arg["data"][] = $row;
        }
        ...
        if (!isset($model->warning) && $flg) {
            $arg["reload"]= "parent.right_frame.location.href='knjl3040index.php?cmd=edit"
                             . "&ENTEXAMYEAR=".$model->examyear."&APPLICANTDIV=".$model->applicantdiv."&TESTDIV=".$model->testdiv."';";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。 
        View::toHTML($model, "knjl3040Form1.html", $arg); 
    }
}
