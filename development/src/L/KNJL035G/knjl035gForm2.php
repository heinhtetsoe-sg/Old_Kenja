...
class knjl035gForm2 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjl035gindex.php", "", "edit");
        ...
        if (!isset($model->warning) && ($model->cmd == "edit2" || $model->cmd == "reset") && $model->year && $model->applicantdiv && $model->item_cd && $model->exemption_cd) {
            $query = knjl035gQuery::getRow($model->year, $model->applicantdiv, $model->item_cd, $model->exemption_cd);
        ...
        if (!isset($model->warning) && $model->cmd != "edit" && $model->cmd != "change"){
            $arg["reload"]  = "parent.left_frame.location.href='knjl035gindex.php?cmd=list"
                            . "&year=" .$model->year."';";
        }
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl035gForm2.html", $arg);
    }
}
