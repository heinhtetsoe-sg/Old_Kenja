    ...
    function getNameList($index) {
        ...
        switch ($index) {
            case 0:
                ...
                $query .= "WHERE YEAR = '".$this->year."' AND";
                $query .= "      NAMECD1 = 'L003' AND";
                $query .= "      NAMECD2 = '".$this->testdiv."'";
                $row = $db->getOne($query);
            ...
            if (strlen($this->names) || strlen($this->name_kana)){
                ...
                $query .= "ORDER BY ACCEPTNO ";
                //受付番号、氏名、氏名かな取得
                $result = $db->query($query);
                while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
                    $opt[] = array("label" => $row["ACCEPTNO"]."　".$row["NAME"]."　(".$row["NAME_KANA"].")",
                                   "value" => $row["ACCEPTNO"]
                                   );
                }
            }
            $this->form->ae( array("type"       => "select",
                                   ...
                                   "size"       => "10",
                                   "value"      => "",
                                   "extrahtml"  => " ondblclick=\"apply_name(this,'".$this->acceptno."');\" STYLE=\"WIDTH:100%\"",
...
<link rel="stylesheet" href="../../common/gk.css">
<style type="text/css">
<!--
...
//function apply_name(obj,acceptno,zip,gzip,zadd,gadd) {
function apply_name(obj,acceptno) {
    if(obj.selectedIndex >= 0){
        var val = obj.options[obj.selectedIndex].text;    
        var arr = val.split("　");
        f.document.forms[0]['ACCEPTNO'].value = arr[0];
//        if(f.btn_submit('reference',zip,gzip,zadd,gadd) != true) {
        if(f.btn_submit('reference') != true) {
            f.closeit();
        } else {
            f.document.forms[0]['ACCEPTNO'].value = acceptno;
        }
    }else{
        alert("名前が選択されていません");
    }
        ...
        <tr class="no_search_line">
          <td>
            <table width="100%" border="0" cellspacing="1" cellpadding="2">
              <tr class="no_search">
                <th nowrap width="40%" align="right">&nbsp;試験区分&nbsp;</th>
                <td width="*" bgcolor="#ffffff" style="color:black";><?php echo $objName->getNameList(0) ?></td>
              </tr>
              <tr class="no_search">
              ...
              <tr class="no_search">
                <td colspan="2" bgcolor="#ffffff"><?php echo $objName->getNameList(1) ?></td>
              </tr>
            </table>
          </td>
        </tr>
