function btn_submit(cmd, zip, zadd) {
    ...
    if (cmd == "send_copy") {
        var sadd2 = document.forms[0].SEND_ADDRESS2.value;
        var stel = document.forms[0].SEND_TELNO.value;

        document.forms[0].SEND_ZIPCD.value = document.forms[0].ZIPCD.value;
        document.forms[0].SEND_ADDRESS1.value = document.forms[0].ADDRESS1.value;
        document.forms[0].SEND_ADDRESS2.value = document.forms[0].ADDRESS2.value;
        document.forms[0].SEND_TELNO.value = document.forms[0].TELNO.value;

        if (sadd2 != document.forms[0].SEND_ADDRESS2.value || stel != document.forms[0].SEND_TELNO.value) {
            vflg = true;
        }
        return false;
    }
...
function doSubmit(cmd) {
    if (cmd == "update") {
        if (
            document.forms[0].RECOM_EXAMNO.value != "" &&
            (document.forms[0].HID_NAME_KANA.value != document.forms[0].NAME_KANA.value ||
                document.forms[0].HID_BIRTHDAY.value != document.forms[0].BIRTHDAY.value ||
                document.forms[0].HID_TELNO.value != document.forms[0].TELNO.value)
        ) {
            if (!confirm("照合処理で設定された情報はクリアされます。\n更新してよろしいでしょうか？")) {
                return true;
            }
            document.forms[0].CLEAR_FLG.value = "1";
        }
    }
