...
class knjl307bForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl307bForm1", "POST", "knjl307bindex.php", "", "knjl307bForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl307bQuery::getNameMst($model->ObjYear, "L003"));
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl307bQuery::getNameMst($model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL307B");
        ...
        View::toHTML($model, "knjl307bForm1.html", $arg); 
