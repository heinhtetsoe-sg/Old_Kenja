...
class knjl091fForm1
{
    ...
    {
        $objForm      = new form;
        $arg["start"] = $objForm->get_start("main", "POST", "knjl091findex.php", "", "main");
        $db = Query::dbCheckOut();

/****************************************** GET DATA!! ***************************************************************/        

        $Row = $db->getRow(knjl091fQuery::get_edit_data($model), DB_FETCHMODE_ASSOC);
        if (!is_array($Row)) {
        ...
        if ($model->cmd == 'change_testdiv2' || $model->cmd == 'change' || $model->cmd == 'update' || (isset($model->warning))) {
            ...
            $model->testdiv2 = $db->getOne(knjl091fQuery::getMaxtestdiv($model, $Row["APPLICANTDIV"]));    //最大testdiv取得
            //合格した入試区分(baseの入試区分と同じ)があれば、それを初期表示
            //$testdiv2 = $db->getOne(knjl091fQuery::getMintestdiv($model, $Row["APPLICANTDIV"]));
            //if (strlen($testdiv2)) $model->testdiv2 = $testdiv2;
        }
        $Row2 = $db->getRow(knjl091fQuery::getRecept($model, $Row["APPLICANTDIV"]), DB_FETCHMODE_ASSOC);
        ...
        //志願者得点データ
        $result = $db->query(knjl091fQuery::getScore($model, $Row["APPLICANTDIV"]));
        ...
        //受験科目 判定名称
        $result = $db->query(knjl091fQuery::getName($model->year, array("L013","L009","L005")));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            if ($row["NAMECD1"]=="L005") $L005Name1[$row["NAMECD2"]] = $row["NAMECD2"]."：".$row["NAME1"];
            if ($row["NAMECD1"]=="L009" && $row["NAME2"] == "") continue;
            if ($row["NAMECD1"]=="L009") $arg["data"]["TESTSUBCLASSCD".$row["NAMECD2"]]  = $row["NAME2"];
...
/*********************************************************** 表示 *************************************************************/
        ...
        //入試制度
        $query = knjl091fQuery::get_name_cd($model->year, "L003", "2");
        ...
        //合否コンボ
        knjl091fForm1::FormatOpt($opt["L013"]);
        if (strlen($model->examno)) $opt["L013"] = knjl091fForm1::GetOpt($db, $model->year, array("L013"));
        $arg["data"]["JUDGEMENT"] = knjl091fForm1::CreateCombo($objForm, "JUDGEMENT", $Row["JUDGEMENT"], "100", $opt["L013"], "onChange=\"change_flg(), btn_submit('change')\";");
        $model->judgement = $Row["JUDGEMENT"];
...
/**************************************** CREATE 15 COMBOS ! ************************************************************/
        ...
        knjl091fForm1::FormatOpt($opt["L011"]);
        knjl091fForm1::FormatOpt($opt["L012"]);
        ...
        if ($judgeNameSpare1[$model->judgement] == '1') {
            ...
            //手続区分に追加
            $opt["L011"] = knjl091fForm1::GetOpt($db, $model->year, array("L011"));
            ...
            if ($model->procedurediv == "1") {
                $opt["L012"] = knjl091fForm1::GetOpt($db, $model->year, array("L012"));
        ...
        //合格コースコンボ
        knjl091fForm1::FormatOpt($opt_cmcd);
        if ($sucFlg == "1") {
            $query = knjl091fQuery::getSucCourse($model->year, $Row["APPLICANTDIV"]);
        ...
        $arg["data"]["COURSEMAJOR"] = knjl091fForm1::CreateCombo($objForm, "COURSEMAJOR", $Row["COURSEMAJOR"], "290", $opt_cmcd, "onChange=\"change_flg()\"");

        //入学コースコンボ
        knjl091fForm1::FormatOpt($opt_enter);
        if ($entFlg == "1") {
            $query = knjl091fQuery::getCourseMajorCoursecode($model->year, $Row["APPLICANTDIV"]);
            $result = $db->query($query);
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $arg["data"]["ENTER_COURSEMAJOR"] = knjl091fForm1::CreateCombo($objForm, "ENTER_COURSEMAJOR", $Row["ENTER_COURSEMAJOR"], "290", $opt_enter, "onChange=\"change_flg()\"");

        //手続区分コンボ
        $arg["data"]["PROCEDUREDIV"] = knjl091fForm1::CreateCombo($objForm, "PROCEDUREDIV", $model->procedurediv, "100", $opt["L011"], "onChange=\"change_flg(), btn_submit('change')\";");

        //入学区分コンボ
        $arg["data"]["ENTDIV"] = knjl091fForm1::CreateCombo($objForm, "ENTDIV", $model->entdiv, "100", $opt["L012"], "onChange=\"change_flg(), btn_submit('change')\";");
        ...
        //支度金
        //支度金フラグ（1:希望する）
        $extra  = "id=\"ENTRANCE_FLG\" ";
        $extra .= ($Row["ENTRANCE_FLG"] == "1") ? "checked " : "";
        $arg["data"]["ENTRANCE_FLG"] = knjCreateCheckBox($objForm, "ENTRANCE_FLG", "1", $extra);
        ...
        //入試区分コンボ
        $namecd1 = "L004";
        knjl091fForm1::FormatOpt($opt[$namecd1]);
        if (strlen($Row["EXAMNO"]) || strlen($Row2["EXAMNO"])) $opt[$namecd1] = knjl091fForm1::GetOpt($db, $model->year, array($namecd1), 0);
        $arg["data"]["TESTDIV2"] = knjl091fForm1::CreateCombo($objForm, "TESTDIV2", $model->testdiv2, "150", $opt[$namecd1], "onChange=\"btn_submit('change_testdiv2')\";");

        //入試回数(2:高校のみ)
        $query = knjl091fQuery::getTestdiv0($model->year, $model->testdiv2, $Row2["TESTDIV0"]);
...
/***************************************************** CREATE BUTTONS ********************************************************/
        ...
        global $sess;
        //かな検索ボタン
        $extra = "style=\"width:80px\" onclick=\"loadwindow('" .REQUESTROOT ."/L/KNJL091F/search_name.php?cmd=search&year='+document.forms[0]['year'].value+'&examno='+document.forms[0]['EXAMNO'].value+'&NAME_SESSID={$sess->id}&frame='+getFrameName(self), event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 260)\"";
        ...
        View::toHTML($model, "knjl091fForm1.html", $arg);
    }
    ...
    function GetOpt(&$db, $year, $namecd, $flg=1, $namecd2="", $namecd3="") {
        ...
        if (is_array($namecd)) {
            $result = $db->query(knjl091fQuery::getName($year, $namecd));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
            {
    ...
    function GetEnterOpt(&$db, $year, $Row) {
        ...
        if (is_array($Row)) {
            $result = $db->query(knjl091fQuery::getCourseMajorCoursecode($year, $Row["APPLICANTDIV"]));
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
            {
                //入学コース取得
