...
//        $arg["TOP"]["YEAR"] = CTRL_YEAR;
        ...
        //試験制度
        $opt = array();
        $result = $db->query(knjl040oQuery::GetName("L003",$model->ObjYear));
        ...
        //入試区分
        $opt = array();
        $result = $db->query(knjl040oQuery::getTestdivMst($model->ObjYear));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            if($row["NAMESPARE2"]=='1') {
                $opt[]    = array("label" => $row["NAMECD2"]."：".$row["NAME1"], "value" => $row["NAMECD2"]);
        ...
        //受験型
        $model->exam_type = "2";
        $exam_type = $db->getRow(knjl040oQuery::GetName("L005", $model->ObjYear, $model->exam_type), DB_FETCHMODE_ASSOC);
        knjCreateHidden($objForm, "EXAM_TYPE", $model->exam_type);
        $arg["TOP"]["EXAM_TYPE"] = $exam_type["NAME1"];
        $divname["EXAM_TYPE"] = $exam_type["NAME1"];
...
//        $objUp->setFileName(CTRL_YEAR."入試_受付データ.csv");
        ...
        //試験会場（デフォルト表示）
        $hall_name = array();
        $result = $db->query(knjl040oQuery::getExamHall($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
...
//                $key = array("受験年度"       => CTRL_YEAR,
               ...
               $flg = array("入試年度"       => array(false,4),
                ...
                $objUp->setSize(array(8 => 5));
                
                $objForm->ae( array("type"        => "text",
                                    "name"        => "EXAMNO",
                                    "extrahtml"   => "style=\"background-color:#ccffcc\" id=\"".$idx."\" onblur=\"this.value=toInteger(this.value);\" OnChange=\"setName(this,".(int)$idx.",0);\" onKeyDown=\"keyChangeEntToTab(this);\"",
                                    "maxlength"   => "5",
                                    "size"        => "5",
...
//                $csv = array(CTRL_YEAR,
                ...
                //出願時の入試区分と違う場合は警告のため行の色を”赤く”する
                $row["BGCOLOR"] = ($row["CHECK_TESTDIV"] == "") ? "#ff0099" : "#ffffff";
                
                $objForm->ae( array("type"        => "text",
                                    "name"        => "EXAMNO",
                                    "extrahtml"   => "onblur=\"this.value=toInteger(this.value);\" id=\"".$row["RECEPTNO"]."\" OnChange=\"setName(this,".(int)$row["RECEPTNO"].",1);\" onKeyDown=\"keyChangeEntToTab(this);\"",
                                    "maxlength"   => "5",
                                    "size"        => "5",
                                    "multiple"    => "1",
                                    "value"       => $row["EXAMNO"]));
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjl040oindex.php", "", "main");

        $arg["finish"]  = $objForm->get_finish();
        View::toHTML($model, "knjl040oForm1.html", $arg); 
    }
}
?>
