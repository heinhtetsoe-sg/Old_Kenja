...
class knjl424Form2 {
    function main(&$model) {
        $objForm = new form;
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjl424index.php", "", "edit");
        ...
        if ($model->leftYear == "") {
            ...
            $model->leftEventClassCd = $db->getOne(knjl424Query::getRecruitClass());
        }

        //警告メッセージを表示しない場合
        if ($model->cmd == "reset" || $model->cmd == "linkClick") {
            $query = knjl424Query::getRecruitSendYmst($model);
        ...
        //イベントコンボ作成
        $query = knjl424Query::getEvent($model);
        $extra = "";
        ...
        if (VARS::get("cmd") != "edit" && VARS::get("cmd") != "linkClick"){
            $arg["reload"]  = "parent.left_frame.location.href='knjl424index.php?cmd=list';";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl424Form2.html", $arg); 
    }
}
//makeCmb
