...
class knjl309rForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl309rForm1", "POST", "knjl309rindex.php", "", "knjl309rForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl309rQuery::getNameMst($model, $model->ObjYear, "L003"));
        ...
        $extra = " onchange=\"return btn_submit('knjl309r');\"";
        ...
        $result = $db->query(knjl309rQuery::getNameMst($model, $model->ObjYear, $namecd1));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        //試験会場選択コンボボックスを作成する
        $result = $db->query(knjl309rQuery::getEntexamHallDat($model->ObjYear, $model->field["APPLICANTDIV"], $model->field["TESTDIV"]));
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL309R");
        ...
        View::toHTML($model, "knjl309rForm1.html", $arg); 
