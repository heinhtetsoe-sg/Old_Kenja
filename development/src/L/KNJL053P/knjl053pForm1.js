...
function CheckScore(obj) {
    if (obj.value != "") {
        if (aPerfect[obj.value] == undefined) {
            alert('{rval MSG901}' + '\n( ' + obj.value + ' )');
            obj.focus();
            return;
        }
...
function showPaste(obj, cnt) {
    ...
    //テキストボックスの名前の配列を作る
    var nameArray = new Array("VALUE1[]","VALUE2[]","VALUE3[]");
            ...
            if (startFlg) {
                ...
                if (clipTextArray[gyouCnt][retuCnt] != undefined) { //対象となるデータがあれば
                    ...
                    if (targetObject) { //テキストボックスがあれば(テキストボックスはあったりなかったりする)
                        ...
                        } else if (aPerfect[clipTextArray[gyouCnt][retuCnt]] == undefined) {
                            alert('{rval MSG901}' + '\n( ' +clipTextArray[gyouCnt][retuCnt] + ' )');
                            return false;
                        }
                    }
                }
