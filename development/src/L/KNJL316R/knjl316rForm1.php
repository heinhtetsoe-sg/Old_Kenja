...
class knjl316rForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl316rForm1", "POST", "knjl316rindex.php", "", "knjl316rForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl316rQuery::getNameMst($model, "L003",""));
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl316rQuery::getCourseMajorcd($model, ""));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        }
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL316R");
        ...
        View::toHTML($model, "knjl316rForm1.html", $arg); 
