...
class knjl030bForm1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjl030bindex.php", "", "main");
        $db           = Query::dbCheckOut();

        $arg["TOP"]["YEAR"] = $model->year ."年度";

        //入試制度コンボ
        $query = knjl030bQuery::getNameMst($model->year, "L003");
        $extra = "onChange=\"return btn_submit('main')\"";
        makeCmb($objForm, $arg, $db, $query, $model->applicantdiv, "APPLICANTDIV", $extra, 1, "TOP", "");

        //入試区分
        $query = knjl030bQuery::getNameMst($model->year, "L004");
        $extra = "onChange=\"return btn_submit('main')\"";
        ...
        //データ
        $result = $db->query(knjl030bQuery::selectQuery($model));
        $arg["data"] = array();
        ...
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            $row["EXAMHALL_NAME"] = View::alink("#",htmlspecialchars($row["EXAMHALL_NAME"]),
                            "onclick=\"loadwindow('knjl030bindex.php?cmd=edit&mode=update&examhallcd=".$row["EXAMHALLCD"] ."',event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(),370,100);\"");
            $arg["data"][] = $row;
        }
        ...
        View::toHTML($model, "knjl030bForm1.html", $arg); 
    }
}
//makeCmb
