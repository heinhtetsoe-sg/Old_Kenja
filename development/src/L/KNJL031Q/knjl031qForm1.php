...
class knjl031qForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjl031qindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //年度設定
        $result = $db->query(knjl031qQuery::selectYearQuery());
        $opt = array();
        //レコードが存在しなければ処理年度を登録
        if ($result->numRows() == 0) { 
        ...
        if (!$flg) {
            if (!isset($model->year)) {
                ...
                $model->year = $db->getOne(knjl031qQuery::DeleteAtExist($model));
            }
        }
        ...
        //入試制度コンボ
        $extra = "onchange=\"return btn_submit('list');\"";
        $query = knjl031qQuery::getNameMst($model->year, "L003");
        makeCombo($objForm, $arg, $db, $query, $model->applicantdiv, "APPLICANTDIV", $extra, 1, $model);

        //テーブルの中身の作成
        $query = knjl031qQuery::selectQuery($model);
        $result = $db->query($query);
            ...
            $hash = array("cmd"             => "edit2",
                          ...
                          "RECNO"           => $row["RECNO"]);
            $row["EXAMNO_FROM_TO"] = View::alink("knjl031qindex.php", $row["EXAMNO_FROM_TO"], "target=\"right_frame\"", $hash);
        ...
        if (!$flg || (!isset($model->warning) && VARS::post("cmd") == "copy")) {
            $arg["reload"][] = "parent.right_frame.location.href='knjl031qindex.php?cmd=edit"
                             . "&year=" .$model->year."&APPLICANTDIV=" .$model->applicantdiv."';";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl031qForm1.html", $arg);
    }
}
