...
class knjl050bForm1
{
    function main(&$model) {
        ...
        $query = knjl050bQuery::getNameMst("L003", $model->ObjYear);
        makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->applicantdiv, $extra, 1);

        //入試区分コンボボックス
        $query = knjl050bQuery::getNameMst("L004", $model->ObjYear);
        makeCmb($objForm, $arg, $db, $query, "TESTDIV", $model->testdiv, $extra, 1);

        //受験科目コンボボックス
        $query = knjl050bQuery::getTestSubclasscd($model);
        makeCmb($objForm, $arg, $db, $query, "TESTSUBCLASSCD", $model->testsubclasscd, $extra, 1);

        //会場コンボボックス
        $query = knjl050bQuery::getHallName($model);
        ...
        //CSV出力ファイル名
        //科目名
        $query = knjl050bQuery::getTestSubclasscd($model, $model->testsubclasscd);
        $titleKamoku = $db->getRow($query, DB_FETCHMODE_ASSOC);
        //会場名
        $query = knjl050bQuery::getHallName($model, $model->examhallcd);
        ...
        if ($model->applicantdiv != "" && $model->testdiv != "" && $model->testsubclasscd != "" && $model->examhallcd != "") {
            //データ取得
            $result = $db->query(knjl050bQuery::SelectQuery($model, ""));
        ...
        //50件以上の場合、切換ボタンで切換
        $getBacCount = $db->getOne(knjl050bQuery::SelectQuery($model, "BAC_COUNT"));
        $getNextCount = $db->getOne(knjl050bQuery::SelectQuery($model, "NEXT_COUNT"));
        if ($getBacCount > 0) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjl050bindex.php", "", "main");

        //フォーム終わり
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl050bForm1.html", $arg);
    }
}
...
function makeHidden(&$objForm, $model, $arr_receptno) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJL050B");
