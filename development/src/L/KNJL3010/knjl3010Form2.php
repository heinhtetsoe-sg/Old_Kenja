...
class knjl3010Form2 {
    function main(&$model) {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjl3010index.php", "", "edit");
        ...
        if (!isset($model->warning) && !VARS::get("chFlg")) {
            $Row = $db->getRow(knjl3010Query::getRow($model), DB_FETCHMODE_ASSOC);
        } else {
            $Row =& $model->field;
        }
        ...
        if (VARS::get("cmd") != "edit") {
            $arg["reload"]  = "parent.left_frame.location.href='knjl3010index.php?cmd=list';";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl3010Form2.html", $arg); 
    }
}
