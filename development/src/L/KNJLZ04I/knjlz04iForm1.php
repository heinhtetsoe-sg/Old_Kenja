...
class knjlz04iForm1
{
    public function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjlz04iindex.php", "", "edit");
        ...
        //年度コンボ
        $query = knjlz04iQuery::getYear($model);
        ...
        $query = knjlz04iQuery::getNameMst($model->leftYear, "L003");
        ...
        $query = knjlz04iQuery::getTestDiv($model);
        makeCmb($objForm, $arg, $db, $query, "TESTDIV", $model->testdiv, $extra, 1);

        //リスト作成
        $result = $db->query(knjlz04iQuery::getList($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            $row["TESTDIV_NAME"] = View::alink("knjlz04iindex.php", $row["TESTDIV_NAME"], "target=\"right_frame\"", $hash);
            $arg["data"][] = $row;
        }
        ...
        if (!isset($model->warning) && $flg) {
            $arg["reload"]= "parent.right_frame.location.href='knjlz04iindex.php?cmd=edit"
                             . "&ENTEXAMYEAR=".$model->leftYear."&APPLICANTDIV=".$model->applicantdiv."&TESTDIV=".$model->testdiv."';";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjlz04iForm1.html", $arg);
    }
}
