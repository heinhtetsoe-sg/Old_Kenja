...
class knjlz04iForm2
{
    public function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjlz04iindex.php", "", "edit");
        ...
        if (!isset($model->warning) && !VARS::get("chFlg")) {
            $query = knjlz04iQuery::getRow($model);
        ...
        if (!isset($model->warning) && $model->cmd == "updEdit") {
            $arg["reload"]  = "parent.left_frame.location.href='knjlz04iindex.php?cmd=list';";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjlz04iForm2.html", $arg);
    }
}
