...
class knjl320nForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl320nForm1", "POST", "knjl320nindex.php", "", "knjl320nForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl320nQuery::getNameMst($model->ObjYear, "L003"));
        ...
        $extra = " onchange=\"return btn_submit('knjl320n');\"";
        $arg["data"]["APPLICANTDIV"] = knjCreateCombo($objForm, "APPLICANTDIV", $model->field["APPLICANTDIV"], $opt, $extra, 1);

        //入試区分
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl320nQuery::getNameMst($model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL320N");
        ...
        View::toHTML($model, "knjl320nForm1.html", $arg); 
