...
class knjl080oForm1
{
    function main(&$model)
    {
        ...
        //入試制度
        $opt = array();
        $result = $db->query(knjl080oQuery::GetName("L003",$model->ObjYear));
        ...
        //入試区分
        $opt = array();
        $result = $db->query(knjl080oQuery::getTestdivMst($model->ObjYear));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            if($row["NAMESPARE2"]=='1') {
                $opt[]  = array("label" => $row["NAMECD2"]."：".$row["NAME1"], "value" => $row["NAMECD2"]);
        ...
        $result = $db->query(knjl080oQuery::GetLeftList($model));
        ...
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            ...
            if ($model->appli_type == "1") {
                for ($i = 0; $i < (4 - strlen($row["TOTAL_RANK2"])); $i++)
                {
                    $sp .= "&nbsp;";
                }
                $rank = "(".$sp.$row["TOTAL_RANK2"]."位) ";
                $rank2 = sprintf("%04d",$row["TOTAL_RANK2"]);
            }
            if (!strlen($row["TOTAL_RANK2"])) //空文字は最後に並ぶように
                $rank2 = "9999";
            $opt_left[] = array("label" => $row["EXAMNO"]."：".$row["ADJOURNMENT"].$rank.$row["NAME"], 
                                "value" => $rank2."-".$row["EXAMNO"]."-".$row["ENTDIV"]);
            $tmp_id[]   = $row["EXAMNO"];
        }
        ...
        //合格者一覧
        $opt_right = array();
        $result = $db->query(knjl080oQuery::GetRightList($model));
        ...
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            if (!in_array($row["EXAMNO"], $tmp_id)) {
                ...
                if (!strlen($row["TOTAL_RANK2"])) //空文字は最後に並ぶように
                    $rank2 = "9999";
                $opt_right[]    = array("label" => $row["EXAMNO"]."：".$row["ADJOURNMENT"].$rank.$row["NAME"], 
                                        "value" => $rank2."-".$row["EXAMNO"]."-".$row["ENTDIV"]);
            }
        }
        ...
        $arg["start"]   = $objForm->get_start("sel", "POST", "knjl080oindex.php", "", "sel");

        $arg["finish"]  = $objForm->get_finish();

        View::toHTML($model, "knjl080oForm1.html", $arg); 
    }
}
?>
