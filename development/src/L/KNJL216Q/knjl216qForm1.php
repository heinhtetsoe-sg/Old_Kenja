...
class knjl216qForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjl216qindex.php", "", "main");
        ...
        $query = knjl216qQuery::getNameMst($model->examyear, "L003");
        makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->field["APPLICANTDIV"], $extra, 1);

        //入試区分コンボ
        $query = knjl216qQuery::getNameMst($model->examyear, "L004");
        makeCmb($objForm, $arg, $db, $query, "TESTDIV", $model->field["TESTDIV"], $extra, 1);

        //重複チェック項目コンボ
        $query = knjl216qQuery::getCenterTitle();
        ...
        View::toHTML($model, "knjl216qForm1.html", $arg);
    }
}
...
function getBaseArray($model, $db) {
    $baseArray = array();
    $query = knjl216qQuery::getBaseDat($model);
...
function getBeforeArray($model, $db) {
    $beforeArray = array();
    $query = knjl216qQuery::getBeforeDat($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $flg = false;
        foreach ($model->baseArrayKakutei as $examno => $baseList) {
            if ($row["TEST_NO"] == $baseList["REMARK1"]) {
                $flg = true;
            }
        }
        if ($flg) continue;
...
function makeList(&$objForm, &$arg, $model, $baseArray, $beforeArray) {
    ...
    if (strlen($model->field["CENTER_TITLE"])) {
        foreach ($baseArray as $examno => $baseList) {
            foreach ($beforeArray as $pageno => $beforeList) {
                if ($model->field["CENTER_TITLE"] == "1" && $beforeList["NAME"] == $baseList["NAME"] &&  $beforeList["FS_CD"] != $baseList["FS_CD"]) {
                    $checkList[$examno][] = $pageno;
                } else if ($model->field["CENTER_TITLE"] == "2" && $beforeList["NAME"] != $baseList["NAME"] &&  $beforeList["FS_CD"] == $baseList["FS_CD"]) {
                    $checkList[$examno][] = $pageno;
                } else if ($model->field["CENTER_TITLE"] == "3" && $beforeList["NAME"] == $baseList["NAME"] &&  $beforeList["FS_CD"] == $baseList["FS_CD"]) {
                    $checkList[$examno][] = $pageno;
                }
            }
        }
    ...
    foreach ($checkList as $examno => $array) {
        ...
        foreach ($array as $key => $pageno) {
            $baseLabel = $examno . "、" . $baseArray[$examno]["NAME"] . "、" . $baseArray[$examno]["NAME"] . "、" . $baseArray[$examno]["FINSCHOOL_NAME"];
            $beforeLabel = $pageno . "、" . $beforeArray[$pageno]["NAME"] . "、" . $beforeArray[$pageno]["NAME"]. "、" . $beforeArray[$pageno]["FINSCHOOL_NAME"];
            //受験番号に対して、複数の公的テスト番号がある場合、
            //１件目は通常表示。２件目以降は●表示。
            if ($key == 0) {
