...
//文字数チェック
function toStringCheck(checkString) {
    var newString = "";
    var count = 0;
    ...
    return ShowDialog(newString,checkString,"15文字以下");
}

//値チェック
function toIntegerCheck(checkString) {
    var newString = "";
    var count = 0;
    for (i = 0; i < checkString.length; i++) {
        ch = checkString.substring(i, i+1);
        if (newString.length < 3 && ch >= "0" && ch <= "9") {
            newString += ch;
        }
    }
    return ShowDialog(newString,checkString,"数字3桁");
}
...
function showPaste(obj) {
    ...
    //テキストボックスの名前の配列を作る
    var nameArray = new Array("NAISINTEN",
                              "KESSEKI",
                              "REMARK");
        ...
        for (k = 0; k < objectNameArray.length; k++) { //テキストボックス名でまわす
            ...
            if (startFlg) {
                if (clipTextArray[j][i] != undefined && clipTextArray[j][i] != '') {
                    ...
                    if (objectNameArray[k].match(/NAISINTEN/) || objectNameArray[k].match(/KESSEKI/)) {
