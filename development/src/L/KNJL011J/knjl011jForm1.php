...
class knjl011jForm1
{
    ...
    {
        $objForm      = new form;
        $arg["start"] = $objForm->get_start("main", "POST", "knjl011jindex.php", "", "main");
        ...
        if ((!isset($model->warning)) && (!is_array($existdata))) {
            //データを取得
            $Row = knjl011jQuery::get_edit_data($model);
        ...
        //------------------------------志願者情報-------------------------------------
        //入試制度コンボ
        $result     = $db->query(knjl011jQuery::get_name_cd($model->year, 'L003'));
        $opt        = array();
        ...
        //受付日（出願受付日）
        $value = ($Row["RECEPTDATE"] != "") ? str_replace("-","/",$Row["RECEPTDATE"]) : str_replace("-","/",CTRL_DATE);
        $extra = " onchange=\"change_flg()\"";
        $arg["data"]["RECEPTDATE"] = View::popUpCalendar2($objForm, "RECEPTDATE", $value, "", "", $extra);

        //受験型コンボ
        $result     = $db->query(knjl011jQuery::get_name_cd($model->year, 'L005'));
        $opt_type   = array();
        for ($testdiv = 1; $testdiv < 7; $testdiv++) {
            $opt_type[$testdiv][] = array("label" => "", "value" => "");
        }
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            for ($testdiv = 1; $testdiv < 7; $testdiv++) {
                ...
                {
                    $opt_type[$testdiv][] = array("label" => $row["NAMECD2"].":".$row["NAME1"],
                                                  "value" => $row["NAMECD2"]);
                }
            }
        }
        ...
        //入試区分チェックボックス
        //出願区分コンボ
        $result     = $db->query(knjl011jQuery::get_name_cd($model->year, 'L004'));
        $opt        = array();
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            $opt[] = array("label" => $row["NAMECD2"].":".$row["NAME1"],
            ...
            $objForm->ae( array("type"        => "checkbox",
                                "name"        => $fieldname,
                                "extrahtml"   => "onclick=\"setExamType(this,".$row["NAMECD2"].")\" onchange=\"change_flg()\" id=\"$fieldname\" " .$checked,
                                "value"       => "1" ));
            $arg["data"][$fieldname] = $objForm->ge($fieldname);
            $arg["data"][$fieldname."_ID"] = $fieldname;
            $arg["data"][$fieldname."_NAME"] = $row["NAME1"];

            $fieldname2 = "EXAM_TYPE" .$row["NAMECD2"];
            $objForm->ae( array("type"        => "select",
                                "name"        => $fieldname2,
                                "size"        => "1",
                                "extrahtml"   => "onchange=\"change_flg()\"",
                                "value"       => strlen($checked) ? $Row[$fieldname2] : "",
                                "options"     => $opt_type[$row["NAMECD2"]] ) );
            $arg["data"][$fieldname2] = $objForm->ge($fieldname2);
        }
        ...
        //性別コンボ
        $result     = $db->query(knjl011jQuery::get_name_cd($model->year, 'Z002'));
        $opt        = array();
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            $opt[] = array("label" => $row["NAMECD2"].":".$row["NAME1"],
        ...
        $objForm->ae( array("type"        => "text",
                            ...
                            "size"        => 14,
                            "maxlength"   => 14,
                            "extrahtml"   => "onblur=\"this.value=toTelNo(this.value), toCopytxt(3, this.value)\" onchange=\"change_flg()\" style=\"display:none\"",
                            "value"       => $Row["TELNO"] ));

        $arg["data"]["TELNO"] = $objForm->ge("TELNO");

        //出身学校コンボ
        $result     = $db->query(knjl011jQuery::getFinschoolcd($model->year));
        $FsDistName = array();
        $opt        = array();
        ...
        $opt = array();
        $value_flg = false;
        $query = knjl011jQuery::getSpecialReasonDiv();
        $result = $db->query($query);
        $opt[] = array('label' => '', 'value' => '');
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
            if ($Row["SPECIAL_REASON_DIV"] == $row["VALUE"]) $value_flg = true;
        }
        $Row["SPECIAL_REASON_DIV"] = ($Row["SPECIAL_REASON_DIV"] && $value_flg) ? $Row["SPECIAL_REASON_DIV"] : $opt[0]["value"];
        $extra = "";
        $arg["data"]["SPECIAL_REASON_DIV"] = knjCreateCombo($objForm, "SPECIAL_REASON_DIV", $Row["SPECIAL_REASON_DIV"], $opt, $extra, 1);

        //塾コンボ
        $result     = $db->query(knjl011jQuery::getPrischoolcd($model->year));
        $opt        = array();
        $opt[] = array("label" => "", "value" => "");
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        $objForm->ae( array("type"        => "text",
                            ...
                            "extrahtml"   => "onblur=\"this.value=toTelNo(this.value)\" onchange=\"change_flg()\"",
                            "value"       => $Row["GFAXNO"] ));
        $arg["data"]["GFAXNO"] = $objForm->ge("GFAXNO");

        //続柄コンボ
        $opt       = array();
        $opt[]     = array("label" => "", "value" => "");
        $result    = $db->query(knjl011jQuery::get_name_cd($model->year, 'H201'));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            $opt[] = array("label" => $row["NAMECD2"].":".$row["NAME1"],
                           "value" => $row["NAMECD2"]);
        ...
        $arg["data"]["EMERGENCYCALL"] = $objForm->ge("EMERGENCYCALL");
        ...
        $arg["data"]["EMERGENCYTELNO"] = $objForm->ge("EMERGENCYTELNO");

        //------------------------------内申科目---------------------------------
        $result = $db->query(knjl011jQuery::get_name_cd($model->year, "L008"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
...
//        $arg["data"]["KASANTEN_ALL"] = $Row["KASANTEN_ALL"];
        ...
        $objForm->ae( array("type"        => "button",
                            ...
                            "value"       => "かな検索",
                            "extrahtml"   => "style=\"width:70px\" onclick=\"loadwindow('" .REQUESTROOT ."/L/KNJL011J/search_name.php?cmd=search&year='+document.forms[0]['year'].value+'&examno='+document.forms[0]['EXAMNO'].value+'&NAME_SESSID={$sess->id}&frame='+getFrameName(self), event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 260)\"") );
        ...
        $objForm->ae( array("type"        => "button",
                            ...
                            "value"       => "かな検索",
                            "extrahtml"   => "style=\"width:70px\" onclick=\"loadwindow('" .REQUESTROOT ."/L/KNJL011J/search_fin_name.php?cmd=search&year='+document.forms[0]['year'].value+'&examno='+document.forms[0]['EXAMNO'].value+'&NAME_SESSID={$sess->id}&frame='+getFrameName(self), event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 335, 370)\"") );
        ...
        $objForm->ae( array("type"        => "button",
                            ...
                            "value"       => "かな検索",
                            "extrahtml"   => "style=\"width:70px\" onclick=\"loadwindow('" .REQUESTROOT ."/L/KNJL011J/search_pri_name.php?cmd=search&year='+document.forms[0]['year'].value+'&examno='+document.forms[0]['EXAMNO'].value+'&NAME_SESSID={$sess->id}&frame='+getFrameName(self), event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}()-180, 320, 370)\"") );
        ...
        //電話番号よりFAX番号へコピーボタン
        $objForm->ae( array("type"      => "button",
                            "name"      => "btn_gtelno_copy",
                            "value"     => "番号コピー",
                            "extrahtml" => "style=\"width:80px\" onclick=\"return btn_submit('gtelno_copy');\"" ) );
        $arg["button"]["btn_gtelno_copy"] = $objForm->ge("btn_gtelno_copy");
        ...
        View::toHTML($model, "knjl011jForm1.html", $arg);
    }
}
?>