...
class SearchNames
{
    ...
    function getNameList(){
        ...
        if (strlen($this->names) || strlen($this->name_kana)){
            ...
            $query .= "  AND APPLICANTDIV = '".$this->applicantdiv."'";//中学
            $query .= "  AND TESTDIV = '2'";//前期
    ...
    function getBtnSearch(){
     ...
     //フォームタブ(開始)
    function getStart(){
        return $this->form->get_start("name", "POST", "knjl011pZenkiExamnoSearch.php", "", "name");
    }
    //フォームタブ(終了)
    function getFinish(){
...
if ($_REQUEST["ZENKI_SESSID"] == "") exit;
$sess = new APP_Session($_REQUEST["ZENKI_SESSID"], 'NAME');
if (!$sess->isCached($_REQUEST["ZENKI_SESSID"], 'NAME')) {
...
function apply_name(obj) {
    if(obj.selectedIndex >= 0){
        var val = obj.options[obj.selectedIndex].text;    
        var arr = val.split("　");
        f.document.forms[0]['DAI1_RECEPTNO'].value = arr[0];
        f.closeit();
    }else{
        alert("名前が選択されていません");
    }
...
<input type="hidden" name="ZENKI_SESSID" value="<?php echo $_REQUEST["ZENKI_SESSID"] ?>">
