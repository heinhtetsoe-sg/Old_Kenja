...
class knjl071rForm1
{
    function main(&$model) {
        ...
        $query = knjl071rQuery::getNameMst("L003", $model->year, $model->fixApplicantDiv);
        makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->applicantdiv, $extra, 1);

        //入試区分コンボ
        $extra = "onchange=\"return btn_submit('changeTest');\" tabindex=-1";
        $query = knjl071rQuery::getNameMst("L024", $model->year);
        makeCmb($objForm, $arg, $db, $query, "TESTDIV", $model->testdiv, $extra, 1);

        //専併コンボ
        $extra = "onchange=\"return btn_submit('changeSh');\"";
        $query = knjl071rQuery::getNameMst("L006", $model->year);
        ...
        $query = knjl071rQuery::getPassMst("L013", $model);
        ...
        //一覧表示
        $arr_receptno = array();
        $model->receptInfo = array();
        if ($model->applicantdiv != "" && $model->testdiv != "" && $model->shdiv != "") {

            //データ取得
            $query = knjl071rQuery::SelectQuery($model);
            $result = $db->query($query);

            //データ表示
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                array_walk($row, "htmlspecialchars_array");
                $model->receptInfo[$row["RECEPTNO"]]["EXAMNO"] = $row["EXAMNO"];
                ...
                $row["OTHER_REMARK"] = $row["OTHER_RECEPTNO"].','.$row["OTHER_TESTDIV_NM"].','.$row["OTHER_SH_NM"].' '.$row["OTHER_JUDGEMENT_NM"];
        ...
        //ボタン作成
        makeBtn($objForm, $arg, $arr_receptno);
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjl071rindex.php", "", "main");

        //フォーム終わり
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl071rForm1.html", $arg);
    }
}
...
//ボタン作成
function makeBtn(&$objForm, &$arg, $arr_receptno) {
    //更新ボタン
    $disBtn = (0 < get_count($arr_receptno)) ? "" : " disabled";
