...
class knjl330uForm1 {
    function main(&$model) {
        $objForm = new form;
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl330uForm1", "POST", "knjl330uindex.php", "", "knjl330uForm1");
        ...
        //入試制度コンボ
        $query = knjl330uQuery::getNameMst($model->examyear, "L003");
        $extra = "onChange=\"return btn_submit('knjl330u')\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["APPLICANTDIV"], "APPLICANTDIV", $extra, 1, "TOP", "");

        //入試区分
        $query = knjl330uQuery::getNameMst($model->examyear, "L004");
        $extra = "";
        makeCmb($objForm, $arg, $db, $query, $model->field["TESTDIV"], "TESTDIV", $extra, 1, "TOP", "");

        //チェックボックス
        $extra = "id=\"RUIKEI_FLG\"";
        $arg["data"]["RUIKEI_FLG"] = knjCreateCheckBox($objForm, "RUIKEI_FLG", "1", $extra);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL330U");
        ...
        View::toHTML($model, "knjl330uForm1.html", $arg);
    }
}
//makeCmb
