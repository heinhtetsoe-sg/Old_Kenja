...
class knjl403hForm2
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjl403hindex.php", "", "edit");
        ...
        if (!isset($model->warning) && ($model->cmd == "edit2" || $model->cmd == "reset") && $model->year && $model->applicantdiv && $model->testdiv && $model->totalcd && $model->examcoursecd && $model->testsubclasscd && $model->exam_type) {
            $query = knjl403hQuery::getRow($model->year, $model->applicantdiv, $model->testdiv, $model->totalcd, $model->examcoursecd, $model->testsubclasscd, $model->exam_type);
        ...
        if (VARS::get("cmd") != "edit") {
            $arg["reload"]  = "parent.left_frame.location.href='knjl403hindex.php?cmd=list2"
                            . "&year=".$model->year."&applicantdiv=".$model->applicantdiv."';";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl403hForm2.html", $arg);
    }
}
