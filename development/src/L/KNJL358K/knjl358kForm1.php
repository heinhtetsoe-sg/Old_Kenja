...
class knjl358kForm1
{
    ...
    $objForm = new form;
    //フォーム作成
    $arg["start"]   = $objForm->get_start("knjl358kForm1", "POST", "knjl358kindex.php", "", "knjl358kForm1");
    $db = Query::dbCheckOut();
    ...
    $opt = array();
    $value_flg = false;
    $query = knjl358kQuery::getSpecialReasonDiv($model);
    ...
    //出力対象ラジオ---1:前期集計表,2:後期集計表,3:附属出身者集計表,4:全体集計表
    $opt_out[0]=1;
    $opt_out[1]=2;
    $opt_out[2]=3;
    $opt_out[3]=4;
    ...
    //中高判別フラグを作成する
    $jhflg = 0;
    $row = $db->getOne(knjl358kQuery::GetJorH());
    if ($row == 1){
        $jhflg = 1;
    }else {
        $jhflg = 2;
    ...
    View::toHTML($model, "knjl358kForm1.html", $arg); 
    }
}
?>
