...
class knjl071pForm1 {
    function main(&$model) {
        ...
        $query = knjl071pQuery::getNameMst("L003", $model->year);
        makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->applicantdiv, $extra, 1);

        //入試区分コンボ
        $extra = "onchange=\"return btn_submit('main');\" tabindex=-1";
        $query = knjl071pQuery::getNameMst(($model->applicantdiv == "2") ? "L004" : "L024", $model->year);
        ...
        //合否コンボ
        $extra = "";
        $query = knjl071pQuery::getNameMst("L013", $model->year);
        ...
        if ($model->applicantdiv != "" && $model->testdiv != "") {
            ...
            //データ取得
            $result = $db->query(knjl071pQuery::SelectQuery($model));
            ...
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                ...
                //更新チェックボックス
                $disJdg = (strlen($row["PROCEDUREDIV"]) || strlen($row["ENTDIV"]) || $row["JUDGEDIV"] == "4") ? " disabled" : "";
                $extra = "onclick=\"bgcolorYellow(this, '{$row["RECEPTNO"]}');\"" .$disJdg;
                $row["CHK_DATA"] = knjCreateCheckBox($objForm, "CHK_DATA"."-".$row["RECEPTNO"], "on", $extra);
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjl071pindex.php", "", "main");

        //フォーム終わり
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl071pForm1.html", $arg);
    }
}
