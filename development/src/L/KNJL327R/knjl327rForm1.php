...
class knjl327rForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl327rForm1", "POST", "knjl327rindex.php", "", "knjl327rForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl327rQuery::getNameMst($model, $model->ObjYear, "L003"));
        ...
        $extra = "onchange=\"return btn_submit('knjl327r');\"";
        $arg["data"]["APPLICANTDIV"] = knjCreateCombo($objForm, "APPLICANTDIV", $model->field["APPLICANTDIV"], $opt, $extra, 1);

        //入試区分
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl327rQuery::getNameMst($model, $model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        $extra = array("id=\"FORM1\" onclick =\" return btn_submit('knjl327r');\"", "id=\"FORM2\" onclick =\" return btn_submit('knjl327r');\"");
        $radioArray = knjCreateRadio($objForm, "FORM", $model->field["FORM"], $extra, $opt_form, get_count($opt_form));
        foreach($radioArray as $key => $val) $arg["data"][$key] = $val;

        //（通知）出力範囲１ラジオボタン 1:全て 2:合格通知書 3:不合格通知書 4:難関コース補欠合格通知書
        $opt_output1 = array(1, 2, 3, 4);
        $model->field["OUTPUT1"] = ($model->field["OUTPUT1"] == "") ? "1" : $model->field["OUTPUT1"];
        $click  = ($model->field["FORM"] == "1") ? "" : " disabled";
        $click .= " onclick =\" return btn_submit('knjl327r');\"";
        $extra  = array("id=\"OUTPUT11\"".$click, "id=\"OUTPUT12\"".$click, "id=\"OUTPUT13\"".$click, "id=\"OUTPUT14\"".$click);
        ...
        View::toHTML($model, "knjl327rForm1.html", $arg); 
    }
}
...
function makeHidden(&$objForm, $model) {
    knjCreateHidden($objForm, "PRGID", "KNJL327R");
