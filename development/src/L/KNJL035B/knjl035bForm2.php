...
class knjl035bForm2
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjl035bindex.php", "", "edit");
        ...
        if (!isset($model->warning) && ($model->cmd == "edit2") && $model->year && $model->applicantdiv && $model->testdiv && $model->desirediv){
            $query = knjl035bQuery::getRow($model->year, $model->applicantdiv, $model->testdiv, $model->desirediv);
        ...
        $query = knjl035bQuery::getNameMst($model->year, "L003");
        makeCombo($objForm, $arg, $db, $query, $Row["APPLICANTDIV"], "APPLICANTDIV", $extra, 1, "blank");

        //入試区分コンボ
        $query = knjl035bQuery::getNameMst($model->year, "L004");
        ...
        //選択コース一覧
        $result = $db->query(knjl035bQuery::getListCourse($model, 'Llist', $Row["APPLICANTDIV"], $Row["TESTDIV"], $Row["DESIREDIV"]));
        $opt_left = array();
        ...
        //コース一覧
        $result = $db->query(knjl035bQuery::getListCourse($model, 'Rlist', $Row["APPLICANTDIV"], $Row["TESTDIV"], $Row["DESIREDIV"]));
        $opt_right = array();
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "window.open('knjl035bindex.php?cmd=list','left_frame');";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl035bForm2.html", $arg);
    }
}
