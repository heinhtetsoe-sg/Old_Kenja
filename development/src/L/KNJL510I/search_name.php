...
class searchNames
{
    ...
    {
        $opt = array();
        if (strlen($this->examno) || strlen($this->names) || strlen($this->name_kana) || strlen($this->finschoolcd)) {
            $db = Query::dbCheckOut();
            $query  = "";
            $query .= "SELECT ";
            $query .= "    EXAMNO, ";
            ...
            if (strlen($this->finschoolcd)) {
                $query .= " AND FS_CD = '" .$this->finschoolcd2 ."' ";
            }
    ...
    public function main($rq)
    {
        ...
        $this->finschoolcd  = $rq["finschoolcd"];
        $this->finschoolcd2 = $rq["finschoolcd"];
    ...
    public function getFinschool()
    {
        ...
        $query .= "     T1.FS_CD || ':' || VALUE(F1.FINSCHOOL_NAME, '') AS LABEL, ";
        ...
        $query .= " AND T1.FS_CD IS NOT NULL ";
        $query .= " GROUP BY ";
        $query .= "     T1.FS_CD, ";
        $query .= "     F1.FINSCHOOL_NAME ";
        $query .= " ORDER BY ";
        ...
        //出身中学校コンボ
        $extra = "";
        $retVal = $this->makeCmb($db, $query, $this->finschoolcd, "finschoolcd", $extra, 1, "BLANK");
        Query::dbCheckIn($db);
        return $retVal;
    }
        ...
        <tr class="no_search_line">
          <td>
            <table width="100%" border="0" cellspacing="1" cellpadding="2">
              ...
              <tr class="no_search">
                <th nowrap width="40%" align="right">&nbsp;氏名カナ&nbsp;</th>
                <td bgcolor="#ffffff"><?php echo $objName->getTextNameKana() ?></td>
              </tr>
              <tr class="no_search">
                <th nowrap width="40%" align="right">&nbsp;出身中学校&nbsp;</th>
                <td bgcolor="#ffffff"><?php echo $objName->getFinschool() ?></td>
              </tr>
              <tr class="no_search">
