...
class knjl090hForm1
{
    ...
    {
        $objForm      = new form;
        $arg["start"] = $objForm->get_start("main", "POST", "knjl090hindex.php", "", "main");
        $db = Query::dbCheckOut();

/****************************************** GET DATA!! ***************************************************************/        

        $Row = $db->getRow(knjl090hQuery::get_edit_data($model), DB_FETCHMODE_ASSOC);
        ...
        if($model->cmd == 'change_testdiv2' || $model->cmd == 'change' || $model->cmd == 'update' || (isset($model->warning))) {
            $Row2 = $db->getRow(knjl090hQuery::getRecept($model), DB_FETCHMODE_ASSOC);
        }else{
            $model->testdiv2 = $db->getOne(knjl090hQuery::getMaxtestdiv($model));    //最大testdiv取得
            $Row2 = $db->getRow(knjl090hQuery::getRecept($model), DB_FETCHMODE_ASSOC);
        }
        ...
        //志願者データ取得
        $Row3 = $db->getRow(knjl090hQuery::getDesire($model), DB_FETCHMODE_ASSOC);

        //同じ入試区分で受験データにも登録されていれば志願、受験コンボ共に変更不可
        $exist = $db->getRow(knjl090hQuery::getRecept($model), DB_FETCHMODE_ASSOC);
        ...
        $opt_testsub = array();
        $number = 0;
        $result = $db->query(knjl090hQuery::getTestsubclasscd($model->year, $Row["APPLICANTDIV"]));
        ...
        //志願者得点データ
        $result = $db->query(knjl090hQuery::getScore($model));
        ...
        //受験科目 内申科目 判定名称
        $result = $db->query(knjl090hQuery::getName($model->year, array("L013","L008","L009")));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
...
/*********************************************************** 表示 *************************************************************/
        ...
        if (isset($model->examno)) {
            ...
            $DESIRE_FLG["EXAMINEE_DIV"] = $db->getOne(knjl090hQuery::get_desire_flg($model));
            if(isset($DESIRE_FLG["EXAMINEE_DIV"])) {
                $arg["data"]["EXAMINEE"] = "【受験】";
            }

            $RECEPT_FLG["HONORDIV"] = $db->getOne(knjl090hQuery::get_recept_flg($model, "HONORDIV"));
            //何れかの入試区分で特待区分が「対象」であればラベルを表示
            if(isset($RECEPT_FLG["HONORDIV"])) {
                $arg["data"]["HONOR"] = "【特待】";
            }
            $RECEPT_FLG["ADJOURNMENTDIV"] = $db->getOne(knjl090hQuery::get_recept_flg($model, "ADJOURNMENTDIV"));
        ...
        //判定
        $judge = $db->getRow(knjl090hQuery::getReceptJudge($model), DB_FETCHMODE_ASSOC); 
...
/**************************************** CREATE 15 COMBOS ! ************************************************************/
        ...
        //入試制度コンボ
        $opt = knjl090hForm1::GetOpt($db, $model->year, array("L003"), 0);
        $arg["data"]["APPLICANTDIV"] = knjl090hForm1::CreateCombo($objForm, "APPLICANTDIV", $Row["APPLICANTDIV"], "150", $opt, "onChange=\"change_flg()\"");

        //出願区分コンボ
//      $opt = knjl090hForm1::GetOpt($db, $model->year, array("L004"), 0);
//      $arg["data"]["TESTDIV"] = knjl090hForm1::CreateCombo($objForm, "TESTDIV", $Row["TESTDIV"], "150", $opt, "onChange=\"change_flg()\"");
        $testdiv = $seq = "";
        $result = $db->query(knjl090hQuery::getName($model->year, array("L004")));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        //出願コースコンボ
        $opt = array();
        $result     = $db->query(knjl090hQuery::get_desirediv($model->year, $Row["APPLICANTDIV"]));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            $opt[] = array("label" => $row["DESIREDIV"].":".$row["EXAMCOURSE_NAME"], "value" => $row["DESIREDIV"]);
        }
        $arg["data"]["DESIREDIV"] = knjl090hForm1::CreateCombo($objForm, "DESIREDIV", $model->desirediv, "150", $opt, "onChange=\"btn_submit('change')\";");

        if (!isset($model->desirediv))
            $model->desirediv = $opt[0]["value"];

        knjl090hForm1::FormatOpt($opt_cmcd);
        knjl090hForm1::FormatOpt($opt["L010"]);
        knjl090hForm1::FormatOpt($opt["L011"]);
        knjl090hForm1::FormatOpt($opt["L012"]);
        knjl090hForm1::FormatOpt($opt_honor1);
        knjl090hForm1::FormatOpt($opt_inflg);
        ...
        if ($model->judgement == '1' || $model->special_measures !='') {
            ...
            //志望学科に追加
            knjl090hForm1::FormatOpt($opt_cmcd,0);
            $result     = $db->query(knjl090hQuery::get_coursemajor($model, $Row["APPLICANTDIV"]));
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
            {
            ...
            //手続区分に追加
            $opt["L011"] = knjl090hForm1::GetOpt($db, $model->year, array("L011"));

            //手続区分が「済み」なら入学区分に値を追加
            if($model->procedurediv == "1") {
                
                $opt["L012"] = knjl090hForm1::GetOpt($db, $model->year, array("L012"));
        ...
        //志望学科コンボ
        $arg["data"]["COURSEMAJOR"] = knjl090hForm1::CreateCombo($objForm, "COURSEMAJOR", $Row["COURSEMAJOR"], "300", $opt_cmcd, "onChange=\"change_flg()\"");

        //特別措置区分(合格が一件もない場合）
        if ((int)$judge["PASS"] == 0 && $model->judgediv != "1") {
            $opt["L010"] = knjl090hForm1::GetOpt($db, $model->year, array("L010"));
        }
        $arg["data"]["SPECIAL_MEASURES"] = knjl090hForm1::CreateCombo($objForm, "SPECIAL_MEASURES", $model->special_measures, "100", $opt["L010"], "onChange=\"btn_submit('change')\";");

        //手続区分コンボ
        $arg["data"]["PROCEDUREDIV"] = knjl090hForm1::CreateCombo($objForm, "PROCEDUREDIV", $model->procedurediv, "100", $opt["L011"], "onChange=\"btn_submit('change')\";");

        //入学区分コンボ
        $arg["data"]["ENTDIV"] = knjl090hForm1::CreateCombo($objForm, "ENTDIV", $model->entdiv, "100", $opt["L012"], "onChange=\"btn_submit('change')\";");

        //特待入学コンボ
        $arg["data"]["HONORDIV1"] = knjl090hForm1::CreateCombo($objForm, "HONORDIV1", $Row["HONORDIV"], "100", $opt_honor1, "onChange=\"change_flg()\"");

        //面接出欠コンボ
        $arg["data"]["INTERVIEW_ATTEND_FLG"] = knjl090hForm1::CreateCombo($objForm, "INTERVIEW_ATTEND_FLG", $Row["INTERVIEW_ATTEND_FLG"], "100", $opt_inflg, "onChange=\"change_flg()\"");

        //入試区分コンボ
        $opt = knjl090hForm1::GetOpt($db, $model->year, array("L004"), 0) ;
        $arg["data"]["TESTDIV2"] = knjl090hForm1::CreateCombo($objForm, "TESTDIV2", $model->testdiv2, "100", $opt, "onChange=\"change_flg(), btn_submit('change_testdiv2')\";");

        //合否区分コンボ(受付データがなければ合否区分コンボに空セット)
        $opt = (!isset($Row2["EXAMNO"]) ? array() : knjl090hForm1::GetOpt($db, $model->year, array("L013")));
        $arg["data"]["JUDGEDIV"] = knjl090hForm1::CreateCombo($objForm, "JUDGEDIV", $model->judgediv, "100", $opt, "onChange=\"btn_submit('change')\";");

        //合否区分が「合格」なら特待区分コンボと延期区分コンボに値を追加
        knjl090hForm1::FormatOpt($opt);
        knjl090hForm1::FormatOpt($opt2);
        if ($model->judgediv == "1") {
        ...
        //特待区分コンボ
        $arg["data"]["HONORDIV2"] = knjl090hForm1::CreateCombo($objForm, "HONORDIV2", $Row2["HONORDIV"], "100", $opt, "onChange=\"change_flg()\"");

        //延期区分コンボ
        $arg["data"]["ADJOURNMENTDIV"] = knjl090hForm1::CreateCombo($objForm, "ADJOURNMENTDIV", $Row2["ADJOURNMENTDIV"], "100", $opt2, "onChange=\"change_flg()\"");

        //志願区分コンボ
        knjl090hForm1::FormatOpt($opt);
        $opt[] = array("label"  =>  '1:有り', "value"  => 1);
        $opt[] = array("label"  =>  '2:無し', "value"  => 2);
        $arg["data"]["APPLICANT_DIV"] = knjl090hForm1::CreateCombo($objForm, "APPLICANT_DIV", $Row3["APPLICANT_DIV"], "100", $opt, $disabled_a." onChange=\"change_flg()\"");

        //受験区分コンボ
        knjl090hForm1::FormatOpt($opt);
        $opt[] = array("label"  =>  '1:有り', "value"  => 1);
        $opt[] = array("label"  =>  '2:欠席', "value"  => 2);
        $arg["data"]["EXAMINEE_DIV"] = knjl090hForm1::CreateCombo($objForm, "EXAMINEE_DIV", $Row3["EXAMINEE_DIV"], "100", $opt, $disabled_a." onChange=\"change_flg()\"");
...
/***************************************************** CREATE BUTTONS ********************************************************/
        ...
        $objForm->ae( array("type"      => "button",
                            ...
                            "value"     => "かな検索",
                            "extrahtml" => "style=\"width:80px\" onclick=\"loadwindow('" .REQUESTROOT ."/L/KNJL090H/search_name.php?cmd=search&year='+document.forms[0]['year'].value+'&examno='+document.forms[0]['EXAMNO'].value+'&NAME_SESSID={$sess->id}&frame='+getFrameName(self), event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 260)\"") );
        ...
        View::toHTML($model, "knjl090hForm1.html", $arg);
    }
    ...
    function GetOpt(&$db, $year, $namecd, $flg=1)
    {
        ...
        if (is_array($namecd)) {
            $result = $db->query(knjl090hQuery::getName($year, $namecd));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
            {
