...
class knjl323nForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl323nForm1", "POST", "knjl323nindex.php", "", "knjl323nForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl323nQuery::getNameMst($model->ObjYear, "L003"));
        ...
        $extra = " onchange=\"return btn_submit('knjl323n');\"";
        $arg["data"]["APPLICANTDIV"] = knjCreateCombo($objForm, "APPLICANTDIV", $model->field["APPLICANTDIV"], $opt, $extra, 1);

        //入試区分
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl323nQuery::getNameMst($model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        //帳票区分ラジオボタンを作成 1:合格名簿 2:奨学生名簿一覧 3:不合格名簿
        $radioValue = array(1, 2, 3);
        if (!$model->field["OUTPUTDIV"]) $model->field["OUTPUTDIV"] = 1;
        $extra = array("id=\"OUTPUTDIV1\"", "id=\"OUTPUTDIV2\"", "id=\"OUTPUTDIV3\"");
        $radioArray = knjCreateRadio($objForm, "OUTPUTDIV", $model->field["OUTPUTDIV"], $extra, $radioValue, get_count($radioValue));
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL323N");
        ...
        View::toHTML($model, "knjl323nForm1.html", $arg); 
