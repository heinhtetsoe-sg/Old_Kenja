...
class knjl021nForm1
{
    ...
    {
        $objForm      = new form;
        $arg["start"] = $objForm->get_start("main", "POST", "knjl021nindex.php", "", "main");

        //一覧表示
        if ((!isset($model->warning)) && $model->cmd !== 'keisan') {
            //データを取得
            $Row = knjl021nQuery::get_edit_data($model);
        } else {
            $Row =& $model->field;
        }
        ...
        //入試制度
        $applicantdiv_name = $db->getOne(knjl021nQuery::get_name_cd($model->year, "L003", $Row["APPLICANTDIV"]));
        $arg["data"]["APPLICANTDIV"] = $applicantdiv_name;

        //入試区分
        $test_name = $db->getOne(knjl021nQuery::get_name_cd($model->year, "L004", $Row["TESTDIV"]));
        $arg["data"]["TESTDIV"] = $test_name;

        //------------------------------内申-------------------------------------
        
        //各項目の教科名称取得
        $query = knjl021nQuery::get_name_cd($model->year, "L008");
        ...
        //戻るボタン
        $link = REQUESTROOT."/L/KNJL011N/knjl011nindex.php?cmd=reference&SEND_PRGID=KNJL021N&SEND_AUTH=".$model->auth."&SEND_APPLICANTDIV=".$model->applicantdiv."&SEND_EXAMNO=".$model->examno;
        ...
        View::toHTML($model, "knjl021nForm1.html", $arg);
    }
}
