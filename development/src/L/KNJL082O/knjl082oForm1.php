...
class knjl082oForm1
{
    function main(&$model)
    {
        ...
        //入試制度
        $opt = array();
        $result = $db->query(knjl082oQuery::GetName("L003",$model->ObjYear));
...
#        $opt[2] = array("label" => "3：手続者"    , "value" => "3");
        $opt[0] = array("label" => "2：手続延期者", "value" => "2");
        ...
        //入試区分
        $opt = array();
        $result = $db->query(knjl082oQuery::getTestdivMst($model->ObjYear));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            $opt[]  = array("label" => $row["NAMECD2"]."：".$row["NAME1"], "value" => $row["NAMECD2"]);

            if ($row["NAMESPARE2"] == "1" && !strlen($model->testdiv)){
        ...
        $result = $db->query(knjl082oQuery::GetLeftList($model));
        ...
        //合格者一覧
        $opt_right = array();
        $result = $db->query(knjl082oQuery::GetRightList($model));
        ...
        $arg["start"]   = $objForm->get_start("sel", "POST", "knjl082oindex.php", "", "sel");

        $arg["finish"]  = $objForm->get_finish();

        View::toHTML($model, "knjl082oForm1.html", $arg); 
    }
}
?>
