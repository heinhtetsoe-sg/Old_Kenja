...
class knjl322rForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl322rForm1", "POST", "knjl322rindex.php", "", "knjl322rForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl322rQuery::getNameMst($model, $model->ObjYear, "L003"));
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl322rQuery::getNameMst($model, $model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL322R");
        ...
        View::toHTML($model, "knjl322rForm1.html", $arg); 
