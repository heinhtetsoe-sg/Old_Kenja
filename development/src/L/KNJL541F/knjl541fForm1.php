...
class knjl541fForm1
{
    function main(&$model) {
        ...
        $query = knjl541fQuery::getNameMst("L003", $model->year);
        makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->applicantdiv, $extra, 1);

        //入試区分コンボボックス
        $extra = "onchange=\"return btn_submit('main');\" tabindex=-1";
        $query = knjl541fQuery::getNameMst($model->applicantdiv == '1' ? "L024" : "L004", $model->year);
        ...
        if ($model->applicantdiv != "" && $model->testdiv != "") {
            ...
            //データ取得
            $result = $db->query(knjl541fQuery::SelectQuery($model));
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjl541findex.php", "", "main");

        //フォーム終わり
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl541fForm1.html", $arg);
    }
}
