...
class searchNames
{
    ...
    public function getNameList()
    {
        ...
        if (strlen($this->names) || strlen($this->name_kana) || strlen($this->receptno)) {
            ...
            $query .= "WHERE T1.ENTEXAMYEAR = '".$this->year."'";
            ...
            if (strlen($this->receptno)) {
                $query .= " AND ( ";
                $strOR = "";
                foreach ($testdivArray as $key => $codeArray) {
                    $testdiv = $codeArray["TESTDIV"];
                    $query .= " {$strOR} BASE_D_012.REMARK{$testdiv} like '%" .$this->receptno ."%' ";
                    $strOR = "OR";
                }
                $query .= " ) ";
            }
    ...
    //氏名かなテキストボックス
    public function getTextName_Kana()
    {
        //氏名かな
        $this->form->ae(array("type"        => "text",
                               "name"        => "name_kana",
    ...
    //受験番号(RECEPTNO)テキストボックス
    public function getTextReceptno()
    {
        //受験番号(RECEPTNO)
        $this->form->ae(array("type"        => "text",
                               "name"        => "receptno",
...
<!--
public  f = <?php echo $objName->getFrame() ?>;

public function setAddrText(obj) {
    try{
        public val = obj.options[obj.selectedIndex].text;    
        public arr = val.split("　");
...
public function apply_name(obj,examno,zip,gzip,zadd,gadd) {
...
public function btn_submit(cmd) {
