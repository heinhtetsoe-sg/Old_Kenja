...
class knjl083qForm1 {
    ...
    function main(&$model) {
        ...
        $arg["start"] = $objForm->get_start("sel", "POST", "knjl083qindex.php", "", "sel");
        ...
        //入試制度
        $query = knjl083qQuery::getNameMst($model->ObjYear, "L003");
        $extra = "Onchange=\"btn_submit('main');\"";
        makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->applicantdiv, $extra, 1);

        //入試区分
        if (SCHOOLKIND == "J") {
            $query = knjl083qQuery::getNameMst($model->ObjYear, "L024");
        } else {
            $query = knjl083qQuery::getNameMst($model->ObjYear, "L004");
        }
        $extra = "Onchange=\"btn_submit('main');\"";
        ...
        if (SCHOOLKIND != "J") {
            //入試区分0
            $getData = $db->getRow(knjl083qQuery::getNameMst($model->ObjYear, "L004", $model->testdiv), DB_FETCHMODE_ASSOC);
            knjCreateHidden($objForm, "TESTDIV0", $getData["ABBV3"]);
        }

        //リストタイトル用
        $arg["LEFT_TITLE"]  = "入学者一覧";
        $arg["RIGHT_TITLE"] = "合格者一覧";

        //入学者・合格者一覧
        $opt_left = $opt_right = array();
        $query = knjl083qQuery::GetLeftList($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $row["PRO_DATE"] = str_replace("-","/",$row["PRO_DATE"]);
            if ($row["LEFT_FLG"] == "1") {
                //入学者一覧
                $shdiv = ($getData["ABBV3"] == "1") ? "：".$row["SHDIV"] : "";
                $opt_left[]  = array("label" => $row["EXAMNO"]."：".$row["NAME"]."：".$row["PRO_DATE"].$shdiv,
                                     "value" => $row["EXAMNO"]);
            } else {
                //合格者一覧
        ...
        //専併区分コンボ
        $query = knjl083qQuery::getNameMst($model->ObjYear, "L006");
        makeCmb($objForm, $arg, $db, $query, "SHDIV", $model->shdiv, "", 1);
        if ($getData["ABBV3"] == "1") $arg["testdiv0_1"] = 1;
        ...
        View::toHTML($model, "knjl083qForm1.html", $arg); 
    }
}
