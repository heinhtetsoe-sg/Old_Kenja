...
class knjl083rForm1 {
    ...
    function main(&$model) {
        ...
        //入試制度
        $query = knjl083rQuery::getNameMst($model->ObjYear, "L003", $model->fixApplicantDiv);
        $extra = "Onchange=\"btn_submit('main');\"";
        makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->applicantdiv, $extra, 1);

        //入試区分
        $namecd1 = $model->applicantdiv == "2" ? "L024" : "L004";
        $query = knjl083rQuery::getNameMst($model->ObjYear, $namecd1);
        $extra = "Onchange=\"btn_submit('main');\"";
        ...
        $query = knjl083rQuery::getLeftRightList($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $row["PRO_DATE"] = str_replace("-","/",$row["PRO_DATE"]);
            if ($row["LEFT_FLG"] == "1") {
                //手続者一覧
                $opt_left[]  = array("label" => $row["RECEPTNO"]."(".$row["EXAMNO"].")：".$row["NAME"]."：".$row["PRO_DATE"],
                                     "value" => $row["RECEPTNO"]."-".$row["EXAMNO"]);
            } else {
                //合格者一覧
                $opt_right[] = array("label" => $row["RECEPTNO"]."(".$row["EXAMNO"].")：".$row["NAME"],
                                     "value" => $row["RECEPTNO"]."-".$row["EXAMNO"]);
            }
        }
        $result->free();
        ...
        $arg["start"] = $objForm->get_start("sel", "POST", "knjl083rindex.php", "", "sel");
        $arg["finish"] = $objForm->get_finish();

        View::toHTML($model, "knjl083rForm1.html", $arg); 
    }
}
