...
class knjl571hForm1
{
    public function main(&$model)
    {
        ...
        //学校種別
        $query = knjl571hQuery::getNameMst($model->year, "L003");
        $extra = "onchange=\"return btn_submit('read');\"";
        makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->applicantdiv, $extra, 1, "");

        //入試判別
        $query = knjl571hQuery::getDistinctId($model);
        $extra = "onchange=\"return btn_submit('read');\"";
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL571H");
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjl571hindex.php", "", "main");

        //フォーム終わり
        $arg["finish"] = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl571hForm1.html", $arg);
    }
}
//クラス一覧リストToリスト作成
function makeListToList(&$objForm, &$arg, $db, $model)
{
    ...
    $leftCnt = $rightCnt = 0;
    $query = knjl571hQuery::getReceptDatData($model);
    ...
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        if ($row["JUDGEDIV"] != "1" && !in_array($row["VALUE"], $leftArr)) {
