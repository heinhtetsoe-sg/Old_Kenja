...
class knjl423Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl423Form1", "POST", "knjl423index.php", "", "knjl423Form1");
        ...
        //分類コンボ作成
        $query = knjl423Query::getRecruitClass();
        $extra = "onchange=\"return btn_submit('knjl423');\"";
        makeCmb($objForm, $arg, $db, $query, "EVENT_CLASS_CD", $model->field["EVENT_CLASS_CD"], $extra, 1);

        //イベントコンボ作成
        $query = knjl423Query::getEvent($model);
        $extra = "onchange=\"return btn_submit('knjl423');\"";
        makeCmb($objForm, $arg, $db, $query, "EVENT_CD", $model->field["EVENT_CD"], $extra, 1);

        //案内コンボ作成
        $query = knjl423Query::getRecruitSendYmst($model);
        $extra = "onchange=\"return btn_submit('knjl423');\"";
...
//            $extra = "onclick=\"return mail_send('');\"";
            ...
            //更新・印刷ボタン
            $extra = "onclick=\"return btn_submit('add');\"";
            $arg["button"]["btn_add"] = knjCreateBtn($objForm, "btn_add", $model->field["SEND_METHOD"] == "03" ? "更 新" : "更新／印刷／プレビュー", $extra);
        }
        //終了ボタン
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL423");
        ...
        View::toHTML($model, "knjl423Form1.html", $arg);
    }
}
//リストToリスト作成
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    //塾一覧取得
    $optR = $optL = array();
    $query = knjl423Query::getRecruitDat();
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        //hidden
        knjCreateHidden($objForm, "EMAIL_".$row["VALUE"], $row["EMAIL"]);

        if ($model->cmd != "knjl423" && in_array($row["VALUE"], $selectdata)) {
