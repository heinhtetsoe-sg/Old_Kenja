...
class knjl011sForm1
{
    ...
    {
        $objForm      = new form;
        $arg["start"] = $objForm->get_start("main", "POST", "knjl011sindex.php", "", "main");
        ...
        if ($model->examno == "" || $model->cmd == "showdiv") {
            $db = Query::dbCheckOut();
            $result = $db->query(knjl011sQuery::get_name_cd($model->year, 'L003'));
            ...
            $model->examno = $db->getOne(knjl011sQuery::getMinExam($defApplicantDiv));
            Query::dbCheckIn($db);
        }
        ...
        if ((!isset($model->warning)) && (!is_array($existdata))) {
            //データを取得
            $Row = knjl011sQuery::get_edit_data($model);
        ...
        if ($model->cmd == 'major' || $model->cmd == 'fin_type') {
            $Row =& $model->field;
        }
        ...
        $result = $db->query(knjl011sQuery::getMajorLMst());
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            $optMajorL[] = array("label" => $row["LABEL"], "value" => $row["VALUE"]);
            if ($Row["MAJORLCD1"] == "") $Row["MAJORLCD1"] = $row["VALUE"];
        }
        $result->free();
        //志望区分（順位）コンボ
        for ($m = 1; $m <= 6; $m++) {
            $defNameCd = "L020";
            $defCdRow = $db->getRow(knjl011sQuery::get_name_cd($model->year, $defNameCd, $m), DB_FETCHMODE_ASSOC);
            ...
            $result = $db->query(knjl011sQuery::getMajorSMst($Row[$nameL] ? $Row[$nameL] : $defLcd));
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
            {
                $optMajorS[] = array("label" => $row["LABEL"], "value" => $row["VALUE"]);
                $extra = "";
                if ($Row["MAJORSCD1"] == "") $Row["MAJORSCD1"] = $row["VALUE"];
            }
            $result->free();
            $nameS = "MAJORSCD" .$m;
            $objForm->ae( array("type"        => "select",
        ...
        //学校コード取得
        $query = knjl011sQuery::getSchoolname();
        ...
        $result = $db->query(knjl011sQuery::getSuisenKubun($model->year));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        //学力検査受検希望教科など
        $optTestSubcd = array();
        $result = $db->query(knjl011sQuery::getTestSubcd($model->year));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        //性別コンボ
        $result     = $db->query(knjl011sQuery::get_name_cd($model->year, 'Z002'));
        $opt        = array();
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            $opt[] = array("label" => $row["NAMECD2"].":".$row["NAME1"],
        ...
        $result = $db->query(knjl011sQuery::get_name_cd($model->year, 'L016'));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        //校種(L019) 1:幼稚園 2:小学校 3:中学校 4:高校
        $opt = array(1, 2, 3, 4);
        $defType = $db->getOne(knjl011sQuery::getFinschoolType($model->year, $Row["FS_CD"]));
        if (!strlen($defType)) $defType = "3";
        $model->finschool_type = ($model->cmd == 'fin_type') ? $model->finschool_type : $defType;
        foreach($opt as $key => $val) $extraArray[$key] = "id=\"FINSCHOOL_TYPE{$val}\" onclick=\"change_flg(); return btn_submit('fin_type');\"";
        $radioArray = knjCreateRadio($objForm, "FINSCHOOL_TYPE", $model->finschool_type, $extraArray, $opt, get_count($opt));
        ...
        $result     = $db->query(knjl011sQuery::getFinschoolcd($model->year, $model->finschool_type));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
...
/***
        ...
        $objForm->ae( array("type"        => "text",
                            ...
                            "size"        => 7,
                            "maxlength"   => 7,
                            "extrahtml"   => "onchange=\"change_flg()\" onblur=\"hiddenWin('" .REQUESTROOT ."/L/KNJL011S/search_fin_name.php?cmd=apply&year='+document.forms[0]['year'].value+'&examno='+document.forms[0]['EXAMNO'].value+'&schoolcd='+document.forms[0]['FS_CD'].value+'&frame='+getFrameName(self))\"",
                            "value"       => $Row["FS_CD"] ) );
...
***/
        ...
        //------------------------------内申科目---------------------------------
        $result = $db->query(knjl011sQuery::get_name_cd($model->year, "L008"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        $objForm->ae( array("type"        => "button",
                            ...
                            "value"       => "かな検索",
                            "extrahtml"   => "style=\"width:70px\" onclick=\"loadwindow('" .REQUESTROOT ."/L/KNJL011S/search_name.php?cmd=search&year='+document.forms[0]['year'].value+'&examno='+document.forms[0]['EXAMNO'].value+'&NAME_SESSID={$sess->id}&frame='+getFrameName(self), event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 260)\"") );
        ...
        $objForm->ae( array("type"        => "button",
                            ...
                            "value"       => "検 索",
                            "extrahtml"   => "onclick=\"loadwindow('" .REQUESTROOT ."/L/KNJL011S/search_fin_name.php?cmd=search&year='+document.forms[0]['year'].value+'&examno='+document.forms[0]['EXAMNO'].value+'&NAME_SESSID={$sess->id}&frame='+getFrameName(self), event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 335, 370)\"") );
...
***/
        ...
        View::toHTML($model, "knjl011sForm1.html", $arg);
    }
}
?>