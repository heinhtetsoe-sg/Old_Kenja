...
class knjl335qForm1 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl335qForm1", "POST", "knjl335qindex.php", "", "knjl335qForm1");
        ...
        //入試制度コンボボックス
        $extra = " onchange=\"return btn_submit('knjl335q');\"";
        $query = knjl335qQuery::getNameMst($model->ObjYear, "L003");
        makeCmb($objForm, $arg, $db, $query, $model->field["APPLICANTDIV"], "APPLICANTDIV", $extra, 1);

        //入試区分コンボボックス
        $extra = " onchange=\"return btn_submit('knjl335q');\"";
        if (SCHOOLKIND == "J") {
            $query = knjl335qQuery::getNameMst($model->ObjYear, "L024");
        } else {
            $query = knjl335qQuery::getNameMstL004($model->ObjYear);
        }
        makeCmb($objForm, $arg, $db, $query, $model->field["TESTDIV"], "TESTDIV", $extra, 1);

        //ラジオボタン（1:入学試験、2:入学試験＆基準試験名簿）
        $opt = array(1, 2, 3);
        $model->field["OUTPUT_SELECT"] = ($model->field["OUTPUT_SELECT"] == "") ? "1" : $model->field["OUTPUT_SELECT"];
        $extra = array();
        foreach($opt as $key => $val) {
            array_push($extra, " id=\"OUTPUT_SELECT{$val}\" onClick=\"btn_submit('')\"");
        }
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL335Q");
        ...
        View::toHTML($model, "knjl335qForm1.html", $arg); 
    }
}
