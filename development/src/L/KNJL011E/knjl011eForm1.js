function btn_submit(cmd, gzip, gadd) {
    ...
    if (cmd == 'disp_clear') {
        for (i = 0; i < document.forms[0].elements.length; i++) {
            if (document.forms[0].elements[i].type == 'select-one' || 
                ...
                if (document.forms[0].elements[i].type == 'select-one') {
                    ...
                    if (document.forms[0].elements[i].id.match(/[1]$/)) {
        ...
        outputLAYER('TESTDIV1',  '');
        outputLAYER('DESIREDIV',  '');
        outputLAYER('SHDIV',  '');
        outputLAYER('LOGIN_ID',  '');
        for (var i=1; i <= 6; i++) {
            outputLAYER('H_TESTDIV_' + i,  '');
            outputLAYER('H_DESIREDIV_' + i,  '');
            outputLAYER('H_EXAMNO_' + i,  '');
        }
