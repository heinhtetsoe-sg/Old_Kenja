...
class knjl021yForm1
{
    function main(&$model)
    {
        ...
        //入試制度
        $opt = array();
        $result = $db->query(knjl021yQuery::GetName("L003", $model->ObjYear));
        ...
        //受験番号自
        if ($model->cmd == "main") {
            $model->examno = $db->getOne(knjl021yQuery::getMinExam($model));
        ...
        if ($model->cmd == "read" || $model->cmd == "back" || $model->cmd == "next")
        {
            ...
            $query  = knjl021yQuery::SelectQuery($model);
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjl021yindex.php", "", "main");

        //フォーム終わり
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。 
        View::toHTML($model, "knjl021yForm1.html", $arg); 
    }
}
