...
class knjl012cForm1
{
    function main(&$model)
    {
        $db = Query::dbCheckOut();
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("main", "POST", "knjl012cindex.php", "", "main");
        ...
        //入試制度
        $opt = array();
        $result = $db->query(knjl012cQuery::GetName("L003",$model->ObjYear));
        ...
        //出願区分
        $opt = array();
        $result = $db->query(knjl012cQuery::GetName("L004",$model->ObjYear));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            $opt[]    = array("label" => $row["NAMECD2"]."：".$row["NAME1"], "value" => $row["NAMECD2"]);
        ...
        View::toHTML($model, "knjl012cForm1.html", $arg);
    }
}
?>
