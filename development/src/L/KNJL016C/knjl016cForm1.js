function btn_submit(cmd, zip, gzip, zadd, gadd) 
{
    ...
    if (cmd == 'addnew') {
        if (document.forms[0].APPLICANTDIV.value == '2' && document.forms[0].TESTDIV.value == '7' && 
            document.forms[0].SELECT_SUBCLASS_DIV.value == '') {
            alert('{rval MSG310}\n( 選択受験科目 )');
            return true;
        }
    }
    ...
    if (cmd == 'disp_clear' || cmd == 'showdiv') {
        ...
        for (i = 0; i < document.forms[0].elements.length; i++) {
            if (document.forms[0].elements[i].type == 'select-one' || document.forms[0].elements[i].type == 'text' || document.forms[0].elements[i].type == 'checkbox') {
                if (document.forms[0].elements[i].name == 'APPLICANTDIV' || document.forms[0].elements[i].name == 'TESTDIV' || document.forms[0].elements[i].name == 'SHDIV') {
                    //入試制度・入試区分・専併区分はクリアしない
