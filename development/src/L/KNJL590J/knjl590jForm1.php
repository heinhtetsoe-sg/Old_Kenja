...
class knjl590jForm1
{
    ...
    {
        $objForm      = new form;
        $arg["start"] = $objForm->get_start("main", "POST", "knjl590jindex.php", "", "main");
        $db = Query::dbCheckOut();

/****************************************** GET DATA!! ***************************************************************/        

        $Row = $db->getRow(knjl590jQuery::get_edit_data($model), DB_FETCHMODE_ASSOC);
        if (!is_array($Row)) {
        ...
        //学校種別
        $query = knjl590jQuery::getNameMst($model->year, "L003");
        ...
        //入試種別
        $query = knjl590jQuery::getTestDiv($model);
        $extra = "onChange=\"change_flg(); return btn_submit('changeTest');\"";
...
/*********************************************************** 表示 *************************************************************/
        ...
        //合否コンボ
        knjl590jForm1::FormatOpt($opt["L013"]);
        if (strlen($model->examno)) $opt["L013"] = knjl590jForm1::GetOpt($db, $model->year, array("L013"));
        $arg["data"]["JUDGEMENT"] = knjl590jForm1::CreateCombo($objForm, "JUDGEMENT", $Row["JUDGEMENT"], "100", $opt["L013"], "onChange=\"change_flg(), btn_submit('change')\";");
        $model->judgement = $Row["JUDGEMENT"];

        //判定名称
        $result = $db->query(knjl590jQuery::getName($model->year, array("L013")));
...
/**************************************** CREATE 15 COMBOS ! ************************************************************/
        ...
        knjl590jForm1::FormatOpt($opt["L011"]);
        knjl590jForm1::FormatOpt($opt["L012"]);
        ...
        if ($judgeNameSpare1[$model->judgement] == '1') {
            ...
            //手続区分に追加
            $opt["L011"] = knjl590jForm1::GetOpt($db, $model->year, array("L011"));
            ...
            if ($model->procedurediv == "1") {
                ...
                $disabled_date = "";
                $disNo = "";
                $opt["L012"] = knjl590jForm1::GetOpt($db, $model->year, array("L012"));
        ...
        //手続区分コンボ
        $arg["data"]["PROCEDUREDIV"] = knjl590jForm1::CreateCombo($objForm, "PROCEDUREDIV", $model->procedurediv, "100", $opt["L011"], "onChange=\"change_flg(), btn_submit('change')\";");

        //手続日
        $arg["data"]["PROCEDUREDATE"] = View::popUpCalendar2($objForm, "PROCEDUREDATE", str_replace("-", "/", $Row["PROCEDUREDATE"]), "", "", $disabled_date);

        //入辞区分コンボ
        $arg["data"]["ENTDIV"] = knjl590jForm1::CreateCombo($objForm, "ENTDIV", $model->entdiv, "100", $opt["L012"], "onChange=\"change_flg(), btn_submit('change')\";");

        //学籍番号
        $extra = "STYLE=\"ime-mode: inactive\" onChange=\"change_flg();\" onblur=\"checkNo(this);\"";
...
/********************************************************* SET DATA ********************************************************/
        ...
        foreach ($model->subClassArr as $subCd => $subName) {
            $arg["SUB_NAME".$subCd] = $subName;
        }

        //成績・内申備考
        $query  = knjl590jQuery::getScoreRemark($model);
        $result = $db->query($query);
        while ($srRow = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $arg["data2"][] = $srRow;
        }
        $result->free();
...
/***************************************************** CREATE BUTTONS ********************************************************/
        ...
        global $sess;
        //かな検索ボタン
        $extra = "style=\"width:80px\" onclick=\"loadwindow('" .REQUESTROOT ."/L/KNJL590J/search_name.php?cmd=search&year='+document.forms[0]['year'].value+'&applicantdiv='+document.forms[0]['APPLICANTDIV'].value+'&testdiv='+document.forms[0]['TESTDIV'].value+'&examno='+document.forms[0]['EXAMNO'].value+'&NAME_SESSID={$sess->id}&frame='+getFrameName(self), event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 260)\"";
        ...
        View::toHTML($model, "knjl590jForm1.html", $arg);
    }
    ...
    function GetOpt(&$db, $year, $namecd, $flg=1, $namecd2="", $namecd3="") {
        ...
        if (is_array($namecd)) {
            $result = $db->query(knjl590jQuery::getName($year, $namecd));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
            {
