...
class knjl580fForm1
{
    function main(&$model)
    {
        ...
        //入試制度
        $query = knjl580fQuery::GetName("L003", $model->ObjYear);
        ...
        $result = $db->query(knjl580fQuery::GetName($namecd1, $model->ObjYear));
        ...
        $query = knjl580fQuery::GetLeftList($model);
        ...
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            if ($row["LEFT_FLG"] == "1") {
                //対象者一覧
                $opt_left[]  = array("label" => $row["RECEPTNO"]."：".$row["NAME"]."：".$row["PRO_DATE"],
                                     "value" => $row["RECEPTNO"].":".$row["EXAMNO"]);
            } else {
                //合格者一覧
                $opt_right[] = array("label" => $row["RECEPTNO"]."：".$row["NAME"],
                                     "value" => $row["RECEPTNO"].":".$row["EXAMNO"]);
            }
        }
        $result->free();
        ...
        //対象者一覧
        $extra = "multiple style=\"width:300px; \" ondblclick=\"move3('right','SPECIALS','APPROVED',1);\"";
        $arg["main_part"]["LEFT_PART"] = knjCreateCombo($objForm, "SPECIALS", "left", $opt_left, $extra, 20);

        //合格者一覧
        $extra = "multiple style=\"width:300px \" ondblclick=\"move3('left','SPECIALS','APPROVED',1);\"";
        ...
        $arg["start"]   = $objForm->get_start("sel", "POST", "knjl580findex.php", "", "sel");

        $arg["finish"]  = $objForm->get_finish();

        View::toHTML($model, "knjl580fForm1.html", $arg); 
    }
}
