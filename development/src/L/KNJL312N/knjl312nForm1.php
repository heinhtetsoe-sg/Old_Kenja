...
class knjl312nForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl312nForm1", "POST", "knjl312nindex.php", "", "knjl312nForm1");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl312nQuery::getNameMst($model->ObjYear, "L003"));
        ...
        $extra = " onchange=\"return btn_submit('knjl312n');\"";
        $arg["data"]["APPLICANTDIV"] = knjCreateCombo($objForm, "APPLICANTDIV", $model->field["APPLICANTDIV"], $opt, $extra, 1);

        //入試区分
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl312nQuery::getNameMst($model->ObjYear, "L004"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjl312nQuery::getNameMst($model->ObjYear, "L009"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        //志望区分コンボ
        $query = knjl312nQuery::getExamcourse($model->ObjYear, $model->field["APPLICANTDIV"], $model->field["TESTDIV"]);
        $extra = "onChange=\"btn_submit('knjl312n');\"";
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL312N");
        ...
        View::toHTML($model, "knjl312nForm1.html", $arg); 
