...
class knjl082qForm1 {
    ...
    function main(&$model) {
        ...
        $arg["start"] = $objForm->get_start("sel", "POST", "knjl082qindex.php", "", "sel");
        ...
        //入試制度
        $query = knjl082qQuery::getNameMst($model->ObjYear, "L003");
        $extra = "Onchange=\"btn_submit('main');\"";
        makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->applicantdiv, $extra, 1);

        //入試区分
        if (SCHOOLKIND == "J") {
            $query = knjl082qQuery::getNameMst($model->ObjYear, "L024");
        } else {
            $query = knjl082qQuery::getNameMst($model->ObjYear, "L004");
        }
        $extra = "Onchange=\"btn_submit('main');\"";
        makeCmb($objForm, $arg, $db, $query, "TESTDIV", $model->testdiv, $extra, 1);

        //リストタイトル用
        $arg["LEFT_TITLE"]  = "延納対象者一覧";
        $arg["RIGHT_TITLE"] = "合格者一覧";

        //対象者・合格者一覧
        $opt_left = $opt_right = array();
        $query = knjl082qQuery::GetLeftList($model);
        ...
        View::toHTML($model, "knjl082qForm1.html", $arg); 
    }
}
