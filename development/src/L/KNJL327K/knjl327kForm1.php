...
class knjl327kForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjl327kForm1", "POST", "knjl327kindex.php", "", "knjl327kForm1");
        ...
        $opt = array();
        $value_flg = false;
        $query = knjl327kQuery::getSpecialReasonDiv($model);
        ...
        //中高判別フラグを作成する
        $jhflg = 0;
        $row = $db->getOne(knjl327kQuery::GetJorH());
        $jhflg = ($row == 1) ? 1 : 2;
        ...
        View::toHTML($model, "knjl327kForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, &$model)
{
    ...
    $query = knjl327kQuery::GetExamno($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
...
function makeHidden(&$objForm, $model, $jhflg)
{
    ...
    $objForm->ae(createHiddenAe("PRGID", "KNJL327K"));
