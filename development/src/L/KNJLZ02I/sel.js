...
function add()
{
    var temp1 = new Array();
    var tempa = new Array();
    var v = document.forms[0].examyear.length;
    ...
    for (var i = 0; i < v; i++)
    {   
        if (w == document.forms[0].examyear.options[i].value) {
            alert("追加した年度は既に存在しています。");
            return false;
        }
    }
    document.forms[0].examyear.options[v] = new Option();
    document.forms[0].examyear.options[v].value = w;
    document.forms[0].examyear.options[v].text = w;
    
    for (var i = 0; i < document.forms[0].examyear.length; i++)
    {  
        temp1[i] = document.forms[0].examyear.options[i].value;
        tempa[i] = document.forms[0].examyear.options[i].text;
    } 
    //sort
    temp1 = temp1.sort();
    tempa = tempa.sort();
    ...
    ClearList(document.forms[0].examyear,document.forms[0].examyear);
    if (temp1.length>0)
    {   
        for (var i = 0; i < temp1.length; i++)
        {   
            document.forms[0].examyear.options[i] = new Option();
            document.forms[0].examyear.options[i].value = temp1[i];
            document.forms[0].examyear.options[i].text =  tempa[i];
            if(w==temp1[i]){
                document.forms[0].examyear.options[i].selected=true;
            }
        }
    } 
//    temp_clear();
