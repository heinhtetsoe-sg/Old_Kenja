...
class knjl351w_2Form1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjl351w_2index.php", "", "main");
        ...
        //radio
        $opt = array(1, 2, 3);
        $model->field["OUTPUT_DIV"] = ($model->field["OUTPUT_DIV"] == "") ? "3" : $model->field["OUTPUT_DIV"];
        $extra = array();
        ...
        //入試制度コンボ
        $query = knjl351w_2Query::get_name_cd($model->entexamYear, "L003");
        $extra = "onchange=\"return btn_submit('main');\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["APPLICANTDIV"], "APPLICANTDIV", $extra, 1, "");

        //入試区分
        $query = knjl351w_2Query::get_name_cdAft($model, "L004", "TESTDIV");
        $extra = "onchange=\"return btn_submit('main');\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["TESTDIV"], "TESTDIV", $extra, 1, "");

        //累計区分
        $query = knjl351w_2Query::get_name_cdAft($model, "L040", "RUIKEI_DIV");
        $extra = "onchange=\"return btn_submit('main');\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["RUIKEI_DIV"], "RUIKEI_DIV", $extra, 1, "");
        ...
        View::toHTML($model, "knjl351w_2Form1.html", $arg);
    }
}
...
{
    $row1 = array();
    $query = knjl351w_2Query::getSchoolData($model);
...
function makeBtn(&$objForm, &$arg) {
    ...
    $arg["button"]["btn_cancel"] = knjCreateBtn($objForm, "btn_cancel", "報告取り下げ", $extra);
    //終了ボタン
