...
function doSubmit() {
    ...
    if (document.forms[0].SPECIALS.length > 0){
        ...
        {
            var str = document.forms[0].SPECIALS.options[i].value;
            var val = str.split('-');
            attribute3.value = attribute3.value + sep + val[1] + '-' + val[2];
            sep = ",";
        }
    }
    ...
    if (document.forms[0].APPROVED.length > 0){
        ...
        {
            var str = document.forms[0].APPROVED.options[i].value;
            var val = str.split('-');
            attribute4.value = attribute4.value + sep + val[1] + '-' + val[2];
            sep = ",";
        }
    }
...
function move2(side, left, right, sort)
{
    ...
    for (var i = 0; i < attribute1.length; i++) {
        var str = attribute1.options[i].value;
        var val = str.split('-');
        if (side == "right" || side == "left") {
            if ( attribute1.options[i].selected && val[2] != 2) {
                y=current1++
