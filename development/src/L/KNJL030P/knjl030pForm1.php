...
class knjl030pForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjl030pindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //年度設定
        $result = $db->query(knjl030pQuery::selectYearQuery());
        $opt = array();
        //レコードが存在しなければ処理年度を登録
        if ($result->numRows() == 0) { 
        ...
        if (!$flg) {
            if (!isset($model->year)) {
                ...
                $model->year = $db->getOne(knjl030pQuery::DeleteAtExist($model));
            }
        }
        ...
        $result = $db->query($query);
            ...
            $hash = array("cmd"             => "edit2",
                          ...
                          "RECNO"           => $row["RECNO"]);
            $row["EXAMNO_FROM_TO"] = View::alink("knjl030pindex.php", $row["EXAMNO_FROM_TO"], "target=\"right_frame\"", $hash);
        ...
        if (!$flg || (!isset($model->warning) && VARS::post("cmd") == "copy")) {
            $arg["reload"][] = "parent.right_frame.location.href='knjl030pindex.php?cmd=edit"
                             . "&year=" .$model->year."&APPLICANTDIV=" .$model->applicantdiv."';";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl030pForm1.html", $arg);
    }
}
