...
class knjl030rForm1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjl030rindex.php", "", "main");
        $db           = Query::dbCheckOut();

        $arg["TOP"]["YEAR"] = $model->year ."年度";

        //入試制度コンボ
        $query = knjl030rQuery::getNameMst($model, "L003");
        $extra = "onChange=\"return btn_submit('main')\"";
        makeCmb($objForm, $arg, $db, $query, $model->applicantdiv, "APPLICANTDIV", $extra, 1, "TOP", "");

        //入試区分
        if ($model->applicantdiv == "2") {
            $query = knjl030rQuery::getNameMst($model, "L024");
        } else {
            $query = knjl030rQuery::getNameMst($model, "L004");
        }
        $extra = "onChange=\"return btn_submit('main')\"";
        ...
        //データ
        $result = $db->query(knjl030rQuery::selectQuery($model));
        $arg["data"] = array();
        ...
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            $row["EXAMHALL_NAME"] = View::alink("#",htmlspecialchars($row["EXAMHALL_NAME"]),
                            "onclick=\"loadwindow('knjl030rindex.php?cmd=edit&mode=update&examhallcd=".$row["EXAMHALLCD"] ."',event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(),370,100);\"");
            $arg["data"][] = $row;
        }
        ...
        View::toHTML($model, "knjl030rForm1.html", $arg); 
    }
}
//makeCmb
