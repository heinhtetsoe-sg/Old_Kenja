...
class knjl033bForm2
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjl033bindex.php", "", "edit");
        ...
        if (!isset($model->warning) && ($model->cmd == "edit2") && $model->year && $model->applicantdiv && $model->judge_kind != ""){
            $query = knjl033bQuery::getRow($model->year, $model->applicantdiv, $model->judge_kind);
        ...
        $query = knjl033bQuery::getNameMst($model, "L003", "APPLICANTDIV");
        makeCombo($objForm, $arg, $db, $query, $Row["APPLICANTDIV"], "APPLICANTDIV", $extra, 1, "blank");

        //奨学金種別
        $extra = "onchange=\"return btn_submit('edit');\"";
        $query = knjl033bQuery::getNameMst($model, "L025", "JUDGE_KIND");
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "parent.left_frame.location.href='knjl033bindex.php?cmd=list"
                            . "&year=" .$model->year."';";
        }
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl033bForm2.html", $arg);
    }
}
