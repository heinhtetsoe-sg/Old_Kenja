...
class knjl341kForm1
{
    ...
    $objForm = new form;
    //フォーム作成
    $arg["start"]   = $objForm->get_start("knjl341kForm1", "POST", "knjl341kindex.php", "", "knjl341kForm1");
    $db = Query::dbCheckOut();
    ...
    $result = $db->query(knjl341kQuery::GetTestdiv($model));
    ...
    $objForm->ae( array("type"      => "select",
                        ...
                        "size"      => 1,
                        "value"     => $model->testdiv,
                        "extrahtml" => "onchange =\" return btn_submit('knjl341k');\"",
                        "options"   => $opt_testdiv ) );
    ...
    $opt = array();
    $value_flg = false;
    $query = knjl341kQuery::getSpecialReasonDiv($model);
    ...
    //中高判別フラグを作成する
    $jhflg = 0;
    $row = $db->getOne(knjl341kQuery::GetJorH());
    if ($row == 1){
        $jhflg = 1;
    }else {
        $jhflg = 2;
    ...
    if ($model->testdiv == 2){
        ...
        $arg["data"]["OUTPUT2_3"] = $objForm->ge("OUTPUT2",3);  //2005.09.04
    }
    ...
    View::toHTML($model, "knjl341kForm1.html", $arg); 
    }
}
?>
