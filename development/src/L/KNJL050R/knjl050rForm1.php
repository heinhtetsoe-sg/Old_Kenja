...
class knjl050rForm1
{
    function main(&$model) {
        ...
        $query = knjl050rQuery::getNameMst("L003", $model->ObjYear, $model->fixApplicantDiv);
        makeCmb($objForm, $arg, $db, $query, "APPLICANTDIV", $model->applicantdiv, $extra, 1);

        //入試区分コンボボックス
        if ($model->applicantdiv == "2") {
            $query = knjl050rQuery::getNameMst("L024", $model->ObjYear);
        } else {
            $query = knjl050rQuery::getNameMst("L004", $model->ObjYear);
        }
        makeCmb($objForm, $arg, $db, $query, "TESTDIV", $model->testdiv, $extra, 1);

        //受験科目コンボボックス
        $query = knjl050rQuery::getTestSubclasscd($model);
        makeCmb($objForm, $arg, $db, $query, "TESTSUBCLASSCD", $model->testsubclasscd, $extra, 1);

        //会場コンボボックス
        $query = knjl050rQuery::getHallName($model);
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjl050rindex.php", "", "main");

        //フォーム終わり
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjl050rForm1.html", $arg);
    }
}
...
function makeHidden(&$objForm, $model, $arr_receptno) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJL050R");
