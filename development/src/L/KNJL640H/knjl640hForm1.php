...
class knjl640hForm1
{
    public function main(&$model)
    {
        ...
        $query = knjl640hQuery::getTestdivMst($model);
        ...
        $arg["start"] = $objForm->get_start("main", "POST", "knjl640hindex.php", "", "main");

        $arg["finish"] = $objForm->get_finish();

        View::toHTML($model, "knjl640hForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model, &$dataflg)
{
    ...
    //対象者リストを作成する
    $result = $db->query(knjl640hQuery::getSelectQuery($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        //欠席者一覧(左側)
        if ($row["JUDGEMENT"] == "3") {
