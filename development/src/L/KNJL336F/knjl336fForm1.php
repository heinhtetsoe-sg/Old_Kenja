...
class knjl336fForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjl336fForm1", "POST", "knjl336findex.php", "", "knjl336fForm1");
        ...
        //入試制度コンボボックス
        $extra = " onchange=\"return btn_submit('knjl336f');\"";
        $query = knjl336fQuery::getNameMst($model->ObjYear, "L003");
        makeCmb($objForm, $arg, $db, $query, $model->field["APPLICANTDIV"], "APPLICANTDIV", $extra, 1);

        //入試区分コンボボックス
        $extra = " onchange=\"return btn_submit('knjl336f');\"";
        $namecd1 = ($model->field["APPLICANTDIV"] == "1") ? "L024" : "L004";
        $query = knjl336fQuery::getNameMst($model->ObjYear, $namecd1);
        makeCmb($objForm, $arg, $db, $query, $model->field["TESTDIV"], "TESTDIV", $extra, 1, "ALL");

        //高校のみ
        if ($model->field["APPLICANTDIV"] == "2") {
            //入試回数コンボボックス
            $query = knjl336fQuery::getTestdiv0($model->ObjYear, $model->field["TESTDIV"]);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJL336F");
        ...
        View::toHTML($model, "knjl336fForm1.html", $arg); 
    }
}
