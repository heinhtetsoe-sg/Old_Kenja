...
class knjz064nForm2
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjz064nindex.php", "", "edit");
        ...
        if (isset($model->year) && isset($model->semester) && isset($model->gakubu_school_kind) &&  isset($model->hr_class) &&  isset($model->hr_class2) && isset($model->condition) && isset($model->groupcd) && !isset($model->warning)) {
            if ($model->cmd === 'edit2') {
                $Row = knjz064nQuery::getCompGroupYMst($model);
            } else {
                $Row =& $model->field;
            }
        } else {
        ...
        //状態区分コンボ作成
        $query = knjz064nQuery::getCondition();
        ...
        if ($model->Properties["unuseEduPlan_Group_GuidancePattern"] == "1") {
            ...
            $query = knjz064nQuery::getGuidancePattern($model, $Row);
        ...
        if (VARS::post("cmd") == "update" || VARS::post("cmd") == "delete") {
            $arg["jscript"] = "window.open('knjz064nindex.php?cmd=list&shori=update','left_frame');";
        }
        ...
        View::toHTML($model, "knjz064nForm2.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model, $setcondition, $setgroupcd, $setName)
{
    ...
    //対象科目一覧取得
    $query = knjz064nQuery::getCompGroupDat($model, $setcondition, $setgroupcd);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    //科目一覧取得
    $query = knjz064nQuery::getGradeKindCompSubclassMst($model, $setcondition);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
