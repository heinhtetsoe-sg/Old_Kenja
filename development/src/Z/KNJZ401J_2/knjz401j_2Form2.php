    ...
    public function main(&$model)
    {
        ...
        if (isset($model->grade) && isset($model->subclasscd) && isset($model->viewcd) && !isset($model->warning) && ($model->cmd != "class")) {
            ...
            if (strlen($model->subclasscd) > 6) {
                $Row = $db->getRow(knjz401j_2Query::getRow2($model->grade, $model->subclasscd, $model->viewcd, $model), DB_FETCHMODE_ASSOC);
            } else {
                $Row = $db->getRow(knjz401j_2Query::getRow($model->grade, $model->subclasscd, $model->viewcd, $model), DB_FETCHMODE_ASSOC);
            }
        } else {
            $Row =& $model->field;
        }
        ...
        //教科コンボ
        $query = knjz401j_2Query::getClassMst($model);
        $extra = "onchange=\"return btn_submit('class');\"";
        makeCmb($objForm, $arg, $db, $query, "CLASSCD", $Row["CLASSCD"], $extra, 1, "blank");

        //科目コンボ
        
        $query = knjz401j_2Query::isNameMst044($Row["CLASSCD"], $model);
        if ($db->getOne($query) > 0) {
            $query = knjz401j_2Query::getSubClassMst044($Row["CLASSCD"], $model);
        } else {
            $query = knjz401j_2Query::getSubClassMst($Row["CLASSCD"], $model);
        }
        //教育課程対応
        ...
        //観点名称
        $arg["data"]["VIEWNAME"] = knjCreateTextBox($objForm, $Row["VIEWNAME"], "VIEWNAME", 48, 75, "");
        ...
        $query = knjz401j_2Query::getStudyrecSubcd($Row["CLASSCD"], $model->grade, $model);
        $extra = "onchange=\"return btn_submit('class');\"";
        //教育課程対応
        if ($model->Properties["useCurriculumcd"] == '1') {
            if ($model->cmd == "class") {
        ...
        if ($model->Properties["useCurriculumcd"] == '1') {
            if ($model->cmd == "class") {
                $query = knjz401j_2Query::getStudyrecViewcd($Row["CLASSCD"], $Row["STUDYREC_SUBCLASSCD"], $model->grade, $model);
            } else {
                $query = knjz401j_2Query::getStudyrecViewcd($Row["CLASSCD"], $value, $model->grade, $model);
            }
        } else {
            $query = knjz401j_2Query::getStudyrecViewcd($Row["CLASSCD"], $value, $model->grade, $model);
        }
        ...
        if (VARS::get("cmd") != "edit") {
            $arg["reload"]  = "parent.left_frame.location.href='knjz401j_2index.php?cmd=list';";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjz401j_2Form2.html", $arg);
    }
}
//コンボ作成
