...
class knjz236Form1 {
    function main(&$model) {
        $objForm      = new form;
        $arg["start"] = $objForm->get_start("list", "POST", "knjz236index.php", "", "list");
        ...
        View::toHTML($model, "knjz236Form1.html", $arg); 
    }
}
/***************************************** 以下関数 *********************************************/
//権限チェック
...
function makeClassCmb(&$objForm, &$arg, $db, &$model) {
    $opt = array();
    $result = $db->query(knjz236Query::GetClass($model));
...
function makeSubclassList(&$arg, $db, $model) {
    ...
    $query = knjz236Query::GetSubClass($model, $model->classcd, "MAIN");
    ...
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $link = View::alink("knjz236index.php",
                            $row["VALUE"],
                            "target=right_frame",
                            array("cmd"        => "sel",
                                  "VALUE" => $row["VALUE"]));
        ...
        $cnt = 0;
        $attendData = "";
        $query = knjz236Query::GetSubClass($model, $row["VALUE"]);
        $attendsub = $db->query($query);
        while ($arow = $attendsub->fetchRow(DB_FETCHMODE_ASSOC)) {
            $bgcolor = ($arow["ATTEND_SUBCLASSCD"] != "" && $arow["MAJOR_ATTEND_SUBCLASSCD"] == "") ? "#ffff00" : "#ffffff";
            $attendData .= ($cnt == 0) ? "" : "<tr> ";
            $attendData .= "<td width=\"20%\" bgcolor=\"#ffffff\" nowrap>".$arow["CALCULATE"]."</td> ";
            $attendData .= "<td width=\"*%\" bgcolor=\"{$bgcolor}\" nowrap>".$arow["ATTEND_NAME"]."</td> </tr>";
            $cnt++;
        }
        $attendsub->free();
