...
class knjz217Form1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjz217index.php", "", "edit");
        ...
        View::toHTML($model, "knjz217Form1.html", $arg); 
    }
}
...
function makeList(&$arg, $db, $model)
{
    ...
    $bifKey = "";
    $query = knjz217Query::getList($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        array_walk($row, "htmlspecialchars_array");
        if ($bifKey !== $row["KEY"]) {
            $cnt = $db->getOne(knjz217Query::getSubclassCnt($row));
