...
class knjz072_2aForm2
{
    public function main(&$model)
    {
        ...
        if (isset($model->subclasscd) && !isset($model->warning)) {
            if ($model->cmd !== 'main') {
                $Row = knjz072_2aQuery::getRow($model, $model->school_kind, $model->curriculum_cd, $model->subclasscd);
            } else {
                $Row =& $model->field;
            }
        } else {
        ...
        //教育委員会用の学校コード取得
        $model->schoolcd = "";
        $model->schoolcd = $db->getOne(knjz072_2aQuery::getSchoolCd());

        //教科取得
        $query = knjz072_2aQuery::getClassData($model);
        ...
        if ($model->Properties["useCurriculumcd"] == '1') {
            ...
            //教育課程
            $opt_kyouiku = array();
            $query = knjz072_2aQuery::getNamecd('Z018');
            $value_flg = false;
            $result = $db->query($query);
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        knjCreateHidden($objForm, "SELECT_CLASSCD", $model->select_classcd);
        knjCreateHidden($objForm, "SELECT_SCHOOL_KIND", $model->select_school_kind);
        knjCreateHidden($objForm, "SELECT_CURRICULUM_CD", $model->select_curriculum_cd);

        Query::dbCheckIn($db);
        $arg["finish"]  = $objForm->get_finish();

        if (VARS::get("cmd") != "edit") {
            $arg["reload"]  = "parent.left_frame.location.href='knjz072_2aindex.php?cmd=list&SELECT_CLASSCD=".$model->select_classcd;
            if ($model->Properties["useCurriculumcd"] == '1') {
        ...
        View::toHTML($model, "knjz072_2aForm2.html", $arg);
    }
}
