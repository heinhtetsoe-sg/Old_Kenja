...
class knjz068eForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjz068eForm1", "POST", "knjz068eindex.php", "", "knjz068eForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //IBコースコンボ
        $query = knjz068eQuery::getIBPrgCourse($model);
        $extra = "onchange=\"return btn_submit('main');\"";
        makeCmb($objForm, $arg, $db, $query, "IBPRG_COURSE", $model->ibprg_course, $extra, 1);

        //学年コンボ
        $query = knjz068eQuery::getIBGrade($model);
        $extra = "";
        ...
        knjCreateHidden($objForm, "PRGID",         "KNJZ068E");
        ...
        View::toHTML($model, "knjz068eForm1.html", $arg);
    }
}
/****************************************************** 以下関数 ************************************************************/
//コンボ作成
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    //クラス一覧
    $opt = array();
    $query = knjz068eQuery::getStudent($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $opt[] = array('label' => $row["LABEL"],
                       'value' => $row["VALUE"]);
