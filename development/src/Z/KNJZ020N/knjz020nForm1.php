...
class knjz020nForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjz020nindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //年度設定
        $result = $db->query(knjz020nQuery::selectYearQuery());
        $opt    = array();
        //レコードが存在しなければ処理年度を登録
        if ($result->numRows() == 0) { 
        ...
        if(!$flg) {
            if (!isset($model->year)) {
                ...
                $model->year = $db->getOne(knjz020nQuery::DeleteAtExist($model));
            }
            if ($model->cmd == 'list') {
                $arg["reload"][] = "parent.right_frame.location.href='knjz020nindex.php?cmd=edit"
        ...
        //入試制度コンボ
        $extra = "onchange=\"return btn_submit('list');\"";
        $query = knjz020nQuery::getNameMst($model, "L003");
        ...
        $query = knjz020nQuery::getNameMst($model, "L004");
        makeCombo($objForm, $arg, $db, $query, "TESTDIV", $model->testdiv, $extra, 1);

        //テーブルの中身の作成
        $bifKey = "";
        $query  = knjz020nQuery::selectQuery($model);
        ...
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
            ...
            $row["TESTSUBCLASSCD"] = View::alink("knjz020nindex.php", $row["TESTSUBCLASSCD"].":".$row["TESTSUBCLASSNAME"], "target=\"right_frame\"", $hash);
            
            //重複受験コースはまとめる
            if ($bifKey !== $row["TOTALCD"]) {
                $cnt = $db->getOne(knjz020nQuery::getEntexamCourseCnt($model, $row["TOTALCD"]));
        ...
        View::toHTML($model, "knjz020nForm1.html", $arg);
    }
}
