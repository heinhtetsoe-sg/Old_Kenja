...
class knjz291_swSubForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform1", "POST", "knjz291_swindex.php", "", "subform1");
        ...
        if ($model->Properties["useCurriculumcd"] == '1') {
            if (!isset($model->warning) && isset($model->staffcd) && isset($model->classcd) && isset($model->school_kind) && isset($model->sdate)) {
                $query = knjz291_swQuery::getRow2($model->staffcd, $model->classcd, $model->sdate, $model);
        ...
        //職員情報
        $info = $db->getRow(knjz291_swQuery::getStfInfo2($model->staffcd), DB_FETCHMODE_ASSOC);
        ...
        if($model->staffcd) {
            $result = $db->query(knjz291_swQuery::getClassSdateEdate($model->staffcd, $model));
        ...
        /******************/
        //教科コンボ
        $query = knjz291_swQuery::getName($model);
        ...
        View::toHTML($model, "knjz291_swSubForm1.html", $arg);
    }
}
