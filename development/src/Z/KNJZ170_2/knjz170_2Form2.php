...
class knjz170_2Form2
{
    function main(&$model){
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjz170_2index.php", "", "edit");
        //警告メッセージを表示しない場合

        if (!isset($model->warning))
        {
            $Row = knjz170_2Query::getRow($model->groupcd);
        } else {
            $Row =& $model->field;
        }
        ...
        //終了ボタンを作成する
        $link = REQUESTROOT."/Z/KNJZ170/knjz170index.php";
        $objForm->ae( array("type" => "button",
                            "name"        => "btn_back",
                            "value"       => "戻 る",
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "parent.left_frame.location.href='knjz170_2index.php?cmd=list';";
        }
                                
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。 
        View::toHTML($model, "knjz170_2Form2.html", $arg); 
    }
}        
?>
