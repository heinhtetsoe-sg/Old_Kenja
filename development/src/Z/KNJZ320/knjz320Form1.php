...
class knjz320form1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]  = $objForm->get_start("MENU", "POST", "knjz320index.php", "", "MENU");
        $db = Query::dbCheckOut();

        //教科、科目、クラス取得
        $result = $db->query(knjz320Query::selectQuery($model, $model->root));
        $opt = $arg["data"] = array();
        $menuid = "";
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
            ...
            if (strtoupper($row["PARENTMENUID"]) == "ROOT")
            {
                ...
                $arg["tree"][] = View::alink("knjz320index.php", htmlspecialchars($row["MENUNAME"]),"target=\"right_frame\"",
                                    array("cmd"     => "main",
                                          "MENUID"  => $row["MENUID"],
        ...
        if($model->cmd =="tree" && $model->ini != "1"){
            $arg["reload"] ="window.open('knjz320index.php?cmd=main&MENUID=$menuid&MN=$mname&PATH=$paths', 'right_frame');";
        }
        unset($model->ini);

        $arg["REQUESTROOT"] = REQUESTROOT;
        View::toHTML($model, "knjz320Form1.html", $arg);
    }
}   
?>
