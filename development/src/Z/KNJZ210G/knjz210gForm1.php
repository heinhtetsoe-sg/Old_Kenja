...
class knjz210gForm1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjz210gindex.php", "", "edit");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //学年コンボ
        $query = knjz210gQuery::getGrade($model);
        ...
        if ($model->Properties["use_prg_schoolkind"] == "1") {
            $setSchoolKind = $db->getOne(knjz210gQuery::getSchoolKind($model->field["GRADE"]));
            $setNameCd = "Z".$setSchoolKind."09";
        } else if ($model->Properties["useSchool_KindField"] == "1" && SCHOOLKIND != "") {
            $setNameCd = "Z".SCHOOLKIND."09";
        }
        $result = $db->query(knjz210gQuery::getNameMst($setNameCd));
        ...
        //学期数の取得
        $semestercount = $db->getOne(knjz210gQuery::getSemestercount($setNameCd));

        //名称マスタD029チェック
        $countcheck = $db->getOne(knjz210gQuery::getNamemstCheck("D029"));
        if ($countcheck == 0) {
            $arg["close_win"] = "close_window();";
        }

        //段階値の上限値取得
        $assesslevel = $db->getOne(knjz210gQuery::getAssesslevel("D029"));
        ...
        $key = 0;
        $bifKey1 = $bifKey2 = "";
        $result = $db->query(knjz210gQuery::selectQuery($model, $semestercount, $assesslevel));
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
            ...
            if ($bifKey1 !== $row["KEY1"]) {
                $row["ROWSPAN"] = $row["CNT_KEY1"] > 0 ? $row["CNT_KEY1"] : 1;
            }
            if ($bifKey2 !== $row["KEY1"]."-".$row["KEY2"]) {
                $row["ROWSPAN2"] = $row["CNT_KEY2"] > 0 ? $row["CNT_KEY2"] : 1;
            }
            $bifKey1 = $row["KEY1"];
            $bifKey2 = $row["KEY1"]."-".$row["KEY2"];
        ...
        View::toHTML($model, "knjz210gForm1.html", $arg);
    }
}
