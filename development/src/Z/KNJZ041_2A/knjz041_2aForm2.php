...
class knjz041_2aForm2 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjz041_2aindex.php", "", "edit");

        //警告メッセージを表示しない場合
        if (!isset($model->warning)) {
            $Row = knjz041_2aQuery::getRow($model->coursecd);
        } else {
            $Row =& $model->field;
        }
        ...
        //校時名称
        $result = $db->query(knjz041_2aQuery::getName());
        $opt = array();        
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "parent.left_frame.location.href='knjz041_2aindex.php?cmd=list';";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjz041_2aForm2.html", $arg); 
    }
}
?>
