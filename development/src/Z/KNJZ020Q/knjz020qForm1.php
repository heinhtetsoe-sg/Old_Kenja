...
class knjz020qForm1 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjz020qindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //年度設定
        $result = $db->query(knjz020qQuery::selectYearQuery());
        $opt    = array();
        //レコードが存在しなければ処理年度を登録
        if ($result->numRows() == 0) { 
        ...
        if (!$flg) {
            if (!isset($model->year)) {
                ...
                $model->year = $db->getOne(knjz020qQuery::DeleteAtExist($model));
            }
            if ($model->cmd == 'list') {
                $arg["reload"][] = "parent.right_frame.location.href='knjz020qindex.php?cmd=edit"
        ...
        //入試制度コンボ
        $extra = "onchange=\"return btn_submit('list');\"";
        $query = knjz020qQuery::getNameMst($model, "L003");
        ...
        $query = knjz020qQuery::getNameMst($model, $model->nameMstTest);
        makeCombo($objForm, $arg, $db, $query, "TESTDIV", $model->testdiv, $extra, 1);

        //テーブルの中身の作成
        $model->checkTotalcd = array();
        $bifKey = "";
        $query  = knjz020qQuery::selectQuery($model);
        ...
        while ( $row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            $row["TESTSUBCLASSCD"] = View::alink("knjz020qindex.php", $row["TESTSUBCLASSCD"].":".$row["TESTSUBCLASSNAME"], "target=\"right_frame\"", $hash);
            ...
            if ($bifKey !== $row["TOTALCD"]) {
                $cnt = $db->getOne(knjz020qQuery::getEntexamCourseCnt($model, $row["TOTALCD"]));
        ...
        View::toHTML($model, "knjz020qForm1.html", $arg);
    }
}
