...
class knjz150Form1
{
    function main(&$model)
    {
       //権限チェック
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("sel", "POST", "knjz150index.php", "", "sel");
        $db             = Query::dbCheckOut();

        //対象年度設定
        $result    = $db->query(knjz150Query::selectYearQuery());
        $opt       = array();
        ...
        //教科書年度一覧取得
        $result      = $db->query(knjz150Query::selectTextBookYearQuery($model));
        ...
        //教科書一覧取得
        $result = $db->query(knjz150Query::selectTextBookQuery($opt_left_id,$model));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        //課程マスタボタンを作成する
        $link = REQUESTROOT."/Z/KNJZ150_2/knjz150_2index.php?mode=1";
