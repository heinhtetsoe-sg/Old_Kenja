...
function doSubmit() {
    ...
    attribute3.value = "";
    sep = "";
    if (document.forms[0].issueyear.length==0 && document.forms[0].issuemaster.length==0) {
        alert('データは存在していません。');
        return false;
    }
    for (var i = 0; i < document.forms[0].issueyear.length; i++)
    {
        attribute3.value = attribute3.value + sep + document.forms[0].issueyear.options[i].value;
        sep = ",";
    }
...
function temp_clear()
{
    ClearList(document.forms[0].issueyear,document.forms[0].issueyear);
    ClearList(document.forms[0].issuemaster,document.forms[0].issuemaster);
}
