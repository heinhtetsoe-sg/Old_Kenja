...
class knjz210mForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjz210mindex.php", "", "edit");
        ...
        if (VARS::get("shori") != "add") {
            $arg["reload"] = "window.open('knjz210mindex.php?cmd=edit','right_frame')";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjz210mForm1.html", $arg);
    }
}
...
function makeList(&$arg, $db, $model)
{
    ...
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if (
            $model->grade == $row["GRADE"] && $model->ibprg_course == $row["IBPRG_COURSE"]
