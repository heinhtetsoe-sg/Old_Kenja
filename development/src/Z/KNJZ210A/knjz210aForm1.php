...
class knjz210aform1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjz210aindex.php", "", "main");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        $value_flg = false;
        $query = knjz210aQuery::getCouseName();
        ...
        //教科コンボ作成
        $query = knjz210aQuery::getClassMst($model);
        $extra = "onchange=\"return btn_submit('main')\"";
        makeCmb($objForm, $arg, $db, $query, "CLASSCD", $model->field["CLASSCD"], $extra, 1, "blank");

        //科目コンボ作成
        $query = knjz210aQuery::getSubclassMst($model->field["CLASSCD"], $model);
        $extra = "onchange=\"return btn_submit('main')\"";
        ...
        if (!isset($model->warning)) {
            $query = knjz210aQuery::selectQuery($model);
            $result = $db->query($query);
        } else {
            $row =& $model->field;
            $result = "";
        ...
        View::toHTML($model, "knjz210aForm1.html", $arg);
    }       
}
