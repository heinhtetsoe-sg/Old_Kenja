...
class knjz010oForm1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjz010oindex.php", "", "edit");

        $db = Query::dbCheckOut();

        //年度設定
        $result    = $db->query(knjz010oQuery::selectYearQuery());
        $opt       = array();
        //レコードが存在しなければ処理年度を登録
        if ($result->numRows() == 0) { 
        ...
        if(!$flg) {
            if (!isset($model->year)) {
                ...
                $model->year = $db->getOne(knjz010oQuery::DeleteAtExist($model));
            }
            $arg["reload"][] = "parent.right_frame.location.href='knjz010oindex.php?cmd=edit"
                             . "&year=" .$model->year."';";
        }
        ...
        $result = $db->query(knjz010oQuery::selectQuery($model->year));
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
             ...
             $row["EXAMCOURSECD"] = View::alink("knjz010oindex.php", $row["EXAMCOURSECD"], "target=\"right_frame\"", $hash);
        ...
        View::toHTML($model, "knjz010oForm1.html", $arg);
    }
}
?>
