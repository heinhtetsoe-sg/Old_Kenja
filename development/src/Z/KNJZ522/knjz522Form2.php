...
class knjz522Form2 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjz522index.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        if (!isset($model->warning) && $model->centercd) {
            $query = knjz522Query::getWorkAdviceCenterMst($model->centercd);
        ...
        //活動地域
        $arg["data"]["AREA_LOCAL"] = knjCreateTextBox($objForm, $Row["AREA_LOCAL"], "AREA_LOCAL", 100, 100, "");
        ...
        if (VARS::get("cmd") != "edit") {
            $arg["reload"]  = "window.open('knjz522index.php?cmd=list','left_frame');";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。 
        View::toHTML($model, "knjz522Form2.html", $arg);
    }
}
?>
