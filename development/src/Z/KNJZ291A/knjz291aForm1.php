...
class knjz291aForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("sel", "POST", "knjz291aindex.php", "", "sel");

        $db = Query::dbCheckOut();
        $db2 = Query::dbCheckOut2();

        $query = knjz291aQuery::selectSchoolCd($model);
        $model->schoolCd = $db->getOne($query);

        $no_year = 0;
        //年度設定
        $result = $db->query(knjz291aQuery::selectYearQuery());
        $opt = array();
        ...
        //職員年度一覧取得
        $result = $db->query(knjz291aQuery::selectQuery($model));
        $opt_left_id = $opt_left = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt_left[]    = array("label" => $row["STAFFCD"]."  ".$row["STAFFNAME"].$row["EDBOARD_STAFFCD"],
                                   "value" => $row["STAFFCD"]);
        ...
        //職員一覧取得
        $result = $db->query(knjz291aQuery::selectStaffQuery($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt_right[] = array("label" => $row["STAFFCD"]."  ".$row["STAFFNAME"].$row["EDBOARD_STAFFCD"],
                                 "value" => $row["STAFFCD"]);
        }
        $result->free();
        ...
        if ($model->Properties["useSchoolWare"] == "1") {
            $link = REQUESTROOT."/Z/KNJZ291_SW/knjz291_swindex.php?mode=1&SEND_PRGID=KNJZ291A&cmd=&SEND_SUBMIT=1&SEND_AUTH=".AUTHORITY;
        } else {
            $link = REQUESTROOT."/Z/KNJZ291/knjz291index.php?mode=1&SEND_PRGID=KNJZ291A&cmd=&SEND_SUBMIT=1&SEND_AUTH=".AUTHORITY;
        }
		...
		if($model->Properties["useSchoolWare"] != "1"){
            ...
            $subdata = "onClick=\"wopen('".REQUESTROOT."/Z/KNJZ291_STAFF_REFLECTION/knjz291_staff_reflectionindex.php?&cmd=&SEND_PRGID=KNJZ291A&cmd=&SEND_SUBMIT=1&SEND_AUTH=".AUTHORITY.$extra."\"";
            $syokuin = knjCreateBtn($objForm, "btn_torikomi", "教職員取込", $subdata);
        }
