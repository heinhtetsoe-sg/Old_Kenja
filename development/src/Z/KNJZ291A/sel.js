...
function doSubmit() {
    ...
    attribute3.value = "";
    sep = "";
    if (document.forms[0].staffyear.length==0 && document.forms[0].staffmaster.length==0) {
        alert('データは存在していません。');
        return false;
    }
    for (var i = 0; i < document.forms[0].staffyear.length; i++)
    {
        attribute3.value = attribute3.value + sep + document.forms[0].staffyear.options[i].value;
        sep = ",";
    }
