...
class knjz030hForm1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjz030hindex.php", "", "edit");


        $db = Query::dbCheckOut();

        //年度設定
        $result    = $db->query(knjz030hQuery::selectYearQuery());
        $opt       = array();
        //レコードが存在しなければ処理年度を登録
        if ($result->numRows() == 0) { 
        ...
        if(!$flg) {
            if (!isset($model->examyear) && (CTRL_YEAR+1 == $opt[0]["value"])) {
                ...
                $model->examyear = $db->getOne(knjz030hQuery::DeleteAtExist($model));
            }
            $arg["reload"][] = "parent.right_frame.location.href='knjz030hindex.php?cmd=edit"
                             . "&year=" .$model->examyear."';";
        }
        ...
        //リスト表示
        $result = $db->query(knjz030hQuery::Listdata($model->examyear));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            $row["DESIREDIV"] = View::alink("knjz030hindex.php", $row["DESIREDIV"],"target=right_frame",
            ...
            $row["APPLICANTDIV"] = $row["APPLICANTDIV"]."：".$row["NAME1"];
        ...
        if (VARS::post("cmd") == "list"){
            $arg["reload"]  = "window.open('knjz030hindex.php?cmd=edit','right_frame');";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjz030hForm1.html", $arg);
        }
    }
?>
