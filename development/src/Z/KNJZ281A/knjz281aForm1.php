...
class knjz281aForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("sel", "POST", "knjz281aindex.php", "", "sel");
        ...
        $flg = "";
        $opt = array();
        $result = $db->query(knjz281aQuery::selectYearQuery());
        ...
        //職名年度一覧取得
        $opt_left = array();
        $result = $db->query(knjz281aQuery::selectQuery($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //職名一覧取得
        $opt_right = array();
        $result = $db->query(knjz281aQuery::selectJobQuery($model));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        /****************/
        //extra
        $link = REQUESTROOT."/Z/KNJZ281_2A/knjz281_2aindex.php?mode=1";
        ...
        $link = REQUESTROOT."/Z/KNJZ281_JOB_REFLECTION/knjz281_job_reflectionindex.php";
        $extra_torikomi = "onclick=\"document.location.href='$link'\"";

        $arg["button"] = array("BTN_MASTER" => knjCreateBtn($objForm, "btn_master", "職名マスタ", $extra_mst),
                               ...
                               "BTN_END"    => knjCreateBtn($objForm, "btn_end", "終 了", $extra_end)."&nbsp;&nbsp;".
                                               knjCreateBtn($objForm, "btn_torikomi", "教育委員会職名取込", $extra_torikomi));
