...
class knjz065Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjz065index.php", "", "edit");
        ...
        if ($model->Properties["useGradeKindCompGroupSemester"] == "1") {
            ...
            //学期コンボ作成
            $query = knjz065Query::getSemester($model);
            $extra = "onchange=\"return btn_submit('combo');\"";
            makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->semester, $extra, 1, $model);
        }

        //学部コンボ作成
        $query = knjz065Query::getSchoolKind();
        $extra = "onchange=\"return btn_submit('combo');\"";
        ...
        //一覧表示
        $key = "";
        $query = knjz065Query::getList($model, "", "");
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //列結合
            if ($key !== $row["CONDITION"].'-'.$row["GROUPCD"]) {
                $cnt = $db->getOne(knjz065Query::getList($model, $row["CONDITION"], $row["GROUPCD"]));
                $row["ROWSPAN"] = $cnt > 0 ? $cnt : 1;
            }

            //状態区分
            $row["CONDITION_NAME"] = $db->getOne(knjz065Query::getCondition($row["CONDITION"]));
        ...
        if (VARS::get("shori") != "update") {
            $arg["jscript"] = "window.open('knjz065index.php?cmd=edit','right_frame')";
        }
        ...
        View::toHTML($model, "knjz065Form1.html", $arg); 
    }
}
