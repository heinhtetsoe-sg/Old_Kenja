...
class knjz418Form1 {
    function main(&$model) {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjz418index.php", "", "edit");
        ...
        //大分類コンボ
        $opt = array();
        $query = knjz418Query::getJobtypeLcd($model);
        $value_flg = false;
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
            if ($model->field["JOBTYPE_LCD"] == $row["VALUE"]) $value_flg = true;
        }
        ...
        View::toHTML($model, "knjz418Form1.html", $arg); 
    }
}
...
function makeList(&$arg, $db, $model) {
    $bifKey = "";
    $query = knjz418Query::getList($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        array_walk($row, "htmlspecialchars_array");
        if ($bifKey !== $row["JOBTYPE_MCD"]) {
            $cnt = $db->getOne(knjz418Query::getJobtypeSCnt($row["JOBTYPE_LCD"], $row["JOBTYPE_MCD"]));
