...
function doSubmit() {
    ...
    attribute3.value = "";
    sep = "";
    if (document.forms[0].shareyear.length==0 && document.forms[0].sharemaster.length==0) {
        alert("データは存在していません。");
        return false;
    }
    for (var i = 0; i < document.forms[0].shareyear.length; i++)
    {
        attribute3.value = attribute3.value + sep + document.forms[0].shareyear.options[i].value;
        sep = ",";
    }
...
function temp_clear()
{
    ClearList(document.forms[0].shareyear,document.forms[0].shareyear);
    ClearList(document.forms[0].sharemaster,document.forms[0].sharemaster);
}
