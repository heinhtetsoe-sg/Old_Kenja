...
class knjz010nForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjz010nindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //年度設定
        $result    = $db->query(knjz010nQuery::selectYearQuery());
        $opt       = array();
        //レコードが存在しなければ処理年度を登録
        if ($result->numRows() == 0) { 
        ...
        if(!$flg) {
            if (!isset($model->year)) {
                ...
                $model->year = $db->getOne(knjz010nQuery::DeleteAtExist($model));
            }
            $arg["reload"][] = "parent.right_frame.location.href='knjz010nindex.php?cmd=edit"
                             . "&year=" .$model->year."';";
        }
        ...
        //テーブルの中身の作成
        $query  = knjz010nQuery::selectQuery($model->year);
        ...
        $result = $db->query($query);
            ...
            $row["EXAMCOURSECD"]   = View::alink("knjz010nindex.php", $row["EXAMCOURSECD"], "target=\"right_frame\"", $hash);
        ...
        View::toHTML($model, "knjz010nForm1.html", $arg);
    }
}
?>
