...
class knjz235wForm7
{
    public function main(&$model)
    {
        ...
        View::toHTML($model, "knjz235wForm7.html", $arg);
    }
}
...
{
    $fieldSeq = "SEQ".$seq;
    $query = knjz235wQuery::getPatternUseMainly($namecd2);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $opt[] = $row["NAMECD2"];
    ...
    $radioArray = paternRadio($objForm, $fieldSeq, $value, $extra, $opt, get_count($opt), $opt);
    foreach ($radioArray as $key => $val) {
        $arg["data"][$key] = $val;
    }
}
