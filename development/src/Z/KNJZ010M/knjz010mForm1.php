...
class knjz010mForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjz010mindex.php", "", "edit");

        $db = Query::dbCheckOut();

        //年度設定
        $result    = $db->query(knjz010mQuery::selectYearQuery());
        $opt       = array();
        //レコードが存在しなければ処理年度を登録
        if ($result->numRows() == 0) { 
        ...
        if(!$flg) {
            if (!isset($model->year)) {
                ...
                $model->year = $db->getOne(knjz010mQuery::DeleteAtExist($model));
            }
            $arg["reload"][] = "parent.right_frame.location.href='knjz010mindex.php?cmd=edit"
                             . "&year=" .$model->year."';";
        }
        ...
        $result = $db->query(knjz010mQuery::selectQuery($model->year));
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
             ...
             $row["EXAMCOURSECD"] = View::alink("knjz010mindex.php", $row["EXAMCOURSECD"], "target=\"right_frame\"", $hash);
        ...
        View::toHTML($model, "knjz010mForm1.html", $arg);
    }
}
?>
