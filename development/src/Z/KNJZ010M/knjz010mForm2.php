...
class knjz010mForm2
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjz010mindex.php", "", "edit");

        //警告メッセージを表示しない場合
        if (!isset($model->warning)){
            $Row = knjz010mQuery::getRow($model->year,$model->examcoursecd,$model->applicantdiv,$model->coursecd,$model->majorcd);
        }else{
            $Row =& $model->field;
        }
        ...
        //入試制度コンボ
        $db = Query::dbCheckOut();
        $result    = $db->query(knjz010mQuery::selectApplicantdiv($model->year));
        ...
        //課程学科コンボ
        $db = Query::dbCheckOut();
        $result    = $db->query(knjz010mQuery::selectTotalcd($model->year));
        $opt       = array();
        $opt[] = array("label" => "",
                       "value" => "");
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "parent.left_frame.location.href='knjz010mindex.php?cmd=list"
                            . "&year=" .$model->year."';";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。 
        View::toHTML($model, "knjz010mForm2.html", $arg);
    }
}
?>
