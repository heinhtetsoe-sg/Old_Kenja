...
class knjz438Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjz438index.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjz438Query::getSecurityHigh());
        //セキュリティーチェック
        ...
        /* 取込 */
        //入学年度
        $query = knjz438Query::getSelectFieldSQL();
        $extra = "onchange=\"return btn_submit('');\"";
        makeCmb($objForm, $arg, $db, $query, "ENTYEAR", $model->field["ENTYEAR"], $extra, 1);
        //課程・学科
        $query = knjz438Query::getSelectFieldSQL2($model);
        $extra = "";
        ...
        knjCreateHidden($objForm, "PRGID", "KNJZ438");
        ...
        View::toHTML($model, "knjz438Form1.html", $arg);
    }
}
...
function makeButton(&$objForm, &$arg, $db, $model)
{
    ...
    if ($model->Properties["useXLS"]) {
        $model->schoolCd = $db->getOne(knjz438Query::getSchoolCd());
        $extra = "onclick=\"return newwin('" . SERVLET_URL . "', '" . $model->schoolCd . "', '" . $model->Properties["xlsVer"] . "');\"";
    } else {
        $extra = "onclick=\"return btn_submit('exec');\"";
    }
    $extra = "onclick=\"return btn_submit('exec');\""; //TODO:エクセル出力しないので、ここで上書きしています。
