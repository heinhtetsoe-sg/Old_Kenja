...
class knjz411aSubForm1
{
    public function main(&$model)
    {
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("search", "POST", "knjz411aindex.php", "", "search");
        $db = Query::dbCheckOut();
        ...
        $query = knjz411aQuery::getSchoolGroup();
        ...
        if ($model->cmd == "search") {
            $query = knjz411aQuery::getSchoolList($model);
            $opt = array();
            $opt[] = array("label" => "","value" => "");
            $value_flg = false;
            $cnt = 1;
        ...
        View::toHTML($model, "knjz411aSubForm1.html", $arg);
    }
}
