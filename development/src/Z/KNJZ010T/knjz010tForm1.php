...
class knjz010tForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjz010tindex.php", "", "edit");

        $db = Query::dbCheckOut();

        //年度設定
        $result    = $db->query(knjz010tQuery::selectYearQuery());
        $opt       = array();
        //レコードが存在しなければ処理年度を登録
        if ($result->numRows() == 0) { 
        ...
        if(!$flg) {
            if (!isset($model->year)) {
                ...
                $model->year = $db->getOne(knjz010tQuery::DeleteAtExist($model));
            }
            $arg["reload"][] = "parent.right_frame.location.href='knjz010tindex.php?cmd=edit"
                             . "&year=" .$model->year."';";
        }
        ...
        $result = $db->query(knjz010tQuery::selectQuery($model->year));
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        View::toHTML($model, "knjz010tForm1.html", $arg);
    }
}
?>
