...
class knjz070_3Form2
{
    function main(&$model)
    {
        ...
        //学期名取得
        $model->arraySeme = array();
        $query = knjz070_3Query::getSemName($model, "leftyear");
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $model->arraySeme[$row["SEMESTER"]] = $row["SEMESTERNAME"];
        }
        ...
        for ($i = 1; $i <= 12; $i++ ) {
            ...
            if ($seq === '007' || $seq === '008' || $seq === '012') {
                ...
                $arg["data"]["SEQ_".$seq] = '*';
                $Row["SEQ_".$seq] = "1";
                $arg["data"]["setData_".$seq] = $zenSpace = "";
                foreach ($model->arraySeme as $semester => $semesterName) {
                    $arg["data"]["SEMESTERNAME".$semester] = $semesterName;
                    $arg["data"]["REMARK".$semester."_".$seq] = createCheckBox($objForm, "REMARK".$semester."_".$seq, "1", (($Row["REMARK".$semester."_".$seq] == "1") ? "checked" : "nocheck")." id=\"REMARK".$semester."_".$seq."\"", "");
                    $checkBx = createCheckBox($objForm, "REMARK".$semester."_".$seq, "1", (($Row["REMARK".$semester."_".$seq] == "1") ? "checked" : "nocheck")." id=\"REMARK".$semester."_".$seq."\"", "");
                    $zenSpace = ($zenSpace != "") ? "　" : "";
                    $arg["data"]["setData_".$seq] .= "{$zenSpace}{$checkBx}<LABEL for=\"REMARK{$semester}_{$seq}\">{$semesterName}</LABEL>";
                    $zenSpace = "　";
                }
                if ($seq === '007' || $seq === '008') {
