...
class knjz032oForm1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjz032oindex.php", "", "edit");

        $db = Query::dbCheckOut();

        //年度設定
        $result    = $db->query(knjz032oQuery::selectYearQuery());
        $opt       = array();
        //レコードが存在しなければ処理年度を登録
        if ($result->numRows() == 0) { 
        ...
        if (!$flg) {
            if (!isset($model->year)) {
                ...
                $model->year = $db->getOne(knjz032oQuery::DeleteAtExist($model));
            }
            $arg["reload"][] = "parent.right_frame.location.href='knjz032oindex.php?cmd=edit"
                             . "&year=" .$model->year."';";
        }
        ...
        $result = $db->query(knjz032oQuery::selectQuery($model->year));
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
             ...
             $row["TESTDIV"] = View::alink("knjz032oindex.php", $row["TESTDIV"], "target=\"right_frame\"", $hash);
             $row["TESTDAY"] = str_replace("-", "/", $row["TESTDAY"]);
             $arg["data"][] = $row;
        }
        $result->free();
        ...
        View::toHTML($model, "knjz032oForm1.html", $arg);
    }
}
?>
