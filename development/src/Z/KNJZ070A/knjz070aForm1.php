...
class knjz070aForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("sel", "POST", "knjz070aindex.php", "", "sel");
        ...
        $opt = array();
        $no_year = 0;
        $query = knjz070aQuery::getYearQuery();
        ...
        //年度IB科目一覧取得
        $opt_left = array();
        $query = knjz070aQuery::getYearSubclassQuery($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //IB科目一覧取得
        $opt_right = array();
        $query = knjz070aQuery::getSubclassQuery($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        View::toHTML($model, "knjz070aForm1.html", $arg); 
    }
}
...
function makeBtn(&$objForm, &$arg) {
    //教科マスタボタン
    $link = REQUESTROOT."/Z/KNJZ070_2A/knjz070_2aindex.php?mode=1";
