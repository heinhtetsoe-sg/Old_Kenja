...
class knjz030bForm2
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjz030bindex.php", "", "edit");
        ...
        if (!isset($model->warning) && ($model->cmd == "edit2") && $model->year && $model->applicantdiv && $model->testdiv && $model->desirediv){
            $query = knjz030bQuery::getRow($model->year, $model->applicantdiv, $model->testdiv, $model->desirediv);
        ...
        $query = knjz030bQuery::getNameMst($model->year, "L003");
        makeCombo($objForm, $arg, $db, $query, $Row["APPLICANTDIV"], "APPLICANTDIV", $extra, 1, "blank");

        //入試区分コンボ
        $query = knjz030bQuery::getNameMst($model->year, "L004");
        ...
        //選択コース一覧
        $result = $db->query(knjz030bQuery::getListCourse($model, 'Llist', $Row["APPLICANTDIV"], $Row["TESTDIV"], $Row["DESIREDIV"]));
        $opt_left = array();
        ...
        //コース一覧
        $result = $db->query(knjz030bQuery::getListCourse($model, 'Rlist', $Row["APPLICANTDIV"], $Row["TESTDIV"], $Row["DESIREDIV"]));
        $opt_right = array();
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "window.open('knjz030bindex.php?cmd=list','left_frame');";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjz030bForm2.html", $arg);
    }
}
