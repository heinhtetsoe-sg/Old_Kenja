...
class knjz021yForm2 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjz021yindex.php", "", "edit");
        ...
        if (!isset($model->warning) && ($model->cmd == "edit2" || $model->cmd == "reset") && $model->year && $model->applicantdiv && $model->testdiv && $model->totalcd && $model->testsubclasscd) {
            $query = knjz021yQuery::getRow($model->year, $model->applicantdiv, $model->testdiv, $model->totalcd, $model->testsubclasscd);
        ...
        /**************/
        //試験科目コンボ
        $query = knjz021yQuery::getTestSubclass($model);
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "parent.left_frame.location.href='knjz021yindex.php?cmd=list2"
                            . "&year=".$model->year."&applicantdiv=".$model->applicantdiv."&testdiv=".$model->testdiv."';";

        }
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjz021yForm2.html", $arg);
    }
}
