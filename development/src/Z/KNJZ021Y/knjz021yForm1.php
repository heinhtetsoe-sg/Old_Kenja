...
class knjz021yForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjz021yindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //年度設定
        $result = $db->query(knjz021yQuery::selectYearQuery());
        $opt    = array();
        //レコードが存在しなければ処理年度を登録
        if ($result->numRows() == 0) { 
        ...
        if(!$flg) {
            if (!isset($model->year)) {
                ...
                $model->year = $db->getOne(knjz021yQuery::DeleteAtExist($model));
            }
            if ($model->cmd == 'list') {
                $arg["reload"][] = "parent.right_frame.location.href='knjz021yindex.php?cmd=edit"
        ...
        //入試制度コンボ
        $extra = "onchange=\"return btn_submit('list');\"";
        $query = knjz021yQuery::getNameMst($model, "L003");
        ...
        $query = knjz021yQuery::getNameMst($model, "L004");
        makeCombo($objForm, $arg, $db, $query, "TESTDIV", $model->testdiv, $extra, 1);

        //テーブルの中身の作成
        $query  = knjz021yQuery::selectQuery($model);
        $result = $db->query($query);
            ...
            $row["TESTSUBCLASSCD"] = View::alink("knjz021yindex.php", $row["TESTSUBCLASSCD"].":".$row["TESTSUBCLASSNAME"], "target=\"right_frame\"", $hash);
            $arg["data"][] = $row;
        }
        $result->free();
        ...
        View::toHTML($model, "knjz021yForm1.html", $arg);
    }
}
