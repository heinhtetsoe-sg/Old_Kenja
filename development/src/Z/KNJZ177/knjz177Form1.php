...
function makeMonthSemeCmb(&$objForm, &$arg, $db, &$model) {
    ...
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        for ($i = $row["S_MONTH"]; $i <= $row["E_MONTH"]; $i++) {
            ...
            $query = knjz177Query::getAppointedDay($model, $row['YEAR'], $model->school_kind, $month, $row['SEMESTER']);
...
function makeList(&$objForm, &$arg, $db, $model) {
    ...
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        for ($i = $row['SMONTH']; $i <= $row['EMONTH']; $i++) {
            ...
            $query = knjz177Query::getAppointedDay($model, $row['YEAR'], $model->school_kind, $target_month, $row['SEMESTER']);
            $appointedDay = $db->getOne($query);
...
function makeButton(&$objForm, &$arg, $model) {
    ...
    $link .= "&SEND_SCHOOLKIND={$model->school_kind}";
