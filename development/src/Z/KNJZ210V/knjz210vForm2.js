...
function doSubmit() {
    ...
    attribute3.value = "";
    sep = "";
    if (document.forms[0].left_chaircd.length==0 && document.forms[0].right_chaircd.length==0) {
        alert('{rval MSG916}');
        return false;
    }

    for (var i = 0; i < document.forms[0].left_chaircd.length; i++) {
        left_chaircd = document.forms[0].left_chaircd.options[i].value;
        attribute3.value = attribute3.value + sep + left_chaircd;
        sep = ",";
    }

    var keikoku_flg = false;
    for (var i = 0; i < document.forms[0].right_chaircd.length; i++) {
        right_chaircd = document.forms[0].right_chaircd.options[i].value.split("_");
        if (right_chaircd[2] == '1') {
            keikoku_flg = true;
        }
    }

    if (keikoku_flg) {
        if (!confirm('成績入力完了済みのデータが削除されます。更新してもよろしいですか？')){
            return false;
        }
    }
