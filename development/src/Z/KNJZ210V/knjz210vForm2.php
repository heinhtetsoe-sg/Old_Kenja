...
class knjz210vForm2
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjz210vindex.php", "", "edit");
        ...
        if (!isset($model->warning) && $model->cmd != "edit2" && $model->subclasscd){
            $query = knjz210vQuery::getChkChrList($model);
            $Row = $db->getRow($query, DB_FETCHMODE_ASSOC);
        } else {
            $Row =& $model->field;
        }
        //科目コンボ作成
        $query = knjz210vQuery::getSubclass($model);
...
//            $model->subclasscd = $Row["SUBCLASSCD"];
        ...
        //区分表示
        $query = knjz210vQuery::getSemesterMst($model->semester);
        $semRow = $db->getRow($query, DB_FETCHMODE_ASSOC);
        $query = knjz210vQuery::getTestcd($model, $model->semester, $model->testcd);
        $testRow = $db->getRow($query, DB_FETCHMODE_ASSOC);
        $arg["RECORD_DIV"] = $semRow["SEMESTERNAME"] . " - " . $testRow["TESTITEMNAME"];

        //対象講座一覧（左）
        $chaircd = $option = array();
        $result = $db->query(knjz210vQuery::getChkChrList($model));
        //教育課程対応
        ...
        //講座一覧（右）
        $option = array();
        $result = $db->query(knjz210vQuery::getChairList($model, $chaircd));
        //教育課程対応
        if ($model->Properties["useCurriculumcd"] == '1') {
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                $option[]  = array("label" => $row["CHAIRCD"]." ".$row["CHAIRNAME"],
        ...
        if (VARS::post("cmd") == "update") {
            $arg["jscript"] = "window.open('knjz210vindex.php?cmd=list&shori=update','left_frame');";
        }
        ...
        View::toHTML($model, "knjz210vForm2.html", $arg);
    }
}
