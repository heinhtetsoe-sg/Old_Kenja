...
class knjz020uForm2 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjz020uindex.php", "", "edit");
        ...
        if (!isset($model->warning) && ($model->cmd == "edit2" || $model->cmd == "reset") && $model->year && $model->applicantdiv && $model->testdiv && $model->totalcd && $model->testsubclasscd) {
            $query = knjz020uQuery::getRow($model->year, $model->applicantdiv, $model->testdiv, $model->totalcd, $model->testsubclasscd);
        ...
        /**************/
        //試験コースコンボ
        $query = knjz020uQuery::getEntExamCourse($model);
        makeCombo($objForm, $arg, $db, $query, $Row["TOTALCD"], "TOTALCD", "", 1, "blank");

        //試験科目コンボ
        $query = knjz020uQuery::getTestSubclass($model);
        ...
        if (VARS::get("cmd") != "edit") {
            $arg["reload"]  = "parent.left_frame.location.href='knjz020uindex.php?cmd=list2"
                            . "&year=".$model->year."&applicantdiv=".$model->applicantdiv."&testdiv=".$model->testdiv."';";

        }
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjz020uForm2.html", $arg);
    }
}
//コンボ作成
