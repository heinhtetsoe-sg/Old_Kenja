...
class knjz020uForm1 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjz020uindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //年度設定
        $result = $db->query(knjz020uQuery::selectYearQuery());
        $opt    = array();
        //レコードが存在しなければ処理年度を登録
        if ($result->numRows() == 0) { 
        ...
        if (!$flg) {
            if (!isset($model->year)) {
                ...
                $model->year = $db->getOne(knjz020uQuery::DeleteAtExist($model));
            }
            if ($model->cmd == 'list') {
                $arg["reload"][] = "parent.right_frame.location.href='knjz020uindex.php?cmd=edit"
        ...
        //入試制度コンボ
        $extra = "onchange=\"return btn_submit('list');\"";
        $query = knjz020uQuery::getNameMst($model, "L003");
        ...
        while ( $row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            $row["TESTSUBCLASSCD"] = View::alink("knjz020uindex.php", $row["TESTSUBCLASSCD"].":".$row["TESTSUBCLASSNAME"], "target=\"right_frame\"", $hash);
            ...
            if ($bifKey !== $row["TOTALCD"]) {
                $cnt = $db->getOne(knjz020uQuery::getEntexamCourseCnt($model, $row["TOTALCD"]));
        ...
        View::toHTML($model, "knjz020uForm1.html", $arg);
    }
}
