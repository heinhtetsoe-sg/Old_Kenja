...
class knjz093kForm1 {
    function main(&$model) {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjz093kindex.php", "", "edit");
        ...
        //リスト表示
        $result = $db->query(knjz093kQuery::selectQuery($model));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
            //レコードを連想配列のまま配列$arg[data]に追加していく。
            array_walk($row, "htmlspecialchars_array");

            $row["REDUCTION_ADD_MONEY"] = (strlen($row["REDUCTION_ADD_MONEY"])) ? number_format($row["REDUCTION_ADD_MONEY"]): "";
            $row["REDUCTION_ADD_MONEY"] = View::alink("knjz093kindex.php", $row["REDUCTION_ADD_MONEY"], "target=\"right_frame\"",
                                                array("cmd"                 => "edit",
                                                      "GRADE"               => $row["GRADE"],
        ...
        if(!isset($model->warning) && VARS::post("cmd") == "copy"){
            $arg["reload"] = "parent.right_frame.location.href='knjz093kindex.php?cmd=edit"
                           . "&year=".$model->year."';";
        }
        $arg["finish"]  = $objForm->get_finish();
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjz093kForm1.html", $arg);
    }
}
?>
