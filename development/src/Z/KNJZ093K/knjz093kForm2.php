...
class knjz093kForm2 {
    function main(&$model) {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjz093kindex.php", "", "edit");

        //警告メッセージを表示しない場合
        if(!isset($model->warning)) {
            $Row = knjz093kQuery::getRow($model, 1);
        } else {
            $Row =& $model->field;
        }
        ...
        //学年コンボ
        $result    = $db->query(knjz093kQuery::getGrade($model->year));
        $opt       = array();
        ...
        if (VARS::get("cmd") != "edit") {
            $arg["reload"]  = "window.open('knjz093kindex.php?cmd=list','left_frame');";
        }
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjz093kForm2.html", $arg);
    }
}
?>
