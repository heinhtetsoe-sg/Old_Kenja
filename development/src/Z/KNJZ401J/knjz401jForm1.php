...
class knjz401jForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("sel", "POST", "knjz401jindex.php", "", "sel");
        ...
        $opt = array();
        $value_flg = false;
        $result = $db->query(knjz401jQuery::getYearList($model));
        ...
        $result = $db->query(knjz401jQuery::selectQuery($model));
        //教育課程対応
        ...
        //観点一覧取得
        $opt_right = array();
        $result = $db->query(knjz401jQuery::selectViewNameQuery($opt_left_id, $model));
        //教育課程対応
        if ($model->Properties["useCurriculumcd"] == '1') {
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        //観点マスタボタン
        $link = REQUESTROOT."/Z/KNJZ401J_2/knjz401j_2index.php?SEND_selectSchoolKind={$model->selectSchoolKind}&year_code=".$model->year;
