...
class knjz216Form2
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjz216index.php", "", "edit");
        ...
        if (!isset($model->warning) && $model->cmd != "changeCmb") {
            ...
            $query = knjz216Query::getSelectData($paraField);
        ...
        //模試
        $query = knjz216Query::getMockname1();
        $extra = "";
        makeCmb($objForm, $arg, $db, $query, $Row["MOCKCD"], "MOCKCD", $extra, "BLANK");

        //学年
        $query = knjz216Query::getGrade("SCHREG_REGD_HDAT");
        ...
        makeListToList($objForm, $arg, $db, $Row, $selectData);
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "parent.left_frame.location.href='knjz216index.php?cmd=list';";
        }
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjz216Form2.html", $arg); 
    }
}

//コース一覧リストToリスト作成
function makeListToList(&$objForm, &$arg, $db, $Row, $selectData)
{
    //コース一覧
    $optData = array();
    $optSelect = array();
    $result = $db->query(knjz216Query::getCourse($Row["GRADE"], ""));
    ...
    foreach ($selectData as $key => $val) {
        if ($key) {
            $query = knjz216Query::getCourse($Row["GRADE"], $key);
