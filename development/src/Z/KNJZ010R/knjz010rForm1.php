...
class knjz010rForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjz010rindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //年度設定
        $result    = $db->query(knjz010rQuery::selectYearQuery($model));
        $opt       = array();
        //レコードが存在しなければ処理年度を登録
        if ($result->numRows() == 0) { 
        ...
        if(!$flg) {
            if (!isset($model->year)) {
                ...
                $model->year = $db->getOne(knjz010rQuery::DeleteAtExist($model));
            }
            $arg["reload"][] = "parent.right_frame.location.href='knjz010rindex.php?cmd=edit"
                             . "&year=" .$model->year."';";
        }
        ...
        //テーブルの中身の作成
        $query  = knjz010rQuery::selectQuery($model, $model->year);
        ...
        $result = $db->query($query);
            ...
            $row["TESTNAME"]       = $row["TESTDIV"] .":". $row["TEST_NAME"] ;
            $row["APPLICANTNAME"]  = $row["APPLICANTDIV"] .":". $row["APPLI_NAME"] ;
            $row["TOTALCDNAME"]    = $row["COURSECD"] . $row["MAJORCD"] .":". $row["COURSENAME"] . $row["MAJORNAME"];
            $row["EXAMCOURSECD"]   = View::alink("knjz010rindex.php", $row["EXAMCOURSECD"], "target=\"right_frame\"", $hash);
        ...
        View::toHTML($model, "knjz010rForm1.html", $arg);
    }
}
?>
