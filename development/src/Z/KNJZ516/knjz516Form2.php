...
class knjz516Form2 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjz516index.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        if (!isset($model->warning) && $model->centercd) {
            $query = knjz516Query::getWelfareAdviceCenterMst($model->centercd);
        ...
        //圏域
        $kyoto = $db->getOne(knjz516Query::getZ010());
        $arg["data"]["AREA_NAME"] = ($kyoto == 'kyoto') ? "圏域" : "地区";
        $opt = array();
        $value_flg = false;
        $query = knjz516Query::getNameMst();
        ...
        if (VARS::get("cmd") != "edit") {
            $arg["reload"]  = "window.open('knjz516index.php?cmd=list','left_frame');";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。 
        View::toHTML($model, "knjz516Form2.html", $arg);
    }
}
?>
