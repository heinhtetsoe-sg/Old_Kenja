...
class knjz351qForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("sel", "POST", "knjz351qindex.php", "", "sel");
        $db             = Query::dbCheckOut();
        ...
        $query = knjz351qQuery::getCourseMajor($model);
        makeCmb($objForm, $arg, $db, $query, "COURSE_MAJOR", $model->field["COURSE_MAJOR"], $extra, 1, "ALL", $model);

        //科目コンボ
        $extra = "onChange=\"btn_submit('subclasscd')\";";
        $query = knjz351qQuery::getSubclassMst($model);
        makeCmb($objForm, $arg, $db, $query, "SUBCLASSCD", $model->field["SUBCLASSCD"], $extra, 1, "", $model);

        //算出先コンボ
        $extra = "onChange=\"btn_submit('edit')\";";
        $query = knjz351qQuery::selectListQuery($db, $model);
        makeCmb($objForm, $arg, $db, $query, "SAKI_TESTCD", $model->field["SAKI_TESTCD"], $extra, 1, "", $model);

        //テキストボックス表示データ取得(算出元)
        $opt_left1 = $opt_right1 = array();
        $result    = $db->query(knjz351qQuery::selectListQuery($db, $model));
        ...
        if (VARS::get("cmd") != "edit" && $model->cmd != 'chenge'){
            $arg["reload"]  = "window.open('knjz351qindex.php?cmd=list','left_frame');";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjz351qForm1.html", $arg);
    }
}
...
function makeCmb(&$objForm, &$arg, $db, $query, $name, &$value, $extra, $size, $blank = "", &$model) {
    ...
    if ($blank == "ALL") $opt[] = array('label' => "--基本設定--", 'value' => "0-000");
    $value_flg = false;
