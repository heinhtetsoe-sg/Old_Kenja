...
class knjz010bForm2
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjz010bindex.php", "", "edit");
        ...
        if (!isset($model->warning) && ($model->cmd == "edit2") && $model->year && $model->examcoursecd && $model->applicantdiv && $model->testdiv && $model->coursecd && $model->majorcd){
            $query = knjz010bQuery::getRow($model->year, $model->examcoursecd, $model->applicantdiv, $model->testdiv, $model->coursecd, $model->majorcd);
        ...
        $query = knjz010bQuery::getNameMst($model->year, "L003");
        makeCombo($objForm, $arg, $db, $query, $Row["APPLICANTDIV"], "APPLICANTDIV", $extra, 1, "blank");

        //入試区分コンボ
        $query = knjz010bQuery::getNameMst($model->year, "L004");
        makeCombo($objForm, $arg, $db, $query, $Row["TESTDIV"], "TESTDIV", "", 1, "blank");

        //課程学科コンボ
        $query = knjz010bQuery::selectTotalcd($model->year);
        ...
        //入学コースコンボ
        $query = knjz010bQuery::selectCourceCode($model->year);
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "parent.left_frame.location.href='knjz010bindex.php?cmd=list"
                            . "&year=" .$model->year."';";
        }
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjz010bForm2.html", $arg);
    }
}
