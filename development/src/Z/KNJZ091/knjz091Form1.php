...
class knjz091Form1
{
    ...
    //フォーム作成
    $arg["start"]   = $objForm->get_start("sel", "POST", "knjz091index.php", "", "sel");
    $db             = Query::dbCheckOut();
    $no_year        = 0;

    //年度コンボ設定
    $result    = $db->query(knjz091Query::selectYearQuery());
    $opt       = array();
    ...
    //出身塾年度一覧取得
    $result      = $db->query(knjz091Query::selectQuery($model));   
    ...
    //出身塾一覧取得
    $result = $db->query(knjz091Query::selectJuniorQuery($opt_left_id,$model));
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
    {
    ...
    //出身塾マスタボタンを作成する
    $link = REQUESTROOT."/Z/KNJZ091_2/knjz091_2index.php?mode=1";
