...
class knjz210fForm1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjz210findex.php", "", "edit");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        $result = $db->query(knjz210fQuery::getNameMst($setNameCd));
        ...
        //学期数の取得
        $semestercount = $db->getOne(knjz210fQuery::getSemestercount($setNameCd));
        ...
        //学年コンボ
        $query = knjz210fQuery::getGrade($model);
        ...
        $key = 0;
        $bifKey = "";
        $result = $db->query(knjz210fQuery::selectQuery($model, $semestercount, $assesslevel));
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
            ...
            //評定設定チェック
            $hyouteicnt = $db->getOne(knjz210fQuery::getjviewStatLevelMst($model, $row));
            if ($hyouteicnt > 0) {
                $row["HYOUTEICHECK"] = 'レ';
            }
            
            if ($bifKey !== $row["KEY"]) {
                $cnt = $db->getOne(knjz210fQuery::getSubclassCnt($model, $row));
        ...
        View::toHTML($model, "knjz210fForm1.html", $arg);
    }
}
