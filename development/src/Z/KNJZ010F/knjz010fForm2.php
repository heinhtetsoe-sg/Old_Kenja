...
class knjz010fForm2 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjz010findex.php", "", "edit");
        ...
        if (!isset($model->warning) && ($model->cmd == "edit2") && $model->year && $model->examcoursecd && $model->applicantdiv && $model->coursecd && $model->majorcd) {
            $query = knjz010fQuery::getRow($model->year, $model->examcoursecd, $model->applicantdiv, $model->coursecd, $model->majorcd);
        ...
        if (!isset($model->warning) && $model->cmd != "edit"){
            $arg["reload"]  = "parent.left_frame.location.href='knjz010findex.php?cmd=list"
                            . "&year=" .$model->year."';";
        }
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjz010fForm2.html", $arg);
    }
}
