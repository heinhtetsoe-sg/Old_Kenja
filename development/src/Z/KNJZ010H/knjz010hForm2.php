...
class knjz010hForm2
{
    function main(&$model)
    {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjz010hindex.php", "", "edit");

        //警告メッセージを表示しない場合
        if (!isset($model->warning)){
            $Row = knjz010hQuery::getRow($model->year,$model->examcoursecd,$model->applicantdiv,$model->coursecd,$model->majorcd);
        }else{
            $Row =& $model->field;
        }
        ...
        //入試制度コンボ
        $db = Query::dbCheckOut();
        $result    = $db->query(knjz010hQuery::selectApplicantdiv($model->year));
        ...
        //課程学科コンボ
        $db = Query::dbCheckOut();
        $result    = $db->query(knjz010hQuery::selectTotalcd($model->year));
        $opt       = array();
        $opt[] = array("label" => "",
                       "value" => "");
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "parent.left_frame.location.href='knjz010hindex.php?cmd=list"
                            . "&year=" .$model->year."';";
        }
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。 
        View::toHTML($model, "knjz010hForm2.html", $arg);
    }
}
?>
