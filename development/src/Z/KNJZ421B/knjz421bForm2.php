...
class knjz421bForm2
{
    public function main(&$model)
    {
        ...
        $objForm = new form();
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjz421bindex.php", "", "edit");

        //警告メッセージを表示しない場合
        if (($model->senkou_no != "") && !isset($model->warning) && $model->cmd != 'chenge_cd' && $model->cmd != 'apply_jobtype' && $model->cmd != 'search') {
            $Row = knjz421bQuery::getRow($model->getyear, $model->senkou_no);
        } else {
            $Row =& $model->field;
        }
        $db = Query::dbCheckOut();
        ...
        // 会社情報
        if ($model->cmd == 'search') {
            $company = $db->getRow(knjz421bQuery::getCollegeOrCompanyMst(trim($Row["COMPANY_CD"])), DB_FETCHMODE_ASSOC);
        ...
        //管轄コンボボックス
        $query = knjz421bQuery::getKankatsu();
        $extra = "";
        ...
        //会社検索ボタン
        $extra = "onclick=\"wopen('" .REQUESTROOT."/X/KNJXSEARCH9_JOB_SS/index.php?&PROGRAMID=KNJZ421B&PATH=/Z/KNJZ421B/knjz421bindex.php&cmd=&target=KNJZ421B','search',0,0,790,470);\"";
        ...
        $industry_sname = $db->getOne(knjz421bQuery::getIndustrySName($Row["INDUSTRY_SCD"]));
        $arg["data"]["INDUSTRY_SNAME"] = $Row["INDUSTRY_SNAME"] ? $Row["INDUSTRY_SNAME"] : $industry_sname;

        //職業小分類(左)
        $extra = " style=\"text-align:right\" onblur=\"this.value=toInteger_syokugyobunrui(this)\"";
        $arg["data"]["JOBTYPE_SCD"] = knjCreateTextBox($objForm, $Row["JOBTYPE_SCD"], "JOBTYPE_SCD", 3, 3, $extra);

        //職業類分細(右)
        $extra = " style=\"text-align:right\" onblur=\"this.value=toInteger_syokugyobunrui(this)\"";
        $arg["data"]["JOBTYPE_SSCD"] = knjCreateTextBox($objForm, $Row["JOBTYPE_SSCD"], "JOBTYPE_SSCD", 3, 2, $extra);
        //hidden
        if ($model->cmd != 'apply_jobtype') {
            ...
            knjCreateHidden($objForm, "JOBTYPE_LCD", $model->field["JOBTYPE_LCD"]);
            knjCreateHidden($objForm, "JOBTYPE_MCD", $model->field["JOBTYPE_MCD"]);
        }

        //職業分類検索ボタン
        $extra = " onclick=\"loadwindow('" .REQUESTROOT ."/X/KNJXSEARCH_JOBTYPE_SS/knjxjobtypess_searchindex.php?cmd=', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 500, 320)\"";
        $arg["button"]["btn_jobType"] = knjCreateBtn($objForm, "btn_jobType", "職業分類検索", $extra);
        $jobType_sName = $db->getOne(knjz421bQuery::getjobTypeSName($model, $Row["JOBTYPE_LCD"], $Row["JOBTYPE_MCD"], $Row["JOBTYPE_SCD"], $Row["JOBTYPE_SSCD"]));
        ...
        //checkbox(会社所在地と同じ)
        if ($Row["COMPANY_ADDR1"] == $Row["SHUSHOKU_ADDR1"] && $Row["COMPANY_ADDR2"]  == $Row["SHUSHOKU_ADDR2"] &&
        ...
        //雇用形態コンボボックス
        $query = knjz421bQuery::getEmploymentStatus();
        $extra = "";
        ...
        if ($model->Properties["useCompany_Sort"] == "1") {
            ...
            //会社職種コンボボックス
            $query = knjz421bQuery::getCompanycd();
            $extra = "";
        ...
        //選考方法コンボボックス1
        $query = knjz421bQuery::getJudgeing();
        $extra = "";
        ...
        if (VARS::get("cmd") != "edit" && $model->cmd != 'chenge_cd' &&  $model->cmd != 'apply_jobtype' && !$model->warning) {
            ...
            $arg["reload"]  = "parent.left_frame.location.href='knjz421bindex.php?cmd=list';";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjz421bForm2.html", $arg);
    }
}
//makeCmb
