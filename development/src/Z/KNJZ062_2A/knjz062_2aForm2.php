...
class knjz062_2aForm2
{
    function main(&$model)
    {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjz062_2aindex.php", "", "edit");
        //教育課程対応
        ...
        if (!isset($model->warning)) {
            $Row = knjz062_2aQuery::getRow($model->classcd, $model, $model->school_kind, "");
        } else {
            $Row =& $model->field;
        }
        $db = Query::dbCheckOut();
        ...
        //教育委員会用の学校コード取得
        $model->schoolcd = "";
        $model->schoolcd = $db->getOne(knjz062_2aQuery::getSchoolCd());
        ...
        if ($model->Properties["useCurriculumcd"] == '1') {
            ...
            //学校校種
            $opt_school = array();
            $query = knjz062_2aQuery::getNamecd('A023');
            $value_flg = false;
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "parent.left_frame.location.href='knjz062_2aindex.php?cmd=list';";
        }
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjz062_2aForm2.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg)
{
    ...
    //終了ボタンを作成する
    $link = REQUESTROOT."/Z/KNJZ062A/knjz062aindex.php";
