...
class knjz090Form1
{
    ...
    //フォーム作成
    $arg["start"]   = $objForm->get_start("sel", "POST", "knjz090index.php", "", "sel");
    $db             = Query::dbCheckOut();
    $no_year        = 0;
    //年度設定
    $result    = $db->query(knjz090Query::selectYearQuery());   
    $opt       = array();
    ...
    //年度科目一覧取得
    $result      = $db->query(knjz090Query::selectQuery($model));   
    $opt_left_id = $opt_left = array();
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
    {
        $opt_left[]    = array("label" => $row["FINSCHOOLCD"]."  ".$row["FINSCHOOL_NAME"], 
    ...
    //科目一覧取得
    $result = $db->query(knjz090Query::selectJuniorQuery($opt_left_id,$model));
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
    {
        $opt_right[] = array("label" => $row["FINSCHOOLCD"]."  ".$row["FINSCHOOL_NAME"], 
                             "value" => $row["FINSCHOOLCD"]);
    }
    ...
    //出身学校マスタボタンを作成する
    $link = REQUESTROOT."/Z/KNJZ090_2/knjz090_2index.php?mode=1";
