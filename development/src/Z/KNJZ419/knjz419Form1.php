...
class knjz419Form1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"] = $objForm->get_start("sel", "POST", "knjz419index.php", "", "sel");
        $db = Query::dbCheckOut();

        //年度設定
        $result    = $db->query(knjz419Query::selectYearQuery());
        $opt       = array();
        ...
        //年度進路アンケート一覧取得
        $result = $db->query(knjz419Query::selectQuery($model));
        $opt_left = array();
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //進路アンケート一覧取得
        $result = $db->query(knjz419Query::selectCourseQuery($model));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //進路アンケートマスタボタンを作成する
        $link = REQUESTROOT."/Z/KNJZ419_2/knjz419_2index.php?mode=1";
        ...
        $arg["info"]    = array("TOP"        => "対象年度",
                                "LEFT_LIST"  => "進路アンケート年度一覧",
                                "RIGHT_LIST" => "進路アンケート一覧");

        $arg["TITLE"]   = "マスタメンテナンス - 進路アンケートマスタ";
