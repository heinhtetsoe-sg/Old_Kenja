...
class knjz020rForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjz020rindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        $model->applicantdiv = '1';
        if (SCHOOLKIND == 'J') {
            $model->applicantdiv = '2';
        }

        //年度設定
        $result = $db->query(knjz020rQuery::selectYearQuery($model));
        $opt    = array();
        //レコードが存在しなければ処理年度を登録
        if ($result->numRows() == 0) { 
        ...
        if(!$flg) {
            if (!isset($model->year)) {
                ...
                $model->year = $db->getOne(knjz020rQuery::DeleteAtExist($model));
            }
            if ($model->cmd == 'list' || $model->cmd == 'chgAppDiv') {
                $arg["reload"][] = "parent.right_frame.location.href='knjz020rindex.php?cmd=edit"
        ...
        $result = $db->query($query);
            ...
            $row["TESTSUBCLASSCD"] = View::alink("knjz020rindex.php", $row["TESTSUBCLASSCD"].":".$row["TESTSUBCLASSNAME"], "target=\"right_frame\"", $hash);
            $arg["data"][] = $row;
        }
        $result->free();
        ...
        View::toHTML($model, "knjz020rForm1.html", $arg);
    }
}
