////左の職員年度リストで選ばれたデータの情報を右の編集用の表へ表示//////////////////////////////////////////////////
function ToEdit()
{
////	document.forms[0].staffmaster.options[i].selected
////	migi = window.parent.right_frame.document.forms[0];
	var w = document.forms[0].staffyear.selectedIndex;

	var selectstaff = document.forms[0].staffyear.options[w].value.split(',');
	job = selectstaff[2];
	sec = selectstaff[3];
	sha = selectstaff[4];
	cla = selectstaff[5];
	...
	var selectstafftext = document.forms[0].staffyear.options[w].text.split('|');
	stacd = selectstaff[0];
	staname = selectstafftext[2];

	parent.right_frame.FromList(job,sec,sha,cla,hclb,sclb,stacd,staname);
