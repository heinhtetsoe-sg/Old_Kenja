...
class knjz514Form2 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjz514index.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        if (!isset($model->warning) && $model->namecd) {
            $query = knjz514Query::getMedicalProstheticsNameMst($model->namecd);
        ...
        if (VARS::get("cmd") != "edit") {
            $arg["reload"] = "window.open('knjz514index.php?cmd=list','left_frame');";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。 
        View::toHTML($model, "knjz514Form2.html", $arg);
    }
}
?>
