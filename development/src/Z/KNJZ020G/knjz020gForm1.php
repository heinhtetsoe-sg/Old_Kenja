...
class knjz020gForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjz020gindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //年度設定
        $query = knjz020gQuery::selectYearQuery();
        ...
        if(!$flg) {
            if (!isset($model->year)) {
                ...
                $model->year = $db->getOne(knjz020gQuery::DeleteAtExist($model));
            }
            if ($model->cmd == 'list') {
                $arg["reload"][] = "parent.right_frame.location.href='knjz020gindex.php?cmd=edit"
        ...
        //入試制度コンボ
        $query = knjz020gQuery::getNameMst($model, "L003");
        $extra = "onchange=\"return btn_submit('list');\"";
        ...
        $query = knjz020gQuery::getNameMst($model, "L004");
        $extra = "onchange=\"return btn_submit('list');\"";
        makeCombo($objForm, $arg, $db, $query, "TESTDIV", $model->testdiv, $extra, 1);

        //課程学科件数取得
        $t_cnt = array();
        $result = $db->query(knjz020gQuery::getDataCnt($model, "totalcd"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //受験コース件数取得
        $e_cnt = array();
        $result = $db->query(knjz020gQuery::getDataCnt($model, "examcoursecd"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //テーブルの中身の作成
        $t_bifKey = $e_bifKey = "";
        $query  = knjz020gQuery::selectQuery($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            $row["TESTSUBCLASSCD"] = View::alink("knjz020gindex.php", $row["TESTSUBCLASSCD"].":".$row["TESTSUBCLASSNAME"], "target=\"right_frame\"", $hash);
        ...
        View::toHTML($model, "knjz020gForm1.html", $arg);
    }
}
