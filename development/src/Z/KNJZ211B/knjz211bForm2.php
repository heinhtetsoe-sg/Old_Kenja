...
class knjz211bForm2 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjz211bindex.php", "", "edit");
        ...
        //学期数の取得
        $semester_count = $db->getOne(knjz211bQuery::getSemestercount("Z009"));

        //観点の満点登録チェック
        $model->disabled = "";
        if ($model->subclasscd != "" && $model->grade != "") {
            $result = $db->query(knjz211bQuery::getViewlist($model));
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                //JVIEWSTAT_LEVEL_MST登録データおよび段階値のMAXの上限値チェック(観点および上限値)
                $getCount = $db->getOne(knjz211bQuery::getCheckViewcd($model, $row["VIEWCD"]));
                if ($getCount == 0) {
        ...
        //段階値数チェック
        $countAssess = $db->getOne(knjz211bQuery::selectCountQuery($model));

        //段階値の上限値の最大値取得
        $model->max_assesshight = $db->getOne(knjz211bQuery::getMaxAssessHight($model));
        ...
        $counter = 0;
        //一覧表示
        $result = $db->query(knjz211bQuery::getRow($model));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        knjCreateHidden($objForm, "PRGID", "KNJZ211B");
        ...
        if (VARS::get("cmd") != "edit" && $model->cmd != 'chenge_cd' && $model->cmd != 'reset' && $model->cmd != 'kakutei'){
            $arg["reload"]  = "parent.left_frame.location.href='knjz211bindex.php?cmd=list';";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjz211bForm2.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg, &$model) {
    //学年教科評定設定ボタンを作成する
    /*$extra = "onclick=\" wopen('".REQUESTROOT."/Z/KNJZ211B/knjz211bindex.php?&SEND_PRGRID=KNJZ211B&SEND_AUTH={$model->auth}&SEND_CLASSCD={$model->field["CLASSCD"]}&SEND_SUBCLASSCD={$model->field["SUBCLASSCD"]}&SEND_GRADE={$model->field["GRADE"]}','SUBWIN2',0,0,screen.availWidth,screen.availHeight);\"closeWin();\"";
