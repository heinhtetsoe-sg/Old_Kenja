...
class knjx_hreportremark_datForm1
{
    function main(&$model)
    {
        //オブジェクト作成
        ...
        //年度一覧コンボボックス
        $query = knjx_hreportremark_datquery::getSelectFieldSQL();
        $extra = "onchange=\"return btn_submit('');\"";
        makeCmb($objForm, $arg, $db, $query, "YEAR", $model->field["YEAR"], $extra, 1);

        if ($model->getPrgId == "KNJD138P") {
            //学年一覧コンボボックス
            $query = knjx_hreportremark_datquery::getGrade($model);
            $extra = "onchange=\"return btn_submit('');\"";
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
        }

        //年組一覧コンボボックス
        $query = knjx_hreportremark_datquery::getSelectFieldSQL2($model);
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx_hreportremark_datindex.php", "", "main");
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjx_hreportremark_datForm1.html", $arg);
    }
}
...
function makeCmb(&$objForm, &$arg, $db, $query, $name, &$value, $extra, $size)
{
    $opt = array();
    $value_flg = false;
    if ($name == "GRADE_HR_CLASS") $opt[] = array("label" => "(全て出力)", "value" => "");
