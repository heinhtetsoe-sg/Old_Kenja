...
class knjz010yForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjz010yindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //年度設定
        $result    = $db->query(knjz010yQuery::selectYearQuery());
        $opt       = array();
        //レコードが存在しなければ処理年度を登録
        if ($result->numRows() == 0) { 
        ...
        if(!$flg) {
            if (!isset($model->year)) {
                ...
                $model->year = $db->getOne(knjz010yQuery::DeleteAtExist($model));
            }
            $arg["reload"][] = "parent.right_frame.location.href='knjz010yindex.php?cmd=edit"
                             . "&year=" .$model->year."';";
        }
        ...
        //テーブルの中身の作成
        $query  = knjz010yQuery::selectQuery($model->year);
        ...
        $result = $db->query($query);
            ...
            $namecd = ($row["APPLICANTDIV"] == "1") ? 'L024' : 'L004';
            $row["ABBV1"] = $db->getOne(knjz010yQuery::getTestDivName($model->year, $namecd, $row["TESTDIV"]));

            $row["APPLICANTNAME"]  = $row["APPLICANTDIV"] .":". $row["NAME1"] ;
            $row["TOTALCDNAME"]    = $row["COURSECD"] . $row["MAJORCD"] .":". $row["COURSENAME"] . $row["MAJORNAME"];
            $row["EXAMCOURSECD"]   = View::alink("knjz010yindex.php", $row["EXAMCOURSECD"], "target=\"right_frame\"", $hash);
        ...
        View::toHTML($model, "knjz010yForm1.html", $arg);
    }
}
?>
