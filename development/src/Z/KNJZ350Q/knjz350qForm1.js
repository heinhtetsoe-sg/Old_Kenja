...
//更新（ＭＹＰ・ＤＰ成績入力）
function doSubmitMypDp() {
    var attribute = document.forms[0].selectdataMypDp;
    attribute.value = "";
    if (document.forms[0].myp_dp_input.length==0 && document.forms[0].myp_dp_delete.length==0) {
        alert('{rval MSG916}');
        return false;
    }
    sep = "";
    for (var i = 0; i < document.forms[0].myp_dp_input.length; i++) {
        attribute.value = attribute.value + sep + document.forms[0].myp_dp_input.options[i].value;
        sep = ",";
    }
    document.forms[0].cmd.value = 'updateMypDp';
    document.forms[0].submit();
    return false;
}
