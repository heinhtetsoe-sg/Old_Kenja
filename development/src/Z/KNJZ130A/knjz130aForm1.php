...
class knjz130aForm1
{
    function main(&$model)
    {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("sel", "POST", "knjz130aindex.php", "", "sel");
        $db = Query::dbCheckOut();
        //年度設定
        $result = $db->query(knjz130aQuery::selectYearQuery());   
        $opt = array();
        $j=0;
        ...
        //名称年度一覧取得
        $result = $db->query(knjz130aQuery::selectQuery($model));   
        ...
        //名称一覧取得
        $opt_right = array();
        $result = $db->query(knjz130aQuery::selectnameQuery($opt_left_id,$model));   
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        //名称マスタボタンを作成する
        $link = REQUESTROOT."/Z/KNJZ130A_2/knjz130a_2index.php";
        $objForm->ae( array("type"        => "button",
                            "name"        => "btn_master",
                            "value"       => " 名称マスタ ",
