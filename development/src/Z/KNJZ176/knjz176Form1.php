...
class knjz176Form1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjz176Form1", "POST", "knjz176index.php", "", "knjz176Form1");
        ...
        //学期コンボ
        $query = knjz176Query::getSemester();
        ...
        View::toHTML($model, "knjz176Form1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    ...
    $result = $db->query(knjz176Query::getStudent($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        if ($row["COUNTFLG"] == "1") {
...
{
    //コピーボタン
    $disabled = ($model->semester == "1") ? "disabled" : "";
    $extra = $disabled ." onclick=\"return btn_submit('copy');\"";
    $arg["button"]["btn_copy"] = knjCreateBtn($objForm, "btn_copy", "前学期からコピー", $extra);
