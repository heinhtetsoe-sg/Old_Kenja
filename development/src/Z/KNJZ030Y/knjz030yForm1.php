...
class knjz030yForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjz030yindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //年度設定
        $result = $db->query(knjz030yQuery::selectYearQuery());
        $opt    = array();
        //レコードが存在しなければ処理年度を登録
        if ($result->numRows() == 0) { 
        ...
        if(!$flg) {
            if (!isset($model->year)) {
                ...
                $model->year = $db->getOne(knjz030yQuery::DeleteAtExist($model));
            }
            $arg["reload"][] = "parent.right_frame.location.href='knjz030yindex.php?cmd=edit"
                             . "&year=" .$model->year."';";
        }
        ...
        //リスト表示
        $tmp = array();
        $result = $db->query(knjz030yQuery::Listdata($model));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            ...
            $tmp[$row["APPLICANTDIV"].$row["TESTDIV"].$row["DESIREDIV"]]["DESIREDIV"]       = View::alink("knjz030yindex.php", $row["DESIREDIV"],"target=right_frame",
        ...
        View::toHTML($model, "knjz030yForm1.html", $arg);
        }
    }
?>
