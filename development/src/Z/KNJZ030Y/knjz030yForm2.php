...
class knjz030yForm2
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjz030yindex.php", "", "edit");
        ...
        if (!isset($model->warning) && ($model->cmd == "edit2") && $model->year && $model->applicantdiv && $model->testdiv && $model->desirediv){
            $query = knjz030yQuery::getRow($model->year, $model->applicantdiv, $model->testdiv, $model->desirediv);
        ...
        $query = knjz030yQuery::getNameMst($model->year, "L003");
        ...
        $query = knjz030yQuery::getNameMst($model->year, $namecd);
        ...
        //選択コース一覧
        $result = $db->query(knjz030yQuery::getListCourse($model, 'Llist', $Row["APPLICANTDIV"], $Row["TESTDIV"], $Row["DESIREDIV"]));
        $opt_left = array();
        ...
        //コース一覧
        $result = $db->query(knjz030yQuery::getListCourse($model, 'Rlist', $Row["APPLICANTDIV"], $Row["TESTDIV"], $Row["DESIREDIV"]));
        $opt_right = array();
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "window.open('knjz030yindex.php?cmd=list','left_frame');";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjz030yForm2.html", $arg);
    }
}
