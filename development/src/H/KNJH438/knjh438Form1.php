...
class knjh438Form1
{
    function main(&$model)
    {
        $objForm = new form();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjh438index.php", "", "edit");
        ...
        //参照年度コンボ
        $query = knjh438Query::getYear("COPYYEAR");
        $extra = "";
        ...
        //対象年度コンボ
        $query = knjh438Query::getYear("MOCKYEAR");
        ...
        if ($model->cmd == "changeMockyear") {
            $arg["reload"] = "window.open('knjh438index.php?cmd=edit', 'right_frame')";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjh438Form1.html", $arg);
    }
}
...
{
    $result = $db->query(knjh438Query::getList($model));
