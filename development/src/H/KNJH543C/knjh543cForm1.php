...
class knjh543cForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjh543cindex.php", "", "sel");
        ...
        //学期コンボボックス
        $query = knjh543cQuery::getSemester();
        $extra = "onchange=\"return btn_submit('list')\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //実力区分コンボボックス
        $query = knjh543cQuery::getProficiencyDiv($model->field["SEMESTER"]);
        $extra = "onchange=\"return btn_submit('list')\"";
        makeCmb($objForm, $arg, $db, $query, "PROFICIENCYDIV", $model->field["PROFICIENCYDIV"], $extra, 1);

        //実力コードコンボボックス
        $query = knjh543cQuery::getProficiencyCd($model->field["SEMESTER"], $model->field["PROFICIENCYDIV"]);
        $extra = "onchange=\"return btn_submit('list')\"";
        ...
        if (VARS::post("cmd") == "list"){
            $arg["reload"] = "window.open('knjh543cindex.php?cmd=sel&SEMESTER={$model->semester}&PROFICIENCYDIV={$model->proficiencydiv}&PROFICIENCYCD={$model->proficiencycd}&DIV={$model->div}','right_frame');";
        }
        ...
        View::toHTML($model, "knjh543cForm1.html", $arg); 
    }
}
...
function makeProficiencySubclassList(&$arg, $db, $model) {
    $p_cnt = $a_cnt = 1;
    $result = $db->query(knjh543cQuery::getProficiencySubclassRepCombDatList($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        //行数取得
        $p_subclass_cnt = $db->getOne(knjh543cQuery::getP_SubclassCnt($model, $row));
        $a_subclass_cnt = $db->getOne(knjh543cQuery::getA_SubclassCnt($model, $row));

        $row["PROFICIENCY_SUBCLASS_NAME"] = View::alink("knjh543cindex.php",
        ...
        $row["COURSE_NAME"] = View::alink("knjh543cindex.php",
                            $row["COURSE_NAME"],
                            "target=right_frame",
                            array("cmd"             => "sel",
                                  "SEND_FLG"        => "A_SUB",
