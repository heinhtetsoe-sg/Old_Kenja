...
class knjh543cForm2 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("sel", "POST", "knjh543cindex.php", "", "sel");
        ...
        if (!isset($model->warning) && $model->cmd != "change" && $model->send_flg == "A_SUB") {
            $query = knjh543cQuery::getProficiencySubclassRepCombDat($model);
        ...
        //合併先科目表示
        $subclassname = "";
        $subclassname = $db->getOne(knjh543cQuery::getProficiencySubclassName($model->proficiency_subclass_cd));
        ...
        //課程学科コース／コースグループコンボ
        if($model->div == "1") {
            $query = knjh543cQuery::getCourseMajor($model, $Row["GRADE"]);
            $arg["data"]["TITLE"] = '課程学科コース';
        } else {
            $query = knjh543cQuery::getCourseGroup($Row["GRADE"]);
        ...
        if (isset($model->message)) {   //更新できたら左のリストを再読込
            $arg["reload"] = "window.open('knjh543cindex.php?cmd=list_update', 'left_frame')";
        }

        View::toHTML($model, "knjh543cForm2.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, &$Row, $name, $target, $size, $model) {
    ...
    //extraセット
    $extraWeight = "style=\"text-align: right\" onblur=\"calc(this);\"";

    if ($Row["GRADE"] && $Row["COURSE"] && $model->proficiency_subclass_cd) {
        $query = knjh543cQuery::selectQuery($model, $Row["GRADE"], $Row["COURSE"], $model->proficiency_subclass_cd);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            if ($row["JOINCD"]) {
                ...
                $setData["WEIGHTING"] = knjCreateTextBox($objForm, $row["WEIGHTING"], "WEIGHTING".$row["VALUE"], 5, 5, $extraWeight);
