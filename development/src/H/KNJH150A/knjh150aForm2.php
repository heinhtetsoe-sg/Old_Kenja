...
class knjh150aForm2
{
    ...
    {
        $objForm        = new form;
        $arg["start"]    = $objForm->get_start("edit", "POST", "knjh150aindex.php", "", "edit");

        if (isset($model->schregno) && !isset($model->warning)) {
            $Row = knjh150aQuery::getStudent_data($model->schregno);
        } else {
            $Row =& $model->field;
        }
        ...
        $db            = Query::dbCheckOut();
        //事前処理チェック
        if (!knjh150aQuery::ChecktoStart($db)) {
        ...
        View::toHTML($model, "knjh150aForm2.html", $arg);
    }
}
?>
