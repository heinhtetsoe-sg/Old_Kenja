...
class knjh212SubForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform1", "POST", "knjh212index.php", "", "subform1");
        ...
        View::toHTML($model, "knjh212SubForm1.html", $arg);
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, &$model) {
    $row1 = $row2 = array();
    //生徒一覧リストを作成する
    $query = knjh212Query::getStudent($model, "1");
    ...
    //対象者一覧リストを作成する
    $query = knjh212Query::getStudent($model, "2");
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
