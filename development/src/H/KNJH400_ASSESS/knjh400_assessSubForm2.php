...
class knjh400_assessSubForm2 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform2", "POST", "knjh400_assessindex.php", "", "subform2");
        ...
        //前年
        $query = knjh400_assessQuery::getDefYearAnsData($model->schregno, $backYear, $semes[$backYear]);
        $cnt = $db->getOne($query);
        if($cnt > 0){
        ...
        //次年
        $query = knjh400_assessQuery::getDefYearAnsData($model->schregno, $nextYear, $semes[$nextYear]);
        $cnt = $db->getOne($query);
        if($cnt > 0){
        ...
        if (isset($model->schregno) && !isset($model->warning)){
            $Row = $db->getRow(knjh400_assessQuery::getSubQuery($model), DB_FETCHMODE_ASSOC);
        } else {
            $Row =& $model->field;
        }
        ...
        //障害名マスタ参照ボタン
        //$extra = "onclick=\"loadwindow('" .REQUESTROOT."/E/KNJh400_assess/knjh400_assessindex.php?cmd=reference1&SCHREGNO=".$model->schregno."', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 650, 500)\"";
        ...
        if($Row["INSTITUTES_CD"] != ""){
            $query = knjh400_assessQuery::getCenter("MEDICAL_CENTER_MST",$Row["INSTITUTES_CD"]);
        ...
        for ($cd=1 ; $cd <= 2; $cd++) {
            ...
            if($Row["EXAM_INST_CD_".$cd] != ""){
                $query = knjh400_assessQuery::getCenter("CHECK_CENTER_MST", $Row["EXAM_INST_CD_".$cd]);
        ...
        for ($cd=1 ; $cd <= 3; $cd++) {
            ...
            if($Row["CONS_INST_CD_".$cd] != ""){
                $query = knjh400_assessQuery::getCenter("WELFARE_ADVICE_CENTER_MST",$Row["CONS_INST_CD_".$cd]);
        ...
        View::toHTML($model, "knjh400_assessSubForm2.html", $arg);
    }
}
