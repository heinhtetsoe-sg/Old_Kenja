...
class knjh537Form2
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjh537index.php", "", "edit");
        ...
        if ($model->thisIsGet || $model->cmd == "reset") {
            $Row = knjh537Query::getRow($model->proficiency_subclass_cd, $db);
        ...
        $arg["data"]["PROFICIENCY_SUBCLASS_CD"] = createTextBox($objForm, $Row["PROFICIENCY_SUBCLASS_CD"], "PROFICIENCY_SUBCLASS_CD", 6, 6, $extra);
        ...
        //教科コード（コンボ）
        $query = knjh537Query::getClasscd($model);
        ...
        if ($model->Properties["useCurriculumcd"] == '1') {
            ...
            //教育課程コード（コンボ）
            $query = knjh537Query::getCurriculumCD($model);
            $extra = "onChange=\"btn_submit('edit')\";";
            makeCombo($objForm, $arg, $db, $query, $Row["CURRICULUM_CD"], 'data', 'CURRICULUM_CD', $extra, 1, $blank = "BLANK");
        }

        //県下統一実力科目コード（コンボ）
        $query = knjh537Query::getPrefSubclasscd();
        $extra = "onChange=\"btn_submit('edit')\";";
        ...
        $extra  = " onClick=\" wopen('".REQUESTROOT."/H/KNJH541/knjh541index.php?";
        $extra .= "cmd=&SEND_PRGID=KNJH537&SEND_AUTH=".AUTHORITY;
        $extra .= "','SUBWIN2',0,0,screen.availWidth,screen.availheight);\"";
        $arg["button"]["btn_prihist"] = createBtn($objForm, "btn_prihist", "県下実力科目登録", $extra);
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "parent.left_frame.location.href='knjh537index.php?cmd=list';";
        }
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjh537Form2.html", $arg); 
    }
}
