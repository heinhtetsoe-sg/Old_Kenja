...
function btn_submit(cmd) {
    ...
    if(cmd == 'hyouzi'){
        ...
        if((document.forms[0].F_KAISU.value == "" && document.forms[0].T_KAISU.value != "")
            || (parseInt(document.forms[0].F_KAISU.value) != "" && parseInt(document.forms[0].T_KAISU.value) != "" && parseInt(document.forms[0].F_KAISU.value) > parseInt(document.forms[0].T_KAISU.value))){
            alert('回数の指定が正しくありません。');
            document.forms[0].YEAR.value = document.forms[0].BEFORE_YEAR.value;     //表示してた年度に戻す
            return false;
        }
    }else if(cmd == 'graph'){
        window.open('knjh815index.php?cmd=graph','GRAPH','left=80,top=200,width=700,height=650');
        return false;
    }
