...
class knjh114cForm1 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjh114cForm1", "POST", "knjh114cindex.php", "", "knjh114cForm1");
        ...
        //学期
        $arg["SEMESTER"] = $db->getOne(knjh114cQuery::getSemesterName());
        ...
        $opt = array();
        $value_flg = false;
        $query = knjh114cQuery::getAuth($model, $model->field["SEMESTER"]);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJH114C");
        ...
        View::toHTML($model, "knjh114cForm1.html", $arg); 
    }
}
/*****************************************************************************************************************/
/***************************************** 以下関数 **************************************************************/
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["TAISYOU"] == "1") {
        ...
        $query = knjh114cQuery::getStudent($model, $model->field["SEMESTER"]);
    }
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
