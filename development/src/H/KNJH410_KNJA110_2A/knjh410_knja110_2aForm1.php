...
class knjh410_knja110_2aForm1 {
    function main(&$model) {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjh410_knja110_2aindex.php", "", "edit");
        $arg["close"] = "";
        $arg["data"] = array();
        ...
        //起動チェック
        if (!knjh410_knja110_2aQuery::ChecktoStart($db)) {
            $link = REQUESTROOT."/A/KNJA110A/knja110aindex.php?cmd=edit&schregno=".$model->schregno;
            $arg["close"] = "closing_window('$link');";
        }

        if ($model->cmd == "list2") {
            $link = REQUESTROOT."/U/KNJH410_KNJA110_2A/knjh410_knja110_2aindex.php?cmd=edit2&schregno=".$model->schregno."&INFO_DIV=".$model->infoDiv;
        ...
        if($model->schregno)
        {
            //地区コード
            $areacd_array = array();
            $result = $db->query(knjh410_knja110_2aQuery::List_AreaCd());
            ...
            $row2 = $db->getRow(knjh410_knja110_2aQuery::getEmergencyInfo($model->schregno),DB_FETCHMODE_ASSOC);
            $result = $db->query(knjh410_knja110_2aQuery::getAddress_all($model));
        ...
        View::toHTML($model, "knjh410_knja110_2aForm1.html", $arg);
    }
}
?>
