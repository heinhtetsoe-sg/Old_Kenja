...
class knjh410_knja110_2aForm2
{
    function main(&$model)
    {
        //フォーム作成
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjh410_knja110_2aindex.php", "", "edit");
        $arg["reload"] = "";
        $db = Query::dbCheckOut();

        //保護者履歴入力ボタン
        $link = REQUESTROOT."/U/KNJH410_KNJA110_2A/knjh410_knja110_2aindex.php?cmd=rireki&SCHREGNO=".$model->schregno;
        $extra = "onclick=\"Page_jumper('$link', 1);\"";
        $arg["button"]["btn_rireki"] = knjCreateBtn($objForm, "btn_rireki", "保護者履歴修正", $extra);

        //保証人履歴入力ボタン
        $link = REQUESTROOT."/U/KNJH410_KNJA110_2A/knjh410_knja110_2aindex.php?cmd=rireki2&SCHREGNO=".$model->schregno;
        ...
        $row1 = $db->getRow(knjh410_knja110_2aQuery::getRow_Address($model->schregno,$model->issuedate),DB_FETCHMODE_ASSOC);
        //保護者情報の取得      (GUARDIAN_DAT、GUARDIAN_ADDRESS_DAT)
        $row2 = $db->getRow(knjh410_knja110_2aQuery::getGuardianAddr($model, $model->schregno,$model->guard_issuedate),DB_FETCHMODE_ASSOC);
        //緊急連絡先情報の取得  (SCHREG_BASE_MST)
        $row3 = $db->getRow(knjh410_knja110_2aQuery::getEmergencyInfo($model->schregno),DB_FETCHMODE_ASSOC);
        //保証人情報の取得      (GUARDIAN_DAT、GUARANTOR_ADDRESS_DAT)
        $row4 = $db->getRow(knjh410_knja110_2aQuery::getGuarantorAddr($model, $model->schregno, $model->guarantor_issuedate),DB_FETCHMODE_ASSOC);
        //その他情報の取得      (SCHREG_SEND_ADDRESS_DAT)
        $row5 = $db->getRow(knjh410_knja110_2aQuery::getSendAddr($model, $model->schregno, $model->send_div),DB_FETCHMODE_ASSOC);
        //家族情報の取得      (SCHREG_RELA_DAT)
        $row6 = $db->getRow(knjh410_knja110_2aQuery::getRelaDat($model, $model->schregno, $model->rela_no),DB_FETCHMODE_ASSOC);

        $copyRow = array(); //コピーボタン:Hiddenで保持用
        //保護者情報の取得      (GUARDIAN_DAT、GUARDIAN_ADDRESS_DAT)
        $copyRow = $db->getRow(knjh410_knja110_2aQuery::getGuardianAddr($model, $model->schregno, "", "copy"),DB_FETCHMODE_ASSOC);
        ...
        $model->form2 = array_merge((array)$row1,(array)$row2,(array)$row3,(array)$row4,(array)$row5,(array)$row6);

        if(!$model->isWarning()){
            $row  = array_merge((array)$row1,(array)$row2,(array)$row3,(array)$row4,(array)$row5,(array)$row6);
        }else{
            $row =& $model->field;
        }
        ...
        if ($model->infoDiv == "2" || $model->infoDiv == "3") {
            ...
            if ($model->infoDiv == "3") $arg["INFO_DIV2_TITLE"]  = "保護者情報２";
...
/******************************************************************************/
        ...
        $result = $db->query(knjh410_knja110_2aQuery::get_name_mst());
...
/********************************************************************************/
        ...
        //兄弟姉妹検索ボタン
        $extra = "style=\"width:140px\"onclick=\"loadwindow('./knjh410_knja110_2aSearchBrother.php?cmd=search&MODE=set&useGuardian2={$model->Properties["useGuardian2"]}&CD=$model->schregno&SCHREGNO='+document.forms[0]['RELA_SCHREGNO'].value+'&STUCD_SESSID={$sess->id}&frame='+getFrameName(self), event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 600, 350)\"";
        ...
        //保護者基礎データの更新
//        $link = REQUESTROOT."/U/KNJH410_KNJA110_2A/knjh410_knja110_2aindex.php?cmd=guardian_hist_dat_kousin&SCHREGNO=".$model->schregno;
...
//        $arg["button"]["GUARDIAN_HIST_DAT_KOUSIN"] = knjCreateBtn($objForm, "GUARDIAN_HIST_DAT_KOUSIN", "保護者基礎データの更新", $extra);
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "parent.top_frame.location.href='knjh410_knja110_2aindex.php?cmd=list';";
        }

        View::toHTML($model, "knjh410_knja110_2aForm2.html", $arg);
    }
}
?>
