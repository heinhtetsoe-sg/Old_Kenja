...
class knjh564fForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjh564fForm1", "POST", "knjh564findex.php", "", "knjh564fForm1");
        ...
        $model->z010 = $db->getOne(knjh564fQuery::getNameMst("Z010", "00"));

        //学期コンボ作成
        $query = knjh564fQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjh564f'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //データ種別コンボ作成
        $query = knjh564fQuery::getDataDiv();
        $extra = "onchange=\"return btn_submit('knjh564f')\"";
        makeCmb($objForm, $arg, $db, $query, "PROFICIENCYDIV", $model->field["PROFICIENCYDIV"], $extra, 1);

        //テスト名称コンボ作成
        $query = knjh564fQuery::getProName($model->field["SEMESTER"], $model);
        $extra = "onchange=\"return btn_submit('knjh564f')\"";
        ...
        $extra = array("id=\"CATEGORY_IS_CLASS1\" onClick=\"return btn_submit('knjh564f')\"", "id=\"CATEGORY_IS_CLASS2\" onClick=\"return btn_submit('knjh564f')\"");
        ...
        //学年コンボ作成
        $query = knjh564fQuery::getGradeHrClass($model->field["SEMESTER"], $model, "GRADE");
        $extra = "onchange=\"return btn_submit('change_grade'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        if ($model->field["CATEGORY_IS_CLASS"] == 2){
            //クラスコンボ作成
            $query = knjh564fQuery::getGradeHrClass($model->field["SEMESTER"], $model, "HR_CLASS");
            $extra = "onchange=\"return btn_submit('knjh564f'), AllClearList();\"";
        ...
        //hidden
        knjCreateHidden($objForm, "NOT_PRINT_JUNI", 1);
        ...
        View::toHTML($model, "knjh564fForm1.html", $arg); 
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["CATEGORY_IS_CLASS"] == 1){
        ...
        //対象外の生徒取得
        $query = knjh564fQuery::getSchnoIdou($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt_idou = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //対象者リストを作成する
        $query = knjh564fQuery::getStudent($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt1 = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $idou = "　";
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJH564F");
