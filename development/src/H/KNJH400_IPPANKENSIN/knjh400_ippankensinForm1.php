...
class knjh400_ippankensinForm1
{
    public function main(&$model)
    {
        $objForm      = new form();
        $arg["start"] = $objForm->get_start("edit", "POST", "knjh400_ippankensinindex.php", "", "edit");

        knjh400_ippankensinForm1::mainYear($objForm, $model, $arg, $model->year);
        $arg['data1'] = $arg['data'];
        unset($arg['data']);

        knjh400_ippankensinForm1::mainYear($objForm, $model, $arg, $model->year - 1);
        $arg['data2'] = $arg['data'];
        unset($arg['data']);

        knjh400_ippankensinForm1::mainYear($objForm, $model, $arg, $model->year - 2);
        $arg['data3'] = $arg['data'];
        unset($arg['data']);

        $arg['setYear1'] = $model->year;
        $arg['setYear2'] = $model->year - 1;
        $arg['setYear3'] = $model->year - 2;

        $arg["finish"]  = $objForm->get_finish();
        View::toHTML($model, "knjh400_ippankensinForm1.html", $arg);
    }
    public function mainYear(&$objForm, &$model, &$arg, $year)
    {
        //警告メッセージを表示しない場合
        if (!isset($model->warning) && isset($model->schregno)) {
            $RowH = knjh400_ippankensinQuery::getMedexamHdat($model, $year);      //生徒健康診断ヘッダデータ取得
            $RowD = knjh400_ippankensinQuery::getMedexamDetDat($model, $year);   //生徒健康診断詳細データ取得
            $arg["NOT_WARNING"] = 1;
        } else {
            $RowH =& $model->field;
            $RowD =& $model->field;
        ...
        if (isset($model->schregno)) {
            //生徒学籍データを取得
            $result = $db->query(knjh400_ippankensinQuery::getSchregBaseMst($model));
            $RowB = $result->fetchRow(DB_FETCHMODE_ASSOC);
            $result->free();
            //生徒学籍番号
            $arg["header"]["SCHREGNO"] = $model->schregno;
            //生徒名前
            $arg["header"]["NAME_SHOW"] = $db->getOne(knjh400_ippankensinQuery::getName($model));
            //生徒生年月日
            $birth_day = explode("-", $RowB["BIRTHDAY"]);
            $arg["header"]["BIRTHDAY"] = $birth_day[0]."年".$birth_day[1]."月".$birth_day[2]."日";
            //生徒学年クラスを取得
            $result = $db->query(knjh400_ippankensinQuery::getSchregRegdDat($model));
        ...
        //未検査項目設定
        $notFieldSet = $sep = "";
        $query = knjh400_ippankensinQuery::getMedexamDetNotExaminedDat($model);
        ...
        $height = $weight = "";
        $sepH = $sepW = "";
        $result = $db->query(knjh400_ippankensinQuery::getHeightWeightList($model));
        ...
        //標準偏差値取得
        $sd = $db->getRow(knjh400_ippankensinQuery::getSD($model), DB_FETCHMODE_ASSOC);
        ...
        //測定評価平均値データ取得
        $phys_avg = $db->getRow(knjh400_ippankensinQuery::getHexamPhysicalAvgDat($model, $RowB["SEX"], $age), DB_FETCHMODE_ASSOC);
        ...
        //視力
        $query = knjh400_ippankensinQuery::getLRBarevisionMark($model);
        $extra = "";
        if ($arg["is_mie"] == "1") {
            //視力・右裸眼(文字)
        ...
        $result  = $db->query(knjh400_ippankensinQuery::getLREar($model, ""));
        $opt     = array();
        $opt[]   = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($arg["is_ear_otherType"] == "1") {
            $query = knjh400_ippankensinQuery::getNameMst($model, "F010");
        ...
        $opt    = array();
        $opt[]  = $optnull;
        $result = $db->query(knjh400_ippankensinQuery::getUric($model, "F020"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $opt    = array();
        $opt[]  = $optnull;
        $result = $db->query(knjh400_ippankensinQuery::getUric($model, "F019"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array("label" => $row["NAME1"],
                           "value" => $row["NAMECD2"]);
        }
        ...
        $opt    = array();
        $opt[]  = $optnull;
        $result = $db->query(knjh400_ippankensinQuery::getUric($model, "F018"));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array("label" => $row["NAME1"],
                           "value" => $row["NAMECD2"]);
        }
        ...
        if ($model->Properties["printKenkouSindanIppan"] != "2") {
            ...
            $result     = $db->query(knjh400_ippankensinQuery::getUriCothertest($model));
            $opt        = array();
            $opt[]      = $optnull;
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //尿:指導区分コンボ
        $result     = $db->query(knjh400_ippankensinQuery::getUriAdvisecd($model));
        $opt        = array();
        $opt[]      = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array("label" => $row["LABEL"],
        ...
        //栄養状態コンボ
        $nutrInfo = $sep = '';
        $result     = $db->query(knjh400_ippankensinQuery::getNutrition($model, ""));
        $opt        = array();
        $opt[]      = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $namecd = substr($row["NAMECD2"], 0, 2);
        ...
        //目の疾病及び異常コンボ
        $result     = $db->query(knjh400_ippankensinQuery::getEyedisease($model, ""));
        $opt        = array();
        $opt[]      = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $namecd = substr($row["NAMECD2"], 0, 2);
        ...
        $opt        = array();
        $opt[]      = $optnull;
        $result     = $db->query(knjh400_ippankensinQuery::getEyedisease5($model, "F051", ""));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $namecd = substr($row["NAMECD2"], 0, 2);
            $opt[] = array("label" => $namecd."  ".$row["NAME1"],
                           "value" => $row["NAMECD2"]);
        ...
        //脊柱・胸部コンボ
        $result     = $db->query(knjh400_ippankensinQuery::getSpinerib($model, ""));
        $opt        = array();
        $opt[]      = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $namecd = substr($row["NAMECD2"], 0, 2);
        ...
        //耳鼻咽頭疾患コンボ
        $result     = $db->query(knjh400_ippankensinQuery::getNosedisease($model, ""));
        $opt        = array();
        $opt[]      = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $namecd = substr($row["NAMECD2"], 0, 2);
        ...
        $opt        = array();
        $opt[]      = $optnull;
        $result     = $db->query(knjh400_ippankensinQuery::getNosedisease567($model, "F061", ""));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $namecd = substr($row["NAMECD2"], 0, 2);
            $opt[] = array("label" => $namecd."  ".$row["NAME1"],
                           "value" => $row["NAMECD2"]);
        ...
        $opt        = array();
        $opt[]      = $optnull;
        $result     = $db->query(knjh400_ippankensinQuery::getNosedisease567($model, "F062", ""));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $namecd = substr($row["NAMECD2"], 0, 2);
            $opt[] = array("label" => $namecd."  ".$row["NAME1"],
                           "value" => $row["NAMECD2"]);
        ...
        $opt        = array();
        $opt[]      = $optnull;
        $result     = $db->query(knjh400_ippankensinQuery::getNosedisease567($model, "F063", ""));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $namecd = substr($row["NAMECD2"], 0, 2);
            $opt[] = array("label" => $namecd."  ".$row["NAME1"],
                           "value" => $row["NAMECD2"]);
        ...
        //皮膚疾患コンボ
        $result     = $db->query(knjh400_ippankensinQuery::getSkindisease($model, ""));
        $opt        = array();
        $opt[]      = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $namecd = substr($row["NAMECD2"], 0, 2);
        ...
        //心臓・臨床医学的検査コンボ
        $result     = $db->query(knjh400_ippankensinQuery::getHeartMedexam($model, ""));
        $opt        = array();
        $opt[]      = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $namecd = substr($row["NAMECD2"], 0, 2);
        ...
        //心臓・疾病及び異常コンボ
        $result     = $db->query(knjh400_ippankensinQuery::getHeartdisease($model, ""));
        $opt        = array();
        $opt[]      = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $namecd = substr($row["NAMECD2"], 0, 2);
        ...
        //心臓・管理区分
        $optnull = array("label" => "","value" => "");   //初期値：空白項目
        $result  = $db->query(knjh400_ippankensinQuery::getManagementDiv($model));
        $opt     = array();
        $opt[]   = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $namecd = substr($row["NAMECD2"], 0, 2);
        ...
        //結核・所見コンボ
        $result     = $db->query(knjh400_ippankensinQuery::getTbRemark($model, ""));
        $opt        = array();
        $opt[]      = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $namecd = substr($row["NAMECD2"], 0, 2);
        ...
        //結核・その他検査コンボ
        $result     = $db->query(knjh400_ippankensinQuery::getTbOthertest($model, ""));
        $opt        = array();
        $opt[]      = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $namecd = substr($row["NAMECD2"], 0, 2);
        ...
        //結核・病名コンボ
        $result     = $db->query(knjh400_ippankensinQuery::getTbName($model, ""));
        $opt        = array();
        $opt[]      = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $namecd = substr($row["NAMECD2"], 0, 2);
        ...
        //結核・指導区分コンボ
        $result     = $db->query(knjh400_ippankensinQuery::getTbAdvise($model));
        $opt        = array();
        $opt[]      = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $namecd = substr($row["NAMECD2"], 0, 2);
        ...
        //寄生虫卵コンボ
        $query = knjh400_ippankensinQuery::getNameMst($model, "F023");
        $extra = "";
        makeCmb($objForm, $arg, $db, $query, "PARASITE", $RowD["PARASITE"], $extra.$entMove, 1, "blank");

        //その他疾病及び異常コンボ
        $result     = $db->query(knjh400_ippankensinQuery::getOtherDisease($model, ""));
        $opt        = array();
        $opt[]      = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $namecd = substr($row["NAMECD2"], 0, 2);
        ...
        //その他疾病及び異常:指導区分コンボ
        $result     = $db->query(knjh400_ippankensinQuery::getOtherAdvisecd($model));
        $opt        = array();
        $opt[]      = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array("label" => $row["LABEL"],
        ...
        if ($arg["is_mie"] == "1") {
            //学校医（内科検診）
            $result     = $db->query(knjh400_ippankensinQuery::getDocCd($model));
            $opt        = array();
            $opt[]      = $optnull;
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                $namecd = substr($row["NAMECD2"], 0, 2);
        ...
        //事後措置コンボ
        $result     = $db->query(knjh400_ippankensinQuery::getTreat($model, ""));
        $opt        = array();
        $opt[]      = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $namecd = substr($row["NAMECD2"], 0, 2);
        ...
        //事後措置2コンボ
        $query = knjh400_ippankensinQuery::getTreat2($model);
        $extra = "style=\"width:170px;\"";
        ...
        /**********************************/
        //既往症
        $result     = $db->query(knjh400_ippankensinQuery::getMedicalHist($model));
        $opt        = array();
        $opt[]      = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array("label" => $row["LABEL"],
        ...
        //運動/指導区分
        $result     = $db->query(knjh400_ippankensinQuery::getGuideDiv($model));
        $opt        = array();
        $opt[]      = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array("label" => $row["LABEL"],
        ...
        //運動/部活動
        $result     = $db->query(knjh400_ippankensinQuery::getJoiningSportsClub($model));
        $opt        = array();
        $opt[]      = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array("label" => $row["LABEL"],
        ...
        /* ボタン作成 */
        //一括更新ボタン1
        $link = REQUESTROOT."/F/KNJh400_ippankensin/knjh400_ippankensinindex.php?cmd=replace1&SCHREGNO=".$model->schregno;
        $extra = "style=\"width:80px\" onclick=\"Page_jumper('{$link}');\"";
        $arg["button"]["btn_replace1"] = knjCreateBtn($objForm, "btn_replace", "一括更新1", $extra);

        //一括更新ボタン2
        $link = REQUESTROOT."/F/KNJh400_ippankensin/knjh400_ippankensinindex.php?cmd=replace2&SCHREGNO=".$model->schregno;
        $extra = "style=\"width:80px\" onclick=\"Page_jumper('{$link}');\"";
        $arg["button"]["btn_replace2"] = knjCreateBtn($objForm, "btn_replace", "一括更新2", $extra);

        //一括更新ボタン3
        $link = REQUESTROOT."/F/KNJh400_ippankensin/knjh400_ippankensinindex.php?cmd=replace3&SCHREGNO=".$model->schregno;
        $extra = "style=\"width:80px\" onclick=\"Page_jumper('{$link}');\"";
        $arg["button"]["btn_replace3"] = knjCreateBtn($objForm, "btn_replace", "一括更新3", $extra);

        //一括更新ボタン4
        $link = REQUESTROOT."/F/KNJh400_ippankensin/knjh400_ippankensinindex.php?cmd=replace4&SCHREGNO=".$model->schregno;
        $extra = "style=\"width:80px\" onclick=\"Page_jumper('{$link}');\"";
        ...
        $tmp = knjh400_ippankensinQuery::getMedexamDetDat($model);
        if ($model->Properties["printKenkouSindanIppan"] == "1") {
