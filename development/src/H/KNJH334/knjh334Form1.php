...
class knjh334Form1
{
    ...
    {
        $objForm      = new form;
        $arg["start"] = $objForm->get_start("list", "POST", "knjh334index.php", "", "list");
        ...
        //参照年度コンボ
        $query = knjh334Query::getReferenceYear();
        ...
        //対象年度コンボ
        $query = knjh334Query::getObjectYear();
        $extra = "onChange=\"btn_submit('changeOyear')\";";
        makeCmb($objForm, $arg, $db, $query, $model->oyear, "OYEAR", $extra);

        //権限設定グループコンボ
        $query = knjh334Query::getUserGroup($model);
        $extra = "onChange=\"btn_submit('changeUser')\";";
        makeCmb($objForm, $arg, $db, $query, $model->usergroup, "USERGROUP", $extra);
        ...
        if ($model->cmd == "changeOyear" || $model->cmd == "changeUser") {
            $arg["reload"] = "window.open('knjh334index.php?cmd=sel&init=1', 'right_frame')";
        } else {
            $arg["reload"] = "window.open('knjh334index.php?cmd=sel', 'right_frame')";
        }

        View::toHTML($model, "knjh334Form1.html", $arg); 
    }
}
...
{
    $result = $db->query(knjh334Query::GetGroupName($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $mainData = "<tr> ";
        $link = View::alink("knjh334index.php",
                            $row["GROUPNAME"],
                            "target=right_frame",
                            array("cmd"       => "sel",
                                  "RYEAR"     => $model->ryear,
        ...
        //目標値グループ設定
        $staffMock = setListData($db, $model, "2", $row["GROUPCD"], "LAST");
...
{
    $rtnData = "";
    $resultMdata = $db->query(knjh334Query::getGroupData($model, $target, $groupcd));
