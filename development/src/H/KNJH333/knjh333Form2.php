...
class knjh333Form2
{
    ...
    {
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("sel", "POST", "knjh333index.php", "", "sel");
        ...
        //グループコードTEXT
        $query = knjh333Query::getGropuName($model->groupcd);
        ...
        //職員目標値グループリストToリスト作成
        makeGroupDataList($objForm, $arg, $db, $model, "STAFF", "1", 10);
        ...
        if (isset($model->message)) { //更新できたら左のリストを再読込
            $arg["reload"] = "window.open('knjh333index.php?cmd=list&init=1', 'left_frame')";
        }

        View::toHTML($model, "knjh333Form2.html", $arg); 
    }
}
...
{
    $arg["STAFFNAME"] = "利用者：".$db->getOne(knjh333Query::getStaffName());
...
function makeGroupDataList(&$objForm, &$arg, $db, &$model, $name, $target, $size)
{
    ...
    $result = $db->query(knjh333Query::selectQuery($model, $target));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        if ($row["JOINCD"]) {
...
{
    $link = REQUESTROOT."/H/KNJH335/knjh335index.php?PROGRAMID=KNJH333";
    //追加ボタン
...
function makeHidden(&$objForm, $model)
{
    ...
    $objForm->ae(createHiddenAe("staffselect"));            //更新用目標値マスタ
