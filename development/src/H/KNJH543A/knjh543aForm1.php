...
class knjh543aForm1
{
    function main(&$model)
    {
        ...
        $arg["start"]   = $objForm->get_start("main", "POST", "knjh543aindex.php", "", "main");
        ...
        //基準の計算方法 4:合算
        $query = knjh543aQuery::getNameMstZ027("4");
        ...
        View::toHTML($model, "knjh543aForm1.html", $arg); 
    }
}
/***************************************    これ以下は関数    **************************************************/
////////////////////////
...
function makeList(&$objForm, &$arg, $db, $standard, &$model) {
    ...
    $opt = array();
    $query = knjh543aQuery::getNameMstZ027();
    ...
    $query = knjh543aQuery::getList($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        //合併先科目コード保管
