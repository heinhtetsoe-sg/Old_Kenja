...
//ビュー作成用クラス
class knjh400_TyousasyoSyokenForm2
{
    public function main(&$model)
    {
        ...
        $arg = array();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("form2", "POST", "knjh400_TyousasyoSyokenindex.php", "", "form2");
        ...
        if ($model->Properties["tyousasyo_shokenTable_Seq"] == "1") {
            ...
            $arg["SELECT_PATTERN_NAME"] = "パターン".$model->selectPattern;

            knjCreateHidden($objForm, "SELECT_PATTERN", $model->selectPattern);
        }
        ...
        $disabled = "disabled";
        $query = knjh400_TyousasyoSyokenQuery::selectQueryAnnualKnjh400TyousasyoSyokenForm2($model);
        ...
        $query = knjh400_TyousasyoSyokenQuery::getGradeCd($model, "set");
        ...
        if (!isset($model->warning) && $model->cmd != 'reload4') {
            if ($model->cmd == "reload2_ok" || $model->cmd == "reload2_cancel") {
                $query = knjh400_TyousasyoSyokenQuery::selectQueryHtrainremarkDat($model);
                ...
                if ($model->cmd !== 'torikomi3' && $model->cmd !== 'torikomi4') {
                    if ($model->Properties["tyousasyo_shokenTable_Seq"] == "1") {
                        $query = knjh400_TyousasyoSyokenQuery::selectQueryForm2Pattern($model);
                    } else {
                        $query = knjh400_TyousasyoSyokenQuery::selectQueryForm2($model);
                    }
                    $row = $db->getRow($query, DB_FETCHMODE_ASSOC);
                    ...
                    if ("1" == $model->Properties["tyousasyo2020"]) {
                        for ($i = 1; $i <= 6; $i++) {
                            $row["TRAIN_REF".$i] = "";
                            $seq = "10".$i;
                            $query = knjh400_TyousasyoSyokenQuery::getHexamEntremarkTrainrefDat($model, $model->annual["YEAR"], $seq);
        ...
        if ($model->entDiv == '4' || $model->entDiv == '5' || $model->entDiv == '7') {
            $arg["addYear"] = 1;
            //追加年度
            $query = knjh400_TyousasyoSyokenQuery::selectYearQuery($model);
            $extra = "onChange=\"return btn_submit('form2');\"";
            makeCmb($objForm, $arg, $db, $query, $model->addYear, "ADD_YEAR", $extra, 1);

            //追加学年
            $query = knjh400_TyousasyoSyokenQuery::selectGradeQuery($model);
            $extra = "";
        ...
        View::toHTML($model, "knjh400_TyousasyoSyokenForm2.html", $arg);
    }
}
...
function makeAttendrecRemark(&$objForm, &$arg, $model, $db, $disabled, $row)
{
    ...
    if ($model->cmd === 'torikomi3') {
        $set_remark = knjh400_TyousasyoSyokenQuery::getSemesRemark($model, $db, $model->annual["YEAR"]);
        $row["ATTENDREC_REMARK"] = $set_remark;
    } elseif ($model->cmd === 'torikomi4') {
        $set_remark = knjh400_TyousasyoSyokenQuery::getHreportremarkDetailDat($db, $model);
        $row["ATTENDREC_REMARK"] = $set_remark;
    }
...
function makeSpecialactrec(&$objForm, &$arg, $model, $db, $disabled, $row)
{
    ...
    if ($model->getSchoolName == "koma") {
        ...
        //臘八摂心皆勤
        $rouhatsuKaikin = "";
        $query = knjh400_TyousasyoSyokenQuery::getRouhatsuKaikin($model);
...
function makeShojikou(&$objForm, &$arg, $model, $db, $disabled, $row)
{
    ...
    if ("1" == $model->Properties["seitoSidoYorokuSogoShoken3Bunkatsu"] || $model->getSchoolName == 'tokiwa') {
        ...
        $query = knjh400_TyousasyoSyokenQuery::yorokuTrainref123($model, $model->annual["YEAR"]);
        ...
        $query = knjh400_TyousasyoSyokenQuery::yorokuSogoshoken($model);
        $sansyou = $db->getRow($query, DB_FETCHMODE_ASSOC);
...
function makeSelectBtn(&$objForm, $model, $div, $name, $label, $target, $disabled = "")
{
    if (!$div || !$name || !$label || !$target) {
        ...
        if ($div == "club") {       //部活動
            ...
            $tgt = "/E/KNJE011D/knjh400_TyousasyoSyokenindex.php";
            $w = 900;
            $h = 600;
