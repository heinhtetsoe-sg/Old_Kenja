...
//ビュー作成用クラス
class knjh400_TyousasyoSyokenForm1
{
    public function main(&$model)
    {
        $objForm = new form();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjh400_TyousasyoSyokenindex.php", "", "edit");
        $db = Query::dbCheckOut();
        ...
        if ($model->Properties["tyousasyo_shokenTable_Seq"] == "1") {
            ...
            //参照パターン
            $query = knjh400_TyousasyoSyokenQuery::getPatternSeq($model);
        ...
        if (!isset($model->warning) && $model->cmd != 'reload3') {
            if ($model->Properties["tyousasyo_shokenTable_Seq"] == "1") {
                $query = knjh400_TyousasyoSyokenQuery::selectQueryPattern($model);
                $row = $db->getRow($query, DB_FETCHMODE_ASSOC);
            } else {
                $query = knjh400_TyousasyoSyokenQuery::selectQuery($model);
                $row = $db->getRow($query, DB_FETCHMODE_ASSOC);
            }

            $query = knjh400_TyousasyoSyokenQuery::getHexamEntremarkRemarkHdat($model);
        ...
        $name = $db->getOne(knjh400_TyousasyoSyokenQuery::getName($model));
        $arg["NAME_SHOW"] = $model->schregno."　".$name;
...
//        }
        ...
        //年次取得
        $model->gradecd = "";
        $model->gradecd = $db->getOne(knjh400_TyousasyoSyokenQuery::getGradeCd($model));
        ...
        View::toHTML($model, "knjh400_TyousasyoSyokenForm1.html", $arg);
    }
}
...
function makeBiko(&$arg, &$objForm, $model, &$row)
{
    ...
    if ("1" != $model->Properties["unuse_KNJE015_HEXAM_ENTREMARK_LEARNING_DAT_REMARK"]) {
        $extra = " onclick=\"loadwindow('".REQUESTROOT."/E/KNJE011D/knjh400_TyousasyoSyokenindex.php?&cmd=formBikouTori&TARGET=REMARK',0,document.documentElement.scrollTop || document.body.scrollTop,1000,650);\"";
...
function makeButton(&$arg, &$objForm, $db, $model)
{
    ...
    if ("1" == $model->Properties["tyousasho6bunkatsu_homeroomShojikouTorikomi"]) {
        $query = knjh400_TyousasyoSyokenQuery::getHrShojikouTorikomiDataCheck($model);
    ...
    //セキュリティーチェック
    $securityCnt = $db->getOne(knjh400_TyousasyoSyokenQuery::getSecurityHigh());
    $csvSetName = "CSV";
    ...
    if ($model->getPrgId || !$model->Properties["useXLS"] || $securityCnt == 0) {
        //データCSVボタン
        $extra = "onClick=\" wopen('".REQUESTROOT."/X/KNJX190/knjx190index.php?program_id=".PROGRAMID."&mode={$model->mode}&SEND_PRGID=KNJE011D&SEND_AUTH={$model->auth}&tyousasyo2020=1','SUBWIN2',0,0,screen.availWidth,screen.availHeight);\"";
        $arg["button"]["btn_check1"] = knjCreateBtn($objForm, "btn_check1", "データ".$csvSetName, $extra);
        //ヘッダデータCSVボタン
        $extra = "onClick=\" wopen('".REQUESTROOT."/X/KNJX191/knjx191index.php?program_id=".PROGRAMID."&mode={$model->mode}&SEND_PRGID=KNJE011D&SEND_AUTH={$model->auth}&tyousasyo2020=1','SUBWIN2',0,0,screen.availWidth,screen.availHeight);\"";
...
function makeHidden(&$objForm, $db, $model)
{
    ...
    knjCreateHidden($objForm, "tyousasyo_shokenTable_Seq", $model->Properties["tyousasyo_shokenTable_Seq"]);

    //プレビュー／印刷用パラメータ
    if ($model->Properties["tyousasyoShokenPreview"] == '1') {
        knjCreateHidden($objForm, "PRGID", "KNJE011D");
        ...
        $query = knjh400_TyousasyoSyokenQuery::getSchoolDiv($model);
...
function torikomiRowTotalstudy($db, $model, &$row)
{
    if (intval($model->gradecd) < "3") {
        //1,2年次　指導要録取込
        $query = knjh400_TyousasyoSyokenQuery::getYouroku($model);
        ...
        $query = knjh400_TyousasyoSyokenQuery::getRecordTotalstudytimeDat($model, $model->exp_year);
        $result = $db->query($query);
