...
class knjh566aForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjh566aForm1", "POST", "knjh566aindex.php", "", "knjh566aForm1");
        ...
        //学期コンボ
        $query = knjh566aQuery::getSemesterMst();
        $extra = "onchange=\"return btn_submit('knjh566a')\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //学年コンボ
        $query = knjh566aQuery::getGradeHrClass($model);
        $extra = "onchange=\"return btn_submit('knjh566a')\"";
        ...
        View::toHTML($model, "knjh566aForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    //クラス一覧リスト作成する
    $row1 = array();
    $query = knjh566aQuery::getStudent($model);
    $result = $db->query($query);
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJH566A");
