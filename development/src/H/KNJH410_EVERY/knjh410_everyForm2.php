...
class knjh410_everyForm2
{
    function main(&$model)
    {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjh410_everyindex.php", "", "edit");

        $db = Query::dbCheckOut();
        
        if($model->cmd == "choice"){
            //選択されたデータを取得する
            $query = knjh410_everyQuery::getChoiceData($model->schregno, $model->recNo);
        ...
        //入力者
        $query = knjh410_everyQuery::getStaffName($model->field["REGISTERCD"]);
        ...
        //タグの頻出データを表示
        $query = knjh410_everyQuery::getTagCnt();
        $result = $db->query($query);
        $sp = "";
        $linkData = "";
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        View::toHTML($model, "knjh410_everyForm2.html", $arg);
    }
} 
//コンボ作成
