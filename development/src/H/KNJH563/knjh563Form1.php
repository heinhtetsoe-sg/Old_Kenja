...
class knjh563Form1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjh563Form1", "POST", "knjh563index.php", "", "knjh563Form1");
        ...
        $extra = array("id=\"CATEGORY_IS_CLASS1\" onClick=\"return btn_submit('knjh563')\"", "id=\"CATEGORY_IS_CLASS2\" onClick=\"return btn_submit('knjh563')\"");
        ...
        //学期コンボ作成
        $query = knjh563Query::getSemester();
        $extra = "onchange=\"return btn_submit('knjh563'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //データ種別
        $query = knjh563Query::getDataDiv();
        $extra = "onchange=\"return btn_submit('knjh563'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "PROFICIENCYDIV", $model->field["PROFICIENCYDIV"], $extra, 1);

        //テスト名称
        $query = knjh563Query::getProName($model);
        $extra = "onchange=\"return btn_submit('knjh563'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "PROFICIENCYCD", $model->field["PROFICIENCYCD"], $extra, 1);

        //学年コンボ作成
        $query = knjh563Query::getGradeHrClass($model, "GRADE");
        $extra = "onchange=\"return btn_submit('knjh563'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        if ($model->field["CATEGORY_IS_CLASS"] == 2){
            //クラスコンボ作成
            $query = knjh563Query::getGradeHrClass($model, "HR_CLASS");
            $extra = "onchange=\"return btn_submit('knjh563'), AllClearList();\"";
        ...
        if ($model->Properties["knjh563hyoshiPrint"]) {
            $arg["show_hyoshi"] = "1";
            //チェックボックスを作成
            $extra = ($model->field["HYOSHI_PRINT"] == "1" || ($model->cmd == '' && $model->Properties["knjh563hyoshiPrint"] == "2")) ? "checked" : "";
            $extra .= " id=\"HYOSHI_PRINT\"";
            $arg["data"]["HYOSHI_PRINT"] = knjCreateCheckBox($objForm, "HYOSHI_PRINT", "1", $extra, "");
        }
        ...
        View::toHTML($model, "knjh563Form1.html", $arg); 
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["CATEGORY_IS_CLASS"] == 1){
        ...
        //対象外の生徒取得
        $query = knjh563Query::getSchnoIdou($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt_idou = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //対象者リストを作成する
        $query = knjh563Query::getStudent($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt1 = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $idou = "　";
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJH563");
    ...
    knjCreateHidden($objForm, "knjh563PrintScoreKansannashi", $model->Properties["knjh563PrintScoreKansannashi"]);
