...
class knjh714Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjh714Form1", "POST", "knjh714index.php", "", "knjh714Form1");
        ...
        $query = knjh714Query::getTestName();
        makeCmb($objForm, $arg, $db, $query, "TESTNAME", $model->field["TESTNAME"], $extra, 1);

        //時限コンボ
        $extra = "onchange=\"return btn_submit('change_header');\"";
        $query = knjh714Query::getPeriod($model);
        ...
        View::toHTML($model, "knjh714Form1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    ...
    //施設情報取得
    $query = knjh714Query::getSisetuList($model);//施設一覧取得
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        //一覧リスト（右側）
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJH714");
