...
#       $query .= " ORDER BY GC";
    ...
    //フォームタブ(開始)
    function getStart() {
        return $this->form->get_start("stucd","POST","knjh020_2aSubForm1.php","","stucd");
    }
    //フォームタブ(終了)
    function getFinish() {
...
function apply_stucd(obj,cmd) {
    if(obj.selectedIndex >= 0) {
        ...
        if(cmd == 'reflect') {
            ...
            f.document.forms[0].RELANAME.value      = arr[3];
            f.document.forms[0].RELAKANA.value      = arr[4];
            f.document.forms[0].RELASEX.value       = (arr[5]=="男") ? 1 : 2;
            f.document.forms[0].RELABIRTHDAY.value  = arr[8];
            f.document.forms[0].RELA_SCHREGNO.value = arr[0];
            f.document.forms[0].REGD_GRD_FLG.value  = '1';
            var gdhr = arr[6].split(",");
            f.document.forms[0].RELA_GRADE.value = gdhr[0];
        }
        f.closeit();
    }else{
        alert("生徒が選択されていません。");
