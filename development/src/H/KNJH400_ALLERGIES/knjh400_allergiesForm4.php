...
class knjh400_allergiesForm4
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("form4", "POST", "knjh400_allergiesindex.php", "", "form4");
        ...
        $extra = "onchange=\"return btn_mit('form4')\"";
        makeCmb($objForm, $arg, $db, $query, "YEAR", $model->field["YEAR"], $extra, 1);

        //警告メッセージを表示しない場合
        if (isset($model->schregno) && !isset($model->warning)) {
            //データ取得 -- MEDEXAM_CARE_HDAT
            $Row = $db->getRow(knjh400_allergiesQuery::getCareHData($model, $model->field["YEAR"], "04"), DB_FETCHMODE_ASSOC);
            //データ取得 -- MEDEXAM_CARE_DAT
            $query = knjh400_allergiesQuery::getCareData($model, $model->field["YEAR"], "04");
            ...
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                ...
                if (strlen($row["CARE_REMARK2"]) > 0) {
                    $Row["DETAIL".$row["CARE_KIND"]."_".$row["CARE_ITEM"]."_".$row["CARE_SEQ"]] = $row["CARE_REMARK2"];
                }
            }
            $result->free();
        } else {
        ...
        $hdat = $db->getRow(knjh400_allergiesQuery::selectQuery($model), DB_FETCHMODE_ASSOC);
            ...
            for ($j=1; $j <= 4; $j++) {
                ...
                for ($k=1; $k <= $cnt[$i][$j]["cnt"]; $k++) {
                    ...
                    if ($i == 1 && $j == 1 && $hdat["CARE_FLG04"] != 1) {
                        $disable = " disabled";
                    } elseif ($i == 1 && $j == 2 && $hdat["CARE_FLG05"] != 1) {
                        $disable = " disabled";
                    }
        ...
        View::toHTML($model, "knjh400_allergiesForm4.html", $arg);
    }
}
...
{
    //前年度からコピーボタン
    $extra = (AUTHORITY >= DEF_UPDATE_RESTRICT && $model->field["YEAR"] == CTRL_YEAR) ? "onclick=\"return btn_mit('form4_copy');\"" : "disabled";
    $arg["button"]["btn_copy"] = knjCreateBtn($objForm, "btn_copy", "前年度からコピー", $extra);

    //更新ボタン
    $extra = (AUTHORITY >= DEF_UPDATE_RESTRICT && $model->field["YEAR"] == CTRL_YEAR) ? "onclick=\"return btn_mit('form4_update');\"" : "disabled";
    $arg["button"]["btn_update"] = knjCreateBtn($objForm, "btn_update", "更 新", $extra);
    //取消ボタン
    $extra = ($model->field["YEAR"] == CTRL_YEAR) ? "onclick=\"return btn_mit('form4_clear');\"" : "disabled";
