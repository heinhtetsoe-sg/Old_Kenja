...
class knjh339bForm1
{
    function main(&$model)
    {
        ...
        //テンプレートデータ種別コンボボックス
        $query = knjh339bQuery::getMockDiv();
        $extra = "onchange=\"btn_submit('main');\"";
        makeCmb($objForm, $arg, $db, $query, $model->data_div, "DATA_DIV", $extra, 1);

        //テンプレートデータ種別コンボボックス
        $query = knjh339bQuery::getMockMst($model);
        $extra = "";
        ...
        //フォーム作成
        $arg["start"]  = $objForm->get_start("main", "POST", "knjh339bindex.php", "", "main");
        $arg["finish"] = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjh339bForm1.html", $arg);
    }
}
