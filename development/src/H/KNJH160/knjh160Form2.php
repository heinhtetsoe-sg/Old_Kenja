...
class knjh160Form2
{
    ...
    {
        $objForm      = new form;
        $arg["start"] = $objForm->get_start("edit", "POST", "knjh160index.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        if (isset($model->schregno) && !isset($model->warning)) {
            $Row = knjh160Query::getStudent_data($db, $model->schregno);
        } else {
            $Row =& $model->field;
        }
        ...
        //事前処理チェック
        if (!knjh160Query::ChecktoStart($db)) {
        ...
        View::toHTML($model, "knjh160Form2.html", $arg);
    }
}
...
function makeOldRegdData(&$arg, $db, $model) {
    ...
    $query = knjh160Query::getOldRegdDat($model->schregno);
