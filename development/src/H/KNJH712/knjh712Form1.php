...
class knjh712Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjh712Form1", "POST", "knjh712index.php", "", "knjh712Form1");
        ...
        $onClick = " onclick =\" return btn_submit('change_header');\"";
        ...
        if ($model->field["KUBUN"] == "1") {
            ...
            $query = knjh712Query::getGradeHrclassName($model);
        ...
        View::toHTML($model, "knjh712Form1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    ...
    if ($model->field["KUBUN"] == "1") {
        ...
        $query = knjh712Query::getSchno($model);//生徒一覧取得
    } else {
        $arg["data"]["TITLE_RIGHT"] = "クラス一覧";
        $query = knjh712Query::getHrClass($model);//クラス一覧取得
    }
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        //一覧リスト（右側）
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJH712");
