...
class knjh140bForm1
{
    public function main($model)
    {
        ...
        $securityCnt = $db->getOne(knjh140bQuery::getSecurityHigh());
        //セキュリティーチェック
        ...
        $result = $db->query(knjh140bquery::getSelectFieldSQL());
        $opt_year  = array();
        ...
        //年組一覧コンボボックス
        $result = $db->query(knjh140bquery::getSelectFieldSQL2($model));
        $opt_gr_hr = array();
        $opt_gr_hr[] = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjh140bQuery::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJH140B");
        knjCreateHidden($objForm, "TEMPLATE_PATH");

        //DB切断
        Query::dbCheckIn($db);
        $arg["start"] = $objForm->get_start("main", "POST", "knjh140bindex.php", "", "main");

        $arg["finish"] = $objForm->get_finish();

        View::toHTML($model, "knjh140bForm1.html", $arg);
    }
}
