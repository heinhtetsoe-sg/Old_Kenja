...
//ビュー作成用クラス
class knjh400_TyousasyoKyuuForm2
{
    public function main(&$model)
    {
        ...
        $arg = array();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("form2", "POST", "knjh400_TyousasyoKyuuindex.php", "", "form2");

        //DB接続
        $db = Query::dbCheckOut();

        $name = $db->getOne(knjh400_TyousasyoKyuuQuery::getName($model));
        ...
        $gradeCd = $model->grade == "" ? "" : $db->getOne(knjh400_TyousasyoKyuuQuery::getGradeCd($model));
        ...
        $disabled = "disabled";
        $query = knjh400_TyousasyoKyuuQuery::selectQueryAnnualKnjh400TyousasyoKyuuForm2($model);
        ...
        $query = knjh400_TyousasyoKyuuQuery::getGradeCd($model, "set");
        ...
        if (!isset($model->warning) && $model->cmd != 'reload4' && $model->cmd != 'reload5') {
            if ($model->cmd == "reload2_ok" || $model->cmd == "reload2_cancel") {
                $query = knjh400_TyousasyoKyuuQuery::selectQueryHtrainremarkDat($model);
                ...
                if ($model->cmd !== 'torikomi3' && $model->cmd !== 'torikomi4') {
                    $query = knjh400_TyousasyoKyuuQuery::selectQueryForm2($model);
                    $row = $db->getRow($query, DB_FETCHMODE_ASSOC);
                } else {
                    $row = $model->field2;
                }
        ...
        if ($model->entDiv == '4' || $model->entDiv == '5' || $model->entDiv == '7') {
            $arg["addYear"] = 1;
            //追加年度
            $query = knjh400_TyousasyoKyuuQuery::selectYearQuery($model);
            $extra = "onChange=\"return btn_submit('form2');\"";
            makeCmb($objForm, $arg, $db, $query, $model->addYear, "ADD_YEAR", $extra, 1);

            //追加学年
            $query = knjh400_TyousasyoKyuuQuery::selectGradeQuery($model);
            $extra = "";
        ...
        if ($model->Properties["useSyojikou3"] == "1") {
            $query = knjh400_TyousasyoKyuuQuery::sansyouData($model);
        ...
        if ($model->cmd == 'reload4') {
            ...
            if (intval($model->combo_gradecd) < "3") {
                $query = knjh400_TyousasyoKyuuQuery::getYouroku($model);
                ...
                $query = knjh400_TyousasyoKyuuQuery::getRecordTotalstudytimeDat($model);
                $result = $db->query($query);
        ...
        if ($model->cmd == 'reload5') {
            $query = knjh400_TyousasyoKyuuQuery::getYourokuDetail($model);
            $resultYouroku = $db->query($query);
            $kaigyou = "";
            $row["TRAIN_REF1"] = "";
            $row["TRAIN_REF2"] = "";
        ...
        if ($model->cmd === 'torikomi3') {
            $set_remark = knjh400_TyousasyoKyuuQuery::getSemesRemark($model, $db, $model->annual["YEAR"]);
            $row["ATTENDREC_REMARK"] = $set_remark;
        } elseif ($model->cmd === 'torikomi4') {
            $set_remark = knjh400_TyousasyoKyuuQuery::getHreportremarkDetailDat($db, $model);
            $row["ATTENDREC_REMARK"] = $set_remark;
        }
        ...
        //学校判定用
        $getSchoolName = $db->getOne(knjh400_TyousasyoKyuuQuery::getSchoolName());
        ...
        if ($getSchoolName == "koma") {
            ...
            //臘八摂心皆勤
            $rouhatsuKaikin = "";
            $query = knjh400_TyousasyoKyuuQuery::getRouhatsuKaikin($model);
        ...
        if ($model->Properties["tyousasyoSougouHyoukaNentani"] == 1) {
            ...
            } elseif (intval($model->combo_gradecd) < "3" && $model->Properties["tyousasyo_SidoYorokYoriYomikomiHyoujiFlg"] == 1) {
                $extra = "onclick=\"return btn_submit('reload4');\" style=\"color:#1E90FF;font:bold;\"";
        ...
        View::toHTML($model, "knjh400_TyousasyoKyuuForm2.html", $arg);
    }
}
