...
//ビュー作成用クラス
class knjh400_TyousasyoKyuuForm1
{
    public function main(&$model)
    {
        $objForm = new form();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjh400_TyousasyoKyuuindex.php", "", "edit");
        $db = Query::dbCheckOut();

        if (!isset($model->warning) && $model->cmd != 'yomikomi') {
            $query = knjh400_TyousasyoKyuuQuery::selectQuery($model);
        ...
        //年次取得
        $model->gradecd = "";
        $model->gradecd = $db->getOne(knjh400_TyousasyoKyuuQuery::getGradeCd($model));
        ...
        //セキュリティーチェック
        $securityCnt = $db->getOne(knjh400_TyousasyoKyuuQuery::getSecurityHigh());
        $csvSetName = "CSV";
        ...
        if ($model->Properties["tyousasyoShokenPreview"] == '1') {
            ...
            $query = knjh400_TyousasyoKyuuQuery::getSchoolDiv();
        ...
        View::toHTML($model, "knjh400_TyousasyoKyuuForm1.html", $arg);
    }
}
