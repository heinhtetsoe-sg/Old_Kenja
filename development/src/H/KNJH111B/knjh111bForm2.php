...
class knjh111bform2 {
    function main(&$model) {
        ...
        //DB接続
        $db = Query::dbCheckOut();
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjh111bindex.php", "", "edit");
        ...
        if (isset($model->schregno) && isset($model->regddate) && !isset($model->warning) && $model->cmd != 'qualifiedCd' && $model->cmd != 'conditionDiv') {
            if (VARS::get("MANAGEMENT_FLG") == "1" || $model->seq == "00") {
                $query = knjh111bQuery::getSchregQualifiedTestDat($model);
            } else {
                $query = knjh111bQuery::getSchregQualifiedHobbyDat($model);
            }
        ...
        if ($model->Properties["useCurriculumcd"] == '1') {
            $query = knjh111bQuery::getSchoolKind($model);
        ...
        $opt = array();
        $value_flg = false;
        $query = knjh111bQuery::getQualifiedMst($Row);
        ...
        if ($model->Properties["useQualifiedManagementFlg"] == '1') {
            $model->managementFlg = $db->getOne(knjh111bQuery::getQualifiedMst_MFlg($Row));
        ...
        if ($model->managementFlg == "1") {
            $query = knjh111bQuery::getRankResultMst($Row["QUALIFIED_CD"], $model);
        } else {
            $query = knjh111bQuery::getSelectedRank($Row["QUALIFIED_CD"], $model);
            $ret = $db->getOne($query);
            if (!isset($ret)) {
                $query = knjh111bQuery::getRank();
            }
        }
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"].($row["SCORE"] ? "(".$row["SCORE"].")" : ""),
                           'value' => $row["VALUE"]);
            if ($Row["RANK"] == $row["VALUE"]) $value_flg = true;
        }
        ...
        //主催
        $query = knjh111bQuery::getPromoter($Row);
        ...
        //一括更新1
        $link = REQUESTROOT."/H/KNJH111B/knjh111bindex.php?cmd=replace1&SCHREGNO=".$model->schregno;
        $extra = "onclick=\"Page_jumper('{$link}');\"";
        $arg["button"]["btn_replace1"] = knjCreateBtn($objForm, "btn_replace1", "一括更新", $extra);
        //ＣＳＶ処理
        $extra = "onClick=\" wopen('".REQUESTROOT."/X/KNJX_H111A/knjx_h111aindex.php?program_id=".PROGRAMID."&SEND_PRGID=KNJH111B&SEND_AUTH=".AUTHORITY."','SUBWIN2',0,0,screen.availWidth,screen.availHeight);\"";
        ...
        View::toHTML($model, "knjh111bForm2.html", $arg);
    }
}
?>
