...
class knjh111bSubForm1 {
    function main(&$model) {
        $objForm        = new form;
        $arg["start"]   = $objForm->get_start("sel", "POST", "knjh111bindex.php", "", "sel");

        $db = Query::dbCheckOut();

        $query = knjh111bQuery::getHrclass($model);
...
/* 編集項目 */
        ...
        //戻る
        $link = REQUESTROOT."/H/KNJH111B/knjh111bindex.php?cmd=back";
        ...
        View::toHTML($model, "knjh111bSubForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    $query = knjh111bQuery::getStudent($model);
    $result   = $db->query($query);
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
