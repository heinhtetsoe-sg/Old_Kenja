...
class knjh341Form2
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjh341index.php", "", "edit");
        ...
        //警告メッセージを表示しない場合
        if ($model->thisIsGet) {
            $Row = knjh341Query::getRow($model->pref_subclasscd, $db);
        } else {
            $Row =& $model->field;
        }
        //模試科目コード
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "parent.left_frame.location.href='knjh341index.php?cmd=list';";
        }
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjh341Form2.html", $arg); 
    }
}
