...
class knjh441Form1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjh441Form1", "POST", "knjh441index.php", "", "knjh441Form1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        $extra = array("id=\"CATEGORY_IS_CLASS1\" onClick=\"return btn_submit('knjh441')\"", "id=\"CATEGORY_IS_CLASS2\" onClick=\"return btn_submit('knjh441')\"");
        ...
        //学年コンボ作成
        $query = knjh441Query::getGradeHrClass($model, "GRADE");
        $extra = "onchange=\"return btn_submit('change_grade'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        if ($model->field["CATEGORY_IS_CLASS"] == 2){
            //クラスコンボ作成
            $query = knjh441Query::getGradeHrClass($model, "HR_CLASS");
            $extra = "onchange=\"return btn_submit('knjh441'), AllClearList();\"";
        ...
        View::toHTML($model, "knjh441Form1.html", $arg); 
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["CATEGORY_IS_CLASS"] == 1) {
        ...
        //対象外の生徒取得
        $query = knjh441Query::getSchnoIdou($model);
        $result = $db->query($query);
        $opt_idou = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //対象者リストを作成する
        $query = knjh441Query::getStudent($model);
        $result = $db->query($query);
        $opt1 = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $idou = "　";
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJH441");
