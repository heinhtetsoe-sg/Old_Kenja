...
class knjh540Form2
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjh540index.php", "", "edit");
        ...
        if (!isset($model->warning)) {
            $Row = $db->getRow(knjh540Query::getList($model, "ONE"), DB_FETCHMODE_ASSOC);
        } else {
            $Row =& $model->field;
        }
        ...
        //区分コンボボックス
        $query = knjh540Query::getCourse($model);
        makeCombo($objForm, $arg, $db, $query, $model->course_div, "COURSE_DIV", "", 1);

        //学年コンボボックス
        $query = knjh540Query::getGrade($model);
        makeCombo($objForm, $arg, $db, $query, $model->grade, "GRADE", "", 1);

        //実力科目コンボボックス
        $query = knjh540Query::getMockSubclass();
        makeCombo($objForm, $arg, $db, $query, $model->proficiency_subclass_cd, "PROFICIENCY_SUBCLASS_CD", "", 1);
        ...
        if (VARS::get("cmd") != "edit") {
            if ($model->cmd != "reset") {
                $arg["reload"]  = "parent.left_frame.location.href='knjh540index.php?cmd=list2';";
            } else {
                $arg["reload"]  = "parent.left_frame.location.href='knjh540index.php?cmd=list&clear=no';";
            }
        }
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjh540Form2.html", $arg); 
    }
}
