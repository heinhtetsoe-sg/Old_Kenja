...
function selectKeiro() {
    ...
    eval("main_flg = frame_name.document.forms[0].FLG_" + keiro_no + ".value"); //すでに登録されているFLGを取得1:電車通学 2:その他通学
...
function reSelect(main_flg) {
    ...
    } else if (main_flg == "2") { //メインフレームのhiddenにある通学方法がその他通学だったらメインフレームのテキストボックスの中身をそのまま表示する
...
function insertDate() {
    ...
    if (document.forms[0].ROSEN_TEXT.value == "" &&
        ...
        if (tugaku_flg == '2') { //通学手段がその他通学だったら文字数チェック(15文字まで)
...
function knjAjax(cmd,targetId) { //この区間は送信処理、
    ...
    httpObj.open("GET","knjh010aindex.php?" + sendData,true);  //POSTメソッドで送るとリクエストヘッダがおかしくなるので
