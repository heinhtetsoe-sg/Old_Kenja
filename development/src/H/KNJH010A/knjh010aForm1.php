...
class knjh010aForm1
{
    public function main(&$model)
    {
        ...
        //現在通っている塾(コンボ)
        $query = knjh010aQuery::getVNameMst("H101", $model);
        $extra = '';
        ...
        //学習時間(コンボ)
        $query = knjh010aQuery::getVNameMst("H103", $model);
        $extra = '';
        makeCmb($objForm, $arg, $db, $query, "STUDYTIME", $row["STUDYTIME"], $extra, 1, "0");

        //けいこごと(コンボ)
        $query = knjh010aQuery::getVNameMst("H102", $model);
        $extra = '';
        ...
        //兄弟姉妹調査(コンボ)
        $query = knjh010aQuery::getVNameMst("H107", $model);
        $extra = '';
        makeCmb($objForm, $arg, $db, $query, "BRO_SISCD", $row["BRO_SISCD"], $extra, 1, "0");

        //住居調査(コンボ)
        $query = knjh010aQuery::getVNameMst("H108", $model);
        $extra = '';
        ...
        //通学方法
        $query = knjh010aQuery::getVNameMst("H100", $model);
        $extra = '';
        ...
        for ($i = 1; $i <= 7; $i++) {
            ...
            if ($row[$flg] == '1') {
                ...
                $query = knjh010aQuery::getStationName($josya_cd);
                list($josya_mei, $rosen_mei) = $db->getRow($query);
                $query = knjh010aQuery::getStationName($gesya_cd);
                list($gesya_mei, $rosen_mei) = $db->getRow($query);
            } elseif ($row[$flg] == '3') {
                $rosen_cd = $row[$hidden_rosen] ? $row[$hidden_rosen] : $row[$rosen];

                $query = knjh010aQuery::getBusCourse($rosen_cd);
        ...
        /**************/
        //性質・学業・他
        $link = REQUESTROOT."/H/KNJH010A/knjh010aindex.php?cmd=subform1&SCHREGNOSUB=".$model->schregno."&PRG=".VARS::get("PRG");
        ...
        //通学方法（スクールバス）
        $school_bus = $db->getCol(knjh010aQuery::getSchoolBus($model));
        ...
        $arg["start"] = $objForm->get_start("edit", "POST", "knjh010aindex.php", "", "edit");
        $arg["finish"] = $objForm->get_finish();
        //インラインフレーム用Javascriptタグ生成
        $arg["IFRAME"] = View::setIframeJs();
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す
        View::toHTML($model, "knjh010aForm1.html", $arg);
    }
}
/********************************************** 以下関数 **********************************************************/
//コンボ作成
