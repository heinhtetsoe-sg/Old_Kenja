...
class knjh010aSubForm2
{
    public function main(&$model)
    {
        ...
        //通学手段
        $opt_shubetsu = array(1, 2); //1:電車通学 2:その他通学
        $model->field["TUGAKU"] = ($model->field["TUGAKU"]) ? $model->field["TUGAKU"] : "1";
        $extra = "onclick=\"tugakuChange();\"";
        $radioArray = knjCreateRadio($objForm, "TUGAKU", $model->field["TUGAKU"], $extra, $opt_shubetsu, count($opt_shubetsu));
        ...
        if ($model->cmd == "get_rosen" || $model->cmd == "get_rosen_from_keiro") {
            //路線名--コンボ
            $query = knjh010aQuery::getRosen($model->field["AREA_SENTAKU"]);
        ...
        if ($model->cmd == "get_station" || $model->cmd == "get_station_from_keiro") {
            if ($model->field["ROSEN_SELECT"]) {
                //乗車駅--コンボ
                $query = knjh010aQuery::getStation($model->field["ROSEN_SELECT"]);
        ...
        View::toHTML($model, "knjh010aSubForm2.html", $arg);
    }
}
/********************************************** 以下関数 **********************************************************/
//コンボ作成
