...
class knjh400_KiousyouUndouForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjh400_KiousyouUndouindex.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //データを取得
        $original = $db->getRow(knjh400_KiousyouUndouQuery::selectQuery($model), DB_FETCHMODE_ASSOC);
        ...
        if (isset($model->schregno) && !isset($model->warning)) {
            unset($model->field);
            $row = $db->getRow(knjh400_KiousyouUndouQuery::selectQuery($model), DB_FETCHMODE_ASSOC);
        } else {
            $row =& $model->field;
        }

        //既往症コンボ
        $query = knjh400_KiousyouUndouQuery::getNameMst("F143");
        ...
        //（運動）指導区分コンボ
        $query = knjh400_KiousyouUndouQuery::getNameMst("F141", "ABBV1");
        makeCmb($objForm, $arg, $db, $query, "GUIDE_DIV", $row["GUIDE_DIV"], "", 1);

        //（運動）部活動コンボ
        $query = knjh400_KiousyouUndouQuery::getNameMst("F142");
        ...
        View::toHTML($model, "knjh400_KiousyouUndouForm1.html", $arg);
    }
}
