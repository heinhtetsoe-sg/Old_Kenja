...
class knjh400_hokensituSubForm3
{
    function main(&$model)
    {
        //オブジェクト作成
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform3", "POST", "knjh400_hokensituindex.php", "", "subform3");
        ...
        $query = knjh400_hokensituQuery::getNameMst('F203');
        //来室理由テキスト入力可コード格納
        $f203Text = array();
        $result = $db->query($query);
        while ($row3 = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            if ($row3["NAMESPARE2"] == "1") {
                $f203Text[] = $row3["VALUE"];
            }
        }
        $result->free();
        knjCreateHidden($objForm, "f203Text", implode(',', $f203Text));

        //来室理由１コンボ作成
        $extra = "onclick=\"OptionUse2(this, 'f203Text');\"";
        makeCmb($objForm, $arg, $db, $query, "VISIT_REASON1", $row["VISIT_REASON1"], $extra, 1);
        //来室理由１テキスト
        $extra = (in_array($row["VISIT_REASON1"], $f203Text)) ? "" : "disabled";
        ...
        //来室理由２テキスト
        $extra = (in_array($row["VISIT_REASON2"], $f203Text)) ? "" : "disabled";
        ...
        //来室理由３テキスト
        $extra = (in_array($row["VISIT_REASON3"], $f203Text)) ? "" : "disabled";
        ...
        //処置１テキスト
        $extra = (in_array($row["TREATMENT1"], $treatmentText, true)) ? "" : "disabled";
        ...
        View::toHTML($model, "knjh400_hokensituSubForm3.html", $arg);
    }
}
...
{
    //登録ボタン
    $update = ($model->cmd == "subform3") ? "insert" : "update";
    $arg["button"]["btn_update"] = knjCreateBtn($objForm, "btn_update", "登 録", "onclick=\"return btn_submit('".$update."');\"");
    if ($model->cmd != "subform3") {
        //取消ボタン
        $arg["button"]["btn_reset"] = knjCreateBtn($objForm, "btn_reset", "取 消", "onclick=\"return btn_submit('subform3_clear');\"");
    }
    //印刷ボタンを作成する
    ...
    if ($model->Properties["useKnjf150_2Button"] == 1) {
        $extra = "onclick=\"loadwindow('../KNJF150_2/knjh400_hokensitu_2index.php?CALL_INFO=KNJF150_3&SCHREGNO={$model->schregno}',0,0,800,600);return;\"";
