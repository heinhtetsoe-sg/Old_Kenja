...
class knjh400_hokensituSubForm5
{
    function main(&$model)
    {
        //オブジェクト作成
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform5", "POST", "knjh400_hokensituindex.php", "", "subform5");
        ...
        //来室理由テキスト入力可コード格納
        $f219Text = array();
        $query = knjh400_hokensituQuery::getNameMst('F219');
        ...
        $result->free();
        knjCreateHidden($objForm, "f219Text", implode(',', $f219Text));

        //来室理由１コンボ作成
        $query = knjh400_hokensituQuery::getNameMst('F219');
        $extra = "onclick=\"OptionUse2(this, 'f219Text');\"";
        makeCmb($objForm, $arg, $db, $query, "VISIT_REASON1", $row["VISIT_REASON1"], $extra, 1);
        //来室理由１テキスト
        $extra = (in_array($row["VISIT_REASON1"], $f219Text)) ? "" : "disabled";
        ...
        //来室理由２テキスト
        $extra = (in_array($row["VISIT_REASON2"], $f219Text)) ? "" : "disabled";
        ...
        //来室理由３テキスト
        $extra = (in_array($row["VISIT_REASON3"], $f219Text)) ? "" : "disabled";
        ...
        //処置テキスト入力可コード格納
        $f220Text = array();
        $query = knjh400_hokensituQuery::getNameMst('F220');
        $result = $db->query($query);
        while ($row1 = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            if ($row1["NAMESPARE2"] == "1") {
                $f220Text[] = $row1["VALUE"];
        ...
        $result->free();
        knjCreateHidden($objForm, "f220Text", implode(',', $f220Text));

        //処置１コンボ作成
        $query = knjh400_hokensituQuery::getNameMst('F220');
        $extra = "onclick=\"OptionUse2(this, 'f220Text');\"";
        makeCmb($objForm, $arg, $db, $query, "TREATMENT1", $row["TREATMENT1"], $extra, 1);
        //処置１テキスト作成
        $extra = (in_array($row["TREATMENT1"], $f220Text)) ? "" : "disabled";
        ...
        //処置２テキスト作成
        $extra = (in_array($row["TREATMENT2"], $f220Text)) ? "" : "disabled";
        ...
        //処置３テキスト作成
        $extra = (in_array($row["TREATMENT3"], $f220Text)) ? "" : "disabled";
        ...
        View::toHTML($model, "knjh400_hokensituSubForm5.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg, $model)
{
    ...
    if ($model->Properties["useKnjf150_2Button"] == 1) {
        $extra = "onclick=\"loadwindow('../KNJF150_2/knjh400_hokensitu_2index.php?CALL_INFO=KNJF150_5&SCHREGNO={$model->schregno}',0,0,800,600);return;\"";
