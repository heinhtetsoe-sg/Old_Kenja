...
class knjh400_hokensituSubForm1
{
    function main(&$model)
    {
        //オブジェクト作成
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform1", "POST", "knjh400_hokensituindex.php", "", "subform1");
        ...
        //来室理由テキスト入力可コード格納
        $f200Text = array();
        $query = knjh400_hokensituQuery::getNameMst('F200');
        ...
        //来室理由２テキスト
        $extra = (in_array($row["VISIT_REASON2"], $f200Text)) ? "" : "disabled";
        ...
        //来室理由３テキスト
        $extra = (in_array($row["VISIT_REASON3"], $f200Text)) ? "" : "disabled";
        ...
        //発生場所コンボ作成
        $query = knjh400_hokensituQuery::getNameMst('F206');
        if ($model->schoolName != "miyagiken") {
            $f206Text = array();
            $result = $db->query($query);
        ...
        //体温２テキストボックス
        $arg["data"]["TEMPERATURE2"] = knjCreateTextBox($objForm, $row["TEMPERATURE2"], "TEMPERATURE2", 4, 4, $extra_Decimal);
        ...
        //体温３テキストボックス
        $arg["data"]["TEMPERATURE3"] = knjCreateTextBox($objForm, $row["TEMPERATURE3"], "TEMPERATURE3", 4, 4, $extra_Decimal);
        ...
        //処置テキスト入力可コード格納
        $treatmentText = array();
        $query = knjh400_hokensituQuery::getNameMst('F208');
        $result = $db->query($query);
        while ($row1 = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            if ($row1["NAMESPARE2"] == "1") {
        ...
        //処置２テキスト
        $extra = (in_array($row["TREATMENT2"], $treatmentText)) ? "" : "disabled";
        ...
        //処置３テキスト
        $extra = (in_array($row["TREATMENT3"], $treatmentText)) ? "" : "disabled";
        ...
        //原因コンボ作成
        $query = knjh400_hokensituQuery::getNameMst('F204');
        if ($model->schoolName != "miyagiken") {
            $f204Text = array();
            $result = $db->query($query);
            while ($row4 = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                if ($row4["NAMESPARE2"] == "1") {
                    $f204Text[] = $row4["VALUE"];
                }
            }
            $result->free();
            knjCreateHidden($objForm, "f204Text", implode(',', $f204Text));
            $extra = "onclick=\"OptionUse2(this, 'f204Text');\"";
        } else {
            $extra = "";
        }
        makeCmb($objForm, $arg, $db, $query, "OCCUR_CAUSE", $row["OCCUR_CAUSE"], $extra, 1);
        //宮城県以外のみ表示
        if ($model->schoolName != "miyagiken") {
            //原因テキスト
            $extra = (in_array($row["OCCUR_CAUSE"], $f204Text, true)) ? "" : "disabled";
            $arg["data"]["OCCUR_CAUSE_TEXT"] = knjCreateTextBox($objForm, $row["OCCUR_CAUSE_TEXT"], "OCCUR_CAUSE_TEXT", 60, 90, $extra);
        }
        ...
        View::toHTML($model, "knjh400_hokensituSubForm1.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg, $model)
{
    ...
    if ($model->Properties["useKnjf150_2Button"] == 1) {
        $extra = "onclick=\"loadwindow('../KNJF150_2/knjh400_hokensitu_2index.php?CALL_INFO=KNJF150_1&SCHREGNO={$model->schregno}',0,0,800,600);return;\"";
        $arg["button"]["btn_knjh400_hokensitu_2"] = KnjCreateBtn($objForm, "btn_knjh400_hokensitu_2", "欠課仮登録", $extra);
    }
}
