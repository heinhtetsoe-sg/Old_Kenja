...
class knjh400_hanteiForm1
{
    function main(&$model)
    {
        ...
        $arg["start"]   = $objForm->get_start("main", "POST", "knjh400_hanteiindex.php", "", "main");

        //データベース接続
        $db = Query::dbCheckOut();

        $schregQuery = knjh400_hanteiQuery::getSchreg($model->year, $model->GAKUSEKI);
        ...
        for($gaku=$model->gakunen;$gaku>0;$gaku--){
            ...
            $schregQuery2 = knjh400_hanteiQuery::getSchreg($ctrlyear, $model->GAKUSEKI);
            $schregRow2 = $db->getRow($schregQuery2, DB_FETCHMODE_ASSOC);
            $grade = $schregRow2["GRADE"];
            $hrClass = $schregRow2["HR_CLASS"];
            $attendno = $schregRow2["ATTENDNO"];
            $cntQuery = knjh400_hanteiQuery::getMockCnt($model, $ctrlyear, $model->GAKUSEKI, $model->field["GYOUSYA"], "1", $grade, $hrClass, $attendno);  //カウント
            ...
            if($cnt > 0){
                ...
                //模試志望校数の最大値と模試回数取得
                $cntQuery = knjh400_hanteiQuery::getMockCnt($model, $ctrlyear, $model->GAKUSEKI, $model->field["GYOUSYA"], "2", $grade, $hrClass, $attendno);
                ...
                //模試データ(CDとか取得)
                $mosiQuery = knjh400_hanteiQuery::getMockCnt($model, $ctrlyear, $model->GAKUSEKI, $model->field["GYOUSYA"], "0", $grade, $hrClass, $attendno);
                ...
                while($mosiRow = $mosiResult->fetchRow(DB_FETCHMODE_ASSOC)){
                    ...
                    $hopeQuery = knjh400_hanteiQuery::getHopeGaku($model, $mosiRow["MOCKCD"], $model->GAKUSEKI, $ctrlyear, $grade, $hrClass, $attendno);
        ...
        View::toHTML($model, "knjh400_hanteiForm1.html", $arg); 
    }
}
//コンボ作成
