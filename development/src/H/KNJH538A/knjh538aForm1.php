...
class knjh538aForm1
{
    public function main(&$model)
    {
        ...
        $arg["start"]   = $objForm->get_start("main", "POST", "knjh538aindex.php", "", "main");
        ...
        //講座コンボ
        $query = knjh538aQuery::selectChairQuery($model);
        $extra = "onchange=\"btn_submit('main');\"";
        ...
        knjCreateHidden($objForm, "PRGID", "KNJH538A");
        ...
        knjCreateHidden($objForm, "SET_AUTH", $model->auth);
        ...
        /* テンプレート呼び出し */
        View::toHTML($model, "knjh538aForm1.html", $arg);
    }
}
...
function makeDataList(&$objForm, &$arg, $db, $model, &$objUp, $hrName, $testName, $subclassName, $headerData)
{
    ...
    $query = knjh538aQuery::getCtl($model);
    $ctrlCnt = $db->getOne($query);

    $query  = knjh538aQuery::selectMainData($model, $execute_date);
    $result = $db->query($query);
    ...
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //満点マスタ
        $query = knjh538aQuery::getPerfect($model, $row);
