...
class knjh340Form1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjh340index.php", "", "edit");
        ...
        //参照年度コンボボックス
        $query = knjh340Query::getYear("RYEAR");
        ...
        //対象年度
        $query = knjh340Query::getYear("OYEAR");
        ...
        if ($model->cmd == "changeOyear") {
            $arg["reload"] = "window.open('knjh340index.php?cmd=edit', 'right_frame')";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjh340Form1.html", $arg); 
    }
}
...
{
    $result = $db->query(knjh340Query::getList($model));

    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        array_walk($row, "htmlspecialchars_array");
        $row["CLICK_DATA"] = $row["COURSE_DIV"].":".$row["GRADE"].":".$row["MOCK_SUBCLASS_CD"];
