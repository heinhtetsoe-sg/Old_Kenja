...
class knjh366bForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjh366bForm1", "POST", "knjh366bindex.php", "", "knjh366bForm1");
        ...
        //学期コンボ
        $query = knjh366bQuery::getSemesterMst();
        $extra = "onchange=\"return btn_submit('knjh366b')\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //学年コンボ
        $query = knjh366bQuery::getGradeHrClass($model);
        $extra = "onchange=\"return btn_submit('knjh366b')\"";
        ...
        View::toHTML($model, "knjh366bForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    //クラス一覧リスト作成する
    $row1 = array();
    $query = knjh366bQuery::getStudent($model);
    $result = $db->query($query);
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJH366B");
