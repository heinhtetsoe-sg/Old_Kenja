function btn_submit(cmd) {
    if (cmd != "subEnd") {
        if ("" == document.forms[0].ACTIONDATE.value || 0 == document.forms[0].TITLE.value.replace(/^[ 　]+/, "").length) {
            alert("{rval MSG301}");
            return true;
        }
        if ("" == document.forms[0].ACTIONHOUR.value && "" != document.forms[0].ACTIONMINUTE.value) {
            alert("分のみの入力は、禁止です。\n行動時間を入力してください。");
            return true;
        }
        if ("" != document.forms[0].ACTIONHOUR.value && "" == document.forms[0].ACTIONMINUTE.value) {
