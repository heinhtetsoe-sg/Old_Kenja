...
class knjh561cForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjh561cForm1", "POST", "knjh561cindex.php", "", "knjh561cForm1");
        ...
        //学期コンボ作成
        $query = knjh561cQuery::getSemester();
        if ($model->field["SELECT_DIV"] == "1" || $model->field["SELECT_DIV"] == "2" ) {
            $extra = "onchange=\"return btn_submit('knjh561c'), AllClearList();\"";
        } else {
            $extra = "onchange=\"return btn_submit('knjh561c');\"";
        }
        ...
        $extra  = array("id=\"SELECT_DIV1\" onclick=\"return btn_submit('knjh561c')\"", "id=\"SELECT_DIV2\" onclick=\"return btn_submit('knjh561c')\"", "id=\"SELECT_DIV3\" onclick=\"return btn_submit('knjh561c')\"");
        ...
        //学年コンボ作成
        $query = knjh561cQuery::getGradeHrClass($model->field["SEMESTER"], $model);
        if ($model->field["SELECT_DIV"] == "1" || $model->field["SELECT_DIV"] == "2") {
            $extra = "onchange=\"return btn_submit('knjh561c'), AllClearList();\"";
        } else {
            $extra = "onchange=\"return btn_submit('knjh561c');\"";
        ...
        $printSubmit = "onclick=\"return btn_submit('knjd615i')\"";
        ...
        View::toHTML($model, "knjh561cForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    //対象一覧リストを作成する
    $query = ($model->field["SELECT_DIV"] == "1") ? knjh561cQuery::getHrClass($model) : knjh561cQuery::getCourse($model);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJH561C");
