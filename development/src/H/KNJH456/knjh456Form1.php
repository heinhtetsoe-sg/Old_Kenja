...
class knjh456Form1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjh456index.php", "", "edit");
        ...
        //参照年度コンボ
        /*$query = knjh456Query::getYear("COPYYEAR");
        $extra = "";
        ...
        //対象年度コンボ
        $query = knjh456Query::getYear("MOCKYEAR");
        ...
        if ($model->cmd == "changeMockyear") {
            $arg["reload"] = "window.open('knjh456index.php?cmd=edit', 'right_frame')";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。 
        View::toHTML($model, "knjh456Form1.html", $arg); 
    }
}
...
{
    $result = $db->query(knjh456Query::getList($model));
