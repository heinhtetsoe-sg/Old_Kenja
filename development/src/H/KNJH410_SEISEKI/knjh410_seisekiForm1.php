...
class knjh410_seisekiForm1
{
    function main(&$model)
    {
        ...
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjh410_seisekiindex.php", "", "edit");
        ...
        $gyousyaQuery = knjh410_seisekiQuery::getGyousya();
        /*$array = array("0" => "全表示",
                       "1" => "ベネッセ",
                       "2" => "駿台");
        $opt = array();
        ...
        $syubetuQuery = knjh410_seisekiQuery::getSyubetu();
        ...
        //チェックボックス
        //教科名取得
        $kyoukaQuery = knjh410_seisekiQuery::getKyouka($model->GAKUSEKI,$model->field["KATA"], $model->field["GYOUSYA"],$model->field["SYUBETU"]);
        ...
        //該当する模試を取得
        $mockQuery = knjh410_seisekiQuery::getMock($model->GAKUSEKI, $gyousya, $kyouka, $model->field["SYUBETU"], $model->field["KATA"], "1");
        $mockCnt = $db->getOne($mockQuery);
        if($mockCnt > 0){
                
            $kyoukaCnt = get_count($model->field["CHECK"]);
            
            $mockQuery = knjh410_seisekiQuery::getMock($model->GAKUSEKI, $gyousya, $kyouka, $model->field["SYUBETU"], $model->field["KATA"]);
            ...
            while($mockRow = $mockResult->fetchRow(DB_FETCHMODE_ASSOC)){
                ...
                //模試ごとの成績を取得
                $seisekiQuery = knjh410_seisekiQuery::getSeiseki($mockRow["MOCKCD"], $model->GAKUSEKI, $model->field["KATA"], $kyouka, $model->field["SYUBETU"]);
        ...
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML6($model, "knjh410_seisekiForm1.html", $arg, $jsplugin, $cssplugin);
    }
}
//コンボ作成
