...
class knjh400_medicalForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjh400_medicalindex.php", "", "main");
        ...
        $query = knjh400_medicalQuery::getSchregData($model->schregno);
        ...
        if($model->btnRadio != "2"){
            ...
            //一番上のth部分
            //対象生徒のデータがある年度を取得
            $query = knjh400_medicalQuery::getmedYear($model->schregno, "MEDEXAM_DET_DAT");
            ...
            //データ取得
            $query = knjh400_medicalQuery::getMedexamData($model->schregno);
            $result = $db->query($query);
                    ...
                    if(!is_array($secVal)){
                        ...
                        if(array_key_exists($secVal, $tdArray)){
                            ...
                            if(in_array($secVal, $anotherArray)){
                                ...
                                if($secVal == "聴力"){
                                    //コード取得
                                    $query = knjh400_medicalQuery::getCode("F010");
                                ...
                                //コード変換のみのもの
                                //コード取得
                                $query = knjh400_medicalQuery::getCode($getCode[$secVal]);
                                $cResult = $db->query($query);
                                while($cRow = $cResult->fetchRow(DB_FETCHMODE_ASSOC)){
                        ...
                        if($secKey != "結核"){
                            foreach($secVal as $thrKey => $thrVal){
                                ...
                                if(array_key_exists($thrVal, $tdArray)){
                                    ...
                                    //コード変換のみ  尿、運動
                                    //コード取得
                                    $query = knjh400_medicalQuery::getCode($getCode[$secKey]);
                                    $cResult = $db->query($query);
                                    while($cRow = $cResult->fetchRow(DB_FETCHMODE_ASSOC)){
                                        if($secKey == "尿"){
                                            $code[$cRow["NAMECD2"]] = $cRow["NAME1"];
                                    ...
                                    //コード変換<BR>備考　心臓
                                    //コード取得
                                    $query = knjh400_medicalQuery::getCode($getCode[$thrVal]);
                                    $cResult = $db->query($query);
                                    while($cRow = $cResult->fetchRow(DB_FETCHMODE_ASSOC)){
                                        $code[$cRow["NAMECD2"]] = $cRow["NAME1"];
                                    }
                            ...
                            foreach($secVal as $thrKey => $thrVal){
                                ...
                                //結核のコードを一度に取得
                                $query = knjh400_medicalQuery::getCode($tbCodeNamecd);
                                $cResult = $db->query($query);
                                while($cRow = $cResult->fetchRow(DB_FETCHMODE_ASSOC)){
                                    $code[$cRow["NAMECD1"]][$cRow["NAMECD2"]] = $cRow["NAME1"];
                                }
            ...
            //一番上th
            //対象生徒のデータがある年度を取得
            $query = knjh400_medicalQuery::getmedYear($model->schregno, "MEDEXAM_TOOTH_DAT");
            $result = $db->query($query);
            ...
            //データ取得
            $query = knjh400_medicalQuery::getToothData($model->schregno);
            $result = $db->query($query);
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
            ...
            foreach($thArray as $key => $val){
                if(!is_array($val)){
                    ...
                    if(array_key_exists($val, $tdArray)){
                        ...
                        //コード変換のみ
                        //コード取得
                        $query = knjh400_medicalQuery::getCode($getCode[$val]);
                        $cResult = $db->query($query);
                        while($cRow = $cResult->fetchRow(DB_FETCHMODE_ASSOC)){
                            $code[$cRow["NAMECD2"]] = $cRow["NAME1"];
                        }
        ...
        View::toHTML($model, "knjh400_medicalForm1.html", $arg);
    }
