...
class knjh436Form1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjh436index.php", "", "main");

        //DB接続
        $db = Query::dbCheckOut();

        //親画面なし
        $securityCnt = $db->getOne(knjh436Query::getSecurityHigh());
        //セキュリティーチェック
        ...
        //出力取込種別ラジオボタン(1:データ取込 2:エラー出力 3:ヘッダ出力)
        $opt = array(1, 2, 3);
        ...
        $query = knjh436Query::getMockRankRangeDat($model);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJH436");
        ...
        View::toHTML($model, "knjh436Form1.html", $arg);
    }
