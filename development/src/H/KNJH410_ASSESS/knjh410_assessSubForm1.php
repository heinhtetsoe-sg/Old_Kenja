...
class knjh410_assessSubForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform1", "POST", "knjh410_assessindex.php", "", "subform1");
        ...
        //年度切替
        $opt = array();
        $result = $db->query(knjh410_assessQuery::getAssessDate($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array( "label" => $row["LABEL"],
                          "value" => $row["VALUE"] );
        }
        if ($model->field["DATE"] == NULL) $model->field["DATE"] = $opt[0]["value"];
        $extra = "onChange=\"btn_submit('changeDate')\"; ";
        $arg["data"]["DATE"] = knjCreateCombo($objForm, "DATE", $model->field["DATE"], $opt, $extra, 1);
        $backYear = $model->year - 1;
        $nextYear = $model->year + 1;
        //その年度の最大のSEMESTERを取得したい
        $query = knjh410_assessQuery::getSemester($model->schregno, $backYear, $nextYear);
        ...
        foreach ($model->assess as $key => $val) {
            ...
            if (isset($model->schregno) && !isset($model->warning)) {
                ...
                //アセスメント解答データ取得
                $query = knjh410_assessQuery::getAssessmentAnsDat($model, explode("_", $model->field["DATE"])[1] , $key);
                $result = $db->query($query);
                while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                    //設問
                    for ($i=1; $i <= 15; $i++) {
            ...
            if ($val["check"] == "1") {
                if ($key == "01") {
                    ...
                    $question = array();
                    $query = knjh410_assessQuery::getAssessmentQMst($key);
                    $result = $db->query($query);
                    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        View::toHTML($model, "knjh410_assessSubForm1.html", $arg);
    }
}
