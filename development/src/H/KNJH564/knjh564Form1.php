...
class knjh564Form1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjh564Form1", "POST", "knjh564index.php", "", "knjh564Form1");
        ...
        //学期コンボ作成
        $query = knjh564Query::getSemester();
        $extra = "onchange=\"return btn_submit('knjh564'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //データ種別コンボ作成
        $query = knjh564Query::getDataDiv();
        $extra = "onchange=\"return btn_submit('knjh564')\"";
        makeCmb($objForm, $arg, $db, $query, "PROFICIENCYDIV", $model->field["PROFICIENCYDIV"], $extra, 1);

        //テスト名称コンボ作成
        $query = knjh564Query::getProName($model);
        $extra = "onchange=\"return btn_submit('knjh564')\"";
        ...
        $extra = array("id=\"CATEGORY_IS_CLASS1\" onClick=\"return btn_submit('knjh564')\"", "id=\"CATEGORY_IS_CLASS2\" onClick=\"return btn_submit('knjh564')\"");
        ...
        //学年コンボ作成
        $query = knjh564Query::getGradeHrClass($model->field["SEMESTER"], $model, "GRADE");
        $extra = "onchange=\"return btn_submit('change_grade'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        if ($model->field["CATEGORY_IS_CLASS"] == 2){
            //クラスコンボ作成
            $query = knjh564Query::getGradeHrClass($model->field["SEMESTER"], $model, "HR_CLASS");
            $extra = "onchange=\"return btn_submit('knjh564'), AllClearList();\"";
        ...
        $z010 = $db->getOne(knjh564Query::getZ010());
        if ($z010 != 'tokiwa') {
            $arg["notprintjunidiv"] = "1";
            //順位を印刷しない
            $extra = $model->field["NOT_PRINT_JUNI"] == "1" ? "checked" : "";
            $extra .= " id=\"NOT_PRINT_JUNI\"";
            $arg["data"]["NOT_PRINT_JUNI"] = knjCreateCheckBox($objForm, "NOT_PRINT_JUNI", "1", $extra, "");
        }

        //学期名を印刷しない
        $extra = $model->field["NOT_PRINT_SEMESTERNAME"] == "1" ? "checked" : "";
        $extra .= " id=\"NOT_PRINT_SEMESTERNAME\"";
        $arg["data"]["NOT_PRINT_SEMESTERNAME"] = knjCreateCheckBox($objForm, "NOT_PRINT_SEMESTERNAME", "1", $extra, "");
        ...
        View::toHTML($model, "knjh564Form1.html", $arg); 
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["CATEGORY_IS_CLASS"] == 1){
        ...
        //対象外の生徒取得
        $query = knjh564Query::getSchnoIdou($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt_idou = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //対象者リストを作成する
        $query = knjh564Query::getStudent($model, $model->field["SEMESTER"]);
        $result = $db->query($query);
        $opt1 = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $idou = "　";
...
function makeCmb(&$objForm, &$arg, $db, $query, $name, &$value, $extra, $size)
{
    ...
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ("1" == $row["DEFAULT"]) $default_val = $row["VALUE"];
    }
    $result->free();
    ...
    if ($name == "SEMESTER") {
        ...
        $value = ($value && $value_flg) ? $value : ($default_val ? $default_val : $opt[0]["value"]);
    }
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJH564");
