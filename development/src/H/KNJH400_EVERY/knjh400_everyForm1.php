...
class knjh400_everyForm1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjh400_everyindex.php", "", "edit");

        $db = Query::dbCheckOut();
        
        //選択した生徒の年組番名前表示
        $query = knjh400_everyQuery::getSchregData($model->schregno);
        ...
        //入力者
        $extra = "";
        $query = knjh400_everyQuery::getUpdateStaff($model->schregno);
        ...
        //タグの頻出データを表示
        $query = knjh400_everyQuery::getTagCnt($moel->schregno);
        $result = $db->query($query);
        $sp = "";
        $linkData = "";
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        if($model->search["TAGSEARCH"] != ""){
            //タグテーブル優先にして検索
            $query = knjh400_everyQuery::getSearchTag($model->schregno, $model->search);
        }else{
            //内容テーブル優先に検索
            $query = knjh400_everyQuery::getSearch($model->schregno, $model->search);
        }
        $result = $db->query($query);
        ...
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
            if($recNo == "" || $recNo != $row["RECNO"]){
                ...
                $data["TITLELINK"] = "<a href=\"knjh400_everyindex.php?cmd=choice&RECNO=".$row["RECNO"]."\" target=\"right_frame\" style=\"display: block;\">".$title."</a>";
        ...
        View::toHTML($model, "knjh400_everyForm1.html", $arg);
    }
}
