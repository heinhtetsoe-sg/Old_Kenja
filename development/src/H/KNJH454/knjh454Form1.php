...
class knjh454Form1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjh454index.php", "", "edit");
...
if($arg["HYOUZI"] == 1){        
        //チェックボックス
        //教科名取得
        $kyoukaQuery = knjh454Query::getKyouka($model->req_field["MOCKCD"], $model->req_field["KATA"]);
        ...
        //表の1行目のデータ作成
        //学校名取得
        $gakunameQuery = knjh454Query::getGakName($model->req_field["MOCKCD"]);
        ...
        foreach($model->bottom_field["CHECK"] as $chkkey => $chkval){
            ...
            //教科のコードで対象の模試のデータ取得
            $dataQuery = knjh454Query::getData($model->req_field["MOCKCD"], $model->req_field["ANALYSIS"], $chkval);
        ...
        //$arg["link"] = REQUESTROOT."/Z/KNJh454_2/knjh454_2index.php";
        Query::dbCheckIn($db);
        //ボタン作成
        ...
        //View::toHTML($model, "knjh454Form1.html", $arg);
        
        
        $jsplugin = "chart.js|Chart.min.js|graph.js";

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML6($model, "knjh454Form1.html", $arg, $jsplugin, $cssplugin);
    }
} 
//ボタン作成
