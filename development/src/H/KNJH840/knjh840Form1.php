...
class knjh840Form1
{
    function main(&$model) {
        $objForm      = new form;
        $arg["start"] = $objForm->get_start("edit", "POST", "knjh840index.php", "", "edit");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //教科
        $extra = " onchange=\"btn_submit('change');\"";
        $query = knjh840Query::getKyouka($model->topfield["EXP_YEAR"], $model->field["SITE"]);
        ...
        if($model->field["KYOKA"] != ""){
            //Queryで中身取得
            $query = knjh840Query::getKamoku($model->topfield["EXP_YEAR"], $model->field["SITE"], $model->field["KYOKA"]);
        ...
        if($model->mode != 0){
            //件数チェック
            //件数0ならmode=0にする
            $query = knjh840Query::getTestClass($model->topfield, $model->field, "1");
        ...
        if($model->mode != 0){
            ...
            //取込日
            $query = knjh840Query::getImportDate();
            ...
            $query = knjh840Query::getTestClass($model->topfield, $model->field);
            ...
            //学年平均・学年最高点取得
            $query = knjh840Query::getTestGrade($model->topfield, $model->field, $sql);
            ...
            if($model->field["KUBUN"] == "0"){
                $arg["header_x"] = "<th class=\"no_search title\">実施回</th>";
                foreach($partNo as $partVal){
                    $arg["header_x"] .= "</tr><tr>";
                ...
                //テスト関係なく人毎に計算してデータ取得
                //クラス平均とクラス最高点
                $query = knjh840Query::getAllClass($model->topfield, $model->field, $sql);
                $aRow = $db->getRow($query, DB_FETCHMODE_ASSOC);
                $allArray = array();
                $allArray = $aRow;
                
                //学年平均と学年最高点
                $query = knjh840Query::getAllGrade($model->topfield, $model->field, $sql);
            ...
            if($model->field["KUBUN"] == "0"){
                ...
                $query = knjh840Query::getPartClass($model->topfield, $model->field, $sql);
                ...
                $query = knjh840Query::getPartGrade($model->topfield, $model->field, $sql);
                ...
                $query = knjh840Query::getPartCnt($model->topfield, $model->field, $sql);
                ...
                //小分野の名称とFIELDNOを取得
                $query = knjh840Query::getQuestNo($model->topfield, $model->field, $sql);
                ...
                $query = knjh840Query::getQuestClass($model->topfield, $model->field, $sql);
                ...
                //小分野ごとの学年平均・学年最高点を取得
                $query = knjh840Query::getQuestGrade($model->topfield, $model->field, $sql);
            ...
            if($model->field["KUBUN"] == "0"){
                //得点率データ取得
                $query = knjh840Query::getGetPercent($model->topfield, $model->field, $sql);
                $pResult = $db->query($query);
                ...
                $query = knjh840Query::getPartClassPoint($model->topfield, $model->field, $sql);
                $pcResult = $db->query($query);
                $ppArray = array();
                $Avg = array();
                ...
                while($pcRow = $pcResult->fetchRow(DB_FETCHMODE_ASSOC)){
                    ...
                    if($Avg[$pcRow["FIELDNO"]]["AVG"] != ""){
                        $Avg[$pcRow["FIELDNO"]]["AVG"] = $Avg[$pcRow["FIELDNO"]]["AVG"]+$pcRow["AVG"];
                    }else{
                        $Avg[$pcRow["FIELDNO"]]["AVG"] = $pcRow["AVG"];
                    }
                }
                //大分野ごと、実施回数ごとに学年平均と学年最高点取得
                $query = knjh840Query::getPartGradePoint($model->topfield, $model->field, $sql);
                $pgResult = $db->query($query);
                while($pgRow = $pgResult->fetchRow(DB_FETCHMODE_ASSOC)){
                    $ppArray[$pgRow["FIELDNO"]][$pgRow["TEACHING_CD"]] += $pgRow;
                    if($Avg[$pgRow["FIELDNO"]]["GRADE_AVG"] != ""){
                        $Avg[$pgRow["FIELDNO"]]["GRADE_AVG"] = $Avg[$pgRow["FIELDNO"]]["GRADE_AVG"]+$pgRow["GRADE_AVG"];
                    }else{
                        $Avg[$pgRow["FIELDNO"]]["GRADE_AVG"] = $pgRow["GRADE_AVG"];
                    }
                }
                ...
                $query = knjh840Query::getQuestClassPoint($model->topfield, $model->field, $sql);
                $qcResult = $db->query($query);
                $qpArray = array();
                ...
                $query = knjh840Query::getQuestGradePoint($model->topfield, $model->field, $sql);
                $qgResult = $db->query($query);
                while($qgRow = $qgResult->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        View::toHTML($model, "knjh840Form1.html", $arg);
    }
}
