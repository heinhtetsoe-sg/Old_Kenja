...
class knjo122Form1
{
    ...
    {
        $objForm = new form;
        $arg["start"] = $objForm->get_start("main", "POST", "knjo122index.php", "", "main");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        if($arg["SCHOOL"] != ""){
            ...
            //郵便番号
            if ($model->Properties["search_zipcd_set_town_to_addr2"] == "1") {
                $school["G_ZIPCD2"] = str_replace("input ", "input {$guard_disabled} ", View::popUpZipCode($objForm, "G_ZIPCD2", $row["G_ZIPCD2"], "G_BUNADDRESS", 10, "ADD2"));
            } else {
                $school["G_ZIPCD2"] = str_replace("input ", "input {$guard_disabled} ", View::popUpZipCode($objForm, "G_ZIPCD2", $row["G_ZIPCD2"], "G_BUNADDRESS"));
            }
            //住所
        ...
        View::toHTML($model, "knjo122Form1.html", $arg);
    }
}
...
function makeButton(&$objForm, &$arg, $model, $db)
{
    ...
    //在学生検索ボタンを作成する
    $extra = "onclick=\"wopen('../../X/KNJXSEARCH2/index.php?PATH=/O/KNJO122/knjo122index.php&cmd=&target=KNJO122','search',0,0,700,500);\"";
