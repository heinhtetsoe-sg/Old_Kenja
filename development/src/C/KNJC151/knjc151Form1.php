...
class knjc151Form1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjc151Form1", "POST", "knjc151index.php", "", "knjc151Form1");
        ...
        //クラス一覧リスト作成する
        $query = knjc151Query::getAuth($model);
        ...
        //クラス一覧リスト作成する
        $months = array();
        $query = knjc151Query::getNameMst("C044", "INT(VALUE(NAMESPARE1, '99'))");
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        $arg["data"]["RATE"] = knjCreateTextBox($objForm, isset($model->field["RATE"]) ? $model->field["RATE"] : 25, "RATE", 3, 3, $extra);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJC151");
        ...
        View::toHTML($model, "knjc151Form1.html", $arg); 
    }
}
?>
