...
class knjx_c031kForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx_c031kindex.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx_c031kQuery::getSecurityHigh($model));
        //セキュリティーチェック
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX_C031K");
        ...
        View::toHTML($model, "knjx_c031kForm1.html", $arg);
    }
}
...
{
    $query = knjx_c031kQuery::selectSemesAll($model);
    $result = $db->query($query);
    $data = array();
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $data[] = $row;
    ...
    for ($dcnt = 0; $dcnt < get_count($data); $dcnt++) {
        for ($i = $data[$dcnt]["S_MONTH"]; $i <= $data[$dcnt]["E_MONTH"]; $i++) {
            ...
            $getdata = $db->getRow(knjx_c031kQuery::selectMonthQuery($year, $month, $model), DB_FETCHMODE_ASSOC);
            if (is_array($getdata)) {
...
function makeBtn(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->Properties["useXLS"]) {
        $model->schoolCd = $db->getOne(knjx_c031kQuery::getSchoolCd());
