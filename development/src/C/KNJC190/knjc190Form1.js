...
function newwin(SERVLET_URL){
    ...
    {
        alert("印刷範囲開始日付が未入力です。");
        return;
    }
    if (document.forms[0].DATE2.value == '')
    {
        alert("印刷範囲終了日付が未入力です。");
        return;
    }
    var objSdate = document.forms[0].DATE1.value.replace("/","");
    var objEdate = document.forms[0].DATE2.value.replace("/","");
    var Sdate = document.forms[0].GAKKISDATE.value.replace("/","");
    var Edate = document.forms[0].GAKKIEDATE.value.replace("/","");
    if (objSdate > objEdate){
        alert('開始日付と終了日付の\n大小が逆です。');
        return false;
    }
    if (objSdate < Sdate || Edate < objSdate || Edate < objEdate || objEdate < Sdate){
        alert('日付が学期範囲外です。\n'+document.forms[0].GAKKISDATE.value+'～'+document.forms[0].GAKKIEDATE.value);
        return false;
    }
