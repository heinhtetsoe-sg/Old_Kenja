...
function checkedDate() {
    ...
    var val          = document.forms[0].SEME_SE.value;
    var se_date      = val.split(',');
    var check_s_date = se_date[0];              //学期開始日付
    var check_e_date = se_date[1];              //学期終了日付
    var input_s_date = document.forms[0].DATE1.value; //印刷範囲開始日付
    var input_e_date = document.forms[0].DATE2.value; //印刷範囲終了日付
    var irekae = '';
    if(input_s_date > input_e_date) {
        irekae       = input_e_date;
        input_e_date = input_s_date;
        input_s_date = irekae;

        document.forms[0].DATE1.value = input_s_date;
        document.forms[0].DATE2.value = input_e_date;
    }

    //範囲チェック
    if (input_s_date < check_s_date || input_e_date > check_e_date) {
        alert("指定範囲が学期をまたがっています。\n指定できる範囲は下記の通りです。\n" + check_s_date + "～" + check_e_date);
        return false;
    }
    return true;
}
