...
function make_key(src) {
    ...
    document.forms[0].stock_chaircd.value =
        document.forms[0]["HIDDEN_" + src.id].value;
...
function IsUserOK_ToJump(URL, syoribi, period, clsstfcd, staffcd) {
    //2005/05/06 賢者-作業依頼書20050506_01
...
function IsUserOK_ToJump_1(URL, syoribi, period, class_staffcd, ccd) {
    var sendAuth = document.forms[0].SEND_AUTH.value;
    wopen(
        URL +
            "?syoribi=" +
...
function Style_Color(set_id) {
    ...
    document.getElementById(set_id).style.color = isUpperIe9()
        ? color_changer(rgbStrToHex(color))
        : color_changer(color);
}
...
function checkHidden(msgMode) {
    ...
    if (
        document.forms[0].stock_chaircd.value == "" ||
        document.forms[0].cntl_dt_key.value == ""
    )
        returnValue = false;
    if (returnValue) {
        return true;
