...
class knjc156Form1 {
    function main(&$model) {
        $objForm = new form;
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjc156Form1", "POST", "knjc156index.php", "", "knjc156Form1");
        ...
        //学期コンボボックスを作成する
        $query = knjc156Query::getSemester();
        $semester_name = $db->getOne($query);
        $arg["data"]["SEMESTER"] = $semester_name;

        //年組コンボ
        $query = knjc156Query::getSelectHrclass($model);
        makeCmb($objForm, $arg, $db, $query, "GRADE_HRCLASS", $model->field["GRADE_HRCLASS"], "onChange=\"return btn_submit('knjc156');\"", 1);
        ...
        knjCreateHidden($objForm, "PRGID",         "KNJC156");
        ...
        View::toHTML($model, "knjc156Form1.html", $arg); 
    }
}
/*********************************************** 以下関数 *******************************************************/
/**********/
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    //クラス一覧
    $row1 = array();
    $result = $db->query(knjc156Query::getAuth($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
