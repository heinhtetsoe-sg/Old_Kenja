...
class knjc154SubForm1 {
    function main(&$model) {
        $objForm = new form;
        $db = Query::dbCheckOut();
        $arg = array();
        $arg["start"]   = $objForm->get_start("subform1", "POST", "knjc154index.php", "", "subform1");
        ...
        $opt = array();
        $value_flg = false;
        $query = knjc154Query::getSemester();
        ...
        if ($model->cmd !== 'replace') {
            ...
            $query = knjc154Query::getCommunication($model->field, $model->schregno);
        ...
        //戻るボタン
        $link = REQUESTROOT."/M/KNJC154/knjc154index.php?cmd=back";
        $objForm->ae( array("type"        => "button",
                            "name"        => "btn_back",
                            "value"       => "戻 る",
        ...
        View::toHTML($model, "knjc154SubForm1.html", $arg);
    }
}
/********************************************** 以下関数 ***********************************************/
//クラス一覧リストToリスト作成
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    $query = knjc154Query::getStudent($model);
    $result = $db->query($query);
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
