...
class knjc030kForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjc030kindex.php", "", "edit");

        //年度内の処理のみを行う。
        if(!$model->checkCtrlDay($model->cntl_dt_key)){
            $model->cntl_dt_key=$model->cntl_dt_year."-04-01";
            $reset_day = knjc030kQuery::keyMoverQuery($model);
        ...
        //SQL文発行(選択日付の学期を取得)
        $query = knjc030kQuery::getTerm($model->cntl_dt_year,$model->cntl_dt_key);     //2005/04/20 TERM_GET_NEW
        $model->termIs = $db->getOne($query);

        //見出し行作成
        $query = knjc030kQuery::getIndexName();
        $result = $db->query($query);
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            $row["width_num"] = ($row["NAMECD2"] == 9) ? "*" : "95";
            $arg["title_index"][] = $row;
        }

        //表示項目取得
        $query = knjc030kQuery::getDispCol();
        $result = $db->query($query);
        ...
        //学年選択コンボ
        $query = knjc030kQuery::gradeCombo();
        $result = $db->query($query);
        ...
        //クラス情報を保持
        $query = knjc030kQuery::get_class_data($model);
        ...
        //SQL文発行(全体の出欠情報を保持)
        $query  = knjc030kQuery::readQuery($model);
        $result = $db->query($query);
...
#          $chairname_box[$row["TARGETCLASS"]][$row["PERIODCD"]] .= $row["SUBCLASSABBV"]."<BR>".$row["STAFFNAME"]."<BR>";
        ...
        */
        ...
        for ($i = 0; $i < $a; $i++){
            ...
            $list_tag["TARGETCLASS"]  = createCheckBox($objForm, "TARGET_CHECK", $class[$i], "", "1");
        ...
        $attendexsits = $db->getOne(knjc030kQuery::getUpdateSelectData($model));
        $ex = (get_count($attendexsits)>0)?1:0;

        //制限処理月のチェック
        $monthch = $db->getOne(knjc030kQuery::ch_Control_Month($model));
        ...
        View::toHTML($model, "knjc030kForm1.html", $arg);
