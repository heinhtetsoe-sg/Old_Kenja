...
function makeMeisai(&$objForm, &$arg, $db, &$model, &$objUp, $cmCnt) {
    ...
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if (!strlen($overRow["COMP_ABSENCE_HIGH"])) $overRow["COMP_ABSENCE_HIGH"] = 9999;
        //累積情報
        ...
        foreach ($model->testcdArray as $key => $codeArray) {
            ...
            if ($codeArray["SIDOU_INPUT"] == "1") {
                ...
                if ($codeArray["SIDOU_INPUT_INF"] == "1" && strlen($slumpRow["MARK"])) {
                    ...
                    //追指導記号が○
                    } else if ($slumpRow["MARK"] == "1") {
                        //算出用(評定２の下限値を元に算出)
                        knjCreateHidden($objForm, $col."_ASSESSLOW2"."-".$counter, $slumpRow["ASSESSLOW2"]);
                    }
                } else if ($codeArray["SIDOU_INPUT_INF"] == "2" && strlen($slumpRow["SCORE"])) {
                    $colorFlgPink = true;
                    //追指導点を換算後の評定が１
                    ...
                    //追指導点を換算後の評定が２以上
                    } else if ("2" <= $slumpRow["ASSESSLEVEL"]) {
                        //算出用(評定２の下限値を元に算出)
                        knjCreateHidden($objForm, $col."_ASSESSLOW2"."-".$counter, $slumpRow["ASSESSLOW2"]);
                    }
                }
            ...
            } else if ($controlFlg == "1" && (!$tentaigakuFlg || AUTHORITY == DEF_UPDATABLE) && $tokiwagiFlg != "1") {
                //テキストボックスを作成
            ...
            if ($model->usePerfect == 'true' && substr($testcd, 5, 2) == '01') {
                ...
                if (strlen($perfectPS) && strlen($score) && $perfectPS > $score) $row["BGCOLOR"] = "violet";
            }
