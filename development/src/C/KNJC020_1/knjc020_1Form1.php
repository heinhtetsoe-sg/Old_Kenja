...
class knjc020_1Form1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjc020_1index.php", "", "edit");
        ...
        //学期取得
        $query = knjc020_1Query::getTerm($model->executedate);
        $model->semester = $db->getOne($query);

        //講座情報取得
        $query = knjc020_1Query::getTargetChair($model);
        ...
        //メインデータ取得
        $query = knjc020_1Query::getTimeTable($model);
        $result = $db->query($query);
        ...
        //テスト時間割チェック
        $sch_test = $db->getOne(knjc020_1Query::checkSchTest($model));

        //テスト時間割の集計フラグ
        $test_countflg = $db->getOne(knjc020_1Query::getTestCountflg($model));
        ...
        View::toHTML($model, "knjc020_1Form1.html", $arg);
