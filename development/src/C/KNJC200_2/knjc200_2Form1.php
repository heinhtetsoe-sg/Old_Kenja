...
class knjc200_2Form1
{
    function main(&$model){
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("knjc200_2Form1", "POST", "knjc200_2index.php", "", "knjc200_2Form1");

        //DB接続
        $db = Query::dbCheckOut();

        //クラス／担任
        $row = $db->getRow(knjc200_2Query::getHrName($model), DB_FETCHMODE_ASSOC);
        $arg["data"]["HR_NAME"] = $row["HR_NAME"] ." ／ " .$row["STAFFNAME"];

        //生徒
        $opt_schno = array();
        $opt_schno[] = array('label' => "　", 'value' => "");//初期値
        $result = $db->query(knjc200_2Query::getSchno($model));
        ...
        $show_seqno = array();
        $result = $db->query(knjc200_2Query::getList($model));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        if ($model->field["SEQNO"] != "" && !isset($model->warning)) {
            $Row = $db->getRow(knjc200_2Query::getListRow($model), DB_FETCHMODE_ASSOC);
        ...
        $result = $db->query(knjc200_2Query::getPeriodcd());
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        $result = $db->query(knjc200_2Query::getDicd());
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        View::toHTML($model, "knjc200_2Form1.html", $arg); 
    }
}
...
function makeDiremarkList(&$objForm, &$arg, $db, $model) {
    ...
    if($model->field["SEQNO"] && $model->field["SCHREGNO"]){
        if(strlen(implode($model->selectdata,','))){
            foreach ($model->selectdata as $key => $val) {
                $query = knjc200_2Query::getDiRemarkcd2($model, "C900", $val);
            ...
            $query = knjc200_2Query::getDiRemarkcd1($model, "C900");
    ...
    $query = knjc200_2Query::getDiRemarkList($model, "C900", $opt_left_id);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
