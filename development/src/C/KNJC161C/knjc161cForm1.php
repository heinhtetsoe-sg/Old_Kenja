...
class knjc161cForm1
{
    public function main(&$model)
    {
        ...
        $objForm = new form();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjc161cForm1", "POST", "knjc161cindex.php", "", "knjc161cForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //学年コンボ
        $query = knjc161cQuery::getGrade($model);
        $extra = "onChange=\"btn_submit('knjc161c')\"";
        ...
        View::toHTML($model, "knjc161cForm1.html", $arg);
    }
}
//makeCmb
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    //クラス一覧
    $row1 = array();
    $result = $db->query(knjc161cQuery::getAuth($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJC161C");
