...
class knjc032aForm1
{
    function main(&$model)
    {
        /* フォーム作成 */
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("main", "POST", "knjc032aindex.php", "", "main");
        ...
        /* 累積 */
        $query = knjc032aQuery::selectAttendSemester($model,0);
        ...
        /* テンプレート呼び出し */
        View::toHTML($model, "knjc032aForm1.html", $arg);
    }
}
...
function makeDataList(&$objForm, &$arg, $db, $model) {
    $query      = knjc032aQuery::selectSemesAll();
    $result     = $db->query($query);
    $data       = array();
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $data[] = $row;
    ...
    //学校マスタ
    $schoolMst = array();
    $query = knjc032aQuery::getSchoolMst();
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
    ...
    for ($dcnt = 0; $dcnt < get_count($data); $dcnt++) {
        for ($i = $data[$dcnt]["S_MONTH"]; $i <= $data[$dcnt]["E_MONTH"]; $i++) {
            ...
            $query  = knjc032aQuery::selectAttendQuery($model, $month, $data[$dcnt]["SEMESTER"], $schoolMst);
            $result = $db->query($query);
            $rowMeisai = array();
            while ($rowMeisai = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                if ($rowMeisai["CONTROL_CODE"] == $rowMeisai["MONTH"]) {
                    if (!isset($model->warning)) {
                        $rowMeisai = makeTextData($rowMeisai, $data[$dcnt]["SEMESTER"], $controlCnt);
                    } else {
                        $rowMeisai = makeErrTextData($model, $rowMeisai["MONTH"], $data[$dcnt]["SEMESTER"], $rowMeisai, $controlCnt);
                    }
                    $controlCnt++;
                } else {
                    $rowMeisai["MONTH"] = "";
        ...
        /* 学期計 */
        $query = knjc032aQuery::selectAttendSemester($model, $data[$dcnt]["SEMESTER"]);
    ...
    //hidden
    knjCreateHidden($objForm, "CONTROL_CNT", $controlCnt);
