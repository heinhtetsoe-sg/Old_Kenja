...
class knjtx012Form2
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjtx012index.php", "", "edit");

        //警告メッセージを表示しない場合
        if (!isset($model->warning) && isset($model->cd)) {
            $Row = knjtx012Query::getRow($model->cd);
        } else {
            $Row =& $model->field;
        }
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "parent.left_frame.location.href='knjtx012index.php?cmd=list';";
        }
                                
        View::toHTML($model, "knjtx012Form2.html", $arg);
    }
} 
?>
