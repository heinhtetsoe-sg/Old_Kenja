...
class knjtx008Form2 {
    function main(&$model) {
        $objForm        = new form;
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjtx008index.php", "", "edit");
        $db  = Query::dbCheckOut();
        if (!isset($model->warning)) {
            $query = knjtx008Query::getCityMst($model->pref_cd, $model->city_cd);
...
//        $Row["PREF_CD"] = ($value && $value_flg) ? $Row["PREF_CD"] : $opt[0]["value"];
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "window.open('knjtx008index.php?cmd=list&ed=1','left_frame');";
        }
        View::toHTML($model, "knjtx008Form2.html", $arg);
    }
}
?>
