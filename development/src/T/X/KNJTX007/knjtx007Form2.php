...
class knjtx007Form2
{
    function main(&$model)
    {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjtx007index.php", "", "edit");

        //警告メッセージを表示しない場合
        if (isset($model->edboardcd) && !isset($model->warning)){
            $Row = knjtx007Query::getRow($model->edboardcd);
        }else{
            $Row =& $model->field;
        }
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "parent.left_frame.location.href='knjtx007index.php?cmd=list';";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。 
        View::toHTML($model, "knjtx007Form2.html", $arg); 
    }
}
?>
