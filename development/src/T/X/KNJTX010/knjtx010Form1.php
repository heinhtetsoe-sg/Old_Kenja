...
class knjtx010Form1
{
    ...
    //フォーム作成
    $arg["start"]   = $objForm->get_start("sel", "POST", "knjtx010index.php", "", "sel");
    ...
    $no_year        = 0;
    //年度設定
    $result    = $db->query(knjtx010Query::selectYearQuery($model));   
    $opt       = array();
    ...
    //科目一覧取得
    $result      = $db->query(knjtx010Query::selectQuery($model));   
    ...
    //一覧取得
    $result = $db->query(knjtx010Query::selectJuniorQuery($opt_left_id,$model));
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
    {
    ...
    View::toHTML($model, "knjtx010Form1.html", $arg); 
    }
}
