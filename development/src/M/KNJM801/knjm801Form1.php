...
class knjm801Form1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjm801Form1", "POST", "knjm801index.php", "", "knjm801Form1");
        ...
        $query = knjm801Query::getAuth(CTRL_YEAR, CTRL_SEMESTER);
        ...
        $opt = array();
        $value_flg = false;
        $query = knjm801Query::getGrpcd($model->field["YEAR"], $model->field["GRADE_HR_CLASS"]);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $extra = array("id=\"OUTPUT_NO1\" onClick=\"return btn_submit('knjm801')\"", "id=\"OUTPUT_NO2\" onClick=\"return btn_submit('knjm801')\"", "id=\"OUTPUT_NO3\" onClick=\"return btn_submit('knjm801')\"");
        ...
        $query = knjm801Query::getSchno($model, CTRL_YEAR, CTRL_SEMESTER);
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
            if ($row["OUTPUT_NO"] == "") {
        ...
        $arg["TOP"]["PRGID"] = knjCreateHidden($objForm, "PRGID", "KNJM801");
        ...
        View::toHTML($model, "knjm801Form1.html", $arg);
    }
}
?>
