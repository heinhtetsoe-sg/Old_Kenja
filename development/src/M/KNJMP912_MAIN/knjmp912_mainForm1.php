...
class knjmp912_mainForm1
{
    function main(&$model){

        $objForm = new form;
        $arg["start"]   = $objForm->get_start("knjmp912_mainForm1", "POST", "knjmp912_mainindex.php", "", "knjmp912_mainForm1");

        if (!isset($model->warning) && $model->getAuth != "" && $model->cmd !== 'edit') {
            $Row = knjmp912_mainQuery::getRow($model);
        } else {
            $Row =& $model->field;
        }
        ...
        $model->getHonjimeCount = $db->getOne(knjmp912_mainQuery::getCloseFlgData($model));
        ...
        //支出細目
        $extra = "";
        $query = knjmp912_mainQuery::getLevySDiv($model);
        makeCombo($objForm, $arg, $db, $query, $Row["HENKIN_L_M_S_CD"], "HENKIN_L_M_S_CD", $extra, 1, "BLANK", $model);
        
        //入金科目(細目)
        $extra = "";
        $query = knjmp912_mainQuery::getCollectSDiv($model);
        ...
        //左側
        $query = knjmp912_mainQuery::getSchno($model, "get");
        ...
        $result->free();
        //右側
        $query = knjmp912_mainQuery::getSchno($model);
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        View::toHTML($model, "knjmp912_mainForm1.html", $arg); 
    }
}
