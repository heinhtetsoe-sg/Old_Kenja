function btn_submit(cmd) {
    if (cmd === 'update') {
        ...
        //支出細目
        if (document.forms[0].HENKIN_L_M_S_CD.value == "") {
            alert('{rval MSG301}' + '(支出細目)');
            return false;
        }
        //入金細目
        if (document.forms[0].COLLECT_L_M_S_CD.value == "") {
            alert('{rval MSG301}' + '(入金細目)');
            return false;
        }
        //単価=0で、生徒選択していたらエラー
        if (document.forms[0].HENKIN_GK.value == 0 || document.forms[0].HENKIN_GK.value == "") {
            alert('{rval MSG301}' + '(返金額)');
            return;
        }
        //生徒未選択エラー
