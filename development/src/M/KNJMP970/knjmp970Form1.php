...
class knjmp970Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjmp970Form1", "POST", "knjmp970index.php", "", "knjmp970Form1");
        ...
        //学期コンボ
        $query = knjmp970Query::getSemester();
        $extra = "onchange=\"return btn_submit('knjmp970'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //出力選択ラジオボタン 1:学年 2:クラス
        $opt_output = array(1, 2);
        $model->field["OUTPUT"] = ($model->field["OUTPUT"] == "") ? "1" : $model->field["OUTPUT"];
        $extra = array("id=\"OUTPUT1\" onclick =\" return btn_submit('knjmp970');\"", "id=\"OUTPUT2\" onclick =\" return btn_submit('knjmp970');\"");
        $radioArray = knjCreateRadio($objForm, "OUTPUT", $model->field["OUTPUT"], $extra, $opt_output, get_count($opt_output));
        foreach($radioArray as $key => $val) $arg["data"][$key] = $val;

        //学年コンボ
        $query = knjmp970Query::getGrade();
        $extra = ($model->field["OUTPUT"] == "2") ? "onchange=\"return btn_submit('knjmp970'), AllClearList();\"" : "";
        ...
        View::toHTML($model, "knjmp970Form1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    $opt = array();
    $query = knjmp970Query::getHrClass($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $opt[] = array('label' => $row["LABEL"],
                       'value' => $row["VALUE"]);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJMP970");
