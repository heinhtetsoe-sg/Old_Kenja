...
{   
    function main(&$model)
    {
        ...
        //スクーリング出席状況詳細（全科目）
        $rec_cnt = 0;   //行番号
        $result = $db->query(knjm210_2Query::getSchDetailSubclassAll($model));
        ...
        //レポート提出状況詳細（全科目）
        $rec_cnt = 0;   //行番号
        $result = $db->query(knjm210_2Query::getRepDetailSubclassAll($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            $rec_cnt++;
        ...
        View::toHTML($model, "knjm210_2Form3.html", $arg); 
    }
}
?>
