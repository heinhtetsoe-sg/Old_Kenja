...
class knjm470Form1
{
    function main(&$model){
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjm470Form1", "POST", "knjm470index.php", "", "knjm470Form1");
        ...
        $objForm->ae( array("type"      => "select",
                            ...
                            "extrahtml" => "onChange=\"return btn_submit('knjm470');\"",
                            "options"   => $opt_seme) );
        ...
        $classcnt  = 0;
        $db = Query::dbCheckOut();
        $query = knjm470Query::getSubclass($model);
        ...
        //クラス選択コンボボックスを作成する
        $db = Query::dbCheckOut();
        $query = knjm470Query::getClass($model);
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        View::toHTML($model, "knjm470Form1.html", $arg); 
    }
}
?>
