...
class knjm713Form2
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjm713index.php", "", "edit");
        ...
        if(!isset($model->warning)){
            if ($model->cmd === 'bank') {
                ...
                $Row = knjm713Query::getRow($model,1);
            }
        } else {
            $Row =& $model->field;
        }
        ...
        $opt[] = array('label' => "", 'value' => "");
        $value_flg = false;
        $query = knjm713Query::getBanchcd($Row["BANK_CD"]);
        $result = $db->query($query);
        ...
        $opt = array();
        $opt[] = array('label' => "", 'value' => "");
        $query = knjm713Query::getNameMst("G203");
        $value_flg = false;
        $result = $db->query($query);
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"]  = "window.open('knjm713index.php?cmd=list','left_frame');";
        }
        
        $arg["finish"]  = $objForm->get_finish();
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。 
        View::toHTML($model, "knjm713Form2.html", $arg);
    }
}
?>
