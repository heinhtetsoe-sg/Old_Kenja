...
class knjm300mForm1
{
    function main(&$model){
        ...
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("knjm300mForm1", "POST", "knjm300mindex.php", "", "knjm300mForm1");
        ...
        //学期
        $opt = array();
        $query = knjm300mQuery::getSemeMst();
        ...
        $extra = "onChange=\"return btn_submit('knjm300m');\"";
        $arg["data"]["GAKKI"] = knjCreateCombo($objForm, "GAKKI", $model->field["GAKKI"], $opt, $extra, 1);

        //クラス
        $query = knjm300mQuery::getAuth(CTRL_YEAR, $model->field["GAKKI"]);
        $class_flg = false;
        $row1 = array();
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        $query = knjm300mQuery::getSchno($model, CTRL_YEAR, $model->field["GAKKI"]);
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        View::toHTML($model, "knjm300mForm1.html", $arg); 
    }
}
...
function makeHidden(&$objForm, $model)
{
    ...
    $objForm->ae(createHiddenAe("PRGID", "KNJM300M"));
