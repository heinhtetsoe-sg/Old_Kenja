...
class knjmp910_approvalForm1 {
    function main(&$model) {
        //オブジェクト作成
        $objForm        = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjmp910_approvalindex.php", "", "main");
        
        if (!isset($model->warning) && $model->getAuth != "" && $model->cmd != "check") {
            $Row = knjmp910_approvalQuery::getRow($model);
        } else {
            $Row =& $model->field;
        }
        ...
        $model->getHonjimeCount = $db->getOne(knjmp910_approvalQuery::getCloseFlgData($model));

        //決裁の状況を取得(比較時に利用)
        if ($model->getOutgoLMcd) {
            $model->getHenkinApproval = $db->getOne(knjmp910_approvalQuery::getLevyData($model, "HENKIN_APPROVAL"));
            $model->getApproval = $db->getOne(knjmp910_approvalQuery::getLevyData($model, "APPROVAL"));
            $model->getCancel   = $db->getOne(knjmp910_approvalQuery::getLevyData($model, "CANCEL"));
        } else {
        ...
        //支出項目
        $query = knjmp910_approvalQuery::getLevyMDiv($model);
        $arg["data"]["LEVY_M_NAME"] = $db->getOne($query);

        //支出金額
        $query = knjmp910_approvalQuery::getRequestGk($model);
        ...
        //受取人氏名
        $extra = "";
        $query = knjmp910_approvalQuery::getOutgoStaff($model);
        makeCombo($objForm, $arg, $db, $query, $Row["OUTGO_CHECK1_STAFF"], "OUTGO_CHECK1_STAFF", $extra, 1, "BLANK", $model);
        ...
        View::toHTML($model, "knjmp910_approvalForm1.html", $arg); 
    }
}
//コンボ作成
...
function makeBtn(&$objForm, &$arg, $model) {
    ...
    $subdata .= "&SEND_PRGID=KNJMP910_APPROVAL&SEND_OUTGO_L_CD={$model->getOutgoLcd}&SEND_OUTGO_M_CD={$model->getOutgoMcd}";
