...
class knjm110mForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knjm110mindex.php", "", "edit");
        ...
        if ($model->Div == 1) {
            //講座コンボ
            $query = knjm110mQuery::getChairList($model);
            $extra = "onChange=\"btn_submit('kch');\"";
            makeCmb($objForm, $arg, $db, $query, "CHAIRCD", $model->Chair, $extra, 1);

            //担当者表示
            $chair_stf = "";
            $result = $db->query(knjm110mQuery::getStaffname($model));
            ...
            //スクーリング一覧
            $syoki = 0;
            $result = $db->query(knjm110mQuery::ReadQuery($model));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                ...
                if($row["EXECUTED"] == 1) {
                    ...
                    $row["EXEDATE"] = View::alink("knjm110mindex.php",
                                                  $exedate,
                                                  "target=\"right_frame\"",
                                                  array("cmd"           => "edit",
        ...
        if ($model->Div == 2) {
            ...
            $query = knjm110mQuery::getExecutedateList($model);
            $extra = "onChange=\"btn_submit('kch');\"";
            ...
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                ...
                if($row["EXECUTED"] == 1) {
                    ...
                    $row["CHAIRNAME"] = View::alink("knjm110mindex.php",
                                                  $row["CHAIRNAME"],
                                                  "target=\"right_frame\"",
                                                  array("cmd"           => "edit",
                                                        "PERIODCD"      => $row["PERIODCD"],
        ...
        if (($model->cmd == "list" || $model->cmd== "kch" || $model->cmd== "init") && VARS::get("ed") != "1")
            $arg["reload"] = "window.open('knjm110mindex.php?cmd=edit&init=1','right_frame');";

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjm110mForm1.html", $arg);
    }
}
