...
class knjm110mForm2 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjm110mindex.php", "", "edit");
        ...
        if ($model->Div == 2) {
            ...
            if (!$model->Chair) {
                $opt_chair = array();
                $result = $db->query(knjm110mQuery::getChairList($model));
            ...
            //講座名称
            $query = knjm110mQuery::getChairname($model);
        ...
        $query = knjm110mQuery::selectPeriod($model);
        $extra = "";
        ...
        if ($model->Exedate && $model->Periodcd && $model->Chair && $model->Schooling_seq) {
            $query = knjm110mQuery::getRemark($model);
        ...
        if (VARS::get("cmd") != "edit" && !isset($model->warning)){
            $arg["reload"] = "window.open('knjm110mindex.php?cmd=list&ed=1','left_frame');";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjm110mForm2.html", $arg);
    }
}
