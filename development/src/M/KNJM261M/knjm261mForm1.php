...
class knjm261mForm1
{
    ...
    {
        $objForm        = new form;
        $arg["start"]   = $objForm->get_start("main", "POST", "knjm261mindex.php", "", "main");
        ...
        $REPORT_SORT = "<a href=\"knjm261mindex.php?cmd=sort&sort=SRT_REPORT\" target=\"_self\" STYLE=\"color:white\">レポートNo.{$setOrder}</a>";
        ...
        $NAME_SORT = "<a href=\"knjm261mindex.php?cmd=sort&sort=SRT_NAME\" target=\"_self\" STYLE=\"color:white\">氏名{$setOrder}</a>";
        ...
        $SUBCLASS_SORT = "<a href=\"knjm261mindex.php?cmd=sort&sort=SRT_SUBCLASS\" target=\"_self\" STYLE=\"color:white\">科目名{$setOrder}</a>";
        ...
        $TIME_SORT = "<a href=\"knjm261mindex.php?cmd=sort&sort=SRT_TIME\" target=\"_self\" STYLE=\"color:white\">登録時刻{$setOrder}</a>";
        $arg["TIME_SORT"] = $TIME_SORT;

        //抽出データ出力
        $schcnt = 0;
        $result = $db->query(knjm261mQuery::getSch($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        View::toHTML($model, "knjm261mForm1.html", $arg); 
    }
}
//makeCmb
