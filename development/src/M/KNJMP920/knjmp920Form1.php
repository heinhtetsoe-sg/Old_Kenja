...
class knjmp920Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjmp920Form1", "POST", "knjmp920index.php", "", "knjmp920Form1");
        ...
        //新規ボタン
        $subdata  = "wopen('".REQUESTROOT."/M/KNJMP920_MAIN/knjmp920_mainindex.php?cmd=main&&SEND_AUTH=".AUTHORITY."&SEND_YEAR={$model->field["YEAR"]}&SUBWIN=SUBWIN2','SUBWIN2',0,0,screen.availWidth,screen.availHeight);";
        ...
        //検索結果
        $query = knjmp920Query::selectQuery($db, $model);
        ...
        if($searchCnt > $checkCnt) {
            ...
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                if ($model->cmd == "search") {
                    ...
                    $subdata = "wopen('".REQUESTROOT."/M/KNJMP920_MAIN/knjmp920_mainindex.php?&cmd=main&SEND_YEAR=".$row["YEAR"]."&SEND_KOUNYU_L_CD=".$row["KOUNYU_L_CD"]."&SEND_KOUNYU_M_CD=".$row["KOUNYU_M_CD"]."&SEND_KOUNYU_L_M_CD=".$row["KOUNYU_L_CD"].$row["KOUNYU_M_CD"]."&SEND_REQUEST_NO=".$row["REQUEST_NO"]."&SEND_AUTH=".AUTHORITY.$extra;
                    $row["REQUEST_NO"] = View::alink("#", htmlspecialchars($row["REQUEST_NO"]),"onclick=\"$subdata\"");
                    
                    $row["KOUNYU_L_NAME"] = $db->getOne(knjmp920Query::getLevyLDiv($model, $row["KOUNYU_L_CD"]));
                    $row["KOUNYU_M_NAME"] = $db->getOne(knjmp920Query::getLevyMDiv($model, $row["KOUNYU_L_CD"], $row["KOUNYU_M_CD"]));
                    ...
                    //支出伺のチェック
                    $RowOutgo = array();
                    $RowOutgo = $db->getRow(knjmp920Query::getOutgoData($model, $setRequestNo), DB_FETCHMODE_ASSOC);
        ...
        View::toHTML($model, "knjmp920Form1.html", $arg);
    }
}
//コンボ作成
