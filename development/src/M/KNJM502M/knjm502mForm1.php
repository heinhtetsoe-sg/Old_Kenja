...
class knjm502mForm1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("sel", "POST", "knjm502mindex.php", "", "sel");
        $db = Query::dbCheckOut();
        ...
        //レポート添削登録確認
        $checkdata = $db->getOne(knjm502mQuery::selectSubclassQuery($model, "COUNT"));
        if ($checkdata == 0){
        ...
        $query = knjm502mQuery::selectSubclassQuery($model, "");
        ...
        View::toHTML($model, "knjm502mForm1.html", $arg); 
    }
}
?>
