...
class knjl111dForm1 {
    function main(&$model) {
        ...
        //転編入学照会日
        $model->field["REMARK8_033"] = str_replace('-', '/', ($model->field["REMARK8_033"] != "") ? $model->field["REMARK8_033"]: CTRL_DATE);
        $extra = "onChange=\"change_flg();\" onkeydown=\"changeEnterToTab(this)\"";
        $arg["data"]["REMARK8_033"] = View::popUpCalendarAlp($objForm, "REMARK8_033", $model->field["REMARK8_033"], $extra, "");
        ...
        for ($i=1; $i <= 5; $i++) {
            //氏名
            $extra = "STYLE=\"ime-mode: active\" onChange=\"change_flg()\" id=\"RELANAME{$i}\" onkeyup=\"keySet('RELANAME{$i}', 'RELAKANA{$i}', 'H');\" onkeydown=\"changeEnterToTab(this)\"";
            $arg["data"]["RELANAME{$i}"] = knjCreateTextBox($objForm, $Row["RELANAME{$i}"], "RELANAME{$i}", 40, 60, $extra);

            //氏名かな
            $extra = "STYLE=\"ime-mode: active\" onChange=\"change_flg();\" id=\"RELAKANA{$i}\" onkeydown=\"changeEnterToTab(this)\"";
            $arg["data"]["RELAKANA{$i}"] = knjCreateTextBox($objForm, $Row["RELAKANA{$i}"], "RELAKANA{$i}", 40, 120, $extra);
        ...
        //かな検索ボタン
        $extra = "style=\"width:80px\" onclick=\"loadwindow('" .REQUESTROOT ."/L/KNJL111D/search_name.php?cmd=search&year='+document.forms[0]['year'].value+'&applicantdiv='+document.forms[0]['APPLICANTDIV'].value+'&testdiv='+document.forms[0]['TESTDIV'].value+'&examno='+document.forms[0]['EXAMNO'].value+'&NAME_SESSID={$sess->id}&frame='+getFrameName(self), event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 320, 260)\"";
