...
class knjm712Form1
{
    ...
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjm712index.php", "", "edit");
        ...
        //コピー用データチェック 入金
        $money_flg  = knjm712Query::selectYearQuery($model->leftSetYear, "COLLECT_MONEY_PAID_M_DAT");
        $money_flg += knjm712Query::selectYearQuery($model->leftSetYear, "COLLECT_MONEY_PAID_S_DAT");

        //コピー用データチェック 入金
        $money_flg += knjm712Query::selectYearQuery($model->leftSetYear + 1, "COLLECT_MONEY_PAID_M_DAT");
        $money_flg += knjm712Query::selectYearQuery($model->leftSetYear + 1, "COLLECT_MONEY_PAID_S_DAT");

        //コピー用データチェック 登録
        $year_flg  = knjm712Query::selectYearQuery($model->leftSetYear, "COLLECT_GRP_MST");
        $year_flg += knjm712Query::selectYearQuery($model->leftSetYear, "COLLECT_GRP_HR_DAT");
        $year_flg += knjm712Query::selectYearQuery($model->leftSetYear, "COLLECT_GRP_DAT");
        $year_flg += knjm712Query::selectYearQuery($model->leftSetYear, "COLLECT_MONEY_DUE_M_DAT");
        $year_flg += knjm712Query::selectYearQuery($model->leftSetYear, "COLLECT_MONEY_DUE_S_DAT");

        //コピー用データチェック 登録
        $year_flg += knjm712Query::selectYearQuery($model->leftSetYear + 1, "COLLECT_GRP_MST");
        $year_flg += knjm712Query::selectYearQuery($model->leftSetYear + 1, "COLLECT_GRP_HR_DAT");
        $year_flg += knjm712Query::selectYearQuery($model->leftSetYear + 1, "COLLECT_GRP_DAT");
        $year_flg += knjm712Query::selectYearQuery($model->leftSetYear + 1, "COLLECT_MONEY_DUE_M_DAT");
        $year_flg += knjm712Query::selectYearQuery($model->leftSetYear + 1, "COLLECT_MONEY_DUE_S_DAT");

        $db = Query::dbCheckOut();

        //コピー用マスタチェック(前年度に登録されているクラスが今年度に存在するか)
        $mst_flg = "";
        $result = $db->query(knjm712Query::mstClassCheck($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if (!$mst_flg) {
            $result = $db->query(knjm712Query::mstMcdCheck($model));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($mst_flg == "") {
            $result = $db->query(knjm712Query::mstScdCheck($model));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //会計グループリスト取得
        $query = knjm712Query::SelectQuery($model);
        $result = $db->query($query);
        $this->DataRow = array();
        $dataCnt = 0;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        View::toHTML($model, "knjm712Form1.html", $arg);        
    }
    
    //表示用配列にデータ値をセット
    function SetDataRow($row)
    {
        $this->DataRow["COLLECT_GRP_CD"][] = View::alink("knjm712index.php", $row["COLLECT_GRP_CD"].":".$row["COLLECT_GRP_NAME"], "target=right_frame",
