...
{   
    function main(&$model)
    {
        ...
        //スクーリング出席状況詳細（科目別）
        $rec_cnt = 0;   //行番号
        $result = $db->query(knjm210m_2Query::getSchDetailSubclass($model));
        ...
        //レポート提出状況詳細（科目別）
        $rec_cnt = 0;   //行番号
        $result = $db->query(knjm210m_2Query::getRepDetailSubclass($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            $rec_cnt++;
        ...
        View::toHTML($model, "knjm210m_2Form2.html", $arg); 
    }
}
?>
