...
{   
    function main(&$model)
    {
        ...
        //講座名（科目名）リスト：抽出
        $opt_chair = array();
        $result = $db->query(knjm210m_2Query::getChairSubclassList($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            array_walk($row, "htmlspecialchars_array");

            //リンク設定（詳細一覧画面：科目別）
            $js = "wopen('knjm210m_2index.php?cmd=form2&SCHNO={$model->schregno}&CHAIR={$row["CHAIRCD"]}&SUBCLASS={$row["SUBCLASSCD"]}&PROGRAMID=PROGRAMID','SUBWIN3',0,0,screen.availWidth,screen.availHeight);";
        ...
        //リンク設定（詳細一覧画面：全科目）
        if ($link_flg) {
            $js = "wopen('knjm210m_2index.php?cmd=form3&SCHNO={$model->schregno}&PROGRAMID=PROGRAMID','SUBWIN3',0,0,screen.availWidth,screen.availHeight);";
        ...
        foreach ($opt_chair as $key => $val) {
            //規定回数
            $Row1 = knjm210m_2Query::getKiteiCount($db,$val["CHAIRCD"]);
            $Row1 = is_array($Row1) ? $Row1 : array();
            //回数・履歴(スクーリング)
            $Row2 = $Row3 = array();
            $num2 = $num3 = 0;
            $cnt1 = $cnt2 = $cnt3 = $cnt4 = 0;
            $query = knjm210m_2Query::getSchCountRireki($db,$val["CHAIRCD"],$model);
            $result = $db->query($query);
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                array_walk($row, "htmlspecialchars_array");

                //履歴(登校・その他)
                if ($row["SCHOOLINGKINDCD"] == "1" || $row["SCHOOLINGKINDCD"] == "3") {
                    if ($row["SCHOOLINGKINDCD"] == "1") {
                        $cnt1 += $row["CREDIT_TIME"];
                        $color_s = "";
                        $color_e = "";
                    }
                    if ($row["SCHOOLINGKINDCD"] == "3") {
                        $cnt2 += $row["CREDIT_TIME"];
                        $color_s = "<span style=\"color:red\">";
                        $color_e = "</span>";
                    }
                    $tmp_row = array("DAYUPP".($num2+1) => $color_s.$row["E_DATE"]."(".$row["CREDIT_TIME"].")".$color_e);
                    if ($num2 == 0) $Row2 = $tmp_row;
                    if ($num2 > 0)  $Row2 = array_merge_recursive($Row2,$tmp_row);
                    $num2++;
                }
                //履歴(放送)
                if ($row["SCHOOLINGKINDCD"] == "2" || $row["SCHOOLINGKINDCD"] == "4") {
                    if ($row["SCHOOLINGKINDCD"] == "2") {
                        $cnt3 += $row["CREDIT_TIME"];
                        $color_s = "<span style=\"color:green\">";
                        $color_e = "</span>";
                    }
                    if ($row["SCHOOLINGKINDCD"] == "4") {
                        $cnt4 += $row["CREDIT_TIME"];
                        $color_s = "<span style=\"color:purple\">";
                        $color_e = "</span>";
                    }
                    $tmp_row = array("DAYMID".($num3+1) => $color_s.$row["E_DATE"]."(".$row["CREDIT_TIME"].")".$color_e);
            ...
            $Row4 = array();
            $num4 = 0;
            $query = knjm210m_2Query::getRepCountRireki($db,$val["CHAIRCD"],$model);
            $result = $db->query($query);
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                array_walk($row, "htmlspecialchars_array");
        ...
        View::toHTML($model, "knjm210m_2Form1.html", $arg); 
    }
}
?>
