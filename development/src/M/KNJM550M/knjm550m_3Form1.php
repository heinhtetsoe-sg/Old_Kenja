...
class knjm550m_3Form1
{
    public function main(&$model)
    {
        $objForm = new form();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjm550m_3Form1", "POST", "knjm550mindex.php", "", "knjm550m_3Form1");
        ...
        $opt_data = array(1, 2, 3);
        $model->type_div = ($model->type_div == "") ? "1" : $model->type_div;
        $extra = array("id=\"TYPE_DIV1\" onClick=\"btn_submit('knjm550m_1')\"", "id=\"TYPE_DIV2\" onClick=\"btn_submit('knjm550m_2')\"", "id=\"TYPE_DIV3\" onClick=\"btn_submit('knjm550m_3')\"");
        $radioArray = knjCreateRadio($objForm, "TYPE_DIV", $model->type_div, $extra, $opt_data, get_count($opt_data));
        foreach ($radioArray as $key => $val) {
            $arg["data"][$key] = $val;
        }
        ...
        if ($model->cmd == "knjm550m_3Search" &&
            ...
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                $row["VALUE"] = $row["GRADE"].$row["HR_CLASS"].$row["ATTENDNO"]."-".$row["SCHREGNO"];
        ...
        View::toHTML($model, "knjm550m_3Form1.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg)
{
    ...
    //名簿印刷ボタン
    $extra = "onclick=\"return newwin('" . SERVLET_URL . "', 'KNJM550M');\"";
    ...
    //印刷ボタン
    $extra = "onclick=\"return newwin('" . SERVLET_URL . "', 'KNJM812A');\"";
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJM550M");
