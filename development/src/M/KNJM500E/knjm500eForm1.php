...
class knjm500eForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjm500eForm1", "POST", "knjm500eindex.php", "", "knjm500eForm1");
        ...
        //学期
        $opt = array();
        $query = knjm500eQuery::getSemeMst();
        ...
        $extra = "onChange=\"return btn_submit('knjm500e');\"";
        $arg["data"]["GAKKI"] = knjCreateCombo($objForm, "GAKKI", $model->field["GAKKI"], $opt, $extra, 1);

        //クラス選択コンボボックスを作成する
        $query = knjm500eQuery::getAuth(CTRL_YEAR, $model->field["GAKKI"]);
        ...
        $query = knjm500eQuery::getSchreg($model);
        ...
        //タイトル
        $opt = array(1, 2);
        if (!$model->field["TITLE_SELECT"]) $model->field["TITLE_SELECT"] = "2";
        $extra = array("id=\"TITLE_SELECT1\" " , "id=\"TITLE_SELECT2\" ");
        $radioArray = knjCreateRadio($objForm, "TITLE_SELECT", $model->field["TITLE_SELECT"], $extra, $opt, get_count($opt));
        ...
        if ($model->cmd != 'clschange') {
            $query = knjm500eQuery::getComment();
        ...
        View::toHTML($model, "knjm500eForm1.html", $arg); 
    }
}
...
function makeHidden(&$objForm, &$arg, $model) {
    ...
    $arg["TOP"]["PRGID"] = knjCreateHidden($objForm, "PRGID", "KNJM500E");
    ...
    $arg["TOP"]["useRepStandarddateCourseDat"] = knjCreateHidden($objForm, "useRepStandarddateCourseDat", $model->Properties["useRepStandarddateCourseDat"]);
}
