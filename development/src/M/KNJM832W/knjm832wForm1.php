...
class knjm832wForm1
{
    function main(&$model){
        ...
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("knjm832wForm1", "POST", "knjm832windex.php", "", "knjm832wForm1");
        ...
        //学期
        $opt = array();
        $query = knjm832wQuery::getSemeMst();
        ...
        //教科
        $opt = array();
        $query = knjm832wQuery::getClass($model->field["GAKKI"]);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        $extra = "onChange=\"return btn_submit('knjm832w');\"";
        ...
        $query = knjm832wQuery::getSubclass($model, $model->field["GAKKI"], $model->field["CLASSCD_SET"]);
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        View::toHTML($model, "knjm832wForm1.html", $arg); 
    }
}
...
function makeHidden(&$objForm)
{
    ...
    $objForm->ae(createHiddenAe("PRGID", "KNJM832W"));
