...
class knjm270sForm1
{
    ...
    {
        $objForm        = new form;
        $arg["start"]   = $objForm->get_start("main", "POST", "knjm270sindex.php", "", "main");
        ...
        //添削者
        $opt_staf = array();
        $result = $db->query(knjm270sQuery::selectStaff($model));
        ...
        //抽出データ出力
        $schcnt = 0;
        $result = $db->query(knjm270sQuery::getSch($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            $subdata = "loadwindow('knjm270sindex.php?cmd=subform1&GRAD_VALUESUB={$row["GRAD_VALUE"]}&SCHREGNOSUB={$row["SCHREGNO"]}&SUBCLASSSUB={$linkSubclass}&STSEQSUB={$row["STANDARD_SEQ"]}&RSEQSUB={$row["REPRESENT_SEQ"]}&DATESUB={$model->Date}&CHAIRSUB={$row["CHAIRCD"]}&RECDAYSUB={$row["RECEIPT_DATE"]}',500,200,350,300)";
        ...
        View::toHTML($model, "knjm270sForm1.html", $arg); 
    }
}
//makeCmb
