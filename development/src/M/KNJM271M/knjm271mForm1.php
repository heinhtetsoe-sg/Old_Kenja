...
class knjm271mForm1
{
    ...
    {
        $objForm        = new form;
        $arg["start"]   = $objForm->get_start("main", "POST", "knjm271mindex.php", "", "main");
        ...
        //添削者
        $opt_staf = array();
        $result = $db->query(knjm271mQuery::selectStaff($model));
        ...
        $REPORT_SORT = "<a href=\"knjm271mindex.php?cmd=sort&sort=SRT_REPORT\" target=\"_self\" STYLE=\"color:white\">レポートNo.{$setOrder}</a>";
        ...
        $NAME_SORT = "<a href=\"knjm271mindex.php?cmd=sort&sort=SRT_NAME\" target=\"_self\" STYLE=\"color:white\">氏名{$setOrder}</a>";
        ...
        $SUBCLASS_SORT = "<a href=\"knjm271mindex.php?cmd=sort&sort=SRT_SUBCLASS\" target=\"_self\" STYLE=\"color:white\">科目名{$setOrder}</a>";
        ...
        $TIME_SORT = "<a href=\"knjm271mindex.php?cmd=sort&sort=SRT_TIME\" target=\"_self\" STYLE=\"color:white\">登録時刻{$setOrder}</a>";
        $arg["TIME_SORT"] = $TIME_SORT;

        //抽出データ出力
        $schcnt = 0;
        $result = $db->query(knjm271mQuery::getSch($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            $subdata = "loadwindow('knjm271mindex.php?cmd=subform1&GRAD_VALUESUB={$row["GRAD_VALUE"]}&SCHREGNOSUB={$row["SCHREGNO"]}&SUBCLASSSUB={$linkSubclass}&STSEQSUB={$row["STANDARD_SEQ"]}&RSEQSUB={$row["REPRESENT_SEQ"]}&DATESUB={$model->Date}&CHAIRSUB={$row["CHAIRCD"]}&RECDAYSUB={$row["RECEIPT_DATE"]}',500,200,350,300)";
        ...
        View::toHTML($model, "knjm271mForm1.html", $arg); 
    }
}
//makeCmb
