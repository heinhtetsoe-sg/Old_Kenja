...
class knjm500mForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjm500mForm1", "POST", "knjm500mindex.php", "", "knjm500mForm1");
        ...
        //学期
        $opt = array();
        $query = knjm500mQuery::getSemeMst();
        ...
        $extra = "onChange=\"return btn_submit('knjm500m');\"";
        $arg["data"]["GAKKI"] = knjCreateCombo($objForm, "GAKKI", $model->field["GAKKI"], $opt, $extra, 1);

        //クラス選択コンボボックスを作成する
        $query = knjm500mQuery::getAuth(CTRL_YEAR, $model->field["GAKKI"]);
        ...
        $query = knjm500mQuery::getSchreg($model);
        ...
        if ($model->cmd != 'clschange') {
            $query = knjm500mQuery::getComment();
        ...
        View::toHTML($model, "knjm500mForm1.html", $arg); 
    }
}
...
function makeHidden(&$objForm, &$arg, $model) {
    ...
    $arg["TOP"]["PRGID"] = knjCreateHidden($objForm, "PRGID", "KNJM500M");
