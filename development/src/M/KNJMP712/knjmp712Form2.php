...
class knjmp712Form2
{
    function main(&$model)
    {
        ...
        $arg["start"] = $objForm->get_start("edit", "POST", "knjmp712index.php", "", "edit");
        ...
        if ($model->cmd == "grpChange") {
            $query = knjmp712Query::getGrpData($model->exp_grpcd);
        ...
        if (isset($model->warning) || $model->cmd == "reload" || $model->cmd == "change2" ||
            ...
            $query = knjmp712Query::getRow($model, 1);
        ...
        //入金済みデータの件数
        $paid_money  = $db->getOne(knjmp712Query::getPaidMoney($model->taisyouYear, "COLLECT_MONEY_PAID_M_DAT", $Row["COLLECT_GRP_CD"]));
        $paid_money += $db->getOne(knjmp712Query::getPaidMoney($model->taisyouYear, "COLLECT_MONEY_PAID_S_DAT", $Row["COLLECT_GRP_CD"]));
        //hidden
        ...
        if (isset($model->warning) || $model->cmd == "reload") {
            ...
            $result = $db->query(knjmp712Query::ReloadSelectClass($model));
        } else {
            $result = $db->query(knjmp712Query::GetSelectClass($model));
        }
        ...
        //クラス一覧取得
        $result = $db->query(knjmp712Query::GetClass($model, $tempcd));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $label = $row["GRADE"].$row["HR_CLASS"]."：".htmlspecialchars($row["HR_NAME"]);
        ...
        if (isset($model->warning) || $model->cmd == "reload") {
            //会計項目割当一覧の値が変更してリロードした時
            $query = knjmp712Query::ReloadSelectMcd($model);
        } else {
            $query = knjmp712Query::GetSelectMcd($model);
        }
        $result = $db->query($query);
        ...
        //会計項目一覧
        $result = $db->query(knjmp712Query::GetMcd($model, $tempcd));
        ...
        if (isset($model->warning) || $model->cmd == "reload") {
            //会計項目割当一覧の値が変更してリロードした時
            $query = knjmp712Query::ReloadSelectScd($model);
        } else {
            $query = knjmp712Query::GetSelectScd($model);
        }
        $result = $db->query($query);
        ...
        //会計細目一覧
        $result = $db->query(knjmp712Query::GetScd($model, $tempcd, $tempcd2));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $label = $row["COLLECT_L_CD"].$row["COLLECT_M_CD"].$row["COLLECT_S_CD"]."：".htmlspecialchars($row["COLLECT_S_NAME"]);
            $val = $row["COLLECT_L_CD"].$row["COLLECT_M_CD"].$row["COLLECT_S_CD"];
        ...
        if (VARS::get("cmd") != "edit") {
            $arg["jscript"] = "window.open('knjmp712index.php?cmd=list','left_frame');";
        }

        $arg["finish"]  = $objForm->get_finish();

        View::toHTML($model, "knjmp712Form2.html", $arg);
    }
