...
class knjm060mForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjm060mForm1", "POST", "knjm060mindex.php", "", "knjm060mForm1");

        //DB接続
        $db = Query::dbCheckOut();

        //親画面なし
        $securityCnt = $db->getOne(knjm060mQuery::getSecurityHigh());
        //セキュリティーチェック
        ...
        //年度コンボを作成する
        $query = knjm060mQuery::GetYear($model);
        $extra = "onchange = \"return btn_submit('knjm060m')\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["YEAR"], "YEAR", $extra, 1, "");

        //学期コンボを作成する
        $query = knjm060mQuery::GetSem($model);
        $extra = "onchange = \"return btn_submit('knjm060m')\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["GAKKI"], "GAKKI", $extra, 1, "ALL");

        //科目コンボを作成する
        $query = knjm060mQuery::GetSub($model);
        $extra = "onchange = \"return btn_submit('knjm060m')\"";
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjm060mQuery::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJM060M");
        ...
        View::toHTML($model, "knjm060mForm1.html", $arg); 
