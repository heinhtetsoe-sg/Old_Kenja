...
class knjm390mForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjm390mindex.php", "", "main");
        ...
        //スクーリング種別
        $query = knjm390mQuery::selectName("M001");
        ...
        //講座コンボ
        $opt_chair = array();
        $result = $db->query(knjm390mQuery::getAuth($model));
        ...
        $query = knjm390mQuery::selectName("B001");
        ...
        $opt_staf = array();
        $result = $db->query(knjm390mQuery::selectStaff($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $opt = array();
        $max_time = $db->getOne(knjm390mQuery::getCreditTime($model));
        ...
        //開講しない科目、講座情報チェック
        $model->Attendchaircd = "";
        $model->not_chairdat = $db->getOne(knjm390mQuery::getAttendChairDataCheck($model));
        //開講しない科目、講座情報取得
        if ($model->not_chairdat > 0) {
            $query = knjm390mQuery::getAttendChairData($model, "");
        ...
        $result = $db->query(knjm390mQuery::getSch($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        View::toHTML($model, "knjm390mForm1.html", $arg); 
    }
}
