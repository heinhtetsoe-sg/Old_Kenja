...
class knjm832Form1
{
    function main(&$model){
        ...
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("knjm832Form1", "POST", "knjm832index.php", "", "knjm832Form1");
        ...
        //教科
        $opt = array();
        $query = knjm832Query::getClass($model->field["GAKKI"]);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        $extra = "onChange=\"return btn_submit('knjm832');\"";
        ...
        $query = knjm832Query::getSubclass($model, $model->field["GAKKI"], $model->field["CLASSCD_SET"]);
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        View::toHTML($model, "knjm832Form1.html", $arg); 
    }
}
...
function makeHidden(&$objForm)
{
    ...
    $objForm->ae(createHiddenAe("PRGID", "KNJM832"));
