 ...
 *
 */
class knjmp910_schregForm1
{
    function main(&$model){

        $objForm = new form;
        $arg["start"]   = $objForm->get_start("knjmp910_schregForm1", "POST", "knjmp910_schregindex.php", "", "knjmp910_schregForm1");

        if (!isset($model->warning) && $model->getAuth != "" && $model->cmd !== 'edit') {
            $Row = knjmp910_schregQuery::getRow($model);
        } else {
            $Row =& $model->field;
        }
        ...
        $model->getHonjimeCount = $db->getOne(knjmp910_schregQuery::getCloseFlgData($model));

        //返金フラグと返金用の入金科目、決済の状況取得(比較時に利用)
        if ($model->getOutgoLMcd) {
            $model->getHenkinFlg = $db->getOne(knjmp910_schregQuery::getLevyData($model, "HENKIN_FLG"));
            $model->getHenkinApproval = $db->getOne(knjmp910_schregQuery::getLevyData($model, "HENKIN_APPROVAL"));
            $model->getCollectLMSGrpcd = $db->getOne(knjmp910_schregQuery::getLevyData($model, "COLLECT"));
            $model->getApproval = $db->getOne(knjmp910_schregQuery::getLevyData($model, "APPROVAL"));
            $model->getCancel   = $db->getOne(knjmp910_schregQuery::getLevyData($model, "CANCEL"));
        } else {
            $model->getHenkinFlg = "";
        ...
        //支出科目
        $query = knjmp910_schregQuery::getLevyMDiv($model);
        ...
        $extra = array("id=\"WARIHURI_DIV1\" onclick=\"return btn_submit('knjmp910_schreg');\"", "id=\"WARIHURI_DIV2\" onclick=\"return btn_submit('knjmp910_schreg');\"");
        ...
        //支出細目
        $extra = "";
        $query = knjmp910_schregQuery::getLevySDiv($model);
        ...
        if ($model->field["WARIHURI_DIV"] == "2") {
            ...
            //請求月
            $extra = "";
            $query = knjmp910_schregQuery::getSeikyuMonth();
        ...
        if ($model->field["WARIHURI_DIV"] == "1") {
            ...
            //左側
            if (!$model->getHenkinFlg) {
                $query = knjmp910_schregQuery::getSchno($model, "get");
            //返金用
            } else {
                $query = knjmp910_schregQuery::getHenkinSchno($model, "get");
            }
            ...
            //右側
            if (!$model->getHenkinFlg) {
                $query = knjmp910_schregQuery::getSchno($model);
            //返金用
            } else {
                $query = knjmp910_schregQuery::getHenkinSchno($model);
            }
            $result = $db->query($query);
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        View::toHTML($model, "knjmp910_schregForm1.html", $arg); 
    }
}
...
function makeButton(&$objForm, &$arg, $model)
{
    ...
    $subdata .= "&SEND_PRGID=KNJMP910_SCHREG&SEND_OUTGO_L_CD={$model->getOutgoLcd}&SEND_OUTGO_M_CD={$model->getOutgoMcd}";
