...
class knjm825Form1
{
    function main(&$model){
        ...
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("knjm825Form1", "POST", "knjm825index.php", "", "knjm825Form1");
        ...
        $extra = "onChange=\"return btn_submit('knjm825');\"";
        $arg["data"]["GAKKI"] = knjCreateCombo($objForm, "GAKKI", $model->field["GAKKI"], $opt, $extra, 1);

        //クラス
        $query = knjm825Query::getAuth(CTRL_YEAR, $model->field["GAKKI"]);
        $class_flg = false;
        $row1 = array();
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        $query = knjm825Query::getSchno($model, CTRL_YEAR, $model->field["GAKKI"]);
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        View::toHTML($model, "knjm825Form1.html", $arg); 
    }
}
...
function makeHidden(&$objForm)
{
    ...
    $objForm->ae(createHiddenAe("PRGID", "KNJM825"));
