...
class knjm210m_3Form2
{
    function main(&$model)
    {
        $objForm       = new form;
        $arg["start"]  = $objForm->get_start("edit", "POST", "knjm210m_3index.php", "", "edit");
        $arg["reload"] = "";

//        $model->schregno = "20031935";
        $Row = knjm210m_3Query::getStudent_data($model->schregno);
        //出席番号
        ...
        //年組コンボボックス
        $result = $db->query(knjm210m_3Query::getGrd_ClasQuery($model));
        $opt_grcl = array();
        ...
        //課程学科
        $result       = $db->query(knjm210m_3Query::getCourse_Subject());
        $opt_coursecd = array();
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        //コース
        $result = $db->query(knjm210m_3Query::getCourseCode());
        $opt_course3 = array();
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
...
//                            "extrahtml"   => "style=width:\"30%\"",
        ...
        //出身中学校
        $result = $db->query(knjm210m_3Query::getFinschoolName());
        $opt = array();
        ...
        $objForm->ae( array("type"        => "text",
                            ...
                            "size"        => 45,
                            "maxlength"   => 75,
                            "value"       => $db->getOne(knjm210m_3Query::getEnt_reason($model->schregno))));
        ...
        $objForm->ae( array("type"        => "text",
                            ...
                            "size"        => 45,
                            "maxlength"   => 75,
                            "value"       => $db->getOne(knjm210m_3Query::getGrd_reason($model->schregno))));
        ...
        //出身塾
        $result = $db->query(knjm210m_3Query::getPrischoolName());
        $opt = array();
        $opt[] = array("label" => "","value" => "");
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
       ...
       $arg["data"]["REMARK3"] = $objForm->ge("REMARK3");
        ...
        View::toHTML($model, "knjm210m_3Form2.html", $arg);
    }
}
?>
