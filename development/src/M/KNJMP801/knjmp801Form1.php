...
class knjmp801Form1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjmp801Form1", "POST", "knjmp801index.php", "", "knjmp801Form1");
        ...
        $query = knjmp801Query::getAuth(CTRL_YEAR, CTRL_SEMESTER);
        ...
        $opt = array();
        $value_flg = false;
        $query = knjmp801Query::getGrpcd($model->field["YEAR"], $model->field["GRADE_HR_CLASS"]);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $extra = array("id=\"OUTPUT_NO1\" onClick=\"return btn_submit('knjmp801')\"", "id=\"OUTPUT_NO2\" onClick=\"return btn_submit('knjmp801')\"", "id=\"OUTPUT_NO3\" onClick=\"return btn_submit('knjmp801')\"");
        ...
        $query = knjmp801Query::getSchno($model, CTRL_YEAR, CTRL_SEMESTER);
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
            if ($row["OUTPUT_NO"] == "") {
        ...
        $arg["TOP"]["PRGID"] = knjCreateHidden($objForm, "PRGID", "KNJMP801");
        ...
        View::toHTML($model, "knjmp801Form1.html", $arg);
    }
}
?>
