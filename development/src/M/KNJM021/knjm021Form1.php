...
class knjm021Form1
{
    function main(&$model){
		...
		//フォーム作成
		$arg["start"]   = $objForm->get_start("knjm021Form1", "POST", "knjm021index.php", "", "knjm021Form1");
		...
		//クラス選択コンボボックスを作成する
		$db = Query::dbCheckOut();
		$query = knjm021Query::getAuth($model->control["年度"],$model->control["学期"]);
		...
		//対象者リストを作成する
		$db = Query::dbCheckOut();
		$query = knjm021Query::getSchreg($model);
		...
		View::toHTML($model, "knjm021Form1.html", $arg); 
	}
}
?>
