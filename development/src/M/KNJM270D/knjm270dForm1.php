...
class knjm270dForm1 {
    function main(&$model) {
        ...
        //学期コンボ
        $query = knjm270dQuery::getSemecmb();
        ...
        //科目リスト
        $query = knjm270dQuery::getSubClassCd($model);
        $extra = "onChange=\"btn_submit('change')\";";
        makeCmb($objForm, $arg, $db, $query, $model->subclassCd, "SUBCLASSCD", $extra, 1, "BLANK");


        //講座リスト
        $query = knjm270dQuery::getChrSubCd($model);
        $extra = "onChange=\"btn_submit('change')\";";
        makeCmb($objForm, $arg, $db, $query, $model->chairCd, "CHAIRCD", $extra, 1, "BLANK");

        //レポート回数・最低提出回数
        $setRow = $db->getRow(knjm270dQuery::getChairCorresSemesDat($model), DB_FETCHMODE_ASSOC);
        $arg["REPO_MAX_CNT"]   = $setRow["REPO_MAX_CNT"];
        $arg["REPO_LIMIT_CNT"] = $setRow["REPO_LIMIT_CNT"];

        //初期化
        $model->data = array();

        //提出回数のリスト
        $query = knjm270dQuery::getSchAttendSemesDat($model);
        ...
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            $row["REPORT_CNT"] = knjCreateTextBox($objForm, $row["REPORT_CNT"], "REPORT_CNT-".$row["SCHREGNO"], 2, 2, $extra);
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjm270dindex.php", "", "main");

        $arg["finish"]  = $objForm->get_finish();

        View::toHTML($model, "knjm270dForm1.html", $arg); 
    }
}
//makeCmb
