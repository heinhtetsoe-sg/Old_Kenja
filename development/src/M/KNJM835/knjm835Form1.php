...
class knjm835Form1
{
    function main(&$model){
        ...
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("knjm835Form1", "POST", "knjm835index.php", "", "knjm835Form1");
        ...
        $extra = "onChange=\"return btn_submit('knjm835');\"";
        $arg["data"]["YEAR"] = knjCreateCombo($objForm, "YEAR", $model->field["YEAR"], $opt, $extra, 1);

        //学期
        $opt = array();
        $query = knjm835Query::getSemeMst($model->field["YEAR"]);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        View::toHTML($model, "knjm835Form1.html", $arg); 
    }
}
...
function makeHidden(&$objForm)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJM835");
