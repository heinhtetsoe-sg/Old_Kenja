...
class knjm290mForm1
{
    function main(&$model){
        ...
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("knjm290mForm1", "POST", "knjm290mindex.php", "", "knjm290mForm1");
        ...
        //学期
        $opt = array();
        $query = knjm290mQuery::getSemeMst();
        ...
        $extra = "onChange=\"return btn_submit('knjm290m');\"";
        $arg["data"]["GAKKI"] = knjCreateCombo($objForm, "GAKKI", $model->field["GAKKI"], $opt, $extra, 1);
        
        //クラス
        $query = knjm290mQuery::getAuth(CTRL_YEAR, $model->field["GAKKI"]);
        $class_flg = false;
        $row1 = array();
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        //教科
        $opt = array();
        $query = knjm290mQuery::getClass($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        if ($model->field["OUTPUT"] === '1') {
            $query = knjm290mQuery::getSchno($model, CTRL_YEAR, $model->field["GAKKI"]);
            $result = $db->query($query);
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
            ...
            $query = knjm290mQuery::getSubclass($model, $model->field["CLASSCD_SET"]);
            $result = $db->query($query);
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
                if (!in_array($row["SCHREGNO"], $selectStudent)) {
        ...
        View::toHTML($model, "knjm290mForm1.html", $arg); 
    }
}
...
function makeHidden(&$objForm, $model)
{
    ...
    $objForm->ae(createHiddenAe("PRGID", "KNJM290M"));
