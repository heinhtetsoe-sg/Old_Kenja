...
class knjm501mForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjm501mForm1", "POST", "knjm501mindex.php", "", "knjm501mForm1");
        ...
        //学期
        $opt = array();
        $query = knjm501mQuery::getSemeMst();
        ...
        $extra = "onChange=\"return btn_submit('knjm501m');\"";
        $arg["data"]["GAKKI"] = knjCreateCombo($objForm, "GAKKI", $model->field["GAKKI"], $opt, $extra, 1);

        //クラス選択コンボボックスを作成する
        $query = knjm501mQuery::getAuth(CTRL_YEAR, $model->field["GAKKI"]);
        ...
        $query = knjm501mQuery::getSchreg($model);
        ...
        if ($model->cmd != 'clschange') {
            $query = knjm501mQuery::getComment();
        ...
        View::toHTML($model, "knjm501mForm1.html", $arg); 
    }
}
...
function makeHidden(&$objForm, &$arg, $model) {
    ...
    $arg["TOP"]["PRGID"] = knjCreateHidden($objForm, "PRGID", "KNJM501M");
