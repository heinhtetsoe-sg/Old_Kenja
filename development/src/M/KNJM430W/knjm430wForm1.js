...
function checkTerm(obj, number) {
    ...
    //補充指導点チェック
    var nam = obj.name;
    var lowScoreName = (nam.match(/SEM1_TERM_VALUE./)) ? "SEM1_INTR_VALUE" : "SEM2_INTR_VALUE";
    lowScoreObject = eval("document.forms[0][\"" + lowScoreName + "_" + number + "\"]");
    var lowScore = (lowScoreObject.value == '') ? 0 : parseInt(lowScoreObject.value);

    var score = parseInt(obj.value);
    if (score > 40) {
        alert('{rval MSG914}'+lowScore+'点～40点以内で入力してください。');
        obj.value = obj.defaultValue;
        return;
    }
    if (score < lowScore) {
