...
class knjmp983Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("sel", "POST", "knjmp983index.php", "", "sel");
        ...
        $flg = "";
        $opt = array();
        $result = $db->query(knjmp983Query::selectYearQuery());
        ...
        //業者年度一覧取得
        $opt_left = array();
        $result = $db->query(knjmp983Query::selectQuery($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //業者一覧取得
        $opt_right = array();
        $result = $db->query(knjmp983Query::selectTraderQuery($model));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        /****************/
        //extra
        $link = REQUESTROOT."/M/KNJMP983_2/knjmp983_2index.php?mode=1";
        ...
        $arg["button"] = array("BTN_MASTER" => knjCreateBtn($objForm, "btn_master", "業者マスタ", $extra_mst),
