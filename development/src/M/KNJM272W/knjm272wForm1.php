...
class knjm272wForm1
{
    ...
    {
        $objForm        = new form;
        $arg["start"]   = $objForm->get_start("main", "POST", "knjm272windex.php", "", "main");

        //DB接続
        $db = Query::dbCheckOut();

        $query = knjm272wQuery::useCombinedDat();
        ...
        //抽出データ出力
        $schcnt = 0;
        $result = $db->query(knjm272wQuery::getSch($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            if ($model->useCombinedDat == "1") {
                $query = knjm272wQuery::getCombSubclassSaki($model, $row);
            ...
            $model->setdata["REPORTNO"][$schcnt] = $row["SCHREGNO"].$Row["REPORTNO"];
        ...
        View::toHTML($model, "knjm272wForm1.html", $arg); 
    }
}
//makeCmb
