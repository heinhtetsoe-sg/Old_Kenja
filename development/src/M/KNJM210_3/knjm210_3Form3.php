...
//        $model->schregno = "20031935";
        $Row         = $db->getRow(knjm210_3Query::getSchregno_name($model->schregno),DB_FETCHMODE_ASSOC);
        $arg["NAME"] = "　　　学籍番号：".$Row["SCHREGNO"]."　　　氏名：".$Row["NAME"];

        //学籍住所データよりデータを取得
        if($model->schregno)
        {
            $result = $db->query(knjm210_3Query::getAddress_all($model->schregno));
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
            {
                $name1 = $db->getOne(knjm210_3Query::List_AreaCd($row["AREACD"]));
        ...
        //保護者情報の取得
        $row = $db->getRow(knjm210_3Query::getGuardianAddr($model->schregno),DB_FETCHMODE_ASSOC);
        ...
        $result = $db->query(knjm210_3Query::get_name_mst());
        $opt = array();
        ...
        View::toHTML($model, "knjm210_3Form3.html", $arg);
    }
}
?>
