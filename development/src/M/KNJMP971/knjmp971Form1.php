...
class knjmp971Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjmp971Form1", "POST", "knjmp971index.php", "", "knjmp971Form1");
        ...
        //学期コンボ
        $query = knjmp971Query::getSemester();
        $extra = "";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //学年コンボ
        $query = knjmp971Query::getGrade();
        $extra = "";
        ...
        View::toHTML($model, "knjmp971Form1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    $query = knjmp971Query::getSchregData($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeBtn(&$objForm, &$arg) {
    //読込ボタン
    $extra = "onclick=\"return btn_submit('knjmp971');\"";
...
function makeHidden(&$objForm, $model, &$arg) {
    ...
    $arg["TOP"]["PRGID"] = knjCreateHidden($objForm, "PRGID", "KNJMP971");
