...
class knjm274wForm1
{
    ...
    {
        $objForm        = new form;
        $arg["start"]   = $objForm->get_start("main", "POST", "knjm274windex.php", "", "main");

        //DB接続
        $db = Query::dbCheckOut();

        $query = knjm274wQuery::useCombinedDat();
        ...
        //抽出データ出力
        $schcnt = 0;
        $result = $db->query(knjm274wQuery::getSch($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            //レポート番号
            $model->setdata["REPORTNO"][$schcnt] = $row["SCHREGNO"].substr(CTRL_YEAR,3,1).$row["CURRICULUM_CD"].$row["SUBCLASSCD"].sprintf("%02d",$row["STANDARD_SEQ"]).sprintf("%02d",$row["REPRESENT_SEQ"]);
            if ($model->useCombinedDat == "1") {
                $query = knjm274wQuery::getCombSubclassSaki($model, $row);
            ...
            //登録日付
            $grad_d = str_replace("-", "/", $row["GRAD_INPUT_DATE"]);
            if($grad_d) $grad_d .= " ";
            $grad_d .= $row["GRAD_TIME"];
            $Row["GRAD_D"] = $grad_d;
        ...
        View::toHTML($model, "knjm274wForm1.html", $arg); 
    }
}
//makeCmb
