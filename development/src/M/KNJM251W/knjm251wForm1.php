...
class knjm251wForm1
{
    ...
    {
        $objForm      = new form;
        $arg["start"] = $objForm->get_start("list", "POST", "knjm251windex.php", "", "list");
        ...
        View::toHTML($model, "knjm251wForm1.html", $arg); 
    }
}
//権限チェック
...
{
    $opt = array();
    $result = $db->query(knjm251wQuery::GetClass($model));
...
function makeSubclassList(&$arg, $db, $model)
{
    ...
    $result = $db->query(knjm251wQuery::GetSubClass($model, $model->classcd, "MAIN"));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $mainData = "<tr> ";
        ...
        $link = View::alink("knjm251windex.php",
                            $row["VALUE"],
                            "target=right_frame",
                            array("cmd"        => "sel",
                                  "VALUE" => $row["VALUE"]));
        ...
        $cnt = 0;
        $attendData = "";
        $attendsub = $db->query(knjm251wQuery::GetSubClass($model, $row["VALUE"]));
