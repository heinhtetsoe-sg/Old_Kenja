...
class knjm828Form1
{
    function main(&$model){
        ...
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("knjm828Form1", "POST", "knjm828index.php", "", "knjm828Form1");
        ...
        $extra = "onChange=\"return btn_submit('knjm828');\"";
        $arg["data"]["YEAR"] = knjCreateCombo($objForm, "YEAR", $model->field["YEAR"], $opt, $extra, 1);

        //学期
        $opt = array();
        $query = knjm828Query::getSemeMst($model->field["YEAR"]);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        View::toHTML($model, "knjm828Form1.html", $arg); 
    }
}
...
function makeHidden(&$objForm)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJM828");
