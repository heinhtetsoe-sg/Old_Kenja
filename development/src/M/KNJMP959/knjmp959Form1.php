...
class knjmp959Form1
{
    function main(&$model){
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjmp959Form1", "POST", "knjmp959index.php", "", "knjmp959Form1");
        ...
        foreach ($opt as $key => $val) {
            array_push($extra, " id=\"OUTPUT{$val}\" onClick=\" return btn_submit('knjmp959')\"");
        }
        ...
        if ($model->output != 1) {
            ...
            $result = $db->query(knjmp959Query::GetGrade());
            ...
            $extra = "onchange =\" return btn_submit('knjmp959');\"";
        ...
        /*------------*/
        $row1 = array();
        if ($model->output == 1) $query = knjmp959Query::GetGrade();
        if ($model->output == 2) $query = knjmp959Query::GetClass($model->grade);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJMP959");
        ...
        View::toHTML($model, "knjmp959Form1.html", $arg);
    }
}
?>
