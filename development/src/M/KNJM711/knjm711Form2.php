...
class knjm711Form2
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjm711index.php", "", "edit");

        //警告メッセージを表示しない場合
        if(!isset($model->warning)){
            $Row = knjm711Query::getRow($model,1);
        }else{
            $Row =& $model->field;
        }
        ...
        $opt = array();
        $value_flg = false;
        $query = knjm711Query::getColectCd($model->year);
        ...
        //詳細登録状況取得
        $query = knjm711Query::getRowShousai($model);
        $result = $db->query($query);
        ...
        //詳細登録ボタン
        $model->prgId = 'KNJM711';
        $subdata  = "wopen('".REQUESTROOT."/M/KNJM711_DETAIL/knjm711_detailindex.php?cmd=main";
        ...
        if (VARS::get("cmd") != "edit" || VARS::get("cmd") != "change2"){
            $arg["reload"]  = "window.open('knjm711index.php?cmd=list','left_frame');";
        }
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。 
        View::toHTML($model, "knjm711Form2.html", $arg);
    }
}
?>
