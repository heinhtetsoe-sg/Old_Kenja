...
class knjm836wForm1
{
    public function main(&$model)
    {
        ...
        $objForm = new form();
        $arg["start"]   = $objForm->get_start("knjm836wForm1", "POST", "knjm836windex.php", "", "knjm836wForm1");
        ...
        $extra = "onChange=\"return btn_submit('knjm836w');\"";
        $arg["data"]["YEAR"] = knjCreateCombo($objForm, "YEAR", $model->field["YEAR"], $opt, $extra, 1);

        //学期
        $opt = array();
        $query = knjm836wQuery::getSemeMst($model->field["YEAR"]);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        //テスト種別
        $opt = array();
        $query = knjm836wQuery::getTestcd($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        View::toHTML($model, "knjm836wForm1.html", $arg);
    }
}
...
function makeHidden(&$objForm)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJM836W");
