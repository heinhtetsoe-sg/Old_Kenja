...
class knjm500kForm1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjm500kForm1", "POST", "knjm500kindex.php", "", "knjm500kForm1");
        ...
        //学期コンボ作成
        $query = knjm500kQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjm500k');\"";
        ...
        //対象学期の開始日・終了日
        $sDate = $db->getOne(knjm500kQuery::getSemesterDate("SDATE",$model->field["SEMESTER"]));
        $eDate = $db->getOne(knjm500kQuery::getSemesterDate("EDATE",$model->field["SEMESTER"]));
        knjCreateHidden($objForm, "SDATE", str_replace("-","/",$sDate));
        knjCreateHidden($objForm, "EDATE", str_replace("-","/",$eDate));

        //学年コンボ
        $query = knjm500kQuery::getGrade($model);
        $extra = "onChange=\"return btn_submit('knjm500k');\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        //クラスコンボ作成
        $query = knjm500kQuery::getHrClass(CTRL_YEAR, $seme, $model);
        $extra = "onchange=\"return btn_submit('knjm500k');\"";
        ...
        View::toHTML($model, "knjm500kForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model, $seme) {
    ...
    //対象外の生徒取得
    $query = knjm500kQuery::getSchnoIdou($model, $seme);
    ...
    //対象者リストを作成する
    $query = knjm500kQuery::getStudent($model, $seme);
    $result = $db->query($query);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJM500K");
