...
class knjmp983_2Form2 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjmp983_2index.php", "", "edit");

        //警告メッセージを表示しない場合
        if (!isset($model->warning) && $model->cmd !== 'edit') {
            $Row = knjmp983_2Query::getRow($model->trader_cd);
        } else {
            $Row =& $model->field;
        }
        ...
        //支払方法
        $extra = "";
        $query = knjmp983_2Query::getNameMst("G217");
        ...
        //戻るボタン
        $link = REQUESTROOT."/M/KNJMP983/knjmp983index.php";
        ...
        if (VARS::get("cmd") != "edit"){
            $arg["reload"] = "parent.left_frame.location.href='knjmp983_2index.php?cmd=list';";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjmp983_2Form2.html", $arg); 
    }
}
//コンボ作成
