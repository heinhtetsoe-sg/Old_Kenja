...
class knjm273wForm1
{
    ...
    {
        $objForm        = new form;
        $arg["start"]   = $objForm->get_start("main", "POST", "knjm273windex.php", "", "main");

        //DB接続
        $db = Query::dbCheckOut();

        $query = knjm273wQuery::useCombinedDat();
        ...
        //添削者
        $opt_staf = array();
        $result = $db->query(knjm273wQuery::selectStaff($model));
        ...
        //評価
        $query = knjm273wQuery::getsubName($model);
        ...
        //抽出データ出力
        $schcnt = 0;
        $result = $db->query(knjm273wQuery::getSch($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            if ($model->useCombinedDat == "1") {
                $query = knjm273wQuery::getCombSubclassSaki($model, $row);
            ...
            $subdata = "loadwindow('knjm273windex.php?cmd=subform1&GRAD_VALUESUB={$row["GRAD_VALUE"]}&SCHREGNOSUB={$row["SCHREGNO"]}&SUBCLASSSUB={$linkSubclass}&STSEQSUB={$row["STANDARD_SEQ"]}&RSEQSUB={$row["REPRESENT_SEQ"]}&DATESUB={$model->Date}&CHAIRSUB={$row["CHAIRCD"]}&RECDAYSUB={$row["RECEIPT_DATE"]}',500,200,350,300)";
        ...
        View::toHTML($model, "knjm273wForm1.html", $arg); 
    }
}
//makeCmb
