...
class knjmp713Form1
{
    ...
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjmp713index.php", "", "edit");

        $db = Query::dbCheckOut();

        //年度コンボ
        $query = knjmp713Query::getYear();
        $extra = "onChange=\" return btn_submit('changeYear')\"";
        makeCmb($objForm, $arg, $db, $query, $model->taisyouYear, "TAISYOU_YEAR", $extra, 1);

        //学年コンボ
        $query = knjmp713Query::getGrade($model);
        $extra = "onChange=\" return btn_submit('changeYear')\"";
        makeCmb($objForm, $arg, $db, $query, $model->taisyouGrade, "TAISYOU_GRADE", $extra, 1);

        //コピー用データチェック 登録
        $query = knjmp713Query::selectYearQuery($model->taisyouYear, $model->taisyouGrade, "COLLECT_GRP_MST");
        $year_flg  = $db->getOne($query);
        $query = knjmp713Query::selectYearQuery($model->taisyouYear, $model->taisyouGrade, "COLLECT_GRP_DAT");
        $year_flg += $db->getOne($query);

        //コピー用マスタチェック(前年度に登録されている項目が今年度に存在するか)
        $mst_flg = "";
        $result = $db->query(knjmp713Query::mstMcdCheck($model));
        ...
        if ($mst_flg == "") {
            $result = $db->query(knjmp713Query::mstScdCheck($model));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //会計グループリスト取得
        $query = knjmp713Query::SelectQuery($model);
        ...
        View::toHTML($model, "knjmp713Form1.html", $arg);
    }
    
    //表示用配列にデータ値をセット
    function setDataRow($row)
    {
        $this->DataRow["COLLECT_GRP_CD"][] = View::alink("knjmp713index.php", $row["COLLECT_GRP_CD"].":".$row["COLLECT_GRP_NAME"]."({$row["GRADE_NAME1"]} {$row["MAJORNAME"]} {$row["COURSECODENAME"]})", "target=right_frame",
