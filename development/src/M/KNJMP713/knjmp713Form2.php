...
class knjmp713Form2
{
    function main(&$model)
    {
        ...
        $arg["start"] = $objForm->get_start("edit", "POST", "knjmp713index.php", "", "edit");

        $db = Query::dbCheckOut();

        //警告メッセージが表示される場合、またはリロードした場合
        if ($model->cmd == "grpChange") {
            $query = knjmp713Query::getGrpData($model->taisyouYear, $model);
        ...
        if (isset($model->warning) || $model->cmd == "reload") {
            ...
            $query = knjmp713Query::ReloadSelectMcd($model);
        } else {
            $query = knjmp713Query::GetSelectMcd($model);
        }
        ...
        //会計項目一覧
        $result = $db->query(knjmp713Query::GetMcd($model, $tempcd));
        ...
        if (isset($model->warning) || $model->cmd == "reload") {
            //会計項目割当一覧の値が変更してリロードした時
            $query = knjmp713Query::ReloadSelectScd($model);
        } else {
            $query = knjmp713Query::GetSelectScd($model);
        }
        $result = $db->query($query);
        ...
        //会計細目一覧
        $result = $db->query(knjmp713Query::GetScd($model, $tempcd, $tempcd2));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $label = $row["COLLECT_L_CD"].$row["COLLECT_M_CD"].$row["COLLECT_S_CD"]."：".htmlspecialchars($row["COLLECT_S_NAME"]);
            $val = $row["COLLECT_L_CD"].$row["COLLECT_M_CD"].$row["COLLECT_S_CD"];
        ...
        if (VARS::get("cmd") != "edit") {
            $arg["jscript"] = "window.open('knjmp713index.php?cmd=list','left_frame');";
        }

        $arg["finish"]  = $objForm->get_finish();

        View::toHTML($model, "knjmp713Form2.html", $arg);
    }
