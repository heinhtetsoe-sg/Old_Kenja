...
class knjm702dForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjm702dForm1", "POST", "knjm702dindex.php", "", "knjm702dForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        View::toHTML($model, "knjm702dForm1.html", $arg);
    }
}
/****************************************************** 以下関数 ************************************************************/
//コンボ作成
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    //クラス一覧
    $opt = array();
    $query = knjm702dQuery::getHrClass();
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $opt[] = array('label' => $row["LABEL"],
                       'value' => $row["VALUE"]);
...
function makeHidden(&$objForm) {
    ...
    knjCreateHidden($objForm, "PRGID",         "KNJM702D");
}
?>
