...
class knjm834Form1
{
    function main(&$model){
        ...
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("knjm834Form1", "POST", "knjm834index.php", "", "knjm834Form1");
        ...
        //クラス
        $query = knjm834Query::getAuth(CTRL_YEAR, $model->field["GAKKI"]);
        $class_flg = false;
        $row1 = array();
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        $extra = "onchange=\"return btn_submit('knjm834');\"";
        ...
        $query = knjm834Query::getSchno($model, CTRL_YEAR, $model->field["GAKKI"]);
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        View::toHTML($model, "knjm834Form1.html", $arg); 
    }
}
...
function makeHidden(&$objForm)
{
    ...
    $objForm->ae(createHiddenAe("PRGID", "KNJM834"));
