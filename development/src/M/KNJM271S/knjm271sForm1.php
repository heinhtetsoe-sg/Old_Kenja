...
class knjm271sForm1
{
    ...
    {
        $objForm        = new form;
        $arg["start"]   = $objForm->get_start("main", "POST", "knjm271sindex.php", "", "main");
        ...
        //添削者
        $opt_staf = array();
        $result = $db->query(knjm271sQuery::selectStaff($model));
        ...
        //抽出データ出力
        $schcnt = 0;
        $result = $db->query(knjm271sQuery::getSch($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            //レポート番号
            $model->setdata["REPORTNO"][$schcnt] = substr(CTRL_YEAR,3,1).$row["CURRICULUM_CD"].$row["SUBCLASSCD"].sprintf("%02d",$row["STANDARD_SEQ"]);
            $model->setdata["STANDARD_SEQ"][$schcnt] = sprintf("%02d",$row["STANDARD_SEQ"]);
            ...
            knjCreateHidden($objForm, "STANDARD_SEQ".$schcnt, $model->setdata["STANDARD_SEQ"][$schcnt]);
            knjCreateHidden($objForm, "REPRESENT_SEQ".$schcnt, $row["REPRESENT_SEQ"]);
            ...
            $subdata = "loadwindow('knjm271sindex.php?cmd=subform1&GRAD_VALUESUB={$row["GRAD_VALUE"]}&SCHREGNOSUB={$row["SCHREGNO"]}&SUBCLASSSUB={$linkSubclass}&STSEQSUB={$row["STANDARD_SEQ"]}&RSEQSUB={$row["REPRESENT_SEQ"]}&DATESUB={$model->Date}&CHAIRSUB={$row["CHAIRCD"]}&RECDAYSUB={$row["RECEIPT_DATE"]}',500,200,350,300)";
        ...
        View::toHTML($model, "knjm271sForm1.html", $arg); 
    }
}
//makeCmb
