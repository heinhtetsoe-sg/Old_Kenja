...
class knjmp736Form1
{
    function main(&$model) {
        $objForm      = new form;
        $arg["start"] = $objForm->get_start("edit", "POST", "knjmp736index.php", "", "edit");

        //DB接続
        $db = Query::dbCheckOut();

        //処理年度コンボ
        $query = knjmp736Query::getYear($model);
        ...
        View::toHTML($model, "knjmp736Form1.html", $arg);
    }
}
...
{
    $info = $db->getRow(knjmp736Query::getStudentName($model), DB_FETCHMODE_ASSOC);
    if (is_array($info)) {
        foreach ($info as $key => $val) {
            $setRow[$key] = $val;
        }
    }
    $query = knjmp736Query::getSlipData($model);
    ...
    //総請求データ
    $query = knjmp736Query::getSlipDataALL($model);
...
{
    //商品の単価をHiddenに持つ
    $query = knjmp736Query::getCollectM($model);
    ...
    $query = knjmp736Query::getMeisaiData($model);
