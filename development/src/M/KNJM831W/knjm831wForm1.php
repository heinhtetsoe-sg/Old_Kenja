...
class knjm831wForm1
{
    function main(&$model){
        ...
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("knjm831wForm1", "POST", "knjm831windex.php", "", "knjm831wForm1");
        ...
        //学期
        $opt = array();
        $query = knjm831wQuery::getSemeMst();
        ...
        //教科
        $opt = array();
        $query = knjm831wQuery::getClass($model->field["GAKKI"]);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        $extra = "onChange=\"return btn_submit('knjm831w');\"";
        ...
        $query = knjm831wQuery::getSubclass($model, $model->field["GAKKI"], $model->field["CLASSCD_SET"]);
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        View::toHTML($model, "knjm831wForm1.html", $arg); 
    }
}
...
function makeHidden(&$objForm)
{
    ...
    $objForm->ae(createHiddenAe("PRGID", "KNJM831W"));
