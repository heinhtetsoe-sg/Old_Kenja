...
class knjmp930_mainForm1 {
    function main(&$model) {
        //オブジェクト作成
        $objForm        = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjmp930_mainindex.php", "", "main");
        
        if (!isset($model->warning) && $model->getAuth != "" && $model->cmd !== 'edit') {
            $Row = knjmp930_mainQuery::getRow($model);
        } else {
            $Row =& $model->field;
        }
        ...
        $model->getHonjimeCount = $db->getOne(knjmp930_mainQuery::getCloseFlgData($model));

        //支出伺の状況を取得(比較時に利用)
        if ($model->getRequestNo) {
            $model->getApproval = $db->getOne(knjmp930_mainQuery::getOutgoData($model, "APPROVAL"));
            $model->getCancel   = $db->getOne(knjmp930_mainQuery::getOutgoData($model, "CANCEL"));
            //支出伺画面から遷移してきていない場合
            if(!$model->getOutgoRequestNo) {
                $model->getOutgoRequestNo = $db->getOne(knjmp930_mainQuery::getOutgoData($model, "REQUEST_NO"));
            }
        } else {
            $model->getApproval = "";
            $model->getCancel   = "";
        ...
        $query = knjmp930_mainQuery::getLevyMDiv();
        ...
        //起案者
        $extra = "";
        $query = knjmp930_mainQuery::getRequestStaff($model);
        ...
        View::toHTML($model, "knjmp930_mainForm1.html", $arg); 
    }
}
//コンボ作成
...
function makeBtn(&$objForm, &$arg, $model, $Row) {
    ...
    if ($model->getRequestNo == "") {
        ...
        $subdata .= "&SEND_PRGID=KNJMP930_MAIN&SEND_OUTGO_L_CD={$Row["SEKOU_L_CD"]}&SEND_OUTGO_M_CD={$Row["SEKOU_M_CD"]}";
...
function makeHidden(&$objForm, $model, $Row) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJMP954");
