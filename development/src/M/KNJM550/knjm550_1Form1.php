...
class knjm550_1Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjm550_1Form1", "POST", "knjm550index.php", "", "knjm550_1Form1");
        ...
        if (($model->cmd == "knjm550_1Search" || $model->cmd == "read" || $model->cmd == "meibo") &&
        ...
        //レポート集計基準日付データ
        if ($model->field["RKIJUN"] == "") {
            $model->field["RKIJUN"] = str_replace("-", "/", CTRL_DATE);
        }
        $arg["data"]["RKIJUN"] = View::popUpCalendar($objForm, "RKIJUN", str_replace("-", "/", $model->field["RKIJUN"]));

        //スクーリング集計基準日付データ
        if ($model->field["SKIJUN"] == "") {
            $model->field["SKIJUN"] = str_replace("-", "/", CTRL_DATE);
        }
        ...
        if (!isset($model->warning) && ($model->cmd == 'read' || $model->cmd == 'meibo')) {
            $setCmd = $model->cmd == 'read' ? "KNJM500" : "KNJM550";
            $model->cmd = 'knjm550_1';
        ...
        View::toHTML($model, "knjm550_1Form1.html", $arg);
    }
}
