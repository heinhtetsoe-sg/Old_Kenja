...
class knjm505dForm1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjm505dForm1", "POST", "knjm505dindex.php", "", "knjm505dForm1");
        ...
        foreach($opt as $key => $val) {
            array_push($extra, " id=\"CHOICE{$val}\" onClick=\"btn_submit('knjm505d')\"");
        }
        $radioArray = knjCreateRadio($objForm, "CHOICE", $model->field["CHOICE"], $extra, $opt, get_count($opt));
        foreach($radioArray as $key => $val) $arg["data"][$key] = $val;

        //学期コンボボックス
        $extra = "onchange=\"return btn_submit('knjm505d');\"";
        $query = knjm505dQuery::getSemester($model);
        ...
        if ($model->field["CHOICE"] == "2") {
            ...
            //クラスコンボボックス
            $extra = "onChange=\"return btn_submit('knjm505d');\"";
            $query = knjm505dQuery::getAuthClass($model);
        ...
        View::toHTML($model, "knjm505dForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    if ($model->field["CHOICE"] == "1") {
        ...
        $arg["CHANGENAME"] = "生徒";
        $query = knjm505dQuery::getAuthStudent($model);
    }
...
{
        knjCreateHidden($objForm, "PRGID", "KNJM505D");
