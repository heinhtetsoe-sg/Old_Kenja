...
class knjm371Form1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjm371Form1", "POST", "knjm371index.php", "", "knjm371Form1");
        ...
        if ($model->field["OUTPUT"] == "1") {
            $arg["student"] = 1;
            $query = knjm371Query::getHrClass($model);
        } else {
            $arg["hr_class"] = 2;
            $query = knjm371Query::getGrade($model);
        }
        $row1 = array();
        ...
        $extra = "onchange=\"return btn_submit('knjm371'),AllClearList();\"";
        ...
        View::toHTML($model, "knjm371Form1.html", $arg); 
    }
}
//リストToリスト作成
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    if ($disable == "1"){
        $query = knjm371Query::getStudentList($model, 'list', $selectdata);
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        //生徒一覧リストを作成する
        if($selectdata){
            $query = knjm371Query::getStudentList($model, 'select', $selectdata);
            $result = $db->query($query);
            $opt1 = array();
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                    $opt1[] = array('label' => $row["LABEL" ],
        ...
        $query = knjm371Query::getHrClassAuth($model, 'list', $selectdata);
        $result = $db->query($query);
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        if($selectdata){
            $query = knjm371Query::getHrClassAuth($model, 'select', $selectdata);
            $result = $db->query($query);
            $opt1 = array();
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                    if (substr($row["VALUE"],0,2) != $model->field["GRADE_HR_CLASS"]) continue;
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJM371");
