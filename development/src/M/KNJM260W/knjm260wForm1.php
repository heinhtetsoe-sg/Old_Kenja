...
class knjm260wForm1
{
    ...
    {
        $objForm        = new form;
        $arg["start"]   = $objForm->get_start("main", "POST", "knjm260windex.php", "", "main");

        //DB接続
        $db = Query::dbCheckOut();

        $query = knjm260wQuery::useCombinedDat();
        ...
        //抽出データ出力
        $schcnt = 0;
        $result = $db->query(knjm260wQuery::getSch($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            if ($model->useCombinedDat == "1") {
                $query = knjm260wQuery::getCombSubclassSaki($model, $row);
        ...
        View::toHTML($model, "knjm260wForm1.html", $arg); 
    }
}
//makeCmb
