...
class knjm736Form1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjm736index.php", "", "edit");

        //生徒の名前を取得
        $row = $db->getRow(knjm736Query::getStudentName($model),DB_FETCHMODE_ASSOC);
        $arg["NAME"] = $row["SCHREGNO"]."　".$row["NAME"];
        $risyuuCnt = $db->getOne(knjm736Query::getRisyuuCnt($model));
        $zanMusyouKaisu = $row["MUSYOU_KAISU"] < $risyuuCnt ? 0 : $row["MUSYOU_KAISU"] - $risyuuCnt;
        $arg["MUSYOU_KAISU"] = $zanMusyouKaisu;

        //リスト取得
        $result = $db->query(knjm736Query::getList($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            array_walk($row, "htmlspecialchars_array");
            //リンク設定
            $row["COLLECT_M_CD"] = View::alink("knjm736index.php", $row["COLLECT_GRP_CD"].":".$row["COLLECT_L_CD"].$row["COLLECT_M_CD"], "target=bottom_frame",
            ...
            if (strlen($row["PAID_MONEY_DIV"])) {
                $row["PAID_MONEY_DIV"] = $db->getOne(knjm736Query::getName($model->year, "G205", $row["PAID_MONEY_DIV"]));
            }
        ...
        View::toHTML($model, "knjm736Form1.html", $arg);
    }
}
?>
