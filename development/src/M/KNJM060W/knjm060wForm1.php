...
class knjm060wForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjm060wForm1", "POST", "knjm060windex.php", "", "knjm060wForm1");

        //DB接続
        $db = Query::dbCheckOut();

        //親画面なし
        $securityCnt = $db->getOne(knjm060wQuery::getSecurityHigh());
        //セキュリティーチェック
        ...
        //年度コンボを作成する
        $query = knjm060wQuery::GetYear($model);
        $extra = "onchange = \"return btn_submit('knjm060w')\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["YEAR"], "YEAR", $extra, 1, "");

        //科目コンボを作成する
        $query = knjm060wQuery::GetSubClass($model);
        $extra = "onchange = \"return btn_submit('knjm060w')\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["SUBCLASSNAME"], "SUBCLASSNAME", $extra, 1, "ALL");

        //テストコンボを作成する
        $query = knjm060wQuery::getTest($model);
        $extra = "onchange = \"return btn_submit('knjm060w')\"";
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjm060wQuery::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJM060W");
        ...
        View::toHTML($model, "knjm060wForm1.html", $arg); 
