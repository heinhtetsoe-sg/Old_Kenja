...
class knjz670Form1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjz670index.php", "", "edit");
...
if($arg["HYOUZI"] == 1){        
        ...
        if($model->req_field["ANALYSIS"] != "2"){
            $order = array("MOCKNM", "偏差値", "60～", "50", "40");
            $maxhensa = 60;
            $minhensa = 30;
        }else{
            $order = array("MOCKNM", "偏差値", "70～", "60", "50");
            $maxhensa = 70;
            $minhensa = 40;
        }
        
        //模試種別
        $syubetuQuery = knjz670Query::getSyubetu($model->req_field["GYOUSYA"]);
        ...
        //チェックボックス
        //教科名取得
        $kyoukaQuery = knjz670Query::getKyouka($model->req_field["YEAR"],$model->req_field["GYOUSYA"], $model->bottom_field["SYUBETU"], $model->req_field["KATA"], $minhensa);
        ...
        foreach($model->bottom_field["CHECK"] as $chkkey => $chkval){
            $kyoukasum = array();
            //模試取得したい
            $mosiQuery = knjz670Query::getMosi($model->req_field["YEAR"], $gyousya, $model->bottom_field["SYUBETU"], $chkval,"","",$minhensa);
            $mosiResult = $db->query($mosiQuery);
            $kamoku = array();
            while($mosiRow = $mosiResult->fetchRow(DB_FETCHMODE_ASSOC)){
                //模試の表示順に配列に入れる
                $mockorder[$chkval][] = $mosiRow["MOCKCD"];
                $kamoku[$mosiRow["MOCKCD"]][$order[0]] = $mosiRow["GRADE"]."年<BR>".$mosiRow["MOCKNAME1"];
            }
            
            //偏差値ごとに人数取得
            for($i=$maxhensa;$i>$minhensa;$i=$i-10){
                $from = $i;
                if($i != $maxhensa){
                    $to = $i+10;
                }else{
                ...
                $cntQuery = knjz670Query::getMosi($model->req_field["YEAR"], $gyousya, $model->bottom_field["SYUBETU"], $chkval, $from, $to);
                ...
                while($cntRow = $cntResult->fetchRow(DB_FETCHMODE_ASSOC)){
                    ...
                    if($bfr != ""){
                        if($kamoku[$cntRow["MOCKCD"]][$bfr]["TOTAL"] != ""){
                            if($cntRow["COUNT"] != ""){
                                $kamoku[$cntRow["MOCKCD"]][$key]["TOTAL"] = $kamoku[$cntRow["MOCKCD"]][$bfr]["TOTAL"] + $cntRow["COUNT"];
                            }else{
            ...
            //平均偏差値取得
            $avgQuery = knjz670Query::getAverage($model->req_field["YEAR"], $gyousya, $model->bottom_field["SYUBETU"], $chkval);
        ...
        for($i=$maxhensa;$i>$minhensa;$i=$i-10){
            ...
            //模試名取得(ラベル)
            $mockQuery = knjz670Query::getMock($i,$gyousya, $model->bottom_field["SYUBETU"],$model->req_field["YEAR"],$choice,"1");
            ...
            while($mockRow = $mockResult->fetchRow(DB_FETCHMODE_ASSOC)){
                $model->hns_graph["label"] .= $lcnm.$mockRow["GRADE"]."年 ".$mockRow["MOCKNAME1"];
                $lcnm = ",";
            ...
            //偏差値ごとにデータ取得
            $dataQuery = knjz670Query::getMock($i,$gyousya, $model->bottom_field["SYUBETU"],$model->req_field["YEAR"],$choice,"2");
            ...
            foreach($model->bottom_field["CHECK"] as $chkkey => $chkval){
                ...
                foreach($roopMock as $mkey => $mval){
                    if(is_array($mdata[$chkval]) && array_key_exists($mval, $mdata[$chkval])){
                        ...
                        if(in_array($mval, $mockorder[$chkval])){
        ...
        //$arg["link"] = REQUESTROOT."/Z/KNJz670_2/knjz670_2index.php";
        Query::dbCheckIn($db);
        //ボタン作成
        ...
        //View::toHTML($model, "knjz670Form1.html", $arg);
        
        
        $jsplugin = "chart.js|Chart.min.js|graph.js";

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML5($model, "knjz670Form1.html", $arg, $jsplugin, $cssplugin);
    }
} 
//ボタン作成
