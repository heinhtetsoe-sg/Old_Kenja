...
class knjz710Form1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knjz710index.php", "", "edit");
        ...
        //参照年度コンボ
        /*$query = knjz710Query::getYear("COPYYEAR");
        $extra = "";
        ...
        //対象年度コンボ
        $query = knjz710Query::getYear("MOCKYEAR");
        ...
        if ($model->cmd == "changeMockyear") {
            $arg["reload"] = "window.open('knjz710index.php?cmd=edit', 'right_frame')";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。 
        View::toHTML($model, "knjz710Form1.html", $arg); 
    }
}
...
{
    $result = $db->query(knjz710Query::getList($model));
