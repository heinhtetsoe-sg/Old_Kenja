...
class knjz710Form2
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjz710index.php", "", "edit");
        ...
        if (!isset($model->warning) && $model->cmd != "kakutei") {
            $query = knjz710Query::getRow($model->mockyear, $model->mockcd, $model->grade);
        ...
        $cpYearQuery = knjz710Query::getCopyYear($mock);
        $extra = "";
        ...
        if ($model->cmd != "edit" && $model->cmd != "reset" && $model->cmd != "kakutei") {
            $arg["reload"]  = "parent.left_frame.location.href='knjz710index.php?cmd=list';";
        }
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjz710Form2.html", $arg); 
    }
}
...
/*function makeList(&$objForm, &$arg, $db, $model, $Row) {
    if (strlen($Row["FIELD_CNT"]) && $Row["FIELD_CNT"] > 0) {
        for ($field_no = 1; $field_no <= $Row["FIELD_CNT"]; $field_no++) {
            ...
            //警告メッセージを表示しない場合
            if (!isset($model->warning) && $model->cmd != "kakutei") {
                $query = knjz710Query::getMockSubclassCd($model->mockyear, $model->mockcd, $model->grade, $field_no);
            ...
            if ($model->companyCd == "00000001") {
                $query = knjz710Query::getMockSubclassTitleBene($model, $field_no);
            } else if ($model->companyCd == "00000002") {
                $query = knjz710Query::getMockSubclassTitleSundai($model, $field_no);
            } else {
                $query = knjz710Query::getMockSubclassTitleZkai($model, $field_no);
            }
            $setSubclassName = $db->getOne($query);
            $setRow["MOSI_SUBCLASS_NAME"] = $setSubclassName;
            //模試科目(賢者)
            $query = knjz710Query::getMockSubclassMst();
...
function makeList(&$objForm, &$arg, $db, $model, $Row) {
        ...
        //対象教科取得
        $kyoukaQuery = knjz710Query::getKyouka($model->mockyear, $model->mockcd, $model->grade);
