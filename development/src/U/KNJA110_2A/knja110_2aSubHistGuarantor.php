...
class knja110_2aSubHistGuarantor {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("sel", "POST", "knja110_2aindex.php", "", "sel");
        ...
        //履歴一覧
        $rirekiFlg = false;
        $query = knja110_2aQuery::getGuarantorHistDat($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if (!isset($model->warning) && $model->cmd != "changeCmb2") {
            if ($rirekiFlg && $model->hist_issuedate) {
                $query = knja110_2aQuery::getGuarantorHistDat($model, $model->hist_issuedate);
                $setRow = $db->getRow($query, DB_FETCHMODE_ASSOC);
            } else if (!$rirekiFlg) {
                $query = knja110_2aQuery::getGuardianDat($model, $model->schregno);
                $setRow = $db->getRow($query, DB_FETCHMODE_ASSOC);
            }
            //クリア
        ...
        $arg["data"][$id] .= " id='$id'";
        //性別コンボボックス
        $query = knja110_2aQuery::getNameMst("Z002");
        $extra = "";
        ...
        $arg["data"][$id] .= " id='$id'";
        //続柄コンボボックス
        $query = knja110_2aQuery::getNameMst("H201");
        $extra = "";
        ...
        //戻るボタン
        $link = REQUESTROOT."/U/KNJA110_2A/knja110_2aindex.php?cmd=back&ini2=1";
        ...
        View::toHTML($model, "knja110_2aSubHistGuarantor.html", $arg);
    }
}
