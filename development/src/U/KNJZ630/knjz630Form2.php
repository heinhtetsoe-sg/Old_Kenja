...
class knjz630Form2
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjz630index.php", "", "edit");
        ...
        $MockNameQuery = knjz630Query::getMockName($model->GAKUSEKI, $model->right_field["NENDO"], $model->right_field["GYOUSYA"]);
        $MockNameResult = $db->query($MockNameQuery);
        while($MockNameRow = $MockNameResult->fetchRow(DB_FETCHMODE_ASSOC)){
            $opt2[] = array("label" => $MockNameRow["MOCKNAME1"],
        ...
        if($model->right_field["MOCKCD"] != ""){
            ...
            //模試名称取得
            $mockQuery = knjz630Query::getMock($model->right_field["MOCKCD"]);
            ...
            //教科名取得
            $kyoukaQuery = knjz630Query::getKyouka($model->GAKUSEKI, $model->right_field["NENDO"], $model->right_field["MOCKCD"], $model->right_field["KATA"]);
        ...
        //$arg["link"] = REQUESTROOT."/Z/KNJz630_2/knjz630_2index.php";
        Query::dbCheckIn($db);
        //ボタン作成
        ...
        //View::toHTML($model, "knjz630Form1.html", $arg);
        
        
        $jsplugin = "chart.js|Chart.min.js|graph.js";

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML5($model, "knjz630Form2.html", $arg, $jsplugin, $cssplugin);
    }
} 
//ボタン作成
