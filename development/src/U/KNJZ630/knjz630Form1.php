...
class knjz630Form1
{
    function main(&$model)
    {
        ...
        $MockNameQuery = knjz630Query::getMockName($model->GAKUSEKI, $model->left_field["NENDO"], $model->left_field["GYOUSYA"]);
        ...
        if($model->left_field["MOCKCD"] != ""){
            ...
            //模試名称取得
            $mockQuery = knjz630Query::getMock($model->left_field["MOCKCD"]);
            ...
            //教科名取得
            $kyoukaQuery = knjz630Query::getKyouka($model->GAKUSEKI, $model->left_field["NENDO"], $model->left_field["MOCKCD"], $model->left_field["KATA"]);
        ...
        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML5($model, "knjz630Form1.html", $arg, $jsplugin, $cssplugin);
    }
} 
//ボタン作成
