...
class knjx_z069Form1
{
    public function main(&$model)
    {
        ...
        //年度一覧コンボボックス
        $query = knjx_z069query::getYearSemester();
        $extra = "onchange=\"return btn_submit('');\"";
        makeCmb($objForm, $arg, $db, $query, "YEAR", $model->field["YEAR"], $extra, 1);

        //学年一覧コンボボックス
        $query = knjx_z069query::getGrade($model);
        $extra = "onchange=\"return btn_submit('');\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1, "ALL");

        //科目一覧コンボボックス
        $query = knjx_z069query::getSubclass($model);
        makeCmb($objForm, $arg, $db, $query, "SUBCLASS", $model->field["SUBCLASS"], "", 1, "ALL");
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx_z069index.php", "", "main");
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjx_z069Form1.html", $arg);
    }
}
