...
class knjx_g081Form1
{
    public function main(&$model)
    {
        ...
        //年度一覧コンボボックス
        $query = knjx_g081query::getSelectFieldSQL();
        $extra = "onchange=\"return btn_submit('');\"";
        makeCmb($objForm, $arg, $db, $query, "YEAR", $model->field["YEAR"], $extra, 1);

        //年組一覧コンボボックス
        $query = knjx_g081query::getSelectFieldSQL2($model);
        $extra = "onchange=\"return btn_submit('');\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE_HR_CLASS", $model->field["GRADE_HR_CLASS"], $extra, 1);

        //生徒一覧コンボボックス
        $query = knjx_g081query::getSelectFieldSQL3($model);
        makeCmb($objForm, $arg, $db, $query, "SCHREGNO", $model->field["SCHREGNO"], "", 1);
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx_g081index.php", "", "main");
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjx_g081Form1.html", $arg);
    }
}
