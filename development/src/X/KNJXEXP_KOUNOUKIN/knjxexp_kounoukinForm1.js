function btn_submit(cmd) {
    if (cmd == "sendSearch") {
        //親画面から送られてきた検索条件を画面上にセットし、後は通常の検索を行なう
        var sendSchregno = document.forms[0].SEND_SCHREGNO.value;
        document.forms[0].SCHREGNO.value = sendSchregno;
        cmd = "search";
    }
    if (cmd == "search") {
        var flg = false;
        for (var i = 0; i < document.forms[0].elements.length; i++) {
            ...
            if (e.type == "checkbox" && e.checked && (e.name == "CONTACT_EXISTS_CHK" || e.name == "EXCEPT_CONTACT_END_CHK")) {
                flg = true;
                break;
            }
        }
