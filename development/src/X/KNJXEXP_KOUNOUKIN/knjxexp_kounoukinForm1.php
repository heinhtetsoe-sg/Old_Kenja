...
class knjxexp_kounoukinForm1
{
    public function main(&$model)
    {
        ...
        //検索種別(KNJP850用)
        if ($model->dispData["search_div_KNJP850"] == "1") {
            makeSearchDivP850($objForm, $arg, $model);
        }
        ...
        } elseif ($model->cmd == "sendSearch") {
            //親画面からこの画面を呼び出す際に、パラメータで渡した条件による検索を行なった状態で開く
            knjCreateHidden($objForm, "SEND_SCHREGNO", $model->sendSchregno);
            $arg["jscript"] = "btn_submit('sendSearch')";
        }
        ...
        View::toHTML($model, "knjxexp_kounoukinForm1.html", $arg);
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model)
{
    if ($model->search["HR_CLASS_HYOUJI_FLG"] == "1" || $model->dispData["sort"] == "1") {
        $arg["KOUMOKU1"] = 'クラス - 番';
        $arg["KOUMOKU2"] = '学籍番号';
        $arg["KOUMOKU2_WIDTH"] = '60px';
    } else {
        $arg["KOUMOKU1"] = '学籍番号';
        $arg["KOUMOKU2"] = 'クラス - 番';
        $arg["KOUMOKU2_WIDTH"] = '100px';
    }

    //テーブルカラムチェック
    $columnCnt = $db->getOne(knjxexp_kounoukinQuery::checkTableColumn("FRESHMAN_DAT", "GRADE"));
    $model->isFreshmanGrade = "";
    if ($columnCnt > 0) {
        $model->isFreshmanGrade = "1";
    }

    $query = knjxexp_kounoukinQuery::GetStudents($model);
    $result = $db->query($query);
...
//KNJP850用の検索種別フォームを作成
function makeSearchDivP850(&$objForm, &$arg, &$model)
{
    //メモ保有者チェック
    $extra = " id=\"CONTACT_EXISTS_CHK\" ";
    $checked = $model->search["CONTACT_EXISTS_CHK"] == "1" ? " checked " : "";
    $arg["CONTACT_EXISTS_CHK"] = knjCreateCheckBox($objForm, "CONTACT_EXISTS_CHK", "1", $extra.$checked);

    //対応完了除外チェック
    $extra = " id=\"EXCEPT_CONTACT_END_CHK\" ";
    $checked = $model->search["EXCEPT_CONTACT_END_CHK"] == "1" ? " checked " : "";
    $arg["EXCEPT_CONTACT_END_CHK"] = knjCreateCheckBox($objForm, "EXCEPT_CONTACT_END_CHK", "1", $extra.$checked);
}
