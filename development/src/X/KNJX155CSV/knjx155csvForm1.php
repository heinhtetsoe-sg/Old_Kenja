...
class knjx155csvForm1
{
    public function main(&$model)
    {
        ...
        //年度一覧コンボボックス
        $result     = $db->query(knjx155csvquery::getSelectFieldSQL());
        $opt_year   = array();
        ...
        //テスト名コンボ
        $query = knjx155csvquery::getTest($model);
        $opt = array();
        $opt[] = array("label" => "(全て出力)","value" => "");
        $value_flg = false;
        $result = $db->query($query);
        while ($test_row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $test_row["LABEL"],
                           'value' => $test_row["VALUE"]);
            if ($model->field["TEST_CD"] == $test_row["VALUE"]) {
                $value_flg = true;
            }
        }
        $model->field["TEST_CD"] = ($model->field["TEST_CD"] && $value_flg) ? $model->field["TEST_CD"] : $opt[0]["value"];
        $extra = "onchange=\"btn_submit('');\"";
        $arg["data"]["TEST_CD"] = knjCreateCombo($objForm, "TEST_CD", $model->field["TEST_CD"], $opt, $extra, 1);

        //年組一覧コンボボックス
        $result      = $db->query(knjx155csvquery::getSelectFieldSQL2($model));
        $opt_gr_hr   = array();
        ...
        //生徒一覧コンボボックス
        $result      = $db->query(knjx155csvquery::getSelectFieldSQL3($model));
        $opt_student   = array();
        ...
        Query::dbCheckIn($db);
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx155csvindex.php", "", "main");
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjx155csvForm1.html", $arg);
    }
