...
class knjx_a121aForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx_a121aindex.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx_a121aQuery::getSecurityHigh($model));
        //セキュリティーチェック
        ...
        //年度一覧コンボボックス
        $result     = $db->query(knjx_a121aquery::getSelectFieldSQL());
        $opt_year   = array();
        ...
        //年組一覧コンボボックス
        $result      = $db->query(knjx_a121aquery::getSelectFieldSQL2($model));   
        $opt_gr_hr   = array();
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjx_a121aQuery::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX_a121a");
        ...
        View::toHTML($model, "knjx_a121aForm1.html", $arg);
    }
}
?>
