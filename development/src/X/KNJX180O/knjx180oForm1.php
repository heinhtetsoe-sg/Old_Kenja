...
class knjx180oForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjx180oindex.php", "", "main");

        //DB接続
        $db = Query::dbCheckOut();

        $securityCnt = $db->getOne(knjx180oQuery::getSecurityHigh());
        //セキュリティーチェック
        ...
        //年度一覧コンボボックス
        $result     = $db->query(knjx180oquery::getSelectFieldSQL());
        $opt_year   = array();
        ...
        //年組一覧コンボボックス
        $result      = $db->query(knjx180oquery::getSelectFieldSQL2($model));
        $opt_gr_hr   = array();
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjx180oQuery::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX180O");
        ...
        View::toHTML($model, "knjx180oForm1.html", $arg);
    }
}
