...
class knjx090bForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx090bindex.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx090bQuery::getSecurityHigh());
        //セキュリティーチェック
        ...
        //年度一覧コンボボックス
        $result = $db->query(knjx090bquery::getSelectFieldSQL($model));
        $opt_year = array();
        ...
        //講座一覧コンボボックス
        $result = $db->query(knjx090bquery::getSelectFieldSQL2($model));
        $opt_chaircd = array();
        $opt_chaircd[] = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //学籍番号一覧コンボボックス
        $result     = $db->query(knjx090bquery::getSelectFieldSQL3($model));
        $opt_schno  = array();
        $opt_schno[] = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $result     = $db->query(knjx090bquery::getSelectFieldSQL4($model));
        $opt_appdate  = array();
        $opt_appdate[] = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $row = str_replace("-", "/", $row);
        ...
        //学年一覧コンボボックス
        $result     = $db->query(knjx090bquery::getSelectFieldSQL5($model));
        $opt_grade  = array();
        $opt_grade[] = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjx090bQuery::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX090B");
        ...
        View::toHTML($model, "knjx090bForm1.html", $arg);
    }
}
