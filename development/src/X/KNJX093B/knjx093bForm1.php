...
class knjx093bForm1
{
    public function main(&$model)
    {
        ...
        $value_flg = false;
        $default = "";
        $query = knjx093bquery::getYearGrade($model);
        ...
        $value_flg = false;
        $query = knjx093bquery::getSelectFieldSQL($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //履修登録日コードコンボ
        $opt = array();
        $value = $model->field["RIREKI_CODE"];
        $value_flg = false;
        $query = knjx093bquery::getRirekiCode($year);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        $value = ($value && $value_flg) ? $value : $opt[0]["value"];
        $extra = "onchange=\"btn_submit('main');\"";
        $arg["data"]["RIREKI_CODE"] = knjCreateCombo($objForm, "RIREKI_CODE", $value, $opt, $extra, 1);

        //学年（表示）
        $grade_label = $db->getOne(knjx093bquery::getGrade($model, $grade));
        ...
        $value_flg = false;
        $query = knjx093bquery::getSubclassStdDat($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt[] = array('label' => $row["LABEL"],
                           'value' => $row["VALUE"]);
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjx093bindex.php", "", "main");

        //フォーム終わり
        $arg["finish"] = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjx093bForm1.html", $arg);
    }
}
