...
class knjx_c031cForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx_c031cindex.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx_c031cQuery::getSecurityHigh($model));
        //セキュリティーチェック
        ...
        //年度一覧コンボボックス
        $query = knjx_c031cQuery::getSelectFieldSQL();
        $extra = "onchange=\"return btn_submit('');\"";
        makeCmb($objForm, $arg, $db, $query, "YEAR", $model->field["YEAR"], $extra, 1);

        //年組一覧コンボボックス
        $query = knjx_c031cQuery::getSelectFieldSQL2($model);
        $extra = " onchange=\"return btn_submit('');\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE_HR_CLASS", $model->field["GRADE_HR_CLASS"], $extra, 1);

        $model->school_kind = $db->getOne(knjx_c031cQuery::getSchoolKind($model));
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX_C031C");
        ...
        View::toHTML($model, "knjx_c031cForm1.html", $arg);
    }
}
...
{
    $query = knjx_c031cQuery::selectSemesAll($model);
    $result = $db->query($query);
    $data = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $data[] = $row;
    ...
    for ($dcnt = 0; $dcnt < get_count($data); $dcnt++) {
        for ($i = $data[$dcnt]["S_MONTH"]; $i <= $data[$dcnt]["E_MONTH"]; $i++) {
            ...
            $getdata = $db->getRow(knjx_c031cQuery::selectMonthQuery($year, $month, $model), DB_FETCHMODE_ASSOC);
            if (is_array($getdata)) {
...
function makeBtn(&$objForm, &$arg, $db, $model)
{
    ...
    if ($model->Properties["useXLS"]) {
        $model->schoolCd = $db->getOne(knjx_c031cQuery::getSchoolCd());
