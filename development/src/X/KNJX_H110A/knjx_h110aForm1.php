...
class knjx_h110aForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx_h110aindex.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx_h110aQuery::getSecurityHigh($model));
        //セキュリティーチェック
        ...
        //年度一覧コンボボックス
        $result = $db->query(knjx_h110aQuery::getSelectFieldSQL());
        $opt_year = array();
        ...
        //年組一覧コンボボックス
        $result = $db->query(knjx_h110aQuery::getSelectFieldSQL2($model));
        $opt_gr_hr = array();
        ...
        //生徒一覧コンボボックス
        $result = $db->query(knjx_h110aQuery::getSelectFieldSQL3($model));
        $opt_student = array();
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX_H110A");
        ...
        View::toHTML($model, "knjx_h110aForm1.html", $arg);
    }
}
//ボタン作成
function makeButton(&$objForm, &$arg, $db, $model)
{
    ...
    if ($model->Properties["useXLS"]) {
        $model->schoolCd = $db->getOne(knjx_h110aQuery::getSchoolCd());
