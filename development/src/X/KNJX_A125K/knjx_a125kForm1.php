...
class knjx_a125kForm1
{
    public function main(&$model)
    {
        ...
        if ($model->prgid == "KNJA126K" && $model->cmd == "sign") {
            //HTTPSをHTTPにする
        ...
        //年度
        if ($model->flg == "sign") {
            $year_sem = $db->getRow(knjx_a125kquery::getSelectFieldSQL($model, $model->flg), DB_FETCHMODE_ASSOC);
            $arg["data"]["YEAR"]    = $year_sem["LABEL"];
            $model->field["YEAR"]   = $year_sem["VALUE"];
            knjCreateHidden($objForm, "YEAR", $year_sem["VALUE"]);
        } else {
            $query = knjx_a125kquery::getSelectFieldSQL($model);
        ...
        //年組
        if ($model->flg == "sign") {
            $grade_hr = $db->getRow(knjx_a125kquery::getSelectFieldSQL2($model, $model->flg), DB_FETCHMODE_ASSOC);
            ...
            $query = knjx_a125kquery::getSelectFieldSQL2($model);
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx_a125kindex.php", "", "main");
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjx_a125kForm1.html", $arg);
    }
}
