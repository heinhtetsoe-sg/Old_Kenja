...
class knjx191Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx191index.php", "", "main");

        //DB接続
        $db = Query::dbCheckOut();

        $securityCnt = $db->getOne(knjx191Query::getSecurityHigh($model));
        //セキュリティーチェック
        ...
        //年度一覧コンボボックス
        $result     = $db->query(knjx191query::getSelectFieldSQL());
        $opt_year   = array();
        ...
        //年組一覧コンボボックス
        $result      = $db->query(knjx191query::getSelectFieldSQL2($model));
        $opt_gr_hr   = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX191");
        ...
        View::toHTML($model, "knjx191Form1.html", $arg);
    }
}
//ボタン作成
function makeButton(&$objForm, &$arg, $model, $db)
{
    ...
    if ($model->Properties["useXLS"]) {
        $model->schoolCd = $db->getOne(knjx191Query::getSchoolCd());
