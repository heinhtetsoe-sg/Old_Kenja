...
class knjx154qForm1
{
    public function main(&$model)
    {
        ...
        $extra .= " id=\"HEADER\"";
        $arg["data"]["HEADER"] = $this->createCheckBox($objForm, "HEADER", "on", $extra, "");
        ...
        $this->createRadio($objForm, $arg, "OUTPUT", $model->field["OUTPUT"], $extra, $opt_shubetsu, get_count($opt_shubetsu));

        //ファイルからの取り込み
        $arg["FILE"] = $this->createFile($objForm, "FILE", 1024000);

        //年度一覧コンボボックス
        $result     = $db->query(knjx154qquery::getSelectFieldSQL());
        $opt_year   = array();
        ...
        //年組一覧コンボボックス
        $result      = $db->query(knjx154qquery::getSelectFieldSQL2($model));
        $opt_gr_hr   = array();
        ...
        Query::dbCheckIn($db);
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx154qindex.php", "", "main");
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjx154qForm1.html", $arg);
    }
