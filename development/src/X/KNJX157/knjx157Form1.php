...
class knjx157Form1
{
    public function main(&$model)
    {
        ...
        //今年度・今学期名及びタイトルの表示
        $arg["data"]["TITLE"] = CTRL_YEAR."年度　ＣＳＶ出力／取込";
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //年度一覧コンボボックス
        $result     = $db->query(knjx157query::getSelectFieldSQL());
        $opt_year   = array();
        ...
        //年組一覧コンボボックス
        $result      = $db->query(knjx157query::getSelectFieldSQL2($model));
        $opt_gr_hr   = array();
        ...
        //生徒一覧コンボボックス
        $result      = $db->query(knjx157query::getSelectFieldSQL3($model));
        $opt_student   = array();
        ...
        Query::dbCheckIn($db);
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx157index.php", "", "main");
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjx157Form1.html", $arg);
    }
