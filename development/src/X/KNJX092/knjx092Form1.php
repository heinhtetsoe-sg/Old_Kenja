...
class knjx092Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx092index.php", "", "main");

        //DB接続
        $db = Query::dbCheckOut();

        //親画面なし
        $securityCnt = $db->getOne(knjx092query::getSecurityHigh());
        //セキュリティーチェック
        ...
        //年度一覧コンボボックス
        $result = $db->query(knjx092query::getSelectFieldSQL());
        $opt_year = array();
        ...
        //講座一覧コンボボックス
        $result = $db->query(knjx092query::getSelectFieldSQL2($model));
        $opt_chaircd = array();
        $opt_chaircd[] = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //施設一覧コンボボックス
        $result = $db->query(knjx092query::getSelectFieldSQL3($model));
        $opt_faccd = array();
        $opt_faccd[] = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjx092query::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX092");
        ...
        View::toHTML($model, "knjx092Form1.html", $arg);
    }
}
