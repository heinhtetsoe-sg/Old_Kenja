...
class knjx_e012bForm1
{
    public function main(&$model)
    {
        ...
        //年度
        if ($model->flg == "sign") {
            $year_sem = $db->getRow(knjx_e012bquery::getSelectFieldSQL($model, $model->flg), DB_FETCHMODE_ASSOC);
            $arg["data"]["YEAR"]    = $year_sem["LABEL"];
            $model->field["YEAR"]   = $year_sem["VALUE"];
            knjCreateHidden($objForm, "YEAR", $year_sem["VALUE"]);
        } else {
            $query = knjx_e012bquery::getSelectFieldSQL($model);
        ...
        //年組
        if ($model->flg == "sign") {
            $grade_hr = $db->getRow(knjx_e012bquery::getSelectFieldSQL2($model, $model->flg), DB_FETCHMODE_ASSOC);
            ...
            $query = knjx_e012bquery::getSelectFieldSQL2($model);
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx_e012bindex.php", "", "main");
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjx_e012bForm1.html", $arg);
    }
}
