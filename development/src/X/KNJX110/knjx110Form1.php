...
class knjx110Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx110index.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx110Query::getSecurityHigh());
        //セキュリティーチェック
        ...
        //実施日付一覧コンボボックス
        $result = $db->query(knjx110query::getSelectFieldSQL($model));
        $opt_date = array();
        $opt_date[] = $optnull;
        ...
        //講座一覧コンボボックス
        $result = $db->query(knjx110query::getSelectFieldSQL2($model));
        $opt_chaircd = array();
        $opt_chaircd[] = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //施設一覧コンボボックス
        $result = $db->query(knjx110query::getSelectFieldSQL3($model));
        $opt_faccd = array();
        $opt_faccd[] = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjx110Query::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX110");
        ...
        View::toHTML($model, "knjx110Form1.html", $arg);
    }
}
