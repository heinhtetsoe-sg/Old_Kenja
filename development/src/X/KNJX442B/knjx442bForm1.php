...
class knjx442bForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx442bindex.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx442bQuery::getSecurityHigh($model));
        //セキュリティーチェック
        ...
        //年度一覧コンボボックス
        $result     = $db->query(knjx442bquery::getSelectFieldSQL());
        $opt_year   = array();
        ...
        //年組一覧コンボボックス
        $result      = $db->query(knjx442bquery::getSelectFieldSQL2($model));
        $opt_gr_hr   = array();
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjx442bQuery::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX442B");
        ...
        View::toHTML($model, "knjx442bForm1.html", $arg);
    }
}
