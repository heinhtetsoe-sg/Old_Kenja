...
class knjx_e061attendrecForm1
{
    public function main(&$model)
    {
        ...
        //年度一覧コンボボックス
        $query = knjx_e061attendrecQuery::getSelectFieldSQL();
        $extra = "onchange=\"return btn_submit('');\"";
        makeCmb($objForm, $arg, $db, $query, "YEAR", $model->field["YEAR"], $extra, 1);

        //学年一覧コンボボックス
        $query = knjx_e061attendrecQuery::getSelectFieldSQL1($model);
        $extra = "onchange=\"return btn_submit('');\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        //年組一覧コンボボックス
        $query = knjx_e061attendrecQuery::getSelectFieldSQL2($model);
        $extra = "onchange=\"return btn_submit('');\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE_HR_CLASS", $model->field["GRADE_HR_CLASS"], $extra, 1);

        //生徒一覧コンボボックス
        $query = knjx_e061attendrecQuery::getSelectFieldSQL3($model);
        $extra = "";
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx_e061attendrecindex.php", "", "main");
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjx_e061attendrecForm1.html", $arg);
    }
}
...
function makeCmb(&$objForm, &$arg, $db, $query, $name, &$value, $extra, $size)
{
    $opt = array();
    $value_flg = false;
    if ($name == "GRADE_HR_CLASS" || $name == "STUDENT") {
