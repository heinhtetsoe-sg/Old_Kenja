...
class knjx160aForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx160aindex.php", "", "main");

        //DB接続
        $db = Query::dbCheckOut();

        //親画面なし
        $securityCnt = $db->getOne(knjx160aQuery::getSecurityHigh());
        //セキュリティーチェック
        ...
        //年度一覧コンボボックス
        $result     = $db->query(knjx160aQuery::getSelectFieldSQL($model));
        $opt_year   = array();
        ...
        //年組一覧コンボボックス
        $result      = $db->query(knjx160aQuery::getSelectFieldSQL2($model));
        $opt_gr_hr   = array();
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX160A");
        ...
        View::toHTML($model, "knjx160aForm1.html", $arg);
    }
}
//ボタン作成
function makeButton(&$objForm, &$arg, $db, $model)
{
    ...
    if ($model->Properties["useXLS"]) {
        $model->schoolCd = $db->getOne(knjx160aQuery::getSchoolCd());
