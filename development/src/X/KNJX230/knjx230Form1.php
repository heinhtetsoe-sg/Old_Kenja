...
class knjx230Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjx230index.php", "", "main");

        //DB接続
        $db = Query::dbCheckOut();

        $securityCnt = $db->getOne(knjx230Query::getSecurityHigh());
        //セキュリティーチェック
        ...
        //年度一覧コンボボックス
        $result     = $db->query(knjx230query::getSelectFieldSQL());
        $opt_year   = array();
        ...
        //年組一覧コンボボックス
        $result      = $db->query(knjx230query::getSelectFieldSQL2($model));
        $opt_gr_hr   = array();
        ...
        //生徒一覧コンボボックス
        $result      = $db->query(knjx230query::getSelectFieldSQL3($model));
        $opt_student   = array();
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjx230Query::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX230");
        ...
        View::toHTML($model, "knjx230Form1.html", $arg);
    }
}
