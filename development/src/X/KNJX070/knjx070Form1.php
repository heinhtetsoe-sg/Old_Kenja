...
class knjx070Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx070index.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx070Query::getSecurityHigh());
        //セキュリティーチェック
        ...
        //年度一覧コンボボックス
        $result = $db->query(knjx070query::getSelectFieldSQL());
        $opt_year  = array();
        ...
        //講座一覧コンボボックス
        $result = $db->query(knjx070query::getSelectFieldSQL2($model));
        $opt_chaircd = array();
        $opt_chaircd[] = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //職員一覧コンボボックス
        $result     = $db->query(knjx070query::getSelectFieldSQL3($model));
        $opt_staffcd  = array();
        $opt_staffcd[] = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjx070Query::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX070");
        ...
        View::toHTML($model, "knjx070Form1.html", $arg);
    }
}
