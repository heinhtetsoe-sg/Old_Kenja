...
class knjx100Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx100index.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx100Query::getSecurityHigh());
        //セキュリティーチェック
        ...
        $initFlg = false;
        $initValue = ""; //初期値
        $result = $db->query(knjx100query::getYearSemesterCombo($model));
        ...
        $opt_date[] = $optnull;
        $value_flg = false;
        $result = $db->query(knjx100query::getExecutedateCombo($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $opt_chaircd[] = $optnull;
        $value_flg = false;
        $result = $db->query(knjx100query::getChaircdCombo($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjx100Query::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX100");
        ...
        View::toHTML($model, "knjx100Form1.html", $arg);
    }
}
