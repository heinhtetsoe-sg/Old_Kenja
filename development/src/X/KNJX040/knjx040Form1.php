...
class knjx040Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx040index.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx040Query::getSecurityHigh());
        //セキュリティーチェック
        ...
        $result     = $db->query(knjx040query::getSelectFieldSQL());
        $opt_year  = array();
        ...
        $objForm->ae(array("type"        => "select",
                            ...
                            "size"        => "1",
                            "extrahtml"   => "",
                            "value"       => ($model->field["YEAR"]!="")?$model->field["YEAR"]:CTRL_YEAR.CTRL_SEMESTER,
                            "options"     => $opt_year ));
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjx040Query::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX040");
        ...
        View::toHTML($model, "knjx040Form1.html", $arg);
    }
}
