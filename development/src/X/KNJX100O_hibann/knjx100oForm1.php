...
class knjx100oForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx100oindex.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx100oQuery::getSecurityHigh());
        //セキュリティーチェック
        ...
        $initFlg = false;
        $initValue = ""; //初期値
        $result = $db->query(knjx100oquery::getYearSemesterCombo());
        ...
        $result->free();
        if ($initFlg) $initValue = CTRL_YEAR."-".CTRL_SEMESTER; //処理年度＆学期
        ...
        $opt_date[] = $optnull;
        $value_flg = false;
        $result = $db->query(knjx100oquery::getExecutedateCombo($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $opt_chaircd[] = $optnull;
        $value_flg = false;
        $result = $db->query(knjx100oquery::getChaircdCombo($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjx100oQuery::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX100O");
        ...
        View::toHTML($model, "knjx100oForm1.html", $arg);
    }
}
?>
