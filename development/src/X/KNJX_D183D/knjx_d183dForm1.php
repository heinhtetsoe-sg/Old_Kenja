...
class knjx_d183dForm1
{
    public function main(&$model)
    {
        ...
        //年度一覧コンボボックス
        $result = $db->query(knjx_d183dquery::getSelectFieldSQL($model));
        $opt_year = array();
        ...
        //年組一覧コンボボックス
        $result      = $db->query(knjx_d183dquery::getSelectFieldSQL2($model));
        $opt_gr_hr   = array();
        ...
        Query::dbCheckIn($db);
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx_d183dindex.php", "", "main");
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjx_d183dForm1.html", $arg);
    }
}
