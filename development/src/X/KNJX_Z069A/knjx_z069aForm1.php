...
class knjx_z069aForm1
{
    public function main(&$model)
    {
        ...
        //年度一覧コンボボックス
        $query = knjx_z069aquery::getYear();
        $extra = "onchange=\"return btn_submit('');\"";
        makeCmb($objForm, $arg, $db, $query, "YEAR", $model->field["YEAR"], $extra, 1);

        //学期一覧コンボボックス
        $query = knjx_z069aquery::getSemester($model);
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //学年一覧コンボボックス
        $query = knjx_z069aquery::getGrade($model);
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx_z069aindex.php", "", "main");
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjx_z069aForm1.html", $arg);
    }
}
