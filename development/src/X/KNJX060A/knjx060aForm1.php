...
class knjx060aForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx060aindex.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx060aQuery::getSecurityHigh());
        //セキュリティーチェック
        ...
        //年度一覧コンボボックス
        $result = $db->query(knjx060aQuery::getSelectFieldSQL($model));
        $opt_year = array();
        ...
        $result = $db->query(knjx060aQuery::getSelectFieldSQL2($model));
        $opt_chaircd  = array();
        $opt_chaircd[] = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //科目一覧コンボボックス
        $result = $db->query(knjx060aQuery::getSelectFieldSQL3($model));
        ...
        $result->free();
        $extra = "";
        $arg["data"]["SUBCLASSCD"] = knjCreateCombo($objForm, "SUBCLASSCD", $model->field["SUBCLASSCD"], $opt_subclasscd, $extra, 1);
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjx060aQuery::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX060A");
        ...
        View::toHTML($model, "knjx060aForm1.html", $arg);
    }
}
