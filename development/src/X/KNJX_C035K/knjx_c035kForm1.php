...
class knjx_c035kForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx_c035kindex.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx_c035kQuery::getSecurityHigh($model));
        //セキュリティーチェック
        ...
        //講座コンボ
        $query = knjx_c035kQuery::getChaircd($model);
        $extra = "";
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX_C035K");
        ...
        View::toHTML($model, "knjx_c035kForm1.html", $arg);
    }
}
...
function makeMonthSemeCmb(&$objForm, &$arg, $db, &$model) {
    $query = knjx_c035kQuery::selectSemesAll($model);
    $result = $db->query($query);
    $data = array();
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $data[] = $row;
...
function setMonth($db, $model, $data) {
    ...
    for ($dcnt = 0; $dcnt < get_count($data); $dcnt++) {
        for ($i = $data[$dcnt]["S_MONTH"]; $i <= $data[$dcnt]["E_MONTH"]; $i++) {
            ...
            $query = knjx_c035kQuery::selectMonthQuery(substr($model->field["YEAR"], 0, 4), $month, $model);
...
function makeBtn(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->Properties["useXLS"]) {
        $model->schoolCd = $db->getOne(knjx_c035kQuery::getSchoolCd());
