...
class knjx181Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx181index.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx181Query::getSecurityHigh($model));
        //セキュリティーチェック
        ...
        //東京仕様かを確認
        $model->getname = "";
        $model->getname = $db->getOne(knjx181Query::getNamecd("Z010", "00"));
        ...
        //年度一覧コンボボックス
        $result     = $db->query(knjx181query::getSelectFieldSQL());
        $opt_year   = array();
        ...
        //年組一覧コンボボックス
        $result      = $db->query(knjx181query::getSelectFieldSQL2($model));
        $opt_gr_hr   = array();
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjx181Query::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX181");
        knjCreateHidden($objForm, "TEMPLATE_PATH");
        knjCreateHidden($objForm, "GETNAME", $model->getname);
        ...
        View::toHTML($model, "knjx181Form1.html", $arg);
    }
}
