...
class knjx030Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx030index.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx030Query::getSecurityHigh());
        //セキュリティーチェック
        ...
        $extra = array("id=\"OUTPUT1\"".$disabled, "id=\"OUTPUT2\"".$disabled, "id=\"OUTPUT3\"".$disabled, "id=\"OUTPUT4\"".$disabled);
        ...
        //disabled
        $disabled = ($model->field["OUTPUT"] == "4") ? "" : " disabled";

        //年度・学期一覧コンボボックス
        $result     = $db->query(knjx030Query::getSelectFieldSQL($model));
        $opt_year   = array();
        ...
        $arg["data"]["YEAR"] = knjCreateCombo($objForm, "YEAR", $model->field["YEAR"], $opt_year, "onchange=\"btn_submit('main');\"".$disabled, 1);

        //年組一覧コンボボックス
        $result      = $db->query(knjx030Query::getSelectFieldSQL2($model));
        $opt_gr_hr   = array();
        ...
        $result->free();
        $arg["data"]["GRADE_HR_CLASS"] = knjCreateCombo($objForm, "GRADE_HR_CLASS", $model->field["GRADE_HR_CLASS"], $opt_gr_hr, $disabled, 1);

        //実行ボタン
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjx030Query::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX030");
        ...
        View::toHTML($model, "knjx030Form1.html", $arg);
    }
}
