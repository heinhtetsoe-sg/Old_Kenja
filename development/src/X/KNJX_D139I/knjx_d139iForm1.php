...
class knjx_d139iForm1
{
    public function main(&$model)
    {
        ...
        //年度一覧コンボボックス
        $query = knjx_d139iquery::getSelectFieldSQL();
        $extra = "disabled onchange=\"return btn_submit('');\"";
        makeCmb($objForm, $arg, $db, $query, "YEARSEMESTER", $model->field["YEARSEMESTER"], $extra, 1);

        //年組一覧コンボボックス
        $query = knjx_d139iquery::getSelectFieldSQL2($model);
        $extra = "disabled ";
        ...
        knjCreateHidden($objForm, "HID_YEARSEMESTER", $model->field["YEARSEMESTER"]);
        knjCreateHidden($objForm, "HID_GRADEHR", $model->field["GRADE_HR_CLASS"]);
        knjCreateHidden($objForm, "HID_SEMESTER", $model->field["SEMESTER"]);
        knjCreateHidden($objForm, "HID_RECACTTBL", $model->recacttbl);

        //DB切断
        Query::dbCheckIn($db);

        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx_d139iindex.php", "", "main");
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjx_d139iForm1.html", $arg);
    }
}
