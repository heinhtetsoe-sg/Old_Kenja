...
class knjx160bForm1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjx160bindex.php", "", "main");

        //DB接続
        $db = Query::dbCheckOut();

        $securityCnt = $db->getOne(knjx160bQuery::getSecurityHigh());
        //セキュリティーチェック
        ...
        //年度一覧コンボボックス
        $result = $db->query(knjx160bquery::getSelectFieldSQL());
        $opt_year = array();
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt_year[] = array("label" => $row["LABEL"], 
                                "value" => $row["VALUE"]);
        }
        $result->free();
        ...
        //年組一覧コンボボックス
        $result = $db->query(knjx160bquery::getSelectFieldSQL2($model));   
        $opt_gr_hr = array();
        $opt_gr_hr[] = array("label" => "(全て出力)", "value" => "");
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt_gr_hr[] = array("label" => $row["LABEL"],
                                 "value" => $row["VALUE"]);
        }
        $result->free();
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjx160bQuery::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX160B");
        ...
        View::toHTML($model, "knjx160bForm1.html", $arg);
    }
}
?>
