...
class knjx152Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx152index.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx152Query::getSecurityHigh($model));
        //セキュリティーチェック
        ...
        //年度一覧コンボボックス
        $result = $db->query(knjx152query::getSelectFieldSQL());
        $opt_year = array();
        ...
        //年組一覧コンボボックス
        $result = $db->query(knjx152query::getSelectFieldSQL2($model));
        $opt_gr_hr = array();
        ...
        //生徒一覧コンボボックス
        $result = $db->query(knjx152query::getSelectFieldSQL3($model));
        $opt_student = array();
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX152");
        ...
        View::toHTML($model, "knjx152Form1.html", $arg);
    }
}
//ボタン作成
function makeButton(&$objForm, &$arg, $db, $model)
{
    ...
    if ($model->Properties["useXLS"]) {
        $model->schoolCd = $db->getOne(knjx152Query::getSchoolCd());
