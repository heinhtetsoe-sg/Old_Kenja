...
class knjx154jForm1
{
    public function main(&$model)
    {
        ...
        //年度一覧コンボボックス
        $result     = $db->query(knjx154jquery::getSelectFieldSQL($model));
        $opt_year   = array();
        ...
        //年組一覧コンボボックス
        $result      = $db->query(knjx154jquery::getSelectFieldSQL2($model));
        $opt_gr_hr   = array();
        ...
        Query::dbCheckIn($db);
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx154jindex.php", "", "main");
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjx154jForm1.html", $arg);
    }
}
