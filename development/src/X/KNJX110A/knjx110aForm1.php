...
class knjx110aForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx110aindex.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx110aQuery::getSecurityHigh());
        //セキュリティーチェック
        ...
        //実施日付一覧コンボボックス
        $result = $db->query(knjx110aquery::getSelectFieldSQL());
        $opt_date = array();
        $opt_date[] = $optnull;
        ...
        //講座一覧コンボボックス
        $result = $db->query(knjx110aquery::getSelectFieldSQL2($model));
        $opt_chaircd = array();
        $opt_chaircd[] = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //施設一覧コンボボックス
        $result = $db->query(knjx110aquery::getSelectFieldSQL3($model));
        $opt_faccd = array();
        $opt_faccd[] = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjx110aQuery::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX110A");
        ...
        View::toHTML($model, "knjx110aForm1.html", $arg);
    }
}
