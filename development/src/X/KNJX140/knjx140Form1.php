...
class knjx140Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx140index.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx140Query::getSecurityHigh());
        //セキュリティーチェック
        ...
        //年度一覧コンボボックス
        $result = $db->query(knjx140query::getSelectFieldSQL());
        $opt_year = array();
        ...
        //ＳＥＱとタイトルコンボ
        $opt_seq = array();
        $result = $db->query(knjx140query::getSelectFieldSQL2($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjx140Query::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX140");
        ...
        View::toHTML($model, "knjx140Form1.html", $arg);
    }
}
