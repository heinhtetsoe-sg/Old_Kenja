...
class knjx160Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx160index.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx160Query::getSecurityHigh());
        //セキュリティーチェック
        ...
        //年度一覧コンボボックス
        $result     = $db->query(knjx160query::getSelectFieldSQL());
        $opt_year   = array();
        ...
        //年組一覧コンボボックス
        $result      = $db->query(knjx160query::getSelectFieldSQL2($model));
        $opt_gr_hr   = array();
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX160");
        ...
        View::toHTML($model, "knjx160Form1.html", $arg);
    }
}
...
function makeButton(&$objForm, &$arg, $db, $model)
{
    ...
    if ($model->Properties["useXLS"]) {
        $model->schoolCd = $db->getOne(knjx160Query::getSchoolCd());
