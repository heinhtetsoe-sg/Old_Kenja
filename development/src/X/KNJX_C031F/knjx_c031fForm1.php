...
class knjx_c031fForm1
{
    public function main(&$model)
    {
        ...
        if ($model->Properties["use_prg_schoolkind"] == "1") {
            ...
            $extra = "id=\"SCHOOL_KIND\" onChange=\"current_cursor('SCHOOL_KIND');btn_submit('')\";";
        ...
        $arg["data"]["SHORI_MEI"] = knjCreateCombo($objForm, "SHORI_MEI", $model->field["SHORI_MEI"], $opt_shori, "style=\"width:60px;\" aria-label=\"データ取込のラジオボタンの処理名\"", $size);
...
function makeMonthSemeCmb(&$objForm, &$arg, $db, &$model)
{
    ...
    /* Edit by HPA for PC-talker 読み start 2020/02/03 */
    $arg["data"]["MONTH"] = knjCreateCombo($objForm, "MONTH", $model->field["MONTH"], $opt_month, "aria-label= \"対象月\"", 1);
}
/* Edit by HPA for PC-talker 読み end 2020/02/20 */
...
function makeBtn(&$objForm, &$arg, $db, $model)
{
    ...
    if ($model->Properties["useXLS"]) {
        ...
        /* Edit by HPA for PC-talker 読み start 2020/02/03 */
        $extra = "aria-label = \"実行\" onclick=\"return btn_submit('exec');\"";
        //今年度・今学期名及びタイトルの表示
    ...
    //終了ボタン
    if ($model->getPrgId) {
        $extra = "aria-label = \"終了\" onclick=\"window.opener.btn_submit('main');closeWin();\"";
    } else {
