...
class knjx_c033kForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx_c033kindex.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx_c033kquery::getSecurityHigh($model));
        //セキュリティーチェック
        ...
        $query = knjx_c033kquery::getSickDiv($year);
        makeCmb($objForm, $arg, $db, $query, "SICK", $model->field["SICK"], "", 1);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX_C033K");
        ...
        View::toHTML($model, "knjx_c033kForm1.html", $arg);
    }
}
...
function makeCmb(&$objForm, &$arg, $db, $query, $name, &$value, $extra, $size)
{
    ...
    } elseif ($name == "SICK" && strlen($syokiti)) {
        $value = ($value && $value_flg) ? $value : $syokiti;
    } else {
...
{
    $query = knjx_c033kquery::selectSemesAll($model->field["YEAR"]);
    $result = $db->query($query);
    $data = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $data[] = $row;
    ...
    for ($dcnt = 0; $dcnt < get_count($data); $dcnt++) {
        for ($i = $data[$dcnt]["S_MONTH"]; $i <= $data[$dcnt]["E_MONTH"]; $i++) {
            ...
            $year = substr($model->field["YEAR"], 0, 4);
            $getdata = $db->getRow(knjx_c033kquery::selectMonthQuery($year, $month, $model), DB_FETCHMODE_ASSOC);
            if (is_array($getdata)) {
...
function makeBtn(&$objForm, &$arg, $db, $model)
{
    ...
    if ($model->Properties["useXLS"]) {
        $model->schoolCd = $db->getOne(knjx_c033kquery::getSchoolCd());
