...
class knjx080aForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx080aindex.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx080aQuery::getSecurityHigh());
        //セキュリティーチェック
        ...
        //年度一覧コンボボックス
        $result = $db->query(knjx080aquery::getSelectFieldSQL());
        $opt_year = array();
        ...
        //講座一覧コンボボックス
        $result     = $db->query(knjx080aquery::getSelectFieldSQL2($model));
        $opt_chaircd  = array();
        $opt_chaircd[] = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //年組一覧コンボボックス
        $result     = $db->query(knjx080aquery::getSelectFieldSQL3($model));
        $opt_trgtcd  = array();
        $opt_trgtcd[] = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt_trgtcd[] = array("label" => $row["HR_NAME"],
                                  "value" => $row["TRGTGRADE"].$row["TRGTCLASS"]);
        }
        $result->free();
        $extra = "";
        $arg["data"]["TRGTCD"] = knjCreateCombo($objForm, "TRGTCD", $model->field["TRGTCD"], $opt_trgtcd, $extra, 1);
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjx080aQuery::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX080A");
        ...
        View::toHTML($model, "knjx080aForm1.html", $arg);
    }
}
