...
class knjx080Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx080index.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx080Query::getSecurityHigh());
        //セキュリティーチェック
        ...
        //年度一覧コンボボックス
        $result = $db->query(knjx080query::getSelectFieldSQL($model));
        $opt_year = array();
        ...
        //講座一覧コンボボックス
        $result     = $db->query(knjx080query::getSelectFieldSQL2($model));
        $opt_chaircd  = array();
        $opt_chaircd[] = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //年組一覧コンボボックス
        $result     = $db->query(knjx080query::getSelectFieldSQL3($model));
        $opt_trgtcd  = array();
        $opt_trgtcd[] = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjx080Query::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX080");
        ...
        View::toHTML($model, "knjx080Form1.html", $arg);
    }
}
