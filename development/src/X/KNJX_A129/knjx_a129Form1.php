...
class knjx_a129Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx_a129index.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx_a129Query::getSecurityHigh($model));
        //セキュリティーチェック
        ...
        //年度一覧コンボボックス
        $result     = $db->query(knjx_a129query::getSelectFieldSQL());
        $opt_year   = array();
        ...
        //年組一覧コンボボックス
        $result      = $db->query(knjx_a129query::getSelectFieldSQL2($model));
        $opt_gr_hr   = array();
        ...
        //ボタン作成
        createButton($objForm, $arg, $db, $model);
        ...
        View::toHTML($model, "knjx_a129Form1.html", $arg);
    }
}

function createButton(&$objForm, &$arg, &$db, &$model)
{
        //実行ボタン
    if ($model->Properties["useXLS"]) {
        $model->schoolCd = $db->getOne(knjx_a129Query::getSchoolCd());
...
function createHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJX_A129");
