...
class knjx060Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx060index.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx060Query::getSecurityHigh());
        //セキュリティーチェック
        ...
        //年度一覧コンボボックス
        $result = $db->query(knjx060query::getSelectFieldSQL());
        $opt_year = array();
        ...
        $result = $db->query(knjx060query::getSelectFieldSQL2($model));
        $opt_chaircd  = array();
        $opt_chaircd[] = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //科目一覧コンボボックス
        $result = $db->query(knjx060query::getSelectFieldSQL3($model));
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjx060Query::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX060");
        ...
        View::toHTML($model, "knjx060Form1.html", $arg);
    }
}
