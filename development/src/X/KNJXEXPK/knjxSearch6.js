function search_submit(cmd){
    ...
    for (var i=0;i<document.forms[0].elements.length;i++){
        ...
        if (e.name == "EXPENSE_L_CD" || e.name == "EXPENSE_M_CD" || e.name == "EXPENSE_S_CD") continue;
...
function chgCombo(obj){
    ...
    var s = document.forms[0]["EXPENSE_S_CD"].value;
    if (obj.name == "EXPENSE_L_CD"){
        var cmb = "EXPENSE_L_CD";
    }else if (obj.name == "EXPENSE_M_CD"){
        var cmb = "EXPENSE_M_CD";
    }else{
        var cmb = "EXPENSE_S_CD";
    }
    location.replace("index.php?cmd=show&showno=6&EXPENSE_L_CD="+l+"&EXPENSE_M_CD="+m+"&EXPENSE_S_CD="+s+"&cmb="+cmb);
    return false;
}
