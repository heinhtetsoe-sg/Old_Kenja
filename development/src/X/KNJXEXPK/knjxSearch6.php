...
class knjxSearch6
{
    function main(&$model){
        ...
        $opt2 = array();
        $opt3 = array();
        $result = $db->query(knjxexpkQuery::selectExpense_S_Cd($model));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            ...
            if ($row["EXPENSE_L_CD"] == $model->Expense["L"]){
                ...
                $opt2[$cd] = array("label"     => $cd ."：" .htmlspecialchars($row["EXPENSE_M_NAME"]),
                            "value"     => $cd);
                if (!$model->Expense["M"]) $model->Expense["M"] =  $cd;
                if ($row["EXPENSE_M_CD"] == substr($model->Expense["M"],2,2)){
                    $cd2 = $row["EXPENSE_L_CD"] .$row["EXPENSE_M_CD"] .$row["EXPENSE_S_CD"];
                    $opt3[$cd2] = array("label"     => $cd2 ."：" .htmlspecialchars($row["EXPENSE_S_NAME"]),
                                "value"     => $cd2);
                    if (!$model->Expense["S"]) $model->Expense["S"] =  $cd2;
                }
            }
        }
        //費目大分類
        ...
        $objForm->ae( array("type"        => "select",
                            "name"        => "EXPENSE_S_CD",
                            "extrahtml"  => "onchange=\"chgCombo(this)\"",
                            "value"      => $model->Expense["S"],
                            "size"        => "1",
                            "options"     => $opt3 ));

        $arg["EXPENSE_S_CD"] = $objForm->ge("EXPENSE_S_CD");
...
#        $result = $db->query(knjxexpkQuery::selectMoney_Paid_M_Dat($model));
        $result = $db->query(knjxexpkQuery::selectMoney_Paid_S_Dat($model));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        $objForm->ae( array("type" 		  => "button",
                            "name"        => "btn_search",
                            "value"       => "検索",
                            "extrahtml"   => "onclick=\"return search_submit('search6');\"" ));
        ...
        View::toHTML($model, "knjxSearch6.html", $arg);
    }
}
?>