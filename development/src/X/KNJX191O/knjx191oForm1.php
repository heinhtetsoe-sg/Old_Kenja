...
class knjx191oForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjx191oindex.php", "", "main");

        //DB接続
        $db = Query::dbCheckOut();

        $securityCnt = $db->getOne(knjx191oQuery::getSecurityHigh());
        //セキュリティーチェック
        ...
        //年度一覧コンボボックス
        $result     = $db->query(knjx191oquery::getSelectFieldSQL());
        $opt_year   = array();
        ...
        //年組一覧コンボボックス
        $result      = $db->query(knjx191oquery::getSelectFieldSQL2($model));
        $opt_gr_hr   = array();
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjx191oQuery::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX191O");
        ...
        View::toHTML($model, "knjx191oForm1.html", $arg);
    }
}
