...
class knjx_hexam_remarkForm1
{
    function main(&$model)
    {
        $objForm        = new form;
        $arg["start"]   = $objForm->get_start("sel", "POST", "knjx_hexam_remarkindex.php", "", "sel");
        $arg["jscript"] = "";
        $db = Query::dbCheckOut();
        ...
        $query = knjx_hexam_remarkQuery::selectQuery($model);
        $Row = $db->getRow($query, DB_FETCHMODE_ASSOC);

        //生徒情報
        $result = $db->query(knjx_hexam_remarkQuery::GetStudent($model));
        ...
        for ($i=0;$i<4;$i++)
        {
            ...
            $objForm->ae(array("type"       => "checkbox",
                                ...
                                "value"     => "1",
                                "extrahtml" => $extra,
                                "checked"   => (($model->data["check"][$i] == "1") ? 1 : 0)));
        ...
        //年組名
        $hr_name = $db->getOne(knjx_hexam_remarkQuery::getHR_Name($model));
        ...
        //戻るボタン
        $linkId = $model->getPrgId;
        $linkIdLow = strtolower($linkId);
        $link = REQUESTROOT."/E/{$linkId}/{$linkIdLow}index.php?cmd=back&SCHREGNO={$model->schregno}&GRADE=".$model->grade."-".$model->hrClass;
        $objForm->ae( array("type"        => "button",
                            "name"        => "btn_back",
                            "value"       => "戻 る",
        ...
        knjCreateHidden($objForm, "SCHREGNO", $model->data["selectdata"]);

        $arg["finish"]  = $objForm->get_finish();
        View::toHTML($model, "knjx_hexam_remarkForm1.html", $arg);
    }
}
?>
