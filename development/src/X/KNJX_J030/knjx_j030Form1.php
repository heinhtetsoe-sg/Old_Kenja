...
class knjx_j030Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjx_j030index.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx_j030Query::getSecurityHigh($model));
        //セキュリティーチェック
        ...
        //年度・学期コンボ
        $query = knjx_j030Query::getYearSemester($model);
        $extra = "onchange=\"btn_submit('');\"";
        makeCmb($objForm, $arg, $db, $query, "YEAR", $model->field["YEAR"], $extra, 1);

        //年組コンボ
        $query = knjx_j030Query::getGradeHrClass($model);
        $extra = "onchange=\"btn_submit('');\"";
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX_J030");
        ...
        View::toHTML($model, "knjx_j030Form1.html", $arg);
    }
}
...
function makeButton(&$objForm, &$arg, $db, $model)
{
    ...
    if ($model->Properties["useXLS"]) {
        $schoolCd = $db->getOne(knjx_j030Query::getSchoolCd());
