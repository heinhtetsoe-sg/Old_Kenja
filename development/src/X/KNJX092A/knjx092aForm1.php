...
class knjx092aForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx092aindex.php", "", "main");

        //DB接続
        $db = Query::dbCheckOut();

        //親画面なし
        $securityCnt = $db->getOne(knjx092aquery::getSecurityHigh());
        //セキュリティーチェック
        ...
        //初期値：空白項目
        $optnull = array("label" => "(全て出力)","value" => "");

        //年度一覧コンボボックス
        $result = $db->query(knjx092aquery::getSelectFieldSQL());
        $opt_year = array();
        ...
        //講座一覧コンボボックス
        $result = $db->query(knjx092aquery::getSelectFieldSQL2($model));
        $opt_chaircd = array();
        $opt_chaircd[] = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //施設一覧コンボボックス
        $result = $db->query(knjx092aquery::getSelectFieldSQL3($model));
        $opt_faccd = array();
        $opt_faccd[] = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $opt_faccd[] = array("label" => $row["FACCD"]." ".$row["FACILITYNAME"],
                                 "value" => $row["FACCD"]);
        }
        $result->free();
        $extra = "";
        $arg["data"]["FACCD"] = knjCreateCombo($objForm, "FACCD", $model->field["FACCD"], $opt_faccd, $extra, 1);
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjx092aquery::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX092A");
        ...
        View::toHTML($model, "knjx092aForm1.html", $arg);
    }
}
