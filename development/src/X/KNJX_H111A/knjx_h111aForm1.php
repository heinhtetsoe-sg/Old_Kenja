...
class knjx_h111aForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjx_h111aindex.php", "", "main");
        ...
        $securityCnt = $db->getOne(knjx_h111aQuery::getSecurityHigh($model));
        //セキュリティーチェック
        ...
        $opt_shori[]    = array("label" => "削除","value" => "3");
        $extra = "style=\"width:60px;\"";
        $model->field["SHORI_MEI"] = ($model->field["SHORI_MEI"] == "") ? "2" : $model->field["SHORI_MEI"];
        $arg["data"]["SHORI_MEI"] = knjCreateCombo($objForm, "SHORI_MEI", $model->field["SHORI_MEI"], $opt_shori, $extra, 1);

        //年度・学期コンボ
        $query = knjx_h111aQuery::getYearSemester();
        $extra = "onchange=\"btn_submit('');\"";
        makeCmb($objForm, $arg, $db, $query, "YEAR", $model->field["YEAR"], $extra, 1);

        //年組コンボ
        $query = knjx_h111aQuery::getGradeHrClass($model);
        $extra = "onchange=\"btn_submit('');\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE_HR_CLASS", $model->field["GRADE_HR_CLASS"], $extra, 1, "ALL");

        //生徒一覧コンボ
        $query = knjx_h111aQuery::getStudent($model);
        $extra = "";
        makeCmb($objForm, $arg, $db, $query, "STUDENT", $model->field["STUDENT"], $extra, 1, "ALL");
        ...
        knjCreateHidden($objForm, "PRGID", "KNJX_H111A");
        ...
        View::toHTML($model, "knjx_h111aForm1.html", $arg);
    }
}
...
function makeButton(&$objForm, &$arg, $db, $model)
{
    ...
    if ($model->Properties["useXLS"]) {
        $schoolCd = $db->getOne(knjx_h111aQuery::getSchoolCd());
