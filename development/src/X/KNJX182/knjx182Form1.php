...
class knjx182Form1
{
    function main(&$model)
    {
        ...
        //年度一覧コンボボックス
        $result     = $db->query(knjx182query::getSelectFieldSQL());
        $opt_year   = array();
        ...
        //年組一覧コンボボックス
        $result      = $db->query(knjx182query::getSelectFieldSQL2($model));   
        $opt_gr_hr   = array();
        ...
        //生徒一覧コンボボックス
        $result      = $db->query(knjx182query::getSelectFieldSQL3($model));   
        $opt_student   = array();
        ...
        Query::dbCheckIn($db);
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjx182index.php", "", "main");
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjx182Form1.html", $arg);
    }
