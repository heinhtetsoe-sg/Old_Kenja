...
window.onload = function () {
    if (sessionStorage.getItem('KNJWFINSearch_CurrentCursor') != null) {
        if (sessionStorage.getItem('KNJWFINSearch_CurrentCursor') == 'search') {
            document.title = '';
            document.getElementById('data').focus();
        } else {
            document.title = '';
            document.getElementById(sessionStorage.getItem('KNJWFINSearch_CurrentCursor')).focus();
        }
        // remove item
        sessionStorage.removeItem('KNJWFINSearch_CurrentCursor');
    } else {
...
function current_cursor(para) {
    sessionStorage.setItem('KNJWFINSearch_CurrentCursor', para);
}
...
function apply_finschoolgetParametr3(obj) {
    if (obj.selectedIndex >= 0) {
        var val = obj.options[obj.selectedIndex].text;
        var arr = val.split(' | ');
        var fscdname = document.forms[0].fscdname.value;
        ...
        //学校名innerHTML 「学校名[全角スペース]地区名」
        if (finschoolname) {
            finschoolname.innerHTML = arr[2] + '　' + arr[6];
        }
    ...
    if (window.event.keyCode == '13') {
        ...
        for (var i = 0; i < textFieldArray.length; i++) {
            if (textFieldArray[i] == obj.name) {
                ...
                // Add by PP for combo box cursor 2020-02-03 start
                // targetObject.focus();
                // Add by PP for combo box cursor 2020-02-03 start
                return;
            }
...
function execAfterEvent() {
    //呼び出し元画面の関数定義の存在確認
    if (typeof parent.current_knjxfin_after_event === 'function') {
        //呼び出し元画面の関数を実行
        // (呼び出し元画面では必要がある場合のみ current_knjxfin_after_event() を実装する)
        parent.current_knjxfin_after_event();
    }
}
