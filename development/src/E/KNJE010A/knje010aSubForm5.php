...
class knje010aSubForm5 {
    function main(&$model) {
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("list", "POST", "knje010aindex.php", "", "edit");
        ...
        $db = Query::dbCheckOut();
        if($model->schregno) {
            $result = $db->query(knje010aQuery::getAward($model));
        ...
        if (VARS::get("cmd") == "right_list"){ 
                $arg["reload"]  = "parent.edit_frame.location.href='knje010aindex.php?cmd=edit'";
        }

        View::toHTML($model, "knje010aSubForm5.html", $arg);
    }
}
