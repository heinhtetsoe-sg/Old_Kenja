...
//ビュー作成用クラス
class knje010aSubForm1 {
    function main(&$model) {
        $objForm = new form;
        $arg = array();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("form3", "POST", "knje010aindex.php", "", "form3");
        ...
        //中高一貫校
        $JH = (get_count($db->getOne(knje010aQuery::getSchoolName('JH'))) > 0) ? "1" : "";

        //SQL文発行
        //年度（年次）取得
        if ($model->cmd == "form3_first") { // すでに別の生徒を開いていた場合そのときの値が保持されているので
        ...
        $query = knje010aQuery::selectQueryAnnual($model, $JH);
        ...
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
            ...
            if ($model->cmd == "form3_first" && $model->exp_year == $row["YEAR"]){
        ...
        $objForm->ae( array("type"       => "select",
                            ...
                            "extrahtml"  => "onChange=\"return btn_submit('form3');\"",
                            "options"    => $opt));
        ...
        $query = knje010aQuery::selectQueryForm3($model, $JH);
        $result = $db->query($query);
        ...
        View::toHTML($model, "knje010aSubForm1.html", $arg);
    }
}
?>