# kanji=漢字
# $Id: readme.txt 74891 2020-06-16 05:09:04Z arakaki $

2006/11/08 m-yama   CVS登録。
                    調査書進学用所見データとヘッダデータのCSV処理呼び出しのボタンを追加した。

2006/11/09 m-yama   権限チェックの見直修正をした。(近大2005/06/27の修正)

2007/07/06 s-yama   指導要録参照ボタンに年度コンボを追加し、参照テーブルを変更した。
                    指導要録参照ボタンのウィンドウの大きさ、表示位置を変更した。

2007/10/10 nakamoto
★ 「年度・学年（年次）」コンボボックスの作成に使用するテーブルを変更した。
-- 卒業生の単位制の場合 → GRD_STUDYREC_DAT
-- 在校生の単位制の場合 → SCHREG_STUDYREC_DAT
-- ※「特別活動の記録＆指導上参考になる諸事項」登録画面の修正
★ 調査書所見データの内容を表示する際の取得する条件を変更した。
-- 単位制の場合、「学籍番号」「年度」の２つの条件で取得
-- ※「特別活動の記録＆指導上参考になる諸事項」登録画面の修正
★ 調査書所見データを更新する際の処理部分を変更した。
-- 単位制の場合、「学籍番号」「年度」の２つの条件で一旦削除するデータを決める
-- ※「特別活動の記録＆指導上参考になる諸事項」登録画面の修正

2007/10/12 yamauchi
// コメントの記号を変更した。

2009/04/21  1.tokio:/usr/local/development/src/E/KNJE010Aからコピーした。
            2.コメントアウトのカット
            3.リファクタ
            4.以下の修正をした。
            --「出欠記録修正」のカット、
            --「出欠の～」のポップアップウィンドウに「出欠備考参照」「要録の出欠の記録備考参照」を追加

2009/04/22  1.以下の修正をした。
            -- リファクタ
            -- ポップアップウィンドウを開いたときの年度と年次の初期値の修正

2009/08/07  1.７．指導上参考となる諸事項の３分割処理追加
...
2009/09/25  1.７．指導上参考となる諸事項の３分割処理(propertiesを参照せず、固定でなしとする)
            ...
            3.更新でTRAIN_REF1、TRAIN_REF2、TRAIN_REF3が消えてしまうバグを修正
...
2010/02/23  1.「通知票より読込」ボタン追加、エラーの文字数チェックを行数チェックに変更
            2.prgInfo.properties の tutisyoSougoushokengakki を参照するよう修正
            3.「総合学習評価」のサイズは prgInfo.properties の tyousasyoSougouHyoukaFieldSize を参照するよう修正

2010/03/17  1.「出欠備考参照」ボタンのパラメータに開始日付、終了日付を追加した。

2010/03/19  1.参照先テーブルで「HREPORTREMARK_DAT」を「RECORD_TOTALSTUDYTIME_DAT」に変更

2010/03/24  1.getPropertiesAll()を使うよう修正
            2.「部活動参照」、「委員会参照」、「資格参照」ボタンは「学年」コンボ内の年度を表示するよう修正
            --「出欠の記録備考」の文字数、行数はプロパティーファイルで指定できるよう修正

2010/04/02  1.年度がない場合、「出欠備考参照」、「要録の出欠の記録備考参照」、「部活動参照」、
...
2010/07/01  1.「KNJE010」と同じに修正(「KNJE010」に3分割のフラグを立てただけ)
...
2011/03/29  1.指導要録参照画面（年度）の活動内容、評価のサイズをプロパティーによって変えるように修正した。
            2.指導要録参照画面（年度）にエラー文字が出る不具合を修正した。
...
2018/04/06  1.資格参照ボタンを検定選択ボタンに変更
            -- 選択学年のデータのみ一覧に表示する
