...
class knje390mSubForm2_3
{
    public function main(&$model)
    {
        ...
        $query = knje390mQuery::getSubQuery2GradeKindSchregGroupDat($model);
        makeCmb($objForm, $arg, $db, $query, "SUBCLASS", $model->field2['SUBCLASS'], $extra, 1, "");

        //教科等の実態情報取得
        if (isset($model->schregno) && !isset($model->warning)) {
            $query = knje390mQuery::getSubQuery2StatusSubclassData($model);
        ...
        $arg["data"]["SUBCLASS_STATUS"]       = getTextOrArea($objForm, "SUBCLASS_STATUS", $moji, $gyou, $Row["STATUS"]);
        $arg["data"]["SUBCLASS_STATUS_COMMENT"] = getTextAreaComment($moji, $gyou);
        setInputChkHidden($objForm, 'SUBCLASS_STATUS', $moji, $gyou, $arg);
        //支援
        $arg["data"]["SUBCLASS_FUTURE_CARE"] = getTextOrArea($objForm, "SUBCLASS_FUTURE_CARE", 15, 30, $Row["FUTURE_CARE"]);
        ...
        View::toHTML($model, "knje390mSubForm2_3.html", $arg);
    }
}
...
{
    $retCnt = 0;
    $query = knje390mQuery::getSubQuery2StatusSubclassList($model);
