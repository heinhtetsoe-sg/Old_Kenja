...
class knje390nSubForm3_2
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform3_2", "POST", "knje390nindex.php", "", "subform3_2");
        ...
        if ($model->cmd == "subform3_check_set"){
            if (isset($model->schregno) && !isset($model->warning)){
                $Row = $db->getRow(knje390nQuery::getSubQuery3CheckGetData($model), DB_FETCHMODE_ASSOC);
            } else {
                $Row =& $model->field3;
            }
        } else {
        ...
        //検査機関
        $query = knje390nQuery::getCheckCentercdComboName();
        ...
        View::toHTML($model, "knje390nSubForm3_2.html", $arg); 
    }
}
...
function makeList(&$arg, $db, $model) {
    $retCnt = 0;
    $query = knje390nQuery::getSubQuery3CheckRecordList($model);
    $result = $db->query($query);
    $centerName = "";
    while ($rowlist = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $rowlist["RECORD_DIV_NAME"] = '検査機関'.($retCnt+1);
        $centerName = $db->getOne(knje390nQuery::getCheckCentercdName($rowlist["CENTERCD"]));
...
{
    //D様式参照ボタンを作成する
    $extra = "onclick=\"loadwindow('" .REQUESTROOT."/E/KNJE390N/knje390nindex.php?cmd=subform2_check_B_sanshou&MAIN_YEAR=".$model->main_year."&SCHREGNO=".$model->schregno."&RECORD_DATE=".$model->record_date."', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 650, 600)\"";
