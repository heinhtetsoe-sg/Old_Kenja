...
class knje390nSubForm4_4
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform4_4", "POST", "knje390nindex.php", "", "subform4_4");
        ...
        $query = knje390nQuery::getCareerguidanceRecordDiv();
        ...
        if ($model->field4["RECORD_DIV"] !== '5') {
            ...
            if ($model->cmd == "subform4_careerguidance_set"){
                if (isset($model->schregno) && !isset($model->warning)){
                    $Row = $db->getRow(knje390nQuery::getSubQuery4CareerguidanceGetData($model), DB_FETCHMODE_ASSOC);
                } else {
                    $Row =& $model->field4;
                }
            } else {
            ...
            //6:決定進路内容(3年次のみ)
                ...
                if (isset($model->schregno) && !isset($model->warning)){
                    $Row2 = $db->getRow(knje390nQuery::getSubQuery4CareerguidanceRecordList($model, "6"), DB_FETCHMODE_ASSOC);
                } else {
                    $Row2 =& $model->field4;
                }
        ...
        //5:年間の成果、課題
            ...
            $arg["RECORD_DIV5"] = "1";
            if (isset($model->schregno) && !isset($model->warning)){
                $Row3 = $db->getRow(knje390nQuery::getSubQuery4CareerguidanceResultChallengeGetData($model), DB_FETCHMODE_ASSOC);
            } else {
                $Row3 =& $model->field4;
            }
            //年間の成果
        ...
        View::toHTML($model, "knje390nSubForm4_4.html", $arg); 
    }
}
...
function makeList(&$arg, $db, $model) {
    ...
    $no1Cnt = 1;
    $no2Cnt = 1;
    $query = knje390nQuery::getSubQuery4CareerguidanceRecordList($model, "");
