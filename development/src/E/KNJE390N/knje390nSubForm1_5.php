    ...
    public function main(&$model)
    {
        ...
        if ($Row["RECORD_DIV"] === '1') {
            ...
            //補装具
                ...
                $result->free();
            //日常生活用具
            } elseif ($Row["RECORD_NO"] === '3') {
                $arg["RECORD_DIV1_23"] = "1";
                $arg["DISP_SUPPLY_YM"] = "";
                $arg["NO_DISP_SUPPLY_YM"] = "1";
            ...
            $extra = "";
            $arg["data"]["SERVICE_CENTER_TEXT"] = knjCreateTextBox($objForm, $Row["SERVICE_CENTER_TEXT"], "SERVICE_CENTER_TEXT", 20, 20, $extra);
            $arg["data"]["SERVICE_CENTER_TEXT_SIZE"] = '<font size="1" color="red">(全角10文字まで)</font>';
            $arg["data"]["SERVICE_CENTER_TEXT_NAME"] = '製作会社';
        } elseif ($Row["RECORD_DIV"] === '2' || $Row["RECORD_DIV"] === '3') {
            $arg["RECORD_DIV23"] = "1";
            if ($Row["RECORD_DIV"] === '2') {
                $arg["RECORD_DIV2"] = "1";
                $arg["data"]["WELFARE_REMARK_NAME"] = '備考';
                //圏域
                if ($model->cmd == "subform1_welfare3" || $model->cmd == "subform1_welfare_set") {
                    $centercd = ($model->cmd == "subform1_welfare3") ? $model->field["CENTERCD"] : $Row["CENTERCD"];
                    $Row["AREACD"] = $db->getOne(knje390nQuery::getSubQuery1WelfareGetAreacd($centercd));
                }
                ...
                //相談・支援事業所
                $extra = "onChange=\"return btn_submit('subform1_welfare3')\"";
                $query = knje390nQuery::getWelfareCentercdComboName($Row["AREACD"]);
                makeCmb($objForm, $arg, $db, $query, "CENTERCD", $Row["CENTERCD"], $extra, 1, 1);
                //担当者
...
function makeList(&$arg, $db, $model)
{
    ...
    while ($rowlist = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        if ($rowlist["RECORD_DIV"] === '1') {
            ...
            //支給量
            $tani = ($rowlist["ITEMCD2"] == "1") ? "(日/月)" : "(時間/月)";
            $rowlist["SIKYU_RYOU"] = $rowlist["ITEMCD"].$tani;
