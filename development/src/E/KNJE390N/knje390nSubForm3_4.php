...
class knje390nSubForm3_4
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform3_4", "POST", "knje390nindex.php", "", "subform3_4");
        ...
        if ($model->cmd == "subform3_work_set"){
            if (isset($model->schregno) && !isset($model->warning)){
                $Row = $db->getRow(knje390nQuery::getSubQuery3WorkGetData($model), DB_FETCHMODE_ASSOC);
            } else {
                $Row =& $model->field3;
            }
        } else {
        ...
        View::toHTML($model, "knje390nSubForm3_4.html", $arg); 
    }
}
...
function makeList(&$arg, $db, $model) {
    $retCnt = 0;
    $query = knje390nQuery::getSubQuery3WorkRecordList($model);
