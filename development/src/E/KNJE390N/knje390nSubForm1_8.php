...
class knje390nSubForm1_8
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform1_8", "POST", "knje390nindex.php", "", "subform1_8");
        ...
        if ($model->cmd == "subform1_training_set" || $model->cmd == "subform1_training"){
            if (isset($model->schregno) && !isset($model->warning)){
                $Row = $db->getRow(knje390nQuery::getSubQuery1TrainingGetData($model), DB_FETCHMODE_ASSOC);
            } else {
                $Row =& $model->field;
            }
        } else {
        ...
        View::toHTML($model, "knje390nSubForm1_8.html", $arg); 
    }
}
