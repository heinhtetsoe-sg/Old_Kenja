...
class knje390nSubList
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("sublist", "POST", "knje390nindex.php", "", "sublist");
        ...
        if ($model->cmd != "subform2_growup_B_sanshou") {
            ...
            if ($model->cmd == "subform2_check_B_sanshou_set" || $model->cmd == "subform2_check_C_sanshou_set"){
                $Row = $db->getRow(knje390nQuery::getSubQuery2CheckGetData($model, $getMaxDate), DB_FETCHMODE_ASSOC);
            }
            //検査日
        ...
        //課題・つけたい力取得
        } else if ($model->cmd == "subform2_growup_B_sanshou") {
            $arg["GROWUP_HYOUJI"] = '1';
            $arg["TITLE"] = '(課題・つけたい力)';
            $Row = $db->getRow(knje390nQuery::getSubQuery2($model, $getMaxDate), DB_FETCHMODE_ASSOC);
        ...
        View::toHTML($model, "knje390nSubList.html", $arg); 
    }
}
...
function makeList(&$arg, $db, $model, $getMaxDate) {
    $retCnt = 0;
    $query = knje390nQuery::getSubQuery2CheckRecordList($model, $getMaxDate);
    ...
    while ($rowlist = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        } else if ($model->cmd === 'subform2_check_C_sanshou' || $model->cmd === 'subform2_check_C_sanshou_set') {
