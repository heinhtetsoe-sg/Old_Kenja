...
class knje390nSubForm1_6
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform1_6", "POST", "knje390nindex.php", "", "subform1_6");
        ...
        if ($model->cmd == "subform1_work_set"){
            if (isset($model->schregno) && !isset($model->warning)){
                $Row = $db->getRow(knje390nQuery::getSubQuery1WorkGetData($model), DB_FETCHMODE_ASSOC);
            } else {
                $Row =& $model->field;
            }
        } else {
        ...
        //事業所
        $query = knje390nQuery::getAdviceCentercdComboName();
        ...
        View::toHTML($model, "knje390nSubForm1_6.html", $arg); 
    }
}
...
function makeList(&$arg, $db, $model) {
    $retCnt = 0;
    $query = knje390nQuery::getSubQuery1WorkRecordList($model);
    $result = $db->query($query);
    $centerName = "";
    while ($rowlist = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $rowlist["RECORD_DIV_NAME"] = '事業所'.($retCnt+1);
        $centerName = $db->getOne(knje390nQuery::getAdviceCentercdName($rowlist["CENTERCD"]));
