...
class knje390nFamily
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("family", "POST", "knje390nindex.php", "", "family");
        ...
        if ($model->cmd == "family_set"){
            if (isset($model->schregno) && !isset($model->warning)){
                $Row = $db->getRow(knje390nQuery::getFamilyGetData($model), DB_FETCHMODE_ASSOC);
            } else {
                $Row =& $model->subfield;
            }
        } else {
        ...
        //続柄
        $query = knje390nQuery::getNameMst("H201");
        ...
        View::toHTML($model, "knje390nFamily.html", $arg); 
    }
}
...
function makeList(&$arg, $db, $model) {
    $retCnt = 0;
    $query = knje390nQuery::getFamilyRecordList($model);
    $result = $db->query($query);
    $centerName = "";
    while ($rowlist = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $tsuzukigaraName = $db->getOne(knje390nQuery::getTsuzukigaraName($rowlist["RELATIONSHIP"]));
