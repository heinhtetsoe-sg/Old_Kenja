...
class knje390nSubForm1_2
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform1_2", "POST", "knje390nindex.php", "", "subform1_2");
        ...
        if ($model->cmd == "subform1_educate_set"){
            if (isset($model->schregno) && !isset($model->warning)){
                $Row = $db->getRow(knje390nQuery::getSubQuery1EducateGetData($model), DB_FETCHMODE_ASSOC);
            } else {
                $Row =& $model->field;
            }
        } else {
        ...
        $query = knje390nQuery::getEducateRecordDiv();
        ...
        //教育機関名等（学校検索）
        $query = knje390nQuery::getSchoolInfo($Row["P_J_SCHOOL_CD"]);
        $schoolRow = $db->getRow($query, DB_FETCHMODE_ASSOC);
        $arg["data"]["P_J_SCHOOL_NAME"] = $schoolRow["FINSCHOOL_NAME"];
        //学校検索
        $setKind = $db->getOne(knje390nQuery::getL019namecd2($Row["RECORD_DIV"]));
        ...
        View::toHTML($model, "knje390nSubForm1_2.html", $arg); 
    }
}
...
function makeList(&$arg, $db, $model) {
    //年号取得
    $arrayL007 = array();
    $query = knje390nQuery::getNameMstL007();
    ...
    $retCnt = 0;
    $query = knje390nQuery::getSubQuery1EducateRecordList($model, "");
    $result = $db->query($query);
    $classShubetsuName = "";
    $divcount1 = 1;
    $divcount2 = 1;
    ...
    while ($rowlist = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($rowlist["RECORD_DIV"] === '4' || $rowlist["RECORD_DIV"] == '5' || $rowlist["RECORD_DIV"] == '6' || $rowlist["RECORD_DIV"] == '7') {
            $query = knje390nQuery::getSchoolInfo($rowlist["P_J_SCHOOL_CD"]);
            $schoolRow = $db->getRow($query, DB_FETCHMODE_ASSOC);
