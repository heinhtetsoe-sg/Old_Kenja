...
class knje463form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knje463index.php", "", "main");
        ...
        $savedat = array();
        $savecnt = 0;
        $query = knje463Query::getChallengedSupportBaseInfoYmst($model, "");
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            if ($row["SPRT_SEQ"] != "") {
                $savedat[$row["SPRT_SEQ"]] = $row["GOAL_TITLE"];
                $savecnt++;
            }
        }
        ...
        View::toHTML($model, "knje463Form1.html", $arg);
    }
}
