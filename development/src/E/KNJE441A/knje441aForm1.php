...
class knje441aForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knje441aForm1", "POST", "knje441aindex.php", "", "knje441aForm1");

        //DB接続
        $db = Query::dbCheckOut();

        //教育委員会チェック
        $getFlg = $db->getOne(knje441aQuery::getIinkaiFlg());
        if ($getFlg !== '1') {
            $arg["jscript"] = "OnEdboardError();";
        }

        //年度
        $extra = "onChange=\"return btn_submit('knje441a');\"";
        $query = knje441aQuery::getYear();
        ...
        View::toHTML($model, "knje441aForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, &$model) {
    $row1 = array();
    $query = knje441aQuery::getSchoolData($model);
    $result = $db->query($query);
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
