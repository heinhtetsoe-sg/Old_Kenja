...
class knje431aForm1 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]  = $objForm->get_start("knje431aForm1", "POST", "knje431aindex.php", "", "knje431aForm1");

        //DB接続
        $db = Query::dbCheckOut();

        //教育委員会チェック
        $getFlg = $db->getOne(knje431aQuery::getNameMst());
        if ($getFlg !== '1') {
            $arg["jscript"] = "OnEdboardError();";
        }

        //年度
        $extra = "onChange=\"return btn_submit('knje431a');\"";
        $query = knje431aQuery::getYear();
        ...
        View::toHTML($model, "knje431aForm1.html", $arg); 
    }
}
...
{
    $row1 = array();
    $query = knje431aQuery::getSchoolData($model);
    $result = $db->query($query);
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
...
function makeHidden(&$objForm, $model, $db) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJE431A");
}
?>
