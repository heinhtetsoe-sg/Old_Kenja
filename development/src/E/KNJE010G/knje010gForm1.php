...
class knje010gForm1
{
    public function main(&$model)
    {
        $objForm = new form();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knje010gindex.php", "", "edit");
        $db = Query::dbCheckOut();

        $model->schoolKind = $db->getOne(knje010gQuery::getSchoolKind($model));
        ...
        if (!isset($model->warning) && !in_array($model->cmd, array("yorokuYoriYomikomi_ok", "yorokuYoriYomikomi_cancel", "reload3")) && !preg_match('/(torikomi|torikomiT)[0-2]/', $model->cmd)) {
            $query = knje010gQuery::selectQuery($model);
            $row = $db->getRow($query, DB_FETCHMODE_ASSOC);
            $query = knje010gQuery::selectHexamEntremarkRemarkHdatQuery($model);
        ...
        //年次取得
        $query = knje010gQuery::getGradeCd($model);
        ...
        $model->getSchoolName = "";
        $model->getSchoolName = $db->getOne(knje010gQuery::getNameMst("Z010"));
        ...
        $getIppanCount = $db->getOne(knje010gQuery::getPositionCheck($model));
        if ($getIppanCount == 0 && $model->getSchoolName !== 'kaijyo') {
            ...
            $opt[] = array("label" => "全て",
                           "value" => "0000");
            $query = knje010gQuery::selectQueryAnnual($model);
        ...
        View::toHTML($model, "knje010gForm1.html", $arg);
    }
}
...
function makeButton(&$objForm, &$arg, $db, &$model)
{
    ...
    if ("1" == $model->Properties["tyousasho6bunkatsu_homeroomShojikouTorikomi"]) {
        $query = knje010gQuery::getHrShojikouTorikomiDataCheck($model);
        $result = $db->query($query);
        $chk_hrname = "";
        $countYears = array();
    ...
    //セキュリティーチェック
    $securityCnt = $db->getOne(knje010gQuery::getSecurityHigh());
    $csvSetName = "CSV";
    ...
    if ($model->getPrgId || !$model->Properties["useXLS"] || $securityCnt == 0) {
        //データCSVボタン
        $extra = "onClick=\" wopen('".REQUESTROOT."/X/KNJX190/knjx190index.php?program_id=".PROGRAMID."&mode={$model->mode}&SEND_PRGID=KNJE010G&SEND_AUTH={$model->auth}&tyousasyo2020=1','SUBWIN2',0,0,screen.availWidth,screen.availHeight);\"";
        $arg["button"]["btn_check1"] = knjCreateBtn($objForm, "btn_check1", "データ".$csvSetName, $extra);
        //ヘッダデータCSVボタン
        $extra = "onClick=\" wopen('".REQUESTROOT."/X/KNJX191/knjx191index.php?program_id=".PROGRAMID."&mode={$model->mode}&SEND_PRGID=KNJE010G&SEND_AUTH={$model->auth}&tyousasyo2020=1','SUBWIN2',0,0,screen.availWidth,screen.availHeight);\"";
...
function makeAttendrecRemark(&$objForm, &$arg, $db, $model, $disabled, $gradeCnt, $grade, $year, $val, $slashFlg)
{
    ...
    if ($model->cmd === "torikomi".$gradeCnt) {
        $set_remark = knje010gQuery::getSemesRemark($model, $db, $year);
        $val = $set_remark;
    } elseif ($model->cmd === "torikomiT".$gradeCnt) {
        $set_remark = knje010gQuery::getHreportremarkDetailDat($db, $model, $year);
        $val = $set_remark;
    //対応しない学年の時は画面の値をセット
...
function makeSpecialactrec(&$objForm, &$arg, $db, $model, $disabled, $gradeCnt, $grade, $year, $val)
{
    ...
    if ($model->getSchoolName == "koma") {
        ...
        //臘八摂心皆勤
        $rouhatsuKaikin = "";
        $query = knje010gQuery::getRouhatsuKaikin($model, $year);
...
function makeNendogoto(&$objForm, &$arg, $db, &$model)
{
    ...
    $query = knje010gQuery::getGdat($model);
    $result = $db->query($query);
    $opt_grades = array();
    ...
    foreach ($opt_grades as $r) {
        ...
        if (!isset($model->warning)) {
            if (($model->cmd == "yorokuYoriYomikomi_ok" || $model->cmd == "yorokuYoriYomikomi_cancel") && ($model->readYear == "0000" || $model->readYear == $year)) {
                $query = knje010gQuery::selectQuery_Htrainremark_Dat($model, $year);
                $row = $db->getRow($query, DB_FETCHMODE_ASSOC);
                ...
                for ($i = 1; $i <= 6; $i++) {
                    $row["TRAIN_REF".$i] = "";
                    $seq = "10".$i;
                    $query = knje010gQuery::getHexamEntremarkTrainrefDat($model, $year, $seq);
                ...
                if (!in_array($model->cmd, array('torikomi0', 'torikomi1', 'torikomi2', 'torikomiT0', 'torikomiT1', 'torikomiT2', "reload3", "yorokuYoriYomikomi_ok", "yorokuYoriYomikomi_cancel"))) {
                    $query = knje010gQuery::selectQueryForm2($model, $year);
                    $row = $db->getRow($query, DB_FETCHMODE_ASSOC);
                    for ($i = 1; $i <= 6; $i++) {
                        $row["TRAIN_REF".$i] = "";
                        $seq = "10".$i;
    ...
    if ("1" == $model->Properties["seitoSidoYorokuSogoShoken3Bunkatsu"] || $model->schoolName == 'tokiwa') {
        ...
        foreach ($opt_grades as $r) {
            ...
            $query = knje010gQuery::yoroku_trainref123($model, $year);
        ...
        foreach ($opt_grades as $r) {
            ...
            $query = knje010gQuery::yoroku_sogoshoken($model, $year);
            $sansyou = $db->getRow($query, DB_FETCHMODE_ASSOC);
...
//    if ($model->Properties["tyousasyoSougouHyoukaNentani"] != 1 || "1" == $model->Properties["tyousasyo2020"]) {
    ...
    if ($model->cmd == "yorokuYoriYomikomi_ok" || $model->cmd == "yorokuYoriYomikomi_cancel") {
        if ($model->Properties["sidouyourokuSansyou"] == 1) {
            $query = knje010gQuery::getYourokuDat($model);
        } else {
            $query = knje010gQuery::selectQuery_Htrainremark_Hdat($model);
        }
...
function makeBiko(&$objForm, &$arg, $db, &$model, &$row)
{
    ...
    if ("1" != $model->Properties["unuse_KNJE015_HEXAM_ENTREMARK_LEARNING_DAT_REMARK"]) {
        $extra = " onclick=\"loadwindow('".REQUESTROOT."/E/KNJE010G/knje010gindex.php?&cmd=formBikoTori&TARGET=REMARK',0,document.documentElement.scrollTop || document.body.scrollTop,1000,650);\"";
...
function reload3setTotalStudy(&$row, $db, $model)
{
    ...
    //3年次　通知票取込
        ...
        $query = knje010gQuery::get_record_totalstudytime_dat($model);
        $result = $db->query($query);
        while ($total_row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeSelectBtn(&$objForm, $model, $div, $name, $label, $target, $year, $disabled = "")
{
    if (!$div || !$name || !$label || !$target) {
        ...
        if ($div == "club") {                   //部活動
            ...
            $tgt = "/E/KNJE010G/knje010gindex.php";
            $sizeW = 900;
            $sizeH = 650;
...
function makeHidden(&$objForm, $db, &$model)
{
    ...
    if ($model->Properties["tyousasyoShokenPreview"] == '1') {
        ...
        knjCreateHidden($objForm, "PRGID", "KNJE010G");
        ...
        $query = knje010gQuery::getSchoolDiv($model);
