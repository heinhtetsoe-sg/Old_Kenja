...
class knje372hForm1
{
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knje372hForm1", "POST", "knje372hindex.php", "", "knje372hForm1");
        ...
        //学期コンボボックスを作成する
        $query = knje372hQuery::getSemester();
        ...
        //学校名取得
        $query = knje372hQuery::getSchoolname();
        $schoolName = $db->getOne($query);

        //学年コンボ
        $query = knje372hQuery::getSelectGrade($model);
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], "onChange=\"return btn_submit('knje372h');\"", 1);

        //テスト種別コンボ
        $query = knje372hQuery::getTestItem($model, $model->field["SEMESTER"]);
        ...
        View::toHTML($model, "knje372hForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    //クラス一覧
    $row1 = array();
    $result = $db->query(knje372hQuery::getAuth($model));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJE372H");
