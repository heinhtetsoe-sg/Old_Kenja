...
class knje372nForm2
{
    function main(&$model)
    {
        ...
        $arg["start"] = $objForm->get_start("edit", "POST", "knje372nindex.php", "", "edit");
        ...
        //推薦枠コード
        $query = knje372nQuery::getAftRecommendationLimitMst($model->oyear, "");
        $extra = "";
        makeCmb($objForm, $arg, $db, $query, $model->field["RECOMMENDATION_CD"], "RECOMMENDATION_CD", $extra, 1, "BLANK");

        //平均点
        $arg["data"]["ASSESS_AVG"] = knjCreateTextBox($objForm, $model->field["ASSESS_AVG"], "ASSESS_AVG", 14, 7, "");

        //教科コンボ
        $query = knje372nQuery::getClassMst($model->oyear);
        ...
        knjCreateHidden($objForm, "SEQ", $model->field["SEQ"]);
        knjCreateHidden($objForm, "hidden_recommendation_cd", $model->recommendation_cd);
        knjCreateHidden($objForm, "hidden_seq", $model->seq);

        Query::dbCheckIn($db);

        if (VARS::get("cmd") != "edit") {
            $arg["jscript"] = "window.open('knje372nindex.php?cmd=list','left_frame');";
        }

        $arg["finish"]  = $objForm->get_finish();

        View::toHTML($model, "knje372nForm2.html", $arg);
    }
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    $result = $db->query(knje372nQuery::getSubclassList($model, "", ""));
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
