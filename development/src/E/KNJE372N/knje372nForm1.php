...
class knje372nForm1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"] = $objForm->get_start("list", "POST", "knje372nindex.php", "", "edit");

        $db = Query::dbCheckOut();

        //参照年度
        $query = knje372nQuery::getYear("RYEAR");
        $extra = "";
        ...
        //対象年度
        $query = knje372nQuery::getYear("OYEAR");
        $extra = "onchange=\" return btn_submit('changeOyear')\"";
        makeCmb($objForm, $arg, $db, $query, $model->oyear, "OYEAR", $extra, 1);

        //一覧表示
        $query = knje372nQuery::getSelectQuery($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            array_walk($row, "htmlspecialchars_array");
            $array = $db->getAll(knje372nQuery::getSubclassName($model->oyear, $row));
            $subclass = "";
            $sep = "";
            ...
            $row["RECOMMENDATION_CD"] = View::alink("knje372nindex.php", $row["RECOMMENDATION_CD"], "target=\"right_frame\"",
                                         array("cmd"               => "select",
                                               "RECOMMENDATION_CD" => $row["RECOMMENDATION_CD"],
                                               "SEQ"               => $row["SEQ"],
                                               "ASSESS_AVG"        => $row["ASSESS_AVG"],
                                               "CLASSCD"           => $row["CLASSCD"]));
        ...
        if ($model->cmd == "changeOyear") {
            $arg["reload"] = "window.open('knje372nindex.php?cmd=edit', 'right_frame')";
        }

        $arg["finish"]  = $objForm->get_finish();

        View::toHTML($model, "knje372nForm1.html", $arg);
    }
}
