...
class tuutihyou {
    function main(&$model) {
        $objForm = new form;
        $arg["start"] = $objForm->get_start("detail", "POST", "knje015index.php", "", "detail");
        ...
        //年度コンボ
        $query = knje015Query::getYear($model->schregno);
        ...
        //通知票所見
        $query = knje015Query::getRemark($model->schregno, $model->year);
        ...
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            ...
            $semester = $db->getone(knje015Query::getSemester($model->year, $row["SEMESTER"]));
...
function makeCmb(&$objForm, &$arg, $db, $query, $name, &$value, $extra, $size, $model) {
    ...
    if($name == "YEAR"){
        $value = ($value && $value_flg) ? $value : $model->gradeYear;
    } else {
