...
class sikaku {
    function main(&$model) {
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("list", "POST", "knje015index.php", "", "edit");
        ...
        $db = Query::dbCheckOut();
        if($model->schregno) {
            $result = $db->query(knje015Query::getAward($model));
        ...
        if (VARS::get("cmd") == "right_list"){ 
                $arg["reload"]  = "parent.edit_frame.location.href='knje015index.php?cmd=edit'";
        }
