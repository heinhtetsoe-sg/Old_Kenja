...
//ビュー作成用クラス
class knje010cSubForm6 {
    function main(&$model) {
        $objForm = new form;
        $arg = array();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("form6", "POST", "knje010cindex.php", "", "form6");
        ...
        $opt = array();
        $disabled = "disabled";
        $query = knje010cQuery::selectQueryAnnual_HTRAINREMARK_DAT($model);
        ...
        //指導要録所見（総合的な学習の時間）取得
        $query = knje010cQuery::selectQuery_Htrainremark_dat2($model);
        ...
        View::toHTML($model, "knje010cSubForm6.html", $arg);
    }
}
?>