...
class knje070eForm1
{
    ...
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knje070eForm1", "POST", "knje070eindex.php", "", "knje070eForm1");
        ...
        //クラス選択コンボボックス
        $query = knje070eQuery::getAuth(CTRL_YEAR, CTRL_SEMESTER, $model);
        $extra = "onchange=\"return btn_submit('knje070e');\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE_HR_CLASS", $model->field["GRADE_HR_CLASS"], $extra, 1, $model);

        //校種を取得
        $query = knje070eQuery::getSchoolKind(substr($model->field["GRADE_HR_CLASS"], 0, 2));
        ...
        //卒業可能学年チェック
        $query = knje070eQuery::checkGrdGrade(substr($model->field["GRADE_HR_CLASS"], 0, 2));
        $grdGrade = $db->getOne($query);

        if ($model->Properties["useProvFlg"] == 1 && $grdGrade > 0) {
            if ('' != $db->getOne(knje070eQuery::getTableColumn("STUDYREC_PROV_FLG_DAT", "PROV_TESTKINDCD"))) {
                //仮評定データ取得
                $prov = $db->getRow(knje070eQuery::getProvData($model), DB_FETCHMODE_ASSOC);
                //SCHREG_STUDYREC_DATの評定有チェック
                $valCnt = $db->getOne(knje070eQuery::getValuationCnt($model));
        ...
        $query = knje070eQuery::getSchoolDiv($model);
        ...
        //近大チェック
        $kindai_flg = $db->getOne(knje070eQuery::checkKindai());
        if (!$kindai_flg) {
            $arg["notKindai"] = '1';
        }
...
//        }
        ...
        //記載責任者コンボボックス
        $query = knje070eQuery::getStaffList();
        ...
        View::toHTML($model, "knje070eForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model, $max_len)
{
    ...
    $opt1 = $opt2 = array();
    //生徒一覧リストを作成する
    $query = knje070eQuery::getList($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        //クラス名称調整
    ...
    if ($model->select_data["selectdata"][0]) {
        $query = knje070eQuery::getList($model, $model->select_data["selectdata"]);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //クラス名称調整
            $zenkaku = (strlen($row["HR_NAME"]) - mb_strlen($row["HR_NAME"])) / 2;
...
//    if ($model->Properties["unUseCsvBtn_YorokuTyousa"] != "1" || $model->Properties["useKNJE070EJavaCsv"] == "1") {
//        //CSVボタン（useXLS=1 => 非表示）
//        if (!$model->Properties["useXLS"]) {
//            if ($model->Properties["useKNJE070EJavaCsv"] == "1") {
...
function makeHidden(&$objForm, $db, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJE070E");
