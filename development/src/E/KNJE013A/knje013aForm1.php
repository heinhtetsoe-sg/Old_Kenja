...
class knje013aForm1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knje013aForm1", "POST", "knje013aindex.php", "", "knje013aForm1");
        ...
        $extra = "onclick =\" return btn_submit('knje013a');\"";
        createRadio($objForm, $arg, "CHOICE", $model->field["CHOICE"], $extra, $opt_choice, count($opt_choice));

        //学期
        $getSemesterName = $db->getOne(knje013aQuery::getSemesterName());
        ...
        if ($model->field["CHOICE"] == "2") {
            $arg["gr_class"] = "ON";
            //クラスコンボボックス
            $extra = "onChange=\"return btn_submit('knje013a');\"";
            $query = knje013aQuery::getAuthClass($model);
        ...
        View::toHTML($model, "knje013aForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    if ($model->field["CHOICE"] == "1") {
        ...
        $arg["CHANGENAME"] = "生徒";
        $query = knje013aQuery::getAuthStudent($model);
    }
    ...
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        if ($model->field["CHOICE"] == "1") {
            $getCount = $db->getOne(knje013aQuery::getHexamEntremarkCountHrclass($row));
            $setValueFlg = ($getCount == 0) ? '　' : '有';
            if ($upCheckFlg == 0) {
                $upCheckFlg = ($getCount == 0) ? "0" : "1";
            }
        } else {
            $getCount = $db->getOne(knje013aQuery::getHexamEntremarkTrainrefCountSchreg($row, ""));
            $setValueFlg = ($getCount == 0) ? '　' : '有';
            if ($upCheckFlg == 0) {
                $upCheckFlg = ($getCount == 0) ? "0" : "1";
            }
...
{
    knjCreateHidden($objForm, "PRGID", "KNJE013A");
