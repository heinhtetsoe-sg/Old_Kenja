...
class knje443aForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knje443aForm1", "POST", "knje443aindex.php", "", "knje443aForm1");

        //DB接続
        $db = Query::dbCheckOut();

        //教育委員会チェック
        $getFlg = $db->getOne(knje443aQuery::getNameMst());
        if ($getFlg !== '1') {
            $arg["jscript"] = "OnEdboardError();";
        }

        //年度
        $extra = "onChange=\"return btn_submit('knje443a');\"";
        $query = knje443aQuery::getYear();
        ...
        View::toHTML($model, "knje443aForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, &$model) {
    $row1 = array();
    $query = knje443aQuery::getSchoolData($model);
    $result = $db->query($query);
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
