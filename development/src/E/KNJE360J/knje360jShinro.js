function btn_submit(cmd) {
    ...
    if (cmd == "shinro_update" && document.forms[0].SEQ.value == "") {
        alert("{rval MSG308}");
        return true;
    }

    if (cmd == "shinro_insert" || cmd == "shinro_update") {
...
function updateNextStudent(schregno, order) {
    ...
    for (var i = 0; i < parent.left_frame.document.links.length; i++) {
        ...
        if (arr[1] == "SCHREGNO=" + schregno) {
            ...
            nextURL = parent.left_frame.document.links[idx].href.replace("edit", "shinro"); //上記の結果
            break;
        }
    }
    document.forms[0].cmd.value = "shinro_insert";
    //クッキー書き込み
