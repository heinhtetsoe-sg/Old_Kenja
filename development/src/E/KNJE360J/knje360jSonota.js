function btn_submit(cmd) {
    ...
    if (cmd == "sonota_update" && document.forms[0].SEQ.value == "") {
        alert("{rval MSG308}");
        return true;
    }

    if (cmd == "sonota_insert" || cmd == "sonota_update") {
...
function updateNextStudent(schregno, order) {
    ...
    for (var i = 0; i < parent.left_frame.document.links.length; i++) {
        ...
        if (arr[1] == "SCHREGNO=" + schregno) {
            ...
            nextURL = parent.left_frame.document.links[idx].href.replace("edit", "sonota"); //上記の結果
            break;
        }
    }
    document.forms[0].cmd.value = "sonota_insert";
    //クッキー書き込み
