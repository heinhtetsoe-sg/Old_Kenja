...
class knje360jSonota
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("sonota", "POST", "knje360jindex.php", "", "sonota");
        ...
        //その他進路コンボ表示設定
        $model->getname = $db->getOne(knje360jQuery::getHyoujiset());
        if ($model->getname === 'kyoto') {
            $arg["SONOTA_SHINRO"] = '1';
        }

        //警告メッセージを表示しない場合
        if ($model->cmd == "sonotaA" || $model->cmd == "sonota_clear") {
            if (isset($model->schregno) && !isset($model->warning) && $model->seq) {
                $Row = $db->getRow(knje360jQuery::getSubQuery5($model), DB_FETCHMODE_ASSOC);
                $arg["NOT_WARNING"] = 1;
            } else {
                $Row =& $model->field;
            }
        ...
        if ($model->getname === 'kyoto') {
            $query = knje360jQuery::getNameMst('E022');
        ...
        View::toHTML($model, "knje360jSonota.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg, $model)
{
    ...
    //追加ボタンを作成する
    $arg["button"]["btn_insert"] = knjCreateBtn($objForm, "btn_insert", "追 加", "onclick=\"return btn_submit('sonota_insert');\"");
    //追加後前の生徒へを作成する
    ...
    //更新ボタンを作成する
    $extra = ($model->seq == "") ? "disabled" : "onclick=\"return btn_submit('sonota_update');\"";
    $arg["button"]["btn_update"] = knjCreateBtn($objForm, "btn_update", "更 新", $extra);
    //クリアボタンを作成する
    $extra = ($model->seq == "") ? "disabled" : "onclick=\"return btn_submit('sonota_clear');\"";
