...
class knje360jShinro
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("shinro", "POST", "knje360jindex.php", "", "shinro");
        ...
        $setval = array();
        $firstflg = true;   //初回フラグ
        $query = knje360jQuery::getSubQuery1List($model);
        ...
        if ($model->schregno && $cnt) {
            ...
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                ...
                if ($row["COURSE_KIND"] == "1") {
                    //学校名
                    $sch_array = $db->getAssoc(knje360jQuery::getSchoolName($model, $row["ENTRYDATE"], $row["SEQ"]));
                    for ($i=1; $i<7; $i++) {
                        if ($i < 3) {
        ...
        //警告メッセージを表示しない場合
        if ($model->cmd == "shinroA" || $model->cmd == "shinro_clear") {
        ...
        $extra = "onchange=\"return btn_submit('shinro');\"";
        ...
        for ($i=1; $i<7; $i++) {
            ...
            if ($i > 2) {
                ...
                if ($model->cmd == "shinroA" || $model->cmd == "shinro_clear") {
                    if (isset($model->schregno) && !isset($model->warning) && $model->entrydate && $model->seq) {
                        $RowD = $db->getRow(knje360jQuery::getSubQuery1Detail($model, $i), DB_FETCHMODE_ASSOC);
                    } else {
                        $RowD =& $model->field;
                    }
                } else {
            ...
            //学校情報
            $college1 = $db->getRow(knje360jQuery::getCollegeInfo($Row["SCHOOL_CD".$i], $Row["FACULTYCD".$i], $Row["DEPARTMENTCD".$i]), DB_FETCHMODE_ASSOC);
            ...
            //学校検索ボタン
            $extra = "onclick=\"loadwindow('" .REQUESTROOT."/X/KNJXSEARCH_FINSCHOOL/knjwfin_searchindex.php?cmd=&target_number={$i}',event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 650, 600)\"";
        ...
        makeCmb($objForm, $arg, $db, $query, "JOBTYPE_LCD1", $Row["JOBTYPE_LCD1"], "", 1, 1);
        ...
        $arg["data"]["WORK_AREA1"] = knjCreateCombo($objForm, "WORK_AREA1", $Row["WORK_AREA1"], $opt_area1, "", 1);

        //紹介区分ラジオボタン 1:学校紹介 2:自己・縁故 3.公務員
        $opt_intro1 = array(1, 2, 3);
        $Row["INTRODUCTION_DIV1"] = ($Row["INTRODUCTION_DIV1"] == "") ? "1" : $Row["INTRODUCTION_DIV1"];
        $extra = array("id=\"INTRODUCTION_DIV11\"", "id=\"INTRODUCTION_DIV12\"", "id=\"INTRODUCTION_DIV13\"");
        $radioArray = knjCreateRadio($objForm, "INTRODUCTION_DIV1", $Row["INTRODUCTION_DIV1"], $extra, $opt_intro1, get_count($opt_intro1));
        ...
        //職業種別（大分類）
        $query = knje360jQuery::getJobtypeLList();
        makeCmb($objForm, $arg, $db, $query, "JOBTYPE_LCD2", $Row["JOBTYPE_LCD2"], "", 1, 1);
        ...
        $arg["data"]["WORK_AREA2"] = knjCreateCombo($objForm, "WORK_AREA2", $Row["WORK_AREA2"], $opt_area2, "", 1);

        //紹介区分ラジオボタン 1:学校紹介 2:自己・縁故 3.公務員
        $opt_intro2 = array(1, 2, 3);
        $Row["INTRODUCTION_DIV2"] = ($Row["INTRODUCTION_DIV2"] == "") ? "1" : $Row["INTRODUCTION_DIV2"];
        $extra = array("id=\"INTRODUCTION_DIV21\"", "id=\"INTRODUCTION_DIV22\"", "id=\"INTRODUCTION_DIV23\"");
        $radioArray = knjCreateRadio($objForm, "INTRODUCTION_DIV2", $Row["INTRODUCTION_DIV2"], $extra, $opt_intro2, get_count($opt_intro2));
        foreach ($radioArray as $key => $val) {
            $arg["data"][$key] = $val;
        }
        ...
        View::toHTML($model, "knje360jShinro.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg, $model)
{
    ...
    //一括更新（進学）ボタン
    $link = REQUESTROOT."/E/KNJE360J/knje360jindex.php?cmd=replace1&SCHREGNO=".$model->schregno."&SEQ=".$model->seq;
    $extra = "onclick=\"window.open('$link','_self');\"";
    $arg["button"]["btn_replace"] = KnjCreateBtn($objForm, "btn_replace", "一括更新\n（進学）", $extra.$disabled);

    //追加ボタンを作成する
    $arg["button"]["btn_insert"] = knjCreateBtn($objForm, "btn_insert", "追 加", "onclick=\"return btn_submit('shinro_insert');\"");
    //追加後前の生徒へを作成する
    ...
    //更新ボタンを作成する
    $extra = ($model->seq == "") ? "disabled" : "onclick=\"return btn_submit('shinro_update');\"";
    $arg["button"]["btn_update"] = knjCreateBtn($objForm, "btn_update", "更 新", $extra);
    //クリアボタンを作成する
    $extra = ($model->seq == "") ? "disabled" : "onclick=\"return btn_submit('shinro_clear');\"";
