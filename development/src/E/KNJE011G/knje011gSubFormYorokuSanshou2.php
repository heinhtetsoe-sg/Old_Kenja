...
//ビュー作成用クラス
class knje011gSubFormYorokuSanshou2
{
    function main(&$model)
    {
        $objForm = new form();
        $arg = array();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("formYorokuSanshou2", "POST", "knje011gindex.php", "", "formYorokuSanshou2");
        ...
        $opt = array();
        $disabled = "disabled";
        $query = knje011gQuery::selectQueryAnnual_HTRAINREMARK_DAT($model);
        ...
        //指導要録所見（総合的な学習の時間）取得
        $query = knje011gQuery::selectQuery_Htrainremark_dat2($model);
        ...
        View::toHTML($model, "knje011gSubFormYorokuSanshou2.html", $arg);
    }
}
