...
class knje011gForm2
{
    public function main(&$model)
    {
        ...
        $arg = array();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("form2", "POST", "knje011gindex.php", "", "form2");
        ...
        $disabled = "disabled";
        $query = knje011gQuery::selectQueryAnnual_knje011gForm2($model);
        ...
        $query = knje011gQuery::getGradeCd($model, "set");
        ...
        if (!isset($model->warning) && $model->cmd != 'reload4') {
            if ($model->cmd == "reload2_ok" || $model->cmd == "reload2_cancel") {
                $query = knje011gQuery::selectQuery_Htrainremark_Dat($model);
                ...
                if ($model->cmd !== 'torikomi3' && $model->cmd !== 'torikomi4') {
                    $query = knje011gQuery::selectQueryForm2($model);
                    $row = $db->getRow($query, DB_FETCHMODE_ASSOC);
        ...
        View::toHTML($model, "knje011gForm2.html", $arg);
    }
}
...
function makeAttendrecRemark(&$objForm, &$arg, $model, $db, $disabled, $row)
{
    ...
    if ($model->cmd === 'torikomi3') {
        $set_remark = knje011gQuery::getSemesRemark($model, $db, $model->annual["YEAR"]);
        $row["ATTENDREC_REMARK"] = $set_remark;
    } elseif ($model->cmd === 'torikomi4') {
        $set_remark = knje011gQuery::getHreportremarkDetailDat($db, $model);
        $row["ATTENDREC_REMARK"] = $set_remark;
    }
...
function makeSpecialactrec(&$objForm, &$arg, $model, $db, $disabled, $row)
{
    ...
    if ($model->getSchoolName == "koma") {
        ...
        //臘八摂心皆勤
        $rouhatsuKaikin = "";
        $query = knje011gQuery::getRouhatsuKaikin($model);
...
function makeShojikou(&$objForm, &$arg, $model, $db, $disabled, $row)
{
    ...
    if ("1" == $model->Properties["seitoSidoYorokuSogoShoken3Bunkatsu"] || $model->getSchoolName == 'tokiwa') {
        ...
        $query = knje011gQuery::yoroku_trainref123($model, $model->annual["YEAR"]);
        ...
        $query = knje011gQuery::yoroku_sogoshoken($model);
        $sansyou = $db->getRow($query, DB_FETCHMODE_ASSOC);
...
//                $rowYouroku = array();
//                $query = knje011gQuery::getYouroku($model, $model->annual["YEAR"]);
...
//                $query = knje011gQuery::get_record_totalstudytime_dat($model, $model->annual["YEAR"]);
//                $result = $db->query($query);
...
function makeSelectBtn(&$objForm, $model, $div, $name, $label, $target, $disabled = "")
{
    if (!$div || !$name || !$label || !$target) {
        ...
        if ($div == "club") {       //部活動
            ...
            $tgt = "/E/KNJE011G/knje011gindex.php";
            $w = 900;
            $h = 600;
