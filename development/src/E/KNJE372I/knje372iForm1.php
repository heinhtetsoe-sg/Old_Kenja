...
class knje372iForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knje372iForm1", "POST", "knje372iindex.php", "", "knje372iForm1");
        ...
        //学期コンボ作成
        $query = knje372iQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knje372i');\"";
        ...
        //対象学期の開始日・終了日
        $sDate = $db->getOne(knje372iQuery::getSemesterDate("SDATE", $model->field["SEMESTER"]));
        $eDate = $db->getOne(knje372iQuery::getSemesterDate("EDATE", $model->field["SEMESTER"]));
        ...
        $extra = array("id=\"DISP1\" onClick=\"return btn_submit('knje372i')\"", "id=\"DISP2\" onClick=\"return btn_submit('knje372i')\"");
        ...
        if ($model->field["DISP"] == 1) {
            //学年コンボ
            $query = knje372iQuery::getGrade($model);
            $extra = "onChange=\"return btn_submit('knje372i');\"";
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
        } elseif ($model->field["DISP"] == 2) {
            //クラスコンボ作成
            $query = knje372iQuery::getHrClass(CTRL_YEAR, $seme, $model);
            $extra = "onchange=\"return btn_submit('knje372i');\"";
            makeCmb($objForm, $arg, $db, $query, "GRADE_HR_CLASS", $model->field["GRADE_HR_CLASS"], $extra, 1);
        }
        
        //テスト種別コンボ
        $query = knje372iQuery::getTestItem($model, $model->field["SEMESTER"]);
        makeCmb($objForm, $arg, $db, $query, "TESTKINDCD", $model->field["TESTKINDCD"], "STYLE=\"WIDTH:140\" ", 1);
        
        //出力日付
        $model->field["OUTPUTDATE"] = isset($model->field["OUTPUTDATE"]) ? $model->field["OUTPUTDATE"] : str_replace("-", "/", CTRL_DATE);
        ...
        View::toHTML($model, "knje372iForm1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model, $seme)
{
    ...
    //対象外の生徒取得
    $query = knje372iQuery::getSchnoIdou($model, $seme);
    ...
    //対象者リストを作成する
    if ($model->field["DISP"] == 2) {
        $query = knje372iQuery::getStudent($model, $seme);
        $result = $db->query($query);
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJE372I");
