...
class knje010SubForm8 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform8", "POST", "knje010index.php", "", "subform8");
        ...
        if ($db->getOne(knje010Query::checkSchoolMst()) > 0) {
            $schoolcd       = sprintf("%012d", SCHOOLCD);
            $school_kind    = $db->getOne(knje010Query::getSchoolKind($model));
        }
        ...
        //出欠の記録
        $sick = $late = $early = 0;
        $result = $db->query(knje010Query::getAttendSemesDat($model, $knjSchoolMst));
        ...
        View::toHTML($model, "knje010SubForm8.html", $arg);
    }
}
?>