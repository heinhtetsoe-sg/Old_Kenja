...
class knje010Form1 {
    function main(&$model) {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knje010index.php", "", "edit");
        $db = Query::dbCheckOut();

        if (!isset($model->warning) && $model->cmd != 'yomikomi') {
            $query = knje010Query::selectQuery($model);
        ...
        //年次取得
        $model->gradecd = "";
        $model->gradecd = $db->getOne(knje010Query::getGradeCd($model));
        ...
        if ($model->Properties["tyousasyoSougouHyoukaNentani"] != 1) {
            if ($model->cmd == "yomikomi") {
                ...
                if (intval($model->gradecd) < "3") {
                    $query = knje010Query::getYouroku($model);
                ...
                //3年次　通知票取込
                    ...
                    $query = knje010Query::get_record_totalstudytime_dat($model);
            ...
            $arg["TOTALSTUDYACT"] = KnjCreateTextArea($objForm, "TOTALSTUDYACT", $model->totalstudyact_gyou, ((int)$model->totalstudyact_moji * 2 + 1), "soft", "style=\"height:{$height}px;\"", $row["TOTALSTUDYACT"]);
            ...
            $arg["TOTALSTUDYVAL"] = KnjCreateTextArea($objForm, "TOTALSTUDYVAL", $model->totalstudyval_gyou, ((int)$model->totalstudyval_moji * 2 + 1), "soft", "style=\"height:{$height}px;\"", $row["TOTALSTUDYVAL"]);
        ...
        $arg["REMARK"] = KnjCreateTextArea($objForm, "REMARK", $model->remark_gyou, ((int)$model->remark_moji * 2 + 1), "soft", "style=\"height:{$height}px;\"", $row["REMARK"]);
        $arg["REMARK_TYUI"] = "(全角{$model->remark_moji}文字X{$model->remark_gyou}行まで)";

        //備考2
        $height = (int)$model->remark2_gyou * 13.5 + ((int)$model->remark2_gyou -1 ) * 3 + 5;
        $arg["REMARK2"] = KnjCreateTextArea($objForm, "REMARK2", $model->remark2_gyou, ((int)$model->remark_moji * 2 + 1), "soft", "style=\"height:{$height}px;\"", $row["REMARK2"]);
        ...
        //セキュリティーチェック
        $securityCnt = $db->getOne(knje010Query::getSecurityHigh());
        $csvSetName = "CSV";
        ...
        if ($model->getPrgId || !$model->Properties["useXLS"] || $securityCnt == 0) {
            //データCSVボタン
            $extra = "onClick=\" wopen('".REQUESTROOT."/X/KNJX190/knjx190index.php?program_id=".PROGRAMID."&mode={$model->mode}&SEND_PRGID=KNJE010&SEND_AUTH={$model->auth}','SUBWIN2',0,0,screen.availWidth,screen.availHeight);\"";
            $arg["button"]["btn_check1"] = knjCreateBtn($objForm, "btn_check1", "データ".$csvSetName, $extra);
            //ヘッダデータCSVボタン
            $extra = "onClick=\" wopen('".REQUESTROOT."/X/KNJX191/knjx191index.php?program_id=".PROGRAMID."&mode={$model->mode}&SEND_PRGID=KNJE010&SEND_AUTH={$model->auth}','SUBWIN2',0,0,screen.availWidth,screen.availHeight);\"";
        ...
        //備考一括更新
        $link = REQUESTROOT."/X/KNJX_HEXAM_REMARK/knjx_hexam_remarkindex.php?program_id=".PROGRAMID."&mode={$model->mode}&SEND_PRGID=KNJE010&SEND_AUTH={$model->auth}&EXP_YEAR={$model->annual["YEAR"]}&EXP_SEMESTER={$model->exp_semester}&SCHREGNO={$model->schregno}&NAME={$model->name}&GRADE={$model->grade}&HR_CLASS={$model->hrClass}";
        ...
        //所見確認用パラメータ
        if ($model->Properties["tyousasyoShokenPreview"] == '1') {
            knjCreateHidden($objForm, "PRGID", "KNJE010");
            ...
            $query = knje010Query::getSchoolDiv();
        ...
        View::toHTML($model, "knje010Form1.html", $arg);
    }
}
?>
