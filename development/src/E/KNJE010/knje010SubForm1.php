...
//ビュー作成用クラス
class knje010SubForm1 {
    function main(&$model) {
        $objForm = new form;
        $arg = array();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("form3", "POST", "knje010index.php", "", "form3");
        ...
        //中高一貫校
        $JH = (get_count($db->getOne(knje010Query::getSchoolName('JH'))) > 0) ? "1" : "";
        ...
        $query = knje010Query::selectQueryAnnual($model, $JH);
        ...
        $query = knje010Query::selectQueryForm3($model, $JH);
        $result = $db->query($query);
        ...
        View::toHTML($model, "knje010SubForm1.html", $arg);
    }
}
?>