...
class knje010Form2 {
    function main(&$model) {
        ...
        $arg = array();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("form2", "POST", "knje010index.php", "", "form2");

        $arg["NAME_SHOW"] = $model->schregno."　".$model->name;

        $model->getPropertiesAll();
        ...
        $disabled = "disabled";
        $query = knje010Query::selectQueryAnnual_knje010Form2($model);
        ...
        $query = knje010Query::getGradeCd($model, "set");
        ...
        if (!isset($model->warning) && $model->cmd != 'reload4') {
            if ($model->cmd == "reload2_ok" || $model->cmd == "reload2_cancel") {
                $query = knje010Query::selectQuery_Htrainremark_Dat($model);
                ...
                if ($model->cmd !== 'torikomi3') {
                    $query = knje010Query::selectQueryForm2($model);
                    $row = $db->getRow($query, DB_FETCHMODE_ASSOC);
                } else {
                    $row = $model->field2;
                }
        ...
        if ($model->Properties["useSyojikou3"] == "1") {
            $query = knje010Query::sansyou_data($model);
        ...
        //出欠の記録備考取込
        if ($model->cmd === 'torikomi3') {
            $set_remark = knje010Query::getSemesRemark($model, $db, $model->annual["YEAR"]);
        ...
        //特別活動の記録
        $arg["SPECIALACTREC"] = KnjCreateTextArea($objForm, "SPECIALACTREC", ((int)$model->specialactrec_gyou + 1), ((int)$model->specialactrec_moji * 2 + 1), "soft", "", $row["SPECIALACTREC"]);
        ...
        View::toHTML($model, "knje010Form2.html", $arg);
    }
}
