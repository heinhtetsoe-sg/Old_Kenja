...
class knje010SubForm5 {
    function main(&$model) {
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("list", "POST", "knje010index.php", "", "edit");
        ...
        $db = Query::dbCheckOut();
        if($model->schregno) {
            $result = $db->query(knje010Query::getAward($model));
        ...
        if (VARS::get("cmd") == "right_list"){ 
                $arg["reload"]  = "parent.edit_frame.location.href='knje010index.php?cmd=edit'";
        }

        View::toHTML($model, "knje010SubForm5.html", $arg);
    }
}
