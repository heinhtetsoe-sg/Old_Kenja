...
class knje410Form1
{
    public function main(&$model)
    {
        ...
        $objForm = new form();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knje410index.php", "", "edit");

        $db  = Query::dbCheckOut();
        $db2 = Query::dbCheckOut2();

        //年度
        $query = knje410Query::getYear();
        $extra = "onchange=\"return btn_submit('main');\"";
        makeCmb($objForm, $arg, $db, $query, "YEAR", $model->field["YEAR"], $extra, 1, "");

        //V_SCHOOL_MSTから学校コードを取得
        $query = knje410Query::getSchoolMst($model);
        ...
        //文書番号
        $query = knje410Query::getTuutatu($model);
        $extra = "onchange=\"return btn_submit('main');\"";
        ...
        //項目名(列)
        $nameArray = array("SHINGAKU_DAIGAKU"
                          ,"SHINGAKU_TANDAI"
                          ,"SHINGAKU_SENMON"
                          ,"SHINGAKU_SONOTA"
                          ,"SHINGAKU_REMARK"
        ...
        $count = 0;
        $bifKey = "";
        $result = $db->query(knje410Query::readQuery($model));
        while ($Row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            foreach ($nameArray as $name) {
                ...
                $readStyle = "";
                $setSize = "3";
                if ($name == "SHINGAKU_GOUKEI" || $name == "SHUSHOKU_GOUKEI" || $name == "ZENTAI_GOUKEI") {
                    $readOnly = " readonly ";
        ...
        //報告履歴
        $query = knje410Query::getReport($model);
        $extra = "";
        ...
        View::toHTML($model, "knje410Form1.html", $arg);
    }
}
