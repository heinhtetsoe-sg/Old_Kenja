...
class knje020Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knje020index.php", "", "edit");
        ...
        if ($model->warning == "") {
            if ($model->cmd === 'tsuchiTorikomi') {
                ...
                $query = knje020Query::getReportRemarkDat($model);
        ...
        //就職用欠席理由
        if ($model->cmd === 'tsuchiTorikomi') {
            $set_remark = knje020Query::getHreportremarkDetailDat($db, $model);
        ...
        //健康診断詳細データ取得
        $query = knje020Query::getMedexamDetDat($model);
        ...
        //視力（右）
        if ($medexam_row["R_VISION_MARK_NAME"]) {
            $arg["data"]["R_VISION_MARK"] = $medexam_row["R_BAREVISION_MARK_NAME"]."　( ".$medexam_row["R_VISION_MARK_NAME"]." )";
        } else {
            $arg["data"]["R_VISION_MARK"] = $medexam_row["R_BAREVISION_MARK_NAME"]."　(　　)";
        }
        //視力（左）
        if ($medexam_row["L_VISION_MARK_NAME"]) {
            $arg["data"]["L_VISION_MARK"] = $medexam_row["L_BAREVISION_MARK_NAME"]."　( ".$medexam_row["L_VISION_MARK_NAME"]." )";
        } else {
            $arg["data"]["L_VISION_MARK"] = $medexam_row["L_BAREVISION_MARK_NAME"]."　(　　)";
        }
        //聴力（右）
        ...
        //指導要録データ
        $sido_cnt = 0;
        $query = knje020Query::getHtrainremarkDat($model);
        ...
        View::toHTML($model, "knje020Form1.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg, $db, $model)
{
    ...
    if ($model->schoolName == "koma") {
        ...
        //臘八摂心皆勤
        $rouhatsuKaikin = "";
        $query = knje020Query::getRouhatsuKaikin($model);
    ...
    //データCSVボタン
    //セキュリティーチェック
    $securityCnt = $db->getOne(knje020Query::getSecurityHigh());
    $csvSetName = "CSV";
...
function makeHidden(&$objForm, $db, $model, $row)
{
    ...
    if ($model->Properties["tyousasyoShokenPreview"] == '1') {
        ...
        $query = knje020Query::getSchoolDiv();
...
{
    //参照パターン
    $query = knje020Query::getPatternSeq($model);
