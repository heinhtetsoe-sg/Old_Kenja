...
class knje020SubForm1
{
    public function main(&$model)
    {
        ...
        $arg = array();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("subform1", "POST", "knje020index.php", "", "subform1");
        ...
        //中高一貫校
        $JH = (get_count($db->getOne(knje020Query::getSchoolName('JH'))) > 0) ? "1" : "";

        //SQL文発行
        //年度（年次）取得
        $query = knje020Query::selectQueryAnnual($model, $JH);
        ...
        $query = knje020Query::selectQuerySubForm1($model, $JH);
        $result = $db->query($query);
        ...
        View::toHTML($model, "knje020SubForm1.html", $arg);
    }
}
