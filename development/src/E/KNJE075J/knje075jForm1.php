...
class knje075jForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knje075jForm1", "POST", "knje075jindex.php", "", "knje075jForm1");
        ...
        if ($model->select_data["selectdata"][0]) {
            for ($i = 0; $i < get_count($model->select_data["selectdata"]); $i++) {
                $query = knje075jQuery::getList2($model->select_data["selectdata"][$i], $model);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJE075J");
        ...
        View::toHTML($model, "knje075jForm1.html", $arg); 
    }
}
