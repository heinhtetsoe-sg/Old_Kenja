...
//ビュー作成用クラス
class knje010dSubFormYorokuSanshou2 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("formYorokuSanshou2", "POST", "knje010dindex.php", "", "formYorokuSanshou2");
        ...
        $opt = array();
        $disabled = "disabled";
        $query = knje010dQuery::selectQueryAnnual_HTRAINREMARK_DAT($model);
        ...
        //指導要録所見（総合的な学習の時間）取得
        $query = knje010dQuery::selectQuery_Htrainremark_dat2($model);
        ...
        $arg["SIDO_TOTALSTUDYACT"] = KnjCreateTextArea($objForm, "SIDO_TOTALSTUDYACT", $model->mojigyou["SIDO_TOTALSTUDYACT"]["gyou"], ((int)$model->mojigyou["SIDO_TOTALSTUDYACT"]["moji"] * 2 + 1), "soft", $extra.$extra2, $row["TOTALSTUDYACT"]);

        //評価
        $height = ((int)$model->mojigyou["SIDO_TOTALSTUDYVAL"]["gyou"] - 1) * 14 + 20;
        $extra2 = " style=\"height:{$height}px;\" ";
        $arg["SIDO_TOTALSTUDYVAL"] = KnjCreateTextArea($objForm, "SIDO_TOTALSTUDYVAL", $model->mojigyou["SIDO_TOTALSTUDYVAL"]["gyou"], ((int)$model->mojigyou["SIDO_TOTALSTUDYVAL"]["moji"] * 2 + 1), "soft", $extra.$extra2, $row["TOTALSTUDYVAL"]);
        ...
        View::toHTML($model, "knje010dSubFormYorokuSanshou2.html", $arg);
    }
}
?>