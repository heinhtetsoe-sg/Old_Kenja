...
class knje010dForm1
{
    public function main(&$model)
    {
        $objForm = new form();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knje010dindex.php", "", "edit");
        $db = Query::dbCheckOut();

        $model->schoolKind = $db->getOne(knje010dQuery::getSchoolKind($model));
        ...
        if (!isset($model->warning) && !in_array($model->cmd, array("yorokuYoriYomikomi_ok", "yorokuYoriYomikomi_cancel", "reload3")) && !preg_match('/(torikomi|torikomiT)[0-2]/', $model->cmd)) {
            $query = knje010dQuery::selectQuery($model);
            $row = $db->getRow($query, DB_FETCHMODE_ASSOC);
            $query = knje010dQuery::selectHexamEntremarkRemarkHdatQuery($model);
        ...
        //年次取得
        $query = knje010dQuery::getGradeCd($model);
        ...
        $model->getSchoolName = "";
        $model->getSchoolName = $db->getOne(knje010dQuery::getNameMst("Z010"));
        ...
        $getIppanCount = $db->getOne(knje010dQuery::getPositionCheck($model));
        ...
        View::toHTML($model, "knje010dForm1.html", $arg);
    }
}
...
function makeButton(&$objForm, &$arg, $db, &$model)
{
    ...
    if ("1" == $model->Properties["tyousasho6bunkatsu_homeroomShojikouTorikomi"]) {
        $query = knje010dQuery::getHrShojikouTorikomiDataCheck($model);
        $result = $db->query($query);
        $chk_hrname = "";
        $countYears = array();
    ...
    //セキュリティーチェック
    $securityCnt = $db->getOne(knje010dQuery::getSecurityHigh());
    $csvSetName = "CSV";
    ...
    if ($model->getPrgId || !$model->Properties["useXLS"] || $securityCnt == 0) {
        //データCSVボタン
        $extra = "onClick=\" wopen('".REQUESTROOT."/X/KNJX190/knjx190index.php?program_id=".PROGRAMID."&mode={$model->mode}&SEND_PRGID=KNJE010D&SEND_AUTH={$model->auth}&tyousasyo2020=1','SUBWIN2',0,0,screen.availWidth,screen.availHeight);\"";
        $arg["button"]["btn_check1"] = knjCreateBtn($objForm, "btn_check1", "データ".$csvSetName, $extra);
        //ヘッダデータCSVボタン
        $extra = "onClick=\" wopen('".REQUESTROOT."/X/KNJX191/knjx191index.php?program_id=".PROGRAMID."&mode={$model->mode}&SEND_PRGID=KNJE010D&SEND_AUTH={$model->auth}&tyousasyo2020=1','SUBWIN2',0,0,screen.availWidth,screen.availHeight);\"";
...
function makeAttendrecRemark(&$objForm, &$arg, $db, $model, $disabled, $gradeCnt, $grade, $year, $val, $slashFlg)
{
    ...
    if ($model->cmd === "torikomi".$gradeCnt) {
        $set_remark = knje010dQuery::getSemesRemark($model, $db, $year);
        $val = $set_remark;
    } elseif ($model->cmd === "torikomiT".$gradeCnt) {
        $set_remark = knje010dQuery::getHreportremarkDetailDat($db, $model, $year);
        $val = $set_remark;
    //対応しない学年の時は画面の値をセット
    } elseif (!in_array($model->cmd, array("yorokuYoriYomikomi_ok", "yorokuYoriYomikomi_cancel", "reload3", "yomikomi", "edit", "reset", "updEdit", "select_pattern"))) {
...
function makeSpecialactrec(&$objForm, &$arg, $db, $model, $disabled, $gradeCnt, $grade, $year, $val)
{
    ...
    if ($model->getSchoolName == "koma") {
        ...
        //臘八摂心皆勤
        $rouhatsuKaikin = "";
        $query = knje010dQuery::getRouhatsuKaikin($model, $year);
...
function makeShojikou(&$objForm, &$arg, $db, $model, $disabled, $gradeCnt, $grade, $year, $row)
{
    ...
    for ($i = 1; $i <= 6; $i++) {
        ...
        $height = $gyou * 13.5 + ($gyou - 1) * 3 + 5;
        $id = "TRAIN_REF".$i."-".$year;
        $extra1 = $disabled." id=\"".$id."\" style=\"height:{$height}px; overflow-y: {$innerScroll}; \" onPaste=\"return showPaste6Bunkatu(this);\" ";
...
function makeNendogoto(&$objForm, &$arg, $db, &$model)
{
    ...
    $query = knje010dQuery::getGdat($model);
    $result = $db->query($query);
    $opt_grades = array();
    ...
    foreach ($opt_grades as $r) {
        ...
        if (!isset($model->warning)) {
            if (($model->cmd == "yorokuYoriYomikomi_ok" || $model->cmd == "yorokuYoriYomikomi_cancel") && ($model->readYear == "0000" || $model->readYear == $year)) {
                $query = knje010dQuery::selectQuery_Htrainremark_Dat($model, $year);
                $row = $db->getRow($query, DB_FETCHMODE_ASSOC);
                ...
                for ($i = 1; $i <= 6; $i++) {
                    $row["TRAIN_REF".$i] = "";
                    $seq = "10".$i;
                    $query = knje010dQuery::getHexamEntremarkTrainrefDat($model, $year, $seq);
                ...
                if (!in_array($model->cmd, array('torikomi0', 'torikomi1', 'torikomi2', 'torikomiT0', 'torikomiT1', 'torikomiT2', "reload3", "yorokuYoriYomikomi_ok", "yorokuYoriYomikomi_cancel"))) {
                    $query = knje010dQuery::selectQueryForm2($model, $year);
                    $row = $db->getRow($query, DB_FETCHMODE_ASSOC);
                    for ($i = 1; $i <= 6; $i++) {
                        $row["TRAIN_REF".$i] = "";
                        $seq = "10".$i;
    ...
    if ("1" == $model->Properties["seitoSidoYorokuSogoShoken3Bunkatsu"] || $model->schoolName == 'tokiwa') {
        ...
        foreach ($opt_grades as $r) {
            ...
            $query = knje010dQuery::yoroku_trainref123($model, $year);
        ...
        foreach ($opt_grades as $r) {
            ...
            $query = knje010dQuery::yoroku_sogoshoken($model, $year);
            $sansyou = $db->getRow($query, DB_FETCHMODE_ASSOC);
    ...
    if (in_array($model->getSchoolName, array("chiyoda"))) {
        $arg["GRADE_TITLE1"] = "４年生";
        $arg["GRADE_TITLE2"] = "５年生";
        $arg["GRADE_TITLE3"] = "６年生";
    } else {
        $arg["GRADE_TITLE1"] = "１年生";
        $arg["GRADE_TITLE2"] = "２年生";
        $arg["GRADE_TITLE3"] = "３年生";
    }
}
...
//    if ($model->Properties["tyousasyoSougouHyoukaNentani"] != 1 || "1" == $model->Properties["tyousasyo2020"]) {
    ...
    if ($model->cmd == "yorokuYoriYomikomi_ok" || $model->cmd == "yorokuYoriYomikomi_cancel") {
        if ($model->Properties["sidouyourokuSansyou"] == 1) {
            $query = knje010dQuery::getYourokuDat($model);
        } else {
            $query = knje010dQuery::selectQuery_Htrainremark_Hdat($model);
        }
...
function makeBiko(&$objForm, &$arg, $db, &$model, &$row)
{
    ...
    if ("1" != $model->Properties["unuse_KNJE015_HEXAM_ENTREMARK_LEARNING_DAT_REMARK"]) {
        $extra = " onclick=\"loadwindow('".REQUESTROOT."/E/KNJE010D/knje010dindex.php?&cmd=formBikoTori&TARGET=REMARK',0,document.documentElement.scrollTop || document.body.scrollTop,1000,650);\"";
    ...
    if ("1" == $model->Properties["useTransferButton_H"]) {
        ...
        $extra .= $disabled;
        $arg["btn_transfer"] = makeSelectBtn($objForm, $model, "transfer", "btn_transfer", "異動情報選択", "REMARK", CTRL_YEAR, $disabled);
    }
}
...
function reload3setTotalStudy(&$row, $db, $model)
{
    ...
    if (intval($model->gradecd) < "3") {
        ...
        $query = knje010dQuery::get_record_totalstudytime_dat($model);
        $result = $db->query($query);
        while ($total_row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeSelectBtn(&$objForm, $model, $div, $name, $label, $target, $year, $disabled = "")
{
    if (!$div || !$name || !$label || !$target) {
        ...
        if ($div == "club") {                   //部活動
            ...
            $tgt = "/E/KNJE010D/knje010dindex.php";
            $sizeW = 900;
            $sizeH = 650;
...
function makeHidden(&$objForm, $db, &$model)
{
    ...
    if ($model->Properties["tyousasyoShokenPreview"] == '1') {
        ...
        knjCreateHidden($objForm, "PRGID", "KNJE010D");
        ...
        $query = knje010dQuery::getSchoolDiv($model);
...
{
    //参照パターン
    $query = knje010dQuery::getPatternSeq($model);
    ...
    if ($model->mode == "ungrd") {
        //指導要録所見一括取込ボタンを作成する
        $extra = "onclick=\"return btn_submit('formYourokuIkkatsuTorikomi_first');\" ";
        $title = "指導要録所見一括取込";
        $arg["btn_form_youroku_ikkatsu_torikomi"] = knjCreateBtn($objForm, "btn_form_youroku_ikkatsu_torikomi", $title, $extra);
    }
