...
class knje011SubForm9 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform9", "POST", "knje011index.php", "", "subform9");
        ...
        if ($db->getOne(knje011Query::checkSchoolMst()) > 0) {
            $schoolcd       = sprintf("%012d", SCHOOLCD);
            $school_kind    = $db->getOne(knje011Query::getSchoolKind($model));
        }
        ...
        //出欠の記録
        $sick = $late = $early = 0;
        $result = $db->query(knje011Query::getAttendSemesDat($model, $knjSchoolMst));
        ...
        View::toHTML($model, "knje011SubForm9.html", $arg);
    }
}
?>