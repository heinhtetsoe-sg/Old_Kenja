...
//ビュー作成用クラス
class knje011SubForm1 {
    function main(&$model) {
        $objForm = new form;
        $arg = array();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("form3", "POST", "knje011index.php", "", "form3");
        ...
        //中高一貫校
        $JH = (get_count($db->getOne(knje011Query::getSchoolName('JH'))) > 0) ? "1" : "";
        ...
        $query = knje011Query::selectQueryAnnual($model, $JH);
        ...
        $query = knje011Query::selectQueryForm3($model, $JH);
        $result = $db->query($query);
        ...
        View::toHTML($model, "knje011SubForm1.html", $arg);
    }
}
?>