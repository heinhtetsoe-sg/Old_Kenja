...
class knje011SubForm5 {
    function main(&$model) {
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("list", "POST", "knje011index.php", "", "edit");
        ...
        $db = Query::dbCheckOut();
        if($model->schregno) {
            $result = $db->query(knje011Query::getAward($model));
        ...
        if (VARS::get("cmd") == "right_list"){ 
                $arg["reload"]  = "parent.edit_frame.location.href='knje011index.php?cmd=edit'";
        }

        View::toHTML($model, "knje011SubForm5.html", $arg);
    }
}
