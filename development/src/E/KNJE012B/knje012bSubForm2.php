...
class knje012bSubForm2
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform2", "POST", "knje012bindex.php", "", "subform2");
        ...
        if ($db->getOne(knje012bQuery::checkSchoolMst()) > 0) {
            ...
            $school_kind    = $db->getOne(knje012bQuery::getSchoolKind($model));
        }
        ...
        for ($i=1; $i<=$model->control["学期数"]; $i++) {
            $result = $db->query(knje012bQuery::getAttendSemesDat($model, $i, $knjSchoolMst));
        ...
        View::toHTML($model, "knje012bSubForm2.html", $arg);
    }
}
