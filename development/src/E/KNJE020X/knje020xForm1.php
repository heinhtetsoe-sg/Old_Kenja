...
class knje020xForm1
{
    ...
    {
        $objForm = new form;
        $arg["start"] = $objForm->get_start("edit", "POST", "knje020xindex.php", "", "edit");

        $db = Query::dbCheckOut();

        //年度学期
        $query = knje020xQuery::getYearSeme();
        ...
        //年組
        $query = knje020xQuery::getHrClass($model);
        $extra = "onChange=\"return btn_submit('edit');\"";
        makeCmb($objForm, $arg, $db, $query, $model->field["HR_CLASS"], "HR_CLASS", $extra, 1);

        //入力項目
        $query = knje020xQuery::getInputVal($model);
        $extra = "onChange=\"return btn_submit('edit');\"";
        ...
        $query = knje020xQuery::getStudentsVal($model, $tableNm, $fieldNm);
        ...
        View::toHTML($model, "knje020xForm1.html", $arg);
    }
}
