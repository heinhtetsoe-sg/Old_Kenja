...
require_once('knje011eModel.inc');
require_once('knje011eQuery.inc');

class knje011eController extends Controller
{
    public $ModelClassName = "knje011eModel";
    public $ProgramID      = "KNJE011E";
    ...
    public function main()
    {
        ...
        while ( true ) {
            switch (trim($sessionInstance->cmd)) {
                ...
                case "form2":       //出欠の～
                    $sessionInstance->setAccessLogDetail("S", $ProgramID);
                    $this->callView("knje011eForm2");
                    break 2;
                case "formSeiseki_first": //「成績参照」の最初の呼出
                case "formSeiseki":       //「成績参照」
                    $sessionInstance->setAccessLogDetail("S", $ProgramID);
                    $this->callView("knje011eSubFormSeiseki");
                    break 2;
                case "formYorokuSanshou_first": //「指導要録参照」の最初の呼出
                case "formYorokuSanshou":       //「指導要録参照」
                    $sessionInstance->setAccessLogDetail("S", $ProgramID);
                    $this->callView("knje011eSubFormYorokuSanshou");
                    break 2;
                case "formYorokuSanshou2_first": //「指導要録参照」の最初の呼出
                case "formYorokuSanshou2":       //「指導要録参照」
                    $sessionInstance->setAccessLogDetail("S", $ProgramID);
                    $this->callView("knje011eSubFormYorokuSanshou2");
                    break 2;
                case "formShojikouTori":     //「指導上参考となる諸事項選択」
                    $sessionInstance->setAccessLogDetail("S", $ProgramID);
                    $this->callView("knje011eSubFormShojikouTori");
                    break 2;
                case "formBikouTori":     //「備考」
                    $sessionInstance->setAccessLogDetail("S", $ProgramID);
                    $this->callView("knje011eSubFormBikouTori");
                    break 2;
                case "update":
                    $this->checkAuth(DEF_UPDATE_RESTRICT, "knje011eForm1");
                    $sessionInstance->setAccessLogDetail("U", $ProgramID);
                    $sessionInstance->getUpdateModel();
                    $sessionInstance->setCmd("updEdit");
                    break 1;
                case "update2":
                    $this->checkAuth(DEF_UPDATE_RESTRICT, "knje011eForm2");
                    $sessionInstance->setAccessLogDetail("U", $ProgramID);
                    $sessionInstance->getUpdateModel();
                ...
                case "hrShojikouTorikomi":
                    ...
                    break 1;
                case "replace":
                    $this->callView("knje011eSubForm3"); //一括更新画面
                    break 2;
                case "replace_update":
                ...
                case "":
                    //分割フレーム作成
                    $args["left_src"] = REQUESTROOT ."/X/KNJXEXP/index.php?PROGRAMID=" .$this->ProgramID ."&TARGET=right_frame&PATH=" .urlencode("/E/KNJE011E/knje011eindex.php?cmd=edit") ."&button=3";
                    ...
                    $args["right_src"] = "knje011eindex.php?cmd=edit&init=1";
                    $args["cols"] = "23%,*";
                    View::frame($args);
                    exit;
                case "back":
                    ...
                    $args["right_src"] = "knje011eindex.php?cmd=edit";
                    $args["cols"] = "23%,*";
                    View::frame($args);
                    exit;
                default:
...
$knje011eCtl = new knje011eController();
//var_dump($_REQUEST);
