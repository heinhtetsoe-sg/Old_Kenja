...
class knje011eForm1
{
    public function main(&$model)
    {
        $objForm = new form();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knje011eindex.php", "", "edit");
        $db = Query::dbCheckOut();

        if (!isset($model->warning) && $model->cmd != 'reload3') {
            $query = knje011eQuery::selectQuery($model);
            $row = $db->getRow($query, DB_FETCHMODE_ASSOC);

            $query = knje011eQuery::getHexamEntremarkRemarkHdat($model);
...
//        }
        ...
        //年次取得
        $model->gradecd = "";
        $model->gradecd = $db->getOne(knje011eQuery::getGradeCd($model));
        ...
        View::toHTML($model, "knje011eForm1.html", $arg);
    }
}
...
function makeBiko(&$arg, &$objForm, $model, &$row)
{
    ...
    if ("1" != $model->Properties["unuse_KNJE015_HEXAM_ENTREMARK_LEARNING_DAT_REMARK"]) {
        $extra = " onclick=\"loadwindow('".REQUESTROOT."/E/KNJE011E/knje011eindex.php?&cmd=formBikouTori&TARGET=REMARK',0,document.documentElement.scrollTop || document.body.scrollTop,1000,650);\"";
...
function makeButton(&$arg, &$objForm, $db, $model)
{
    ...
    if ("1" == $model->Properties["tyousasho6bunkatsu_homeroomShojikouTorikomi"]) {
        $query = knje011eQuery::getHrShojikouTorikomiDataCheck($model);
    ...
    //セキュリティーチェック
    $securityCnt = $db->getOne(knje011eQuery::getSecurityHigh());
    $csvSetName = "CSV";
    ...
    if ($model->getPrgId || !$model->Properties["useXLS"] || $securityCnt == 0) {
        //データCSVボタン
        $extra = "onClick=\" wopen('".REQUESTROOT."/X/KNJX190/knjx190index.php?program_id=".PROGRAMID."&mode={$model->mode}&SEND_PRGID=KNJE011E&SEND_AUTH={$model->auth}&tyousasyo2020=1','SUBWIN2',0,0,screen.availWidth,screen.availHeight);\"";
        $arg["button"]["btn_check1"] = knjCreateBtn($objForm, "btn_check1", "データ".$csvSetName, $extra);
        //ヘッダデータCSVボタン
        $extra = "onClick=\" wopen('".REQUESTROOT."/X/KNJX191/knjx191index.php?program_id=".PROGRAMID."&mode={$model->mode}&SEND_PRGID=KNJE011E&SEND_AUTH={$model->auth}&tyousasyo2020=1','SUBWIN2',0,0,screen.availWidth,screen.availHeight);\"";
...
//    //備考一括更新
//    $link = REQUESTROOT."/X/KNJX_HEXAM_REMARK/knjx_hexam_remarkindex.php?program_id=".PROGRAMID."&mode={$model->mode}&SEND_PRGID=KNJE011E&SEND_AUTH={$model->auth}&EXP_YEAR={$model->annual["YEAR"]}&EXP_SEMESTER={$model->exp_semester}&SCHREGNO={$model->schregno}&NAME={$model->name}&GRADE={$model->grade}&HR_CLASS={$model->hrClass}";
...
function makeHidden(&$objForm, $db, $model)
{
    ...
    //プレビュー／印刷用パラメータ
    if ($model->Properties["tyousasyoShokenPreview"] == '1') {
        knjCreateHidden($objForm, "PRGID", "KNJE011E");
        ...
        $query = knje011eQuery::getSchoolDiv($model);
...
function torikomiRowTotalstudy($db, $model, &$row)
{
    if (intval($model->gradecd) < "3") {
        //1,2年次　指導要録取込
        $query = knje011eQuery::getYouroku($model);
        ...
        $query = knje011eQuery::get_record_totalstudytime_dat($model, $model->exp_year);
        $result = $db->query($query);
