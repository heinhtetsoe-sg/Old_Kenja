...
//ビュー作成用クラス
class knje011eSubFormYorokuSanshou2
{
    function main(&$model)
    {
        $objForm = new form();
        $arg = array();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("formYorokuSanshou2", "POST", "knje011eindex.php", "", "formYorokuSanshou2");
        ...
        $opt = array();
        $disabled = "disabled";
        $query = knje011eQuery::selectQueryAnnual_HTRAINREMARK_DAT($model);
        ...
        //指導要録所見（総合的な学習の時間）取得
        $query = knje011eQuery::selectQuery_Htrainremark_dat2($model);
        ...
        $arg["TOTALSTUDYACT"] = KnjCreateTextArea($objForm, "TOTALSTUDYACT", $model->mojigyou["SIDO_TOTALSTUDYACT"]["gyou"], ($model->mojigyou["SIDO_TOTALSTUDYACT"]["moji"] * 2 + 1), "soft", $extra.$extra2, $row["TOTALSTUDYACT"]);

        //評価
        $height = ($model->mojigyou["SIDO_TOTALSTUDYVAL"]["gyou"] - 1) * 14 + 20;
        $extra2 = " style=\"height:{$height}px;\" ";
        $arg["TOTALSTUDYVAL"] = KnjCreateTextArea($objForm, "TOTALSTUDYVAL", $model->mojigyou["SIDO_TOTALSTUDYVAL"]["gyou"], ($model->mojigyou["SIDO_TOTALSTUDYVAL"]["moji"] * 2 + 1), "soft", $extra.$extra2, $row["TOTALSTUDYVAL"]);
        ...
        View::toHTML($model, "knje011eSubFormYorokuSanshou2.html", $arg);
    }
}
