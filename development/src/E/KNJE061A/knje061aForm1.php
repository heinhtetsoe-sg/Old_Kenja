...
class knje061aForm1
{
    public function main(&$model)
    {
        ...
        //学年
        $result = $db->query(knje061aQuery::selectQueryAnnual($model));
        $opt = array();
        ...
        $result = $db->query(knje061aquery::selectQueryHRClass($model));
        $opt = array();
        ...
        //コース
        $result = $db->query(knje061aquery::selectCourse($model));
        $opt = array();
        $opt[] = (array("label" => "", "value" => ""));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //生徒
        $result = $db->query(knje061aquery::selectSchregno($model));
        $opt = array();
        $opt[] = (array("label" => "", "value" => ""));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knje061aindex.php", "", "main");
        $arg["finish"]  = $objForm->get_finish();

        View::toHTML($model, "knje061aForm1.html", $arg);
    }
}
