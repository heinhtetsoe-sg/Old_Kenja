...
class knje063form2
{
    public function main(&$model)
    {
        $objForm = new form();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knje063index.php", "", "edit");
        ...
        if ($model->Properties["useCurriculumcd"] == '1') {
            //教育課程
            $query = knje063Query::getCurriculum();
        ...
        //checkbox
        $query = knje063Query::getMaxRegdYear($model);
        ...
        if ($model->Properties["knje063showValuationCreditNote"] == "1") {
            $arg["showValuationCreditNote"] = "1";
            $arg["totalwidth"] = "1000";
        } else {
            $arg["totalwidth"] = "700";
        }
        ...
        //履歴表示
        $this->makeListRireki($objForm, $db, $arg, $model);
        ...
        if (isset($model->message)) {
            $arg["reload"]  = "window.open('knje063index.php?cmd=list&SCHREGNO=$model->schregno','top_frame');";
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knje063Form2.html", $arg);
    }

    //表示データ作成
    public function makeSelectData($db, &$model)
    {
        $Row = array();
        $Row["YEAR"]                = "";
        $Row["SUBCLASSCD"]          = "";
        ...
        if (isset($model->schregno) && !isset($model->warning)) {
            if ($model->cmd == "add_year" || $model->cmd == "subclasscd") {
                ...
                $query = knje063Query::selectQuery($model);
    ...
    //年度作成
    public function makeYear($db, &$model)
    {
        //年度取得
        $query = knje063Query::selectQueryYear($model);
        $result = $db->query($query);
        $make_year = array();
    ...
    //教科作成
    public function makeClass(&$objForm, $db, &$arg, &$model)
    {
        ...
        while ($row_c = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            if ($model->class_select=="" && $i==0) {
                ...
                $i++;
            } elseif ($model->class_select==$classcd) {
                $class_chk = $classcd;
                $show["CLASSNAME"]        = $row_c["CLASSNAME"];
                $show["CLASSABBV"]        = $row_c["CLASSABBV"];
                $show["CLASSNAME_ENG"] = $row_c["CLASSNAME_ENG"];
    ...
    //科目作成
    public function makeSubclass(&$objForm, $db, &$arg, $class_chk, $Row, $model)
    {
    ...
    //評定、単位作成
    public function makeSetData(&$objForm, $db, &$arg, $model, $Row)
    {
        //設定種別にあわせてrowspan値を変化
        $arg["span"] = (int)$model->grade_range + 1;
        //設定種別名称設定
        $schoolName[0] = "在籍中";
        $schoolName[1] = "在籍前";
        $schoolName[2] = "高認試験";
        ...
        for ($i = 0; $i < $model->grade_range; $i++) {
            ...
            $query = knje063Query::getStudyRecCnt($Row, $i, $model);
    ...
    //履歴表示
    public function makeListRireki(&$objForm, $db, &$arg, $model)
    {
        ...
        if (isset($model->schregno)) {
            $query = knje063Query::getListRireki($model);
            $result = $db->query($query);
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                $row["CALC_DATE"] = str_replace("-", "/", $row["CALC_DATE"]);
                $row["SCHOOLNAME"] = $schoolName[$row["SCHOOLCD"]];
                $row["CMD_DIV_NAME"] = $cmdDivName[$row["CMD_DIV"]];
                ...
                $lineKey = $row["CALC_DATE"]."-".$row["CALC_TIME"]."-".$row["SCHOOLCD"]."-".$row["YEAR"]."-".$row["ANNUAL"]."-".$row["SUBCLASSCD"];
                if ($firstflg || ($lineKey == $lineKeyPre)) {
                    //同一処理日時、データ種別、年次、科目のデータを連結(初回データ設定も含む)
    ...
    //
    public function setData($row, &$setval, $firstflg, $join = "")
    {
        if ($firstflg || $join != "join") {
            $setval = $row;
        }
        //仮評定(在籍中のみ)
        if ($row["SCHOOLCD"] == "0" && $row["PROV_FLG"]) {
            $setval["PROV_FLG".$row["BEF_AFT_DIV"]] = 'レ';
        }
        $setval["VALUATION".$row["BEF_AFT_DIV"]]    = $row["VALUATION"];
        $setval["GET_CREDIT".$row["BEF_AFT_DIV"]]   = $row["GET_CREDIT"];
        $setval["ADD_CREDIT".$row["BEF_AFT_DIV"]]   = $row["ADD_CREDIT"];
        $setval["COMP_CREDIT".$row["BEF_AFT_DIV"]]  = $row["COMP_CREDIT"];
        if ($row["YUUKOU_FLG"]) {
            $setval["YUUKOU_FLG".$row["BEF_AFT_DIV"]] = 'レ';
        }
        $setval["REMARK1"] = $row["REMARK1"];
        $setval["REMARK2"] = $row["REMARK2"];
        $setval["COLOR"] = "white";
    ...
    //hidden作成
    public function makeHidden(&$objForm, $Row)
    {
        //cmd
