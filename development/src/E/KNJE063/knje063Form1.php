...
class knje063form1
{
    public function main(&$model)
    {
        ...
        $genKyuu = $db->getOne(knje063Query::getGenkyuu());
        $schoolDiv = $db->getOne(knje063Query::getSchoolMst());
        ...
        if ($model->schregno) {
            $colorJudgeAr = array();
            $query = knje063Query::selectQuery($model, "HENSYU", "CHECK_COLOR");
        ...
        //CSV処理ボタン
        $extra = ($model->schregno) ? "onClick=\" wopen('".REQUESTROOT."/X/KNJX_E063REMARK/knjx_e063remarkindex.php?PROGRAMID=".PROGRAMID."&SEND_PRGID=".PROGRAMID."&SEND_AUTH=".AUTHORITY."&EXP_YEAR={$model->exp_year}&EXP_SEMESTER={$model->exp_semester}&GRADE={$model->grade}&HR_CLASS={$model->hr_class}','SUBWIN2',0,0,screen.availWidth,screen.availHeight);\"" : "disabled";
