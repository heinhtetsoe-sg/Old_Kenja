...
class knje071Form1
{
    ...
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knje071Form1", "POST", "knje071index.php", "", "knje071Form1");
        ...
        //クラス選択コンボボックス
//        $query = knje071Query::getAuth(CTRL_YEAR, CTRL_SEMESTER, $model);
//        $extra = "onchange=\"return btn_submit('knje071');\"";
//        makeCmb($objForm, $arg, $db, $query, "GRADE_HR_CLASS", $model->field["GRADE_HR_CLASS"], $extra, 1);

        //年組のMAX文字数取得
        $query = knje071Query::getList($model);
        $max_len = 0;
...
//        //卒業可能学年チェック
//        $query = knje071Query::checkGrdGrade(substr($model->field["GRADE_HR_CLASS"],0,2));
//        $grdGrade = $db->getOne($query);

//        if ($model->Properties["useProvFlg"] == 1 && $grdGrade > 0) {
//            if ('' != $db->getOne(knje071Query::getTableColumn("STUDYREC_PROV_FLG_DAT", "PROV_TESTKINDCD"))) {
//                //仮評定データ取得
//                $prov = $db->getRow(knje071Query::getProvData($model), DB_FETCHMODE_ASSOC);
//                //SCHREG_STUDYREC_DATの評定有チェック
//                $valCnt = $db->getOne(knje071Query::getValuationCnt($model));
//
...
//        }
        ...
        $query = knje071Query::getSchoolDiv($model);
        ...
        //近大チェック
        $kindai_flg = $db->getOne(knje071Query::checkKindai());
        if (!$kindai_flg) {
            $arg["notKindai"] = '1';
        }
        ...
        //記載責任者コンボボックス
        $query = knje071Query::getStaffList();
        ...
        View::toHTML($model, "knje071Form1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model, $max_len)
{
    ...
    if ($model->select_data["selectdata"][0]) {
        $query = knje071Query::getList($model, $model->select_data["selectdata"]);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //クラス名称調整
            $zenkaku = (strlen($row["HR_NAME"]) - mb_strlen($row["HR_NAME"])) / 2;
...
function makeHidden(&$objForm, $db, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJE071");
