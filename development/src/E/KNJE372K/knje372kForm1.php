...
class knje372kForm1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knje372kForm1", "POST", "knje372kindex.php", "", "knje372kForm1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        $extra = array("id=\"CATEGORY_IS_CLASS1\" onClick=\"return btn_submit('knje372k')\"", "id=\"CATEGORY_IS_CLASS2\" onClick=\"return btn_submit('knje372k')\"");
        ...
        //クラスコンボ作成
        $query = knje372kQuery::getGradeHrClass($model);
        $extra = "onchange=\"return btn_submit('knje372k'), AllClearList();\"";
        ...
        View::toHTML($model, "knje372kForm1.html", $arg); 
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["CATEGORY_IS_CLASS"] == 1) {
        ...
        //対象外の生徒取得
        $query = knje372kQuery::getSchnoIdou($model);
        $result = $db->query($query);
        $opt_idou = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //対象者リストを作成する
        $query = knje372kQuery::getStudent($model);
        $result = $db->query($query);
        $opt1 = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $idou = "　";
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJE372K");
