...
class knje011aSubForm3
{
    function main(&$model)
    {
        $objForm        = new form;
        $arg["start"]   = $objForm->get_start("sel", "POST", "knje011aindex.php", "", "sel");
        $arg["jscript"] = "";
        $db = Query::dbCheckOut();
        ...
        $result = $db->query(knje011aQuery::GetStudent($model));
        ...
        $arg["data"]["REMARK"] = KnjCreateTextArea($objForm, "REMARK", $model->remark_gyou, ((int)$model->remark_moji * 2 + 1), "soft", "style=\"height:{$height}px;\"", $Row["REMARK"]);
        $arg["data"]["REMARK_TYUI"] = "(全角{$model->remark_moji}文字X{$model->remark_gyou}行まで)";

        //備考2
        $extra = "onblur=\"this.value=toInteger(this.value)\";";
        $arg["data"]["REMARK2"] = KnjCreateTextArea($objForm, "REMARK2", $model->remark2_gyou, ((int)$model->remark_moji * 2 + 1), "soft", "style=\"height:{$height}px;\"", $Row["REMARK2"]);
        ...
        //年組名
        $hr_name = $db->getOne(knje011aQuery::getHR_Name($model));
        ...
        //戻るボタン
        $link = REQUESTROOT."/E/KNJE011A/knje011aindex.php?cmd=back&ini2=1";
        $objForm->ae( array("type"        => "button",
                            "name"        => "btn_back",
                            "value"       => "戻 る",
        ...
        View::toHTML($model, "knje011aSubForm3.html", $arg);
    }
}
?>
