# kanji=漢字
# $Id: readme.txt 74893 2020-06-16 05:11:34Z arakaki $
...
2010/04/05  1.「出欠の記録 ＆ 特別活動の記録 ＆ 指導上参考になる諸事項」のポップアップウィンドウのサイズ修正
            2.年度がない場合、「出欠備考参照」、「要録の出欠の記録備考参照」、「部活動参照」、
            　「委員会参照」、「資格参照」ボタンを使用不可にした。
            3.部活動参照、委員会参照画面の出席番号をカットして、学籍番号を表示した。
            4.年度取得に卒業生の条件が含まれていない不具合を修正した。
            5.学年（年次）コンボ取得ＳＱＬにソートを追加した。
...
2010/06/22  1.不要な関数のカット

2010/07/07  1.「部活動参照」の条件を04/01"以上"、03/31"以下"に修正
            2.3分割の桁数をプロパティーファイルで変更できるよう修正

2010/07/20  1.CSVの権限チェックに対応

2010/07/22  1.KNJE010Aをコピーして作成
            2.「特別活動の記録」のサイズをプロパティーで変更できるように修正
...
2011/05/26  1.データCSV、ヘッダデータCSVボタンの押下後の"MSG300"メッセージが出ないように修正
...
2015/02/26  1.ボタン名が全て表示されるよう修正
            2.総合的な学習のの時間の項目に学年(KNJA023のNAMESPARE2,3にて判断)により下記のボタンを追加および修正
              - 1,2年次は指導要録取込ボタンを追加
              - 3年次は通知票取込ボタンを追加
            3.ボタンレイアウト修正
            
2015/03/05  1.総合的な学習の時間の項目に学年の判断を修正
                - KNJA023のNAMESPARE2,3にて判断 → SCHREG_REGD_GDATのGRADE_CDにて判断

2015/03/27  1.京都仕様(名称マスタZ010のNAME1で判断)の場合、総合的な学習の時間の活動内容と評価は入力不可とする。
               - ただし、取込更新は可

2015/07/22  1.総合的な学習の時間の指導要録取込ボタンについて修正
               - プロパティ「tyousasyoSougouHyoukaNentani」!= '1'の時（通年の設定の時）、学年(年次)の年度を参照して、データを取り込むよう修正
...
2017/05/25  1.欠席日数参照ボタン追加(Form2.php)
            2.readme日付修正

2017/05/30  1.欠席日数参照ボタンをカット
            2.出欠の記録参照ボタン追加
...
2018/03/30  1.資格参照ボタンを検定選択ボタンに変更
            -- 選択学年のデータのみ一覧に表示する
...
2018/08/27  1.ボタンカット
            -- 委員会参照、表彰参照、部活動参照、大会記録備考参照
            2.ボタン追加
            -- 委員会選択、賞選択、部活動選択、記録備考選択
            3.出欠の記録＆特別活動～画面で更新時は更新ボタンを押下できないよう変更

2018/08/29  1.記録備考選択画面の戻るボタンの不具合修正

2018/12/25  1.共通プログラムに変更
...
2019/07/05  1.年次(ANNUAL)の表示はGRADE_CDがあれば優先して表示する
...
2020/01/27  1.プロパティtyousasyo2020が1の場合、調査書A4フォームへの変更にともなう修正
            ・指導上参考となる諸事項6分割はHEXAM_ENTREMARK_TRAINREF_DATを使用する
            ・備考はHEXAM_ENTREMARK_REMARK_HDATを使用する
            ・項目の番号と表示順変更
            ・指導上参考となる諸事項、備考の参照画面追加

2020/01/30  1.2020/01/27 1.の修正漏れ対応

2020/02/03  1.2020/01/27 1.、2020/01/30 1.の修正をカット (別IDのKNJE011Dで作成する）
