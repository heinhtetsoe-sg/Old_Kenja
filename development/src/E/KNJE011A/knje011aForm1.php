...
class knje011aForm1 {
    function main(&$model) {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knje011aindex.php", "", "edit");
        $db = Query::dbCheckOut();

        if (!isset($model->warning) && $model->cmd != 'reload3') {
            $query = knje011aQuery::selectQuery($model);
        ...
        //年次取得
        $model->gradecd = "";
        $model->gradecd = $db->getOne(knje011aQuery::getGradeCd($model));
        ...
        if ($model->Properties["tyousasyoSougouHyoukaNentani"] != 1) {
            if ($model->cmd == "yomikomi") {
                ...
                if (intval($model->gradecd) < "3") {
                    $query = knje011aQuery::getYouroku($model);
                ...
                //3年次　通知票取込
                    ...
                    $query = knje011aQuery::get_record_totalstudytime_dat($model, $model->exp_year);
        ...
        //セキュリティーチェック
        $securityCnt = $db->getOne(knje011aQuery::getSecurityHigh());
        $csvSetName = "CSV";
        ...
        if ($model->getPrgId || !$model->Properties["useXLS"] || $securityCnt == 0) {
            //データCSVボタン
            $extra = "onClick=\" wopen('".REQUESTROOT."/X/KNJX190/knjx190index.php?program_id=".PROGRAMID."&mode={$model->mode}&SEND_PRGID=KNJE011A&SEND_AUTH={$model->auth}','SUBWIN2',0,0,screen.availWidth,screen.availHeight);\"";
            $arg["button"]["btn_check1"] = knjCreateBtn($objForm, "btn_check1", "データ".$csvSetName, $extra);
            //ヘッダデータCSVボタン
            $extra = "onClick=\" wopen('".REQUESTROOT."/X/KNJX191/knjx191index.php?program_id=".PROGRAMID."&mode={$model->mode}&SEND_PRGID=KNJE011A&SEND_AUTH={$model->auth}','SUBWIN2',0,0,screen.availWidth,screen.availHeight);\"";
        ...
        //備考一括更新
        $link = REQUESTROOT."/X/KNJX_HEXAM_REMARK/knjx_hexam_remarkindex.php?program_id=".PROGRAMID."&mode={$model->mode}&SEND_PRGID=KNJE011A&SEND_AUTH={$model->auth}&EXP_YEAR={$model->annual["YEAR"]}&EXP_SEMESTER={$model->exp_semester}&SCHREGNO={$model->schregno}&NAME={$model->name}&GRADE={$model->grade}&HR_CLASS={$model->hrClass}";
        ...
        //所見確認用パラメータ
        if ($model->Properties["tyousasyoShokenPreview"] == '1') {
            knjCreateHidden($objForm, "PRGID", "KNJE011A");
            ...
            $query = knje011aQuery::getSchoolDiv();
        ...
        View::toHTML($model, "knje011aForm1.html", $arg);
    }
}
?>
