...
//ビュー作成用クラス
class knje011aSubForm1 {
    function main(&$model) {
        $objForm = new form;
        $arg = array();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("form3", "POST", "knje011aindex.php", "", "form3");
        ...
        //中高一貫校
        $JH = (get_count($db->getOne(knje011aQuery::getSchoolName('JH'))) > 0) ? "1" : "";
        ...
        $query = knje011aQuery::selectQueryAnnual($model, $JH);
        ...
        $query = knje011aQuery::selectQueryForm3($model, $JH);
        $result = $db->query($query);
        ...
        View::toHTML($model, "knje011aSubForm1.html", $arg);
    }
}
?>