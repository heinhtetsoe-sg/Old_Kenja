...
class knje070bForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knje070bForm1", "POST", "knje070bindex.php", "", "knje070bForm1");
        ...
        //クラス選択コンボボックス
        $query = knje070bQuery::getAuth(CTRL_YEAR, CTRL_SEMESTER, $model);
        $extra = "onchange=\"return btn_submit('knje070b');\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE_HR_CLASS", $model->field["GRADE_HR_CLASS"], $extra, 1, $model);

        //校種を取得
        $query = knje070bQuery::getSchoolKind(substr($model->field["GRADE_HR_CLASS"],0,2));
        ...
        //卒業可能学年チェック
        $query = knje070bQuery::checkGrdGrade(substr($model->field["GRADE_HR_CLASS"],0,2));
        $grdGrade = $db->getOne($query);

        if ($model->Properties["useProvFlg"] == 1 && $grdGrade > 0) {
            if ('' != $db->getOne(knje070bQuery::getTableColumn("STUDYREC_PROV_FLG_DAT", "PROV_TESTKINDCD"))) {
                //仮評定データ取得
                $prov = $db->getRow(knje070bQuery::getProvData($model), DB_FETCHMODE_ASSOC);
                //SCHREG_STUDYREC_DATの評定有チェック
                $valCnt = $db->getOne(knje070bQuery::getValuationCnt($model));
        ...
        $query = knje070bQuery::getSchoolDiv($model);
        ...
        //近大チェック
        $kindai_flg = $db->getOne(knje070bQuery::checkKindai());
        if (!$kindai_flg) {
            $arg["notKindai"] = '1';
        }
        ...
        //記載責任者コンボボックス
        $query = knje070bQuery::getStaffList();
        ...
        View::toHTML($model, "knje070bForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model, $max_len) {
    ...
    $opt1 = $opt2 = array();
    //生徒一覧リストを作成する
    $query = knje070bQuery::getList($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        //クラス名称調整
        ...
        if (!in_array($row["SCHREGNO"], $model->select_data["selectdata"])) {
            $credit_prefix = ($row["CREDIT"] < 77) ? "●" : "";
            $opt1[]= array('label' => $row["SCHREGNO"].' '.$hr_name.' '.$row["ATTENDNO"].'番 '. $credit_prefix . $row["NAME_SHOW"],
                           'value' => $row["VALUE"]);
        }
    }
    $result->free();
    ...
    if ($model->select_data["selectdata"][0]) {
        $query = knje070bQuery::getList($model, $model->select_data["selectdata"]);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //クラス名称調整
            $zenkaku = (strlen($row["HR_NAME"]) - mb_strlen($row["HR_NAME"])) / 2;
            ...
            for ($j=0; $j < ($max_len - ($zenkaku * 2 + $hankaku)); $j++) $hr_name .= "&nbsp;";
            $credit_prefix = ($row["CREDIT"] < 77) ? "●" : "";
            $opt2[]= array('label' => $row["SCHREGNO"].' '.$hr_name.' '.$row["ATTENDNO"].'番 '. $credit_prefix .$row["NAME_SHOW"],
                           'value' => $row["VALUE"]);
        }
        $result->free();
    }
...
function makeBtn(&$objForm, &$arg, $model) {
    ...
    if ($model->Properties["unUseCsvBtn_YorokuTyousa"] != "1" || $model->Properties["useKNJE070BJavaCsv"] == "1") {
        //CSVボタン（useXLS=1 => 非表示）
        if (!$model->Properties["useXLS"]) {
            if ($model->Properties["useKNJE070BJavaCsv"] == "1") {
...
function makeHidden(&$objForm, $db, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJE070B");
