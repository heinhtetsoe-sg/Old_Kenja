...
class knje420aForm1 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]  = $objForm->get_start("knje420aForm1", "POST", "knje420aindex.php", "", "knje420aForm1");

        //DB接続
        $db = Query::dbCheckOut();

        //教育委員会チェック
        $getFlg = $db->getOne(knje420aQuery::getNameMst());
        if ($getFlg !== '1') {
            $arg["jscript"] = "OnEdboardError();";
        }

        //年度
        $extra = "onChange=\"return btn_submit('knje420a');\"";
        $query = knje420aQuery::getYear();
        ...
        View::toHTML($model, "knje420aForm1.html", $arg); 
    }
}
...
{
    $row1 = array();
    $query = knje420aQuery::getSchoolData($model);
    $result = $db->query($query);
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
...
function makeHidden(&$objForm, $model, $db) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJE420A");
}
?>
