...
class knje390SubForm3_6
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform3_6", "POST", "knje390index.php", "", "subform3_6");
        ...
        $query = knje390Query::getSupportSchoolRecordDiv();
        ...
        if ($model->field3["RECORD_DIV"] !== '1') {
            ...
            if ($model->cmd == "subform3_school_set"){
                if (isset($model->schregno) && !isset($model->warning)){
                    $Row = $db->getRow(knje390Query::getSubQuery3SchoolGetData($model, $model->field3["RECORD_DIV"]), DB_FETCHMODE_ASSOC);
                } else {
                    $Row =& $model->field3;
                }
            } else {
            ...
            if (isset($model->schregno) && !isset($model->warning)){
                $Row2 = $db->getRow(knje390Query::getSubQuery3SchoolGetData($model, "1"), DB_FETCHMODE_ASSOC);
            } else {
                $Row2 =& $model->field3;
            }
            //将来像
        ...
        View::toHTML($model, "knje390SubForm3_6.html", $arg); 
    }
}
...
function makeList(&$arg, $db, $model) {
    $retCnt = 0;
    $query = knje390Query::getSubQuery3SchoolRecordList($model);
...
{
    //D様式参照ボタン
    $extra = "onclick=\"loadwindow('" .REQUESTROOT."/E/KNJE390/knje390index.php?cmd=subform2_growup_B_sanshou&MAIN_YEAR=".$model->main_year."&SCHREGNO=".$model->schregno."&RECORD_DATE=".$model->record_date."', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 650, 600)\"";
