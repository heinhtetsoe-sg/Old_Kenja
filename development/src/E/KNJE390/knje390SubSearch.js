...
window.onload = function () { 
        if (sessionStorage.getItem("KNJE390SubSearch_CurrentCursor") != null) {
        document.title = "";
        if (sessionStorage.getItem("KNJE390SubSearch_CurrentCursor") == 'btn_search') {
            document.getElementById('search_result').focus();
        }
        // remove item
        sessionStorage.removeItem('KNJE390SubSearch_CurrentCursor');  
    } else {
        // first loading focus
...
function current_cursor(para) {
    sessionStorage.setItem("KNJE390SubSearch_CurrentCursor", para);
}
...
function btn_submit(cmd) {
    ...
    if (sessionStorage.getItem("KNJE390SubSearch_CurrentCursor") != null) {
        document.title = "";
        document.getElementById(sessionStorage.getItem("KNJE390SubSearch_CurrentCursor")).blur();
    }
