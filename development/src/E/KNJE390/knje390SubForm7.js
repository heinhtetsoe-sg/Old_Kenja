...
window.onload = function () {
    if (sessionStorage.getItem("KNJE390SubForm7_CurrentCursor915") != null) {
        // textbox 915 error
        document.getElementsByName(sessionStorage.getItem("KNJE390SubForm7_CurrentCursor915"))[0].focus();
        var value = document.getElementsByName(sessionStorage.getItem("KNJE390SubForm7_CurrentCursor915"))[0].value;
        document.getElementsByName(sessionStorage.getItem("KNJE390SubForm7_CurrentCursor915"))[0].value = "";
        document.getElementsByName(sessionStorage.getItem("KNJE390SubForm7_CurrentCursor915"))[0].value = value;
        sessionStorage.removeItem("KNJE390SubForm7_CurrentCursor915");
    } else {
        sessionStorage.removeItem("KNJE390SubForm7_CurrentCursor915");
        if (sessionStorage.getItem("KNJE390SubForm7_CurrentCursor") != null) {
            document.title = "";
            document.getElementById(sessionStorage.getItem("KNJE390SubForm7_CurrentCursor")).focus();
            // remove item
...
function current_cursor(para) {
    sessionStorage.setItem("KNJE390SubForm7_CurrentCursor", para);
}
...
function btn_submit(cmd) {
    ...
    if (sessionStorage.getItem("KNJE390SubForm7_CurrentCursor") != null) {
        document.title = "";
        document.getElementById(sessionStorage.getItem("KNJE390SubForm7_CurrentCursor")).blur();
    }
    ...
    if (cmd == 'subform7_formatnew') {
        if (!confirm('{rval MSG108}')) {
            return false;
        }
    }
    if (cmd == 'subform7_updatemain') {
        //作成年月日チェック
   ...
   if (document.forms[0].SCHREGNO.value == "") {
    ...
    for (var i = 0; i < parent.left_frame.document.links.length; i++) {
          ...
          if (arr[1] == "SCHREGNO="+schregno) {
            ...
            nextURL = parent.left_frame.document.links[idx].href.replace("edit","subform7");    //上記の結果
            break;
        }
    }
    document.forms[0].cmd.value = 'subform7_updatemain';
    //クッキー書き込み
