...
class knje390SubSearch
{
    function main(&$model)
    {
        ...
        View::toHTML($model, "knje390SubSearch.html", $arg); 
    }
}
...
function makeList(&$objForm, &$arg, $db, $model) {
    $i = 0;
    $query = knje390Query::getSearchWelfareService($model);
    ...
    $extra = "id=\"btn_sentaku\" onclick=\"parent.current_cursor_Inspection(); current_cursor('btn_sentaku'); return btn_check_submit('".$i."')\" aria-label=\"選択\"";
...
function makeBtn(&$objForm, &$arg)
{
    //検索ボタン
    // Add by PP for PC-Talker  2020-02-03 start
    $extra = " id=\"btn_search\" onclick=\"current_cursor('btn_search'); return btn_submit('welfare_useservice_search')\" aria-label=\"検索\"";
