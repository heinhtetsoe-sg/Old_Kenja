...
window.onload = function () {
    if (sessionStorage.getItem("KNJE390SubForm5_CurrentCursor915") != null) {
        document.getElementsByName(sessionStorage.getItem("KNJE390SubForm5_CurrentCursor915"))[0].focus();
        var value = document.getElementsByName(sessionStorage.getItem("KNJE390SubForm5_CurrentCursor915"))[0].value;
        document.getElementsByName(sessionStorage.getItem("KNJE390SubForm5_CurrentCursor915"))[0].value = "";
        document.getElementsByName(sessionStorage.getItem("KNJE390SubForm5_CurrentCursor915"))[0].value = value;
        sessionStorage.removeItem("KNJE390SubForm5_CurrentCursor915");
    } else {
        sessionStorage.removeItem("KNJE390SubForm5_CurrentCursor915");
        if (sessionStorage.getItem("KNJE390SubForm5_CurrentCursor") != null) {
            document.title = "";
            document.getElementById(sessionStorage.getItem("KNJE390SubForm5_CurrentCursor")).focus();
            // remove item
...
function current_cursor(para) {
    sessionStorage.setItem("KNJE390SubForm5_CurrentCursor", para);
}
...
function btn_submit(cmd) {
    ...
    if (sessionStorage.getItem("KNJE390SubForm5_CurrentCursor") != null) {
        document.title = "";
        document.getElementById(sessionStorage.getItem("KNJE390SubForm5_CurrentCursor")).blur();
    }
    ...
    if (cmd == 'subform5_formatnew') {
        if (!confirm('{rval MSG108}')) {
            return false;
        }
    }
    if (cmd == 'subform5_updatemain') {
        //作成年月日チェック
    ...
    for (var i = 0; i < parent.left_frame.document.links.length; i++) {
          ...
          if (arr[1] == "SCHREGNO="+schregno) {
            ...
            if (order == 0 && i == parent.left_frame.document.links.length-1) {
              ...
              nextURL = parent.left_frame.document.links[idx].href.replace("edit", "subform5");    //上記の結果
            break;
        }
    }
    document.forms[0].cmd.value = 'subform5_updatemain';
    //クッキー書き込み
