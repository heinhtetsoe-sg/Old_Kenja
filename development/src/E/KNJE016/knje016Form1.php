...
class knje016Form1
{
    ...
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knje016Form1", "POST", "knje016index.php", "", "knje016Form1");
        ...
        //学期コンボ
        $extra = "onChange=\"return btn_submit('knje016');\"";
        $query = knje016Query::getSemester();
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //表示指定ラジオボタン 1:クラス 2:個人
        $opt_disp = array(1, 2);
        $model->field["DISP"] = ($model->field["DISP"] == "") ? "1" : $model->field["DISP"];
        $extra = array("id=\"DISP1\" onClick=\"return btn_submit('knje016')\"", "id=\"DISP2\" onClick=\"return btn_submit('knje016')\"");
        ...
        //1行のみ出力チェックボックス
        $extra  = $model->field["ONELINE"] || $model->cmd == '' ? " checked " : "";
        $extra .= "id=\"ONELINE\"";
        $arg["data"]["ONELINE"] = knjCreateCheckBox($objForm, "ONELINE", "on", $extra);

        //画面サイズ切替
        $arg["data"]["WIDTH"] = ($model->field["DISP"] == 1) ? "650" : "800";

        if ($model->field["DISP"] == 1) {
            //学年コンボ
            $extra = "onChange=\"return btn_submit('knje016');\"";
            $query = knje016Query::getGrade($model);
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
        } elseif ($model->field["DISP"] == 2) {
            //年組コンボ
            $extra = "onChange=\"return btn_submit('knje016');\"";
            $query = knje016Query::getHrClass($model);
        ...
        View::toHTML($model, "knje016Form1.html", $arg);
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model)
{
    ...
    if ($model->field["DISP"] == 2) {
        //生徒取得
        $query = knje016Query::getSchList($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //クラス名称調整
...
function makeHidden(&$objForm, $model)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJE016");
