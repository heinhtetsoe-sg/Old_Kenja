...
class knje070aForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knje070aForm1", "POST", "knje070aindex.php", "", "knje070aForm1");
        ...
        //大学コンボ
        $query = knje070aQuery::getUniversity(CTRL_YEAR);
        $extra = "onchange=\"return btn_submit('knje070a');\"";
        makeCmb($objForm, $arg, $db, $query, "STAT_CD", $model->field["STAT_CD"], $extra, 1, "BLANK");

        //学部・学科コンボ
        $query = knje070aQuery::getFacultyDepartment(CTRL_YEAR, $model->field["STAT_CD"]);
        $extra = "onchange=\"return btn_submit('knje070a');\"";
        ...
        $query = knje070aQuery::getSchoolDiv($model);
        ...
        //近大チェック
        $kindai_flg = $db->getOne(knje070aQuery::checkKindai());
        if (!$kindai_flg) {
            $arg["notKindai"] = '1';
        }
        ...
        //記載責任者コンボボックス
        $query = knje070aQuery::getStaffList();
        ...
        View::toHTML($model, "knje070aForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    $opt1 = $opt2 = array();
    //生徒一覧リストを作成する
    $query = knje070aQuery::getList($model);
    ...
    if ($model->select_data["selectdata"][0]) {
        $query = knje070aQuery::getList($model, $model->select_data["selectdata"]);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeBtn(&$objForm, &$arg, $model) {
    ...
    if ($model->Properties["unUseCsvBtn_YorokuTyousa"] != "1" || $model->Properties["useKNJE070AJavaCsv"] == "1") {
        //CSVボタン（useXLS=1 => 非表示）
        if (!$model->Properties["useXLS"]) {
            if ($model->Properties["useKNJE070AJavaCsv"] == "1") {
