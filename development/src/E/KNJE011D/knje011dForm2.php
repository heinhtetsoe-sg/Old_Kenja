...
class knje011dForm2
{
    public function main(&$model)
    {
        ...
        $arg = array();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("form2", "POST", "knje011dindex.php", "", "form2");
        ...
        $disabled = "disabled";
        $query = knje011dQuery::selectQueryAnnual_knje011dForm2($model);
        ...
        $query = knje011dQuery::getGradeCd($model, "set");
        ...
        if (!isset($model->warning) && $model->cmd != 'reload4') {
            if ($model->cmd == "reload2_ok" || $model->cmd == "reload2_cancel") {
                $query = knje011dQuery::selectQuery_Htrainremark_Dat($model);
                ...
                if ($model->cmd !== 'torikomi3' && $model->cmd !== 'torikomi4') {
                    if ($model->mode != 'grd' && $model->Properties["tyousasyo_shokenTable_Seq"] == "1") {
                        $query = knje011dQuery::selectQueryForm2Pattern($model);
                    } else {
                        $query = knje011dQuery::selectQueryForm2($model);
                    }
                    $row = $db->getRow($query, DB_FETCHMODE_ASSOC);
                    ...
                    if ("1" == $model->Properties["tyousasyo2020"]) {
                        for ($i = 1; $i <= 6; $i++) {
                            $row["TRAIN_REF".$i] = "";
                            $seq = "10".$i;
                            $query = knje011dQuery::getHexamEntremarkTrainrefDat($model, $model->annual["YEAR"], $seq);
        ...
        View::toHTML($model, "knje011dForm2.html", $arg);
    }
}
...
function makeAttendrecRemark(&$objForm, &$arg, $model, $db, $disabled, $row)
{
    ...
    if ($model->cmd === 'torikomi3') {
        $set_remark = knje011dQuery::getSemesRemark($model, $db, $model->annual["YEAR"]);
        $row["ATTENDREC_REMARK"] = $set_remark;
    } elseif ($model->cmd === 'torikomi4') {
        $set_remark = knje011dQuery::getHreportremarkDetailDat($db, $model);
        $row["ATTENDREC_REMARK"] = $set_remark;
    }
...
function makeSpecialactrec(&$objForm, &$arg, $model, $db, $disabled, $row)
{
    ...
    if ($model->getSchoolName == "koma") {
        ...
        //臘八摂心皆勤
        $rouhatsuKaikin = "";
        $query = knje011dQuery::getRouhatsuKaikin($model);
...
function makeShojikou(&$objForm, &$arg, $model, $db, $disabled, $row)
{
    ...
    if ("1" == $model->Properties["seitoSidoYorokuSogoShoken3Bunkatsu"] || $model->getSchoolName == 'tokiwa') {
        ...
        $query = knje011dQuery::yoroku_trainref123($model, $model->annual["YEAR"]);
        ...
        $query = knje011dQuery::yoroku_sogoshoken($model);
        $sansyou = $db->getRow($query, DB_FETCHMODE_ASSOC);
...
function makeSelectBtn(&$objForm, $model, $div, $name, $label, $target, $disabled = "")
{
    if (!$div || !$name || !$label || !$target) {
        ...
        if ($div == "club") {       //部活動
            ...
            $tgt = "/E/KNJE011D/knje011dindex.php";
            $w = 900;
            $h = 600;
