...
//ビュー作成用クラス
class knje011dSubFormSeiseki {
    function main(&$model) {
        $objForm = new form;
        $arg = array();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("formSeiseki", "POST", "knje011dindex.php", "", "formSeiseki");
        ...
        //中高一貫校
        $JH = (get_count($db->getOne(knje011dQuery::getSchoolName('JH'))) > 0) ? "1" : "";
        ...
        $query = knje011dQuery::selectQueryAnnual($model, $JH);
        ...
        $query = knje011dQuery::selectQueryForm3($model, $JH);
        $result = $db->query($query);
        ...
        View::toHTML($model, "knje011dSubFormSeiseki.html", $arg);
    }
}
?>