...
class knje011dForm1
{
    public function main(&$model)
    {
        $objForm = new form();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knje011dindex.php", "", "edit");
        $db = Query::dbCheckOut();
        ...
        if ($model->mode != 'grd' && $model->Properties["tyousasyo_shokenTable_Seq"] == "1") {
            ...
            //参照パターン
            $query = knje011dQuery::getPatternSeq($model);
        ...
        if (!isset($model->warning) && $model->cmd != 'reload3') {
            if ($model->mode != 'grd' && $model->Properties["tyousasyo_shokenTable_Seq"] == "1") {
                $query = knje011dQuery::selectQueryPattern($model);
                $row = $db->getRow($query, DB_FETCHMODE_ASSOC);
            } else {
                $query = knje011dQuery::selectQuery($model);
                $row = $db->getRow($query, DB_FETCHMODE_ASSOC);
            }

            $query = knje011dQuery::getHexamEntremarkRemarkHdat($model);
        ...
        //年次取得
        $model->gradecd = "";
        $model->gradecd = $db->getOne(knje011dQuery::getGradeCd($model));
        ...
        View::toHTML($model, "knje011dForm1.html", $arg);
    }
}
...
function makeBiko(&$arg, &$objForm, $model, &$row)
{
    ...
    if ("1" != $model->Properties["unuse_KNJE015_HEXAM_ENTREMARK_LEARNING_DAT_REMARK"]) {
        $extra = " onclick=\"loadwindow('".REQUESTROOT."/E/KNJE011D/knje011dindex.php?&cmd=formBikouTori&TARGET=REMARK',0,document.documentElement.scrollTop || document.body.scrollTop,1000,650);\"";
...
function makeButton(&$arg, &$objForm, $db, $model)
{
    ...
    if ("1" == $model->Properties["tyousasho6bunkatsu_homeroomShojikouTorikomi"]) {
        $query = knje011dQuery::getHrShojikouTorikomiDataCheck($model);
    ...
    //セキュリティーチェック
    $securityCnt = $db->getOne(knje011dQuery::getSecurityHigh());
    $csvSetName = "CSV";
...
function makeHidden(&$objForm, $db, $model)
{
    ...
    if ($model->Properties["tyousasyoShokenPreview"] == '1') {
        ...
        $query = knje011dQuery::getSchoolDiv($model);
...
function torikomiRowTotalstudy($db, $model, &$row)
{
    if (intval($model->gradecd) < "3") {
        //1,2年次　指導要録取込
        $query = knje011dQuery::getYouroku($model);
        ...
        $query = knje011dQuery::get_record_totalstudytime_dat($model, $model->exp_year);
        $result = $db->query($query);
