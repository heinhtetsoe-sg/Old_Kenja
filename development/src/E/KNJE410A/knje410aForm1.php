...
class knje410aForm1 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]  = $objForm->get_start("knje410aForm1", "POST", "knje410aindex.php", "", "knje410aForm1");

        //DB接続
        $db = Query::dbCheckOut();

        //教育委員会チェック
        $getFlg = $db->getOne(knje410aQuery::getNameMst());
        if ($getFlg !== '1') {
            $arg["jscript"] = "OnEdboardError();";
        }

        //年度
        $extra = "onChange=\"return btn_submit('knje410a');\"";
        $query = knje410aQuery::getYear();
        ...
        View::toHTML($model, "knje410aForm1.html", $arg); 
    }
}
...
{
    $row1 = array();
    $query = knje410aQuery::getSchoolData($model);
    $result = $db->query($query);
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
...
function makeHidden(&$objForm, $model, $db) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJE410A");
}
?>
