...
class knje063pform1
{
    function main(&$model)
    {
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("list", "POST", "knje063pindex.php", "", "right_list");

        $db = Query::dbCheckOut();

        $arg["SCHREGNO"] = $model->schregno;
        $arg["NAME"]     = htmlspecialchars($model->name);
        $arg["ENT_DIV"]  = $db->getOne(knje063pQuery::selectEnt($model->schregno));
        ...
        $arg["YEAR"] = View::alink("knje063pindex.php", "<font color=\"white\">年度</font>", "",
                        array("cmd"=>"sort", "sort"=>"YEAR")) .$order[$model->sort["YEAR"]];

        $arg["SUBCLASSCD"] = View::alink("knje063pindex.php", "<font color=\"white\">科目名</font>", "",
        ...
        if($model->schregno)
        {
            $result = $db->query(knje063pQuery::selectQuery($model));
        ...
        View::toHTML($model, "knje063pForm1.html", $arg);
    }
    ...
    function getShomeiCheck($db, $year, $schregno, $useSeitoSidoYorokuShomeiKinou) {
        ...
        if ($useSeitoSidoYorokuShomeiKinou == 1) {
            $query = knje063pQuery::getOpinionsWk($year, $schregno);
