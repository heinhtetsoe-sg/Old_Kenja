...
class knje075mForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knje075mForm1", "POST", "knje075mindex.php", "", "knje075mForm1");
        ...
        if ($model->select_data["selectdata"][0]) {
            for ($i = 0; $i < get_count($model->select_data["selectdata"]); $i++) {
                $query = knje075mQuery::getList2($model->select_data["selectdata"][$i], $model);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJE075M");
        ...
        View::toHTML($model, "knje075mForm1.html", $arg); 
    }
}
