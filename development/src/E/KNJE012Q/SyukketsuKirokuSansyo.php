...
class SyukketsuKirokuSansyo {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform9", "POST", "knje012qindex.php", "", "subform9");
        ...
        if ($db->getOne(knje012qQuery::checkSchoolMst()) > 0) {
            $schoolcd       = sprintf("%012d", SCHOOLCD);
            $school_kind    = $db->getOne(knje012qQuery::getSchoolKind2($model));
        }
        ...
        //出欠の記録
        $sick = $late = $early = 0;
        $result = $db->query(knje012qQuery::getAttendSemesDat($model, $knjSchoolMst));
        ...
        //取込ボタン
        $year = VARS::get("YEAR");
        $extra = "disabled style=\"color:#1E90FF;font:bold\" onclick=\"return dataPositionSet('ATTENDREC_REMARK-{$year}');\"";
        ...
        View::toHTML($model, "SyukketsuKirokuSansyo.html", $arg);
    }
}
?>