...
class knje154Form1
{
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knje154Form1", "POST", "knje154index.php", "", "knje154Form1");
        ...
        //学期コンボ作成
        $query = knje154Query::getSemester();
        $extra = "onchange=\"return btn_submit('knje154'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);
        //学年末は今学期とする
        $seme = $model->field["SEMESTER"] == "9" ? CTRL_SEMESTER : $model->field["SEMESTER"];

        //学年コンボ作成
        $query = knje154Query::getSchoolName();
        $schoolName = $db->getOne($query);
        if ($schoolName == 'CHIBEN') {
            $query = knje154Query::getGradeChiben($seme, $model);
            $extra = "onchange=\"return btn_submit('knje154'), AllClearList();\"";
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
        } else {
            $query = knje154Query::getGrade($seme, $model);
            $extra = "onchange=\"return btn_submit('knje154'), AllClearList();\"";
            makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);
        }
        ...
        View::toHTML($model, "knje154Form1.html", $arg); 
    }
}
...
function makeClassList(&$objForm, &$arg, $db, $model, $seme) {
    ...
    //クラス一覧リストを作成する
    $query = knje154Query::getHrClass($seme, $model->field["GRADE"]);
...
function makeHidden(&$objForm, $model, $seme) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJE154");
