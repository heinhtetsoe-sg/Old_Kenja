...
class knje431Form1
{
    public function main(&$model)
    {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knje431index.php", "", "edit");

        $db     = Query::dbCheckOut();
        $db2 = Query::dbCheckOut2();

        //年度
        $query = knje431Query::getYear();
        $extra = "onchange=\"return btn_submit('main');\"";
        makeCmb($objForm, $arg, $db, $query, "YEAR", $model->field["YEAR"], $extra, 1, "");

        //V_SCHOOL_MSTから学校コードを取得
        $query = knje431Query::getSchoolMst($model);
        $rtnRow = $db->getRow($query, DB_FETCHMODE_ASSOC);
        $model->schoolcd = $rtnRow["KYOUIKU_IINKAI_SCHOOLCD"];

        //年度
        $arg["TITLE"] = "就職進路状況(就職先の都道府県別の就職者数)";

        //文書番号
        $query = knje431Query::getTuutatu($model);
        $extra = "onchange=\"return btn_submit('main');\"";
        ...
        //都道府県タイトル
        $query = knje431Query::getPref();
        $result = $db->query($query);
        while ($prefRow = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $arg["prefData"][] = $prefRow;
        }
        ...
        $count = 0;
        $bifKey = "";
        $query = knje431Query::readQuery($model);
        $result = $db->query($query);
        ...
        //報告履歴
        $query = knje431Query::getReport($model);
        $extra = "";
        ...
        View::toHTML($model, "knje431Form1.html", $arg);
    }
}
