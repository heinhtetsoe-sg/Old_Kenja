...
class knje360SubForm4
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform4", "POST", "knje360index.php", "", "subform4");
        ...
        $setval = array();
        $firstflg = true;   //初回フラグ
        $query = knje360Query::getSubQuery4($model, 'list');
        ...
        if ($model->cmd == "subform4A" || $model->cmd == "subform4_clear") {
            if (isset($model->schregno) && !isset($model->warning) && $model->entrydate && $model->seq) {
                $Row = $db->getRow(knje360Query::getSubQuery4($model), DB_FETCHMODE_ASSOC);
            } else {
                $Row =& $model->field;
            }
        } else {
        ...
        $arg["data1"]["STAFFNAME"] = $db->getOne(knje360Query::getStaffName($staffcd));
        ...
        View::toHTML($model, "knje360SubForm4.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg, $model)
{
    ...
    if ($model->type == "btn") {
        ...
        $arg["button"]["btn_end"] = knjCreateBtn($objForm, "btn_end", "戻 る", "onclick=\"return window.open('knje360index.php?cmd=edit&SCHREGNO=$model->schregno','right_frame')\"");
    } else {
