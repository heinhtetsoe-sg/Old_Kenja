...
class knje360SubForm5
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform5", "POST", "knje360index.php", "", "subform5");
        ...
        if ($model->cmd == "subform5A" || $model->cmd == "subform5_clear") {
            if (isset($model->schregno) && !isset($model->warning) && $model->seq) {
                $Row = $db->getRow(knje360Query::getSubQuery5($model), DB_FETCHMODE_ASSOC);
                $arg["NOT_WARNING"] = 1;
            } else {
                $Row =& $model->field;
            }
        ...
        if ($model->getname === 'kyoto') {
            $query = knje360Query::getNameMst('E022');
        ...
        View::toHTML($model, "knje360SubForm5.html", $arg);
    }
}
