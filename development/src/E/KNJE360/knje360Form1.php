...
class knje360Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knje360index.php", "", "edit");
        ...
        $setClubName = "";
        $setComma    = "";
        $query = knje360Query::getClubName($model);
        ...
        //委員会取得
        $model->getname = $db->getOne(knje360Query::getHyoujiset());
        $commi = $db->getRow(knje360Query::getCommitName($model), DB_FETCHMODE_ASSOC);
        ...
        //評定平均取得
        $average = $db->getOne(knje360Query::getValueAverage($model));
        $arg["data"]["AVERAGE"] = $average;

        //出身学校名取得
        $finschool = $db->getOne(knje360Query::getFinschoolName($model));
        $arg["data"]["FINSCHOOL"] = $finschool;

        //出力データ選択ラジオボタン 1:全て 2:受験報告（進学） 3.受験報告（就職）
        $def = $model->Properties["knje360ShubetDefault"];
        if ($def != "1" && $def != "3") {
            $def = "2";
        }
        $opt = array(1, 2, 3);
        ...
        if ($model->schregno != "" && $model->output == "2") {
            ...
            $arg["DATE_LABEL"] = View::alink("knje360index.php", "日付{$label_mark}", " style=\"text-decoration:underline; color:white;\" ", $linkData);
            ...
            $label = View::alink("knje360index.php", "学校名{$label_mark}", " style=\"text-decoration:underline; color:white;\" ", $linkData);
        ...
        $setval = array();
        $firstflg = true;   //初回フラグ
        $cnt = get_count($db->getcol(knje360Query::selectQuery($model, $asc_or_desc)));
        if ($model->schregno && $cnt) {
            $result = $db->query(knje360Query::selectQuery($model, $asc_or_desc));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        View::toHTML($model, "knje360Form1.html", $arg);
    }
}
