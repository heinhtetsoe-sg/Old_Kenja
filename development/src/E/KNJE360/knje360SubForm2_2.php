...
class knje360SubForm2_2
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform2_2", "POST", "knje360index.php", "", "subform2_2");
        ...
        if ($model->cmd == "replace2_college") {
            ...
            if ($model->Properties["useTokyotoShinroTyousasyo"] == "1") {
                ...
                $query = knje360Query::getTokyoLcd($model->replace["field"]["SCHOOL_CATEGORY_CD"]);
                $extra = "onchange=\"collegeSelectEvent2();\"";
                ...
                $query = knje360Query::getTokyoMcd($model->replace["field"]["SCHOOL_CATEGORY_CD"], $model->replace["field"]["TOKYO_L_CD"]);
                $extra = "onchange=\"collegeSelectEvent2();\"";
        ...
        //調査書発行チェックボックス
        $extra = " id=\"ISSUE\" ";
        $checked = $model->Properties["KNJE360_DEF_ISSUE"] == "1" ? " checked " : "";
        ...
        View::toHTML($model, "knje360SubForm2_2.html", $arg);
    }
}
