...
class knje360SubForm6
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform6", "POST", "knje360index.php", "", "subform6");
        ...
        $tmpSeq = "";
        if ($model->schregno != "") {
            $query = knje360Query::getSubQuery6($model);
        ...
        foreach ($setval as $seq => $Row) {
            ...
            $certif_no = "";
            if (isset($seq)) {
                $query = knje360Query::getCertifNo($model, $seq);
        ...
        View::toHTML($model, "knje360SubForm6.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg, $model, $tmpSeq)
{
    ...
    //進路先選択ボタン
    $link = REQUESTROOT."/E/KNJE360/knje360index.php?cmd=replace6&SCHREGNO=".$model->schregno;
