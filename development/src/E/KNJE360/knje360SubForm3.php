...
class knje360SubForm3
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform3", "POST", "knje360index.php", "", "subform3");
        ...
        if ($model->cmd == "subform3A" || $model->cmd == "subform3_clear") {
            if (isset($model->schregno) && !isset($model->warning) && $model->seq) {
                $Row = $db->getRow(knje360Query::getSubQuery3($model), DB_FETCHMODE_ASSOC);
                $arg["NOT_WARNING"] = 1;
            } else {
                $Row =& $model->field;
            }
        ...
        // 会社情報
        $company = $db->getRow(knje360Query::getCollegeOrCompanyMst(trim($Row["STAT_CD"])), DB_FETCHMODE_ASSOC);
        ...
        if ($model->cmd === 'pdf' || ($model->cmd === 'subform3' && $model->field["SENKOU_NO"] != "")
            ...
            $setcompany = $db->getRow(knje360Query::getJobOfferList($model), DB_FETCHMODE_ASSOC);
        ...
        //職業種別（中分類）
        $query = knje360Query::getJobtypeMList($Row["JOBTYPE_LCD"]);
        $extra = "onChange=\"btn_submit('subform3')\"";
        makeCmb($objForm, $arg, $db, $query, "JOBTYPE_MCD", $Row["JOBTYPE_MCD"], $extra, 1, 1);

        //職業種別（小分類）
        $query = knje360Query::getJobtypeSList($Row["JOBTYPE_LCD"], $Row["JOBTYPE_MCD"]);
        ...
        View::toHTML($model, "knje360SubForm3.html", $arg);
    }
}
...
{
    //会社検索ボタンを作成する
    $extra = "onclick=\"wopen('" .REQUESTROOT."/X/KNJXSEARCH9/index.php?PATH=/E/KNJE360/knje360index.php&cmd=&target=KNJE360','search',0,0,790,470);\"";
