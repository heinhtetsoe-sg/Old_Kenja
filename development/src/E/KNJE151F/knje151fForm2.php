...
class knje151fForm2
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("form2", "POST", "knje151findex.php", "", "form2");
        ...
        $query = knje151fQuery::getNameMst($setNameCd);
        ...
        $result = $db->query(knje151fQuery::selectViewcdQuery($model));
        ...
        //選択教科
        $electdiv = $db->getrow(knje151fQuery::getClassMst($model, $model->field["CLASSCD"]), DB_FETCHMODE_ASSOC);
        ...
        $kuuhaku_frg = false;
        //一覧表示
        $result = $db->query(knje151fQuery::selectQuery($model, $execute_date, $view_key));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            //学籍番号を配列で取得
            ...
            foreach ($view_key as $code => $col)
            {
                ...
                if (DEF_UPDATE_RESTRICT <= AUTHORITY) {
                    ...
                    $query = knje151fQuery::getJviewName_Viewcd($model);
                    $viewcd_count = $db->getOne($query);
                    //学年の確認(追加)
                    if ($row["GRADE"]) {
                        $query = knje151fQuery::getSchool_kind($row["GRADE"]);
                        $School_kind = $db->getOne($query);
                        $query = knje151fQuery::getJviewName_School_kind($model, $School_kind);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJE151F");
        ...
        View::toHTML($model, "knje151fForm2.html", $arg);
    }
}
//コンボ作成
