    ...
    if (cmd == 'reset'){
        ...
        for (var i = 0; i < document.forms[0].elements.length; i++ ) {
            ...
            if (e.type == 'text' && e.value != '') {
                ...
                var strPattern = new RegExp(document.forms[0].SETSHOW.value.split(',').join('|'));
                var errMsg     = document.forms[0].SETSHOW.value.split(',').join('または')
                //評定
                if (nam.match(/STATUS9./)) {
                    if (electdiv == '0' && !str.match(/1|2|3|4|5/)) {
                        alert('{rval MSG901}'+'「1～5」を入力して下さい。\n（評定）');
                        return;
                    } else if (electdiv != '0' && !str.match(strPattern)) { 
                        alert('{rval MSG901}'+'「'+errMsg+'」を入力して下さい。\n（評定）');
                        return;
                    }
...
function autoInput(value) {
    for (var i = 0; i < document.forms[0].elements.length; i++ ) {
        var e = document.forms[0].elements[i];
        if (e.type == 'text' && e.value == '' && !e.name.match(/STATUS9./)) {
            e.value = value;
        }
    }
}