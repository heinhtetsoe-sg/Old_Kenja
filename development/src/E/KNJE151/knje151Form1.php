...
class knje151Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("form1", "POST", "knje151index.php", "", "form1");
        ...
        foreach ($setShowArr as $index => $value) {
            $arg["TYPE_SHOW".($index + 1)] = mb_convert_kana($value, "A");
        }
        ...
        foreach ($setShowArr as $key => $val) {
            $dataArray[] = array("VAL"  => "\"javascript:setClickValue('".($key + 1)."')\"",
                                 "NAME" => $val);
        }
        ...
        View::toHTML($model, "knje151Form1.html", $arg);
    }
}
//コンボ作成
