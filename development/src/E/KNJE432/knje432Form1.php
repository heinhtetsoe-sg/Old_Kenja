...
class knje432Form1
{
    public function main(&$model)
    {
        ...
        $objForm = new form();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knje432index.php", "", "edit");

        $db     = Query::dbCheckOut();
        $db2 = Query::dbCheckOut2();

        //年度
        $query = knje432Query::getYear();
        $extra = "onchange=\"return btn_submit('main');\"";
        makeCmb($objForm, $arg, $db, $query, "YEAR", $model->field["YEAR"], $extra, 1, "");

        //V_SCHOOL_MSTから学校コードを取得
        $query = knje432Query::getSchoolMst($model);
        $rtnRow = $db->getRow($query, DB_FETCHMODE_ASSOC);
        $model->schoolcd = $rtnRow["KYOUIKU_IINKAI_SCHOOLCD"];
        $model->prefcd = $rtnRow["PREF_CD"];

        //年度
        $arg["TITLE"] = "就職進路状況(産業別の就職者数)";

        //文書番号
        $query = knje432Query::getTuutatu($model);
        $extra = "onchange=\"return btn_submit('main');\"";
        ...
        //就職進路タイトル
        $query = knje432Query::getIndustry();
        $result = $db->query($query);
        $model->industryData = array();
        $tourokuCd = 1;
        while ($industryRow = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $setCd = $tourokuCd < 10 ? "0".$tourokuCd : $tourokuCd;
            $model->industryData[] = array("LCD" => $industryRow["INDUSTRY_LCD"], "TOUROKU_CD" => $setCd);
            $arg["industryData"][] = $industryRow;
            $tourokuCd++;
        }
        $result->free();
        ...
        //項目名(列)
        $nameArray = array();
        for ($i = 1; $i <= (COUNT($model->industryData) - 1) * 2; $i++) {
            $nameArray[] = "COUNT".$i;
        }
        $nameArray[] = "COUNT98";
        $nameArray[] = "COUNT99";
        ...
        $count = 0;
        $bifKey = "";
        $query = knje432Query::readQuery($model);
        $result = $db->query($query);
        while ($Row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            foreach ($nameArray as $name) {
                ...
                if ($name != "COUNT98" && $name != "COUNT99") {
                    ...
                    $val = $name != "COUNT99" ? $totalCnt1 : $totalCnt2;
                    $readOnly = " readonly ";
        ...
        //報告履歴
        $query = knje432Query::getReport($model);
        $extra = "";
        ...
        View::toHTML($model, "knje432Form1.html", $arg);
    }
}
