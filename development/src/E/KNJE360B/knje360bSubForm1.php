...
class knje360bSubForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform1", "POST", "knje360bindex.php", "", "subform1");
        ...
        $setval   = array();
        $firstflg = true;   //初回フラグ
        $query    = knje360bQuery::getSubQuery1List($model);
        ...
        if ($model->schregno && $cnt) {
            ...
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                ...
                if ($row["COURSE_KIND"] == "1") {
                    //学校名
                    $sch_array = $db->getAssoc(knje360bQuery::getSchoolName($model, $row["ENTRYDATE"], $row["SEQ"]));
        ...
        if ($model->cmd == "subform1A" || $model->cmd == "subform1_clear") {
            if (isset($model->schregno) && !isset($model->warning) && $model->entrydate && $model->seq) {
                $Row = $db->getRow(knje360bQuery::getSubQuery1($model, $model->entrydate), DB_FETCHMODE_ASSOC);
                $arg["NOT_WARNING"] = 1;
            } else {
                $Row =& $model->field;
            }
        ...
        //調査名
        $query = knje360bQuery::getQuestionnaireList();
        ...
        for ($i = 1; $i <= $model->kibouCnt; $i++) {
            ...
            if ($i > 2) {
                ...
                if ($model->cmd == "subform1A" || $model->cmd == "subform1_clear") {
                    if (isset($model->schregno) && !isset($model->warning) && $model->entrydate && $model->seq) {
                        $RowD = $db->getRow(knje360bQuery::getSubQuery1Detail($model, $i), DB_FETCHMODE_ASSOC);
                    } else {
                        $RowD =& $model->field;
                    }
                } else {
        ...
        View::toHTML($model, "knje360bSubForm1.html", $arg);
    }
}
...
{
    //職業検索ボタンを作成する
    $extra = "style=\"width:70px\" onclick=\"loadwindow('" .REQUESTROOT."/X/KNJXSEARCH_JOB_SS/knjxjob_search_ssindex.php?cmd=&target_number=1', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 380, 350)\"";
    $arg["button"]["btn_jobsearch1"] = knjCreateBtn($objForm, "btn_jobsearch1", "職業検索", $extra);
    $extra = "style=\"width:70px\" onclick=\"loadwindow('" .REQUESTROOT."/X/KNJXSEARCH_JOB_SS/knjxjob_search_ssindex.php?cmd=&target_number=2', event.clientX + function () {var scrollX = document.documentElement.scrollLeft || document.body.scrollLeft;return scrollX;}(), event.clientY + function () {var scrollY = document.documentElement.scrollTop || document.body.scrollTop;return scrollY;}(), 380, 350)\"";
    $arg["button"]["btn_jobsearch2"] = knjCreateBtn($objForm, "btn_jobsearch2", "職業検索", $extra);

    $disabled = ($model->mode == "grd") ? " disabled" : "";
    //一括更新（進学）ボタン
    $link  = REQUESTROOT."/E/KNJE360B/knje360bindex.php?cmd=replace1&SCHREGNO=".$model->schregno."&SEQ=".$model->seq;
...
function makeHidden(&$objForm, $db, $model, $Row)
{
    ...
    knjCreateHidden($objForm, "JOBTYPE_SSCD1", $Row["JOBTYPE_SSCD1"]);
    knjCreateHidden($objForm, "JOBTYPE_MCD2", $Row["JOBTYPE_MCD2"]);
    knjCreateHidden($objForm, "JOBTYPE_SCD2", $Row["JOBTYPE_SCD2"]);
    knjCreateHidden($objForm, "JOBTYPE_SSCD2", $Row["JOBTYPE_SSCD2"]);
