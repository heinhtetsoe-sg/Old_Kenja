...
class knje360bSubForm4
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform4", "POST", "knje360bindex.php", "", "subform4");
        ...
        $setval   = array();
        $firstflg = true;   //初回フラグ
        $query    = knje360bQuery::getSubQuery4($model, 'list');
        ...
        if ($model->cmd == "subform4A" || $model->cmd == "subform4_clear") {
            if (isset($model->schregno) && !isset($model->warning) && $model->entrydate && $model->seq) {
                $Row = $db->getRow(knje360bQuery::getSubQuery4($model), DB_FETCHMODE_ASSOC);
            } else {
                $Row =& $model->field;
            }
        } else {
        ...
        $arg["data1"]["STAFFNAME"] = $db->getOne(knje360bQuery::getStaffName($staffcd));
        ...
        View::toHTML($model, "knje360bSubForm4.html", $arg);
    }
}
...
{
    //追加ボタンを作成する
    $arg["button"]["btn_insert"]  = knjCreateBtn($objForm, "btn_insert", "追 加", "onclick=\"return btn_submit('subform4_insert');\"");
    //更新ボタンを作成する
    $extra = ($model->seq == "") ? "disabled" : "onclick=\"return btn_submit('subform4_update');\"";
    $arg["button"]["btn_update"]  = knjCreateBtn($objForm, "btn_update", "更 新", $extra);
    //クリアボタンを作成する
    $extra = ($model->seq == "") ? "disabled" : "onclick=\"return btn_submit('subform4_clear');\"";
    ...
    if ($model->type == "btn") {
        ...
        $arg["button"]["btn_end"] = knjCreateBtn($objForm, "btn_end", "戻 る", "onclick=\"return window.open('knje360bindex.php?cmd=edit&SCHREGNO=$model->schregno','right_frame')\"");
    } else {
