function btn_submit(cmd)
{
	...
	//進路相談ボタン押し下げ時
    if (cmd == 'subform4'){
        loadwindow('knje360bindex.php?cmd=subform4&TYPE=btn',0,0,760,680);
        return true;
    }

    if ((cmd == 'subform5_update') && (document.forms[0].SEQ.value == "")){
            alert('{rval MSG308}');
            return true;
    }

    if (cmd == 'subform5_insert' || cmd == 'subform5_update') {
...
function updateNextStudent(schregno, order)
{
   if (document.forms[0].SCHREGNO.value == "") {
    ...
    document.forms[0].cmd.value = 'subform5_insert';
    //クッキー書き込み
