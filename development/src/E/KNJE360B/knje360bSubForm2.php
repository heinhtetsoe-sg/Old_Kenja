...
class knje360bSubForm2
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform2", "POST", "knje360bindex.php", "", "subform2");
        ...
        if ($model->cmd == "subform2_college") {
            if ($model->Properties["useCollegeExamCalendar"] === '1') {
                //募集区分
                $query = knje360bQuery::getNameMst('E044');
            ...
            $query = knje360bQuery::getLimit($model->field);
            ...
            if ($model->Properties["useTokyotoShinroTyousasyo"] == "1") {
                ...
                $query = knje360bQuery::getTokyoLcd($model->field["SCHOOL_CATEGORY_CD"]);
                $extra = "onchange=\"collegeSelectEvent2();\"";
                ...
                $query = knje360bQuery::getTokyoMcd($model->field["SCHOOL_CATEGORY_CD"], $model->field["TOKYO_L_CD"]);
                $extra = "onchange=\"collegeSelectEvent2();\"";
        ...
        //証明書学校データ
        $query = knje360bQuery::getCertifSchoolDat($model);
        ...
        //調査書発行チェックボックス
        $setIssue = $model->seq ? $Row["ISSUE"] : $model->Properties["KNJE360B_DEF_ISSUE"];
        ...
        View::toHTML($model, "knje360bSubForm2.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg, $model)
{
    ...
    //一括更新ボタン
    $link = REQUESTROOT."/E/KNJE360B/knje360bindex.php?cmd=replace2&SCHREGNO=".$model->schregno."&SEQ=".$model->seq;
