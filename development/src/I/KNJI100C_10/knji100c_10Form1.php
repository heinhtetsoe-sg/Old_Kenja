...
class knji100c_10Form1 {

    function main(&$model) {

        $objForm      = new form;

        $arg["start"] = $objForm->get_start("edit", "POST", "knji100c_10index.php", "", "edit");
...
/********************************************************************************/
        ...
        //対象外の生徒取得
        $opt_idou = array();
        $result = $db->query(knji100c_10Query::getSchnoIdou($model));
        ...
        if ($model->coursemajor || $model->grade || $model->sex || $model->grade_hr_class) {
            //リストtoリスト右
            $result = $db->query(knji100c_10Query::getStudent_right($model));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            if ($model->selectdata_l && $model->cmd != "changeGrade") {
                $result = $db->query(knji100c_10Query::getStudent_left($model));
                while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                    $idou = (in_array(substr($row["VALUE"],9), $opt_idou)) ? "●" : "　";
...
/********************************************************************************/
        ...
        //サブシステムコンボボックス
        $query = knji100c_10Query::getSubSystem();
        ...
        $opt_left = $opt_right = $itemlist = $item = array();

        if (!in_array($model->cmd, array("edit", "changeGrade"))) {
            //保存したフィードを取得
            $query = knji100c_10Query::getFieldSql($model);
        ...
        if($model->selectdata_r){
            foreach($item as $key){
                $opt_left[] = array("label" => $itemlist[$key],"value" => $key);
            }
        }
...
/**********************************************************************************/
        ...
        View::toHTML($model, "knji100c_10Form1.html", $arg);
    }
}
