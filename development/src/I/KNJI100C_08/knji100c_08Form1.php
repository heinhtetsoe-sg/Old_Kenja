...
class knji100c_08Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knji100c_08index.php", "", "edit");
...
/********************************************************************************/
        ...
        //対象外の生徒取得
        $opt_idou = array();
        $result = $db->query(knji100c_08Query::getSchnoIdou($model));
        ...
        if ($model->coursemajor || $model->grade || $model->sex || $model->grade_hr_class) {
            //リストtoリスト右
            $result = $db->query(knji100c_08Query::getStudent_right($model));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            if ($model->selectdata_l && $model->cmd != "changeGrade") {
                $result = $db->query(knji100c_08Query::getStudent_left($model));
                while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                    $idou = (in_array(substr($row["VALUE"],9), $opt_idou)) ? "●" : "　";
...
/********************************************************************************/
        ...
        //サブシステムコンボボックス
        $query = knji100c_08Query::getSubSystem();
        ...
        if (!in_array($model->cmd, array("edit", "changeGrade"))) {
            //保存したフィードを取得
            $query = knji100c_08Query::getFieldSql($model);
...
/**********************************************************************************/
        ...
        View::toHTML($model, "knji100c_08Form1.html", $arg);
    }
}
