...
class knji100c_13Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knji100c_13index.php", "", "edit");
...
/********************************************************************************/
        ...
        //対象外の生徒取得
        $opt_idou = array();
        $result = $db->query(knji100c_13Query::getSchnoIdou($model));
        ...
        //リストtoリスト右
        $opt_right = array();
        $result = $db->query(knji100c_13Query::getStudent_right($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($model->selectdata_l && $model->cmd != "changeGrade") {
            $result = $db->query(knji100c_13Query::getStudent_left($model));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                $idou = (in_array(substr($row["VALUE"],9), $opt_idou)) ? "●" : "　";
        ...
        //出力対象生徒一覧
        $extra = "multiple STYLE=\"WIDTH:180px\" WIDTH=\"180\" ondblclick=\"move1('right')\"";
        $arg["main_part"]["LEFT_PART_L"] = knjCreateCombo($objForm, "left_select_l", "", $opt_left, $extra, 30);

        //生徒一覧
        $extra = "multiple STYLE=\"WIDTH:180px\" WIDTH=\"180\" ondblclick=\"move1('left')\"";
...
/********************************************************************************/
        ...
        //サブシステムコンボボックス
        $query = knji100c_13Query::getSubSystem();
        ...
        //書き出し項目一覧
        $extra = "multiple STYLE=\"WIDTH:180\" WIDTH=\"180\" ondblclick=\"move('right','left_select_r','right_select_r')\" ";
        $arg["main_part"]["LEFT_PART_R"] = knjCreateCombo($objForm, "left_select_r", "left_select_r", $opt_left, $extra, 30);

        //項目一覧
        $extra = "multiple STYLE=\"WIDTH:180\" WIDTH=\"180\" ondblclick=\"move('left','left_select_r','right_select_r')\" ";
...
/**********************************************************************************/
        ...
        View::toHTML($model, "knji100c_13Form1.html", $arg);
    }
}
