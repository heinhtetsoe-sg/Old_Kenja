...
/********************************************************************************/
        ...
        //校種取得
        $query = knji100c_03Query::getSchoolKind($model);
        ...
        if (!in_array($model->cmd, array("edit", "changeGrade"))) {
            //保存したフィードを取得
            $query = knji100c_03Query::getFieldSql($model);
...
function makeMonthSemeCmb(&$objForm, &$arg, $db, &$model)
{
    ...
    for ($dcnt = 0; $dcnt < get_count($data); $dcnt++) {
        for ($i = $data[$dcnt]["S_MONTH"]; $i <= $data[$dcnt]["E_MONTH"]; $i++) {
            ...
            $getdata = $db->getRow(knji100c_03Query::getMonthName($month, $model), DB_FETCHMODE_ASSOC);
            if (is_array($getdata)) {
