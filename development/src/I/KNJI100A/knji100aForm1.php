...
/********************************************************************************/
        //ラジオ(1:学年,2:個人)
        $opt = array(1, 2);
        $model->field["TAISYOSENTAKU"] = ($model->field["TAISYOSENTAKU"] == "") ? "1" : $model->field["TAISYOSENTAKU"];
        $extra = array();
        foreach($opt as $key => $val) {
            array_push($extra, " id=\"TAISYOSENTAKU{$val}\" onClick=\"btn_submit('')\"");
        }
        $radioArray = knjCreateRadio($objForm, "TAISYOSENTAKU", $model->field["TAISYOSENTAKU"], $extra, $opt, get_count($opt));
        ...
        $value_flg = false;
        $query = knji100aQuery::getAuth($model);
        ...
        if ($model->field["TAISYOSENTAKU"] == "2") {
            ...
            if (isset($model->selectdata_l)){
                $grade = explode(",", $model->selectdata_l);
            }
        }
        ...
        if ($model->field["TAISYOSENTAKU"] == "2") {
            ...
            //リストtoリスト右
            $opt_right = array();
            $result = $db->query(knji100aQuery::getGrade_right($model));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                if (!in_array($row["VALUE"], $grade)) {
            ...
            $opt_left = array();
            if ($model->selectdata_l) {
                $result = $db->query(knji100aQuery::getGrade_left($model));
                while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
/********************************************************************************/
        ...
        //書きだし後に項目一覧を保存する
        if ($model->cmd == "" || $model->field["KAKIDASHI_UP"] == "1") {
            $extra = "checked='checked' ";
        } else {
            $extra = "";
        }
        $extra .= " id=\"KAKIDASHI_UP\"";
