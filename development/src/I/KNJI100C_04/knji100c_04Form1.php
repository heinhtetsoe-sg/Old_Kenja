...
class knji100c_04Form1
{
    public function main(&$model)
    {
        $objForm      = new form();

        $arg["start"] = $objForm->get_start("edit", "POST", "knji100c_04index.php", "", "edit");
        ...
        //対象外の生徒取得
        $opt_idou = array();
        $result = $db->query(knji100c_04Query::getSchnoIdou($model));
        ...
        if ($model->coursemajor || $model->grade || $model->sex || $model->grade_hr_class) {
            //リストtoリスト右
            $result = $db->query(knji100c_04Query::getStudent_right($model));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            if ($model->selectdata_l && $model->cmd != "changeGrade") {
                $result = $db->query(knji100c_04Query::getStudent_left($model));
                while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                    $idou = (in_array(substr($row["VALUE"], 9), $opt_idou)) ? "●" : "　";
        ...
        //サブシステムコンボボックス
        $query = knji100c_04Query::getSubSystem();
        ...
        //テスト種別コンボ
        $query = knji100c_04Query::getTestKind($model);
        $extra = "onchange=\"return btn_submit('edit')\"";
        makeCmb($objForm, $arg, $db, $query, $model->testKind, "TESTKIND", $extra, 1, "blank");
        if ($model->recordTable == "KIN_RECORD_DAT") {
            $arg["useTestkindCmb"] = 1;
        }
        ...
        if (!in_array($model->cmd, array("edit", "changeGrade"))) {
            //保存したフィードを取得
            $query = knji100c_04Query::getFieldSql($model);
        ...
        if ($model->selectdata_r) {
            foreach ($item as $key) {
                $mark = "";
                if (in_array($key, $model->schItemList)) {
                    $mark = "◆";
                } else {
                    $mark = "◇";
                }
                $opt_left[] = array("label" => $mark.$item_array[$key]["label"],"value" => $item_array[$key]["order"]."-".$key);
            }
        }
        ...
        foreach ($model->item as $key => $val) {
            foreach ($val as $field => $label) {
                $mark = "";
                if (in_array($field, $model->schItemList)) {
                    $mark = "◆";
                } else {
                    $mark = "◇";
                }
                if (!in_array($field, $item)) {
                    $opt_right[] = array("label" => $mark.$label,"value" => sprintf("%03d", $key)."-".$field);
                }
            }
        }
        ...
        knjCreateHidden($objForm, "schItemList", implode(',', $model->schItemList));

        Query::dbCheckIn($db);
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knji100c_04Form1.html", $arg);
    }
}
...
function makeCmb(&$objForm, &$arg, $db, $query, &$value, $name, $extra, $size, $blank = "")
{
    ...
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        if ($name == "TESTKIND" && "30101" == $row["VALUE"]) {
            continue;
        }
