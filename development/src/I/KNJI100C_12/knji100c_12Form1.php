...
class knji100c_12Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knji100c_12index.php", "", "edit");
...
/********************************************************************************/
        ...
        //対象外の生徒取得
        $opt_idou = array();
        $result = $db->query(knji100c_12Query::getSchnoIdou($model));
        ...
        //リストtoリスト右
        $opt_right = array();
        $result = $db->query(knji100c_12Query::getStudent_right($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($model->selectdata_l && $model->cmd != "changeGrade") {
            $result = $db->query(knji100c_12Query::getStudent_left($model));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                $idou = (in_array(substr($row["VALUE"],9), $opt_idou)) ? "●" : "　";
...
/********************************************************************************/
        ...
        //サブシステムコンボボックス
        $query = knji100c_12Query::getSubSystem();
        ...
        //試験区分のコンボボックス
        $query = knji100c_12Query::GetTestdiv();
        $extra = "";
...
/**********************************************************************************/
        ...
        View::toHTML($model, "knji100c_12Form1.html", $arg);
    }
}
