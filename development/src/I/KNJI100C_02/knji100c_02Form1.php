...
class knji100c_02Form1 {

    function main(&$model) {

        $objForm      = new form;

        $arg["start"] = $objForm->get_start("edit", "POST", "knji100c_02index.php", "", "edit");
...
/********************************************************************************/
        ...
        //対象外の生徒取得
        $opt_idou = array();
        $result = $db->query(knji100c_02Query::getSchnoIdou($model));
        ...
        if ($model->coursemajor || $model->grade || $model->sex || $model->grade_hr_class) {
            //リストtoリスト右
            $result = $db->query(knji100c_02Query::getStudent_right($model));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            if ($model->selectdata_l && $model->cmd != "changeGrade") {
                $result = $db->query(knji100c_02Query::getStudent_left($model));
                while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                    $idou = (in_array(substr($row["VALUE"],9), $opt_idou)) ? "●" : "　";
...
/********************************************************************************/
        //サブシステムコンボボックス
        $query = knji100c_02Query::getSubSystem();
        ...
        //校種取得
        $query = knji100c_02Query::getSchoolKind($model);
        $model->schKind = $db->getOne($query);
        ...
        if (!in_array($model->cmd, array("edit", "changeGrade"))) {
            //保存したフィードを取得
            $query = knji100c_02Query::getFieldSql($model);
        ...
        if($model->selectdata_r){
            foreach($item as $key){
                switch($key){
                    ...
                    case "NOTICE":
                    case "NONOTICE":
                        list($cd, $name) = explode('-', $item_array[$key]["label"]);
                        $attendname = $db->getOne(knji100c_02Query::getAttendName($cd));
                        $val = ($attendname) ? $attendname.'時数' : $name;
                        break;
                    default:
        ...
        foreach($model->item as $key => $val){
            foreach ($val as $field => $label) {
                switch($field){
                    ...
                    case "NOTICE":
                    case "NONOTICE":
                        list($cd, $name) = explode('-', $label);
                        $attendname = $db->getOne(knji100c_02Query::getAttendName($cd));
                        $label = ($attendname) ? $attendname.'時数' : $name;
                        break;
                    default:
                        break;
                }
...
/**********************************************************************************/
        ...
        View::toHTML($model, "knji100c_02Form1.html", $arg);
    }
}
...
{
    $query      = knji100c_02Query::getSemeMonth();
    $result     = $db->query($query);
    $data       = array();
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $data[] = $row;
    ...
    for ($dcnt = 0; $dcnt < get_count($data); $dcnt++) {
        for ($i = $data[$dcnt]["S_MONTH"]; $i <= $data[$dcnt]["E_MONTH"]; $i++) {
            ...
            $getdata = $db->getRow(knji100c_02Query::getMonthName($month, $model), DB_FETCHMODE_ASSOC);
            if (is_array($getdata)) {
