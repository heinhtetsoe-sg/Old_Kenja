...
class knji100c_16Form1 {

    function main(&$model) {

        $objForm      = new form;

        $arg["start"] = $objForm->get_start("edit", "POST", "knji100c_16index.php", "", "edit");
...
/********************************************************************************/
        ...
        //対象外の生徒取得
        $opt_idou = array();
        $result = $db->query(knji100c_16Query::getSchnoIdou($model));
        ...
        //リストtoリスト右
        $opt_right = array();
        $result = $db->query(knji100c_16Query::getStudent_right($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($model->selectdata_l && $model->cmd != "changeGrade") {
            $result = $db->query(knji100c16Query::getStudent_left($model));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                $idou = (in_array(substr($row["VALUE"],9), $opt_idou)) ? "●" : "　";
...
/********************************************************************************/
        ...
        //サブシステムコンボボックス
        $query = knji100c_16Query::getSubSystem();
        ...
        $itemlist["CLUBINFO"] = "個人指導情報";

        if (!in_array($model->cmd, array("edit", "changeGrade"))) {
            //保存したフィードを取得
            $query = knji100c_16Query::getFieldSql($model);
...
/**********************************************************************************/
        ...
        $model->output = $model->output ? $model->output : "1";
        ...
        View::toHTML($model, "knji100c_16Form1.html", $arg);
    }
}
