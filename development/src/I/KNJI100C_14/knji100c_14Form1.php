...
class knji100c_14Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knji100c_14index.php", "", "edit");
...
/********************************************************************************/
        ...
        //対象外の生徒取得
        $opt_idou = array();
        $result = $db->query(knji100c_14Query::getSchnoIdou($model));
        ...
        //リストtoリスト右
        $opt_right = array();
        $result = $db->query(knji100c_14Query::getStudent_right($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($model->selectdata_l && $model->cmd != "changeGrade") {
            $result = $db->query(knji100c_14Query::getStudent_left($model));
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                $idou = (in_array(substr($row["VALUE"],9), $opt_idou)) ? "●" : "　";
...
/********************************************************************************/
        ...
        //サブシステムコンボボックス
        $query = knji100c_14Query::getSubSystem();
        ...
        if (!in_array($model->cmd, array("edit", "changeGrade"))) {
            //保存したフィードを取得
            $query = knji100c_14Query::getFieldSql($model);
...
/**********************************************************************************/
        ...
        knjCreateHidden($objForm, "OUTPUT", $model->output);

        Query::dbCheckIn($db);
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knji100c_14Form1.html", $arg);
    }
}
