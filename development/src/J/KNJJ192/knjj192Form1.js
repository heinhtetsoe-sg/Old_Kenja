...
function doSubmit() {
    ...
    if (document.forms[0].CLASS_SELECTED.length == 0) {
        alert('{rval MSG304}'+'（出力対象クラス）');
        return true;
    }

    for (var i = 0; i < document.forms[0].CLASS_SELECTED.length; i++) {
        selectdata.value = selectdata.value + sep + document.forms[0].CLASS_SELECTED.options[i].value;
        sep = ",";
    }

    document.forms[0].selectdata.value = selectdata.value;
