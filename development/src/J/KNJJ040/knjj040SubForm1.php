...
class knjj040SubForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform1", "POST", "knjj040index.php", "", "subform1");

        //クラスコンボボックスを作成する
        $query = knjj040Query::getHrClass($model);
        ...
        //入部日付
        $model->field["SUB_SDATE"] = ($model->field["SUB_SDATE"]) ? $model->field["SUB_SDATE"] : str_replace("-","/",CTRL_DATE);
        $arg["data"]["SUB_SDATE"] = View::popUpCalendar($objForm, "SUB_SDATE", $model->field["SUB_SDATE"]);
...
function makeStudentList(&$objForm, &$arg, $db, &$model) {
    $row1 = $row2 = array();
    //生徒一覧リストを作成する
    $query = knjj040Query::getStudent($model, "1");
    ...
    //対象者一覧リストを作成する
    $query = knjj040Query::getStudent($model, "2");
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "CLUBCODEALL", $model->field["CLUBCODEALL"]);
