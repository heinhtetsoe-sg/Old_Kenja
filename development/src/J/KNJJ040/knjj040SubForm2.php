...
class knjj040SubForm2 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform2", "POST", "knjj040index.php", "", "subform2");
        ...
        View::toHTML($model, "knjj040SubForm2.html", $arg);
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, &$model) {
    $row1 = $row2 = array();
    //生徒一覧リストを作成する
    $query = knjj040Query::getClubStudent($model, "1");
    ...
    //対象者一覧リストを作成する
    $query = knjj040Query::getClubStudent($model, "2");
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
