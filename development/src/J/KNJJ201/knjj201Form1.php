...
class knjj201Form1
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("sel", "POST", "knjj201index.php", "", "sel");
        $db             = Query::dbCheckOut();

        //年度設定
        $result    = $db->query(knjj201Query::selectYearQuery());
        $opt       = array();
        ...
        //年度課程一覧取得
        $result      = $db->query(knjj201Query::selectQuery($model));
        $opt_left_id = $opt_left = array();
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            $opt_left[]    = array("label" => $row["DIV"].$row["EXECUTIVECD"]."  ".$row["DIV_NAME"]."  ".$row["EXECUTIVECD_NAME"],
                                   "value" => $row["DIV"].$row["EXECUTIVECD"]);
            $opt_left_id[] = $row["DIV"].$row["EXECUTIVECD"];
        }
        $opt_right = array();

        //課程一覧取得
        $result = $db->query(knjj201Query::selectFareQuery($opt_left_id,$model));
        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
             $opt_right[] = array("label" => $row["DIV"].$row["EXECUTIVECD"]."  ".$row["DIV_NAME"]."  ".$row["EXECUTIVECD_NAME"],
                                  "value" => $row["DIV"].$row["EXECUTIVECD"]);
        }
        ...
        //課程マスタボタンを作成する
        $link = REQUESTROOT."/J/KNJJ201_2/knjj201_2index.php?mode=1";
