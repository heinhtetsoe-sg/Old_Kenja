...
class sikaku {
    function main(&$model) {
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("list", "POST", "knja120bindex.php", "", "edit");
        ...
        $db = Query::dbCheckOut();
        if($model->schregno) {
            $result = $db->query(knja120bQuery::getAward($model));
        ...
        if (VARS::get("cmd") == "right_list"){ 
                $arg["reload"]  = "parent.edit_frame.location.href='knja120bindex.php?cmd=edit'";
        }

        View::toHTML($model, "sikaku.html", $arg);
    }
}
