...
class knjb247Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjb247Form1", "POST", "knjb247index.php", "", "knjb247Form1");
        ...
        //学期コンボ
        $query = knjb247Query::getSemester();
        $extra = "onchange=\"return btn_submit('knjb247'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //学年コンボ
        $query = knjb247Query::getGrade();
        $extra = "onchange=\"return btn_submit('knjb247'), AllClearList();\"";
        ...
        //出欠集計範囲初期値
        if ($model->cmd == "") {
            $model->field["DATE_FROM"]  = str_replace("-", "/", $db->getOne(knjb247Query::getSemester("1")));
        ...
        View::toHTML($model, "knjb247Form1.html", $arg);
    }
}
//リストToリスト作成
function makeListToList(&$objForm, &$arg, $db, $model) {
    //クラス一覧リスト作成
    $opt = array();
    $query = knjb247Query::getHrClassList($model);
...
function makeHidden(&$objForm, $model, $db) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJB247");
    knjCreateHidden($objForm, "cmd");

    //日付チェック用
    $date = $db->getRow(knjb247Query::getSemester("9"), DB_FETCHMODE_ASSOC);
