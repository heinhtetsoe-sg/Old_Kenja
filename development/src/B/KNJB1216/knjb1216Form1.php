...
class knjb1216Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjb1216index.php", "", "main");
        ...
        //履修履歴コンボ
        $query = knjb1216Query::getRirekiCode($model);
        ...
        View::toHTML($model, "knjb1216Form1.html", $arg);
    }
}

//生徒データ表示
function makeStudentInfo(&$objForm, &$arg, $db, &$model) {
    $info = $db->getRow(knjb1216Query::getStudentInfoData($model), DB_FETCHMODE_ASSOC);
    if (is_array($info)) {
        foreach ($info as $key => $val) $setRow[$key] = $val;
        $setRow["ANNUAL"] = ($setRow["ANNUAL"]) ? trim(sprintf("%2d", $setRow["ANNUAL"]))."年次" : "";
        $model->cmcArray["COURSECD"] = $info["COURSECD"];
        $model->cmcArray["MAJORCD"] = $info["MAJORCD"];
        $model->cmcArray["COURSECODE"] = $info["COURSECODE"];
    }
...
function makeMeisai(&$objForm, &$arg, $db, &$model) {
    ...
    $model->div1_name = $model->div2_name = $model->div3_name = "";
    $model->div1_price_cnt = $model->div2_price_cnt = $model->div3_price_cnt = $model->total_price_cnt = 0;
    $meisai_cnt = 0;
    $query = knjb1216Query::getMeisaiQuery($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $sonzai_cnt = $db->getOne(knjb1216Query::getSchregTextbookSubclassDatCnt($model, $row, "", 1));
        //今年度のSCHREG_TEXTBOOK_CHKFIN_DAT
        $executed = $db->getOne(knjb1216Query::getSchregTextbookFinDat($model, $row, "", 1));
        //過去の購入済み件数（教科書ごと）
        $past_cnt = $db->getOne(knjb1216Query::getSchregTextbookSubclassDatCnt($model, $row, "past"));
        //今年度の購入済み件数（教科書ごと）
        $this_cnt = $db->getOne(knjb1216Query::getSchregTextbookSubclassDatCnt($model, $row, "this"));
        //同じ教科書で異なる科目の件数
        $same_txt_cnt = $db->getOne(knjb1216Query::getSubclassTextbookDatCnt($model, $row, "1"));
        ...
        $model->div1_name = $row["DIV"] == "1" ? $row["DIV_NAME"] : $model->div1_name;
        $model->div2_name = $row["DIV"] == "2" ? $row["DIV_NAME"] : $model->div2_name;
        $model->div3_name = $row["DIV"] == "3" ? $row["DIV_NAME"] : $model->div3_name;
        ...
        //購入済み件数
        $cnt = $db->getOne(knjb1216Query::getSchregTextbookSubclassDatCnt($model, $row));
        $row["ZUMI"] = ($cnt > 0) ? "レ" : "";
        ...
        //同一教科書件数
        $txt_cnt = $db->getOne(knjb1216Query::getSubclassTextbookDatCnt($model, $row));
    ...
    $arg["DIV1_NAME"]  = $model->div1_name; 
    $arg["DIV2_NAME"]  = $model->div2_name;
    $arg["DIV3_NAME"]  = $model->div3_name;
...
function makeHidden(&$objForm, $model){ 
    ...
    knjCreateHidden($objForm, "PRGID", "KNJB1216");
