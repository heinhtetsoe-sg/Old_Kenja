...
class knjb0031_2Form1
{
    function main(&$model)
    {
        $objForm        = new form;
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjb0031_2index.php", "", "edit");
        //DB接続
        $db = Query::dbCheckOut();

        //対象年度
        $arg["TERM_SHOW"] = $db->getOne(knjb0031_2Query::getSemesterMst($model));

        //コース
        $gcRow = $db->getRow(knjb0031_2Query::getGradeCouse($model), DB_FETCHMODE_ASSOC);
        ...
        //科目
        $arg["SUBCLASS_SHOW"] = $db->getOne(knjb0031_2Query::getSubclassMst($model));
        ...
        if (!isset($model->warning)) {
            //ＤＢ情報
            $query = knjb0031_2Query::getChairList($model);
        ...
        foreach ($chairList as $counter => $Row) {
            ...
            for ($div = 1; $div <= 2; $div++) {
                ...
                if (!isset($model->warning)) {
                    $no = 1;
                    $result = $db->query(knjb0031_2Query::getStaff($model->term, $Row["CHAIRCD"], $chargediv));
                    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                        $Row_Staff["STAFFCD".$soeji.$no]        = $row["STAFFCD"];
                        $Row_Staff["STAFFNAME_SHOW".$soeji.$no] = $row["STAFFNAME_SHOW"];
                        $no++;
                        if ($maxNo < $no) break;
                    }
                    $result->free();
                    ...
                    for ($no = 1; $no <= $maxNo; $no++) {
                        $Row_Staff["STAFFCD".$soeji.$no]        = $model->fields["STAFFCD".$soeji.$no][$counter];
                        $Row_Staff["STAFFNAME_SHOW".$soeji.$no] = $model->fields["STAFFNAME_SHOW".$soeji.$no][$counter];
                    }
                }

                for ($no = 1; $no <= $maxNo; $no++) {
                    //STAFFCDフィールドサイズ変更対応
                    $setsize = ($model->Properties["useStaffcdFieldSize"] === '10') ? 10 : 8;
                    //職員コード
                    $name = "STAFFCD".$soeji.$no;
                    $extra = "onblur=\"this.value=toAlphanumeric(this.value);\" onPaste=\"return showPaste(this);\"";
                    $setData[$name] = knjCreateTextBox($objForm, $Row_Staff[$name], $name."-".$counter, 10, $setsize, $extra);
                    //職員名称
                    $name = "STAFFNAME_SHOW".$soeji.$no;
                    $extra = "STYLE=\"WIDTH:100%\" WIDTH=\"100%\" onPaste=\"return showPaste(this);\"";
                    $setData[$name] = knjCreateTextBox($objForm, $Row_Staff[$name], $name."-".$counter, 31, 30, $extra);
                }
            } //for
        ...
        View::toHTML($model, "knjb0031_2Form1.html", $arg); 
    }
}
//ボタン作成
