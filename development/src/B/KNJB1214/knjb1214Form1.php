...
class knjb1214Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("main", "POST", "knjb1214index.php", "", "main");
        ...
        //履修履歴コンボ
        $query = knjb1214Query::getRirekiCode($model);
        ...
        View::toHTML($model, "knjb1214Form1.html", $arg);
    }
}

//生徒データ表示
function makeStudentInfo(&$objForm, &$arg, $db, &$model) {
    $info = $db->getRow(knjb1214Query::getStudentInfoData($model), DB_FETCHMODE_ASSOC);
    if (is_array($info)) {
...
function makeMeisai(&$objForm, &$arg, $db, &$model) {
    ...
    $meisai_cnt = 0;
    $query = knjb1214Query::getMeisaiQuery($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $sonzai_cnt = $db->getOne(knjb1214Query::getSchregTextbookSubclassDatCnt($model, $row, "", 1));
        //今年度のSCHREG_TEXTBOOK_CHKFIN_DAT
        $executed = $db->getOne(knjb1214Query::getSchregTextbookFinDat($model, $row, "", 1));
        //過去の購入済み件数（教科書ごと）
        $past_cnt = $db->getOne(knjb1214Query::getSchregTextbookSubclassDatCnt($model, $row, "past"));
        //今年度の購入済み件数（教科書ごと）
        $this_cnt = $db->getOne(knjb1214Query::getSchregTextbookSubclassDatCnt($model, $row, "this"));
        //同じ教科書で異なる科目の件数
        $same_txt_cnt = $db->getOne(knjb1214Query::getSubclassTextbookDatCnt($model, $row, "1"));
        ...
        //購入済み件数
        $cnt = $db->getOne(knjb1214Query::getSchregTextbookSubclassDatCnt($model, $row));
        $row["ZUMI"] = ($cnt > 0) ? "レ" : "";
        ...
        //同一教科書件数
        $txt_cnt = $db->getOne(knjb1214Query::getSubclassTextbookDatCnt($model, $row));
...
function makeHidden(&$objForm, $model){ 
    ...
    knjCreateHidden($objForm, "PRGID", "KNJB1214");
