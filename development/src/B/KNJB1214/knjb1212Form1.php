...
class knjb1212Form1 {
    ...
    function main(&$model) {
        ...
        //履修履歴コンボ
        $query = knjb1212Query::getRirekiCode($model);
...
function makeMeisai(&$objForm, &$arg, $db, &$model) {
    ...
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        $model->div1_price_cnt += ($row["DIV"] == "1" && $extra == "checked") ? (int)$row["TEXTBOOKUNITPRICE"] : 0;
        $model->div2_price_cnt += ($row["DIV"] == "2" && $extra == "checked") ? (int)$row["TEXTBOOKUNITPRICE"] : 0;
        $model->div3_price_cnt += ($row["DIV"] == "3" && $extra == "checked") ? (int)$row["TEXTBOOKUNITPRICE"] : 0;
        $model->total_price_cnt += ($extra == "checked") ? (int)$row["TEXTBOOKUNITPRICE"] : 0;
