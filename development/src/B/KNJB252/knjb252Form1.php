...
class knjb252Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjb252Form1", "POST", "knjb252index.php", "", "knjb252Form1");
        ...
        //学期コンボ
        $query = knjb252Query::getSemester();
        $extra = "onchange=\"return btn_submit('knjb252'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //年組コンボ
        $query = knjb252Query::getHrClass($model);
        $extra = "onchange=\"return btn_submit('knjb252'), AllClearList();\"";
        ...
        //出欠集計範囲初期値セット
        if ($model->cmd == "") {
            $model->field["DATE_FROM"]  = str_replace("-", "/", $db->getOne(knjb252Query::getSemester("1")));
        ...
        View::toHTML($model, "knjb252Form1.html", $arg);
    }
}
//リストToリスト作成
function makeListToList(&$objForm, &$arg, $db, $model) {
    //生徒一覧リスト作成
    $opt = array();
    $query = knjb252Query::getStudentList($model);
...
function makeHidden(&$objForm, $db, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJB252");
    ...
    //日付チェック用
    $date = $db->getRow(knjb252Query::getSemester("9"), DB_FETCHMODE_ASSOC);
