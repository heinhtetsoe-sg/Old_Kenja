...
class knjb3030SubFormHr
{
    public function main(&$model)
    {
        ...
        $arg = array();
        //フォーム作成
        $arg["start"]   = $objForm->get_start("subformHr", "POST", "knjb3030index.php", "", "subformHr");
        ...
        //SQL文発行
        //組略称（学年・組）取得
        $query = knjb3030Query::selectQuerySubFormHr($model, $model->term);
        ...
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            //選択（チェック）
            $row["CHECK"] = knjCreateCheckBox($objForm, "CHECK", $row["GRADE_CLASS"].",".$row["HR_NAMEABBV"], $setId.$check, 1);
            $row["CHECK_NUM"] = $i;
        ...
        View::toHTML($model, "knjb3030SubFormHr.html", $arg);
    }
}
