...
class knjb1307Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjb1307Form1", "POST", "knjb1307index.php", "", "knjb1307Form1");
        ...
        $extra = array("id=\"CATEGORY_IS_CLASS1\" onClick=\"return btn_submit('knjb1307')\"", "id=\"CATEGORY_IS_CLASS2\" onClick=\"return btn_submit('knjb1307')\"");
        ...
        //学期コンボ
        $query = knjb1307Query::getSemester($model);
        $extra = "onchange=\"return btn_submit('knjb1307');\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //学年コンボ
        $query = knjb1307Query::getRegdGdat($model);
        $extra = "onchange=\"return btn_submit('knjb1307');\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        if ($model->field["CATEGORY_IS_CLASS"] == 2){
            //クラスコンボ作成
            $query = knjb1307Query::getGradeHrClass($model);
            $extra = "onchange=\"return btn_submit('knjb1307')\"";
        ...
        //履修履歴コンボ
        $query = knjb1307Query::getRirekiCode($model);
        $extra = "";
        ...
        knjCreateHidden($objForm, "PRGID", "KNJB1307");
        ...
        View::toHTML($model, "knjb1307Form1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    if ($model->field["CATEGORY_IS_CLASS"] == 1){
        $query = knjb1307Query::getRegdHdat($model);
        $arg["data"]["CLASS_LABEL"] = 'クラス';
    } else {
        $query = knjb1307Query::getStudent($model);
