...
class knjb256Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjb256Form1", "POST", "knjb256index.php", "", "knjb256Form1");
        ...
        //学期コンボ
        $query = knjb256Query::getSemester();
        $extra = "onchange=\"return btn_submit('knjb256'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //学年コンボ
        $query = knjb256Query::getGrade();
        $extra = "onchange=\"return btn_submit('knjb256'), AllClearList();\"";
        ...
        if ($model->cmd == "") {
            //開始月
            $semS = $db->getRow(knjb256Query::getSemester(), DB_FETCHMODE_ASSOC);
            $defaultS = $semS["VALUE"].'-'.sprintf("%02d", $semS["S_MONTH"]);

            //終了月
            $semE = $db->getOne(knjb256Query::getSemester(CTRL_DATE));
        ...
        View::toHTML($model, "knjb256Form1.html", $arg);
    }
}
//リストToリスト作成
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    //クラス一覧リスト作成
    $optR = $optL = array();
    $query = knjb256Query::getHrClassList($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model, $info) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJB256");
