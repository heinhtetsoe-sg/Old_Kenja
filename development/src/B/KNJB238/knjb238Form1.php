...
class knjb238Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjb238Form1", "POST", "knjb238index.php", "", "knjb238Form1");
        ...
        //学期コンボ
        $query = knjb238Query::getSemester();
        $extra = "onchange=\"return btn_submit('knjb238'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //科目コンボ
        $query = knjb238Query::getSubclass($model);
        $extra = "onchange=\"return btn_submit('knjb238'), AllClearList();\"";
        ...
        //出欠集計範囲初期値
        if ($model->cmd == "") {
            $model->field["DATE_FROM"]  = str_replace("-", "/", $db->getOne(knjb238Query::getSemester("1")));
        ...
        View::toHTML($model, "knjb238Form1.html", $arg);
    }
}
//リストToリスト作成
function makeListToList(&$objForm, &$arg, $db, $model) {
    //講座一覧リスト作成
    $opt = array();
    $query = knjb238Query::getChairList($model);
...
function makeHidden(&$objForm, $model, $db) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJB238");
    ...
    //日付チェック用
    $date = $db->getRow(knjb238Query::getSemester("9"), DB_FETCHMODE_ASSOC);
