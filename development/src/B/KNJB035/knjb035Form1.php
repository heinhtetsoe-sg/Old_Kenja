...
class knjb035Form1
{
    function main(&$model){
        ...
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjb035index.php", "", "edit");
		...
		if ($model->jikanwari==1){
	        ...
	        //SQL文発行(学期を取得)
	        $db     = Query::dbCheckOut();
    	    $result = $db->query(knjb035Query::getSemester());
	        ...
	        //SQL文発行(ＳＥＱ：タイトルを取得)
    	    $result = $db->query(knjb035Query::getSeqTitle($model->semester));
        	while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC))
	        {
	            array_walk($row, "htmlspecialchars_array");
		...
		//---------------------------------通常---------------------------------
			...
			for($i=0;$i<7;$i++) 
            	$arg["date_youbi".$i] = Date_Calc::daysToDate(Date_Calc::dateToDays($tmp[2]+$i,$tmp[1],$tmp[0]), "%m/%d");

	        //指定日より学期を取得
    	    $db = Query::dbCheckOut();

	        $query = knjb035Query::getY2t(str_replace("/","-",$model->executedate));
    	    $y2t_seme = $db->getOne($query);

	        $query = knjb035Query::getY2t_name(str_replace("/","-",$model->executedate));
        ...
        //SQL文発行(校時情報を保持)
        $query = knjb035Query::getVnamemst();
        ...
        //SQL文発行(クラス情報を保持)
        if ($model->jikanwari==1) $query = knjb035Query::getGrCl_hdat($model->semester);
        if ($model->jikanwari==2) $query = knjb035Query::getGrCl_hdat($y2t_seme);
   	    $result = $db->query($query);
   	    $grcl_cnt = 0;
       	while( $Row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
		...
		if ($model->jikanwari==1){
			...
			if (isset($model->seq)){
		        //SQL文発行(全体の情報を保持)
    		    $result = $db->query(knjb035Query::readQuery_Kihon($model));
        		while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC))
	        	{
		...
		//---------------------------------通常---------------------------------
	        ...
	        //SQL文発行(全体の情報を保持)
   		    $result = $db->query(knjb035Query::readQuery_Tujou(str_replace("/","-",$OutDate1),
        ...
        View::toHTML($model, "knjb035Form1.html", $arg);
    }
}
?>
