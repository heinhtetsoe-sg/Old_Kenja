...
class knjb248Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjb248Form1", "POST", "knjb248index.php", "", "knjb248Form1");
        ...
        //学期コンボ
        $query = knjb248Query::getSemester();
        $extra = "onchange=\"return btn_submit('knjb248'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //学年コンボ
        $query = knjb248Query::getGrade();
        $extra = "onchange=\"return btn_submit('knjb248'), AllClearList();\"";
        ...
        if ($model->cmd == "") {
            //開始月
            $semS = $db->getRow(knjb248Query::getSemester(), DB_FETCHMODE_ASSOC);
            $defaultS = $semS["VALUE"].'-'.sprintf("%02d", $semS["S_MONTH"]);

            //終了月
            $semE = $db->getOne(knjb248Query::getSemester(CTRL_DATE));
        ...
        View::toHTML($model, "knjb248Form1.html", $arg);
    }
}
//リストToリスト作成
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    //クラス一覧リスト作成
    $optR = $optL = array();
    $query = knjb248Query::getHrClassList($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model, $info) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJB248");
