...
function dispInit() {
    if ($("#SCH_DIV3").is(":checked")) {
        ...
        //レイアウト編集
        $("[name=btn_layout]").prop("disabled", true);
    }
}
            ...
            if ($("#OPERATION_RADIO1").is(":checked")) {
                ...
                LObj.checkMeiboAndFac(
                    ...
                    function () {
                        ...
                        compactCell(dragIdArray[1], dragIdArray[2]);
                    }
                );
                compactCell(dragIdArray[1], dragIdArray[2]);
                ...
                var srcDataVal = $("#" + drag_elm.id)[0].getAttribute(
                    "data-val"
                );
                if (srcDataVal != "") {
                    for (
                        ...
                        if (
                            $(
                                "#KOMA_" +
                                    ...
                                    "_" +
                                    lineCnt
                            )[0].getAttribute("data-val") == srcDataVal
                        ) {
                ...
                var targetDataVal = $("#" + targetBox.id)[0].getAttribute(
                    "data-val"
                );
                if (targetDataVal != "") {
                    for (
                        ...
                        if (
                            $(
                                "#KOMA_" +
                                    ...
                                    "_" +
                                    lineCnt
                            )[0].getAttribute("data-val") == targetDataVal
                        ) {
                            alert("同じ時間に同じ講座は設定できません。");
                            obj.style.backgroundColor = "";
                            return;
                ...
                $(window).on(
                    "setCellObjEvent",
                    function (e, cellObjE, srcBoxE, targetBoxE) {
                        if (targetBoxE.id.indexOf("KOMA_99") !== -1) {
                            var objE = $("#" + targetBoxE.id)[0];
                            if (!objE) {
                                $("#templary_cell_data").append(targetBoxE);
                            }
                        }
                    }
                );
                ...
                var CObj = new CellObj();

                CObj.execCellObj_boxTobox(
                    drag_elm,
                    $(baseTdtag("KOMA_9999_999_999"))[0],
                    false
                );
                CObj.execCellObj_boxTobox(targetBox, drag_elm, false);
                CObj.execCellObj_boxTobox(
                    $("#KOMA_9999_999_999")[0],
                    targetBox,
                    false
                ...
                LObj.checkMeiboAndFac(
                    ...
                    function () {
                        ...
                        var idArray = targetBox.id.split("_");
                        ...
                        var idArray = drag_elm.id.split("_");
                        for (
                            lineCnt = 2;
                            lineCnt <= document.forms[0].MAX_LINE.value;
                            lineCnt++
                        ...
                        compactCell(dragIdArray[1], dragIdArray[2]);
                        compactCell(dropIdArray[1], dropIdArray[2]);
                    }
                );
                compactCell(dragIdArray[1], dragIdArray[2]);
                compactCell(dropIdArray[1], dropIdArray[2]);
...
function f_click(event) {
    ...
    includingChair(document.forms[0].selectStartTD.value);
...
function setChair() {
    ...
    if (haveSelectData) {
        for (var i = 0, selectCnt = 0; i < selectList.length; i++) {
            ...
            if (
                ...
                var targetStaffList = [
                    parseInt(document.forms[0].MAX_LINE.value) + 1,
                ];
                for (
                    lineCnt = 2;
                    lineCnt <= document.forms[0].MAX_LINE.value;
                    ...
                    if (
                        $(
                            "#KOMA_" + dateCd + "_" + period + "_" + lineCnt
                        )[0].getAttribute("data-val") == ""
                    ) {
                        targetStaffList = [lineCnt + selectCnt];
                        break;
                    }
                }
            } else {
...
function insertCellData(targetBox, selectOption, linkingText) {
    ...
    ret["class"] = targetBox.getAttribute("class");
    ...
    if ($("#SCH_DIV1").is(":checked")) {
        ...
        var newDate = $("input[name=ALL_DATE]").val().split(",")[
            parseInt(targetBoxIdArray[1])
        ];
    }
        ...
        if (list.length > 1) {
            ...
            for (var i = 0; i < list.length; i++) {
                inHtml +=
                    ...
                    "_" +
                    i +
                    '" class="inner_box" draggable="true">' +
                    list[i] +
                    "</div>";
            }
            inHtml +=
                '<div id="' +
                e.currentTarget.id +
                '_all" class="inner_box" style="background-color:#CCFFCC" draggable="true">全件移動</div>';
...
function hidukekouziInitFunc() {
    ...
    if ($("#SCH_DIV1").is(":checked")) {
        ...
        var newDataListDisp = new Array();
        $("#hidukekouzi_hiduke")[0].innerHTML = "";
        for (var i = 0; i < newDataList.length; i++) {
            var sdataTime = new Date(Date.parse(newDataList[i]));
    ...
    for (var i = 0; i < newDataListDisp.length; i++) {
        //移動不可の日付校時を入れる
...
function hidukekouzi_moveButton() {
    ...
    compactCell(idArray[1], idArray[2]);
    var idArray = targetBox.id.split("_");
    compactCell(idArray[1], idArray[2]);

    var srcLineCnt = null;
    for (lineCnt = 2; lineCnt <= document.forms[0].MAX_LINE.value; lineCnt++) {
        if (
            $("#KOMA_" + idArray[1] + "_" + idArray[2] + "_" + lineCnt)[0]
                .getAttribute("data-val")
                .indexOf(chairList[0]) > -1
        ) {
            targetBox = $(
...
function deleteSelectInitFunc() {
    ...
    var sdataTime = new Date(
        Date.parse(
            $("input[name=ALL_DATE]").val().split(",")[parseInt(idArray[1])]
        )
    );
    var year = sdataTime.getFullYear();
    var month = sdataTime.getMonth() + 1;
...
function copyMoveBox_change() {
    ...
    if (srcDateCd && targetDateCd) {
        if (!movedisabled || !copydisabled) {
            var flg = checkOverCell(
                srcDateCd,
                targetDateCd,
                ret[0],
                ret[1],
...
function checkOverCell(
    ...
    for (var i = srcStartNum; i <= srcEndNum; i++) {
        for (var j = 2; j <= document.forms[0].MAX_LINE.value; j++) {
            if (
                $("#KOMA_" + srcDateCd + "_" + i + "_" + j)[0].getAttribute(
                    "data-val"
                ) != ""
            ) {
                cnt++;
    ...
    for (var i = srcStartNum; i <= srcEndNum; i++) {
        for (var j = 2; j <= document.forms[0].MAX_LINE.value; j++) {
            var targetId =
                ...
                "_" +
                j;
            if ($("#" + targetId)[0].getAttribute("data-val") != "") {
                cnt++;
            }
        }
    }
    return cnt > document.forms[0].MAX_LINE.value - 1;
}
...
function includingChair(srcBoxId) {
    if (srcBoxId == "") {
        for (
            var lineCnt = 1;
            lineCnt <= document.forms[0].MAX_LINE.value;
            ...
            for (
                var dateCnt = 0;
                dateCnt < document.forms[0].DATECNT_MAX.value;
                dateCnt++
            ) {
                for (
                    var periCnt = 1;
                    periCnt <= document.forms[0].MAX_PERIOD.value;
                    periCnt++
                ) {
                    $(
                        "#KOMA_" + dateCnt + "_" + periCnt + "_" + lineCnt
                    ).removeClass("zyuuhukuColor");
                }
            }
        }
        return;
    ...
    var newDataList = $("input[name=ALL_DATE]").val().split(",");
    var idArray = srcBoxId.split("_");
    BoxDataVal = $("#" + srcBoxId)[0].getAttribute("data-val");
    var BoxDataValList = BoxDataVal == "" ? new Array() : BoxDataVal.split(":");
    var targetDay = $("#" + srcBoxId)[0]
        .getAttribute("data-def")
        .split("_")[0];
    var targetPeriod = $("#" + srcBoxId)[0]
        .getAttribute("data-def")
        .split("_")[1];
    var includingList = {
        targetDay: targetDay,
    ...
    $.ajax({
        ...
        data: {
            AJAX_PARAM: JSON.stringify({
                AJAX_INCLUDING_CHAIR_PARAM: JSON.stringify(includingList),
            }),
            cmd: "getincludingChairParam",
            YEAR_SEME: document.forms[0].YEAR_SEME.value,
            START_DATE: $("input[name=START_DATE]").val(),
            END_DATE: $("input[name=END_DATE]").val(),
        },
        ...
        for (var i = 0; i < paramList.length; i++) {
            for (var j = 0; j < newDataList.length; j++) {
                if (paramList[i]["EXECUTEDATE"] == newDataList[j]) {
                    paramList[i]["targetDayIdx"] = j;
                    break;
                }
            }
        }
                ...
                for (
                    ...
                    for (var i = 0; i < paramList.length; i++) {
                        var targetPeriod = $(
                            ...
                            .split("_")[1];
                        if (
                            paramList[i]["targetDayIdx"] == dateCnt &&
                            paramList[i]["PERIODCD"] == targetPeriod
                        ) {
                            var BoxDataVal = $(
                                "#KOMA_" +
                                    dateCnt +
                            ...
                            for (var k = 0; k < BoxDataValList.length; k++) {
                                if (
                                    BoxDataValList[k] == paramList[i]["CHAIRCD"]
                                ) {
                                    $(
                                        "#KOMA_" +
                                            dateCnt +
...
function compactCell(dateCnt, periodCnt) {
    var srcLineCnt = null;
    for (lineCnt = 2; lineCnt <= document.forms[0].MAX_LINE.value; lineCnt++) {
        if (!srcLineCnt) {
            if (
                $("#KOMA_" + dateCnt + "_" + periodCnt + "_" + lineCnt)[0]
                    .getAttribute("data-val")
                    .indexOf(":") > -1
            ) {
                srcLineCnt = lineCnt;
            ...
            if (
                $(
                    "#KOMA_" + dateCnt + "_" + periodCnt + "_" + lineCnt
                )[0].getAttribute("data-val") == ""
            ) {
                var srcBox = $(
                    "#KOMA_" + dateCnt + "_" + periodCnt + "_" + srcLineCnt
                )[0];
                var targetBox = $(
                    "#KOMA_" + dateCnt + "_" + periodCnt + "_" + lineCnt
                )[0];
                var CObj = new CellObj();
                CObj.execCellObj_popTobox(srcBox, 0, targetBox, false);
                return;
            }
        }
    }
    ...
    for (lineCnt = 2; lineCnt <= document.forms[0].MAX_LINE.value; lineCnt++) {
        if (!srcLineCnt) {
            ...
            if (
                ...
                var CObj = new CellObj();
                CObj.execCellObj_boxTobox(srcBox, targetBox, false);
                srcLineCnt++;
            }
        }
    }
}
function dipliStdViewFunc() {
    var newDataList = $("input[name=ALL_DATE]").val().split(",");
    var weekList = new Array();
    $("#tyousei3 .week_getu").each(function (index, element) {
        weekList[index] = element.innerText;
    });

    var idArray = document.forms[0].selectStartTD.value.split("_");

    $("#dipliStdViewPeriod option").remove();
    for (
        var dateCnt = 0;
        dateCnt < document.forms[0].DATECNT_MAX.value;
        dateCnt++
        ...
        for (
            ...
            var option = $("<option>")
                .val(dateCnt + "_" + periCnt)
                .text(newDataList[dateCnt] + " " + weekList[periCnt - 1])
                .prop(
                    "selected",
                    dateCnt + "_" + periCnt == idArray[1] + "_" + idArray[2]
                );
            $("#dipliStdViewPeriod").append(option);
        }
    }
    dipliStdVieSelectList(idArray[1], idArray[2]);
}
function dipliStdVieSelectList(dateCnt, periCnt) {
    var newDataList = $("input[name=ALL_DATE]").val().split(",");
    var list = new Array();
    for (lineCnt = 2; lineCnt <= document.forms[0].MAX_LINE.value; lineCnt++) {
        var dataVal = $(
    ...
    $.ajax({
        ...
        data: {
            AJAX_PARAM: JSON.stringify({
                AJAX_DIPLISTDVIEW_PARAM: JSON.stringify(paramList),
            }),
            cmd: "dipliStdViewParam",
            YEAR_SEME: document.forms[0].YEAR_SEME.value,
            START_DATE: $("input[name=START_DATE]").val(),
            END_DATE: $("input[name=END_DATE]").val(),
        },
    }).done(function (data, textStatus, jqXHR) {
        $("#dipliStdViewBoxTable tr").remove();
        paramList = $.parseJSON(data);
        $("#dipliStdViewBoxTable").append(
            '<tr class="no_search"><td>生徒番号</td><td>出席番号</td><td>氏名</td><td>試験科目番号</td><td>試験科目名</td></tr>'
        );
        for (var i = 0; i < paramList.length; i++) {
            paramList[i]["ATTENDNO"] = !paramList[i]["ATTENDNO"]
                ? ""
                : paramList[i]["ATTENDNO"];
            paramList[i]["NAME_SHOW"] = !paramList[i]["NAME_SHOW"]
                ? ""
                : paramList[i]["NAME_SHOW"];
            paramList[i]["CHAIRNAME"] = !paramList[i]["CHAIRNAME"]
                ? ""
                : paramList[i]["CHAIRNAME"];
            $("#dipliStdViewBoxTable").append(
                '<tr bgcolor="#FFFFFF"><td>' +
                    paramList[i]["SCHREGNO"] +
                    "</td><td>" +
                    paramList[i]["ATTENDNO"] +
                    "</td><td>" +
                    paramList[i]["NAME_SHOW"] +
...
function dipliStdViewPeriodOnChange() {
    var selectedVal = $("#dipliStdViewPeriod option:selected").val();
    dipliStdVieSelectList(selectedVal.split("_")[0], selectedVal.split("_")[1]);
}
