...
class knjb3042Form1
{
    public function main(&$model)
    {
        ...
        //画面左側の切り替え(職員、年組)
        $setMongon = $model->field['SCH_DIV'] == '3' ? "(試験監督)" : "";
        $model->leftMenuArray = array("教職員".$setMongon, "年組", "科目", "講座");
        ...
        */
        ...
        if ($model->field['SCH_DIV'] == '1') {
            ...
            for ($dateCnt = 0; $dateCnt < 14; $dateCnt++) {
                ...
                $model->dateArray[] = array("RENBAN" => $dateCnt, "JPN" => $dispDate, "DATE_OR_WEEK" => $setDate, "CD" => $setWeek["CD"], 'CSS' => $setWeek["CSS"]);
                $model->dateCntMax++;
        ...
        $testRadioDisable = ($model->Properties["KNJB3042_SchTestPattern"] == "1") ? " disabled='disabled'" : '';
        $extra = array(" id=\"TEST_RADIO1\" onclick=\"$('#testCdBox').hide()\" "," id=\"TEST_RADIO2\"  onclick=\"$('#testCdBox').show()\"" . $testRadioDisable);
        foreach ($opt as $key => $val) {
...
function checkMeiboAndFac(&$db, &$model, $meiboFacParam)
{
    ...
    for ($i = 0; $i < get_count($meiboFacParam); $i++) {
        ...
        $response[] = array('targetDay'=>$meiboFacParam[$i]['targetDay'],'id'=>$meiboFacParam[$i]['id'],'kouzi'=>$kouzi,'list'=>$responseParts);
    }
    return $response;
}
...
function cellLoopMain(&$objForm, &$arg, $db, &$model, $leftMenuArray, $toCellActiveList, $toChairList2, $schStaffArray = null, $visibleList = null)
{
    ...
    $arg["setWidth"] = get_count($weekTitleArray) * get_count($model->periodArray) * 110;
}
