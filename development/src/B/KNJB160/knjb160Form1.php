...
class knjb160Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjb160index.php", "", "main");

        //DB接続
        $db = Query::dbCheckOut();

        $securityCnt = $db->getOne(knjb160Query::getSecurityHigh());
        //セキュリティーチェック
        ...
        $result   = $db->query(knjb160query::getSelectYear());
        $opt_year = array();
        ...
        //講座一覧コンボボックス
        $result = $db->query(knjb160query::getSelectChair($model));
        $opt_chaircd = array();
        $opt_chaircd[] = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //年組一覧コンボボックス
        $result = $db->query(knjb160query::getSelectHrclass($model));
        $opt_hrclass = array();
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //生徒一覧コンボボックス
        $result = $db->query(knjb160query::getSelectStudent($model));
        $opt_student = array();
        $opt_student[] = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($model->Properties["useXLS"]) {
            $model->schoolCd = $db->getOne(knjb160Query::getSchoolCd());
        ...
        knjCreateHidden($objForm, "PRGID", "KNJB160");
        ...
        View::toHTML($model, "knjb160Form1.html", $arg);
    }
}
