...
class knjb243Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjb243Form1", "POST", "knjb243index.php", "", "knjb243Form1");
        ...
        //学期コンボ
        $query = knjb243Query::getSemester();
        $extra = "onchange=\"return btn_submit('knjb243'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //科目コンボ
        $query = knjb243Query::getSubclass($model);
        $extra = "onchange=\"return btn_submit('knjb243'), AllClearList();\"";
        ...
        //実施日付初期値
        if ($model->cmd == "") {
            $model->field["DATE_FROM"]  = str_replace("-", "/", $db->getOne(knjb243Query::getSemester("1")));
        ...
        View::toHTML($model, "knjb243Form1.html", $arg);
    }
}
//リストToリスト作成
function makeListToList(&$objForm, &$arg, $db, $model) {
    //講座一覧リスト作成
    $opt = array();
    $query = knjb243Query::getChairList($model);
...
function makeHidden(&$objForm, $model, $db) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJB243");
    knjCreateHidden($objForm, "cmd");
    knjCreateHidden($objForm, "useCurriculumcd", $model->Properties["useCurriculumcd"]);

    //日付チェック用
    $date = $db->getRow(knjb243Query::getSemester("9"), DB_FETCHMODE_ASSOC);
