...
class knjb238aForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjb238aForm1", "POST", "knjb238aindex.php", "", "knjb238aForm1");
        ...
        //学期コンボ
        $query = knjb238aQuery::getSemester();
        $extra = "onchange=\"return btn_submit('knjb238a'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //科目コンボ
        $query = knjb238aQuery::getSubclass($model);
        $extra = "onchange=\"return btn_submit('knjb238a'), AllClearList();\"";
        ...
        //出欠集計範囲初期値
        if ($model->cmd == "") {
            $model->field["DATE_FROM"]  = str_replace("-", "/", $db->getOne(knjb238aQuery::getSemester("1")));
        ...
        View::toHTML($model, "knjb238aForm1.html", $arg);
    }
}
//リストToリスト作成
function makeListToList(&$objForm, &$arg, $db, $model) {
    //講座一覧リスト作成
    $opt = array();
    $query = knjb238aQuery::getChairList($model);
...
function makeHidden(&$objForm, $model, $db) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJB238A");
    ...
    //日付チェック用
    $date = $db->getRow(knjb238aQuery::getSemester("9"), DB_FETCHMODE_ASSOC);
