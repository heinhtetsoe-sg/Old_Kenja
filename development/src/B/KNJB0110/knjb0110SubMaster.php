...
class knjb0110SubMaster
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("submaster", "POST", "knjb0110index.php", "", "submaster");

        //DB接続
        $db = Query::dbCheckOut();

        $arg["TITLE"] = $model->field["YEAR"].'年度　職員一覧';
        ...
        View::toHTML($model, "knjb0110SubMaster.html", $arg); 
    }
}
...
function makeList(&$objForm, &$arg, $db, $model) {
    $i = 0;
    $query = knjb0110Query::getStaffMst($model);
