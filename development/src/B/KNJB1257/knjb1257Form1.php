...
class knjb1257Form1 {
    function main(&$model) {
        ...
        $arg["start"]   = $objForm->get_start("main", "POST", "knjb1257index.php", "", "main");

        /* 処理年度 */
        $query = knjb1257Query::getExeYear($model);
        $extra = "onChange=\"return btn_submit('change');\"";
        makeCombo($objForm, $arg, $db, $query, $model->year, "YEAR", $extra, 1);

        //履修履歴コンボ
        $query = knjb1257Query::getRirekiCode($model);
        $extra = "onChange=\"return btn_submit('change');\"";
        makeCombo($objForm, $arg, $db, $query, $model->rirekiCode, "RIREKI_CODE", $extra, 1);
        
        /* 処理学期 */
        $query = knjb1257Query::getSemester($model);
        $extra = "onChange=\"return btn_submit('change');\"";
        makeCombo($objForm, $arg, $db, $query, $model->semester, "SEMESTER", $extra, 1);

        /* 対象学級 */
        $query      = knjb1257Query::getScregRegdHdat($model);
        ...
        //グループ名
        $query = knjb1257Query::getGroupCnt($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //GROUPCDが600以上は背景色を変える
            $row["COLOR"] = ($row["GROUPCD"] < '600') ? "" : "bgcolor=\"#FF69B4\"";

            $row["SIZE"] = (int)$row["COUNT"] * (int)$model->cellWidth;
        ...
        $query = knjb1257Query::getSubclassName($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //GROUPCDが600以上は背景色を変える
            $row["COLOR"] = ($row["GROUPCD"] < '600') ? "" : "bgcolor=\"#FF69B4\"";
            ...
            $subclass_name_width[$row["GROUPCD"]] += ($seru_no_haba > $model->cellWidth) ? $seru_no_haba : $model->cellWidth;
        }
        ...
        knjCreateHidden($objForm, "PRGID", "KNJB1257");
        ...
        View::toHTML($model, "knjb1257Form1.html", $arg);
    }
}
/***************************************************** 以下関数 *****************************************************/
/******************************/
...
function makeList(&$objForm, &$arg, $db, $model) {
    $query  = knjb1257Query::getList($model);
    $result = $db->query($query);
    $data = array();
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $td = '';
        ...
        foreach ($model->listArray as $subclass_select_data) {
            $query = knjb1257Query::checkSubclassStdSelectDat($subclass_select_data["GROUPCD"], $subclass_select_data["SUBCLASSCD"], $row["SCHREGNO"], $model);
            $cnt = $db->getOne($query);
            ...
            $td .="<td width=\"{$model->cellWidth}px\" {$color}>{$check_box}</td>\n";
