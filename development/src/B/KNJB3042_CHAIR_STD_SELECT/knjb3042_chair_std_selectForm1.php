...
class knjb3042_chair_std_selectForm1 {
    function main(&$model) {
        //オブジェクト作成
        $objForm = new form;
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjb3042_chair_std_selectForm1", "POST", "knjb3042_chair_std_selectindex.php", "", "knjb3042_chair_std_selectForm1");
        ...
        knjCreateHidden($objForm, "SELECT_PERIOD", $model->field["SELECT_PERIOD"]);
        knjCreateHidden($objForm, "SELECT_SYORIBI", $model->field["SELECT_SYORIBI"]);
        ...
        //学期開始日～学期終了日取得
        $query = knjb3042_chair_std_selectQuery::getSemesterDate($model);
        ...
        if ($model->cmd == 'getStdOverlap') {
            ...
            $query = knjb3042_chair_std_selectQuery::getStdOverlap($model);
            $result = $db->query($query);
        ...
        // 重複講座一覧取得
        $query = knjb3042_chair_std_selectQuery::getOverlapChairList($model);
        ...
        // 講座情報取得
        $query = knjb3042_chair_std_selectQuery::getChairInfo($model);
        $result = $db->query($query);
        $row = $result->fetchRow(DB_FETCHMODE_ASSOC);
        if ($row) {
        ...
        // 講座の職員情報取得
        $query = knjb3042_chair_std_selectQuery::getChairStaffList($model, $model->field["OVERLAP_CHAIR"]);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $staff = array();
        ...
        // 講座の名簿取得
        $query = knjb3042_chair_std_selectQuery::getChairStd($model, $model->field["OVERLAP_CHAIR"]);
        ...
        if ($model->field["SELECT_TYPE"] == "1") {
            $query = knjb3042_chair_std_selectQuery::getChairGroup($model, $model->field["OVERLAP_CHAIR"]);
        } else {
            $query = knjb3042_chair_std_selectQuery::getChairCompGroup($model, $model->field["OVERLAP_CHAIR"]);
        }
        $result = $db->query($query);
        $chairCnt = 1;
        ...
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            // 講座の職員情報取得
            $query = knjb3042_chair_std_selectQuery::getChairStaffList($model, $row["CHAIRCD"]);
            ...
            // 講座の名簿取得
            $query = knjb3042_chair_std_selectQuery::getChairStd($model, $row["CHAIRCD"]);
        ...
        // クラス情報取得
        $query = knjb3042_chair_std_selectQuery::getHrInfo($model, $allChairCd);
        $result = $db->query($query);
        $hrCnt = 0;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $data = array();
            ...
            $query = knjb3042_chair_std_selectQuery::getHrStd($model, $row['GRADE'],$row['HR_CLASS'], $stdData["SCHREGNO"]);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJB3042_CHAIR_STD_SELECT");
        ...
        View::toHTML5($model, "knjb3042_chair_std_selectForm1.html", $arg);
    }
}
