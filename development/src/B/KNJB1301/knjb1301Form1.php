...
class knjb1301Form1 {
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjb1301Form1", "POST", "knjb1301index.php", "", "knjb1301Form1");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        //学期コンボ
        $query = knjb1301Query::getSemester();
        $extra = "onchange=\"return btn_submit('main')\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //年組コンボ
        $query = knjb1301Query::getSelectGrade($model);
        $extra = "onchange=\"return btn_submit('main')\"";
        makeCmb($objForm, $arg, $db, $query, "GRADE", $model->field["GRADE"], $extra, 1);

        //履修履歴コンボ
        $query = knjb1301Query::getRirekiCode($model);
        $extra = "";
        makeCmb($objForm, $arg, $db, $query, "RIREKI_CODE", $model->field["RIREKI_CODE"], $extra, 1);

        //学校名取得
        $query = knjb1301Query::getSchoolName();
        ...
        knjCreateHidden($objForm, "PRGID",         "KNJB1301");
        ...
        View::toHTML($model, "knjb1301Form1.html", $arg);
    }
}
/****************************************************** 以下関数 ************************************************************/
//コンボ作成
...
function makeListToList(&$objForm, &$arg, $db, $model) {
    //クラス一覧
    $opt = array();
    $query = knjb1301Query::getCategoryName($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        $opt[] = array('label' => $row["LABEL"],
                       'value' => $row["VALUE"]);
