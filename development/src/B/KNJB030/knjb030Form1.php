...
class knjb030Form1
{
    function main(&$model)
    {
       ...
       if (AUTHORITY != DEF_UPDATABLE){
        ...
        $objForm      = new form;
        $arg["start"] = $objForm->get_start("list", "POST", "knjb030index.php", "", "edit");
        ...
        $db     = Query::dbCheckOut();
        $result = $db->query(knjb030Query::SelectComb_seme());
		...
		$objForm->ae( array("type"       => "radio",
		                    "name"       => "group",
                            "extrahtml"  => "onclick=\"btn_submit('list')\"",
                            "value"      => $model->group,
							"options"    => $opt_group));

		$arg["btn_group1"] = $objForm->ge("group",1);
		$arg["btn_group2"] = $objForm->ge("group",2);
        ...
        $db     = Query::dbCheckOut();
        $result = $db->query(knjb030Query::SelectList_test($model->term,$model->group));
        $i = 0;
        while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
			...
			if ($i>5000){
             ...
             $row["URL"] = View::alink("knjb030index.php", $row["CHAIRCD"], "target=right_frame",
                                         array("cmd"        => "edit",
                                              "CHAIRCD"     => $row["CHAIRCD"],
                                              "term"        => $model->term));
        ...
        if ($model->cmd == "list" && VARS::get("ed") != "1")
            $arg["reload"] = "window.open('knjb030index.php?cmd=edit&init=1','right_frame');";

        View::toHTML($model, "knjb030Form1.html", $arg); 
    }
}
?>
