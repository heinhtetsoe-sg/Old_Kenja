...
class knjb030Form2
{
    ...
    {
        $objForm        = new form;
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjb030index.php", "", "edit");

		//１レコード取得（講座データ）
        if ($model->cmd != "group" && !isset($model->warning))
        {    
            $Row = knjb030Query::getRow_test($model->term, $model->chaircd);
        } else {
            $Row =& $model->fields;
        }
		//レコード取得（その他）
        ...
        {    
			...
			//使用施設
	        $result = $db->query(knjb030Query::getFac($model->term, $model->chaircd));
			...
			//教科書
	        $result = $db->query(knjb030Query::getTextbook($model->term, $model->chaircd));
	        $opt_lab = $opt_val = array();
	        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
    	    {
			...
			//科目担任
	        $result = $db->query(knjb030Query::getStaff($model->term, $model->chaircd));
			...
			//受講クラス
	        $result = $db->query(knjb030Query::getGradeClass($model->term, $model->chaircd, $Row["GROUPCD"]));
	        $opt_lab = $opt_val = array();
	        while($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
    	    {
        ...
        if ($model->cmd == "group"){
	        if ($Row["GROUPCD"] > "0000"){
				//授業回数
	            $Row_lesson = knjb030Query::getRow_lesson($model->term, $Row["GROUPCD"]);
        ...
        $objForm->ae( array("type"        => "select",
                            ...
                            "options"      => knjb030Query::getSubclass(substr($model->term,0,4)) ));
        ...
        $objForm->ae( array("type"        => "select",
                            ...
                            "size"        => 1,
                            "value"       => $Row["TAKESEMES"],
                            "options"      => knjb030Query::getTakesemes(substr($model->term,0,4)) ));
        ...
        $objForm->ae( array("type"        => "select",
                            ...
                            "value"       => $Row["GROUPCD"],
                            "options"     => knjb030Query::getGroup(substr($model->term,0,4)) ));
        ...
        if ($model->cmd != "group" && VARS::get("cmd") != "edit" && !isset($model->warning)){
            $arg["reload"]  = "window.open('knjb030index.php?cmd=list&ed=1','left_frame');";
        }
        //インラインフレーム用Javascriptタグ生成
        $arg["IFRAME"] = View::setIframeJs();

        View::toHTML($model, "knjb030Form2.html", $arg); 
    }
}
?>
