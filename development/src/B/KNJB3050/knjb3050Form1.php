...
class knjb3050Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjb3050Form1", "POST", "knjb3050index.php", "", "knjb3050Form1");
        ...
        if ($model->Properties["useLc_Hrclass"] != "1") {
            ...
            $lcFlag = $db->getOne(knjb3050Query::getMainDataHeadLCCount($model))!=0;
        }
        knjCreateHidden($objForm, "LCFLAG" ,$lcFlag?'1':'0');
        
        //上行作成
        $query = knjb3050Query::getMainDataHead($model);
        $extra = '';
        $value=null;
        ...
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            $staff = array();
            $result2 = $db->query(knjb3050Query::getMainDataHeadSTAFF($model, $data['CHAIRCD']));
            ...
            $query = $lcFlag?knjb3050Query::getMainDataLC($model,$row['VALUE']):knjb3050Query::getMainData($model,$row['VALUE']);
        ...
        //下行作成
        $cnt = 0;
        $result = $db->query($lcFlag?knjb3050Query::getMainData2HeadLC($model) : knjb3050Query::getMainData2Head($model));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            $data=array();
            $data['HR_NAME'] = $row['HR_NAME'];
            
            $query = $lcFlag?knjb3050Query::getMainData2LC($model,$row['CHAIRCD'],$row['TRGTGRADE'],$row['TRGTCLASS']):knjb3050Query::getMainData2($model,$row['CHAIRCD'],$row['TRGTGRADE'],$row['TRGTCLASS']);
        ...
        //最終出欠日付
        $query = knjb3050Query::maxDate($db, $model);
        $maxDate = str_replace('-','/',$db->getOne($query));
        $arg["head"]["MAX_DATE"] = $maxDate;

        //ボタン作成
        makeBtn($objForm, $arg, $lcFlag, $model);
        ...
        View::toHTML($model, "knjb3050Form1.html", $arg);
    }
}
...
//ボタン作成
function makeBtn(&$objForm, &$arg, $lcFlag, $model) {
    //HRに戻す
    $extra = " onclick=\"return retrunHr();\"";
    $arg["button"]["btn_backHr"] = knjCreateBtn($objForm, "btn_backHr", ($lcFlag ? "授業クラス" : "ホームルーム")."に戻す", $extra);
    //上段へ移動
    ...
    //更新ボタン
    $disabled = "";
    if ($model->getParam["authority"] < DEF_UPDATE_RESTRICT) {
        $disabled = "disabled";
    }
    $extra = "onclick=\"return doPopup('".REQUESTROOT."');\" ".$disabled;
