...
class knjb0030Form1
{
    public function main(&$model)
    {
        ...
        $objForm      = new form();
        $arg["start"] = $objForm->get_start("list", "POST", "knjb0030index.php", "", "edit");
        ...
        //char_datの確認
        $query = knjb0030Query::getChairDatcount($model, $model->term);
        $getCount = $db->getOne($query);
        knjCreateHidden($objForm, "CHAIR_DAT_COUNT", $getCount);

        $result = $db->query(knjb0030Query::selectCombSeme());
...
//                            "extrahtml" =>    "onClick=\"return btn_submit('copy');\" style=\"width:165px\""));
        ...
        //教科コンボ
        $query = knjb0030Query::getClassMst($model, $model->term);
        ...
        if ($model->group == "1") {
            ...
            if ($model->fields["sort_name"] == "group_name") {
                $arg["group_name"] = View::alink("knjb0030index.php", "<font color='#ffffff'>群名称</font>", "", array("cmd"=>"sort", "sort_name"=>"group_name")) .$sankaku;
            } else {
                $arg["group_name"] = View::alink("knjb0030index.php", "<font color='#ffffff'>群名称</font>", "", array("cmd"=>"sort", "sort_name"=>"group_name"));
            }
        }

        if ($model->fields["sort_name"] == "url_name") {
            $arg["url_name"] = View::alink("knjb0030index.php", "<font color='#ffffff'>講座コード</font>", "", array("cmd"=>"sort", "sort_name"=>"url_name")) .$sankaku;
        } else {
            $arg["url_name"] = View::alink("knjb0030index.php", "<font color='#ffffff'>講座コード</font>", "", array("cmd"=>"sort", "sort_name"=>"url_name"));
        }
        if ($model->fields["sort_name"] == "subclass_name") {
            $arg["subclass_name"] = View::alink("knjb0030index.php", "<font color='#ffffff'>科目</font>", "", array("cmd"=>"sort", "sort_name"=>"subclass_name")) .$sankaku;
        } else {
            $arg["subclass_name"] = View::alink("knjb0030index.php", "<font color='#ffffff'>科目</font>", "", array("cmd"=>"sort", "sort_name"=>"subclass_name"));
        }

        $model->sort_name = $model->fields["sort_name"];

        $result = $db->query(knjb0030Query::selectListTest($model->term, $model->group, $model, $asc_or_desc));
        $i = 0;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($model->cmd == "list" && VARS::get("ed") != "1") {
            $arg["reload"] = "window.open('knjb0030index.php?cmd=edit&init=1','right_frame');";
        }

        View::toHTML($model, "knjb0030Form1.html", $arg);
    }
}
//makeCmb
