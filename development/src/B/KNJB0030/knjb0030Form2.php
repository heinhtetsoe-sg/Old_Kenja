...
class knjb0030Form2
{
    ...
    {
        $objForm        = new form();
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjb0030index.php", "", "edit");

        //１レコード取得（講座データ）
        if ($model->cmd != "group" && !isset($model->warning)) {
            $Row = knjb0030Query::getRowTest($model, $model->term, $model->chaircd);
        ...
        if ($model->cmd != "group" && !isset($model->warning)) {
            ...
            //使用施設
            $result = $db->query(knjb0030Query::getFac($model->term, $model->chaircd));
            ...
            //教科書
            $result = $db->query(knjb0030Query::getTextbook($model->term, $model->chaircd));
            $opt_lab = $opt_val = array();
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            //科目担任
            $result = $db->query(knjb0030Query::getStaff($model->term, $model->chaircd));
            ...
            //受講クラス
            $result = $db->query(knjb0030Query::getGradeClass($model, $model->term, $model->chaircd, $Row["GROUPCD"]));
            $opt_lab = $opt_val = array();
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        if ($model->cmd == "group") {
            if ($Row["GROUPCD"] > "0000") {
                //授業回数
                $Row_lesson = knjb0030Query::getRowLesson($model->term, $Row["GROUPCD"]);
        ...
        $objForm->ae(array("type"        => "select",
                            ...
                            "options"      => knjb0030Query::getSubclass($model, substr($model->term, 0, 4)) ));
        ...
        $objForm->ae(array("type"        => "select",
                            ...
                            "size"        => 1,
                            "value"       => $Row["TAKESEMES"],
                            "options"      => knjb0030Query::getTakesemes(substr($model->term, 0, 4)) ));
        ...
        $objForm->ae(array("type"        => "select",
                            ...
                            "options"      => knjb0030Query::getRisyuuCourse(substr($model->term, 0, 4)) ));
        ...
        $objForm->ae(array("type"        => "select",
                            ...
                            "value"       => $Row["GROUPCD"],
                            "options"     => knjb0030Query::getGroup(substr($model->term, 0, 4)) ));
        ...
        $jumping = REQUESTROOT."/B/KNJB0050/knjb0050index.php";
        //リンクボタンを作成する
        $objForm->ae(array("type" => "button",
                            "name"        => "btn_jump",
                            "value"       => "名簿入力画面へ",
        ...
        if ($model->cmd != "group" && VARS::get("cmd") != "edit" && !isset($model->warning)) {
            $arg["reload"]  = "window.open('knjb0030index.php?cmd=list&ed=1','left_frame');";
        }
        //インラインフレーム用Javascriptタグ生成
        $arg["IFRAME"] = View::setIframeJs();

        View::toHTML($model, "knjb0030Form2.html", $arg);
    }
}
