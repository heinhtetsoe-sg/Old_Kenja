...
class knjb241Form1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjb241Form1", "POST", "knjb241index.php", "", "knjb241Form1");
        ...
        //学期コンボ
        $query = knjb241Query::getSemester();
        $extra = "onchange=\"return btn_submit('knjb241'), AllClearList();\"";
        makeCmb($objForm, $arg, $db, $query, "SEMESTER", $model->field["SEMESTER"], $extra, 1);

        //科目コンボ
        $query = knjb241Query::getSubclass($model);
        $extra = "onchange=\"return btn_submit('knjb241'), AllClearList();\"";
        ...
        if ($model->cmd == "") {
            //開始月
            $semS = $db->getRow(knjb241Query::getSemester(), DB_FETCHMODE_ASSOC);
            $defaultS = $semS["VALUE"].'-'.sprintf("%02d", $semS["S_MONTH"]);

            //終了月
            $semE = $db->getOne(knjb241Query::getSemester(CTRL_DATE));
        ...
        View::toHTML($model, "knjb241Form1.html", $arg);
    }
}
//リストToリスト作成
function makeListToList(&$objForm, &$arg, $db, $model) {
    ...
    //講座一覧リスト作成
    $optR = $optL = array();
    $query = knjb241Query::getChairList($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model, $info) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJB241");
