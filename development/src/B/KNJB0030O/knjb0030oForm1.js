function btn_submit(cmd) {
    if (cmd == 'copy'){
        ...
        if (insertyear == 0) {
            if (!confirm('対象年度のデータを削除して、コピーします。よろしいでしょうか？')) {
                return false;
            }
        } else {
		...
		if (year_seme < document.forms[0].ctrl_year_semester.value){
		    ...
		    alert('コピー不可。対象年度は、'+year+seme+' 以降を選択して下さい。');
    	    return false;
		}
		if (year_seme > year_seme2){
		} else {
		    alert('コピー不可。参照学期は、対象年度より過去の学期を選択して下さい。');
    	    return false;
		}
		...
		if (document.forms[0].check.checked){
			if (tmp[0] != tmp2[0]){
			    alert('コピー不可。生徒もコピーは、年度をまたがってはコピーできません。');
    		    return false;
			}
		}
    }
