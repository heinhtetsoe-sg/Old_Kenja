...
class knjb036Form1
{
    function main(&$model){
        ...
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjb036index.php", "", "edit");
		...
		$objForm->ae( array("type"       => "radio",
                            ...
                            "value"      => isset($model->jikanwari) ? $model->jikanwari : 1,
							"options"    => $opt_jikanwari));

		$arg["btn_jikanwari1"] = $objForm->ge("jikanwari",1);
		$arg["btn_jikanwari2"] = $objForm->ge("jikanwari",2);

		if (!isset($model->jikanwari)) $model->jikanwari = 1;
        ...
        //SQL文発行(施設を取得)
        $db     = Query::dbCheckOut();
   	    $result = $db->query(knjb036Query::getFacility());
		...
		if ($model->jikanwari==1){
	        ...
	        //SQL文発行(学期を取得)
	        $db     = Query::dbCheckOut();
    	    $result = $db->query(knjb036Query::getSemester());
        	while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC))
	        {
	            array_walk($row, "htmlspecialchars_array");
	        ...
	        //SQL文発行(ＳＥＱ：タイトルを取得)
    	    $result = $db->query(knjb036Query::getSeqTitle($model->semester));
        	while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC))
	        {
	            array_walk($row, "htmlspecialchars_array");
		...
		//---------------------------------通常---------------------------------
			...
			for($i=0;$i<7;$i++) 
            	$arg["date_youbi".$i] = Date_Calc::daysToDate(Date_Calc::dateToDays($tmp[2]+$i,$tmp[1],$tmp[0]), "%m/%d") ."<BR>";

	        //指定日より学期を取得
    	    $db = Query::dbCheckOut();

	        $query = knjb036Query::getY2t(str_replace("/","-",$model->executedate));
    	    $y2t_seme = $db->getOne($query);

	        $query = knjb036Query::getY2t_name(str_replace("/","-",$model->executedate));
        ...
        //SQL文発行(校時情報を保持)
        $query = knjb036Query::getVnamemst();
		...
		if ($model->jikanwari==1){
			...
			if (isset($model->seq)){
		        //SQL文発行(全体の情報を保持)
    		    $result = $db->query(knjb036Query::readQuery_Kihon($model));
        		while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC))
	        	{
            		...
            		//データを配列に保持
	            	$hr_nameabbv_box[$row["NAMECD2"]][$row["DAYCD"]] .= $row["HR_NAMEABBV"]."<BR>";
		...
		//---------------------------------通常---------------------------------
	        ...
	        //SQL文発行(全体の情報を保持)
   		    $result = $db->query(knjb036Query::readQuery_Tujou($model,str_replace("/","-",$OutDate1),str_replace("/","-",$OutDate2),$y2t_seme));
       		while( $row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        	{
           		//実施日付を曜日コードに変換
        ...
        View::toHTML($model, "knjb036Form1.html", $arg);
    }
}
?>
