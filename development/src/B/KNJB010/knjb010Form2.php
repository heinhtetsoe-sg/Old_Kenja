...
class knjb010Form2
{
    function main(&$model)
    {
        ...
        $arg["start"] = $objForm->get_start("edit", "POST", "knjb010index.php", "", "edit");
        
        $arg["GROUPCD"] = $model->groupcd;
        $arg["NAME"]    = $model->name;
        $chaircd = $option = array();
        
        $result = $db->query(knjb010Query::GetGroup($model->groupcd));
        ...
        $option = array();
        $result = $db->query(knjb010Query::GetChair($chaircd));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            $option[]  = array("label" => $row["CHAIRCD"]." ".$row["CHAIRNAME"],
                               "value" => $row["CHAIRCD"]);
        ...
        $result = $db->query(knjb010Query::selectQueryChairClsDat($chaircd, $model->groupcd));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
            $option[] = array("label" => $row["HR_NAME"],"value" => $row["TRGTGRADE"]."-".$row["TRGTCLASS"]);
            $class[] = sprintf("%02d-%02d",$row["TRGTGRADE"],$row["TRGTCLASS"]);
        }
        ...
        //クラス一覧
        $option = array();
        $result = $db->query(knjb010Query::GetClass($class));
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC))
        {
        ...
        if (VARS::post("cmd") == "update") {
            $arg["jscript"] = "window.open('knjb010index.php?cmd=list','left_frame');";
        }

        $arg["finish"]  = $objForm->get_finish();

        View::toHTML($model, "knjb010Form2.html", $arg);
    }
}
?>
