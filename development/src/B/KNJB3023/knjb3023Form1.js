...
function refreshClass(cmd) {
    ...
    var classCd = "";
    // 選択セルから行番号取得
    var idArray = $('input[name=selectTD]').val();
    if (idArray) {
        idArray = idArray.split("_");
        // 職員コード取得
        var staffCell = $('[name=staffLine]')[idArray[3]];
        staffCd = $(staffCell).attr('data-keyname');
        // 教科コード取得
        var classCell = $('[name=classLine]')[idArray[3]];
        classCd = $(classCell).attr('data-keyname');
    }
    ...
    $.ajax({
        ...
        for (var i = 0; i < paramList.length; i++) {
            var option = $('<option>').val(paramList[i]['VALUE']).text(paramList[i]['LABEL']);
            if (paramList[i]['VALUE'] != '' && classCd != paramList[i]['VALUE']) {
                continue;
            }
            if (classCd == paramList[i]['VALUE']) {
...
function setSubClass() {
    ...
    var staffCd = "";
    var line = idArray[3];
    if ($('td[name=staffLine][data-line='+line+']')[0]) {
        var staff = $('td[name=staffLine][data-line='+line+']');
        staffCd = $(staff).attr('data-keyname');
    }
    // 選択された教員に教科の行があるか判定
    // 担当者の行を取得
    var targetLine = "";
    var staffLines = $('td[name=staffLine][data-keyname='+ staffCd + ']');
    for (let staffCnt = 0; staffCnt < staffLines.length; staffCnt++) {
        var element = staffLines[staffCnt];
        var line = $(element).attr('data-line');
        // 同じ教科のセルを判定
        if ($('td[name=classLine][data-line='+ line + ']')[0]) {
            var classElement = $('td[name=classLine][data-line='+ line + ']')[0];
            if ($(classElement).attr('data-keyname') == (ajaxParam['CLASSCD'] + '-' + ajaxParam['SCHOOL_KIND'])) {
                targetLine = line;
                break;
            }
        }
    ...
    if (!targetLine) {
        alert('選択された教員は指定された教科の資格がありません。');
        return false;
    }
    ...
    // 「反映」押下時、選択しているセルの教員の教科へ追加する。
    // ［国語］のセルを選択していても、教科で［数学］が選択されている場合は［数学］の行へ追加
    var idArray = $('input[name=selectTD]').val().trim().split("_");
    var week    = idArray[1];
    var period  = idArray[2];

    var targetId = "#KOMA_" + week + "_" + period + "_" + targetLine;
    var targetElement = $(targetId)[0];

    var cellObj = new CellObj();
    cellObj.setCellObj(targetElement);
    ...
    columnCheck.targetWeek = week;
    columnCheck.targetPeriod = period;
    columnCheck.targetStaffCd = cellObj.cell.staffcd;
    if (!columnCheck.checkRecord(records)) {
...
function getContent(cmd) {
    ...
    $('td[data-update=1]').each(function () {
        ...
        delData["STAFFCD"] = cellObj.cell.staffcd;
    ...
    $('td[data-update=1]').each(function () {
        ...
        for (var i = 0; i < cellObj.cell.dataValList.length; i++) {
            ...
            updData["STAFFCD"] = cellObj.cell.staffcd;
...
function f_dragover(event, obj){
    ...
    if(obj.id=='TRASH_BOX'){
        ...
        if ($('#OPERATION_RADIO1').is(':checked') || $('#OPERATION_RADIO2').is(':checked')) {
            ...
            // ドラッグ元の教科を取得(最小値・最大値)
            var classCd = $('td[name=classLine][data-line=' + idArray1[3] + ']').attr('data-keyname');
            var classLineMin = $('td[name=classLine][data-keyname=' + classCd + ']:first').attr('data-line');
            var classLineMax = $('td[name=classLine][data-keyname=' + classCd + ']:last').attr('data-line');
            // 移動範囲が同一教科の場合は移動可能、
            // 移動範囲が別教科の場合は、同一教科の最小値・最大値の範囲内に限定する
            var idArray2=obj.id.split('_');
            var line = Number(idArray2[3]);
            // ドラッグ中のセル背景色
            if (line < Number(classLineMin)) {
                line = classLineMin;
            }
            if (line > Number(classLineMax)) {
                line = classLineMax;
            }
            var targetId = '#' + idArray2[0] + '_' + idArray2[1] + '_' + idArray2[2] + '_' + line;
...
function f_drop(event){
    ...
    //id名からドラッグされた要素を取得
    var dragElement = $('#' + dragIdArray[0]+'_' + dragIdArray[1] + '_' + dragIdArray[2] + '_' + dragIdArray[3])[0];
    if (!dragElement) {
        event.preventDefault();
        return;
    }
    ...
    if ($('#OPERATION_RADIO1').is(':checked') || $('#OPERATION_RADIO2').is(':checked')) {
        // ドラッグ元の教科を取得(最小値・最大値)
        var classCd = $('td[name=classLine][data-line=' + dragIdArray[3] + ']').attr('data-keyname');
        var classLineMin = $('td[name=classLine][data-keyname=' + classCd + ']:first').attr('data-line');
        var classLineMax = $('td[name=classLine][data-keyname=' + classCd + ']:last').attr('data-line');
        // 移動範囲が同一教科の場合は移動可能、
        // 移動範囲が別教科の場合は、同一教科の最小値・最大値の範囲内に限定する
        var dropIdArray=obj.id.split('_');
        var line = Number(dropIdArray[3]);
        // ドラッグ中のセル背景色
        if (line < Number(classLineMin)) {
            line = classLineMin;
        }
        ...
        var targetId = '#' + dropIdArray[0] + '_' + dropIdArray[1] + '_' + dropIdArray[2] + '_' + line;
    ...
    if (dragElement != dropElement) {
        if ($('#OPERATION_RADIO1').is(':checked') || $('#OPERATION_RADIO2').is(':checked')) {
            ...
            if (isCheck) {
                // 列の追加チェック
                var columnCheck = new checkColumnObj();
                columnCheck.targetWeek = targetCellObj.cell.week;
                columnCheck.targetPeriod = targetCellObj.cell.period;
                columnCheck.targetStaffCd = targetCellObj.cell.staffcd;
                if (!isCopy) {
                    if (!columnCheck.checkMove(srcCellObj, cntNum)) {
                        alert(columnCheck.errorMessage);
                        isCheck = false;
                    }
                } else {
                    if (!columnCheck.checkCopy(srcCellObj, cntNum)) {
                        this.errorMessage = columnCheck.errorMessage;
                        alert(columnCheck.errorMessage);
                        isCheck = false;
                    }
                }
...
function deleteSelectInitFunc(){
    ...
    var title = weekWaAbbvArray[idArray[1]];
    // 校時取得
    var titleKouzi = "";
    var elements = $('#0_' + idArray[2]);
    if (elements) {
        titleKouzi = $(elements).text();
        title += "　" + $(elements).text();
    }

    // 教科・担当者取得
    var titleStaff = "";
    if ($('td[name=staffLine]')[idArray[3]]) {
        var staff = $('td[name=staffLine]')[idArray[3]];
        title += "<br>" + $(staff).text().replace("<br />", "　");
    }

    $('#deleteSelect_targetName')[0].innerHTML = title;
...
function deleteSelect_deleteButton() {
    ...
    // 選択されている曜日校時
    var selectElement = $('#' + idArray[0] + "_" + idArray[1] + "_" + idArray[2] + "_" + idArray[3])[0];
    if (!selectElement) {
        return;
    }
...
function copyMoveBoxInitFunc(){
    ...
    for (var i = 0; i < weekWaAbbvArray.length; i++) {
        ...
        var weekDisp = "(" + element + ")";
        // 曜日 範囲選択
        var input = $('<input type="radio" onclick="copyMoveBox_change()" id="copyMoveBox_fromWeekRadio'+i+'" name="copyMoveBox_fromWeekRadio" />');
        $(input).val(i);
        $(input).text(weekDisp);
        if (i == week) {
            $(input).prop("checked", true);
        }
        $('#copyMoveBox_fromWeek').append(input);
        $("#copyMoveBox_fromWeek").append('<label for="copyMoveBox_fromWeekRadio'+i+'">'+weekDisp+'</label><br>');

        // 曜日 開始曜日
        var input = $('<input type="radio" onclick="copyMoveBox_change()" id="copyMoveBox_toWeekRadio'+i+'" name="copyMoveBox_toWeekRadio" />');
        $(input).val(i);
        $(input).text(weekDisp);
        if (i == week) {
            $(input).prop("checked", true);
        }
        $('#copyMoveBox_toWeek').append(input);
        $("#copyMoveBox_toWeek").append('<label for="copyMoveBox_toWeekRadio'+i+'">'+weekDisp+'</label><br>');
    }

    // 校時
    $('#copyMoveBox_fromPeriodSelect option').remove();
    $('#copyMoveBox_toPeriodSelect option').remove();
    ...
    for (var i = 0; i < elements.length; i++) {
        ...
        if (elements[i].id.indexOf('0_') === 0) {
            var option = $('<option>').val(i+1).text(elements[i].innerText).prop('selected', (i+1==period));
            $('#copyMoveBox_fromPeriodSelect').append(option);
            var option = $('<option>').val(i+1).text(elements[i].innerText).prop('selected', (i+1==period));
            $('#copyMoveBox_toPeriodSelect').append(option);
            len++;
        }
    }
    $('#copyMoveBox_fromPeriodSelect').prop('size',len);
    $('#copyMoveBox_toPeriodSelect').prop('size',len);
    // チェック
    copyMoveBox_change();
}
...
function copyMoveBox_change(){
    ...
    // 移動元曜日の選択チェック
    var fromSelected = $('input[name=copyMoveBox_fromWeekRadio]:checked').val();
    if (!fromSelected || fromSelected == "") {
        errorMsg += "曜日が選択されていません。" + "<br>";
        checkFlg = false;
    }
    // 移動元曜日の飛び石選択チェック
    var fromPeriodSelected = $('#copyMoveBox_fromPeriodSelect option:selected');
    var firstSelected = $(fromPeriodSelected)[0];
    var lastSelected = $(fromPeriodSelected)[$(fromPeriodSelected).length - 1];
    var firstPeriod = $(firstSelected).val();
    var lastPeriod = $(lastSelected).val();
    if ((lastPeriod - firstPeriod) != ($(fromPeriodSelected).length - 1)) {
        errorMsg += "校時が飛び石選択されています。" + "<br>";
        checkFlg = false;
    }

    // 移動先曜日の選択チェック
    var toSelected = $('input[name=copyMoveBox_toWeekRadio]:checked').val();
    if (!toSelected || toSelected == "") {
        errorMsg += "移動先の曜日が選択されていません。" + "<br>";
        checkFlg = false;
    }
    // 移動先校時の選択チェック
    var toPeriodSelected = $('#copyMoveBox_toPeriodSelect option:selected');
    if (!$(toPeriodSelected[0]).val()) {
        errorMsg += "移動先の校時が選択されていません。" + "<br>";
        checkFlg = false;
    }
    // 移動先校時の範囲チェック
    if (fromSelected == toSelected) {
        if (firstPeriod == $(toPeriodSelected[0]).val()) {
            errorMsg += "当日です。" + "<br>";
            checkFlg = false;
        }
    }

    var maxPeriod = $('input[name=PERIODCNT_MAX]').val();
    if ((parseInt($(toPeriodSelected[0]).val(), 10) + $(fromPeriodSelected).length - 1) > maxPeriod) {
        errorMsg += "範囲エラーです。" + "<br>";
        checkFlg = false;
    }

    var periodList = [];
    for (var i = firstPeriod; i <= lastPeriod; i++) {
        periodList.push(i);
    }

    var blockCell = new BlockCellObj();
    blockCell.setSrc(fromSelected, periodList);
    blockCell.setTarget(toSelected, $(toPeriodSelected[0]).val());
    blockCell.maxPeriod = maxPeriod;
    blockCell.maxLine = $('input[name=LINECNT_MAX]').val();
    // 移動チェック
...
//レイアウト変更初期化
function layoutClassStaffBoxInitFunc() {
    ...
    $.ajax({
        ...
        var paramList = $.parseJSON(data);
        // 科目のリスト初期化
        var selectClass = $('select[name=LAYOUT_CLASSCD]').val();
        $('select[name=LAYOUT_CLASSCD] option').remove();
        for (var i = 0; i < paramList.length; i++) {
            var option = $('<option>').val(paramList[i]['VALUE']).text(paramList[i]['LABEL']);
            $('select[name=LAYOUT_CLASSCD]').append(option);
        }
        $('select[name=LAYOUT_CLASSCD]').val(selectClass);
    });
...
function staffCmbChage() {
    ...
    //検索クリア
    if ($('input[name=CLASS_STAFF_SEARCH]')[0]) {
        $('input[name=CLASS_STAFF_SEARCH]').val('');
    }

    // リストの初期化
    $('#category_classstaff_selected option').remove();
    $('#category_classstaff_name option').remove();
    ...
    $.ajax({
        ...
        var classCd = $('select[name=LAYOUT_CLASSCD]').val();
        var staffCd = $('select[name=LAYOUT_STAFFCD]').val();

        for (var i = 0; i < paramList.length; i++) {
            var param = paramList[i];
            // 検索条件に該当する場合は左側リストへ追加
            var isClass = false;
            var isStaff = false;
            // 教科が選択されている場合のみ検索条件に加える
            // 教科が選択されていない場合、教科の条件は満たしたことになる
            if (classCd) {
                var checkcd = param["CLASSCD"] + "-" + param["SCHOOL_KIND"];
                if (checkcd == classCd) {
                    isClass = true;
                }
            } else {
            ...
            // 教科が選択されていない場合、教科の条件は満たしたことになる
            if (staffCd) {
                if (param["STAFFCD"] == staffCd) {
                    isStaff = true;
                }
            } else {
                isStaff = true;
            }
            var value = param["CLASSCD"] + "-" + param["SCHOOL_KIND"] + " " + param["STAFFCD"];
            var label = param["CLASSCD"] + "-" + param["SCHOOL_KIND"] + " " + param["CLASSABBV"];
            label += " " + param["STAFFCD"] + " " + param["STAFFNAME"];
            var option = $('<option>').val(value).text(label);
            // 検索条件を満たしている場合は左側へ追加
            // 検索条件を満たしていない場合は右側へ追加
            if (isClass && isStaff) {
                $('select[name=category_classstaff_selected]').append(option);
            } else {
                $('select[name=category_classstaff_name]').append(option);
            }
        }
    });
...
//職員名の検索
function searchClassStaffName(obj) {
    $('#category_classstaff_name')[0].innerHTML = globalLayoutRightList;
...
function dispLoad(upperName) {
    ...
    var selectClass = $('select[name=LAYOUT_CLASSCD]').val();
    var selectStaff = $('select[name=LAYOUT_STAFFCD]').val();
...
function dispChange(upperName) {
    ...
    var newSelectedList = $('#category_classstaff_selected option');
    ...
    dispList.each(function(index, element) {
        var classCd = $(element.cells[0]).attr('data-keyname');
        var staffCd = $(element.cells[1]).attr('data-keyname');

        $(element).hide();
        //左リスト
        newSelectedList.each(function(leftIndex, leftElement) {
            var leftElmArray = $(leftElement).val().split(' ');
            var showClassCd = leftElmArray[0];
            var showStaffCd = leftElmArray[1];
            if (classCd == showClassCd && staffCd == showStaffCd) {
                $(element).show();
    ...
    dispList.each(function(index, element) {
        ...
        if (parentIdxArray[index]) {
            $(element).show();
            if (document.forms[0].visibleLine.value) {
                document.forms[0].visibleLine.value += ',';
            }
            document.forms[0].visibleLine.value += index;
        }
    });
    $('#dialogBox').hide();
}
