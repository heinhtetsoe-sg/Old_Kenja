...
class knjb0055Form1 {
    function main(&$model) {
        ...
        $query = knjb0055Query::getSemester();
        ...
        /* リスト */
        /**********/
        $query = knjb0055Query::getSubclassList($model);
        $result = $db->query($query);
        $firstFlg = true;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            if (!$row["CLASSCD"]) {
            ...
            $row["SUBCLASSNAME"] = View::alink("knjb0055index.php", htmlspecialchars($row["SUBCLASSNAME"]), "target='right_frame'",
                                array("cmd"        => "edit",
        ...
        View::toHTML($model, "knjb0055Form1.html", $arg);
    }
}
?>
