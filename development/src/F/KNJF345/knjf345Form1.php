...
class knjf345Form1
{
    public function main(&$model)
    {
        ...
        $objForm = new form();
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjf345index.php", "", "edit");
        ...
        $rowCount = 0;
        $query = knjf345Query::readQuery($model);
        $result = $db->query($query);
        while ($Row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //画面に表示する値
            $textArray['ATTEND_NAME']   = $Row['ATTEND_NAME'];
            $textArray['NAME']          = $Row['NAME'];
            $textArray['DETAIL_RESULT'] = $Row['DETAIL_RESULT'];
            $textArray['DETAIL_REMARK'] = $Row['DETAIL_REMARK'];
            $arg["data"][] = $textArray;
            ...
            knjCreateHidden($objForm, "{$rowPrefix}DETAIL_RESULT", $Row['DETAIL_RESULT']);
            knjCreateHidden($objForm, "{$rowPrefix}DETAIL_REMARK", $Row['DETAIL_REMARK']);

            $rowCount++;
        }

        //報告済み日付
        $tempLastHoukokuDate = $db2->getOne(knjf345Query::getMaxFixedDate($model));
        $lastHoukokuDate = '';
        ...
        //確定データ
        $query = knjf345Query::getFixed($model);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJF345");
        ...
        View::toHTML($model, "knjf345Form1.html", $arg);
    }
}
