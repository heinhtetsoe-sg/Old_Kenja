...
class knjf175bForm1 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]  = $objForm->get_start("knjf175bForm1", "POST", "knjf175bindex.php", "", "knjf175bForm1");
        ...
        //学期
        $arg["data"]["SEMESTER"] = $db->getOne(knjf175bQuery::getSemeName());

        //校種コンボ
        if ($model->Properties["use_prg_schoolkind"] == "1") {
            $arg["schkind"] = "1";
            $query = knjf175bQuery::getSchkind($model);
            $extra = "onchange=\"return btn_submit('knjf175b');\"";
        ...
        //印影出力チェックボックス
        $query = knjf175bQuery::getStampCnt($model);
        $StampCnt = $db->getOne($query);
        if ($StampCnt > 0) {
            $query = knjf175bQuery::getStampName($model);
        ...
        if (($model->Properties["useSchool_KindField"] == "1" && SCHOOLKIND != "") || $model->Properties["use_prg_schoolkind"] == "1") {
            //学校名称2表示
            $info = $db->getRow(knjf175bQuery::getSchoolMst($model, CTRL_YEAR), DB_FETCHMODE_ASSOC);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJF175B");
        ...
        View::toHTML($model, "knjf175bForm1.html", $arg); 
    }
}
//makeCmb
