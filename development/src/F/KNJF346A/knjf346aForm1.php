...
class knjf346aForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjf346aForm1", "POST", "knjf346aindex.php", "", "knjf346aForm1");
        ...
        //教育委員会チェック
        $getFlg = $db->getOne(knjf346aQuery::getNameMst());
        if ($getFlg !== '1') {
            $arg["jscript"] = "OnEdboardError();";
        }

        //年度
        $extra = "onChange=\"return btn_submit('knjf346a');\"";
        $query = knjf346aQuery::getYear();
        ...
        View::toHTML($model, "knjf346aForm1.html", $arg);
    }
}
...
{
    $row1 = array();
    $query = knjf346aQuery::getSchoolData($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model, $db)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJF346A");
