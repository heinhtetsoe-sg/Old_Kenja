...
class knjf302aForm1 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]  = $objForm->get_start("knjf302aForm1", "POST", "knjf302aindex.php", "", "knjf302aForm1");

        //DB接続
        $db = Query::dbCheckOut();

        //教育委員会チェック
        $getFlg = $db->getOne(knjf302aQuery::getNameMst());
        if ($getFlg !== '1') {
            $arg["jscript"] = "OnEdboardError();";
        }

        //年度
        $extra = "onChange=\"return btn_submit('knjf302a');\"";
        $query = knjf302aQuery::getYear();
        ...
        View::toHTML($model, "knjf302aForm1.html", $arg); 
    }
}
...
{
    $row1 = array();
    $query = knjf302aQuery::getSchoolData($model);
    $result = $db->query($query);
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
...
function makeHidden(&$objForm, $model, $db) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJF302A");
}
?>
