...
class knjf342aForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjf342aForm1", "POST", "knjf342aindex.php", "", "knjf342aForm1");
        ...
        //教育委員会チェック
        $getFlg = $db->getOne(knjf342aQuery::getNameMst());
        if ($getFlg !== '1') {
            $arg["jscript"] = "OnEdboardError();";
        }

        //年度
        $extra = "onChange=\"return btn_submit('knjf342a');\"";
        $query = knjf342aQuery::getYear();
        ...
        View::toHTML($model, "knjf342aForm1.html", $arg);
    }
}
...
{
    $row1 = array();
    $query = knjf342aQuery::getSchoolData($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model, $db)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJF342A");
