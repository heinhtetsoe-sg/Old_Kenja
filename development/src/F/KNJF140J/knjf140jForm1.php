...
class knjf140jForm1
{
    public function main(&$model)
    {
        ...
        if ($model->Properties["use_prg_schoolkind"] == "1") {
            $query = knjf140jQuery::getNameMstA023($model, CTRL_YEAR);
            ...
            $query = knjf140jQuery::getNameMstA023($model);
            $cnt = $db->getCol($query);
            if (1 < $cnt) {
                $extra = "onChange=\"btn_submit('')\";";
                makeCmb($objForm, $arg, $db, $query, "SCHOOL_KIND", $model->field["SCHOOL_KIND"], $extra, 1);
        ...
        $securityCnt = $db->getOne(knjf140jQuery::getSecurityHigh());
        //セキュリティーチェック
        ...
        $query = knjf140jquery::getSelectFieldSQL($model);
        $extra = "onchange=\"btn_submit('');\"";
        makeCmb($objForm, $arg, $db, $query, "YEAR", $model->field["YEAR"], $extra, 1);

        //年組一覧コンボボックス
        $query = knjf140jquery::getSelectFieldSQL2($model);
        $extra = "";
        ...
        //出力取込種別ラジオボタン
        $opt_shubetsu = array(1, 2, 3, 4, 5); //1:ヘッダ出力 2:データ取込 3:エラー出力 4:データ出力 5:コード一覧出力
        ...
        knjCreateHidden($objForm, "PRGID", "KNJF140J");
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("main", "POST", "knjf140jindex.php", "", "main");
        $arg["finish"]  = $objForm->get_finish();

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjf140jForm1.html", $arg);
    }
}
/************************************************ 以下関数 **************************************************************/
//コンボ作成
