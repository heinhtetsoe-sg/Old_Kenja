...
class knjf306aForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjf306aForm1", "POST", "knjf306aindex.php", "", "knjf306aForm1");

        //DB接続
        $db = Query::dbCheckOut();

        //教育委員会チェック
        $getFlg = $db->getOne(knjf306aQuery::getNameMst());
        if ($getFlg !== '1') {
            $arg["jscript"] = "OnEdboardError();";
        }

        //年度
        $extra = "onChange=\"return btn_submit('knjf306a');\"";
        $query = knjf306aQuery::getYear();
        ...
        View::toHTML($model, "knjf306aForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, &$model) {
    $row1 = array();
    $query = knjf306aQuery::getSchoolData($model);
    $result = $db->query($query);
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
