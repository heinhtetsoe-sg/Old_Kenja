...
class knjf100cForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjf100cForm1", "POST", "knjf100cindex.php", "", "knjf100cForm1");
        ...
        if ($model->Properties["use_prg_schoolkind"] == "1") {
            $arg["schkind"] = "1";
            $query = knjf100cQuery::getSchkind($model);
            $extra = "onchange=\"return btn_submit('knjf100c');\"";
        ...
        if (($model->Properties["useSchool_KindField"] == "1" && SCHOOLKIND != "") || $model->Properties["use_prg_schoolkind"] == "1") {
            //学校名称2表示
            $schoolname2 = $db->getOne(knjf100cQuery::getSchoolName2($model));
            $arg["SCH_NAME2"] = (strlen($schoolname2) > 0) ? "<<".$schoolname2.">>" : "";
        }

        //部クラブコンボボックスを作成する
        $query = knjf100cQuery::getClubList($model);
        $extra = "onChange=\"return btn_submit('knjf100c');\"";
        ...
        View::toHTML($model, "knjf100cForm1.html", $arg);
    }
}
...
function makeStudentList(&$objForm, &$arg, $db, &$model) {
    $right = $left = array();
    //一覧リストを作成する
    $query = knjf100cQuery::getClubMember($model);
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJF100C");
