...
class knjf344aForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjf344aForm1", "POST", "knjf344aindex.php", "", "knjf344aForm1");
        ...
        //教育委員会チェック
        $getFlg = $db->getOne(knjf344aQuery::getNameMst());
        if ($getFlg !== '1') {
            $arg["jscript"] = "OnEdboardError();";
        }

        //年度
        $extra = "onChange=\"return btn_submit('knjf344a');\"";
        $query = knjf344aQuery::getYear();
        ...
        View::toHTML($model, "knjf344aForm1.html", $arg);
    }
}
...
{
    $row1 = array();
    $query = knjf344aQuery::getSchoolData($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model, $db)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJF344A");
