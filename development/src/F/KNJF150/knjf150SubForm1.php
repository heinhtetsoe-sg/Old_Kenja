...
class knjf150SubForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform1", "POST", "knjf150index.php", "", "subform1");
        ...
        //来室理由テキスト入力可コード格納
        $f200Text = array();
        $query = knjf150Query::getNameMst('F200');
        ...
        //処置テキスト入力可コード格納
        $treatmentText = array();
        $query = knjf150Query::getNameMst('F208');
        $result = $db->query($query);
        while ($row1 = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            if ($row1["NAMESPARE2"] == "1") {
        ...
        //原因コンボ作成
        $query = knjf150Query::getNameMst('F204');
        if ($model->schoolName != "miyagiken") {
            $f204Text = array();
            $result = $db->query($query);
        ...
        View::toHTML($model, "knjf150SubForm1.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg, $model)
{
    if ($model->sendProgramId == 'KNJF150G') {
        ...
        if ($model->Properties["useKnjf150_2Button"] == 1) {
            $extra = "onclick=\"loadwindow('../KNJF150_2/knjf150_2index.php?CALL_INFO=KNJF150_1&SCHREGNO={$model->schregno}',0,0,800,600);return;\"";
