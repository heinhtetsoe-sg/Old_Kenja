...
class knjf150Form1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjf150index.php", "", "edit");
        ...
        $setval = array();
        $firstflg = true;   //初回フラグ
        $cnt = get_count($db->getcol(knjf150Query::selectQuery($model)));
        if ($model->schregno && $cnt) {
            $result = $db->query(knjf150Query::selectQuery($model));
        ...
        View::toHTML($model, "knjf150Form1.html", $arg);
    }
}
