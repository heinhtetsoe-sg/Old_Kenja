...
class knjf150SubForm3
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform3", "POST", "knjf150index.php", "", "subform3");
        ...
        $query = knjf150Query::getNameMst('F203');
        //来室理由テキスト入力可コード格納
        $f203Text = array();
        ...
        //処置テキスト入力可コード格納
        $treatmentText = array();
        $query = knjf150Query::getNameMst('F210');
        $result = $db->query($query);
        ...
        View::toHTML($model, "knjf150SubForm3.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg, $model)
{
    if ($model->sendProgramId == 'KNJF150G') {
        ...
        if ($model->Properties["useKnjf150_2Button"] == 1) {
            $extra = "onclick=\"loadwindow('../KNJF150_2/knjf150_2index.php?CALL_INFO=KNJF150_3&SCHREGNO={$model->schregno}',0,0,800,600);return;\"";
