...
class knjf150SubForm5
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform5", "POST", "knjf150index.php", "", "subform5");
        ...
        //来室理由テキスト入力可コード格納
        $f219Text = array();
        $query = knjf150Query::getNameMst('F219');
        ...
        //処置テキスト入力可コード格納
        $f220Text = array();
        $query = knjf150Query::getNameMst('F220');
        $result = $db->query($query);
        while ($row1 = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            if ($row1["NAMESPARE2"] == "1") {
                $f220Text[] = $row1["VALUE"];
        ...
        View::toHTML($model, "knjf150SubForm5.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg, $model)
{
    if ($model->sendProgramId == 'KNJF150G') {
        ...
        if ($model->Properties["useKnjf150_2Button"] == 1) {
            $extra = "onclick=\"loadwindow('../KNJF150_2/knjf150_2index.php?CALL_INFO=KNJF150_5&SCHREGNO={$model->schregno}',0,0,800,600);return;\"";
