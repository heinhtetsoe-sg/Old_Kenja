...
class knjf030cForm1
{
    function main(&$model){
        ...
        //hiddenを作成する(必須)
        $query = knjf030cQuery::getSchoolJudge();
        $schoolJudge = $db->getOne($query);
        $schoolJudge = ($schoolJudge) ? "J" : "H";
        makeHidden($objForm, $schoolJudge);
...
function makeClassItiran(&$objForm, &$arg, $db, &$model) {
        ...
        //対象クラスリスト
        $extra = "multiple style=\"width=300px\" width=\"300px\" ondblclick=\"move1('left', $chdt)\"";
        $arg["data"]["CLASS_NAME"] = knjCreateCombo($objForm, "CLASS_NAME", "", $row1, $extra, 18);

        //出力クラスリスト
        $extra = "multiple style=\"width=300px\" width=\"300px\" ondblclick=\"move1('right', $chdt)\"";
...
function makeHidden(&$objForm, $schoolJudge) {
    ...
    knjCreateHidden($objForm, "SCHOOL_JUDGE", $schoolJudge);
