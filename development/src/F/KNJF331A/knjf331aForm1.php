...
class knjf331aForm1 {
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"]  = $objForm->get_start("knjf331aForm1", "POST", "knjf331aindex.php", "", "knjf331aForm1");

        //DB接続
        $db = Query::dbCheckOut();

        //教育委員会チェック
        $getFlg = $db->getOne(knjf331aQuery::getNameMst());
        if ($getFlg !== '1') {
            $arg["jscript"] = "OnEdboardError();";
        }

        //年度
        $extra = "onChange=\"return btn_submit('knjf331a');\"";
        $query = knjf331aQuery::getYear();
        ...
        View::toHTML($model, "knjf331aForm1.html", $arg); 
    }
}
...
{
    $row1 = array();
    $query = knjf331aQuery::getSchoolData($model);
    $result = $db->query($query);
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
...
function makeHidden(&$objForm, $model, $db) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJF331A");
