...
class knjf170Form2
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjf170index.php", "", "edit");
        ...
        if (!isset($model->warning) && !VARS::get("changeFlg")) {
            ...
            $query = knjf170Query::getRow($model, $date);
            $Row = $db->getRow($query, DB_FETCHMODE_ASSOC);

            //欠席データ取得
            if ($model->Properties["useNurseoffAttend"] == "1") {
                $attend = array();
                $query = knjf170Query::getNurseoffAttendDat($model, $date);
        ...
        if ($model->z010 == "miyagiken") {
            ...
            $arg["not_miyagiken"] = "1";
            //天気コンボ作成
            $query = knjf170Query::getNameMst($model);
        ...
        if (VARS::get("cmd") != "edit" && !isset($model->warning)) {
            if ($model->cmd == "clear") {
                ...
                $arg["reload"]  = "parent.left_frame.location.href='knjf170index.php?cmd=list';";
            }
        }

        //テンプレートのHTMLを読み込んでデータを$arg経由で渡す。
        View::toHTML($model, "knjf170Form2.html", $arg);
    }
}
