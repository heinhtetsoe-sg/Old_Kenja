...
class knjf150bSubForm2
{
    function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform2", "POST", "knjf150bindex.php", "", "subform2");
        ...
        //けがの場所テキスト入力可コード格納
        $f207Text = array();
        $query = knjf150bQuery::getNameMst('F207');
        ...
        //来室理由テキスト入力可コード格納
        $f201Text = array();
        $query = knjf150bQuery::getNameMst('F201');
        $result = $db->query($query);
        while ($row1 = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            if ($row1["NAMESPARE2"] == "1") {
                $f201Text[] = $row1["VALUE"];
        ...
        $query = knjf150bQuery::getNameMst('F216');
        makeCmb($objForm, $arg, $db, $query, "OCCUR_ACT", $row["OCCUR_ACT"], $extra, 1);

        //行動詳細コンボ作成
        $detail = $db->getRow(knjf150bQuery::getNameMst('F216', $row["OCCUR_ACT"]), DB_FETCHMODE_ASSOC);
        if($row["OCCUR_ACT"] && ($detail["NAMESPARE1"] == "1")){
            $query = knjf150bQuery::getNameMst('B001');
        ...
        } else if($row["OCCUR_ACT"] && ($detail["NAMESPARE1"] == "2")){
            $query = knjf150bQuery::getClubMst();
            $extra = (get_count($db->getCol($query))) ? "" : " style=\"width:120px;\"";
            makeCmb($objForm, $arg, $db, $query, "OCCUR_ACT_DETAIL", $row["OCCUR_ACT_DETAIL"], $extra, 1);
        } else if($row["OCCUR_ACT"]){
            $query = knjf150bQuery::getNameMst('F217', $row["OCCUR_ACT"]);
            $extra = (get_count($db->getCol($query))) ? "" : " style=\"width:120px;\"";
            makeCmb($objForm, $arg, $db, $query, "OCCUR_ACT_DETAIL", $row["OCCUR_ACT_DETAIL"], $extra, 1);
        } else {
        ...
        //処置テキスト入力可コード格納
        $f209Text = array();
        $query = knjf150bQuery::getNameMst('F209');
        $result = $db->query($query);
        while ($row1 = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            if ($row1["NAMESPARE2"] == "1") {
                $f209Text[] = $row1["VALUE"];
        ...
        View::toHTML($model, "knjf150bSubForm2.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg, $model)
{
    ...
    if($model->Properties["useKnjf150_2Button"] == 1) {
        $extra = "onclick=\"loadwindow('../KNJF150B_2/knjf150b_2index.php?CALL_INFO=KNJF150B_2&SCHREGNO={$model->schregno}',0,0,800,600);return;\"";
