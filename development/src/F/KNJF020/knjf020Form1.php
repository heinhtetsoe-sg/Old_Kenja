...
class knjf020Form1
{
    ...
    {
        $objForm      = new form();
        $arg["start"] = $objForm->get_start("edit", "POST", "knjf020index.php", "", "edit");
        ...
        if (isset($model->schregno)) {
            ...
            //生徒学年クラスを取得
            $result = $db->query(knjf020Query::getSchregRegdDatData($model));
        ...
        if ($model->is_f020_otherdisese_hyouji2) {
            ...
            $arg["data"]["OTHERDISEASE3"] = knjCreateTextBox($objForm, $RowT["OTHERDISEASE3"], "OTHERDISEASE3", 40, 60, $extra.$entMove["OTHERDISEASE3"]);

            //その他の疾病及び異常テキスト 3行目
            $extra = ($RowT["OTHERDISEASECD4"] == '99' || $model->Properties["printKenkouSindanIppan"] == "3") ? "" : "disabled style=\"background-color:darkgray\"";
            $arg["data"]["OTHERDISEASE4"] = knjCreateTextBox($objForm, $RowT["OTHERDISEASE4"], "OTHERDISEASE4", 40, 60, $extra.$entMove["OTHERDISEASE4"]);
        }

        //口腔の疾病及び異常コンボ
        $query = knjf020Query::getNameMst($model, "F531");
        ...
        if ($model->is_f020_otherdisese_hyouji) {
            ...
            $arg["data"]["OTHERDISEASE_REMARK2"] = knjCreateTextBox($objForm, $RowT["OTHERDISEASE_REMARK2"], "OTHERDISEASE_REMARK2", 40, 60, $extra);
            ...
            $arg["data"]["OTHERDISEASE_REMARK4"] = knjCreateTextBox($objForm, $RowT["OTHERDISEASE_REMARK4"], "OTHERDISEASE_REMARK4", 40, 60, $extra);
        }

        //所見コンボボックス
        $query = knjf020Query::getNameMst($model, "F540");
        if ($arg["is_not_mie"] == "1") {
            $extra = "style=\"width:150px;\" onChange=\"syokenNyuryoku(this, document.forms[0].DENTISTREMARK, ['".join("','", (array)$nameSpare2["F540"])."'])\"".$entMove["DENTISTREMARKCD"];
        } else {
        ...
        if ($arg["is_not_mie"] == "1") {
            //所見テキスト
            if (!in_array($RowT["DENTISTREMARKCD"], (array)$nameSpare2["F540"])) {
                $extra = "disabled";
            } else {
                $extra = "";
            }
            $arg["data"]["DENTISTREMARK"] = knjCreateTextBox($objForm, $RowT["DENTISTREMARK"], "DENTISTREMARK", 40, 20, $extra.$entMove["DENTISTREMARK"]);
        }

        if ($model->is_f020_dentistremark_hyouji) {
            //所見コンボボックス 2行目
            $query = knjf020Query::getNameMst($model, "F540");
            $extra = "style=\"width:150px;\" onChange=\"syokenNyuryoku(this, document.forms[0].DENTISTREMARK_REMARK2, ['".join("','", (array)$nameSpare2["F540"])."'])\"".$entMove["DENTISTREMARK_REMARK1"];
            makeCombo($objForm, $arg, $db, $query, $RowT["DENTISTREMARK_REMARK1"], "DENTISTREMARK_REMARK1", $extra, 1, "BLANK");

            //所見テキスト 2行目
            if (!in_array($RowT["DENTISTREMARK_REMARK1"], (array)$nameSpare2["F540"])) {
                $extra = "disabled";
            } else {
                $extra = "";
            }
            $arg["data"]["DENTISTREMARK_REMARK2"] = knjCreateTextBox($objForm, $RowT["DENTISTREMARK_REMARK2"], "DENTISTREMARK_REMARK2", 40, 20, $extra.$entMove["DENTISTREMARK_REMARK2"]);

            //所見コンボボックス 3行目
            $query = knjf020Query::getNameMst($model, "F540");
            $extra = "style=\"width:150px;\" onChange=\"syokenNyuryoku(this, document.forms[0].DENTISTREMARK_REMARK4, ['".join("','", (array)$nameSpare2["F540"])."'])\"".$entMove["DENTISTREMARK_REMARK2"];
            makeCombo($objForm, $arg, $db, $query, $RowT["DENTISTREMARK_REMARK3"], "DENTISTREMARK_REMARK3", $extra, 1, "BLANK");

            //所見テキスト 3行目
            if (!in_array($RowT["DENTISTREMARK_REMARK3"], (array)$nameSpare2["F540"])) {
                $extra = "disabled";
            } else {
                $extra = "";
            }
            $arg["data"]["DENTISTREMARK_REMARK4"] = knjCreateTextBox($objForm, $RowT["DENTISTREMARK_REMARK4"], "DENTISTREMARK_REMARK4", 40, 20, $extra.$entMove["DENTISTREMARK_REMARK4"]);
        }
        ...
        $arg["data"]["DENTISTREMARK_CO"] = knjCreateTextBox($objForm, $RowT["DENTISTREMARK_CO"], "DENTISTREMARK_CO", 2, 2, $extra.$entMove["DENTISTREMARK_CO"]);
        ...
        if ($model->z010 === "mieken") {
            //学校医コンボボックス(三重のみ)
            $extra = "";
            $arg["data"]["DOC_NAME"] = knjCreateTextBox($objForm, $RowT["DOC_NAME"], "DOC_NAME", 20, 10, $extra.$entMove["DOC_NAME"]);
        }
        ...
        if ($arg["is_mie"] == "1") {
            ...
            $extra = "style=\"width:250px;\" onChange=\"syokenNyuryoku(this, document.forms[0].DENTISTTREAT, ['".join("','", (array)$nameSpare2["F541"])."'])\"".$entMove["DENTISTTREATCD"];
        }
        ...
        if ($arg["is_mie"] != "1") {
            ...
            if ($model->Properties["printKenkouSindanIppan"] == "2") {
                ...
                if (!in_array($RowT["DENTISTTREATCD"], (array)$nameSpare2["F541"])) {
                    $extra .= "disabled";
                } else {
                    $extra .= "";
                }
        ...
        View::toHTML($model, "knjf020Form1.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg, $model, $entMove)
{
    ...
    //一括更新ボタン
    $link = REQUESTROOT."/F/KNJF020/knjf020index.php?cmd=replace&SCHREGNO=".$model->schregno;
