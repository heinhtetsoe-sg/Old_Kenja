...
class knjf023bForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjf023bindex.php", "", "edit");
        ...
        //年組コンボ
        $query = knjf023bQuery::getHrClass($model);
        ...
        $setInNamecd1 .= "'";
        $query = knjf023bQuery::getNameMstSpare2($model, $setInNamecd1);
        $nameSpare2 = array();
        ...
        $result->free();
        $nameSpare2['F540'] = array();

        //サブタイトル
        $arg["SUBTITLE"] = $opt[$model->screen-1]["label"];

        //表幅
        $arg["WIDTH"] = ($model->screen == 1) ? "3200" : "4170";
        ...
        if ($model->z010 === "kumamoto") {
            ...
            $minGrade = $db->getOne(knjf023bQuery::getMinGrade($model));
        ...
        if ($model->screen == 1 && $kumaRenovation == '1') {
        ...
        if (!isset($model->warning) && $model->grade_hr_class != "") {
            //生徒一覧
            $query = knjf023bQuery::getSchToothInfo($model);
            $result = $db->query($query);
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                $schList[] = $row;
            }
        ...
        foreach ($nameM as $namecd1) {
            //名称マスタよりデータ取得・格納
            $query = knjf023bQuery::getNameMst($model, $namecd1);
            $optname = "opt".$namecd1;
        ...
        foreach ($schList as $counter => $Row) {
            ...
            if ($model->screen == "1") {
                ...
                if ($kumaRenovation == '') {
                    ...
                    $arg['non_kuma_Renovation'] = '1';
                } else {
                    $jaws_jointcd  = makeCmbReturn($objForm, $optF510, $Row["JAWS_JOINTCD"], "JAWS_JOINTCD"."-".$counter, $extra, 1, "BLANK");
                    $setData["JAWS_JOINTCD"] = $jaws_jointcd;
                    $jaws_jointcd3 = makeCmbReturn($objForm, $optF512, $Row["JAWS_JOINTCD3"], "JAWS_JOINTCD3"."-".$counter, $extra, 1, "BLANK");
                ...
                if ($arg["is_mie"] == "1") {
                    ...
                    $extra = " onChange=\"syokenNyuryoku(this, 'DENTISTREMARK', ['".join("','", $nameSpare2["F540"])."'], '{$counter}')\"";
                }
                $setData["DENTISTREMARKCD"] = makeCmbReturn($objForm, $optF540, $Row["DENTISTREMARKCD"], "DENTISTREMARKCD"."-".$counter, $extra, 1, "BLANK");
                if ($arg["is_not_mie"] == "1") {
                    //所見テキスト
                    if (!in_array($Row["DENTISTREMARKCD"], $nameSpare2["F540"])) {
                        $extra = "disabled";
                    } else {
                        $extra = "";
                    }
                ...
                if ($model->is_f020_dentistremark_hyouji) {
                    ...
                    //所見テキスト 2行目
                    if (!in_array($Row["DENTISTREMARK_REMARK1"], $nameSpare2["F540"])) {
                        $extra = "disabled";
                    } else {
                        $extra = "";
                    }
                    ...
                    //所見テキスト 3行目
                    if (!in_array($Row["DENTISTREMARK_REMARK3"], $nameSpare2["F540"])) {
                        $extra = "disabled";
                    } else {
                        $extra = "";
                    }
                ...
                //所見(CO)本数
                $extra = "";
                $setData["DENTISTREMARK_CO"] = knjCreateTextBox($objForm, $Row["DENTISTREMARK_CO"], "DENTISTREMARK_CO"."-".$counter, 2, 2, $extra);

                //所見(GO)チェックボックス
                if ($Row["DENTISTREMARK_GO"] == 1) {
                ...
                $setData["DENTISTREMARK_GO"] = knjCreateCheckBox($objForm, "DENTISTREMARK_GO"."-".$counter, '1', $extra.$ari_nasi_go);

                //所見(G)チェックボックス
                if ($Row["DENTISTREMARK_G"] == 1) {
                ...
                $setData["DENTISTREMARK_G"] = knjCreateCheckBox($objForm, "DENTISTREMARK_G"."-".$counter, '1', $extra.$ari_nasi_g);
                ...
                if ($arg["is_mie"] == "1") {
                    ...
                    $extra  = " onChange=\"";
                    $extra .= "syokenNyuryoku(this, 'DENTISTTREAT', ['".join("','", $nameSpare2["F541"])."'], '{$counter}');";
                    $extra .= "syokenNyuryoku(this, 'DENTISTTREAT2', ['".join("','", $nameSpare2["F541"])."'], '{$counter}');";
                    $extra .= "syokenNyuryoku(this, 'DENTISTTREAT3', ['".join("','", $nameSpare2["F541"])."'], '{$counter}');";
                    $extra .= "\"";
                }
                ...
                if ($arg["is_not_mie"] == "1") {
                    //学校歯科医・事後措置テキスト
                    if (!in_array($Row["DENTISTTREATCD"], $nameSpare2["F541"])) {
                        $extra = "disabled";
                    } else {
                        $extra = "";
                    }
                    $setData["DENTISTTREAT"] = knjCreateTextBox($objForm, $Row["DENTISTTREAT"], "DENTISTTREAT"."-".$counter, 20, 30, $extra);

                    //事後措置テキスト2
                    $setData["DENTISTTREAT2"] = knjCreateTextBox($objForm, $Row["DENTISTTREAT2"], "DENTISTTREAT2"."-".$counter, 20, 30, $extra);

                    //事後措置テキスト3
                    $setData["DENTISTTREAT3"] = knjCreateTextBox($objForm, $Row["DENTISTTREAT3"], "DENTISTTREAT3"."-".$counter, 20, 30, $extra);
                }
            } else {
                foreach ($model->shisiki as $key => $val) {
                    //歯式コンボ
        ...
        View::toHTML($model, "knjf023bForm1.html", $arg);
    }
}
