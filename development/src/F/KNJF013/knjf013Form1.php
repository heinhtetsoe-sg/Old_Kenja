...
class knjf013Form1
{
    ...
    {
        $objForm        = new form();
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjf013index.php", "", "edit");
        //DB接続
        $db = Query::dbCheckOut();
        ...
        if ($model->z010name1 == "fukuiken" || ($model->z010name1 == "koma" || ($model->Properties["printKenkouSindanIppan"] == "1" && $model->Properties["KenkouSindan_Ippan_Pattern"] == "1"))
            || ($model->Properties["useSpecial_Support_School"] != "1" && $model->Properties["unEar_db"] == "1")) {
        ...
        //サイズ調整（幅）
        $width_array = array("1" => $input_form1_width, "2" => "3685", "3" => $input_form3_width, "4" => "1740");
        $arg["widthHeader"] = $width_array[$model->input_form] + 50;
        $arg["widthMeisai"] = $width_array[$model->input_form] + $f1lbSpace;
        //サイズ調整（高さ）
        $arg["heightHeader"] = "50";
        ...
        if (!isset($model->warning) && $model->hr_class != "") {
            $query = knjf013Query::getMedexamList($model);
        ...
        //標準偏差値取得
        $sd = $db->getRow(knjf013Query::getSD($model), DB_FETCHMODE_ASSOC);

        //測定評価平均値データ取得
        $phys_avg = array();
        $query = knjf013Query::getHexamPhysicalAvgDat($model);
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        foreach ($nameM as $namecd1 => $flg) {
            //名称マスタよりデータ取得・格納
            $query = knjf013Query::getNameMst($model, $namecd1, $flg);
            $optname = "opt".$namecd1;
        ...
        //栄養状態NAMECD取得
        $nutrInfo = $sep = '';
        $result     = $db->query(knjf013Query::getNutrition($model, ""));
        $opt        = array();
        $opt[]      = $optnull;
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        foreach ($medexamList as $counter => $Row) {
            ...
            if ($model->input_form == "1") {
                ...
                $extra = "onblur=\"return Num_Check(this);\"";
                if ($model->z010name1 == "koma" || ($model->Properties["printKenkouSindanIppan"] == "1" && $model->Properties["KenkouSindan_Ippan_Pattern"] == "1")) {
                    $setData["R_EAR_DB_1000"] = makeCmbReturn2($objForm, $optF010, $Row["R_EAR_DB_1000"], "R_EAR_DB_1000"."-".$counter, $extra.$disEarR, 1, "BLANK");
                    $setData["L_EAR_DB_1000"] = makeCmbReturn2($objForm, $optF010, $Row["L_EAR_DB_1000"], "L_EAR_DB_1000"."-".$counter, $extra.$disEarL, 1, "BLANK");
                } else {
                ...
                if ($model->Properties["useEar4000Hz"] == "1") {
                    $extra = "onblur=\"return Num_Check(this);\"";
                    if ($model->z010name1 == "koma" || ($model->Properties["printKenkouSindanIppan"] == "1" && $model->Properties["KenkouSindan_Ippan_Pattern"] == "1")) {
                        $setData["R_EAR_DB_4000"] = "4000Hz：&nbsp;".makeCmbReturn2($objForm, $optF010, $Row["R_EAR_DB_4000"], "R_EAR_DB_4000"."-".$counter, $extra.$disEarR, 1, "BLANK");
                        $setData["L_EAR_DB_4000"] = "4000Hz：&nbsp;".makeCmbReturn2($objForm, $optF010, $Row["L_EAR_DB_4000"], "L_EAR_DB_4000"."-".$counter, $extra.$disEarL, 1, "BLANK");
                    } else {
            ...
            if ($model->input_form == "2") {
                //栄養状態
                $extra = "style=\"width:170px;\" onChange=\"syokenNyuryoku(this, 'NUTRITIONCD_REMARK', '{$counter}')\"";
                ...
                //鼻・副鼻腔疾患
                $extra = "style=\"width:170px;\" onChange=\"syokenNyuryoku(this, 'NOSEDISEASECD_REMARK2', '{$counter}')\"";
                $setData["NOSEDISEASECD6"] = makeCmbReturn2($objForm, $optF062, $Row["NOSEDISEASECD6"], "NOSEDISEASECD6"."-".$counter, $extra, 1, "BLANK");
                $extra  = "STYLE=\"WIDTH:100%\" WIDTH=\"100%\"";
                $extra .= ((int)$Row["NOSEDISEASECD6"] < 2) ? " disabled" : "";
                ...
                //口腔咽頭疾患・異常
                $extra = "style=\"width:170px;\" onChange=\"syokenNyuryoku(this, 'NOSEDISEASECD_REMARK3', '{$counter}')\"";
                $setData["NOSEDISEASECD7"] = makeCmbReturn2($objForm, $optF063, $Row["NOSEDISEASECD7"], "NOSEDISEASECD7"."-".$counter, $extra, 1, "BLANK");
                $extra  = "STYLE=\"WIDTH:100%\" WIDTH=\"100%\"";
                $extra .= ((int)$Row["NOSEDISEASECD7"] < 2) ? " disabled" : "";
                ...
                //皮膚疾患
                $extra = "style=\"width:170px;\" onChange=\"syokenNyuryoku(this, 'SKINDISEASECD_REMARK', '{$counter}')\"";
                $setData["SKINDISEASECD"] = makeCmbReturn2($objForm, $optF070, $Row["SKINDISEASECD"], "SKINDISEASECD"."-".$counter, $extra, 1, "BLANK");
                $extra  = "STYLE=\"WIDTH:100%\" WIDTH=\"100%\"";
                $extra .= ((int)$Row["SKINDISEASECD"] < 2) ? " disabled" : "";
                ...
                if ($arg["is_mie"] == "1") {
                    ...
                    $extra = "style=\"width:170px;\" onChange=\"syokenNyuryoku(this, 'TB_NAME_REMARK1', '{$counter}')\"";
                }
                ...
                if ($arg["is_not_mie"] == "1") {
                    $extra  = "STYLE=\"WIDTH:100%\" WIDTH=\"100%\"";
                    $extra .= ((int)$Row["TB_NAMECD"] < 2) ? " disabled" : "";
                ...
                //心臓・臨床医学的検査
                $extra = "style=\"width:170px;\" onChange=\"syokenNyuryoku(this, 'HEART_MEDEXAM_REMARK', '{$counter}')\"";
                ...
                //心臓・疾病及び異常
                $extra = "style=\"width:170px;\" onChange=\"syokenNyuryoku(this, 'HEARTDISEASECD_REMARK', '{$counter}')\"";
            ...
            if ($model->input_form == "3") {
                ...
                if ($arg["is_mie"] == "1") {
                    ...
                    $extra = "style=\"width:170px;\" onChange=\"syokenNyuryoku(this, 'OTHER_REMARK2', '{$counter}')\"";
                }
                $setData["OTHERDISEASECD"] = makeCmbReturn2($objForm, $optF140, $Row["OTHERDISEASECD"], "OTHERDISEASECD"."-".$counter, $extra, 1, "BLANK");
                if ($arg["is_not_mie"] == "1") {
                    $extra  = "";
                    $extra .= ((int)$Row["OTHERDISEASECD"] < 2) ? " disabled" : "";
                ...
                if ($arg["is_mie"] == "1") {
                    ...
                    $extra = "style=\"width:170px;\" onChange=\"syokenNyuryoku(this, 'TREAT_REMARK1', '{$counter}')\"";
                }
                ...
                if ($arg["is_not_mie"] == "1") {
                    ...
                    $extra  = "STYLE=\"WIDTH:100%\" WIDTH=\"100%\"";
                    $extra .= ((int)$Row["TREATCD"] < 2) ? " disabled" : "";
        ...
        //未検査項目設定
        $notFieldSet = $sep = "";
        $query = knjf013Query::getMedexamDetNotExaminedDat($model);
        $result = $db->query($query);
        ...
        View::toHTML($model, "knjf013Form1.html", $arg);
    }
}
//コンボ作成
