...
class knjf034bForm1
{
    function main(&$model){
        ...
        //フォーム作成
        $arg["start"]   = $objForm->get_start("knjf034bForm1", "POST", "knjf034bindex.php", "", "knjf034bForm1");
        ...
        if ($disable == 1) {
            if ($model->Properties["useSpecial_Support_Hrclass"] == "1") {
                $query = knjf034bQuery::getGhrCd($model);
            } else if ($model->Properties["useFi_Hrclass"] == "1") {
                $query = knjf034bQuery::getFiGradeHrclass($model);
            } else {
                $query = knjf034bQuery::getHrClassAuth(CTRL_YEAR, CTRL_SEMESTER, AUTHORITY, STAFFCD, $model);
            }
            ...
            $objForm->ae( array("type"       => "select",
                                ...
                                "extrahtml"  => "onchange=\"return btn_submit('knjf034b'),AllClearList();\"",
        ...
        if ($disable == 1) { 
            if ($model->Properties["useSpecial_Support_Hrclass"] == "1") {
                $query = knjf034bQuery::GetS_S_Students($model);//生徒一覧取得（実クラス）
            } else if ($model->Properties["useFi_Hrclass"] == "1") {
                $query = knjf034bQuery::getFiStudents($model);//生徒一覧取得（FI）
            } else {
                $query = knjf034bQuery::getClassStudents($model);//生徒一覧取得
            }
            $result = $db->query($query);
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        knjCreateHidden($objForm, "useFormKNJF034B_2", $model->Properties["useFormKNJF034B_2"]);
        ...
        View::toHTML($model, "knjf034bForm1.html", $arg); 
    }
}
?>
