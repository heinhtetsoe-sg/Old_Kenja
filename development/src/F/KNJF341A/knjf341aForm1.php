...
class knjf341aForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjf341aForm1", "POST", "knjf341aindex.php", "", "knjf341aForm1");
        ...
        //教育委員会チェック
        $getFlg = $db->getOne(knjf341aQuery::getNameMst());
        if ($getFlg !== '1') {
            $arg["jscript"] = "OnEdboardError();";
        }

        //年度
        $extra = "onChange=\"return btn_submit('knjf341a');\"";
        $query = knjf341aQuery::getYear();
        ...
        View::toHTML($model, "knjf341aForm1.html", $arg);
    }
}
...
{
    $row1 = array();
    $query = knjf341aQuery::getSchoolData($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model, $db)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJF341A");
