...
class knjf344Form1
{
    public function main(&$model)
    {
        ...
        $objForm = new form();
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjf344index.php", "", "edit");
        ...
        $rowCount = 0;
        $query = knjf344Query::readQuery($model);
        $result = $db->query($query);
        while ($Row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //画面に表示する値
            $textArray['ATTEND_NAME'] = $Row['ATTEND_NAME'];
            $textArray['NAME']        = $Row['NAME'];
            $textArray['RESULT']      = $Row['RESULT'];
            $textArray['REMARK']      = $Row['REMARK'];
            $arg["data"][] = $textArray;
            ...
            knjCreateHidden($objForm, "{$rowPrefix}RESULT", $Row['RESULT']);
            knjCreateHidden($objForm, "{$rowPrefix}REMARK", $Row['REMARK']);

            $rowCount++;
        }

        //報告済み日付
        $tempLastHoukokuDate = $db2->getOne(knjf344Query::getMaxFixedDate($model));
        $lastHoukokuDate = '';
        ...
        //確定データ
        $query = knjf344Query::getFixed($model);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJF344");
        ...
        View::toHTML($model, "knjf344Form1.html", $arg);
    }
}
