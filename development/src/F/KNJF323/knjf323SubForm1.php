...
class knjf323SubForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform1", "POST", "knjf323index.php", "", "subform1");
        ...
        if (isset($model->schregno) && !isset($model->warning)) {
            ...
            $Row = $db->getRow(knjf323Query::getCareHData($model, $model->field["YEAR"], "01"), DB_FETCHMODE_ASSOC);
            //データ取得 -- MEDEXAM_CARE_DAT
            $query = knjf323Query::getCareData($model, $model->field["YEAR"], "01");
        ...
        View::toHTML($model, "knjf323SubForm1.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg, $model) {
    //前年度からコピーボタン
    $extra = (AUTHORITY >= DEF_UPDATE_RESTRICT && $model->field["YEAR"] == CTRL_YEAR) ? "onclick=\"return btn_submit('subform1_copy');\"" : "disabled";
    $arg["button"]["btn_copy"] = knjCreateBtn($objForm, "btn_copy", "前年度からコピー", $extra);

    //更新ボタン
    $extra = (AUTHORITY >= DEF_UPDATE_RESTRICT) ? "onclick=\"return btn_submit('subform1_update');\"" : "disabled";
