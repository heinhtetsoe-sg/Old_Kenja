...
class knjf323SubForm3 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform3", "POST", "knjf323index.php", "", "subform3");
        ...
        if (isset($model->schregno) && !isset($model->warning)) {
            ...
            $Row = $db->getRow(knjf323Query::getCareHData($model, $model->field["YEAR"], "03"), DB_FETCHMODE_ASSOC);
            //データ取得 -- MEDEXAM_CARE_DAT
            $query = knjf323Query::getCareData($model, $model->field["YEAR"], "03");
        ...
        View::toHTML($model, "knjf323SubForm3.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg, $model) {
    //前年度からコピーボタン
    $extra = (AUTHORITY >= DEF_UPDATE_RESTRICT && $model->field["YEAR"] == CTRL_YEAR) ? "onclick=\"return btn_submit('subform3_copy');\"" : "disabled";
    $arg["button"]["btn_copy"] = knjCreateBtn($objForm, "btn_copy", "前年度からコピー", $extra);

    //更新ボタン
    $extra = (AUTHORITY >= DEF_UPDATE_RESTRICT) ? "onclick=\"return btn_submit('subform3_update');\"" : "disabled";
