...
class knjf323SubForm6 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform6", "POST", "knjf323index.php", "", "subform6");
        ...
        $extra = "onchange=\"return btn_submit('subform6')\"";
        makeCmb($objForm, $arg, $db, $query, "YEAR", $model->field["YEAR"], $extra, 1);

        //警告メッセージを表示しない場合
        if (isset($model->schregno) && !isset($model->warning)) {
            //データ取得 -- MEDEXAM_CARE_HDAT
            $Row = $db->getRow(knjf323Query::getCareHData($model, $model->field["YEAR"], "06"), DB_FETCHMODE_ASSOC);
            //データ取得 -- MEDEXAM_CARE_DAT
            $query = knjf323Query::getCareData($model, $model->field["YEAR"], "06");
            ...
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                ...
                if (strlen($row["CARE_REMARK1"]) > 0) {
                    if ($row["CARE_KIND"]."_".$row["CARE_ITEM"]."_".$row["CARE_SEQ"] == '01_01_02') {
                        ...
                        if (in_array('春', $array)) $Row["REASON".$row["CARE_SEQ"]."_1"] = 1;
                        if (in_array('夏', $array)) $Row["REASON".$row["CARE_SEQ"]."_2"] = 1;
                        if (in_array('秋', $array)) $Row["REASON".$row["CARE_SEQ"]."_3"] = 1;
                        if (in_array('冬', $array)) $Row["REASON".$row["CARE_SEQ"]."_4"] = 1;
                    } else {
        ...
        View::toHTML($model, "knjf323SubForm6.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg, $model) {
    //前年度からコピーボタン
    $extra = (AUTHORITY >= DEF_UPDATE_RESTRICT && $model->field["YEAR"] == CTRL_YEAR) ? "onclick=\"return btn_submit('subform6_copy');\"" : "disabled";
    $arg["button"]["btn_copy"] = knjCreateBtn($objForm, "btn_copy", "前年度からコピー", $extra);

    //更新ボタン
    $extra = (AUTHORITY >= DEF_UPDATE_RESTRICT) ? "onclick=\"return btn_submit('subform6_update');\"" : "disabled";
