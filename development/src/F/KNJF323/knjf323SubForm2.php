...
class knjf323SubForm2 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("subform2", "POST", "knjf323index.php", "", "subform2");
        ...
        if (isset($model->schregno) && !isset($model->warning)) {
            ...
            $Row = $db->getRow(knjf323Query::getCareHData($model, $model->field["YEAR"], "02"), DB_FETCHMODE_ASSOC);
            //データ取得 -- MEDEXAM_CARE_DAT
            $query = knjf323Query::getCareData($model, $model->field["YEAR"], "02");
            $result = $db->query($query);
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
                $Row["CHECK".$row["CARE_KIND"]."_".$row["CARE_ITEM"]."_".$row["CARE_SEQ"]] = 1;
                if (strlen($row["CARE_REMARK1"]) > 0) $Row["TEXT".$row["CARE_KIND"]."_".$row["CARE_ITEM"]."_".$row["CARE_SEQ"]] = $row["CARE_REMARK1"];
            }
            $result->free();
        } else {
            $Row =& $model->field;
        ...
        $cnt[1][3] = array("cnt" => 2,  "txt" => array(2));
        $cnt[1][4] = array("cnt" => 2,  "txt" => array());
        $cnt[2][1] = array("cnt" => 2,  "txt" => array());
        $cnt[2][2] = array("cnt" => 3,  "txt" => array(3));
        $cnt[2][3] = array("cnt" => 3,  "txt" => array());
            ...
            for ($j=1; $j <= 4 ; $j++) {
                ...
                for ($k=1; $k <= $cnt[$i][$j]["cnt"] ; $k++) {
                    ...
                    if (in_array($k, $cnt[$i][$j]["txt"])) {
                        ...
                        $name = "TEXT".$cd;
                        $moji = ($i == 2) ? 40 : 20;
                        $moji = ("TEXT01_03_02" == $name) ? 40 : $moji;
        ...
        $arg["data"]["TEXT02_04_00"] = KnjCreateTextArea($objForm, "TEXT02_04_00", 7, (20 * 2 + 1), "soft", "style=\"height:{$height}px;\"", $Row["TEXT02_04_00"]);
        ...
        View::toHTML($model, "knjf323SubForm2.html", $arg);
    }
}
...
function makeBtn(&$objForm, &$arg, $model) {
    //前年度からコピーボタン
    $extra = (AUTHORITY >= DEF_UPDATE_RESTRICT && $model->field["YEAR"] == CTRL_YEAR) ? "onclick=\"return btn_submit('subform2_copy');\"" : "disabled";
    $arg["button"]["btn_copy"] = knjCreateBtn($objForm, "btn_copy", "前年度からコピー", $extra);

    //更新ボタン
    $extra = (AUTHORITY >= DEF_UPDATE_RESTRICT) ? "onclick=\"return btn_submit('subform2_update');\"" : "disabled";
