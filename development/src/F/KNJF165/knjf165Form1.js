function btn_submit(cmd) {
    //更新
    if (cmd == 'update') {
        if (document.forms[0].CLASS_SELECTED.length == 0 && document.forms[0].CLASS_NAME.length == 0) {
            alert('{rval MSG916}');
            return;
        }
        //左
        ...
        //右
        attribute3 = document.forms[0].selectright;
        attribute3.value = "";
        sep = "";
        for (var i = 0; i < document.forms[0].CLASS_NAME.length; i++) {
            attribute3.value = attribute3.value + sep + document.forms[0].CLASS_NAME.options[i].value;
            sep = ",";
        }
    }
