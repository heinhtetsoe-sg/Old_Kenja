...
function newwin(SERVLET_URL){
    ...
    //印刷対象チェック
    if ((document.forms[0].GANKA_KENSIN.checked == false) &&
        (document.forms[0].KEKKA_HA.checked==false) &&
        (document.forms[0].KEKKA_HA2.checked==false) &&
        (document.forms[0].TEIKI_KENSIN.checked==false) &&
        (document.forms[0].TEIKI_KENSIN_ITIRAN.checked==false)
    ) {
        alert('出力項目を選択して下さい。');
        return;
    }
