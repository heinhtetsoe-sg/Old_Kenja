...
class knjf346Form1
{
    public function main(&$model)
    {
        ...
        $objForm = new form();
        //フォーム作成
        $arg["start"] = $objForm->get_start("edit", "POST", "knjf346index.php", "", "edit");
        ...
        $rowCount = 0;
        $query = knjf346Query::readQuery($model);
        $result = $db->query($query);
        while ($Row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
            //画面に表示する値
            $textArray['ATTEND_NAME'] = $Row['ATTEND_NAME'];
            $textArray['NAME']        = $Row['NAME'];
            $textArray['JUDGEMENT']   = $Row['JUDGEMENT'];
            $textArray['JUDGEMENT_DATE'] = str_replace("-", "/", $Row['JUDGEMENT_DATE']);
            $arg["data"][] = $textArray;
            ...
            knjCreateHidden($objForm, "{$rowPrefix}JUDGEMENT", $Row['JUDGEMENT']);
            knjCreateHidden($objForm, "{$rowPrefix}JUDGEMENT_DATE", $Row['JUDGEMENT_DATE']);

            $rowCount++;
        }

        //報告済み日付
        $tempLastHoukokuDate = $db2->getOne(knjf346Query::getMaxFixedDate($model));
        $lastHoukokuDate = '';
        ...
        //確定データ
        $query = knjf346Query::getFixed($model);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJF346");
        ...
        View::toHTML($model, "knjf346Form1.html", $arg);
    }
}
