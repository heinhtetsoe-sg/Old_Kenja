...
class knjf030eForm1
{
    function main(&$model){
        ...
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("knjf030eForm1", "POST", "knjf030eindex.php", "", "knjf030eForm1");
        ...
        $extra = array("id=\"KUBUN1\" onClick=\"btn_submit('knjf030e')\"", "id=\"KUBUN2\" onClick=\"btn_submit('knjf030e')\"");
        $radioArray = knjCreateRadio($objForm, "KUBUN", $model->field["KUBUN"], $extra, $opt_data, get_count($opt_data));
        foreach($radioArray as $key => $val) $arg["data"][$key] = $val;

        if ($model->Properties["not_useFi_Hrclass"] == "1") {
            $arg["HR_DIV_NAME"] = "クラス選択";
        } else {
            $arg["HR_DIV_NAME"] = "FIクラス選択";
        }
        if ($model->Properties["not_useFi_Hrclass"] == "1") {
            if ($model->field["KUBUN"] == 1) $arg["clsno"] = $model->field["KUBUN"];
        } else {
            if ($model->field["KUBUN"] == 1) $arg["ficlsno"] = $model->field["KUBUN"];
        }
        ...
        View::toHTML($model, "knjf030eForm1.html", $arg); 
...
function makeClassItiran(&$objForm, &$arg, $db, &$model) {
        $row1 = array();
        $query = knjf030eQuery::getHrClassList($model);
        ...
        if ($model->field["KUBUN"] == 2) {
            ...
            $query = knjf030eQuery::getSchno($model);//生徒一覧取得
            $result = $db->query($query);
            while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model) {
    ...
    knjCreateHidden($objForm, "PRGID", "KNJF030E");
    ...
    knjCreateHidden($objForm, "not_useFi_Hrclass", $model->Properties["not_useFi_Hrclass"]);
}
