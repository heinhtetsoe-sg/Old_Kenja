...
class knjf030dForm1
{
    function main(&$model){
        ...
        $objForm = new form;
        $arg["start"]   = $objForm->get_start("knjf030dForm1", "POST", "knjf030dindex.php", "", "knjf030dForm1");

        //リスト表示選択
        $opt = array(1, 2); //1:クラス選択 2:個人選択
        if (!$model->field["KUBUN"]) $model->field["KUBUN"] = 1;
        $onClick = " onclick =\" return btn_submit('knjf030d');\"";
        ...
        $db = Query::dbCheckOut();
        $row1 = array();
        $query = knjf030dQuery::getHrClassList($model);
        ...
        if ($model->field["KUBUN"] == 2) {
            ...
            $query = knjf030dQuery::getSchno($model);//生徒一覧取得
            $result = $db->query($query);
            while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
        ...
        View::toHTML($model, "knjf030dForm1.html", $arg); 
