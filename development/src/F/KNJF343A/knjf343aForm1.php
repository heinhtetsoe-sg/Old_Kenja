...
class knjf343aForm1
{
    public function main(&$model)
    {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjf343aForm1", "POST", "knjf343aindex.php", "", "knjf343aForm1");
        ...
        //教育委員会チェック
        $getFlg = $db->getOne(knjf343aQuery::getNameMst());
        if ($getFlg !== '1') {
            $arg["jscript"] = "OnEdboardError();";
        }

        //年度
        $extra = "onChange=\"return btn_submit('knjf343a');\"";
        $query = knjf343aQuery::getYear();
        ...
        View::toHTML($model, "knjf343aForm1.html", $arg);
    }
}
...
{
    $row1 = array();
    $query = knjf343aQuery::getSchoolData($model);
    $result = $db->query($query);
    while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
...
function makeHidden(&$objForm, $model, $db)
{
    ...
    knjCreateHidden($objForm, "PRGID", "KNJF343A");
