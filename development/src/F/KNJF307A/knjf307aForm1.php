...
class knjf307aForm1 {
    ...
    function main(&$model) {
        ...
        //フォーム作成
        $arg["start"] = $objForm->get_start("knjf307aForm1", "POST", "knjf307aindex.php", "", "knjf307aForm1");

        //DB接続
        $db = Query::dbCheckOut();

        //教育委員会チェック
        $getFlg = $db->getOne(knjf307aQuery::getNameMst());
        if ($getFlg !== '1') {
            $arg["jscript"] = "OnEdboardError();";
        }

        //年度
        $extra = "onChange=\"return btn_submit('knjf307a');\"";
        $query = knjf307aQuery::getYear();
        ...
        View::toHTML($model, "knjf307aForm1.html", $arg); 
    }
}
...
function makeListToList(&$objForm, &$arg, $db, &$model) {
    $row1 = array();
    $query = knjf307aQuery::getSchoolData($model);
    $result = $db->query($query);
    while($row = $result->fetchRow(DB_FETCHMODE_ASSOC)){
