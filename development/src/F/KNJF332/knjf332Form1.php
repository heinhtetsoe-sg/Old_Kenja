...
class knjf332Form1
{
    function main(&$model) {
        ...
        $objForm = new form;
        //フォーム作成
        $arg["start"]   = $objForm->get_start("edit", "POST", "knjf332index.php", "", "edit");
        ...
        $count = 0;
        $bifKey = "";
        $query = knjf332Query::ReadQuery($model);
        ...
        //報告済み日付
        $query = knjf332Query::getReport($model);
        $setExeDate = "";
        $result = $db->query($query);
        while ($row = $result->fetchRow(DB_FETCHMODE_ASSOC)) {
        ...
        //確定データ
        $query = knjf332Query::getFixed($model);
        ...
        knjCreateHidden($objForm, "PRGID", "KNJF332");
        ...
        View::toHTML($model, "knjf332Form1.html", $arg); 
    }
}
