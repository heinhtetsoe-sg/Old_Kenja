...
class Date_Calc {
    ...
    function dayOfWeek($day="",$month="",$year="")
    {
        ...
        $day =     ( floor((13 * (int)$month - 1) / 5) +
                $day + ((int)$year % 100) +
                floor(((int)$year % 100) / 4) +
                floor(((int)$year / 100) / 4) - 2 *
                floor((int)$year / 100) + 77);

        $weekday_number = (((int)$day - 7 * floor((int)$day / 7)));
    ...
    function julianDate($day="",$month="",$year="")
    {
        ...
        $julian = ($days[(int)$month - 1] + (int)$day);
    ...
    function quarterOfYear($day="",$month="",$year="")
    {
        ...
        $year_quarter = (intval(((int)$month - 1) / 3 + 1));
    ...
    function nextDayOfWeek($dow,$day="",$month="",$year="",$format="%Y%m%d",$onOrAfter=false)
    {
        ...
        elseif($curr_weekday > $dow)
            $days += 7 - ( $curr_weekday - (int)$dow );
        else
            $days += (int)$dow - $curr_weekday;
    ...
    function beginOfNextWeek($day="",$month="",$year="",$format="%Y%m%d")
    {
        ...
        $date = Date_Calc::daysToDate(Date_Calc::dateToDays((int)$day+7,$month,$year),"%Y%m%d");
    ...
    function dateToDays($day,$month,$year)
    {
        ...
        return ( floor((  146097 * (int)$century)    /  4 ) +
                floor(( 1461 * (int)$year)        /  4 ) +
                floor(( 153 * (int)$month +  2) /  5 ) +
                (int)$day +  1721119);
    ...
    function NWeekdayOfMonth($occurance,$dayOfWeek,$month,$year,$format="%Y%m%d")    {
        ...
        $DOW1day = sprintf("%02d",(((int)$occurance - 1) * 7 + 1));
        $DOW1 = Date_Calc::dayOfWeek($month,$year,$DOW1day);

        $wdate = ((int)$occurance - 1) * 7 + 1 +
                (7 + (int)$dayOfWeek - (int)$DOW1) % 7;
