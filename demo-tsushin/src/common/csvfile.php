...
class csvFile
{
    ...
    {
        global $sess;
        $session = new APP_Session($sess->id, 'csvFile');
        $session->clear();
        if ($this->data){            
...
EOP;
        ...
        $if = "<iframe name=\"upiframe\" style=\"display:none\" src=\"".REQUESTROOT ."/common/csvfile.php?cmd=upload\"></iframe>\n";
...
if ($_POST["session"]){
    $session = new APP_Session($_POST["session"], 'csvFile');
...
<body>
<form enctype="multipart/form-data" action="csvfile.php" method="post">
...
//ファイル書出し
    ...
    if (is_array($sessData["csv"]) && is_array($sessData["header"])){
        ...
        //Edge対応で、変換前のファイル名を保持するよう、変更。
        $filenameBase = $sessData["filename"];
        $filename = mb_convert_encoding($filenameBase, "SJIS-win", mb_internal_encoding());
        ...
        if (stristr($_SERVER['HTTP_USER_AGENT'], "MSIE") || stristr($_SERVER['HTTP_USER_AGENT'], "Trident")) {
            ...
            header("Content-disposition: attachment; filename*=UTF-8''".rawurlencode($filenameBase));
        }
