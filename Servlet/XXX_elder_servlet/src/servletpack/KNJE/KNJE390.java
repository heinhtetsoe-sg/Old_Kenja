...
public class KNJE390 {

    private static final Log log = LogFactory.getLog(KNJE390.class);

    private static final String newLine = "\n";

    private static final String FORM_A1 = "KNJE390_A_1_2.frm";
    private static final String FORM_B1 = "KNJE390_B_1_2.frm";
    private static final String FORM_D1 = "KNJE390_D_1_2.frm";
    private boolean _hasData;

    /** 中央寄せ */
    private static final String ATTR_CENTERING = "Hensyu=3";

    KNJE390_sien _sienObj = null;
    ...
    public void svf_out(
        ...
        try {
            ...
            if ("1".equals(_param._printB) && "1".equals(_param._tokushiShienPlanPatern)) {
                _sienObj = new KNJE390_sien();
                _sienObj.svf_out(request, response);
            }
    ...
    private void printMain(final DB2UDB db2, final Vrw32alp svf, final Param param) {
        ...
        int printBIdx = -1;
        if ("1".equals(param._printA)) formList.add(new KNJE390B(param)); // Ｂ．プロフィール
        if ("1".equals(param._printB)) {
            formList.add(new KNJE390C(param)); // Ｃ．支援内容・計画  ※登録順番に依存しているので、入れ替えは注意。
            printBIdx = formList.size();
        }
        if ("1".equals(param._printC)) formList.add(new KNJE390D(param)); // Ｄ．移行支援計画
        if ("1".equals(param._printD)) formList.add(new KNJE390A(param)); // Ａ．アセスメント票
        if ("1".equals(param._printE)) formList.add(new KNJE390E(param));
        if ("1".equals(param._printF)) formList.add(new KNJE390F(param));
        if ("1".equals(param._printG)) formList.add(new KNJE390G(param));
        if ("1".equals(param._printH)) formList.add(new KNJE390H(param));
        ...
        for (final Iterator it = studentList.iterator(); it.hasNext();) {
            ...
            //for (final Iterator fit = formList.iterator(); fit.hasNext();) {
                //Bを印刷指定していて、プロパティが立っていて、KNJE390_sienが生成済みで、Bの出力順になったら処理。
                if ("1".equals(param._printB) && "1".equals(param._tokushiShienPlanPatern) && _sienObj != null && cnt == printBIdx-1) {
    ...
    // プロフィール
    private static class KNJE390B extends KNJE390_0 {
        ...
        public KNJE390B(final Param param) {
            super(param);
        }
        ...
        public void printMain(final Vrw32alp svf, final DB2UDB db2, final Map student) {
            ...
            form._form1 = !"".equals(_param._formA_1) ? _param._formA_1 : "KNJE390_A_1.frm";
            form._form2 = !"".equals(_param._formA_2) ? _param._formA_2 : "KNJE390_A_2.frm";
            form._form2n = 4;
            form._recMax1 = 14;
            form._recMax2 = 43;
            form._isForm1 = true;
        ...
        public void printStudent(final DB2UDB db2, final Map student, final String schregno, final Form form) {
            ...
            if (!FORM_A1.equals(_param._formA_1)) {
                ...
                form.VrsOut("ENTRY_PERSON1", getString("UPDATE_STAFFNAME1", updateMap));  // 記入者
            }
            ...
            sqlSchregRelaDat.append(" ORDER BY LPAD(T1.RELANO, 2, '0') ");
            ...
            final String challengeDnames = newLineReplace(getString("CHALLENGED_NAMES", main), "");
            svfVrListOut(form, "DIAG_NAME", getTokenList(challengeDnames, form.fieldKeta("DIAG_NAME1"))); // 障害名診断名
        ...
        private void printBRecordIryou(final DB2UDB db2, final String schregno, final Form form) {
            ...
            for (int k = 0; k < medRecordDivList.size(); k++) {
                ...
                if ("header".equals(recordDiv)) {
                    ...
                    div2name = getTokenList("かかりつけ医", form.fieldKeta("DIVIDE7_2"));
                } else if ("3".equals(recordDiv))  {
                    div2name = getTokenList("入院・既往歴", form.fieldKeta("DIVIDE7_2"));
                }
        ...
        private void printBRecordKenkouKanri(final DB2UDB db2, final String schregno, final Form form) {
            ...
            dFukuyaku.attribute("GRP8" + odiv + "_1", ATTR_PAINT_GRAY_FILL + "," + ATTR_COLOR_GRAY); // グループ3
            dFukuyaku.addCenter("DIVIDE8" + odiv + "_1", singleton(kenkoukanri)); // 区分1
            dFukuyaku.addAll("GRP8" + odiv + "_2", "01"); // グループ2
            dFukuyaku.attribute("GRP8" + odiv + "_2", ATTR_PAINT_GRAY_FILL + "," + ATTR_COLOR_GRAY); // グループ3
            dFukuyaku.addCenter("DIVIDE8" + odiv + "_2", singleton("服薬")); // 区分2
            for (int i = 0; i < kenkou.size(); i++) {
                final Map row = (Map) kenkou.get(i);
                final String si = String.valueOf(i);
                final FieldData d = new FieldData();
                d.add("GRP8" + odiv + "_3", seq(0, 2)); // グループ3
                d.attribute("GRP8" + odiv + "_3", ATTR_PAINT_GRAY_FILL + "," + ATTR_COLOR_GRAY); // グループ3

                final List<String> seq = new ArrayList<String>();
                final List<String> title = new ArrayList<String>();
                final List<String> data = new ArrayList<String>(); // 薬品名
                title.add("薬品名");
                data.add(getString("MEDICINE_NAME", row));
                seq.add("0");
                final List<String> byomeiTokenList = KNJE390_0.getTokenList(getString("DISEASE_CONDITION_NAME", row), form.fieldKeta("MEDICINE" + odiv + ""));
                for (int bi = 0; bi < Math.max(1, byomeiTokenList.size()); bi++) {
                    title.add(bi == 0 ? "病名・症状" : "");
                    data.add(bi < byomeiTokenList.size() ? byomeiTokenList.get(bi) : ""); // 病名
                    seq.add("1");
                }
                d.add("DIVIDE8" + odiv + "_3", title); // 区分3
                d.add("GRP8" + odiv + "_3", seq); // グループ3
                d.add("GRP8" + odiv + "_4", seq); // グループ4
                d.add("MEDICINE" + odiv + "", data);

                d.addAll("GRP8" + odiv + "_5", si); // グループ5
                d.attribute("GRP8" + odiv + "_5", ATTR_PAINT_GRAY_FILL + "," + ATTR_COLOR_GRAY); // グループ3
                d.addAll("GRP8" + odiv + "_5_2", si); // グループ5
                d.attribute("GRP8" + odiv + "_5_2", ATTR_PAINT_GRAY_FILL + "," + ATTR_COLOR_GRAY); // グループ3
                d.add("HOWTO_NAME" + odiv + "", asList(new String[] {"服用状況", "留意点"})); // 服用状況留意点
            ...
            dgSonotanoArerugi.addAll("GRPSHOKU_1", "SH"); // グループ1
            ...
            dgSonotanoArerugi.addAll("GRPSHOKU_2", "02"); // グループ2
            ...
            final String siArerugi = String.valueOf(4) + String.valueOf(0);
            ...
            setRecordFieldDataAll(all, "GRP1", "FF");
            setRecordFieldDataList(all, "DIV1", extendStringList(singleton(kenkoukanri), all.size(), true));
        ...
        private void printBRecordFukushi(final DB2UDB db2, final String schregno, final Form form) {
            ...
            for (int k = 0; k < fukushiRecordDivList.size(); k++) {
                ...
                if ("1".equals(recordDiv)) {
                    ...
                    useD = D8;
                    gd.addAll("GRP82" + odiv + "_1", "S" + "H"); // グループ1
                    gd.attribute("GRP82" + odiv + "_1", ATTR_PAINT_GRAY_FILL + "," + ATTR_COLOR_GRAY); // グループ
                    gd.addCenter("DIVIDE82" + odiv + "_1", singleton(grp1Fukushi)); // 区分1
                    gd.addAll("GRP82" + odiv + "_2", recordDiv + "H"); // グループ2
                    gd.attribute("GRP82" + odiv + "_2", ATTR_PAINT_GRAY_FILL + "," + ATTR_COLOR_GRAY); // グループ
                    gd.addCenter("DIVIDE82" + odiv + "_2", asList(grp2Hosougu)); // 区分2
                    ...
                    for (int l = 0; l < fukushiRecordNoList.size(); l++) {
                        ...
                        if ("1".equals(recordNo)) {
                            ...
                            gd2.addCenter("DIVIDE82" + odiv + "_3", singleton("補装具")); // 区分3
                            ...
                            for (int i = 0; i < fukushi1.size(); i++) {
                                ...
                                d.addWithKeta("REMARK82" + odiv + "", name, form); // 補装具
                                gd2.addFieldData(d);
                            }
                    ...
                    gd.setFieldDiv("GRP1", "GRP82" + odiv + "_1");
                    gd.setFieldDiv("DIV1", "DIVIDE82" + odiv + "_1");
                    gd.setFieldDiv("GRP2", "GRP82" + odiv + "_2");
                    gd.setFieldDiv("DIV2", "DIVIDE82" + odiv + "_2");
                    ...
                    gd.addAll("GRP82" + odiv + "_2", recordDiv + "H"); // グループ2
                    gd.attribute("GRP82" + odiv + "_2", ATTR_PAINT_GRAY_FILL + "," + ATTR_COLOR_GRAY); // グループ
                    gd.addCenter("DIVIDE82" + odiv + "_2", asList(grp2JigyoushoCerviceNoRiyou)); // 区分2
                    for (int i = 0; i < fukushi.size(); i++) {
                        final Map row = (Map) fukushi.get(i);
                        ...
                        for (int j = 1; j <= 2; j++) {
                            ...
                            final FieldData d = new FieldData();
                            d.addAll("GRP82" + odiv + "_3", "1" + String.valueOf(j)); // グループ3
                            final String kubun;
                            if (j == 1) {
                                kubun = "事業所";
                            } else {
                            ...
                            d.addCenter("DIVIDE82" + odiv + "_3", singleton(kubun)); // 区分3
                            ...
                            d.addAll("GRP82" + odiv + "_4", String.valueOf(j)); // グループ4
                            d.addAll("GRP82" + odiv + "_4_2", String.valueOf(j)); // グループ4
                            d.addAll("GRP82" + odiv + "_4_3", String.valueOf(j)); // グループ4
                            d.addWithKeta("REMARK82" + odiv + "", text.toString(), form); // 補装具
                            gd.addFieldData(d);
                        }
                    }
    ...
    // 支援内容・計画
    private static class KNJE390C extends KNJE390_0 {
        private String useD; // コードチェック用
        private String D1 = "1";
        private String D2 = "2";
        private String D4 = "4";
        ...
        public KNJE390C(final Param param) {
            super(param);
        }
        public void printMain(final Vrw32alp svf, final DB2UDB db2, final Map student) {
            final String schregno = getString("SCHREGNO", student);
            ...
            final Form form = new Form(_param, svf);
            form._form1 = !"".equals(_param._formB) ? _param._formB : "KNJE390_B_1.frm";
            form._form2 = null;
            form._form2n = 4;
            form._recMax1 = FORM_B1.equals(_param._formB) ? 37 : 38;
            form._recMax2 = 43;
            form.setForm(form._form1, 4);
            form._isForm1 = true;
    ...
    private static class KNJE390D extends KNJE390_0 {
        public KNJE390D(final Param param) {
            super(param);
        }
        public void printMain(final Vrw32alp svf, final DB2UDB db2, final Map student) {
            final String schregno = getString("SCHREGNO", student);
            ...
            final Form form = new Form(_param, svf);
            form._form1 = "KNJE390_C_1.frm";
            form._form2 = "KNJE390_C_2.frm";
            form._form2n = 4;
            form._recMax1 = 29;
            form._recMax2 = 43;
            form._isForm1 = true;
...
//                        final String si = String.valueOf(i);
    ...
    // アセスメント票
    private static class KNJE390A extends KNJE390_0 {
        public KNJE390A(final Param param) {
            super(param);
        }
        public void printMain(final Vrw32alp svf, final DB2UDB db2, final Map student) {
            final String schregno = getString("SCHREGNO", student);
            ...
            final Form form = new Form(_param, svf);
            form._form1 = !"".equals(_param._formD) ? _param._formD : "KNJE390_D_1.frm";
            ...
            printARecordShougaiNoGaiyou(main1, form);
        ...
        public void printStudent(final DB2UDB db2, final String schregno, final Map student, final Form form, final Map main1) {
            ...
            if (FORM_D1.equals(_param._formD)) {
                String[] get_token = KNJ_EditEdit.get_token(getString("CHALLENGED_NAMES", main1), 34, 4);
                if (null != get_token) {
                    List wkStr = Arrays.asList(get_token);  //昔の長い入力データが改行されて余計に出ないよう、制限。
                    svfVrListOutn(form, "DIAG_NAME", wkStr); // 障害名診断名
                }
                ...
                svfVrListOutn(form, "DIAG_NAME", getTokenList(getString("CHALLENGED_NAMES", main1), 30)); // 障害名診断名
                svfVrsOutWithSize(form, "ENTRY_PERSON", new String[] {"1", "2"}, new int[] {10, 16}, getString("UPDATE_STAFFNAME1", updateMap));  // 記入者
            }
        ...
        private void printARecordShougaiNoGaiyou(final Map main1, final Form form) {
            // 障害の概要
            ...
            dGaiyou.addCenter("DIVIDE5_1", asList(new String[] {"障害の", "概要"})); // 区分1
            ...
            dGaiyou.addAll("GRP5_15", "0"); // グループ1
            ...
            for (final Iterator it = check1List.iterator(); it.hasNext();) {
                ...
                for (int i = 1; i <= 2; i++) {
                    ...
                    if (i == 1) {
                        ...
                        d0.addAll("GRP6_2", recordSeq); // グループ4
        ...
        private void printARecordJittaiBunseki(final DB2UDB db2, final String schregno, final String recordDate, final Form form) {
            ...
            for (final Iterator it = KnjDbUtils.query(db2, " SELECT * FROM CHALLENGED_ASSESSMENT_STATUS_GROWUP_DAT WHERE YEAR = '" + _param._year + "' ").iterator(); it.hasNext();) {
                ...
                if ("0".equals(dataDiv)) {
                    statusName = KnjDbUtils.getString(row, "STATUS_NAME");
                    growupName = KnjDbUtils.getString(row, "GROWUP_NAME");
                } else {
                    dataDivMap.put(Integer.valueOf(dataDiv), KnjDbUtils.getString(row, "DATA_DIV_NAME"));
                }
            }
    ...
    private static class KNJE390E extends KNJE390_0 {

        public KNJE390E(final Param param) {
            super(param);
        }
    ...
    private static class KNJE390F extends KNJE390_0 {

        public KNJE390F(final Param param) {
            super(param);
        }
    ...
    private static class KNJE390G extends KNJE390_0 {

        public KNJE390G(final Param param) {
            super(param);
        }
    ...
    private static class KNJE390H extends KNJE390_0 {
        static final SimpleDateFormat df = new SimpleDateFormat("yyyy/M/d");

        public KNJE390H(final Param param) {
            super(param);
        }
    ...
    private static class Param {
        ...
        final String _tokushiShienPlanPatern;
        ...
        Param(final DB2UDB db2, final HttpServletRequest request) throws SQLException {
            ...
            _formA_1 = StringUtils.defaultString(request.getParameter("useFormNameE390_A_1"), "");
            _formA_2 = StringUtils.defaultString(request.getParameter("useFormNameE390_A_2"), "");
            _formB = StringUtils.defaultString(request.getParameter("useFormNameE390_B_1"), "");
            _formD = StringUtils.defaultString(request.getParameter("useFormNameE390_D_1"), "");
            ...
            _tokushiShienPlanPatern = request.getParameter("TokushiShienPlanPatern");
        }
