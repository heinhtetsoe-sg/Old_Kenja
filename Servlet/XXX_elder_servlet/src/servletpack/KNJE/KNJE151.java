// kanji=漢字
/*
 * $Id: 8065e6bb6a916629ccaf67b60ace95ba4d45a07d $
 *
 ...
 * <<クラスの説明>>。
 * @author nakamoto
 * @version $Id: 8065e6bb6a916629ccaf67b60ace95ba4d45a07d $
 */
public class KNJE151 {

    private static final Log log = LogFactory.getLog("KNJE151.class");
    ...
    private String getJviewStatSql() {
        ...
        stb.append("         CASE WHEN T2.ELECTDIV = '1' OR T3.NAME1 IS NOT NULL THEN ");
        stb.append("              CASE WHEN T1.VALUATION = 11 THEN 'A' ");
        stb.append("                   WHEN T1.VALUATION = 22 THEN 'B' ");
        stb.append("                   WHEN T1.VALUATION = 33 THEN 'C' ");
        stb.append("                   ELSE RTRIM(CHAR(T1.VALUATION)) ");
        stb.append("              END");
        stb.append("              ELSE RTRIM(CHAR(T1.VALUATION)) ");
        ...
        stb.append("         LEFT JOIN V_NAME_MST T3 ON T3.YEAR = T1.YEAR AND ");
        stb.append("                                    T3.NAMECD1 = 'D065' AND ");
        if ("1".equals(_param._useCurriculumcd)) {
            stb.append("                                T1.CLASSCD || '-' || T1.SCHOOL_KIND || '-' || T1.CURRICULUM_CD || '-' || ");
        }
        stb.append("                                    T1.SUBCLASSCD = T3.NAME1 ");
        stb.append("     WHERE ");
        stb.append("         T1.SCHOOLCD = '0' AND ");
        stb.append("         T1.YEAR = '" + _param._year + "' AND ");
        if ("1".equals(_param._useCurriculumcd)) {
            stb.append("         T1.CLASSCD || '-' || T1.SCHOOL_KIND || '-' || T1.CURRICULUM_CD || '-' || ");
        }
        stb.append("         T1.SUBCLASSCD = '" + _param._subclassCd + "' AND ");
    ...
    private class Param {
        ...
        private String getHrName(final DB2UDB db2) {
            ...
            String name = null;
            try {
                String sql = "SELECT HR_NAME FROM SCHREG_REGD_HDAT WHERE YEAR = '" + _year + "' AND SEMESTER = '" + _schregSemester + "' AND GRADE || HR_CLASS = '" + _gradeHrClass + "' ";
                ps = db2.prepareStatement(sql);
                rs = ps.executeQuery();
                while (rs.next()) {
