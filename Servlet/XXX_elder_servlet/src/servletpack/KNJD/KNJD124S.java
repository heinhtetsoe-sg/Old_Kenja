/**
 *
 *    学校教育システム 賢者 [成績管理] 成績点票（３学期制）
 *
 *    2008/02/26 o-naka 新規作成 KNJD122Tを土台に作成
 */
...
public class KNJD124S {

    private static final Log log = LogFactory.getLog(KNJD124S.class);
                     ...
                     throws ServletException, IOException
    {
        ...
        try {
            ...
            param[22] = request.getParameter("chikokuHyoujiFlg"); // プロパティ：遅刻数表示切替対応（1なら実遅刻数、それ以外なら換算後の余り）

            param[23] = request.getParameter("absentCov"); 		// 欠課換算区分
            param[24] = request.getParameter("absentCovLate"); 	// 欠課換算数
            param[25] = request.getParameter("amariKuriage"); 	// 余り繰り上げ数
    ...
    private void setHead(DB2UDB db2,Vrw32alp svf,String param[]) {
        ...
        final String form = "1".equals(param[21]) ? "KNJD124S_P.frm" : "KNJD124S.frm";
        ...
        try {
	        ...
	        log.fatal("semesdiv="+definecode.semesdiv + "   absent_cov="+param[23] + "   absent_cov_late="+param[24]+" amariKuriage"+param[25]);
        } catch( Exception ex ){
    ...
    private boolean printSvfOutMeisai( Vrw32alp svf, ResultSet rs, String param[], int total[][], Map attendmap ){
        ...
        try {
            ...
            printSvfOutDetal( svf, "POINT1_1",   rs.getString("SEM1_INTR_SCORE"), getTestAttend( rs.getString("SEM1_INTR_ATTEND"), attendmap ) );
            printSvfOutDetal( svf, "GRADING1_1", (rs.getString("SEM1_INTR_VALUE_DI") != null) ? rs.getString("SEM1_INTR_VALUE_DI") : rs.getString("SEM1_INTR_VALUE"), null );
            printSvfOutDetal( svf, "POINT1_2",   rs.getString("SEM1_TERM_SCORE"), getTestAttend( rs.getString("SEM1_TERM_ATTEND"), attendmap ) );
            printSvfOutDetal( svf, "GRADING1_2", (rs.getString("SEM1_TERM_VALUE_DI") != null) ? rs.getString("SEM1_TERM_VALUE_DI") : rs.getString("SEM1_TERM_VALUE"), null );
            printSvfOutDetal( svf, "AVERAGE1",   (rs.getString("SEM1_VALUE_DI") != null) ? rs.getString("SEM1_VALUE_DI") : rs.getString("SEM1_VALUE"),      null );

            printSvfOutDetal( svf, "POINT2_1",   rs.getString("SEM2_INTR_SCORE"), getTestAttend( rs.getString("SEM2_INTR_ATTEND"), attendmap ) );
            printSvfOutDetal( svf, "GRADING2_1", (rs.getString("SEM2_INTR_VALUE_DI") != null) ? rs.getString("SEM2_INTR_VALUE_DI") : rs.getString("SEM2_INTR_VALUE"), null );
            printSvfOutDetal( svf, "POINT2_2",   rs.getString("SEM2_TERM_SCORE"), getTestAttend( rs.getString("SEM2_TERM_ATTEND"), attendmap ) );
            printSvfOutDetal( svf, "GRADING2_2", (rs.getString("SEM2_TERM_VALUE_DI") != null) ? rs.getString("SEM2_TERM_VALUE_DI") : rs.getString("SEM2_TERM_VALUE"), null );
            printSvfOutDetal( svf, "AVERAGE2",   (rs.getString("SEM2_VALUE_DI") != null) ? rs.getString("SEM2_VALUE_DI") : rs.getString("SEM2_VALUE"),      null );

            printSvfOutDetal( svf, "POINT3_1",   rs.getString("SEM3_TERM_SCORE"), getTestAttend( rs.getString("SEM3_TERM_ATTEND"), attendmap ) );
            printSvfOutDetal( svf, "GRADING3_1", (rs.getString("SEM3_VALUE_DI") != null) ? rs.getString("SEM3_VALUE_DI") : rs.getString("SEM3_VALUE"), null );
            ...
            printSvfOutDetal( svf, "AVERAGE4",   (rs.getString("GRAD_VALUE_DI") != null) ? rs.getString("GRAD_VALUE_DI") : rs.getString("GRAD_VALUE"),      null );
            ...
            accumMeisai( total, i++, rs.getString("GRAD_VALUE_T") );
    ...
    String prestatementRecord(String param[],int pdiv) {
        ...
        try {
            ...
            stb.append(            "T2.SEM3_TERM_ATTEND, ");
            ...
            stb.append(            "SEM3_TERM_VALUE_DI, SEM3_VALUE_DI, ");
            ...
            //出欠集計データの表
			final String absentCov 		= param[23]; 
			final String absentCovLate 	= param[24];
			final String amariKuriage	= param[25];

            stb.append(",ATTEND_B AS(");
            if ("1".equals(absentCov) || "3".equals(absentCov)) {
                //学期でペナルティ欠課を算出する場合
                stb.append(    "SELECT  T1.SCHREGNO, ");
                ...
                if( "3".equals(absentCov) ){
                    //学期でペナルティ欠課を算出（小数点あり）
                    stb.append(                "VALUE(SUM(ABSENT1),0) + DECIMAL(FLOAT(VALUE(SUM(LATE_EARLY),0)) / " + absentCovLate + ",4,1) AS ABSENT2, ");
                } else{
                    //学期でペナルティ欠課を算出
                    stb.append(                "VALUE(SUM(ABSENT1),0) + VALUE(SUM(LATE_EARLY),0) / " + absentCovLate + " AS ABSENT2, ");
                }
                stb.append(                    "MOD(VALUE(SUM(LATE_EARLY),0) , " + absentCovLate + ") AS LATE_EARLY ");
            ...
            } else if ("5".equals(absentCov)) {
                //通年でペナルティ欠課を算出・・・例．余りを一捨二入
                stb.append(    "SELECT  T1.SCHREGNO, ");
                stb.append(            "VALUE(SUM(ABSENT1),0) AS ABSENT, ");
                stb.append(            "VALUE(SUM(ABSENT1),0) + VALUE(SUM(LATE_EARLY),0) / " + absentCovLate);
                stb.append(                   "+ (CASE WHEN MOD(VALUE(SUM(LATE_EARLY),0) , " + absentCovLate + ") >= " + amariKuriage + " THEN 1 ELSE 0 END) AS ABSENT2, ");
                stb.append(            "CASE WHEN MOD(VALUE(SUM(LATE_EARLY),0) , " + absentCovLate + ") >= " + amariKuriage + " THEN 0 ");
                stb.append(                 "ELSE MOD(VALUE(SUM(LATE_EARLY),0) , " + absentCovLate + ") END AS LATE_EARLY ");
                stb.append(           ",VALUE(SUM(LATE_EARLY),0) AS LATE_EARLY2 ");
                stb.append(    "FROM    ATTEND_A T1 ");
                stb.append(    "GROUP BY T1.SCHREGNO ");
            } else{
                ...
                stb.append(    "SELECT  T1.SCHREGNO, ");
                stb.append(            "VALUE(SUM(ABSENT1),0) AS ABSENT, ");
                if( "4".equals(absentCov) ){
                    //通年でペナルティ欠課を算出（小数点あり）
                    stb.append(        "VALUE(SUM(ABSENT1),0) + DECIMAL(FLOAT(VALUE(SUM(LATE_EARLY),0)) / " + absentCovLate + ",4,1) AS ABSENT2, ");
                    stb.append(        "MOD(VALUE(SUM(LATE_EARLY),0) , " + absentCovLate + ") AS LATE_EARLY ");
                } else if( "2".equals(absentCov) ){
                    //通年でペナルティ欠課を算出
                    stb.append(        "VALUE(SUM(ABSENT1),0) + VALUE(SUM(LATE_EARLY),0) / " + absentCovLate + " AS ABSENT2, ");
                    stb.append(        "MOD(VALUE(SUM(LATE_EARLY),0) , " + absentCovLate + ") AS LATE_EARLY ");
                } else{
            ...
            stb.append(        "SEM3_TERM_VALUE_DI, SEM3_VALUE_DI, ");
            if ("1".equals(param[21])) {
                stb.append(        "CASE WHEN P0.PROV_FLG = '1' THEN GRAD_VALUE ELSE NULL END AS PROV_FLG, ");   //仮評定
                stb.append(        "CASE WHEN P0.PROV_FLG = '1' THEN NULL ELSE GRAD_VALUE END AS GRAD_VALUE, "); //学年評定
            } else {
                stb.append(        "GRAD_VALUE, ");
            }
            stb.append(        "GRAD_VALUE AS GRAD_VALUE_T, "); //学年評定(合計用)
            stb.append(        "GRAD_VALUE_DI, ");
            stb.append(        "COMP_CREDIT, GET_CREDIT, ");
            ...
            if ("1".equals(param[22]) || definecode.schoolmark.substring( 0, 3 ).equals("KIN")) {
