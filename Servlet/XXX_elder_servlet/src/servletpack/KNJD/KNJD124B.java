 ...
 *
 *    2008/03/19 o-naka 新規作成 KNJD123Hを土台に作成
 */
...
public class KNJD124B {

    private static final Log log = LogFactory.getLog(KNJD124B.class);
                     ...
                     throws ServletException, IOException
    {
        ...
        try {
            ...
            param[20] = request.getParameter("useProvFlg"); // プロパティ：仮評定の追加対応
        } catch( Exception ex ) {
    ...
    private void setHead(DB2UDB db2,Vrw32alp svf,String param[]) {
        ...
        final String form = "1".equals(param[20]) ? "KNJD124B_P.frm" : "KNJD124B.frm";
        ...
        try {
            ...
            rs = db2.getResultSet();
            if (rs.next()) {
                ret = svf.VrsOut("CHAIRNAME",rs.getString("CHAIRNAME"));
            }
            rs.close();
        } catch( Exception ex ){
            log.error("setHead_ hrclass_staff error!", ex );
        ...
        try {
            ...
            ret = svf.VrsOut("A_DATE",param[9]+"年度"+param[10]+"月"+param[11]+ "日");
        } catch( Exception ex ){
    ...
    private boolean printSvfMain(DB2UDB db2,Vrw32alp svf,String param[]) {
        ...
        try {
            ...
            rs = db2.getResultSet();
            while (rs.next()) {
                if (printSvfOutMeisai(svf, rs, param, total, attendmap)) {
                    nonedata = true;
                    linex++;
                }
            }
            //明細行は５０行まで->足りない場合は空行を出力！
            if (linex < 50) {
                for ( ; linex < 50 ; linex++ ) {
    ...
    private boolean printSvfOutMeisai(Vrw32alp svf, ResultSet rs, String param[], int total[][], Map attendmap) {
        ...
        try {
            ...
            printSvfOutDetal( svf, "POINT3_1",   rs.getString("SEM3_INTR_SCORE"), getTestAttend( rs.getString("SEM3_INTR_ATTEND"), attendmap ) ); //３学期中間
            printSvfOutDetal( svf, "POINT3_2",   rs.getString("SEM3_TERM_SCORE"), getTestAttend( rs.getString("SEM3_TERM_ATTEND"), attendmap ) ); //３学期学年末
            printSvfOutDetal( svf, "AVERAGE3",   rs.getString("SEM3_VALUE"),      null );//３学期評価

            if ("1".equals(param[20])) {
                printSvfOutDetal( svf, "PROV_FLG",   rs.getString("PROV_FLG"), null);
            }
            printSvfOutDetal( svf, "ASSESS10",   rs.getString("GRAD_SCORE"),      null );//学年評定(10段階)
            ...
            if( rs.getString("LATE") != null  &&  0 < Integer.parseInt( rs.getString("LATE") ) )
                printSvfOutDetal( svf, "LATE", null, rs.getString("LATE") ); //遅刻数

            if( rs.getString("EARLY") != null  &&  0 < Integer.parseInt( rs.getString("EARLY") ) )
                printSvfOutDetal( svf, "EARLY", null, rs.getString("EARLY") ); //早退数
    ...
    private void printSvfOutTotal(Vrw32alp svf, int total[][]) {
        try {
            ...
            String fname[] = { "TOTAL_POINT1_1", "TOTAL_POINT1_2", "TOTAL_AVERAGE1",
                               ...
                               "TOTAL_POINT3_1", "TOTAL_POINT3_2", "TOTAL_AVERAGE3",
    ...
    String prestatementRecord(String param[],int pdiv) {
        ...
        try {
            ...
            stb.append(    "GROUP BY SCHREGNO,SEMESTER ");
            stb.append(       ") ");
