/*
 * $Id: b0960f3419b232b12e9576643a7c3cbb29b26ad5 $
 *
 * 作成日: 2015/08/04
 * 作成者: maesiro
 *
 ...
 * 洛南高校付属中学校　中学通知票
 */
public class KNJD185R {

    private static final Log log = LogFactory.getLog(KNJD185R.class);
    ...
    private void printMain(final DB2UDB db2, final Vrw32alp svf) {
        ...
        for (final Student student : Student.getStudentList(db2, _param)) {
            
            log.info(" schregno = " + student._schregno);
            
            if (_param._isHyousiPrint) {
                svf.VrSetForm("KNJD185R_1.frm", 1);
                printHyoshi(svf, student);
                svf.VrEndPage();
            }
            
            svf.VrSetForm("KNJD185R_2.frm", 4);
    ...
    private void printMain(final Vrw32alp svf, final Student student) {
        ...
        // 道徳欄
        if ("J".equals(_param._schoolKind) && ("3".equals(_param._semester) || SEMEALL.equals(_param._semester))) {
            final String doutoku = StringUtils.defaultString(student._hreportremarkDatSeme3Remark2);
            final ShokenSize doutokuSize = ShokenSize.getShokenSize(null, 21, 8);
            final List<String> doutokuToken = getTokenList(doutoku, doutokuSize._mojisu * 2);
            for (int i = 0; i < Math.min(doutokuToken.size(), doutokuSize._gyo); i++) {
                svf.VrsOutn("DOUTOKU", i + 1, doutokuToken.get(i));
            }
        } else {
        	if (null != _param._whitespaceImagePath) {
        		svf.VrsOut("WHITESPACE", _param._whitespaceImagePath);
        	}
        }
        ...
        for (int semesi = 0; semesi < _param.getTargetSemes().length; semesi++) {
            ...
            svf.VrsOutn("TOTALACT", semesline, student.getClassKekka(_param, _param._year, semeAllIf3, "90")); // 総合活動
            svf.VrsOutn("SPECIALACT", semesline, student.getClassKekka(_param, _param._year, semeAllIf3, "94")); // 特別活動
            ...
            final ShokenSize comSize = ShokenSize.getShokenSize(_param._HREPORTREMARK_DAT_COMMUNICATION_SIZE_J, 17, 4);
        ...
        for (int cli = 0; cli < classCdGroupList.size(); cli++) {
            ...
            for (int subi = 0; subi < subclassList.size(); subi++) {
                ...
                final boolean isDoutoku = "50-J".equals(subclass._classkey);
                for (int tii = 0; tii < _param._testitemList.size(); tii++) {
                    final Testitem testitem = _param._testitemList.get(tii);

                    if (isDoutoku) {
                    	if (null != _param._slashBsImagePath) {
                    		svf.VrsOut("SLASH" + String.valueOf(tii + 1), _param._slashBsImagePath); // スラッシュ
                    	}
                    }
...
//            _requireFlg = requireFlg;
    ...
    private static class Student {
        ...
        String _hreportremarkDatSeme3Remark2;
        List _regdList = null;
        ...
        public String getClassKekka(final Param param, final String year, final String semester, final String classcd) {
            BigDecimal rtn = null;
            for (final Iterator it = _attendSubclassMap.keySet().iterator(); it.hasNext();) {
                final String subclasscd = (String) it.next();
                if (!subclasscd.startsWith(classcd)) {
                    continue;
                }
                final BigDecimal bd = (BigDecimal) getMappedMap(_attendSubclassMap, subclasscd).get(year + "-" + semester);
                if (null == bd) {
                    continue;
                }
                rtn = null == rtn ? bd : rtn.add(bd);
            }
            if (null == rtn) {
                return "";
            }
        ...
        public static void setHreportremarkCommunication(final Param param, final DB2UDB db2, final List studentList) {
            final StringBuffer stb = new StringBuffer();
            stb.append(" SELECT SEMESTER, COMMUNICATION, REMARK2 ");
            ...
            try {
                ...
                for (final Iterator it = studentList.iterator(); it.hasNext();) {
                    ...
                    while (rs.next()) {
                        ...
                        if ("3".equals(rs.getString("SEMESTER"))) {
                            student._hreportremarkDatSeme3Remark2 = rs.getString("REMARK2");
                        }
                    }
...
//            stb.append("   CRE.REQUIRE_FLG, ");
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    if (null == studentMap.get(schregno)) {
                        ...
                        final Student student = new Student(grade, hrClass, hrName, hrClassName1, staffname, attendno, schregno, name, realName, useRealName);
            ...
            final Map mThisYear = new HashMap(param._attendParamMap);
            loadAttendance(db2, param, param._year, param._semester, param._date, studentMap, mThisYear);
            if (SEMEALL.equals(param._semester) && param._is3nen) {
                ...
                for (final Iterator it = regdMap.entrySet().iterator(); it.hasNext();) {
                    ...
                    final Map mYear = new HashMap(param._attendParamMap);
                    loadAttendance(db2, param, split[0], "9", date, studentMap, mYear);
                }
            }
        ...
        public boolean isPass(final Param param) {
            boolean rtn = true;
            final Map subclassHyoteiMap = getMappedMap(_scoreMap, "9990008");
            if (subclassHyoteiMap.isEmpty()) {
                log.info(" gakunen hyoka empty:" + _schregno);
            }
    ...
    private static class Testitem {
        ...
        public static List getTestitemList(final DB2UDB db2, final Param param) {
            ...
            final String[] cds = {"1990008", "2990008", "9990008"};
    ...
    private static class Param {
        ...
        final String _slashBsImagePath;
        final int _kChikoku;
...
//        private String _certifSchoolPrincipalName;
        ...
        Param(final DB2UDB db2, final HttpServletRequest request) throws SQLException {
            ...
            _schoolLogoImagePath = getImageFilePath("SCHOOL_LOGO_J." + _extension);
            _whitespaceImagePath = getImageFilePath("whitespace.png");
            _slashBsImagePath = getImageFilePath("slash_bs.jpg");
