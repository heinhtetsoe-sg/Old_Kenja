// kanji=漢字
/*
 * $Id: aa2446dbd58fbb1e03b4f279c4d0bfda27db4a4c $
 *
 * 作成日: 2007/05/14
 * 作成者: nakamoto
 *
 ...
 * 
 * @author nakamoto
 * @version $Id: aa2446dbd58fbb1e03b4f279c4d0bfda27db4a4c $
 */
public class KNJD654L {
    private static final Log log = LogFactory.getLog(KNJD654L.class);

    private final String FORM_FILE = "KNJD654A.frm";
//    private final String FORM_FILE2 = "KNJD654A_2.frm";
    private final String FORM_FILE3 = "KNJD654A_3.frm";
    private final String FORM_FILE4 = "KNJD654A_4.frm";
    private final String FORM_FILE5 = "KNJD654A_5.frm";
...
//            }
    ...
    private boolean svfPrintMain(final Vrw32alp svf, final DB2UDB db2, final String grade) {
        ...
        if (_param._testKindCd.startsWith(TESTCD9900)) {
            form1 = FORM_FILE4;
            form2 = FORM_FILE5;
        } else {
            form1 = FORM_FILE;
        ...
        for (final Iterator itc = courseList.iterator(); itc.hasNext();) {
            ...
            if (!hrAverage.isEmpty() || !subclassDistribution.isEmpty()) {
                ...
                hasData = printGroupAverage(svf, printHomerooms, course, null, avgSubclasses, hrAverage);
                // log.debug("== 科目別得点分布表 ==");
                ...
                for (final Iterator it = pageSubclassListMap.keySet().iterator(); it.hasNext();) {
                    ...
                    if (SCHOOL_KIND_J.equals(_param._schoolKind) && 1 != page.intValue()) {
                        break;
                    }
                    if (1 != page.intValue()) {
                    ...
                    hasData = printSubclassDistribution(svf, printHomerooms, subclassList, subclassDistribution, _param._schoolKind, 1 == page.intValue()) || hasData;
                    if (hasData) {
                        svf.VrEndPage();
                    }
                }
    ...
    private Map splitSubclass(final List subclasses, final String schoolKind) {
        ...
        if (SCHOOL_KIND_J.equals(schoolKind)) {
            final int max = 6;
...
//        boolean hasData = false;
//        
//        final String schoolKind = "99".equals(paramGrade) ? "H" : "98".equals(paramGrade) ? SCHOOL_KIND_J : null;
...
//            _param.load(db2, grade);
//            
//            final List printGroups = PrintGroup.getPrintGroups(db2, _param, grade);
//            final List avgSubclasses = SubClass.getSubclasses(db2, _param, grade, true); // 平均点表示用科目
//            final Map hrAverage = AverageDat.loadHrAverage(db2, _param, grade, avgSubclasses);
//            
//            if (!hrAverage.isEmpty()) {
//                printHeader(svf, "GRADE" + String.valueOf(gradeCount + 1));
//                // log.debug("== クラス別平均点一覧表 ==");
//                hasData = printGroupAverage(svf, printGroups, String.valueOf(gradeCount + 1), avgSubclasses, hrAverage);
//                gradeCount += 1;
...
//    }
     ...
     */
    private boolean printGroupAverage(final Vrw32alp svf, final List printGroups, final Course course, final String i, final List subClasses, final Map hrAverage) {
        int hrc = 0;
        boolean hasData = false;
        ...
        final int maxLine = null == i ? 11 : 11;
     ...
     */
    private boolean printSubclassDistribution(final Vrw32alp svf, final List printGroups, final List subclasses, final Map subclassDistribution, final String schoolKind, final boolean isForm1) {
        // クラス別平均点
        final String IDX_SUBC_ALL3;
        final String IDX_SUBC_ALL5;
        final int maxHrColumn = 11;
        final int maxScoreLine = _param._testKindCd.startsWith(TESTCD9900) ? 6 : 11;
        ...
        for (final Iterator itsub = subclasses.iterator(); itsub.hasNext();) {
            ...
            if (SUBCLASSCD_ALL9.equals(subclass._code)) {
                sc = null;
            } else if (SCHOOL_KIND_J.equals(schoolKind)) {
                if (SUBCLASSCD_ALL3.equals(subclass._code)) {
                    sc = IDX_SUBC_ALL3;
                } else if (SUBCLASSCD_ALL5.equals(subclass._code)) {
                    sc = IDX_SUBC_ALL5;
    ...
    private static class AverageDat {
        ...
        private static String sqlHrAverage(final Param param, final String grade) {
            ...
            stb.append("   AND TESTKINDCD || TESTITEMCD || SCORE_DIV = '" + param._testKindCd + "' ");
...
//                    final String coursecode = "2".equals(param._group) ? rs.getString("COURSECODE") : "0000";
    ...
    private static class Param {
        ...
        Param(final HttpServletRequest request) throws ServletException {
            ...
            if (_testKindCd.startsWith(TESTCD9900)) {
        ...
        private void setTestName(final DB2UDB db2) {
            ...
            ResultSet rs = null;
            try {
                final String sql = "SELECT TESTITEMNAME " + "FROM TESTITEM_MST_COUNTFLG_NEW_SDIV " + "WHERE YEAR = '" + _ctrlYear + "' " + "AND SEMESTER = '" + _semester + "' "
                        + "AND TESTKINDCD || TESTITEMCD || SCORE_DIV = '" + _testKindCd + "' ";
                ps = db2.prepareStatement(sql);
                rs = ps.executeQuery();
                if (rs.next()) {
