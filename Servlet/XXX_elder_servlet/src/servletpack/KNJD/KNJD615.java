// kanji=漢字
/*
 * $Id: 99064ac1cd73b5ba4d6517a505d0cf6cf0fdca54 $
 *
 * 作成日: 2007/05/14
 * 作成者: nakamoto
 *
 ...
 * @author nakamoto
 * @version $Id: 99064ac1cd73b5ba4d6517a505d0cf6cf0fdca54 $
 */
public class KNJD615 {
    private static final Log log = LogFactory.getLog(KNJD615.class);
    ...
    private int printSubclasses(
        ...
        for (final Iterator sit = stulist.iterator(); sit.hasNext();) {
            final Student student = (Student) sit.next();
            printScoreDetail(student, svf, subclass, line);
        }        
        svf.VrEndRecord();
        return 1;
    }
            ...
            if (knjdobj.isPrintScore()) {
                ...
                if (null != detail._score) {
                    if (detail.isFail(detail._score.getScoreAsInt())) {
                        svf.VrAttribute("SCORE"+ student._gnum,  "Paint=(1,50,1),Bold=1");
                    }
                    svf.VrsOut("SCORE"+ student._gnum,  detail._score.getScore() );
                    if (detail.isFail(detail._score.getScoreAsInt())) {
                        svf.VrAttribute("SCORE"+ student._gnum,  "Paint=(0,0,0),Bold=0");
                    }
                }
            } else {
                // 成績
                if (null != detail._patternAssess) {
                    ...
                    svf.VrsOut("SCORE"+ student._gnum,  detail._patternAssess.getScore() );
                    if (detail.isFail(detail._patternAssess.getScoreAsInt())) {
                        svf.VrAttribute("SCORE"+ student._gnum,  "Paint=(0,0,0),Bold=0");
                    }
...
//                    }
            ...
            if (null != chkField) {
                ...
                if (0 != value) {
                    if (detail._isOver) {
                        svf.VrAttribute(field+ student._gnum,  "Paint=(1,70,1),Bold=1");
                    }
                    svf.VrsOut(field+ student._gnum,  _knjdobj.getAbsentFmt().format(chkField.floatValue()));
                    if (detail._isOver) {
                        svf.VrAttribute(field+ student._gnum,  "Paint=(0,0,0),Bold=0");
                    }
                }
            }
        }
    ...
    private void printOnLastpage(
        ...
        if (null != total) {
            svf.doNumberingSvfOut("TOTAL", student._gnum, total);  //総合点
            svf.doNumberingSvfOut("AVERAGE", student._gnum, average);  //平均点
        }
        //順位（学級）
        final int classRank = student._total._student._classRank;
        if (1 <= classRank) {
            svf.doNumberingSvfOut("CLASS_RANK", student._gnum, String.valueOf(classRank));
        }
        //順位（学年orコース）
        final int rank = student._total._student._rank;
        if (1 <= rank) {
            svf.doNumberingSvfOut("RANK", student._gnum, String.valueOf(rank));
        }
        //欠点科目数
...
//                _testName = "期末２";
    ...
    private class HRInfo implements Comparable {
        ...
        private void loadScoreDetail(
            ...
            try {
                ps = db2.prepareStatement(sqlStdSubclassDetail(_hrclassCd));
                rs = ps.executeQuery();
...
//                        countA += totalObj._avgcount;
    ...
    private class Student implements Comparable {
        ...
        boolean isAttendPerfect() {
            ...
            for (final Iterator itD = _scoreDetails.values().iterator(); itD.hasNext();) {
                final ScoreDetail detail = (ScoreDetail) itD.next();
                final Double kekka = detail._isCombinedSubclass ? detail._replacedSick : detail._absent;
...
//        }
    ...
    private class ScoreDetail {
        ...
        private boolean _isCombinedSubclass;
        private Double _absent;
        private Double _replacedSick;
        ...
        public String getFormFile() {
            if (isKumamoto() || isChiben()) {
                if ("1".equals(_formSelect)) {
                    return "1".equals(_subclassMax) ? "KNJD615_2.frm" : "KNJD615_4.frm";
                } else {
                    return "1".equals(_subclassMax) ? "KNJD615_1.frm" : "KNJD615_3.frm";
                }
            } else {
                if ("1".equals(_formSelect)) {
