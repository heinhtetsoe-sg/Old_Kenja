// kanji=漢字
/*
 * $Id: a6bfc805cd9d94de06c81cb00a7ebefd5a2582e3 $
 *
 * 作成日: 2007/05/14
 * 作成者: nakamoto
 *
 ...
 * @author nakamoto
 * @version $Id: a6bfc805cd9d94de06c81cb00a7ebefd5a2582e3 $
 */
public class KNJD615D {
    private static final Log log = LogFactory.getLog(KNJD615D.class);
    ...
    private static KNJD065_COMMON createKnjd065Obj(
        ...
        log.debug(" testKindCd = " + testKindCd);
...
//        if (!param.isChiben9909() && "9900".equals(testKindCd) && param.isChiben()) { return new KNJD062A_GRADE_HYOKA(db2, param); }
        ...
        if ("9901".equals(testKindCd)) { return new KNJD062A_GRADE_HYOKA(db2, param); }
    ...
    private boolean printSub(
        ...
        for (final Iterator it = subClasses.iterator(); it.hasNext();) {
            ...
            final int used = printSubclasses(svf, hrInfo, subclass, line, stulist);
            line += used;
    ...
    private int printSubclasses(
        ...
        printStudentsSubclasses(svf, hrInfo, subclass, line, stulist);  // 生徒別該当科目成績を印字する処理
        svf.VrEndRecord();
        return 1;
    }
...
//                _testName = "期末２";
    ...
    private class HRInfo implements Comparable {
        ...
        private Map _courseAttendSubclassCdMap;
        ...
        void load(
            ...
            _courseAttendSubclassCdMap = getCourseAttendSubclassCdMap(db2, courseCd);
        }
        
        private List getAttendSubclassList(final String course) {
            if (null == _courseAttendSubclassCdMap.get(course)) {
                return Collections.EMPTY_LIST;
            }
            return (List) _courseAttendSubclassCdMap.get(course);
        }
        ...
        private boolean isAttendSubclassCd(final SubClass subclass) {
            ...
            for (final Iterator itc = courses.iterator(); itc.hasNext();) {
                ...
                List attendSubclassCdList = (List) _courseAttendSubclassCdMap.get(course);
        ...
        private String sqlStdNameInfo(final String hrClass, final String courseCd) {
            ...
            stb.append("INNER  JOIN SCHREG_BASE_MST W3 ON W3.SCHREGNO = W1.SCHREGNO ");
            stb.append("LEFT   JOIN SCHREG_BASE_MST W4 ON W4.SCHREGNO = W1.SCHREGNO ");
            stb.append(                              "AND ((W4.GRD_DIV NOT IN ('1', '4') AND W4.GRD_DATE < W2.EDATE) ");
            stb.append(                                "OR (W4.ENT_DIV NOT IN('1', '2', '3') AND W4.ENT_DATE > W2.SDATE)) ");
            stb.append("LEFT   JOIN SCHREG_TRANSFER_DAT W5 ON W5.SCHREGNO = W1.SCHREGNO ");
        ...
        private void loadScoreDetail(
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    if (classCd.compareTo(KNJDefineSchool.subject_U) <= 0 || classCd.equals(KNJDefineSchool.subject_T) || _param.isNishiyama() && classCd.equals("94")) {
...
//                }
        ...
        private String sqlStdPreviousCredits() {
            ...
            stb.append("    AND ((T1.SCHOOLCD = '0' AND (CLASSCD BETWEEN '" + KNJDefineSchool.subject_D + "' AND '" + KNJDefineSchool.subject_U + "' OR CLASSCD = '" + KNJDefineSchool.subject_T + "' " + (_param.isNishiyama() ? " OR CLASSCD = '94' " : "") + "))");
...
//                        countA += totalObj._avgcount;
        ...
        void print(
            ...
            //学級平均
            if (null != _avgHrTotal) {
                svf.VrsOut("TOTAL" + String.valueOf(_param._formMaxLine + 2), _avgHrTotal);
            }
            if (null != _avgHrAverage) {
                svf.VrsOut("AVERAGE" + String.valueOf(_param._formMaxLine + 2), _avgHrAverage);
            }
            //欠点者数
            if (null != _failHrTotal) {
                svf.VrsOut("TOTAL" + String.valueOf(_param._formMaxLine + 4), _failHrTotal);
            }
            //最高点
            if (null != _maxHrTotal) {
                svf.VrsOut("TOTAL" + String.valueOf(_param._formMaxLine + 5), _maxHrTotal);
            }
            //最低点
            if (null != _minHrTotal) {
                svf.VrsOut("TOTAL" + String.valueOf(_param._formMaxLine + 6), _minHrTotal);
            }
            //出席率・欠席
            if (null != _perHrPresent) {
                svf.VrsOut("PER_ATTEND", DEC_FMT1.format(_perHrPresent.setScale(1,BigDecimal.ROUND_HALF_UP)));
            }
            if (null != _perHrAbsent) {
                svf.VrsOut("PER_ABSENCE", DEC_FMT1.format(_perHrAbsent.setScale(1,BigDecimal.ROUND_HALF_UP)));
            }
        }
    }
    ...
    private class Student implements Comparable {
        ...
        void printOnPage(
            ...
            svf.doNumberingSvfOut("name", _gnum, _name);    // 氏名
            ...
            if (_knjdobj.hasJudgementItem()) { printMark(svf); }
        }
        ...
        void printOnLastpage(
            ...
            if (_knjdobj.hasCompCredit()) {
                //今年度履修単位数
                svf.doNumberingSvfOutNonZero("COMP_CREDIT", _gnum, String.valueOf(_compCredit));
                //今年度修得単位数
                svf.doNumberingSvfOutNonZero("GET_CREDIT", _gnum, String.valueOf(_getCredit));
            }
            if (_knjdobj.hasJudgementItem()) {
                printOtherCredits(svf);  // 前年度までの単位数を印字
            }
            if (0 < _specialAbsent) {
                svf.doNumberingSvfOutNonZero("SP_KEKKA", _gnum, String.valueOf(_specialAbsent));
            }
        }
...
//        }
        ...
        void print(
            ...
            if (log.isDebugEnabled()) { log.debug("subclassname="+_subclasscode + " " + _subclassabbv+"   line="+line + "  i="+i); }
            //教科名
            svf.VrsOut("course1", _classabbv);
            //科目名
            if (_electdiv) svf.VrAttribute("subject1", "Paint=(2,70,2),Bold=1");
            svf.VrsOut("subject1", _subclassabbv);
            if (_electdiv) svf.VrAttribute("subject1", "Paint=(0,0,0),Bold=0");
            //単位数
            if (0 != _maxcredit) {
                if (_maxcredit == _mincredit) { svf.VrsOut("credit1",  String.valueOf(_maxcredit)); }
                else  { svf.VrsOut("credit1",  String.valueOf(_mincredit) + " \uFF5E " + String.valueOf(_maxcredit)); }
            }
            //授業時数
            if (0 != _jisu) { svf.VrsOut("lesson1",  String.valueOf(_jisu)); }
            //学級平均・合計
            if (null != _scoreaverage) { svf.VrsOut("AVE_CLASS",  _scoreaverage); }
            if (null != _scoresubaverage) { svf.VrsOut("AVE_SUBCLASS",  _scoresubaverage); }
            if (null != _scoretotal) { svf.VrsOut("TOTAL_SUBCLASS",  _scoretotal + "/" + _scoreCount); }
            if (null != _scoreMax) { svf.VrsOut("MAX_SCORE",  _scoreMax); }
            if (null != _scoreMin) { svf.VrsOut("MIN_SCORE",  _scoreMin); }
            if (null != _scoreFailCnt) { svf.VrsOut("FAIL_STD",  _scoreFailCnt); }
            //項目名
...
//            int column = line % MAX_COLUMN + 1;
            ...
            if (knjdobj.isPrintScore()) {
                // 素点
                if (null != _score) {
                    if (isFail(_score.getScoreAsInt())) {
                ...
                if (null != _patternAssess) {
                    if (isFail(_patternAssess.getScoreAsInt())) {
                        svf.VrAttribute("SCORE"+ gnum,  "Paint=(1,50,1),Bold=1");
                    }
...
//                    }
            ...
            if (null != _absent) {
                ...
                if (0 != value) {
                    ...
                    svf.VrsOut(field+ gnum,  _knjdobj.getAbsentFmt().format(_absent.floatValue()));
                    if (_isOver) {
    ...
    private class Total implements Comparable {
        ...
        void print(
            ...
            final String total = _student._scoreSum;
            final String average = _student._scoreAvg;
            if (null != total) {
                svf.doNumberingSvfOut("TOTAL", gnum, total);  //総合点
                svf.doNumberingSvfOut("AVERAGE", gnum, average);  //平均点
            }
            //順位（学級）
            final int classRank = _student._classRank;
            if (1 <= classRank) {
                svf.doNumberingSvfOut("CLASS_RANK", gnum, String.valueOf(classRank));
            }
            //順位（学年orコース）
            final int rank = _student._rank;
            if (1 <= rank) {
                svf.doNumberingSvfOut("RANK", gnum, String.valueOf(rank));
            }
            //欠点科目数
            if (0 < _countFail) {
                svf.doNumberingSvfOut("FAIL", gnum, String.valueOf(_countFail));
            }
        }
    ...
    private static class Param {
        ...
        public String getFormFile() {
            if ("1".equals(_formSelect)) {
                return "1".equals(_subclassMax) ? "KNJD615D_2.frm" : "KNJD615D_4.frm";
            } else {
                return "1".equals(_subclassMax) ? "KNJD615D_1.frm" : "KNJD615D_3.frm";
            }
        }
...
//        }
        ...
        public boolean isNishiyama() {
            return "nishiyama".equals(_schoolName);
        }
