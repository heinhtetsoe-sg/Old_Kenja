...
public class KNJD184N {

    private static final Log log = LogFactory.getLog(KNJD184N.class);

    private static final String[] MONTH_ARRAY = {"04", "05", "06", "07", "08", "09", "10", "11", "12", "01", "02", "03"};
    private static final String SUCLASS_999999 = "99-H-99-999999";
    ...
    private void printMain(final DB2UDB db2, final Vrw32alp svf) {
        ...
        setSubclassAttendance(db2, studentList);  // 欠課時数
    ...
    private void setCover(final Vrw32alp svf, final Student student) {
        svf.VrSetForm("KNJD184N_1.frm", 1);

        // 年度
        svf.VrsOut("NENDO", _param._ctrlYear + "年度");

        // 校種
        svf.VrsOut("SCHOOL_KIND", "高等部");
        ...
        // 年組番
        svf.VrsOut("HR_ATTTENDNO", student._gradeCdHrclassnameAttendno);
        ...
        // 校長
        final String principal = trim(_param._certifSchoolDat._principalName);
        final String priIdx = KNJ_EditEdit.getMS932ByteLength(principal) > 32 ? "_3": KNJ_EditEdit.getMS932ByteLength(principal) > 26 ? "_2": "";
        svf.VrsOut("PRINCIPAL_NAME" + priIdx, principal);
        ...
        // 学校名
        svf.VrsOut("SCHOOLNAME", trim(_param._certifSchoolDat._schoolName));
    ...
    private void setCompletion(final Vrw32alp svf, final Student student) {
        svf.VrSetForm("KNJD184N_4.frm", 1);
        ...
        // 学年
        svf.VrsOut("GRADE", "高等部第" + String.valueOf(Integer.parseInt(student._gradeCd)) + "学年");
    ...
    private void setData(final Vrw32alp svf, final Student student) {
        svf.VrSetForm("KNJD184N_2.frm", 4);
        ...
        svf.VrsOut("HR_ATTENDNO", student._gradeCdHrclassnameAttendno); // 年組番
        ...
        int listCnt = 1;
        String befClassCd = "";
        final String[] notPrintCd = {"333333", "555555", "999999"};
        for (Iterator it2 = student._rankMap.keySet().iterator(); it2.hasNext();) {
            String subclassCd = (String) it2.next();
            final Subclass sub = (Subclass) _param._subClassMst.get(subclassCd);

            final String classCd = StringUtils.substring(subclassCd, 0, 2);
            final String onlySubclasscd = StringUtils.split(subclassCd, "-")[3];

            if (Arrays.asList(notPrintCd).contains(onlySubclasscd) || onlySubclasscd.startsWith("99999")) {
                continue;
            }
            ...
            //合併科目なら元と先を印字する
            if (_param._subClassRepMap.containsKey(subclassCd)) {
                final String sakiCd = (String) _param._subClassRepMap.get(subclassCd);
                final Subclass sakiSub = (Subclass) _param._subClassMst.get(sakiCd);
                final String subIdxSaki   = KNJ_EditEdit.getMS932ByteLength(sakiSub._subClassName) > 30 ? "_3": KNJ_EditEdit.getMS932ByteLength(sakiSub._subClassName) > 20 ? "_2": "";
                final String subIdxMoto   = KNJ_EditEdit.getMS932ByteLength(sub._subClassName) > 16 ? "_3": KNJ_EditEdit.getMS932ByteLength(sub._subClassName) > 10 ? "_2": "";
                svf.VrsOut("SUBCLASSNAME" + subIdxSaki, sakiSub._subClassName); // 科目名称(合併先)
                svf.VrsOut("SUBCLASSNAME2" + subIdxMoto, sub._subClassName);    // 科目名称(合併元)
            } else {
    ...
    private void setData99(final Vrw32alp svf, final Student student) {
        svf.VrSetForm("KNJD184N_3.frm", 4);
        ...
        //所見
        final List<String> communication = KNJ_EditKinsoku.getTokenList(student._communication, 26, 4);
        if (communication != null) {
            for (int i = 0; i < communication.size(); i++) {
                String commStr = communication.get(i);
        ...
        final List<String> attRemark = KNJ_EditKinsoku.getTokenList(student._attendrecRemark, 20, attRow);
        if (attRemark != null) {
            for (int i = 0; i < attRemark.size(); i++) {
                String attendrecRemark = attRemark.get(i);
        ...
        //評価
        final List<String> remark4s = KNJ_EditKinsoku.getTokenList(student._remark4, 30, 5);
        if (remark4s != null) {
            for (int i = 0; i < remark4s.size(); i++) {
                String remark4 = remark4s.get(i);
        ...
        final Rank rank9 = (Rank) student._rankMap.get(SUCLASS_999999);
        if (null != rank9) {
            svf.VrsOut("TOTALAVG_VALUE", rank9._avg);       // 点数
            svf.VrsOut("TOTALAVG_RANK", rank9._courseRank); // 順位
        }
        final String cmcSubCd99 = student._cmcCd + SUCLASS_999999;
        final CourseAvg courseAvg9 = (CourseAvg) _param._courseAvgMap.get(cmcSubCd99);
        ...
        final List creditList = new ArrayList();
        final List kekkaList = new ArrayList();
        ...
        for (Iterator it2 = student._rankMap.keySet().iterator(); it2.hasNext();) {
            ...
            final Rank rank = (Rank) student._rankMap.get(subclassCd);
            if (null != rank) {
                svf.VrsOut("VALUE", rank._score);     // 評価
                svf.VrsOut("RANK", rank._courseRank); // 順位
                svf.VrsOut("PROV", rank._hyoutei);    // 評定、仮評定
                svf.VrsOut("CREDIT", rank._credits);  // 単位
                final String remarkIdx   = KNJ_EditEdit.getMS932ByteLength(rank._remark) > 18 ? "_3": KNJ_EditEdit.getMS932ByteLength(rank._remark) > 14 ? "_2": "";
                svf.VrsOut("REMARK" + remarkIdx, rank._remark); // 備考
                hyoteiList.add(rank._hyoutei);
                creditList.add(rank._credits);
            }
            ...
            String kekka = "0";
            if (sa != null) {
                String rawSick = (sa._isCombinedSubclass) ? formatBigDecimal(sa._rawReplacedSick) : formatBigDecimal(sa._rawSick);
                if (rawSick == null) {
                    rawSick = "";
                } else if (NumberUtils.isNumber(rawSick)) {
                    rawSick = String.valueOf(new BigDecimal(rawSick).intValue());
                }
                kekka = StringUtils.defaultString(rawSick, "0");
                if ("".equals(kekka)) kekka = "0";
                kekkaList.add(kekka);
            }
            svf.VrsOut("KEKKA", kekka);
        ...
        svf.VrsOut("TOTAL_PROV", average(hyoteiList)); // 評定（合計）
        svf.VrsOut("TOTAL_CREDIT", sum(creditList));// 単位（合計）
        svf.VrsOut("TOTAL_KEKKA", sum(kekkaList));  // 欠課（合計）
    ...
    private String average(final List intList) {
        int sum = 0;
        int count = 0;
        for (final Iterator it = intList.iterator(); it.hasNext();) {
        ...
        return count > 0 ? new BigDecimal(sum).divide(new BigDecimal(count), 1, BigDecimal.ROUND_HALF_UP).toString() : null;
    }

    private String sum(final List intList) {
        int sum = 0;
    ...
    private List getSchList(final DB2UDB db2) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                student._gradeCdHrclassnameAttendno = gradeCdString + " 年 " + StringUtils.defaultString(student._hrClassName1) + " 組 " + String.valueOf(Integer.parseInt(student._attendNo)) + " 番";
                retList.add(student);
            }
                    ...
                    if (_param._testCd99) {
                        ...
                        if (_param._subClassRepMap.containsKey(subclassCd)) {
                            if (_param._isOutputDebug) {
                                log.info(" 99 not print " + subclassCd);
                            }
                            continue;
                        }
                    } else {
                        // 学期末以外は合併先科目を印字しない
                        if (_param._subClassRepMap.values().contains(subclassCd)) {
                            if (_param._isOutputDebug) {
                                log.info(" not print " + subclassCd);
                            }
                            continue;
                        }
                    }

                    student._rankMap.put(subclassCd, new Rank(score, hyoutei, avg, rank, credits, remark));
    ...
    private String getSchRankSql() {
        ...
        stb.append("     RANK9.SCORE as HYOUTEI, ");
        stb.append("     RANK.AVG, ");
        ...
        stb.append("     LEFT JOIN RECORD_SCORE_DAT RANK9 ON RANK9.YEAR          = RANK.YEAR ");
        stb.append("                                         AND RANK9.SEMESTER      = RANK.SEMESTER ");
        stb.append("                                         AND RANK9.TESTKINDCD    = '99' ");
        stb.append("                                         AND RANK9.TESTITEMCD    = '00' ");
        stb.append("                                         AND RANK9.SCORE_DIV     = '09' ");
        stb.append("                                         AND RANK9.CLASSCD       = RANK.CLASSCD ");
        stb.append("                                         AND RANK9.SCHOOL_KIND   = RANK.SCHOOL_KIND ");
        stb.append("                                         AND RANK9.CURRICULUM_CD = RANK.CURRICULUM_CD ");
        stb.append("                                         AND RANK9.SUBCLASSCD    = RANK.SUBCLASSCD ");
        stb.append("                                         AND RANK9.SCHREGNO      = RANK.SCHREGNO ");
        stb.append("     LEFT JOIN SCHREG_REGD_DAT REGD ON REGD.SCHREGNO = RANK.SCHREGNO ");
        stb.append("                                   AND REGD.YEAR     = RANK.YEAR ");
        stb.append("                                   AND REGD.SEMESTER = '" + _param._semester2 + "' ");
        stb.append("     LEFT JOIN CREDIT_MST CREDIT ON CREDIT.YEAR          = REGD.YEAR ");
        stb.append("                                AND CREDIT.COURSECD      = REGD.COURSECD ");
        stb.append("                                AND CREDIT.MAJORCD       = REGD.MAJORCD ");
        stb.append("                                AND CREDIT.GRADE         = REGD.GRADE ");
        stb.append("                                AND CREDIT.COURSECODE    = REGD.COURSECODE ");
        ...
        stb.append("     CAST(NULL AS SMALLINT) as HYOUTEI, ");
        stb.append("     CAST(NULL AS DECIMAL(8, 5)) as AVG, ");
        stb.append("     CAST(NULL AS SMALLINT), ");
        stb.append("     CREDIT.CREDITS, ");
        stb.append("     CAST(NULL AS VARCHAR(1)) AS REMARK ");
        stb.append(" FROM ");
        stb.append("     RECORD_SCORE_DAT SCORE ");
        stb.append("     LEFT JOIN SCHREG_REGD_DAT REGD ON REGD.SCHREGNO = SCORE.SCHREGNO ");
        stb.append("                                   AND REGD.YEAR     = SCORE.YEAR ");
        stb.append("                                   AND REGD.SEMESTER = '" + _param._semester2 + "' ");
        stb.append("     LEFT JOIN CREDIT_MST CREDIT ON CREDIT.YEAR          = REGD.YEAR ");
        stb.append("                                AND CREDIT.COURSECD      = REGD.COURSECD ");
        stb.append("                                AND CREDIT.MAJORCD       = REGD.MAJORCD ");
        stb.append("                                AND CREDIT.GRADE         = REGD.GRADE ");
        stb.append("                                AND CREDIT.COURSECODE    = REGD.COURSECODE ");
        stb.append("                                AND CREDIT.CLASSCD       = SCORE.CLASSCD ");
        stb.append("                                AND CREDIT.SCHOOL_KIND   = SCORE.SCHOOL_KIND ");
        stb.append("                                AND CREDIT.CURRICULUM_CD = SCORE.CURRICULUM_CD ");
        stb.append("                                AND CREDIT.SUBCLASSCD    = SCORE.SUBCLASSCD ");
        stb.append(" WHERE ");
        stb.append("         SCORE.YEAR     = '" + _param._ctrlYear + "' ");
        stb.append("     AND SCORE.SEMESTER = '" + _param._semester + "' ");
        stb.append("     AND SCORE.TESTKINDCD || SCORE.TESTITEMCD || SCORE.SCORE_DIV = '" + _param._testCd + "' ");
        stb.append("     AND SCORE.SCHREGNO = ? ");
        stb.append("     AND SCORE.VALUE_DI = '*' ");
        stb.append(" ORDER BY ");
        stb.append("     SCHREGNO, ");
    ...
    /** 欠課時数セット */
    private void setSubclassAttendance(final DB2UDB db2, final List schList) {
        try {
            String sql = "";
            final Semester sem = (Semester) _param._semsterMap.get(_param._semester);
            ...
            for (final Iterator it = KnjDbUtils.query(db2, sql).iterator(); it.hasNext();) {
                ...
                final Student student = Student.getStudent(KnjDbUtils.getString(row, "SCHREGNO"), schList);
                if (student == null || !"9".equals(KnjDbUtils.getString(row, "SEMESTER"))) {
                    continue;
                }
                final String subclassCd     = KnjDbUtils.getString(row, "SUBCLASSCD");

                final boolean isCombinedSubclass = "1".equals(KnjDbUtils.getString(row, "IS_COMBINED_SUBCLASS"));
                ...
                final SubclassAttendance sa = new SubclassAttendance(isCombinedSubclass, rawSick, rawReplacedSick);
        ...
        if (_param._testCd99) {
            ...
            for (Iterator iterator = _param._semsterList.iterator(); iterator.hasNext();) {
                ...
                final int semeEdate = Integer.parseInt(StringUtils.remove(seme._eDate, "-"));
                final int paramDate = Integer.parseInt(StringUtils.remove(_param._idouDate, "-"));
                final String setEdate;
                final String sDate = seme._sDate;
                if (semeEdate > paramDate) {
                    setEdate = _param._idouDate;
                    breakFlg = true;
                } else {
                ...
                for (final Iterator rit = KnjDbUtils.query(db2, sql).iterator(); rit.hasNext();) {
                    ...
                    // 学期末は合併元科目を印字しない
                    if (_param._subClassRepMap.containsKey(subclasscd)) {
                        //log.info(" 99 not print attend " + subclasscd);
                        continue;
                    }
...
//                		log.info(" add " + schregno + " sick2 (" + sDate + " ~ " + setEdate  + ")" + subclasscd + " = " + sick2 + "( => " + att._kekka + ")");
    ...
    private void setOtherInfo(final DB2UDB db2, final List schList) {
        ...
        try {
            ...
            for (Iterator it = schList.iterator(); it.hasNext();) {
                ...
                student._qualifiedList.addAll(KnjDbUtils.getColumnDataList(KnjDbUtils.query(db2, ps, new Object[] {student._schregNo}), "QUALIFIED"));
            }
    ...
    private static class Student {
        ...
        String _gradeCdHrclassnameAttendno = "";
        ...
        /** 科目別出欠 */
        final Map _subclassAttendanceMap = new TreeMap();;
    ...
    private static class SubclassAttendance {
        final boolean _isCombinedSubclass;
        final BigDecimal _rawSick;
        final BigDecimal _rawReplacedSick;

        SubclassAttendance(final boolean isCombinedSubclass, final BigDecimal rawSick, final BigDecimal rawReplacedSick) {
            _isCombinedSubclass = isCombinedSubclass;
            _rawSick = rawSick;
    ...
    private static class Param {
        ...
        Param(final DB2UDB db2, final HttpServletRequest request) throws SQLException {
            ...
            _semsterMap             = getSemsterMap(db2, _ctrlYear);
        ...
        private static String getDbPrginfoProperties(final DB2UDB db2, final String propName) {
            return KnjDbUtils.getOne(KnjDbUtils.query(db2, " SELECT VALUE FROM PRGINFO_PROPERTIES WHERE PROGRAMID = 'KNJD184N' AND NAME = '" + propName + "' "));
        }
        ...
        private Map getSemsterMap(final DB2UDB db2, final String year) {
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    retMap.put(semester, new Semester(semester, semesterName, sDate, eDate));
                }
            } catch (SQLException ex) {
                log.error("getSemester exception!", ex);
            } finally {
        ...
        private Map getSubClassMst(final DB2UDB db2) {
            ...
            try {
                final StringBuffer stb = new StringBuffer();
                stb.append(" WITH COMBINED_SUBCLASSCD AS( ");
                stb.append("     SELECT ");
                ...
                stb.append("     case ");
                stb.append("         when COMB.SUBCLASSCD is not null then 9 ");
                stb.append("         else 0 ");
                stb.append("     end AS REPLACEFLG  ");
                stb.append(" FROM ");
                ...
                stb.append("     LEFT JOIN COMBINED_SUBCLASSCD COMB ON COMB.COMBINED_CLASSCD       = SUB.CLASSCD");
                stb.append("                                       AND COMB.COMBINED_SCHOOL_KIND   = SUB.SCHOOL_KIND ");
                stb.append("                                       AND COMB.COMBINED_CURRICULUM_CD = SUB.CURRICULUM_CD ");
                stb.append("                                       AND COMB.COMBINED_SUBCLASSCD    = SUB.SUBCLASSCD ");
                stb.append(" ORDER BY ");
                stb.append("     SUBCLASSCD ");
                ...
                while (rs.next()) {
                    ...
                    retMap.put(subclassCd, new Subclass(classCd, className, subclassName, replaceflg));
                }
            } catch (SQLException ex) {
        ...
        private CertifSchoolDat getCertifSchoolDat(final DB2UDB db2) {
            ...
            sql.append("     AND CERTIF_KINDCD = '104' ");//104:通知表（高校）
