// kanji=漢字
/*
 * $Id: f8abad89d7deace02bad4ff24a3646cd5cf81714 $
 *
...
/**
 * 個人成績表印刷
 * @version $Id: f8abad89d7deace02bad4ff24a3646cd5cf81714 $
 */
public class KNJD192J {

    private static final Log log = LogFactory.getLog(KNJD192J.class);
...
//    private static final String HYOTEI_TESTCD = "9990009";
    ...
    public void svf_out(final HttpServletRequest request, final HttpServletResponse response) throws Exception {
        log.fatal("$Revision: 57312 $ $Date: 2017-11-30 12:05:04 +0900 (木, 30 11 2017) $"); // CVSキーワードの取り扱いに注意
    ...
    private void printMain(final DB2UDB db2, final Vrw32alp svf) throws SQLException {
        ...
        final String form = "KNJD192J.frm";
        final int maxCol = 2;
        final int maxLine = 4;
    ...
    private void printStudent(final Vrw32alp svf, final Map avgMap, final int line, final int col, final Student student) {
        final String title = KenjaProperties.gengou(Integer.parseInt(_param._year)) + "年度 " + StringUtils.defaultString(student._gradeName1) + " " + StringUtils.defaultString(_param._semesterName) + StringUtils.defaultString(_param._testName) + " 個人成績票";
        ...
        int akaten = 0;
        for (int subi = 0; subi < Math.min(11, student._subclassList.size()); subi++) {
            final Subclass subclass = (Subclass) student._subclassList.get(subi);
            final String ssubi = String.valueOf(subi + 1);
            if (subi == 2) {
                svf.VrsOutn("CLASS_NAME" + col + "_1", line, subclass._classname); // 教科名
            } else if (subi == 9) {
                svf.VrsOutn("CLASS_NAME" + col + "_2", line, subclass._classname); // 教科名
            }
            ...
            svf.VrsOutn("GRADE_AVE" + col + "_" + ssubi, line, sishagonyu(AverageDat.getAverageDat(avgMap, AverageDat.avgKey("1", student, subclass._subclasscd))._avg)); // 学年平均
            svf.VrsOutn("HR_AVE" + col + "_" + ssubi, line, sishagonyu(AverageDat.getAverageDat(avgMap, AverageDat.avgKey("2", student, subclass._subclasscd))._avg)); // クラス平均
        }
        ...
        svf.VrsOutn("GRADE_ALL_AVE" + col, line, sishagonyu(AverageDat.getAverageDat(avgMap, AverageDat.avgKey("1", student, _999999))._avg)); // 学年合計平均
        svf.VrsOutn("GRADE_AVE_AVE" + col, line, sishagonyu(AverageDat.getAverageDat(avgMap, AverageDat.avgKey("1", student, _999999AVG))._avg)); // 学年平均平均
        svf.VrsOutn("HR_ALL_AVE" + col, line, sishagonyu(AverageDat.getAverageDat(avgMap, AverageDat.avgKey("2", student, _999999))._avg)); // クラス合計平均
        svf.VrsOutn("HR_AVE_AVE" + col, line, sishagonyu(AverageDat.getAverageDat(avgMap, AverageDat.avgKey("2", student, _999999AVG))._avg)); // クラス平均平均
        svf.VrsOutn("HR_RANK" + col, line, null == subclassTotal ? "" : subclassTotal._rankHr); // クラス順位
        svf.VrsOutn("HR_NUM" + col, line, AverageDat.getAverageDat(avgMap, AverageDat.avgKey("2", student, _999999))._count); // クラス人数
        svf.VrsOutn("GRADE_RANK" + col, line, null == subclassTotal ? "" : subclassTotal._rankGrade); // 学年順位
        svf.VrsOutn("GRADE_NUM" + col, line, AverageDat.getAverageDat(avgMap, AverageDat.avgKey("1", student, _999999))._count); // 学年人数
        svf.VrsOutn("RED_POINT" + col, line, String.valueOf(akaten)); // 赤点
        svf.VrsOutn("SUBJECT_NUM" + col, line, String.valueOf(student._subclassList.size())); // 科目数
    }
...
//                    }
                    ...
                    subclass._classname = KnjDbUtils.getString(row, "CLASSABBV");
        ...
        private static String getChairStdSql(final Param param) {
            ...
            stb.append("     VALUE(L3.CLASSABBV, L3.CLASSNAME) AS CLASSABBV ");
            stb.append(" FROM ");
            ...
            stb.append("     LEFT JOIN CLASS_MST L3 ON T1.CLASSCD = L3.CLASSCD ");
