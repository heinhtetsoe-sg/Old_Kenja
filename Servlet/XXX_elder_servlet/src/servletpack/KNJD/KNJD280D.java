...
/**
 * 学校教育システム 賢者 [成績管理]  立志舎 成績状況一覧
 */
public class KNJD280D {

    private static final Log log = LogFactory.getLog(KNJD280D.class);
    ...
    protected void printMain(final DB2UDB db2, final Vrw32alp svf, final Param param) {
    	...
    	for (int i = 0; i < studentList.size(); i++) {
    		...
    		final String form = isSaturdayCourse ? "KNJD280D_2.frm" : "KNJD280D_1.frm";
    		svf.VrSetForm(form, 4);
    		final String[] semTestcds;
			if (isSaturdayCourse) {
    			semTestcds = new String[] {ZENKI_KIMATSU, KOUKI_KIMATSU};
				svf.VrsOut("TEST_NAME1", "前期"); // 試験名称 前期期末
				svf.VrsOut("TEST_NAME2", "後期"); // 試験名称 後期期末
    		} else {
				semTestcds = new String[] {ZENKI_CHUKAN, ZENKI_KIMATSU, KOUKI_CHUKAN, KOUKI_KIMATSU};
    		...
    		for (int subi = 0; subi < record; subi++) {
    			if (subi < subclassList.size()) {
    				...
    				if (null != subclass._subclassAttendance && null != subclass._subclassAttendance._attend) {
    					...
    					svf.VrsOut("ATTEND2", "0"); // 出席出席数
    				}

    				if (isSaturdayCourse) {
    					if ("1".equals(subclass._sVal1)) {
    						svf.VrsOut("NET", "〇"); // ネット
    					}
    				}
...
//    				final List scoreList = new ArrayList();
					for (int si = 0; si < semTestcds.length; si++) {
						...
						if (semester.compareTo(param._semester) <= 0) {
							...
							if (!param._d083Name1List.contains(subclass._subclasscd)) {
...
//							}
					...
					final String retest = subclass.getScore(SLUMP);
					if (NumberUtils.isDigits(retest)) {
						final int retestPassScore = NumberUtils.isDigits(subclass._retestPassScore) ? Integer.parseInt(subclass._retestPassScore) : 40; 
						svf.VrsOut("RETEST", retestPassScore <= Integer.parseInt(retest) ? "合格" : "不合格"); // 再試験
					}
    			} else {
...
//            _subclassabbv = subclassabbv;
    ...
    private Param createParam(final HttpServletRequest request, final DB2UDB db2) {
        log.fatal("$Revision: 69211 $ $Date: 2019-08-13 19:19:23 +0900 (火, 13 8 2019) $"); // CVSキーワードの取り扱いに注意
    ...
    private static class Param {
        ...
        Param(final HttpServletRequest request, final DB2UDB db2) {
            ...
            _d083Name1List = KnjDbUtils.getColumnDataList(KnjDbUtils.query(db2, " SELECT NAME1 FROM V_NAME_MST WHERE YEAR = '" + _year + "' AND NAMECD1 = 'D083' ORDER BY NAMECD2 "), "NAME1");
        }
		...
		private static String getDbPrginfoProperties(final DB2UDB db2, final String propName) {
            return KnjDbUtils.getOne(KnjDbUtils.query(db2, " SELECT VALUE FROM PRGINFO_PROPERTIES WHERE PROGRAMID = 'KNJD280D' AND NAME = '" + propName + "' "));
        }
