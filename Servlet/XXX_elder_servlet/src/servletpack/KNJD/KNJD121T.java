/**
 *
 *    学校教育システム 賢者 [成績管理] 成績チエックリスト（成績点票）前期３回／後期２回
 *
 *    2006/12/08 o-naka 新規作成 --- KNJD122を土台に作成
 *                      前期期末2素点・前期期末2評価を追加
 *  2006/12/21 o-naka   NO003:SQLの不具合を修正した。
 *  2006/12/26 o-naka   NO004:文字評定を出力するテーブルを変更した。RELATIVEASSESS_MST → ASSESS_MST
 *                      NO005:文字評定を出力する条件を変更した。科目コード='900100' → 教科コード='90'
 */
...
public class KNJD121T {

    private static final Log log = LogFactory.getLog(KNJD121T.class);
...
log.debug("SUBCLASSCD="+request.getParameter("SUBCLASSCD"));
    ...
    private void setHead(DB2UDB db2,Vrw32alp svf,String param[]){
        ...
        if ("1".equals(param[20])) {
            svf.VrSetForm("KNJD121_2P.frm", 4);
        } else {
            svf.VrSetForm("KNJD121_2.frm", 4);
        }
