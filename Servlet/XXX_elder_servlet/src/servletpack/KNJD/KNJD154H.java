/*
 * $Id: 27427eeccc2392d1b5cb06a71381f3189432e2da $
 *
 * 作成日: 2018/07/06
 * 作成者: tawada
 *
...
/**
 * 学校教育システム 賢者 [成績管理]  個人成績表
 */

public class KNJD154H {

    private static final Log log = LogFactory.getLog(KNJD154H.class);
    ...
    private List createStudents(final DB2UDB db2, final Param param) {
        ...
        try {
            ...
            while (rs1.next()) {
                ...
                student._gradeName1 = rs1.getString("GRADE_NAME1");
    ...
    private static class Student {
        ...
        private static String sqlRegdData (final Param param) {
            ...
            stb.append(    "        GDAT.GRADE_NAME1, T3.COURSENAME, T4.MAJORNAME, T6.COURSECODENAME, GDAT.SCHOOL_KIND ");
...
//            }
        ...
        protected void printHeaderBase(final Vrw32alpWrap svf, final Student student) {
            ...
            // KNJZ235Wで指定した帳票パターンがCの時、学部＋学科＋学年
            final String chohyoPattern = (String) _param._hrepCondition001Map.get(student._schoolKind);
            if (null != chohyoPattern && "3".equals(chohyoPattern)) {
                svf.VrsOut("SUBJECT",  student._courseName);
    ...
    private static class KNJD154A extends KNJD154Base {
        ...
        public KNJD154A(DB2UDB db2, Vrw32alpWrap svf, KNJD154H.Param param) {
            super(db2, svf, param);
...
//                log.debug(" koma kekka =" + spKomaKekka + " , group kekka = " + spGroupKekka + " , absence high = " + ah);
        ...
        private boolean printSubclassInfo (
            ...
            if (si._subclassname != null) {
            	svf.VrsOut("SUBCLASS" + (si._subclassname.length() > 20 ? "3" : si._subclassname.length() > 13 ? "2" : "1"),  si._subclassname);
            }
...
//            log.debug(" 科目= " + si._subclassname + " ( " + si._subclassCd + " )");
    ...
    private static class KNJD154C extends KNJD154Base {
        ...
        public KNJD154C(DB2UDB db2, Vrw32alpWrap svf, KNJD154H.Param param) {
            super(db2, svf, param);
...
//            setAbsenceFieldAttributeBase(svf, student.getAbsenceHigh(subclass._subclassCd), sick, "KEKKA" + j, "EARLY" + j);
    ...
    private static class Param {
        ...
        /** HREPORT_CONDITION_DATに登録されている帳票パターン取得（KNJZ235Wでセット） */
        private final Map _hrepCondition001Map;
        ...
        Param(final HttpServletRequest request, final DB2UDB db2) {
            ...
            _hrepCondition001Map = getHreportConditionDat(db2, _schoolCd);
        ...
        private Map getHreportConditionDat(final DB2UDB db2, final String schoolCd) {
            ...
            stb.append(" SELECT ");
            stb.append("     SCHOOL_KIND,REMARK1 ");
            stb.append(" FROM ");
            ...
            log.debug("HREPORT_CONDITION_DAT sql = " + stb.toString());
            try {
                ps = db2.prepareStatement(stb.toString());
                rs = ps.executeQuery();
                while (rs.next()) {
                    final String scoolKind = rs.getString("SCHOOL_KIND");
                    final String remark1   = rs.getString("REMARK1");
                    retMap.put(scoolKind, remark1);
                }
