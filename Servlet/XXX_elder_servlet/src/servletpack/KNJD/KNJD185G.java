// kanji=漢字
/*
 * $Id: 79775cac533bbfa1d408ddfa1047225c644e6a8a $
 */
 ...
 */
public class KNJD185G {
    private static final Log log = LogFactory.getLog(KNJD185G.class);
    ...
    private void setHyoushi(final Vrw32alp svf) {
        svf.VrSetForm("KNJD185G_3.frm", 1);
    ...
    private static class Student {
        ...
        Student(final String schregno, final String name, final String hrName, final String staffName1, final String staffName2, final String staffName3, final String attendno, final String grade, final String coursecd, final String majorcd, final String course, final String majorname, final String hrClassName1, final String entyear, final boolean useSubclass555555) {
            ...
            _useSubclass555555 = useSubclass555555;
            _attendMap = new TreeMap();
            _subclassMap = new LinkedMap();
            _communication = new ArrayList();
        }
        ...
        private static List getStudentList(final DB2UDB db2, final Param param) {
            ...
            stb.append("            ,W12.STAFFNAME AS STAFFNAME3 ");
            ...
            stb.append("     LEFT JOIN STAFF_MST W12 ON W12.STAFFCD = W6.TR_CD3 ");
            stb.append("     LEFT JOIN MAJOR_MST W9 ON W9.COURSECD = W1.COURSECD ");
            stb.append("                  AND W9.MAJORCD = W1.MAJORCD ");
            stb.append("     LEFT JOIN TOTAL5 ON TOTAL5.GRADE  = W1.GRADE ");
            stb.append("                     AND TOTAL5.COURSECD = W1.COURSECD ");
            stb.append("                     AND TOTAL5.MAJORCD = W1.MAJORCD ");
            stb.append("                     AND TOTAL5.COURSECODE = W1.COURSECODE ");
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    final String staffname3 = StringUtils.defaultString(rs.getString("STAFFNAME3"));
                    final boolean useSubclass555555 = "1".equals(rs.getString("USE_TOTAL5"));
                    students.add(new Student(rs.getString("SCHREGNO"), rs.getString("NAME"), rs.getString("HR_NAME"), staffname1, staffname2, staffname3, attendno, rs.getString("GRADE"), rs.getString("COURSECD"), rs.getString("MAJORCD"), rs.getString("COURSE"), rs.getString("MAJORNAME"), rs.getString("HR_CLASS_NAME1"), rs.getString("ENT_YEAR"), useSubclass555555));
                }
            } catch (Exception e) {
        ...
        public static void setHreportremarkCommunication(final Param param, final DB2UDB db2, final List studentList) {
            ...
            try {
                ...
                for (final Iterator it = studentList.iterator(); it.hasNext();) {
                    ...
                    while (rs.next()) {
                        student._communication.add(rs.getString("COMMUNICATION"));
                    }
    ...
    private static class Score {
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    if (StringUtils.split(subclasscd, "-").length == 4) {
                    	if (SUBCLASSCD999999.equals(StringUtils.split(subclasscd, "-")[3])) {
                        	subclasscd = SUBCLASSCD999999;
                    	} else if (SUBCLASSCD555555.equals(StringUtils.split(subclasscd, "-")[3])) {
                        	subclasscd = SUBCLASSCD555555;
                    	}
                    }
        ...
        private static String sqlScore(final Param param) {
            ...
            stb.append("     AND (CH1.SUBCLASSCD IS NOT NULL OR W3.SUBCLASSCD = '999999' OR W3.SUBCLASSCD = '555555') ");
            ...
            //成績不振科目データの表
            stb.append(" WHERE ");
            stb.append("     SUBSTR(T1.SUBCLASSCD, 1, 2) BETWEEN '" + KNJDefineSchool.subject_D + "' AND '" + KNJDefineSchool.subject_U + "' OR SUBSTR(T1.SUBCLASSCD, 1, 2) = '" + KNJDefineSchool.subject_T + "' OR T1.SUBCLASSCD like '%" + SUBCLASSCD999999 + "' OR T1.SUBCLASSCD like '%" + SUBCLASSCD555555 + "'");
            stb.append(" UNION ");
    ...
    private static class SubclassMst implements Comparable {
        ...
        public SubclassMst(final String classcd, final String subclasscd, final String classabbv, final String classname, final String subclassabbv, final String subclassname, final String isAttendSubclass, final int classShoworder3, final int subclassShoworder3) {
        	_classcd = classcd;
    ...
    private static class Form {
        ...
        final String[] _testcds_H = new String[] {"1990008", "2990008", "9990008", TESTCD_GAKUNEN_HYOTEI};
        ...
        private String makeStbTestCdCondition(String tblAssignName) {
            ...
            for (int i = 0; i < _testcds.length; i++) {
                ...
                if (seme.compareTo(_param._semester) <= 0 || !_param.isNotLastSemester()) {
        ...
        void printAttendance(final Vrw32alp svf, final Student student) {
            ...
            for (int i = 0; i < lastrow; i++) {
                ...
                if (!"9".equals(semes)) {
                	//"3学期"の出力は、学年末/学年末以外で違うので分ける。
            		if (Integer.parseInt(_param._semester) < 3 || i < 3) {
                        svf.VrsOutn("SEMESTER2", i+1, semester._semestername);
            		} else {
                        svf.VrsOutn("SEMESTER1", i+1, semester._semestername);
            		}
                } else {
          	        svf.VrsOutn("SEMESTER2", 3, semester._semestername);
                }
                //指定学期以降のデータは出力しない。
                ...
                final int line = "9".equals(semes) ? 3 : i + 1;
                if (!_param._lastsemester.equals(semes) || !"3".equals(semes)) { //◆
                    ...
                    svf.VrsOut("LESSON2", String.valueOf(att._lesson));       // 授業日数
                    svf.VrsOut("SUSPEND2", String.valueOf(att._suspend + att._mourning)); // 出停・忌引等日数
                    svf.VrsOut("MUST2", String.valueOf(att._mLesson));        // 出席すべき日数
                    svf.VrsOut("PRESENT2", String.valueOf(att._present));     // 出席日数
                    svf.VrsOut("NOTICE2", String.valueOf(att._absent));       // 欠席日数
                    svf.VrsOut("LATE2", String.valueOf(att._late));           // 遅刻回数
                    svf.VrsOut("EARLY2", String.valueOf(att._early));         // 早退回数
                }
            }
        }
        ...
        void printCommunication(final Vrw32alp svf, final Student student) {
        	...
        	for (int i = 0;i < Integer.parseInt(_param._lastsemester);i++) {
        	    ...
        	    if (i < lpcnt) {
                    final String[] commComment = get_token((String)student._communication.get(i), 60, 4);
                    for (int j = 0;j < commComment.length; j++) {
                        svf.VrsOutn("COMM" + String.valueOf(j + 1), i+1, commComment[j]);
                    }
        	    }
        	}
        }
        ...
        void printHeader(final DB2UDB db2, final Vrw32alp svf, final Student student) {
            ...
            final List<String> tlist = new ArrayList<String>(3);
            if (!StringUtils.isBlank(student._staffName1)) tlist.add(student._staffName1);
            if (!StringUtils.isBlank(student._staffName2)) tlist.add(student._staffName2);
            if (!StringUtils.isBlank(student._staffName3)) tlist.add(student._staffName3);
            if (tlist.size() == 1) {
            	final String teacherfield1 = getMS932ByteLength(tlist.get(0)) > 20 ? "2" : "1";
                svf.VrsOut("TEACHER_NAME1_" + teacherfield1, tlist.get(0)); // 担任1
            } else if (tlist.size() > 1) {
            	final String teacherfield2 = getMS932ByteLength(tlist.get(0)) > 20 ? "2" : "1";
                svf.VrsOut("TEACHER_NAME2_" + teacherfield2, tlist.get(0)); // 担任1
            	final String teacherfield3 = getMS932ByteLength(tlist.get(1)) > 20 ? "2" : "1";
                svf.VrsOut("TEACHER_NAME3_" + teacherfield3, tlist.get(1)); // 担任2
                if (tlist.size() > 2) {
                	final String teacherfield4 = getMS932ByteLength(tlist.get(2)) > 20 ? "2" : "1";
                    svf.VrsOut("TEACHER_NAME4_" + teacherfield4, tlist.get(2)); // 担任2
                }
            }

            //高校のみ
            svf.VrsOut("DEPARTMENT_NAME", student._majorname); // 学科名
            ...
            //学年末以外
    		if (_param.isNotLastSemester()) {
                svf.VrsOut("HR_NAME2", student._hrName + " " + (StringUtils.isBlank(student._attendno) ? "" : student._attendno + "番")); //クラス+出席番号
                svf.VrsOut("NAME2", student._name); // 氏名
    		}

        }

        //※ここでは、総合計以外は、フォームの構成(レコード出力)の関係で、後処理で出力しているので無視している。
        //※表の左側の項目名称は、フォームの構成(レコード出力)の関係から、ここでまとめて扱う。
        void printTotalScore(final Vrw32alp svf, final Student student) {

            final String[] printSeme = _param.getPrintSemester();

        	final Semester semester1 = _param.getSemester(printSeme[0]);
            svf.VrsOut("SEMESTER1_1", semester1._semestername);
            final Semester semester2 = _param.getSemester(printSeme[1]);
            svf.VrsOut("SEMESTER1_2", semester2._semestername);

            SubClass subClassGokei = student.getSubClass(student._useSubclass555555 ? SUBCLASSCD555555 : SUBCLASSCD999999);
            if (null != subClassGokei) {
                for (int i = 0; i < _testcds.length; i++) {
                    final Score score = subClassGokei.getScore(_testcds[i]);
                    if (score != null) {
                    	String datSemester = _testcds[i].substring(0,1);
                        //出力帳票とsubclasscdの学期でパターンが分かれる。
            		    if ("1".equals(datSemester)) {
                            svf.VrsOut("TOTAL1", score._score);
                            svf.VrsOut("AVERAGE1", sishaGonyu(score._avg));
                        } else if ("2".equals(datSemester)) {
                            svf.VrsOut("TOTAL2", score._score);
                            svf.VrsOut("AVERAGE2", sishaGonyu(score._avg));
                        } else if (TESTCD_GAKUNEN_HYOKA.equals(_testcds[i])) {
                            svf.VrsOut("TOTAL9", score._score);
                            svf.VrsOut("AVERAGE9", sishaGonyu(score._avg));
                        }
                    }
                }
            }
        ...
        void printScore(final Vrw32alp svf, final Student student) {
            ...
            for (final Iterator it = subclassList.iterator(); it.hasNext();) {
                ...
                boolean chgclsnameflg = false;
                String[] subclspart = StringUtils.split(subClass._mst._subclasscd, "-");
                String[] bksubclspart = StringUtils.split(bksubclscd, "-");
                //総合計は、フォームの構成(レコード出力)の関係で、前処理で出力しているので無視する。
                if (student.getSubClass(SUBCLASSCD999999) == subClass || student.getSubClass(SUBCLASSCD555555) == subClass) {
                	continue;
                }
                ...
                if (!"".equals(bksubclscd) ) {
                	if ((!bksubclspart[0].equals(subclspart[0]))) {
                		clscnt++;
                		chgclsnameflg = true;
                	}
                } else {
                ...
                //教科名
                String clsnmfield = getMS932ByteLength(subClass._mst._classabbv) > 4 ? "2" : "1";
                if (chgclsnameflg) {
                	if (subClass._mst._classabbv != null) {
                        svf.VrsOut("CLASS_NAME" + clsnmfield, subClass._mst._classabbv);
                	}
                }
                svf.VrsOut("GRPCD", String.valueOf(clscnt));
                //科目名
                String subclsnmfield = getMS932ByteLength(subClass._mst._subclassname) > 12 ? "2_1" : "1";
                svf.VrsOut("SUBCLASS_NAME" + subclsnmfield, subClass._mst._subclassname);
                ...
                for (int i = 0; i < _testcds.length; i++) {
                    ...
                    if (score != null) {
                        ...
                        //出力帳票とsubclasscdの学期でパターンが分かれる。
            		    if ("1".equals(datSemester)) {
                            svf.VrsOut("SCORE1", score._score);
                        } else if ("2".equals(datSemester)) {
                            svf.VrsOut("SCORE2", score._score);
                        } else if (TESTCD_GAKUNEN_HYOKA.equals(_testcds[i])) {
                            svf.VrsOut("SCORE9", score._score);
                        } else if (TESTCD_GAKUNEN_HYOTEI.equals(_testcds[i])) {
                            svf.VrsOut("VALUE", score._score);
                        }
                    }
                }
        	    subclscnt++;
            ...
            //出力していなかった場合、最初の1回分EndRecordを付加する。
            while (subclscnt < brkcnt) {
                svf.VrEndRecord();
                subclscnt++;
            }
        }
    ...
    private Param createParam(final HttpServletRequest request, final DB2UDB db2) {
        log.fatal("$Revision: 71690 $ $Date: 2020-01-10 21:43:35 +0900 (金, 10 1 2020) $"); // CVSキーワードの取り扱いに注意
    ...
    private static class Param {
        ...
        Param(final HttpServletRequest request, final DB2UDB db2) {
            ...
            _form._testcds = _form._testcds_H;
        ...
        public String getImagePath() {
            final String path = _documentRoot + "/" + (null == _imagePath || "".equals(_imagePath) ? "" : _imagePath + "/") + "SCHOOLLOGO_H.jpg";
