// kanji=漢字
/*
 * $Id: 033df8ffb990411eecb592e7a62ee53c7b81ed54 $
 *
 ...
 * <<クラスの説明>>。
 * @author m-yama
 * @version $Id: 033df8ffb990411eecb592e7a62ee53c7b81ed54 $
 */
public class KNJD192 {

    private static final Log log = LogFactory.getLog(KNJD192.class);
    ...
    private void setHead(final Vrw32alp svf) {
        final String form = "2".equals(_param._subclassMax) ? "KNJD192_2.frm" : "KNJD192.frm";
        ...
        try {
            ...
            while (rankRs.next()) {
                ...
                if (subclassCd.equals(SUBCLASSALL)) {
                    ...
                    student._testAll._rank = rankRs.getString(_param.getRankField());
                    student._testAll._rank2 = rankRs.getString(_param.getClassRankField());
                    if (_param._isChiben) {
    ...
    private void setAvg(final DB2UDB db2, final Student student) throws SQLException {
        ...
        try {
            ...
            final String rankSql;
            if (_param._isRecordRankVdat) {
                rankSql = getReportCardInfo.getRecordAverageVTestAppointSql(_param._year, _param._semester, student._schregno, null, null, _param._groupDiv, student._grade, student._hrClass, course, _param._kindItem);
            } else {
                rankSql = getReportCardInfo.getRecordAverageTestAppointSql(_param._year, _param._semester, student._schregno, null, null, _param._groupDiv, student._grade, student._hrClass, course, _param._kindItem);
            }
            db2.query(rankSql);
            rankRs = db2.getResultSet();
...
//            return score < _param.getFailValue();
    ...
    private class Param {
        ...
        boolean _isRecordRankVdat = false;
        ...
        Param(final DB2UDB db2, final HttpServletRequest request) throws Exception {
            ...
            _isChukyo = "chukyo".equals(_z010);
            if (_isChukyo && "9".equals(_semester)) {
                if ("9900".equals(request.getParameter("TESTCD"))) {
                    // 学年成績
                    _kindItem = request.getParameter("TESTCD");
                    _isRecordRankVdat = true;
                } else if ("9900_2".equals(request.getParameter("TESTCD"))) {
                    // 学年評定
                    _kindItem = "9900";
                } else {
                    _kindItem = request.getParameter("TESTCD");
