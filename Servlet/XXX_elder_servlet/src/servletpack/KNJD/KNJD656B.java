/*
 * $Id: a501b4494822a533fdc019136f8d0ce254ac7ec7 $
 *
 * 作成日: 2010/05/18
 * 作成者: maesiro
 *
 ...
 * 智辯五條 成績一覧表
 */
public class KNJD656B {

    private static final Log log = LogFactory.getLog(KNJD656B.class);
    ...
    private void printMain(final DB2UDB db2, final Vrw32alp svf, final String hrClass) {
        svf.VrSetForm("KNJD656B.frm", 1);
        
        final List students = getStudents(db2, hrClass);
        int[][] assessCount = new int[ASSESSESMARK.length][_param._mockSubclassCds.length];
        printHeader(svf, hrClass);
        int i = 1;
        boolean hasData = false;
        for (Iterator it = students.iterator(); it.hasNext();) {
            final Student student = (Student) it.next();
            if (printStudent(svf, student, assessCount, i)) {
                hasData = true;;
            }
            i += 1;
        }
    ...
    private void printFooter(final Vrw32alp svf, int[][] assessCount) {
        for (int a = 0; a < ASSESSESMARK.length; a++) {
            ...
            for (int s = 0; s < _param._mockSubclassCds.length; s++) {
                String mockSubclassCd = _param._mockSubclassCds[s];
                int mockSubclassIdx = getMockSubclassIndex(mockSubclassCd);
                svf.VrsOutn("TOTALASSESS" + mockSubclassIdx, (a + 1), String.valueOf(assessCount[a][s]));
            }
        }
    }

    private void printHeader(final Vrw32alp svf, final String hrClass) {
        svf.VrsOut("NENDO", KNJ_EditDate.h_format_JP_N(_param._loginDate) + "度");
        ...
        final String subtitle = (String) _param._mockNames.get(_param._mockCd);
        svf.VrsOut("SUBTITLE", "(" + (subtitle == null ? "" : subtitle) + ")");
    ...
    private int getMockSubclassIndex(String mockSubclassCd) {
        if (null != mockSubclassCd && null != _param._mockSubclassCds && _param._mockSubclassCds.length != 0) {
            for (int i = 0; i < _param._mockSubclassCds.length; i++) {
                if (mockSubclassCd.equals(_param._mockSubclassCds[i])) {
                    return i + 1;
                }
            }
        }
    ...
    private boolean printStudent(Vrw32alp svf, Student student, int[][] assessCount, int line) {
        boolean hasData = false;
        ...
        for (Iterator it = student._mockScores.iterator(); it.hasNext();) {
            ...
            if (-1 != score._assessMarkCd && score._assessMarkCd <= ASSESSESMARK.length) {
                assessCount[score._assessMarkCd - 1][idxSubclass - 1] += 1;
                svf.VrsOutn("ASSESS" + idxSubclass, line, score._assessMark);
            }
            svf.VrsOutn("SCORE" + idxSubclass, line, score._score);
            hasData = true;
        }
        return hasData;
    }
    ...
    private List getStudents(final DB2UDB db2, final String hrClass) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                MockSubclassScore mockSubclassScore = new MockSubclassScore(mockCd, mockSubclassCd, score, assessMark, assessMarkCd);
    ...
    private class Param {
        ...
        private void setMockNames(DB2UDB db2) {
            ...
            try {
                ...
                stb.append("         T2.GROUPNAME1, T3.MOCKNAME1 ");
                ...
                while (rs.next()) {
                    ...
                    String name = rs.getString("MOCKNAME1");
