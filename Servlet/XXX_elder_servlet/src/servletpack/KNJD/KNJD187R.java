...
public class KNJD187R {

    private static final Log log = LogFactory.getLog(KNJD187R.class);
    ...
    private static final String SDIV3010101 = "3010101"; //3学期中間
    ...
    private void printSvfMain(final DB2UDB db2, final Vrw32alp svf, final Student student) {
        final String form = "KNJD187R.frm";
        ...
        for (Iterator itSubclass = subclassList.iterator(); itSubclass.hasNext();) {
            ...
            if (student._printSubclassMap.containsKey(subclassCd)) {
                ...
                //科目名
                svf.VrsOutn("SUBCLASS_NAME", line, subclassMst._subclassname);
                ...
                //点数
                final String score = scoreData.score(_param._testCd);
                if(!"".equals(score) && !"*".equals(score)) {
                    totalScore += Integer.parseInt(score) * Integer.parseInt(credit);
                }
        ...
        if(dataFlg) {
            //総点
            svf.VrsOut("TOTAL_SCORE", String.valueOf(totalScore));

            //平均点
            if(!"0".equals(totalCredit)) {
                //総点 / 単位
                svf.VrsOut("AVERAGE", new BigDecimal(totalScore).divide(new BigDecimal(totalCredit), 1, BigDecimal.ROUND_HALF_UP).toString());
            } else {
                svf.VrsOut("AVERAGE", "0"); //1学期
            }
        }
    ...
    private void printTitle(final DB2UDB db2, final Vrw32alp svf, final Student student) {
        ...
        svf.VrsOut("TITLE", _param._nendo + "　" + student._gradename + "　" + semesterName + "中間試験成績"); //タイトル
        ...
        svf.VrsOut("PRINCIPAL_NAME", StringUtils.defaultString(_param._certifSchoolJobName) + StringUtils.defaultString(_param._certifSchoolPrincipalName)); //校長名
        svf.VrsOut("NOTE", _param._nendo + "　" + semesterName + "中間の成績を以下の通り、通知致します。"); //通知文
        ...
        svf.VrsOut("COMMENT", "（）内は無届内数　　　　（"+ KNJ_EditDate.h_format_JP_MD(_param._sdate) +"　～　" + KNJ_EditDate.h_format_JP_MD(_param._edate) +"）"); //出欠集計範囲
    ...
    private void printAttend(final Vrw32alp svf, final Student student) {
        ...
        final String absence = String.valueOf(sick_only + notice_only + nonotice_only) + "(" + String.valueOf(nonotice_only) + ")";
        svf.VrsOut("ABSENCE", absence); // 欠席日数（無届）

        //欠課時数
        BigDecimal late = BigDecimal.ZERO;
        BigDecimal sick2 = BigDecimal.ZERO;
        BigDecimal nonoticeOnly = BigDecimal.ZERO;
        for (final Iterator it = student._attendSubClassMap.keySet().iterator(); it.hasNext();) {
            final String subclasscd = (String) it.next();
            if (Arrays.asList(SUBCLASS_REIHAI, SUBCLASS_JINKEN).contains(subclasscd)) {
                continue;
            }
            final Map<String, SubclassAttendance> attendSubMap = student._attendSubClassMap.get(subclasscd);
            if (attendSubMap.containsKey(_param._semester)) {
                final SubclassAttendance attendance= attendSubMap.get(_param._semester);
                if (null != attendance) {
        ...
        final String kekka = sick2.toString() + "(" + nonoticeOnly.toString() + ")";
        svf.VrsOut("KEKKA", kekka); //欠課時数（無届）
        svf.VrsOut("LATE", late.toString()); // 遅刻
        ...
        svf.VrsOut("WORSHIP1", sick2.toString()); //礼拝 欠席
        svf.VrsOut("WORSHIP2", late.toString()); //礼拝 遅刻
        ...
        svf.VrsOut("RIGHTS1", sick2.toString()); //人権講座 欠席
        svf.VrsOut("RIGHTS2", late.toString()); //人権講座 遅刻
    }
    ...
    //指定講座の学期単位の出欠取得
    private Map getChairSemesAttend(final DB2UDB db2, final String schregno, final String chaircd) {
        ...
        stb.append("     AND S1.CHAIRCD  = '"+ chaircd +"' ");
        stb.append(" ) ");
        //メイン
        stb.append(" SELECT ");
        ...
        try {
            ...
            while (rs.next()) {
                ...
                map.put("LESSON", StringUtils.defaultString(rs.getString("LESSON")));
                map.put("OFFDAYS", StringUtils.defaultString(rs.getString("OFFDAYS")));
                map.put("ABSENT", StringUtils.defaultString(rs.getString("ABSENT")));
                map.put("SUSPEND", StringUtils.defaultString(rs.getString("SUSPEND")));
                map.put("MOURNING", StringUtils.defaultString(rs.getString("MOURNING")));
                map.put("ABROAD", StringUtils.defaultString(rs.getString("ABROAD")));
                map.put("SICK", StringUtils.defaultString(rs.getString("SICK")));
                map.put("NOTICE", StringUtils.defaultString(rs.getString("NOTICE")));
                map.put("NONOTICE", StringUtils.defaultString(rs.getString("NONOTICE")));
                map.put("NURSEOFF", StringUtils.defaultString(rs.getString("NURSEOFF")));
                map.put("LATE", StringUtils.defaultString(rs.getString("LATE")));
                map.put("EARLY", StringUtils.defaultString(rs.getString("EARLY")));

                final String semes = StringUtils.defaultString(rs.getString("SEMESTER"));
                rtnMap.put(semes, map);
            }
        } catch (SQLException ex) {
            log.debug("Exception:", ex);
        } finally {
    ...
    private static class Attendance {
        ...
        private static void load(
            ...
            final String sdate = dateRange._sdate.compareTo(param._sdate) > 0 ? param._sdate : dateRange._sdate;
    ...
    private static class Param {
        ...
        Param(final DB2UDB db2, final HttpServletRequest request) throws SQLException {
            ...
            _testCd = "1".equals(_semester) ? SDIV1010101 : "2".equals(_semester) ? SDIV2010101 : SDIV3010101; //対象学期-0101-01
            ...
            _attendParamMap.put("attendKekkaCalcKwanseiMethod", "1"); // 関西学院の欠課算出 (1日欠席分を除く)
        ...
        private void setCertifSchoolDat(final DB2UDB db2) {
            ...
            _certifSchoolJobName = StringUtils.defaultString(KnjDbUtils.getString(row, "JOB_NAME"));
