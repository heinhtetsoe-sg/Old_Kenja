/*
 * $Id: 27434847d67e3df3f117049ff9354f908f0a05ef $
 *
 * 作成日: 2018/09/10
 * 作成者: tawada
 *
...
public class KNJD126K {

    private static final Log log = LogFactory.getLog(KNJD126K.class);
    ...
    private String _paramDisplayHyoutei;
    ...
    public void svf_out(HttpServletRequest request, HttpServletResponse httpservletresponse) throws ServletException, IOException {
        ...
        try {
            ...
            _paramDisplayHyoutei = request.getParameter("displayHyoutei");//プロパティ(displayHyoutei = 1以外のとき、新フォームで出力)
    ...
    private void getHeaderData(DB2UDB db2, Vrw32alp svf) {
        ...
        if ("6".equals(_paramKantenHyouji)) {
            svf.VrSetForm(!"1".equals(_paramDisplayHyoutei) ? "KNJD126K_2_2.frm" : "KNJD126K_2.frm", 1);
        } else {
            svf.VrSetForm(!"1".equals(_paramDisplayHyoutei) ? "KNJD126K_1_2.frm" : "KNJD126K_1.frm", 1);
        }
        try {
            _nendo = KenjaProperties.gengou(Integer.parseInt(_paramCtrlY)) + "\u5E74\u5EA6";//平成XX年度
        } catch (Exception ex) {
        ...
        try {
            ...
            db2.commit();
            _printDateString = KNJ_EditDate.h_format_JP(as1[0]) + as1[1] + "\u6642" + as1[2] + "\u5206" + "\u3000\u73FE\u5728";//平成XX年XX月XX日XX時XX分　現在
        } catch (Exception ex) {
    ...
    private void getJviewname(DB2UDB db2, Vrw32alp svf, Map map) {
        try {
            ...
            if (0 < i) {
                _viewcdString = "('" + _viewcdString + s + _kanten2keta + "99" + "')";//観点コード 学習内容と様子
    ...
    private void printHeader(Vrw32alp svf) {
        ...
        if (!"1".equals(_paramDisplayHyoutei)) {
            svf.VrsOut("VALUE_NAME", "1".equals(_useHyoukaHyouteiFlg) ? "評価" : "評定");//評価評定名称
        }
    }
    ...
    private void printExam(Vrw32alp svf, ResultSet rs, int i, Map map) {
        try {
            ...
            if (!"1".equals(_paramDisplayHyoutei)) {
                svf.VrsOutn("STATUS8", i + 1, rs.getString("STATUS8"));//評価
                svf.VrsOutn("SCHREGNO", i + 1, rs.getString("SCHREGNO"));//学籍番号
                svf.VrsOutn("NAME", i + 1, rs.getString("NAME_SHOW"));//氏名
            } else {
            ...
            if (rs.getString("VIEWCD") != null && _viewcdString != null) {
                if (null != _notUseJviewStatusText) {
                    if (!"9".equals((String)map.get(rs.getString("VIEWCD")))) {
                        ...
                        svf.VrsOutn("STATUS" + (String)map.get(rs.getString("VIEWCD")), i + 1, setStatus);//観点状況
                    } else {
                        final List token = KNJ_EditKinsoku.getTokenList(rs.getString("STATUS"), 24);
                        for (int j = 0; j < token.size(); j++) {
                            svf.VrsOutn("STATUS" + (String)map.get(rs.getString("VIEWCD")) + "_" + (j + 1), i + 1,  (String) token.get(j));//学習内容と様子
                        }
                    }
                } else {
                    if ("9".equals((String)map.get(rs.getString("VIEWCD")))) {
                        ...
                        svf.VrsOutn("STATUS" + (String)map.get(rs.getString("VIEWCD")), i + 1, rs.getString("STATUS"));//観点状況
                    }
                }
            }
        } catch (Exception ex) {
    ...
    private String statementMeisai() {
        ...
        try {
            ...
            stb.append("    REMARK1 IS NOT NULL ");
            stb.append(" ) ");
            ...
            stb.append("        TBL3.score AS status8 ");
            stb.append("FROM    SCHNO TBL1 ");
            stb.append("        LEFT JOIN JVIEWSTAT TBL2 ON TBL2.SCHREGNO = TBL1.SCHREGNO ");
            stb.append("        LEFT JOIN RECORD_SCORE_DAT TBL3 ");
            stb.append("             ON TBL3.YEAR = '" + _paramCtrlY + "' ");
            stb.append("            AND TBL3.SEMESTER = '" + _paramSemester + "' ");
            stb.append("            AND TBL3.TESTKINDCD = '99' ");
            stb.append("            AND TBL3.TESTITEMCD = '00' ");
            if ("TESTITEM_MST_COUNTFLG_NEW_SDIV".equals(_useTestCountflg)) {
                if ("9".equals(_paramSemester) && !"1".equals(_useHyoukaHyouteiFlg)) {
                    stb.append("            AND TBL3.SCORE_DIV = '09' ");
                } else {
                    stb.append("            AND TBL3.SCORE_DIV = '08' ");
                }
            } else {
                    stb.append("            AND TBL3.SCORE_DIV = '00' ");
            }
            stb.append("            AND TBL3.CLASSCD || '-' || TBL3.SCHOOL_KIND || '-' || TBL3.CURRICULUM_CD || '-' || TBL3.SUBCLASSCD = '" + _paramSubclasscd + "' ");
            stb.append("            AND TBL3.SCHREGNO = TBL1.SCHREGNO ");
