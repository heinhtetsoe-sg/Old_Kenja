// kanji=漢字
/*
 * $Id: bdb6c205e6e99fd8384f8d8cbd2a9672c684937f $
 *
 * 作成日: 2007/05/14
 * 作成者: nakamoto
 *
 ...
 * @author nakamoto
 * @version $Id: bdb6c205e6e99fd8384f8d8cbd2a9672c684937f $
 */
public class KNJD617 {
    private static final Log log = LogFactory.getLog(KNJD617.class);
    ...
    private KNJD065_COMMON createKnjd065Obj(
        ...
        log.debug(" testKindCd = " + _param._testKindCd + " , isChiben = " + _param.isChiben());

        // 成績一覧表(KNJD062) 中間
        if ("0101".equals(_param._testKindCd)) { return new KNJD062A_INTER(db2); }

        // 成績一覧表(KNJD062) 期末1 or 期末2
        if ("0201".equals(_param._testKindCd) || "0202".equals(_param._testKindCd)) { return new KNJD062A_TERM(db2); }

        // 成績一覧表(KNJD062) 学期
        if (!"9".equals(_param._semester)) { return new KNJD062A_GAKKI(db2); }

        // 成績一覧表(KNJD062) 学年(智辯・評価)
        if (!_param.isChiben9909() && "9900".equals(_param._testKindCd) && _param.isChiben()) { return new KNJD062A_GRADE_HYOKA(db2); }
        ...
        for (final Iterator it = subClasses.iterator(); it.hasNext();) {
            if (0 == line % _param._formMaxColumn) {
                ...
                for (int i = 0; i < 5; i++) {
                    ...
                    final String hankoName = (String) _param._hankoNameMap.get(key);
                    if (null != hankoName) {
                        svf.VrsOut("JOBNAME" + key, hankoName);
                    }
                }
                // 生徒名等を印字
                    ...
                    if (0 < student._specialAbsent) {
                        if (_param._isRuikei) {
                            ...
                            if (student._specialAbsent > absenceHigh999) {
                                svf.VrAttribute("SP_KEKKA" + gnum,  "Paint=(1,40,1),Bold=1");
                            }
                        }
            ...
            if (0 != value) {
                if (_param._isRuikei) {
                    if (detail._compIsOver) {
                        svf.VrAttribute(field + gnum, "Paint=(1,40,1),Bold=1");
...
//        String _fieldChaircd;
        ...
        void printHeader(
            ...
            if (_param._isRuikei) {
                ...
                svf.VrAttribute("NOTE3",  "Paint=(1,70,1),Bold=1");
                svf.VrsOut("NOTE3",  " " );
                svf.VrsOut("NOTE4",  "：未修得" + comment );
            }

            svf.VrAttribute("NOTE5",  "Paint=(1,50,1),Bold=1");
            svf.VrsOut("NOTE5",  " " );
...
//                _testName = "期末２";
    ...
    private class HRInfo implements Comparable {
        ...
        private String sqlHrclassAverage(final String hrClass, final String courseCd) {
            ...
            stb.append(   "AND  W3.TESTKINDCD || W3.TESTITEMCD = '" + _param.getTestKindcdForRecord() + "' ");
        ...
        private String sqlStdTotalRank() {
            ...
            if (!_param.isHousei()) {
                ...
                stb.append(           "AND  R1.TESTKINDCD || R1.TESTITEMCD = '" + _param.getTestKindcdForRecord() + "' ");
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    if (specialGroupCd != null) {
                        Integer specialLessonMinutes = (Integer) rs.getObject("SPECIAL_LESSON_MINUTES0");
...
//                        }
        ...
        private String sqlStdSubclassDetail(String hrclassCd) {
            ...
            stb.append("            W3.TESTKINDCD || W3.TESTITEMCD = '" + _param.getTestKindcdForRecord() + "' AND ");
...
//                        countA += totalObj._avgcount;
                ...
                if (null != attend) {
                    ...
                    if ("2".equals(_param._lessonDaysPrintFlg)) {
                        ...
                        arrj = Param.setMaxMin(arrj[0], arrj[1], attend._lesson);
                    }
                }
        ...
        private String sqlSubclassGradeAvg() {
            ...
            stb.append("    AND T1.TESTKINDCD || T1.TESTITEMCD = '" + _param.getTestKindcdForRecord() + "'");
        ...
        private String getHouteiJisuSql(final Student student, final SubClass subclass, final Param parameter, final boolean isGroup) {
            ...
            if (isGroup) {
                ...
                stb.append("         SUBCLASSCD = '" + subclass._subclasscode + "' ");
            }
            return stb.toString();
        }
...
//            _chaircd = chaircd;
    ...
    private class Total implements Comparable {
        ...
        private void compute() {
            ...
            _student._specialLesson = specialLesson;
        ...
        private int getSpecialAttendExe(final int kekka) {
            ...
            if (retIndex > 0) {
                ...
                seisu = isKirisute(hasu) ? seisu : seisu + 1;
            } else {
        ...
        private boolean isKirisute(int hasu) {
            String tokubetuKatudoKansan = _param._knjSchoolMst._tokubetuKatudoKansan;
            if ("1".equals(tokubetuKatudoKansan)) { // 特活換算：二捨三入 (五捨六入)
                return hasu < 6;
            } else if ("2".equals(tokubetuKatudoKansan)) { // 特活換算：四捨五入
                return hasu < 5;
            } else if ("3".equals(tokubetuKatudoKansan)) { // 特活換算：切り上げ
                return hasu == 0;
            } else if ("4".equals(tokubetuKatudoKansan)) { // 特活換算：切り下げ
                return true;
            }
            return hasu < 6;
        }
        ...
        private boolean isAddTotal(
            ...
            if (knjdObj.isGakunenMatu() && "2".equals(_param._scoreFlg) && null != replacemoto && 0 < replacemoto.intValue()) { return false; }
            return true;
        }
    ...
    private static class Param {
        ...
        /** 生徒別授業時数 */
        final boolean _printStudentLesson;
        
        /** 授業日数/出席すべき日数のどちらを表示するか */
        final String _lessonDaysPrintFlg;
        
        /** C005：欠課換算法修正 */
        final Map _subClassC005 = new HashMap();
        
        /** D047：判子欄名称 */
        final Map _hankoNameMap;
        ...
        Param(final HttpServletRequest request, final DB2UDB db2) {
            ...
            loadNameMstC005(db2);
            setWarnSemester(db2);
            _hankoNameMap = getHankoName(db2);
        ...
        private String getTestName(
            ...
            if (_schoolCdSakai.equals(_schoolCd) && "9900".equals(testcd)
                && "9".equals(_semester) && "1".equals(_scoreFlg)
            ) {
                testitemname = "学年評価";
            }
            if ((_schoolCdYonago.equals(_schoolCd) || _schoolCdSakai.equals(_schoolCd))
                 && "0201".equals(testcd) && "2".equals(_scoreFlg)
            ) {
                testitemname = "仮評価";
            }
            if (_schoolCdKoryou.equals(_schoolCd) && "9900".equals(testcd)
                && "9".equals(_semester) && "1".equals(_scoreFlg)
            ) {
                testitemname = "学年評価";
            }
        ...
        public void setRankTable() {
            if (("9900".equals(_testKindCd) && "1".equals(_scoreFlg)) || (!"9900".equals(_testKindCd) && "2".equals(_scoreFlg))) {
        ...
        public int getFailValue() {
            return (null == _ketten || "".equals(_ketten)) ? -1 : Integer.parseInt(_ketten);
        }
        ...
        public String getFormFile() {
            if ("1".equals(_formSelect)) { // 45名
                return "1".equals(_subclassMax) ? (_printStudentLesson ? "KNJD617_10.frm" : "KNJD617_6.frm") : "KNJD617_8.frm";
            } else { // 50名
                return "1".equals(_subclassMax) ? (_printStudentLesson ? "KNJD617_9.frm" : "KNJD617_5.frm") : "KNJD617_7.frm";
            }
        }
        ...
        public String getTestKindcdForRecord() {
            // 学年末の中央育英の9901はレコードは9900を参照する
            if ("9".equals(_semester) && "224030".equals(_schoolCd) && "9901".equals(_testKindCd)) {
                return "9900";
            }
            return _testKindCd;
        }
        ...
        private Map getHankoName(final DB2UDB db2) {
            final Map ret = new HashMap();
            PreparedStatement ps = null;
            ResultSet rs = null;
            try {
                final String sql = "SELECT * FROM NAME_MST WHERE NAMECD1 = 'D047' ";
                ps = db2.prepareStatement(sql);
                rs = ps.executeQuery();
                while (rs.next()) {
                    if (NumberUtils.isNumber(rs.getString("NAMECD2"))) {
                        final String key = String.valueOf(Integer.parseInt(rs.getString("NAMECD2")));
                        ret.put(key, rs.getString("NAME1"));
                    }
                }
            } catch (Exception e) {
                log.error("Exception", e);
