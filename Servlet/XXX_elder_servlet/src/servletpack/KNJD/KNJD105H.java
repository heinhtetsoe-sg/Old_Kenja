// kanji=漢字
/*
 * $Id: 15876d1b3feb66643d44e529bcaf55a5870a49a8 $
 *
 ...
 * 個人成績票。
 * @author takaesu
 * @version $Id: 15876d1b3feb66643d44e529bcaf55a5870a49a8 $
 */
public class KNJD105H {
    private static final Log log = LogFactory.getLog(KNJD105H.class);
...
//    private static final String BAR_CHART_LEGEND3 = "BarChartLegendCourseGroup.png";
    ...
    private static final String GROUP_DIV_HR = "2";
    ...
    public void svf_out(
        ...
        try {
            ...
            for (final Iterator it = students.iterator(); it.hasNext();) {
                ...
                Form.printStudent(_param, svf, student);
                _hasData = true;
            }
    ...
    private static void loadExam(final DB2UDB db2, final Param param, final List students, final Exam exam) throws SQLException {
        ...
        Student.loadSubClasses(db2, param, exam, students);
        Exam.loadAverageDat(db2, param, exam);
        Student.loadRecord(db2, param, exam, students);
        Exam.loadScoreDistribution(db2, param, exam, students);
//        Exam.loadCourseWeightingSubclassCdListMap(db2, exam, param);
...
//        }
    ...
    private static class Form {
        ...
        private static void printStudent(final Param param, Vrw32alp svf, final Student student) {
...
//            printBarGraph(svf, param, exam, student);
            ...
            // 成績
            printSubclass(svf, param, param._thisExam, student);

            // 前回の中間成績
            printSubclass2(svf, param, student);
        }
        ...
        private static void printStatic(final Vrw32alp svf, final Param param, final Student student) {
...
//            svf.VrsOut("STAFFNAME", param._remark2 + StringUtils.defaultString(student._staffname));
            ...
            for (int i = 0; i < param._beforeExamList.size(); i++) {
                ...
                svf.VrsOut("PRE_SEMESTER_NAME" + line, beforeExam._semestername);
                ...
                if (param._isJunior) {
                    ...
                    svf.VrsOut("PRE_DEVI_NAME" + line + "_2_1", "コース");
                    svf.VrsOut("PRE_DEVI_NAME" + line + "_2_2", "順位");
                } else {
                    svf.VrsOut("PRE_SCORE_NAME" + line, "得点");
...
//            final String barLegendImage;
            ...
            if (param._isJunior) {
                svf.VrsOut("ITEM_RANK", "クラス順位");
                svf.VrsOut("ITEM_DEVIATION", "コース順位");
            } else {
                svf.VrsOut("ITEM_RANK", "学年順位");
...
//            svf.VrsOut("BAR_LEGEND", param._imagePath + "/" + barLegendImage);
        ...
        private static JFreeChart createRaderChart(final DefaultCategoryDataset dataset) {
            final KNJD105HSpiderWebPlot plot = new KNJD105HSpiderWebPlot();
        ...
        private static void printRadarGraph(final Vrw32alp svf, final Param param, final Student student) {
            // データ作成
...
//                }
                ...
                if (!"99".equals(exam._kindCd) && null == exam._avgDat.get(avgKey(GROUP_DIV_GRADE, record._subClass._code, student, param))) {
                    continue;
                }
...
//                }
                ...
                final BigDecimal thisDev = param._isJunior ? record._hrDeviation : record._gradeDeviation;
                dataset.addValue(thisDev, "本人偏差値", key);// MAX80
//                dataset.addValue(50.0, "偏差値50", name);
                ...
                if (param.isPrintRadarWithBefore()) {
                    ...
                    if (null != beforeRecord) {
                        beforeDeviation = param._isJunior ? beforeRecord._hrDeviation : beforeRecord._gradeDeviation;
                        dataset.addValue(beforeDeviation, "前回の偏差値", key);
                        
                    }
                }
                log.info("レーダーグラフ⇒" + record._subClass + ", " + thisDev + " <- " + beforeDeviation);
            }
            ...
            try {
                ...
                if (0 < dataset.getColumnCount() && outputFile.exists()) {
                    ...
                    // 画像
                    final File f = new File(param._imagePath + "/" + (param.isPrintRadarWithBefore() ? RADER_CHART_LEGEND_NO50_WITH_BEFORE : RADER_CHART_LEGEND_NO50));
                    if (f.exists()) {
                        svf.VrsOut("RADER_LEGEND", f.getPath());
                    }
        ...
        private static void printScoreDist(final Vrw32alp svf, final Param param, final Exam exam, final Student student) {
            final String groupDiv = param._isJunior ? GROUP_DIV_HR : GROUP_DIV_GRADE;
            final String scoreDistKey = Student.scoreDistKey(groupDiv, param, student);
...
//                    log.info(" min = " + min + ", max = " + max);
                ...
                final AverageDat avgDat = (AverageDat) exam._avgDat.get(avgKey(groupDiv, allsubclasscd(param), student, param));
                if (null != avgDat) {
                    svf.VrsOutn("DIST_NUM", maxColum, avgDat._count);
                    svf.VrsOutn("DIST_AVE", maxColum, sishaGonyuStr(avgavg));
                    svf.VrsOutn("DIST_MAX", maxColum, avgDat._highscore);
                    svf.VrsOutn("DIST_MIN", maxColum, avgDat._lowscore);
...
//                }
                ...
                if (null == subclassMap.get(record._subClass._code) && null == exam._avgDat.get(avgKey(groupDiv, record._subClass._code, student, param))) {
                    continue;
                }
...
//                if ((param._isJunior || param._isHigh && _9900.equals(param._testCd)) && !exam.getAttendSubclass(student.gradeCourse()).contains(record._subClass._code)) {
//                    isAmikake = true;
//                }
...
//                        log.info(" min = " + min + ", max = " + max);
                ...
                final AverageDat avgDat = (AverageDat) exam._avgDat.get(avgKey(groupDiv, record._subClass._code, student, param));
                if (null != avgDat) {
                    svf.VrsOutn("DIST_NUM", column, avgDat._count);
                    svf.VrsOutn("DIST_AVE", column, sishaGonyuStr(avgDat._avg));
                    svf.VrsOutn("DIST_MAX", column, avgDat._highscore);
                    svf.VrsOutn("DIST_MIN", column, avgDat._lowscore);
        ...
        private static void printSubclass(final Vrw32alp svf, final Param param, final Exam exam, final Student student) {
            final Record rec9 = (Record) student.record(exam).get(allsubclasscd(param));
            if (null != rec9) {
                svf.VrsOut("SCORE5", rec9._score);
                if (param._isJunior) {
                    svf.VrsOut("RANK5",      rec9._hrRank);
                    svf.VrsOut("DEVIATION5", rec9._courseRank);
                } else {
                    svf.VrsOut("RANK5",      rec9._gradeRank);
                    svf.VrsOut("DEVIATION5", sishaGonyuStr(rec9._gradeDeviation));
                }
            }
//            final AverageDat avg9 = (AverageDat) exam._avgDat.get(avgKey(groupDiv, allsubclasscd(param), student, param));
//            if (null != avg9) {
...
//                }
                ...
                final Record record = (Record) student.record(exam).get(subClass._code);
//                final AverageDat avgDat = (AverageDat) exam._avgDat.get(avgKey(groupDiv, subClass._code, student, param));
...
//                }
                printScoreClassname(svf, clazz._code, clazz._name, new String[] {"CLASS", "CLASS_2"});

                // 科目
                boolean isAmikake = false;
//                if ((_param._isJunior || _param._isHigh && _9900.equals(param._testCd)) && !exam.getAttendSubclass(student.gradeCourse()).contains(subClass._code)) {
//                    isAmikake = true;
//                }
                printScoreSubclassname(svf, subClass._name, new String[] {"SUBCLASS_3_1", "SUBCLASS_3_2", "SUBCLASS_2", "SUBCLASS"}, isAmikake);

                if (null == record) {
                    continue;
                }

                if (param.isKetten(record._score)) {
                    svf.VrsOut("SCORE", "(" + record._score + ")");
                } else {
                    svf.VrsOut("SCORE", record._score);
                }

                if (param._isJunior) {
                    svf.VrsOut("RANK", record._hrRank);
                    svf.VrsOut("DEVIATION", record._courseRank);
                } else {
                    svf.VrsOut("RANK", record._gradeRank);
                    svf.VrsOut("DEVIATION", sishaGonyuStr(record._gradeDeviation));
                }
...
//                }
        ...
        public static void printScoreClassname(final Vrw32alp svf, final String classcd, final String classname, final String[] field) {
            if (getMS932ByteLength(classname) > 6) {
                svf.VrsOut(field[0], classcd);
                svf.VrAttribute(field[0], "X=10000");
                svf.VrsOut(field[1], classname);
            } else {
                svf.VrsOut(field[0], classname);
                svf.VrsOut(field[1], classcd);
                svf.VrAttribute(field[1], "X=10000");
            }
        }

        public static void printScoreSubclassname(final Vrw32alp svf, final String subclassname, final String[] field, boolean isAmikake) {
            if (StringUtils.defaultString(subclassname).length() > 9) {
                svf.VrsOut(field[0], subclassname);
                if (isAmikake) {
                    svf.VrAttribute(field[0], "Paint=(1,90,2),Bold=1");
                    svf.VrAttribute(field[1], "Paint=(1,90,2),Bold=1");
                }
            } else if (StringUtils.defaultString(subclassname).length() > 7) {
                svf.VrsOut(field[2], subclassname);
                if (isAmikake) {
                    svf.VrAttribute(field[2], "Paint=(1,90,2),Bold=1");
                }
            } else {
                svf.VrsOut(field[3], subclassname);
                if (isAmikake) {
                    svf.VrAttribute(field[3], "Paint=(1,90,2),Bold=1");
                }
            }
        }
        
        private static void printSubclass2(final Vrw32alp svf, final Param param, final Student student) {
            ...
            for (int ei = 0; ei < param._beforeExamList.size(); ei++) {
                ...
                if (null != rec9) {
                    if (param._isJunior) {
                        svf.VrsOut("PRE_SCORE5_" + line, rec9._score);
                        svf.VrsOut("PRE_DEVI5_" + line, rec9._courseRank);
                    } else {
                        svf.VrsOut("PRE_SCORE5_" + line, rec9._score);
                        svf.VrsOut("PRE_DEVI5_" + line, sishaGonyuStr(rec9._gradeDeviation));
                    }
                }
            }
...
//                    }
                    ...
                    // 教科
                    final Class clazz = (Class) param._classes.get(subClass.getClassKey(param));
                    printScoreClassname(svf, clazz._code, clazz._name, new String[] {"PRE_CLASS", "PRE_CLASS_2"});
...
//                        isAmikake = true;
//                    }
                    printScoreSubclassname(svf, subClass._name, new String[] {"PRE_SUBCLASS_3_1", "PRE_SUBCLASS_3_2", "PRE_SUBCLASS_2", "PRE_SUBCLASS"}, isAmikake);
                    ...
                    if (param._isJunior) {
                        svf.VrsOut("PRE_SCORE" + line, record._score);
                        svf.VrsOut("PRE_DEVI" + line, record._courseRank);
                    } else {
                        svf.VrsOut("PRE_SCORE" + line, record._score);
                        svf.VrsOut("PRE_DEVI" + line, sishaGonyuStr(record._gradeDeviation));
                    }
                    printed = true;
                }
...
//  }
    ...
    private static String allsubclasscd(final Param param) {
        return param._isJunior ? ALL5 : ALL9;
    }
    
    private static String avgKey(final String groupDiv, String subclasscd, final Student student, final Param param) {
        if (GROUP_DIV_GRADE.equals(groupDiv)) {
            return subclasscd;
        } else if (GROUP_DIV_HR.equals(groupDiv)) {
            return student._hrClass + "-" + subclasscd;
        } else if (GROUP_DIV_COURSEGROUP.equals(groupDiv)) {
...
//        private Map _courseWeightingSubclassCdListMap;
        ...
        public Exam(final String year, final String semester, final String testCd, final String useCurriculumcd) {
            ...
            _kindCd = null == _testCd ? null : _testCd.substring(0, 2);
            _itemCd = null == _testCd ? null : _testCd.substring(2, 4);
            _scoreDiv = null == _testCd ? null : _testCd.substring(4);
        }
        
        private boolean isInvalid() {
            return !NumberUtils.isDigits(_kindCd) || !NumberUtils.isDigits(_itemCd) || !NumberUtils.isDigits(_scoreDiv);
        }

        private String testkindTestitem() {
...
//      ResultSet rs = null;
//      final String flg = _9900.equals(exam._testCd) ? "2" : "1";
//      try {
...
//  } 
        ...
        private void loadExam(final DB2UDB db2, final Param param) {
            ...
            _examName = "";
            try {
                final String sql = "SELECT testitemname FROM testitem_mst_countflg_new_sdiv "
                    + " WHERE year='" + _year + "' AND semester='" + _semester + "'"
                    + " AND testkindcd ='" + _kindCd + "' "
                    + " AND testitemcd ='" + _itemCd + "'"
                    + " AND score_div ='" + _scoreDiv + "'"
                    ;
                log.debug(" sql = " + sql);
            ...
            _semestername = StringUtils.defaultString((String) param._semesternameMap.get(_semester));
            _title = KenjaProperties.gengou(Integer.parseInt(_year)) + "(" + _year + ")年度" + "　" + _semestername + "　" + _examName;
        }
        ...
        private static void loadAverageDat(
            ...
            sql.append("SELECT");
            if ("1".equals(param._useCurriculumcd)) {
                sql.append("  case when t1.subclasscd in ('" + ALL3 + "', '" + ALL5 + "', '" + ALL9 + "') then t1.subclasscd  ");
                sql.append("   else t1.classcd || '-' || t1.school_kind || '-' || t1.curriculum_cd || '-' || t1.subclasscd ");
                sql.append("  end as subclasscd,");
            } else {
            ...
            sql.append("  t1.avg,");
            sql.append("  t1.avg_div,");
            sql.append("  t1.stddev,");
            sql.append("  t1.highscore,");
            sql.append("  t1.lowscore,");
            sql.append("  t1.count,");
            sql.append("  t1.coursecd,");
            sql.append("  t1.majorcd,");
            sql.append("  t1.coursecode,");
            if (_9900.equals(param._thisExam.testkindTestitem())) {
                sql.append("  value(t2.perfect, 5) as perfect");
            } else {
                sql.append("  value(t2.perfect, 100) as perfect");
            }
            sql.append(" FROM");
            sql.append("  record_average_sdiv_dat t1");
            sql.append("  LEFT JOIN perfect_record_sdiv_dat t2 ON t2.year = t1.year AND ");
            ...
            sql.append("    t2.score_div= t1.score_div AND");
            if ("1".equals(param._useCurriculumcd)) {
            ...
            sql.append("    t2.grade = (case when t2.div = '01' then '00' else '" + exam._grade + "' end) AND");
            sql.append("    t2.coursecd || t2.majorcd || t2.coursecode = ");
            sql.append("        (case when t2.div in ('01', '02') then '00000000' ");
            sql.append("              when t2.div = '04' then '0' || t1.majorcd || '0000' ");
            sql.append("              else t1.coursecd || t1.majorcd || t1.coursecode end) ");
            sql.append(" WHERE");
            sql.append("   t1.year='" + exam._year + "' ");
            sql.append("   AND t1.semester='" + exam._semester + "' ");
            sql.append("   AND t1.testkindcd='" + exam._kindCd + "' ");
            sql.append("   AND t1.testitemcd='" + exam._itemCd + "' ");
            sql.append("   AND t1.score_div='" + exam._scoreDiv + "' ");
            sql.append("   AND t1.grade='" + exam._grade + "' ");
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    if ("1".equals(avgDiv)) {
                        ...
                        avgKey = hrClass + "-" + subclasscd;
                    } else if ("5".equals(avgDiv)) {
            ...
            log.debug("テストコード=" + exam._kindCd + exam._itemCd + " の成績平均データの件数=" + exam._avgDat.size());
        }
        ...
        private static void loadScoreDistribution(
            ...
            stb.append(" WHERE ");
            stb.append("  t1.year = '" + param._year + "' ");
            stb.append("  and t1.semester = '" + (SEMEALL.equals(param._semester) ? param._ctrlSeme : param._semester) + "' ");
            stb.append("  and t1.grade = '" + param._grade + "' ");
            stb.append(") ");
            stb.append("SELECT");
            if ("1".equals(param._useCurriculumcd)) {
            ...
            if (SEMEALL.equals(exam._semester) && _9900.equals(exam.testkindTestitem())) {
            ...
            stb.append(" LEFT JOIN perfect_record_sdiv_dat t3 ON");
            ...
            stb.append("    t1.score_div=t3.score_div AND");
            if ("1".equals(param._useCurriculumcd)) {
            ...
            stb.append("  t1.testkindcd='" + exam._kindCd + "' AND");
            stb.append("  t1.testitemcd='" + exam._itemCd + "' AND");
            stb.append("  t1.score_div='" + exam._scoreDiv + "' ");
            ...
            stb.append("  t1.testkindcd='" + exam._kindCd + "' AND");
            stb.append("  t1.testitemcd='" + exam._itemCd + "' AND");
            stb.append("  t1.score_div='" + exam._scoreDiv + "' AND");
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    final String rtnKey = Student.scoreDistKey(groupDiv, param, rs.getString("grade"), rs.getString("hr_class"), rs.getString("coursegroupCd"), rs.getString("coursecd") + rs.getString("majorcd") + rs.getString("coursecode"));
                    ...
                    if (allsubclasscd(param).equals(subclasscd) && !_9900.equals(exam.testkindTestitem())) {
                        scoreList.add(score);
                    } else {
                        Number scorePercent;
                        final int perfect = rs.getInt("PERFECT");
    ...
    private static class Student implements Comparable {
        ...
        public Map record(final Exam exam) {
            return getMappedMap(_record, exam);
        }
        
        public Map subclasses(final Exam exam) {
            return getMappedMap(_subclasses, exam);
        }
...
//        }
        ...
        private static String scoreDistKey(final String groupDiv, final Param param, final Student student) {
            return scoreDistKey(groupDiv, param, student._grade, student._hrClass, student._coursegroupCd, student._courseCd + student._majorCd + student._courseCode);
        }
        
        private static String scoreDistKey(final String groupDiv, final Param param, final String grade, final String hrClass, final String coursegroupCd, final String course) {
            String rtnKey;
            if (GROUP_DIV_GRADE.equals(groupDiv)) {
                rtnKey = grade;
            } else if (GROUP_DIV_HR.equals(groupDiv)) {
        ...
        private static List createStudents(final DB2UDB db2, final Param param) throws SQLException {
            ...
            sql +="  t1.coursecode,";
            sql +="  base.name,";
            sql +="  t4.group_cd as coursegroupCd,";
            sql +="  t6.staffname";
            sql +=" FROM";
            sql +="  schreg_regd_dat t1 ";
            sql +="  inner join schreg_base_mst base on base.schregno = t1.schregno ";
            sql +="  inner join schreg_regd_hdat regdh on regdh.YEAR = t1.YEAR and regdh.SEMESTER = t1.SEMESTER and regdh.GRADE = t1.GRADE and regdh.HR_CLASS = t1.HR_CLASS ";
            sql +="    LEFT JOIN course_group_cd_dat t4 ON t1.year=t4.year AND";
            sql +="      t1.grade=t4.grade AND";
            sql +="      t1.coursecd=t4.coursecd AND";
            sql +="      t1.majorcd=t4.majorcd AND";
            sql +="      t1.coursecode=t4.coursecode";
            sql +="    LEFT JOIN schreg_regd_hdat t5 ON t1.year=t5.year AND";
            sql +="      t1.semester=t5.semester AND";
            sql +="      t1.grade=t5.grade AND";
            sql +="      t1.hr_class=t5.hr_class ";
            sql +="    LEFT JOIN staff_mst t6 ON t6.staffcd=t5.tr_cd1 ";
            sql +=" WHERE";
            sql +="  t1.year='" + param._year + "' AND";
            sql +="  t1.semester='" + (SEMEALL.equals(param._semester) ? param._ctrlSeme : param._semester) + "' AND";
            sql +="  t1.schregno IN " + SQLUtils.whereIn(true, param.getScregnos(db2));
            sql +=" ORDER BY";
            sql +="  t1.grade, t1.hr_class, t1.attendno ";
        ...
        private static void loadSubClasses(final DB2UDB db2, final Param param, final Exam exam, final List students) throws SQLException {
            ...
            try {
                ...
                rtn.append("select");
                if ("1".equals(param._useCurriculumcd)) {
                    rtn.append("  distinct T1.CLASSCD || '-' || T1.SCHOOL_KIND || '-' || T1.CURRICULUM_CD || '-' || T1.SUBCLASSCD AS SUBCLASSCD");
                } else {
                    rtn.append("  distinct T1.SUBCLASSCD");
                }
                rtn.append(" from");
                rtn.append("  RECORD_SCORE_DAT T1 ");
                rtn.append(" where");
                rtn.append("  T1.YEAR = '" + exam._year + "' and");
                rtn.append("  T1.SEMESTER = '" + exam._semester + "' and");
                rtn.append("  T1.TESTKINDCD = '" + exam._kindCd + "' and");
                rtn.append("  T1.TESTITEMCD = '" + exam._itemCd + "' and");
                rtn.append("  T1.SCORE_DIV = '" + exam._scoreDiv + "' and");
                rtn.append("  T1.SCHREGNO = ? ");
                rtn.append("     AND NOT EXISTS (SELECT 'X' FROM SUBCLASS_REPLACE_COMBINED_DAT ");
                rtn.append("               WHERE YEAR = '" + exam._year + "' ");
                rtn.append("                 AND ATTEND_CLASSCD = T1.CLASSCD ");
                rtn.append("                 AND ATTEND_SCHOOL_KIND = T1.SCHOOL_KIND ");
                rtn.append("                 AND ATTEND_CURRICULUM_CD = T1.CURRICULUM_CD ");
                rtn.append("                 AND ATTEND_SUBCLASSCD = T1.SUBCLASSCD) ");

                log.debug("成績入力データの科目のSQL=" + rtn.toString());

                ps = db2.prepareStatement(rtn.toString());
                for (final Iterator it = students.iterator(); it.hasNext();) {
                    final Student student = (Student) it.next();
        ...
        private static void loadRecord(
            ...
            final String gradeRankField  = "2".equals(param._outputKijun) ? "grade_avg_rank" : "3".equals(param._outputKijun) ? "grade_deviation_rank" : "grade_rank";
            final String hrRankField  = "2".equals(param._outputKijun) ? "class_avg_rank" : "3".equals(param._outputKijun) ? "class_deviation_rank" : "class_rank";
            final String courseRankField = "2".equals(param._outputKijun) ? "course_avg_rank" : "3".equals(param._outputKijun) ? "course_deviation_rank" : "course_rank";
            final String majorRankField = "2".equals(param._outputKijun) ? "major_avg_rank" : "3".equals(param._outputKijun) ? "major_deviation_rank" : "major_rank";
            ...
            sql += "  t2.score,";
            sql += "  t2." + gradeRankField + " as grade_rank,";
            sql += "  t2.grade_deviation,";
            sql += "  t2." + hrRankField + " as hr_rank,";
            sql += "  t2.class_deviation as hr_deviation,";
            sql += "  t2." + majorRankField + " as major_rank,";
            sql += "  t2.major_deviation,";
            sql += "  t2." + courseRankField + " as course_rank,";
            sql += "  t2.course_deviation,";
            sql += "  t2.avg,";
            sql += "  t3.div,";
            if (_9900.equals(exam.testkindTestitem())) {
                sql += "  value(t3.perfect, 5) as perfect";
            } else {
                sql += "  value(t3.perfect, 100) as perfect";
            }
            sql += " FROM";
            sql += "  record_score_dat t1 ";
            sql += " LEFT JOIN record_rank_sdiv_dat t2 ON";
            sql += "    t1.year=t2.year AND";
            sql += "    t1.semester=t2.semester AND";
            sql += "    t1.testkindcd=t2.testkindcd AND";
            sql += "    t1.testitemcd=t2.testitemcd AND";
            sql += "    t1.score_div=t2.score_div AND";
            if ("1".equals(param._useCurriculumcd)) {
                sql += "    t1.classcd=t2.classcd AND";
                sql += "    t1.school_kind=t2.school_kind AND";
                sql += "    t1.curriculum_cd=t2.curriculum_cd AND";
            }
            sql += "    t1.subclasscd=t2.subclasscd AND";
            sql += "    t1.schregno=t2.schregno ";
            sql += " LEFT JOIN perfect_record_sdiv_dat t3 ON";
            sql += "    t1.year=t3.year AND";
            sql += "    t1.semester=t3.semester AND";
            sql += "    t1.testkindcd=t3.testkindcd AND";
            sql += "    t1.testitemcd=t3.testitemcd AND";
            sql += "    t1.score_div=t3.score_div AND";
            if ("1".equals(param._useCurriculumcd)) {
                sql += "    t1.classcd=t3.classcd AND";
                sql += "    t1.school_kind=t3.school_kind AND";
                sql += "    t1.curriculum_cd=t3.curriculum_cd AND";
            }
            sql += "    t1.subclasscd=t3.subclasscd AND";
            sql += "    t3.grade = (case when t3.div = '01' then '00' else ? end) AND ";
            sql += "    t3.coursecd || t3.majorcd || t3.coursecode = ";
            sql += "        (case when t3.div in ('01', '02') then '00000000' ";
            sql += "              when t3.div = '04' then '0' || ? || '0000' ";
            sql += "              else ? end) ";
            sql += " WHERE";
            sql += "  t1.year='" + exam._year + "' AND";
            sql += "  t1.semester='" + exam._semester + "' AND";
            sql += "  t1.testkindcd='" + exam._kindCd + "' AND";
            sql += "  t1.testitemcd='" + exam._itemCd + "' AND";
            sql += "  t1.score_div='" + exam._scoreDiv + "' AND";
            sql += "  t1.schregno=?";
            sql +=" union all ";
            sql +="SELECT";
            sql +="  subclasscd,";
            ...
            sql += "  t2.course_deviation,";
            sql +="   t2.avg,";
            sql +="   cast(null as varchar(2)) as div, ";
            sql +="   cast(null as smallint) as perfect";
            sql +=" FROM record_rank_sdiv_dat t2 ";
            sql +=" WHERE";
            sql +="  year='" + exam._year + "' AND";
            sql +="  semester='" + exam._semester + "' AND";
            sql +="  testkindcd='" + exam._kindCd + "' AND";
            sql +="  testitemcd='" + exam._itemCd + "' AND";
            sql +="  score_div='" + exam._scoreDiv + "' AND";
            sql +="  subclasscd IN ('" + ALL3 + "', '" + ALL5 + "', '" + ALL9 + "') AND";
            sql +="  schregno=?";
            ...
            try {
                ...
                for (final Iterator it = students.iterator(); it.hasNext();) {
                    ...
                    ps.setString(5, student._schregno);
                    rs = ps.executeQuery();
                    while (rs.next()) {
                        final String subclasscd = rs.getString("subclasscd");
                        final String gradeRank = rs.getString("grade_rank");
                        final BigDecimal gradeDeviation = rs.getBigDecimal("grade_deviation");
                        final String hrRank = rs.getString("hr_rank");
                        final BigDecimal hrDeviation = rs.getBigDecimal("hr_deviation");
                        final String majorRank = rs.getString("major_rank");
                        final BigDecimal majorDeviation = rs.getBigDecimal("major_deviation");
                        final String courseRank = rs.getString("course_rank");
                        final BigDecimal courseDeviation = rs.getBigDecimal("course_deviation");

                        if (ALL3.equals(subclasscd) || ALL5.equals(subclasscd) || ALL9.equals(subclasscd)) {
                            final SubClass subClass = new SubClass(rs.getString("subclasscd"));
                            final String score = rs.getString("score");
                            final Record rec = new Record(subClass, score, null, rs.getBigDecimal("avg"), gradeRank, gradeDeviation, hrRank, hrDeviation, majorRank, majorDeviation, courseRank, courseDeviation);
                            ...
                            final String score = rs.getString("score");
                            String scorePercent = null;
                            final int perfect = rs.getInt("PERFECT");
                            if (100 == perfect) {
                                scorePercent = score;
                            } else if (null != score) {
                                scorePercent = new BigDecimal(rs.getString("SCORE")).multiply(new BigDecimal(100)).divide(new BigDecimal(perfect), 0, BigDecimal.ROUND_HALF_UP).toString();
                            }
                            final Record rec = new Record(subClass, score, scorePercent, null, gradeRank, gradeDeviation, hrRank, hrDeviation, majorRank, majorDeviation, courseRank, courseDeviation);
                            student.record(exam).put(subClass._code, rec);
                        }
                    }
                }
    ...
    private static class Record {
        ...
        Record(
                ...
                final String hrRank,
                final BigDecimal hrDeviation,
                final String majorRank,
                final BigDecimal majorDeviation,
        ...
        public String toString() {
            return _subClass + "/" + _score + "/" + _gradeRank + "/" + _gradeDeviation;
        }
    }
    ...
    private static class KNJD105HSpiderWebPlot extends SpiderWebPlot {
...
//            imageFileCheck(BAR_CHART_LEGEND2);
            ...
            _ketten = Integer.parseInt(!NumberUtils.isDigits(request.getParameter("KETTEN")) ? "0" : request.getParameter("KETTEN"));
...
//            loadClassOrder(db2);
        ...
        private boolean isPrintRadarWithBefore() {
            return "01".equals(_beforeExam0._kindCd);
        }
        ...
        public String[] getScregnos(final DB2UDB db2) throws SQLException {
            ...
            for (int i = 0; i < _categorySelected.length; i++) {
                ...
                try {
                    final String sql = " select"
                            + "    GRADE, HR_CLASS, ATTENDNO, SCHREGNO as schregno"
                            + " from"
                            + "    SCHREG_REGD_DAT"
                            + " where"
                            + "    YEAR = '" + _year + "' and"
                            + "    SEMESTER = '" + (SEMEALL.equals(_semester) ? _ctrlSeme : _semester) + "' and"
                            + "    GRADE = '" + grade + "' and"
                            + "    HR_CLASS = '" + room + "'"
                            + " order by "
                    ...
                    rs = ps.executeQuery();
                    while (rs.next()) {
                        schregnoList.add(rs.getString("schregno"));
                    }
                } catch (final SQLException e) {
            ...
            final String[] rtn = new String[schregnoList.size()];
            int i = 0;
...
//            boolean differentYear = false;
            ...
            try {
                ...
                stb.append("     t1.TESTKINDCD || t1.TESTITEMCD as testkindcdtestitemcd, ");
                ...
                stb.append("       and t2.TESTKINDCD = '" + _thisExam._kindCd + "' ");
                stb.append("       and t2.TESTITEMCD = '" + _thisExam._itemCd + "' ");
                stb.append("       and t2.SCORE_DIV = '" + _thisExam._scoreDiv + "' ");
...
//                stb.append("     and t1.TESTKINDCD <> '99' ");
                ...
                while (rs.next()) {
                    ...
                    final String beforetestkindcdtestitemcd = rs.getString("testkindcdtestitemcd");
                    final String scoreDiv = rs.getString("score_div");
                    
                    if (_9900.equals(_thisExam.testkindTestitem())) {
                        // 前回の試験は9900を表示
                        if (!_9900.equals(beforetestkindcdtestitemcd)) {
                            continue;
                        }
                    } else {
                        // 前回の試験は9900以外を表示
                    ...
                    if (!_thisExam._scoreDiv.equals(scoreDiv)) {
                        continue;
                    }
...
//            }
        ...
        private void loadSchoolKind(final DB2UDB db2) {
...
//        }
        ...
        public boolean isKetten(final String score) {
            if (NumberUtils.isDigits(score)) {
                return Integer.parseInt(score) < _ketten;
            }
            return false;
        }
