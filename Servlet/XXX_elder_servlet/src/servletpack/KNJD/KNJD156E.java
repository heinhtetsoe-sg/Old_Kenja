// kanji=漢字
/*
 * $Id: 6c7d80eb2f610cbe26a9586a6fb0bacf5ebb2641 $
 *
 * 作成日: 2019/08/28
 *
...
public class KNJD156E {

    private static final Log log = LogFactory.getLog(KNJD156E.class);
    private static final String _999999 = "999999";
    private static final String COURSE_GRP = "5";
    ...
    public void svf_out (
        ...
        try {
            ...
            for (final Iterator it = students.iterator(); it.hasNext();) {
                ...
                printMain(db2, svf, param, student);
    ...
    private static class Subclass {
        ...
        public String getAvgDiff(final String beforeTestcd, final String testcd) {
            final String avg = (String) _scoreMap.get("SCHREG_AVG" + testcd);
            final BigDecimal decimalAvg = null != avg ? new BigDecimal(avg).setScale(1, BigDecimal.ROUND_HALF_UP) : new BigDecimal("0");

            final String beforeAvg = (String) _scoreMap.get("SCHREG_AVG" + beforeTestcd);
            final BigDecimal decimalBeforeAvg = null != beforeAvg ? new BigDecimal(beforeAvg).setScale(1, BigDecimal.ROUND_HALF_UP) : new BigDecimal("0");

            final BigDecimal decimalCal = decimalAvg.subtract(decimalBeforeAvg);

            return decimalCal.toString();
        }
        ...
        public static String getSubclassSql(final Param param) {
            ...
            stb.append("         AND T_AVG.AVG_DIV = '" + param._avgDiv + "' ");
            stb.append("         AND T_AVG.GRADE = T3.GRADE ");
            stb.append("         AND T_AVG.COURSECD = '0' ");
            if (COURSE_GRP.equals(param._avgDiv)) {
                stb.append("         AND T_AVG.MAJORCD = T3.GROUP_CD ");
            } else {
            ...
            stb.append("        AND T_RANK.SUBCLASSCD IN ('999999') ");
            stb.append("  ORDER BY ");
    ...
    private void printMain(final DB2UDB db2, final Vrw32alp svf, final Param param, final Student student) {
        final String form = "H".equals(param._schoolKind) ? "KNJD156E_2.frm" : "KNJD156E_1.frm";
        svf.VrSetForm(form, 4);

        svf.VrsOut("TITLE", KNJ_EditDate.getAutoFormatYear(db2, Integer.parseInt(param._year))+ "年度　　成　績　個　票"); //
        ...
        svf.VrsOut("NAME", student._name + "　" + student._courseCodeName); // 氏名
        svf.VrsOut("AVG_NAME", COURSE_GRP.equals(param._avgDiv) ? "コース平均" : "学年平均");
        ...
        //平均の平均
        final Map totalAvgMap = new HashMap<String, String>();
        for (int testi = 0; testi < param._testCds.length; testi++) {
            ...
            for (int i = 0; i < student._subclassList.size(); i++) {
                ...
                if (subclass == subclassMap.get(_999999)) {
                    continue;
                }
                ...
                if (null != subclass._scoreMap.get("AVG" + param._testCds[testi])) {
                    final String avg = (String) subclass._scoreMap.get("AVG" + param._testCds[testi]);
                    BigDecimal setAvg = new BigDecimal(avg).setScale(1, BigDecimal.ROUND_HALF_UP);
                    totalAvg = totalAvg.add(setAvg);
                    avgCnt++;
                }
            }
            if (avgCnt == 0) {
                totalAvgMap.put(param._testCds[testi], "");
            } else {
                final BigDecimal setVal = totalAvg.divide(new BigDecimal(avgCnt), 1, BigDecimal.ROUND_HALF_UP);
                totalAvgMap.put(param._testCds[testi], setVal.toString());
            }
        }
        ...
        for (int testi = 0; testi < param.getTargetTestKindCds().length; testi++) {
            ...
            if (null != subclassMap.get(_999999)) {
                ...
                svf.VrsOut("GRADE_AVEALL_" + fieldFooter[testi], sishaGonyu((String) totalAvgMap.get(testCd))); // 平均の平均点
                if (1 <= testi) {
                    final String avgDiff = subc.getAvgDiff(param._testCds[testi - 1], testCd);
                    svf.VrsOut("RANK" + fieldFooter[testi], avgDiff); // 順位変動
                }
            }
        }
    ...
    private File getRadarGraph(final Param param, final Student student) {
        ...
        for (final Iterator it = student._subclassList.iterator(); it.hasNext();) {
            ...
            if (_999999.equals(subclass._subclasscd)) {
                continue;
            }
            final String score = (String) subclass._scoreMap.get("SCORE" + param._testCd);
            if (null == score) {
                continue;
            }

            final String key = subclass._subclassabbv;
            dataset.addValue(Integer.parseInt(score), "本人得点", key);// MAX80
        }
...
//          renderer1.setLinesVisible(false);
    ...
    private static class RaderChart extends SpiderWebPlot {
        ...
        protected void drawLabel(Graphics2D g2, Rectangle2D plotArea, double value,
            ...
            final String valueLabel = new BigDecimal(value).setScale(0, BigDecimal.ROUND_HALF_UP).toString();
        ...
        protected void drawBaseDeviation(Graphics2D g2, Rectangle2D plotArea, double extent) {
            ...
            final Font valueLabelFont = null == _valueLabelFont ? getLabelFont() : _valueLabelFont;
            final String valueLabel = new BigDecimal(100).setScale(0, BigDecimal.ROUND_HALF_UP).toString();
            final Rectangle2D valueLabelBounds = valueLabelFont.getStringBounds(valueLabel, frc);
            final LineMetrics valueLm = valueLabelFont.getLineMetrics(valueLabel, frc);
            final double valueAscent = valueLm.getAscent();

            final double[][] valueAngle = {{0, 180.0, 0.05}, {50, 94, 0.46}, {100, 92, 0.91}};
            for (int i = 0; i < valueAngle.length; i++) {
                final String valueLabel1 = new BigDecimal(valueAngle[i][0]).setScale(0, BigDecimal.ROUND_HALF_UP).toString();
    ...
    private static class Param {
        ...
        private void setCertifSchoolDat(final DB2UDB db2) {
            ...
            final String certCd = "J".equals(_schoolKind) ? "110" : "109";
            sql.append(" SELECT SCHOOL_NAME, JOB_NAME, PRINCIPAL_NAME, REMARK2, REMARK4, REMARK5 FROM CERTIF_SCHOOL_DAT ");
            sql.append(" WHERE YEAR = '" + _year + "' AND CERTIF_KINDCD = '" + certCd + "' ");
