...
public class KNJD126J {

    private static final Log log = LogFactory.getLog(KNJD126J.class);
    ...
    public void svf_out(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        log.fatal(" $Revision: 72498 $ $Date: 2020-02-20 10:31:43 +0900 (木, 20 2 2020) $");
    ...
    private void getHeaderData(DB2UDB db2, Vrw32alp svf, String as[]) {
        ...
        if ("6".equals(_kantenHyouji)) {
            svf.VrSetForm("KNJD126J_2.frm", 1);
        } else {
            svf.VrSetForm("KNJD126J.frm", 1);
        }
        ...
        try {
			if(_isSeireki) {
                _nendo = _ctrlY + "年度"; //XXXX年度
        	} else {
                _nendo = KNJ_EditDate.gengou(db2, Integer.parseInt(_ctrlY)) + "年度";//平成XX年度
        	}
        } catch (Exception exception) {
    ...
    private void getJviewname(DB2UDB db2, String as[], Map map) {
        try {
    ...
    private String statementMeisai(String as[]) {
        ...
        try {
            ...
            if ("KIN_RECORD_DAT".equals(_useRecordDat)) {
                ...
                if ("TESTITEM_MST_COUNTFLG_NEW_SDIV".equals(_useTestCountflg)) {
                    if ("9".equals(_semester) && !"1".equals(_useHyoukaHyouteiFlg)) {
