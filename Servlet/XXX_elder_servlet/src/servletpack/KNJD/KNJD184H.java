/*
 * $Id: ba54b0b8f853a9ae563decb78e30e7285fd1ee83 $
 *
 * 作成日: 2012/02/21
 * 作成者: maesiro
 *
...
public class KNJD184H {

    private static final Log log = LogFactory.getLog(KNJD184H.class);
    ...
    private void printMain(final DB2UDB db2, final Vrw32alp svf) {
        ...
        RecordTotalStudyTimeDat.setTotalStudy(db2, studentList, _param);
    ...
    private void printSvfHyoshi(final DB2UDB db2, final Vrw32alp svf, final Student student) {
        svf.VrSetForm("KNJD184H_1.frm", 1);
    ...
    private void printSvfMainSeiseki(
        ...
        final String form = "KNJD184H_2.frm";
        ...
        printSvfCatholicSpirit(svf, student);
    ...
    private void printSvfAttendSemes(final Vrw32alp svf, final Student student) {
        ...
        final int pchars = getParamSizeNum(_param._HREPORTREMARK_DAT_ATTENDREC_REMARK_SIZE_H, 0);
        final int plines = getParamSizeNum(_param._HREPORTREMARK_DAT_ATTENDREC_REMARK_SIZE_H, 1);
        final int chars = (-1 == pchars || -1 == plines) ? 20 : pchars;
    ...
    private void printSvfRecordRank(final Vrw32alp svf, final Student student) {
        ...
        final String _30101 = ScoreSubclass.KOUSA + "30101"; // 3学期中間
        final String _30201 = ScoreSubclass.KOUSA + "30201"; // 3学期期末

        final String _19900 = ScoreSubclass.KOUSA + "19900"; // 1学期成績
        final String _29900 = ScoreSubclass.KOUSA + "29900"; // 2学期成績
        final String _99901 = ScoreSubclass.KOUSA + "99901"; // 学年成績
        ...
        if (null != scoreSubclassAll) {
            ...
            fieldst.put(_30101, "5");
            fieldst.put(_30201, "6");
            for (int i = 0; i < _param._proficiencyCdList.size(); i++) { // "7" 〜 "9" 最大3件
                final String proficiencyCd = (String) _param._proficiencyCdList.get(i);
                fieldst.put(ScoreSubclass.JITURYOKU + proficiencyCd, String.valueOf(6 + 1 + i));
            }
            fieldst.put(_19900, "10");
            fieldst.put(_29900, "11");
            fieldst.put(_99901, "12");
        ...
        fields.put(_30101, "SCORE3_1");
        fields.put(_30201, "SCORE3_2");
        ...
        fields.put(_99901, "VALUE3");
        fields.put(_99900, "VALUE4");
...
//            log.fatal(" subclasscd = " + scoreSubclass._subclasscd);
            ...
            svf.VrsOut("SUBCLASS", scoreSubclass._subclassname);
            
            for (final Iterator its = fields.keySet().iterator(); its.hasNext();) {
                final String semtestcd = (String) its.next();
                final String field = (String) fields.get(semtestcd);

                if (semtestcd.startsWith(ScoreSubclass.JITURYOKU)) {

                    // 同一教科のすべての科目の実力テストの成績データ
                    final List sameClassJituryokuList = new ArrayList();
                    for (final Iterator itj = student._scoreSubclassList.iterator(); itj.hasNext();) {
                        final ScoreSubclass scoreSubclass0 = (ScoreSubclass) itj.next();
                        if (SUBCLASSALL.equals(scoreSubclass0._subclasscd)) {
                            continue;
                        }
                        if (scoreSubclass0._classcd.equals(scoreSubclass._classcd)) {
                            final ScoreRank scoreRank0 = (ScoreRank) scoreSubclass0._scoreRankMap.get(semtestcd);
                            if (null != scoreRank0) {
                                sameClassJituryokuList.add(scoreSubclass0);
                            }
                        }
                    }
                    if (sameClassJituryokuList.size() > 0) {
                        final ScoreSubclass scoreSubclassJituryoku = (ScoreSubclass) sameClassJituryokuList.get(0);
                        if (sameClassJituryokuList.size() > 1) {
                            log.warn("同一教科の複数の科目に実力テストのデータがある。（一番左表示の科目 " + scoreSubclassJituryoku._subclassname + "のみ表示。）");
                            for (final Iterator itj = sameClassJituryokuList.iterator(); itj.hasNext();) {
                                final ScoreSubclass same = (ScoreSubclass) itj.next();
                                log.warn(" " + same._subclasscd + ":" + same._subclassname + " = " + same._scoreRankMap.get(semtestcd));
                            }
                        }
                        final ScoreRank scoreRankJituryoku = (ScoreRank) scoreSubclassJituryoku._scoreRankMap.get(semtestcd);
                        svf.VrsOut(field, scoreRankJituryoku._score);
                    } else {
                        svf.VrsOut(field, "");
                    }
                } else {
                    ...
                    final String flg = "99".equals(getTestkindcd(semtestcd)) ? "2" : "1";
//                    log.fatal(" testKindCd = " + getTestkindcd(semtestcd));
                    final boolean isPrintScore = CombinedSubclass.isPrint(student._combinedSubclassList, flg, scoreSubclass._subclasscd, _param);
                    if (!isPrintScore) {
                        continue;
                    }
                    if (semtestcd.startsWith(ScoreSubclass.KOUSA) && (semtestcd.endsWith("9900") || semtestcd.endsWith("9901"))) {
                        svf.VrsOut(field, scoreRank._value);
                        if (semtestcd.endsWith("99900")) {
                            svf.VrsOut("CREDIT", scoreRank._getCredit);
                        }
                    } else {
     ...
     * @param student
     */
    private void printSvfCatholicSpirit(final Vrw32alp svf, final Student student) {
        final int keta = 50;
        final int lines = 2;

        if (null != student._recordTotalStudyTimeDat) {
            setTotalStudy(svf, "Catholic1_", keta, lines, student._recordTotalStudyTimeDat._totalStudyAct);
            
            setTotalStudy(svf, "Catholic2_", keta, lines, student._recordTotalStudyTimeDat._totalStudyTime);
        }
    }

    private void setTotalStudy(final Vrw32alp svf, final String field, final int keta, final int lines, final String s) {
        final List list = knjobj.retDividString(s, keta, lines);
        if (null != list) {
            if (list.size() <= 1 || getMS932ByteCount(s) <= keta) {
                svf.VrsOut(field + "1", (String) list.get(0));
                
            } else {
                for (int i = 0 ; i < list.size(); i++) {
                    svf.VrsOut(field + (i + 2), (String) list.get(i));
                }
            }
        }
    }
    ...
    private static class Student {
        ...
        RecordTotalStudyTimeDat _recordTotalStudyTimeDat = null;
...
//            log.debug(" scoreSubclassList = " + _scoreSubclassList);
//            log.debug(" recordTotalStudieTimeDat1 = " + _recordTotalStudyTimeDat);
        }
    }
    ...
    private static class RecordTotalStudyTimeDat {
         ...
         * @param param
         */
        public static void setTotalStudy(final DB2UDB db2, final List studentList, final Param param) {
            PreparedStatement ps = null;
            ResultSet rs = null;
            ...
            try {
                ...
                for (final Iterator it = studentList.iterator(); it.hasNext();) {
                    ...
                    student._recordTotalStudyTimeDat = null;
                    ...
                    while (rs.next()) {
                        ...
                        student._recordTotalStudyTimeDat = totalStudy;
                    }
                }
        ...
        public String toString() {
            return "RecordTotalStudyTimeDat(" + _totalStudyAct + ", " + _totalStudyTime + ")";
        }
    }
...
//            log.fatal(" combined subclass list = " + list);
    ...
    private static class HReportRemarkDat {
        ...
        public static Map getHReportRemarkDatMap(final DB2UDB db2, final Param param, final String schregno) {
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    final HReportRemarkDat hReportRemarkDat = new HReportRemarkDat(semester, attendrecRemark);
        ...
        public String toString() {
            return "HReportRemarkDat(" + _semester + ": attendrecRemark= " + _attendrecRemark + ")";
        }
    }
    ...
    private static class ScoreSubclass {
        ...
        public ScoreSubclass(final String classcd, final String subclasscd, final String classname, final String subclassname) {
            _classcd = classcd;
            ...
            try {
              ...
              log.debug(" subclass sql = " + sql);
                ...
                while (rs.next()) {
                  ...
                  if (null == ss) {
                      ss = new ScoreSubclass(classcd, subclasscd, rs.getString("CLASSNAME"), rs.getString("SUBCLASSNAME"));
                      list.add(ss);
                  }
                  ...
                  final ScoreRank scoreRank = new ScoreRank(score, value, getCredit, avg, rank);
        ...
        private static String getScoreSubclassSql(final Param param, final String schregno) {
            ...
            stb.append("     AND T1.SCHREGNO = '" + schregno + "' ");
            stb.append("     AND T1.RANK_DATA_DIV = '11' "); // 傾斜総合点
            ...
            stb.append("     T2.MAJOR_RANK AS RANK, "); // コースグループ
            ...
            stb.append("     AND T1.SCHREGNO = '" + schregno + "' ");
            stb.append("     AND (T1.TESTKINDCD = '99' AND (T1.TESTITEMCD = '00' OR T1.TESTITEMCD = '01') AND T1.SCORE_DIV = '00' OR T1.SCORE_DIV = '01') ");
            stb.append(" UNION ALL ");
            stb.append(" SELECT  ");
            stb.append("     '" + KOUSA + "' AS TEST_DIV_NAME, ");
            stb.append("     '00' AS CLASSCD, ");
            if ("1".equals(param._useCurriculumcd)) {
                stb.append("     '0' AS SCHOOL_KIND, ");
                stb.append("     '00' AS CURRICULUM_CD, ");
            }
            stb.append("     T1.SUBCLASSCD, ");
            stb.append("     T1.SEMESTER || T1.TESTKINDCD || T1.TESTITEMCD AS SEMTESTCD, ");
            stb.append("     T1.SCORE, ");
            ...
            stb.append("     T1.AVG, ");
            stb.append("     T1.MAJOR_RANK AS RANK, "); // コースグループ
            stb.append("     CAST(NULL AS VARCHAR(8)) AS PROFICIENCY_SUBCLASS_CD ");
            stb.append(" FROM ");
            ...
            stb.append(" SELECT  ");
            if ("1".equals(param._useCurriculumcd)) {
                stb.append("     T1.CLASSCD || '-' || T1.SCHOOL_KIND || '-' || T1.CURRICULUM_CD AS CLASSCD, ");
    ...
    private class Param {
        ...
        final String _HREPORTREMARK_DAT_ATTENDREC_REMARK_SIZE_H;
       ...
       /** 各学校における定数等設定 */
        ...
        Param(final DB2UDB db2, final HttpServletRequest request) throws SQLException {
            ...
            _HREPORTREMARK_DAT_ATTENDREC_REMARK_SIZE_H = null == request.getParameter("HREPORTREMARK_DAT_ATTENDREC_REMARK_SIZE_H") ? "" : StringUtils.replace(request.getParameter("HREPORTREMARK_DAT_ATTENDREC_REMARK_SIZE_H"), "+", " ");
