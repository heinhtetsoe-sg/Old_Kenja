// kanji=漢字
/*
 * $Id: fd90e727e52482299deca4d26ec1aae725238381 $
 *
 * 作成日: 2007/05/14
 * 作成者: nakamoto
 *
 ...
 * @author nakamoto
 * @version $Id: fd90e727e52482299deca4d26ec1aae725238381 $
 */
public class KNJD615G {
    private static final Log log = LogFactory.getLog(KNJD615G.class);
            ...
            for (int h = 0; h < hrclass.length; h++) {
                ...
                if (_param._outputCourse) {
                    ...
                    for (final Iterator it = courses.iterator(); it.hasNext();) {
                        ...
                        final HRInfo hrInfo = new HRInfo(hrclass[h]);  //HR組
    ...
    private static KNJD065_COMMON createKnjd065Obj(
        ...
        log.debug(" testKindCd = " + param._testKindCd + " , isChiben = " + param.isChiben());

        // 成績一覧表(KNJD062) 中間
        if ("0101".equals(param._testKindCd)) { return new KNJD062A_INTER(db2, param); }

        // 成績一覧表(KNJD062) 期末1 or 期末2
        if ("0201".equals(param._testKindCd) || "0202".equals(param._testKindCd)) { return new KNJD062A_TERM(db2, param); }

        // 成績一覧表(KNJD062) 学期
        if (!"9".equals(param._semester)) { return new KNJD062A_GAKKI(db2, param); }

        // 成績一覧表(KNJD062) 学年(智辯・評価)
        if (!param.isChiben9909() && "9900".equals(param._testKindCd) && param.isChiben()) { return new KNJD062A_GRADE_HYOKA(db2, param); }
        ...
        for (final Iterator it = subClasses.iterator(); it.hasNext();) {
            ...
            if (subclassesnum == line + 1) {
                for (final Iterator it1 = stulist.iterator(); it1.hasNext();) {
                    ...
                    printTotal(svf, hrInfo, student);
                    if (!student.hasKesshi(hrInfo, null)) {
                        printAttendInfo(svf, student._gnum, student._attendInfo);
                    }
                    ...
                    if (_knjdobj.hasJudgementItem()) {
                        printOtherCredits(svf, student);  // 前年度までの単位数を印字
                    }
            ...
            printSubclass(svf, line, subclass);  // 該当科目名等を印字
            printStudentsSubclass(svf, hrInfo, subclass, line, stulist);  // 生徒別該当科目成績を印字する処理
            svf.VrEndRecord();
            line += 1;
            hasData = true;
        }
    ...
    private void printSubclass(
        ...
        //科目名
        if (subclass._electdiv || subclass._saki) svf.VrAttribute("subject1", "Paint=(2,70,2),Bold=1");
        svf.VrsOut("subject1", subclass._subclassabbv);
        if (subclass._electdiv || subclass._saki) svf.VrAttribute("subject1", "Paint=(0,0,0),Bold=0");
        //単位数
    ...
    private void printHrInfo(
        ...
        //学級平均
        if (null != hr._avgHrTotal) {
            svf.VrsOut("TOTAL" + String.valueOf(_param._formMaxLine + 2), hr._avgHrTotal);
        }
        if (null != hr._avgHrAverage) {
            svf.VrsOut("AVERAGE" + String.valueOf(_param._formMaxLine + 2), hr._avgHrAverage);
        }
        //欠点者数
        if (null != hr._failHrTotal) {
            svf.VrsOut("TOTAL" + String.valueOf(_param._formMaxLine + 4), hr._failHrTotal);
        }
        //最高点
        if (null != hr._maxHrTotal) {
            svf.VrsOut("TOTAL" + String.valueOf(_param._formMaxLine + 5), hr._maxHrTotal);
        }
        //最低点
        if (null != hr._minHrTotal) {
            svf.VrsOut("TOTAL" + String.valueOf(_param._formMaxLine + 6), hr._minHrTotal);
        }
        //出席率・欠席
        if (null != hr._perHrPresent) {
            svf.VrsOut("PER_ATTEND", DEC_FMT1.format(hr._perHrPresent.setScale(1,BigDecimal.ROUND_HALF_UP)));
        }
        if (null != hr._perHrAbsent) {
            svf.VrsOut("PER_ABSENCE", DEC_FMT1.format(hr._perHrAbsent.setScale(1,BigDecimal.ROUND_HALF_UP)));
        }
    }
    ...
    private void printTotal(
        ...
        if (!student.hasKesshi(hrInfo, null)) {
            if (null != student._scoreSum) {
                if (_param._useRankAverage) {
                    svf.doNumberingSvfOut("AVERAGE", student._gnum, student._scoreAvg);    //平均点
                } else {
                    svf.doNumberingSvfOut("AVERAGE", student._gnum, student._scoreSum);      //総合点
                }
            }
            if ("9900".equals(_param._testKindCd)) {
                svf.doNumberingSvfOut("VIEW", student._gnum, student._gaiHyou);      //概評
            }
            //順位（学級）
            if (1 <= student._classRank) {
                svf.doNumberingSvfOut("CLASS_RANK", student._gnum, String.valueOf(student._classRank));
            }
            //順位（学年orコース）
            if (1 <= student._rank) {
                svf.doNumberingSvfOut("RANK", student._gnum, String.valueOf(student._rank));
            }
        }
        //欠点科目数
        if (0 < student._total._countFail) {
            svf.doNumberingSvfOut("FAIL", student._gnum, String.valueOf(student._total._countFail));
        }
    }
     ...
     * @param svf
     */
    private void printOtherCredits(
            final Vrw32alpWrap svf,
            final Student student
    ) {
        // 今年度認定単位数
     ...
     * @param stulist：List hrInfo._studentsのsublist
     */
    private void printStudentsSubclass(
            final Vrw32alpWrap svf,
            final HRInfo hrInfo,
            final SubClass subclass,
            final int line,
        ...
        for (final Iterator it = stulist.iterator(); it.hasNext();) {
            final Student student = (Student) it.next();
            if (hrInfo.getNotTargetSubclassList(student.getCourse()).contains(subclass.getSubclassCd())) {
                continue;
            }
            if (student._scoreDetails.containsKey(subclass._subclasscode)) {
              ...
              if (!student.hasKesshi(hrInfo, subclass.getSubclassCd())) {
                  printScoreDetail(svf, line, hrInfo, student, detail);
              }
              printDetailAttend(svf, line, hrInfo, student, detail);
            }
        }
    }
    ...
    private void printScoreDetail(
        ...
        final ScoreValue sv = _knjdobj.isPrintScore() ? detail._score : detail._patternAssess; // 素点 or 成績
        if (null != sv) {
            final String scoreField = "SCORE" + student._gnum;
            if (detail.isFail(sv.getScoreAsInt())) {
                svf.VrAttribute(scoreField,  "Paint=(1,50,1),Bold=1");
            }
            svf.VrsOut(scoreField,  sv.getScore() );
            if (detail.isFail(sv.getScoreAsInt())) {
                svf.VrAttribute(scoreField,  "Paint=(0,0,0),Bold=0");
            }
        }
    }
     ...
     * @param gnum 生徒の行番
     */
    private void printDetailAttend(
            final Vrw32alpWrap svf,
            final int line,
            final HRInfo hrInfo,
            final Student student,
        ...
        if (null != detail._absent) {
            final List attendSubclassList = hrInfo.getAttendSubclassList(student.getCourse(), detail._subClass.getSubclassCd());
            int value = 0;
            double dValue = 0.0;
            if (attendSubclassList.isEmpty()) {
                value = (int) Math.round(detail._absent.doubleValue() * 10.0);
                dValue = detail._absent.doubleValue();
            } else {
                for (final Iterator it = attendSubclassList.iterator(); it.hasNext();) {
                    final String attendSubclasscd = (String) it.next();
                    final ScoreDetail d = (ScoreDetail) student._scoreDetails1.get(attendSubclasscd);
                    if (null != d && null != d._absent) {
                        value += (int) Math.round(d._absent.doubleValue() * 10.0);
                        dValue += d._absent.doubleValue();
                    }
                }
            }
            if (0 != value) {
                final String field = _knjdobj.getSvfFieldNameKekka() + student._gnum;
                if (detail._isOver) {
                    svf.VrAttribute(field,  "Paint=(1,70,1),Bold=1");
                }
                svf.VrsOut(field,  _knjdobj.getAbsentFmt().format(dValue));
                if (detail._isOver) {
    ...
    private void printAttendInfo(final Vrw32alpWrap svf, int gnum, final AttendInfo at) {
        svf.doNumberingSvfOutNonZero("PRESENT", gnum, at._mLesson);      // 出席すべき日数
        svf.doNumberingSvfOutNonZero("SUSPEND", gnum, at._suspend);      // 出席停止
        svf.doNumberingSvfOutNonZero("KIBIKI", gnum,  at._mourning);     // 忌引
        svf.doNumberingSvfOutNonZero("ABSENCE", gnum, at._absent);       // 欠席日数
        svf.doNumberingSvfOutNonZero("ATTEND", gnum,  at._present);      // 出席日数
        svf.doNumberingSvfOutNonZero("TOTAL_LATE", gnum, at._late);      // 遅刻回数
        svf.doNumberingSvfOutNonZero("LEAVE", gnum,   at._early);        // 早退回数
        svf.doNumberingSvfOutNonZero("ABROAD", gnum,  at._transDays);    // 留学等実績
    }
    ...
    private static abstract class KNJD065_COMMON {
        ...
        void printHeader(
            ...
            svf.VrsOut("ITEM4", _param._useRankAverage ? "平均点" : "総合点");
            svf.VrsOut("ITEM5", "概評");
            svf.VrsOut("ITEM6", _item1Name + "・" + _item2Name);

            final String rankName = _param.getRankName();
            svf.VrsOut((rankName.length() > 3) ? "ITEM7_2" : "ITEM7", rankName + "順位");
            svf.VrsOut("ITEM8", "学年");
...
//                _testName = "期末２";
    ...
    private class HRInfo implements Comparable {
        ...
        void load(
            ...
            loadStudentsInfo(db2, courseCd);
            loadAttend(db2);
            loadHrclassAverage(db2, courseCd);
            loadRank(db2, courseCd);
            loadScoreDetail(db2);
            ...
            for (final Iterator it = _students.iterator(); it.hasNext();) {
                final Student student = (Student) it.next();
                for (final Iterator sit = student._scoreDetails.entrySet().iterator(); sit.hasNext();) {
                    final Map.Entry e = (Map.Entry) sit.next(); 
                    student._scoreDetails1.put(e.getKey().toString().substring(1), e.getValue());
                }
            }
        ...
        private Map getCourseNotTargetSubclassCdMap(final DB2UDB db2, final String courseCd) {
            ...
            PreparedStatement ps = null;
            ResultSet rs = null;
            final String flg = _param._testKindCd.startsWith("99") ? "2" : "1";
            try {
                final StringBuffer stb = new StringBuffer();
                stb.append("   SELECT ");
                ...
                if (null != courseCd) {
                    ...
                    if (!"9".equals(_param._semester)) {
                        ...
                        stb.append(" AND T3.SEMESTER = '" + _param._semeFlg + "' ");
                    }
        ...
        private boolean isNotTargetSubclassCd(final SubClass subclass) {
            ...
            for (final Iterator itc = courses.iterator(); itc.hasNext();) {
                ...
                List notTargetSubclassCdList = (List) _courseNotTargetSubclassCdMap.get(course);
                if (notTargetSubclassCdList == null) {
                    notTargetSubclassCdList = Collections.EMPTY_LIST;
                }
                if (!notTargetSubclassCdList.contains(subclass.getSubclassCd())) {
                    isNotTarget = false;
                }
            }
            return isNotTarget;
        ...
        private void loadStudents(
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    final Student student = new Student(rs.getString("SCHREGNO"), this, rs.getString("COURSECD"), rs.getString("MAJORCD"), rs.getString("COURSECODE"), gnum);
        ...
        private String sqlHrclassStdList(final String hrClass, final String courseCd) {
            ...
            stb.append("SELECT  W1.SCHREGNO,W1.ATTENDNO, W1.COURSECD, W1.MAJORCD, W1.COURSECODE ");
        ...
        private void loadStudentsInfo(
            ...
            ResultSet rs = null;
            try {
                final String sql = sqlStdNameInfo(_hrclassCd, courseCd);
                ps = db2.prepareStatement(sql);
                rs = ps.executeQuery();
                while (rs.next()) {
         ...
         */
        private String sqlStdNameInfo(final String hrClass, final String courseCd) {
            final StringBuffer stb = new StringBuffer();
        ...
        private void loadAttend(
            ...
            try {
                ...
                _param._hasuuMap = AttendAccumulate.getHasuuMap(_param._attendSemesMap, _param._semesterDateS, _param._date); // _sDate: 年度開始日, _date: LOGIN_DATE
                _param._semesFlg = ((Boolean) _param._hasuuMap.get("semesFlg")).booleanValue();
        ...
        private void loadRank(
            ...
            PreparedStatement ps = null;
            ResultSet rs = null;
            PreparedStatement psGaiHyou = null;
            ResultSet rsGaiHyou = null;

            try {
                final String sql = sqlStdTotalRank(_hrclassCd, courseCd);
                ps = db2.prepareStatement(sql);
                ...
                while (rs.next()) {
                    ...
                    if (null != student._scoreAvg) {
                        final String sqlGaiHyou = sqlGaiHyou(_param, student._scoreAvg, student._courseCd, student._majorCd, student._courseCode);
                        psGaiHyou = db2.prepareStatement(sqlGaiHyou);

                        rsGaiHyou = psGaiHyou.executeQuery();
                        while (rsGaiHyou.next()) {

                            student._gaiHyou = rsGaiHyou.getString("ASSESSMARK");
                        }
                    } else {
                        student._gaiHyou = "";
                    }
                ...
                DbUtils.closeQuietly(null, ps, rs);
                DbUtils.closeQuietly(null, psGaiHyou, rsGaiHyou);
                db2.commit();
            }
        }
         ...
         */
        private String sqlStdTotalRank(final String hrClass, final String courseCd) {
            final StringBuffer stb = new StringBuffer();
            ...
            if ("on".equals(_param._valueFlg)) {
                ...
                stb.append(         "W3.SUBCLASSCD NOT IN( ");
                stb.append(        "SELECT  ");
                if ("1".equals(_param._useCurriculumcd)) {
                    stb.append(       " R2.ATTEND_CLASSCD || '-' || R2.ATTEND_SCHOOL_KIND || '-' || R2.ATTEND_CURRICULUM_CD || '-' || ");
                }
                stb.append(                 "R2.ATTEND_SUBCLASSCD ");
                ...
                if (rankUseAvg) {
                    ...
                    if ("5".equals(_param._totalRank)) stb.append(   ",MAJOR_AVG_RANK AS TOTAL_RANK ");
                } else if (rankUseDev) {
                    ...
                    if ("5".equals(_param._totalRank)) stb.append(   ",MAJOR_DEVIATION_RANK AS TOTAL_RANK ");
                } else {
                    ...
                    if ("5".equals(_param._totalRank)) stb.append(   ",MAJOR_RANK AS TOTAL_RANK ");
                }
...
//                }
         ...
         * @param courseCd 
         */
        private String sqlGaiHyou(final Param param, final String scoreAvg, final String courseCd, final String majorCd, final String courseCode) {
            final StringBuffer stb = new StringBuffer();

            if ("9".equals(param._semester) && "9900".equals(param._testKindCd)) {
                stb.append(" SELECT ");
                stb.append("     T1.ASSESSMARK ");
                stb.append(" FROM ");
                
                if ("1".equals(_param._useAssessCourseMst)) {
                    stb.append("     ASSESS_COURSE_MST T1 ");
                    stb.append(" WHERE ");
                    stb.append("     T1.ASSESSCD = '4' ");
                    stb.append("     AND T1.COURSECD = '" + courseCd + "' ");
                    stb.append("     AND T1.MAJORCD = '" + majorCd + "' ");
                    stb.append("     AND T1.COURSECODE = '" + courseCode + "' ");
                } else {
                    stb.append("     ASSESS_MST T1 ");
                    stb.append(" WHERE ");
                    stb.append("     T1.ASSESSCD = '4' ");
                }
                stb.append("     AND ROUND(" + scoreAvg + ", 0) BETWEEN ASSESSLOW AND ASSESSHIGH ");
                
            } else {
                stb.append(" WITH LEBEL_T AS ( ");
                stb.append(" SELECT ");
                ...
                if ("1".equals(_param._useAssessCourseMst)) {
                    ...
                    stb.append("     AND COURSECODE = '" + courseCode + "' ");
                } else {
                ...
                stb.append("     AND T2.ASSESSLEVEL BETWEEN ASSESSLOW AND ASSESSHIGH ");
            }
        ...
        private String sqlStdSubclassDetail(String hrclassCd) {
            ...
            stb.append(     "FROM   CHAIR_STD_DAT W1, CHAIR_DAT W2, SCHNO_A W3 ");
            stb.append(     "WHERE  W1.YEAR = '" + _param._year + "' ");
            ...
            stb.append(    "FROM    RECORD_RANK_DAT W3, SCHNO_A W1 ");
            ...
            stb.append("            W3.SCHREGNO = W1.SCHREGNO AND W1.LEAVE = 0 ");
            stb.append(     ") ");
            ...
            stb.append(    "FROM    RECORD_SCORE_DAT W3, SCHNO_A W1 ");
            stb.append(    "WHERE   W3.YEAR = '" + _param._year + "' AND ");
            stb.append("            W3.SEMESTER = '" + _param._semester + "' AND ");
            stb.append("            W3.TESTKINDCD || W3.TESTITEMCD = '" + _param._testKindCd + "' AND ");
            ...
            stb.append(    "FROM    SUPP_EXA_DAT W3, SCHNO_A W1 ");
            stb.append(    "WHERE   W3.YEAR = '" + _param._year + "' AND ");
            stb.append("            W3.SEMESTER = '" + _param._semester + "' AND ");
            stb.append("            W3.TESTKINDCD || W3.TESTITEMCD = '" + _param._testKindCd + "' AND ");
            stb.append("            W3.SCHREGNO = W1.SCHREGNO AND W1.LEAVE = 0 ");
            ...
            if ("1".equals(_param._useCurriculumcd)) {
                stb.append("     T8.CLASSCD = T1.CLASSCD AND ");
                stb.append("     T8.SCHOOL_KIND = T1.SCHOOL_KIND ");
            } else {
...
//            }
            if (!"9900".equals(_param._testKindCd)) {
                stb.append("     WHERE T9.SUBCLASSCD IS NULL ");
            }
    ...
    log.debug(stb.toString());
        ...
        private void loadScoreDetail(
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    final String subclassCd = rs.getString("SUBCLASSCD").substring(1);
                    ...
                    if (classCd.compareTo(KNJDefineSchool.subject_U) <= 0 || classCd.equals(KNJDefineSchool.subject_T)) {
                        ...
                        final ScoreDetail scoreDetail = new ScoreDetail(
                                ...
                                ScoreValue.create(rs.getString("SCORE"), classCd),
                                ScoreValue.create(rs.getString("PATTERN_ASSESS"),classCd),
            ...
            try {
                ...
                _param._hasuuMap = AttendAccumulate.getHasuuMap(_param._attendSemesMap, _param._sDate, _param._date); // _sDate: 年度開始日, _date: LOGIN_DATE
                _param._semesFlg = ((Boolean) _param._hasuuMap.get("semesFlg")).booleanValue();
        ...
        SubClass getSubClass(
            ...
            final SubClass subClass = new SubClass(subclasscode, classabbv, subclassabbv, electdiv, credit, saki);
...
//              log.debug("total="+total+", totalMax="+totalMax+", totalMin="+totalMin);
    ...
    private class Student implements Comparable {
        ...
        private String _gaiHyou;
        private int _classRank;
        private int _rank;
        private final Map _scoreDetails = new HashMap();
        private final Map _scoreDetails1 = new HashMap();
        ...
        Student(final String code, final HRInfo hrInfo, final String courseCd, final String majorCd, final String courseCode, final int gnum) {
            _gnum = gnum;
            _code = code;
            _hrInfo = hrInfo;
            _courseCd = courseCd;
            ...
            if (null == subclasscd) {
                for (final Iterator cait = hrInfo.getCombinedAttendSubclassMap(course).entrySet().iterator(); cait.hasNext();) {
                    ...
                    for (final Iterator ait = attendSubclasscdList.iterator(); ait.hasNext();) {
                        ...
                        final ScoreDetail scoreAtt = (ScoreDetail) _scoreDetails1.get(attendSubclasscd);
    ...
    private static class ScoreValue {
        ...
        static ScoreValue create(
            ...
            if (KNJDefineSchool.subject_T.equals(classcd)) return null;
...
//        }
    ...
    private class Total implements Comparable {
        ...
        private void compute() {
            ...
            BigDecimal specialAbsentBd = new BigDecimal(0);
            for (final Iterator it = _student._spGroupAbsentMinutes.values().iterator(); it.hasNext();) {
                ...
                specialAbsentBd = specialAbsentBd.add(getSpecialAttendExe(groupAbsentMinutes.intValue()));
            }
            if (specialAbsentBd.doubleValue() > 0) {
                specialAbsentBd = specialAbsentBd.setScale(0, BigDecimal.ROUND_HALF_UP);
            }
            ...
            _student._specialAbsent = specialAbsentBd.intValue();
         ...
         */
        private BigDecimal getSpecialAttendExe(final int kekka) {
            ...
            if ("1".equals(_param._knjSchoolMst._tokubetuKatudoKansan)) { // 特活換算：二捨三入 (五捨六入)
                rtn = bigD.setScale(0, hasu < 6 ? BigDecimal.ROUND_FLOOR : BigDecimal.ROUND_CEILING); // hasu < 6 ? 0 : 1;
            } else if ("2".equals(_param._knjSchoolMst._tokubetuKatudoKansan)) { // 特活換算：四捨五入
                rtn = bigD.setScale(0, BigDecimal.ROUND_UP);
            } else if ("3".equals(_param._knjSchoolMst._tokubetuKatudoKansan)) { // 特活換算：切り上げ
                rtn = bigD.setScale(0, BigDecimal.ROUND_CEILING);
            } else if ("4".equals(_param._knjSchoolMst._tokubetuKatudoKansan)) { // 特活換算：切り下げ
                rtn = bigD.setScale(0, BigDecimal.ROUND_FLOOR);
            } else if ("0".equals(_param._knjSchoolMst._tokubetuKatudoKansan)) { // 換算無し
                rtn = bigD;
            } else {
                rtn = bigD.setScale(0, hasu < 6 ? BigDecimal.ROUND_FLOOR : BigDecimal.ROUND_CEILING); // hasu < 6 ? 0 : 1;
            }
    ...
    private Param createParam(HttpServletRequest request, final DB2UDB db2) {
    ...
    private static class Param {
        ...
        private final String _useAssessCourseMst;
        ...
        public String getFormFile() {
            if ("1".equals(_formSelect)) {
                return "KNJD615G_2.frm";
            } else {
                return "KNJD615G_1.frm";
            }
        }
        ...
        public String getTermKekka() {
            return KNJ_EditDate.h_format_JP(_sDate) + FROM_TO_MARK + getDateE();
        }
...
//            }
            _semesterDateS = _sDate;

            // テスト名称
            _testItemName = getTestName(db2, _year, _semester, _testKindCd);
        }
        ...
        private void loadAttendSemesArgument(DB2UDB db2) {
            ...
            try {
                ...
                _attendSemesMap = AttendAccumulate.getAttendSemesMap(db2, _schoolName, _year);
            } catch (Exception e) {
