// kanji=漢字
/*
 * $Id: 63daac530f8c34278f6763e1309a2e186ec8b87a $
 */
 ...
 */
public class KNJD187G {
    private static final Log log = LogFactory.getLog(KNJD187G.class);
    ...
    private static class Student {
        ...
        public static void setHreportremarkCommunication(final Param param, final DB2UDB db2, final List<Student> studentList) {
            ...
            stb.append("   AND SEMESTER <= '" + (SEMEALL.equals(param._semester) ? param._lastSemester : param._semester) + "' ");
            ...
            try {
                ...
                for (final Student student : studentList) {
                    ...
                    String sep = "";
                    while (rs.next()) {
                        if ("".equals(StringUtils.defaultString(rs.getString("COMMUNICATION"), ""))) continue;
                        student._communication += sep;
                        final String[] token = get_token(rs.getString("COMMUNICATION"), 68, 99);
                        int maxidx = token.length - 1;
                        ...
                        for (int cnt = 0;cnt <= maxidx;cnt++) {
                            if (!"".equals(StringUtils.defaultString(token[cnt], "")) && !"\n".equals(token[cnt])&& !"\r\n".equals(token[cnt])) student._communication += StringUtils.defaultString(token[cnt], "");
                        }
                        sep = "\n";
                    }
                    DbUtils.closeQuietly(rs);
        ...
        public static void setTotalStudy(final DB2UDB db2, final Param param, final List<Student> studentList) {
            ...
            try {
                ...
                if (SEMEALL.equals(param._semester)) {
                    stb.append("     AND T1.SEMESTER = '" + param._knjSchoolMst._semesterDiv + "' ");
                } else {
...
//                }
    ...
    private static class SubClass implements Comparable<SubClass> {
        ...
        public String getCredit(final DB2UDB db2, final Student student, final String year) throws SQLException {
            final StringBuffer stb = new StringBuffer();
            String rstr = "";

            stb.append(" SELECT CREDITS ");
            stb.append(" FROM ");
            stb.append("   CREDIT_MST ");
            stb.append(" WHERE ");
            stb.append("   YEAR = '" + year + "' AND ");
            stb.append("   GRADE = '" + student._grade + "' AND ");
            stb.append("   COURSECD || MAJORCD || COURSECODE = '" + student._course + "' AND ");
            stb.append("   CLASSCD || '-' || SCHOOL_KIND || '-' || CURRICULUM_CD || '-' || SUBCLASSCD = '" + _mst._subclasscd + "' ");
            ...
            try {
                ...
                if (rs.next()) {
                    rstr  = rs.getString("CREDITS");
                }
            } catch (final SQLException e) {
                log.error("単位数取得でエラー", e);
                throw e;
            } finally {
                db2.commit();
                DbUtils.closeQuietly(null, ps, rs);
    ...
    private static class Score {
        ...
        private boolean enableCredit(final Param param) {
            if (NumberUtils.isDigits(_replacemoto) && Integer.parseInt(_replacemoto) >= 1 && !"1".equals(param._knjd187gAddAttendSubclassGetCredit)) {
                return false;
            }
            return true;
        }
...
//            }
            ...
            stb.append("            W3.CLASSCD || '-' || W3.SCHOOL_KIND || '-' || W3.CURRICULUM_CD || '-' || ");
            stb.append("     W3.SUBCLASSCD AS SUBCLASSCD ");
            stb.append("    ,CREDIT.COMP_CREDIT AS COMP_CREDIT ");
            stb.append("    ,W2.PROV_FLG ");
            stb.append("    ,CREDIT.GET_CREDIT ");
            stb.append("    ,CREDIT.ADD_CREDIT ");
            ...
            stb.append("        AND W2.SUBCLASSCD = W3.SUBCLASSCD ");
            stb.append("        AND W2.SCHREGNO = W3.SCHREGNO ");
            stb.append("    LEFT JOIN RECORD_SCORE_DAT CREDIT ON CREDIT.YEAR = W3.YEAR ");
            stb.append("        AND CREDIT.SEMESTER = W3.SEMESTER ");
            stb.append("        AND CREDIT.TESTKINDCD = W3.TESTKINDCD ");
            stb.append("        AND CREDIT.TESTITEMCD = W3.TESTITEMCD ");
            stb.append("        AND CREDIT.SCORE_DIV = W3.SCORE_DIV ");
            stb.append("        AND CREDIT.CLASSCD = W3.CLASSCD ");
            stb.append("        AND CREDIT.SCHOOL_KIND = W3.SCHOOL_KIND ");
            stb.append("        AND CREDIT.CURRICULUM_CD = W3.CURRICULUM_CD ");
            stb.append("        AND CREDIT.SUBCLASSCD = W3.SUBCLASSCD ");
            stb.append("        AND CREDIT.SCHREGNO = W3.SCHREGNO ");
            stb.append("    WHERE   W3.YEAR = '" + param._year + "' ");
            stb.append("            AND W3.SEMESTER = '9' ");
            stb.append("            AND W3.TESTKINDCD = '99' ");
            stb.append("            AND W3.TESTITEMCD = '00' ");
            stb.append("            AND W3.SCORE_DIV = '09' ");
...
//            }
            ...
            if ("1".equals(param._zouka)) {
                stb.append("        ,CASE WHEN T33.GET_CREDIT IS NOT NULL OR T33.ADD_CREDIT IS NOT NULL THEN VALUE(T33.GET_CREDIT, 0) + VALUE(T33.ADD_CREDIT, 0) END AS GET_CREDIT ");
            } else {
    ...
    private abstract static class Form {
        ...
        abstract void print(final DB2UDB db2, final IPdf ipdf, final Student student) throws SQLException;
...
//            }
        ...
        public static class FormE extends Form {
            void init(final List<String> testcdList) {
                _testcds = setTestcd3(testcdList, new String[] {"1010108", "1990008", "2010108", "2990008","3010108", "3990008"}, 4, false);
            ...
            void print(final DB2UDB db2, final IPdf ipdf, final Student student) throws SQLException {
                final String _form;
                _form = "KNJD187G.frm";
...
//                ipdf.setString("ZIPCD", address._zipcd); // 郵便番号
            ...
            void printEAttendance(final IPdf ipdf, final Student student) {
                for (int i = 0; i < 4; i++) {
                    ...
                    final Attendance att = (Attendance) student._attendMap.get(String.valueOf(cnt));
                    if (null == att || att._lesson == 0) {
                        continue;
                    }

                    if(i != 3) {
                        if (i >= Integer.parseInt(param()._semester)) {
                            continue;
            ...
            void printECommunication(final IPdf ipdf, final Student student) {
                final String[] token = get_token(student._communication, 68, 12);
...
//                                name = StringUtils.defaultString(param().getSemestername(name));
//                            }
                            name = StringUtils.defaultString(param().getSemestername(String.valueOf(ti + 1)));
                            name = StringUtils.defaultString(name);
                        }

                        final int ketaName = KNJ_EditEdit.getMS932ByteLength(name);
                        field =  "ATTEND_TESTNAME" + (ketaName > 10 ? "_4" : ketaName > 8 ? "_3" : ketaName > 6 ? "2" : "");
                        final int pos = ti + 1;
                ...
                ipdf.VrsOut("DATE", "出欠集計日:" + KNJ_EditDate.h_format_JP(db2, param()._date));
            ...
            void printEScore(final IPdf ipdf, final Student student, final DB2UDB db2) throws SQLException {
                ...
                for (int sti = 0; sti < semesterTestItemEntrySetList.size(); sti++) {
                    ...
                    if (SEMEALL.equals(semester)) {
                        for (int i = 0; i < subclassList.size(); i++) {
                            ...
                            final int ci = i + 1;
                            if(!subClass._mst._isMoto) {
                                String credit = subClass.getCredit(db2, student, param()._year);
                                ipdf.VrsOutn("CREDIT1_1", ci, credit);
                            }
...
//                                    ipdf.VrsOut("GRADE_CNT" + tisHyotei, rank._count); // 学年人数 or 学科人数
    ...
    protected Param createParam(final HttpServletRequest request, final DB2UDB db2) {
        log.fatal("$Revision: 76111 $ $Date: 2020-08-20 17:41:55 +0900 (木, 20 8 2020) $"); // CVSキーワードの取り扱いに注意
    ...
    protected static class Param {
        ...
        final String _printTuishidoAmikakeKakko; // 追指導点を網掛けしてカッコつき表示
        final String _addPastCredit; // 前年度までの修得単位数を加算する
        final String _knjd187gAddAttendSubclassGetCredit; // 合併元科目の単位数を加算する
        ...
        final String _noPrintHogosha; // 保護者欄を印字しない
...
//        private int _gradCredits;  // 卒業認定単位数
        ...
        Param(final HttpServletRequest request, final DB2UDB db2) {
            ...
            _knjd187gAddAttendSubclassGetCredit = request.getParameter("knjd187gAddAttendSubclassGetCredit");
            ...
            if (PATTERN_E.equals(_patarnDiv)) {
...
//            }
        ...
        private Map<String, Semester> loadSemester(final DB2UDB db2, final String year, final String grade) {
            ...
            for (final Map row : KnjDbUtils.query(db2, sql)) {
                ...
                if (maxSems < Integer.parseInt(KnjDbUtils.getString(row, "SEMESTER")) && !SEMEALL.equals(KnjDbUtils.getString(row, "SEMESTER")))  maxSems = Integer.parseInt(KnjDbUtils.getString(row, "SEMESTER"));

            }
            _lastSemester = String.valueOf(maxSems);
            return map;
        }
