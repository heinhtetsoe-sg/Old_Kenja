// kanji=漢字
/*
 * $Id: fef9cfc4308fc0f7acc57cc0bedfdb267309bb9d $
 *
 * 作成日: 2011/06/14 18:31:18 - JST
 * 作成者: nakamoto
 *
 ...
 * 科目別得点上位一覧
 * @author nakamoto
 * @version $Id: fef9cfc4308fc0f7acc57cc0bedfdb267309bb9d $
 */
public class KNJD621C {

    private static final Log log = LogFactory.getLog(KNJD621C.class);

    private static final String FORM_FILE1 = "KNJD621C_1.frm";
    private static final String FORM_FILE2 = "KNJD621C_2.frm";
    ...
    private void dumpParam(final HttpServletRequest request, final Param param) {
        ...
        log.debug("テスト=" + param._mockName + " Form-File=" + getFormFile());
    }
    ...
    private class Param {
        ...
        private String[] getSubclassName(
            ...
            for (int i = 0; i < subclassCd.length; i++) {
                if (subclassCd[i].equals(SUBCLASS_ALL)) {
                    ...
                    final String sql = "SELECT SUBCLASS_NAME FROM MOCK_SUBCLASS_MST WHERE MOCK_SUBCLASS_CD = '" + subclassCd[i] + "' ";
                    db2.query(sql);
                    final ResultSet rs = db2.getResultSet();
                    try {
                        rs.next();
                        rtnVal[i] = rs.getString("SUBCLASS_NAME");
                    } finally {
        ...
        private String getMockName(
            ...
            String rtnVal = "";
            final String sql = "SELECT MOCKNAME1 AS MOCKNAME FROM MOCK_MST WHERE MOCKCD = '" + mockCd + "'";
            
            db2.query(sql);
            final ResultSet rs = db2.getResultSet();
            try {
                rs.next();
                rtnVal = rs.getString("MOCKNAME");
            } finally {
                DbUtils.closeQuietly(rs);
                db2.commit();
            }
    ...
    private static class Student {
        ...
        public static List loadStudents(
            ...
            try {
                while (rs.next()) {
                    final int rank = rs.getInt("GRADE_RANK");
                    final String score = rs.getString("SCORE") == null ? "" : rs.getString("SCORE");
                    ...
                    final Student student = new Student(rank, rs.getString("NAME"), hrName, score, assessLevel);
        ...
        private static String getStudentsSql(final String subclassCd, final Param param) {
            ...
            if ("1".equals(param._outputAssessLevel)) {
                stb.append("    LEFT JOIN MOCK_ASSESS_LEVEL_MST L4 ON  L4.YEAR = T1.YEAR ");
                stb.append("         AND L4.MOCKCD = T1.MOCKCD ");
                stb.append("         AND L4.MOCK_SUBCLASS_CD = T1.MOCK_SUBCLASS_CD ");
                // 学年
            ...
            stb.append("    AND T1.MOCKCD = '" + param._mockCd + "' ");
            stb.append("    AND T1.MOCK_SUBCLASS_CD = '" + subclassCd + "' ");
            stb.append("    AND T1.MOCKDIV = '1' ");
            stb.append("    AND T1.GRADE_RANK <= " + param._rankRange + " ");
