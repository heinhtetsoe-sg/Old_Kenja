...
public class KNJD183F {

    private static final Log log = LogFactory.getLog(KNJD183F.class);

    private static final String SEMEALL = "9";
    private static final String SELECT_CLASSCD_UNDER = "89";
    private static final String SUBCLASS_REIHAI = "95-H-3-950100"; //礼拝
    private static final String SUBCLASS_TANKYU = "90-H-3-900100"; //総合的な探求の時間

    private static final String SDIV1990008 = "1990008"; //1学期評定(評価
    private static final String SDIV2990008 = "2990008"; //2学期評定(評価
    private static final String SDIV3990008 = "3990008"; //3学期評定(評価
    private static final String SDIV9990008 = "9990008"; //学年5段階評定(評価

    private static final String ALL3 = "333333";
    private static final String ALL5 = "555555";
    private static final String ALL9 = "999999";

    private static final String HYOTEI_TESTCD = "9990008";
    ...
    private void printSvfMain(final DB2UDB db2, final Vrw32alp svf, final Student student) {
        final String form = "KNJD183F.frm";
        ...
        //総合的な探求の時間
        printTankyu(db2, svf, student, subclassList);
        ...
        final Map<String, CREDIT> subclasscdCreditMap = _param._creditMstMap.get(regdKey);
        for (Iterator itCredit = subclasscdCreditMap.keySet().iterator(); itCredit.hasNext();) {
            final String subclassCd = (String) itCredit.next();
            //修得すべき単位数
            final CREDIT credit = (CREDIT) subclasscdCreditMap.get(subclassCd);
            if (credit != null) {
                if (NumberUtils.isNumber(credit._credits)) {
                    requiredCredit += Integer.parseInt(credit._credits);
                }
            }
            //修得単位数
            if (student._printSubclassMap.containsKey(subclassCd)) {
                ...
                if (scoreData != null) {
                    final String getCredit = StringUtils.defaultString(scoreData.getCredit(SDIV9990008), "0");
                    if (NumberUtils.isNumber(getCredit)) {
                        totalGetCredit += Integer.parseInt(getCredit);
                    }
                }
            }
        }
        svf.VrsOut("COMP_CREDIT", String.valueOf(requiredCredit)); //修得すべき単位数
        svf.VrsOut("GET_CREDIT", String.valueOf(totalGetCredit)); //修得単位数
        ...
        for (Iterator itSubclass = subclassList.iterator(); itSubclass.hasNext();) {
            ...
            if(Integer.parseInt(subclassMst._classcd) > Integer.parseInt(SELECT_CLASSCD_UNDER)) {
                continue;
            }

            svf.VrsOutn("CLASS_NAME", line, subclassMst._classname); //教科名
            svf.VrsOutn("SUBCLASS_NAME", line, subclassMst._subclassname); //科目名
            svf.VrsOutn("SC_SELECT", line, "1".equals(subclassMst._electdiv) ? "選" : "必"); //必修

            //単位情報
            final CREDIT credit = _param.getCredit(student, subclassCd);
            if (credit != null) svf.VrsOutn("SC_CREDIT", line, credit._credits); //単位
            ...
            if (student._printSubclassMap.containsKey(subclassCd)) {
                ...
                for (final Iterator it = _param._semesterMap.keySet().iterator(); it.hasNext();) {
                    ...
                    final String semesField = semester;
                    ...
                    if (student._attendSubClassMap.containsKey(subclassCd)) {
                        ...
                        if (attendSubMap.containsKey(semester)) {
                            ...
                            BigDecimal mourning = BigDecimal.ZERO;
                            BigDecimal lesson = BigDecimal.ZERO;
                            if (attendance._sick != null) sick = sick.add(attendance._sick);
                            if (attendance._mourning != null) mourning = mourning.add(attendance._mourning);
                            if (attendance._lesson != null) lesson = lesson.add(attendance._lesson);
                            svf.VrsOutn("SC_SICK" + semesField, line, sick.toString()); //欠席
                            svf.VrsOutn("SC_MOURNING" + semesField, line, mourning.toString()); //忌引
                            svf.VrsOutn("SC_LESSON" + semesField, line, lesson.toString()); //授業時数
                        }
                    }

                    //欠課時数超過の判定
                    boolean isOver1 = false; // 1/3以上の欠課
                    boolean isOver2 = false; // 1/2以上の欠課
                    String attendSemes = ""; //欠課時数の参照学期
                    final int gradeCd = Integer.parseInt(StringUtils.defaultString(student._gradeCd, "0"));
                    if ("1".equals(semester) || ("2".equals(semester) && (gradeCd == 1 || gradeCd == 2))) {
                        //「全学年の1学期」または「1・2年の2学期」の場合、単学期の集計
                        attendSemes = semester;
                    } else if ("3".equals(semester) || ("2".equals(semester) && gradeCd == 3)) {
                        //「全学年の3学期」または「3年の2学期」の場合、累積の集計
                        attendSemes = SEMEALL;

                    }
                    if (student._attendSubClassMap.containsKey(subclassCd)) {
                        final Map<String, SubclassAttendance> attendSubMap = student._attendSubClassMap.get(subclassCd);
                        if (attendSubMap.containsKey(attendSemes)) {
                            final SubclassAttendance attendance = attendSubMap.get(attendSemes);

                            //欠課時数超過の判定
                            int jisu = (attendance._lesson == null)? 0 : attendance._lesson.intValue(); // 授業時数 = LEASON(LEASON - ABROAD) - ABSENT - MOURNING
                            jisu = (attendance._absent == null)? jisu : jisu  - attendance._absent.intValue();
                            jisu = (attendance._mourning == null)? jisu : jisu  - attendance._mourning.intValue();
                            if (jisu != 0) {
                                int jougenti1 = jisu / 3; // 端数切捨て
                                int jougenti2 = jisu / 2; // 端数切捨て
                                if (jougenti1 > 0) isOver1 = jougenti1 < attendance._absent.doubleValue(); // 1/3以上の欠課ならならTrue
                                if (jougenti2 > 0) isOver2 = jougenti2 < attendance._absent.doubleValue(); // 1/2以上の欠課ならならTrue
                            }
                        }
                    }

                    //評定
                    final String testcd = ("1".equals(semester)) ? SDIV1990008 : ("2".equals(semester)) ? SDIV2990008 : ("3".equals(semester)) ? SDIV3990008 : "";
                    String score = StringUtils.defaultString(scoreData.score(testcd));
                    //欠課時数超過 の場合、評定を編集
                    if ("1".equals(_param._semester) || ("2".equals(_param._semester) && (gradeCd == 1 || gradeCd == 2))) {
                        //出力対象 ⇒ 「全学年の1学期」または「1・2年の2学期」
                        if ("".equals(score) || !NumberUtils.isDigits(score)) {
                            //未入力
                            if(isOver1) {
                                score = "*0";
                            } else {
                                score = "0"; //未入力で授業時数の1/3以下の欠課の場合、「0」を出力
                            }
                        } else if (isOver1) {
                            //授業時数の1/3以上の欠課の場合
                            if (3 >= Integer.parseInt(score)) {
                                //評定が3以下
                                score = "*" + score;
                            } else {
                                //評定が4以上
                    ...
                    } else if (SEMEALL.equals(_param._semester) || ("2".equals(_param._semester) && gradeCd == 3)) {
                        //出力対象 ⇒ 「全学年の9学期」または「3年の2学期」
                        score = "*" + score;
                        if(isOver2) score = "×"; //授業時数の1/2以上の欠課（未履修）の場合、「×」を出力
                    }
                    svf.VrsOutn("SC_DIV" + semesField, line, score);

                    //クラス
                    final String classRemark = (scoreData._classRemarkMap.containsKey(semester)) ? (String) scoreData._classRemarkMap.get(semester) : "";
                    svf.VrsOutn("SC_HR_ABBV" + semesField, line, classRemark);

                }

                //担当者
                final String trNameField = KNJ_EditEdit.getMS932ByteLength(scoreData._staffname) > 10 ? "2" : "1";
                svf.VrsOutn("CHAIR_TR_NAME" + trNameField , line, scoreData._staffname);
    ...
    private void printTitle(final DB2UDB db2, final Vrw32alp svf, final Student student) {
        ...
        //ヘッダ
        final String title = _param._loginYear + "年度" + semesterInfo._semestername + "　成績通知表";
        svf.VrsOut("TITLE", title); //タイトル
        svf.VrsOut("PRINT_DATE", _param._loginDate.replace('-', '/') + "　現在"); //出力日
        ...
        if (student._attendSubClassMap.containsKey(SUBCLASS_REIHAI)) {
            ...
            for (final Iterator it = _param._semesterMap.keySet().iterator(); it.hasNext();) {
                ...
                if (attendSubMap.containsKey(semester)) {
                    ...
                    if (attendance._mourning != null) mourning = mourning.add(attendance._mourning);
                    if (attendance._lesson != null) lesson = lesson.add(attendance._lesson);
                    svf.VrsOut("PRAY_SICK" + semesField, sick.toString()); //欠席
                    svf.VrsOut("PRAY_MOURNING" + semesField, mourning.toString()); //忌引
                    svf.VrsOut("PRAY_LESSON" + semesField, lesson.toString()); //授業時数
                }
            }
        }

        //生徒会/HR役員/係
        VrsOutnRenban(svf, "COMMITTEE", KNJ_EditEdit.get_token(student._committee, 40, 5));

        //所属クラブ
        VrsOutnRenban(svf, "CLUB", KNJ_EditEdit.get_token(student._club, 40, 5));

        //備考
        for (final Iterator it = student._communicationMap.keySet().iterator(); it.hasNext();) {
            final String semester = (String) it.next();
            if ("2".equals(semester) && !_param._semes2Flg) continue;
            if ("3".equals(semester) && !_param._semes3Flg) continue;
            if (SEMEALL.equals(semester)) continue;
            final String remark = (String) student._communicationMap.get(semester);
            VrsOutnRenban(svf, "REMARK" + semester, KNJ_EditEdit.get_token(remark, 70, 7));
        }

        //学校名
        svf.VrsOut("SCHOOL_NAME", "青山学院高等部");
    ...
    private void printTankyu(final DB2UDB db2, final Vrw32alp svf, final Student student, final List subclassList) {
        //総合的な探求の時間 必修
        for (Iterator itSubclass = subclassList.iterator(); itSubclass.hasNext();) {
            final SubclassMst subclassMst = (SubclassMst) itSubclass.next();
            if(SUBCLASS_TANKYU.equals(subclassMst._subclasscd)) {
                svf.VrsOut("SC_SELECT", "1".equals(subclassMst._electdiv) ? "選" : "必"); //必修
            }
        }

        //総合的な探求の時間 単位
        final CREDIT tankyuCredit = _param.getCredit(student, SUBCLASS_TANKYU);
        if (tankyuCredit != null) svf.VrsOut("SC_CREDIT", tankyuCredit._credits);

        //総合的な探求の時間 修得・未修得・未履修
        String spGet = "未履修"; //どちらも空欄の場合
        if (student._printSubclassMap.containsKey(SUBCLASS_TANKYU)) {
            final ScoreData scoreData = (ScoreData) student._printSubclassMap.get(SUBCLASS_TANKYU);
            if (scoreData != null) {
                final String getCredit = StringUtils.defaultString(scoreData.getCredit(SDIV9990008));
                final String compCredit = StringUtils.defaultString(scoreData.compCredit(SDIV9990008));
                if (!"".equals(getCredit) && !"".equals(compCredit)) {
                    //どちらも空欄でない場合
                    spGet = "修得";
                } else if ("".equals(getCredit) && !"".equals(compCredit)) {
                    //GET_CREDITのみ空欄の場合
                    spGet = "未修得";
                }
            }
        }
        svf.VrsOut("SP_GET", spGet); //修得・未修得・未履修
    }
    ...
    private void printAttend(final Vrw32alp svf, final Student student) {
        ...
        for (final Iterator it = _param._semesterMap.keySet().iterator(); it.hasNext();) {
            ...
            int early = 0;
            if (null != att) {
                lesson = att._lesson + att._offdays + att._abroad;
            ...
            svf.VrsOut("SEMESTER_LESSON" + semesField, String.valueOf(lesson)); // 授業日数
            svf.VrsOut("SEMESTER_MOURNING" + semesField, String.valueOf(suspend)); // 出席停止・忌引き等日数
            svf.VrsOut("SEMESTER_MUST" + semesField, String.valueOf(mlesson)); // 出席しなければならない日数
            svf.VrsOut("SEMESTER_SICK" + semesField, String.valueOf(absent)); // 欠席日数
            svf.VrsOut("SEMESTER_LATE" + semesField, String.valueOf(late)); // 遅刻回数
            svf.VrsOut("SEMESTER_EARLY" + semesField, String.valueOf(early)); // 早退回数
        }
    }
    ...
    private List getList(final DB2UDB db2) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                student._attendno = NumberUtils.isDigits(rs.getString("ATTENDNO")) ? "No." + String.valueOf(Integer.parseInt(rs.getString("ATTENDNO"))) : rs.getString("ATTENDNO");
                ...
                student._communicationMap.put("1", StringUtils.defaultString(rs.getString("COMMUNICATION1"))); //備考 1学期
                student._communicationMap.put("2", StringUtils.defaultString(rs.getString("COMMUNICATION2"))); //備考 2学期
                student._communicationMap.put("3", StringUtils.defaultString(rs.getString("COMMUNICATION3"))); //備考 3学期
                student._communicationMap.put(SEMEALL, StringUtils.defaultString(rs.getString("COMMUNICATION9"))); //備考 9学期
    ...
    private String getStudentSql() {
        ...
        stb.append("            ,R1.REMARK1 AS COMMITTEE ");
        stb.append("            ,R1.REMARK2 AS CLUB ");
        stb.append("            ,R1_1.REMARK3 AS COMMUNICATION1 ");
        stb.append("            ,R1_2.REMARK3 AS COMMUNICATION2 ");
        stb.append("            ,R1_3.REMARK3 AS COMMUNICATION3 ");
        stb.append("            ,R1_9.REMARK3 AS COMMUNICATION9 ");
        ...
        stb.append("     LEFT JOIN HREPORTREMARK_DAT R1_1 ON R1_1.YEAR = REGD.YEAR AND R1_1.SEMESTER = '1' AND R1_1.SCHREGNO = REGD.SCHREGNO "); //HREPORTREMARK_DAT 1学期
        stb.append("     LEFT JOIN HREPORTREMARK_DAT R1_2 ON R1_2.YEAR = REGD.YEAR AND R1_2.SEMESTER = '2' AND R1_2.SCHREGNO = REGD.SCHREGNO "); //HREPORTREMARK_DAT 2学期
        stb.append("     LEFT JOIN HREPORTREMARK_DAT R1_3 ON R1_3.YEAR = REGD.YEAR AND R1_3.SEMESTER = '3' AND R1_3.SCHREGNO = REGD.SCHREGNO "); //HREPORTREMARK_DAT 3学期
        stb.append("     LEFT JOIN HREPORTREMARK_DAT R1_9 ON R1_9.YEAR = REGD.YEAR AND R1_9.SEMESTER = '9' AND R1_9.SCHREGNO = REGD.SCHREGNO "); //HREPORTREMARK_DAT 9学期
    ...
    private class Student {
        ...
        String _committee;
        String _club;
        final Map _communicationMap = new TreeMap();
        ...
        private void setSubclass(final DB2UDB db2) {
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    scoreData._classRemarkMap.put(rs.getString("SEMESTER"), StringUtils.defaultString(rs.getString("CLASS_REMARK")));
        ...
        private String prestatementSubclass() {
            ...
            final String[] sdivs = { SDIV1990008, SDIV2990008, SDIV3990008, SDIV9990008 };
            ...
            stb.append("            , B023.NAME1 AS CLASS_REMARK ");
            ...
            stb.append("     LEFT JOIN CHAIR_DETAIL_DAT CD004 ");
            stb.append("            ON CD004.YEAR     = T1.YEAR ");
            stb.append("           AND CD004.SEMESTER = T1.SEMESTER ");
            stb.append("           AND CD004.CHAIRCD  = T5.CHAIRCD ");
            stb.append("           AND CD004.SEQ      = '004' ");
            stb.append("     LEFT JOIN V_NAME_MST B023 ");
            stb.append("            ON B023.YEAR    = CD004.YEAR ");
            stb.append("           AND B023.NAMECD1 = 'B023' ");
            stb.append("           AND B023.NAMECD2 = CD004.REMARK1 ");
            ...
            if (SEMEALL.equals(_param._semester)) {
                stb.append("       AND CSTF.SEMESTER = '" + _param._loginSemester + "' ");
            } else {
            ...
            stb.append("                        WHERE ");
            stb.append("                              T1.YEAR       = '" + _param._loginYear + "' ");
            stb.append("                          AND T1.CLASSCD || '-' || T1.SCHOOL_KIND || '-' || T1.CURRICULUM_CD || '-' || T1.SUBCLASSCD = '" + SUBCLASS_TANKYU + "' ");
            stb.append("                        GROUP BY T1.YEAR, T1.SEMESTER, T1.TESTKINDCD, T1.TESTITEMCD, T1.SCORE_DIV  ");
            stb.append("                     ) T2");
            stb.append("                  ON T2.YEAR       = T1.YEAR ");
            stb.append("                 AND T2.SEMESTER   = T1.SEMESTER ");
            ...
            stb.append("                 AND T2.TESTITEMCD = T1.TESTITEMCD ");
            stb.append("                 AND T2.SCORE_DIV  = T1.SCORE_DIV ");
            stb.append("      WHERE T1.CLASSCD || '-' || T1.SCHOOL_KIND || '-' || T1.CURRICULUM_CD || '-' || T1.SUBCLASSCD = '" + SUBCLASS_TANKYU + "' ");
            stb.append(" ) ");
            stb.append(" SELECT ");
            stb.append("       T1.* ");
    ...
    private static class ScoreData {
        ...
        final Map _classRemarkMap = new HashMap(); // クラス
    ...
    private static class SubclassAttendance {
        ...
        public SubclassAttendance(final BigDecimal lesson, final BigDecimal attend, final BigDecimal sick, final BigDecimal late, final BigDecimal early, final BigDecimal mourning, final BigDecimal offdays, final Double absent) {
            _lesson = lesson;
            _attend = attend;
            _sick = sick;
            _late = late;
...
//                                final BigDecimal offdays = rs.getBigDecimal("OFFDAYS");
                                final SubclassAttendance subclassAttendance = new SubclassAttendance(lesson, attend, sick2, late, early, mourning, null, absent);
    ...
    private static class Param {
        ...
        private Map<String, Map<String, CREDIT>> _creditMstMap;
        ...
        private Map loadSemester(final DB2UDB db2) {
            ...
            try {
                final String sql = "select"
                        + "   SEMESTER,"
                        + "   CASE WHEN SEMESTER = '3' THEN '学年末' ELSE SEMESTERNAME END AS SEMESTERNAME ,"
                        + "   SDATE,"
                        + "   EDATE"
                        + " from"
            ...
            try {
                ...
                while (rs.next()) {
                    if (!_subclassMstMap.containsKey(rs.getString("SUBCLASSCD"))) {
                        final SubclassMst mst = new SubclassMst(
                                                        ...
                                                        new Integer(rs.getInt("CLASS_SHOWORDER3")),
                                                        new Integer(rs.getInt("SUBCLASS_SHOWORDER3")),
                                                        rs.getString("CALCULATE_CREDIT_FLG"),
        ...
        private CREDIT getCredit(final Student student, final String subclasscd) {
            final String regdKey = student._coursecd + student._majorcd + student._grade + student._coursecode;
            final Map<String, CREDIT> subclasscdCreditMap = _creditMstMap.get(regdKey);
            if (null == subclasscdCreditMap) {
                return null;
            }

            final CREDIT credit = subclasscdCreditMap.get(subclasscd);
        ...
        private void setCreditMst(
            ...
            try {
                ...
                sql.append(" SELECT ");
                sql.append("   T1.COURSECD || T1.MAJORCD || T1.GRADE || T1.COURSECODE AS REGD_KEY, ");
                sql.append("   T1.* ");
                ...
                while (rs.next()) {
                    ...
                    final String require_flg = StringUtils.defaultString(rs.getString("REQUIRE_FLG"));
                    final String key = classcd + "-" + school_kind + "-" + curriculum_cd + "-" + subclasscd;
                    CREDIT credit = new CREDIT(year, coursecd, majorcd, grade, coursecode, classcd, school_kind, curriculum_cd, subclasscd, credits, require_flg);
                    _creditMstMap.get(regdKey).put(key, credit);
                }
            } catch (Exception e) {
                log.error("Exception", e);
            } finally {
        ...
        private List getTestKindItemList(DB2UDB db2, final boolean useSubclassControl, final boolean addSemester) {
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    } else if (!SEMEALL.equals(semester._semester) && "08".equals(scoreDiv)) { // 9学期以外の08=9学期以外の仮評定
                        testItem._printKettenFlg = -1;
                    } else {
