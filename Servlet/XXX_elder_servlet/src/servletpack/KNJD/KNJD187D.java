...
/*
 * $Id: 8e632c3ee9a9466330a985640c6d841ed0a7b75f $
 *
 * 作成日: 2020/04/14
 * 作成者: matsushima
 *
...
public class KNJD187D {

    private static final Log log = LogFactory.getLog(KNJD187D.class);
...
//    private static final String HYOTEI_TESTCD = "9990009";
    ...
    private void printSvfMain(final DB2UDB db2, final Vrw32alp svf, final Student student) {
        final String form = "KNJD187D.frm";
        ...
        //明細部
        final List subclassList = printSubClassList(db2,student);
        final List printsubclassList = printsubclassListRemoveD026(subclassList);
        ...
        for (Iterator itSubclass = printsubclassList.iterator(); itSubclass.hasNext();) {
        	final String key = (String)itSubclass.next();
        	if(!_param._subclassMstMap.containsKey(key)) continue;
        	final SubclassMst subclassMst = (SubclassMst) _param._subclassMstMap.get(key);
            final String subclassCd = subclassMst._subclasscd;
        	svf.VrsOutn(subclassMst._subclassname.length() <= 4 ? "SUBCLASS_NAME1" : "SUBCLASS_NAME2",line, subclassMst._subclassname); //科目名

    		//単位数
    		svf.VrsOutn("CREDIT",line, _param.getCredits(student, subclassCd)); //単位数
            ...
            if (student._printSubclassMap.containsKey(subclassCd)) {
            	...
            	//1学期
            	svf.VrsOutn("SCORE1",line, scoreData.score(SDIV1990008)); //成績
            	svf.VrsOutn("VALUE1",line, scoreData.score(SDIV1990009)); //評定

            	//2学期
            	if(_param._semes2Flg) {
            		svf.VrsOutn("SCORE2",line, scoreData.score(SDIV2990008)); //成績
            		svf.VrsOutn("VALUE2",line, scoreData.score(SDIV2990009)); //評定
            	}

            	//3学期
            	if(_param._semes3Flg) {
            		svf.VrsOutn("SCORE3",line, scoreData.score(SDIV3990008)); //成績
            	}

            	//学年末
            	svf.VrsOutn("SCORE9",line, scoreData.score(SDIV9990009)); //評定
                ...
                if (student._attendSubClassMap.containsKey(subclassCd)) {
                    ...
                    for (final Iterator it = _param._semesterMap.keySet().iterator(); it.hasNext();) {
                        ...
                        svf.VrsOutn("NOTICE" + semester,line, sick.toString()); //欠課
                    ...
                    //科目が存在しない場合、'0'を印字
                    svf.VrsOutn("NOTICE1",line, "0"); //欠課
                    if(_param._semes2Flg) svf.VrsOutn("NOTICE2",line, "0"); //欠課
                    if(_param._semes3Flg) svf.VrsOutn("NOTICE3",line, "0"); //欠課
                    svf.VrsOutn("NOTICE9",line, "0"); //欠課
                }
            dataFlg = true;
            line++;
        }
    ...
    private boolean isAddKekkaTotal(final SubclassMst subclassMst, final Collection<String> subclasscdSet) {
    	if (subclassMst.isMoto()) {
    		...
    		return subclasscdSet.contains(subclassMst._combined);
    	}
    	return true;
	}
	...
	private void printTitle(final DB2UDB db2, final Vrw32alp svf, final Student student) {
    	//明細部以外を印字
		svf.VrsOut("TITLE", _param._loginYear + "年度　通知表"); //年度
        ...
        svf.VrsOut("JOB_NAME1", "校長");
        svf.VrsOut("JOB_NAME2", "担任");

        final String prNameField = KNJ_EditEdit.getMS932ByteLength(_param._certifSchoolPrincipalName) > 28 ? "2" : "1";
        svf.VrsOut("PRINCIPAL_NAME1_" + prNameField , _param._certifSchoolPrincipalName); //校長名

        final String trNameField = KNJ_EditEdit.getMS932ByteLength(student._staffname) > 28 ? "2" : "1";
        svf.VrsOut("STAFF_NAME" + trNameField, student._staffname);//担任氏名

        //所見・修了証
        if(_param._semes3Flg) {
        	VrsOutnRenban(svf, "REMARK", KNJ_EditEdit.get_token(student._communication, 40, 5)); //所見
            svf.VrsOut("GRADE", student._gradename);
            svf.VrsOut("DATE", (Integer.parseInt(_param._loginYear) + 1) + "年" + "3月31日"); //西暦修了日付
            svf.VrsOut("SCHOOL_NAME2", _param._certifSchoolSchoolName); //学校名
            svf.VrsOut("JOB_NAME1_2", "校長");
            svf.VrsOut("PRINCIPAL_NAME2_" + prNameField , _param._certifSchoolPrincipalName); //校長名
        } else {
    ...
    private void printTotal(final Vrw32alp svf, final Student student) {
        ...
        if(scoreData != null) {
            //1学期
            svf.VrsOutn("AVE",1, sishaGonyu(scoreData.avg(SDIV1990008))); //総合平均値
            final Rank hrRank1 = (Rank) scoreData._hrRankMap.get(SDIV1990008);
            if(hrRank1 != null) {
                svf.VrsOutn("RANK1", 1, hrRank1._rank + "/" + hrRank1._count); //クラス内順位
            }
            final Rank courseRank1 = (Rank) scoreData._courceRankMap.get(SDIV1990008);
            if(courseRank1 != null) {
                svf.VrsOutn("RANK2", 1, courseRank1._rank + "/" + courseRank1._count); //コース内順位
            }
            //2学期
            if(_param._semes2Flg) {
                svf.VrsOutn("AVE",2, sishaGonyu(scoreData.avg(SDIV2990008))); //総合平均値
                final Rank hrRank2 = (Rank) scoreData._hrRankMap.get(SDIV2990008);
                if(hrRank2 != null) {
                    svf.VrsOutn("RANK1", 2, hrRank2._rank + "/" + hrRank2._count); //クラス内順位
                }
                final Rank courseRank2 = (Rank) scoreData._courceRankMap.get(SDIV2990008);
                if(courseRank2 != null) {
                    svf.VrsOutn("RANK2", 2, courseRank2._rank + "/" + courseRank2._count); //コース内順位
                }
            }

            //3学期
            if(_param._semes3Flg) {
                svf.VrsOutn("AVE",3, sishaGonyu(scoreData.avg(SDIV3990008))); //総合平均値
                final Rank hrRank3 = (Rank) scoreData._hrRankMap.get(SDIV3990008);
                if(hrRank3 != null) {
                    svf.VrsOutn("RANK1", 3, hrRank3._rank + "/" + hrRank3._count); //クラス内順位
                }
                final Rank courseRank3 = (Rank) scoreData._courceRankMap.get(SDIV3990008);
                if(courseRank3 != null) {
                    svf.VrsOutn("RANK2", 3, courseRank3._rank + "/" + courseRank3._count); //コース内順位
                }
            }

            //学年末
            if((!"03".equals(_param._gradeCd) && _param._semes3Flg) || "03".equals(_param._gradeCd)) {
                svf.VrsOutn("AVE",4, sishaGonyu(scoreData.avg(SDIV9990009))); //合計
                final Rank hrRank9 = (Rank) scoreData._hrRankMap.get(SDIV9990009);
                if(hrRank9 != null) {
                    svf.VrsOutn("RANK1", 4, hrRank9._rank + "/" + hrRank9._count); //クラス内順位
                }
                final Rank courseRank9 = (Rank) scoreData._courceRankMap.get(SDIV9990009);
                if(courseRank9 != null) {
                    svf.VrsOutn("RANK2", 4, courseRank9._rank + "/" + courseRank9._count); //コース内順位
                }
            }
        }
    }

    private List printsubclassListRemoveD026(List retList) {
        for (final Iterator it = retList.iterator(); it.hasNext();) {
        	final String key = (String)it.next();
        	if(_param._subclassMstMap.containsKey(key)) {
        		final SubclassMst subclassMst = (SubclassMst) _param._subclassMstMap.get(key);
    ...
    private List printSubClassList(final DB2UDB db2,Student stu) {
    	...
    	stb.append(" WHERE ");
    	stb.append("     T2.YEAR    = '" + _param._loginYear + "'       AND ");
    	stb.append("     T2.GRADE    = '" + stu._grade + "'       AND ");
    	stb.append("     T2.HR_CLASS = '" + stu._hrClass + "'       AND ");
    	stb.append("     T2.SCHREGNO = '" + stu._schregno + "'       AND ");
    	stb.append("     T2.SEMESTER = (SELECT ");
    	stb.append("                         MAX(SEMESTER) ");
    	stb.append("                     FROM ");
    	stb.append("                         SCHREG_REGD_DAT W2 ");
    	stb.append("                     WHERE ");
    	stb.append("                         W2.YEAR = '" + _param._loginYear +"'                            AND ");
    	stb.append("                         W2.SEMESTER <= '" + _param._semester + "'                            AND ");
    	stb.append("                         W2.SCHREGNO = T2.SCHREGNO ");
    	stb.append("                     ) ");
    	stb.append(" ) SELECT ");
    	stb.append("     S2.CLASSCD || '-' || S2.SCHOOL_KIND || '-' || S2.CURRICULUM_CD || '-' ||  SUBCLASSCD AS SUBCLASSCD ");
    	stb.append(" FROM ");
    	stb.append("     CHAIR_STD_DAT S1, ");
    	stb.append("     CHAIR_DAT S2 ");
    	stb.append(" WHERE ");
    	stb.append("     S1.YEAR         = '" + _param._loginYear + "' AND ");
    	stb.append("     S1.SEMESTER <= '" + _param._semester + "'      AND ");
    	stb.append("     S2.YEAR     = S1.YEAR               AND ");
    	stb.append("     S2.SEMESTER <= '" + _param._semester + "'      AND ");
    	stb.append("     S2.SEMESTER = S1.SEMESTER               AND ");
    	stb.append("     S2.CHAIRCD  = S1.CHAIRCD               AND ");
    	stb.append("     EXISTS(SELECT     'X' FROM     SCHNO S3 WHERE     S3.SCHREGNO = S1.SCHREGNO GROUP BY     SCHREGNO ");
    	stb.append(" ) AND SUBCLASSCD <= '89'      AND SUBCLASSCD NOT LIKE '50%'  GROUP BY      S1.SCHREGNO,      S2.CLASSCD,      S2.SCHOOL_KIND,      S2.CURRICULUM_CD,      S2.SUBCLASSCD ");
		...
		try {
			...
			while (rs.next()) {
				retList.add(rs.getString("SUBCLASSCD"));
			}
		} catch (Exception e) {
			log.error("Exception", e);
		} finally {
    ...
    private void printAttend(final Vrw32alp svf, final Student student) {
        for (final Iterator it = _param._semesterMap.keySet().iterator(); it.hasNext();) {
            ...
            svf.VrsOutn("SICK", line, String.valueOf(absent));   // 欠席日数
...
//            stb.append("        (T5.YEAR IS NOT NULL AND T5.VALUE_DI IS NOT NULL) "); // とりあえず RECORD_SCORE_DATにはある科目
    ...
    private static class Rank {
        ...
        public Rank(final String rank, final String avgRank, final String count, final String avg, final String highscore) {
            _rank = StringUtils.defaultString(rank);
            _avgRank = StringUtils.defaultString(avgRank);
            _count = StringUtils.defaultString(count);
            _avg = StringUtils.defaultString(avg);
            _highscore = StringUtils.defaultString(highscore);
        }
    }
    ...
    private static class Attendance {
        ...
        private static void load(
            ...
            log.debug(" attendance = " + dateRange);
        ...
        private static void load(final DB2UDB db2,
            ...
            for (final Iterator it2 = param._attendSemesterDetailList.iterator(); it2.hasNext();) {
                ...
                try {
                    ...
                    log.debug(" subclass attendance sql = " + sql);
                    ps = db2.prepareStatement(sql);
...
//          log.debug(" semesterDetail = " + semesterDetail + " , " + _semesterDetailList.indexOf(semesterDetail));
    ...
    private static class Param {
        ...
        private static String getDbPrginfoProperties(final DB2UDB db2, final String propName) {
            return KnjDbUtils.getOne(KnjDbUtils.query(db2, " SELECT VALUE FROM PRGINFO_PROPERTIES WHERE PROGRAMID = 'KNJD187D' AND NAME = '" + propName + "' "));
        }
        ...
        private String getCredits(final Student student, final String subclasscd) {
        	...
        	if (!subclasscdCreditMap.containsKey(subclasscd)) {
        		log.debug(" no credit_mst : " + subclasscd);
        	}
			return credits;
        }
        ...
        private String getGradeCd(final DB2UDB db2) {
            final StringBuffer sql = new StringBuffer();
            sql.append(" SELECT GRADE_CD FROM SCHREG_REGD_GDAT WHERE YEAR = '" + _loginYear + "' AND GRADE = '" + _grade + "' ");
            return KnjDbUtils.getOne(KnjDbUtils.query(db2, sql.toString()));
        }
