...
/*
 * $Id: d8455c6b38a6008bd16d911afacb901276a487ab $
 *
 * 作成日: 2020/04/14
 * 作成者: matsushima
 *
...
public class KNJD187J {

    private static final Log log = LogFactory.getLog(KNJD187J.class);
    ...
    private static final String KANTENBETSU_KIRIKAE_YEAR = "2021";
    private static final int NEW_VIEW_MAX_LINE = 3;
    private static final int NEW_VIEW_MAX_SUBCLASSCLASS_LINE = 8;
    ...
    private void printMain(final DB2UDB db2, final Vrw32alp svf) {
        ...
        for (Iterator iterator = studentList.iterator(); iterator.hasNext();) {
            ...
            final String form = _param._loginYear.compareTo(KANTENBETSU_KIRIKAE_YEAR) < 0 ?  "KNJD187J.frm" : "KNJD187J_2.frm";
            ...
            //各教科の学習の記録
            if (_param._loginYear.compareTo(KANTENBETSU_KIRIKAE_YEAR) < 0) {
                printSvfViewRecord(svf, student, viewClassList);
            } else {
                printSvfViewRecord2021(svf, student, viewClassList);
            }
    ...
    private void printTitle(final DB2UDB db2, final Vrw32alp svf, final Student student) {
        ...
        //課外活動の記録
        String committee = getSchregCommittee(db2, student._schregno, "1"); //委員会・生徒会
        String club = getSchregClub(db2, student._schregno, "1"); //部活動
        String committeeField = KNJ_EditEdit.getMS932ByteLength(committee) > 32 ? "_2" : "";
        svf.VrsOut("COMMITTEE1" + committeeField, committee);//1学期 学級活動
        svf.VrsOut("CLUB1", club); //1学期 部活動
        if(_param._semes2Flg) {
            committee = getSchregCommittee(db2, student._schregno, "2"); //委員会・生徒会
            club = getSchregClub(db2, student._schregno, "2"); //部活動
            committeeField = KNJ_EditEdit.getMS932ByteLength(committee) > 32 ? "_2" : "";
            svf.VrsOut("COMMITTEE2" + committeeField, committee);//2学期 学級活動
            svf.VrsOut("CLUB2", club); //2学期 部活動
        }
        if(_param._semes3Flg) {
            committee = getSchregCommittee(db2, student._schregno, "3"); //委員会・生徒会
            club = getSchregClub(db2, student._schregno, "3"); //部活動
            committeeField = KNJ_EditEdit.getMS932ByteLength(committee) > 32 ? "_2" : "";
            svf.VrsOut("COMMITTEE3" + committeeField, committee);//3学期 学級活動
            svf.VrsOut("CLUB3", club); //3学期 部活動
        }

        //初見・通信欄
        VrsOutnRenban(svf, "FIELD1", KNJ_EditKinsoku.getTokenList(student._communication, 61, 4));

        //担任氏名
        svf.VrsOut("TR_NAME1", student._staffname);
        ...
        for (final Iterator it = viewClassList.iterator(); it.hasNext();) {
            ...
            for (int i = 0; i < viewClass.getViewSize(); i++) {
                ...
                for (final Iterator itv = viewRecordList.iterator(); itv.hasNext();) {
                    ...
                    if(idx >= VIEW_MAX_LINE) {
                        if(!"".equals(subClassCd) && !subClassCd.equals(viewRecord._subclasscd)) {
                            //教科が変わった場合
                            grp++;
                            idx = 0;
                            if("1".equals(div) && grp > VIEW_MAX_LINE) {
                                div = "2";
                                line = 1;
                            }
                        } else {
                            //教科が変わっていない(観点が5レコードより多い)場合
                            continue;
                        }
                        subClassCd = viewRecord._subclasscd;
                        subClassName = viewRecord._subclassname;
                        maxFlg = true;
                    }
                    ...
                    if(!maxFlg) {
                        if(!"".equals(subClassCd) && !subClassCd.equals(viewRecord._subclasscd)) {
                            //教科が変わった場合
                            while (VIEW_MAX_LINE > idx) {
                                //教科名を最終文字まで出力
                                final String setName = subClassName.length() > idx ? subClassName.substring(idx, idx + 1) : "";
                                svf.VrsOutn("CLASS_NAME" + div, line, setName); // 教科名
                                svf.VrsOutn("VIEW_NAME" + div, line, "");
                                svf.VrsOutn("VIEW_STATUS" + div + "_1", line, "");
                                svf.VrsOutn("VIEW_STATUS" + div + "_2", line, "");
                                svf.VrsOutn("VIEW_STATUS" + div + "_3", line, "");
                                svf.VrsOutn("VIEW_STATUS" + div + "_4", line, "");
                                svf.VrsOutn("GRPCD", line, String.valueOf(grp)); // グループ
                                idx++;
                                line++;
                            }
                            subClassCd = viewRecord._subclasscd;
                    ...
                    if("".equals(subClassName)) subClassName = viewRecord._subclassname;
                    final String setName = subClassName.length() > idx ? subClassName.substring(idx, idx + 1) : "";
                    svf.VrsOutn("CLASS_NAME" + div, line, setName); // 教科名
                    svf.VrsOutn("VIEW_NAME" + div, line, viewRecord._viewname); // 観点
                    svf.VrsOutn("VIEW_STATUS" + div + "_1", line, viewRecord._status1); // 1学期 評価
                    if(_param._semes2Flg) {
                        svf.VrsOutn("VIEW_STATUS" + div + "_2", line, viewRecord._status2); // 2学期 評価
                    }
                    if(_param._semes3Flg) {
                        svf.VrsOutn("VIEW_STATUS" + div + "_3", line, viewRecord._status3); // 3学期 評価
                    }
                    if(_param._semes9Flg) {
                        svf.VrsOutn("VIEW_STATUS" + div + "_4", line, viewRecord._status9); // 9学期 評価
                    }
                    svf.VrsOutn("GRPCD", line, String.valueOf(grp)); // グループ
                    subClassCd = viewRecord._subclasscd;
                    subClassName = viewRecord._subclassname;
    ...
    /**
     * 2021年度以降に使用する観点別学習状況
     * @param svf
     * @param student
     */
    private void printSvfViewRecord2021(final Vrw32alp svf, final Student student, final List viewClassList) {
        int grp = 1; // 教科数をカウント
        int line = 1;
        String div = "3";
        for (final Iterator it = viewClassList.iterator(); it.hasNext();) {
            ...
            if("3".equals(div) && grp > NEW_VIEW_MAX_SUBCLASSCLASS_LINE) {
                div = "4";
                grp = 1;
                line = 1;
            }

            final String subClassName = StringUtils.defaultString(viewClass._subclassname);
            final int subClassNameLength = subClassName.length();
            final String subClassNameFieldName = subClassNameLength > 4 ? "3" : subClassNameLength > 3 ? "2" : "1";
            svf.VrsOutn("CLASS_NAME" + div + "_" + subClassNameFieldName, grp, subClassName); // 教科名

            for (int i = 0; i < NEW_VIEW_MAX_LINE; i++) {
                if (viewClass.getViewSize() <= i) {
                    svf.VrsOutn("VIEW_NAME"   + div,        line, "");
                    svf.VrsOutn("VIEW_STATUS" + div + "_1", line, "");
                    svf.VrsOutn("VIEW_STATUS" + div + "_2", line, "");
                    svf.VrsOutn("VIEW_STATUS" + div + "_3", line, "");
    ...
    private void printScore(final DB2UDB db2, final Vrw32alp svf, final Student student) {
        ...
        for (Iterator itSubclass = subclassList.iterator(); itSubclass.hasNext();) {
            ...
            if (student._printSubclassMap.containsKey(subclassCd)) {
                ...
                //合併元科目の取得
                final List motoSubclassCdList = getAttendSubclass(db2, subclassMst._subclasscd);
                if(motoSubclassCdList.size() > 0) {
                    //合併先科目として登録されている場合、表示しない
                    continue;
                }
                ...
                //学年末評定
                String div9 = scoreData.score(SDIV9990009);
                if(_param._semes9Flg) {
                    if(!"".equals(sakiSubclassCd)) {
                        //合併先科目が取得できた場合、合併先科目の学年末評定を取得
                        div9= getRecordRankSdivDatOne(db2, SDIV9990009, sakiSubclassCd, student._schregno, "SCORE");
                    }
                }
                ...
                //1学期
                svf.VrsOut("VAL1", getKansan(scoreData.score(SDIV1990008))); //評価

                //2学期
                if(_param._semes2Flg) {
                    svf.VrsOut("VAL2", getKansan(scoreData.score(SDIV2990008))); //評価
                }

                //3学期
                if(_param._semes3Flg) {
                    svf.VrsOut("VAL3", getKansan(scoreData.score(SDIV3990008))); //評価
                }

                //学年末
                if(_param._semes9Flg) {
                    svf.VrsOut("DIV9", div9); //評定
                }
    ...
    private String getKansan(final String score) {
        if ("".equals(score)) return score;
        for (final Iterator it = _param._kansanList.iterator(); it.hasNext();) {
            final Kansan kansan = (Kansan) it.next();
            if (Integer.parseInt(kansan._low) <= Integer.parseInt(score) && Integer.parseInt(kansan._high) >= Integer.parseInt(score)) {
                return kansan._level;
            }
        }
        return score;
    }
    ...
    //対象生徒の委員会・生徒会を取得
    private String getSchregCommittee (final DB2UDB db2, final String schregno, final String semester) {
        String rtnStr = "";
        String conect = "";
        final StringBuffer stb = new StringBuffer();
        ...
        stb.append("     AND T1.SEMESTER IN ('" + semester + "', '" + SEMEALL + "') "); //対象学期 + 9学期
    ...
    //合併元科目を取得
    private List getAttendSubclass (final DB2UDB db2, final String subclasscd) {
        final List rtnList = new ArrayList();
        final StringBuffer stb = new StringBuffer();
        stb.append(" SELECT ATTEND_CLASSCD ||'-'|| ATTEND_SCHOOL_KIND ||'-'|| ATTEND_CURRICULUM_CD ||'-'|| ATTEND_SUBCLASSCD AS SUBCLASSCD ");
        stb.append("   FROM SUBCLASS_REPLACE_COMBINED_DAT ");
        stb.append("  WHERE YEAR = '" + _param._loginYear + "' ");
        stb.append("    AND COMBINED_CLASSCD ||'-'|| COMBINED_SCHOOL_KIND ||'-'|| COMBINED_CURRICULUM_CD ||'-'|| COMBINED_SUBCLASSCD = '" + subclasscd + "' ");
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                rtnList.add(StringUtils.defaultString(rs.getString("SUBCLASSCD")));
            }
        } catch (SQLException ex) {
            log.debug("Exception:", ex);
        } finally {
    ...
    //RECORD_RANK_SDIV_DAT(指定項目)を取得
    private String getRecordRankSdivDatOne (final DB2UDB db2, final String testcd, final String subclasscd, final String schregno, final String column) {
        String rtnStr = "";
        final StringBuffer stb = new StringBuffer();
        stb.append(" SELECT * FROM RECORD_RANK_SDIV_DAT ");
        stb.append(" WHERE YEAR = '" + _param._loginYear + "' ");
    ...
    private class Student {
                ...
                while (rs.next()) {
                    ...
                    final String classcd = StringUtils.defaultString(rs.getString("CLASSCD"));
                      ...
                      if("".equals(subclasscd)) {
                          subclasscd = rs.getString("CLASSCD_CHAIR") + "-" + rs.getString("SCHOOL_KIND_CHAIR") + "-" + rs.getString("CURRICULUM_CD_CHAIR") + "-" + rs.getString("SUBCLASSCD_CHAIR");
                      } else if (!"999999".equals(subclasscd)) {
        ...
        private String prestatementSubclass() {
            ...
            final String[] sdivs = {SDIV1990008, SDIV2990008, SDIV3990008, SDIV9990009};
            ...
            stb.append("       T1.* ");
            stb.append("       , T5.CLASSCD AS CLASSCD_CHAIR ");
            stb.append("       , T5.SCHOOL_KIND AS SCHOOL_KIND_CHAIR ");
            stb.append("       , T5.CURRICULUM_CD AS CURRICULUM_CD_CHAIR ");
            stb.append("       , T5.SUBCLASSCD AS SUBCLASSCD_CHAIR ");
            stb.append("     FROM CHAIR_A T5 ");
            stb.append("     INNER JOIN CLASS_MST T3 ");
            stb.append("        ON T3.CLASSCD = SUBSTR(T5.SUBCLASSCD,1,2) ");
            stb.append("       AND T3.SCHOOL_KIND   = T5.SCHOOL_KIND ");
            stb.append("     INNER JOIN SUBCLASS_MST T4 ");
            stb.append("        ON T4.SUBCLASSCD    = T5.SUBCLASSCD ");
            ...
            stb.append("     LEFT JOIN RECORD0 T1 ");
            ...
            stb.append("       T1.* ");
            stb.append("       , CAST(NULL AS VARCHAR(1)) AS CLASSCD_CHAIR ");
            stb.append("       , CAST(NULL AS VARCHAR(1)) AS SCHOOL_KIND_CHAIR ");
            stb.append("       , CAST(NULL AS VARCHAR(1)) AS CURRICULUM_CD_CHAIR ");
            stb.append("       , CAST(NULL AS VARCHAR(1)) AS SUBCLASSCD_CHAIR ");
            ...
            stb.append(" ORDER BY ");
            stb.append("     VALUE(CLASSCD,      CLASSCD_CHAIR), ");
            stb.append("     VALUE(SCHOOL_KIND,  SCHOOL_KIND_CHAIR), ");
            stb.append("     VALUE(CURRICULUM_CD,CURRICULUM_CD_CHAIR), ");
            stb.append("     VALUE(SUBCLASSCD,   SUBCLASSCD_CHAIR) ");
    ...
    private static class ViewRecord {
        ...
        private static String getViewRecordSql(final Param param, final String schregno) {
            ...
            stb.append("     , T1.VIEWNAME ");
            stb.append("     , T1.VIEWCD ");
            stb.append("     , CASE WHEN T3_1.STATUS = 'N' THEN '*' ELSE T3_1.STATUS END AS STATUS1 ");
            stb.append("     , CASE WHEN T3_2.STATUS = 'N' THEN '*' ELSE T3_2.STATUS END AS STATUS2 ");
            stb.append("     , CASE WHEN T3_3.STATUS = 'N' THEN '*' ELSE T3_3.STATUS END AS STATUS3 ");
            stb.append("     , CASE WHEN T3_9.STATUS = 'N' THEN '*' ELSE T3_9.STATUS END AS STATUS9 ");
    ...
    private static class Param {
        ...
        private static String getDbPrginfoProperties(final DB2UDB db2, final String propName) {
            return KnjDbUtils.getOne(KnjDbUtils.query(db2, " SELECT VALUE FROM PRGINFO_PROPERTIES WHERE PROGRAMID = 'KNJD187J' AND NAME = '" + propName + "' "));
        }
