...
public class KNJD126Q {

    private static final Log log = LogFactory.getLog(KNJD126Q.class);
    ...
    private String _notUseJviewScoreText;
    ...
    private void getHeaderData(final DB2UDB db2, Vrw32alp svf) {
        //観点の種類が6種類
        if ("6".equals(_paramKantenHyouji)) {
            svf.VrSetForm("KNJD126Q_2.frm", 1);
        } else {
            svf.VrSetForm("KNJD126Q.frm", 1);
        }
        try {
        ...
        try {
            ...
            for(ResultSet rs = db2.getResultSet(); rs.next();)
                _notUseJviewScoreText = rs.getString("ABBV1");//表示しないコード
    ...
    private void printExam(Vrw32alp svf, ResultSet rs, int i, Map map) {
        try {
            ...
            if (rs.getString("VIEWCD") != null && _viewcdString != null) {
                if (null != _notUseJviewScoreText) {
                    if (!"9".equals((String)map.get(rs.getString("VIEWCD")))) {
                        final String setScore = (_notUseJviewScoreText.equals(rs.getString("SCORE"))) ? "": rs.getString("SCORE");
                        svf.VrsOutn("SCORE" + (String)map.get(rs.getString("VIEWCD")), i + 1, setScore);//観点状況および評定
                    } else {
                        svf.VrsOutn("SCORE" + (String)map.get(rs.getString("VIEWCD")), i + 1, rs.getString("SCORE"));//観点状況および評定
                    }
                } else {
                    svf.VrsOutn("SCORE" + (String)map.get(rs.getString("VIEWCD")), i + 1, rs.getString("SCORE"));//観点状況および評定
                }
    ...
    private String statementMeisai() {
        ...
        try {
            ...
            stb.append("SELECT ");
            stb.append("    SCHREGNO, VIEWCD, SCORE ");
            stb.append("FROM ");
            ...
            stb.append("        TBL2.viewcd, TBL2.SCORE ");
