...
/*
 * $Id: 7c809e58ec956ad81c0fcad812bb0d8c3bb7bd05 $
 *
 * 作成日: 2018/08/27
 * 作成者: yogi
 *
...
public class KNJD659F {
    ...
    private void printMain(final DB2UDB db2, final Vrw32alp svf) {
        final String form;
        int colcntmax;
        if ("1".equals(_param._gradeCd) || "2".equals(_param._gradeCd)) {
        	form = "KNJD659F_1.frm";
        	colcntmax = 19;
        } else if ("3".equals(_param._gradeCd) || "4".equals(_param._gradeCd)) {
        	form = "KNJD659F_2.frm";
        	colcntmax = 12;
        } else if ("5".equals(_param._gradeCd) || "6".equals(_param._gradeCd)) {
        	form = "KNJD659F_3.frm";
        	colcntmax = 10;
        } else {
        	form = "";
        ...
        for (int lpcnt = 0;lpcnt < _param._grhr_ClassSelected.length;lpcnt++) {
            ...
            for (int pi = 0; pi < studentList.size(); pi++) {
                ...
                //観点毎にループ
                int colcnt = 1;
                for (Iterator ite = _param._behaviorSemsMstMap.keySet().iterator();ite.hasNext();) {
                    final String kcode = (String)ite.next();
                    //取得できれば出力処理を行う
                    final String recval = (String)student._recordMap.get(kcode);
                    if (recval != null) {
                        svf.VrsOutn("RECORD" + colcnt, rowcnt+1, StringUtils.defaultString((String) _param._cdNameMap.get(recval)));
                    }
                    colcnt++;
                }
                svf.VrEndRecord();
    ...
    private void setTitle(final DB2UDB db2, final Vrw32alp svf, final String hrsel, final int colcntmax) {
    	//svf.VrsOut("", )); //年度
    	//svf.VrsOut("", ); //学期
    	svf.VrsOut("TITLE", KNJ_EditDate.gengou(db2, Integer.parseInt(_param._year)) + "年度　" + _param._semester + "学期　行動の記録"); //年度+学期+タイトル
        ...
        for (Iterator ite = _param._behaviorSemsMstMap.keySet().iterator();ite.hasNext();) {
            ...
            final String kname = (String)_param._behaviorSemsMstMap.get(kcode);
            final int knamelen = KNJ_EditEdit.getMS932ByteLength(kname);
            final String kanmefield;
            if ("1".equals(_param._gradeCd) || "2".equals(_param._gradeCd)) {
            	kanmefield = knamelen > 17 ? "2" : "1";
            } else if ("3".equals(_param._gradeCd) || "4".equals(_param._gradeCd)) {
            	kanmefield = knamelen > 20 ? "2" : "1";
            } else if ("5".equals(_param._gradeCd) || "6".equals(_param._gradeCd)) {
            	kanmefield = "1";
            } else {
            	kanmefield = "";
    ...
    private static class Student {
        ...
        public static String sql(final Param param, final String hrsel) {
            ...
            stb.append("   LEFT JOIN BEHAVIOR_SEMES_DAT BSD ");
            stb.append("      ON REGD.YEAR = BSD.YEAR ");
            stb.append("     AND REGD.SEMESTER = BSD.SEMESTER ");
...
//            _documentroot = request.getParameter("DOCUMENTROOT");
            ...
            _behaviorSemsMstMap = getbehaviorSemsMst(db2);
            _hrInfoMap = getHrInfo(db2);
            loadSemester(db2, _year);
            _cdNameMap = getMapNameMst(db2, "NAME1", "NAMESPARE1", "D036");
        }
...
//        }
        ...
        private Map getbehaviorSemsMst(final DB2UDB db2) {
            Map rtnMap = new LinkedMap();
            PreparedStatement ps = null;
            ResultSet rs = null;
            try {
                final String sql = "SELECT CODE, VIEWNAME FROM BEHAVIOR_SEMES_MST WHERE YEAR = '" + _year + "' AND GRADE = '" + _grade + "' ";
                ps = db2.prepareStatement(sql);
                rs = ps.executeQuery();
                while (rs.next()) {
                	rtnMap.put(rs.getString("CODE"), rs.getString("VIEWNAME"));
                }
            } catch (Exception e) {
                log.fatal("exception!", e);
            } finally {
        ...
        private Map getMapNameMst(final DB2UDB db2, final String field1, final String field2, final String namecd1) {
            Map rtn = new HashMap();
            PreparedStatement ps = null;
            ResultSet rs = null;
            try {
                ps = db2.prepareStatement(" SELECT " + field1 + ", " + field2 + " FROM NAME_MST WHERE NAMECD1 = '" + namecd1 + "' ");
                rs = ps.executeQuery();
                while (rs.next()) {
                    rtn.put(rs.getString(field1), rs.getString(field2));
                }
            } catch (Exception e) {
                log.error("exception!", e);
            }
