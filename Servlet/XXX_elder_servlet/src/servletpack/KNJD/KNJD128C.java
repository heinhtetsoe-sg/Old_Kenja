/**
 *
 *    学校教育システム 賢者 [成績管理] 成績伝票（考査：１学期２回、２学期４回）
 *
 *    2010/04/17 o-naka KNJD128Bを元に新規作成
 */
...
public class KNJD128C {

    private static final Log log = LogFactory.getLog(KNJD128C.class);
                     ...
                     throws ServletException, IOException
    {
        ...
        try {
            log.fatal("$Revision: 70648 $"); // CVSキーワードの取り扱いに注意
    ...
    private void setHead(DB2UDB db2,Vrw32alp svf,String param[]) {
        ...
        final String form = "1".equals(param[21]) ? "KNJD128C_P.frm" : "KNJD128C.frm";
        ...
        try {
            ...
            log.fatal("absentCov = " + _knjSchoolMst._absentCov + ", absentCovLate = " + _knjSchoolMst._absentCovLate);
        } catch( Exception ex ){
    ...
    private String getTestField(String test) {
        ...
        if ("20102".equals(test)) return "TESTNAME2_2";
        if ("20201".equals(test)) return "TESTNAME2_3";
        if ("20202".equals(test)) return "TESTNAME2_4";
        return "DUMMY";
    }
    ...
    private boolean printSvfOutMeisai( Vrw32alp svf, ResultSet rs, String param[], int total[][], Map attendmap, int linex ){
        ...
        try {
            ...
            printSvfOutDetal( svf, "AVERAGE7" + fieldNo,   (rs.getString("SEM1_VALUE7_DI") != null) ? rs.getString("SEM1_VALUE7_DI") : rs.getString("SEM1_VALUE7"),      null );
            ...
            printSvfOutDetal( svf, "POINT2_2" + fieldNo,   rs.getString("SEM2_INTR2_SCORE"), getTestAttend( rs.getString("SEM2_INTR2_ATTEND"), attendmap ) );
            printSvfOutDetal( svf, "AVERAGE12" + fieldNo,   (rs.getString("SEM2_VALUE12_DI") != null) ? rs.getString("SEM2_VALUE12_DI") : rs.getString("SEM2_VALUE12"),      null );
            printSvfOutDetal( svf, "POINT2_3" + fieldNo,   rs.getString("SEM2_TERM_SCORE"), getTestAttend( rs.getString("SEM2_TERM_ATTEND"), attendmap ) );
            printSvfOutDetal( svf, "POINT2_4" + fieldNo,   rs.getString("SEM2_TERM2_SCORE"), getTestAttend( rs.getString("SEM2_TERM2_ATTEND"), attendmap ) );
            ...
            accumMeisai( total, i++, rs.getString("SEM1_VALUE7") );
            accumMeisai( total, i++, rs.getString("SEM1_VALUE") );
            accumMeisai( total, i++, rs.getString("SEM2_INTR_SCORE") );
            accumMeisai( total, i++, rs.getString("SEM2_INTR2_SCORE") );
            accumMeisai( total, i++, rs.getString("SEM2_VALUE12") );
    ...
    private void printSvfOutTotal( Vrw32alp svf, int total[][] )
    {
        try {
            ...
            String fname[] = { "TOTAL1_1", 
                               "TOTAL1_2", "TOTAL_AVERAGE7", "TOTAL_AVERAGE1",
                               "TOTAL2_1", 
                               "TOTAL2_2", "TOTAL_AVERAGE12",
                               "TOTAL2_3", 
                               "TOTAL2_4", 
                               "TOTAL_AVERAGE4" };
    ...
    String prestatementRecord(String param[],int pdiv) {
        ...
        try {
            ...
            stb.append(    "FROM    SCH_CHR_TEST T1,ATTEND_DAT T2, ATTEND_DI_CD_DAT T3 ");
            stb.append(    "WHERE   T1.YEAR = '" + param[0] + "' AND ");
            stb.append(            "T1.SEMESTER = '2' AND T1.TESTKINDCD = '01' AND T1.TESTITEMCD = '02' AND ");
            stb.append(            "T1.CHAIRCD = '" + param[2] + "' AND ");
            stb.append(            "T3.YEAR = '" + param[0] + "' AND ");
            stb.append(            "T3.DI_CD = T2.DI_CD AND ");
            stb.append(            "T2.YEAR = '" + param[0] + "' AND ");
            ...
            stb.append(            "T3.REP_DI_CD IN('1','2','3','4','5','6','8','9','10','11','12','13','14','29','30','31') ");
            stb.append(    "UNION ");
            stb.append(    "SELECT  '2' AS SEMES, '04' AS TESTKINDCD, T2.SCHREGNO, ");
            stb.append(            "T2.DI_CD ");
            stb.append(    "FROM    SCH_CHR_TEST T1,ATTEND_DAT T2, ATTEND_DI_CD_DAT T3 ");
            stb.append(    "WHERE   T1.YEAR = '" + param[0] + "' AND ");
            ...
            stb.append(            "MIN(CASE WHEN SEMES = '2' AND TESTKINDCD = '02' THEN INT(DI_CD) END) AS SEM2_INTR2_ATTEND, ");
            stb.append(            "MIN(CASE WHEN SEMES = '2' AND TESTKINDCD = '03' THEN INT(DI_CD) END) AS SEM2_TERM_ATTEND, ");
            stb.append(            "MIN(CASE WHEN SEMES = '2' AND TESTKINDCD = '04' THEN INT(DI_CD) END) AS SEM2_TERM2_ATTEND ");
            ...
            stb.append("             ,min(CASE WHEN SEMESTER||TESTKINDCD||TESTITEMCD||SCORE_DIV = '2010201' THEN SCORE END) as SEM2_INTR2_SCORE ");
            stb.append("             ,min(CASE WHEN SEMESTER||TESTKINDCD||TESTITEMCD||SCORE_DIV = '2020101' THEN SCORE END) as SEM2_TERM_SCORE ");
            stb.append("             ,min(CASE WHEN SEMESTER||TESTKINDCD||TESTITEMCD||SCORE_DIV = '2020201' THEN SCORE END) as SEM2_TERM2_SCORE ");
            ...
            stb.append("             ,min(CASE WHEN SEMESTER||TESTKINDCD||TESTITEMCD||SCORE_DIV = '1990100' THEN VALUE END) as SEM1_VALUE7 ");
            ...
            stb.append("             ,min(CASE WHEN SEMESTER||TESTKINDCD||TESTITEMCD||SCORE_DIV = '2010201' THEN VALUE END) as SEM2_INTR2_VALUE ");
            stb.append("             ,min(CASE WHEN SEMESTER||TESTKINDCD||TESTITEMCD||SCORE_DIV = '2990100' THEN VALUE END) as SEM2_VALUE12 ");
            stb.append("             ,min(CASE WHEN SEMESTER||TESTKINDCD||TESTITEMCD||SCORE_DIV = '2020101' THEN VALUE END) as SEM2_TERM_VALUE ");
            stb.append("             ,min(CASE WHEN SEMESTER||TESTKINDCD||TESTITEMCD||SCORE_DIV = '2020201' THEN VALUE END) as SEM2_TERM2_VALUE ");
            ...
            stb.append("             ,min(CASE WHEN SEMESTER||TESTKINDCD||TESTITEMCD||SCORE_DIV = '1990100' THEN VALUE_DI END) as SEM1_VALUE7_DI ");
            ...
            stb.append("             ,min(CASE WHEN SEMESTER||TESTKINDCD||TESTITEMCD||SCORE_DIV = '2010201' THEN VALUE_DI END) as SEM2_INTR2_VALUE_DI ");
            stb.append("             ,min(CASE WHEN SEMESTER||TESTKINDCD||TESTITEMCD||SCORE_DIV = '2990100' THEN VALUE_DI END) as SEM2_VALUE12_DI ");
            stb.append("             ,min(CASE WHEN SEMESTER||TESTKINDCD||TESTITEMCD||SCORE_DIV = '2020101' THEN VALUE_DI END) as SEM2_TERM_VALUE_DI ");
            stb.append("             ,min(CASE WHEN SEMESTER||TESTKINDCD||TESTITEMCD||SCORE_DIV = '2020201' THEN VALUE_DI END) as SEM2_TERM2_VALUE_DI ");
            ...
            stb.append(            "T2.SEM2_INTR2_ATTEND, T2.SEM2_TERM2_ATTEND, ");
            stb.append(            "SEM1_INTR_VALUE_DI, SEM1_TERM_VALUE_DI, ");
            stb.append(            "SEM1_VALUE_DI, SEM1_VALUE7_DI, ");
            stb.append(            "SEM2_VALUE_DI, SEM2_VALUE12_DI, ");
            stb.append(            "SEM2_INTR_VALUE_DI, SEM2_TERM_VALUE_DI, ");
            stb.append(            "SEM2_INTR2_VALUE_DI, SEM2_TERM2_VALUE_DI, SEM3_VALUE_DI, ");
            ...
            stb.append(        "SEM2_VALUE12, ");
            stb.append(        "SEM2_VALUE, ");
            stb.append(        "SEM2_INTR2_SCORE, SEM2_INTR2_VALUE, ");
            ...
            stb.append(        "SEM2_INTR2_ATTEND, SEM2_TERM2_ATTEND, ");
            stb.append(        "SEM1_INTR_VALUE_DI, SEM1_TERM_VALUE_DI, ");
            stb.append(        "SEM1_VALUE_DI, SEM1_VALUE7_DI, ");
            stb.append(        "SEM2_VALUE_DI, SEM2_VALUE12_DI, ");
            stb.append(        "SEM2_INTR_VALUE_DI, SEM2_TERM_VALUE_DI, ");
