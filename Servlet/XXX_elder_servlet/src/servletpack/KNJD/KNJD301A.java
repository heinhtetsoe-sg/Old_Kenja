// kanji=漢字
/*
 * $Id: 8b7848f9d76080bbca39c782a810e65e60c730f2 $
 *
 * 作成日: 2007/05/14
 * 作成者: nakamoto
 *
 ...
 * @author nakamoto
 * @version $Id: 8b7848f9d76080bbca39c782a810e65e60c730f2 $
 */
public class KNJD301A {
    private static final Log log = LogFactory.getLog(KNJD301A.class);
    ...
    public void printMain(
        ...
        Map bkupReportInfo = new HashMap();
        _hasData = false;
        ////学年別クラス別クラブ登録生徒リスト作成
        Map clubinfo = createClubInfo(db2, param);
        setStaffName(db2, param, clubinfo);

        if (param._dsMap.size() == 0) {
            loadAllScoreDetail(db2, param);
        }
        ...
        for (int h = 0; h < param._classSelected.length; h++) { //印刷対象HR組
            ...
            param._grade = StringUtils.substring(hrInfo._hrclassCd,  0, 2);
            hrInfo._courses = courses;
            hrInfo.load(db2, param, null); //帳票に出力する情報を作成する。KNJD615Vと同じ処理を実施。
            if (hrInfo._students.size() > 0) {
                _hasData = true;
            }
            //作成データを保持する。
            bkupReportInfo.put(param._classSelected[h], hrInfo);
        }
        if (_hasData) {
            // 印刷処理
            printSummary(db2, ipdf, param, bkupReportInfo, clubinfo);
        }
    }

    private void loadAllScoreDetail(
            final DB2UDB db2,
            final Param param
    ) {
        ...
        //Map subclsMap = new HashMap();

        if (param._schreglist != null) {
            for (int ii = 0;ii < param._classSelected.length;ii++) {
                String ghr_key = (String)param._classSelected[ii];
                final List chkschreglist = (List)param._schreglist.get(ghr_key);
                if (chkschreglist != null) {
                    for (int jj = 0;jj < chkschreglist.size();jj++) {
                        schregfind += sep + "'" + chkschreglist.get(jj) + "'";
                        sep = ",";
                    }
                }
            }
        ...
        try {
            final String sql = sqlStdSubclassDetail(param, schregfind);
            ...
            while (rs.next()) {
                ...
                Map arrangewk = (Map)param._subclsMap.get(rs.getString("GRADE") + rs.getString("HR_CLASS"));
                if (null == arrangewk) {
                    arrangewk = new HashMap();
                    param._subclsMap.put(rs.getString("GRADE") + rs.getString("HR_CLASS"), arrangewk);
                }
                if (classCd.compareTo(KNJDefineSchool.subject_U) <= 0 || classCd.equals(KNJDefineSchool.subject_T)) {
                    final ScoreDetail scoreDetail = new ScoreDetail(
                            ...
                            getSubClass(rs, arrangewk),
                            rs.getString("SCORE"),
                            rs.getString("SCORE_DI"),
                            "", //rs.getString("ASSESS_LEVEL"), //☆彡
                            "", //rs.getString("KARI_HYOUTEI"), //☆彡
                            "", //rs.getString("PROV_FLG"), //☆彡
                    ...
                    getwk = (List)param._dsMap.get(key);
                    if (null == getwk) {
                        getwk = new ArrayList();
                        param._dsMap.put(key, getwk);
                    }
                    getwk.add(scoreDetail);
                }
            }
        } catch (Exception e) {
    ...
    private SubClass getSubClass(
        ...
        try {
            ...
            if (rs.getString("PERFECT") != null) { perfect = rs.getInt("PERFECT"); }
        } catch (Exception e) {
             log.error("Exception", e);
        }
        //科目クラスのインスタンスを更新して返す
        ...
        if (!subclasses.containsKey(subclasscode)) {
            ...
            subclass = new SubClass(subclasscode, classabbv, subclassabbv, subclassname, electdiv, credit,  perfect, staffname);
     ...
     */
    private String sqlStdSubclassDetail(final Param param, final String schregnos) {
        final StringBuffer stb = new StringBuffer();

        stb.append("WITH ");

        //対象生徒の表 クラスの生徒(対象生徒を一括取得する)
...
//        stb.append("         AND W1.GRADE = ? AND W1.HR_CLASS = ? ");
        stb.append("         AND W1.SCHREGNO IN (" + schregnos + ") ");
        stb.append(") ");
        ...
        if ("1".equals(param._useCurriculumcd)) {
            ...
            stb.append(" W2.CLASSCD, W2.SCHOOL_KIND, W2.SUBCLASSCD, ");
        }
        stb.append("            W2.SUBCLASSCD AS ALL_SUBCLASSCD, W2.SEMESTER, W1.APPDATE, W1.APPENDDATE ");
        ...
        stb.append("            SUBCLASSCD AS ALL_SUBCLASSCD, CREDITS ");
        ...
        stb.append("        AND EXISTS(SELECT 'X' FROM CHAIR_A T3 WHERE T3.ALL_SUBCLASSCD = ");
        if ("1".equals(param._useCurriculumcd)) {
        ...
        stb.append("    SELECT  T1.SCHREGNO, T1.ALL_SUBCLASSCD, T1.CREDITS");
        ...
        stb.append("                              T2.COMBINED_SUBCLASSCD = T1.ALL_SUBCLASSCD)");
        stb.append("    UNION SELECT T1.SCHREGNO, ");
        if ("1".equals(param._useCurriculumcd)) {
            stb.append("            T2.COMBINED_CLASSCD || '-' || T2.COMBINED_SCHOOL_KIND || '-' || T2.COMBINED_CURRICULUM_CD || '-' || ");
        }
        stb.append("                 T2.COMBINED_SUBCLASSCD AS ALL_SUBCLASSCD, SUM(T1.CREDITS) AS CREDITS");
        ...
        stb.append("            T2.ATTEND_SUBCLASSCD = T1.ALL_SUBCLASSCD");
        ...
        stb.append("   SELECT GRADE, SUBCLASSCD");
        stb.append("     , CLASSCD ");
        stb.append("     , SCHOOL_KIND ");
        stb.append("     , CURRICULUM_CD ");
        stb.append("     , COUNT(*) AS COUNT ");
        stb.append("          FROM RELATIVEASSESS_MST ");
        //stb.append("          WHERE GRADE = ? AND ASSESSCD = '3' ");
        stb.append("          WHERE ASSESSCD = '3' ");
        stb.append("   GROUP BY GRADE, SUBCLASSCD");
        stb.append("     , CLASSCD ");
        stb.append("     , SCHOOL_KIND ");
        stb.append("     , CURRICULUM_CD ");
        stb.append("   ) ");
        ...
        stb.append("     W3.SUBCLASSCD AS ALL_SUBCLASSCD ");
        stb.append("     , W3.SCORE ");
        stb.append("     , CASE WHEN VALUE(T2.COUNT, 0) > 0 THEN ");
        stb.append("          (SELECT MAX(L3.ASSESSLEVEL) ");
        stb.append("           FROM RELATIVEASSESS_MST L3 ");
//        stb.append("           WHERE L3.GRADE = ? AND L3.ASSESSCD = '3' ");
        stb.append("           WHERE L3.GRADE = T2.GRADE AND L3.ASSESSCD = '3' ");
        ...
        if ("TESTITEM_MST_COUNTFLG_NEW".equals(param._useTestCountflg)) {
        ...
        stb.append("        LEFT JOIN REL_COUNT T2 ON T2.GRADE = W1.GRADE AND T2.SUBCLASSCD = W3.SUBCLASSCD ");
        if ("1".equals(param._useCurriculumcd)) {
        ...
        stb.append("            W3.TESTKINDCD = '" + param._testKindCd.substring(0, 2) + "' AND ");
        stb.append("            W3.TESTITEMCD = '" + param._testKindCd.substring(2, 4) + "' ");
        if (!"TESTITEM_MST_COUNTFLG_NEW".equals(param._useTestCountflg)) {
            stb.append("        AND W3.SCORE_DIV = '" + param._testKindCd.substring(4) + "' ");
        }
        stb.append("     ) ");
        ...
        stb.append("    SELECT  W3.SCHREGNO, ");
        stb.append("            W3.CLASSCD || '-' || W3.SCHOOL_KIND || '-' || W3.CURRICULUM_CD || '-' || W3.SUBCLASSCD AS ALL_SUBCLASSCD ");
        ...
        stb.append("            W3.SEMESTER = '" + param._semester + "' AND ");
        stb.append("            W3.TESTKINDCD = '" + param._testKindCd.substring(0, 2) + "' AND ");
        stb.append("            W3.TESTITEMCD = '" + param._testKindCd.substring(2, 4) + "' AND ");
        if ("TESTITEM_MST_COUNTFLG_NEW".equals(param._useTestCountflg)) {
            String scoreDiv = !"99".equals(param._testKindCd.substring(0, 2)) ? "01" : "00";
            stb.append("            W3.SCORE_DIV = '" + scoreDiv + "' AND ");
        } else {
            stb.append("            W3.SCORE_DIV = '" + param._testKindCd.substring(4) + "' AND ");
        }
        stb.append("            EXISTS(SELECT 'X' FROM SCHNO_A W1 ");
        stb.append("                   WHERE W3.SCHREGNO = W1.SCHREGNO AND W1.LEAVE = 0) ");
        //stb.append("         AND W3.SCHREGNO IN (?) ");
        stb.append("         AND W3.SCHREGNO IN ("+schregnos+") ");
        stb.append("     ) ");
...
//        stb.append("     ) ");
        ...
        if ("TESTITEM_MST_COUNTFLG_NEW".equals(param._useTestCountflg)) {
            ...
            stb.append("            W3.SUBCLASSCD AS ALL_SUBCLASSCD, ");
            stb.append("            W3.SLUMP AS SLUMP, ");
            stb.append("            NULL AS SLUMP_MARK, ");
            stb.append("            NULL AS SLUMP_SCORE ");
            stb.append("    FROM    RECORD_SLUMP_DAT W3 ");
            stb.append("    INNER JOIN SCHNO_A W4 ON W3.SCHREGNO = W4.SCHREGNO ");
            stb.append("       AND W4.LEAVE = 0 ");
            ...
            stb.append("        LEFT JOIN REL_COUNT T2 ON T2.GRADE = W4.GRADE AND T2.SUBCLASSCD = W3.SUBCLASSCD ");
            if ("1".equals(param._useCurriculumcd)) {
                stb.append("     AND T2.CLASSCD = W3.CLASSCD ");
                stb.append("     AND T2.SCHOOL_KIND = W3.SCHOOL_KIND ");
                stb.append("     AND T2.CURRICULUM_CD = W3.CURRICULUM_CD ");
            ...
            stb.append("           FROM RELATIVEASSESS_MST L3 ");
            //stb.append("           WHERE L3.GRADE = ? AND L3.ASSESSCD = '3' ");
            stb.append("           WHERE L3.GRADE = W4.GRADE AND L3.ASSESSCD = '3' ");
            stb.append("             AND W3.SCORE BETWEEN L3.ASSESSLOW AND L3.ASSESSHIGH  ");
            stb.append("             AND L3.SUBCLASSCD = W3.SUBCLASSCD ");
            if ("1".equals(param._useCurriculumcd)) {
                stb.append("     AND L3.CLASSCD = W3.CLASSCD ");
        ...
        if ("1".equals(param._useCurriculumcd)) {
            ...
            stb.append(" W2.CLASSCD, W2.SCHOOL_KIND, ");
        }
        stb.append("             W2.SUBCLASSCD, ");
        stb.append("             W2.ALL_SUBCLASSCD, ");
        ...
        stb.append("           COMBINED_SUBCLASSCD AS ALL_SUBCLASSCD");
        ...
        stb.append("           ATTEND_SUBCLASSCD AS ALL_SUBCLASSCD, MAX(PRINT_FLG2) AS PRINT_FLG");
        stb.append("    FROM   SUBCLASS_REPLACE_COMBINED_DAT");
        stb.append("    WHERE  YEAR = '" + param._year + "'");
        stb.append("    GROUP BY ");
        if ("1".equals(param._useCurriculumcd)) {
        ...
        //メイン表
        stb.append(" SELECT  value(SUBM.ELECTDIV,'0') || T1.ALL_SUBCLASSCD AS SUBCLASSCD,T1.SCHREGNO ");
        stb.append("        ,T3.SCORE AS SCORE ");
        stb.append("        ,SCH.GRADE ");
        stb.append("        ,SCH.HR_CLASS ");
...
//        stb.append("        ,KARIHYO.PROV_FLG ");
        ...
        stb.append("        ,CASE WHEN T9.ALL_SUBCLASSCD IS NOT NULL THEN -1");
        stb.append("              WHEN T10.ALL_SUBCLASSCD IS NOT NULL THEN 1 ELSE 0 END AS REPLACEMOTO ");
        ...
        stb.append("   ON T3.ALL_SUBCLASSCD = T1.ALL_SUBCLASSCD ");
        stb.append("   AND T3.SCHREGNO = T1.SCHREGNO");
        stb.append(" LEFT JOIN RECORD_SCORE T33 ");
        stb.append("   ON T33.ALL_SUBCLASSCD = T1.ALL_SUBCLASSCD ");
        stb.append("   AND T33.SCHREGNO = T1.SCHREGNO");
//        stb.append(" LEFT JOIN RECORD_KARI_HYOUTEI KARIHYO ON KARIHYO.SUBCLASSCD = T1.SUBCLASSCD AND KARIHYO.SCHREGNO = T1.SCHREGNO");
        //合併先科目の表
        stb.append(" LEFT JOIN COMBINED_SUBCLASS T9 ");
        stb.append("   ON T9.ALL_SUBCLASSCD =  T1.ALL_SUBCLASSCD ");
        //合併元科目の表
        stb.append(" LEFT JOIN ATTEND_SUBCLASS T10 ");
        stb.append("   ON T10.ALL_SUBCLASSCD = T1.ALL_SUBCLASSCD ");
        stb.append(" LEFT JOIN CREDITS_B T11 ON T11.SCHREGNO = T1.SCHREGNO");
        stb.append("   AND T11.ALL_SUBCLASSCD = T1.ALL_SUBCLASSCD ");
        stb.append(" LEFT JOIN SUBCLASS_MST SUBM ON SUBM.CLASSCD || '-' || SUBM.SCHOOL_KIND || '-' || SUBM.CURRICULUM_CD || '-' || SUBM.SUBCLASSCD = T1.ALL_SUBCLASSCD");
        stb.append(" LEFT JOIN CLASS_MST CM ON CM.CLASSCD || '-' || CM.SCHOOL_KIND = T1.CLASSCD || '-' || T1.SCHOOL_KIND ");
        //成績不振科目データの表
        stb.append(" LEFT JOIN RECORD_SLUMP K1 ON K1.SCHREGNO = T1.SCHREGNO ");
        stb.append("   AND K1.ALL_SUBCLASSCD = T1.ALL_SUBCLASSCD");
        ...
        stb.append("     AND W4.TESTKINDCD = '" + param._testKindCd.substring(0, 2) + "' ");
        stb.append("     AND W4.TESTITEMCD = '" + param._testKindCd.substring(2, 4) + "' ");
        if ("TESTITEM_MST_COUNTFLG_NEW".equals(param._useTestCountflg)) {
            String scoreDiv = !"99".equals(param._testKindCd.substring(0, 2)) ? "01" : "00";
            stb.append("     AND W4.SCORE_DIV = '" + scoreDiv + "' ");
        } else {
            stb.append("     AND W4.SCORE_DIV = '" + param._testKindCd.substring(4) + "' ");
        }
        stb.append("     AND W4.CLASSCD || '-' || W4.SCHOOL_KIND || '-' || W4.CURRICULUM_CD || '-' || W4.SUBCLASSCD = T1.ALL_SUBCLASSCD AND W4.SCHREGNO = T1.SCHREGNO ");
        stb.append(" LEFT JOIN PERFECT_RECORD_DAT PERF ON PERF.YEAR = '" + param._year + "' AND PERF.SEMESTER = '" + param._semester + "' AND PERF.TESTKINDCD = '" + param._testKindCd.substring(0, 2) + "'  AND PERF.TESTITEMCD = '" + param._testKindCd.substring(2, 4) + "' ");
        stb.append("     AND PERF.CLASSCD || '-' || PERF.SCHOOL_KIND || '-' || PERF.CURRICULUM_CD || '-' || PERF.SUBCLASSCD = T1.ALL_SUBCLASSCD ");
        ...
        stb.append(" ORDER BY SCH.GRADE, SCH.HR_CLASS, T1.ALL_SUBCLASSCD, T1.SCHREGNO");

        return stb.toString();
    }

    private Map createClubInfo(final DB2UDB db2, final Param param) {
    	int maxreccnt = "J".equals(param._schoolkind) ? 45 : 25;
        Map retmap = new LinkedMap();
        List pagelist = null;
        List memberlist = null;
        List paramgradelist = new ArrayList();
        final String sql =  getMemberInfosql(param);
        List schreglst = new ArrayList();
        //log.debug("sql:"+sql);
        ...
        try {
            ...
            while (rs.next()) {
                ...
                final String clss = rs.getString("HR_CLASS");
                ...
                final String majorcode = rs.getString("MAJORCODE");
                final String key = grade + clss;
                //メンバリスト切り分け条件(CLUBコード違い/ページ一杯)
                if ( ("".equals(clubcdbk) || !clubcdbk.equals(clubcd))
                        || reccnt > maxreccnt) {
                    reccnt = 1;
                    //クラブ違いならクラスを作成
                    if (("".equals(clubcdbk) || !clubcdbk.equals(clubcd))) {
                        pagelist = new ArrayList();
                        ClubInfo addwk = new ClubInfo(clubcd, pagelist);
                        retmap.put(clubcd, addwk);
                    }

                    memberlist = new ArrayList();
                    pagelist.add(memberlist);
                } else {
                    pagelist = (List)((ClubInfo)retmap.get(clubcd))._pagelist;
                    memberlist = (List)pagelist.get(pagelist.size() - 1);
                }
                MemberInfo memberinf = new MemberInfo(schregno, grade, clss, hrname, attno, sex, name, coursecode, majorcode);
                memberlist.add(memberinf);
                reccnt++;
                if (!paramgradelist.contains(key)) {
                    paramgradelist.add(key);
                    schreglst = new ArrayList();
                    param._schreglist.put(key, schreglst);
                }
                schreglst.add(schregno);
                if (!clubcdbk.equals(clubcd)) {
                    clubcdbk = clubcd;
                }
            }
            if (paramgradelist.size() > 0) {
                param._classSelected = (String[])paramgradelist.toArray(new String[paramgradelist.size()]);
            }
        } catch (final Exception ex) {
            log.error("部員情報取得エラー:" + sql, ex);
        } finally {
            db2.commit();
    ...
    private String getMemberInfosql(final Param param) {
        final StringBuffer stb = new StringBuffer();
        stb.append(" SELECT ");
        stb.append("     SCHD.CLUBCD ");
        ...
        stb.append("     ,SBM.NAME ");
        stb.append("     ,SRD.COURSECD || SRD.MAJORCD AS MAJORCODE ");
        stb.append("     ,SRD.COURSECD || SRD.MAJORCD || SRD.COURSECODE AS COURSECODE ");
        stb.append("     ,Z002.ABBV1 AS SEX ");
        stb.append("     ,MAX(SCHD.SDATE) AS SDATE ");
        stb.append(" FROM ");
        stb.append("     SCHREG_CLUB_HIST_DAT SCHD ");
        stb.append("     LEFT JOIN SCHREG_REGD_DAT SRD ");
        stb.append("       ON SCHD.SCHREGNO = SRD.SCHREGNO ");
        stb.append("       AND SRD.YEAR = '" + param._year + "' ");
        stb.append("       AND SRD.SEMESTER = '" + param._semester + "' ");
        stb.append("     LEFT JOIN SCHREG_REGD_HDAT HDAT ");
        stb.append("       ON HDAT.YEAR = SRD.YEAR ");
        stb.append("       AND HDAT.SEMESTER = SRD.SEMESTER ");
        stb.append("       AND HDAT.GRADE = SRD.GRADE ");
        stb.append("       AND HDAT.HR_CLASS = SRD.HR_CLASS ");
        stb.append("     LEFT JOIN SCHREG_BASE_MST SBM ");
        stb.append("       ON SBM.SCHREGNO = SRD.SCHREGNO ");
        stb.append("     LEFT JOIN NAME_MST Z002 ");
        stb.append("       ON NAMECD1 = 'Z002' ");
        stb.append("       AND NAMECD2 = SBM.SEX ");
        stb.append(" WHERE ");
        stb.append("     SRD.SCHREGNO IS NOT NULL ");
        stb.append("     AND (SCHD.EDATE IS NULL OR DATE('"+ param._sdate + "') < SCHD.EDATE) ");
        stb.append("     AND SCHD.CLUBCD IN " + param._clubinstate + " ");
        stb.append("     AND SCHD.SCHOOLCD = '" + param._schoolcd + "' ");
        ...
        stb.append("     ,SRD.SCHREGNO ");
        stb.append("     ,SBM.NAME ");
        stb.append("     ,SRD.COURSECD || SRD.MAJORCD ");
        stb.append("     ,SRD.COURSECD || SRD.MAJORCD || SRD.COURSECODE ");
        stb.append("     ,Z002.ABBV1 ");
        stb.append(" ORDER BY ");
        stb.append("     SCHD.CLUBCD ");
        stb.append("     ,SRD.GRADE DESC ");
        stb.append("     ,SRD.HR_CLASS ");
        stb.append("     ,SRD.ATTENDNO ");
        stb.append("     ,SRD.SCHREGNO ");
        stb.append("     ,SDATE ");
    ...
    private void setStaffName(final DB2UDB db2, final Param param, final Map clubinfo) {
        final String sql =  getStaffNamesql(param);
        //log.debug("sql:"+sql);
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            ...
            while (rs.next()) {
                ...
                Set keyset = clubinfo.keySet();
                if (clubname != null && keyset.contains(clubcd)) {
                    ClubInfo clbwk = (ClubInfo)clubinfo.get(clubcd);
                    clbwk._clubname = clubname;
                    clbwk._staffname.add(staffname);
                }
            }
        } catch (final Exception ex) {
            log.error("クラブ情報取得エラー:" + sql, ex);
        } finally {
            db2.commit();
            DbUtils.closeQuietly(null, ps, rs);
        }
    ...
    private String getStaffNamesql(final Param param) {
        final StringBuffer stb = new StringBuffer();
        stb.append(" SELECT DISTINCT ");
        stb.append("     SCHD.CLUBCD AS CLUBCD, ");
        stb.append("     T1.CLUBNAME AS CLUBNAME, ");
        stb.append("     L2.STAFFCD, ");
        stb.append("     L2.STAFFNAME ");
        stb.append(" FROM ");
        stb.append("   SCHREG_CLUB_HIST_DAT SCHD ");
        stb.append("   LEFT JOIN CLUB_MST T1 ");
        stb.append("     ON T1.SCHOOLCD = SCHD.SCHOOLCD ");
        stb.append("     AND T1.SCHOOL_KIND = SCHD.SCHOOL_KIND ");
        stb.append("     AND T1.CLUBCD = SCHD.CLUBCD ");
        ...
        stb.append("                    AND CLUBCD IN " + param._clubinstate + " ");
        ...
        stb.append(" WHERE ");
        stb.append("   SCHD.SCHOOL_KIND = '" + param._schoolkind + "' ");
        stb.append("   AND (SCHD.EDATE IS NULL OR DATE('"+ param._sdate + "') < SCHD.EDATE) ");
        stb.append("   AND SCHD.CLUBCD IN " + param._clubinstate + " ");
        stb.append(" ORDER BY ");
        stb.append("     SCHD.CLUBCD, ");
    ...
    private void printSummary(final DB2UDB db2,final IPdf ipdf, final Param param, final Map bkupReportInfo, final Map clubinfo) {
        //サブクラスコード->対象クラスのリストを作成する。
        String formname = "J".equals(param._schoolkind) ? "KNJD301A_2.frm" : "KNJD301A.frm";

        int maxoutsubcls = "J".equals(param._schoolkind) ? 34 : 72;
        Set clbkeys = clubinfo.keySet();
        for (final Iterator clbit = clbkeys.iterator(); clbit.hasNext();) {
            ipdf.VrSetForm(formname, 4);
            ClubInfo clbinfo = (ClubInfo)clubinfo.get(clbit.next());
            int outhead_pcnt = 0;

            List detailpagelist = createDetailPageList(param, clbinfo._pagelist, bkupReportInfo);

            for (final Iterator itl = detailpagelist.iterator(); itl.hasNext();) {
                DetailSubClassInfo detwk = (DetailSubClassInfo)itl.next();
                Map detailmap = detwk._detailinfo;
                Set keyset = detailmap.keySet();
                int detailcnt = 0;
                List pagemember = null;
                for (final Iterator itd = keyset.iterator(); itd.hasNext();) {
                    List subclasslst = (List)detailmap.get((String)itd.next());
                    if (detailcnt > maxoutsubcls) {
                        break;
                    }
                    //ページの最初の処理は、成績詳細を出力する前にページ共通の処理(生徒名称と関連情報の出力)
                    if (detailcnt == 0) { //if (detailcnt % maxoutsubcls == 0) {
                		pagemember = (List)clbinfo._pagelist.get(outhead_pcnt);
                		int ttllinecnt = 1;
                		if (pagemember.size() > 0) {
                			setTitle(db2, param, ipdf, clbinfo);
                			for (final Iterator itp = pagemember.iterator(); itp.hasNext();) {
                				MemberInfo memberinf = (MemberInfo)itp.next();
                				printSummarySub1(param, bkupReportInfo, memberinf, ipdf, ttllinecnt); //出席番号、名前、合計、平均等の各科目の得点以外の列を出力。
                				ttllinecnt++;
                			}
                		}
                		outhead_pcnt++;
                    ...
                    if (pagemember == null) {
                        //ページメンバーが居ない状態で処理する必要はないので、次ページデータを取得するためにループを抜ける。
                        break;
                    }
                    for (final Iterator its = subclasslst.iterator(); its.hasNext();) {
                        SubClass sbcls = (SubClass)its.next();
                        //各科目毎に列で出力
                        printSummarySub2(param, bkupReportInfo, sbcls, pagemember, ipdf); //各科目の得点を出力。
                        break; //1回処理すれば、後は同じ情報のはずなので、すぐに抜ける。
                    }
                    //ページ内最後の詳細成績出力なら、改ページ
                    if (detailcnt > 1 && detailcnt % maxoutsubcls == 0) { //if (detailcnt % maxoutsubcls == (maxoutsubcls-1)) {
                        ipdf.VrEndPage();
                        if (itd.hasNext()) {
                            ipdf.VrSetForm(formname, 4);
                        }
                ...
                if (detailcnt < maxoutsubcls) { //if (detailcnt % maxoutsubcls != 0) {
                    for (int leastcnt = maxoutsubcls - (detailcnt % maxoutsubcls);leastcnt > 0;leastcnt--) {
                        ipdf.VrEndRecord();
                    }
                    ipdf.VrEndPage();
                    if (itl.hasNext()) {
    ...
    private void setTitle(final DB2UDB db2,final Param param, final IPdf ipdf, final ClubInfo clbinfo) {
        //Title
        ipdf.VrsOut("TITLE", param._testItem._testitemname + " 部活動別成績一覧表");  // タイトル
        String setYear = KNJ_EditDate.h_format_JP_N(db2,param._year + "/04/01");
        ...
        ipdf.VrsOut("ymd1", printDateTime);          // 日付
        ipdf.VrsOut("year2", setYear+"度");          // タイトルの年度
        ipdf.VrsOut("CLUB_NAME", clbinfo._clubname); // 部活名
        String advisorname = "";
        for (int cnt = 0;cnt < clbinfo._staffname.size();cnt++) {
            advisorname += (cnt == 0 ? "" : "、") + (String)clbinfo._staffname.get(cnt);
        }
        ipdf.VrsOut("HR_TEACHER", advisorname);      // 顧問
    }

    private void printSummarySub1(final Param param, final Map bkupReportInfo, final MemberInfo memberinf, final IPdf ipdf, final int linecnt) {
        HRInfo datcls = memberinf.getHRInfo(bkupReportInfo);

        if (datcls != null) {
            ipdf.VrsOutn("DEPARTMENT_NAME", linecnt, (String)datcls._majorMap.get(memberinf._majorcode));   //学科
            ipdf.VrsOutn("HR_NAME", linecnt, memberinf._hrclassname + memberinf._attendno);                 //年組番号
            ipdf.VrsOutn("name1", linecnt, memberinf._name);             //氏名
            ipdf.VrsOutn("SEX", linecnt, memberinf._sex);                                    //性別
            ipdf.VrsOutn("COURSE_NAME", linecnt, (String)datcls._courseMap.get(memberinf._coursecode));     //コース
            Student stuinfo = (Student)datcls.getStudentInfo(memberinf._schregno);

            ipdf.VrsOutn("TOTAL5", linecnt, stuinfo._FscoreSum);                             //5教科計
            ipdf.VrsOutn("AVERAGE5", linecnt, stuinfo._FscoreAvg);                           //5教科平均
            if (stuinfo._scoreAvg != null && !"".equals(stuinfo._scoreAvg)) {
                ipdf.VrsOutn("TOTAL", linecnt, String.valueOf(stuinfo._scoreSum));           //9教科合計
                ipdf.VrsOutn("AVERAGE", linecnt, stuinfo._scoreAvg);                         //9教科平均
            }
            if (stuinfo._classRank > 0) {
                ipdf.VrsOutn("RANK1_1", linecnt, String.valueOf(stuinfo._classRank));        //クラス内順位
                ipdf.VrsOutn("RANK1_2", linecnt, String.valueOf(datcls._students.size()));   //クラス人数
            }
            if (stuinfo._rank > 0) {
                ipdf.VrsOutn("RANK2_1", linecnt, String.valueOf(stuinfo._rank));             //学年順位
                ipdf.VrsOutn("RANK2_2", linecnt, String.valueOf(stuinfo._rankDenominator));  //学年人数
            }
            ipdf.VrsOutn("TEACHER_NAME", linecnt, datcls._staffName);
        }
    }

    private void printSummarySub2(final Param param, final Map bkupReportInfo,final SubClass sbcls, final List memberlist, final IPdf ipdf) {

        ipdf.VrsOut("course1", sbcls._classabbv);            //教科
        String subclsfield = "SUBCLASS";
        String subclsnameconv = KNJ_EditEdit.convertZenkakuSuuji(sbcls._subclassname);
        if (KNJ_EditEdit.getMS932ByteLength(subclsnameconv) > 14) {
            final String[] tokens = KNJ_EditEdit.get_token(subclsnameconv, 16, 2);
            ipdf.VrsOut(subclsfield + "_1", tokens[0]);      //科目
            ipdf.VrsOut(subclsfield + "_2", tokens[1]);      //科目
        } else {
            ipdf.VrsOut(subclsfield, sbcls._subclassname);   //科目
        }

        int rowcnt = 1;
        for (final Iterator itm = memberlist.iterator(); itm.hasNext();) {
            MemberInfo memberinf = (MemberInfo)itm.next();
            HRInfo datcls = memberinf.getHRInfo(bkupReportInfo);
            String dispscore = "";
            if (datcls != null) {
                Student studetailinf = (Student)datcls._studentMap.get(memberinf._schregno);
                for (final Iterator itr = studetailinf._scoreDetails.keySet().iterator(); itr.hasNext();) {
                    String findk = (String)itr.next();
                    if (findk.indexOf(sbcls._subclasscode) > 0) {
                        ScoreDetail retdetail = (ScoreDetail)studetailinf._scoreDetails.get(findk);
                        dispscore = retdetail._score;
                    }
                }
            }
            final String scorefield = "SCORE1";
            ipdf.VrsOutn(scorefield, rowcnt, dispscore);  //得点
            rowcnt++;
        }
    }

    private List createDetailPageList(final Param param, final List pagelist, final Map bkupReportInfo) {
        List retlist = new ArrayList();
        for (final Iterator it = pagelist.iterator(); it.hasNext();) {
            //ページ毎に全科目を先に追加して、各クラスのコードを後ろにくっつける
            DetailSubClassInfo retcls = new DetailSubClassInfo();
            for (int ii = 0; ii < param._subclsListAll.size();ii++) {
            	SubClass addwk = (SubClass)param._subclsListAll.get(ii);
                List subclassdat = new ArrayList();
                subclassdat.add(addwk);
                retcls._detailinfo.put(addwk._subclasscode, subclassdat);
            }
            retlist.add(retcls);

            List pagemember = (List)it.next();
            for (final Iterator itp = pagemember.iterator(); itp.hasNext();) {
                MemberInfo memberinf = (MemberInfo)itp.next();
                HRInfo datcls = memberinf.getHRInfo(bkupReportInfo);
                List subjectwklist = new ArrayList(datcls._subclasses.values());
                for (final Iterator its = subjectwklist.iterator(); its.hasNext();) {
                    final SubClass sc = (SubClass)its.next();
                    final String subclsstr = sc._subclasscode.substring(8);
                    if (retcls._detailinfo.containsKey(subclsstr)) {
                        List addwk = (List)retcls._detailinfo.get(subclsstr);
                        addwk.add(sc);
                    }
                }
            }
    ...
    private class DetailSubClassInfo {
        Map _detailinfo;
//        String _detailinfograde;
        DetailSubClassInfo () {
//            _detailinfograde = "";
            _detailinfo = new LinkedMap();
        }
    }
...
//    private Map createPageList(final Param param, final List bkupReportInfo) {
...
//    }
    ...
    private static class HRInfo implements Comparable {
        ...
        private Map _courseMap = new HashMap();
        private Map _majorMap = new HashMap();
        ...
        public void load(
            ...
            final long R1Start = System.currentTimeMillis();
            ...
            final long R8End = System.currentTimeMillis();
            log.info(" 8load attendSubclass elapsed time = " + (R8End - R1Start) + "[ms]");
        ...
        private void loadAttendRemark(final DB2UDB db2, final Param param) {
            ...
            try {
                ...
                stb.append("      AND INT(RMK.MONTH) + CASE WHEN INT(RMK.MONTH) < 4 THEN 12 ELSE 0 END <= " + param.getAttendRemarkMonth(db2, param._logindate) + " ");
                stb.append(" WHERE ");
                ...
                while (rs.next()) {
                    ...
                    if (null == rs.getString("REMARK") && !"1".equals(param._knjd301aPrintNullRemark)) {
                        continue;
                    }
...
//                stb.append("SELECT  W1.SCHREGNO,W1.ATTENDNO, W1.COURSECD || W1.MAJORCD AS MAJOR, W1.COURSECD || W1.MAJORCD || W1.COURSECODE AS COURSE ");
                stb.append("SELECT  W1.SCHREGNO,W1.ATTENDNO, W3.NAME, W1.COURSECD || W1.MAJORCD AS MAJOR, W1.COURSECD || W1.MAJORCD || W1.COURSECODE AS COURSE ");
                stb.append(" ,M1.MAJORNAME AS MAJORNAME ,C1.COURSECODENAME AS COURSENAME  ");
                stb.append("FROM    SCHREG_REGD_DAT W1 ");
                stb.append("   LEFT JOIN COURSECODE_MST C1 ");
                stb.append("     ON C1.COURSECODE = W1.COURSECODE ");
                stb.append("   LEFT JOIN MAJOR_MST M1 ");
                stb.append("     ON M1.COURSECD = W1.COURSECD ");
                stb.append("     AND M1.MAJORCD = W1.MAJORCD ");
...
//                    }
                    ...
                    _courseMap.put(rs.getString("COURSE"), StringUtils.defaultString(rs.getString("COURSENAME")));
                    _majorMap.put(rs.getString("MAJOR"), StringUtils.defaultString(rs.getString("MAJORNAME")));
                }
            } catch (Exception e) {
                log.error("Exception", e);
            } finally {
                    ...
                    try {
                        ...
                        if (!row.isEmpty()) {
                            ...
                            if (null != d1) {
                                ...
                                transInfo = new TransInfo(d1, n1, KNJ_EditDate.h_format_JP(db2, d1));
                            } else if (null != d2) {
                                final String n2 = KnjDbUtils.getString(row, "KBN_NAME2");
                                transInfo = new TransInfo(d2, n2, KNJ_EditDate.h_format_JP(db2, d2));
                            }
                            if (null == transInfo) {
                            ...
                            if (param._isTokiwagi) {
                                student._scholarshipName = null == KnjDbUtils.getString(row, "SCHOLARSHIP_ABBV1") ? "" : "(" + KnjDbUtils.getString(row, "SCHOLARSHIP_ABBV1") + ")";
                            }
                        }
                    } catch (Exception e) {
                        log.error("Exception", e);
        ...
        private void loadHrclassAverage(
            ...
            try {
                Set cSet = _courseMap.keySet();
                Set mSet = _majorMap.keySet();
                final String course = SQLUtils.whereIn(true, toArray(cSet));
                final String major = SQLUtils.whereIn(true, toArray(mSet));
...
//            }
            ...
            stb.append("   AND  W3.TESTKINDCD = '" + param._testKindCd.substring(0, 2) + "'");
            stb.append("   AND  W3.TESTITEMCD = '" + param._testKindCd.substring(2, 4) + "'");
            if (!"TESTITEM_MST_COUNTFLG_NEW".equals(param._useTestCountflg)) {
                stb.append("   AND  W3.SCORE_DIV = '" + param._testKindCd.substring(4) + "'");
            }
...
//                stb.append("                  AND CASE WHEN W2.EDATE < '" + param._date + "' THEN W2.EDATE ELSE '" + param._date + "' END BETWEEN W5.TRANSFER_SDATE AND W5.TRANSFER_EDATE ");
                ...
                stb.append("     SELECT  WA1.SCHREGNO,WA1.SEMESTER ");
                stb.append("            ,WA1.GRADE,WA1.COURSECD, WA1.MAJORCD, WA1.COURSECODE ");
//                stb.append("            ,CASE WHEN W3.SCHREGNO IS NOT NULL THEN 1 ");
//                stb.append("                  WHEN W4.SCHREGNO IS NOT NULL THEN 1 ");
//                stb.append("                  WHEN W5.SCHREGNO IS NOT NULL THEN 1 ");
//                stb.append("                  ELSE 0 END AS LEAVE ");
                stb.append("             , 0 AS LEAVE ");
                stb.append("     FROM    SCHREG_REGD_DAT WA1 ");
                stb.append("     INNER JOIN V_SEMESTER_GRADE_MST W2 ON W2.YEAR = WA1.YEAR AND W2.SEMESTER = WA1.SEMESTER AND W2.GRADE = WA1.GRADE ");
//                //               転学・退学者で、異動日が[学期終了日または出欠集計日の小さい日付]より小さい日付の場合
//                stb.append("     LEFT JOIN SCHREG_BASE_MST W3 ON W3.SCHREGNO = W1.SCHREGNO ");
//                stb.append("                  AND W3.GRD_DIV IN('2','3') ");
//                stb.append("                  AND W3.GRD_DATE < CASE WHEN W2.EDATE < '" + param._date + "' THEN W2.EDATE ELSE '" + param._date + "' END ");
...
//                stb.append("                  AND CASE WHEN W2.EDATE < '" + param._date + "' THEN W2.EDATE ELSE '" + param._date + "' END BETWEEN W5.TRANSFER_SDATE AND W5.TRANSFER_EDATE ");
                ...
                stb.append("     WHERE   WA1.YEAR = '" + param._year + "' ");
                if (SEMEALL.equals(param._semester)) {
                    stb.append("     AND WA1.SEMESTER = '" + param._semeFlg + "' ");
                } else {
                    stb.append("     AND WA1.SEMESTER = '" + param._semester + "' ");
                    stb.append("     AND NOT EXISTS(SELECT 'X'  FROM  SCHREG_BASE_MST S1");
                    stb.append("                    WHERE S1.SCHREGNO = WA1.SCHREGNO AND S1.GRD_DIV IN('2','3') AND S1.GRD_DATE < W2.SDATE) ");
                }
                stb.append(") ");
                stb.append(" ,GRADECOUNT AS ( ");
                stb.append(" SELECT ");
                stb.append("   S1.GRADE, ");
                stb.append("   COUNT(S1.SCHREGNO) AS NUMOFSTUDENT_INGRADE");
                stb.append(" FROM SCHREG_REGD_DAT S1 ");
//                stb.append(" LEFT JOIN RECORD_RANK_SDIV_DAT R1 ");
//                stb.append("   ON R1.YEAR = S1.YEAR ");
//                stb.append("   AND R1.SEMESTER = S1.SEMESTER ");
//                stb.append("   AND R1.SCHREGNO = S1.SCHREGNO ");
                stb.append(" WHERE  S1.YEAR = '" + param._year + "' ");
                stb.append("   AND  S1.SEMESTER = '" + param._semester + "' ");
//                stb.append("   AND  R1.TESTKINDCD || R1.TESTITEMCD || R1.SCORE_DIV = '" + param._testKindCd + "' ");
//                stb.append("   AND  R1.SUBCLASSCD = '" + param.SUBCLASSCD999999 + "' ");
                stb.append("   AND  EXISTS(SELECT 'X' FROM SCHNO_B W2 ");
                stb.append("                WHERE S1.SCHREGNO = W2.SCHREGNO AND W2.LEAVE = 0) ");
                stb.append(" GROUP BY ");
                stb.append("   S1.GRADE ");
                stb.append(") ");
                stb.append(" ,FIVEKYOUKASUM AS ( ");
                stb.append("SELECT  W3.SCHREGNO ");
//              if (OUTPUT_KJUN2.equals(param._outputKijun)) {
//                  stb.append("   ,W3.CLASS_AVG_RANK AS CLASS_RANK");
//              } else if (OUTPUT_KJUN3.equals(param._outputKijun)) {
//                  stb.append("   ,W3.CLASS_DEVIATION_RANK AS CLASS_RANK");
//              } else {
...
//              stb.append("       ," + param._rankFieldName + "  AS TOTAL_RANK ");
                ...
                stb.append(" LEFT JOIN SCHREG_REGD_DAT S1 ");
                stb.append("   ON W3.YEAR = S1.YEAR ");
                stb.append("   AND W3.SEMESTER = S1.SEMESTER ");
                stb.append("   AND W3.SCHREGNO = S1.SCHREGNO ");
                stb.append(" WHERE  W3.YEAR = '" + param._year + "' ");
                stb.append("   AND  W3.SEMESTER = '" + param._semester + "' ");
                stb.append("   AND  W3.TESTKINDCD = '" + param._testKindCd.substring(0, 2) + "'");
                stb.append("   AND  W3.TESTITEMCD = '" + param._testKindCd.substring(2, 4) + "'");
                ...
                stb.append("   AND  W3.SUBCLASSCD = '555555' ");
                stb.append("   AND  EXISTS(SELECT 'X' FROM SCHNO_A W1 ");
                stb.append("                WHERE W3.SCHREGNO = W1.SCHREGNO AND W1.LEAVE = 0) ");
                stb.append(") ");
                ...
                stb.append("       ,W3.SCORE AS TOTAL_SCORE ");
                stb.append("       ,DECIMAL(ROUND(FLOAT(W3.AVG)*10,0)/10,5,1) AS TOTAL_AVG ");
                stb.append("       ,G1.NUMOFSTUDENT_INGRADE ");
                stb.append("       ,FKS.TOTAL_SCORE AS FKYOUKA_TOTAL ");
                stb.append("       ,FKS.TOTAL_AVG AS FKYOUKA_AVG ");
                if ("TESTITEM_MST_COUNTFLG_NEW".equals(param._useTestCountflg)) {
                    stb.append("  FROM  RECORD_RANK_DAT W3 ");
                } else {
                    stb.append("  FROM  RECORD_RANK_SDIV_DAT W3 ");
                ...
                stb.append("   AND W3.SEMESTER = S1.SEMESTER ");
                stb.append("   AND W3.SCHREGNO = S1.SCHREGNO ");
                stb.append(" LEFT JOIN GRADECOUNT G1 ");
                stb.append("   ON G1.GRADE = S1.GRADE ");
                stb.append(" LEFT JOIN FIVEKYOUKASUM FKS ");
                stb.append("   ON FKS.SCHREGNO = W3.SCHREGNO ");
                stb.append(" WHERE  W3.YEAR = '" + param._year + "' ");
                stb.append("   AND  W3.SEMESTER = '" + param._semester + "' ");
                stb.append("   AND  W3.TESTKINDCD = '" + param._testKindCd.substring(0, 2) + "'");
                stb.append("   AND  W3.TESTITEMCD = '" + param._testKindCd.substring(2, 4) + "'");
                ...
                for (final Iterator it = _students.iterator(); it.hasNext();) {
                    ...
                    try {
                        rs = ps.executeQuery();
                        if (rs.next()) {
                            student._classRank = Integer.parseInt(StringUtils.defaultString(rs.getString("CLASS_RANK"), "0"));
                            student._rank = rs.getInt("TOTAL_RANK");
                            student._rankDenominator = rs.getInt("NUMOFSTUDENT_INGRADE");
                            student._scoreSum = rs.getString("TOTAL_SCORE");
                            student._scoreAvg = rs.getString("TOTAL_AVG");
                            student._FscoreSum = rs.getString("FKYOUKA_TOTAL");
                            student._FscoreAvg = rs.getString("FKYOUKA_AVG");
                        }
                    } catch (Exception e) {
                        log.error("Exception", e);
                    } finally {
        ...
        private void loadScoreDetail(
            ...
            List detailScoreList = (ArrayList) param._dsMap.get(_hrclassCd.substring(0, 2)+_hrclassCd.substring(2));

            Map arrangewk = (Map)param._subclsMap.get(_hrclassCd.substring(0, 2)+_hrclassCd.substring(2));
            if (null != arrangewk) {
                for (final Iterator it = arrangewk.keySet().iterator(); it.hasNext();) {
                    final String kval = (String) it.next();
                    _subclasses.put(kval, arrangewk.get(kval));
                }
            }

            if (detailScoreList != null) {
                for (int ii = 0;ii < detailScoreList.size();ii++) {
                    ScoreDetail findwk= (ScoreDetail)detailScoreList.get(ii);
                    final Student student = getStudent(findwk._schregno);
                    student._scoreDetails.put(findwk._subClass._subclasscode, findwk);
                }
            }
                        ...
                        if (null == scoreDetail) {
                            ...
                            for (final Iterator it = _subclasses.keySet().iterator(); it.hasNext();) {
                                ...
                                if (subclasscd.substring(1).equals(rs.getString("SUBCLASSCD"))) {
                                    ...
                                    scoreDetail = new ScoreDetail(null, subClass, null, null, null, null, null, null, null, null, null, null, null, null, null);
...
//                        countA += totalObj._avgcount;
        ...
        private void setSubclassGradeAverage(final DB2UDB db2, final Param param) {
            ...
            stb.append("    AND T1.TESTKINDCD = '" + param._testKindCd.substring(0, 2) + "'");
            stb.append("    AND T1.TESTITEMCD = '" + param._testKindCd.substring(2, 4) + "'");
            if (!"TESTITEM_MST_COUNTFLG_NEW".equals(param._useTestCountflg)) {
                stb.append("    AND T1.SCORE_DIV = '" + param._testKindCd.substring(4) + "'");
            }
        ...
        public Student getStudentInfo(final String schregno) {
            Student retinfo = null;
            for (final Iterator its = _students.iterator(); its.hasNext();) {
                Student chkinfo = (Student) its.next();
                if (chkinfo._schregno.equals(schregno)) {
                    retinfo = chkinfo;
                    break;
                }
            }
    ...
    private static class Student implements Comparable {
        ...
        private String _FscoreSum; //5教科合計
        private String _FscoreAvg; //5教科平均
        private int _classRank;
        private int _rank;
        private int _rankDenominator;
    ...
    static class Form {
        ...
        public boolean print(final IPdf ipdf, final Param param, final HRInfo hrInfo) {
            ...
            if ("2".equals(param._outputPattern)) {
                if (Form2.print2(ipdf, param, hrInfo)) {
                    hasData = true;
                }
            } else {
        ...
        public boolean outputCsv(final List outputList, final Param param, final HRInfo hrInfo) {
            ...
            if ("2".equals(param._outputPattern)) {
                ...
                if (Form1.outputCsv(outputList, param, hrInfo, studentsAll)) {
                    hasData = true;
                }
            }
            return hasData;
...
//        }
        ...
        private static void printAttendInfo(final IPdf ipdf, final Param param, final AttendInfo attendInfo, final int line) {
            ...
            ipdf.VrsOutn(fieldPresent, line, String.valueOf(attendInfo._present));      // 出席日数 (0は表示する)
        ...
        private static class Form2 {
            ...
            private static List getPageList(final List list, final Param param) {
                ...
                for (final Iterator it = list.iterator(); it.hasNext();) {
                    ...
                    if (null == current || currentLines + Math.min(1, studentLine) > param._formMaxLine) {
                        current = new ArrayList();
                        currentLines = 0;
                        rtn.add(current);
                    }
            ...
            public static boolean print2(final IPdf ipdf, final Param param, final HRInfo hrInfo) {
                boolean hasData = false;
                ...
                for (int pi = 0; pi < pageList.size(); pi++) {
                    ...
                    printHeader2(ipdf, param, hrInfo);
...
//                        ipdf.VrsOutn("RANK1", line, student.getKettenTanni(param)); // 欠点単位数
...
//                        ipdf.VrsOutn("RANK3", line, student.getKekkaOverKamokuCount(param)); // 欠課時数多科目数
                        ipdf.VrsOutn("FAIL1", line, student.getKettenKamokuCount(param)); // 欠点科目数
//                        if (SEMEALL.equals(param._semester)) {
//                            ipdf.VrsOutn("R_CREDIT", line, zeroToNull(student._compCredit)); // 履修単位数
//                            ipdf.VrsOutn("C_CREDIT", line, zeroToNull(student._getCredit)); // 修得単位数
//                        }
                        ...
                        for (int si = 0; si < detaillist.size(); si++) {
                            ...
                            ipdf.VrsOutn("GRADING" + scol, line, StringUtils.defaultString(detail._scoreDi, detail._score)); // 成績
                            ...
                            ipdf.VrsOutn("KEKKA" + scol + pf, line, getKekkaString(param, detail._absent)); // 欠課数
            ...
            private static void printHeader2(final IPdf ipdf, final Param param, final HRInfo hrInfo) {
                if (isAlpPdf(ipdf)) {
                    ipdf.VrsOut("TITLE", param._nendo + "年度　" + StringUtils.defaultString(param._semesterName) + "  " + StringUtils.defaultString(param._testItem._testitemname) + "　成績一覧表　"); // 年度
                } else {
                    ipdf.VrsOut("NENDO", param._nendo + "年度"); // 年度
                    ipdf.VrsOut("TERM", param._semesterName); // 学期
                    ipdf.VrsOut("TEST", param._testItem._testitemname); // テスト名
                }
//                svf.setString("TITLE", null); // タイトル
                ipdf.VrsOut("CLASSNAME", hrInfo._hrName); // クラス名

                //svf.VrAttribute("NOTE0", "＊：単位保留者");
                ...
                ipdf.VrsOut("DATE2", param._termKekka); // 出欠集計範囲
...
//                if ("9900".equals(param._testKindCd.substring(0, 4))) {
//                    svf.setString("DETAIL1_2", "単位数"); // 項目
//                } else {
//                    svf.setString("DETAIL1_2", "平均点"); // 項目
//                }
...
//              ipdf.VrsOut("T_RANK1", "欠点単位数"); // 項目
//              ipdf.VrsOut("T_RANK2", "欠点科目数"); // 項目

                ipdf.VrsOut("DATE3", param._termKekka); // 出欠の記録の集計範囲
        ...
        static class Form1 {
            ...
            private static boolean print(
                ...
                if (Param._isDemo) {
//                    if (ipdf instanceof AlpPdf) {
//                        AlpPdf a = (AlpPdf) ipdf;
//                        param._formMaxLine = a.getFieldCount("No");
//                        param._formMaxColumn = a.getFieldCount("項目");
//                        log.info(" (column, maxline) = (" + param._formMaxColumn + ", " + param._formMaxLine + ")");
//                    }
                }
...
//                      }
                    ...
                    for (int subi = 0, size = subclassList.size(); subi < size; subi++) {
                        ...
                        printSubclass(ipdf, param, subi + 1, subclass, pat3isKariHyotei);
                        ...
                        for (int sti = 0; sti < stulist.size(); sti++) {
                            ...
                            if (null != detail) {
                            	printDetail(ipdf, param, detail, line, subi + 1, subclass, pat3isKariHyotei);
                            }
                        }
...
//                final IPdf ipdf = null;
                ...
                final String title = param._nendo + "年度　" + param._title;
                if (PATTERN4.equals(param._outputPattern)) {
                    header1Line.addAll(Arrays.asList(new String[] {"", title + "（成績の記録）"}));
                } else if (PATTERN5.equals(param._outputPattern)) {
                    header1Line.addAll(Arrays.asList(new String[] {"", title + "（欠課時数と出欠の記録）"}));
                } else {
                    header1Line.addAll(Arrays.asList(new String[] {"", title}));
                }

                final List header2Line = newLine(headerLineList);
                header2Line.addAll(Arrays.asList(new String[] {hrInfo._hrName, "", "出欠集計範囲：" + param._termKekka, "", "", "", "", "", "担任：" + StringUtils.defaultString(hrInfo._staffName)}));
                ...
                for (int coli = 0, size = printSubclassList.size(); coli < size; coli++) {
                    ...
                    //printSubclasses1(ipdf, param, col, subclass);
                    ...
                    for (final Iterator its = stulist.iterator(); its.hasNext();) {
                        ...
                        if (student._scoreDetails.containsKey(subclass._subclasscode)) {
                            final ScoreDetail detail = (ScoreDetail) student._scoreDetails.get(subclass._subclasscode);
                            //printDetail(ipdf, param, detail, line, col);
                            ...
                            if (null != scoreLine) {
                                ...
                                if (PATTERN3.equals(param._outputPattern)) {
                                    // 評定
                                	final boolean isPrint = pat3isKariHyotei && "1".equals(detail._provFlg) || !pat3isKariHyotei && null == detail._provFlg; // 仮評定ならPROV_FLG=1が対象。そうでなければPROV_FLG=NULLが対象。
                    ...
                    //学級平均・合計
                    //printSubclassStat(ipdf, param, col, subclass);
                    if (PATTERN3.equals(param._outputPattern)) {
                ...
                for (final Iterator its = stulist.iterator(); its.hasNext();) {
                    ...
                    final String scoreAvg = param._isSapporo && !param._testKindCd.startsWith("99") ? "" : student._scoreAvg;
                    final String classRank = 0 >= student._classRank ? "" : String.valueOf(student._classRank);
                    final String rank = 0 >= student._rank ? "" : String.valueOf(student._rank);
                ...
                if (!PATTERN5.equals(param._outputPattern)) {
                    ...
                    if (PATTERN1.equals(param._outputPattern)) {
                        ...
                        columnStudentTotalFooterList[1].set(1, param._isSapporo && !param._testKindCd.startsWith("99") || null == hrInfo._avgHrAverage ? "" : hrInfo._avgHrAverage);
...
//              }
                ...
                ipdf.VrsOut("TITLE", param._nendo + "年度　" + param._title);
                if (PATTERN4.equals(param._outputPattern)) {
                    ipdf.VrsOut("SUBTITLE", "（成績の記録）");
                } else if (PATTERN5.equals(param._outputPattern)) {
                    ipdf.VrsOut("SUBTITLE", "（欠課時数と出欠の記録）");
                }
//                if (!PATTERN1.equals(param._outputPattern) && !PATTERN3.equals(param._outputPattern) && !PATTERN4.equals(param._outputPattern) && !PATTERN5.equals(param._outputPattern)) {
//                    if (param._printPrgid) {
//                        ipdf.VrsOut("PRGID", param._prgId);
//                        ipdf.VrsOut("MARK", "/");
//                    }
//                }
                if (param._isMieken) {
            ...
            private static void printHrInfo(
                ...
                if (null != hrInfo._avgHrTotal) {
                    if (PATTERN1.equals(param._outputPattern)) {
                        ...
                        svfsetString1(ipdf, "TOTAL1", param._formMaxLine + 3, "", col, hrInfo._avgHrTotal); // 学級合計
                    } else if (PATTERN4.equals(param._outputPattern)) {
                ...
                if (null != hrInfo._avgHrAverage) {
                    if (PATTERN1.equals(param._outputPattern)) {
                        ...
                        svfsetString1(ipdf, "AVERAGE1", param._formMaxLine + 3, "", col, hrInfo._avgHrAverage); // 学級平均
                    } else if (PATTERN4.equals(param._outputPattern)) {
                ...
                if (null != hrInfo._failHrTotal) {
                    if (PATTERN1.equals(param._outputPattern)) {
                        ...
                        svfsetString1(ipdf, "FAIL1", param._formMaxLine + 5, "", col, hrInfo._failHrTotal); // 欠点者数
                    } else if (PATTERN4.equals(param._outputPattern)) {
                ...
                if (null != hrInfo._perHrPresent) {
                    if (!PATTERN3.equals(param._outputPattern) && !PATTERN4.equals(param._outputPattern)) {
                        ipdf.VrsOut("PER_ATTEND", DEC_FMT1.format(hrInfo._perHrPresent.setScale(1,BigDecimal.ROUND_HALF_UP)) + (Param._isDemo ? "%" : ""));
                    }
                }
                if (null != hrInfo._perHrSick) {
                    if (!PATTERN3.equals(param._outputPattern) && !PATTERN4.equals(param._outputPattern)) {
                        ipdf.VrsOut("PER_ABSENCE", DEC_FMT1.format(hrInfo._perHrSick.setScale(1,BigDecimal.ROUND_HALF_UP)) + (Param._isDemo ? "%" : ""));
                    }
                }
            }
...
//                }
            ...
            private static void printStudentName(
                ...
                if (PATTERN3.equals(param._outputPattern)) {
                    //ipdf.VrsOutn("NAME" + (namelen > 30 ? "3" : namelen > 20 ? "2" : "1"), line, name);    // 氏名
                    ipdf.VrsOutn("NAME" + nameNo, line, name);    // 氏名
                } else if (Param._isDemo) {
                    //ipdf.VrAttributen("No[0]/生徒氏名", line - 1, "");
            ...
            public static int setRecordString(IPdf ipdf, String field, int gyo, int retsu, String data) {
                if (Param._isDemo) {
//                    if (ipdf instanceof AlpPdf) {
//                        final AlpPdf apdf = (AlpPdf) ipdf;
//                        return apdf.VrsOutn(field, retsu, gyo, data);
//                    }
    ...
    protected Param createParam(final HttpServletRequest request, final DB2UDB db2) {
        log.fatal("$Revision: 75363 $ $Date: 2020-07-10 19:12:18 +0900 (金, 10 7 2020) $"); // CVSキーワードの取り扱いに注意
...
//        log.debug("欠点参照：" + str);
    ...
    private class ClubInfo {
        ...
        String _clubname;
        final List _staffname;
        final List _pagelist;
        ClubInfo (final String clubcd, final List pagelist) {
        	_clubcd = clubcd;
        	_pagelist = pagelist;
        	_staffname = new ArrayList();
        	_clubname = "";
        }
    }
    ...
    private class MemberInfo {
    	...
    	final String _majorcode;
    	MemberInfo (final String schregno, final String grade, final String hrclass, final String hrclassname, final String attendno, final String sex, final String name,
                      final String coursecode, final String majorcode) {
    		_schregno = schregno;
    		_grade = grade;
    		_hrclass = hrclass;
    	...
    	HRInfo getHRInfo(final Map bkupReportInfo) {
    		Set keyset = bkupReportInfo.keySet();
    		if (!keyset.contains(_grade+_hrclass)) {
    			return null;
    		}
    		return (HRInfo)bkupReportInfo.get(_grade+_hrclass);
    	}
    }
    ...
    protected static class Param {
        ...
        /** 学年 */
        String _grade;            //Javaからの取得情報ではなく、D615Vの移植処理との連携用。HrInfoに応じて都度変化する。
        String[] _classSelected;  //Javaからの取得情報ではなく、D615Vの移植処理との連携用。クラブメンバのクラス情報を内部で設定。
        Map _schreglist;
        /** 出欠集計日付 */
        final String _sdate;
        final String _edate;
        ...
        final String _knjd301aPrintNullRemark;
        ...
        final String _knjd301aPrintPerfect;
        ...
        final List _subclsListAll;

        final Map _dsMap = new HashMap();
        final Map _subclsMap = new HashMap();

        final String _useTestCountflg;

        /**
         * @deprecated 宮部さん作成のKNJD301A_0.frm使用デモ用。デモがすんだらカット
         */
        ...
        Param(final HttpServletRequest request, final DB2UDB db2) {
            ...
            _grade = null;
            _classSelected = null;
            _schreglist = new LinkedMap();
            _sdate = KNJ_EditDate.H_Format_Haifun(request.getParameter("SEME_SDATE"));
            _edate = KNJ_EditDate.H_Format_Haifun(request.getParameter("SEME_EDATE"));

            _useTestCountflg = request.getParameter("useTestCountflg");

            _testKindCd = request.getParameter("TESTKINDCD");  //テスト・成績種別
            _outputRank = "2"; //request.getParameter("OUTPUT_RANK");
            _creditDrop = false; //(request.getParameter("OUTPUT4") != null);
            _documentroot = request.getParameter("DOCUMENTROOT");
            _notOutputSougou = false; //"1".equals(request.getParameter("OUTPUT5"));
            _useSlumpSdivDatSlump = false;  //"1".equals(request.getParameter("OUTPUT6"));
            _kettenshaSuuNiKessaShaWoFukumenai = false;  //"1".equals(request.getParameter("OUTPUT7"));
            _outputBiko = false;  //"1".equals(request.getParameter("OUTPUT_BIKO"));
            ...
            _outputOrder = "1";  //StringUtils.defaultString(request.getParameter("OUTPUT_ORDER"), "1");
            _takeSemes = PATTERN2.equals(_outputPattern) ? request.getParameter("TAKESEMES") : null;
            _nendo = KNJ_EditDate.gengou(db2, Integer.parseInt(_year));
            ...
            if (PATTERN2.equals(_outputPattern)) {
                _formname = "KNJD301A_2.frm";
                _formMaxLine = 25;
                _formMaxColumn = 19;
            } else if (PATTERN3.equals(_outputPattern)) {
                _formname = "KNJD301A_3.frm";
                _formMaxLine = 45;
                _formMaxColumn = 17;
            } else if (PATTERN4.equals(_outputPattern)) {
                _formname = "KNJD301A_4.frm";
                _formMaxLine = 45;
                _formMaxColumn = 38;
            } else if (PATTERN5.equals(_outputPattern)) {
                _formname = "KNJD301A_5.frm";
                _formMaxLine = 45;
                _formMaxColumn = 37;
            } else { // PATTERN1.equals(_outputPattern) || "KNJD615L".equals(request.getParameter("PRGID"))
                if (_isDemo) {
                    _formname = "KNJD301A_0.frm";
                } else {
                    _formname = "KNJD301A_1.frm";
                }
                _formname2 = "KNJD301A_1_2.frm";
                _formMaxLine = 45;
                _formMaxColumn = 20;
                _form2MaxColumn = 32;
            }
            ...
            _knjd301aPrintNullRemark = request.getParameter("knjd301aPrintNullRemark");
            _knjd301aPrintPerfect = request.getParameter("knjd301aPrintPerfect");
            _isOutputStudentJisu = false; //"1".equals(request.getParameter("OUTPUT_STUDENT_JISU"));
            ...
            _clubinstate = "(";
            if (_clubSelected != null) {
                for (int cnt = 0;cnt < _clubSelected.length;cnt++) {
                    _clubinstate += "'" + _clubSelected[cnt] + "'";
                    if (cnt + 1 < _clubSelected.length ) {
                     _clubinstate += ", ";
                    }
                }
            }
            ...
            if ("TESTITEM_MST_COUNTFLG_NEW".equals(_useTestCountflg)) {
            ...
            _termKekka = KNJ_EditDate.h_format_JP(db2, _yearDateS) + FROM_TO_MARK + KNJ_EditDate.h_format_JP(db2, _logindate);
            _now = getNow(db2);
            // テスト名称
            ...
            _scoreDiv = "TESTITEM_MST_COUNTFLG_NEW".equals(_useTestCountflg) ? "" : _testKindCd.substring(4);
            _isGakunenMatu = SCORE_DIV_09.equals(_scoreDiv);
            _hasCompCredit = SCORE_DIV_09.equals(_scoreDiv);
            _isPrintPerfect = PATTERN1.equals(_outputPattern) && SCORE_DIV_01.equals(_scoreDiv) && "1".equals(_knjd301aPrintPerfect);
            ...
            loadNameMstD016(db2);
            _subclsListAll = getSubClassMst(db2);
        }

        private static String getDbPrginfoProperties(final DB2UDB db2, final String propName) {
            return KnjDbUtils.getOne(KnjDbUtils.query(db2, " SELECT VALUE FROM PRGINFO_PROPERTIES WHERE PROGRAMID = 'KNJD301A' AND NAME = '" + propName + "' "));
        }
        ...
        private List getSubClassMst(final DB2UDB db2) {
        	List retList = new ArrayList();
        	...
        	stb.append("   T1.SUBCLASSCD, ");
        	stb.append("   CASE WHEN T2.ELECTDIV IS NULL THEN '0' ELSE T2.ELECTDIV END AS ELECTDIV, ");
        	stb.append("   T3.CLASSABBV, ");
        	stb.append("   T2.SUBCLASSNAME, ");
        	stb.append("   T2.SUBCLASSABBV ");
        	stb.append(" FROM ");
        	stb.append("   SUBCLASS_YDAT T1 ");
        	...
        	stb.append(" WHERE ");
        	stb.append("   T1.YEAR = '" + _year + "' AND T1.SCHOOL_KIND = '" + _schoolkind + "' ");
        	stb.append("   AND T1.CLASSCD <= '89' ");
        	stb.append(" ORDER BY ");
        	stb.append(" T1.CLASSCD, ");
        	stb.append(" T1.SUBCLASSCD ");
            ...
            try {
                ...
                while (rs.next()) {
                    final String subclasscd = rs.getString("SUBCLASSCD");
                    final boolean electdiv = "1".equals(rs.getString("ELECTDIV"));
                    final String classabbv = rs.getString("CLASSABBV");
                    final String subclassname = rs.getString("SUBCLASSNAME");
                    final String subclassabbv = rs.getString("SUBCLASSABBV");
                    final SubClass addwk = new SubClass(subclasscd, classabbv, subclassabbv, subclassname, electdiv, 100, 100, "");
                    retList.add(addwk);
                }
            } catch (Exception e) {
                log.error("Exception", e);
        ...
        private TestItem getTestItem(
            ...
            try {
                ...
                if ("TESTITEM_MST_COUNTFLG_NEW".equals(_useTestCountflg)) {
                    ...
                    stb.append("   NULL AS SIDOU_INPUT, ");
                    stb.append("   NULL AS SIDOU_INPUT_INF ");
                    stb.append(" FROM ");
                    ...
                    stb.append(" SELECT ");
                    stb.append("   TESTITEMNAME, ");
                    stb.append("   SIDOU_INPUT, ");
                    stb.append("   SIDOU_INPUT_INF ");
                    stb.append(" FROM ");
                    stb.append("   TESTITEM_MST_COUNTFLG_NEW_SDIV ");
                    stb.append(" WHERE ");
                    stb.append("   YEAR = '" + year + "' ");
                    stb.append("   AND SEMESTER = '" + semester + "' ");
                    stb.append("   AND TESTKINDCD || TESTITEMCD || SCORE_DIV = '" + testcd + "' ");
                }
