 ...
 */
public class KNJD280B {

    private static final Log log = LogFactory.getLog(KNJD280B.class);
    ...
    protected void printMain(final DB2UDB db2, final Vrw32alp svf, final Param param) {
    	...
    	for (int i = 0; i < studentList.size(); i++) {
    		...
    		final String form = isSaturdayCourse ? "KNJD280B_2.frm" : "KNJD280B_1.frm";
    		...
    		svf.VrsOut("TITLE", param._nendo + "　成績詳細（" + StringUtils.defaultString(student._majorabbv) + "）"); // タイトル
...
//            _subclassabbv = subclassabbv;
    ...
    private Param createParam(final HttpServletRequest request, final DB2UDB db2) {
        log.fatal("$Revision: 69204 $ $Date: 2019-08-13 18:53:58 +0900 (火, 13 8 2019) $"); // CVSキーワードの取り扱いに注意
    ...
    private static class Param {
		...
		private static String getDbPrginfoProperties(final DB2UDB db2, final String propName) {
            return KnjDbUtils.getOne(KnjDbUtils.query(db2, " SELECT VALUE FROM PRGINFO_PROPERTIES WHERE PROGRAMID = 'KNJD280B' AND NAME = '" + propName + "' "));
        }
