// kanji=漢字
/*
 * $Id: a4ca0cdf491b03d22cb8cd33a62a59c341831a44 $
 *
 * 作成日: 2007/01/30
 * 作成者: yamasiro
 *
...
public class KNJD173B {
    ...
    private static final Log log = LogFactory.getLog(KNJD173B.class);
    ...
    private boolean printSvf (
        ...
        parameter.setIntrAbsentMonth(db2);
    ...
    private void loadNameMstD016(final DB2UDB db2, final Param parameter) throws SQLException {
        ...
        final String sql = "SELECT NAMECD2, NAMESPARE1 FROM V_NAME_MST WHERE YEAR = '" + parameter._year + "' AND NAMECD1 = 'D016'";
        db2.query(sql);
     ...
     *  フォームを設定します。
     */
    private void setSvfForm (
            final Vrw32alpWrap svf,
            final Param parameter
    ) {
//        String formId = "";
//        if (2 < definecode.semesdiv) {
//            if (parameter._isprintB5) {
//                if (parameter._frmNo == 1) {
//                    svf.VrSetForm("KNJD173_2_5.frm", 4); // ３学期用宛先無し
//                    formId = "KNJD173_2_5.frm";
//                } else {
//                    svf.VrSetForm("KNJD173_2_" + (4 + parameter._frmNo) + ".frm", 4); // ３学期用Ｂ５サイズ
//                    formId = "KNJD173_2_" + (4 + parameter._frmNo) + ".frm";
//                }
//            } else if (!parameter._isprintGardianaAddress) {
//                if (parameter._frmNo == 1) {
//                    svf.VrSetForm("KNJD173_2.frm", 4); // ３学期用宛先無し
//                    formId = "KNJD173_2.frm";
//                } else {
//                    svf.VrSetForm("KNJD173_2_" + parameter._frmNo + ".frm", 4); // ３学期用宛先無し
//                    formId = "KNJD173_2_" + parameter._frmNo + ".frm";
//                }
//            }
//        } else {
//            if (parameter._isprintB5) {
//                if (parameter._frmNo == 1) {
//                    svf.VrSetForm("KNJD173_4.frm", 4); // ２学期用Ｂ５サイズ
//                    formId = "KNJD173_4.frm";
//                } else {
//                    svf.VrSetForm("KNJD173_4_" + parameter._frmNo + ".frm", 4); // ２学期用Ｂ５サイズ
//                    formId = "KNJD173_4_" + parameter._frmNo + ".frm";
//                }
//            } else if (!parameter._isprintGardianaAddress) {
//                if (parameter._frmNo == 1) {
//                    svf.VrSetForm("KNJD173_3.frm", 4); // ２学期用宛先無し
//                    formId = "KNJD173_3.frm";
//                } else {
//                    svf.VrSetForm("KNJD173_3_" + parameter._frmNo + ".frm", 4); // ２学期用宛先無し
//                    formId = "KNJD173_3_" + parameter._frmNo + ".frm";
//                }
//            } else {
//                svf.VrSetForm("KNJD173_1.frm", 4);  // ２学期用宛先有り
//                formId = "KNJD173_1.frm";
//            }
//        }
//        log.debug("フォームＩＤ=" + formId);
        svf.VrSetForm("KNJD173B.frm", 4);
...
//log.debug("成績明細SQL = "+prestatementSubclass(parameter));
        ...
        try {
            ...
            while( rs1.next() ){
                setSvfForm(svf, parameter);
                // 学籍情報等の印刷
                printSvfRegdOut( db2, svf, rs1, parameter );
                // 修得済み単位数の集計
                scredits2 = 0;
                doAddScredits(svf, arrps[4], rs1.getString("SCHREGNO"));
                // 出欠データの印刷
                printSvfAttend(svf, arrps[3], rs1.getString("SCHREGNO"), parameter);
                if (!parameter._isprintB5) { //TODO：未使用のため削除？
    ...
    private void printSvfRegdOut (
        ...
        if (parameter._isprintGardianaAddress){
            ...
            svf.VrsOut("ADDRESS1",     rs.getString("GUARD_ADDR1"));      //住所1
            svf.VrsOut("ADDRESS2",     rs.getString("GUARD_ADDR2"));      //住所2
        }
        ...
        if (parameter._isSpecialactremark) {
            setHreport(svf, rs, item, "SPECIALACTREMARK", "人間と社会");
            item++;
        }
        ...
        // 学習の記録学期名称
        svf.VrsOut("SEMESTER1",  (String) parameter._arrsemeNameMap.get("1") );
        svf.VrsOut("SEMESTER2",  (String) parameter._arrsemeNameMap.get("2") );
        // 出欠の記録学期名称
        svf.VrsOut("SEMES2_1",  (String) parameter._arrsemeNameMap.get("1") );
        svf.VrsOut("SEMES2_2",  (String) parameter._arrsemeNameMap.get("2") );
    ...
    private boolean printSvfRecDetail (
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (1 == replaceflg && _isNoPrintMoto) { continue; }
//                if (1 == parseInt(rs.getString("PRINT_FLG"))) { continue; }
                if (hasData) {
                    svf.VrEndRecord();
                    i++;
                }
    ...
    private boolean printSvfRecDetailOut (
        ...
        int replaceflg = rs.getInt("REPLACEFLG");  // 合併先=9 合併元=1 合併先＆元=2
        String calculate_credit_flg = rs.getString("CALCULATE_CREDIT_FLG");  // 固定=1　加算=2
        if (null == calculate_credit_flg) calculate_credit_flg = "0";
        // 学期の評価・欠課：合併科目は非表示
        if (0 == replaceflg  || 1 == replaceflg  || 9 == replaceflg) {
            /**** １学期・中間（評価） ****/
            if (parameter.isPrint1()) {
                // 評価
                String value = rs.getString("SEM1_INTR_VALUE");
                if (isCharacter(value)) svf.VrAttribute("GRADING1", "Hensyu=3");
                svf.VrsOut("GRADING1", value);
            }
            /**** １学期・末（評価） ****/
            if (parameter.isPrint2()) {
                // 評価
                String value = rs.getString("SEM1_VALUE");
                if (isCharacter(value)) svf.VrAttribute("GRADING2", "Hensyu=3");
                svf.VrsOut("GRADING2", value);
            }
            /**** １学期・（欠課・授業） ****/
            if (parameter.isPrint1() || parameter.isPrint2()) {
                // 欠課
                String absent = (9 == replaceflg) ? rs.getString("REPLACE_ABSENT_SEM1"): rs.getString("ABSENT_SEM1");
                if (9 == replaceflg && "2".equals(calculate_credit_flg) && !_isNoPrintMoto) absent = "";
                svf.doSvfOutNonZero("KEKKA1", absent);
                // 授業
                String lesson = (9 == replaceflg) ? rs.getString("REPLACE_LESSON_SEM1"): rs.getString("LESSON_SEM1");
                if (9 == replaceflg && "2".equals(calculate_credit_flg) && !_isNoPrintMoto) lesson = "";
                svf.doSvfOutNonZero("LESSON1", lesson);
            }
            /**** ２学期・中間（評価） ****/
            if (parameter.isPrint3()) {
                // 評価
                String value = rs.getString("SEM2_INTR_VALUE");
                if (isCharacter(value)) svf.VrAttribute("GRADING3", "Hensyu=3");
                svf.VrsOut("GRADING3", value);
            }
            /**** ２学期・末（評価） ****/
            if (parameter.isPrint4()) {
                // 評価
                String value = rs.getString("SEM2_VALUE");
                if (isCharacter(value)) svf.VrAttribute("GRADING4", "Hensyu=3");
                svf.VrsOut("GRADING4", value);
            }
            /**** ２学期・（欠課・授業） ****/
            if (parameter.isPrint3() || parameter.isPrint4()) {
                // 欠課
                String absent = (9 == replaceflg) ? rs.getString("REPLACE_ABSENT_SEM2"): rs.getString("ABSENT_SEM2");
                if (9 == replaceflg && "2".equals(calculate_credit_flg) && !_isNoPrintMoto) absent = "";
                svf.doSvfOutNonZero("KEKKA2", absent);
                // 授業
                String lesson = (9 == replaceflg) ? rs.getString("REPLACE_LESSON_SEM2"): rs.getString("LESSON_SEM2");
                if (9 == replaceflg && "2".equals(calculate_credit_flg) && !_isNoPrintMoto) lesson = "";
                svf.doSvfOutNonZero("LESSON2", lesson);
            }
        }
        /**** 学年末 ****/
        if (parameter.isPrint4()) {
            // 評価
            doOutGradGrades(svf, rs, parameter);
            // 欠課
            // 授業
            doOutGradAbsent(svf, rs);
            // 履修単位数
            doOutComp_Credits(svf, rs);
            // 修得単位数
            doOutAddGet_Credits(svf, rs);
        }
    ...
    private void doOutCredits (
        ...
        String credits = null;
        int replaceflg = 0;
        String calculate_credit_flg = null;

        try {
            svf.VrsOut("R_DIV", rs.getString("NAMESPARE1"));

            credits = rs.getString("CREDITS");
            replaceflg = rs.getInt("REPLACEFLG");  // 合併先=9 合併元=1 合併先＆元=2
            calculate_credit_flg = rs.getString("CALCULATE_CREDIT_FLG");  // 固定=1　加算=2
        } catch (SQLException e) {
            log.error("SQLException", e);
        }
    ...
    private void doOutGradGrades (
        ...
        if (null == grad_value) { return; }
        if (null == calculate_credit_flg) { calculate_credit_flg = "0"; };
        
        if (1 == replaceflg || 2 == replaceflg) {
            return;
        }
        final String classCd;
        if ("1".equals(paramater._useCurriculumcd)) {
            if ("1".equals(paramater._useClassDetailDat)) {
    ...
    private void doOutGradAbsent (
        ...
        try {
            replaceflg = rs.getInt("REPLACEFLG");  // 合併先=9 合併元=1 合併先＆元=2
            absent = 9 == replaceflg ? rs.getString("REPLACE_ABSENT_TOTAL"): rs.getString("ABSENT_TOTAL");
            calculate_credit_flg = rs.getString("CALCULATE_CREDIT_FLG");  // 固定=1　加算=2
            lesson = 9 == replaceflg ? rs.getString("REPLACE_LESSON_TOTAL"): rs.getString("LESSON_TOTAL");
        } catch (SQLException e) {
            log.error("SQLException", e);
        }
        if (null == calculate_credit_flg) { calculate_credit_flg = "0"; };
        
        if (9 == replaceflg && "2".equals(calculate_credit_flg) && !_isNoPrintMoto) { return; }
        
        svf.doSvfOutNonZero("TOTAL_KEKKA",  absent);
        svf.doSvfOutNonZero("TOTAL_LESSON", lesson);
    }
     ...
     * <pre>
     * 修得済み単位数(合計)を印字します。
     * ・int scredits は今年度の修得単位数・・・学年末の場合。
    ...
    private void doOutAddGet_Credits (
        ...
        String calculate_credit_flg = null;
        try {
            credits = rs.getString("GET_CREDIT");
            replaceflg = rs.getInt("REPLACEFLG");  // 合併先=9 合併元=1 合併先＆元=2
            calculate_credit_flg = rs.getString("CALCULATE_CREDIT_FLG");  // 固定=1　加算=2
        } catch (SQLException e) {
            log.error("SQLException", e);
    ...
    /** 
     * 出欠明細を印字します。
     * TODO:2009/06/26現在集計が合わないため、授業日数、出席日数以外は空欄としておく。
     */
...
//        svf.VrsOutn("LESSON", i, rs.getString("LESSON") );  // 授業日数
        int kibiki = rs.getInt("MOURNING") + rs.getInt("SUSPEND") + ("true".equals(parameter._useVirus) ? rs.getInt("VIRUS") : 0) + ("true".equals(parameter._useKoudome) ? rs.getInt("KOUDOME") : 0);
        if (0 < kibiki) svf.VrsOutn("KIBIKI", i, String.valueOf(kibiki) );  // 出停・忌引日数
//        svf.VrsOutn("ABROAD", i, rs.getString("TRANSFER_DATE") );  // 留学中の授業日数
//        svf.VrsOutn("PRESENT", i, rs.getString("MLESSON") );  // 出席しなければならない日数
        int absenct = rs.getInt("SICK");
        if (0 < absenct) svf.VrsOutn("ABSENCE", i, String.valueOf(absenct) );  // 欠席日数
//        svf.VrsOutn("ATTEND", i, rs.getString("PRESENT") );  // 出席日数

        //特別活動日数（上段：行事の日数、下段：行事の出席日数）
//        svf.VrsOutn("SPECIALACT1", i, "0"); //TODO:仕様が未確定のためとりあえずゼロを印字しておく。
//        svf.VrsOutn("SPECIALACT2", i, "0"); //TODO:仕様が未確定のためとりあえずゼロを印字しておく。
    }
    ...
    private String prestatementSubclass (final Param parameter) {
        ...
        stb.append("   SELECT COMBINED_SUBCLASSCD AS SUBCLASSCD, MAX(CALCULATE_CREDIT_FLG) AS CALCULATE_CREDIT_FLG, MIN(ATTEND_SUBCLASSCD) AS ATTEND_SUBCLASSCD");
        if ("1".equals(parameter._useCurriculumcd)) {
        ...
        stb.append("       AND T1.SEMESTER <= '" + parameter._testSeme + "'"); //TODO
        stb.append("       AND T2.YEAR  = '" + parameter._year + "'");
        stb.append("       AND T2.SEMESTER <= '" + parameter._testSeme + "'"); //TODO
        ...
        stb.append("         , SEM1_INTR_SCORE, SEM1_TERM_SCORE, SEM1_TERM_VALUE");
        stb.append("         , CASE WHEN SEM1_INTR_VALUE IS NOT NULL THEN RTRIM(CHAR(SEM1_INTR_VALUE)) ELSE SEM1_INTR_VALUE_DI END AS SEM1_INTR_VALUE");
        stb.append("         , CASE WHEN SEM1_VALUE IS NOT NULL THEN RTRIM(CHAR(SEM1_VALUE)) ELSE SEM1_VALUE_DI END AS SEM1_VALUE");
        stb.append("         , SEM2_INTR_SCORE, SEM2_TERM_SCORE, SEM2_TERM_VALUE");
        stb.append("         , CASE WHEN SEM2_INTR_VALUE IS NOT NULL THEN RTRIM(CHAR(SEM2_INTR_VALUE)) ELSE SEM2_INTR_VALUE_DI END AS SEM2_INTR_VALUE");
        ...
        if ("TESTITEM_MST_COUNTFLG_NEW".equals(parameter._testTable)) {
        ...
        // 時間割データの表
        log.debug("_year="+parameter._year+", _testSeme="+parameter._testSeme+", _attend_date="+parameter._attend_date+", _date="+parameter._date+", _grade_hr_class="+parameter._grade_hr_class);
        stb.append(", T_SCH_CHR_DAT AS(");
        stb.append(    "SELECT  T0.SCHREGNO, T2.SUBCLASSCD, T2.SEMESTER, T1.EXECUTEDATE, T1.PERIODCD, T1.CHAIRCD, T1.DATADIV ");
        if ("1".equals(parameter._useCurriculumcd)) {
            stb.append("   , T2.CLASSCD ");
            stb.append("   , T2.SCHOOL_KIND ");
            stb.append("   , T2.CURRICULUM_CD ");
        }
        stb.append(    "FROM    SCH_CHR_DAT T1, SEMESTER_MST T3, CHAIR_DAT T2, CHAIR_STD_DAT T0 ");
        stb.append(    "WHERE   T1.YEAR = '" + parameter._year + "' ");
        stb.append(        "AND T1.EXECUTEDATE BETWEEN '" + parameter._attend_date + "' AND '" + parameter._date + "' ");
        stb.append(        "AND T3.YEAR = '" + parameter._year + "' ");
        stb.append(        "AND T3.SEMESTER <= '" + parameter._testSeme + "' AND T3.SEMESTER <> '9' "); //TODO
        stb.append(        "AND T1.EXECUTEDATE BETWEEN T3.SDATE AND T3.EDATE ");
        stb.append(        "AND T2.YEAR = '" + parameter._year + "' ");
        stb.append(        "AND T2.SEMESTER = T3.SEMESTER ");
        stb.append(        "AND T2.CHAIRCD = T1.CHAIRCD ");
        //                      名簿の存在および有効期間チェック
        stb.append(        "AND T0.YEAR = '" + parameter._year + "' ");
        stb.append(        "AND T0.SCHREGNO = ? ");
        stb.append(        "AND T0.YEAR = T2.YEAR ");
        ...
        stb.append(                       "WHERE   T4.SCHREGNO = T0.SCHREGNO ");
        stb.append(                           "AND (( T4.ENT_DIV IN('4','5') AND T1.EXECUTEDATE < T4.ENT_DATE ) ");
        stb.append(                             "OR ( T4.GRD_DIV IN('2','3') AND T1.EXECUTEDATE > T4.GRD_DATE )) ) ");
        ...
        stb.append(    "SELECT  T1.SCHREGNO, T2.SUBCLASSCD, T2.SEMESTER, T1.ATTENDDATE, T1.PERIODCD, T1.DI_CD, T1.CHAIRCD ");
        if ("1".equals(parameter._useCurriculumcd)) {
            stb.append("   , T2.CLASSCD ");
            stb.append("   , T2.SCHOOL_KIND ");
            stb.append("   , T2.CURRICULUM_CD ");
        ...
        stb.append(            ", SEMESTER_MST T3, CHAIR_DAT T2 ");
        stb.append(    "WHERE   T1.SCHREGNO = ? ");
        stb.append(        "AND T1.YEAR = '" + parameter._year + "' ");
        stb.append(        "AND T1.ATTENDDATE BETWEEN '" + parameter._attend_date + "' AND '" + parameter._date + "' ");
        stb.append(        "AND EXISTS(SELECT 'X' FROM SCH_CHR_DAT T4 ");
        stb.append(                   "WHERE  T4.YEAR = '" + parameter._year + "' ");
        stb.append(                      "AND T4.EXECUTEDATE = T1.ATTENDDATE ");
        stb.append(                      "AND T4.PERIODCD = T1.PERIODCD ");
        stb.append(                      "AND T4.CHAIRCD = T1.CHAIRCD ) ");
        stb.append(        "AND T3.YEAR = '" + parameter._year + "' ");
        stb.append(        "AND T3.SEMESTER <= '" + parameter._testSeme + "' AND T3.SEMESTER <> '9' "); //TODO
        stb.append(        "AND T1.ATTENDDATE BETWEEN T3.SDATE AND T3.EDATE ");
        stb.append(        "AND T2.YEAR = '" + parameter._year + "' ");
        stb.append(        "AND T2.SEMESTER = T3.SEMESTER ");
        stb.append(        "AND T2.CHAIRCD = T1.CHAIRCD ");
        //                      名簿の存在および有効期間チェック
        stb.append(        "AND EXISTS(SELECT  'X' FROM CHAIR_STD_DAT T4 ");
        stb.append(                   "WHERE   T4.YEAR = '" + parameter._year + "' ");
        stb.append(                       "AND T4.SCHREGNO = T1.SCHREGNO ");
        stb.append(                       "AND T4.YEAR = T2.YEAR ");
        stb.append(                       "AND T4.SEMESTER = T2.SEMESTER ");
        stb.append(                       "AND T4.CHAIRCD = T2.CHAIRCD ");
        stb.append(                       "AND T1.ATTENDDATE BETWEEN T4.APPDATE AND T4.APPENDDATE) ");
        //                      出欠カウントフラグのチェック
        if( parameter.definecode.useschchrcountflg ){
            stb.append(    "AND NOT EXISTS(SELECT  'X' FROM SCH_CHR_COUNTFLG T4 ");
        ...
        stb.append(                           "AND (( T4.ENT_DIV IN('4','5') AND T1.ATTENDDATE < T4.ENT_DATE ) ");
        stb.append(                             "OR ( T4.GRD_DIV IN('2','3') AND T1.ATTENDDATE > T4.GRD_DATE )) ) ");
        //                      留学日、休学日を除外
        stb.append(        "AND NOT EXISTS(SELECT  'X' FROM SCHREG_TRANSFER_DAT T4 ");
        stb.append(                       "WHERE   T4.SCHREGNO = T1.SCHREGNO ");
        stb.append(                           "AND T4.TRANSFERCD IN('1','2') AND T1.ATTENDDATE BETWEEN T4.TRANSFER_SDATE AND T4.TRANSFER_EDATE ) ");
        stb.append(") ");
        ...
        // 休学時の欠課カウントありの場合、「授業」は休学を引かない。また、「欠課」に休学を足す。
        // 授業時数
        stb.append("           ,COUNT(*) - COUNT(L2.SCHREGNO) - COUNT(L1.SCHREGNO) ");
        if ("1".equals(parameter._knjSchoolMst._subOffDays)) {
            stb.append("        + COUNT(L1.SCHREGNO) ");
        }
        stb.append("            AS LESSON ");
        // 欠課時数
        stb.append("           ,SUM(CASE WHEN (CASE WHEN ATDD.REP_DI_CD IN ('29','30','31') THEN VALUE(ATDD.ATSUB_REPL_DI_CD, ATDD.REP_DI_CD) ELSE ATDD.REP_DI_CD END) IN('4','5','6','14','11','12','13') THEN 1 ELSE 0 END) ");
        if ("1".equals(parameter._knjSchoolMst._subOffDays)) {
            stb.append("        + COUNT(L1.SCHREGNO) ");
        }
        if ("1".equals(parameter._knjSchoolMst._subAbsent)) {
            stb.append("        + SUM(CASE WHEN ATDD.REP_DI_CD IN('1','8') THEN 1 ELSE 0 END) ");
        }
        if ("1".equals(parameter._knjSchoolMst._subSuspend)) {
            stb.append("        + SUM(CASE WHEN ATDD.REP_DI_CD IN('2','9') THEN 1 ELSE 0 END) ");
        }
        if ("1".equals(parameter._knjSchoolMst._subMourning)) {
            stb.append("        + SUM(CASE WHEN ATDD.REP_DI_CD IN('3','10') THEN 1 ELSE 0 END) ");
        }
        if ("true".equals(parameter._useVirus)) {
            if ("1".equals(parameter._knjSchoolMst._subVirus)) {
                stb.append("        + SUM(CASE WHEN ATDD.REP_DI_CD IN('19','20') THEN 1 ELSE 0 END) ");
            }
        }
        if ("true".equals(parameter._useKoudome)) {
            if ("1".equals(parameter._knjSchoolMst._subKoudome)) {
                stb.append("        + SUM(CASE WHEN ATDD.REP_DI_CD IN('25','26') THEN 1 ELSE 0 END) ");
            }
        }
        ...
        stb.append("            LEFT JOIN T_ATTEND_DAT L0 ON L0.ATTENDDATE=T1.EXECUTEDATE AND L0.PERIODCD=T1.PERIODCD AND L0.CHAIRCD=T1.CHAIRCD ");
        //休学
        stb.append("            LEFT JOIN SCHREG_TRANSFER_DAT L1 ON L1.SCHREGNO=T1.SCHREGNO AND L1.TRANSFERCD = '2' AND T1.EXECUTEDATE BETWEEN L1.TRANSFER_SDATE AND L1.TRANSFER_EDATE ");
        //留学
        stb.append("            LEFT JOIN SCHREG_TRANSFER_DAT L2 ON L2.SCHREGNO=T1.SCHREGNO AND L2.TRANSFERCD = '1' AND T1.EXECUTEDATE BETWEEN L2.TRANSFER_SDATE AND L2.TRANSFER_EDATE ");
        stb.append("            LEFT JOIN ATTEND_DI_CD_DAT ATDD ON ATDD.YEAR = '" + parameter._year + "' AND ATDD.DI_CD = L0.DI_CD ");
        ...
        // 休学時の欠課カウントありの場合、「授業」は休学を引かない。また、「欠課」に休学を足す。
        // 授業時数
        stb.append("           ,SUM(VALUE(LESSON,0) - VALUE(ABROAD,0) - VALUE(OFFDAYS,0) ");
        if ("1".equals(parameter._knjSchoolMst._subOffDays)) {
        ...
        stb.append("    WHERE   SCHREGNO = ? ");
        stb.append("        AND YEAR = '" + parameter._year + "' ");
        stb.append("        AND SEMESTER <= '" + parameter._testSeme + "' "); //TODO
        stb.append("        AND (CASE WHEN INT(T1.MONTH) < 4 THEN RTRIM(CHAR(INT(T1.SEMESTER) + 1 )) ELSE T1.SEMESTER END )||T1.MONTH <= '" + KNJC053_BASE.retSemesterMonthValue (parameter._attend_month) + "' ");
        stb.append("    GROUP BY T1.SEMESTER, T1.SUBCLASSCD ");
        if ("1".equals(parameter._useCurriculumcd)) {
            stb.append("   , T1.CLASSCD ");
            stb.append("   , T1.SCHOOL_KIND ");
        ...
        if (parameter.definecode.absent_cov == 1 || parameter.definecode.absent_cov == 3) {
            ...
            stb.append(              ",VALUE(SUM(CASE WHEN SEMESTER = '1' THEN LESSON ELSE NULL END),0) AS LESSON_SEM1 ");
            stb.append(              ",VALUE(SUM(CASE WHEN SEMESTER = '2' THEN LESSON ELSE NULL END),0) AS LESSON_SEM2 ");
            stb.append(              ",VALUE(SUM(LESSON),0) AS LESSON_SEM9 ");
            ...
            stb.append(", ATTEND_B AS(");
            stb.append(       "SELECT  T1.SUBCLASSCD, T1.ABSENT_SEM9, T1.LESSON_SEM9, T2.LESSON_SEM1, T2.LESSON_SEM2 ");
            if ("1".equals(parameter._useCurriculumcd)) {
                stb.append("   , T1.CLASSCD ");
                stb.append("   , T1.SCHOOL_KIND ");
                stb.append("   , T1.CURRICULUM_CD ");
        ...
        stb.append("         , SUM(T2.LESSON_SEM1) AS LESSON_SEM1");
        stb.append("         , SUM(T2.LESSON_SEM2) AS LESSON_SEM2");
        stb.append("         , SUM(T2.LESSON_SEM9) AS LESSON_SEM9");
        ...
        // 休学時の欠課カウントありの場合、「授業」は休学を引かない。また、「欠課」に休学を足す。
        // 授業時数
        stb.append("           VALUE(LESSON,0) - VALUE(ABROAD,0) - VALUE(OFFDAYS,0) ");
        if ("1".equals(parameter._knjSchoolMst._subOffDays)) {
            stb.append("        + VALUE(OFFDAYS,0) ");
        }
        stb.append("            AS LESSON, ");
        ...
        stb.append("         YEAR = '" + parameter._year + "' AND ");
        stb.append("         SEMESTER <= '" + parameter._testSeme + "' "); //TODO
        stb.append("     AND (CASE WHEN INT(MONTH) < 4 THEN RTRIM(CHAR(INT(SEMESTER) + 1 )) ELSE SEMESTER END ) || MONTH <= '" + KNJC053_BASE.retSemesterMonthValue (parameter._attend_month) + "' ");
        stb.append("     ) ");
        stb.append(" , T_ABSENT_SEM1_INTR AS ( ");
        stb.append("     SELECT ");
        if ("1".equals(parameter._useCurriculumcd)) {
            stb.append("   CLASSCD, ");
            stb.append("   SCHOOL_KIND, ");
        ...
        stb.append("         SUBCLASSCD, ");
        stb.append("         SUM(VALUE(LESSON,0)) AS LESSON_SEM1_INTR, ");
        stb.append("         SUM(VALUE(ABSENT,0)) AS ABSENT_SEM1_INTR ");
        stb.append("     FROM ");
        stb.append("         T_ABSENT ");
        stb.append("     WHERE ");
        stb.append("         SEMESTER = '1' AND MONTH IN " + parameter._intr1MonthIn + " ");
        stb.append("     GROUP BY ");
        if ("1".equals(parameter._useCurriculumcd)) {
            stb.append("   CLASSCD, ");
            stb.append("   SCHOOL_KIND, ");
        ...
        stb.append("         SUBCLASSCD ");
        stb.append("     ) ");
        stb.append(" , T_ABSENT_SEM2_INTR AS ( ");
        stb.append("     SELECT ");
        if ("1".equals(parameter._useCurriculumcd)) {
            stb.append("   CLASSCD, ");
            stb.append("   SCHOOL_KIND, ");
        ...
        stb.append("         SUBCLASSCD, ");
        stb.append("         SUM(VALUE(LESSON,0)) AS LESSON_SEM2_INTR, ");
        stb.append("         SUM(VALUE(ABSENT,0)) AS ABSENT_SEM2_INTR ");
        stb.append("     FROM ");
        stb.append("         T_ABSENT ");
        stb.append("     WHERE ");
        stb.append("         SEMESTER = '2' AND MONTH IN " + parameter._intr2MonthIn + " ");
        stb.append("     GROUP BY ");
        if ("1".equals(parameter._useCurriculumcd)) {
            stb.append("   CLASSCD, ");
            stb.append("   SCHOOL_KIND, ");
        ...
        stb.append("         SUBCLASSCD ");
        stb.append("     ) ");
        stb.append(", T_ABSENT_SEM1_INTR_REPLACE AS(");
        stb.append("   SELECT  T1.COMBINED_SUBCLASSCD AS SUBCLASSCD ");
        if ("1".equals(parameter._useCurriculumcd)) {
            stb.append("   , T1.COMBINED_CLASSCD AS CLASSCD ");
            stb.append("   , T1.COMBINED_SCHOOL_KIND AS SCHOOL_KIND ");
        ...
        stb.append("         , SUM(T2.ABSENT_SEM1_INTR) AS REPLACE_ABSENT_SEM1_INTR");
        stb.append("         , SUM(T2.LESSON_SEM1_INTR) AS REPLACE_LESSON_SEM1_INTR");
        stb.append("   FROM    SUBCLASS_REPLACE_COMBINED_DAT T1, T_ABSENT_SEM1_INTR T2");
        stb.append("   WHERE   T1.YEAR = '" + parameter._year + "'");
        if ("1".equals(parameter._useCurriculumcd)) {
            stb.append("   AND T1.ATTEND_CLASSCD = T2.CLASSCD ");
            stb.append("   AND T1.ATTEND_SCHOOL_KIND = T2.SCHOOL_KIND ");
        ...
        stb.append(" )");
        stb.append(", T_ABSENT_SEM2_INTR_REPLACE AS(");
        stb.append("   SELECT  T1.COMBINED_SUBCLASSCD AS SUBCLASSCD");
        if ("1".equals(parameter._useCurriculumcd)) {
            stb.append("   , T1.COMBINED_CLASSCD AS CLASSCD ");
            stb.append("   , T1.COMBINED_SCHOOL_KIND AS SCHOOL_KIND ");
        ...
        stb.append("         , SUM(T2.ABSENT_SEM2_INTR) AS REPLACE_ABSENT_SEM2_INTR");
        stb.append("         , SUM(T2.LESSON_SEM2_INTR) AS REPLACE_LESSON_SEM2_INTR");
        stb.append("   FROM    SUBCLASS_REPLACE_COMBINED_DAT T1, T_ABSENT_SEM2_INTR T2");
        stb.append("   WHERE   T1.YEAR = '" + parameter._year + "'");
        if ("1".equals(parameter._useCurriculumcd)) {
            stb.append("   AND T1.ATTEND_CLASSCD = T2.CLASSCD ");
            stb.append("   AND T1.ATTEND_SCHOOL_KIND = T2.SCHOOL_KIND ");
        ...
        stb.append("       , CASE WHEN T5.SUBCLASSCD IS NOT NULL AND T9.SUBCLASSCD IS NOT NULL THEN 2");  // 合併先かつ合併元
        stb.append("              WHEN T5.SUBCLASSCD IS NOT NULL THEN 9");  // 合併先
        stb.append("              WHEN T9.SUBCLASSCD IS NOT NULL THEN 1");  // 合併元
        stb.append("              ELSE 0 END AS REPLACEFLG");  // 通常
        ...
        if ("1".equals(parameter._useCurriculumcd)) {
            stb.append("                                             ELSE T2.CLASSCD || T2.SCHOOL_KIND || T2.CURRICULUM_CD || T2.SUBCLASSCD END AS ORDER1"); //(4)科目コード
        } else {
            stb.append("                                             ELSE T2.SUBCLASSCD END AS ORDER1"); //(4)科目コード
        }
        ...
        stb.append("       , L1.ABSENT_SEM1_INTR, L1.LESSON_SEM1_INTR");
        stb.append("       , L2.ABSENT_SEM2_INTR, L2.LESSON_SEM2_INTR");
        stb.append("       , L3.REPLACE_ABSENT_SEM1_INTR, L3.REPLACE_LESSON_SEM1_INTR");
        stb.append("       , L4.REPLACE_ABSENT_SEM2_INTR, L4.REPLACE_LESSON_SEM2_INTR");
        stb.append("       , T3.LESSON_SEM1, T3.LESSON_SEM2, T3.LESSON_SEM9 AS LESSON_TOTAL");
        stb.append("       , T8.LESSON_SEM1 AS REPLACE_LESSON_SEM1");
        stb.append("       , T8.LESSON_SEM2 AS REPLACE_LESSON_SEM2");
        stb.append("       , T8.LESSON_SEM9 AS REPLACE_LESSON_TOTAL");
        ...
        stb.append(" LEFT JOIN T_ABSENT_SEM1_INTR L1 ON L1.SUBCLASSCD = T2.SUBCLASSCD");
        if ("1".equals(parameter._useCurriculumcd)) {
        ...
        stb.append(" LEFT JOIN T_ABSENT_SEM2_INTR L2 ON L2.SUBCLASSCD = T2.SUBCLASSCD");
        if ("1".equals(parameter._useCurriculumcd)) {
            stb.append("                 AND L2.CLASSCD = T2.CLASSCD");
            stb.append("                 AND L2.SCHOOL_KIND = T2.SCHOOL_KIND");
            stb.append("                 AND L2.CURRICULUM_CD = T2.CURRICULUM_CD");
        }
        stb.append(" LEFT JOIN T_ABSENT_SEM1_INTR_REPLACE L3 ON L3.SUBCLASSCD = T2.SUBCLASSCD");
        if ("1".equals(parameter._useCurriculumcd)) {
            stb.append("                 AND L3.CLASSCD = T2.CLASSCD");
            stb.append("                 AND L3.SCHOOL_KIND = T2.SCHOOL_KIND");
            stb.append("                 AND L3.CURRICULUM_CD = T2.CURRICULUM_CD");
        }
        stb.append(" LEFT JOIN T_ABSENT_SEM2_INTR_REPLACE L4 ON L4.SUBCLASSCD = T2.SUBCLASSCD");
        if ("1".equals(parameter._useCurriculumcd)) {
            stb.append("                 AND L4.CLASSCD = T2.CLASSCD");
            stb.append("                 AND L4.SCHOOL_KIND = T2.SCHOOL_KIND");
            stb.append("                 AND L4.CURRICULUM_CD = T2.CURRICULUM_CD");
        }

        stb.append(" ORDER BY ORDER1, ORDER2");
        return stb.toString();
    }
     ...
     * 日付の表示形式のクラスです。
     * @author yamasiro
     * @version $Id: a4ca0cdf491b03d22cb8cd33a62a59c341831a44 $
     */
    ...
    private static class Param {
        ...
        private final String _testSeme;
        ...
        private Map _arrsemeNameMap;
        private Map _arrtestNameMap;
        ...
        private String _intr1MonthIn;
        private String _intr2MonthIn;
        ...
        Param(final HttpServletRequest request) {
            ...
            _testTable = request.getParameter("TABLE_NAME"); // テスト項目データのテーブル名
            _testItem = request.getParameter("TEST_ITEM");  // 1-01-01:前期中間、1-99-00:前期末、2-01-01:後期中間、9-99-00:学年末
            _testSeme = _testItem.substring(0, 1);
            _isGakunenmatu = "9".equals(_testSeme);
            String ctrlSeme = request.getParameter("SEMESTER");
            _semester = "9".equals(_testSeme) ? ctrlSeme : _testSeme;  // 1-3:学期
            log.debug("テスト項目="+_testItem);
            _grade_hr_class = request.getParameter("GRADE_HR_CLASS");  // 学年・組
            _date = KNJ_EditDate.H_Format_Haifun( request.getParameter("TAISYOBI") );  //異動基準日
        ...
        private boolean isPrint1() {
            return true;
        }
        
        private boolean isPrint2() {
            return !"1-01-01".equals(_testItem);
        }
        
        private boolean isPrint3() {
            return "2".equals(_testSeme) || "9".equals(_testSeme);
        }
        
        private boolean isPrint4() {
            return "9".equals(_testSeme);
        }
        ...
        private void setHead (final DB2UDB db2) {
            ...
            _arrsemeNameMap = new HashMap();
            try {
                db2.query("SELECT SEMESTER,SEMESTERNAME FROM SEMESTER_MST WHERE YEAR = '" + _year + "' ORDER BY SEMESTER");
                final ResultSet rs = db2.getResultSet();
                while (rs.next()) {
                    _arrsemesName.add(rs.getString("SEMESTERNAME"));
                    _arrsemeNameMap.put(rs.getString("SEMESTER"), rs.getString("SEMESTERNAME"));
                }
                db2.commit();
                rs.close();
            } catch (SQLException e) {
            ...
            // 中間テスト項目名
            _arrtestNameMap = new HashMap();
            try {
                db2.query(getTestName());
                final ResultSet rs = db2.getResultSet();
                while (rs.next()) {
                    if ("TESTITEM_MST_COUNTFLG_NEW".equals(_testTable)) {
                        _arrtestNameMap.put(rs.getString("SEMESTER"), rs.getString("TESTITEMNAME"));
                    } else {
                        _arrtestNameMap.put(rs.getString("SEMESTER"), rs.getString("SEMESTERNAME") + rs.getString("TESTITEMNAME"));
                    }
                }
                db2.commit();
                rs.close();
...
//              stb.append("     rtrim(ltrim(substr(T2.TESTITEMNAME,1,9))) as TESTITEMNAME, "); //頭全角３文字
                ...
                stb.append("     T1.SEMESTER || T2.TESTKINDCD || T2.TESTITEMCD AS TESTCD ");
                stb.append(" FROM ");
                stb.append("     SEMESTER_MST T1, ");
                stb.append("     " + _testTable + " T2 ");
                stb.append(" WHERE ");
                ...
                stb.append(" AND T1.SEMESTER in ('1','2') ");
                stb.append(" AND T2.TESTKINDCD || T2.TESTITEMCD = '0101' ");
                stb.append(" ORDER BY ");
                stb.append("     TESTCD ");
            } catch( Exception ex ){
         ...
         * 中間の欠課時数の集計月を取得する 
         */
        private void setIntrAbsentMonth(final DB2UDB db2) {
            PreparedStatement ps = null;
            ResultSet rs = null;
            _intr1MonthIn = "('')";
            _intr2MonthIn = "('')";
            try {
                ps = db2.prepareStatement(sqlNameMstZ004());
                rs = ps.executeQuery();
                while (rs.next()) {
                    String code = rs.getString("CODE");
                    int smonth = (3 < rs.getInt("SMONTH")) ? rs.getInt("SMONTH") : rs.getInt("SMONTH") + 12;
                    int emonth = (3 < rs.getInt("EMONTH")) ? rs.getInt("EMONTH") : rs.getInt("EMONTH") + 12;

                    String seq = "";
                    String monthIn = "";
                    for (int i = smonth; i <= emonth; i++) {
                        String month = (i < 13) ? String.valueOf(i) : String.valueOf(i - 12);
                        if (month.length() == 1) month = "0" + month;
                        monthIn = monthIn + seq + month;
                        seq = "','";
                    }

                    if ("0112".equals(code)) _intr1MonthIn = "('" + monthIn + "')";
                    if ("0212".equals(code)) _intr2MonthIn = "('" + monthIn + "')";
                }
            } catch (final Exception ex) {
                log.error("名称マスタZ004のロードでエラー", ex);
            } finally {
                db2.commit();
                DbUtils.closeQuietly(null, ps, rs);
            }
            log.debug("欠課時数の集計月　前期中間=" + _intr1MonthIn + "　後期中間=" + _intr2MonthIn);
        }
        private String sqlNameMstZ004() {
            final String sql;
            sql = ""
                + " SELECT "
                + "     NAMECD2 AS CODE, "
                + "     NAMESPARE1 AS SMONTH, "
                + "     NAMESPARE2 AS EMONTH "
                + " FROM "
                + "     NAME_MST "
                + " WHERE "
                + "     NAMECD1='Z004' AND "
                + "     NAMECD2 IN ('0112','0212') AND "
                + "     NAMESPARE1 IS NOT NULL AND "
                + "     NAMESPARE2 IS NOT NULL "
                + " ORDER BY "
                + "     NAMECD2 "
            ;
            return sql;
