/**
 *
 *	学校教育システム 賢者 [成績管理] 成績チエックリスト（成績点票）年５回考査対応
 *
 *	2006/05/16 o-naka 新規作成 KNJD123を土台に作成
 *	                  後期期末2素点・後期期末2評価を追加
 *
 *	2006/10/19 nakamoto NO001:最低点０が出力されない不具合を修正した。
 *  2006/11/17 nakamoto NO002:履修単位・修得単位の欄を追加した
 *	2006/12/20 m-yama   NO003:SQLの不具合を修正した。
...
public class KNJD122T {

    private static final Log log = LogFactory.getLog(KNJD122T.class);
...
log.debug("SUBCLASSCD="+request.getParameter("SUBCLASSCD"));
	...
	private void setHead(DB2UDB db2,Vrw32alp svf,String param[]){
        ...
        if ("1".equals(param[20])) {
            svf.VrSetForm("KNJD122_2P.frm", 4);
        } else {
            svf.VrSetForm("KNJD122_2.frm", 4);
        }
