// kanji=漢字
/*
 * $Id: 1eccd6c7e500ba1f0ad21b0f3a048c19d63de9f1 $
 *
 * 作成日: 2007/05/14
 * 作成者: nakamoto
 *
 ...
 * 成績一覧表（明治中高）
 * @author maesiro
 * @version $Id: 1eccd6c7e500ba1f0ad21b0f3a048c19d63de9f1 $
 */
public class KNJD617G {
    private static final Log log = LogFactory.getLog(KNJD617G.class);
    ...
    // 平均点一覧HR最大表示数
    private static int HR_MAX = 6;
    private static final int HR_MAX6 = 6;
    private static final int HR_MAX11 = 11;
    ...
    public void svf_out(
        ...
        log.fatal("$Revision: 58068 $ $Date: 2018-01-18 17:23:30 +0900 (木, 18 1 2018) $"); // CVSキーワードの取り扱いに注意
        ...
        try {
            ...
            if ("2".equals(_param._printDiv)) {
                ...
                for (int i = 0; i < _param._selectData.length; i++) {
                    ...
                    final List groupList = Group.getGroupList(db2, _param, gradeHr);
                    log.debug(" groupList = " + groupList);
                    for (final Iterator it = groupList.iterator(); it.hasNext();) {
                        final Group group = (Group) it.next();
    ...
    private static String nullBlank(final Object o) {
    ...
    private void printMain(final DB2UDB db2, final Vrw32alp svf, final Group group, final String gradeHr) {

        boolean hasData = false;

        final Map hrclassMap = getHrclass(db2, group._code, gradeHr);

        final List students = getStudentList(db2, group._code, gradeHr);
        if (students.isEmpty()) {
            return;
        }
        ...
        group._courseNotTargetSubclassCdMap = getCourseNotTargetSubclassCdMap(db2, group._code);
        group._courseAttendSubclassCdMap = getCourseAttendSubclassCdMap(db2, group._code);

        final List allsubclasscds = getSubclasscds(db2, group, students, gradeHr);

        final List pageSubclassListList = getPageSubclassListList(allsubclasscds, 26);
        ...
        for (int spi = 0; spi < pageSubclassListList.size(); spi++) {
            ...
            if ("2".equals(_param._printDiv)) {
                svf.VrSetForm(_useForm_1_2 ? "KNJD617G_1_2.frm" : "KNJD617G_1.frm", 1);
            } else {
                svf.VrSetForm("KNJD617G_3.frm", 1);
            }
            ...
            if (pageStudentListList.size() > 0) {
                ...
                printStudents(svf, hrclassMap, group, totalPage, subclasscds, isLastSubclassPage, pageStudentList);
            }
            svf.VrEndPage();
        ...
        if (studentTotalPage > 1) {
            for (int pi = 1; pi < studentTotalPage; pi++) {
                for (int spi = 0; spi < pageSubclassListList.size(); spi++) {
                    ...
                    final boolean isLastSubclassPage = (spi == pageSubclassListList.size() - 1);
                    if ("2".equals(_param._printDiv)) {
                        svf.VrSetForm("KNJD617G_2.frm", 1);
                    } else {
                        svf.VrSetForm("KNJD617G_3.frm", 1);
                    }
    ...
    private void printStudents(final Vrw32alp svf,
        ...
        for (int si = 0; si < pageStudentList.size(); si++) {
            ...
            printStudent(svf, hrclassMap, subclasscds, si + 1, student, g, isLastSubclassPage);
        }
    }
    ...
    private Map getCourseNotTargetSubclassCdMap(final DB2UDB db2, final String selected) {
        final Map map = new HashMap();
        PreparedStatement ps = null;
        ResultSet rs = null;
        final String flg = null != _param._testcd && _param._testcd.startsWith("99") ? "2" : "1";
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
                if (null != _param._testcd && _param._testcd.startsWith("99")) {
                    //getMappedList(map, rs.getString("COURSE")).add(rs.getString("ATTEND_SUBCLASSCD"));
                } else {
                    getMappedList(map, rs.getString("COURSE")).add(rs.getString("COMBINED_SUBCLASSCD"));
                }
            }
        } catch (Exception e) {
            log.error("exception!", e);
...
//            log.fatal(" course = " + course + ", not target subclasscd = " + map.get(course));
//        }
        return map;
    }
    ...
    private Map getCourseAttendSubclassCdMap(final DB2UDB db2, final String selected) {
        ...
        try {
            ...
            while (rs.next()) {
                getMappedList(map, rs.getString("COURSE")).add(rs.getString("ATTEND_SUBCLASSCD"));
                getMappedList(getMappedMap(getMappedMap(map, "COMBINED_ATTEND"), rs.getString("COURSE")), rs.getString("COMBINED_SUBCLASSCD")).add(rs.getString("ATTEND_SUBCLASSCD"));
            }
        } catch (Exception e) {
            log.error("exception!", e);
        } finally {
    ...
    private List getSubclasscds(final DB2UDB db2, final Group g, final List students, final String gradeHr) {
        final String selected = g._code;
        ...
        for (final Iterator it = courseSubclassCdMap.keySet().iterator(); it.hasNext();) {
            ...
            final List attendSubclasscdList = (List) g._courseNotTargetSubclassCdMap.get(course);
...
//                log.fatal(" course = " + course + ", subclasscd = " + subclassCdSet + " => " + debugcol);
    ...
    private void printStudent(final Vrw32alp svf, final Map hrclassMap, final List subclasscds, final int si, final Student student,
            final Group g, final boolean isLastSubclassPage) {
        
        boolean hasKesshi = false;
        final List attendSubclassCdList = getMappedList(g._courseAttendSubclassCdMap, student._course);
        for (final Iterator ait = attendSubclassCdList.iterator(); ait.hasNext();) {
            final String attendSubclasscd = (String) ait.next();
            final Rank rank = (Rank) student._ranks.get(attendSubclasscd);
            if (null == rank || null != rank._valueDi) {
                hasKesshi = true;
            }
        }

        if (!hasKesshi) {
            final Rank stRank = (Rank) student._ranks.get(_param._allsubclasscd);
            
            if (null != stRank) {
                final String grankField = "1".equals(_param._printDiv) ? "GRADE_RANK" : "RANK";
                svf.VrsOutn(grankField, si, null == stRank._gradeRank ? "" : stRank._gradeRank.toString());
        ...
        svf.VrsOutn("HR_CLASS_NO", si, student._hrclass + "-" + (null == student._attendno ? "" : student._attendno));
        svf.VrsOutn("IN_STUDENT", si, ("0".equals(student._inoutCd) ? "M" : ""));
        svf.VrsOutn("NAME" + (getMS932ByteCount(student._name) > 16 ? "3" : getMS932ByteCount(student._name) > 12 ? "2" : ""), si, student._name);
        ...
        for (int subi = 0; subi < subclasscds.size(); subi++) {
            ...
            if (getMappedList(g._courseNotTargetSubclassCdMap, student._course).contains(rank._subclasscd)) {
                continue;
            }
            boolean hasAttendSubclassKesshi = false;
            final List combinedAttendSubclasscdList = getMappedList(getMappedMap(getMappedMap(g._courseAttendSubclassCdMap, "COMBINED_ATTEND"), student._course), rank._subclasscd);
            for (final Iterator it = combinedAttendSubclasscdList.iterator(); it.hasNext();) {
                final String attendSubclasscd = (String) it.next();
                final Rank attendSubclassRank = (Rank) student._ranks.get(attendSubclasscd);
                if (null == attendSubclassRank || null == attendSubclassRank._score || null != attendSubclassRank._valueDi) {
                    hasAttendSubclassKesshi = true;
                    break;
                }
            }
            if (!hasAttendSubclassKesshi) {
                svf.VrsOutn("SUBCLASS_SCORE" + (subi + 1), si, rank._score);
            }
        }
        if (isLastSubclassPage && !hasKesshi) {
            final Rank rank = (Rank) student._ranks.get(_param._allsubclasscd);
            if (null != rank) {
...
//                svf.VrsOutn("MALE_FEMALE_RANK", si, nullBlank(student._rankBySex));
    ...
    private List getStudentList(DB2UDB db2, final String selected, final String gradeHr) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                student._ranks.put(subclasscd, new Rank(subclasscd, score, valueDi, avg, gradeRank, classRank, courseRank, courseGroupRank));
            }
...
//    }
    ...
    private boolean printAvg(final Vrw32alp svf, final Map hrclassMap, final List subclasscds, final boolean isLastSubclassPage) {
        ...
        if (isLastSubclassPage) {
            printHrAverage(svf, hrclassMap, "SUBCLASS_TOTAL", "", _param._allsubclasscd);
    ...
    private Map getHrclass(final DB2UDB db2, final String selected, final String gradeHr) {
...
//        Integer _rankBySex = null;
        ...
        public Integer getTotalRank(final Param param) {
            final Rank rank = (Rank) _ranks.get(param._allsubclasscd);
            if (null == rank) {
                return null;
            }
            if (GROUP_GRADE.equals(param._groupdiv)) {
    ...
    private static class Rank {
        ...
        public Rank(final String subclasscd, final String score, final String valueDi, final String avg, final Integer gradeRank, final Integer classRank, final Integer courseRank, final Integer majorRank) {
            _subclasscd = subclasscd;
            _score = score;
            _valueDi = valueDi;
            _avg = avg;
        ...
        private static String sqlRank(final Param param, final String gradeHr) {
            ...
            stb.append("     LEFT JOIN RECORD_SCORE_DAT T3 ON T3.YEAR = T2.YEAR ");
            stb.append("         AND T3.SEMESTER = T2.SEMESTER ");
            stb.append("         AND T3.TESTKINDCD = T2.TESTKINDCD ");
            stb.append("         AND T3.TESTITEMCD = T2.TESTITEMCD ");
    ...
    private static class Group {
        final String _code;
        Map _courseNotTargetSubclassCdMap;
        Map _courseAttendSubclassCdMap;
        ...
        private static List getGroupList(final DB2UDB db2, final Param param, final String gradeHr) {
            final List list = new ArrayList();
            PreparedStatement ps = null;
            ResultSet rs = null;
            try {
                final String sql = sqlGroup(param, gradeHr);
                ps = db2.prepareStatement(sql);
                rs = ps.executeQuery();
                while (rs.next()) {
        ...
        private static String sqlGroup(final Param param, final String gradeHr) {
            final StringBuffer stb = new StringBuffer();
            stb.append(" SELECT DISTINCT ");
            if (GROUP_GRADE.equals(param._groupdiv)) {
    ...
    private class Param {
        ...
        final String _attendSubclass; // 親科目のみを対象とした平均点や順位を印字
        final String _allsubclasscd;
        ...
        Param(final HttpServletRequest request, final DB2UDB db2) {
            ...
            _selectData = request.getParameterValues("CATEGORY_SELECTED");  //PRINT_DIV='1' 学年-組
            _attendSubclass = request.getParameter("ATTEND_SUBCLASS");
            if ("1".equals(_attendSubclass)) {
            	// KNJD105Gと同様に合計点、平均点の科目コードを変更する
                if ("J".equals(_schoolKind)) {
                	_allsubclasscd = _testcd != null && _testcd.startsWith("99") ? _999999 : _555555;
                } else {
                	_allsubclasscd = _999999;
                }
            } else {
            	_allsubclasscd = _999999;
            }
