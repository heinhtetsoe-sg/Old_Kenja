/*
 * $Id: cf37aae6d84ebf1d7bb3c15643cabafb73382b19 $
 *
 * 作成日: 2011/05/16
 *
...
/**
 * 学校教育システム 賢者 [成績管理]  熊本・個人成績表（通算）
 */

public class KNJD615C {
    ...
    private void printMain(final DB2UDB db2, final Vrw32alp svf) {
        ...
        if (PATTERN1.equals(_param._outputPattern)) {
            form = "KNJD615C_5.frm";
        } else if (PATTERN2.equals(_param._outputPattern)) {
            form = "KNJD615C_6.frm";
        ...
        for (final Iterator it = studentList.iterator(); it.hasNext();) {
            ...
            for (final Iterator itp = student._pageMap.keySet().iterator(); itp.hasNext();) {
                final Integer p = (Integer) itp.next();
                final List printGrade = (List) student._pageMap.get(p);
                
                log.debug(" page = " + p + ", printGrade = " + printGrade);
                
                svf.VrSetForm(form, 4);
                
                printHeader(svf, student, printGrade);
                
                if (printSeiseki(svf, student, printGrade)) {
                    hasData = true;
                }
            }
            if (!hasData) {
    ...
    private static List getPrintGrade(final DB2UDB db2, final Param param, final String schregno) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                final Set testcds = param.getTestItems(year);
                printGrades.add(new YearGrade(year, grade, isRyunen, isRyunenNext, testcds));
            }
        } catch (Exception e) {
            log.error("exception!", e);
        } finally {
        ...
        for (final Iterator it = param._testitemsMap.entrySet().iterator(); it.hasNext();) {
            final Map.Entry entry = (Map.Entry) it.next();
            final String year = (String) entry.getKey();
            final Set testitems = (Set) entry.getValue();
            
            try {
                final StringBuffer stb = new StringBuffer();
                stb.append(" SELECT ");
                if (param._isTableTestitemMstCountflgNewSdiv) {
                    stb.append("   YEAR, SEMESTER, TESTKINDCD || TESTITEMCD || SCORE_DIV AS TESTCD, VALUE(TESTITEMNAME, '') AS TESTITEMNAME ");
                } else {
                    stb.append("   YEAR, SEMESTER, TESTKINDCD || TESTITEMCD AS TESTCD, VALUE(TESTITEMNAME, '') AS TESTITEMNAME ");
                }
                stb.append(" FROM ");
                stb.append("   " + param._testitemMstTable + " ");
                stb.append(" WHERE ");
                ...
                while (rs.next()) {
                    ...
                    final TestItem testitem = TestItem.getTestItem(year, semester, testcd, testitems);
                    if (null != testitem) {
    ...
    private void printHeader(final Vrw32alp svf, final Student student, final List printGrade) {
        svf.VrsOut("TITLE",  "個人成績表（通算）");
        svf.VrsOut("NAME", student._name);
        
        for (final Iterator it = student._regdMapList.iterator(); it.hasNext();) {
            final Map map = (Map) it.next();
            final YearGrade yg = YearGrade.getYearGrade((String) map.get("YEAR"), (String) map.get("GRADE_CD"), printGrade);
            if (null == yg) {
                continue;
            }
            final String sgradeCd = (String) map.get("GRADE_CD");
            final String sattendno = (String) map.get("ATTENDNO");
            final String hrName = (String) map.get("HR_NAME");
        ...
        for (final Iterator ityg = printGrade.iterator(); ityg.hasNext();) {
            ...
            svf.VrsOut("GRADE_NAME" + yg._printLine, Integer.parseInt(yg._gradeCd) + "学年");
            
            // 学期ごとに表示
            for (final Iterator its = yg._testitems.iterator(); its.hasNext();) {
                final TestItem testitem = (TestItem) its.next();
                final String line = yg._printLine + "_" + (SEMES9.equals(testitem._semester) ? "3" : testitem._semester) + "_" + testitem._line;
                
                if (SEMES9.equals(testitem._semester) && _param._hyoteiTestcd.equals(testitem._testcd)) {
                    svf.VrsOut("GRAD_VALUENAME" + line, "学年評定");
                    svf.VrsOut("SEM_TESTNAME" + line, "学年成績");
                } else {
                    svf.VrsOut("SEM_TESTNAME" + line, testitem._testitemname);
                }
            }
            svf.VrsOut("ABSENCE_NAME" + yg._printLine + "_3_9", "欠課時数");
        }
    }

    private boolean printSeiseki(final Vrw32alp svf, final Student student, final List printGrade) {
        ...
        for (final Iterator it = student._recordSubclasses.values().iterator(); it.hasNext();) { // 教科
            ...
            printSubclassLine(svf, recSubclass, recSubclass999999, printGrade); // 科目の1行
    ...
    private void printSubclassLine(
            ...
            final List printGrade) {
        
        if (null != recSubclass) {
            // 科目名
            final String subclassname = recSubclass._subclass._subclassname;
            if (null != subclassname && subclassname.length() > 6) {
                svf.VrsOut("SUBCLASS2_1", subclassname.substring(0, 6));
                svf.VrsOut("SUBCLASS2_2", subclassname.substring(6));
            } else {
            ...
            for (final Iterator ityg = printGrade.iterator(); ityg.hasNext();) {
                ...
                for (final Iterator its = yg._testitems.iterator(); its.hasNext();) {
                    ...
                    if (null != recSubclass.getRecordScore(testitem)) {
                        final RecordScore recScore = recSubclass.getRecordScore(testitem);
                        if (SEMES9.equals(testitem._semester) && _param._hyoteiTestcd.equals(testitem._testcd)) {
                            if ("1".equals(_param._assessLevel)) {
                                svf.VrsOut("SCORE" + line, recScore._assessLevelGrade);

                            } else {
                                // 学年評定
                                if ("2".equals(_param._outputHyoutei)) {
                                    svf.VrsOut("GRAD_VALUE" + yg._printLine, recScore._studyrecValuation);
                                } else {
                                    svf.VrsOut("GRAD_VALUE" + yg._printLine, recScore._score);
                                }
                                // 学年成績
                                svf.VrsOut("SCORE" + line, recScore._vScore);
                            }
                        } else {
                            if ("1".equals(_param._assessLevel)) {
                                svf.VrsOut("SCORE" + line, recScore._assessLevelGrade);
                    ...
                    if (null != recSubclass999999.getRecordScore(testitem)) {
                        final RecordScore recScore999999 = recSubclass999999.getRecordScore(testitem);
                        if (SEMES9.equals(testitem._semester) && _param._hyoteiTestcd.equals(testitem._testcd)) {
                            svf.VrsOut("GRAD_AVERAGE" + yg._printLine, avgToString(recScore999999._avg));
                            svf.VrsOut("AVERAGE" + line, avgToString(recScore999999._vAvg));
                        } else {
                            svf.VrsOut("AVERAGE" + line, avgToString(recScore999999._avg));
                        }
                    }
                }
                
                svf.VrsOut("ABSENCE" + yg._printLine, (String) recSubclass._kekkas.get(yg._year));
            }
        }
    }
    
    private String avgToString(final String avg) {
    ...
    private static class Student {
        ...
        List _printGrade = Collections.EMPTY_LIST;
        Map _pageMap = Collections.EMPTY_MAP;
        Student(final String schregno) {
            _schregno = schregno;
        }
        
        public TestItem getTestItem(final String year, final String semester, final String testcd) {
            if (null == year || null == semester || null == testcd) {
                return null;
            }
            TestItem rtn = null;
            for (final Iterator it = _printGrade.iterator(); it.hasNext();) {
                final YearGrade yg = (YearGrade) it.next();
                rtn = TestItem.getTestItem(year, semester, testcd, yg._testitems);
                if (null != rtn) {
                    break;
                }
            }
        ...
        public RecordSubclass getRecordSubclass(final String subclasscd) {
            ...
            for (final Iterator it = _recordSubclasses.keySet().iterator(); it.hasNext();) {
                ...
                final RecordSubclass rec = (RecordSubclass) _recordSubclasses.get(subclass);
        ...
        public static List getStudentList(final DB2UDB db2, final Param param) {
            ...
            for (final Iterator it = students.iterator(); it.hasNext();) {
                ...
                student._printGrade = getPrintGrade(db2, param, student._schregno);
                
                Integer page = new Integer(1);
                student._pageMap = new TreeMap();
                ...
                paramMap.put("useTestCountflg", param._testitemMstTable);

                int ygLine = 0;
                for (final Iterator ityg = student._printGrade.iterator(); ityg.hasNext();) {
                    final YearGrade yg = (YearGrade) ityg.next();
                    ...
                    getMappedList(student._pageMap, page).add(yg);
                    yg._printLine = ygLine;
                }

                student._recordSubclasses = RecordSubclass.load(db2, param, student, student._printGrade);
                ...
                for (final Iterator ityg = student._printGrade.iterator(); ityg.hasNext();) {
                    ...
                    try {
                        if (null == param._psMap.get("ATTEND_" + yg._year)) {
                            ...
                            // 出欠の情報
                            final KNJDefineCode definecode0 = param.setClasscode0(db2, yg._year);
                            final String periodInState = AttendAccumulate.getPeiodValue(db2, definecode0, yg._year, SSEMESTER, ESEMESTER);
...
//                log.debug(" record score sql = " + sql);
                ps = db2.prepareStatement(sql);
                rs = ps.executeQuery();
                while (rs.next()) {
                    final String year = rs.getString("YEAR");
                    ...
                    final TestItem testitem = student.getTestItem(year, semes, testcd);
...
//                        log.debug(" 科目無し : " + subclasscd);
                    ...
                    final RecordScore recordScore = new RecordScore(testitem, score, avg, vScore, vAvg, studyrecValuation);
                    
                    recSubclass._scores.put(testitem, recordScore);
                }
            ...
            // 学年の評定段階値
            try {
                final String assessLevelSql = getAssessLevelMstSql(param, param._year, param._semester, student._schregno, null, null, "1", null, param._useCurriculumcd);
                ps = db2.prepareStatement(assessLevelSql);
                rs = ps.executeQuery();
                while (rs.next()) {

                    final TestItem testitem = student.getTestItem(rs.getString("YEAR"), rs.getString("SEMESTER"), rs.getString("TESTKINDCD") + rs.getString("TESTITEMCD"));
...
//                        log.debug(" 科目無し : " + subclassCd);
                    ...
                    final RecordSubclass recSubclass = (RecordSubclass) recordSubclassMap.get(subclass);
                    
                    if (null == recSubclass || null == testitem) {
                        continue;
                    }
                    final RecordScore recScore = recSubclass.getRecordScore(testitem);
                    if (null != recScore) {
                        recScore._assessLevelGrade = rs.getString("ASSESSLEVEL");
                    }
                }
            } catch (SQLException e) {
                log.error("exception!", e);
        ...
        public static String getAssessLevelMstSql(
            ...
            stb.append("     L2.* ");
            stb.append(" FROM ");
            stb.append("     " + param._recordRankDatTable + " T0 ");
        ...
        public RecordScore getRecordScore(final TestItem testitem) {
            return (RecordScore) _scores.get(testitem);
        }
        
        public static String sql(final Param param, final String schregno, final List printGrade) {
            ...
            for (final Iterator ityg = printGrade.iterator(); ityg.hasNext();) {
                ...
                for (final Iterator it = yg._testitems.iterator(); it.hasNext();) {
            ...
            stb.append(" FROM  ");
            stb.append("     " + param._recordRankDatTable + " T1 ");
            ...
            if ("1".equals(param._outputSougou)) {
                ...
                if ("1".equals(param._useCurriculumcd)) {
                    ...
                    stb.append("     SUBSTR(T1.SUBCLASSCD, 1, 2) <> '90' ");
                }
            } else {
                stb.append(" UNION ");
                stb.append(" SELECT DISTINCT  ");
            ...
            if (param._isTableTestitemMstCountflgNewSdiv) {
                ...
                stb.append("     T0.TESTKINDCD || T0.TESTITEMCD AS TESTCD, ");
            }
            ...
            stb.append("     LEFT JOIN " + param._recordRankDatTable + " T1 ON T1.SCHREGNO = T0.SCHREGNO ");
            stb.append("         AND T1.YEAR = T0.YEAR ");
            stb.append("         AND T1.SEMESTER = T0.SEMESTER ");
            stb.append("         AND T1.TESTKINDCD || T1.TESTITEMCD = T0.TESTKINDCD || T0.TESTITEMCD ");
            if (param._isTableTestitemMstCountflgNewSdiv) {
            ...
            stb.append("     LEFT JOIN " + param._recordRankVDatTable + " T2 ON T2.SCHREGNO = T0.SCHREGNO ");
            stb.append("         AND T2.YEAR = T0.YEAR ");
            stb.append("         AND T2.SEMESTER = T0.SEMESTER ");
            stb.append("         AND T2.TESTKINDCD || T2.TESTITEMCD = T0.TESTKINDCD || T0.TESTITEMCD ");
            if (param._isTableTestitemMstCountflgNewSdiv) {
                stb.append("         AND T2.SCORE_DIV = '08' "); // 学年成績
            }
            if ("1".equals(param._useCurriculumcd)) {
            ...
            if ("1".equals(param._useCurriculumcd)) {
                ...
                stb.append("         AND T3.CLASSCD = SUBSTR(T0.SUBCLASSCD, 1, 2) ");
            }
    ...
    private static class TestItem implements Comparable {
        ...
        final String _testcd; // TESTKINDCD + TESTITEMCD
        final String _line;
        String _testitemname = "";
        public TestItem(final String year, final String semester, final String testcd, final String line) {
            _year = year;
            _semester = semester;
            _testcd = testcd;
            _line = line;
    ...
    private static class RecordScore {
        ...
        public String toString() {
            return "RecordScore(" + _testcd + ":" + _score + ":" + _assessLevelGrade + ")";
        }
    }
    ...
    private static class YearGrade implements Comparable {
        ...
        final Set _testitems;
        int _printLine;
        YearGrade(final String year, final String gradeCd, final boolean isRyunen, final boolean isRyunenNext, final Set testitems) {
            _year = year;
            _gradeCd = gradeCd;
            _isRyunen = isRyunen;
    ...
    private static class Param {
        ...
        final Map _testitemsMap;
        final String _useVirus;
        final String _useKoudome;
        final boolean _isTableTestitemMstCountflgNewSdiv;
        final String _testitemMstTable;
        final String _recordRankDatTable;
        final String _recordRankVDatTable;
        final String _hyoteiTestcd;
        ...
        Param(final DB2UDB db2, final HttpServletRequest request) throws SQLException {
            ...
            _outputPattern = request.getParameter("OUTPUT_PATTERN");
            _outputHyoutei = request.getParameter("OUTPUT_HYOUTEI");
            _outputSougou = request.getParameter("OUTPUT_SOUGOU");
            _assessLevel = request.getParameter("ASSESS_LEVEL");
            _useCurriculumcd = request.getParameter("useCurriculumcd");
            _useVirus = request.getParameter("useVirus");
            _useKoudome = request.getParameter("useKoudome");
            _testitemsMap = new HashMap();
            
            _subclassMap = getAllSubclasses(db2);
            
            _isTableTestitemMstCountflgNewSdiv = "TESTITEM_MST_COUNTFLG_NEW_SDIV".equals(request.getParameter("useTestCountflg"));
            if (_isTableTestitemMstCountflgNewSdiv) {
                _testitemMstTable = "TESTITEM_MST_COUNTFLG_NEW_SDIV";
                _recordRankDatTable = "RECORD_RANK_SDIV_DAT";
                _recordRankVDatTable = "RECORD_RANK_SDIV_DAT";
                _hyoteiTestcd = _9900 + "09";
            } else {
                _testitemMstTable = "TESTITEM_MST_COUNTFLG_NEW";
                _recordRankDatTable = "RECORD_RANK_DAT";
                _recordRankVDatTable = "RECORD_RANK_V_DAT";
                _hyoteiTestcd = _9900;
            }
        ...
        private Set getTestItems(final String year) {
            if (null == _testitemsMap.get(year)) {
                final Set testitems = new TreeSet();
                final String sem1 = "1";
                final String sem2 = "2";
                final String sem3 = "3";
                final String sem9 = "9";
                final String scoreDiv = _isTableTestitemMstCountflgNewSdiv ? "08" : "";
                if (PATTERN1.equals(_outputPattern)) {
                    testitems.add(new TestItem(year, sem1, "0101" + scoreDiv, "1"));
                    testitems.add(new TestItem(year, sem1, "0201" + scoreDiv, "2"));
                    testitems.add(new TestItem(year, sem1, "0202" + scoreDiv, "3"));
                    testitems.add(new TestItem(year, sem1, _9900 + scoreDiv,  "9"));
                    testitems.add(new TestItem(year, sem2, "0101" + scoreDiv, "1"));
                    testitems.add(new TestItem(year, sem2, "0201" + scoreDiv, "2"));
                    testitems.add(new TestItem(year, sem2, _9900 + scoreDiv,  "9"));
                    testitems.add(new TestItem(year, sem9, _hyoteiTestcd,  "9"));
                    ...
                    testitems.add(new TestItem(year, sem2, "0101" + scoreDiv, "1"));
                    testitems.add(new TestItem(year, sem2, "0201" + scoreDiv, "2"));
                    testitems.add(new TestItem(year, sem2, "0202" + scoreDiv, "3"));
                    testitems.add(new TestItem(year, sem2, _9900 + scoreDiv,  "9"));
                    testitems.add(new TestItem(year, sem9, _hyoteiTestcd,  "9"));
                    ...
                    testitems.add(new TestItem(year, sem2, "0201" + scoreDiv, "2"));
                    testitems.add(new TestItem(year, sem2, _9900 + scoreDiv,  "9"));
                    testitems.add(new TestItem(year, sem3, "0201" + scoreDiv, "1"));
                    testitems.add(new TestItem(year, sem3, _9900 + scoreDiv,  "2"));
                    testitems.add(new TestItem(year, sem9, _hyoteiTestcd,  "9"));
                }
                _testitemsMap.put(year, testitems);
            }
            return (Set) _testitemsMap.get(year);
        }
        ...
        private Map getSemesterMst(final DB2UDB db2, final String year) {
            ...
            ResultSet rs = null;
            try {
                ps = db2.prepareStatement("SELECT SDATE, EDATE FROM SEMESTER_MST WHERE YEAR = '" + year + "' AND SEMESTER = '9' ");
                rs = ps.executeQuery();
                if (rs.next()) {
