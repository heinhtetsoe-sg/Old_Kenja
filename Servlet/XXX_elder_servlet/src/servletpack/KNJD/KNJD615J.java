// kanji=漢字
/*
 * $Id: a7f6748629a66deebb12a75cbc24099a6b52d59c $
 *
 * 作成日: 2007/05/14
 * 作成者: nakamoto
 *
 ...
 * 成績順一覧表（土佐塾）
 * @author maesiro
 * @version $Id: a7f6748629a66deebb12a75cbc24099a6b52d59c $
 */
public class KNJD615J {
    private static final Log log = LogFactory.getLog(KNJD615J.class);
    ...
    private static final List _totalSubclasscdList = Arrays.asList(new String[] {_333333, _555555, _999999, _99999A, _99999B, _999999AVG, });
    ...
    public void svf_out(
        ...
        log.fatal("$Revision: 64071 $ $Date: 2018-12-20 00:20:06 +0900 (木, 20 12 2018) $"); // CVSキーワードの取り扱いに注意
    ...
    private List getSubclasscds(final DB2UDB db2, final List students, final String gradeHr) {
        ...
        col.removeAll(_totalSubclasscdList);
    ...
    private static class Student {
        ...
        public static List getStudentList(final DB2UDB db2, final Param param, final String gradeHr) {
            ...
            for (final Iterator it = KnjDbUtils.query(db2, sqlRank).iterator(); it.hasNext();) {
                ...
                if (!(_totalSubclasscdList.contains(KnjDbUtils.getString(row, "SUBCLASSCD")))) {
...
//      }
    ...
    private static boolean isAkaten(final Rank rank, final Param param) {
        if (!rank._subclasscd.startsWith("9999") && NumberUtils.isNumber(rank._score) && Double.parseDouble(rank._score)  < param._akaten) {
            return true;
        }
        return false;
    }
    ...
    private static class Average {
        ...
        private static Average NULL = new Average(null, null, null, null, null, null);
        ...
        final String _count;
        final String _avg;
        public Average(final String subclasscd, final String score, final String highscore, final String lowscore, final String count, final String avg) {
        ...
        private static Map getAverageMap(final DB2UDB db2, final Param param) {
            ...
            for (final Iterator it = KnjDbUtils.query(db2, sql).iterator(); it.hasNext();) {
                ...
                final String avg = KnjDbUtils.getString(row, "AVG");
                final Average average = new Average(subclasscd, score, highscore, lowscore, count, avg);
        ...
        private static String sqlAverage(final Param param) {
            ...
            stb.append("     , DOUBLE(T1.AVG) AS AVG ");
            stb.append(" FROM ");
            ...
            stb.append("     AND T1.AVG_DIV IN ('1', '2', '3', '7') "); // 学科、クラス、コース、寮
            stb.append("     AND T1.TESTKINDCD || T1.TESTITEMCD || T1.SCORE_DIV = '" + param._testcd + "' ");
    ...
    private void print1(final DB2UDB db2, final Vrw32alp svf, final String gradeHr) {
        final String form = "KNJD615J_1.frm";
        ...
        final String title = KNJ_EditDate.gengou(db2, Integer.parseInt(_param._year)) + "年度　" + StringUtils.defaultString(_param._gradename) + "　" + StringUtils.defaultString(_param._testitemname);
            ...
            for (int sbpi = 0; sbpi < pageSubclassListList.size(); sbpi++) {
                ...
                for (int sti = 0; sti < studentList.size(); sti++) {
                    ...
                    if (null != rank999999) {
                        ...
                        svf.VrsOutn("HR_NUM", line, Average.getAverage(avgMap, Average.avgKey("2", student, _999999))._count); // クラス人数
                        svf.VrsOutn("GRADE_RANK", line, toStr(rank999999._gradeRank)); // 学年順位
                        svf.VrsOutn("GRADE_NUM", line, Average.getAverage(avgMap, Average.avgKey("1", student, _999999))._count); // 学年人数
                    }

                    int akaten = 0;
                    int kamokusu = 0;
                    for (final Iterator rankit = student._ranks.values().iterator(); rankit.hasNext();) {
                        final Rank rank = (Rank) rankit.next();
                        if (isAkaten(rank, _param)) {
                            akaten += 1;
                        }
                        if (!_totalSubclasscdList.contains(rank._subclasscd)) {
                            kamokusu += 1;
                        }
                    }
                    svf.VrsOutn("RED_POINT", line, String.valueOf(akaten)); // 赤点
                    svf.VrsOutn("SUBCLASS_NUM", line, String.valueOf(kamokusu)); // 科目数
                }
                
                for (int subi = 0; subi < subclassList.size(); subi++) {
                    
                    final String subclasscd = (String) subclassList.get(subi);
                    boolean isPrintSubClassname = false;
                    int k;
                    if (isPrintSubClassname) {
                        k = 2;
                        svf.VrsOut("GRPCD", null); // グループコード
                        svf.VrsOut("CLASS_NAME", null); // 教科名
                        svf.VrsOut("SUBCLASS_NAME", null); // 科目名
                        ...
                        */
                        ...
                        k = 1;
                        final String subclassname = (String) _param._subclassnameMap.get(subclasscd);
                        svf.VrsOut("CLASS_NAME1", subclassname); // 科目名
                    }
                    for (int sti = 0; sti < studentList.size(); sti++) {
                        final Student student = (Student) studentList.get(sti);
                        ...
                        if (null != rank) {
                            svf.VrsOut("SCORE" + k + "_" + line, rank._score); // 素点
                            if (isAkaten(rank, _param)) {
                                svf.VrAttribute("SCORE" + k + "_" + line, "Palette=9,UnderLine=(0,1,1)"); // 素点
                            }
                        }
                    }
                    svf.VrEndRecord();
                ...
                for (int i = subclassList.size(); i < maxColumn; i++) {
                    svf.VrsOut("CLASS_NAME1", String.valueOf(i)); // 科目名
        ...
        if ("2".equals(_param._printDiv) && _hasData) {
            final String form2 = "KNJD615J_2.frm";
            ...
            for (int sbpi = 0; sbpi < pageSubclassListList.size(); sbpi++) {
                ...
                final List akatenSchregnoList = getMappedList(akatenMap, "SUBCLASS=TOTAL|HR_CLASS=TOTAL");
                log.info(" akaten total = " + akatenSchregnoList.size());
                
                svf.VrsOut("RED_TOTAL", String.valueOf(akatenSchregnoList.size())); // 赤点合計
                svf.VrsOut("RED_NUM", String.valueOf(new ArrayList(new HashSet(akatenSchregnoList)).size())); // 赤点人数
                svf.VrsOut("DATE2", StringUtils.replace(_param._loginDate, "-", "/")); // 印刷日付
                
                final int maxLineAvg = 7;
                for (int j = 0; j < Math.min(hrAvgHrclassList.size(), maxLineAvg); j++) {
                    final int line = j + 1;
                    ...
                    if ("TOTAL".equals(hrClass)) {
                        final Average averageRyo999999 = Average.getAverage(avgMap, Average.avgKey("1", _param._grade, "000", "00000000", _999999));
                        final Average averageRyo999999AVG = Average.getAverage(avgMap, Average.avgKey("1", _param._grade, "000", "00000000", _999999AVG));
                        
                        svf.VrsOutn("TOTAL", line, sishagonyu(averageRyo999999._avg)); // 合計
                        svf.VrsOutn("AVE", line, sishagonyu(averageRyo999999AVG._avg)); // 平均
                        ...
                        svf.VrsOutn("TOTAL", line, sishagonyu(averageHr999999._avg)); // 合計
                        svf.VrsOutn("AVE", line, sishagonyu(averageHr999999AVG._avg)); // 平均
                    }
                }

                final int maxLineAkaten = 6;
                for (int j = 0; j < Math.min(akatenHrclassList.size(), maxLineAkaten); j++) {
                    final int line = j + 1;
                ...
                for (int j = 0; j < Math.min(countHrclassList.size(), maxLineCount); j++) {
                    ...
                    if ("TOTAL".equals(KnjDbUtils.getString(hr, "HR_CLASS"))) {
                        ...
                        svf.VrsOutn("AVE_NAME3", line, StringUtils.defaultString(KnjDbUtils.getString(hr, "HR_CLASS_NAME1"))); // 人数名称
                        svf.VrsOutn("NUM", line, "(" + StringUtils.defaultString(KnjDbUtils.getString(hr, "REGD_COUNT")) + ")"); // 合計
                    }
                }

                svf.VrsOutn("AVE_NAME4", 1, "最高点");
                svf.VrsOutn("AVE_NAME4", 2, "最低点");
                // 科目の列表示
...
//                        svf.VrsOut("SUBCLASS_NAME" + (getMS932ByteCount(subclassname) <= 4 ? "1" : "2"), subclassname); // 科目名
                        ...
                        k = 1;
                        svf.VrsOut("CLASS_NAME1", subclassname); // 教科名
                    }
                    ...
                    for (int j = 0; j < Math.min(hrAvgHrclassList.size(), maxLineAvg); j++) {
                        ...
                        final String hrClass = KnjDbUtils.getString(hr, "HR_CLASS");
                        if ("TOTAL".equals(hrClass)) {
                            final Average average = Average.getAverage(avgMap, Average.avgKey("1", _param._grade, "000", "00000000", subclasscd));
                            svf.VrsOut("AVE" + k + "_" + line, sishagonyu(average._avg)); // 平均
                        } else if ("RYO".equals(hrClass)) {
                    ...
                    for (int j = 0; j < Math.min(akatenHrclassList.size(), maxLineAkaten); j++) {
                        ...
                        final Map hr = (Map) akatenHrclassList.get(j);
                        final String hrClass = KnjDbUtils.getString(hr, "HR_CLASS");
                        svf.VrsOut("RED" + k + "_" + line, String.valueOf(getMappedList(akatenMap, "SUBCLASS=" + subclasscd + "|HR_CLASS=" + hrClass).size())); // 赤点
                    }
                    ...
                    for (int j = 0; j < Math.min(countHrclassList.size(), maxLineCount); j++) {
                        ...
                        if ("TOTAL".equals(hrClass)) {
                            ...
                            svf.VrsOut("EXAM_NUM" + k + "_" + line, averageHr._count); // 受験者数
                        } else {
                    ...
                    final Average average = Average.getAverage(avgMap, Average.avgKey("1", _param._grade, "000", "00000000", subclasscd));
                    svf.VrsOut("NAX" + k, average._highscore); // 最高点
                    svf.VrsOut("MIN" + k, average._lowscore); // 最低点
                    svf.VrEndRecord();
                }
    ...
    private static class Param {
        ...
        Param(final HttpServletRequest request, final DB2UDB db2) {
            ...
            SCHOOLCD = StringUtils.defaultString(getSchregRegdGdat(db2, "SCHOOL_KIND"));
