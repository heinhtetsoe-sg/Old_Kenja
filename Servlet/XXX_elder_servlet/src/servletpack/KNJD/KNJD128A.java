 ...
 *
 *    2010/04/17 o-naka KNJD127Aを元に新規作成
 */
...
public class KNJD128A {

    private static final Log log = LogFactory.getLog(KNJD128A.class);
                     ...
                     throws ServletException, IOException
    {
        ...
        try {
            log.fatal("$Revision: 68679 $"); // CVSキーワードの取り扱いに注意
    ...
    private void setHead(DB2UDB db2,Vrw32alp svf,String param[]) {
        ...
        final String form = "1".equals(param[21]) ? "KNJD128A_P.frm" : "KNJD128A.frm";
        ret = svf.VrSetForm(form, 4);
        log.info("===> VrSetForm():" + ret);
        KNJ_Get_Info getinfo = new KNJ_Get_Info();
        KNJ_Get_Info.ReturnVal returnval = null;
        ret = svf.VrsOut("NENDO", KNJ_EditDate.getAutoFormatYear(db2, Integer.parseInt(param[0])) + "年度");
        //作成日(現在処理日)
        try {
            returnval = getinfo.Control(db2);
            ret = svf.VrsOut("DATE", KNJ_EditDate.getAutoFormatDate(db2, returnval.val3));
        } catch( Exception ex ){
