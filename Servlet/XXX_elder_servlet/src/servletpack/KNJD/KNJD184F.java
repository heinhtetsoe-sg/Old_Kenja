/*
 * $Id: 1e423745b2616098efa442bbd96b6d6c9c5f0fc8 $
 *
 * 作成日: 2012/02/21
 * 作成者: maesiro
 *
...
public class KNJD184F {

    private static final Log log = LogFactory.getLog(KNJD184F.class);
    ...
    private void printSvfMainSeiseki(
        ...
        final String form = "KNJD184F.frm";
...
//        svf.VrsOut("SCHOOL_NAME", _param._certifSchoolRemark3);
        ...
        svf.VrsOut("SEMESTER3_1", (String) _param._semesternameMap.get("1")); // 学期
        svf.VrsOut("SEMESTER3_2", (String) _param._semesternameMap.get("2")); // 学期
        svf.VrsOut("SEMESTER3_3", (String) _param._semesternameMap.get("9")); // 学期
    }
    ...
    private void printMoral(final Vrw32alp svf, final Student student) {
        for (final Iterator it = student._moralMap.keySet().iterator(); it.hasNext();) {
            final String semester = (String) it.next();
            final Moral moral = (Moral) student._moralMap.get(semester);
            if (moral == null) {
                continue;
            }

            List mlistwk = knjobj.retDividString(moral.getMoral(), moral.getDataInputKeta() * 2, moral.getDataInputGyo());
            if (mlistwk != null && mlistwk.size() > 0 && null != (String) mlistwk.get(0)) {
                final String sem = ("9".equals(semester)) ? "3" : semester;
                VrsOutnRenban(svf, "MORAL" + sem, mlistwk);
            }
        }
    }
    ...
    private void printSvfRecordRank(final Vrw32alp svf, final Student student) {
        ...
        for (final Iterator it = student._subclassList.iterator(); it.hasNext();) {
            ...
            for (final Iterator tit = testcdList.iterator(); tit.hasNext();) {
                ...
                if (_19900.equals(semtestcd)) {
                    svf.VrsOutn("SCORE1", line, detail._score); // 学期評価
                } else if (_29900.equals(semtestcd)) {
                    svf.VrsOutn("SCORE2", line, detail._score); // 学期評価
                } else if (_99901.equals(semtestcd)) {
                    svf.VrsOutn("VALUE", line, detail._score); // 学期評価
                }
            }
            line++;
        }
        ...
        if (Integer.parseInt(_param._knjSchoolMst._semesterDiv) <= Integer.parseInt(_param._semester)) {
            ...
            for (final Iterator it = student._subclassList.iterator(); it.hasNext();) {
                ...
                if (student.getAttendSubclass(student._gradeCourse).contains(subclass._subclasscd)) {
                    continue;
                }
                ...
                for (final Iterator tit = testcdList.iterator(); tit.hasNext();) {
                    ...
                    if (_99900.equals(semtestcd)) {
                        svf.VrsOutn("VIEW", line, detail._score); // 評定
                    }
                }
                line++;
            }
        ...
        if (null != scoreSubclass999999) {
            for (final Iterator it = testcdList.iterator(); it.hasNext();) {
                ...
                if (_19900.equals(semtestcd)) {
                    svf.VrsOutn("SCORE1", maxLine, detail._score); // 学期評価
                    svf.VrsOut("AVERAGE1", detail._avg); // 平均点
                } else if (_29900.equals(semtestcd)) {
                    svf.VrsOutn("SCORE2", maxLine, detail._score); // 学期評価
                    svf.VrsOut("AVERAGE2", detail._avg); // 平均点
                } else if (_99901.equals(semtestcd)) {
                    svf.VrsOutn("VALUE", maxLine, detail._score); // 学期評価
                    svf.VrsOut("AVERAGE3", detail._avg); // 概評
                }
            }
        }
    }
    ...
    private static class Student {
        ...
        Map _moralMap = Collections.EMPTY_MAP;
        ...
        public void load(final DB2UDB db2, final Param param) {
            ...
            _subclassList = Subclass.getSubclassList(db2, param, _schregno);
            _courseWeightingSubclassCdListMap = loadCourseWeightingSubclassCdListMap(db2, param);
            _moralMap = Moral.getMoralMap(db2, param, _schregno);
        }
...
//            }
        ...
        private static List getAttendSemesDatList(final DB2UDB db2, final Param param, final String schregno) {
            ...
            try {
                if (null == param._psMap.get("ATTEND_SEMES")) {
                    ...
                    param._psMap.put("ATTEND_SEMES", db2.prepareStatement(sql));
                }
                ps = (PreparedStatement) param._psMap.get("ATTEND_SEMES");
    ...
    private static class Subclass {
        ...
        private static String getSubclassSql(final Param param, final String schregno) {
            ...
            stb.append("     AND (T1.TESTKINDCD = '99' AND T1.TESTITEMCD = '00' AND T1.SCORE_DIV = '00' OR T1.TESTKINDCD = '99' AND T1.TESTITEMCD = '01' AND T1.SCORE_DIV = '00') ");
            ...
            stb.append("     AND (T1.TESTKINDCD = '99' AND T1.TESTITEMCD = '00' OR T1.TESTKINDCD = '99' AND T1.TESTITEMCD = '01') ");
     ...
     * 道徳
     */
    private static class Moral {
        final String _semester;
        final String _remark1;
        final String _datasize;
        ...
        private String getMoral() {
            return StringUtils.defaultString(_remark1, "");
        }
        private int getDataInputKeta() {
            int retVal = 22; //デフォルト値
            if ("".equals(_datasize) || _datasize == null) {
                return retVal;
            }
            String cutwk[] = StringUtils.split(_datasize, '*');
        ...
        public static Map getMoralMap(final DB2UDB db2, final Param param, final String schregno) {
            Map moralMap = new HashMap();
            PreparedStatement ps = null;
            ResultSet rs = null;
            try {
                final String sql = getMoralSql(param, schregno);
                //log.debug(" moral sql = " + sql);
                ps = db2.prepareStatement(sql);
                rs = ps.executeQuery();
                ...
                while (rs.next()) {
                    ...
                    concatwk += delim + StringUtils.defaultString(remark1, "");
                    chkWkMap.put(semester, concatwk);
                }

                for (Iterator it = chkWkMap.keySet().iterator(); it.hasNext();) {
                    final String semester = (String) it.next();
                    final String remark1 = (String) chkWkMap.get(semester);

                    final Moral moral = new Moral(semester, remark1, datasize);
                    moralMap.put(semester, moral);
                }
            } catch (SQLException e) {
                log.error("exception!", e);
            } finally {
        ...
        private static String getMoralSql(final Param param, final String schregno) {
            final StringBuffer stb = new StringBuffer();
