 ...
 * クラス別希望状況一覧
 */
public class KNJB240 {

    private static final Log log = LogFactory.getLog(KNJB240.class);
    ...
    private boolean printStudent(final Vrw32alp svf, final Kouza kouza) {
        ...
        for (final Iterator it2 = monthList.iterator(); it2.hasNext();) {
            final List monthSchChrDatList = (List) it2.next();
            for (final Iterator it = kouza._studentList.iterator(); it.hasNext();) {
                final Student student = (Student) it.next();

                svf.VrSetForm("KNJB240.frm", 4);
                svf.VrsOut("TITLE", "講座別　月単位　欠課状況一覧");
                ...
                String seqM = "";
                String seqS = "";
                for (final Iterator it3 = kouza._staffList.iterator(); it3.hasNext();) {
                    final Staff staff = (Staff) it3.next();
                ...
                for (final Iterator it3 = monthSchChrDatList.iterator(); it3.hasNext();) {
                    final SchChrDat schChrDat = (SchChrDat) it3.next();
                    svf.VrsOut("SEMESTER", schChrDat._semestername);
                    svf.VrsOut("MONTH", schChrDat._month + "月");
                    svf.VrsOut("DAY", schChrDat._day);
                    svf.VrsOut("WEEK", "(" + KNJ_EditDate.h_format_W(schChrDat._executedate) + ")");
                    svf.VrsOut("PERIODCD", schChrDat._periodabbv);
                    if (schChrDat._attendDatMap.containsKey(student._schregno)) {
                        ...
                        //欠課があるデータのみ出力する
                        if (attendDat._dicdname != null) {
                            svf.VrsOut("DI_NAME", attendDat._dicdname);
                            svf.VrsOut("REMARK", attendDat._diremark);
                            svf.VrEndRecord();
                            hasData = true;
                        }
                    }
