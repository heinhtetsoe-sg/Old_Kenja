// kanji=漢字
/*
 * $Id: a501424e00652fa74d72ed6d714bd9d540ce637e $
 *
 ...
 * <<クラスの説明>>。
 * @author m-yama
 * @version $Id: a501424e00652fa74d72ed6d714bd9d540ce637e $
 */
public class KNJH564 {

    private static final Log log = LogFactory.getLog(KNJH564.class);
    ...
    private boolean printMain(final HttpServletResponse response, final DB2UDB db2, final Vrw32alp svf) throws SQLException {
        ...
        if (_param._isTokiwa) {
            form = "KNJH564_2.frm";
        } else if ("1".equals(_param._notPrintJuni)) {
            form = "KNJH564.frm";
        } else {
            // 順位出力用
            form = "KNJH564_3.frm";
        ...
        if (_param._formFieldMap.isEmpty()) {
        	try {
        		_param._formFieldMap = SvfField.getSvfFormFieldInfoMapGroupByName(svf);
        	} catch (Throwable t) {
    ...
    private int fieldKeta(final String field) {
    	int keta = 0;
        try {
        	final SvfField f = _param._formFieldMap.get(field);
        	keta = f._fieldLength;
        } catch (Throwable t) {
        }
        return keta;
    ...
    private void setPrintOut(final Vrw32alp svf, final Student student, final int fieldNo) {
        ...
        if (!"1".equals(_param._notPrintSemestername)) {
        	svf.VrsOutn("SEMESTER", fieldNo, _param._semesterName);
        }
        svf.VrsOutn("TESTNAME", fieldNo, _param._testName);
        if (!_param._isTokiwa) {
            final String avgName = _param.getAvgName();
            svf.VrsOutn("AVG_NAME1", fieldNo, "学級");
            svf.VrsOutn("AVG_NAME2", fieldNo, avgName);
        }
        final String hrName = student._hrName + "(" + student._attendNo + ")";
        svf.VrsOutn("HR_NAME", fieldNo, hrName);
        svf.VrsOutn("NAME" + (KNJ_EditEdit.getMS932ByteLength(student._name) > 16 ? "" : "1"), fieldNo, student._name);

        int fieldCnt = 1;
        for (final String subclassCd : student._testSubclass.keySet()) {
            final TestScore testScore = student._testSubclass.get(subclassCd);
            String subclassnameField = "SUBCLASS" + fieldCnt;
            if (fieldKeta(subclassnameField) < KNJ_EditEdit.getMS932ByteLength(testScore._name) && fieldKeta(subclassnameField) < fieldKeta(subclassnameField + "_2")) {
            	subclassnameField = subclassnameField + "_2";
            }
            svf.VrsOutn(subclassnameField, fieldNo, testScore._name);
            svf.VrsOutn("SCORE" + fieldCnt, fieldNo, StringUtils.defaultString(testScore._score, ""));
...
//            svf.VrsOutn("CLASS_RANK" + fieldCnt, fieldNo, testScore._rank2);
        ...
        if (student._hasScore) {
            ...
            svf.VrsOutn("AVERAGEL_SCORE", fieldNo, StringUtils.defaultString(student._averageScore, ""));
            log.debug("個人平均点 = " + student._averageScore);
        }
    }

    private void setPrintData(final DB2UDB db2, final List<Student> studentList) throws SQLException {
    ...
    private static class Student {
        ...
        public static List<Student> getStudentList(final DB2UDB db2, final Param param) throws SQLException  {
            final List<Student> rtnStudent = new ArrayList<Student>();
...
//                    + " 席次：" + _rank;
        ...
        public static void setRank(final DB2UDB db2, final List<Student> studentList, final Param param) throws SQLException {
            PreparedStatement ps = null;
            ResultSet rs = null;
            try {
        ...
        private static String getRankSql(final Param param) {
            ...
            stb.append(" INNER JOIN PROFICIENCY_SUBCLASS_MST L1 ON L1.PROFICIENCY_SUBCLASS_CD=T1.PROFICIENCY_SUBCLASS_CD");
            ...
            stb.append("  L3.rank_data_div ='" + param._rankDataDiv + "' ");
            stb.append("  ) OR ( ");
            stb.append(" L3.rank_data_div IS NULL AND l3.rank_div IS NULL AND T1.SCORE_DI = '*' ");
            stb.append("  )) AND ");
        ...
        public static void setRankALL(final DB2UDB db2, final List<Student> studentList, final Param param) throws SQLException {
            PreparedStatement ps = null;
            ResultSet rs = null;
            try {
        ...
        public static void setAvg(final DB2UDB db2, final List<Student> studentList, final Param param) throws SQLException {
            PreparedStatement ps = null;
            ResultSet rs = null;
    ...
    private Param createParam(final DB2UDB db2, final HttpServletRequest request) throws Exception {
        log.fatal("$Revision: 75595 $"); // CVSキーワードの取り扱いに注意
...
//        final String _rankDiv;
        ...
        final String _notPrintSemestername;
        ...
        final String _nendo;
        Map<String, SvfField> _formFieldMap = new HashMap();
...
//            _rankDiv = request.getParameter("RANK_DIV");
            ...
            _notPrintSemestername = request.getParameter("NOT_PRINT_SEMESTERNAME");
        ...
        private String getTestName(final DB2UDB db2) throws SQLException {
            String rtn = StringUtils.defaultString(KnjDbUtils.getOne(KnjDbUtils.query(db2, "SELECT PROFICIENCYNAME1 FROM PROFICIENCY_MST WHERE PROFICIENCYDIV = '" + _proficiencydiv + "' AND PROFICIENCYCD = '" + _proficiencycd + "' ")));
            return rtn;
        }

        private String getSemesterName(final DB2UDB db2) throws SQLException {
            String rtn = StringUtils.defaultString(KnjDbUtils.getOne(KnjDbUtils.query(db2, "SELECT SEMESTERNAME FROM SEMESTER_MST WHERE YEAR = '" + _year + "' AND SEMESTER = '" + _semester + "' ")));
            return rtn;
        }
