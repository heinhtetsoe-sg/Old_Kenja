...
public class KNJH442C {

    private static final Log log = LogFactory.getLog(KNJH442C.class);
...
//        }
    ...
    private void printSvfMain(final DB2UDB db2, final Vrw32alp svf, final List subclassList, final List studentList) {
        final String form = "KNJH442C.frm";

        for (Iterator iterator = studentList.iterator(); iterator.hasNext();) {
            final Student student = (Student) iterator.next();

            svf.VrSetForm(form , 1);
            printTitle(db2, svf, subclassList, student); //明細部以外を印字

            //科目評定
            final String[] classCd = {"01","02","03","04","05","06"};
            final String[] className = {"国語","地理","公民","数学","理科","英語"};
            final Map classAveMap = getRecordRankAvg(db2, student._schregno);
            for (int i = 0; i < classCd.length; i++) {
            	final String cd = classCd[i];
            	final String name = className[i];
            	final int column = i+1;

            	svf.VrsOut("CLASS_NAME1_" + column, name);

                //評定平均
                svf.VrsOut("CLASS_AVE" + column, (String) classAveMap.get(cd));

                int line = 1;
                final Map map = getSubclassName(db2, student._schregno, cd);
                for (final Iterator it = map.keySet().iterator(); it.hasNext();) {
                    final String subclassCd = (String) it.next();
                    final String subclassName = (String) map.get(subclassCd);
                    final String field = KNJ_EditEdit.getMS932ByteLength(subclassName) > 16 ? "4" : KNJ_EditEdit.getMS932ByteLength(subclassName) > 12 ? "3" : KNJ_EditEdit.getMS932ByteLength(subclassName) > 10 ? "2" : "1";
                    svf.VrsOutn("SUBCLASS_NAME" + column + "_" + field, line, subclassName); //科目名称
                    if (student._printSubclassMap.containsKey(subclassCd)) {
                    	...
                    	svf.VrsOutn("SUBCLASS_DIV" + column, line, scoreData.score(SDIV9990009)); //評定
                    	line++;
                    }
                }
            }
            ...
            svf.VrsOut("CLASS_NAME2_2", "数学");
            svf.VrsOut("CLASS_NAME2_3", "英語");
            ...
            if(map != null) {
            	...
            	//実力テスト 明細
                svf.VrsOut("MOCK_SCORE1", (String)map.get("MOCK_NATIONAL_LANGUAGE_AVG"));
                svf.VrsOut("MOCK_SCORE2", (String)map.get("MOCK_MATH_AVG"));
                svf.VrsOut("MOCK_SCORE3", (String)map.get("MOCK_ENGLISH_AVG"));
                svf.VrsOut("MOCK_SCORE4", (String)map.get("MOCK_TOTAL_AVG"));

                //総合成績
                svf.VrsOutn("TOTAL_SCORE", 1, (String)map.get("TEST_VALUATION_AVG")); //評定平均
                svf.VrsOutn("TOTAL_SCORE", 2, (String)map.get("TEST_VALUATION_PERCENT_SCORE")); //評定80
                svf.VrsOutn("TOTAL_SCORE", 3, (String)map.get("MOCK_TOTAL_PERCENT_SCORE")); //実力20
                svf.VrsOutn("TOTAL_SCORE", 4, (String)map.get("TOTAL_SCORE")); //総合成績
            }
	...
	private void printTitle(final DB2UDB db2, final Vrw32alp svf, final List subclassList, final Student student) {
    	//明細部以外を印字

        //ヘッダ
    	String title = student._coursename;
    	final Map mockTrainremarkMap = getMockTrainremark(db2, student._schregno);
    	if("1".equals(_param._titlediv)){
    		title += " 指定校推薦 ";
    		title += (String) mockTrainremarkMap.get("HOPE_COLLEGE_NAME1");
    		title += (String) mockTrainremarkMap.get("HOPE_COURSE_NAME1");
    	} else {
    		title += " 日大付属特別選抜 ";
    		title += (String) mockTrainremarkMap.get("HOPE_COLLEGE_NAME2");
    		title += (String) mockTrainremarkMap.get("HOPE_COURSE_NAME2");
    	}
        ...
        //行動の記録
        svf.VrsOutn("ACT_REC", 1, (String) mockTrainremarkMap.get("GRADE1_REMARK"));
        svf.VrsOutn("ACT_REC", 2, (String) mockTrainremarkMap.get("GRADE2_REMARK"));
        svf.VrsOutn("ACT_REC", 3, (String) mockTrainremarkMap.get("GRADE3_REMARK"));

        //条件
        VrsOutnRenban(svf, "CONDITION", KNJ_EditEdit.get_token((String) mockTrainremarkMap.get("CONDITION"), 50, 3));
    ...
    private void printAttend(final DB2UDB db2, final Vrw32alp svf, final Student student) {
    	int line = 1;
    	final Map attend = getAttend(db2, student._schregno);
    	for (final Iterator it = attend .keySet().iterator(); it.hasNext();) {
            final String year = (String) it.next();
            final Map att = (Map) attend.get(year);
            svf.VrsOutn("ABSENT", line, (String) att.get("SICK")); // 欠席
            svf.VrsOutn("LATE", line, (String) att.get("LATE"));   // 遅刻
            svf.VrsOutn("EARLY", line, (String) att.get("EARLY")); // 早退
            line++;
        }
    }
...
//                student._hrname = Integer.parseInt(rs.getString("GRADE_CD")) + "年" + rs.getString("HR_CLASS_NAME1") + "組" ;
    ...
    //過年度含む履修科目を取得
    private Map getSubclassName (final DB2UDB db2, final String schregno, final String classcd) {
        ...
        try {
            ...
            while (rs.next()) {
            	...
            	rtnMap.put(subclasscd, StringUtils.defaultString(rs.getString("SUBCLASSNAME")));
            }
        } catch (SQLException ex) {
            log.debug("Exception:", ex);
        } finally {
    ...
    //過年度含む履修科目の評定平均
    private Map getRecordRankAvg (final DB2UDB db2, final String schregno) {
        ...
        stb.append("   AVG(R1.SCORE) AS SCORE_AVG ");
        stb.append(" FROM ");
        stb.append("   SCHNO T1 ");
        stb.append("   INNER JOIN RECORD_RANK_SDIV_DAT R1 ");
        stb.append("      ON R1.YEAR = T1.YEAR ");
        stb.append("     AND R1.SEMESTER || R1.TESTKINDCD || R1.TESTITEMCD || R1.SCORE_DIV = '"+ SDIV9990009 +"' ");
        stb.append("     AND R1.SCHREGNO = T1.SCHREGNO ");
        stb.append("   INNER JOIN CHAIR_A T2 ");
        stb.append("      ON T2.SUBCLASSCD    = R1.SUBCLASSCD ");
        stb.append("     AND T2.SUBCLASSCD    = R1.SUBCLASSCD ");
        stb.append("     AND T2.CLASSCD       = R1.CLASSCD ");
        stb.append("     AND T2.SCHOOL_KIND   = R1.SCHOOL_KIND ");
        stb.append("     AND T2.CURRICULUM_CD = R1.CURRICULUM_CD ");
        stb.append("   INNER JOIN CLASS_MST T3 ");
        stb.append("      ON T3.CLASSCD     = R1.CLASSCD ");
        stb.append("     AND T3.SCHOOL_KIND = R1.SCHOOL_KIND ");
        stb.append("   INNER JOIN SUBCLASS_MST T4 ");
        stb.append("      ON T4.SUBCLASSCD    = R1.SUBCLASSCD ");
        stb.append("     AND T4.CLASSCD       = R1.CLASSCD ");
        stb.append("     AND T4.SCHOOL_KIND   = R1.SCHOOL_KIND ");
        stb.append("     AND T4.CURRICULUM_CD = R1.CURRICULUM_CD ");
        stb.append(" GROUP BY ");
        stb.append("   R1.CLASSCD ");
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
            	final String key =  StringUtils.defaultString(rs.getString("CLASSCD"));
            	final String val =  StringUtils.defaultString(rs.getString("SCORE_AVG"));
                rtnMap.put(key, val);
            }
        } catch (SQLException ex) {
            log.debug("Exception:", ex);
    ...
    //出欠状況の取得
    private Map getAttend (final DB2UDB db2, final String schregno) {
    	Map rtnMap = new TreeMap();
    	final String year = _param._edate.substring(0,4);
    	final String month = _param._edate.substring(5,7);
        final StringBuffer stb = new StringBuffer();
        stb.append(" WITH SCHNO_A AS( ");
        stb.append(" SELECT DISTINCT ");
        stb.append("     REGD.YEAR, ");
        ...
        stb.append("   SUM(VALUE(T2.LATE, 0)) AS LATE, ");
        stb.append("   SUM(VALUE(T2.EARLY, 0)) AS EARLY ");
        stb.append(" FROM ");
        stb.append("   SCHNO T1 ");
        stb.append("   LEFT JOIN V_ATTEND_SEMES_DAT T2 ");
        stb.append("          ON T2.YEAR = T1.YEAR ");
        stb.append("         AND T2.SCHREGNO = T1.SCHREGNO ");
        stb.append("         AND T2.YEAR  <= '"+ year +"' ");
        stb.append("         AND T2.MONTH <= '"+ month +"' ");
        ...
        try {
            ...
            while (rs.next()) {
            	...
            	map.put("SICK", StringUtils.defaultString(rs.getString("SICK")));
            	map.put("LATE", StringUtils.defaultString(rs.getString("LATE")));
            	map.put("EARLY", StringUtils.defaultString(rs.getString("EARLY")));
            	rtnMap.put(StringUtils.defaultString(rs.getString("YEAR")), map);
            }
        } catch (SQLException ex) {
            log.debug("Exception:", ex);
        } finally {
    ...
    private Map getAftSchregRecommendationRank (final DB2UDB db2, final String schregno) {
        ...
        try {
            ...
            while (rs.next()) {
            	...
            	rtnMap.put("MOCK_TOTAL_SCORE_GRADE_RANK", StringUtils.defaultString(rs.getString("MOCK_TOTAL_SCORE_GRADE_RANK")));
            	rtnMap.put("MOCK_TOTAL_SCORE_CLASS_RANK", StringUtils.defaultString(rs.getString("MOCK_TOTAL_SCORE_CLASS_RANK")));
            	rtnMap.put("MOCK_TOTAL_SCORE_COURSE_RANK", StringUtils.defaultString(rs.getString("MOCK_TOTAL_SCORE_COURSE_RANK")));
            	rtnMap.put("TOTAL_SCORE", sishaGonyu(StringUtils.defaultString(rs.getString("TOTAL_SCORE")), 2));
            	rtnMap.put("CNT", StringUtils.defaultString(rs.getString("CNT")));
            }
        } catch (SQLException ex) {
            log.debug("Exception:", ex);
        } finally {
    ...
    //所見
    private Map getMockTrainremark (final DB2UDB db2, final String schregno) {
        Map rtnMap = new HashMap();
        final StringBuffer stb = new StringBuffer();
        stb.append(" SELECT ");
        stb.append("   * ");
        stb.append(" FROM ");
        stb.append("   MOCK_TRAINREMARK_DAT ");
        stb.append(" WHERE ");
        ...
        try {
            ...
            rs = ps.executeQuery();
            while (rs.next()) {
            	rtnMap.put("GRADE1_REMARK", StringUtils.defaultString(rs.getString("GRADE1_REMARK")));
            	rtnMap.put("GRADE2_REMARK", StringUtils.defaultString(rs.getString("GRADE2_REMARK")));
            	rtnMap.put("GRADE3_REMARK", StringUtils.defaultString(rs.getString("GRADE3_REMARK")));
            	rtnMap.put("CONDITION", StringUtils.defaultString(rs.getString("CONDITION")));
            	rtnMap.put("HOPE_COLLEGE_NAME1", StringUtils.defaultString(rs.getString("HOPE_COLLEGE_NAME1")));
            	rtnMap.put("HOPE_COURSE_NAME1", StringUtils.defaultString(rs.getString("HOPE_COURSE_NAME1")));
            	rtnMap.put("HOPE_COLLEGE_NAME2", StringUtils.defaultString(rs.getString("HOPE_COLLEGE_NAME2")));
            	rtnMap.put("HOPE_COURSE_NAME2", StringUtils.defaultString(rs.getString("HOPE_COURSE_NAME2")));
            }
        } catch (SQLException ex) {
            log.debug("Exception:", ex);
        } finally {
        ...
        private void setSubclass(final DB2UDB db2) {
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    final String score = sishaGonyu(StringUtils.defaultString(rs.getString("SCORE")), 1);
                    final String avg = sishaGonyu(StringUtils.defaultString(rs.getString("AVG")), 1);
...
//            if (null == dateRange || null == dateRange._sdate || null == dateRange._edate || dateRange._sdate.compareTo(param._sdate) > 0) {
            if (null == dateRange || null == dateRange._sdate || null == dateRange._edate) {
                return;
            }
//            final String edate = dateRange._edate.compareTo(param._edate) > 0 ? param._edate : dateRange._edate;
            final String edate = dateRange._edate;
...
//            _sdate = getSemesterMst(db2, SEMEALL, "SDATE"); //年度開始日
//            _edate = getSemesterMst(db2, SEMEALL, "EDATE"); //年度終了日
            _sdate = request.getParameter("S_MONTH") + "-01";
            _edate = request.getParameter("E_MONTH") + "-01";
        ...
        private static String getDbPrginfoProperties(final DB2UDB db2, final String propName) {
            return KnjDbUtils.getOne(KnjDbUtils.query(db2, " SELECT VALUE FROM PRGINFO_PROPERTIES WHERE PROGRAMID = 'KNJH442C' AND NAME = '" + propName + "' "));
        }
        ...
        private void setSubclassMst(
            ...
            try {
            	...
            	stb.append(" AND SCHREGNO IN " + SQLUtils.whereIn(true, _categorySelected));
                stb.append(" ), RECORD AS( ");
            	stb.append("   SELECT DISTINCT ");
