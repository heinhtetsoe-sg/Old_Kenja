/*
 * $Id: 8c812607844f53c765da8a84d958ab12a185b4b5 $
 *
 * 作成日: 2012/02/21
 * 作成者: maesiro
 *
...
public class KNJH566A {

    private static final Log log = LogFactory.getLog(KNJH566A.class);
    ...
    private void printStudent(
        ...
        final String form = "KNJH566A.frm";
    ...
    private void printSvfPrificiencyRank(final Vrw32alp svf, final Student student, final List profCdList, final List subclassProfList, final ScoreSubclass subclassProf999999) {
        for (int profcdi = 0; profcdi < profCdList.size() && profcdi < 10; profcdi++) {
            ...
            final String profName = StringUtils.defaultString((String) _param._proficiencyNameMap.get(profCd));
...
//        final List allvalueList1 = new ArrayList();
//        for (int gradei = 0; gradei < grades.length; gradei++) {
//            final String grade = grades[gradei];
//            allvalueList1.addAll((List) gradevalueListMap.get(grade));
//        }
//        svf.VrsOut("THURU_VALUE", "(通年)" + String.valueOf(getListAvg(allvalueList1))); // 通年
    ...
    private void printHyouka(final Vrw32alp svf, final String[] grades, final List subclassList, final ScoreSubclass subclass999999) {
        ...
        // final List allvalueList = new ArrayList();
        for (int gradei = 0; gradei < grades.length; gradei++) {
            ...
            for (final Iterator it = subclassList.iterator(); it.hasNext();) {
                ...
                if (scorerank != null && NumberUtils.isNumber(scorerank._score)) {
                    ...
                    // allvalueList.add(scorerank._score);
                }
            }
            svf.VrsOut("TOTAL_VALUE" + String.valueOf(gradei + 1), getListAvg(valueList)); // 評定
        }
        // final String hyouteiHeikin = StringUtils.defaultString(getListAvg(allvalueList));
    ...
    private static class ScoreSubclass {
        ...
        public static List getScoreSubclassList(final DB2UDB db2, final Param param, final String schregno) {
            ...
            final KNJH566A.KNJE070_1 e70 = new KNJH566A.KNJE070_1(db2, definecode, schregno, param._year, param._semester, m);
            for (final Iterator it = e70._printData._studyrecData.iterator(); it.hasNext();) {
                final KNJH566A.KNJE070_1.StudyrecClass sc = (KNJH566A.KNJE070_1.StudyrecClass) it.next();
                ...
                for (final Iterator itd = sc._studyrecDatList.iterator(); itd.hasNext();) {
                    final KNJH566A.KNJE070_1.StudyrecDat sd = (KNJH566A.KNJE070_1.StudyrecDat) itd.next();    
                    ...
                    for (final Iterator itg = sd._gradesList.iterator(); itg.hasNext();) {
                        final KNJH566A.KNJE070_1.Grades grades = (KNJH566A.KNJE070_1.Grades) itg.next();
        ...
        private static String getScoreSubclassSql(final Param param, final String schregno) {
            ...
            stb.append("     T3.GRADE, T3.COURSECD, T3.MAJORCD, T3.COURSECODE, T4.GROUP_CD ");
            stb.append(" FROM  ");
            ...
            if ("2".equals(param._kijunten)) {
                stb.append("     AND T1.RANK_DATA_DIV = '03' "); // 平均点
            } else {
            ...
            if ("1".equals(param._juni)) {
                ...
                stb.append("     AND T1.RANK_DIV = '03' "); // コース
            } else {
                stb.append("     AND T1.RANK_DIV = '05' "); // コースグループ
            }
//            // 実力合併元科目は表示対象から除く
...
//            stb.append(" WHERE T6.SUBCLASSCD IS NOT NULL ");
            ...
            stb.append("     CAST(NULL AS VARCHAR(8)) AS PROFICIENCY_SUBCLASS_CD ");
            stb.append(" FROM ");
            ...
            stb.append("     '" + JITURYOKU + "' AS TEST_DIV_NAME, ");
            if ("1".equals(param._useCurriculumcd)) {
//                stb.append("     (CASE WHEN T1.PROFICIENCY_SUBCLASS_CD = '999999' THEN '00' ELSE T2.CLASSCD END) AS CLASSCD, ");
//                stb.append("     (CASE WHEN T1.PROFICIENCY_SUBCLASS_CD = '999999' THEN '0' ELSE T2.SCHOOL_KIND END) AS SCHOOL_KIND, ");
//                stb.append("     (CASE WHEN T1.PROFICIENCY_SUBCLASS_CD = '999999' THEN '00' ELSE T2.CURRICULUM_CD END) AS CURRICULUM_CD, ");
                stb.append("     (CASE WHEN T1.PROFICIENCY_SUBCLASS_CD = '999999' THEN '00' ELSE T3.CLASSCD END) AS CLASSCD, ");
                stb.append("     (CASE WHEN T1.PROFICIENCY_SUBCLASS_CD = '999999' THEN '0' ELSE T3.SCHOOL_KIND END) AS SCHOOL_KIND, ");
                stb.append("     (CASE WHEN T1.PROFICIENCY_SUBCLASS_CD = '999999' THEN '00' ELSE T3.CURRICULUM_CD END) AS CURRICULUM_CD, ");
            } else {
//                stb.append("     (CASE WHEN T1.PROFICIENCY_SUBCLASS_CD = '999999' THEN '00' ELSE SUBSTR(T2.SUBCLASSCD, 1, 2) END) AS CLASSCD, ");
                stb.append("     (CASE WHEN T1.PROFICIENCY_SUBCLASS_CD = '999999' THEN '00' ELSE SUBSTR(T3.SUBCLASSCD, 1, 2) END) AS CLASSCD, ");
            }
//            stb.append("     (CASE WHEN T1.PROFICIENCY_SUBCLASS_CD = '999999' THEN '999999' ELSE T2.SUBCLASSCD END) AS SUBCLASSCD, ");
            stb.append("     (CASE WHEN T1.PROFICIENCY_SUBCLASS_CD = '999999' THEN '999999' ELSE T3.PROFICIENCY_SUBCLASS_CD END) AS SUBCLASSCD, ");
    ...
    private class Param {
       ...
       /** 各学校における定数等設定 */
        ...
        Param(final DB2UDB db2, final HttpServletRequest request) throws SQLException {
            ...
            _proficiencyNameMap = getProficiencyName(db2);
        }
        ...
        private Map getProficiencyName(final DB2UDB db2) {
            Map rtn = new HashMap();
            PreparedStatement ps = null;
            ResultSet rs = null;
            try {
                final StringBuffer sql = new StringBuffer();
                sql.append(" SELECT ");
                sql.append("     T2.YEAR || T2.SEMESTER || T1.PROFICIENCYDIV || T1.PROFICIENCYCD AS SEMTESTCD, ");
                sql.append("     T1.PROFICIENCYNAME1 ");
                sql.append(" FROM ");
                sql.append("     PROFICIENCY_MST T1 ");
                sql.append("     INNER JOIN (SELECT DISTINCT YEAR, SEMESTER, PROFICIENCYDIV, PROFICIENCYCD FROM PROFICIENCY_YMST) T2 ON T2.PROFICIENCYDIV = T1.PROFICIENCYDIV ");
                sql.append("         AND T2.PROFICIENCYCD = T1.PROFICIENCYCD ");
                sql.append(" ORDER BY ");
                sql.append("     T2.YEAR || T2.SEMESTER || T1.PROFICIENCYDIV || T1.PROFICIENCYCD ");                
                
                ps = db2.prepareStatement(sql.toString());
                rs = ps.executeQuery();
                while (rs.next()) {
                    rtn.put(rs.getString("SEMTESTCD"), rs.getString("PROFICIENCYNAME1"));
                }
            } catch (SQLException ex) {
                log.debug("getZ010 exception!", ex);
            } finally {
...
//         * @param sqlStudyrec
//         */
//        protected Map setGradeTitle(
//                final PrintData printData,
//                final SqlStudyrec sqlStudyrec
//        ) {
//            final Map titles = new TreeMap();
//            
//            final String sql = sqlStudyrec.getGakusekiSqlString(printData);
//            
//            PreparedStatement ps = null;
//            ResultSet rs = null;
//            try {
...
//                rs = ps.executeQuery();
//
//                final boolean isPrintGradeTitleGakunen = isPrintGradeTitleGakunen(printData);
//                while (rs.next()) {
//                    if (isPrintGradeTitleGakunen && null != rs.getString("DROP_YEAR")) {
//                        printData._ryunenYears.add(rs.getString("DROP_YEAR"));
//                        continue;
//                    }
//                    
//                    final String year = rs.getString("YEAR");
//                    final String annual = rs.getString("ANNUAL");
//                    final String key = isPrintGradeTitleGakunen ? annual : year;
//                    if (null == key) {
//                        log.debug("年度または年次がnull === SCHREGNO=" + printData._schregno + "  KEY=" + key + "===");
//                        continue;
//                    }
//                    final int intKey = getIntKey(key, printData);
//                    if (0 > intKey) {
//                        continue;
//                    }
//                    final String k = (0 == intKey) ? "0" : key;
//                    final Title title = new Title(key, intKey, year, annual, printData);
//                    titles.put(k, title);
//                }
//                if (isPrintGradeTitleGakunen) {
//                    log.fatal(" 留年した年度 = " + printData._ryunenYears);
//                }
//
...
//        protected int getIntKey(final String key, final PrintData printData) {
//            int intKey = Integer.parseInt(key);
//            if (isPrintGradeTitleGakunen(printData)) {
//                if (_param._isChuKouIkkan && 4 > intKey) {
//                    intKey = -1;
//                }
//                if (_param._isHesetuKou  && 3 < intKey) {
//                    intKey -= 3;
//                }
//                if (intKey < 0) {
//                    log.warn(" key = " + key + ", intKey = " + intKey + "(cyukouikkan = " + _param._isChuKouIkkan + ", heisetukou = " + _param._isHesetuKou + ")");
//                }
//            }
//            return intKey;
//        }

//        protected void printGradeTitle(final PrintData printData, final Vrw32alp svf) {
//            for (final Iterator it = printData._titles.keySet().iterator(); it.hasNext();) {
//                final String key = (String) it.next();
//                final Title title = (Title) printData._titles.get(key);
//                
//                final boolean isGakunen = isPrintGradeTitleGakunen(printData);
//                final String[] nameArray = getTitlenameArray(printData, title);
//                final int i = title._pos;
//                if (_param._isTokiwa) {
//                    svf.VrsOut("GRADE1_" + i + "_4" , title._name);  // 学習の記録　左
//                    svf.VrsOut("GRADE2_" + i + "_4" , title._name);  // 学習の記録　右
//                } else {
//                    svf.VrsOut("GRADE1_" + i + "_1" , nameArray[0]);  // 学習の記録　左
//                    svf.VrsOut("GRADE1_" + i + "_2" , nameArray[1]);
//                    svf.VrsOut("GRADE1_" + i + "_3" , nameArray[2]);
//                    svf.VrsOut("GRADE2_" + i + "_1" , nameArray[0]);  // 学習の記録　右
//                    svf.VrsOut("GRADE2_" + i + "_2" , nameArray[1]);
//                    svf.VrsOut("GRADE2_" + i + "_3" , nameArray[2]);
//                }
//                
//                if (_param._isFormTitleFormat2) {
//                    svf.VrsOut("GRADE3_" + i + "_1" , title._name);  // 出欠の記録　左
//                    svf.VrsOut("GRADE6_" + i + "_1" , title._name);  // 出欠の記録　右
//                } else if (_param._isTokiwa) {
//                    if (NumberUtils.isDigits(title._annual)) {
//                        svf.VrsOut("GRADE3_" + i + "_1" , String.valueOf(Integer.parseInt(title._annual)));  // 出欠の記録　左
//                        svf.VrsOut("GRADE6_" + i + "_1" , String.valueOf(Integer.parseInt(title._annual)));  // 出欠の記録　右
//                    }
//                } else if (!isGakunen || "入学前".equals(title._name)) {
//                    svf.VrsOut("GRADE3_" + i + "_1" , nameArray[0]);  // 出欠の記録　左
//                    svf.VrsOut("GRADE3_" + i + "_2" , nameArray[1]);
//                    svf.VrsOut("GRADE3_" + i + "_3" , nameArray[2]);
//                    svf.VrsOut("GRADE6_" + i + "_1" , nameArray[0]);  // 出欠の記録　右
//                    svf.VrsOut("GRADE6_" + i + "_2" , nameArray[1]);
//                    svf.VrsOut("GRADE6_" + i + "_3" , nameArray[2]);
//                } else {
//                    svf.VrsOut("GRADE3_" + i + "_2" , nameArray[1]);  // 出欠の記録　左
//                    svf.VrsOut("GRADE6_" + i + "_2" , nameArray[1]);  // 出欠の記録　右
//                }
//                if (_param._isTokiwa) {
//                    svf.VrsOut("GRADE5_" + i + "_4" , title._name);  // 指導上参考となる諸事項
//                } else if (!isGakunen) {
//                    svf.VrsOut("GRADE5_" + i + "_1" , nameArray[0]);  // 指導上参考となる諸事項
//                    svf.VrsOut("GRADE5_" + i + "_2" , nameArray[1]);
//                    svf.VrsOut("GRADE5_" + i + "_3" , nameArray[2]);
//                } else {
//                    svf.VrsOut("GRADE5_" + i + "_2" , title._name);  // 指導上参考となる諸事項
//                }
//                svf.VrsOut("GRADE4_" + i, title._name);  // 特別活動の記録
//            }
//        }
...
//            final String certifKindCd = (String) printData._paramap.get("CERTIFKIND");
...
//                        final String specialDiv = studyrecDat._specialDiv;
                        // log.debug(" studyrecDat = [" + studyrecDat.toString()+ "]");

//                        final String keySubclassCd = "1".equals(_param._useCurriculumcd) ? classcd + "-" + schoolKind + "-" + curriculumCd + "-" + subclasscd : subclasscd;
...
//                            svf.VrsOut("R2",       StringUtils.defaultString(studyrecDat._assessLevel));    //学習成績概評
//                            svf.VrsOut("MARU",     StringUtils.defaultString(studyrecDat._subclassname));    //特Ａ
                        }
...
//                            for (Iterator itg = studyrecDat._gradesList.iterator(); itg.hasNext();) {
//                                final Grades grades = (Grades) itg.next();
//                                hasGrades = printData._titles.containsKey(grades._annualYear) || hasGrades;
//                            }
                            if (!hasGrades) { continue; }
                            // 年次（年度）が０の場合は以降の処理をしない
//                            if ("0".equals(_definecode.schooldiv) && "0".equals(rs.getString("ANNUAL"))) {
//                                continue;
//                            }
                            // 未履修科目は出力しないの場合
                            ...
                            //  科目コードのブレイク
//                            s_subclasscd = studyrecDat._subclasscd;
//                            final String ccd = studyrecDat._classcd + ("1".equals(_param._useCurriculumcd) ? studyrecDat._schoolKind : "");
//                            svf.VrsOut("CLASSCD", StringUtils.defaultString(ccd));         //教科コード
//                            if (credit != null) {
//                                svf.VrsOut("CREDIT", credit); //修得単位数
//                            }
                            
//                            //  科目評定(学年)の出力
//                            for (Iterator itg = studyrecDat._gradesList.iterator(); itg.hasNext();) {
//                                final Grades grades = (Grades) itg.next();
//                                if (null != grades._grades) {
//                                    final Title title = (Title) printData._titles.get(grades._annualYear);
//                                    final int gradeInt = Double.valueOf(grades._grades).intValue();
//                                    if (null != title && 0 != gradeInt) {
//                                        svf.VrsOut("GRADES" + title._pos, String.valueOf(gradeInt));
//                                    }
//                                }
//                            }
                            svf.VrEndRecord();
...
//                        s_classcd = studyrecClass._classcd;
//                        s_schoolKind = studyrecClass._schoolKind;
                    }
                } catch (Exception e) {
                    log.error("exception!", e);
                }
...
//        protected void setForm(final PrintData printData) {
//            printGradeTitle(printData);
//        }
...
//                printData._titles = setGradeTitle(printData, sqlStudyrec);
//                printData.setTitlePosition();
...
//            _param._isFormTitleFormat2 = _param._isTottori || _param._isMeiji || _isFormKyoto || _param._isKyoai;
        }
...
//            sqlStudyrec.setTokuA(true);
         ...
         * <<クラスの説明>>。
         * @author yamasiro
         * @version $Id: 8c812607844f53c765da8a84d958ab12a185b4b5 $
         */
...
//             */
//            public void setTokuA(final boolean atype) {
//                _isTokuA = atype;
//            }
...
//                if (_param._isJisyuukan) {
//                    stb.append("    WHERE CLASSCD BETWEEN '" + KNJDefineSchool.subject_D + "' AND '" + KNJDefineSchool.subject_U + "' OR SUBCLASSCD = '941001'");
//                } else {
...
//                    if (_param._isJisyuukan) {
//                        stb.append("    WHERE   CLASSCD BETWEEN '" + KNJDefineSchool.subject_D + "' AND '"+KNJDefineSchool.subject_U + "' OR SUBCLASSCD = '941001'");
//                    } else if (_param._isHosei) {
//                        stb.append("    WHERE   CLASSCD BETWEEN '" + KNJDefineSchool.subject_D + "' AND '86' ");
//                    } else {
                        stb.append("    WHERE   CLASSCD BETWEEN '" + KNJDefineSchool.subject_D + "' AND '"+KNJDefineSchool.subject_U + "'");
//                    }
...
//                if (_param._isJisyuukan) {
//                    stb.append(" WHERE  T1.CLASSCD BETWEEN '" + KNJDefineSchool.subject_D + "' AND '" + KNJDefineSchool.subject_U + "' OR T1.SUBCLASSCD = '941001'");
//                } else {
...
//                        stb.append(" WHERE CLASSCD BETWEEN '" + KNJDefineSchool.subject_D + "' AND '" + KNJDefineSchool.subject_U + "' OR SUBCLASSCD = '941001'");
//                    } else if (_param._isHosei) {
//                        stb.append(" WHERE CLASSCD BETWEEN '" + KNJDefineSchool.subject_D + "' AND '85'");
//                    } else {
                        stb.append(" WHERE CLASSCD BETWEEN '" + KNJDefineSchool.subject_D + "' AND '" + KNJDefineSchool.subject_U + "' ");
//                    }
                }
...
//                    stb.append("       AND (T1.CLASSCD BETWEEN '" + KNJDefineSchool.subject_D + "' AND '" + KNJDefineSchool.subject_U+"' OR T1.CLASSCD = '" + KNJDefineSchool.subject_T + "' OR T1.SUBCLASSCD = '941001')");
//                } else if (_param._isHirokoku) {
//                    stb.append("       AND (T1.CLASSCD BETWEEN '" + KNJDefineSchool.subject_D + "' AND '" + KNJDefineSchool.subject_U+"' OR T1.CLASSCD = '" + KNJDefineSchool.subject_T + "' OR T1.CLASSCD = '" + KNJDefineSchool.subject_L + "')");
//                } else {
...
//            private Map _titles = Collections.EMPTY_MAP;  // 学年（年度）項目名
...
//                int i = 0;
//                for (final Iterator it = _titles.keySet().iterator(); it.hasNext();) {
//                    final String key = (String) it.next();
//                    final Title title = (Title) _titles.get(key);
//                    title._pos = ++i;
//                    // log.debug(" pos = " + title._pos + ", title = " + title);
//                }
//            }
            
//            public Collection getYears() {
//                final List years = new ArrayList();
//                for (final Iterator ity = _titles.values().iterator(); ity.hasNext();) {
//                    final Title title = (Title) ity.next();
