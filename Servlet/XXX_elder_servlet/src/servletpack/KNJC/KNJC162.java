...
public class KNJC162 {

    private static final Log log = LogFactory.getLog(KNJC162.class);
     ...
     *  印刷処理
     */
    private boolean printMain(final DB2UDB db2, final Vrw32alp svf, final List csvlist) {
        final boolean csvFlg = (csvlist != null) ? true : false;

        final Map formFieldMapMap = new HashMap();
        ...
        for (final Iterator it = homerooms.iterator(); it.hasNext();) {
            ...
            for (int pi = 0; pi < pageList.size(); pi++) {
                ...
                final String form;
                if (SAKAE.equals(_param._z010)) {
                    form = "KNJC162_2.frm";
                } else {
                    form = "KNJC162.frm";
                }
                svf.VrSetForm(form, 1);
                if (!formFieldMapMap.containsKey(form)) {
                    formFieldMapMap.put(form, SvfField.getSvfFormFieldInfoMapGroupByName(svf));
                }
                final Map fieldInfoMapGroupByName = (Map) formFieldMapMap.get(form);
                svf.VrsOut("NENDO", nendo);
                svf.VrsOut("DATE", _param.getPrintDateStr(db2));
                svf.VrsOut("RANGE", _param.getRange(db2));
                svf.VrsOut("SCHOOLNAME", _param._knjSchoolMst._schoolName1);

                svfVrsOutKintaiName(svf, hokenshitsuKekkaMei, "ANO_ROOM_NAME");
                ...
                for (int seq = 1; seq <= 6; seq++) {
                    ...
                    svf.VrsOut("JOB" + String.valueOf(seq), job);
                }
                ...
                for (int li = 0; li < studentList.size(); li++) {
                    ...
                    if ("1".equals(_param._use_school_detail_gcm_dat)) {
                        final String setItemName = (String) _param._attendItemNameMap.get(student._courseCd + student._majorCd);
                        if (!StringUtils.isBlank(setItemName)) {
                            svfVrsOutKintaiName(svf, setItemName, "ABSENTNAME");
                        } else {
                            svfVrsOutKintaiName(svf, kessekiName, "ABSENTNAME");
                        }
                    } else {
                        svfVrsOutKintaiName(svf, kessekiName, "ABSENTNAME");
                    }
                    ...
                    svf.VrsOutn("ATTEND_NO", line, Integer.valueOf(student._attendNo).toString());
                    ...
                    if ("1".equals(_param._use_SchregNo_hyoji)) {
                        ...
                        svf.VrsOutn("NAME2", line, showName);
                    } else  {
                        final int showNameByte = KNJ_EditEdit.getMS932ByteLength(showName);
                        final String showNameField = showNameByte > 30 ? "4" : showNameByte > 20 ? "3" : "";
                        svf.VrsOutn("NAME" + showNameField, line, showName);
                    }
                    ...
                    //欠席事由
                    if (SAKAE.equals(_param._z010)) {
                        if (!"".equals(student._attend._absencestr)) {
                            final String reasonfield = 20 >= KNJ_EditEdit.getMS932ByteLength(student._attend._absencestr) ? "REASON1" : "REASON2";
                            svf.VrsOutn(reasonfield, line, student._attend._absencestr);
                        }

                    }

                    svf.VrsOutn("SEX", line, student._sexname);
                    csvDataList.add(student._sexname); //性別

                    if (student._attend != null) {
                        specialLessons = printAttendanceCount(db2, student, line, svf, student._attend, csvDataList, csvFlg);
                    }
                    ...
                    if (!SAKAE.equals(_param._z010) && null != student._remark1) {
                        final int keta = KNJ_EditEdit.getMS932ByteLength(student._remark1.toString());
                        final String[] check = {"REMARK", "REMARK2", "REMARK3", "REMARK4_1"};
                        String fieldname = null;
                        for (int i = 0; i < check.length; i++) {
                            final SvfField field = (SvfField) fieldInfoMapGroupByName.get(check[i]);
                            if (null != field) {
                                if (null == fieldname) {
                                    fieldname = check[i];
                                } else if (null != fieldname && (((SvfField) fieldInfoMapGroupByName.get(fieldname))._fieldLength) < keta) {
                                    fieldname = check[i];
                                }
                            }
                        }
                        if (null != fieldname) {
                            svf.VrsOutn(fieldname, line, student._remark1.toString());
                        }
                        remark = student._remark1.toString();
                    } else {
                        remark = getSchregAttendRemark(db2, student._schregno);
                        svf.VrsOutn("REMARK", line, remark);
                    }
                ...
                svf.VrsOut("NOTE1", " ");
                svf.VrsOut("NOTE2", "：特活進級" + comment + ",欠席" + comment);
                ...
                final String specialLessons = getOutputFromTo(specialLessonMin, specialLessonMax);
                log.debug("specialLessons   = " + specialLessons);
                svf.VrsOut("SP_TIME", String.valueOf(specialLessons));
     ...
     * @return 特別活動時数(端数は切捨)
     */
    private int printAttendanceCount(final DB2UDB db2, final Student student, final int line, final Vrw32alp svf, final AttendanceCount a, List csvDataList, final boolean csvFlg) {
        svf.VrsOutn("CLASS_DAYS1", line, String.valueOf(a._lessonDays));
        svf.VrsOutn("MOURNING", line, String.valueOf(a._mourningDays));
        svf.VrsOutn("SUSPEND", line, String.valueOf(a._suspendDays));
        svf.VrsOutn("ABROAD", line, String.valueOf(a._abroadDays));
        svf.VrsOutn("ATTEND", line, String.valueOf(a._mlessonDays));
        ...
        try {
            ...
            if ("1".equals(_param._useSchool_KindField)) {
                paramMap.put("SCHOOL_KIND", _param.SCHOOLKIND);
                paramMap.put("SCHOOLCD", _param.SCHOOLCD);
            }
            if ("1".equals(_param._use_school_detail_gcm_dat)) {
        ...
        if (!_param._useAbsenceWarn && knjSchoolMst._kessekiOutBunsi != null && knjSchoolMst._kessekiOutBunbo != null) {
            String bunsi = knjSchoolMst._kessekiOutBunsi;
            String bunbo = knjSchoolMst._kessekiOutBunbo;
            BigDecimal sick = new BigDecimal(a._sickDays);
            BigDecimal limit = new BigDecimal(a._mlessonDays).multiply(new BigDecimal(bunsi)).divide(new BigDecimal(bunbo), 1, BigDecimal.ROUND_FLOOR);
            if (sick.compareTo(limit) > 0) {
                svf.VrAttributen("ABSENT", line, AMIKAKE_ATTR1);
            }
        } else if (_param._useAbsenceWarn && knjSchoolMst._kessekiWarnBunsi != null && knjSchoolMst._kessekiWarnBunbo != null) {
            String bunsi = knjSchoolMst._kessekiWarnBunsi;
            String bunbo = knjSchoolMst._kessekiWarnBunbo;
            BigDecimal sick = new BigDecimal(a._sickDays);
            BigDecimal limit = new BigDecimal(a._mlessonDays).multiply(new BigDecimal(bunsi)).divide(new BigDecimal(bunbo), 1, BigDecimal.ROUND_FLOOR);
//            log.info(" schregno = " + student._schregno + " / limit  " + limit + " / sick = " + sick);
            if (sick.compareTo(limit) > 0) {
                svf.VrAttributen("ABSENT", line, AMIKAKE_ATTR2);
            }
        }
        svf.VrsOutn("ABSENT", line, String.valueOf(a._sickDays));

        svf.VrsOutn("TOTAL_LESSON", line, String.valueOf(a._attendDays));
        svf.VrsOutn("LATE", line, String.valueOf(a._lateDays));
        svf.VrsOutn("EARLY", line, String.valueOf(a._earlyDays));
        svf.VrsOutn("CLASS_ATTEND", line, a.getAttendancePercentage());
        if (null != _param.getKintaiName("14")) {
            svf.VrsOutn("ANOTHER_ROOM", line, String.valueOf(a._nurseOff));
        }
        ...
        svf.VrsOutn("SP_LHR", line, String.valueOf(spAttendLhr.setScale(0, BigDecimal.ROUND_HALF_UP)));
        svf.VrsOutn("SP_SCHOOL", line, String.valueOf(spAttendSchoolEvent.setScale(0, BigDecimal.ROUND_HALF_UP)));
        svf.VrsOutn("SP_STUGOV", line, String.valueOf(spAttendCommittee.setScale(0, BigDecimal.ROUND_HALF_UP)));
        svf.VrsOutn("SP_ANOTHER", line, String.valueOf(spAttendEtc.setScale(0, BigDecimal.ROUND_HALF_UP)));

        if (spAttendTotal.doubleValue() != 0.0) {
            if (!_param._useAbsenceWarn && (0.0 < student._compAbsenceHighSpecial999 && student._compAbsenceHighSpecial999 < spAttendTotal.doubleValue())) {
                svf.VrAttribute("SP_TOTAL",  AMIKAKE_ATTR1);
            }
            log.debug("student = " + student._name + " , special subclass attend total = " + spAttendTotal + "(" + student._compAbsenceHighSpecial999 + ")");
        }
        svf.VrsOutn("SP_TOTAL", line, String.valueOf(spAttendTotal));
        ...
        //log.fatal("schregno = " + student._schregno + " , special subclass lesson total = " + spLessonTotal);
    ...
    private static class Homeroom {
        ...
        private static List getHomeRooms(final DB2UDB db2, final Param param) {
            ...
            try {
                ...
                for (final Iterator it = homerooms.iterator(); it.hasNext();) {
                    final Homeroom homeroom = (Homeroom) it.next();
                    homeroom._studentAllList = getHrStudentList(db2, param, homeroom._hrclass);
                }
        ...
        private static List getHrStudentList(final DB2UDB db2, final Param param, final String hrClass) throws SQLException {
            // HRの生徒を取得
            final List studentList = Student.getStudentList(db2, param, hrClass);
            ...
            //欠席事由を集計
            Map absmap = getSummaryAbsence(db2, param, hrClass);
            String absencestr;

            ResultSet rs = null;
            // 1日単位
            param._psAttendanceDays.setString(1, hrClass);
            ...
            // 時数単位
            param._psAttendance.setString(1, hrClass);
            ...
            if (null != param._psAttendanceRemark) {
                param._psAttendanceRemark.setString(1, hrClass);
            ...
            // 特別活動グループコード999の上限値
            param._psSchregAbsenceHigh.setString(1, hrClass);
        ...
        private static Map getSummaryAbsence(final DB2UDB db2, final Param param, final String hrClass) throws SQLException {
            Map retmap = new HashMap();
            if (SAKAE.equals(param._z010)) {
                final String sqlabs = sqlSummaryAbsence(param, hrClass);
        ...
        private static String sqlSummaryAbsence(final Param param, final String hrClass) {
            ...
            stb.append("   AND SBD.GRADE = '" + param._grade + "' ");
    ...
    private void svfVrsOutKintaiName(final Vrw32alp svf, final String kintaiName, final String field) {
        String fieldName = "";
        svf.VrsOut(field + "2_1", "");
        svf.VrsOut(field + "1", "");
    ...
    private static class Param {
        ...
        Param(final HttpServletRequest request, final DB2UDB db2) {
            ...
            try {
                ...
                if (isHoutei()) {
                    sql = getHouteiJisuSql(this);
                } else {
                    sql = getJituJisuSql(this);
                }
            ...
            _schoolKind = getSchoolKind(db2, this);
        ...
        private void setWarnSemester(final DB2UDB db2) throws SQLException {
            final StringBuffer stb = new StringBuffer();
        ...
        private String getHouteiJisuSql(final Param parameter) {
            ...
            stb.append("     AND T2.GRADE  = '" + parameter._grade + "' AND T2.HR_CLASS = ? ");
            return stb.toString();
        }
    ...
    private static class Student {
        ...
        private static List getStudentList(final DB2UDB db2, final Param param, final String hrClass) throws SQLException {
            final List studentList = new ArrayList();
            final String sqlSchregRegdDat = sqlSchregRegdDat(param, hrClass);
        ...
        /** 学生を得るSQL */
        private static String sqlSchregRegdDat(final Param param, final String hrClass) {
            final StringBuffer stb = new StringBuffer();
            stb.append(" SELECT ");
            stb.append("     T1.SCHREGNO, ");
            stb.append("     T1.GRADE, ");
    ...
    private static String getSchoolKind(final DB2UDB db2, final Param param) {
        final StringBuffer sql = new StringBuffer();
        sql.append(" SELECT SCHOOL_KIND FROM SCHREG_REGD_GDAT WHERE YEAR = '" + param._year + "' AND GRADE = '" + param._grade + "' ");
