// kanji=漢字
/*
 * $Id: 5629051853846fe32108a8fbb4f67a93d6caf994 $
 *
 */
...
/*
 *  学校教育システム 賢者 [出欠管理] 単位修得について（イエローカード）
 */
public class KNJC153 {

    private static final Log log = LogFactory.getLog(KNJC153.class);
    ...
    public void svf_out (
        ...
        log.info(" $Id: 5629051853846fe32108a8fbb4f67a93d6caf994 $ ");
    ...
    private void printMain (
        ...
        for (final Iterator it2 = studentList.iterator(); it2.hasNext();) {
            ...
            BigDecimal rate = student.getSickRate();
            if (student._daysAttendance._sickDays > 0) {
            	log.info(" student " + student._schregno + "(" + student._grade + student._hrclass + "-" + student._attendno + "): sick = " + student._daysAttendance._sickDays + ", attend = " + student._daysAttendance._attendDays + ", rate = " + rate);
            }
...
//                final int line = j + 1;
//                svf.VrsOutn("SUBCLASS_NAME", line, null); // 科目名
//                svf.VrsOutn("REASON", line, null); // 理由
//            }

            final List tokenList = KNJ_EditKinsoku.getTokenList(student._remark1, 35 * 2);
            for (int i = 0; i < Math.min(tokenList.size(), 6); i++) {
            	final int line = i + 1;
            	final String token = (String) tokenList.get(i);
                svf.VrsOutn("NOTE", line, token); // 連絡事項
            }
    ...
    private static class Student {
        ...
        public BigDecimal getSickRate() {
        	if (0 >= _daysAttendance._mlessonDays) {
        		return null;
        	}
        	if (0 == _daysAttendance._sickDays) {
        		return new BigDecimal(0);
        	}
			return new BigDecimal(_daysAttendance._sickDays).multiply(new BigDecimal(100)).divide(new BigDecimal(_daysAttendance._mlessonDays), 1, BigDecimal.ROUND_HALF_UP);
		}
            ...
            try {
                ...
                for (final Iterator it = KnjDbUtils.query(db2, sql).iterator(); it.hasNext();) {
                    ...
                    final Student st = new Student(
                            ...
                            KnjDbUtils.getString(row, "REMARK1")
                            );
                    studentList.add(st);
                }
            } catch (Exception e) {
        ...
        private static String sqlSchregRegdDat(final Param param) {
            ...
            stb.append("     LEFT JOIN HEXAM_MONTH_REMARK_DAT T5 ON T5.YEAR = T1.YEAR ");
            stb.append("         AND T5.MONTH = '" + param._month + "' ");
    ...
    private static class Param {
        ...
        Param(final HttpServletRequest request, final DB2UDB db2) {
            ...
            _printDateStr = getDateString(request.getParameter("PRINT_DATE"));
        ...
        private Map getGradeDateRangeListMap(final DB2UDB db2, final String year) {
        	...
        	try {
            	...
            	while (cal.before(maxDateCalendar)) {
            		for (final Iterator it = KnjDbUtils.query(db2, ps, new Object[] { df.format(cal.getTime()) }).iterator(); it.hasNext();) {
            			final Map row = (Map) it.next();
                		if (null == row) {
                			continue;
                		}
        ...
        private String getDateString(final String date) {
            ...
            final String nen = _seirekiFlg ? String.valueOf(Integer.parseInt(date.substring(0,  4))) + "年" : KNJ_EditDate.h_format_JP_N(date);
