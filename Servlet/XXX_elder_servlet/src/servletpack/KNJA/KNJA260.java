// kanji=漢字
/*
 * $Id: 2f2ef300c4b7c0a2d09b7b8a0c9202561eda75b9 $
 *
 * 作成日: 2011/03/08 16:46:41 - JST
 * 作成者: m-yama
 *
 ...
 * <<クラスの説明>>。
 * @author m-yama
 * @version $Id: 2f2ef300c4b7c0a2d09b7b8a0c9202561eda75b9 $
 */
public class KNJA260 extends AbstractXls {

    private static final Log log = LogFactory.getLog("KNJA260.class");
    ...
    private String getSql(final Map attendMap) {
        ...
        if("9".equals(_param._semester)){
            ...
            } else if ("010199".equals(_param._testKindCd)) {
                intrTerm = "SEM" + _param._semester + "_INTR_VALUE";
            } else if ("0201".equals(_param._testKindCd)) {
                intrTerm = "SEM" + _param._semester + "_TERM_SCORE";
            } else if ("020199".equals(_param._testKindCd)) {
                intrTerm = "SEM" + _param._semester + "_TERM_VALUE";
            } else if ("0202".equals(_param._testKindCd)) {
                intrTerm = "SEM" + _param._semester + "_TERM2_SCORE";
            } else if ("020299".equals(_param._testKindCd)) {
                intrTerm = "SEM" + _param._semester + "_TERM2_VALUE";
            } else if ("9900".equals(_param._testKindCd)) {
        ...
        int cnt = 0;
        if ((_param._testKindCd.length() >= 6 && "99".equals(_param._testKindCd.substring(5, 6))) ||
            "9900".equals(_param._testKindCd)
        ) {
            for (final Iterator iter = _param._subClassList.iterator(); iter.hasNext();) {
                final SubclassData subclassData = (SubclassData) iter.next();
                stb.append("     ,SUM( CASE ");
                if ("1".equals(_param._useCurriculumcd)) {
                    stb.append("            M1.CLASSCD || '-' || M1.SCHOOL_KIND || '-' || M1.CURRICULUM_CD || '-' || ");
                }
                stb.append("             M1.SUBCLASSCD WHEN '" + subclassData._cd + "' THEN CASE M1." + intrTerm + "_DI WHEN '=' THEN int(8888) WHEN '-' THEN int(9999) ELSE M1." + intrTerm + " END ELSE NULL END ) AS TOKUTEN" + cnt + " ");
                stb.append("     ,SUM( CASE ");
                if ("1".equals(_param._useCurriculumcd)) {
                    stb.append("            M1.CLASSCD || '-' || M1.SCHOOL_KIND || '-' || M1.CURRICULUM_CD || '-' || ");
                }
                stb.append("             M1.SUBCLASSCD WHEN '" + subclassData._cd + "' THEN L1.NOTICE_LATE ELSE NULL END) as DI" + cnt + " ");
                cnt++;
            }
        } else {
            for (final Iterator iter = _param._subClassList.iterator(); iter.hasNext();) {
                ...
                stb.append("             M1.SUBCLASSCD WHEN '" + subclassData._cd + "' THEN M1." + intrTerm + " ELSE NULL END ) AS TOKUTEN" + cnt + " ");
                stb.append("     ,SUM( CASE ");
                if ("1".equals(_param._useCurriculumcd)) {
                    stb.append("            M1.CLASSCD || '-' || M1.SCHOOL_KIND || '-' || M1.CURRICULUM_CD || '-' || ");
                }
    ...
    private Object getAttendData(final Map attendMap) {
        ...
        stb.append("            SUBCLASSCD, T2.SEMESTER)S1  ");
        ...
        if (_param._isUseVirus) {
            stb.append("            SUM(W1.VIRUS) VIRUS, ");
        }
        if (_param._isUseKoudome) {
    ...
    private String getTestName() throws SQLException {
        String testname = "";
        if ("TESTITEM_MST_COUNTFLG".equals(_param._tableName) && "9900".equals(_param._testKindCd)) {
            testname = "評価成績";
        } else if ("010199".equals(_param._testKindCd)) {
            testname = "中間成績";
        } else if ("020199".equals(_param._testKindCd)) {
    ...
    private String getTestNameSql() {
        ...
        stb.append("     TESTKINDCD || TESTITEMCD AS VALUE ");
        stb.append(" FROM ");
        stb.append("    " + _param._tableName + " ");
        stb.append(" WHERE ");
        stb.append("     YEAR = '" + _param._year + "' ");
        if ("TESTITEM_MST_COUNTFLG_NEW".equals(_param._tableName)) {
            stb.append("     AND SEMESTER = '" + _param._semester + "' ");
    ...
    private class Param {
        ...
        private final boolean _isUseVirus;
        private final boolean _isUseKoudome;
        ...
        Param(final DB2UDB db2, final HttpServletRequest request) throws SQLException {
            ...
            _tableName = null != request.getParameter("useTestCountflg") && !"".equals(request.getParameter("useTestCountflg")) ? request.getParameter("useTestCountflg") : "TESTITEM_MST_COUNTFLG_NEW";
            _isUseVirus = null != request.getParameter("useVirus") && !"".equals(request.getParameter("useVirus")) ? true : false;
            _isUseKoudome = null != request.getParameter("useKoudome") && !"".equals(request.getParameter("useKoudome")) ? true : false;
