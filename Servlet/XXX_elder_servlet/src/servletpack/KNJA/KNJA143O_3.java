// kanji=漢字
/*
 * $Id: 8f605ba0ab0f43a68b9e1054bff4e71a167f5575 $
 *
 ...
 * <<クラスの説明>>。
 * @author nakamoto
 * @version $Id: 8f605ba0ab0f43a68b9e1054bff4e71a167f5575 $
 */
public class KNJA143O_3 {

    private static final Log log = LogFactory.getLog("KNJA143O_3.class");
    ...
    private void printMain(final DB2UDB db2, final Vrw32alp svf) throws SQLException {
        ...
        final String form = "KNJA143O_3.frm";
        ...
        for (int j = 0; j < studentList.size(); j++) {
            ...
            int chkCnt = 1;
            if (printandchkrosen(svf, student._josya1, student._gesya1, chkCnt, cnt)) {chkCnt++;}
            if (printandchkrosen(svf, student._josya2, student._gesya2, chkCnt, cnt)) {chkCnt++;}
            if (printandchkrosen(svf, student._josya3, student._gesya3, chkCnt, cnt)) {chkCnt++;}
            if (printandchkrosen(svf, student._josya4, student._gesya4, chkCnt, cnt)) {chkCnt++;}
            if (printandchkrosen(svf, student._josya5, student._gesya5, chkCnt, cnt)) {chkCnt++;}
            if (printandchkrosen(svf, student._josya6, student._gesya6, chkCnt, cnt)) {chkCnt++;}
            if (printandchkrosen(svf, student._josya7, student._gesya7, chkCnt, cnt)) {chkCnt++;}
    ...
    private boolean printandchkrosen(final Vrw32alp svf, final String jyosya, final String gesya, final int chkCnt, final int cnt) {
    	if ("".equals(StringUtils.defaultString(jyosya, "")) &&  "".equals(StringUtils.defaultString(gesya, ""))) {
    		...
    		if (!"".equals(StringUtils.defaultString(jyosya, ""))) {
                ...
                svf.VrsOutn("SECTION" + chkCnt + "_1_" + (ketaName <= 14 ? "1" : ketaName <= 20 ? "2" : "3"), cnt, jyosya); // 乗車
    		}
    		if (!"".equals(StringUtils.defaultString(gesya, ""))) {
                final int ketaName = KNJ_EditEdit.getMS932ByteLength(gesya);
                svf.VrsOutn("SECTION" + chkCnt + "_2_" + (ketaName <= 14 ? "1" : ketaName <= 20 ? "2" : "3"), cnt, gesya); // 乗車
    		}
    	}
    	return true;
    }
