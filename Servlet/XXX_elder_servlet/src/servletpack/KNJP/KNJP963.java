...
public class KNJP963 {

    private static final Log log = LogFactory.getLog(KNJP963.class);
    ...
    private void printMain(final DB2UDB db2, final Vrw32alp svf) {
        ...
        for (Iterator iterator = lmstMap.keySet().iterator(); iterator.hasNext();) {
            ...
            final String title = gengou + "年度　" + lmst._lName + "　学年別支出伺明細表";
    ...
    private void printHeader(final DB2UDB db2, final Vrw32alp svf, final String title) {
        ...
        if ("1".equals(_param._printDiv)) {
            ...
            if ("1".equals(_param._outGoDiv)) {
                ...
                svf.VrsOut("SUBTITLE2", "支出伺日：" + reqYmF + "\uFF5E" + reqYmT);
            } else {
                final String outYmF = KNJ_EditDate.h_format_Seireki_M(_param._outYearMonthF + "-01");
                final String outYmT = KNJ_EditDate.h_format_Seireki_M(_param._outYearMonthT + "-01");
                svf.VrsOut("SUBTITLE2", "支出決定日：" + outYmF + "\uFF5E" + outYmT);
            }
        }
    ...
    private Map getLmstMap(final DB2UDB db2) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                //SchregDatをセット
                final SchregDat schregDat = new SchregDat(levyLCd, levyLName, schregno, name, grade, hrClass, hrName, attendno, outGoMoney);
    ...
    private String getSchregSql() {
        ...
        stb.append("     SUM(VALUE(OUTSCH.OUTGO_MONEY, 0)) AS OUTGO_MONEY ");
        stb.append(" FROM ");
        stb.append("     LEVY_REQUEST_OUTGO_SCHREG_DAT OUTSCH ");
        stb.append("     INNER JOIN SCH_T ");
        stb.append("          ON SCH_T.YEAR         = OUTSCH.YEAR ");
        stb.append("         AND SCH_T.SCHREGNO     = OUTSCH.SCHREGNO ");
        stb.append("         AND SCH_T.SCHOOL_KIND  = OUTSCH.SCHOOL_KIND ");
        stb.append("     INNER JOIN LEVY_REQUEST_OUTGO_DAT OUTGO ");
        stb.append("          ON OUTGO.YEAR        = OUTSCH.YEAR ");
        ...
        if ("1".equals(_param._printDiv)) {
            stb.append("     AND OUTGO.REQUEST_NO = '" + _param._requestNo + "' ");
        } else {
            if ("1".equals(_param._outGoDiv)) {
                stb.append(" AND OUTGO.REQUEST_DATE BETWEEN DATE('" + _param._reqYearMonthF + "-01" + "') AND last_day(DATE('" + _param._reqYearMonthT + "-01" + "')) ");
            } else {
                stb.append(" AND OUTGO.REQUEST_DATE BETWEEN DATE('" + _param._outYearMonthF + "-01" + "') AND last_day(DATE('" + _param._outYearMonthT + "-01" + "')) ");
            }
        }
        stb.append(" GROUP BY ");
        stb.append("     LMST.LEVY_L_CD, ");
    ...
    private class Param {
        ...
        private final String _outYearMonthF;
        private final String _outYearMonthT;
        ...
        Param(final DB2UDB db2, final HttpServletRequest request) throws SQLException {
            ...
            _outYearMonthF = request.getParameter("OUTGO_MONTH_F");
            _outYearMonthT = request.getParameter("OUTGO_MONTH_T");
