// kanji=漢字
/*
 * $Id: 0e91f165658eb0d51c7fb81a4057558127517870 $
 *
...
public class KNJP400J {


    private static final Log log = LogFactory.getLog(KNJP400J.class);
                     ...
                     throws ServletException, IOException
    {
        ...
        // メイン処理
        try {
            for (int i = 11; i <= Integer.parseInt(paramcd);i++) {
                param[4] = String.valueOf(i);
                mainProcess(db2, svf, param, false);
            }
            if ("3".equals(param[3])) {
                param[4] = MCD13;
                mainProcess(db2, svf, param, true);
            }
        } catch (Exception e) {
...
for(int ia=0 ; ia<param.length ; ia++) log.debug("param["+ia+"]="+param[ia]);
    ...
    private void printHead(final Vrw32alp svf, final String param[], final String grade, final boolean month3Flg)
    {
        ...
        try {
            ...
            final String setTimes = month3Flg ? param[3] + "月分" : param[4].substring(1) + "期分";
            ret = svf.VrsOut("TIMES"        , setTimes );
            ret = svf.VrEndRecord();
