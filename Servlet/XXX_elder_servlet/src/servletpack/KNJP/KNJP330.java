// kanji=漢字
/*
 * $Id: 7c24268d4fd6e21c2ed58b76c1f324870d318f15 $
 *
 * 作成日: 2010/10/04
 * 作成者: m-yama
 *
...
public class KNJP330 {


    private static final Log log = LogFactory.getLog(KNJP330.class);

    Param _param;
    private boolean _hasData;
    private String FORM_TYPE9 = "1";
    private String hikiotoshi = "162"; // 150 * 1.08
...
log.debug("printMain end!");
    ...
    private void printMeisai(final Vrw32alp svf, final ResultSet rs)
    {
        try {
            ...
            String paternCd = "";
            if (keigenMoney > 0) {
                if (rs.getInt("GRANT_MONEY") > 0) {
                    svf.VrSetForm("KNJP330_B.frm", 1);
                    paternCd = "B";
                } else {
                    svf.VrSetForm("KNJP330_A.frm", 1);
                    paternCd = "A";
                }
            } else {
                if (rs.getInt("GRANT_MONEY") > 0) {
                    svf.VrSetForm("KNJP330_D.frm", 1);
                    paternCd = "D";
                } else {
                    svf.VrSetForm("KNJP330_C.frm", 1);
                    paternCd = "C";
                }
            }
            ...
            if ("A".equals(paternCd)) {
                ...
                final int totalMoney = rs.getInt("MONEY_DUE") - (totalReducCountryMoney + rs.getInt("REDUCTIONMONEY") + rs.getInt("ADJUSTMENT_MONEY"));
            ...
            if ("B".equals(paternCd)) {
                svf.VrsOut("MONEY1"            , String.valueOf(totalReducMoney));
                final int totalMoney = rs.getInt("MONEY_DUE") - (rs.getInt("GRANT_MONEY") + totalReducCountryMoney + rs.getInt("REDUCTIONMONEY") + rs.getInt("ADJUSTMENT_MONEY")) + rs.getInt("FEE");
                svf.VrsOut("MONEY2"            , String.valueOf(totalMoney));
                svf.VrsOut("MONEY3_1"            , rs.getString("MONEY_DUE"));
                svf.VrsOut("GRANT_NAME"          , rs.getString("GRANTNAME") + "減免額");
                svf.VrsOut("MONEY3_2"            , rs.getString("GRANT_MONEY"));
                svf.VrsOut("MONEY3_3"            , String.valueOf(totalReducCountryMoney));
                svf.VrsOut("MONEY3_4"            , rs.getString("REDUCTIONMONEY"));
                svf.VrsOut("MONEY3_5"            , rs.getString("ADJUSTMENT_MONEY"));
                svf.VrsOut("MONEY3_6"            , rs.getString("FEE"));
                svf.VrsOut("TOTAL"            , String.valueOf(totalMoney));
            ...
            if ("D".equals(paternCd)) {
                final int totalMoney = rs.getInt("MONEY_DUE") - rs.getInt("GRANT_MONEY") + rs.getInt("FEE");
                svf.VrsOut("MONEY1"            , String.valueOf(totalMoney));
                svf.VrsOut("MONEY2_1"            , rs.getString("MONEY_DUE"));
                svf.VrsOut("GRANT_NAME"          , rs.getString("GRANTNAME") + "減免額");
                svf.VrsOut("MONEY2_2"            , rs.getString("GRANT_MONEY"));
                svf.VrsOut("MONEY2_3"            , rs.getString("FEE"));
                svf.VrsOut("TOTAL"            , String.valueOf(totalMoney));
                svf.VrsOut("BANKDATE"         , _param._printDate2);
            }
    ...
    private String todoufukenMeisai()
    {
        ...
        try {
            ...
            stb.append("          YEAR, ");
            stb.append("          SCHREGNO, ");
            stb.append("          MIN(GRANTCD) AS GRANTCD ");
            stb.append("      FROM ");
            ...
            stb.append("     AND T1.GRANTCD = T2.GRANTCD ");
            stb.append(" ), SCH_T AS ( ");
            ...
            stb.append("     '" + hikiotoshi + "' AS FEE, ");
            stb.append("     VALUE(BURDEN.TOTAL_BURDEN_CHARGE, 0) AS ADJUSTMENT_MONEY, ");
            stb.append("     VALUE(GRANT_T.GRANTNAME, '') AS GRANTNAME, ");
            stb.append("     VALUE(GRANT_T.GRANT_MONEY, 0) AS GRANT_MONEY, ");
            ...
            stb.append("     CASE WHEN t7.MONEY_DUE >= VALUE(t1.REDUCTIONMONEY, 0) + VALUE(BURDEN.TOTAL_BURDEN_CHARGE, 0) + VALUE(BASE_M.B_PAID_MONEY, 0) + VALUE(ADD_M.A_PAID_MONEY, 0) - VALUE(ADJUST.TOTAL_ADJUSTMENT_MONEY, 0) THEN t7.MONEY_DUE - (VALUE(t1.REDUCTIONMONEY, 0) + VALUE(BURDEN.TOTAL_BURDEN_CHARGE, 0) + VALUE(BASE_M.B_PAID_MONEY, 0) + VALUE(ADD_M.A_PAID_MONEY, 0) - VALUE(ADJUST.TOTAL_ADJUSTMENT_MONEY, 0)) + " + hikiotoshi + " ELSE (VALUE(t1.REDUCTIONMONEY, 0) + VALUE(BURDEN.TOTAL_BURDEN_CHARGE, 0) + VALUE(BASE_M.B_PAID_MONEY, 0) + VALUE(ADD_M.A_PAID_MONEY, 0) - VALUE(ADJUST.TOTAL_ADJUSTMENT_MONEY, 0)) - t7.MONEY_DUE END TOTAL_MONEY ");
            stb.append(" FROM ");
            stb.append("     SCH_T ");
            ...
            stb.append("     LEFT JOIN GRANT_T ON GRANT_T.YEAR = SCH_T.YEAR ");
            stb.append("          AND GRANT_T.SCHREGNO = SCH_T.SCHREGNO ");
            stb.append(") ");
            stb.append("SELECT ");
            stb.append("    * ");
            stb.append("FROM ");
...
//log.debug(stb);
    ...
    private class Param {
        ...
        Param(final DB2UDB db2, final HttpServletRequest request) throws SQLException {
            ...
            _inState += ")";
            _printDate = KNJ_EditDate.h_format_JP(_date1);
            _printWeek = KNJ_EditDate.h_format_W(_date2);
            _printDate2 = KNJ_EditDate.h_format_JP(_date2);
