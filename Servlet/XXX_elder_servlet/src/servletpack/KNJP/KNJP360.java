// kanji=漢字
/*
 * $Id: e6b903410180e705afff186b4b917ae958db345c $
 *
...
public class KNJP360 {


    private static final Log log = LogFactory.getLog(KNJP360.class);
...
for(int ia=0 ; ia<param.length ; ia++) log.debug("param["+ia+"]="+param[ia]);
    ...
    private void getHeaderData(DB2UDB db2,Vrw32alp svf,String param[]){
        ...
        //NO003
        if (param[6].equals("1")) {
            svf.VrSetForm("KNJP360_5.frm", 1);
        } else if (param[6].equals("2")) {
            svf.VrSetForm("KNJP360_3.frm", 1);
        } else {
            svf.VrSetForm("KNJP360_4.frm", 4);
        }
    //  作成日
        try {
    ...
    private void printMeisai(Vrw32alp svf,String param[],ResultSet rs,int gyo)
    {
        try {
            ...
            if (rs.getString("REDUCTIONMONEY_1") != null || rs.getString("REDUCTIONMONEY_2") != null) {
                svf.VrsOutn("TOTAL_AID"  ,gyo    , String.valueOf(rs.getInt("REDUCTIONMONEY_1") + rs.getInt("REDUCTIONMONEY_2")) );
            } else {
                svf.VrsOutn("TOTAL_AID"  ,gyo    , "" );
            }
            svf.VrsOutn("PREFECTURE"    ,gyo    , rs.getString("PREF") );
            svf.VrsOutn("REMARK"        ,gyo    , rs.getString("REDUC_REMARK") );   //NO002
...
//            svf.VrsOutn("ADD_MARK2"     ,gyo    , rs.getString("REDUC_ADD_FLG_2") );
            ...
            svf.VrsOutn("DEC_MONEY1"    ,gyo    , rs.getString("PLAN_MONEY") );
...
log.debug("マックスクラス="+classall);
    ...
    private String meisaiSql(String param[],String gradeHrclass)
    {
        ...
        try {
            ...
            stb.append("    t3.REDUC_REMARK "); //NO002
            stb.append("FROM ");
            ...
            stb.append("    LEFT JOIN NAME_MST t10 ON t10.NAMECD1 = 'G213' ");
            stb.append("         AND t10.NAMECD2 = T3.REDUC_RANK_1 ");
            stb.append("    LEFT JOIN NAME_MST t11 ON t11.NAMECD1 = 'G213' ");
...
//log.debug(stb);
    ...
    private String getKuniSql(String param[],String gradeHrclass) {
        ...
        try {
            ...
            stb.append("     SELECT ");
            stb.append("         SCHREGNO, ");
            stb.append("         SUM(CASE WHEN PLAN_CANCEL_FLG = '1' THEN NULL ");
            stb.append("                  ELSE PLAN_MONEY END) AS PLAN_MONEY, ");
            ...
            stb.append("     (VALUE(t4.ADD_PLAN_MONEY1, 0) + VALUE(t4.ADD_PLAN_MONEY2, 0)) + VALUE(t4.PLAN_MONEY, 0) AS PLAN_MONEY_TOTAL, ");
            stb.append("     t8.PREF, ");
