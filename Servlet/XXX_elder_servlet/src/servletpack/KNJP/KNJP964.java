...
public class KNJP964 {

    private static final Log log = LogFactory.getLog(KNJP964.class);
    ...
    private void printMain(final DB2UDB db2, final Vrw32alp svf, final String gradeHr) {
        ...
        for (Iterator iterator = printLmst.iterator(); iterator.hasNext();) {
            ...
            svf.VrsOut("DECISION_TITLE", "未決裁額：");
    ...
    private String getSchregSql(final String gradeHr) {
        ...
        stb.append("     SUM(CASE WHEN VALUE(OUTGO.OUTGO_APPROVAL, '0') <> '1' THEN VALUE(OUTSCH.OUTGO_MONEY, 0) ELSE 0 END) AS REQUEST_MONEY, ");
        stb.append("     SUM(CASE WHEN VALUE(OUTGO.OUTGO_APPROVAL, '0')  = '1' THEN VALUE(OUTSCH.OUTGO_MONEY, 0) ELSE 0 END) AS OUTGO_MONEY ");
        stb.append(" FROM ");
        stb.append("     LEVY_REQUEST_OUTGO_SCHREG_DAT OUTSCH ");
        stb.append("     INNER JOIN SCH_T ");
        stb.append("          ON OUTSCH.YEAR        = SCH_T.YEAR ");
        stb.append("         AND OUTSCH.SCHREGNO    = SCH_T.SCHREGNO ");
        stb.append("         AND OUTSCH.SCHOOL_KIND = SCH_T.SCHOOL_KIND ");
        stb.append("     INNER JOIN LEVY_REQUEST_OUTGO_DAT OUTGO ");
        stb.append("          ON OUTGO.SCHOOLCD    = OUTSCH.SCHOOLCD ");
        stb.append("         AND OUTGO.SCHOOL_KIND = OUTSCH.SCHOOL_KIND ");
        stb.append("         AND OUTGO.YEAR        = OUTSCH.YEAR ");
        stb.append("         AND OUTGO.OUTGO_L_CD  = OUTSCH.OUTGO_L_CD ");
        stb.append("         AND OUTGO.OUTGO_M_CD  = OUTSCH.OUTGO_M_CD ");
        stb.append("         AND OUTGO.REQUEST_NO  = OUTSCH.REQUEST_NO ");
        stb.append("     LEFT JOIN LEVY_L_MST LMST ");
        stb.append("          ON LMST.SCHOOLCD     = OUTSCH.SCHOOLCD ");
        stb.append("         AND LMST.SCHOOL_KIND  = OUTSCH.SCHOOL_KIND ");
        stb.append("         AND LMST.YEAR         = OUTSCH.YEAR ");
        stb.append("         AND LMST.LEVY_L_CD    = OUTSCH.OUTGO_L_CD ");
        stb.append(" WHERE ");
        stb.append("             VALUE (OUTGO.OUTGO_CANCEL, '0') = '0' ");
        if ("1".equals(_param._outGoDiv)) {
            stb.append("     AND OUTGO.REQUEST_DATE BETWEEN DATE('" + _param._reqYearMonth + "-01') AND last_day(DATE('" + _param._reqYearMonth + "-01')) ");
        } else {
            stb.append("     AND OUTGO.OUTGO_DATE BETWEEN DATE('" + _param._outYearMonth  + "-01') AND last_day(DATE('" + _param._outYearMonth  + "-01')) ");
        }
        stb.append(" GROUP BY ");
        stb.append("     LMST.YEAR, ");
        stb.append("     SCH_T.SCHREGNO, ");
    ...
    private class Param {
        ...
        private final String _reqYearMonth;
        private final String _outYearMonth;
        ...
        Param(final DB2UDB db2, final HttpServletRequest request) throws SQLException {
            ...
            _reqYearMonth = request.getParameter("REQUEST_YMONTH");
            _outYearMonth = request.getParameter("OUTGO_YMONTH");
