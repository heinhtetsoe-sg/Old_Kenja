/*
 * $Id: 1c61e42d09416136ee32cc6313603bb757b1f8f6 $
 *
 * 作成日: 2015/03/13
 * 作成者: maesiro
 *
...
public class KNJP962 {

    private static final Log log = LogFactory.getLog(KNJP962.class);
    ...
    private void printMain(final DB2UDB db2, final Vrw32alp svf, final String gradeHr) {
        ...
        for (Iterator iterator = printLmst.iterator(); iterator.hasNext();) {
            ...
            svf.VrSetForm("KNJP962.frm", 4);
            final String gengou = nao_package.KenjaProperties.gengou(Integer.parseInt(_param._year));
            svf.VrsOut("TITLE", gengou + "年度　クラス別収入伺明細表");
            svf.VrsOut("HR_NAME1", hrClass._hrName);
            svf.VrsOut("PRINT_DATE", KNJ_EditDate.h_format_JP(_param._ctrlDate));
            svf.VrsOut("SUB_TITLE", "決定額：");
            svf.VrsOut("SUB_TITLE_MONEY", String.valueOf(hrClass._totalMoney));
            svf.VrsOut("DECISION_TITLE", "未決済額：");
            svf.VrsOut("DECISION_MONEY", String.valueOf(hrClass._totalRequestMoney));
            ...
            for (Iterator itSch = hrClass._schList.iterator(); itSch.hasNext();) {
                ...
                svf.VrsOut("BILL", String.valueOf(Integer.parseInt(schregDat._requestMoney) + Integer.parseInt(schregDat._incomeMoney)));
            ...
            _hasData = true;
            svf.VrsOut("BILL_TOTAL", String.valueOf(hrClass._totalMoney));
            svf.VrEndRecord();
        }
    ...
    private List getList(final DB2UDB db2, final String gradeHr) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                final SchregDat schregDat = new SchregDat(schregno, name, grade, hrClass, hrName, attendno, requestMoney, incomeMoney);
                setHrClass._schList.add(schregDat);
                setHrClass._totalCnt++;
                setHrClass._totalRequestMoney += Integer.parseInt(requestMoney);
                setHrClass._totalMoney += Integer.parseInt(incomeMoney);
                final int setSyuukeiKey = Integer.parseInt(requestMoney) + Integer.parseInt(incomeMoney);
                final String keyIncome = String.valueOf(setSyuukeiKey);
                if (setHrClass._syuukei.containsKey(keyIncome)) {
                    int syuukei = ((Integer) setHrClass._syuukei.get(keyIncome)).intValue();
                    syuukei++;
                    setHrClass._syuukei.put(keyIncome, new Integer(syuukei));
                } else {
                    setHrClass._syuukei.put(keyIncome, new Integer(1));
                }
                befHrClass = hrClass;
            }
    ...
    private String getSchregSql(final String gradeHr) {
        ...
        if ("1".equals(_param._incomeDiv)) {
            ...
            stb.append("           AND MONTH(T1.INCOME_DATE) = " + _param._incomeMonth + " ");
        }
        stb.append(" ) ");
        stb.append(" SELECT ");
        ...
        stb.append("     SUM(CASE WHEN VALUE(I1.INCOME_APPROVAL, '0') = '1' THEN 0 ELSE VALUE(T1.INCOME_MONEY, 0) END) AS REQUEST_MONEY, ");
        stb.append("     SUM(CASE WHEN VALUE(I1.INCOME_APPROVAL, '0') = '1' THEN VALUE(T1.INCOME_MONEY, 0) ELSE 0 END) AS INCOME_MONEY ");
        stb.append(" FROM ");
        stb.append("     EXISTL LMST ");
        stb.append("     LEFT JOIN SCH_T ON SCH_T.YEAR = LMST.YEAR ");
        stb.append("     LEFT JOIN LEVY_REQUEST_INCOME_SCHREG_DAT T1 ON LMST.YEAR = T1.YEAR ");
        stb.append("           AND LMST.INCOME_L_CD = T1.INCOME_L_CD ");
        stb.append("           AND SCH_T.SCHREGNO = T1.SCHREGNO ");
        stb.append("     LEFT JOIN LEVY_REQUEST_INCOME_DAT I1 ON T1.YEAR = I1.YEAR ");
        ...
        if ("1".equals(_param._incomeDiv)) {
            ...
            stb.append("          AND MONTH(I1.INCOME_DATE) = " + _param._incomeMonth + " ");
        }
        stb.append(" GROUP BY ");
        stb.append("     SCH_T.SCHREGNO, ");
        stb.append("     SCH_T.NAME, ");
    ...
    private class Param {
        ...
        private final String _incomeMonth;
        ...
        Param(final DB2UDB db2, final HttpServletRequest request) throws SQLException {
            ...
            _incomeMonth = request.getParameter("INCOME_MONTH");
