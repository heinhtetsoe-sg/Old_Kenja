...
public class KNJM490school {

    private static final Log log = LogFactory.getLog(KNJM490school.class);
...
for(int i=0 ; i<4 ; i++) log.debug("param["+i+"]="+param[i]);
        ...
        if (setSvfMain(db2,svf,param,ps,ps2,ps2_1,ps3)) nonedata = true;    //帳票出力のメソッド
    ...
    //  終了処理
        ret = svf.VrQuit();
        preStatClose(ps,ps2,ps2_1,ps3);         //preparestatementを閉じる
        db2.commit();
        db2.close();                //DBを閉じる
    ...
    private boolean setSvfMain(
        ...
        try {
            ...
            while( rs.next() ){
                ...
                ret = svf.VrsOut("SEQ1"         ,rs.getString("SCH_SEQ_MIN"));      //規定数
...
log.debug("seqdate" + String.valueOf(seqdate));
                        ret = svf.VrsOut("DATE_VALUE" + kindcnt ,String.valueOf(seqdate));
                        seqcnt++;
                        kindcnt++;
                    }else {
                        break;
    ...
    private String preStat(String param[])
    {
        ...
        try {
            ...
            stb.append("    t5.SEM1_TERM_SCORE,t5.SEM1_VALUE,t5.SEM2_VALUE,t5.SEM2_TERM_SCORE, t5.GRAD_VALUE, t6.SCH_SEQ_MIN, ");
...
//          stb.append("    AND t7.SCHOOLINGKINDCD = '1' ");
            ...
            stb.append("   t3.TAKESEMES,t4.SUBCLASSABBV,t4.SUBCLASSNAME, ");
            stb.append("   t5.SEM1_TERM_SCORE,t5.SEM1_VALUE,t5.SEM2_VALUE,t5.SEM2_TERM_SCORE, t5.GRAD_VALUE, t6.SCH_SEQ_MIN,t7.SCHOOLINGKINDCD, t8.CREDITS ");
            if (!param[2].equals("0")){
                stb.append(") ");
                stb.append("SELECT ");
                stb.append("    * ");
