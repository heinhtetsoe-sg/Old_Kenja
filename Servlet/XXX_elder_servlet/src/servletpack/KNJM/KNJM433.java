 ...
 *  @version $Id: d7619a637a234a9cf110d4e8cf37814949454f49 $
 */

public class KNJM433 {

    private static final Log log = LogFactory.getLog(KNJM433.class);
    ...
    private void setHead(
        ...
        svf.VrSetForm("KNJM433.frm", 4);              //共通フォーム
    ...
    private boolean mainProcess(
        ...
        try {
            ...
            final Map hmSpAttend = new HashMap();       //学籍番号と特別活動出席
            final Map hmSpTAttend = new HashMap();      //学籍番号と特別活動出席全て
            int schno = 0;
            ...
            while (rs.next()) {
                ...
                //特別活動出席
                final BigDecimal spAttend = getSpAttend(db2, param, rs.getString("SCHREGNO"), "YEAR");
                hmSpAttend.put(rs.getString("SCHREGNO"), spAttend);        //学籍番号に特別活動

                //特別活動出席
                final BigDecimal spTAttend = getSpAttend(db2, param, rs.getString("SCHREGNO"), "ALL");
                hmSpTAttend.put(rs.getString("SCHREGNO"), spTAttend);        //学籍番号に特別活動全て
                ...
                if (schno == 55) {
                    param[5] = param[5] + ")";
                    //印字メイン
                    if (printMain(db2, svf, param, hm1, hm2, hmMtCredit, hmSdCredit, hmAqCredit, hmTtCredit, hmSpAttend, hmSpTAttend, classcd, schno, scharay)) {
                        nonedata = true;
                    }
                    hm1.clear();                                            //行番号情報を削除
                    schno = 0;
     ...
     * @throws Exception
     */
    private BigDecimal getSpAttend(
            final DB2UDB db2,
            final String[] param,
            final String schno,
            final String sqlpattern
        ...
        BigDecimal retAttend = new BigDecimal("0");
        ResultSet rsSpAttend = null;
        try {
            db2.query(sqlGetSpAttend(param, schno, sqlpattern));
            rsSpAttend = db2.getResultSet();
            while (rsSpAttend.next()) {
                retAttend = rsSpAttend.getBigDecimal("CREDIT_TIME");
            }
        } finally {
            db2.commit();
            DbUtils.closeQuietly(rsSpAttend);
        }
    ...
    private boolean printMain(
            ...
            final Map hmSpAttend,
            final Map hmSpTAttend,
            final String classcd, final int schno, final String[] scharay
    ) throws Exception {
        ...
        try {
            ...
            if (lcount > 0) {
                ...
                for (int meidocnt = lcount; meidocnt < max; meidocnt++) {
                ...
                svf.VrsOut("SP_ATTEND_NAME", "特別活動出席数");
                printFixationFieldData(svf, hm1, hmSpAttend, "SP_ATTEND", schno, scharay);
                svf.VrsOut("SP_ATTEND_NAME", "特別活動出席計");
                printFixationFieldData(svf, hm1, hmSpTAttend, "SP_ATTEND", schno, scharay);
    ...
    private void printFixationFieldData(
        ...
        for (int hrcnt = 0; hrcnt < schno; hrcnt++) {
            ...
            if ("SP_ATTEND".equals(fieldName) || "SP_TATTEND".equals(fieldName)) {
                final BigDecimal iData = (BigDecimal) mpData.get(scharay[hrcnt]);
                svf.VrsOutn(fieldName, iLine.intValue(), String.valueOf(iData.toString()));
            } else {
    ...
    private void setSubclassDeta(
        ...
        try {
            ...
            final String gradValueField = "GRAD_VALUE";
            final String attributeGray = "PAINT=(1,80,2)";
            final String attributeGray2 = "PAINT=(1,90,2)";
            final String attributeNormal = "PAINT=(0,0,0)";
            while (rs.next()) {
                final Integer int1 = (Integer) hm1.get(rs.getString("SCHREGNO"));
                final int iclasscd = rs.getInt("CLASSCD");
                final boolean amikake = iclasscd <= 90 && (null == rs.getString("SETPOINT") && null == rs.getString("SETVALUE") && null == rs.getString("GET_CREDIT") && null == rs.getString("GRAD_VALUE"));
                final boolean amikake2;
                if (NumberUtils.isDigits(rs.getString("GRAD_VALUE"))) {
                    amikake2 = 1 == Integer.parseInt(rs.getString("GRAD_VALUE"));
                } else {
                    amikake2 = iclasscd <= 90 && (null != rs.getString("SETPOINT") && 30 > rs.getInt("SETPOINT"));
                }
                if (amikake) {
                    svf.VrAttributen(pointField, int1.intValue(), attributeGray);
                    svf.VrAttributen(valueField, int1.intValue(), attributeGray);
                    svf.VrAttributen(gradValueField, int1.intValue(), attributeGray);
                } else if (amikake2) {
                    svf.VrAttributen(pointField, int1.intValue(), attributeGray2);
                    svf.VrAttributen(valueField, int1.intValue(), attributeGray2);
                    svf.VrAttributen(gradValueField, int1.intValue(), attributeGray2);
                }
                final String setPoint = "*".equals(rs.getString("SETPOINT_DI")) ? rs.getString("SETPOINT_DI") : rs.getString("SETPOINT");
                svf.VrsOutn(pointField, int1.intValue(), setPoint);
                svf.VrsOutn(valueField, int1.intValue(), rs.getString("SETVALUE"));
                svf.VrsOutn(gradValueField, int1.intValue(), rs.getString("GRAD_VALUE"));
                if (amikake || amikake2) {
                    svf.VrAttributen(pointField, int1.intValue(), attributeNormal);
                    svf.VrAttributen(valueField, int1.intValue(), attributeNormal);
                    svf.VrAttributen(gradValueField, int1.intValue(), attributeNormal);
                }
            }
        } finally {
            DbUtils.closeQuietly(rs);
    ...
    private String preStat2(final String[] param) {
        ...
        stb.append("    AND substr(s1.SUBCLASSCD, 1, 2) NOT BETWEEN '92' AND '94' ");
        stb.append("GROUP BY ");
        stb.append("    t1.CHAIRCD,s2.CLASSCD,s2.CLASSABBV, ");
        if ("1".equals(param[11])) {
    ...
    private String preStat3(final String[] param) {
        ...
        if (param[1].equals("1")) {
            ...
            stb.append("    SEM1_TERM_SCORE_DI AS SETPOINT_DI, ");
            stb.append("    SEM1_VALUE AS SETVALUE, ");
        } else {
            stb.append("    SEM2_TERM_SCORE AS SETPOINT, ");
            stb.append("    SEM2_TERM_SCORE_DI AS SETPOINT_DI, ");
            stb.append("    SEM2_VALUE AS SETVALUE, ");
        }
...
//        stb.append("     EXECUTEDATE <= '" + param[4].replace('/', '-') + "' AND ");
    ...
    private String sqlGetAqCredit(final String[] param, final String schno) {
        ...
        stb.append("     l2.SUBCLASSCD, ");
//        stb.append("     CASE WHEN VALUE(l3.GRAD_VALUE,0) BETWEEN 2 AND 7 ");
//        stb.append("          THEN 1 ");
...
////        stb.append("     AND t2.GRAD_VALUE = 1 ");
...
////        stb.append("     AND int(VALUE(t1.GRAD_VALUE,'0')) BETWEEN 2 AND 7 ");
...
//        stb.append("     ) t1 ");
        stb.append(" ) ");
        stb.append(" SELECT ");
        stb.append("     SUM(VALUE(t1.GET_CREDIT,0)) AS CREDIT_CNT ");
        stb.append(" FROM ");
        stb.append("     SCHTBL t1 ");
    ...
    /** 特別活動出席を取得 */
    private String sqlGetSpAttend(final String[] param, final String schno, final String sqlpattern) {
        final StringBuffer stb = new StringBuffer();
        stb.append(" SELECT ");
        stb.append("     SCHREGNO, ");
        stb.append("     sum(value(CREDIT_TIME,0)) AS CREDIT_TIME ");
        stb.append(" FROM ");
        stb.append("     SPECIALACT_ATTEND_DAT ");
        stb.append(" WHERE ");
        if ("YEAR".equals(sqlpattern)) {
            stb.append("     YEAR = '" + param[0] + "' AND ");
        }
        stb.append("     SCHREGNO = '" + schno + "' ");
        stb.append(" GROUP BY ");
