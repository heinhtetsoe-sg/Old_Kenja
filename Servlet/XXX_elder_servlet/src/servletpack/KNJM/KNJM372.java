...
/*
 * $Id: 4adc52b2b7f349add7feee20f1a103f08d79ee64 $
 *
 * 作成日: 2019/01/07
 * 作成者: yamashiro
 *
...
public class KNJM372 {

    private static final Log log = LogFactory.getLog(KNJM372.class);
    ...
    protected void printMain(final DB2UDB db2, final Vrw32alp svf, final Param param) {
        ...
        for (int i = 0; i < studentList.size(); i++) {
            ...
            svf.VrSetForm("KNJM372.frm", 4);

            svf.VrsOut("TITLE", "特別追加レポート"); // タイトル
            ...
            svf.VrsOut("LIMIT_DATE", KNJ_EditDate.h_format_JP(db2, param._printDate) + "（" + KNJ_EditDate.h_format_W(param._printDate) + "）");
            ...
            for (int subi = 0; subi < record; subi++) {
                if (subi < subclassList.size()) {
                    ...
                    final String setAddRep = getAddRep(subclass._repoLimitCnt, subclass._rVal1, subclass._schoolingLimitCnt, attendCnt);
                    svf.VrsOut("REPORT1", setAddRep); // 追加レポート数
    ...
    private String getAddRep(final String repLimit, final String passCnt, final String atendLimit, final int attendCnt) {
    ...
    private static class Param {
        ...
        private static String getDbPrginfoProperties(final DB2UDB db2, final String propName) {
            return KnjDbUtils.getOne(KnjDbUtils.query(db2, " SELECT VALUE FROM PRGINFO_PROPERTIES WHERE PROGRAMID = 'KNJM372' AND NAME = '" + propName + "' "));
        }
