 ...
 *  @version $Id: 0d23c4f03ae72b84a63450d3c18c6eb6bb0272c6 $
 */
public class KNJM210M extends HttpServlet {

    private static final Log log = LogFactory.getLog(KNJM210M.class);
    ...
    private void getParam(final HttpServletRequest request, final DB2UDB db2) throws Exception {
        ...
        _hmparam.put("useCreditTime", request.getParameter("useCreditTime"));       //CREDIT_TIME合算、NULLは０とする等
        //タイトル
    ...
    private void setForm(final Vrw32alp svf) {
        ...
        if (_hmparam.get("PRINTKIND").equals("SCLSUB") || _hmparam.get("PRINTKIND").equals("SCLALL")) {
            svf.VrSetForm("KNJM210M_1.frm", 1);
        } else {
            svf.VrSetForm("KNJM210M_2.frm", 1);
        }
    ...
    private void setSclData(
        ...
        svf.VrsOutn("CREDIT_TIME", gyo, rs.getString("CREDIT_TIME"));
    ...
    private String sqlSclSub() {
        ...
        if ("1".equals(_hmparam.get("useCreditTime"))) {
            stb.append("        VALUE(CREDIT_TIME, 0.0) AS CREDIT_TIME,");
        } else {
            stb.append("        VALUE(CREDIT_TIME, 1.0) AS CREDIT_TIME,");
        }
        ...
        stb.append("    CASE WHEN T1.SCHOOLINGKINDCD = '2' THEN NULL ELSE T2.NAME1 END AS PERIOD_NAME,");
