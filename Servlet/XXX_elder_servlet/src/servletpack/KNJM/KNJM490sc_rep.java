// kanji=漢字
/*
 * $Id: cce06265e8aa72dbb0d3863d7c6d35b006e49b13 $
 *
 * 作成日: 2005/06/28 15:20:08 - JST
 * 作成者: m-yama
 *
...
public class KNJM490sc_rep {

    private static final Log log = LogFactory.getLog(KNJM490sc_rep.class);
...
for(int i=0 ; i<4 ; i++) log.debug("param["+i+"]="+param[i]);
    ...
    private boolean setSvfMain(
        ...
        String gval   = "*";            //前回評定
        try {
            ret = svf.VrSetForm("KNJM490_2.frm", 4);            //セットフォーム
            ...
            while( rs.next() ){
                ...
                ret = svf.VrsOut("SEQ1_1"       ,rs.getString("SCH_SEQ_MIN"));      //規定数スクーリング
                ret = svf.VrsOut("SEQ1_2"       ,rs.getString("REP_SEQ_ALL"));      //規定数レポート
...
log.debug("seqdate" + String.valueOf(seqdate));
                        ret = svf.VrsOut("DATE" + kindcnt   ,String.valueOf(seqdate));
                        seqcnt++;
                        kindcnt++;
                    }else {
                        break;
                ...
                ret = svf.VrsOut("SEQ2_1"           ,String.valueOf(seqcnt));       //実数スクーリング
                rs2.close();
                seqcnt = 0;
                ...
                ps4.setString(3,rs.getString("SUBCLASSCD"));    //科目コード
                ps4.setString(4,rs.getString("SCHREGNO"));      //学籍番号
                        ...
                        if (befseq.equalsIgnoreCase(rs4.getString("STANDARD_SEQ")) && !setflg){
                            if (gval.equals("受")){
                                if (rs4.getString("GRDVALUE").equals("無")){
                                    ret = svf.VrsOut("VALUE" + rs4.getString("STANDARD_SEQ")    ,String.valueOf("受"));
//                                  seqcnt++;
                                }else {
                                    ret = svf.VrsOut("VALUE" + rs4.getString("STANDARD_SEQ")    ,String.valueOf("再"));
//                                  seqcnt++;
                                }
                            }
                            setflg = true;
                            ...
                            if (!befseq.equalsIgnoreCase(rs4.getString("STANDARD_SEQ"))){
                                ret = svf.VrsOut("VALUE" + rs4.getString("STANDARD_SEQ")    ,rs4.getString("GRDVALUE"));
                ...
                ret = svf.VrsOut("SEQ2_2"           ,String.valueOf(seqcnt));       //実数レポート
                rs4.close();
                seqcnt = 0;
    ...
    private String preStat(String param[])
    {
        ...
        try {
            ...
            stb.append("    t5.SEM1_TERM_SCORE,t5.SEM1_VALUE,t5.SEM2_VALUE,t5.SEM2_TERM_SCORE, t5.GRAD_VALUE, t6.SCH_SEQ_MIN,t6.REP_SEQ_ALL, ");
            stb.append("    t7.SCHOOLINGKINDCD, t8.CREDITS ");
...
//          stb.append("    AND t7.SCHOOLINGKINDCD = '1' ");
            ...
            stb.append("   t3.TAKESEMES,t4.SUBCLASSABBV,t4.SUBCLASSNAME, ");
            stb.append("   t5.SEM1_TERM_SCORE,t5.SEM1_VALUE,t5.SEM2_VALUE,t5.SEM2_TERM_SCORE, t5.GRAD_VALUE, t6.SCH_SEQ_MIN,t6.REP_SEQ_ALL,t7.SCHOOLINGKINDCD, t8.CREDITS ");
            if (!param[2].equals("0")){
                stb.append(") ");
                stb.append("SELECT ");
                stb.append("    * ");
