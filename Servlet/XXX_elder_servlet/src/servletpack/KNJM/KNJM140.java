// kanji=漢字
/*
 * $Id: 26ceae5a476e5e59176b1b1c4ead30425ee949d9 $
 *
 ...
 *  学校教育システム 賢者 [通信制]
 *
 *                  ＜ＫＮＪＭ１４０＞  ＳＨＲ時間割チェックリスト
 *
 *  2005/04/06 m-yama 作成日
 *<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<*/

public class KNJM140 extends HttpServlet {

    private static final Log log = LogFactory.getLog(KNJM140.class);
    int len = 0;            //列数カウント用
    int ccnt    = 0;
    String chairnm[];
    String chaircd[];
...
for(int ia=0 ; ia<param.length ; ia++) log.debug("[KNJM140]param["+ia+"]="+param[ia]);
        //SQL作成
        try {
    ...
    PreparedStatement ps4)
    {
        ...
        try {
            ...
            while( rs.next() ){
                ...
                svf.VrsOutn("PERIODCD"        ,gyo, rs.getString("ABBV1") );
