...
public class KNJL302H {

    private static final Log log = LogFactory.getLog(KNJL302H.class);
	...
	private void setHeader(
		...
		int ret = 0;
        if (false && 0 != ret) { ret = 0; }
		ret = svf.VrSetForm("KNJL302H.frm", 4);
	...
	private boolean setSvfout(
		...
		try {
			...
			while( rs.next() ){
				...
				ret = svf.VrsOut("EXAM_TYPE",rs.getString("EXAMHALL_NAME"));//受験型
				//レコード数カウント
				reccnt++;
	...
	//	パラメータ（なし）
		try {
            ...
            stb.append("    MEISYOU_GET(T2.EXAM_TYPE,'L005',1) AS EXAM_TYPE_NAME, ");
			stb.append("    T3.EXAMHALL_NAME, ");
		...
		//	'全て'以外の場合（入試区分）
			...
			stb.append("    LEFT JOIN ENTEXAM_RECEPT_DAT T2 ON T2.ENTEXAMYEAR	= '"+param[0]+"' AND ");
			stb.append("                                       T2.APPLICANTDIV	= '"+param[1]+"' AND ");
			stb.append("                                       T2.TESTDIV		= T1.TESTDIV AND ");
			stb.append("                                       T2.EXAMNO		= T1.EXAMNO ");
