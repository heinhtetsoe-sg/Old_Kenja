...
public class KNJL342H {

    private static final Log log = LogFactory.getLog(KNJL342H.class);
	...
	private void setHeader(
		...
		int ret = 0;
        if (false && 0 != ret) { ret = 0; }
		ret = svf.VrSetForm("KNJL342H.frm", 4);
	...
	private boolean setSvfout(
		...
		try {
			...
			while( rs.next() ){
				...
				ret = svf.VrsOut(setformatArea("NAME", 11, honordiv + rs.getString("NAME"))
	...
	//	パラメータ（なし）
		try {
			...
			stb.append("            AND (W2.ENTDIV='1' OR W2.ENTDIV IS NULL) ");//基礎データ.入学区分
