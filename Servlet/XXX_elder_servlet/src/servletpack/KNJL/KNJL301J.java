 ...
 *
 */
public class KNJL301J {
    private static final String FORM_NAME = "KNJL301J.frm";
    private static final String KUGIRI = "／";
    private boolean _hasData;
    Param _param;

    private static final Log log = LogFactory.getLog(KNJL301J.class);
    ...
    private void outPutPrint(final Vrw32alp svf, final List student, final String total_page) {
        ...
        for (Iterator it = student.iterator(); it.hasNext();) {
			...
			if ((gyo > 25) || (!stestdiv.equals(sudent._testdiv) && !stestdiv.equals("0"))) {
			...
			// 志願者(かな氏名)
			svf.VrsOutn(setformatArea("NAME_KANA", sudent._name_kana, 20, "1", "2"), gyo, sudent._name_kana);
			// 志願者(氏名)
			svf.VrsOutn(setformatArea("NAME", sudent._name, 10, "1", "2") , gyo, sudent._name);
			// 性別
			if(sudent._sex != null){
			...
			if(sudent._remark1.equals("")){
				prischool = sudent._prischool_name;
			} else {
				prischool = sudent._prischool_name + KUGIRI + sudent._remark1;
			}
			svf.VrsOutn(setformatArea("PRISCHOOL_NAME", prischool, 15, "1", "2_1") , gyo, prischool);
			// 志願者郵便番号
			if(!sudent._zipcd.equals("")){
				svf.VrsOutn("ZIPCD" 	, gyo, "〒"+sudent._zipcd);
			}
			// 志願者住所
			String saddress = sudent._address1 + sudent._address2;
			svf.VrsOutn(setformatArea("ADDRESS", saddress, 25, "1", "2_1") , gyo, saddress);
			// 保護者電話番号
			svf.VrsOutn("FINSCHOOL_TELNO" 	, gyo, sudent._telno);
			// 出身学校：出身学校名
			svf.VrsOutn(setformatArea("FINSCHOOL_NAME", sudent._finschool_name, 15, "1", "2_1") , gyo, sudent._finschool_name);
    ...
    private String getStudentSql(final String test_div){
		...
		stb.append("    LEFT OUTER JOIN FINSCHOOL_MST T1 ON");
		stb.append("        W2.FS_CD = T1.FINSCHOOLCD ");						// 出身学校コード
		stb.append("    LEFT OUTER JOIN PRISCHOOL_MST T2 ON");
		stb.append("        W2.PRISCHOOLCD = T2.PRISCHOOLCD");					// 塾コード
		stb.append("    LEFT OUTER JOIN NAME_MST T3 ON ");
		stb.append("         T3.NAMECD1 = 'L004' and");							// 名称区分
		stb.append("         T3.NAMECD2 = W1.TESTDIV");							// 名称コード
		stb.append(" where");
    ...
    private String getTotalPageSql(final String test_div){
        ...
        rtn = " select"
			...
			+ "     FROM   ENTEXAM_DESIRE_DAT W1, ENTEXAM_APPLICANTBASE_DAT W2 "
            + "     where"
    ...
    private class Param {
        ...
        Param(final DB2UDB db2, HttpServletRequest request) throws Exception {
            ...
            String val = "111";
            request.setAttribute("LOGIN_DATE",(Object)val);
