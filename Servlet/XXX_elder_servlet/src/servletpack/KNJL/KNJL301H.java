...
public class KNJL301H {

    private static final Log log = LogFactory.getLog(KNJL301H.class);
	...
	//	ＳＶＦ作成処理
		...
		try {
			...
			ps1 = db2.prepareStatement(preStat1(param));		//志願者一覧preparestatement
	...
	private void setHeader(
		...
		int ret = 0;
        if (false && 0 != ret) { ret = 0; }
		ret = svf.VrSetForm("KNJL301H.frm", 4);
	...
	private boolean setSvfout(
		...
		try {
			...
			while( rs.next() ){
				...
				ret = svf.VrsOut("TOTAL_ALL",rs.getString("TOTAL_ALL"));		//内申点
				ret = svf.VrsOut("KASANTEN_ALL",rs.getString("KASANTEN_ALL"));	//加算点
				//レコード数カウント
				reccnt++;
	...
	//	パラメータ（入試区分）
		try {
			...
			stb.append("    T1.TOTAL_ALL,  ");
			stb.append("    T1.KASANTEN_ALL, ");
			stb.append("    T1.RITU_NAME, ");
			stb.append("    T1.FINSCHOOL_NAME ");
			stb.append("FROM ");
			stb.append("    (SELECT W1.TESTDIV, W1.EXAMNO, W2.NAME, W2.NAME_KANA, W2.SEX, W2.FS_NAME, W3.TOTAL_ALL, W3.KASANTEN_ALL, T3.NAME1 AS RITU_NAME, T2.FINSCHOOL_NAME ");
			stb.append("     FROM   ENTEXAM_DESIRE_DAT W1, ENTEXAM_APPLICANTBASE_DAT W2  ");
            stb.append("     LEFT JOIN ENTEXAM_APPLICANTCONFRPT_DAT W3 ON W3.ENTEXAMYEAR = W2.ENTEXAMYEAR AND ");
		...
		//	'全て'以外の場合（入試区分）
			...
			stb.append("            AND W1.APPLICANT_DIV='1') T1  ");
			if (param[1].equals("1")){
