 ...
 *  2004/12/22 nakamoto 条件追加（志願者区分='1'）      NO001
 *  2004/12/27 nakamoto 総ページ数は入試区分毎に出力    NO002
 *  2005/01/07 nakamoto 受験型を帳票項目に追加          NO003
 *  2005/01/12 nakamoto 受験番号○○◇◇◇の、○○の部分が変わったら改ページする    NO004
 *             nakamoto コミットが入れられるところではコミットする  NO005
 *  2005/01/12 yamashiro 受験型は名称を出力 N006
 *  2007/01/10 m-yama   NO007 受験型名称出力を会場名に変更
 **/

public class KNJL302 {

    private static final Log log = LogFactory.getLog(KNJL302.class);
...
for(int i=0 ; i<param.length ; i++) log.debug("param["+i+"]="+param[i]);
        ...
        try {
            ...
            ps1 = db2.prepareStatement(preStat1(param));        //受験者一覧preparestatement
    ...
    private void setHeader(
        ...
        int ret = 0;
        if (false && 0 != ret) { ret = 0; }
        ret = svf.VrSetForm("KNJL302.frm", 4);
    ...
    private boolean setSvfout(
        ...
        try {
            ...
            while( rs.next() ){
                ...
                //ret = svf.VrsOut("EXAM_TYPE",rs.getString("EXAM_TYPE"));  //受験型 NO003
                ret = svf.VrsOut("EXAM_TYPE",rs.getString("EXAMHALL_NAME"));//受験型 NO006 → 会場名 NO007
                //レコード数カウント
                reccnt++;
    ...
    //  パラメータ（なし）
        try {
            ...
            stb.append("    T2.EXAM_TYPE, ");   /* NO003 */
            stb.append("    MEISYOU_GET(T2.EXAM_TYPE,'L005',1) AS EXAM_TYPE_NAME, ");    /* 05/01/12 yamashiro N006 */
            stb.append("    T3.EXAMHALL_NAME ");    /* NO007 */
            stb.append("FROM ");
...
//          stb.append("            AND W1.TESTDIV='"+param[2]+"' ");
            ...
            stb.append("            AND W1.APPLICANT_DIV='1'  ");   /* NO001 */
            stb.append("            AND W1.EXAMINEE_DIV='1') T1  ");
            stb.append("    LEFT JOIN ENTEXAM_RECEPT_DAT T2 ON T2.ENTEXAMYEAR   = '"+param[0]+"' AND ");/* NO003 */
            stb.append("                                       T2.APPLICANTDIV  = '"+param[1]+"' AND ");/* NO003 */
            stb.append("                                       T2.TESTDIV       = T1.TESTDIV AND ");    /* NO003 */
            stb.append("                                       T2.EXAMNO        = T1.EXAMNO ");         /* NO003 */
            stb.append("    LEFT JOIN ENTEXAM_HALL_DAT T3 ON T2.TESTDIV = T3.TESTDIV ");
            stb.append("         AND T2.EXAM_TYPE = T3.EXAM_TYPE ");
            stb.append("         AND T2.RECEPTNO BETWEEN T3.S_RECEPTNO AND T3.E_RECEPTNO ");
...
//          stb.append("            AND W1.TESTDIV='"+param[2]+"' ");
            ...
            stb.append("            AND W1.APPLICANT_DIV='1'  ");   /* NO001 */
            stb.append("            AND W1.EXAMINEE_DIV='1'  ");
            stb.append("     GROUP BY W1.TESTDIV ) T1  ");   /* NO004 */
        } catch( Exception e ){
