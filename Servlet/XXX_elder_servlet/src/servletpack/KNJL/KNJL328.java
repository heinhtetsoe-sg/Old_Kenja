 ...
 *	2005/12/30 m-yama   欠席者は、省く NO002
 *	2006/01/05 yamashiro 最終統計行は欠席者を含める NO003
 *                       入試区分’全て’を指定した場合、最終試験区分の内容が試験回数分出力される不具合を修正 NO004
 *
 */
...
public class KNJL328 {

    private static final Log log = LogFactory.getLog(KNJL328.class);
    ...
    void printSvfMain(DB2UDB db2, Vrw32alp svf, PreparedStatement ps2,
		...
		try{
            ...
            //while ( rs.next() ){
            //    getTestdiv(db2, ps4, rs.getString("TESTDIV"));                           //入試区分名称
            //    printSvfMeibo(db2, svf, rs.getString("TESTDIV"), ps2, ps3, ps4);    //名簿の印刷
            //}

            for( int i = 0 ; i <= testdiv ; i++ ){
                //NO004 getTestdiv( db2, ps4, String.valueOf(testdiv) );        //入試区分名称
                //NO004 printSvfMeibo( db2, svf, String.valueOf(testdiv), ps2, ps3, ps4 );    //名簿の印刷
                getTestdiv( db2, psTestDiv, String.valueOf( i ) );              //入試区分名称 NO004
                printSvfMeibo( db2, svf, String.valueOf( i ), ps2, ps3, ps4 );    //名簿の印刷 NO004
            }
    ...
    void printSvfTestdiv(Vrw32alp svf, String testdiv, PreparedStatement ps4 )
	{
        ...
        try {
            if( pageno + 1 == totalpageno )
                ret = svf.VrSetForm("KNJL328_2.frm", 1);
            else
                ret = svf.VrSetForm("KNJL328_1.frm", 1);
...
//for( int i=0 ; i<param.length ; i++ )log.debug("param["+i+"]="+param[i]);
     ...
     */
	//NO003 String statementMeiboCommon()
	String statementMeiboCommon( int pdiv )  //NO003
	{
...
************************* */
            ...
            if( pdiv == 1 )  //NO003
                stb.append(          "W1.EXAMINEE_DIV <> '2' AND ");	//NO002
	...
	String statementMeiboMeisai()
	{
		...
		try{
            ...
            //NO003 stb.append( statementMeiboCommon() );   //共通部分
            stb.append( statementMeiboCommon( 0 ) );   //共通部分 NO003
	...
	String statementMeiboTotalPage()
	{
		...
		try{
            stb.append("SELECT CASE WHEN 0 < MOD(COUNT(DISTINCT W1.EXAMNO),100) THEN COUNT(DISTINCT W1.EXAMNO)/100 + 1 ELSE COUNT(DISTINCT W1.EXAMNO)/100 END AS COUNT ");

            //NO003 stb.append( statementMeiboCommon() );   //共通部分
            stb.append( statementMeiboCommon( 1 ) );   //共通部分 NO003
