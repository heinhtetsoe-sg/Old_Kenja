/*
 * $Id: 22006c00e7e0703e35784145bc68b5a982953b28 $
 *
 * 作成日: 2017/11/20
 * 作成者: tawada
 *
...
public class KNJL361W {

    private static final Log log = LogFactory.getLog(KNJL361W.class);
    ...
    private void printMain(final DB2UDB db2, final Vrw32alp svf, final SchoolData schoolData) {
        svf.VrSetForm("KNJL361W.frm", 4);
        final List printMieList    = getMiekenList(db2, schoolData);
        final List printKengaiList = getKengaiList(db2, schoolData);
        final List printTotalList  = getTotalList(db2, schoolData);
        int printLine = 1;
        int gpCd =1;
        String befKengaiCtiyCd = "";
        String befCtiyCd = "";

        setTitle(svf, schoolData);//タイトル
        for (Iterator iterator = schoolData._cmList.iterator(); iterator.hasNext();) {
        ...
        for (Iterator iterator = printMieList.iterator(); iterator.hasNext();) {
            ...
            int colCnt =1;
            for (Iterator iterator2 = schoolData._cmList.iterator(); iterator2.hasNext();) {
    ...
    private void setTitle(final Vrw32alp svf, final SchoolData schoolData) {
    ...
    private List getMiekenList(final DB2UDB db2, final SchoolData schoolData) {
        final List retList = new ArrayList();
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            final String sql = getMieKennaiSql(schoolData);
            log.debug(" sql =" + sql);
    ...
    //三重県内データ
    private String getMieKennaiSql(final SchoolData schoolData) {
        final StringBuffer stb = new StringBuffer();
        stb.append(" WITH BASEDATA AS ( ");
        stb.append("                     SELECT ");
        stb.append("                         B1.FS_CD ");
        stb.append("                     FROM ");
        stb.append("                         EDBOARD_ENTEXAM_APPLICANTBASE_DAT B1 ");
        ...
        stb.append("                     WHERE ");
        stb.append("                             B1.EDBOARD_SCHOOLCD = '" + schoolData._schoolCd + "' ");
        stb.append("                         AND B1.ENTEXAMYEAR  = '" + _param._entexamYear + "' ");
    ...
    private List getKengaiList(final DB2UDB db2, final SchoolData schoolData) {
        final List retList = new ArrayList();
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            final String sql = getKengaiSql(schoolData);
            log.debug(" sql =" + sql);
            ps = db2.prepareStatement(sql);
            rs = ps.executeQuery();
            while (rs.next()) {
    ...
    //三重県外データ
    private String getKengaiSql(final SchoolData schoolData) {
        final StringBuffer stb = new StringBuffer();
    ...
    private List getTotalList(final DB2UDB db2, final SchoolData schoolData) {
        final List retList = new ArrayList();
        PreparedStatement ps = null;
        ResultSet rs = null;
        try {
            final String sql = getTotalSql(schoolData);
            log.debug(" sql =" + sql);
            ps = db2.prepareStatement(sql);
            rs = ps.executeQuery();
            while (rs.next()) {
    ...
    //合計データ
    private String getTotalSql(final SchoolData schoolData) {
        final StringBuffer stb = new StringBuffer();
        ...
        for (Iterator iterator = schoolData._cmList.iterator(); iterator.hasNext();) {
            ...
            stb.append("                 FROM ");
            stb.append("                     V_EDBOARD_ENTEXAM_APPLICANTBASE_DAT B1 ");
            stb.append("                     INNER JOIN NAME_MST N1 ON N1.NAMECD1   = 'L013' ");
            stb.append("                                           AND B1.JUDGEMENT = N1.NAMECD2 ");
            stb.append("                 WHERE ");
            stb.append("                         B1.EDBOARD_SCHOOLCD = '" + schoolData._schoolCd + "' ");
    ...
    private class Param {
            ...
            for (int i = 0; i < _selected.length; i++) {
                ...
                try {
                    ...
                    while (rs.next()) {
                        ...
                        final List cmList = getCouseMajorList(db2, schoolCd);
                        final SchoolData schoolData = new SchoolData(schoolCd, schoolName, courseCd, courseName, distName, cmList);
        ...
        private List getCouseMajorList(final DB2UDB db2, final String schoolcd) {
            final List retList = new ArrayList();
            PreparedStatement ps = null;
            ResultSet rs = null;
            try {
                final String sql = getSqlgetCouseMajor(schoolcd);
                log.debug(" sql =" + sql);
                ps = db2.prepareStatement(sql);
                rs = ps.executeQuery();
        ...
        private String getSqlgetCouseMajor(final String schoolCd) {
            final StringBuffer stb = new StringBuffer();
            ...
            stb.append(" FROM ");
            stb.append("     EDBOARD_ENTEXAM_APPLICANTBASE_DAT B1 ");
            stb.append("     LEFT JOIN V_EDBOARD_COURSE_MAJOR_MST V1 ON B1.ENTEXAMYEAR  = V1.YEAR ");
            stb.append("                                     AND B1.EDBOARD_SCHOOLCD = V1.EDBOARD_SCHOOLCD ");
            stb.append("                                     AND B1.SUC_COURSECD     = V1.COURSECD ");
            stb.append("                                     AND B1.SUC_MAJORCD      = V1.MAJORCD ");
            stb.append("     LEFT JOIN V_EDBOARD_COURSECODE_MST V2 ON B1.ENTEXAMYEAR = V2.YEAR ");
            stb.append("                                     AND B1.EDBOARD_SCHOOLCD = V2.EDBOARD_SCHOOLCD ");
            stb.append("                                     AND B1.SUC_COURSECODE   = V2.COURSECODE ");
            stb.append(" WHERE ");
            stb.append("         B1.EDBOARD_SCHOOLCD = '" + schoolCd + "' ");
            stb.append("     AND B1.ENTEXAMYEAR      = '" + _entexamYear + "' ");
