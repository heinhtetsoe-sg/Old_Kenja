/*
 * $Id: 3c8fe1e75db73bcc04667ddad0d63b5fb0817a39 $
 *
 * 作成日: 2015/09/08
 * 作成者: nakamoto
 *
 ...
 *                  ＜ＫＮＪＬ３００Ｎ＞  座席ラベル
 **/
public class KNJL508J_1 {

    private static final Log log = LogFactory.getLog(KNJL508J_1.class);
    ...
    private String sql() {
        ...
        stb.append(" ), SRCHPASSDATA_TBL AS ( ");
        stb.append(" SELECT ");
        stb.append("  T1.ENTEXAMYEAR, ");
        stb.append("  T1.APPLICANTDIV, ");
        stb.append("  T1.TESTDIV, ");
        ...
        stb.append("  CASE WHEN T4.EXAMNO IS NOT NULL OR TM_TEKI.INTERVIEW_DIV IS NULL THEN '' ELSE '面' END AS INTERVIEW, ");
        ...
        for (Iterator itr = _param._testdivMap.keySet().iterator();itr.hasNext();) {
            ...
            stb.append("       WHEN T5_00"+datcnt+".EXAMNO IS NOT NULL THEN '〇' ");
        ...
        stb.append("   AND T2.APPLICANTDIV = T1.APPLICANTDIV ");
        stb.append("   AND T2.EXAMNO = T1.EXAMNO ");
        stb.append("  INNER JOIN SRCHPASSDATA_TBL T10 ");
        stb.append("    ON T10.ENTEXAMYEAR = T1.ENTEXAMYEAR ");
        stb.append("   AND T10.APPLICANTDIV = T1.APPLICANTDIV ");
        stb.append("   AND T10.TESTDIV = T1.TESTDIV ");
        stb.append("   AND T10.EXAM_TYPE = T1.EXAM_TYPE ");
        stb.append("   AND T10.RECEPTNO = T1.RECEPTNO ");
        ...
        stb.append("  LEFT JOIN PASTTEST_TBL T4 "); //★他とは違う結合なので注意
        stb.append("    ON T4.ENTEXAMYEAR = T10.ENTEXAMYEAR ");
        stb.append("   AND T4.APPLICANTDIV = T10.APPLICANTDIV ");
        stb.append("   AND T4.TESTDIV = T10.TESTDIV ");
        stb.append("   AND T4.EXAMNO = T10.EXAMNO ");
        int srchdatcnt = 1;
        for (Iterator ite = _param._testdivMap.keySet().iterator();ite.hasNext();) {
            ...
            stb.append("   AND T5_00"+srchdatcnt+".EXAMNO = T2.EXAMNO ");
            srchdatcnt++;
        }
    ...
    private void printMain(final DB2UDB db2, final Vrw32alp svf) {
        ...
        if ("1".equals(_param._applicantdiv)) {
            svf.VrSetForm("KNJL508J_1_1.frm", 1);
        } else {
            svf.VrSetForm("KNJL508J_1_2.frm", 1);
        }
