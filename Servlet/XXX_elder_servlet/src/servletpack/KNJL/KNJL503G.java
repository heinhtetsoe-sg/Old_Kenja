/*
 * $Id: 1f94e6d71d62baff36415ffd710bc4c76b20c1d5 $
 *
 * 作成日: 2018/12/19
 * 作成者: matsushima
 *
...
public class KNJL503G {

    private static final Log log = LogFactory.getLog(KNJL503G.class);
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (ALL.equals(_param._examCouse)) {
                    setTitle(db2, svf, nendo, "全出願者", shName);
                } else {
                    setTitle(db2, svf, nendo, sucCourse, shName);
                }
    ...
    private void setTitle(final DB2UDB db2, final Vrw32alp svf, final String nendo, final String courseName, final String shName) {
        //svf.VrsOut("TITLE", nendo + "　" + _param._applicantdivName + "　" + _param._testdivAbbv1 + "　合否判定結果一覧(" + courseName + ")");
        svf.VrsOut("TITLE", nendo + "　" + _param._applicantdivName + "　" + _param._testdivAbbv1 + "　合否判定結果一覧");
    ...
    private String sql() {
        ...
        if ("2".equals(_param._judge)) {
            ...
            stb.append("     AND VBASE.JUDGEMENT = '2' ");
        }
        stb.append(" ORDER BY ");
        stb.append("     VBASE.EXAMNO ");
        return stb.toString();
