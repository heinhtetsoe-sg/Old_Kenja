 ...
 *  2005/01/12 nakamoto 受験番号○○◇◇◇の、○○の部分が変わったら改ページする    NO002
 *             nakamoto コミットが入れられるところではコミットする  NO003
 **/

public class KNJL301 {

    private static final Log log = LogFactory.getLog(KNJL301.class);
...
for(int i=0 ; i<param.length ; i++) log.debug("param["+i+"]="+param[i]);
    ...
    private void setHeader(
        ...
        int ret = 0;
        if (false && 0 != ret) { ret = 0; }
        ret = svf.VrSetForm("KNJL301.frm", 4);
    ...
    private boolean setSvfout(
        ...
        try {
            ...
            while( rs.next() ){
                ...
                if ((gyo > 50) || (!testdiv.equals(rs.getString("TESTDIV")) && !testdiv.equals("0"))) { /* NO002 */
                    gyo = 1;
                    pagecnt++;
                }
                //ヘッダ
    ...
    //  パラメータ（入試区分）
        try {
            ...
            stb.append("    SUBSTR(T1.EXAMNO,1,2) EXAMNO2, ");  /* NO002 */
            stb.append("    T1.NAME, ");
...
//          stb.append("            AND W1.TESTDIV='"+param[2]+"' ");
            ...
            stb.append("     GROUP BY W1.TESTDIV, SUBSTR(W1.EXAMNO,1,2) ) T1  ");   /* NO002 */
        } catch( Exception e ){
