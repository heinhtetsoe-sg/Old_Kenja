 ...
 *	学校教育システム 賢者 [入試管理]
 *
 *					＜ＫＮＪＬ３２３Ｈ＞  特待生合格者名簿・不合格者名簿
 *
 *	2007/11/07 RTS 作成日
 *
 **/

public class KNJL323H {

    private static final Log log = LogFactory.getLog(KNJL323H.class);
	...
	//	パラメータの取得
		try {
	        ...
	        param[3] = request.getParameter("OUTPUT");         				//帳票種類1:特待生合格者名簿,2:不合格者名簿,3:合格者名簿
		} catch( Exception ex ) {
	...
	private void setHeader(
		...
		int ret = 0;
        if (false && 0 != ret) { ret = 0; }
		ret = svf.VrSetForm("KNJL323H.frm", 4);
        if (param[3].equals("1")){
    		ret = svf.VrsOut("TITLE", "合格");
        } else {
    		ret = svf.VrsOut("TITLE", "特待生合格");
        }
