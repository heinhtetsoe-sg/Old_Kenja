 ...
 *
 *	2004/12/14
 *  2004/12/21 受験型名称の表記を追加
 *  2005/01/05 座席番号（受付番号）の前ゼロは出力しない
 *	2005/01/12 nakamoto コミットが入れられるところではコミットする(db2.commit();)	NO001
 *
 */
...
public class KNJL312 {

    private static final Log log = LogFactory.getLog(KNJL312.class);
...
log.info("nonedata = "+nonedata);
    ...
    private void printSvf(DB2UDB db2, Vrw32alp svf)
	{
        ...
        try {
            ...
            printHead(db2, svf, ps2, psTestDiv);		                        //見出し項目のセット＆出力
            ps1 = db2.prepareStatement( statementMeiboMeisai() );
            printMeibo(db2, svf, ps1, ps2, ps3);                   	//名簿のセット＆出力 04/12/21Modify
        } catch( Exception ex ){
     ...
     *  svf print 見出し印刷処理
     */
    private void printHead(DB2UDB db2, Vrw32alp svf, PreparedStatement ps2, PreparedStatement psTestDiv)
	{
		int ret = 0;
        if (false && 0 != ret) { ret = 0; }
		ResultSet rs = null;
        ...
        try {
            ret = svf.VrSetForm("KNJL312.frm", 4);
	...
	//	作成日(現在処理日)
        ...
        //受験科目
		try{
			ps2.setString( 1, "L009" );
			ps2.setString( 2, param[5] );
			rs = ps2.executeQuery();
			if ( rs.next()  &&  rs.getString(1) != null )
...
log.info("exam_type_hall = "+exam_type_hall);
                    printsvfKaijyou(db2, svf, rs, ps2, ps3);      // 04/12/21Modify
    ...
    private void printsvfMeibo(Vrw32alp svf, ResultSet rs)
	{
		...
		try{
            ...
            ret = svf.VrAttribute( "RECEPTNO", "Hensyu=1" );    // 05/01/05
	 		...
	 		ret = svf.VrsOut( "POINT",       ( rs.getString("SCORE")     != null )? rs.getString("SCORE")     : ""  ); //得点
            ...
            ret = svf.VrsOut( "SEX",         ( rs.getString("SEXNAME")   != null )? rs.getString("SEXNAME")   : ""  ); //性別
	 		ret = svf.VrsOut( "EXAM_COURSE", ( rs.getString("EXAMCOURSE_NAME")   != null )? rs.getString("EXAMCOURSE_NAME")   : ""  ); //コース名
...
for( int i=0 ; i<param.length ; i++ )log.debug("param["+i+"]="+param[i]);
	...
	private String statementMeiboCommon()
	{
		...
		try{
            ...
            //stb.append(       "INNER JOIN ENTEXAM_SCORE_DAT W4 ON W4.ENTEXAMYEAR = W1.ENTEXAMYEAR AND ");    //NO001
