/*
 * $Id: DaoUpdateAttendSemesSubmDat.java 75473 2020-07-16 07:19:19Z maeshiro $
 *
 * 作成日: 2011/10/14
 * 作成者: maesiro
 *
 ...
 * 出欠月別中分類累積データを更新する。
 * @author maesiro
 * @version $Id: DaoUpdateAttendSemesSubmDat.java 75473 2020-07-16 07:19:19Z maeshiro $
 */
public class DaoUpdateAttendSemesSubmDat extends AbstractDaoUpdateAttendDat {
    /** テーブル名 */
    public static final String TABLE_NAME = "ATTEND_SEMES_SUBM_DAT";
     ...
     * @param options オプション
     */
    public DaoUpdateAttendSemesSubmDat(
    ...
    public void save(
            ...
            final List<AttendSemesSubm> list
    ) throws SQLException {
        Insert dao = null;
        try {
    ...
    public void delete(final Header header) throws SQLException {
        ...
        try {
            ...
            for (final String schoolKind : _options.getSchoolKind()) {
                ...
                log.info("月別中分類累積の削除件数: " + delCount + "件。");
    ...
    private static class Insert extends AbstractDaoUpdator {
        ...
        public int insert(
            ...
            for (final AttendSemesSubm subm : list) {
                ...
                ps.setString(c++, String.valueOf(subm.getKintai().getAltCode()));
                ps.setString(c++, subm.getSublCd());
                ps.setString(c++, subm.getSubmCd());
                ps.setInt(c++, subm.getCount());
